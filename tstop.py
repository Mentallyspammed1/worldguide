...
try:
    ...
    if not 0 < TRAILING_PERCENT < 1:
         logger.warning(f"TRAILING_PERCENT ({TRAILING_PERCENT}) is outside typical range (0, 1). Ensure this is intended.")
    ...
    if RECV_WINDOW_INT <= 0 or RECV_WINDOW_INT > 60000:
         logger.warning(f"BYBIT_RECV_WINDOW ({RECV_WINDOW}) is outside typical range (1, 60000). Ensure this is intended.")
...
if CATEGORY not in ["linear", "inverse", "spot"]:
    ...
    raise SystemExit(1)
if CATEGORY == "spot":
    logger.warning("Trailing stops are typically for derivatives (linear/inverse). Spot market trailing stops might behave differently or not be supported via this endpoint.")
...
logger.info(f"Configuration loaded: BASE_URL={BASE_URL}, CATEGORY={CATEGORY}, "
...
def make_request(method: str, endpoint: str, params: Optional[Dict[str, Any]] = None, retries: int = MAX_RETRIES) -> Optional[Dict]:
    """
    ...
    for attempt in range(1, retries + 1):
        ...
        try:
            ...
            if ret_code == 0:
                ...
                logger.warning(f"API error (retCode={ret_code}): {ret_msg} for {endpoint}. Attempt {attempt}/{retries}.")
                ...
                if ret_code in retryable_codes and attempt < retries:
                    ...
                    logger.error(f"API call failed permanently after {attempt} attempts: {endpoint} (retCode={ret_code}, retMsg={ret_msg})")
                    return None
...
def set_trailing_stop(symbol: str, side: str, trailing_distance: float) -> bool:
    ...
    try:
        ...
        logger.error(f"Error formatting trailing distance {trailing_distance} with precision {TRAILING_DISTANCE_PRECISION}: {e}")
    ...
    if response and response.get("retCode") == 0:
        logger.info(f"Successfully set trailing stop for {symbol} [{side}] at absolute distance {trailing_distance_str}")
        return True
    else:
    ...
    if response and response.get('retCode') == 0:
        try:
            ...
            if time_diff > recv_window_int:
                logger.warning(f"System clock out of sync with Bybit server by {time_diff}ms. Required sync <= {recv_window_int}ms.")
                logger.warning("Consider using NTP or similar to synchronize your system clock. API errors (like retCode 10004) may occur.")
                return False
        ...
        for pos in positions:
            try:
                ...
                try:
                     ...
                     logger.warning(f"Could not parse trailingStop parameter '{current_trailing_stop_param}' for {symbol}. Assuming not set.")
                ...
                if is_profitable:
                    ...
                    if not is_trailing_stop_set:
                        ...
                        if trailing_distance_to_set <= 0:
                             logger.warning(f"Calculated trailing distance for {symbol} is zero or negative ({trailing_distance_to_set:.8f}). Skipping set.")
                             continue
                        ...
                        if not success:
                            logger.error(f"  -> Failed to set trailing stop for {symbol} [{side}]. Will retry on next cycle.")
                    else:
                        ...
                        logger.debug(f"  -> Trailing stop already set for profitable position {symbol} [{side}]. No update needed.")
                ...
                logger.exception(f"Unexpected error processing position data for {pos.get('symbol', 'Unknown')}: {e}. Raw data: {pos}")
                # Continue to the next position even if one fails

        if positions: # Only log summary if there were positions fetched
             logger.info(f"Summary: Total positions checked: {len(positions)}, "
