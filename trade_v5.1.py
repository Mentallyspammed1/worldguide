        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any | None:
            ...
            while retries <= max_retries:
                try:
                    ...
                    if any(phrase in err_str for phrase in ["order not found", "order does not exist", "unknown order", "order already cancelled"]):
                        logger.warning(f"{Fore.YELLOW}Order vanished or unknown for {func.__name__}: {e}. Returning None (no retry).{Style.RESET_ALL}")
                        return None  # OrderNotFound is often final
                    elif any(phrase in err_str for phrase in ["insufficient balance", "insufficient margin", "margin is insufficient"]):
                         logger.error(f"{Fore.RED}Insufficient essence (funds/margin) for {func.__name__}: {e}. Aborting (no retry).{Style.RESET_ALL}")
                         return None
                    elif any(phrase in err_str for phrase in ["invalid order", "parameter error", "size too small", "price too low", "price too high"]):
                         logger.error(f"{Fore.RED}Invalid parameters/limits for {func.__name__}: {e}. Aborting (no retry). Check logic/config.{Style.RESET_ALL}")
                         return None
...
class ScalpingBot:
    ...
    def __init__(self, config_file: str = CONFIG_FILE_NAME, state_file: str = STATE_FILE_NAME) -> None:
        ...
        self.sl_trigger_by: str | None = self.config["risk_management"].get("sl_trigger_by")  # e.g., MarkPrice, LastPrice
        self.tp_trigger_by: str | None = self.config["risk_management"].get("tp_trigger_by")  # e.g., MarkPrice, LastPrice
        self.enable_trailing_stop_loss: bool = self.config["risk_management"]["enable_trailing_stop_loss"]  # New TSL flag
    ...
    def _configure_logging_level(self) -> None:
        ...
        if file_handler: file_handler.setLevel(logging.DEBUG)  # File log remains detailed
    ...
    def _load_state(self) -> None:
        ...
        try:
                ...
                with open(self.state_file, encoding='utf-8') as f:
                    ...
                    if isinstance(saved_state, list):
                        ...
                        if os.path.exists(state_backup_file):
                            try: os.remove(state_backup_file)
                            except OSError: logger.warning(f"Could not remove old state backup file: {state_backup_file}")
                    else:
            ...
            logger.error(f"{Fore.RED}Error decoding state file {self.state_file}: {e}. Attempting to restore backup or start fresh.{Style.RESET_ALL}")
            # Try restoring backup
            if os.path.exists(state_backup_file):
                try:
                    shutil.copyfile(state_backup_file, self.state_file)
                    logger.warning(f"{Fore.YELLOW}Restored state from backup file {state_backup_file}. Retrying load...{Style.RESET_ALL}")
                    self._load_state()  # Retry loading after restoring backup
                    return  # Exit this attempt, the retry will handle it
                except Exception as backup_err:
                    logger.error(f"{Fore.RED}Failed to restore state from backup {state_backup_file}: {backup_err}. Starting fresh.{Style.RESET_ALL}")
                    self.open_positions = []
                ...
                try:
                    ...
                    logger.warning(f"{Fore.YELLOW}Backed up corrupted state file to {self.state_file}.corrupted_... Starting fresh.{Style.RESET_ALL}")
                except Exception as backup_err:
                     logger.error(f"{Fore.RED}Could not back up corrupted state file: {backup_err}{Style.RESET_ALL}")
                self.open_positions = []
                self._save_state()  # Create a fresh state file
        except Exception as e:
            logger.error(f"{Fore.RED}Fatal: Failed to load state from {self.state_file}: {e}{Style.RESET_ALL}", exc_info=True)
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            with open(self.state_file, 'w', encoding='utf-8') as f:
                json.dump(self.open_positions, f, indent=4, default=str)  # Use default=str for non-serializable types if any sneak in
    ...
    def load_config(self, config_file: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"{Fore.YELLOW}Crafted a default spellbook: '{config_file}'. "
            ...
            logger.critical(f"{Fore.RED}Error parsing spellbook {config_file}: {e}. Check syntax. Aborting.{Style.RESET_ALL}")
            sys.exit(1)
        except Exception as e:
    ...
    def create_default_config(self, config_file: str) -> None:
        ...
        default_config = {
            ...
            "exchange": {
                ...
                "testnet_mode": os.getenv("TESTNET_MODE", "True").lower() in ("true", "1", "yes"),  # Explicit testnet flag
            },
            "trading": {
                "symbol": os.getenv("TRADING_SYMBOL", "BTC/USDT:USDT"),
                "timeframe": os.getenv("TIMEFRAME", DEFAULT_TIMEFRAME),
                "simulation_mode": os.getenv("SIMULATION_MODE", "True").lower() in ("true", "1", "yes"),  # Internal simulation
            ...
            "risk_management": {
                ...
                "atr_sl_multiplier": ATR_MULTIPLIER_SL, "atr_tp_multiplier": ATR_MULTIPLIER_TP,
                "stop_loss_percentage": 0.005, "take_profit_percentage": 0.01,  # Used if use_atr_sl_tp is false
        ...
        if os.getenv("ORDER_SIZE_PERCENTAGE"): default_config["risk_management"]["order_size_percentage"] = float(os.environ["ORDER_SIZE_PERCENTAGE"])
    ...
    def validate_config(self) -> None:
        ...
        try:
            # --- Helpers ---
            def is_positive(v): return isinstance(v, (int, float)) and v > 0
            def is_non_negative(v): return isinstance(v, (int, float)) and v >= 0
            def is_percentage(v): return isinstance(v, (int, float)) and 0 <= v <= 1
            def is_string(v): return isinstance(v, str) and v
            def is_bool(v): return isinstance(v, bool)
            def is_pos_int(v): return isinstance(v, int) and v > 0
            def is_non_neg_int(v): return isinstance(v, int) and v >= 0

            # --- Structure ---
            req = {"exchange", "trading", "order_book", "indicators", "risk_management"}
            if not req.issubset(self.config.keys()): raise ValueError(f"Missing sections: {req - self.config.keys()}")
            for s in req:
                if not isinstance(self.config[s], dict): raise ValueError(f"Section '{s}' must be a dictionary.")

            # --- Exchange ---
            cfg = self.config["exchange"]
            if not is_string(cfg.get("exchange_id")): raise ValueError("exchange.exchange_id required")
            if cfg["exchange_id"] not in ccxt.exchanges: raise ValueError(f"Exchange '{cfg['exchange_id']}' unknown")
            if not is_bool(cfg.get("testnet_mode")): raise ValueError("exchange.testnet_mode must be boolean")

            # --- Trading ---
            cfg = self.config["trading"]
            if not is_string(cfg.get("symbol")): raise ValueError("trading.symbol required")
            if not is_string(cfg.get("timeframe")): raise ValueError("trading.timeframe required")
            if not is_bool(cfg.get("simulation_mode")): raise ValueError("trading.simulation_mode must be boolean")
            if cfg.get("entry_order_type") not in ["market", "limit"]: raise ValueError("trading.entry_order_type must be 'market' or 'limit'")
            if not is_non_negative(cfg.get("limit_order_offset_buy")): raise ValueError("trading.limit_order_offset_buy must be non-negative")
            if not is_non_negative(cfg.get("limit_order_offset_sell")): raise ValueError("trading.limit_order_offset_sell must be non-negative")

            # --- Order Book ---
            cfg = self.config["order_book"]
            if not is_pos_int(cfg.get("depth")): raise ValueError("order_book.depth must be positive integer")
            if not is_positive(cfg.get("imbalance_threshold")): raise ValueError("order_book.imbalance_threshold must be positive")
            ...
            for p in periods:
                if not is_pos_int(cfg.get(p)): raise ValueError(f"indicators.{p} must be positive integer")
            if not is_non_negative(cfg.get("volatility_multiplier")): raise ValueError("indicators.volatility_multiplier non-negative")
            if cfg['macd_short_period'] >= cfg['macd_long_period']: raise ValueError("indicators.macd_short_period < macd_long_period required")

            # --- Risk Management ---
            cfg = self.config["risk_management"]
            if not is_bool(cfg.get("use_atr_sl_tp")): raise ValueError("risk.use_atr_sl_tp must be boolean")
            if cfg["use_atr_sl_tp"]:
                if not is_positive(cfg.get("atr_sl_multiplier")): raise ValueError("risk.atr_sl_multiplier positive required if using ATR")
                if not is_positive(cfg.get("atr_tp_multiplier")): raise ValueError("risk.atr_tp_multiplier positive required if using ATR")
            else:
                if not is_percentage(cfg.get("stop_loss_percentage")): raise ValueError("risk.stop_loss_percentage (0-1) required if not using ATR")
                if not is_percentage(cfg.get("take_profit_percentage")): raise ValueError("risk.take_profit_percentage (0-1) required if not using ATR")

            valid_triggers = ["MarkPrice", "LastPrice", "IndexPrice", None]  # Allow None if not specified
            if cfg.get("sl_trigger_by") not in valid_triggers: raise ValueError(f"risk.sl_trigger_by invalid, use: {valid_triggers}")
            if cfg.get("tp_trigger_by") not in valid_triggers: raise ValueError(f"risk.tp_trigger_by invalid, use: {valid_triggers}")

            if not is_bool(cfg.get("enable_trailing_stop_loss")): raise ValueError("risk.enable_trailing_stop_loss must be boolean")
            if cfg["enable_trailing_stop_loss"]:
                tsl_pct = cfg.get("trailing_stop_loss_percentage")
                if not (is_positive(tsl_pct) and tsl_pct < 1): raise ValueError("risk.trailing_stop_loss_percentage (0 < pct < 1) required if TSL enabled")

            if not is_percentage(cfg.get("order_size_percentage")): raise ValueError("risk.order_size_percentage (0-1) required")
            if not is_pos_int(cfg.get("max_open_positions")): raise ValueError("risk.max_open_positions >= 1 required")
            if not is_non_neg_int(cfg.get("time_based_exit_minutes")): raise ValueError("risk.time_based_exit_minutes >= 0 required")
            if not is_positive(cfg.get("strong_signal_adjustment_factor")): raise ValueError("risk.strong_signal_adjustment_factor positive required")
            if not is_positive(cfg.get("weak_signal_adjustment_factor")): raise ValueError("risk.weak_signal_adjustment_factor positive required")
    ...
    def _initialize_exchange(self) -> ccxt.Exchange:
        ...
        if not self.simulation_mode and not creds_found:
             logger.critical(f"{Fore.RED}API Key/Secret essence missing. Cannot trade live/testnet. Aborting.{Style.RESET_ALL}")
             sys.exit(1)
        elif not creds_found:
             logger.warning(f"{Fore.YELLOW}API Key/Secret missing. Running in full simulation mode only.{Style.RESET_ALL}")
        ...
        try:
            ...
            if self.timeframe not in exchange.timeframes:
                ...
                logger.critical(f"{Fore.RED}Timeframe '{self.timeframe}' not supported by {self.exchange_id}. Available: {available[:15]}... Aborting.{Style.RESET_ALL}")
                sys.exit(1)
            if self.symbol not in exchange.markets:
                available = list(exchange.markets.keys())
                logger.critical(f"{Fore.RED}Symbol '{self.symbol}' not found on {self.exchange_id}. Available: {available[:10]}... Check config. Aborting.{Style.RESET_ALL}")
                sys.exit(1)
            ...
            try:
                ...
                logger.warning(f"{Fore.YELLOW}Optional check: Failed to fetch server time, proceeding cautiously: {e}{Style.RESET_ALL}")

            logger.info(f"{Fore.GREEN}Connection established with {self.exchange_id.upper()}.{Style.RESET_ALL}")
            return exchange

        except ccxt.AuthenticationError as e:
             logger.critical(f"{Fore.RED}Authentication failed for {self.exchange_id.upper()}: {e}. Check API keys/permissions. Aborting.{Style.RESET_ALL}")
             sys.exit(1)
    ...
    def _load_market_info(self) -> None:
        ...
        try:
            ...
            if not self.market_info.get('precision', {}).get('amount') or \
               not self.market_info.get('precision', {}).get('price'):
            ...
            logger.critical(f"{Fore.RED}Failed to load crucial market info for {self.symbol}: {e}. Cannot continue. Aborting.{Style.RESET_ALL}", exc_info=True)
            sys.exit(1)
    ...
    def fetch_market_price(self) -> float | None:
        ...
        if ticker and ticker.get('last') is not None:
            ...
            logger.debug(f"Current market price ({self.symbol}): {price:.{self.market_info['precision']['price']}f}")  # Use market precision
            return price
        else:
    ...
    def fetch_order_book(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if bids and asks:
                ...
                else: logger.debug(f"Order Book ({self.symbol}) Imbalance: N/A (Zero Bid/Ask Vol)")
            else: logger.warning(f"{Fore.YELLOW}Order book data incomplete for {self.symbol}. Bids: {len(bids)}, Asks: {len(asks)}{Style.RESET_ALL}")
            return result
        except Exception as e:
    ...
    def fetch_historical_data(self, limit: int | None = None) -> pd.DataFrame | None:
        ...
        if required_limit is None:
             required_limit = max(
                 self.volatility_window + 1, self.ema_period, self.rsi_period + 1,
        ...
        try:
            ...
            if len(df) < initial_len: logger.debug(f"Dropped {initial_len - len(df)} rows with NaNs from history.")

            min_required_len = max(self.rsi_period + 1, self.macd_long_period, self.atr_period + 1)
            if len(df) < min_required_len:
                 logger.warning(f"{Fore.YELLOW}Insufficient history after cleaning for {self.symbol}. Got {len(df)}, needed ~{min_required_len}. Indicators may fail.{Style.RESET_ALL}")
                 return None
            ...
            logger.error(f"{Fore.RED}Error fetching/processing history for {self.symbol}: {e}{Style.RESET_ALL}", exc_info=True)
            return None
    ...
    def fetch_balance(self, currency_code: str | None = None) -> float:
        ...
        try:
            ...
            if free_balance is None or free_balance == 0.0:
                 usable_balance = balance_data.get('info', {}).get('result', {}).get('list', [{}])[0].get('totalAvailableBalance')  # Example for Bybit V5 Unified
                 if usable_balance is not None:
                     logger.debug(f"Using 'totalAvailableBalance' ({usable_balance}) as free balance for {quote_currency}")
            ...
            return free_balance
             ...
             if self.simulation_mode:
                 logger.warning(f"{Fore.YELLOW}Simulation mode: Returning dummy balance 10000 {quote_currency}.{Style.RESET_ALL}")
                 return 10000.0
             else:
    ...
    def fetch_order_status(self, order_id: str, symbol: str | None = None) -> dict[str, Any] | None:
        ...
        if not order_id: return None  # No ID to fetch
        ...
        try:
            ...
            logger.warning(f"{Fore.YELLOW}Order {order_id} not found on exchange ({target_symbol}). Assumed closed/cancelled.{Style.RESET_ALL}")
    ...
    def calculate_volatility(close_prices: pd.Series, window: int) -> float | None:
        if close_prices is None or len(close_prices) < window + 1: return None
        try:
    ...
    def calculate_ema(close_prices: pd.Series, period: int) -> float | None:
        if close_prices is None or len(close_prices) < period: return None
        try:
    ...
    def calculate_rsi(close_prices: pd.Series, period: int) -> float | None:
        if close_prices is None or len(close_prices) < period + 1: return None
        try:
    ...
    def calculate_macd(close_prices: pd.Series, short_period: int, long_period: int, signal_period: int) -> tuple[float | None, float | None, float | None]:
        ...
        if close_prices is None or len(close_prices) < min_len: return None, None, None
        try:
            ...
            if pd.isna(macd_val) or pd.isna(signal_val) or pd.isna(hist_val): return None, None, None
    ...
    def calculate_stoch_rsi(close_prices: pd.Series, rsi_period: int, stoch_period: int, k_period: int, d_period: int) -> tuple[float | None, float | None]:
        ...
        if close_prices is None or len(close_prices) < min_len: return None, None
        try:
            rsi = ScalpingBot.calculate_rsi(close_prices, rsi_period)  # Use static call
            if rsi is None: return None, None  # Need RSI first
            ...
            if len(stoch_rsi) < max(k_period, d_period): return None, None
    ...
    def calculate_atr(high_prices: pd.Series, low_prices: pd.Series, close_prices: pd.Series, period: int) -> float | None:
        ...
        if high_prices is None or low_prices is None or close_prices is None or \
           len(high_prices) < min_len or len(low_prices) < min_len or len(close_prices) < min_len: return None
        try:
    ...
    def calculate_order_size(self, current_price: float) -> float:
        ...
        if self.market_info is None: return 0.0  # Should not happen if init worked
        quote_currency = self.market_info['quote']
        balance = self.fetch_balance(currency_code=quote_currency)
        if balance <= 0:
            logger.warning(f"{Fore.YELLOW}Insufficient balance ({balance:.4f} {quote_currency}) for order size calc.{Style.RESET_ALL}")
            return 0.0
        ...
        if self.volatility_multiplier > 0:
            ...
            if volatility is not None and volatility > 1e-9:
                ...
                logger.info(f"Volatility ({volatility:.5f}) adjusted size factor: {size_factor:.3f}. Adjusted size: {final_size_quote:.4f} {quote_currency}")
            else: logger.debug("Volatility adjustment skipped (no data or low volatility).")
        else: logger.debug("Volatility adjustment disabled.")

        # Check Min Cost
        min_cost = self.market_info.get('limits', {}).get('cost', {}).get('min')
        if min_cost is not None and final_size_quote < min_cost:
             logger.warning(f"{Fore.YELLOW}Order size {final_size_quote:.4f} {quote_currency} below min cost {min_cost}. Setting size to 0.{Style.RESET_ALL}")
             return 0.0
        ...
        try:
            ...
            if min_amount is not None and amount_precise < min_amount:
                logger.warning(f"{Fore.YELLOW}Order amount {amount_precise:.{self.market_info['precision']['amount']}f} {self.market_info['base']} below min amount {min_amount}. Setting size to 0.{Style.RESET_ALL}")
                return 0.0
        except Exception as e:
            logger.error(f"{Fore.RED}Error checking/applying amount precision/limits: {e}{Style.RESET_ALL}")
            return 0.0

        logger.info(f"{Fore.CYAN}Calculated order size: {final_size_quote:.{self.market_info['precision']['price']}f} {quote_currency} ({amount_precise:.{self.market_info['precision']['amount']}f} {self.market_info['base']}){Style.RESET_ALL}")
    ...
    def _calculate_sl_tp_prices(self, entry_price: float, side: str, current_price: float, atr: float | None) -> tuple[float | None, float | None]:
        ...
        if self.use_atr_sl_tp:
            ...
            if self.base_stop_loss_pct is None or self.base_take_profit_pct is None:
                 logger.warning(f"{Fore.YELLOW}Fixed % SL/TP enabled but percentages not set in config. Cannot calculate SL/TP.{Style.RESET_ALL}")
                 return None, None
        ...
        try:
            ...
            logger.error(f"{Fore.RED}Error applying precision to SL/TP prices: {e}. SL={stop_loss_price}, TP={take_profit_price}{Style.RESET_ALL}")
            return None, None
        ...
        if side == "buy" and stop_loss_price is not None and stop_loss_price >= current_price:
            ...
            logger.warning(f"{Fore.YELLOW}Calculated SL ({stop_loss_price:.{price_precision}f}) >= current price ({current_price:.{price_precision}f}) for LONG. Adjusting to {adj_sl:.{price_precision}f}.{Style.RESET_ALL}")
            stop_loss_price = float(self.exchange.price_to_precision(self.symbol, adj_sl))
             ...
             logger.warning(f"{Fore.YELLOW}Calculated SL ({stop_loss_price:.{price_precision}f}) <= current price ({current_price:.{price_precision}f}) for SHORT. Adjusting to {adj_sl:.{price_precision}f}.{Style.RESET_ALL}")
             stop_loss_price = float(self.exchange.price_to_precision(self.symbol, adj_sl))
    ...
    def compute_trade_signal_score(self, price: float, indicators: dict[str, float | tuple[float | None, ...] | None], orderbook_imbalance: float | None) -> tuple[int, list[str]]:
        ...
        if orderbook_imbalance is not None:
            ...
            if orderbook_imbalance < imb_buy_thresh: score += 1.0; reasons.append(f"{Fore.GREEN}[+1.0] OB Buy Pressure (Imb: {orderbook_imbalance:.2f} < {imb_buy_thresh:.2f}){Style.RESET_ALL}")
            elif orderbook_imbalance > self.imbalance_threshold: score -= 1.0; reasons.append(f"{Fore.RED}[-1.0] OB Sell Pressure (Imb: {orderbook_imbalance:.2f} > {self.imbalance_threshold:.2f}){Style.RESET_ALL}")
            else: reasons.append(f"{Fore.WHITE}[ 0.0] OB Balanced (Imb: {orderbook_imbalance:.2f}){Style.RESET_ALL}")
        else: reasons.append(f"{Fore.WHITE}[ 0.0] OB Data N/A{Style.RESET_ALL}")

        # 2. EMA Trend (+/- 1)
        ema = indicators.get('ema')
        if ema is not None:
            if price > ema * 1.0002: score += 1.0; reasons.append(f"{Fore.GREEN}[+1.0] Price > EMA ({price:.2f} > {ema:.2f}){Style.RESET_ALL}")
            elif price < ema * 0.9998: score -= 1.0; reasons.append(f"{Fore.RED}[-1.0] Price < EMA ({price:.2f} < {ema:.2f}){Style.RESET_ALL}")
            else: reasons.append(f"{Fore.WHITE}[ 0.0] Price near EMA ({price:.2f} ~ {ema:.2f}){Style.RESET_ALL}")
        else: reasons.append(f"{Fore.WHITE}[ 0.0] EMA N/A{Style.RESET_ALL}")

        # 3. RSI Momentum/OB/OS (+/- 1)
        rsi = indicators.get('rsi')
        if rsi is not None:
            if rsi < RSI_OVERSOLD: score += 1.0; reasons.append(f"{Fore.GREEN}[+1.0] RSI Oversold ({rsi:.1f} < {RSI_OVERSOLD}){Style.RESET_ALL}")
            elif rsi > RSI_OVERBOUGHT: score -= 1.0; reasons.append(f"{Fore.RED}[-1.0] RSI Overbought ({rsi:.1f} > {RSI_OVERBOUGHT}){Style.RESET_ALL}")
            else: reasons.append(f"{Fore.WHITE}[ 0.0] RSI Neutral ({rsi:.1f}){Style.RESET_ALL}")
        else: reasons.append(f"{Fore.WHITE}[ 0.0] RSI N/A{Style.RESET_ALL}")

        # 4. MACD Momentum/Cross (+/- 1)
        macd_line, macd_signal = indicators.get('macd_line'), indicators.get('macd_signal')
        if macd_line is not None and macd_signal is not None:
            if macd_line > macd_signal: score += 1.0; reasons.append(f"{Fore.GREEN}[+1.0] MACD Line > Signal ({macd_line:.4f} > {macd_signal:.4f}){Style.RESET_ALL}")
            else: score -= 1.0; reasons.append(f"{Fore.RED}[-1.0] MACD Line <= Signal ({macd_line:.4f} <= {macd_signal:.4f}){Style.RESET_ALL}")
        else: reasons.append(f"{Fore.WHITE}[ 0.0] MACD N/A{Style.RESET_ALL}")

        # 5. Stochastic RSI OB/OS (+/- 1)
        stoch_k, stoch_d = indicators.get('stoch_k'), indicators.get('stoch_d')
        if stoch_k is not None and stoch_d is not None:
            if stoch_k < STOCH_OVERSOLD and stoch_d < STOCH_OVERSOLD: score += 1.0; reasons.append(f"{Fore.GREEN}[+1.0] StochRSI Oversold (K={stoch_k:.1f}, D={stoch_d:.1f} < {STOCH_OVERSOLD}){Style.RESET_ALL}")
            elif stoch_k > STOCH_OVERBOUGHT and stoch_d > STOCH_OVERBOUGHT: score -= 1.0; reasons.append(f"{Fore.RED}[-1.0] StochRSI Overbought (K={stoch_k:.1f}, D={stoch_d:.1f} > {STOCH_OVERBOUGHT}){Style.RESET_ALL}")
            else: reasons.append(f"{Fore.WHITE}[ 0.0] StochRSI Neutral (K={stoch_k:.1f}, D={stoch_d:.1f}){Style.RESET_ALL}")
        else: reasons.append(f"{Fore.WHITE}[ 0.0] StochRSI N/A{Style.RESET_ALL}")
    ...
    def place_entry_order(
        ...
        try:
            ...
            if stop_loss_price is not None:
                ...
                if self.sl_trigger_by: params['slTriggerBy'] = self.sl_trigger_by  # Add trigger type if configured
            if take_profit_price is not None:
                params['takeProfitPrice'] = self.exchange.price_to_precision(self.symbol, take_profit_price)
                if self.tp_trigger_by: params['tpTriggerBy'] = self.tp_trigger_by  # Add trigger type if configured
            ...
            min_cost = limits.get('cost', {}).get('min')
            if min_amount is not None and amount_precise < min_amount:
                 logger.warning(f"{Fore.YELLOW}Entry amount {amount_precise:.{amount_precision}f} {base_currency} below min {min_amount}. Cannot place.{Style.RESET_ALL}")
                 return None
            cost_estimate = amount_precise * (limit_price_precise or current_price)
            if min_cost is not None and cost_estimate < min_cost:
                 logger.warning(f"{Fore.YELLOW}Estimated entry cost {cost_estimate:.{price_precision}f} {quote_currency} below min {min_cost}. Cannot place.{Style.RESET_ALL}")
                 return None
        ...
        if self.simulation_mode:
            ...
            simulated_order = {
                "id": sim_id, "clientOrderId": sim_id, "timestamp": int(time.time() * 1000),
                "datetime": pd.to_datetime('now', utc=True).isoformat(), "symbol": self.symbol,
                "type": order_type, "side": side, "amount": amount_precise,
                "price": limit_price_precise if order_type == "limit" else None,
                "average": sim_entry_price if sim_status == 'closed' else None,
                "cost": sim_cost if sim_status == 'closed' else 0.0, "status": sim_status,
                "filled": amount_precise if sim_status == 'closed' else 0.0,
                "remaining": 0.0 if sim_status == 'closed' else amount_precise,
                "info": {
                    "simulated": True, "confidence": confidence_level,
        ...
        # --- Live Trading ---
            ...
            logger.info(f" -> Details: Size={amount_precise:.{amount_precision}f} {base_currency}, Limit={limit_price_precise}, Params={params}")
            order = None
            try:
                ...
                if order:
                    ...
                    log_price = f"Price: {oprice:.{price_precision}f}" if oprice else f"Market (~{current_price:.{price_precision}f})"
                    if oavg: log_price += f", Avg: {oavg:.{price_precision}f}"
                    ...
                    logger.error(f"{Fore.RED}LIVE {action_desc} order placement failed: API returned None.{Style.RESET_ALL}")
                    return None
                ...
                logger.error(f"{Fore.RED}LIVE {action_desc} Order Failed (Unexpected Error): {e}{Style.RESET_ALL}", exc_info=True)
                return None
    ...
    def cancel_order_by_id(self, order_id: str, symbol: str | None = None) -> bool:
        ...
        try:
            ...
            logger.warning(f"{Fore.YELLOW}Order {order_id} not found for cancellation (already closed/cancelled?).{Style.RESET_ALL}")
        ...
        try:
            if self.exchange.has['fetchOpenOrders']:
                ...
                for order in open_orders:
                 ...
                 logger.warning(f"{Fore.YELLOW}Exchange lacks fetchOpenOrders, attempting cancelAllOrders for {target_symbol}...{Style.RESET_ALL}")
                 ...
                 logger.error(f"{Fore.RED}Exchange does not support fetchOpenOrders or cancelAllOrders. Cannot cancel automatically.{Style.RESET_ALL}")
                 return 0
            logger.info(f"Order cancellation finished for {target_symbol}. Cancelled: {cancelled_count if cancelled_count >= 0 else 'Unknown'}.")
    ...
    def _check_pending_entries(self, indicators: dict) -> None:
        ...
        for position in self.open_positions:
            if position['status'] == STATUS_PENDING_ENTRY:
                ...
                if order_info is None:
                    logger.warning(f"{Fore.YELLOW}Pending entry order {entry_order_id} status unknown/not found. Removing.{Style.RESET_ALL}")
                ...
                if order_status == 'closed':
                    if entry_price is None:
                        logger.error(f"{Fore.RED}Entry order {entry_order_id} closed but average price missing! Cannot manage. Removing.{Style.RESET_ALL}")
                        positions_to_update.append({'id': entry_order_id, 'action': 'remove'})
                        continue

                    orig_size = position.get('original_size', position['size'])  # Use original if stored, else current
                    if filled_amount is None or abs(filled_amount - orig_size) / orig_size > 0.05:  # Check > 5% deviation
                         logger.warning(f"{Fore.YELLOW}Entry order {entry_order_id} closed but filled amount ({filled_amount}) differs >5% from requested ({orig_size}). Using filled amount.{Style.RESET_ALL}")
                    ...
                    position['entry_time'] = order_info.get('timestamp', time.time() * 1000) / 1000  # Use fill time if available
                    ...
                    if current_price:
                        ...
                        sl_price, tp_price = self._calculate_sl_tp_prices(
                            ...
                            current_price=current_price, atr=atr_value
                        )
                        ...
                        logger.info(f"Stored SL={sl_price}, TP={tp_price} for activated position {entry_order_id}")
                         ...
                         logger.error(f"{Fore.RED}Could not fetch current price after entry fill for {entry_order_id}. SL/TP prices not calculated!{Style.RESET_ALL}")
                    ...
                    logger.warning(f"{Fore.YELLOW}Pending entry order {entry_order_id} failed (Status: {order_status}, Reason: {reason}). Removing.{Style.RESET_ALL}")
                    positions_to_update.append({'id': entry_order_id, 'action': 'remove'})
        ...
        if positions_to_update:
            ...
            for pos in self.open_positions:
                if pos['id'] in ids_to_remove: continue
                elif pos['id'] in updates_dict: new_positions_list.append(updates_dict[pos['id']])
                else: new_positions_list.append(pos)
    ...
    def _manage_active_positions(self, current_price: float) -> None:
        ...
        logger.debug(f"Managing active positions against current price: {current_price:.{self.market_info['precision']['price']}f}...")
        positions_to_remove_ids = []
        positions_to_update_state = {}  # Store state updates (like TSL price)

        for position in self.open_positions:
            if position.get('status') != STATUS_ACTIVE: continue
            ...
            if order_info is None:
                logger.warning(f"{Fore.YELLOW}Active position's main order {pos_id} not found. Assuming closed/cancelled externally. Removing.{Style.RESET_ALL}")
                ...
                take_profit_hit_price = order_info.get('info', {}).get('takeProfitPrice')  # Check if TP price is in info

                # Infer reason more reliably if possible
                if stop_loss_hit_price and abs(float(stop_loss_hit_price) - exit_price) / exit_price < 0.001:  # SL triggered
                ...
                elif take_profit_hit_price and abs(float(take_profit_hit_price) - exit_price) / exit_price < 0.001:  # TP triggered
                    ...
                    sl_triggered = stored_sl_price and ((side == 'buy' and exit_price <= stored_sl_price * 1.001) or (side == 'sell' and exit_price >= stored_sl_price * 0.999))
                    tp_triggered = stored_tp_price and ((side == 'buy' and exit_price >= stored_tp_price * 0.999) or (side == 'sell' and exit_price <= stored_tp_price * 1.001))
                    if sl_triggered: exit_reason = f"Stop Loss Triggered (Inferred - Order {pos_id})"
                    elif tp_triggered: exit_reason = f"Take Profit Triggered (Inferred - Order {pos_id})"
                    else: exit_reason = f"Order Closed (Reason unclear - {pos_id})"
            ...
            # --- 2. Time-Based Exit Check (if order still open) ---
                ...
                if time_elapsed_minutes >= self.time_based_exit_minutes:
                    logger.info(f"{Fore.YELLOW}Time limit ({self.time_based_exit_minutes} min) reached for position {pos_id}. Initiating market close.{Style.RESET_ALL}")
                    ...
                    if self.cancel_order_by_id(pos_id):  # Cancel original order first
                        ...
                        if market_close_order: exit_price = market_close_order.get('average', current_price)
                        else: exit_reason = None; logger.critical(f"{Fore.RED}FAILED MARKET CLOSE FOR TIME EXIT - {pos_id} REMAINS OPEN! MANUAL INTERVENTION!{Style.RESET_ALL}")
                    else:
                        exit_reason = None; logger.error(f"{Fore.RED}Failed to cancel original order {pos_id} for time exit. Close aborted.{Style.RESET_ALL}")
            ...
            if not exit_reason and self.enable_trailing_stop_loss:
                ...
                if not is_tsl_active:
                    ...
                    if stored_tp_price:  # Activate halfway to TP
                        activate_threshold = entry_price + (stored_tp_price - entry_price) * 0.5 if side == 'buy' else \
                                             entry_price - (entry_price - stored_tp_price) * 0.5
                    if activate_threshold and ((side == 'buy' and current_price >= activate_threshold) or (side == 'sell' and current_price <= activate_threshold)):
                        ...
                        new_tsl_price = max(potential_tsl, breakeven_plus) if side == 'buy' else min(potential_tsl, breakeven_plus)
                        new_tsl_price = float(self.exchange.price_to_precision(self.symbol, new_tsl_price))
                        logger.info(f"{Fore.MAGENTA}Trailing Stop ACTIVATED for {side.upper()} {pos_id} at {new_tsl_price:.{self.market_info['precision']['price']}f} (Price: {current_price:.{self.market_info['precision']['price']}f}){Style.RESET_ALL}")
                # Update
                elif is_tsl_active:
                    tsl_factor = (1 - self.trailing_stop_loss_percentage) if side == 'buy' else (1 + self.trailing_stop_loss_percentage)
                    potential_tsl = current_price * tsl_factor
                    if (side == 'buy' and potential_tsl > trailing_sl_price) or (side == 'sell' and potential_tsl < trailing_sl_price):
                        new_tsl_price = float(self.exchange.price_to_precision(self.symbol, potential_tsl))
                        logger.info(f"{Fore.MAGENTA}Trailing Stop UPDATED for {side.upper()} {pos_id} to {new_tsl_price:.{self.market_info['precision']['price']}f} (Price: {current_price:.{self.market_info['precision']['price']}f}){Style.RESET_ALL}")

                # Apply TSL Update via edit_order (NEEDS VERIFICATION)
                if new_tsl_price is not None:
                    logger.warning(f"{Fore.YELLOW}Attempting TSL update via edit_order for {pos_id} to SL={new_tsl_price}. VERIFY EXCHANGE/CCXT SUPPORT!{Style.RESET_ALL}")
                    try:
                        edit_params = {'stopLossPrice': self.exchange.price_to_precision(self.symbol, new_tsl_price)}
                        if self.sl_trigger_by: edit_params['slTriggerBy'] = self.sl_trigger_by  # Include trigger type if set
                        ...
                        if edited_order:
                            logger.info(f"{Fore.MAGENTA}Successfully modified order {pos_id} with new TSL price {new_tsl_price:.{self.market_info['precision']['price']}f}.{Style.RESET_ALL}")
                            positions_to_update_state[pos_id] = {
                                'trailing_stop_price': new_tsl_price,
                                'stop_loss_price': new_tsl_price,  # Update stored SL
                                'last_update_time': time.time()
                            }
                        else: logger.error(f"{Fore.RED}Failed TSL update for {pos_id} (edit_order returned None).{Style.RESET_ALL}")
                    except ccxt.NotSupported as e:
                        logger.error(f"{Fore.RED}TSL update failed: edit_order for SL not supported by Exchange/CCXT for {pos_id}: {e}. Disable TSL or implement cancel/replace.{Style.RESET_ALL}")
                        # Disable TSL for future iterations if not supported?
                        # self.enable_trailing_stop_loss = False
                    except Exception as e:
                        logger.error(f"{Fore.RED}Error modifying order {pos_id} for TSL: {e}{Style.RESET_ALL}", exc_info=True)
            ...
            if exit_reason:
                ...
                if exit_price is not None:
                    pnl_quote = (exit_price - entry_price) * position['size'] if side == 'buy' else (entry_price - exit_price) * position['size']
                    pnl_pct = (exit_price / entry_price - 1) * 100 if side == 'buy' and entry_price else \
                              (entry_price / exit_price - 1) * 100 if side == 'sell' and exit_price else 0.0
                    pnl_color = Fore.GREEN if pnl_quote >= 0 else Fore.RED
                    logger.info(f"{pnl_color}---> Position {pos_id} Closed. Reason: {exit_reason}. Est. PnL: {pnl_quote:.4f} {self.market_info['quote']} ({pnl_pct:.3f}%){Style.RESET_ALL}")
                    self.daily_pnl += pnl_quote
                else:
                    logger.warning(f"{Fore.YELLOW}---> Position {pos_id} Closed. Reason: {exit_reason}. Exit price unknown, cannot calculate PnL.{Style.RESET_ALL}")
        ...
        if positions_to_remove_ids or positions_to_update_state:
            ...
            for pos in self.open_positions:
                pos_id = pos['id']
                if pos_id in positions_to_remove_ids: continue
                if pos_id in positions_to_update_state: pos.update(positions_to_update_state[pos_id])
    ...
    def _place_market_close_order(self, position: dict[str, Any], current_price: float) -> dict[str, Any] | None:
        ...
        logger.warning(f"{log_color}Placing MARKET CLOSE order for position {pos_id} (Side: {close_side}, Size: {size})...{Style.RESET_ALL}")

        # Simulation
        if self.simulation_mode:
            sim_id = f"sim_close_{int(time.time() * 1000)}_{close_side[:1]}"
            size * current_price
            simulated_order = {"id": sim_id, "type": "market", "side": close_side, "amount": size, "average": current_price, "status": 'closed', "filled": size, "info": {"simulated": True, "position_id": pos_id, "reduceOnly": True}}  # Added reduceOnly flag
            logger.info(f"{log_color}[SIMULATION] Market Close Order: ID {sim_id}, Size {size}, AvgPrice {current_price}{Style.RESET_ALL}")
            return simulated_order
        ...
        # Live Trading
            ...
            try:
                ...
                if order:
                    ...
                    ostatus = order.get('status', STATUS_UNKNOWN)
                    logger.info(f"{log_color}---> LIVE Market Close Order Placed: ID {oid}, AvgPrice ~{oavg:.{self.market_info['precision']['price']}f}, Status {ostatus}{Style.RESET_ALL}")
                    return order
                else:
                    logger.error(f"{Fore.RED}LIVE Market Close order placement failed: API returned None.{Style.RESET_ALL}")
                    return None
            except (ccxt.InsufficientFunds, ccxt.InvalidOrder, ccxt.ExchangeError) as e:
                 logger.error(f"{Fore.RED}LIVE Market Close Order Failed ({type(e).__name__}): {e}{Style.RESET_ALL}")
                 return None
            except Exception as e:
                logger.error(f"{Fore.RED}LIVE Market Close Order Failed (Unexpected Error): {e}{Style.RESET_ALL}", exc_info=True)
                return None
    ...
    def _calculate_indicators(self, historical_data: pd.DataFrame) -> dict[str, Any]:
        ...
        logger.info(f"Indicators: EMA={log_inds['ema']}, RSI={log_inds['rsi']}, ATR={log_inds['atr']}, "
    ...
    def _process_signals_and_entry(self, market_data: dict, indicators: dict) -> None:
        ...
        if abs(signal_score) >= 1 or logger.isEnabledFor(logging.DEBUG):
            for reason in reasons: logger.info(f" -> {reason}")

        # Check if new position can be opened
        can_open_new = len(self.open_positions) < self.max_open_positions
        if not can_open_new:
             logger.info(f"{Fore.YELLOW}Max open positions ({self.max_open_positions}) reached. No new entries.{Style.RESET_ALL}")
             return
        if not can_trade:
        ...
        if entry_side:
            ...
            logger.info(f"{log_color}Potential {entry_side.upper()} entry signal detected (Score: {signal_score}){Style.RESET_ALL}")
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"{loop_prefix} Current Price: {current_price:.{self.market_info['precision']['price']}f}, OB Imbalance: {f'{market_data['order_book_imbalance']:.3f}' if market_data['order_book_imbalance'] is not None else 'N/A'}")
                ...
                logger.critical(f"{Fore.RED}{loop_prefix} Critical error in main loop: {e}{Style.RESET_ALL}", exc_info=True)
    ...
    def shutdown(self) -> None:
        ...
        if CLOSE_POSITIONS_ON_EXIT and active_positions:
             logger.warning(f"{Fore.YELLOW}Attempting market close for {len(active_positions)} active position(s)...{Style.RESET_ALL}")
             current_price = self.fetch_market_price()
             if current_price:
                 for position in list(active_positions):  # Iterate copy
                    close_order = self._place_market_close_order(position, current_price)
                    if close_order:
                        logger.info(f"{Fore.YELLOW}Market close order placed for position {position['id']}.{Style.RESET_ALL}")
                        # Find and remove the position from the main list
                        self.open_positions = [p for p in self.open_positions if p['id'] != position['id']]
                    else:
                        logger.error(f"{Fore.RED}Failed market close for {position['id']}. Manual check needed!{Style.RESET_ALL}")
             else:
                 logger.error(f"{Fore.RED}Cannot fetch price for market close. Positions remain open! Manual check needed!{Style.RESET_ALL}")
        elif active_positions:
            logger.warning(f"{Fore.YELLOW}{len(active_positions)} position(s) remain active. Manual management required.{Style.RESET_ALL}")
            for pos in active_positions: logger.warning(f" -> ID={pos['id']}, Side={pos['side']}, Size={pos['size']}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if e.code != 0: logger.error(f"Bot exited via SystemExit (Code: {e.code}).")
        else: logger.info("Bot exited cleanly.")
        exit_code = e.code

    except Exception as e:
        logger.critical(f"{Fore.RED}An unhandled critical error broke the main spell: {e}{Style.RESET_ALL}", exc_info=True)
        exit_code = 1
