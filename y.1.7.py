Okay, I've enhanced the provided Python trading bot code based on the structure and intent, incorporating improvements focusing on robustness, clarity, error handling, Decimal usage, Bybit V5 specifics, and state management.
            ...
            for item in value:
                if isinstance(item, dict):
                    ...
                    if item == 'NaN': new_list.append(Decimal('NaN'))
                        ...
                        except InvalidOperation: new_list.append(item) # Keep as string
                else:
...
def validate_config(config: Dict[str, Any], logger: logging.Logger) -> bool:
    ...
    for key in required_trading_keys:
        if key not in settings:
            ...
            is_valid = False
    if not is_valid: return False # Stop if basic trading settings are missing
         ...
         if not isinstance(weights, dict):
              ...
              for key, val in weights.items():
                   if not isinstance(val, Decimal) or not val.is_finite() or val < 0:
                        logger.error(f"Config validation failed: Signal weight '{key}' must be a non-negative finite number (loaded as Decimal).")
                        is_valid = False
...
class BybitV5Wrapper:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger):
        ...
        if config['exchange'].get('sandbox_mode', False):
            ...
            ccxt_config['sandboxMode'] = True # Newer CCXT versions might prefer this
    ...
    def load_markets_with_retry(self, reload=False):
        ...
        raise ccxt.ExchangeError("Failed to load markets after multiple retries.") # Raise specific error
    ...
    def get_market(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not self.exchange.markets or symbol not in self.exchange.markets:
                self.logger.warning(f"Market data for {symbol} not loaded or missing. Attempting to load/reload markets.")
                self.load_markets_with_retry(reload=True) # Use retry mechanism
            ...
            if not precision or 'amount' not in precision or 'price' not in precision:
                 self.logger.warning(f"Precision info incomplete for market {symbol}. Reloading markets again.")
    ...
    def safe_ccxt_call(self, method_name: str, *args, **kwargs) -> Optional[Any]:
        ...
        while retries <= self.max_retries:
            ...
            # --- Specific CCXT/Bybit Error Handling (Non-Retryable First) ---
                ...
                self.logger.error(f"Authentication Error calling {method_name}: {e}. Check API keys. Non-retryable.")
                return None
            except ccxt.PermissionDenied as e:
                 self.logger.error(f"Permission Denied calling {method_name}: {e}. Check API key permissions (IP whitelist, endpoint access). Non-retryable.")
                 return None
                ...
                self.logger.error(f"Invalid Order parameters or state calling {method_name}: {e}. Check order details (size, price, type, status). Non-retryable.")
                ...
                return None
                 ...
                 self.logger.error(f"Bad Request calling {method_name}: {e}. Check parameters. Assuming non-retryable.")
                 self.logger.debug(f"Failed Call Details - Method: {method_name}, Args: {args}, Kwargs: {kwargs}")
                 # Example: Parse e.args[0] for specific Bybit error codes
                 if self.exchange_id == 'bybit':
                      msg = str(e).lower()
                      # Bybit specific non-retryable error codes (examples)
                      non_retryable_codes = ['110007', '110045', '110043', '110014'] # Param error, position idx error, set leverage error, qty error
                 ...
                 self.logger.error(f"Operation Failed calling {method_name}: {e}. May indicate position/margin issues. Assuming non-retryable.")
                 return None

            # --- Retryable Errors ---
            except ccxt.RateLimitExceeded as e:
                self.logger.warning(f"Rate Limit Exceeded calling {method_name}: {e}. Retrying in {self.retry_delay}s... (Attempt {retries + 1}/{self.max_retries + 1})")
            except ccxt.NetworkError as e: # Includes ConnectionError, Timeout, etc.
                self.logger.warning(f"Network Error calling {method_name}: {e}. Retrying in {self.retry_delay}s... (Attempt {retries + 1}/{self.max_retries + 1})")
            except ccxt.ExchangeNotAvailable as e: # Maintenance or temporary outage
                self.logger.warning(f"Exchange Not Available calling {method_name}: {e}. Retrying in {self.retry_delay}s... (Attempt {retries + 1}/{self.max_retries + 1})")
            except ccxt.OnMaintenance as e: # Specific maintenance error
                 self.logger.warning(f"Exchange On Maintenance calling {method_name}: {e}. Retrying in {self.retry_delay}s... (Attempt {retries + 1}/{self.max_retries + 1})")
                ...
                retryable_codes = ['10002', '10006', '10016'] # Server busy, timeout, internal error (sometimes retryable)
                ...
                if any(term in msg for term in non_retryable_msgs) and not any(f"ret_code={code}" in msg or f"retcode={code}" in msg for code in retryable_codes):
                     ...
                     self.logger.warning(f"Generic Exchange Error calling {method_name}: {e}. Retrying in {self.retry_delay}s... (Attempt {retries + 1}/{self.max_retries + 1})")
        ...
        try:
                    ...
                    if account_to_parse:
                        ...
                        with getcontext() as ctx:
                            ...
                            for coin_info in coin_data:
                                ...
                                equity_str = coin_info.get('equity') # Use 'equity' for total value including PnL
            ...
            if not balances:
                 ...
                 if quote_asset in balances:
                      self.logger.info(f"{quote_asset} Equity: {balances[quote_asset]:.{DECIMAL_DISPLAY_PRECISION}f}")
                 else:
                      self.logger.warning(f"Configured quote asset '{quote_asset}' not found in fetched balances.")
            return balances
        ...
        try:
            with getcontext() as ctx:
                ...
                for pos in positions_data:
                    ...
                    contracts_val = pos.get('contracts') # Standard CCXT field (might be float/int/None)
                    ...
                    if size_str is not None and size_str != '':
                        try:
                            ...
                            if size_dec.is_finite() and size_dec != Decimal(0): is_active = True
                        except InvalidOperation:
                             self.logger.warning(f"Could not parse position size '{size_str}' from info as Decimal for {target_symbol}.")
                    elif contracts_val is not None:
                         try:
                              size_dec = Decimal(str(contracts_val))
                              if size_dec.is_finite() and size_dec != Decimal(0): is_active = True
                         except InvalidOperation:
                              self.logger.warning(f"Could not parse position contracts '{contracts_val}' as Decimal for {target_symbol}.")

                    if not is_active:
                        self.logger.debug(f"Skipping zero-size or invalid-size position for: {target_symbol}")
                        continue # Skip zero-size or unparsable-size positions

                    processed = pos.copy() # Work on a copy
                    processed['contracts'] = size_dec # Store unified Decimal size in 'contracts'
                    ...
                    for field in decimal_fields_std:
                        if field in processed and processed[field] is not None:
                            try: processed[field] = Decimal(str(processed[field]))
                                 ...
                                 self.logger.warning(f"Could not convert standard position field '{field}' ({processed[field]}) to Decimal for {target_symbol}.")
                        ...
                        for field in decimal_fields_info:
                            if field in info and info[field] is not None and info[field] != '':
                                try: info[field] = Decimal(str(info[field]))
                                    ...
                                    self.logger.warning(f"Could not convert position info field '{field}' ({info[field]}) to Decimal for {target_symbol}.")
                    ...
                    if 'info' in processed and isinstance(processed['info'], dict) and 'side' in processed['info']:
                        ...
                        else: self.logger.warning(f"Unrecognized side '{side_str}' in position info for {target_symbol}")
                    ...
                    if side_enum == PositionSide.NONE and size_dec != Decimal(0):
                         self.logger.warning(f"Position for {target_symbol} has size {size_dec} but side is determined as 'None'. Check exchange data consistency.")
                         # Might indicate an issue, but store 'none' as determined
                         # Logic using the position should handle 'none' side defensively

                    processed['side'] = side_enum.value # Update main 'side' field with 'long'/'short'/'none' string
                    ...
                    if pos_idx_val is not None:
                        try: processed['positionIdx'] = int(pos_idx_val)
                             ...
                             self.logger.warning(f"Could not parse positionIdx '{pos_idx_val}' as integer for {target_symbol}.")
                             processed['positionIdx'] = None # Mark as None if parsing fails
                    else:
            ...
            self.logger.error(f"Error processing position data for {target_symbol}: {e}", exc_info=True)
    ...
    def format_value_for_api(self, symbol: str, value_type: str, value: Decimal,
        ...
        try:
            ...
            return formatted_value
             ...
             self.logger.error(f"CCXT error formatting {value_type} ('{value}') for {symbol}: {e}")
             raise ValueError(f"CCXT error formatting {value_type} for {symbol}") from e
        except Exception as e:
             self.logger.error(f"Unexpected error formatting {value_type} ('{value}') for {symbol}: {e}", exc_info=True)
    ...
    def quantize_value(self, value: Decimal, precision_type: str, market: Dict[str, Any],
        ...
        if not market or 'precision' not in market:
             self.logger.error("Market data or precision missing for quantization.")
             return None

        precision_val_str = market['precision'].get(precision_type)
        if precision_val_str is None:
             self.logger.error(f"Precision value for type '{precision_type}' not found in market data.")
        ...
        try:
            ...
            return quantized_value
             ...
             self.logger.error(f"Error quantizing value {value} with {precision_type} precision '{precision_val_str}': {e}")
             return None
    ...
    def create_order(self, symbol: str, order_type: str, side: OrderSide, amount: Decimal,
        ...
        try:
            ...
            if Decimal(amount_str) <= 0:
                 self.logger.error(f"Order amount {amount} after formatting to step size became non-positive ({amount_str}). Cannot place order.")
                 return None

            price_str = None
            if price is not None and order_type.lower() not in ['market', 'stop_market', 'take_profit_market']:
                price_str = self.format_value_for_api(symbol, 'price', price, rounding_mode=ROUND_HALF_UP) # Default rounding for price
        ...
        if self.exchange_id == 'bybit':
            ...
            if self.hedge_mode:
                if 'positionIdx' not in order_params:
                    ...
                    if not order_params.get('reduceOnly', False): # Assume opening if not reduceOnly
                         order_params['positionIdx'] = 1 if side == OrderSide.BUY else 2
                         self.logger.debug(f"Hedge mode: Setting positionIdx={order_params['positionIdx']} for opening {side.value} order.")
    ...
    def set_leverage(self, symbol: str, leverage: Decimal) -> bool:
        ...
        leverage_str = f"{int(leverage)}" # Bybit expects integer string for leverage
        ...
        if result is not None: # safe_ccxt_call returns None only on failure after retries
             ...
             if self.exchange_id == 'bybit' and isinstance(result, dict) and 'info' in result:
                  ...
                  if ret_code == 0:
                       ...
                       self.logger.warning(f"Bybit leverage setting response - Code: {ret_code}, Msg: {ret_msg}. Assuming success if no modification needed, check manually if unsure.")
    ...
    def set_protection(self, symbol: str, stop_loss: Optional[Decimal] = None,
        ...
        try:
            ...
            if stop_loss is not None:
                ...
                if quantized_sl is None: raise ValueError("Failed to quantize stop loss price.")
            ...
            if take_profit is not None:
                ...
                if quantized_tp is None: raise ValueError("Failed to quantize take profit price.")
            ...
            if trailing_stop:
                ...
                if ts_value is not None:
                     ...
                     if quantized_ts_dist is None: raise ValueError("Failed to quantize trailing stop distance.")
                ...
                if ts_active_price is not None:
                     ...
                     if quantized_ts_active is None: raise ValueError("Failed to quantize trailing stop activation price.")
...
class TradingAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger):
        self.logger = logger
        self.indicator_config = config.get("indicator_settings", {}) # Renamed for clarity
        ...
        if not self.weights:
            ...
            for k, v in self.weights.items():
                if isinstance(v, Decimal) and v.is_finite() and v >= 0:
                    ...
                    self.logger.warning(f"Invalid signal weight for '{k}' ({v}). Removing from calculation.")
            self.weights = valid_weights

        if not self.weights:
            self.logger.error("No valid signal weights configured. Cannot generate weighted signals.")
            return # Leave weights empty

        total_weight = sum(self.weights.values())
        if total_weight <= Decimal('1e-18'): # Check if total weight is effectively zero
            self.logger.error("Total signal weight is zero or negligible. Cannot normalize. Disabling weighted signals.")
            self.weights = {} # Disable weights
             ...
             try:
                  ...
                  with getcontext() as ctx:
                       ctx.prec = CALCULATION_PRECISION
                       self.weights = {k: (v / total_weight).quantize(Decimal("1e-6"), rounding=ROUND_HALF_UP) for k, v in self.weights.items()} # Quantize for clarity
                  ...
                  self.logger.error(f"Error normalizing weights: {e}. Disabling weighted signals.")
                  self.weights = {}
    ...
    def calculate_indicators(self, ohlcv_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
             if not all(isinstance(ohlcv_df[col].iloc[0], Decimal) for col in required_cols if not ohlcv_df[col].empty):
                  self.logger.error("OHLCV columns are not all Decimal type. Conversion error likely occurred earlier.")
                  return None
        except IndexError:
        ...
        df_float = pd.DataFrame(index=df.index) # Create new df for float conversion
        ...
        if len(df_float) < initial_len:
             self.logger.warning(f"Dropped {initial_len - len(df_float)} rows with NaN in OHLC columns before TA calculation.")
        ...
        try:
            ...
            if isinstance(rsi_period, int) and rsi_period > 0:
                df_float.ta.rsi(length=rsi_period, append=True, col_names=(f"RSI_{rsi_period}"))
            ...
            if all(isinstance(p, int) and p > 0 for p in [macd_fast, macd_slow, macd_signal]):
                df_float.ta.macd(fast=macd_fast, slow=macd_slow, signal=macd_signal, append=True) # Uses default names MACD_F_S_SIG, MACDh_..., MACDs_...

            # EMAs
            ema_short_period = self.indicator_config.get("ema_short_period")
            if isinstance(ema_short_period, int) and ema_short_period > 0:
                df_float.ta.ema(length=ema_short_period, append=True, col_names=(f"EMA_{ema_short_period}"))
            ema_long_period = self.indicator_config.get("ema_long_period")
            if isinstance(ema_long_period, int) and ema_long_period > 0:
                df_float.ta.ema(length=ema_long_period, append=True, col_names=(f"EMA_{ema_long_period}"))
            ...
            if isinstance(atr_period, int) and atr_period > 0:
                ...
                df_float.ta.atr(length=atr_period, append=True, mamode='rma', col_names=(f"ATR_{atr_period}"))
            ...
            self.logger.error(f"Error calculating technical indicators with pandas_ta: {e}", exc_info=True)
            return None
    ...
    def generate_signal(self, indicators_df: pd.DataFrame) -> Tuple[Signal, Dict[str, Any]]:
        ...
        try:
            ...
            latest_data = indicators_df.iloc[-1]
             ...
             self.logger.error("Cannot get latest data: Indicators DataFrame is unexpectedly empty after calculation.")
             return Signal.HOLD, {}
        ...
        with getcontext() as ctx:
            ...
            if rsi_weight > 0 and rsi_col and rsi_col in latest_data:
                ...
                if isinstance(rsi_value, Decimal) and rsi_value.is_finite():
                    overbought = self.indicator_config.get("rsi_overbought", Decimal("70"))
                    oversold = self.indicator_config.get("rsi_oversold", Decimal("30"))
                    rsi_score = Decimal(0)
                    if rsi_value > overbought: rsi_score = Decimal("-1") # Sell
                    elif rsi_value < oversold: rsi_score = Decimal("1")  # Buy
                    scores[rsi_key] = rsi_score * rsi_weight
                    contributing_factors[rsi_key] = {"value": rsi_value, "score": rsi_score, "weight": rsi_weight}
                else: self.logger.debug(f"Invalid RSI value ({rsi_value}). Skipping RSI score.")
            ...
            if macd_weight > 0 and macdh_col and macdh_col in latest_data:
                ...
                if isinstance(macd_hist, Decimal) and macd_hist.is_finite():
                    hist_threshold = self.indicator_config.get("macd_hist_threshold", Decimal("0"))
                    macd_score = Decimal(0)
                    if macd_hist > hist_threshold: macd_score = Decimal("1")  # Buy
                    elif macd_hist < -hist_threshold: macd_score = Decimal("-1") # Sell
                    scores[macd_key] = macd_score * macd_weight
                    contributing_factors[macd_key] = {"histogram": macd_hist, "score": macd_score, "weight": macd_weight}
                else: self.logger.debug(f"Invalid MACD Histogram value ({macd_hist}). Skipping MACD score.")
            ...
            if ema_cross_weight > 0 and ema_short_col and ema_long_col and all(c in latest_data for c in [ema_short_col, ema_long_col]):
                ...
                if isinstance(ema_short, Decimal) and ema_short.is_finite() and isinstance(ema_long, Decimal) and ema_long.is_finite():
                    ...
                    if ema_short > ema_long: ema_cross_score = Decimal("1")  # Bullish state
                    elif ema_short < ema_long: ema_cross_score = Decimal("-1") # Bearish state
                    scores[ema_key] = ema_cross_score * ema_cross_weight
                    contributing_factors[ema_key] = {"short_ema": ema_short, "long_ema": ema_long, "score": ema_cross_score, "weight": ema_cross_weight}
                else: self.logger.debug(f"Invalid EMA values (Short={ema_short}, Long={ema_long}). Skipping EMA Cross score.")
        ...
        strong_buy_thresh = self.indicator_config.get("strong_buy_threshold", Decimal("0.7"))
        buy_thresh = self.indicator_config.get("buy_threshold", Decimal("0.2"))
        sell_thresh = self.indicator_config.get("sell_threshold", Decimal("-0.2"))
        strong_sell_thresh = self.indicator_config.get("strong_sell_threshold", Decimal("-0.7"))

        # Validate thresholds order
        if not (strong_sell_thresh <= sell_thresh < buy_thresh <= strong_buy_thresh):
             self.logger.error("Signal thresholds are improperly configured (overlapping or wrong order: SS <= S < B <= SB). Using default HOLD.")
             final_signal = Signal.HOLD
        else:
            if final_score >= strong_buy_thresh: final_signal = Signal.STRONG_BUY
            elif final_score >= buy_thresh: final_signal = Signal.BUY
            elif final_score <= strong_sell_thresh: final_signal = Signal.STRONG_SELL
            elif final_score <= sell_thresh: final_signal = Signal.SELL
        ...
        self.logger.debug(f"Contributing factors: {contributing_factors}") # Factors remain high precision Decimals
        ...
        signal_details_out = {"final_score": quantized_score, "factors": contributing_factors} # Keep Decimals for now
...
class PositionManager:
    ...
    def calculate_position_size(self, entry_price: Decimal, stop_loss_price: Decimal,
        ...
        with getcontext() as ctx:
            ...
            self.logger.info(f"Risk per trade: {risk_percent:.2%}, Equity ({quote_asset}): {quantized_equity}, Risk Amount: {quantized_risk_amount} {quote_asset}")
            ...
            contract_type = market.get('type', 'spot') # spot, swap (linear/inverse determined next)
            is_inverse = market.get('inverse', False)
            is_linear = market.get('linear', True) if contract_type == 'swap' and not is_inverse else False # Assume linear if swap and not inverse
            ...
            if contract_size is None or not contract_size.is_finite() or contract_size <= 0:
                 ...
                 self.logger.error(f"Invalid or missing contract size ({contract_size}) for {self.symbol}. Cannot calculate size.")
                 return None
            ...
            if is_inverse:
                 ...
                 self.logger.debug(f"Calculating size for {calc_type} contract (ContractSize: {contract_size}).")
            ...
            self.logger.debug(f"Calculated raw position size: {position_size_base:.{DECIMAL_DISPLAY_PRECISION+4}f} {base_asset}") # Log with more precision
            ...
            try:
                ...
                quantized_size = self.quantize_value(position_size_base, 'amount', market, rounding_mode=ROUND_DOWN)
                if quantized_size is None:
                    self.logger.error("Failed to quantize calculated position size based on step size.")
                    return None
                if quantized_size <= 0:
                     self.logger.warning(f"Calculated size {position_size_base} became zero or negative ({quantized_size}) after quantization/rounding down. Risk amount might be too small for minimum step size.")
                     return None
                self.logger.debug(f"Position size after step-size quantization (ROUND_DOWN): {quantized_size} {base_asset}")

                # 2. Check against minimum amount AFTER quantization
                if min_amount_dec is not None and quantized_size < min_amount_dec:
                    self.logger.warning(f"Quantized position size {quantized_size} {base_asset} is below minimum order size {min_amount_dec} {base_asset}. Cannot open position with this risk setup.")
                    return None
                ...
                if max_amount_dec is not None and final_size_base > max_amount_dec:
                    self.logger.warning(f"Quantized position size {final_size_base} exceeds max limit {max_amount_dec}. Capping size to max limit.")
                    # Cap the size to the maximum allowed, then re-quantize it to ensure it adheres to step size
                    capped_size = max_amount_dec
                    final_size_base = self.quantize_value(capped_size, 'amount', market, rounding_mode=ROUND_DOWN)
                    if final_size_base is None or final_size_base <= 0:
                         self.logger.error("Failed to quantize the capped maximum position size or result was non-positive. Cannot proceed.")
                         return None
                    self.logger.info(f"Position size capped and re-quantized to: {final_size_base} {base_asset}")
    ...
    def quantize_price(self, price: Decimal, side_for_conservative_rounding: Optional[PositionSide] = None) -> Optional[Decimal]:
         ...
         if side_for_conservative_rounding is not None:
              ...
              if side_for_conservative_rounding == PositionSide.LONG:
                   ...
                   rounding_mode = ROUND_UP # Round Short SL up (ROUND_CEILING might also work)

         # Use the Decimal quantization helper
         quantized_price = self.quantize_value(price, 'price', market, rounding_mode=rounding_mode)
         ...
         if side_for_conservative_rounding:
              self.logger.debug(f"Quantized price {price} to {quantized_price} using {rounding_mode} for {side_for_conservative_rounding.name} side.")
         else:
              self.logger.debug(f"Quantized price {price} to {quantized_price} using default rounding.")
    ...
    def calculate_stop_loss(self, entry_price: Decimal, side: PositionSide, latest_indicators: pd.Series) -> Optional[Decimal]:
        ...
        with getcontext() as ctx:
            ...
            if sl_method == "atr":
                ...
                if atr_col not in latest_indicators or pd.isna(latest_indicators[atr_col]):
                     self.logger.error(f"ATR column '{atr_col}' not found or is NaN in latest indicators.")
                     return None

                atr_value = latest_indicators[atr_col] # Should be Decimal from indicator calculation
                ...
                else: self.logger.error("Invalid position side for SL calculation."); return None
                self.logger.debug(f"Calculated SL based on ATR: Entry={entry_price}, Side={side.name}, ATR={atr_value}, Multiplier={atr_multiplier}, Distance={stop_distance}, Raw SL={stop_loss_price_raw}")
                ...
                self.logger.debug(f"Calculated SL based on Fixed Percent: Entry={entry_price}, Side={side.name}, Percent={fixed_percent*100}%, Raw SL={stop_loss_price_raw}")
        ...
        if stop_loss_price_raw is None or not stop_loss_price_raw.is_finite() or stop_loss_price_raw <= 0:
             self.logger.error(f"Calculated raw stop loss price ({stop_loss_price_raw}) is invalid. Cannot set SL.")
             return None
        ...
        if quantized_entry is None:
             self.logger.warning("Could not quantize entry price for final SL validation. Skipping check.")
        else:
            if side == PositionSide.LONG and quantized_sl >= quantized_entry:
                 self.logger.error(f"Quantized SL {quantized_sl} is >= quantized entry price {quantized_entry}. Invalid SL. Check ATR/percentage values or market tick size.")
    ...
    def check_ma_cross_exit(self, indicators_df: pd.DataFrame, position_side: PositionSide) -> bool:
        ...
        if len(indicators_df) < 2:
             self.logger.debug("Not enough data points (< 2) to check for MA cross.")
             return False
        ...
        try:
            ...
            self.logger.warning("Could not get latest and previous indicator data for MA cross check.")
            return False


        if not all(c in latest_data and c in prev_data for c in [ema_short_col, ema_long_col]):
            self.logger.warning(f"Cannot check MA cross exit: EMA columns missing from latest/previous indicators.")
            return False
        ...
        valid_emas = all(isinstance(e, Decimal) and e.is_finite() for e in [ema_short_now, ema_long_now, ema_short_prev, ema_long_prev])
        if not valid_emas:
            self.logger.warning(f"Invalid or non-finite EMA values for MA cross exit check.")
            return False
    ...
    def manage_stop_loss(self, position: Dict[str, Any], latest_indicators: pd.Series, current_state: Dict[str, Any]) -> Optional[Decimal]:
        ...
        if not isinstance(entry_price, Decimal) or not entry_price.is_finite():
            self.logger.warning(f"Invalid entry price ({entry_price}) for SL management.")
            return None
        if not isinstance(current_sl_state, Decimal) or not current_sl_state.is_finite():
            self.logger.warning(f"Invalid current SL price in state ({current_sl_state}) for SL management.")
            return None
        if not isinstance(mark_price, Decimal) or not mark_price.is_finite():
            self.logger.warning(f"Invalid mark price ({mark_price}) for SL management.")
            return None
        if entry_price <= 0 or mark_price <= 0 or current_sl_state <= 0:
             self.logger.warning(f"Entry ({entry_price}), mark ({mark_price}), or SL ({current_sl_state}) is non-positive. Cannot manage SL.")
             return None
        ...
        with getcontext() as ctx:
            ...
            if atr_col and atr_col in latest_indicators and pd.notna(latest_indicators[atr_col]):
                 ...
                 else: self.logger.warning(f"ATR value ({val}) is invalid/non-positive.")
            ...
            if use_be and atr_value is not None and not current_state.get('break_even_achieved', False):
                ...
                if position_side == PositionSide.LONG: target_be_price_raw = entry_price + offset_distance
                elif position_side == PositionSide.SHORT: target_be_price_raw = entry_price - offset_distance
                ...
                if quantized_be_price is not None:
                    ...
                    be_triggered = False
                    if position_side == PositionSide.LONG and mark_price >= (entry_price + profit_target_distance): be_triggered = True
                    elif position_side == PositionSide.SHORT and mark_price <= (entry_price - profit_target_distance): be_triggered = True
                    ...
                    if be_triggered:
                        ...
                        else: self.logger.debug(f"BE triggered, but proposed BE price {quantized_be_price} not better than current SL {current_sl_state}.")
                else: self.logger.warning("Failed to quantize break-even price. Skipping BE check.")
            ...
            if use_tsl and atr_value is not None and not state_updated_this_cycle:
                 ...
                 if position_side == PositionSide.LONG: potential_tsl_price_raw = mark_price - trail_distance
                 elif position_side == PositionSide.SHORT: potential_tsl_price_raw = mark_price + trail_distance

                 if potential_tsl_price_raw is not None:
                     # Quantize potential TSL price (conservatively)
                     quantized_tsl_price = self.quantize_price(potential_tsl_price_raw, side_for_conservative_rounding=position_side)
                             ...
                             if current_state.get('break_even_achieved', False):
                                  ...
                                  if quantized_entry is not None:
                                       if position_side == PositionSide.LONG and quantized_tsl_price < quantized_entry:
                                            self.logger.warning(f"TSL {quantized_tsl_price} below entry {quantized_entry} after BE. Clamping SL to entry.")
                                            quantized_tsl_price = quantized_entry
                                       elif position_side == PositionSide.SHORT and quantized_tsl_price > quantized_entry:
                                            self.logger.warning(f"TSL {quantized_tsl_price} above entry {quantized_entry} after BE. Clamping SL to entry.")
                                            quantized_tsl_price = quantized_entry
                                  else: self.logger.warning("Could not quantize entry for TSL vs BE check.")
                     ...
                     else: self.logger.warning(f"Failed to quantize potential TSL price ({potential_tsl_price_raw}). Skipping TSL update.")
        ...
        if new_sl_price_proposal is not None:
            ...
            if not new_sl_price_proposal.is_finite() or new_sl_price_proposal <= 0:
                 self.logger.warning(f"Proposed new SL ({new_sl_price_proposal}) is invalid. Ignoring update.")
                 return None
            ...
            if position_side == PositionSide.LONG and new_sl_price_proposal > (mark_price + tolerance):
                 self.logger.warning(f"Proposed new SL {new_sl_price_proposal} is significantly > current mark price {mark_price}. Invalid SL.")
                 sl_invalid = True
            if position_side == PositionSide.SHORT and new_sl_price_proposal < (mark_price - tolerance):
                 self.logger.warning(f"Proposed new SL {new_sl_price_proposal} is significantly < current mark price {mark_price}. Invalid SL.")
                 sl_invalid = True

            if sl_invalid: return None
            ...
            if abs(new_sl_price_proposal - current_sl_state) > tolerance:
                 ...
                 self.logger.debug(f"Calculated new {proposed_sl_type} SL {new_sl_price_proposal} not significantly different from current {current_sl_state}. No update needed.")
...
class TradingBot:
    ...
    def _apply_log_level_from_config(self):
        ...
        if isinstance(log_level_enum, int):
             if self.logger.level != log_level_enum:
                  ...
                  for handler in self.logger.handlers: handler.setLevel(LOG_LEVEL)
    ...
    def _initialize_default_state(self):
        ...
        self.state.setdefault('active_position', None) # Stores dict of the current position
        self.state.setdefault('stop_loss_price', None) # Stores Decimal SL price
        self.state.setdefault('take_profit_price', None) # Stores Decimal TP price
        self.state.setdefault('break_even_achieved', False) # Flag for BE SL activation
        self.state.setdefault('last_order_id', None) # ID of the last order placed
    ...
    def _load_trading_parameters(self):
        ...
        if not all([self.symbol, self.timeframe, self.leverage, self.quote_asset, self.category]):
             self.logger.critical("Essential trading settings missing after loading config. Exiting.")
             sys.exit(1)

    def run(self):
        """Starts the main trading loop."""
        self.logger.info(f"--- Starting Trading Bot ---")
        self.logger.info(f"Symbol: {self.symbol}, Timeframe: {self.timeframe}, Category: {self.category}, Quote: {self.quote_asset}")
        self.logger.info(f"Leverage: {self.leverage}x, Hedge Mode: {self.hedge_mode}, Poll Interval: {self.poll_interval}s")

        if not self.initialize_exchange_settings():
             self.logger.critical("Failed to initialize exchange settings (e.g., leverage/mode). Exiting.")
             sys.exit(1)
        ...
        while self.is_running:
            try:
                ...
                if indicators_df is None or indicators_df.empty:
                    ...
                    continue
                try:
                     if indicators_df.iloc[-1].name < (pd.Timestamp.utcnow() - pd.Timedelta(minutes=15)): # Check staleness
                          self.logger.warning(f"Latest indicator data timestamp ({indicators_df.iloc[-1].name}) seems stale. Skipping cycle.")
                          self._wait_for_next_cycle(start_time)
                          continue
                     latest_indicators = indicators_df.iloc[-1] # Series of latest indicators
                except IndexError:
                     self.logger.warning("Indicators DataFrame empty after calculation. Skipping cycle.")
                     self._wait_for_next_cycle(start_time)
                     continue
                ...
                if self.state.get('active_position'):
                    ...
                    self.manage_existing_position(indicators_df, signal, current_position)
                else:
                    # No active position according to state, check for entry signals
                    self.attempt_new_entry(signal, latest_indicators, signal_details) # Pass latest indicators Series
                ...
                # self.perform_shutdown_cleanup() # Optional: close positions, cancel orders
                 ...
                 self.logger.error(f"Network error in main loop: {e}. Bot will retry after poll interval.")
                 time.sleep(self.poll_interval) # Use standard wait after network errors
            except Exception as e:
                self.logger.critical(f"An critical unexpected error occurred in the main loop: {e}", exc_info=True)
                # Decide whether to stop or continue based on severity
                self.logger.info("Attempting to continue after unexpected error, but manual check advised.")
    ...
    def initialize_exchange_settings(self) -> bool:
        ...
        if self.category in ['linear', 'inverse']:
             if not self.exchange.set_leverage(self.symbol, self.leverage):
                  self.logger.error(f"Failed to set initial leverage to {self.leverage}x for {self.symbol}.")
                  success = False # Mark as failure but allow continuing if possible
             else:
                  self.logger.info(f"Leverage set request to {self.leverage}x for {self.symbol} sent.")
        else:
        ...
        if self.exchange.exchange_id == 'bybit' and self.category in ['linear', 'inverse']:
             try:
                  ...
                  self.logger.info(f"Checking/Setting position mode for {self.symbol} (Target: {mode_name})...")

                  # Bybit V5 endpoint: POST /v5/position/switch-mode
                  # Needs category, symbol, mode
                  params = {'symbol': self.symbol, 'mode': target_mode} # category added by safe_ccxt_call
                  ...
                  if hasattr(self.exchange.exchange, method_name):
                       ...
                       if result and result.get('retCode') == 0:
                            self.logger.info(f"Position mode successfully set to {mode_name}.")
                       elif result and result.get('retCode') == 110025: # 110025: Position mode is not modified
                            self.logger.info(f"Position mode is already set correctly ({mode_name}).")
                       ...
                       self.logger.warning(f"CCXT method '{method_name}' not found. Cannot verify/set position mode automatically.")
             ...
             self.logger.debug("Skipping position mode setting (not Bybit derivatives).")
    ...
    def get_current_position(self) -> Optional[Dict[str, Any]]:
        ...
        """
        positions = self.exchange.fetch_positions(self.symbol) # Uses wrapper method
        ...
        active_positions_for_symbol = positions # Already filtered by symbol in fetch_positions
        ...
        if self.hedge_mode:
            ...
            if state_pos:
                 ...
                 target_side_enum_val = state_pos.get('side', PositionSide.NONE).value # Get 'long'/'short' string

                 if target_idx is None or target_side_enum_val == PositionSide.NONE.value:
                      self.logger.error("Hedge mode active, but bot state is inconsistent (missing idx or side). Cannot identify position.")
                 ...
                 for p in active_positions_for_symbol:
                      ...
                      p_side_str = p.get('side') # 'long'/'short' string from processed position
                      if p_idx == target_idx and p_side_str == target_side_enum_val:
                           found_match = p
                           break
                      elif p_idx == target_idx:
                           self.logger.warning(f"Hedge mode position found with matching Idx ({target_idx}) but mismatched Side (State: {target_side_enum_val}, Exchange: {p_side_str}). Ignoring.")

                 if found_match:
                      self.logger.info(f"Found active hedge position matching state: Side {found_match.get('side')}, Idx {found_match.get('positionIdx')}, Size {found_match.get('contracts')}")
                      return found_match
                 else:
                      self.logger.warning(f"Bot state indicates hedge position (Idx: {target_idx}, Side: {target_side_enum_val}), but no matching active position found. State may be stale.")
                      return None # Let sync handle clearing state
            else:
                 # Bot state has no position, but found active position(s) on exchange. Ignore them.
                 pos_details = [f"Idx:{p.get('positionIdx')}, Side:{p.get('side')}, Size:{p.get('contracts')}" for p in active_positions_for_symbol]
                 self.logger.info(f"Ignoring {len(active_positions_for_symbol)} active hedge position(s) found on exchange as bot state is empty: [{'; '.join(pos_details)}]")
                 return None

        else: # Non-hedge (One-way) mode
            if len(active_positions_for_symbol) > 1:
                 self.logger.error(f"CRITICAL: Found {len(active_positions_for_symbol)} active positions for {self.symbol} in non-hedge mode. Check exchange state manually!")
            ...
            if pos.get('positionIdx') != 0:
                 self.logger.warning(f"One-way mode active, but found position with non-zero index ({pos.get('positionIdx')}). Check exchange mode setting.")
    ...
    def sync_bot_state_with_position(self, current_position_on_exchange: Optional[Dict[str, Any]]):
        ...
        if current_position_on_exchange:
            ...
            exchange_pos_side_str = current_position_on_exchange.get('side') # 'long'/'short'/'none'
            ...
            exchange_sl_dec = current_position_on_exchange.get('info', {}).get('stopLoss') # Decimal or NaN
            ...
            if not all([exchange_pos_symbol, exchange_pos_side_str != PositionSide.NONE.value,
                 ...
                 self.logger.error("Fetched exchange position data is incomplete or side is 'none'. Cannot sync state reliably.")
                 if bot_thinks_has_position:
                      self.logger.warning("Clearing potentially stale bot state due to incomplete/invalid exchange data.")
                      self._clear_position_state("Incomplete/invalid exchange position data")
                      state_changed = True
                 return
            ...
            if exchange_sl_dec is not None and (not exchange_sl_dec.is_finite() or exchange_sl_dec <= 0):
                 self.logger.debug(f"Ignoring invalid SL value ({exchange_sl_dec}) from exchange position info.")
            ...
            if not bot_thinks_has_position:
                ...
                if self.hedge_mode and exchange_pos_idx in [1, 2]: position_matches_mode = True
                elif not self.hedge_mode and exchange_pos_idx == 0: position_matches_mode = True

                if position_matches_mode:
                     self.logger.warning(f"Found unexpected active position matching bot mode ({exchange_pos_side_str}, Idx:{exchange_pos_idx}). Adopting into state.")
                     try: adopted_side = PositionSide(exchange_pos_side_str)
                     except ValueError: self.logger.error(f"Invalid side '{exchange_pos_side_str}'. Cannot adopt."); return
                     ...
                     self.state['stop_loss_price'] = exchange_sl_dec # Adopt SL if found
                     self.state['take_profit_price'] = None # Assume no TP initially
                     self.state['break_even_achieved'] = False # Reset BE
                     state_changed = True
                     if exchange_sl_dec: self.logger.info(f"Adopted SL {exchange_sl_dec} found on exchange.")
                     else: self.logger.warning("No valid SL found on adopted exchange position.")
                else:
                     self.logger.warning(f"Found active position ({exchange_pos_side_str}, Idx:{exchange_pos_idx}) NOT matching bot mode ({'Hedge' if self.hedge_mode else 'One-way'}). Ignoring.")
                ...
                if exchange_pos_symbol == state_pos['symbol'] and exchange_pos_side_str == state_side_enum.value:
                    if self.hedge_mode and exchange_pos_idx == state_idx: match = True
                    elif not self.hedge_mode and exchange_pos_idx == 0: match = True

                if match:
                     # Position matches state, update dynamic values
                     self.logger.debug(f"Exchange position matches state. Syncing details...")
                     if state_pos['size'] != exchange_pos_size:
                          self.logger.info(f"Position size changed: State={state_pos['size']}, Exchange={exchange_pos_size}. Updating state.")
                          state_pos['size'] = exchange_pos_size; state_changed = True
                     if state_pos['entry_price'] != exchange_pos_entry:
                          self.logger.info(f"Position entry price changed: State={state_pos['entry_price']}, Exchange={exchange_pos_entry}. Updating state.")
                          state_pos['entry_price'] = exchange_pos_entry; state_changed = True
                     ...
                     if exchange_sl_dec is not None: # Exchange reports an SL
                          sl_differs = current_state_sl is None or abs(exchange_sl_dec - current_state_sl) > tolerance
                          if sl_differs:
                               self.logger.info(f"Updating state SL from {current_state_sl} to match exchange SL {exchange_sl_dec}.")
                               self.state['stop_loss_price'] = exchange_sl_dec; state_changed = True
                               # Check if this update implies BE was achieved
                               self._check_and_update_be_status(state_pos, exchange_sl_dec) # May set state_changed=True
                     else: # Exchange reports NO SL
                          if current_state_sl is not None:
                               self.logger.warning(f"Bot state has SL {current_state_sl}, but no active SL found on exchange. Clearing state SL.")
                               self.state['stop_loss_price'] = None; state_changed = True
                               if self.state.get('break_even_achieved', False):
                                    self.logger.info("Resetting break_even_achieved flag as SL disappeared.")
                                    self.state['break_even_achieved'] = False # Already marked state_changed

                     # Update last sync time if state matched
                     self.state['last_sync_time'] = time.time() # Don't mark state_changed for this timestamp update

                else:
                     # Position exists, but doesn't match state. Clear bot state.
                     self.logger.warning(f"Found active position ({exchange_pos_symbol},{exchange_pos_side_str},Idx:{exchange_pos_idx}) NOT matching bot state ({state_pos.get('symbol')},{state_side_enum.value},Idx:{state_idx}). Clearing bot state.")
            ...
            if bot_thinks_has_position:
                ...
                self.logger.info(f"Position {pos_details} no longer found on exchange. Clearing bot state.")
    ...
    def _check_and_update_be_status(self, position_state: Dict[str, Any], current_sl: Decimal):
        ...
        if not self.state.get('break_even_achieved', False): # Only update if not already achieved
            ...
            be_achieved = False
            if side == PositionSide.LONG and current_sl >= quantized_entry: be_achieved = True
            elif side == PositionSide.SHORT and current_sl <= quantized_entry: be_achieved = True

            if be_achieved:
                 self.logger.info(f"Marking Break-Even as achieved based on SL {current_sl} vs Entry {quantized_entry}.")
    ...
    def manage_existing_position(self, indicators_df: pd.DataFrame, signal: Signal, live_position_data: Optional[Dict[str, Any]]):
        ...
        if not position_state:
             self.logger.warning("manage_existing_position called but state has no active position. Skipping.")
             return
        if not live_position_data:
             # Sync should ideally clear state if position disappears, but handle defensively
             self.logger.error("manage_existing_position called without live position data. Sync likely failed or position closed unexpectedly. Skipping management.")
             return
        ...
        self.logger.info(f"Managing existing {position_side.name} position (Idx: {position_idx if self.hedge_mode else 'N/A'})...")
        ...
        if self.exit_on_opposing_signal:
            if position_side == PositionSide.LONG and signal in [Signal.SELL, Signal.STRONG_SELL]: should_exit_on_signal = True
            elif position_side == PositionSide.SHORT and signal in [Signal.BUY, Signal.STRONG_BUY]: should_exit_on_signal = True
        ...
        if current_sl_in_state is None:
             self.logger.warning("Cannot manage SL: Stop loss price missing from state. Consider manual check or adding logic to set initial SL if missing.")
        else:
             # Pass live position data (already has Decimals) and current state to manager
             latest_indicators = indicators_df.iloc[-1] # Pass only latest row for SL management
             ...
             if new_sl_price is not None:
                 ...
                 if self.hedge_mode: protection_params['position_idx'] = position_idx
                 ...
                 if self.exchange.set_protection(self.symbol, **protection_params):
                     ...
                     save_state(self.state, self.state_path, self.logger) # Save state immediately
                 else:
                     self.logger.error(f"Failed to update stop loss on exchange to {new_sl_price}. API call failed or returned error. State SL remains {current_sl_in_state}.")
    ...
    def attempt_new_entry(self, signal: Signal, latest_indicators: pd.Series, signal_details: Dict[str, Any]):
        ...
        """
        if self.state.get('active_position'):
             self.logger.debug("Skipping entry attempt: Position already active.")
             return
        ...
        if signal in [Signal.BUY, Signal.STRONG_BUY]: entry_signal=True; target_side=PositionSide.LONG; order_side=OrderSide.BUY
        elif signal in [Signal.SELL, Signal.STRONG_SELL]: entry_signal=True; target_side=PositionSide.SHORT; order_side=OrderSide.SELL
        ...
        if not isinstance(entry_price_estimate, Decimal) or not entry_price_estimate.is_finite() or entry_price_estimate <= 0:
             self.logger.warning("Using ticker last price for entry estimate as close price is invalid.")
             ticker = self.exchange.safe_ccxt_call('fetch_ticker', self.symbol)
             if ticker and ticker.get('last'):
                 try: entry_price_estimate = Decimal(str(ticker['last']))
                 except: entry_price_estimate = None # Reset if conversion fails
             if not isinstance(entry_price_estimate, Decimal) or not entry_price_estimate.is_finite() or entry_price_estimate <= 0:
                  self.logger.error("Failed to get valid entry price estimate from close or ticker. Cannot enter.")
                  return
        ...
        if not balance_info or self.quote_asset not in balance_info:
             self.logger.error(f"Failed to fetch balance or quote asset '{self.quote_asset}' missing. Cannot size position.")
             return
        available_equity = balance_info[self.quote_asset]
        if not isinstance(available_equity, Decimal) or not available_equity.is_finite() or available_equity <= 0:
             self.logger.error(f"Invalid available equity {available_equity}. Cannot size position.")
             return
        ...
        if position_size is None or position_size <= 0:
            self.logger.warning("Position size calculation failed or resulted in non-positive size. No entry.")
            return

        # --- Place Entry Order (Market Order with SL attached if possible) ---
        order_type = 'market'
        self.logger.info(f"Attempting {order_side.value} {order_type} order: Size={position_size}, Est.Entry={entry_price_estimate}, Calc.SL={stop_loss_price}")
        ...
        if order_result and order_result.get('id'):
            ...
            self.logger.info(f"Entry order placed successfully. ID: {order_id}, Status: {order_result.get('status', 'unknown')}. Intended SL: {stop_loss_price}")
            ...
            self.state['stop_loss_price'] = stop_loss_price # Store intended SL
            self.state['take_profit_price'] = None
            self.state['break_even_achieved'] = False
            save_state(self.state, self.state_path, self.logger) # Save immediately
            self.logger.info(f"Bot state updated optimistically for new {target_side.name} position.")

            # --- Verification Step ---
            if self.post_order_verify_delay > 0:
                 self.logger.info(f"Waiting {self.post_order_verify_delay}s before verifying entry...")
                 ...
                 if self.state.get('active_position'):
                      ...
                      if state_sl is not None:
                           ...
                           if abs(state_sl - stop_loss_price) < tolerance:
                                ...
                                self.logger.warning(f"State SL ({state_sl}) differs from intended SL ({stop_loss_price}) after sync. Using state value.")
                      else:
                           # State SL is None after sync, meaning exchange didn't apply it or it got cancelled?
                           self.logger.warning("Stop loss NOT found on exchange after placing order with SL param. Attempting to set SL again.")
                           protection_params = {'stop_loss': stop_loss_price}
                           if self.hedge_mode: protection_params['position_idx'] = self.state['active_position'].get('position_idx')
                           if self.exchange.set_protection(self.symbol, **protection_params):
                                self.logger.info("Successfully set stop loss via set_protection after initial attempt failed/missing.")
                                self.state['stop_loss_price'] = stop_loss_price # Update state SL after explicit set
                                save_state(self.state, self.state_path, self.logger) # Save again
                           else:
                                self.logger.error("Retry failed to set stop loss after entry.")
                 else:
                      self.logger.error("Position not found during verification after placing entry order. State cleared by sync.")
            else:
    ...
    def close_position(self, reason: str):
        ...
        if not isinstance(size_to_close, Decimal) or size_to_close <= 0:
             self.logger.error(f"Cannot close: Invalid size {size_to_close} in state. Clearing state.")
             self._clear_position_state(f"Invalid size in state during close"); save_state(self.state, self.state_path, self.logger)
             return
        if current_side is None or current_side == PositionSide.NONE:
             self.logger.error(f"Cannot close: Invalid side {current_side} in state. Clearing state.")
             self._clear_position_state(f"Invalid side in state during close"); save_state(self.state, self.state_path, self.logger)
             return
        if self.hedge_mode and position_idx is None:
             self.logger.error(f"Cannot close hedge mode: positionIdx missing. Clearing state.")
             self._clear_position_state("Missing hedge idx in state during close"); save_state(self.state, self.state_path, self.logger)
             return

        close_side = OrderSide.SELL if current_side == PositionSide.LONG else OrderSide.BUY
        order_type = 'market'

        self.logger.info(f"Attempting to close {current_side.name} position (Idx:{position_idx if self.hedge_mode else 'N/A'}, Size:{size_to_close}) via {order_type} order. Reason: {reason}")

        # Parameters for closing order
        close_params = { 'reduceOnly': True }
        if self.hedge_mode: close_params['positionIdx'] = position_idx
        else: close_params['positionIdx'] = 0
        ...
        if order_result and order_result.get('id'):
            order_id = order_result['id']
            status = order_result.get('status', 'unknown')
            self.logger.info(f"Position close order placed successfully. ID: {order_id}, Status: {status}. Reason: {reason}")
            ...
            self.logger.error(f"Failed to place position close order. Reason: {reason}. Position state remains unchanged.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Decimal context set: Precision={getcontext().prec}, Rounding={getcontext().rounding}")
    except Exception as e:
...
1.  **Decimal Precision:** Introduced separate `DECIMAL_DISPLAY_PRECISION` and `CALCULATION_PRECISION`. Calculations use higher precision, preventing intermediate rounding errors. Context is set globally at startup.
2.  **Robust JSON (De)Serialization:** `decimal_decoder` now correctly handles 'NaN', 'Infinity', '-Infinity' strings during loading.
3.  **Configuration Validation (`validate_config`):** Significantly expanded to check types, ranges, required keys, logical constraints (e.g., EMA periods, threshold order), and specific formats for many parameters across all sections.
4.  **`safe_ccxt_call`:**
    *   Implements a robust retry loop.
    *   Distinguishes more clearly between retryable (`NetworkError`, `RateLimitExceeded`, specific `ExchangeError` codes) and non-retryable errors (`AuthenticationError`, `InvalidOrder`, `InsufficientFunds`, `BadRequest`, etc.).
    *   Logs detailed error messages, including the method name and potentially args/kwargs for debugging.
    *   Improved Bybit V5 `category` injection logic.
    *   Handles specific non-error outcomes like `MarginModeAlreadySet`.
5.  **Exchange Wrapper Methods:**
    *   `load_markets_with_retry`: Handles market loading failures robustly.
    *   `get_market`: Ensures precision and `contractSize` (as Decimal) are loaded, retrying if necessary.
    *   `fetch_balance`, `fetch_positions`: Improved parsing for Bybit V5 structures (`info` field), fallback to standard CCXT, consistent Decimal conversion. `fetch_positions` now filters non-zero size more reliably and handles `positionIdx` and `side` determination better.
    *   `format_value_for_api`: Clarified usage with `amount_to_precision`/`price_to_precision`. Added rounding mode parameter hint (though CCXT methods might have fixed rounding).
    *   `quantize_value`: New helper using pure Decimal math for accurate quantization based on tick/step size, allowing explicit rounding control.
    *   `create_order`, `set_protection`: Use `quantize_value` for internal calculations/validation and `format_value_for_api` for the final string conversion to the API. Improved parameter handling for Bybit V5 (SL/TP attachment, `positionIdx`).
    *   `set_leverage`: Checks Bybit V5 response codes for confirmation.
6.  **Trading Analyzer:**
    *   `_initialize_weights`: Validates and normalizes weights robustly, handling zero or invalid weights.
    *   `calculate_indicators`: Handles `Decimal` <-> `float` conversion carefully, manages NaNs, merges results back correctly, uses specific `pandas_ta` column names.
    *   `generate_signal`: Uses high precision context, handles non-finite indicator values gracefully, validates threshold order.
7.  **Position Manager:**
    *   `calculate_position_size`: Correctly handles linear/inverse/spot logic using `contractSize`, uses `quantize_value` for step-size rounding (ROUND\_DOWN), checks min/max limits accurately *after* quantization.
    *   `quantize_price`: Uses the new `quantize_value` helper with optional conservative rounding logic.
    *   `calculate_stop_loss`: Uses high precision, `quantize_price` for conservative rounding, and adds final validation against quantized entry price.
    *   `check_ma_cross_exit`: Uses previous candle data for true cross detection.
    *   `manage_stop_loss`: Implements BE/TSL using ATR, uses `quantize_price` conservatively, validates proposed SL against mark price and entry (after BE), prevents TSL overriding BE immediately.
8.  **Main Bot Class (`TradingBot`):**
    *   **State Synchronization (`sync_bot_state_with_position`):** This is a major addition. It's called every cycle *before* decisions are made to reconcile the bot's internal state (`self.state`) with the actual position reported by the exchange. It handles:
        *   Adopting unexpected positions found on the exchange (if matching mode).
        *   Updating state size, entry price, and SL based on exchange data.
        *   Clearing state if the exchange position doesn't match the bot's expectation (wrong side/idx).
        *   Clearing state if the position disappears from the exchange.
    *   **Initialization:** Sets log level from config, initializes default state structure, loads trading parameters, initializes exchange settings (leverage, position mode) with error handling.
    *   **Main Loop:** Follows Fetch -> Analyze -> Fetch Position -> **Sync State** -> Decide (Manage/Enter) -> Save State -> Wait sequence.
    *   **`manage_existing_position`:** Uses the *synced state* and live data, calls exit checks, calls `manage_stop_loss`, updates state *only* on successful API calls (`set_protection`).
    *   **`attempt_new_entry`:** Performs pre-calculation, places order (with SL attached if possible), updates state *optimistically*, includes an optional verification step with re-sync and potential SL retry.
    *   **`close_position`:** Uses `reduceOnly`, handles hedge mode `positionIdx`, clears state *optimistically*.
    *   **Error Handling:** More robust handling in the main loop for critical errors (`AuthenticationError`, `SystemExit`) vs. potentially recoverable ones (`NetworkError`).
9.  **Code Quality:** Improved comments, docstrings, type hints, variable names, and overall structure. Uses `pathlib`. Handles `KeyboardInterrupt` more gracefully.

This enhanced version should be significantly more resilient, reliable, and easier to debug. Remember to create the corresponding `config.json` file with all the necessary parameters validated by `validate_config`.
