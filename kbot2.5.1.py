...
def get_current_position(symbol: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    for pos in positions_data:
        ...
        if side in pos_dict and contracts_str is not None:
            try:
                ...
                # Mark side as found
                if side == "long": long_pos_found = True
                else: short_pos_found = True
    ...
    if not long_pos_found and not short_pos_found:
         ...
         logger.warning(Fore.YELLOW + f"Both LONG and SHORT positions found for {symbol}. Pyrmethus assumes one-way mode and will manage the first position found for each side. Please ensure your exchange account is configured for one-way trading.")
...
def print_status_panel(
    ...
    if not stoch_k.is_nan():
         if stoch_k < CONFIG.stoch_oversold_threshold: stoch_color = Fore.GREEN; stoch_desc = f"{stoch_color}Oversold (<{CONFIG.stoch_oversold_threshold})"
         elif stoch_k > CONFIG.stoch_overbought_threshold: stoch_color = Fore.RED; stoch_desc = f"{stoch_color}Overbought (>{CONFIG.stoch_overbought_threshold})"
         else: stoch_color = Fore.YELLOW; stoch_desc = f"{stoch_color}Neutral ({CONFIG.stoch_oversold_threshold}-{CONFIG.stoch_overbought_threshold})"
    ...
    if not fast_ema.is_nan() and not slow_ema.is_nan():
        if fast_ema > slow_ema: ema_cross_color = Fore.GREEN; ema_desc = f"{ema_cross_color}Bullish" # Changed from Cross to just Bullish as it's current state, not a historical event
        elif fast_ema < slow_ema: ema_cross_color = Fore.RED; ema_desc = f"{ema_cross_color}Bearish" # Changed
    ...
    status_data = [
        ...
        [section_color + f"EMA Fast/Slow ({CONFIG.fast_ema_period}/{CONFIG.slow_ema_period})", f"{ema_cross_color}{fast_ema_str} / {slow_ema_str}", ema_desc], # Display EMA periods from CONFIG
    ...
    try:
        ...
        if potential_long:
            if CONFIG.trade_only_with_trend:
             ...
             reason_parts = []
             # EMA Check
             if ema_bullish_cross: reason_parts.append(f"EMA Bullish ({CONFIG.fast_ema_period}/{CONFIG.slow_ema_period})")
             elif ema_bearish_cross: reason_parts.append(f"EMA Bearish ({CONFIG.fast_ema_period}/{CONFIG.slow_ema_period})")
             else: reason_parts.append(f"No EMA Cross ({CONFIG.fast_ema_period}/{CONFIG.slow_ema_period})")
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    current_atr = indicators.get('atr', Decimal('NaN')) if indicators else Decimal('NaN') # Use NaN default if indicators is None
    ...
    if can_trade_logic:
        ...
        if is_flat and signals.get("reason") != "Skipped due to missing data/insufficient candles" and indicators is not None and not current_atr.is_nan(): # Equity checked in can_trade_logic
            ...
            # If a trade was attempted, main loop sleep handles the pause.
             ...
             logger.warning("Skipping trade entry logic due to earlier critical data failure (positions/equity) or signal generation skipped.")
