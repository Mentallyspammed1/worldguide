    ...
    def _calculate_trade_parameters(
        ...
        try:
            ...
            if sl_distance_price <= 0:
                logger.error(f"Calculated SL distance ({sl_distance_price}) is invalid (<=0). Cannot calculate trade size.")
                return None
    ...
    def _set_position_protection(
        ...
        if (is_tsl and tsl_distance is not None and not tsl_distance.is_nan() and tsl_distance > 0
            ...
            if tsl_activation_price_str in ["NaN", "ERR"] or safe_decimal(tsl_activation_price_str) <= 0:
                 logger.error(f"Failed to format valid positive TSL activation price ({tsl_activation_price_str}). Cannot activate TSL.")
                 return False
    ...
    def manage_trailing_stop(
        ...
        try:
            ...
            if tsl_distance_price <= 0:
                 logger.warning(f"Calculated invalid TSL distance ({tsl_distance_price}) after min tick adjustment. Skipping TSL.")
                 return
    ...
    def print_status_panel(
        ...
        if indicators:
            ...
            def fmt_ind(key: str, prec: int = 1, default_prec: int = 1, commas: bool = False) -> Text:
                 ...
                 return self._format_decimal(
                    dec_val, precision=prec, default_precision=default_prec, add_commas=commas
                 )
        ...
        if active_position_data and position_side_str:
            ...
            if tsl_active_from_pos:
                 ...
                 if tracked_protection != "ACTIVE_TSL": prot_status_text.append("?", style="yellow") # Indicate potential mismatch
                 ...
                 if tracked_protection != "ACTIVE_SLTP": prot_status_text.append("?", style="yellow") # Indicate potential mismatch
    ...
    def graceful_shutdown(self) -> None:
        ...
        try:
            ...
            if isinstance(cancel_resp, dict) and cancel_resp.get('retCode') == V5_SUCCESS_RETCODE:
                 ...
                 logger.info(f"Cancel all orders command successful. Found {len(cancelled_list)} items in response list (may include non-order items).")
