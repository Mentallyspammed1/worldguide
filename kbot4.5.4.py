...
- Corrected retry logic in `fetch_with_retries`.
- Improved robustness of Decimal/float conversions for indicators.
- Ensured V5 API parameters (prices, distances) are passed as correctly formatted strings.
- Refined position verification logic after order placement/closure.
    ...
    **kwargs: Any,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Check for None result which might indicate an issue not raised as exception
            # Allow None explicitly if the function might return it normally (e.g., fetch_ohlcv on error)
            # This check is basic; specific functions might need tailored checks.
            # if result is None:
            #     logger.warning(f"{func_name} returned None on attempt {attempt + 1}. Retrying if possible.")
            #     # Treat None as a retryable issue for now, unless it's the last attempt
            #     if attempt < max_retries:
            #         time.sleep(delay_seconds)
            #         continue # Go to next attempt
            #     else:
            #         # If None on last attempt, raise a generic error or return None based on expected behavior
            #         raise RuntimeError(f"{func_name} returned None after {max_retries + 1} attempts.")
            ...
            if attempt < max_retries:
                ...
                logger.error(
                    f"{Fore.RED}Max retries ({max_retries + 1}) reached for {func_name} after retryable error. Last error: {e}"
                )
                # Loop will end, last_exception will be raised below
        except ccxt.AuthenticationError as e:
            logger.critical(
                f"{Fore.RED + Style.BRIGHT}Authentication error: {e}. Check API keys. Halting.",
                exc_info=False,  # Less verbose for auth failure
            )
            raise e  # Re-raise critical error immediately
    ...
    # If the loop finished without returning (i.e., all retries failed or a non-retryable error occurred)
    # raise the last captured exception.
    if last_exception:
        raise last_exception
    else:
        # This case should ideally not be reached if the loop logic is correct.
        # It might happen if the function returns None on the last attempt and wasn't caught above.
        # Or if max_retries is -1 (though the range would be empty).
        raise RuntimeError(
            f"Function {func_name} failed after {max_retries + 1} attempts without raising a specific exception."
        )
...
class ExchangeManager:
    ...
    def get_balance(self) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        try:
            ...
            # This fallback is potentially brittle and depends on CCXT's internal structure.
            if (
                        ...
                        if available_balance.is_nan() and "coin" in unified_acc_info:
                            ...
                            if coin_list and isinstance(coin_list, list):
                                ...
                                if settle_coin_info:
                                    ...
                                    # 'availableToWithdraw' seems most analogous to 'free' balance for the coin
                                    available_balance = safe_decimal(
    ...
    def get_current_position(
        ...
        try:
            ...
            params = {
                ...
                # "positionIdx": self.config.position_idx, # Pass index if API uses it for filtering (may not be needed if symbol is passed)
            ...
            for pos in fetched_positions_list:
                # CCXT standard structure might have symbol directly
                pos_symbol_ccxt = pos.get("symbol")
                # Fallback to info structure if needed
                info = pos.get("info", {})
                ...
                # Check if symbol matches (prefer info symbol if available) and index matches
                symbol_matches = (pos_symbol_info == market_id) or (
                    pos_symbol_ccxt == self.config.symbol
                )
                index_matches = pos_idx == self.config.position_idx

                if symbol_matches and index_matches:
                    # Prefer using the 'info' dict as it usually contains the most complete V5 data
            ...
            # If side is 'None' but size is non-zero, it indicates a hedge mode position where side isn't directly relevant
            # We infer the side based on the position index or context if needed, but for general state, just store details.
            # For one-way mode (positionIdx=0), side should be 'Buy' or 'Sell'.
            # For hedge mode (positionIdx=1 for Buy/Long, 2 for Sell/Short), side might be 'None'.
            ...
            if side == "buy":
                ...
                side == "none" and self.config.position_idx == 1
            ):  # Hedge mode buy side
                position_side_key = "long"
            elif (
                side == "none" and self.config.position_idx == 2
            ...
            if position_side_key:
                ...
                # This case might occur if size is non-zero but side/posIdx combination is unexpected
                logger.warning(
...
class IndicatorCalculator:
    ...
    def calculate_indicators(
        ...
        try:
            ...
            for col in required_cols:
                if not df_calc[
                    col
                ].empty:  # Check if column is not empty before accessing iloc[0]
                    first_val = df_calc[col].iloc[0]
                    if isinstance(first_val, Decimal):  # Check type of first element
                    ...
                    logger.warning(f"Column {col} is empty, skipping conversion.")
                    df_calc[col] = pd.Series(
                        dtype=float
                    )  # Ensure it's float type even if empty
    ...
    def _calculate_adx(
        ...
        # Also handle potential NaN in ATR series
        pdi_s_raw = np.where(
...
class SignalGenerator:
    ...
    def generate_signals(
        ...
        try:
            ...
            significant_move = True  # Default to True (pass)
            atr_reason = "ATR Filter OFF"
            if self.config.atr_move_filter_multiplier > 0:  # Filter is enabled
                if atr.is_nan() or atr <= 0:
                    atr_reason = f"ATR Filter Skipped (Invalid ATR: {atr})"
                    significant_move = False  # Cannot evaluate, block signal
            ...
            # else: Filter is disabled, significant_move remains True
...
class OrderManager:
    ...
    def __init__(self, config: TradingConfig, exchange_manager: ExchangeManager):
        ...
        if not exchange_manager or not exchange_manager.exchange:
            logger.critical(
                "OrderManager cannot initialize: Exchange instance missing from ExchangeManager."
            )
            # This is a critical failure, should ideally halt the bot.
            # Raising an exception here might be better than continuing with None.
    ...
    def _calculate_trade_parameters(
        ...
        try:
            ...
            # Ensure SL distance is positive before calculating risk per contract
            if sl_distance_price <= 0:
                logger.error(
                    f"Calculated SL distance ({sl_distance_price}) is invalid (<=0). Cannot calculate trade size."
                )
                return None
            ...
            if self.config.market_type == "inverse":
                ...
                # Use entry price for better accuracy if available, else current price
                value_per_point = (
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict]:
        ...
        try:
            ...
            # Check if fetch_with_retries returned None or an error occurred
            if order is None:
                logger.error(
    ...
    def _set_position_protection(
        ...
        """
        if not self.exchange:
            logger.error("Cannot set protection: Exchange instance is missing.")
            return False
        ...
        if (
            ...
            # Check if formatting resulted in valid positive numbers (or non-NaN/ERR strings)
            if (
                tsl_distance_str in ["NaN", "ERR"]
                or safe_decimal(tsl_distance_str) <= 0
            ):
                logger.error(
                    f"Failed to format valid positive TSL distance ({tsl_distance_str}). Cannot activate TSL."
                )
                return False
            if (
                tsl_activation_price_str in ["NaN", "ERR"]
                or safe_decimal(tsl_activation_price_str) <= 0
            ):
                logger.error(
                    f"Failed to format valid positive TSL activation price ({tsl_activation_price_str}). Cannot activate TSL."
                )
                return False
        ...
        # Only if not activating TSL and at least one valid SL or TP price is given
            ...
            if sl_price is not None and not sl_price.is_nan() and sl_price > 0:
                ...
                if (
                    sl_str in ["NaN", "ERR"] or safe_decimal(sl_str) <= 0
                ):  # Double check formatted value
                    logger.warning(
            ...
            if tp_price is not None and not tp_price.is_nan() and tp_price > 0:
                ...
                if (
                    tp_str in ["NaN", "ERR"] or safe_decimal(tp_str) <= 0
                ):  # Double check formatted value
                    logger.warning(
        ...
        try:
            ...
            # Check if a unified method exists and is preferred (requires testing)
            # if hasattr(self.exchange, 'set_trading_stop'):
            #     method_to_call = self.exchange.set_trading_stop
            # else:
    ...
    def place_risked_market_order(
        ...
        if side not in ["buy", "sell"]:
            ...
            return False
        # Ensure necessary inputs are valid before proceeding
        if atr.is_nan() or atr <= 0:
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
            ...
            return False
        if current_price.is_nan() or current_price <= 0:
            logger.error("Entry Aborted: Invalid Current Price provided.")
            return False
        ...
        elif self.config.enable_journaling and avg_entry_price.is_nan():
            logger.warning(
                "Logging entry to journal with N/A entry price as it was not available."
            )
            self.log_trade_entry_to_journal(side, filled_qty, Decimal("NaN"), order_id)
    ...
    def manage_trailing_stop(
        ...
        try:
            ...
            if tsl_distance_price < min_tick_size:
                ...
                tsl_distance_price = min_tick_size
            # Ensure distance is positive after adjustment
            if tsl_distance_price <= 0:
                logger.warning(
                    f"Calculated invalid TSL distance ({tsl_distance_price}) after min tick adjustment. Skipping TSL."
                )
                return
    ...
    def _write_journal_row(self, data: Dict[str, Any]):
        ...
        try:
            ...
            with file_path.open("a", newline="", encoding="utf-8") as csvfile:
                ...
                # Prepare row data, ensuring all fields exist and formatting Decimals/None
                row_to_write = {}
                for field in fieldnames:
                    value = data.get(field)
                    if isinstance(value, Decimal):
                        # Handle NaN explicitly before normalizing
                        if value.is_nan():
                            row_to_write[field] = "NaN"
                        else:
    ...
    def log_trade_entry_to_journal(
        ...
        avg_price: Decimal,  # Average fill price (can be NaN)
        ...
        data = {
            ...
            "Quantity": qty,
            "AvgPrice": avg_price,  # Pass Decimal (NaN handled by _write_journal_row)
            "OrderID": order_id,
    ...
    def print_status_panel(
        ...
        if indicators:
            ...
            def fmt_ind(
                ...
                # Handle potential None or non-Decimal values from indicators dict
                val = indicators.get(key)
                if isinstance(val, (Decimal, int, float)):
                    dec_val = safe_decimal(val)  # Ensure it's Decimal for formatting
                else:
                    dec_val = Decimal("NaN")  # Treat other types as NaN
        ...
        if active_position_data and position_side_str:
            ...
            # Protection Status (Combine tracker and position data for best view)
            tracked_protection = protection_tracker.get(
                position_side_str
            )  # 'ACTIVE_SLTP', 'ACTIVE_TSL', None
            sl_from_pos = active_position_data.get("stop_loss_price")  # Decimal or None
            tp_from_pos = active_position_data.get(
                "take_profit_price"
            )  # Decimal or None
            tsl_active_from_pos = active_position_data.get(
                "is_tsl_active", False
            )  # Boolean
            tsl_trigger_from_pos = active_position_data.get(
                "tsl_trigger_price"
            )  # Decimal or None

            pos_text.append(" | Prot: ", style="dim")
            prot_status_text = Text("None", style="dim")  # Default
            prot_details = Text("")

            # Determine status based primarily on position data, use tracker as fallback/confirmation
            ...
            elif tracked_protection:  # If position shows no stops, but tracker does
                prot_status_text = Text(
                    f"Tracked:{tracked_protection}", style="yellow"
                )  # Show tracker state
...
class TradingBot:
    ...
    def __init__(self):
        ...
        except ValueError as e:  # Catch init error if exchange instance was missing
            logger.critical(f"TradingBot initialization failed: {e}. Halting.")
            sys.exit(1)
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        # Rationale for multiple state fetches:
        # The state is re-fetched after potentially state-changing actions (TSL management, exit orders)
        # to ensure subsequent decisions (like entry orders) are based on the most current information available
        # from the exchange, minimizing the risk of acting on stale data within the same cycle.
        # This adds latency but prioritizes accuracy for sequential decisions within the cycle.
        if can_run_trade_logic:
            ...
            if (
                ...
                # Double-check essential parameters before calling entry function
                if total_equity is None or total_equity.is_nan() or total_equity <= 0:
                    logger.error(
                        "Entry skipped: Invalid equity detected just before entry attempt."
                    )
                    cycle_status = "FAIL:INVALID_EQUITY_PRE_ENTRY"
                elif current_atr.is_nan() or current_atr <= 0:
                    logger.error(
                        "Entry skipped: Invalid ATR detected just before entry attempt."
                    )
                    cycle_status = "FAIL:INVALID_ATR_PRE_ENTRY"
                else:
    ...
    def graceful_shutdown(self) -> None:
        ...
        try:
            ...
            # Check V5 response structure if possible (often contains a list or success code)
            if (
                ...
                logger.info(
                    f"Cancel all orders command successful. Found {len(cancelled_list)} items in response list (may include non-order items)."
                )
            elif isinstance(
                cancel_resp, list
...
if __name__ == "__main__":
    ...
    try:
        ...
        # sys.exit(e.code) # Propagate the exit code - No need to explicitly call exit again
        ...
        # Attempt notification even on critical failure
        try:
            termux_notify(
