    ...
    def print_status_panel(
        ...
        if indicators:
            ...
            def fmt_ind(key: str, prec: int = 1, default_prec: int = 1, style: str = "white") -> Text:
                val = indicators.get(key)
                dec_val = (
                    safe_decimal(val) if isinstance(val, (Decimal, int, float, str)) else Decimal("NaN")
                )  # Added str
            ...
            adx_text.append(
                fmt_ind(
                    "adx",
                    prec=1,
                    style="yellow" if adx_is_valid and adx_val_dec > self.config.min_adx_level else "dim yellow",
                )
            )
            adx_text.append(" [+DI:")
        ...
        try:
            ...
            if final_positions is not None:
                ...
                if not positions_to_close:
                    logger.info(f"{Fore.GREEN}No lingering positions found.{Style.RESET_ALL}")
                else:
