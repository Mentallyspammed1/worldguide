...
class OrderManager:
    ...
    def manage_trailing_stop(
        ...
        try:
            ...
            if should_activate_tsl:
                ...
                logger.trade(
                    f"Attempting TSL activation (Dist ~{tsl_distance_price.normalize()}, ActPx {activation_price.normalize():.{DEFAULT_PRICE_DP}f})..."
                )
    ...
    def close_position(
        ...
        logger.trade(
            f"{Fore.YELLOW}Closing market order ({close_order_id}) submitted for {position_side.upper()}. AvgClosePrice: {avg_close_price.normalize() if not avg_close_price.is_nan() else '[Pending/N/A]'}{Style.RESET_ALL}"
        )
        termux_notify(
    ...
    def print_status_panel(
        ...
        if indicators:
            ...
            if indicators.get("stoch_kd_bullish"):
                ...
                stoch_text.append(" [b red]▼[/]", style="red")
...
class TradingBot:
            ...
            if not self.shutdown_requested and sleep_duration > 0:
                ...
                try:
                    while (
                        ...
                        time.sleep(
                            min(0.5, sleep_duration)
                        )  # Sleep in small chunks to check flag
                except KeyboardInterrupt:
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        if indicators is None:
            logger.warning(
                f"{Fore.YELLOW}Indicator calculation failed (Cycle {cycle_count}).{Style.RESET_ALL}"
            )
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
            logger.error(
                f"{Fore.RED}Invalid equity ({total_equity}). Logic skipped (Cycle {cycle_count}).{Style.RESET_ALL}"
            )
            cycle_status = "FAIL:FETCH_EQUITY"
            can_run_trade_logic = False
        if current_positions is None:
            logger.error(
                f"{Fore.RED}Failed fetching positions. Logic skipped (Cycle {cycle_count}).{Style.RESET_ALL}"
            )
        ...
        if current_atr.is_nan() or current_atr <= 0:
            ...
            if cycle_status != "WARN:INDICATORS_FAILED":
                logger.error(
                    f"{Fore.RED}Invalid ATR ({current_atr}). Logic skipped (Cycle {cycle_count}).{Style.RESET_ALL}"
                )
                cycle_status = "FAIL:INVALID_ATR"
                can_run_trade_logic = False
            else:
                logger.warning(
                    f"{Fore.YELLOW}Invalid ATR ({current_atr}) due to indicator failure. Logic skipped.{Style.RESET_ALL}"
                )
                can_run_trade_logic = False
        ...
        if can_run_trade_logic:
            ...
            if current_pos_side and indicators:
                ...
                if positions_after_tsl is None:
                    logger.error(
                        f"{Fore.RED}Failed re-fetching positions after TSL check (Cycle {cycle_count}). Using potentially stale state.{Style.RESET_ALL}"
                    )
            ...
            ):  # Only check exit if in position and signals are valid
                logger.debug(
                ...
                if exit_reason:
                    ...
                    if not exit_triggered_by_signal:
                        ...
                        if positions_after_exit is None:
                            logger.error(
                                f"{Fore.RED}Failed re-fetching positions after signal exit (Cycle {cycle_count}).{Style.RESET_ALL}"
                            )
                            cycle_status = "WARN:POS_REFETCH_EXIT_FAIL"
                        else:
                            live_positions_state = (
                                positions_after_exit  # Update live state
                            )
                            final_positions_for_panel = copy.deepcopy(
                                live_positions_state
                            )  # Update panel state
    ...
    def graceful_shutdown(self) -> None:
        ...
        if (
            ...
            logger.error(
                f"{Fore.RED}Cannot shutdown: Exchange Manager/Instance/Market Info missing.{Style.RESET_ALL}"
            )
            termux_notify(
                ...
                for side in ["long", "short"]:
                    ...
                    if pos_data:
                        ...
                        if qty.copy_abs() >= POSITION_QTY_EPSILON:
                            logger.warning(
                                f"{Fore.YELLOW}Lingering {side.upper()} position (Qty: {qty.normalize()}) found.{Style.RESET_ALL}"
                            )
                            positions_to_close.append((side, qty))

                if not positions_to_close:
                    logger.info(
                        f"{Fore.GREEN}No lingering positions found.{Style.RESET_ALL}"
                    )
                else:
                    logger.warning(
                    ...
                    for side, qty in positions_to_close:
                        ...
                        if close_success:
                            closed_count += 1
                            logger.info(
                                f"{Fore.GREEN}{Style.BRIGHT}Closure initiated/confirmed for {side.upper()}.{Style.RESET_ALL}"
                            )
                        else:
                            logger.error(
                                f"{Fore.RED}Closure FAILED for {side.upper()}. MANUAL INTERVENTION REQUIRED.{Style.RESET_ALL}"
                            )

                    if closed_count == len(positions_to_close):
                        logger.info(
                            f"{Fore.GREEN}{Style.BRIGHT}All detected positions ({closed_count}) closed/initiated.{Style.RESET_ALL}"
                        )
                    else:
                        logger.warning(
                            f"{Fore.YELLOW}Attempted {len(positions_to_close)} closures, {closed_count} succeeded/initiated. MANUAL CHECK NEEDED.{Style.RESET_ALL}"
                        )
            else:
                logger.error(
                    f"{Fore.RED}Failed fetching positions during shutdown check. MANUAL CHECK REQUIRED for {symbol}!{Style.RESET_ALL}"
                )
                termux_notify(
