# -*- coding: utf-8 -*-
"""
Scalping Bot v6.4 - Pyrmethus Enhanced Edition (Bybit V5 Optimized)
...
Key Enhancements V6.4 (Compared to V6.2 & Previous):
- Fixed SyntaxError in compute_trade_signal_score related to chained conditional logic.
- Refactored order book imbalance calculation within compute_trade_signal_score for clarity.
- Updated internal version references to v6.4 for consistency with filename.
- Updated default log and state filenames to reflect v6.4.
- Updated logger name to ScalpingBotV6.4.
...
STATE_FILE_NAME: str = "scalping_bot_v6.4_state.json" # Updated state file name for v6.4
LOG_FILE_NAME: str = "scalping_bot_v6.4.log" # Updated log file name for v6.4
...
logger = logging.getLogger("ScalpingBotV6.4") # Updated logger name for v6.4
...
class ScalpingBot:
    """
    Pyrmethus Enhanced Scalping Bot v6.4. Optimized for Bybit V5 API.
    ...
    **Disclaimer:** Trading cryptocurrencies involves significant risk. Use at your own risk.
    ...
    def __init__(self, config_file: str = CONFIG_FILE_NAME, state_file: str = STATE_FILE_NAME) -> None:
        ...
        logger.info(f"{Fore.MAGENTA}{Style.BRIGHT}--- Pyrmethus Scalping Bot v6.4 Awakening ---{Style.RESET_ALL}")
        ...
        logger.info(f"{Fore.GREEN}{Style.BRIGHT}Scalping Bot V6.4 initialized. Symbol: {self.symbol}, Timeframe: {self.timeframe}{Style.RESET_ALL}")
    ...
    def create_default_config(self, config_file: str) -> None:
        ...
        # Default config structure with comments (as used in v6.4)
        default_config = {
            ...
            "risk_management": {
                "order_size_percentage": 0.02, # Proportion of available balance to use per order (e.g., 0.02 = 2%).
                "max_open_positions": 1, # Maximum number of concurrent active or pending entry positions.
                "use_atr_sl_tp": True, # Use ATR for Stop Loss and Take Profit calculations? If false, uses percentages below.
                "atr_sl_multiplier": ATR_MULTIPLIER_SL, # Multiplier for ATR to set Stop Loss distance.
                "atr_tp_multiplier": ATR_MULTIPLIER_TP, # Multiplier for ATR to set Take Profit distance.
                "stop_loss_percentage": 0.005, # Percentage distance for Stop Loss (e.g., 0.005 = 0.5%). Used if use_atr_sl_tp=false. Set null to disable.
                "take_profit_percentage": 0.01, # Percentage distance for Take Profit (e.g., 0.01 = 1%). Used if use_atr_sl_tp=false. Set null to disable.
                "sl_trigger_by": "MarkPrice", # Price type to trigger Stop Loss (e.g., 'MarkPrice', 'LastPrice', 'IndexPrice'). Null uses exchange default. Check Bybit V5 docs.
                "tp_trigger_by": "MarkPrice", # Price type to trigger Take Profit (e.g., 'MarkPrice', 'LastPrice', 'IndexPrice'). Null uses exchange default. Check Bybit V5 docs.
                "enable_trailing_stop_loss": False, # [EXPERIMENTAL] Enable Trailing Stop Loss via order edits? Requires exchange/CCXT support ('editOrder').
                "trailing_stop_loss_percentage": 0.003, # Percentage distance for Trailing Stop Loss activation/trail (e.g., 0.003 = 0.3%). Used if enable_trailing_stop_loss=true.
                "time_based_exit_minutes": 60, # Automatically market close position after this many minutes? Set null or 0 to disable.
                "strong_signal_adjustment_factor": 1.0, # Multiplier for order size on strong signals (score >= STRONG_SIGNAL_THRESHOLD_ABS). Default 1.0 = no adjustment.
                "weak_signal_adjustment_factor": 1.0, # Multiplier for order size on weak signals (score < STRONG_SIGNAL_THRESHOLD_ABS). Default 1.0 = no adjustment.
            },
        }
        ...
        try:
            ...
            with open(config_file, "w", encoding='utf-8') as f:
                ...
                f.write(f"# Configuration Spellbook for Pyrmethus Scalping Bot v{self.__class__.__name__.split('V')[-1]}\n\n") # Use class name to get version
    ...
    # Note: These are instance methods in this version, not static
    # calculate_volatility, calculate_ema, calculate_rsi, calculate_macd, calculate_stoch_rsi, calculate_atr
    # (Implementations are the same as provided in v6.2)
    # @staticmethod # Removed staticmethod decorator, these are instance methods
    def calculate_volatility(self, close_prices: pd.Series, window: int) -> Optional[float]:
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_ema(self, close_prices: pd.Series, period: int) -> Optional[float]:
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_rsi(self, close_prices: pd.Series, period: int) -> Optional[float]:
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_macd(self, close_prices: pd.Series, short_p: int, long_p: int, signal_p: int) -> Tuple[Optional[float],Optional[float],Optional[float]]:
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_rsi_series(self, close_prices: pd.Series, period: int) -> Optional[pd.Series]:
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_stoch_rsi(self, close_prices: pd.Series, rsi_p: int, stoch_p: int, k_p: int, d_p: int) -> Tuple[Optional[float],Optional[float]]:
        ...
        try:
            # Use instance method call: self.calculate_rsi_series
            rsi_s=self.calculate_rsi_series(close_prices,rsi_p)
    ...
    # @staticmethod # Removed staticmethod decorator
    def calculate_atr(self, high_prices: pd.Series, low_prices: pd.Series, close_prices: pd.Series, period: int) -> Optional[float]:
    ...
    # (Implementations are the same as provided in v6.2, except compute_trade_signal_score)
    ...
    def compute_trade_signal_score(self, price: float, indicators: Dict[str, Any], orderbook_imbalance: Optional[float]) -> Tuple[int, List[str]]:
        """
        Computes a trade signal score based on various indicators and order book imbalance.
        Score interpretation: Positive = Buy bias, Negative = Sell bias, 0 = Neutral.
        Absolute score indicates confidence.
        """
        func_name = "compute_trade_signal_score"; score = 0.0; reasons = []
        # Define thresholds (could be moved to config/constants if needed)
        RSI_OS, RSI_OB = 35, 65 # RSI OverSold/OverBought levels
        STOCH_OS, STOCH_OB = 25, 75 # Stochastic RSI OverSold/OverBought levels
        EMA_THRESH_MULT = 0.0002 # Price must be this % away from EMA to score
        MACD_HIST_ZERO_THRESH = 1e-9 # Threshold for MACD histogram crossing zero

        # Helper to format reason strings consistently
        def fmt_reason(t, p, c, v, d=2):
            reason_color = Fore.GREEN if p > 0 else (Fore.RED if p < 0 else Fore.WHITE)
            value_str = self.format_indicator(v, d) if v is not None else "N/A"
            return f"{reason_color}[{p:+.1f}] {t:<5} {c:<25} (Val: {value_str}){Style.RESET_ALL}"

        # --- Order Book Imbalance ---
        imb_p = 0.0 # Points for imbalance
        imb_c = "N/A" # Condition description
        imb_v = orderbook_imbalance # Actual value
        if imb_v is not None:
            imb = imb_v
            imb_t = self.imbalance_threshold # Ask/Bid Ratio threshold from config
            if imb_t <= 0:
                imb_c = f"Invalid Thresh ({imb_t})"
            elif imb == float('inf'): # Only asks available, strong sell signal
                imb_p = -1.0
                imb_c = "Sell (Imb=Inf)"
            else:
                # Calculate the buy threshold (inverse of sell threshold)
                # If Ask/Bid ratio < buy_t, means bids significantly outweigh asks -> Buy Signal
                buy_t = 1.0 / imb_t
                # Default state in this branch is neutral
                imb_c = f"Neutral ({buy_t:.3f}-{imb_t:.3f})"
                # Check conditions to override neutral
                if imb < buy_t:
                    imb_p = 1.0
                    imb_c = f"Buy (Imb<{buy_t:.3f})"
                elif imb > imb_t: # Ask/Bid ratio > sell threshold -> Sell Signal
                    imb_p = -1.0
                    imb_c = f"Sell (Imb>{imb_t:.3f})"
        else:
            imb_c = "OB N/A" # Order book data was not available

        score += imb_p
        reasons.append(fmt_reason("OB", imb_p, imb_c, imb_v, 3))
        ...
        if ema is not None and ema > 1e-9:
            upper_thresh = ema * (1.0 + EMA_THRESH_MULT)
            lower_thresh = ema * (1.0 - EMA_THRESH_MULT)
            ema_c = "Price ~ EMA"
            if price > upper_thresh:
                ema_p = 1.0
                ema_c = f"Price>EMA+{EMA_THRESH_MULT*100:.2f}%"
            elif price < lower_thresh:
                ema_p = -1.0
                ema_c = f"Price<EMA-{EMA_THRESH_MULT*100:.2f}%"
        else:
            ema_c = "EMA N/A"
        score += ema_p
        reasons.append(fmt_reason("EMA", ema_p, ema_c, ema, self.price_decimals))
        ...
        if rsi is not None:
            ...
            if rsi < RSI_OS:
                rsi_p = 1.0
                rsi_c = f"Oversold (<{RSI_OS})"
            elif rsi > RSI_OB:
                rsi_p = -1.0
                rsi_c = f"Overbought (>{RSI_OB})"
        else:
            rsi_c = "RSI N/A"
        score += rsi_p
        reasons.append(fmt_reason("RSI", rsi_p, rsi_c, rsi, 1))
        ...
        if macd_h is not None:
            macd_c = "Hist ~ 0 (Neutral)"
            if macd_h > MACD_HIST_ZERO_THRESH:
                macd_p = 1.0
                macd_c = "Hist > 0 (Bull Cross)"
            elif macd_h < -MACD_HIST_ZERO_THRESH:
                macd_p = -1.0
                macd_c = "Hist < 0 (Bear Cross)"
        else:
            macd_c = "MACD N/A"
        score += macd_p
        reasons.append(fmt_reason("MACD", macd_p, macd_c, macd_h, self.price_decimals + 2))

        # --- Stochastic RSI Overbought/Oversold ---
        k, d = indicators.get('stoch_k'), indicators.get('stoch_d')
        stoch_p = 0.0
        stoch_c = "N/A"
        if k is not None and d is not None:
            stoch_c = "Neutral"
            if k < STOCH_OS and d < STOCH_OS:
                stoch_p = 1.0
                stoch_c = f"Oversold (K&D<{STOCH_OS})"
            elif k > STOCH_OB and d > STOCH_OB:
                stoch_p = -1.0
                stoch_c = f"Overbought (K&D>{STOCH_OB})"
        else:
            stoch_c = "Stoch N/A"
        score += stoch_p
        stoch_vs = f"K:{self.format_indicator(k,1)}/D:{self.format_indicator(d,1)}"
        reasons.append(f"{fmt_reason('Stoch', stoch_p, stoch_c, None)} -> {stoch_vs}")

        # --- Final Score ---
        final_score = int(round(score))
        logger.debug(f"[{func_name}] Raw Score: {score:.2f}, Final: {final_score}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Note: Uses updated STATE_FILE_NAME and LOG_FILE_NAME defaults from v6.4
        ...
        print(f"\n{Fore.MAGENTA}Pyrmethus Bot v6.4 has concluded its watch. Exit Code: {exit_code}{Style.RESET_ALL}")
        sys.exit(exit_code)
