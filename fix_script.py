...
class BybitCCXTClient:
    ...
    async def initialize_markets(self, retries=3):
        ...
        for attempt in range(retries):
            try:
                ...
                self.logger.info(f"Successfully loaded {len(self.markets)} markets from {self._temp_exchange_for_init.name}.")
    ...
    def get_market_category(self, symbol: str) -> str:
        ...
        self.logger.debug(f"Could not determine category from market details for {symbol}, guessing based on quote currency.")
    ...
    async def fetch_with_retry(self, method_name: str, *args, **kwargs) -> Optional[Any]:
        ...
        for attempt in range(retries):
            try:
                ...
                if 'too many visits' in error_str or 'system busy' in error_str or 'service unavailable' in error_str or 'api concurrency limit' in error_str:
    ...
    async def fetch_klines(self, symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
        ...
        try:
            ...
            initial_rows = len(df)
            df.dropna(subset=['open', 'high', 'low', 'close', 'volume'], inplace=True) # Drop rows where essential values are missing
            dropped_rows = initial_rows - len(df)
            if dropped_rows > 0:
                 self.logger.warning(f"Dropped {dropped_rows} rows with NaN in essential OHLCV columns from kline data for {symbol}.")
...
class TradingAnalyzer:
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        min_data_points_needed = max(
             ...
             self.indicator_settings.get('macd_slow', 26) + self.indicator_settings.get('macd_signal', 9), # MACD needs slow+signal
        ...
        if len(df) < min_data_points_needed:
             self.logger.warning(f"Insufficient data points ({len(df)} < {min_data_points_needed} required) for some indicators. Results may be inaccurate or contain NaNs.")
        ...
        df_numeric.dropna(subset=['open', 'high', 'low', 'close'], inplace=True) # Drop rows if essential prices are NaN
        if df_numeric.empty:
        ...
        strategy_ta = [
            ...
            {"kind": "adosc"}, # Accumulation/Distribution Oscillator
             ...
             {"kind": "sma", "close": "volume", "length": is_.get("volume_ma_period", 20), "prefix": "VOL"}, # Calculate Volume SMA
        ]

        # Create the Strategy object
        custom_strategy = ta.Strategy(
            name="TradingAnalysisStrategy",
            description="Comprehensive TA Strategy using pandas_ta",
            ta=strategy_ta
        )
        ...
        try:
           ...
           if pandas_ta_vol_sma_name in df_numeric.columns and self.col_names['vol_ma'] != pandas_ta_vol_sma_name:
               ...
               self.logger.debug(f"Renamed volume MA column from {pandas_ta_vol_sma_name} to {self.col_names['vol_ma']}")
        ...
        if missing_cols:
             self.logger.warning(f"Some indicator columns are missing after calculation for {self.symbol}: {missing_cols}")
    ...
    def _calculate_levels(self, df: pd.DataFrame, current_price: Decimal) -> dict:
        ...
        try:
            ...
            current_price_f = float(current_price) # Already Decimal, convert for consistency
            ...
            if pd.isna(high_last) or pd.isna(low_last):
                self.logger.warning("NaN values in last candle's H/L, cannot calculate standard pivot points.")
                 ...
                 try:
                    ...
                    pivot_levels = {
                        "S1": (2 * pivot) - high_lf,   "R1": (2 * pivot) - low_lf,
                        "S2": pivot - (high_lf - low_lf), "R2": pivot + (high_lf - low_lf),
                        "S3": low_lf - 2 * (high_lf - pivot), "R3": high_lf + 2 * (pivot - low_lf),
                    }
                    for label, value in pivot_levels.items():
                         try:
                             value_dec = Decimal(str(value))
                             # Classify based on current price
                             if value_dec < current_price: levels["support"]["PP "+label] = value_dec # Prefix to distinguish
                             else: levels["resistance"]["PP "+label] = value_dec
                         except InvalidOperation:
    ...
    def _analyze_orderbook(self, orderbook: dict, current_price: Decimal, levels: dict) -> dict:
        ...
        if not orderbook or not isinstance(orderbook.get('bids'), list) or not isinstance(orderbook.get('asks'), list) or not orderbook['bids'] or not orderbook['asks']:
        ...
        try:
            ...
            bids_raw = [[Decimal(str(p)), Decimal(str(s))] for p, s in orderbook.get('bids', []) if p is not None and s is not None]
            asks_raw = [[Decimal(str(p)), Decimal(str(s))] for p, s in orderbook.get('asks', []) if p is not None and s is not None]
            ...
            if total_value > 0:
                ...
                if bid_ask_ratio > Decimal("0.6"): analysis["pressure"] = f"{NEON_GREEN}High Buy Pressure{RESET}"
                elif bid_ask_ratio < Decimal("0.4"): analysis["pressure"] = f"{NEON_RED}High Sell Pressure{RESET}"
            ...
            proximity_pct = Decimal(str(self.orderbook_settings.get("cluster_proximity_pct", 0.1))) / 100 # Convert percentage
                ...
                if bid_cluster_value >= cluster_threshold:
                    ...
                    if cluster_id not in processed_clusters:
                        analysis["clusters"].append({
                            ...
                            "avg_price": bids_near['price'].mean() if not bids_near.empty else level_price # Add avg price
                        })
    ...
    def _interpret_trend(self, last_row: pd.Series, prev_row: pd.Series) -> Tuple[List[str], Dict[str, str]]:
        ...
        if flags.get("ema_alignment"):
            ...
            if pd.notna(ema_short) and pd.notna(ema_long):
                ...
                summary.append(f"{color}EMA Alignment: {curr_state}{RESET} (S:{self._format_val(ema_short, price_precision)} {'><'[ema_short > ema_long]} L:{self._format_val(ema_long, price_precision)})")
            else:
    ...
    def _interpret_oscillators(self, last_row: pd.Series, prev_row: pd.Series) -> Tuple[List[str], Dict[str, str]]:
        ...
        if flags.get("williams_r_threshold"):
             ...
             if pd.notna(wr):
                 ...
                 level, color = "Neutral", NEON_YELLOW
                 if wr >= ob: level, color = "Overbought", NEON_RED # Closer to 0 is OB
                 elif wr <= os: level, color = "Oversold", NEON_GREEN # Closer to -100 is OS
        ...
        if flags.get("stoch_rsi_cross"):
            ...
            if pd.notna(k_now) and pd.notna(d_now) and pd.notna(k_prev) and pd.notna(d_prev):
                 ...
                 if k_now > d_now: current_state = "Bullish"
                 elif k_now < d_now: current_state = "Bearish"
                 ...
                 if k_now > d_now and k_prev <= d_prev: # Bullish cross
                     ...
                     if current_state == "Bullish": color = NEON_GREEN
                     elif current_state == "Bearish": color = NEON_RED
    ...
    def _interpret_macd(self, last_row: pd.Series, prev_row: pd.Series) -> Tuple[List[str], Dict[str, str]]:
        ...
        if flags.get("macd_cross"):
            ...
            if pd.notna(line_now) and pd.notna(sig_now) and pd.notna(line_prev) and pd.notna(sig_prev):
                ...
                if line_now > sig_now: current_state = "Bullish"
                elif line_now < sig_now: current_state = "Bearish"
             ...
             if pd.notna(upper) and pd.notna(lower) and pd.notna(middle) and pd.notna(close_val):
                ...
                if close_val > upper:
                    ...
                    bb_status += f"{color}Price Above Upper Band{RESET}"
                     ...
                     if close_val > middle: signal = "Above Mid"
                     else: signal = "Below Mid"
    ...
    def _interpret_volume(self, last_row: pd.Series, prev_row: pd.Series) -> Tuple[List[str], Dict[str, str]]:
        ...
        if flags.get("volume_confirmation"):
            ...
            if pd.notna(volume) and pd.notna(vol_ma) and vol_ma > 0: # Check vol_ma > 0 to avoid division errors implicitly
                 ...
                 try:
                     ...
                     if vol_dec > high_thresh: level, color = "High", NEON_GREEN
                     elif vol_dec < low_thresh: level, color = "Low", NEON_RED
                 except (InvalidOperation, TypeError):
                      # Fallback to float comparison if Decimal conversion fails
                      if volume > vol_ma * 1.5: level, color = "High", NEON_GREEN
                      elif volume < vol_ma * 0.7: level, color = "Low", NEON_RED
        ...
        if flags.get("obv_trend"):
             ...
             if pd.notna(obv_now) and pd.notna(obv_prev):
                 trend = "Flat"
                 color = NEON_YELLOW
                 if obv_now > obv_prev: trend, color = "Increasing", NEON_GREEN # Confirming uptrend?
                 elif obv_now < obv_prev: trend, color = "Decreasing", NEON_RED # Confirming downtrend?
        ...
        if flags.get("adi_trend"): # Flag name might be confusing, it's ADOSC
             ...
             if pd.notna(adosc_now) and pd.notna(adosc_prev):
                 ...
                 if adosc_now > 0 and adosc_now > adosc_prev: trend, color = "Accumulation", NEON_GREEN
                 elif adosc_now < 0 and adosc_now < adosc_prev: trend, color = "Distribution", NEON_RED
                 elif adosc_now > adosc_prev: trend, color = "Increasing", NEON_GREEN # Generic rise
                 elif adosc_now < adosc_prev: trend, color = "Decreasing", NEON_RED # Generic fall
    ...
    def _interpret_levels_orderbook(self, current_price: Decimal, levels: dict, orderbook_analysis: dict) -> List[str]:
        ...
        if levels:
             ...
             supports = sorted(levels.get("support", {}).items(), key=lambda item: abs(float(item[1]) - current_price_float))
             resistances = sorted(levels.get("resistance", {}).items(), key=lambda item: abs(float(item[1]) - current_price_float))
             pivot = levels.get("pivot")

             if pivot: summary.append(f"Pivot Point: ${self._format_val(pivot, price_precision)}")

             if supports: summary.append("Nearest Support:")
             for name, price in supports[:3]: # Show top 3 nearest
                  summary.append(f"  > {name}: ${self._format_val(price, price_precision)}")

             if resistances: summary.append("Nearest Resistance:")
        ...
        if orderbook_analysis and orderbook_analysis.get('total_bid_usd', 0) + orderbook_analysis.get('total_ask_usd', 0) > 0:
             ...
             clusters = sorted(orderbook_analysis.get("clusters", []), key=lambda x: x.get('cluster_value_usd', Decimal(0)), reverse=True)
             if clusters:
                 summary.append(f"{NEON_PURPLE}Significant OB Clusters (Top 5 by Value):{RESET}")
                 for cluster in clusters[:5]:
                      color = NEON_GREEN if cluster.get("type") == "Support" else NEON_RED if cluster.get("type") == "Resistance" else NEON_YELLOW
                      ...
                      avg_price_f = self._format_val(cluster.get('avg_price'), price_precision) # Show avg price of cluster
                      summary.append(f"  {color}{cluster.get('type')} near {cluster.get('level_name')} (${level_price_f}) - Value: ${cluster_val_f} (Avg Price: ${avg_price_f}){RESET}")
             else:
    ...
    def analyze(self, df_klines: pd.DataFrame, current_price: Decimal, orderbook: Optional[dict]) -> dict:
        ...
        if df_klines.empty:
            ...
            return analysis_result
             ...
             analysis_result["interpretation"]["summary"] = [f"{NEON_YELLOW}Warning: Insufficient kline data ({len(df_klines)} rows) for full analysis.{RESET}"]
        ...
        try:
            ...
            if not df_with_indicators.empty:
                ...
                analysis_result["raw_indicators"] = {
                    k: self._format_val(v, 5) if isinstance(v, (Decimal, float, np.floating)) else # Use higher precision for raw data
                       (v.isoformat() if isinstance(v, (pd.Timestamp, datetime)) else str(v) if pd.notna(v) else None)
                    for k, v in last_row_indicators.items() if k != 'timestamp' # Exclude timestamp from raw indicators dict
                }
...
async def run_analysis_loop(symbol: str, interval_config: str, client: BybitCCXTClient, analyzer: TradingAnalyzer, logger_instance: logging.Logger):
    ...
    while True:
        ...
        try:
            ...
            if logger_instance.isEnabledFor(logging.DEBUG):
                try:
                    ...
                    log_data = json.loads(json.dumps(analysis_result, default=str)) # Convert Decimals etc. to strings first
            ...
            logger_instance.critical(f"{NEON_RED}Authentication Error: {e}. Check API Key/Secret. Stopping analysis loop for {symbol}.{RESET}")
            break # Stop the loop for this symbol on auth errors
        except ccxt.InvalidNonce as e:
             logger_instance.critical(f"{NEON_RED}Invalid Nonce Error: {e}. Check system time sync. Stopping analysis loop for {symbol}.{RESET}")
        ...
        if elapsed_time > analysis_interval_sec:
             logger_instance.warning(f"Analysis cycle for {symbol} took longer ({elapsed_time:.2f}s) than configured interval ({analysis_interval_sec}s). Consider increasing interval or checking performance.")
...
async def main():
    ...
    if markets_loaded:
        ...
        temp_logger.critical(f"{NEON_RED}Failed to load markets after multiple retries. Cannot proceed. Exiting.{RESET}")
    ...
    while True:
        try:
                 ...
                 for quote in ['USDT', 'USD', 'BTC', 'ETH', 'EUR']:
                     if symbol_input.endswith(quote) and len(symbol_input) > len(quote):
                         ...
                         if potential_symbol in valid_symbols:
                             ...
                             print(f"{NEON_YELLOW}Interpreted '{symbol_input}' as {formatted_symbol}{RESET}")
                             break # Found a match
            ...
            if formatted_symbol in valid_symbols:
                ...
                print(f"{NEON_RED}Invalid or unknown symbol: '{symbol_input}' (Formatted: '{formatted_symbol}'). {RESET}")
                ...
                if find_similar:
                     suggestions = ", ".join(find_similar[:5]) # Limit suggestions
                     print(f"{NEON_YELLOW}Did you mean one of these? {suggestions}{RESET}")
        except EOFError:
             print(f"\n{NEON_YELLOW}Input stream closed. Exiting.{RESET}")
             return
        except KeyboardInterrupt:
             print(f"\n{NEON_YELLOW}Operation cancelled by user during input. Exiting.{RESET}")
             return
    ...
    while True:
        try:
            interval_input_raw = input(f"{NEON_BLUE}Enter timeframe [{', '.join(VALID_INTERVALS)}] (default: {default_interval}): {RESET}").strip()
            ...
            # Check if input is in CCXT format (e.g., '1h', '1d') and map back
                ...
                print(f"{NEON_YELLOW}Using interval {interval_key} (mapped from CCXT format {interval_input}){RESET}")
                break
            else:
                print(f"{NEON_RED}Invalid interval '{interval_input_raw}'. Choose from Bybit formats: [{', '.join(VALID_INTERVALS)}] or common CCXT formats (e.g., 1m, 5m, 1h, 4h, 1d).{RESET}")
        except EOFError:
             print(f"\n{NEON_YELLOW}Input stream closed. Exiting.{RESET}")
             return
        except KeyboardInterrupt:
    ...
    logger.info(f"Starting analysis for {NEON_PURPLE}{symbol}{RESET} (Category: {market_type}) on interval {NEON_PURPLE}{interval_key}{RESET} (CCXT: {ccxt_interval}).")
    ...
    try:
        ...
        logger.info(f"{NEON_YELLOW}Analysis loop interrupted by user.{RESET}")
         ...
         logger.critical(f"{NEON_RED}A critical error occurred outside the main analysis loop: {e}{RESET}", exc_info=True)
    finally:
        # 6. Cleanup
             ...
             for handler in handlers:
                 try:
                     ...
                     logger.removeHandler(handler)
                      ...
                      print(f"{NEON_RED}Error closing logger handler: {log_close_err}{RESET}")
        logging.shutdown()
