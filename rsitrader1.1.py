...
def display_position_status(position: Dict[str, Any], price_precision: int = 4, amount_precision: int = 8) -> None:
    ...
    tsl_str: str = f" | Active TSL: {tsl:.{price_precision}f}" if isinstance(tsl, (float, int)) else "" # Show TSL only if active
    ...
    print(f"{color}Position Status: {status_text}{RESET} | Entry: {entry_str} | Qty: {qty_str} | Target SL: {sl_str} | Target TP: {tp_str}{tsl_str}")
    ...
    if oco_id:
         ...
         if sl_id: print(f"    SL Order ID: {sl_id}")
         if tp_id: print(f"    TP Order ID: {tp_id}")
...
def load_config(filename: str = CONFIG_FILE) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- Validation ---
        def validate(key: str, expected_type: type, condition: Optional[Callable[[Any], bool]] = None, is_required: bool = True):
            ...
            if not isinstance(value, expected_type):
                raise TypeError(f"Config key '{key}' has incorrect type. Expected {expected_type.__name__}, got {type(value).__name__}.")
        ...
        if config_data.get("enable_trailing_stop"):
             ...
             validate("trailing_stop_activation_atr_multiplier", (float, int), lambda x: x >= 0) # Activation can be zero
...
try:
    ...
    if exchange.has.get('oco'):
         ...
         log_warning(f"Exchange {exchange_id} does NOT appear to support OCO orders via ccxt `has['oco']` flag. SL/TP will be placed separately.")
...
try:
    ...
    if price_prec is not None: price_precision_digits = int(exchange.decimal_to_precision(price_prec, ccxt.ROUND, counting_mode=exchange.precisionMode))
    if amount_prec is not None: amount_precision_digits = int(exchange.decimal_to_precision(amount_prec, ccxt.ROUND, counting_mode=exchange.precisionMode))

    # Get minimum tick size (price increment)
    if price_prec is not None:
        try:
            min_tick = float(price_prec) if isinstance(price_prec, (float, int)) else float(exchange.price_to_precision(symbol, 10**(-price_precision_digits)))
            # Handle potential zero precision
            if min_tick <= 0: min_tick = 1 / (10 ** price_precision_digits) if price_precision_digits >= 0 else 0.01
        except Exception:
    ...
    if min_amount: log_info(f"Minimum Order Amount: {min_amount} {market_info.get('base', '')}")
    ...
    try:
        ...
        for key, default_value in position_default_structure.items():
             if key in loaded_state:
                 ...
                 if loaded_value is not None and default_value is not None and \
                    not isinstance(loaded_value, expected_type) and not allow_int_for_float:
                     issues.append(f"Type mismatch for '{key}': Expected {expected_type.__name__}, Got {type(loaded_value).__name__}")
                 else:
        ...
        # Log issues
        if issues: log_warning(f"Issues loading state file '{filename}' (using defaults for problematic keys): {'; '.join(issues)}")
        extra_keys = set(loaded_state.keys()) - set(position_default_structure.keys())
        if extra_keys: log_warning(f"Extra keys found in state file (ignored): {extra_keys}")
...
def identify_potential_order_block(df: pd.DataFrame, vol_thresh_mult: float, lookback_len: int) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if len(completed_candles_df) >= min_periods_vol:
             rolling_vol = completed_candles_df['volume'].rolling(window=lookback_len, min_periods=min_periods_vol).mean()
        ...
        for i in range(start_index, end_index, -1):
            ...
            if impulse_candle.isnull().any() or ob_candle.isnull().any(): continue
            ...
            if not bullish_ob and ob_is_bearish and is_bullish_impulse and is_high_volume and impulse_sweeps_ob_low:
                bullish_ob = {'high': ob_candle['high'], 'low': ob_candle['low'], 'time': ob_candle.name, 'type': 'bullish'}
            ...
            # Bearish OB: Bullish OB candle, high-vol bearish impulse sweeping high
                ...
                bearish_ob = {'high': ob_candle['high'], 'low': ob_candle['low'], 'time': ob_candle.name, 'type': 'bearish'}
...
def calculate_technical_indicators(
    df: Optional[pd.DataFrame], rsi_len: int, stoch_params: Dict[str, int],
    ...
    if calc_vol_ma: required_base.append('volume')
    ...
    try:
        ...
        if calc_vol_ma:
            if 'volume' in df_processed.columns:
                ...
                df_processed[vol_ma_col] = df_processed['volume'].rolling(window=vol_ma_len, min_periods=min_p_vol).mean()
        ...
        if rows_dropped > 0: log_debug(f"Dropped {rows_dropped} rows with NaN indicators.")
...
def calculate_position_size(
    ...
    current_price: float, stop_loss_price: float, risk_perc: float
) -> Optional[float]:
    ...
    try:
        log_debug(f"Calculating position size: Entry={current_price:.{price_precision_digits}f}, SL={stop_loss_price:.{price_precision_digits}f}, Risk={risk_perc*100:.2f}%")
        ...
        contract_size: float = float(market.get('contractSize', 1.0))
        ...
        if is_inverse:
             ...
             log_warning(f"Calculating size for INVERSE contract ({trading_symbol}). Sizing formula assumes risk is calculated based on quote currency balance and loss per unit is measured in quote currency. VERIFY this matches your exchange's PnL calculation for inverse contracts.")
        ...
        # Bybit Unified specific check (example, might need adjustment based on API changes)
        elif exchange_instance.id == 'bybit' and quote_currency == 'USDT' and 'info' in balance and isinstance(balance['info'], dict) and 'result' in balance['info'] and isinstance(balance['info']['result'], dict) and 'list' in balance['info']['result']:
             for wallet_info in balance['info']['result']['list']:
                 if isinstance(wallet_info, dict) and wallet_info.get('accountType') == 'UNIFIED' and wallet_info.get('coin') == quote_currency:
                     ...
                     balance_source_info = f"Bybit Unified 'availableToWithdraw' or 'walletBalance'"
                     break

        if available_balance <= 0:
            log_error(f"No available {quote_currency} balance ({available_balance}) found. Source checked: {balance_source_info}. Full details: {balance.get(quote_currency, balance.get('info', 'N/A'))}")
            return None
        log_info(f"Available balance ({quote_currency}): {available_balance:.{price_precision_digits}f} (Source: {balance_source_info})")
        ...
        if price_diff <= min_tick / 2:
            log_error(f"Stop-loss {stop_loss_price:.{price_precision_digits}f} too close to entry {current_price:.{price_precision_digits}f} (Diff: {price_diff:.{price_precision_digits+4}f} <= Min Tick/2).")
            return None
        ...
        try:
            quantity_adjusted = float(exchange_instance.amount_to_precision(trading_symbol, quantity_base))
             ...
             quantity_adjusted = math.floor(quantity_base * scale) / scale if quantity_base > 0 else math.ceil(quantity_base * scale) / scale
        ...
        if min_amount_limit is not None and quantity_adjusted < min_amount_limit:
            log_error(f"Calculated quantity {quantity_adjusted:.{amount_precision_digits}f} < minimum {min_amount_limit} {base_currency}.")
            return None

        estimated_cost = quantity_adjusted * current_price
        if min_cost_limit is not None and estimated_cost < min_cost_limit:
             log_error(f"Estimated cost {estimated_cost:.{price_precision_digits}f} {quote_currency} < minimum {min_cost_limit} {quote_currency}. Need Qty > {min_cost_limit / current_price:.{amount_precision_digits}f}")
             return None

        # Final sanity check vs balance
        cost_buffer_factor = 0.99
        if estimated_cost > available_balance * cost_buffer_factor:
             log_error(f"Estimated cost ({estimated_cost:.{price_precision_digits}f}) exceeds {cost_buffer_factor*100:.0f}% of available balance ({available_balance:.{price_precision_digits}f}). Reduce risk %.")
             return None
...
def place_market_order(
    exchange_instance: ccxt.Exchange, trading_symbol: str,
    side: str, amount: float, reduce_only: bool = False
    ...
    try:
        ...
        log_info(f"Attempting to place {side.upper()} market order: {amount_formatted:.{amount_precision_digits}f} {base_currency} on {trading_symbol} {'(ReduceOnly)' if reduce_only else ''}...")
        ...
        if SIMULATION_MODE:
            ...
            try:
                 ...
                 def fetch_sim_ticker(exch, sym): return exch.fetch_ticker(sym)
            ...
            order = {
                ...
                'timestamp': int(time.time() * 1000), 'datetime': pd.Timestamp.now(tz='UTC').isoformat(),
                'status': 'closed', 'symbol': trading_symbol, 'type': 'market',
                'side': side, 'price': sim_price, 'average': sim_price,
                'amount': amount_formatted, 'filled': amount_formatted, 'remaining': 0.0,
                'cost': sim_cost, 'fee': None, 'reduceOnly': params.get('reduceOnly', False),
                'info': {'simulated': True}
            }
        else:
            log_warning(f"!!! LIVE MODE: Placing real market order {'(ReduceOnly)' if params.get('reduceOnly') else ''}.")
            order = exchange_instance.create_market_order(symbol=trading_symbol, side=side, amount=amount_formatted, params=params)
        ...
        if order:
            ...
            log_info(f"Order Result | ID: {order_id or 'N/A'}, Status: {status or 'N/A'}, Avg Price: {price_str}, Filled: {filled_str} {base_currency}, Cost: {cost_str} {quote_currency}, ReduceOnly: {reduce_info}")
        else:
        ...
        return None
         ...
         log_error(f"Invalid market order for {side} {amount_formatted:.{amount_precision_digits}f} {trading_symbol}: {e}")
         return None
...
def place_sl_tp_orders(
    exchange_instance: ccxt.Exchange, trading_symbol: str, position_side: str,
    ...
    # --- Input Validation ---
    if not (isinstance(quantity, (float, int)) and quantity > 0 and
            position_side in ['long', 'short'] and
            isinstance(sl_price, (float, int)) and sl_price > 0 and
            isinstance(tp_price, (float, int)) and tp_price > 0):
        log_error(f"Invalid inputs for SL/TP placement: Qty={quantity}, Side={position_side}, SL={sl_price}, TP={tp_price}")
        return None, None, None
    # Basic logic check
    if (position_side == 'long' and sl_price >= tp_price) or \
       (position_side == 'short' and sl_price <= tp_price):
    ...
    try:
        ...
        if qty_fmt <= 0: raise ValueError("Quantity non-positive after formatting.")

        # --- Prepare Common Parameters ---
        is_contract = market.get('swap') or market.get('future') or market.get('contract')
        params = {'reduceOnly': True} if is_contract else {}
        if params: log_debug("Applying 'reduceOnly=True' to protection orders.")
        ...
        stop_market_possible = exchange_instance.has.get('createStopMarketOrder', False) or \
                               'STOP_MARKET' in exchange_instance.options.get('createOrderTypes', []) or \
                               exchange_instance.id in ['binance', 'bybit'] # Assume common ones support it via params

        stop_limit_possible = exchange_instance.has.get('createStopLimitOrder', False) or \
                              'STOP_LOSS_LIMIT' in exchange_instance.options.get('createOrderTypes', []) or \
                              'StopLimit' in exchange_instance.options.get('createOrderTypes', []) # e.g., Bybit
        ...
        if stop_market_possible:
            ...
            sl_limit_raw = sl_price_fmt - limit_offset_ticks if close_side == 'sell' else sl_price_fmt + limit_offset_ticks
            sl_limit_price = float(exchange_instance.price_to_precision(trading_symbol, sl_limit_raw))
            log_warning(f"Selected SL type: stopLimit (fallback). Trigger: {sl_price_fmt}, Limit: {sl_limit_price}. Fill not guaranteed.")
        else:
            log_error(f"Cannot determine supported stop order type (stopMarket/stopLimit) for {exchange_instance.id}. Cannot place SL.")
        ...
        if use_oco:
            log_info(f"Attempting OCO {close_side.upper()} order: Qty={qty_fmt}, TP Limit={tp_price_fmt}, SL Trigger={sl_price_fmt} ({sl_order_type}) {'(ReduceOnly)' if params else ''}")
            ...
            try:
                ...
                if exchange_instance.id == 'binance': # Example for Binance
                    ...
                    sl_type_for_oco = 'STOP_MARKET' if sl_order_type == 'stopMarket' else 'STOP_LOSS_LIMIT'
                    # TP type is usually LIMIT_MAKER or just LIMIT
                    tp_type_for_oco = 'LIMIT_MAKER' # Prefer maker to save fees
                    ...
                    if SIMULATION_MODE:
                        ...
                        log_error("Live OCO placement for Binance needs specific implementation beyond this generic structure. Simulating failure.")
                ...
                # elif exchange_instance.id == 'okx': ...
                    ...
                    log_warning(f"Exchange {exchange_instance.id} supports OCO, but specific parameter structure not implemented here. Falling back to separate SL/TP orders.")
                    use_oco = False # Force fallback to separate orders

                # If OCO attempt was made (simulated or real) and seems successful
                if oco_ref_id:
                    log_info(f"OCO order request processed {'(Simulated)' if SIMULATION_MODE else ''}. Ref ID: {oco_ref_id}")
                    return None, None, oco_ref_id # Return OCO ref ID, clear separate IDs

            except ccxt.NotSupported as e:
                log_warning(f"OCO order placement attempt failed (NotSupported or Structure Unknown): {e}. Falling back to separate SL/TP orders.")
                use_oco = False # Ensure fallback
                 ...
                 log_error(f"Unexpected error during OCO placement: {e}. Falling back to separate SL/TP orders.", exc_info=True)
                 use_oco = False
        ...
        if not use_oco and sl_order_type is not None:
            ...
            try:
                log_info(f"Attempting SL ({sl_order_type}): {close_side.upper()} {qty_fmt} @ trigger ~{sl_price_fmt} {'Limit '+str(sl_limit_price) if sl_limit_price else ''} {'(ReduceOnly)' if params else ''}")
                ...
                if SIMULATION_MODE:
                    ...
                    sl_order_result = exchange_instance.create_order(
                        ...
                        amount=qty_fmt, price=sl_limit_price, # price is limit price for stopLimit, ignored otherwise
                        params=sl_params_sep
                    )
                    if sl_order_result and sl_order_result.get('id'):
                        sl_order_id = sl_order_result['id']
                        sl_placed = True
                        log_info(f"Separate SL order request processed. ID: {sl_order_id}, Status: {sl_order_result.get('status', 'N/A')}")
                        time.sleep(0.75)
                    else:
            ...
            try:
                 log_info(f"Attempting TP (limit): {close_side.upper()} {qty_fmt} @ limit {tp_price_fmt} {'(ReduceOnly)' if params else ''}")
                 ...
                 if SIMULATION_MODE:
                      ...
                      log_warning(f"!!! LIVE MODE: Placing real separate limit TP order.")
                      ...
                      if tp_order_result and tp_order_result.get('id'):
                           ...
                           tp_placed = True
                           log_info(f"Separate TP order request processed. ID: {tp_order_id}, Status: {tp_order_result.get('status', 'N/A')}")
                           time.sleep(0.75)
                      else:
        ...
        if orders_were_tracked:
            ...
            if oco_order_id:
                ...
                if oco_order_id not in open_order_ids:
                     log_info(f"{NEON_YELLOW}Tracked OCO order {oco_order_id} no longer open. Assuming position closed via SL/TP.{RESET}")
        ...
        if position['status'] is not None and order_check_passed and is_contract and can_fetch_positions:
            ...
            try:
                 ...
                 if positions_data and isinstance(positions_data, list):
                      for pos_info in positions_data:
                          if isinstance(pos_info, dict) and pos_info.get('symbol') == trading_symbol:
                              ...
                              try:
                                   ...
                                   log_warning(f"Could not parse position size ('{size_key}': {pos_size_str}) from fetch_positions result.")
                              ...
                              if abs(current_position_size) > (min_amount or 1e-9): # Check against min_amount or small epsilon
                                  ...
                                  if (position['status'] == 'long' and pos_side == 'long' and current_position_size > 0) or \
                                     (position['status'] == 'short' and pos_side == 'short' and current_position_size > 0): # Some exchanges report size as positive for short too
                                       log_warning(f"Position check: Found active {pos_side} position ({current_position_size}) on exchange, matching local direction. Order check may have been inaccurate. Keeping local state.")
                                       found_pos = True
                                       # We won't reset state if position matches local direction
                                  else:
                                       # Size or direction mismatch! Indicates state inconsistency.
                                       log_error(f"Position check: Found MISMATCHED position on exchange! Local: {position['status']}, Exchange: {pos_side} Size={current_position_size}. Resetting local state. MANUAL REVIEW RECOMMENDED.")
                                       position_reset_required = True
                                   ...
                                   log_info("Position check: No active position found via fetch_positions, confirming closure.")
                      ...
                      position_reset_required = order_check_passed and orders_were_tracked # Only reset if order check suggested it
            ...
            if order_to_cancel_id:
                 ...
                 try:
                     if not cancel_order_with_retry(exchange_instance, order_to_cancel_id, trading_symbol):
                         log_error(f"Failed to confirm cancellation of leftover order {order_to_cancel_id}. Manual check advised.")
                 except Exception as e:
                     log_error(f"Error cancelling leftover order {order_to_cancel_id}: {e}. Manual check advised.", exc_info=False)
            else:
                 log_debug(f"No corresponding order ID was tracked or needed cancellation.")
...
def update_trailing_stop(
    exchange_instance: ccxt.Exchange, trading_symbol: str,
    current_price: float, last_atr: Optional[float]
    ) -> None:
    """Checks and updates Trailing Stop Loss if conditions are met."""
    global position
    if position['status'] is None or not enable_trailing_stop: return
    ...
    log_debug(f"TSL Check: Active {position['status']} position. Current SL/OCO Order: {active_sl_order_id}, Last TSL Price: {position.get('current_trailing_sl_price')}")
    ...
    if position['status'] == 'long':
        ...
        if current_high > activation_thresh:
            ...
            log_debug(f"Long TSL Active. High ({current_high:.{price_precision_digits}f}) > Threshold ({activation_thresh:.{price_precision_digits}f}). Potential New TSL: {potential_new_tsl_price:.{price_precision_digits}f}")
    elif position['status'] == 'short':
        activation_thresh = entry_price - (last_atr * trailing_stop_activation_atr_multiplier)
        ...
        if current_low < activation_thresh:
            ...
            log_debug(f"Short TSL Active. Low ({current_low:.{price_precision_digits}f}) < Threshold ({activation_thresh:.{price_precision_digits}f}). Potential New TSL: {potential_new_tsl_price:.{price_precision_digits}f}")
    ...
    if tsl_activated and potential_new_tsl_price is not None:
        ...
        if position['status'] == 'long' and new_tsl_price_fmt > current_effective_sl_price and new_tsl_price_fmt < current_price:
            should_update_tsl = True
        elif position['status'] == 'short' and new_tsl_price_fmt < current_effective_sl_price and new_tsl_price_fmt > current_price:
            should_update_tsl = True

        if should_update_tsl:
            log_debug(f"TSL Improvement Check OK: New ({new_tsl_price_fmt:.{price_precision_digits}f}) vs Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) vs Price ({current_price:.{price_precision_digits}f})")
        else:
             log_debug(f"TSL Improvement Check Failed: New ({new_tsl_price_fmt:.{price_precision_digits}f}) vs Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) vs Price ({current_price:.{price_precision_digits}f})")

    # --- Execute TSL Update (Cancel Old, Place New - Handles OCO or Separate) ---
    if should_update_tsl and new_tsl_price_fmt is not None:
        log_info(f"{NEON_YELLOW}Trailing Stop Update! Moving SL from ~{current_effective_sl_price:.{price_precision_digits}f} to {new_tsl_price_fmt:.{price_precision_digits}f}{RESET}")
        ...
        try:
            ...
            if cancel_success:
                 ...
                 log_error(f"TSL Update: Failed to cancel old order {old_order_id}. Aborting TSL update. Position may remain protected by old order.")
                 return

            # --- Step 2: Place the new protection orders (OCO or Separate) ---
            log_info(f"TSL Update: Placing new protection orders with SL at {new_tsl_price_fmt:.{price_precision_digits}f} and original TP at {original_tp_price:.{price_precision_digits}f}")
            ...
            if new_oco_id or new_sl_id: # Check if at least SL/OCO part was successful
                log_info(f"{NEON_GREEN}TSL Update: New protection orders placed successfully. New SL Trigger: {new_tsl_price_fmt:.{price_precision_digits}f}{RESET}")
                ...
                log_error(f"TSL Update CRITICAL ERROR: Failed to place new protection orders after cancelling old one {old_order_id}. POSITION IS NOW UNPROTECTED.")
            ...
            log_error(f"TSL Update CRITICAL ERROR: Error during TSL update process: {e}. POSITION MAY BE UNPROTECTED.", exc_info=True)
    ...
    while True:
        try:
            ...
            df_indicators = calculate_technical_indicators(
                ohlcv_df, rsi_length, stoch_params_dict,
            ...
            if df_indicators is None or df_indicators.empty:
                 log_warning(f"Indicator calculation failed or empty DataFrame. Waiting...")
                 neon_sleep_timer(sleep_interval_seconds)
                 continue

            # --- Step 4: Get Latest Data Point & Values ---
            if len(df_indicators) < 1: continue # Should be handled by calc func, but safety check
            ...
            try:
                ...
                if needs_atr_calc and atr_col and atr_col in latest_data and pd.notna(latest_data[atr_col]):
                    ...
                    if atr_val > 0: last_atr = atr_val
                    else: log_warning(f"Invalid ATR value ({atr_val}) <= 0.")
                elif needs_atr_calc: log_warning("ATR calculation required but column/value missing/NaN.")
                ...
                if needs_vol_ma_calc and vol_ma_col and vol_ma_col in latest_data and pd.notna(latest_data[vol_ma_col]):
                    ...
                    if vol_ma_val > 0: last_volume_ma = vol_ma_val
                elif needs_vol_ma_calc: log_debug("Volume MA calculation required but column/value missing/NaN.")
                ...
                if any(math.isnan(v) for v in [current_price, last_rsi, last_stoch_k, last_stoch_d]):
                 ...
                 log_error(f"Error extracting latest data or value invalid at {latest_timestamp}: {e}. Data: {latest_data.to_dict()}", exc_info=False)
                 neon_sleep_timer(sleep_interval_seconds)
                 continue
            ...
            bullish_ob, bearish_ob = identify_potential_order_block(df_indicators, ob_volume_threshold_multiplier, ob_lookback)
            ...
            if position['status'] is not None:
                log_info(f"Monitoring {position['status'].upper()} position entered at {position.get('entry_price', 'N/A'):.{price_precision_digits}f}...")
                ...
                if execute_fallback_exit:
                    ...
                    log_warning(f"Attempting FALLBACK MARKET EXIT for {position['status'].upper()} position: {fallback_reason}")
                    ...
                    if exit_qty is None or exit_qty <= 0:
                        ...
                        order_id_to_cancel = position.get('oco_order_id') or position.get('sl_order_id') or position.get('tp_order_id')
                        # Also try cancelling the other if separate orders were used
                        other_order_id = position.get('tp_order_id') if position.get('sl_order_id') else position.get('sl_order_id')

                        cancellation_ok = True
                        if order_id_to_cancel:
                             log_info(f"Cancelling protection order(s) before fallback exit: {order_id_to_cancel} {f', also {other_order_id}' if other_order_id and not position.get('oco_order_id') else ''}...")
                             try:
                                 if not cancel_order_with_retry(exchange, order_id_to_cancel, symbol): cancellation_ok = False
                                 if other_order_id and not position.get('oco_order_id'): # Cancel second order if separate
                                      if not cancel_order_with_retry(exchange, other_order_id, symbol): cancellation_ok = False
                             except Exception as cancel_e:
                                  log_error(f"Error cancelling protection orders during fallback: {cancel_e}", exc_info=False)
                                  cancellation_ok = False

                        # 2. Place Market Exit Order (Only if cancellation seemed ok)
                        if cancellation_ok:
                             log_info("Protection orders cancelled or already closed. Proceeding with fallback market exit.")
                             close_side = 'sell' if position['status'] == 'long' else 'buy'
                             fallback_exit_order = place_market_order(exchange, symbol, close_side, exit_qty, reduce_only=True)

                             if fallback_exit_order and fallback_exit_order.get('id'):
                                 log_info(f"Fallback market exit order processed: ID {fallback_exit_order.get('id', 'N/A')}")
                                 ...
                                 log_error(f"Fallback market order placement FAILED. Result: {fallback_exit_order}. POSITION MAY STILL BE OPEN. MANUAL INTERVENTION REQUIRED.")
                        else:
                             log_error("Failed to confirm cancellation of protection orders. Aborting fallback market exit. MANUAL REVIEW RECOMMENDED.")
                else:
            ...
            # === B. LOGIC TO CHECK FOR NEW ENTRIES (Only if position is None) ===
                ...
                if entry_volume_confirmation_enabled:
                    if current_volume is not None and last_volume_ma is not None and last_volume_ma > 0:
                        ...
                        else: log_debug(f"Volume Not Confirmed: Vol ({current_volume:.2f}) <= Threshold ({vol_thresh:.2f})")
                    else: log_debug("Volume confirmation skipped (missing data).")
                else:
                    vol_confirmed = True # Always true if disabled

                # --- B.2: Base Signal Conditions (RSI + Stoch) ---
                base_long_signal = last_rsi < rsi_oversold and last_stoch_k < stoch_oversold and last_stoch_d < stoch_oversold
                base_short_signal = last_rsi > rsi_overbought and last_stoch_k > stoch_overbought and last_stoch_d > stoch_overbought
                ...
                if base_long_signal and bullish_ob:
                    ...
                    entry_zone_high = bullish_ob['high'] # Maybe allow slight penetration above? Keep simple: within OB.
                    ...
                    else: log_debug(f"Price ({current_price:.{price_precision_digits}f}) outside Bullish OB.")
                    ...
                    else: log_debug(f"Price ({current_price:.{price_precision_digits}f}) outside Bearish OB.")
                ...
                if base_long_signal and ob_confirmed and vol_confirmed and entry_ob and entry_ob['type'] == 'bullish':
                     side_to_enter = 'long'
                     entry_reason_full = f"RSI({last_rsi:.1f})<OS, Stoch({last_stoch_k:.1f},{last_stoch_d:.1f})<OS, {ob_reason}" + (", Vol Confirmed" if entry_volume_confirmation_enabled else "")
                elif base_short_signal and ob_confirmed and vol_confirmed and entry_ob and entry_ob['type'] == 'bearish':
                     side_to_enter = 'short'
                     entry_reason_full = f"RSI({last_rsi:.1f})>OB, Stoch({last_stoch_k:.1f},{last_stoch_d:.1f})>OB, {ob_reason}" + (", Vol Confirmed" if entry_volume_confirmation_enabled else "")
                else: # Log reason for no entry if close
                    if (base_long_signal or base_short_signal):
                        reasons = []
                        if not ob_confirmed: reasons.append("OB Price Invalid")
                        if not vol_confirmed and entry_volume_confirmation_enabled: reasons.append("Volume Low")
                        if reasons: log_debug(f"Entry Blocked: Base signal met but failed checks: {', '.join(reasons)}")
                ...
                if side_to_enter and entry_reason_full and entry_ob:
                    ...
                    if enable_atr_sl_tp:
                        ...
                        if last_atr:
                            ...
                            sl_price_calc = current_price - sl_dist if side_to_enter == 'long' else current_price + sl_dist
                            tp_price_calc = current_price + tp_dist if side_to_enter == 'long' else current_price - tp_dist
                            log_info(f"Calculated ATR SL/TP: SL={sl_price_calc:.{price_precision_digits}f}, TP={tp_price_calc:.{price_precision_digits}f}")
                        else:
                        ...
                        log_info(f"Calculated Fixed % SL/TP: SL={sl_price_calc:.{price_precision_digits}f}, TP={tp_price_calc:.{price_precision_digits}f}")
                    ...
                    if side_to_enter == 'long':
                        ...
                        if potential_ob_sl > sl_price_calc and potential_ob_sl < current_price:
                            ...
                            log_info(f"Adjusting SL tighter based on Bullish OB low: {adj_sl_price:.{price_precision_digits}f}")
                    else: # short
                        potential_ob_sl = ob_high + sl_buffer_ticks
                        if potential_ob_sl < sl_price_calc and potential_ob_sl > current_price:
                            adj_sl_price = potential_ob_sl
                            log_info(f"Adjusting SL tighter based on Bearish OB high: {adj_sl_price:.{price_precision_digits}f}")
                    ...
                    if (side_to_enter == 'long' and (final_sl_price_fmt >= current_price or final_tp_price_fmt <= current_price)) or \
                       (side_to_enter == 'short' and (final_sl_price_fmt <= current_price or final_tp_price_fmt >= current_price)):
                         log_error(f"Invalid final SL/TP for {side_to_enter.upper()}: SL={final_sl_price_fmt}, TP={final_tp_price_fmt}, Price={current_price}. Skipping.")
                         continue

                    # Calculate Size
                    entry_qty = calculate_position_size(exchange, symbol, current_price, final_sl_price_fmt, risk_percentage)
                    if entry_qty is None or entry_qty <= 0:
                        log_error(f"Failed to calculate valid position size. Skipping entry.")
                        continue
                    ...
                    if entry_order and entry_order.get('id'):
                        ...
                        if avg_price and avg_price > 0: entry_price_actual = float(avg_price)
                        if filled and filled > 0: filled_qty_actual = float(filled)
                        ...
                        if status == 'closed' or (status == 'open' and filled_qty_actual > 0):
                            ...
                            log_info(f"Entry order processed: ID {entry_order_id}, AvgPrice: {entry_price_actual:.{price_precision_digits}f}, Filled: {filled_qty_actual:.{amount_precision_digits}f}")
                        else:
                             log_error(f"Entry order {entry_order_id} status uncertain ('{status}', Filled: {filled}). Assuming failure.")
                    else:
                    ...
                    if entry_successful:
                        ...
                        if oco_id or sl_id: # Check if at least SL/OCO placed
                            position.update({
                                'status': side_to_enter, 'entry_price': entry_price_actual,
                                'quantity': filled_qty_actual, 'order_id': entry_order_id,
                                'stop_loss': final_sl_price_fmt, 'take_profit': final_tp_price_fmt,
                            ...
                            if not oco_id:
                                if not sl_id: log_error("Entry successful, but separate SL placement FAILED. POSITION UNPROTECTED.")
                                if not tp_id: log_warning("Entry successful, but separate TP placement FAILED.")
                            log_info(f"Position {entry_order_id} opened and state updated.")
                        else:
                            # Entry was successful, but SL/OCO placement failed! Critical.
                            log_error("CRITICAL: Entry order filled, but FAILED to place SL/OCO protection orders. Attempting to close position immediately.")
                            # Attempt emergency market close
                            emergency_close_side = 'sell' if side_to_enter == 'long' else 'buy'
                            emergency_close_order = place_market_order(exchange, symbol, emergency_close_side, filled_qty_actual, reduce_only=True)
                            if emergency_close_order and emergency_close_order.get('id'):
                                 log_warning(f"Emergency market close order placed (ID: {emergency_close_order.get('id')}). State not updated to reflect this failed entry.")
                            else:
                                 log_error("!!! EMERGENCY MARKET CLOSE FAILED !!! MANUAL INTERVENTION REQUIRED IMMEDIATELY !!!")
                            # Do NOT update position state to 'active' as it's unprotected or being closed.

                    else: # Entry failed
                        log_error(f"Entry attempt failed. Position state not updated.")
            ...
            log_info(f"Cycle complete ({pd.Timestamp.now(tz='UTC') - cycle_start_time}). Waiting {sleep_interval_seconds}s...")
            neon_sleep_timer(sleep_interval_seconds)
        ...
        # --- Graceful Shutdown ---
            ...
            if not SIMULATION_MODE:
                ...
                try:
                    ...
                    def fetch_open_orders_exit(exch, sym): return exch.fetch_open_orders(sym)
                    ...
                    if not open_orders:
                        ...
                        for order in open_orders:
                            ...
                            try:
                                log_info(f"Cancelling order ID: {o_id} ({order.get('type','?')}/{order.get('side','?')})...")
                                if cancel_order_with_retry(exchange, o_id, symbol):
                                     cancelled += 1
                                else: failed += 1 # cancel_order_with_retry returned False unexpectedly
                                time.sleep(0.3)
                            except Exception as cancel_e:
                        ...
                        if failed > 0: log_error("Manual check of open orders recommended.")
        ...
        # --- Main Loop Error Handling ---
            ...
            try: save_position_state()
            except Exception as save_e: log_error(f"Failed to save state during error: {save_e}")
            neon_sleep_timer(60)
