...
def print_neon_header() -> None:
    ...
    print(
        f"{NEON_PINK}{Style.BRIGHT}    Enhanced RSI/OB Trader Neon Bot - Configurable v1.1    {RESET}"
    )
    print(f"{NEON_CYAN}{'=' * 70}{RESET}")
...
def display_position_status(
    ...
    tsl_str: str = (
        f" | Active TSL: {tsl:.{price_precision}f}"
        if isinstance(tsl, (float, int))
        else ""
    ...
    if not oco_id and not sl_id and not tp_id and status is not None:
        print(
            f"    {NEON_YELLOW}Warning: Position active but no protection order IDs tracked.{RESET}"
        )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if last_exception:
                ...
                raise RuntimeError(
                    f"API call '{func.__name__}' failed unexpectedly."
                )  # Should not happen
    ...
    try:
        ...
        def validate(
            ...
            if condition and not condition(value):
                raise ValueError(
                    f"Config key '{key}' value ({value}) failed validation condition."
                )
        ...
        validate(
            "timeframe", str, lambda x: len(x) > 0
        )  # Could add check against exchange.timeframes later
        ...
        needs_atr = config_data.get("enable_atr_sl_tp") or config_data.get(
        ...
        validate(
            "retry_initial_delay", (float, int), lambda x: x > 0, is_required=False
        )
        validate(
            "retry_backoff_factor", (float, int), lambda x: x >= 1, is_required=False
        )
...
try:
    ...
    exchange_config: Dict[str, Any] = {
        ...
        "options": {
            "defaultType": "swap"
            if ":" in config["symbol"] or "SWAP" in config["symbol"].upper()
            else "spot",
...
try:
    ...
    if timeframe not in exchange.timeframes:
        ...
        log_error(
            f"Timeframe '{timeframe}' not supported by {exchange_id} for {symbol}. Available: {available_tf}"
        )
        sys.exit(1)
        ...
        try:
            min_tick = (
                ...
                if isinstance(price_prec, (float, int))
                    ...
                    exchange.price_to_precision(symbol, 10 ** (-price_precision_digits))
                )
            )
    ...
    log_error(
        f"An error occurred while validating symbol/timeframe or getting precision: {e}",
        exc_info=True,
    )
    sys.exit(1)
...
if SIMULATION_MODE:
    ...
    try:
        if sys.stdin.isatty():
            user_confirm = input(
                f"{NEON_RED}TYPE 'LIVE' TO CONFIRM or press Enter/Ctrl+C to exit: {RESET}"
            )
...
def load_position_state(filename: str = POSITION_STATE_FILE) -> None:
    ...
    if not os.path.exists(filename):
        log_info(
            f"No position state file found ({filename}). Starting with empty state."
        )
        return
    ...
    try:
        ...
        if entry_time_str and isinstance(entry_time_str, str):
            try:
                ...
                log_error(
                    f"Could not parse entry_time '{entry_time_str}' from state file. Setting to None."
                )
        ...
        if updated_count > 0:
            ...
            log_warning(
                f"Loaded state file {filename}, but applied 0 valid values. Using default state."
            )
            position = position_default_structure.copy()

    except json.JSONDecodeError as e:
        log_error(
            f"Error decoding JSON from state file {filename}: {e}. Starting with empty state.",
            exc_info=False,
        )
        position = position_default_structure.copy()
    except Exception as e:
        log_error(
            f"Error loading position state from {filename}: {e}. Starting with empty state.",
            exc_info=True,
        )
        position = position_default_structure.copy()
...
def fetch_ohlcv_data(
    ...
    try:
        ...
        if df.empty:
            log_warning(
                f"DataFrame empty after cleaning NaN OHLCV for {trading_symbol} ({tf})."
            )
            return None
        ...
        if not isinstance(e, RETRYABLE_EXCEPTIONS):
            log_error(
                f"Unexpected non-retryable error fetching OHLCV: {e}", exc_info=True
            )
        return None
    ...
    try:
        ...
        if len(completed_candles_df) >= min_periods_vol:
            rolling_vol = (
                ...
                .rolling(window=lookback_len, min_periods=min_periods_vol)
                .mean()
            )
            if not rolling_vol.empty and pd.notna(rolling_vol.iloc[-1]):
                avg_volume = rolling_vol.iloc[-1]
        if (
            avg_volume == 0 and len(completed_candles_df) > 0
        ):  # Fallback if rolling mean failed
        ...
        for i in range(start_index, end_index, -1):
            ...
            if (
                ...
                log_debug(
                    f"Potential Bullish OB found at {ob_candle.name.strftime('%Y-%m-%d %H:%M')}"
                )
                if bearish_ob:
                    break  # Found both
            ...
            # Bearish OB: Bullish OB candle, high-vol bearish impulse sweeping high
                ...
                log_debug(
                    f"Potential Bearish OB found at {ob_candle.name.strftime('%Y-%m-%d %H:%M')}"
                )
                if bullish_ob:
                    break  # Found both
...
def calculate_technical_indicators(
    ...
    if not all(col in df.columns for col in required_base):
        log_error(
            f"Missing required columns for indicators: Need {required_base}, got {df.columns.tolist()}"
        )
        return None
    ...
    try:
        ...
        if calc_vol_ma:
            if "volume" in df_processed.columns:
                ...
                df_processed[vol_ma_col] = (
                    df_processed["volume"]
                    .rolling(window=vol_ma_len, min_periods=min_p_vol)
                    .mean()
                )
        ...
        valid_indicator_cols = [
            col for col in indicator_cols if col in df_processed.columns
        ]
        ...
        log_debug(
            f"Indicator calculation complete. DataFrame has {len(df_processed)} rows."
        )
        return df_processed
...
def calculate_position_size(
    ...
    if current_price <= 0 or stop_loss_price <= 0:
        log_error(
            f"Invalid prices for size calculation: Entry={current_price}, SL={stop_loss_price}"
        )
        return None
    ...
    try:
        ...
        if quote_currency in balance:
            ...
            if isinstance(balance[quote_currency], dict):
                ...
                if (
                    available_balance == 0.0
                    and balance[quote_currency].get("total", 0.0) > 0
                ):
                    # Fallback to total if free is 0 but total exists (less safe)
                    available_balance = float(balance[quote_currency]["total"])
                    balance_source_info = (
                        f"balance['{quote_currency}']['total'] (used as free was 0)"
                    )
                    log_warning(
                        f"Using 'total' {quote_currency} balance ({available_balance}), 'free' was 0."
                    )
            ...
            for wallet_info in balance["info"]["result"]["list"]:
                if (
                    ...
                    available_balance = float(
                        wallet_info.get(
                            "availableToWithdraw", wallet_info.get("walletBalance", 0.0)
                        )
                    )
                    balance_source_info = (
        ...
        try:
            ...
            log_warning(
                f"Could not use exchange.amount_to_precision: {e}. Using manual rounding."
            )
        ...
        if quantity_adjusted <= 0:
            log_error(
                f"Calculated adjusted quantity ({quantity_adjusted}) is zero or negative."
            )
            return None
...
def place_market_order(
    ...
    try:
        ...
        if amount_formatted <= 0:
            log_error(
                f"Amount {amount} became zero or negative ({amount_formatted}) after precision. Cannot order."
            )
            return None
        ...
        if reduce_only and is_contract_market:
            ...
            log_warning(
                "ReduceOnly requested but market is not contract type. Ignoring param."
            )
        ...
        if order:
            ...
            price_str = (
                f"{avg_price:.{price_precision_digits}f}"
                if isinstance(avg_price, (float, int))
                else "N/A"
            )
            filled_str = (
                f"{filled:.{amount_precision_digits}f}"
                if isinstance(filled, (float, int))
                else "N/A"
            )
            cost_str = (
                f"{cost:.{price_precision_digits}f}"
        ...
        log_error(
            f"Insufficient funds for {side} {amount_formatted:.{amount_precision_digits}f} {trading_symbol}: {e}"
        )
        return None
        ...
        ccxt.OrderNotFound
    ) as e:  # Should be rare for market orders unless rejected instantly
        log_error(f"OrderNotFound error placing market order for {trading_symbol}: {e}")
        return None
    except ccxt.ExchangeError as e:
        log_error(
            f"Exchange error placing {side} market order for {trading_symbol}: {e}"
        )
        return None
    except Exception as e:
        if not isinstance(e, RETRYABLE_EXCEPTIONS):
            log_error(
                f"Unexpected error placing {side} market order: {e}", exc_info=True
            )
        return None
...
def place_sl_tp_orders(
    ...
    oco_ref_id: Optional[str] = (
        None  # ID referencing the OCO group or one of the orders
    )
    ...
    if (position_side == "long" and sl_price >= tp_price) or (
        ...
        log_error(
            f"Invalid SL/TP prices for {position_side.upper()} position: SL={sl_price}, TP={tp_price}"
        )
        return None, None, None
    ...
    try:
        ...
        stop_limit_possible = (
            ...
            or "STOP_LOSS_LIMIT"
            in exchange_instance.options.get("createOrderTypes", [])
        ...
        if stop_market_possible:
            ...
            sl_limit_raw = (
                sl_price_fmt - limit_offset_ticks
                if close_side == "sell"
                else sl_price_fmt + limit_offset_ticks
            )
            sl_limit_price = float(
                exchange_instance.price_to_precision(trading_symbol, sl_limit_raw)
            )
            log_warning(
            ...
            try:
                ...
                if exchange_instance.id == "binance":  # Example for Binance
                    ...
                    if SIMULATION_MODE:
                        ...
                        log_warning(
                            "!!! LIVE MODE: Placing real OCO order (Binance structure)."
                        )
                        ...
                        raise ccxt.NotSupported(
                            "Live OCO for Binance needs specific handling."
                        )
                ...
                if oco_ref_id:
                    ...
                    return (
                        ...
                        None,
                        oco_ref_id,
                    )  # Return OCO ref ID, clear separate IDs
                ...
                log_error(
                    f"OCO order placement failed: {e}. Falling back to separate SL/TP orders."
                )
                use_oco = False
            except Exception as e:
                log_error(
                    f"Unexpected error during OCO placement: {e}. Falling back to separate SL/TP orders.",
                    exc_info=True,
                )
                use_oco = False
        ...
        if not use_oco and sl_order_type is not None:
            ...
            try:
                ...
                sl_params_sep["stopPrice"] = (
                    sl_price_fmt  # Common param for stop orders
                )
                ...
                if SIMULATION_MODE:
                    ...
                    log_warning(
                        f"!!! LIVE MODE: Placing real separate {sl_order_type} SL order."
                    )
                    ...
                    if sl_order_result and sl_order_result.get("id"):
                        ...
                        log_error(
                            f"Separate SL order placement failed or did not return ID. Result: {sl_order_result}"
                        )
                ...
                log_error(
                    f"Unexpected error placing separate SL order: {e}", exc_info=True
                )
            ...
            try:
                ...
                if SIMULATION_MODE:
                    ...
                    if tp_order_result and tp_order_result.get("id"):
                        ...
                        log_error(
                            f"Separate TP order placement failed or did not return ID. Result: {tp_order_result}"
                        )
                ...
                log_error(
                    f"Unexpected error placing separate TP order: {e}", exc_info=True
                )
            ...
            if not sl_placed and not tp_placed:
                ...
                log_warning(
                    "Separate TP placed, but SL placement FAILED. POSITION PARTIALLY PROTECTED."
                )
            elif not tp_placed:
    ...
    # --- Handle Top-Level Errors (including decorator retries failing) ---
        ...
        log_error(
            f"Insufficient funds error during SL/TP placement (unexpected with reduceOnly): {e}"
        )
        return None, None, None
...
def check_position_and_orders(
    ...
    log_debug(
        f"Checking state for active {position['status']} position on {trading_symbol}..."
    )
    ...
    try:
        ...
        oco_order_id: Optional[str] = position.get(
            "oco_order_id"
        )  # Might be list ID or one of the orders
        ...
        if (
            position["status"] is not None
            and order_check_passed
            and is_contract
            and can_fetch_positions
        ):
            log_info(
                "Order check suggests closure or no orders tracked. Verifying actual position status..."
            )
            try:
                ...
                positions_data = fetch_positions_safely(
                ...
                if positions_data and isinstance(positions_data, list):
                    for pos_info in positions_data:
                        if (
                            isinstance(pos_info, dict)
                            and pos_info.get("symbol") == trading_symbol
                        ):
                            ...
                            if (
                                size_key not in pos_info
                                and "info" in pos_info
                                and isinstance(pos_info["info"], dict)
                            ):
                                # Check common 'info' fields (e.g., Bybit uses 'size')
                                size_key = pos_info["info"].get(
                                    "size", size_key
                                )  # Example fallback

                            pos_size_str = pos_info.get(
                                size_key, pos_info.get("info", {}).get(size_key, "0")
                            )
                            try:
                                ...
                                current_position_size = (
                                    0.0  # Assume zero if unparseable
                                )

                            # Check side if available to be extra sure
                            pos_side = pos_info.get(
                                "side", pos_info.get("info", {}).get("side", "unknown")
                            ).lower()
                            log_debug(
                                f"Fetched position data: Size={current_position_size}, Side={pos_side}"
                            )
                            ...
                            if abs(current_position_size) > (
                                ...
                                if (
                                    ...
                                    position_reset_required = True
                                    assumed_close_reason = (
                                        "State Mismatch (Position Fetch)"
                                    )
                                    # Cancel any potentially lingering tracked orders if mismatch found
                                    order_to_cancel_id = (
                                        sl_order_id or tp_order_id or oco_order_id
                                    )
                    ...
                    log_warning(
                        "Position check: fetch_positions returned empty or invalid data."
                    )
                ...
                log_warning(
                    "fetch_positions not supported by exchange. Cannot use position fallback check."
                )
                # Rely solely on order check result
                position_reset_required = order_check_passed and orders_were_tracked
            except Exception as e:
                log_error(
                    f"Error during fetch_positions check: {e}. Relying on order check.",
                    exc_info=False,
                )
                # Rely solely on order check result
                position_reset_required = order_check_passed and orders_were_tracked
        ...
        if position_reset_required:
            log_info(
                f"Position closure detected/assumed (Reason: {assumed_close_reason}). Resetting local state."
            )

            if order_to_cancel_id:
                log_info(
                    f"Attempting to cancel leftover {cancel_side_label} order: {order_to_cancel_id}"
                )
                try:
                ...
                log_debug(
                    "No corresponding order ID was tracked or needed cancellation."
                )
...
def update_trailing_stop(
    ...
    if position["status"] == "long":
        ...
        if current_price > highest_seen:
            ...
            log_debug(
                f"TSL: New high for long: {current_price:.{price_precision_digits}f}"
            )
        ...
        if current_price < lowest_seen:
            ...
            log_debug(
                f"TSL: New low for short: {current_price:.{price_precision_digits}f}"
            )
    ...
    if tsl_activated and potential_new_tsl_price is not None:
        try:
            ...
            log_error(
                f"TSL Update: Failed to format potential TSL price {potential_new_tsl_price}: {e}"
            )
            return
    ...
    if should_update_tsl and new_tsl_price_fmt is not None:
        ...
        if current_qty is None or original_tp_price is None:
            log_error(
                "TSL Update Error: Missing quantity or original TP price in state."
            )
            return
        ...
        try:
            ...
            cancel_success = cancel_order_with_retry(
                exchange_instance, old_order_id, trading_symbol
            )
            if cancel_success:
                log_info(
                    f"TSL Update: Old order {old_order_id} cancellation successful or order already closed."
                )
                time.sleep(1.0)
            else:
                log_error(
            ...
            if new_oco_id or new_sl_id:  # Check if at least SL/OCO part was successful
                ...
                position["stop_loss"] = (
                    new_tsl_price_fmt  # Update target SL to TSL price
                )
                position["current_trailing_sl_price"] = (
                    new_tsl_price_fmt  # Mark TSL as active
                )
                save_position_state()
                display_position_status(
                    position, price_precision_digits, amount_precision_digits
                ...
                if not new_tp_id and not new_oco_id:  # Warn if separate TP failed
                    log_warning(
                        "TSL Update: New SL placed, but separate TP placement failed."
                    )
            else:
                ...
                position["current_trailing_sl_price"] = (
                    None  # TSL no longer active/valid
                )
    ...
    while True:
        try:
            ...
            if position_was_reset:
                log_info(
                    "Position state reset by check (SL/TP/Closed). Checking for new signals."
                )
            ...
            if df_indicators is None or df_indicators.empty:
                log_warning(
                    "Indicator calculation failed or empty DataFrame. Waiting..."
                )
                neon_sleep_timer(sleep_interval_seconds)
                continue
            ...
            try:
                ...
                if (
                    needs_atr_calc
                    and atr_col
                    and atr_col in latest_data
                    and pd.notna(latest_data[atr_col])
                ):
                    ...
                    log_warning(
                        "ATR calculation required but column/value missing/NaN."
                    )
                ...
                if (
                    needs_vol_ma_calc
                    and vol_ma_col
                    and vol_ma_col in latest_data
                    and pd.notna(latest_data[vol_ma_col])
                ):
                    ...
                    log_debug(
                        "Volume MA calculation required but column/value missing/NaN."
                    )

                # Check for NaN in essential values
                if any(
                    math.isnan(v)
                    for v in [current_price, last_rsi, last_stoch_k, last_stoch_d]
                ):
                ...
                if execute_fallback_exit:
                    ...
                    if exit_qty is None or exit_qty <= 0:
                        ...
                        if order_id_to_cancel:
                            ...
                            try:
                                if not cancel_order_with_retry(
                                    exchange, order_id_to_cancel, symbol
                                ):
                                ...
                                if other_order_id and not position.get(
                                    ...
                                    if not cancel_order_with_retry(
                                        exchange, other_order_id, symbol
                                    ):
                                        cancellation_ok = False
                            except Exception as cancel_e:
                                log_error(
                                    f"Error cancelling protection orders during fallback: {cancel_e}",
                                    exc_info=False,
                                )
                                cancellation_ok = False
                        ...
                        if cancellation_ok:
                            ...
                            if fallback_exit_order and fallback_exit_order.get("id"):
                                ...
                                log_info(
                                    "Resetting local state after fallback exit attempt."
                                )
                                position.update(position_default_structure.copy())
                                save_position_state()
                                display_position_status(
                                ...
                                log_info(
                                    "Proceeding to next cycle after fallback exit."
                                )
                    ...
                    log_debug(
                        f"Monitoring {position['status'].upper()} position. Waiting for SL/TP/TSL."
                    )
            ...
            # === B. LOGIC TO CHECK FOR NEW ENTRIES (Only if position is None) ===
                ...
                if entry_volume_confirmation_enabled:
                    if (
                        ...
                        if current_volume > vol_thresh:
                            vol_confirmed = True
                            log_debug(
                                f"Volume Confirmed: Vol ({current_volume:.2f}) > Threshold ({vol_thresh:.2f})"
                            )
                        else:
                            log_debug(
                ...
                if base_long_signal and bullish_ob:
                    ...
                    if entry_zone_low <= current_price <= entry_zone_high:
                        ...
                        log_debug(
                            f"Price ({current_price:.{price_precision_digits}f}) outside Bullish OB."
                        )
                    ...
                    if entry_zone_low <= current_price <= entry_zone_high:
                        ...
                        log_debug(
                            f"Price ({current_price:.{price_precision_digits}f}) outside Bearish OB."
                        )
                ...
                if (
                    ...
                    if base_long_signal or base_short_signal:
                        ...
                        if reasons:
                            log_debug(
                                f"Entry Blocked: Base signal met but failed checks: {', '.join(reasons)}"
                            )
                ...
                if side_to_enter and entry_reason_full and entry_ob:
                    ...
                    if enable_atr_sl_tp:
                        if last_atr:
                            ...
                            log_error(
                                "Cannot calculate ATR SL/TP: Invalid ATR. Skipping entry."
                            )
                            continue
                    else:  # Fixed %
                        sl_mult = (
                            ...
                            if side_to_enter == "long"
                            else 1 + stop_loss_percentage
                        )
                        tp_mult = (
                            1 + take_profit_percentage
                            if side_to_enter == "long"
                            else 1 - take_profit_percentage
                        )
                    ...
                    final_sl_price = (
                        adj_sl_price if adj_sl_price is not None else sl_price_calc
                    )
                    ...
                    try:
                        ...
                        log_error(
                            f"Failed to format final SL/TP prices: {fmt_e}. Skipping entry."
                        )
                        continue
                    ...
                    if entry_qty is None or entry_qty <= 0:
                        log_error(
                            "Failed to calculate valid position size. Skipping entry."
                        )
                        continue
                    ...
                    entry_order = place_market_order(
                        exchange, symbol, entry_order_side, entry_qty, reduce_only=False
                    )
                    ...
                    if entry_order and entry_order.get("id"):
                        ...
                        if status == "closed" or (
                            status == "open" and filled_qty_actual > 0
                        ):
                        ...
                        log_error(
                            f"Entry market order placement failed. Result: {entry_order}"
                        )

                    # Place SL/TP and Update State ONLY if Entry Successful
                    if entry_successful:
                        log_info(
                            f"Entry successful. Placing protection orders for position {entry_order_id}..."
                        )
                        ...
                        if oco_id or sl_id:  # Check if at least SL/OCO placed
                            ...
                            if not oco_id:
                                ...
                                if not tp_id:
                                    log_warning(
                                        "Entry successful, but separate TP placement FAILED."
                                    )
                            log_info(
                                f"Position {entry_order_id} opened and state updated."
                            )
                        else:
                            ...
                            emergency_close_side = (
                                "sell" if side_to_enter == "long" else "buy"
                            )
            ...
            if not SIMULATION_MODE:
                ...
                try:
                    ...
                    if not open_orders:
                        ...
                        log_warning(
                            f"Found {len(open_orders)} open orders. Cancelling..."
                        )
                        ...
                        for order in open_orders:
                            ...
                            try:
                                ...
                                log_error(
                                    f"Failed to cancel order {o_id}: {cancel_e}",
                                    exc_info=False,
                                )
                                failed += 1
                        log_info(
                            f"Shutdown cancellation: Success/Closed={cancelled}, Failed={failed}"
                        )
                        if failed > 0:
                    ...
                    log_error(
                        f"Error fetching/cancelling orders on exit: {e}", exc_info=True
                    )
        ...
        # --- Main Loop Error Handling ---
            ...
            log_error(
                f"Network Error: {e}. Relying on retries within next cycle.",
                exc_info=False,
            )
            neon_sleep_timer(sleep_interval_seconds)
        except ccxt.ExchangeError as e:
...
if __name__ == "__main__":
    try:
        ...
        log_error(
            f"A critical error occurred during bot initialization: {main_exec_e}",
            exc_info=True,
        )
        print_shutdown_message()
        sys.exit(1)
