        ...
        try:
            ...
            if (
                ...
                if os.getenv("TERMUX_VERSION"):
                    ...
                    print(
                        f"{Style.BRIGHT}pip install {' '.join(_packages_to_install)}{Style.RESET_ALL}"
                    )
    ...
    **kwargs: Any,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_exception = e
            logger.error(
                f"{Fore.RED}Exchange error during {func_name}: {e}{Style.RESET_ALL}"
            )
            # Decide if specific ExchangeErrors are retryable - here we retry generic ones
            if attempt < max_retries:
                logger.warning(
                    f"{Fore.YELLOW}Retrying generic exchange error in {delay_seconds}s...{Style.RESET_ALL}"
                )
                time.sleep(delay_seconds)
            else:
                logger.error(
                    f"{Fore.RED}Max retries reached after exchange error for {func_name}.{Style.RESET_ALL}"
                )
                break
        # Catching general Exception is acceptable here to log truly unexpected errors (W0718)
            ...
            last_exception = e
            logger.error(
                f"{Fore.RED}Unexpected error during {func_name}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
...
class TradingConfig:
    ...
    def _get_env(
        ...
        log_method(
            f"{color}Using {key}: {log_value_display} (from {source}){Style.RESET_ALL}"
        )
...
class IndicatorCalculator:
    ...
    def _safe_to_float(x, col_name):
        ...
        if isinstance(x, str):
            try:
                ...
                logger.debug(
                    f"Could not convert string '{x}' in column '{col_name}' to float."
                )
                return float("nan")
        if x is None:
            return float("nan")
        ...
        logger.warning(
            f"Unexpected type {type(x)} in column '{col_name}', converting to NaN."
        )
        return float("nan")
    ...
    def calculate_indicators(
        ...
        if df is None or df.empty:
            logger.error(
                f"{Fore.RED}No DataFrame provided for indicators.{Style.RESET_ALL}"
            )
            return None
        ...
        if not all(c in df.columns for c in required_cols):
            ...
            logger.error(
                f"{Fore.RED}DataFrame missing required columns for indicators: {missing}{Style.RESET_ALL}"
            )
            return None
        ...
        try:
            ...
            if df_calc.empty:
                logger.error(
                    f"{Fore.RED}DataFrame empty after NaN drop during indicator calculation.{Style.RESET_ALL}"
                )
                return None
            ...
            logger.info(
                f"{Fore.GREEN}{Style.BRIGHT}Indicator patterns woven successfully.{Style.RESET_ALL}"
            )
        ...
        # Catching general Exception acceptable here for complex calc failure (W0718)
            ...
            logger.error(
                f"{Fore.RED}Error weaving indicator patterns: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
...
class SignalGenerator:
    ...
    def generate_signals(
        ...
        try:
            ...
            adx_dir_str = (
                "+DI"
                if adx_long_direction
                else ("-DI" if adx_short_direction else "NONE")
            )
            ...
            if final_long_signal:
                ...
                if not base_long_signal and not base_short_signal:
                    ema_status = (
                        ...
                        else ("<" if ema_bearish_cross else "=")
                    )
                    ...
                    reason_parts.append(
                        f"Long Blocked (Trend Filter: {trend_reason_str})"
                    )
                elif base_short_signal and not trend_allows_short:
                    reason_parts.append(
                        f"Short Blocked (Trend Filter: {trend_reason_str})"
                    )
        ...
        # Catching general Exception acceptable here for complex signal gen failure (W0718)
            ...
            logger.error(
                f"{Fore.RED}Error generating signals: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
...
class OrderManager:
    ...
    def _calculate_trade_parameters(
        ...
        # Catching general Exception acceptable here for unexpected calc failure (W0718)
            ...
            logger.error(
                f"Unexpected error calculating trade parameters for {side} side: {e}",
                exc_info=True,
            )
            return None
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"{Fore.RED}Order placement failed ({type(e).__name__}): {e}{Style.RESET_ALL}"
            )
            termux_notify(
        ...
        # Catching general Exception acceptable here for unexpected order failure (W0718)
            ...
            logger.error(
                f"{Fore.RED}Unexpected error placing market order: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
    ...
    def _set_position_protection(
        ...
        logger.trade(
            f"{Fore.CYAN}Attempting to {action_desc} for {position_side.upper()} {symbol}...{Style.RESET_ALL}"
        )
    ...
    def _verify_position_state(
        ...
        for attempt in range(max_attempts):
            ...
            if (
                not actual_side and short_pos_data
            ):  # Check actual_side instead of active_pos_data
            ...
            if expected_side is None:  # Expecting flat
                ...
                actual_state_str = (
                    ...
                    else f"{actual_side.upper()} Qty={actual_qty.normalize()}"
                )  # type: ignore
                ...
                actual_state_str = (
                    ...
                    if is_currently_flat
                        ...
                        f"{actual_side.upper()} Qty={actual_qty.normalize()}"
                        if actual_side
                        else "UNKNOWN"
                    )
                )
    ...
    def place_risked_market_order(
        ...
        if (
            not trade_params or "qty" not in trade_params
        ):  # Check qty exists and is not None
            logger.error(
        ...
        if qty_to_order is None or qty_to_order.is_nan() or qty_to_order <= 0:
            logger.error(
                f"Entry Aborted: Invalid quantity ({qty_to_order}) calculated."
            )
            return False
        ...
        avg_entry_price_str = (
            avg_entry_price.normalize() if not avg_entry_price.is_nan() else "[N/A]"
        )
        logger.info(
    ...
    def manage_trailing_stop(
        ...
        try:
            ...
            if should_activate_tsl:
                ...
                act_target_str = (
                    activation_price.normalize().to_eng_string()
                )  # Use eng string for formatting
                logger.trade(
    ...
    def close_position(
        ...
        avg_close_price_log = (
            avg_close_price.normalize()
            if not avg_close_price.is_nan()
            else "[Pending/N/A]"
        )
        logger.trade(
        ...
        if not verification_ok:
            ...
            if final_pos_state:  # Check the last known state if verification failed
                ...
                if (
                    long_data
                    and safe_decimal(long_data.get("qty")).copy_abs()
                    >= POSITION_QTY_EPSILON
                ):
                    lingering_side = "long"
                    lingering_qty = safe_decimal(
                        long_data.get("qty")
                    )  # Fixed indentation (W0311)
                elif (
                    short_data
                    and safe_decimal(short_data.get("qty")).copy_abs()
                    >= POSITION_QTY_EPSILON
                ):
            ...
            final_state_log = (
                "FLAT"
                if not lingering_side
                else f"{lingering_side.upper()} Qty={lingering_qty.normalize()}"
            )
            logger.error(
...
class StatusDisplay:
    ...
    def _add_indicator_info(
        self, panel_content: Text, indicators: Optional[Dict], price_dp: int
    ):
        ...
        def fmt_ind(
            ...
            dec_val = (
                safe_decimal(val)
                if isinstance(val, (Decimal, int, float, str, bool))
                else Decimal("NaN")
            )
        ...
        adx_style = (
            "yellow"
            if adx_is_valid and adx_val_dec > self.config.min_adx_level
            else "dim yellow"
        )
        ...
        if active_position_data and position_side_str:
            ...
            if tsl_active_from_pos:
                ...
                if tracked_protection != "ACTIVE_TSL":
                    prot_status_text.append(
                        "?", style="bright_yellow"
                    )  # Fixed multiple statements (C0321)
    ...
    def _add_signal_info(self, panel_content: Text, signals: Dict):
        ...
        if signals.get("long"):
            ...
            sig_style = "yellow"
        elif "No Signal:" not in str(sig_reason) and "Initializing" not in str(
            sig_reason
        ):
            sig_style = "white"
    ...
    def print_status_panel(
        ...
        self._add_position_info(
            panel_content, positions, protection_tracker, price_dp, amount_dp
        )
...
class TradingBot:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.critical(
                f"{Fore.RED}{Style.BRIGHT}TradingBot init failed: {e}. Halting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        ...
        logger.info(
            f"{Fore.GREEN}{Style.BRIGHT}Pyrmethus components initialized successfully.{Style.RESET_ALL}"
        )
    ...
    def _setup_signal_handlers(self):
        ...
        # Catching general Exception acceptable here for OS-level signal handling issues (W0718)
            ...
            logger.warning(
                f"{Fore.YELLOW}Could not set signal handlers (OS/thread issue?): {e}{Style.RESET_ALL}"
            )
    ...
    def _fetch_data_and_state(
        ...
        if df is None or df.empty:
            logger.error(
                f"{Fore.RED}Cycle Aborted (Cycle {cycle_count}): Market data fetch failed.{Style.RESET_ALL}"
            )
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
            logger.error(
                f"{Fore.RED}Invalid equity ({total_equity}). Logic might be skipped.{Style.RESET_ALL}"
            )
            status = "FAIL:FETCH_EQUITY"
        if current_positions is None:
            logger.error(
                f"{Fore.RED}Failed fetching positions. Logic might be skipped.{Style.RESET_ALL}"
            )
    ...
    def _calculate_indicators_and_signals(
        ...
        if indicators is None:
            logger.warning(
                f"{Fore.YELLOW}Indicator calculation failed.{Style.RESET_ALL}"
            )
            cycle_status = (
                "WARN:INDICATORS_FAILED"
                if cycle_status == "OK"
                else cycle_status + "|INDICATORS_FAILED"
            )
            signals["reason"] = "Skipped (Indicator Calc Failed)"
        elif current_atr.is_nan() or current_atr <= 0:
            logger.error(
                f"{Fore.RED}Invalid ATR ({current_atr}). Logic might be skipped.{Style.RESET_ALL}"
            )
            cycle_status = (
                "FAIL:INVALID_ATR"
                if cycle_status == "OK"
                else cycle_status + "|INVALID_ATR"
            )
            ...
            if can_gen_signals:
                ...
                cycle_status = (
                    "WARN:SIGNAL_GEN_SKIPPED"
                    if cycle_status == "OK"
                    else cycle_status + "|SIGNAL_SKIP"
                )
    ...
    def _manage_positions(
        ...
        if current_pos_side and indicators:
            ...
            if (
                ...
                if positions_after_tsl is None:
                    ...
                    cycle_status = (
                        "WARN:POS_REFETCH_TSL_FAIL"
                        if cycle_status == "OK"
                        else cycle_status + "|TSL_REFETCH_FAIL"
                    )
                else:
                    live_positions_state = (
                        positions_after_tsl  # Update live state for subsequent logic
                    )
                ...
                logger.warning(
                    f"Cannot manage TSL for {current_pos_side}: Invalid ATR ({current_atr})"
                )

        # --- Check for Signal-Based Exits ---
        exit_triggered_by_signal = False
        if (
            indicators and current_pos_side
        ):  # Only check exit if in position and indicators are valid
            ...
            if exit_reason:
                ...
                if not exit_triggered_by_signal:
                    cycle_status = (
                        "FAIL:EXIT_ORDER_FAILED"
                        if cycle_status == "OK"
                        else cycle_status + "|EXIT_FAIL"
                    )
                    logger.error(
                    ...
                    if positions_after_exit is None:
                        logger.error(
                            f"{Fore.RED}Failed re-fetching positions after signal exit.{Style.RESET_ALL}"
                        )
                        cycle_status = (
        ...
        if (
            is_flat
            and not exit_triggered_by_signal
            and indicators
            ...
            if (
                ...
                cycle_status = (
                    "FAIL:INVALID_STATE_PRE_ENTRY"
                    if cycle_status == "OK"
                    else cycle_status + "|PRE_ENTRY_FAIL"
                )
            else:
                ...
                if entry_successful:
                    ...
                    if positions_after_entry is not None:
                        live_positions_state = (
                            positions_after_entry  # Update state for panel
                        )
                    else:
                    ...
                    logger.error(
                        f"{Fore.RED}Entry sequence failed for {entry_side}.{Style.RESET_ALL}"
                    )
                    cycle_status = (
                        "FAIL:ENTRY_SEQUENCE"
                        if cycle_status == "OK"
                        else cycle_status + "|ENTRY_FAIL"
                    )
                    ...
                    if positions_after_failed_entry is not None:
                        live_positions_state = (
                            positions_after_failed_entry  # Update state for panel
                        )
                        if not (
                            live_positions_state.get("long")
                            or live_positions_state.get("short")
                        ):
                            self.order_manager.protection_tracker = {
                                "long": None,
                                "short": None,
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        (
            ...
            current_positions,
            cycle_status,
        ) = self._fetch_data_and_state(cycle_count)

        # Abort early if basic data fetching failed
        if cycle_status.startswith("FAIL:FETCH_OHLCV") or cycle_status.startswith(
            "FAIL:PROCESS_CANDLE"
        ):
        ...
        indicators, current_atr, signals, cycle_status = (
            self._calculate_indicators_and_signals(
                df,
                current_price,
        ...
        if (
            ...
            if current_positions is None:
                ...
                cycle_status = (
                    "FAIL:POS_UNAVAILABLE"
                    if cycle_status == "OK"
                    else cycle_status + "|POS_UNAVAILABLE"
                )
            if total_equity is None:
                logger.error("Core logic skipped: Equity data unavailable.")
                cycle_status = (
                    "FAIL:EQUITY_UNAVAILABLE"
                    if cycle_status == "OK"
                    else cycle_status + "|EQUITY_UNAVAILABLE"
                )
    ...
    def _close_lingering_positions(self) -> int:
        ...
        try:
            ...
            if final_positions is not None:
                ...
                if not positions_to_close:
                    logger.info(
                        f"{Fore.GREEN}No lingering positions found to close.{Style.RESET_ALL}"
                    )
                    return 0

                logger.warning(
                    f"Attempting emergency close for {len(positions_to_close)} lingering position(s)..."
                )
                ...
                termux_notify(
                    f"{self.config.symbol} Shutdown Issue",
                    "Failed pos check! Manual verify.",
                )
        ...
        # Catching general Exception acceptable here for critical shutdown step (W0718)
            ...
            termux_notify(
                f"{self.config.symbol} Shutdown Issue", f"Error closing pos: {e}"
            )
    ...
    def graceful_shutdown(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Cancel all orders response snippet: {str(cancel_resp)[:200]}..."
            )
            ...
            # Add check for other success indicators if needed, e.g., empty dict might be success
                ...
                logger.info(
                    "Cancel all orders returned empty dict, assuming success (no active orders?)."
                )
            else:
                logger.warning(
            ...
            logger.warning(
                "Exchange does not support cancel_all_orders with specified params. Skipping."
            )
        # Catching general Exception acceptable here for non-critical cancel step (W0718)
        except Exception as e:
            logger.error(
                f"{Fore.RED}Error cancelling orders: {e}{Style.RESET_ALL}",
                exc_info=False,
            )
        ...
        logger.info(
            f"{Fore.CYAN}Checking for lingering positions for {symbol} to close...{Style.RESET_ALL}"
        )
        ...
        termux_notify(
            "Shutdown Complete",
            f"{self.config.symbol} shutdown finished ({closed_count} pos closed).",
        )
