    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.error(f"{Fore.RED}Max retries ({max_retries + 1}) reached for {func_name} after retryable error. Last error: {e}")
    ...
    def _validate_value(self, key: str, value: Any, min_val: Optional[Union[int, float, Decimal]], max_val: Optional[Union[int, float, Decimal]], allowed_values: Optional[List[Any]]) -> bool:
        ...
        if allowed_values:
            ...
            if comp_value not in lower_allowed:
                logger.error(f"{Fore.RED}Validation failed for {key}: Invalid value '{value}'. Allowed: {allowed_values}.")
                return False
...
class ExchangeManager:
    ...
    def _format_v5_param(
        ...
        # Apply ROUND_DOWN specifically for amounts here if that's the standard requirement
        if param_type == "amount":
            formatted_str = self.format_amount(decimal_value, rounding_mode=ROUND_DOWN)
        else:
    ...
    def calculate_indicators(
        ...
        try:
            ...
            if len(df_calc) < min_required_len:
                logger.error(f"{Fore.RED}Insufficient data ({len(df_calc)} rows < required ~{min_required_len}) for indicators.")
                return None
...
class OrderManager:
    ...
    def _set_position_protection(
        ...
        try:
            ...
            if response and response.get("retCode") == V5_SUCCESS_RETCODE:
                ...
                logger.error(f"{Fore.RED}{action_desc} failed for {position_side.upper()} {symbol}. API Response: Code={ret_code}, Msg='{ret_msg}'")
            ...
            logger.error(f"{Fore.RED}Unexpected error during {action_desc} for {position_side.upper()} {symbol}: {e}", exc_info=True)
    ...
    def _verify_position_state(
        ...
        for attempt in range(max_attempts):
            ...
            if current_positions is None:
                logger.warning(f"{action_context} Verification Warning: Failed to fetch position state on attempt {attempt + 1}.")
                if attempt < max_attempts - 1:
                    time.sleep(delay_seconds)
                    continue
                else:
                    logger.error(f"{Fore.RED}{action_context} Verification Failed: Could not fetch position state after {max_attempts} attempts.")
            ...
            if attempt < max_attempts - 1:
                ...
                time.sleep(delay_seconds)
                 ...
                 logger.error(f"{Fore.RED}{action_context} Verification FAILED after {max_attempts} attempts. Final state check: {log_msg}")
    ...
    def place_risked_market_order(
        ...
        if not verification_ok:
            logger.error(f"{Fore.RED}Entry Failed: Position verification FAILED after market order {order_id}. Manual check required! Attempting cleanup...")
            self._handle_entry_failure(side, qty_to_order) # Attempt cleanup
            return False
        ...
        if not active_pos_data: # Should not happen if verification_ok is True, but safeguard
            logger.error(f"{Fore.RED}Internal Error: Position verified OK but data missing for {position_side} in final state {final_pos_state}. Halting entry.")
        ...
        if not set_stops_ok:
            logger.error(f"{Fore.RED}Entry Failed: Failed to set initial SL/TP after establishing position {position_side.upper()}. Attempting emergency close!")
    ...
    def close_position(
        ...
        logger.trade(f"{Fore.YELLOW}Attempting to CLOSE {position_side.upper()} pos ({qty_to_close.normalize()} {symbol}) | Reason: {reason}...")
        ...
        if not clear_stops_ok:
            logger.warning(f"{Fore.YELLOW}Failed to confirm protection clear via API for {position_side.upper()}. Proceeding cautiously...")
        else:
        ...
        logger.trade(f"{Fore.YELLOW}Closing market order ({close_order_id}) submitted for {position_side.upper()}. AvgClosePrice: {avg_close_price.normalize() if not avg_close_price.is_nan() else '[Pending/N/A]'}")
    ...
    def _handle_entry_failure(
        ...
        if current_positions is None:
            ...
            logger.error(f"{Fore.RED}Could not fetch positions during entry failure handling for {failed_entry_side}. MANUAL CHECK REQUIRED!")
        ...
        if lingering_pos_data:
            ...
            if current_qty.copy_abs() >= POSITION_QTY_EPSILON:
                logger.error(f"{Fore.RED}Lingering {position_side_to_check.upper()} pos (Qty: {current_qty.normalize()}) found after entry fail. Attempting emergency close.")
...
class StatusDisplay:
    ...
    def print_status_panel(
        ...
        if indicators:
            ...
            adx_dec = adx_val if isinstance(adx_val, Decimal) else safe_decimal(adx_val)
            adx_text.append(fmt_ind('adx', prec=1, style="yellow" if not adx_dec.is_nan() and adx_dec > self.config.min_adx_level else "dim yellow"))
        ...
        elif "No Signal:" not in sig_reason and "Initializing" not in sig_reason: sig_style = "white"
...
class TradingBot:
            ...
            if not self.shutdown_requested and sleep_duration > 0:
                ...
                try:
                    while time.monotonic() < sleep_end_time and not self.shutdown_requested:
                        time.sleep(min(0.5, sleep_duration)) # Check for shutdown request every 0.5s
                except KeyboardInterrupt:
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        try:
            ...
            logger.error(f"{Fore.RED}Cycle Aborted (Cycle {cycle_count}): Error processing latest candle: {e}", exc_info=False)
        ...
        if current_atr.is_nan() or current_atr <= 0:
            # Allow logic to run if indicators failed generally, but ATR is critical for risk
            if cycle_status == "WARN:INDICATORS_FAILED":
                logger.error(f"{Fore.RED}Invalid ATR ({current_atr}) despite indicator warning. Logic skipped (Cycle {cycle_count}).")
                cycle_status = "FAIL:INVALID_ATR"; can_run_trade_logic = False
            else: # If indicators were OK but ATR is bad (shouldn't happen with checks in calc)
        ...
        if can_run_trade_logic:
            ...
            # Re-fetch position state AFTER TSL check (only if TSL might have been activated)
            # Optimization: Only refetch if TSL activation was attempted or state might have changed
            # For simplicity and robustness, we refetch here if a position exists.
            if current_pos_side:
                ...
                if positions_after_tsl is None:
                    logger.error(f"{Fore.RED}Failed re-fetching positions after TSL check (Cycle {cycle_count}). Using potentially stale state.")
                    cycle_status = "WARN:POS_REFETCH_TSL_FAIL"
                else:
                    live_positions_state = positions_after_tsl; final_positions_for_panel = copy.deepcopy(live_positions_state)
                    active_long_pos = live_positions_state.get("long", {}); active_short_pos = live_positions_state.get("short", {})
                    long_qty = safe_decimal(active_long_pos.get("qty", "0")); short_qty = safe_decimal(active_short_pos.get("qty", "0"))
                    has_long_pos = long_qty.copy_abs() >= POSITION_QTY_EPSILON; has_short_pos = short_qty.copy_abs() >= POSITION_QTY_EPSILON
                    is_flat = not has_long_pos and not has_short_pos
            ...
            if can_gen_signals and current_pos_side:
                ...
                if exit_reason:
                    ...
                    logger.trade(f"{Fore.YELLOW}Attempting signal exit for {exit_side.upper()} (Qty: {qty_to_close_on_exit.normalize()}) | Reason: {exit_reason}")
                    ...
                    else: logger.info(f"Signal-based exit initiated/confirmed for {exit_side}.")
            ...
            if exit_triggered_by_signal and exit_side:
                ...
                if positions_after_exit is None:
                    ...
                    live_positions_state = positions_after_exit; final_positions_for_panel = copy.deepcopy(live_positions_state)
                    active_long_pos = live_positions_state.get("long", {}); active_short_pos = live_positions_state.get("short", {})
                    long_qty = safe_decimal(active_long_pos.get("qty", "0")); short_qty = safe_decimal(active_short_pos.get("qty", "0"))
                    has_long_pos = long_qty.copy_abs() >= POSITION_QTY_EPSILON; has_short_pos = short_qty.copy_abs() >= POSITION_QTY_EPSILON
                    is_flat = not has_long_pos and not has_short_pos
    ...
    def graceful_shutdown(self) -> None:
        ...
        logger.info(f"{Fore.CYAN}Cancelling active non-positional orders for {symbol} (Category: {self.config.bybit_v5_category})...")
        try:
        ...
        except Exception as e: logger.error(f"{Fore.RED}Error cancelling orders: {e}", exc_info=False)
                ...
                for side in ["long", "short"]:
                    ...
                    if pos_data:
                        ...
                        if qty.copy_abs() >= POSITION_QTY_EPSILON:
                            logger.warning(f"{Fore.YELLOW}Lingering {side.upper()} position (Qty: {qty.normalize()}) found.")
                            positions_to_close.append((side, qty))

                if not positions_to_close: logger.info(f"{Fore.GREEN}No lingering positions found.")
                else:
                    ...
                    for side, qty in positions_to_close:
                        ...
                        if close_success: closed_count += 1; logger.info(f"{Fore.GREEN}Closure initiated/confirmed for {side.upper()}.")
                        else: logger.error(f"{Fore.RED}Closure FAILED for {side.upper()}. MANUAL INTERVENTION REQUIRED.")

                    if closed_count == len(positions_to_close): logger.info(f"{Fore.GREEN}All detected positions ({closed_count}) closed/initiated.")
                    else: logger.warning(f"{Fore.YELLOW}Attempted {len(positions_to_close)} closures, {closed_count} succeeded/initiated. MANUAL CHECK NEEDED.")
            else:
