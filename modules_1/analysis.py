    ...
    class DummyTA:
        def __getattr__(self, name):
            def method(*args, **kwargs):
                ...
                logging.getLogger(__name__).error(f"pandas_ta not installed. Cannot call function '{name}'.")
...
try:
    ...
    print("ERROR: Failed importing components from utils in analysis.py.", file=sys.stderr)
    ...
    DEFAULT_INDICATOR_PERIODS = {}
    CCXT_INTERVAL_MAP = {}
    FIB_LEVELS = [Decimal(str(f)) for f in [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]]  # Basic Fib levels
...
class TradingAnalyzer:
    ...
    INDICATOR_CONFIG: Dict[str, Dict[str, Any]] = {
        ...
        "PSAR": {
            ...
            "multi_cols": {"PSAR_long": "PSARl_{initial}_{step}_{max}", "PSAR_short": "PSARs_{initial}_{step}_{max}"},
            "type": "decimal",
            "concat": True,
            "min_data": 2,
        },
    ...
    def __init__(
        ...
        self.indicator_values: Dict[str, Union[Decimal, float, None]] = {}  # Stores latest indicator values
        self.signals: Dict[str, int] = {"BUY": 0, "SELL": 0, "HOLD": 1}  # Default signal state
        ...
        self.df_calculated: pd.DataFrame = pd.DataFrame()  # Stores DataFrame with indicators

        # Pre-build map for indicator output types (Decimal/float)
        self.indicator_type_map: Dict[str, str] = {
            ind_key: details.get("type", "float") for ind_key, details in self.INDICATOR_CONFIG.items()
        }
        for details in self.INDICATOR_CONFIG.values():
            if "multi_cols" in details:
                sub_type = details.get("type", "float")  # Assume multi-cols have same type as parent
        ...
        if not self.weights:
            self.logger.warning(f"Weight set '{self.active_weight_set_name}' empty {self.symbol}.")
        ...
        if not self.df_calculated.empty and ta is not None:  # Check if pandas_ta was imported
            ...
            self.logger.error("pandas_ta not installed. Cannot calculate technical indicators.")
            self.indicator_values = {}
        else:
            self.logger.error(f"DataFrame prep failed {self.symbol}. No indicators calculated.")
            self.indicator_values = {}
    ...
    def _validate_and_prepare_df_calculated(self) -> None:
        ...
        for col in required_cols:
            if col not in self.df_calculated.columns:
                self.logger.critical(f"Column '{col}' missing for {self.symbol}. Invalidating DataFrame.")
            ...
            try:
                ...
                self.logger.critical(
                    f"Failed convert column '{col}' to numeric/float64 {self.symbol}: {e}", exc_info=True
                )
                self.df_calculated = pd.DataFrame()
            ...
            if self.df_calculated[col].isna().any():
                ...
                self.logger.warning(f"{nan_count} NaN(s) found in '{col}' after conversion {self.symbol}.")
        ...
        if self.df_calculated.empty:
            self.logger.error(f"DataFrame empty after dropping NaNs in OHLC {self.symbol}.")
        ...
        for ind_key, ind_details in self.INDICATOR_CONFIG.items():
            if enabled_cfg.get(ind_key.lower(), False):
                ...
                config_key = ind_details.get("params_map", {}).get(period_key) if period_key else None
                if config_key:
                    param_val = self.get_period(config_key)  # Use helper to get period/default
                    if isinstance(param_val, (int, float, Decimal)) and Decimal(str(param_val)) > 0:
    ...
    def get_period(self, key: str) -> Any:
        ...
        if config_val is not None:
            ...
            return config_val  # Return raw config value if no default or conversion failed
    ...
    def _calculate_volume_ma(self, df: pd.DataFrame, length: int) -> Optional[pd.Series]:
        ...
        if "volume" not in df.columns:
            self.logger.warning(f"Volume MA calc skipped {self.symbol}: 'volume' column missing.")
            return None
        if not (isinstance(length, int) and length > 0):
            self.logger.warning(f"Volume MA calc skipped {self.symbol}: Invalid length {length}.")
            return None
        # Ensure volume is float type for TA function
        volume_series = df["volume"].astype(float).fillna(0)
        if len(volume_series) < length:
            self.logger.debug(f"Insufficient data for Volume MA {length} on {self.symbol}.")
            return pd.Series(np.nan, index=df.index)  # Return Series of NaNs matching index
        try:
            ...
            self.logger.error(f"Error calculating SMA for volume {self.symbol}: {e}", exc_info=True)
            return None

    def _calculate_all_indicators(self) -> None:
        """Calculates all enabled indicators and stores results in df_calculated."""
        if self.df_calculated.empty:
            self.logger.warning(f"Skipping indicator calculation for {self.symbol}: DataFrame is empty/invalid.")
            return
        if ta is None:
            self.logger.error("Skipping indicator calculation: pandas_ta library is not available.")
            return
        ...
        for ind_key, ind_details in self.INDICATOR_CONFIG.items():
            ...
            for ta_param, cfg_key in ind_details.get("params_map", {}).items():
                ...
                try:  # Convert Decimal/str to float/int for pandas-ta/TA-Lib
                    if isinstance(param_value, Decimal):
                        ...
                        params_for_ta[ta_param] = float(param_value) if "." in param_value else int(param_value)
                    ...
                    is_params_valid = False
                    self.logger.error(f"Param conversion error for {cfg_key} ({param_value}): {e}")
                    break
            if not is_params_valid:
                self.logger.warning(f"Skipping indicator {ind_key} for {self.symbol} due to invalid parameters.")
                continue
            ...
            try:
                ...
                min_len = int(params_for_ta.get(min_len_key, ind_details.get("min_data", 1)))
                if len(df_work) < min_len:
                    self.logger.debug(f"Skip {ind_key}: data len {len(df_work)} < required {min_len}.")
                    continue
                ...
                if func_name == "_calculate_volume_ma":
                    ...
                    if func_args:  # If function signature expects specific columns
                        ...
                        result = func_obj(input_df, **params_for_ta)  # Requires function to handle DataFrame input
                ...
                col_name = (
                    self._format_ta_column_name(ind_details.get("main_col_pattern", ""), params_for_ta)
                ...
                if isinstance(result, pd.Series):
                    if col_name:  # Needs a defined column name pattern
                        ...
                        if should_concat:
                            ...
                            df_work = pd.concat(
                                [df_work, result.to_frame(name=col_name).astype("float64")], axis=1, copy=False
                            )
                        else:
                            # Assign Series as a new column (ensure float type)
                            df_work[col_name] = result.astype("float64")
                        self.ta_column_names[ind_key] = col_name  # Map internal key to the column name
                    else:
                        self.logger.error(f"Missing 'main_col_pattern' for Series result of {ind_key}.")
                    ...
                    if should_concat:
                        ...
                        try:
                            ...
                            for c in result.columns:
                                try:
                                    piece[c] = pd.to_numeric(result[c], errors="coerce").astype("float64")
                                except:
                                    self.logger.warning(f"Could not convert col {c} from {ind_key} result.")
                        # Drop columns that would be duplicated if they already exist in df_work
                        cols_to_drop = [c for c in piece.columns if c in df_work.columns]
                        if cols_to_drop:
                            self.logger.debug(f"Dropping duplicate columns before concat: {cols_to_drop}")
                        ...
                        self.logger.error(f"Indicator {ind_key} (concat=False) returned DataFrame.")
                        continue
                else:  # Unexpected result type
                    self.logger.warning(f"Indicator {ind_key} returned unexpected type: {type(result)}")
                    continue
                        ...
                        if actual_col_name in df_work.columns:
                            ...
                            partial_match = next(
                                (
                                    ...
                                    and all(p in col for p in map(str, params_for_ta.values()))
                                ),
                                None,
                            )
                            if partial_match:
                                ...
                                self.logger.debug(f"Available columns after {ind_key}: {list(df_work.columns)}")

            except Exception as e:
                self.logger.error(f"Error calculating indicator {ind_key} for {self.symbol}: {e}", exc_info=True)
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        if self.df_calculated.empty:
            self.logger.warning(f"Cannot update latest values {self.symbol}: calculated DataFrame is empty.")
            self.indicator_values = {}
            return
        try:
            ...
            all_keys = set(self.indicator_type_map.keys()) | set(ohlcv_map.keys()) | set(self.ta_column_names.keys())
            for key in all_keys:
            ...
            for int_key, actual_col in self.ta_column_names.items():
                if actual_col in latest_ind_row.index:
                    ...
                    target_type = self.indicator_type_map.get(int_key, "float")  # Default float
                    if pd.notna(val):
                        try:
                            latest_values[int_key] = Decimal(str(val)) if target_type == "decimal" else float(val)
                        except (InvalidOperation, TypeError, ValueError) as conv_err:
                            self.logger.debug(f"Conv err {int_key} ('{val}') to {target_type}: {conv_err}")
                            latest_values[int_key] = None  # Set to None on conversion error
            ...
            for disp_key, src_col in ohlcv_map.items():
                ...
                if pd.notna(val_ohlcv):
                    try:
                        ...
                        self.logger.debug(f"Conv err OHLCV {disp_key} ('{val_ohlcv}'): {conv_err}")
                        latest_values[disp_key] = None  # Set to None on conversion error
                # else keep None
            ...
            log_items = []
            keys_to_log = [CLOSE_KEY, ATR_KEY, EMA_SHORT_KEY, EMA_LONG_KEY, RSI_KEY, STOCHRSI_K_KEY, STOCHRSI_D_KEY]
            for k in keys_to_log:
                v = self.indicator_values.get(k)
                if v is not None:
                    prec = price_prec + 2 if k == ATR_KEY else price_prec if isinstance(v, Decimal) else 4
                    try:
            ...
            self.logger.error(f"Error update latest vals {self.symbol}: {e}", exc_info=True)
    ...
    def calculate_fibonacci_levels(self, window: Optional[int] = None) -> Dict[str, Decimal]:
        ...
        try:
            ...
            quant = min_tick if min_tick and min_tick > Decimal(0) else Decimal(f"1e-{price_prec}")

            if diff > Decimal(0):
                for level_pct_dec in FIB_LEVELS:
                    price_raw = period_high - (diff * level_pct_dec)
                    levels[f"Fib_{level_pct_dec * 100:.1f}%"] = (price_raw / quant).quantize(
                        Decimal("1"), rounding=ROUND_DOWN
                    ) * quant
            else:  # Flat price range
                level_q = (period_high / quant).quantize(Decimal("1"), rounding=ROUND_DOWN) * quant
                for level_pct_dec in FIB_LEVELS:
            ...
            self.logger.error(f"Fibonacci calculation error {self.symbol}: {e}", exc_info=False)  # Less verbose
    ...
    def get_nearest_fibonacci_levels(self, current_price: Decimal, num_levels: int = 5) -> List[Tuple[str, Decimal]]:
        ...
        try:
            ...
            self.logger.error(f"Error find nearest Fib {self.symbol}: {e}", exc_info=False)
            return []
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: Optional[Dict[str, Any]]) -> str:
        ...
        valid_core_count = sum(1 for key in self.INDICATOR_CONFIG if pd.notna(self.indicator_values.get(key)))
        ...
        if not (isinstance(current_price, Decimal) and current_price > 0):
            self.logger.warning(f"Invalid current price for signal gen {self.symbol}. HOLD.")
            return "HOLD"

        active_weights = self.weights
        if not active_weights:
            self.logger.warning(f"Weight set '{self.active_weight_set_name}' empty {self.symbol}. HOLD.")
            return "HOLD"
        ...
        for ind_key_lower, weight_val in active_weights.items():
            ...
            try:
                ...
                self.logger.error(f"Error in check method {method_name}: {e}", exc_info=True)

            debug_scores[ind_key_lower] = f"{score_float:.3f}" if pd.notna(score_float) else "NaN"
            if pd.notna(score_float):
                try:
                    ind_score = Decimal(str(score_float))
                    clamped = max(Decimal("-1"), min(Decimal("1"), ind_score))  # Clamp score
                    score += clamped * weight
                    w_sum += abs(weight)
                    active_checks += 1
                except (InvalidOperation, TypeError):
                    nan_checks += 1
                    self.logger.debug(f"Score conversion failed for {ind_key_lower}: {score_float}")
            else:
                nan_checks += 1
        ...
        if w_sum > 0:  # Only determine BUY/SELL if weights contributed
            ...
            self.logger.warning(f"No indicators contributed to score {self.symbol}. HOLD.")
    ...
    def _check_momentum(self) -> float:  # Simplified scaling
        ...
        try:
            ...
            score = mom_dec / close
            return float(max(Decimal("-1"), min(Decimal("1"), score * 10)))  # Example scaling
        except:
            return 0.0
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        if not all(isinstance(v, Decimal) and pd.notna(v) and v >= 0 for v in [vol, vol_ma]) or mult <= 0:
            return np.nan
        if vol_ma == 0:
            return 0.0  # Avoid division by zero
        ratio = vol / vol_ma
        return 1.0 if ratio > mult else -0.4 if ratio < (1 / mult if mult > 0 else 0) else 0.0
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if width > 0:
            ...
            return float(max(Decimal("-0.7"), min(Decimal("0.7"), pos * Decimal("0.7"))))
        return 0.0
    ...
    def calculate_entry_tp_sl(
        ...
        # Basic validation
        if signal not in ["BUY", "SELL"] or not isinstance(entry_price_estimate, Decimal) or entry_price_estimate <= 0:
            return entry_price_estimate, None, None

        atr = self.indicator_values.get(ATR_KEY)
        if not (isinstance(atr, Decimal) and atr > 0):
            self.logger.warning(f"TP/SL Calc {self.symbol}: ATR invalid ({atr}). Using default % if configured.")
            ...
            try:
                ...
                if default_atr_pct > 0:
                    ...
                    self.logger.error(f"Cannot calculate TP/SL {self.symbol}: ATR invalid and no valid default ATR %.")
                    return entry_price_estimate, None, None
            except:
                self.logger.error(
        ...
        try:
            ...
            if signal == "BUY":
                tp = ((entry_price_estimate + tp_offset) / quant).quantize(Decimal("1"), rounding=ROUND_UP) * quant
                sl = ((entry_price_estimate - sl_offset) / quant).quantize(Decimal("1"), rounding=ROUND_DOWN) * quant
            else:  # SELL
                tp = ((entry_price_estimate - tp_offset) / quant).quantize(Decimal("1"), rounding=ROUND_DOWN) * quant
                sl = ((entry_price_estimate + sl_offset) / quant).quantize(Decimal("1"), rounding=ROUND_UP) * quant
            ...
            self.logger.error(f"Error calculating TP/SL {self.symbol}: {e}", exc_info=False)  # Less verbose log
            return entry_price_estimate, None, None
        except Exception as e:  # Catch any other unexpected error
            self.logger.error(f"Unexpected error calculating TP/SL {self.symbol}: {e}", exc_info=True)
            return entry_price_estimate, None, None
