...
from utils import (CONFIG_FILE, DEFAULT_INDICATOR_PERIODS, NEON_RED,
                   ...
                   RESET, RETRY_DELAY_SECONDS, VALID_INTERVALS)
...
from utils import NEON_GREEN, NEON_RED, NEON_YELLOW, RESET, get_price_precision # <-- IMPORT ADDED
...
def calculate_position_size(
    ...
    try:
        ...
        if risk_value_decimal > 0 and risk_amount_quote <= 0 :
            lg.warning(f"Calculated risk amount {risk_amount_quote} {quote_currency} for {symbol} (risk {risk_value_decimal:.2%}) is non-positive. Balance: {balance:.4f}")
        ...
        try:
            ...
            if contract_size <= 0: contract_size = Decimal('1'); lg.warning(f"Contract size invalid for {symbol}, defaulted to 1.")
        except (InvalidOperation, ValueError, TypeError):
            contract_size = Decimal('1'); lg.warning(f"Contract size parse error for {symbol}, defaulted to 1.")
        ...
        if market_info.get('linear', True) or not is_contract: 
             ...
             else: lg.error(f"Pos sizing denom err ({symbol}): SLDist={sl_distance_per_unit}, ContrSize={contract_size}."); return None
        elif market_info.get('inverse', False): 
             if entry_price == 0 or initial_stop_loss_price == 0: 
                 lg.error(f"Pos sizing err ({symbol}): Entry or SL price zero for inverse contract."); return None
             loss_per_contract_in_quote = contract_size * abs(Decimal('1')/entry_price - Decimal('1')/initial_stop_loss_price)
             if loss_per_contract_in_quote > 0: calculated_size = risk_amount_quote / loss_per_contract_in_quote
             else: lg.error(f"Pos sizing err ({symbol}): Loss per contract zero/neg for inverse."); return None
        else: lg.error(f"Unsupported market type for sizing {symbol}. Market: {market_info.get('type')}"); return None

        if not (calculated_size and calculated_size > 0):
             lg.error(f"Initial size calc zero/neg: {calculated_size}. RiskAmt={risk_amount_quote:.4f}, SLDist={sl_distance_per_unit}, ContrSize={contract_size}")
             return None
        
        # Use get_price_precision for formatting the log output
        price_precision_for_log = get_price_precision(market_info, lg)
        lg.info(f"Position Sizing ({symbol}): Balance={balance:.2f}, Risk={risk_value_decimal:.2%}, RiskAmt={risk_amount_quote:.4f} {quote_currency}")
        lg.info(f"  Entry={entry_price:.{price_precision_for_log}f}, SL={initial_stop_loss_price:.{price_precision_for_log}f}, SL Dist Per Unit={sl_distance_per_unit:.{price_precision_for_log}f}")
        ...
        if min_amount_str is not None and str(min_amount_str).strip():
            try: 
                ...
                if val >= 0: min_amount = val # Allow 0 min_amount
            except InvalidOperation: lg.warning(f"Invalid min_amount_str '{min_amount_str}' for {symbol}. Using 0."); min_amount = Decimal('0')
        
        max_amount: Optional[Decimal] = None
        if max_amount_str is not None and str(max_amount_str).strip():
            try: 
                val = Decimal(str(max_amount_str))
                if val > 0 : max_amount = val 
            except InvalidOperation: lg.warning(f"Invalid max_amount_str '{max_amount_str}' for {symbol}. No upper amount limit.")

        min_cost = Decimal('0')
        if min_cost_str is not None and str(min_cost_str).strip():
            try: 
                val = Decimal(str(min_cost_str))
                if val >= 0: min_cost = val # Allow 0 min_cost
            except InvalidOperation: lg.warning(f"Invalid min_cost_str '{min_cost_str}' for {symbol}. Using 0."); min_cost = Decimal('0')

        max_cost: Optional[Decimal] = None
        if max_cost_str is not None and str(max_cost_str).strip():
            try: 
                val = Decimal(str(max_cost_str))
                if val > 0: max_cost = val
            except InvalidOperation: lg.warning(f"Invalid max_cost_str '{max_cost_str}' for {symbol}. No upper cost limit.")


        adjusted_size = calculated_size
        if min_amount > 0 and adjusted_size < min_amount:
             lg.warning(f"Calculated size {calculated_size:.8f} < min amount {min_amount}. Adjusting to min for {symbol}.")
             adjusted_size = min_amount
        if max_amount and adjusted_size > max_amount :
             lg.warning(f"Calculated size {calculated_size:.8f} > max amount {max_amount}. Adjusting to max for {symbol}.")
        ...
        if market_info.get('linear', True) or not is_contract:
             ...
             if cost_calc_price > 0 and contract_size > 0: estimated_cost = adjusted_size * contract_size / cost_calc_price
        
        if estimated_cost is None: lg.error(f"Could not estimate cost for {symbol} (price or contract size invalid)."); return None
        ...
        if min_cost > 0 and estimated_cost < min_cost:
             ...
             if market_info.get('linear', True) or not is_contract:
                 ...
                 cost_per_unit_denom = contract_size / cost_calc_price # This is value of 1 contract in base. We need cost per unit.
             ...
             if not (required_size_for_min_cost and required_size_for_min_cost > 0):
                 lg.error(f"Cannot calc required size for min_cost for {symbol} (denom/price invalid)."); return None
             ...
             if max_amount and required_size_for_min_cost > max_amount:
                  lg.error(f"Cannot meet min_cost for {symbol} without exceeding max_amount. Aborted."); return None
             if min_amount > 0 and required_size_for_min_cost < min_amount: # Check against actual min_amount, not calculated_size
                 lg.error(f"Req. size for min_cost {required_size_for_min_cost} < min_amount {min_amount}. Limits conflict for {symbol}. Aborted."); return None
             ...
             if estimated_cost is None: lg.error(f"Could not re-estimate cost for {symbol} after min_cost adj."); return None
        ...
        if max_cost and estimated_cost > max_cost: 
             ...
             if market_info.get('linear', True) or not is_contract:
                 ...
                 if cost_per_unit_denom_mc and cost_per_unit_denom_mc > 0: size_for_max_cost = max_cost / cost_per_unit_denom_mc
             ...
             if not (size_for_max_cost and size_for_max_cost > 0):
                 lg.error(f"Cannot calc max size for max_cost for {symbol}."); return None
             lg.info(f"  Reduced size by max_cost: {size_for_max_cost:.8f} {size_unit}")
             
             if min_amount > 0 and size_for_max_cost < min_amount:
                  lg.error(f"Size reduced for max_cost {size_for_max_cost} < min_amount {min_amount}. Aborted for {symbol}."); return None
        ...
        try:
            ...
            lg.error(f"Error applying CCXT amount_to_precision for {symbol} on size {adjusted_size}: {e_ccxt_prec}. Attempting manual quantization.")
        ...
        if not (final_size and final_size > 0):
             lg.error(f"Final position size zero or negative ({final_size}) after all adjustments for {symbol}. Aborted.")
             return None
        ...
        if market_info.get('linear', True) or not is_contract:
             if cost_calc_price_final > 0 and contract_size > 0: final_cost_est = final_size * cost_calc_price_final * contract_size
        elif market_info.get('inverse', False):
             if cost_calc_price_final > 0 and contract_size > 0: final_cost_est = final_size * contract_size / cost_calc_price_final
        
        if final_cost_est is not None and min_cost > 0 and final_cost_est < min_cost:
            lg.error(f"Final size {final_size} results in cost {final_cost_est:.4f} < min_cost {min_cost} for {symbol}. Aborted.")
            return None
...
class TradingAnalyzer:
    ...
    def _validate_and_prepare_df_calculated(self) -> None:
        ...
        for col in required_cols:
            ...
            if not pd.api.types.is_numeric_dtype(self.df_calculated[col]):
                self.logger.error(f"{NEON_RED}Column '{col}' is still not numeric after all processing for {self.symbol}. Type: {self.df_calculated[col].dtype}{RESET}")
    ...
    def _check_momentum(self) -> float:
        ...
        if scaling_factor == Decimal('0'): return 0.0 # Avoid division by zero if threshold_pct is very small
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        try:
            ...
            if ratio < (Decimal("1") / multiplier if multiplier > Decimal('0') else Decimal('0')): return -0.4 # Low volume
    ...
    def _check_stoch_rsi(self) -> float:
        ...
        score = 0.0
        if k_float < oversold_thresh and d_float < oversold_thresh: score = 0.8 # Base for oversold
        elif k_float > overbought_thresh and d_float > overbought_thresh: score = -0.8 # Base for overbought
        
        diff = k_float - d_float # K - D
        if score > 0 and diff > 0: score = 1.0 # K crossing up D in oversold is stronger buy
        elif score < 0 and diff < 0: score = -1.0 # K crossing down D in overbought is stronger sell
        elif abs(diff) > cross_thresh: # Significant cross outside OB/OS zones
            score = 0.6 if diff > 0 else -0.6
        elif k_float > d_float and score == 0.0 : score = 0.2 # K above D, not OB/OS
        elif k_float < d_float and score == 0.0 : score = -0.2 # K below D, not OB/OS
            
        if 40 < k_float < 60 and 40 < d_float < 60: score *= 0.5 # Dampen if in neutral zone
        return score
    ...
    def _check_cci(self) -> float:
        ...
        if moderate_os <= cci_float < 0: return 0.1 # Rising from moderate oversold
        if 0 < cci_float <= moderate_ob: return -0.1# Falling from moderate overbought
        return 0.0
    ...
    def _check_wr(self) -> float: # Williams %R
        ...
        if wr_float <= oversold: return 1.0   # Strongly oversold
        if wr_float >= overbought: return -1.0 # Strongly overbought
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        lower_bb, middle_bb, upper_bb, last_close = lower_bb_val, middle_bb_val, upper_bb_val, last_close_val # mypy cast

        if last_close <= lower_bb: return 1.0  # Touched/below lower band (potential buy)
        if last_close >= upper_bb: return -1.0 # Touched/above upper band (potential sell)
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict], current_price: Decimal) -> float: # current_price unused here but part of signature pattern
        ...
        try:
            ...
            total_bid_qty = sum(b[1] for b in bids[:num_levels] if len(b)==2 and isinstance(b[1], Decimal) and pd.notna(b[1]))
            total_ask_qty = sum(a[1] for a in asks[:num_levels] if len(a)==2 and isinstance(a[1], Decimal) and pd.notna(a[1]))
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if not (quantize_unit > Decimal('0')): quantize_unit = Decimal(f'1e-{price_precision}') # Safety for quantize_unit
