...
try:
    ...
    print(f"{Fore.YELLOW}Ensure 'config_models.py' is present and defines AppConfig, APIConfig, StrategyConfig, SMSConfig, LoggingConfig, load_config.{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Make sure StrategyConfig includes 'attach_sl_tp_to_entry', 'sl_trigger_by', 'tp_trigger_by', 'sl_order_type', 'close_on_stop'.{Style.RESET_ALL}")
    sys.exit(1)
...
class EhlersStrategyPybitEnhanced:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.contract_multiplier: Decimal = DECIMAL_CONTEXT.create_decimal("1.0") # For value calculation (esp. inverse)
        ...
        self.close_on_stop: bool = getattr(self.strategy_config, 'close_on_stop', False) # Option to close position on bot stop
        ...
        if self.sl_trigger_by not in valid_triggers:
            logger.warning(f"Invalid sl_trigger_by '{self.sl_trigger_by}'. Defaulting to '{TRIGGER_BY_MARK}'. Valid: {valid_triggers}")
            self.sl_trigger_by = TRIGGER_BY_MARK
        if self.tp_trigger_by not in valid_triggers:
            logger.warning(f"Invalid tp_trigger_by '{self.tp_trigger_by}'. Defaulting to '{TRIGGER_BY_MARK}'. Valid: {valid_triggers}")
            self.tp_trigger_by = TRIGGER_BY_MARK
        if self.sl_order_type not in valid_sl_types:
            logger.warning(f"Invalid sl_order_type '{self.sl_order_type}'. Defaulting to '{ORDER_TYPE_MARKET}'. Valid: {valid_sl_types}")
            self.sl_order_type = ORDER_TYPE_MARKET

        logger.info(f"{Fore.CYAN}Pyrmethus enhances the Ehlers Strategy for {self.symbol} (TF: {self.timeframe}) using Pybit HTTP...{Style.RESET_ALL}")
    ...
    def _initialize(self) -> bool:
        ...
        try:
            ...
            if time_diff > self.api_config.max_time_sync_diff: # Check against configured max diff
                 logger.critical(f"{Back.RED}CRITICAL: Client-Server time difference ({time_diff:.2f}s) exceeds allowed limit ({self.api_config.max_time_sync_diff}s). Ensure system clock is synchronized via NTP. Halting.{Style.RESET_ALL}")
                 send_sms_alert(f"CRITICAL: Clock skew error ({time_diff:.1f}s) for {self.symbol} bot! Halting.", self.sms_config)
                 self._safe_close_session()
                 return False
            elif time_diff > 5: # Warn if moderately high
                 logger.warning(f"{Fore.YELLOW}Client-Server time difference is {time_diff:.2f} seconds. Monitor clock synchronization.{Style.RESET_ALL}")
            ...
            if self.category in ['linear', 'inverse']:
                ...
                target_pybit_pos_mode = POSITION_MODE_ONE_WAY if pos_mode_target_str == 'MergedSingle' else POSITION_MODE_HEDGE
                if target_pybit_pos_mode == POSITION_MODE_HEDGE:
                     logger.error(f"{Back.RED}Hedge Mode (BothSide) is configured but NOT fully supported by this script's current logic. Ensure strategy accounts for separate Buy/Sell positions. Use 'MergedSingle' unless logic is adapted.{Style.RESET_ALL}")
                     # return False # Consider making this fatal

                if not self._set_position_mode(mode=target_pybit_pos_mode):
                     logger.warning(f"{Fore.YELLOW}Could not explicitly set position mode to '{pos_mode_target_str}'. Ensure it's correct in Bybit UI.{Style.RESET_ALL}")
                else:
                ...
                if not self._set_margin_mode(mode=margin_mode_target, leverage=leverage_str):
                     logger.warning(f"{Fore.YELLOW}Could not explicitly set margin mode to '{margin_mode_target}'. Ensure it's correct in Bybit UI.{Style.RESET_ALL}")
                else:
    ...
    def _fetch_and_set_market_info(self) -> bool:
                ...
                if response and response.get('retCode') == RET_CODE_OK:
                    ...
                    if result_list:
                        ...
                        if matched_data:
                             ...
                             logger.debug(f"Response received for category '{category_attempt}', but symbol {self.symbol} not in the list.")
                    else:
        ...
        if not found_category or not market_data:
            logger.error(f"{Back.RED}Failed to find market info for symbol {self.symbol} in any tried category: {possible_categories}.{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            self.contract_multiplier = safe_decimal_conversion(market_data.get('contractMultiplier', '1'), 'contract_multiplier', ctx) or ctx.create_decimal("1.0")

            # --- Validate Essential Details ---
            missing_details = []
            if self.min_qty is None or self.min_qty <= 0: missing_details.append(f"Minimum Quantity ({self.min_qty})")
            if self.qty_step is None or self.qty_step <= 0: missing_details.append(f"Quantity Step ({self.qty_step})")
            if self.price_tick is None or self.price_tick <= 0: missing_details.append(f"Price Tick ({self.price_tick})")
            if not self.base_coin: missing_details.append("Base Coin")
            if not self.quote_coin: missing_details.append("Quote Coin")
            if self.contract_multiplier is None or self.contract_multiplier <= 0: missing_details.append(f"Contract Multiplier ({self.contract_multiplier})")

            if missing_details:
                logger.error(f"{Back.RED}Failed to extract or validate essential market details! Issues: {'; '.join(missing_details)}{Style.RESET_ALL}")
    ...
    def _set_position_mode(self, mode: int) -> bool:
        ...
        logger.info(f"Attempting to set position mode for {self.symbol} (category {self.category}) to {mode} ({mode_desc})...")
        try:
            ...
            if "cannot be switched" in str(pybit_e.response).lower():
                 logger.warning("Could not switch position mode, likely due to active positions/orders. Ensure mode is correct.")
    ...
    def _set_margin_mode(self, mode: str, leverage: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Pybit API Error setting margin mode: {pybit_e} (Status: {pybit_e.status_code}, Response: {pybit_e.response})", exc_info=False)
            if "cannot be modified" in str(pybit_e.response).lower():
                 logger.warning("Could not switch margin mode, likely due to active positions/orders. Ensure mode is correct.")
                 return True # Treat as success (already set or cannot change)
            return False
        except Exception as e:
    ...
    def _get_available_balance(self) -> Optional[Decimal]:
        ...
        try:
            ...
            if coin_balance_data:
                ...
                logger.warning(f"Could not find balance details for coin '{coin_to_check}' within the response list for account {account_type}.")
        ...
        try:
            ...
            if not (pos_response and pos_response.get('retCode') == RET_CODE_OK):
                ...
                if not position_list:
                    ...
                    if pos_qty is not None and pos_qty > ctx.create_decimal("0.0") and side_str in [SIDE_BUY, SIDE_SELL]:
                        ...
                        if self.entry_price is None:
                            logger.warning(f"Position found ({side_str} {pos_qty_str}), but average price ('{avg_price_str}') is invalid. State may be inaccurate.")
                        ...
                        if self.current_side != old_side or self.current_qty != old_qty:
                             logger.info(f"Position State Changed: Side={self.current_side}, Qty={self.current_qty}, Entry={format_price(self.symbol, self.entry_price, self.price_tick) if self.entry_price else 'N/A'}")
            ...
            if not (kline_response and kline_response.get('retCode') == RET_CODE_OK):
                ...
                if not kline_list:
                    ...
                    if not ohlcv_df.empty:
                         ...
                         logger.warning("OHLCV data processing resulted in an empty DataFrame after handling errors/NaNs.")
    ...
    def _calculate_indicators(self, ohlcv_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        required_len = max(self.strategy_config.indicator_settings.evt_length, self.strategy_config.indicator_settings.atr_period) + 5
        if len(ohlcv_df) < required_len:
            logger.warning(f"Cannot calculate indicators: Insufficient data length ({len(ohlcv_df)} candles). Need at least {required_len}.")
            return None
        ...
        try:
            ...
            if nan_cols:
                 logger.warning(f"NaN values found in critical indicator columns of latest row ({latest_row.name}): {nan_cols}. Cannot generate reliable signal.")
    ...
    def _generate_signals(self, df_ind: pd.DataFrame) -> Optional[Literal['long', 'short']]:
        ...
        try:
            ...
            if not all(col in latest.index and pd.notna(latest[col]) for col in [buy_col, sell_col]):
                 logger.warning(f"EVT Buy/Sell signal columns missing or NaN in latest data ({latest_time}). Cannot generate signal.")
                 return None
            ...
            if buy_signal and sell_signal:
                logger.warning(f"Both Buy and Sell signals are active simultaneously on latest candle ({latest_time}). Ignoring signals.")
                return None
            elif buy_signal:
                logger.info(f"{Fore.GREEN}BUY signal generated based on EVT Buy flag at {latest_time}.{Style.RESET_ALL}")
                return POS_LONG
            elif sell_signal:
                logger.info(f"{Fore.RED}SELL signal generated based on EVT Sell flag at {latest_time}.{Style.RESET_ALL}")
                return POS_SHORT
            else:
                return None
    ...
    def _calculate_sl_tp(self, df_ind: pd.DataFrame, side: Literal['long', 'short'], entry_price_approx: Decimal) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        try:
            ...
            sl_price_adjusted = (stop_loss_price_raw / self.price_tick).quantize(ctx.create_decimal('0'), rounding=rounding_mode_sl) * self.price_tick
            ...
            if tp_multiplier > 0:
                ...
                if (side == POS_LONG and take_profit_price_raw <= entry_price_approx) or \
                   (side == POS_SHORT and take_profit_price_raw >= entry_price_approx):
                    ...
                    tp_price_adjusted_candidate = (take_profit_price_raw / self.price_tick).quantize(ctx.create_decimal('0'), rounding=rounding_mode_tp) * self.price_tick
    ...
    def _calculate_position_size(self, entry_price_approx: Decimal, stop_loss_price: Decimal) -> Optional[Decimal]:
        ...
        ctx = DECIMAL_CONTEXT
        # --- Input Validation ---
        if not all([self.qty_step, self.min_qty, self.price_tick, self.base_coin, self.quote_coin, self.contract_multiplier]):
        ...
        try:
            ...
            logger.debug(f"Available Balance: {available_balance:.4f} {self.quote_coin}, Risk %: {risk_percent:.2%}, Risk Amount: {risk_amount_quote:.4f} {self.quote_coin}")
            ...
            position_size_adjusted = (position_size_raw / self.qty_step).quantize(ctx.create_decimal('0'), rounding=ROUND_DOWN) * self.qty_step

            if position_size_adjusted <= ctx.create_decimal(0):
                 logger.warning(f"Calculated position size is zero after step adjustment. Raw: {position_size_raw}, Step: {self.qty_step}. Insufficient balance or risk settings too low?")
                 return None
            ...
            if position_size_adjusted < self.min_qty:
                logger.warning(f"Calculated size ({position_size_adjusted}) is below minimum allowed quantity ({self.min_qty}). Insufficient capital for configured risk, or risk % too low.")
                return None
    ...
    def _format_qty(self, qty: Optional[Decimal]) -> Optional[str]:
        ...
        """
        ctx = DECIMAL_CONTEXT
        if qty is None or qty < ctx.create_decimal(0): return None
    ...
    def _format_price_str(self, price: Optional[Decimal]) -> Optional[str]:
        ...
        """
        ctx = DECIMAL_CONTEXT
        if price is None or price <= ctx.create_decimal(0): return None
        if self.price_tick is None or self.price_tick <= ctx.create_decimal(0):
            logger.warning(f"Cannot format price: Invalid price_tick ({self.price_tick}). Returning raw string.")
            return str(price)

        try:
            quantized_price = (price / self.price_tick).quantize(ctx.create_decimal('0'), rounding=ROUND_HALF_UP) * self.price_tick
    ...
    def _place_order(self, params: Dict[str, Any]) -> Optional[Dict]:
        ...
        price_params = ['price', 'triggerPrice', 'stopLoss', 'takeProfit', 'slLimitPrice', 'tpLimitPrice'] # Add limit prices if used
        for p_name in price_params:
            if p_name in params and params[p_name] is not None:
                ...
                if not isinstance(price_str, str) or not price_str:
                    logger.error(f"Invalid '{p_name}' parameter type or empty value: {price_str}. Must be non-empty string.")
                    return None
                try:
        ...
        if params['orderType'] == ORDER_TYPE_LIMIT and 'price' in params: order_desc_parts.append(f"@ {params['price']}")
        if params.get('triggerPrice'): order_desc_parts.append(f"(Trig: {params['triggerPrice']} {params.get('triggerBy', '')})")
        ...
        if params.get('takeProfit'):
            ...
            tp_limit = f" Limit:{params['tpLimitPrice']}" if params.get('tpLimitPrice') else "" # Assuming tpOrderType is Limit implicitly or set
            order_desc_parts.append(f"TP: {params['takeProfit']}{tp_trig}{tp_limit}")
        if params.get('reduceOnly'): order_desc_parts.append("[ReduceOnly]")
        if params.get('timeInForce'): order_desc_parts.append(f"[{params['timeInForce']}]")
        if params.get('orderLinkId'): order_desc_parts.append(f"LinkID: {params['orderLinkId'][:10]}...")
        ...
        try:
            ...
            if ret_code == RET_CODE_OK:
                if order_id:
                    logger.success(f"{Fore.GREEN}✅ Order placed successfully! OrderID: {format_order_id(order_id)} (Latency: {latency:.3f}s){Style.RESET_ALL}")
                    return result_data
                else:
             ...
             logger.error(f"{Back.RED}Pybit API Error during order placement: {pybit_e}{Style.RESET_ALL}", exc_info=False)
             ...
             if pybit_e.status_code == 403 and "Timestamp" in str(pybit_e.response):
                 logger.critical(f"{Back.RED}Timestamp/recvWindow error. Check system clock sync and recv_window ({self.api_config.recv_window}ms).{Style.RESET_ALL}")
                 alert_msg += " (Timestamp/RecvWindow Error)"
             send_sms_alert(alert_msg, self.sms_config)
             return None
        except Exception as e:
            logger.error(f"Unexpected exception during order placement: {e}", exc_info=True)
            send_sms_alert(f"CRITICAL: Unexpected error placing {self.symbol} order: {type(e).__name__}", self.sms_config)
            return None

    def _cancel_single_order(self, order_id: Optional[str] = None, order_link_id: Optional[str] = None, reason: str = "Strategy Action") -> bool:
        """
        ...
        try:
            ...
            elif ret_code in [RET_CODE_ORDER_NOT_FOUND, RET_CODE_ORDER_NOT_FOUND_OR_CLOSED, RET_CODE_ORDER_CANCELLED_OR_REJECTED, RET_CODE_ORDER_FILLED] or \
                 any(msg in ret_msg for msg in ["order does not exist", "order not found", "already been filled", "already closed", "has been cancelled or rejected"]):
                 logger.warning(f"Order {log_id} not found or already inactive (Code: {ret_code}, Msg: {ret_msg}). Assuming cancellation is effectively successful.")
                 return True
            else:
                logger.error(f"Failed to cancel order {log_id}. Code: {ret_code}, Msg: {response.get('retMsg')}")
                send_sms_alert(f"ERROR: Failed to cancel {self.symbol} order {log_id}! Code:{ret_code}", self.sms_config)
                return False
            ...
            send_sms_alert(f"CRITICAL: API Error cancelling {self.symbol} order {log_id}! Status:{pybit_e.status_code}", self.sms_config)
            return False
        except Exception as e:
            logger.error(f"Unexpected exception cancelling order {log_id}: {e}", exc_info=True)
            send_sms_alert(f"CRITICAL: Unexpected error cancelling {self.symbol} order {log_id}: {type(e).__name__}", self.sms_config)
            return False
    ...
    def _cancel_all_open_orders(self, reason: str = "Strategy Action") -> bool:
        ...
        try:
            ...
            if ret_code_reg == RET_CODE_OK:
                ...
                logger.info(f"Cancelled {num_cancelled} regular open order(s) for {self.symbol}.")
                 ...
                 logger.error(f"Failed to cancel regular orders for {self.symbol}. Code: {ret_code_reg}, Msg: {response_reg.get('retMsg')}")
                 all_cancelled = False # Mark failure

        except (InvalidRequestError, FailedRequestError) as pybit_e:
            logger.error(f"Pybit API Error cancelling regular orders: {pybit_e} (Status: {pybit_e.status_code}, Response: {pybit_e.response})", exc_info=False)
            all_cancelled = False
        except Exception as e:
        ...
        if self.category in ['linear', 'inverse']:
            try:
                ...
                if ret_code_cond == RET_CODE_OK:
                    ...
                    logger.error(f"Failed to cancel conditional orders for {self.symbol}. Code: {ret_code_cond}, Msg: {response_cond.get('retMsg')}")
                    all_cancelled = False # Mark failure

            except (InvalidRequestError, FailedRequestError) as pybit_e:
                logger.error(f"Pybit API Error cancelling conditional orders: {pybit_e} (Status: {pybit_e.status_code}, Response: {pybit_e.response})", exc_info=False)
                all_cancelled = False
            except Exception as e:
        ...
        if not all_cancelled:
             ...
             send_sms_alert(f"WARNING: Failure cancelling some {self.symbol} orders during '{reason}'. Check UI!", self.sms_config)
            ...
            if should_exit:
                ...
                if close_order_result and close_order_result.get('orderId'):
                    ...
                    if state_updated and self.current_side == POS_NONE:
                        ...
                        logger.critical(f"{Back.RED}CRITICAL FAILURE TO CLOSE POSITION! State still shows {self.current_side} or failed to update. Manual intervention required!{Style.RESET_ALL}")
    ...
    def _handle_entry(self, signal: Literal['long', 'short'], df_ind: pd.DataFrame, current_price: Decimal) -> bool:
        ...
        order_link_id = f"{signal[:1]}_{self.symbol.replace('/','')}_{int(time.time()*1000)}"[-36:] # Max 36 chars for V5
        ...
        if self.current_side != expected_side:
            if self.current_side == POS_NONE:
                 logger.error(f"Entry order ({format_order_id(entry_order_id)}) placed, but state update shows NO position. Order likely rejected, zero-filled, or closed instantly. Check Bybit Order History.")
            else:
        ...
        if self.current_qty < min_expected_qty:
            logger.warning(f"Potential partial fill or state discrepancy for entry {format_order_id(entry_order_id)}. Ordered: {position_size}, Actual Position Qty: {self.current_qty}. Proceeding with actual quantity.")

        actual_entry_price = self.entry_price
        if actual_entry_price is None:
             logger.error("Position confirmed, but failed to get the average entry price from state update. SL/TP accuracy might be slightly affected if placed separately.")
    ...
    def _close_position_immediately(self, reason: str):
        ...
        logger.warning(f"{Fore.YELLOW}🚨 Attempting IMMEDIATE MARKET CLOSE of {self.current_side} position due to: {reason}{Style.RESET_ALL}")
        ...
        close_qty_str = self._format_qty(self.current_qty)
        if not close_qty_str:
             logger.critical(f"Failed to format current quantity {self.current_qty} for emergency close order ({reason}). Cannot close.")
             send_sms_alert(f"CRITICAL: Format error during emergency close of {self.symbol}! Manual check!", self.sms_config)
             return False
        ...
        if close_order_result and close_order_result.get('orderId'):
            closed_order_id = close_order_result['orderId']
            logger.success(f"{Fore.GREEN}✅ Emergency Close Market Order ({format_order_id(closed_order_id)}) placed successfully ({reason}).{Style.RESET_ALL}")
            send_sms_alert(f"INFO: Emergency close of {self.symbol} {self.current_side} position executed ({reason}).", self.sms_config)
            ...
            if self.current_side == POS_NONE:
                 ...
                 logger.critical(f"{Back.RED}Emergency close order placed, but state still shows position {self.current_side}! Manual check required!{Style.RESET_ALL}")
                 send_sms_alert(f"CRITICAL: Emergency close of {self.symbol} FAILED TO CONFIRM! Manual check!", self.sms_config)
                 return False
        else:
            logger.critical(f"{Back.RED}{Fore.WHITE}❌ Failed to place Emergency Close Market Order ({reason}). Manual intervention required!{Style.RESET_ALL}")
            send_sms_alert(f"CRITICAL: Failed to place emergency close order for {self.symbol}! Manual check!", self.sms_config)
            return False
        ...
        try:
            ...
            if self.current_side == POS_NONE and not exit_triggered:
                if current_price is None:
                 ...
                 logger.info(f"Monitoring {self.current_side.upper()} position ({pos_qty_display} @ {entry_price_display}). Waiting for exit signal or SL/TP.")
    ...
    def stop(self, initiated_by_user: bool = True):
        ...
        logger.warning(f"{Fore.YELLOW}--- Initiating Strategy Shutdown ({'User Request' if initiated_by_user else 'Internal Stop'}) ---{Style.RESET_ALL}")
        ...
        if self.close_on_stop and final_position_check_possible:
             ...
             if self.current_side != POS_NONE:
                 logger.warning(f"Attempting to close open {self.current_side} position due to 'close_on_stop' config...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if strategy: # Check if strategy object was successfully created
             ...
             strategy.stop(initiated_by_user=isinstance(sys.exc_info()[1], KeyboardInterrupt)) # Pass True if stopped by Ctrl+C
        else:
