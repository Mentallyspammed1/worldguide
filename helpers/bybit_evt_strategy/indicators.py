...
def calculate_levels(df_period: pd.DataFrame, current_price: float | None = None) -> dict[str, Any]:
    ...
    if len(df_period) >= 2:
        try:
            ...
            if not prev_row[required_cols].isnull().any():
                ...
                logger.warning("Previous candle data contains NaN, skipping pivot calculation.")
        except IndexError:
    ...
    try:
        ...
        if pd.notna(period_high) and pd.notna(period_low):
            ...
            logger.warning("Could not calculate Fib retracements due to NaN in period High/Low.")
    except Exception as e:
    ...
    try:
        ...
        if cp is not None and pd.notna(cp):
            ...
            logger.debug("Cannot classify S/R relative to current price/pivot (price or pivot is None/NaN).")
    except Exception as e:
...
def calculate_vwma(close: pd.Series, volume: pd.Series, length: int) -> pd.Series | None:
    ...
    if length <= 0:
        ...
        return None
    if len(close) < length:
        logger.debug(f"VWMA data length {len(close)} < period {length}. Result will have NaNs.")  # Allow calculation
    ...
    if flags.get("use_evt"):  # Generic flag or strategy-specific check
        try:
            ...
            if evt_len > 1 and evt_mult > 0:
                ...
                logger.warning(f"Invalid parameters for EVT (len={evt_len}, mult={evt_mult}), skipping.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # Example Config
    test_config = {
        "indicator_settings": {"min_data_periods": 50, "atr_period": 14, "evt_length": 7, "evt_multiplier": 2.5},
