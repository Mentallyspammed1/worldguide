...
def calculate_standard_pivot_points(
    ...
    if not all(
        isinstance(v, (int, float)) and pd.notna(v) for v in [high, low, close]
    ):  # Use pd.notna
        logger.warning("Invalid input for standard pivot points (NaN or non-numeric).")
        return {}
    if low > high:
        logger.warning(f"Low ({low}) > High ({high}) for pivot calc.")
    pivots = {}
    try:
        ...
        logger.error(f"Error calculating standard pivots: {e}", exc_info=True)
        return {}
    return pivots
...
def calculate_fib_pivot_points(
    ...
    if not all(
        ...
        return {}
    if low > high:
        logger.warning(f"Low ({low}) > High ({high}) for Fib pivot calc.")
    fib_pivots = {}
    try:
        pivot = (high + low + close) / 3.0
        fib_range = high - low
        if abs(fib_range) < 1e-9:
            logger.warning("Zero range, cannot calculate Fib Pivots accurately.")
            fib_pivots["PP"] = pivot
            return fib_pivots
        fib_pivots["PP"] = pivot
        ...
        fib_pivots["S3"] = pivot - (1.000 * fib_range)
        fib_pivots["R3"] = pivot + (1.000 * fib_range)
    except Exception as e:
        logger.error(f"Error calculating Fib pivots: {e}", exc_info=True)
        return {}
    return fib_pivots
...
def calculate_levels(
    ...
    if (
        ...
        logger.warning("Cannot calculate levels: Invalid DataFrame.")
        return levels
    ...
    if len(df_period) >= 2:
        try:
            prev_row = df_period.iloc[
                -2
            ]  # Use second to last row for previous candle HLC
            if not prev_row[required_cols].isnull().any():
                ...
                logger.warning(
                    "Previous candle data contains NaN, skipping pivot calculation."
                )
        except IndexError:
            logger.warning("IndexError calculating pivots (need >= 2 rows).")
        except Exception as e:
            logger.error(f"Error calculating pivots: {e}", exc_info=True)
    else:
        logger.warning("Cannot calculate pivots: Need >= 2 data points.")

    levels["standard_pivots"] = standard_pivots
    levels["fib_pivots"] = fib_pivots
    levels["pivot"] = (
        standard_pivots.get("PP") if standard_pivots else fib_pivots.get("PP")
    )
    ...
    try:
        ...
        if pd.notna(period_high) and pd.notna(period_low):
            ...
            if abs(period_diff) > 1e-9:
                ...
                logger.debug("Period range near zero, skipping Fib retracements.")
        else:
            logger.warning(
                "Could not calculate Fib retracements due to NaN in period High/Low."
            )
    except Exception as e:
        logger.error(f"Error calculating Fib retracements: {e}", exc_info=True)
    ...
    try:
        ...
        cp = (
            ...
            else levels.get("pivot")
        )

        if cp is not None and pd.notna(cp):
            all_levels = {
                **{f"Std {k}": v for k, v in standard_pivots.items() if pd.notna(v)},
                **{
                    f"Fib {k}": v
                    for k, v in fib_pivots.items()
                    if k != "PP" and pd.notna(v)
                },
            ...
            logger.debug(
                "Cannot classify S/R relative to current price/pivot (price or pivot is None/NaN)."
            )
    except Exception as e:
        logger.error(f"Error classifying S/R levels: {e}", exc_info=True)
...
def calculate_vwma(
    ...
    if length <= 0:
        logger.error(f"VWMA length must be positive: {length}")
        return None
    if len(close) < length:
        logger.debug(
            f"VWMA data length {len(close)} < period {length}. Result will have NaNs."
        )  # Allow calculation
...
def ehlers_volumetric_trend(
    ...
    if len(df) < length + 2:
        logger.warning(
            f"EVT skipped: Insufficient data rows ({len(df)}) for length {length}."
        )
        return df
    ...
    try:
        ...
        if len(df_out) > 0 and pd.notna(vwma_valid[0]):
            smoothed.iloc[0] = vwma_valid[0]
        if len(df_out) > 1 and pd.notna(vwma_valid[1]):
            smoothed.iloc[1] = vwma_valid[1]  # Simple init
        ...
        up_trend_cond = valid_comparison & (
            df_out[smooth_col] > shifted_smooth * mult_h
        )
        down_trend_cond = valid_comparison & (
        ...
        trend_shifted = df_out[trend_col].shift(
            1, fill_value=0
        )  # Previous period's trend (fill start with 0)
...
def calculate_all_indicators(df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        logger.error("Input DataFrame empty.")
        return pd.DataFrame()
    required_cols = ["open", "high", "low", "close", "volume"]
    if not all(col in df.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df.columns]
        logger.error(f"Input DataFrame missing: {missing}.")
        return df.copy()
    ...
    if len(df_out.dropna(subset=required_cols)) < min_rows:
        logger.warning(
            f"Insufficient valid rows ({len(df_out.dropna(subset=required_cols))}) < {min_rows}. Results may be NaN/inaccurate."
        )
    ...
    if PANDAS_TA_AVAILABLE:
        ...
        if flags.get("use_atr", False) and atr_len > 0:
            try:
                ...
                logger.error(f"Error calculating ATR({atr_len}): {e}", exc_info=False)
        ...
        logger.warning(
            "pandas_ta not available. Skipping standard indicators (ATR, etc.)."
        )
    ...
    if flags.get("use_evt"):  # Generic flag or strategy-specific check
        try:
            ...
            if evt_len > 1 and evt_mult > 0:
                ...
                logger.warning(
                    f"Invalid parameters for EVT (len={evt_len}, mult={evt_mult}), skipping."
                )
        except Exception as e:
            logger.error(f"Error calculating EVT: {e}", exc_info=True)
...
if __name__ == "__main__":
    print("-" * 60)
    print("--- Indicator Module Demo (v1.1) ---")
    print("-" * 60)
    ...
    print(
        f"NaNs in last row of added indicators ({len(added_cols)} cols): {last_row_nans}"
    )
