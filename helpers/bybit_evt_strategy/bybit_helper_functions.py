...
if not logger.hasHandlers():
    ...
    formatter = logging.Formatter(
        "%(asctime)s [%(levelname)s] {%(filename)s:%(lineno)d:%(funcName)s} - %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
    )
...
def _get_v5_category(market: dict[str, Any]) -> Literal["linear", "inverse", "spot", "option"] | None:
...
def set_leverage(exchange: ccxt.bybit, symbol: str, leverage: int, config: Config) -> bool:
    ...
    if leverage <= 0:
        logger.error(f"{Fore.RED}{log_prefix} Leverage must be positive: {leverage}{Style.RESET_ALL}")
        return False
    try:
        ...
        if err_code == 110043 or "leverage not modified" in err_str:
            ...
            logger.error(f"{Fore.RED}{log_prefix} ExchangeError setting leverage: {e}{Style.RESET_ALL}")
        ...
        send_sms_alert(
            f"[{symbol.split('/')[0]}] ERROR: Failed set leverage {leverage}x (Unexpected)", config.sms_config
        )
        return False
...
@retry_api_call(max_retries_override=3, initial_delay_override=1.0)
def fetch_usdt_balance(exchange: ccxt.bybit, config: Config) -> tuple[Decimal, Decimal] | None:
    ...
    try:
        ...
        if not unified_info:
            logger.warning(f"{log_prefix} Account type '{account_type_target}' not found in response.")
            return None
        ...
        if usdt_info:
            ...
            logger.warning(f"{log_prefix} {usdt_symbol} details not found in UNIFIED coin list.")

        if available is None:
            logger.warning(f"{log_prefix} Failed to parse available {usdt_symbol} balance. Assuming 0.")
        ...
        logger.critical(
            f"{Back.RED}{log_prefix} Unexpected error fetching balance: {e}{Style.RESET_ALL}", exc_info=True
        )
...
def place_market_order_slippage_check(
    ...
    effective_max_slippage = (
        max_slippage_pct_override if max_slippage_pct_override is not None else api_conf.default_slippage_pct
    )
    logger.info(
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category:
            logger.error(f"{Fore.RED}{log_prefix} Cannot determine category for {symbol}. Aborting.{Style.RESET_ALL}")
            return None
        ...
        if effective_max_slippage > 0:
            ...
            ob_analysis = analyze_order_book(
                exchange, symbol, api_conf.shallow_ob_fetch_depth, api_conf.order_book_fetch_limit, config
            )
            ...
            if best_bid and best_ask and best_bid > Decimal("0"):
                ...
                if spread_pct > effective_max_slippage:
                    ...
                    send_sms_alert(
                        f"[{market_base}] ORDER ABORT ({side.upper()}): High Spread {spread_pct:.4%}", config.sms_config
                    )
                    return None
            else:
                logger.warning(
        ...
        if client_order_id:
            ...
            if len(valid_coid) < original_len:
                logger.warning(f"{log_prefix} Client OID truncated: '{valid_coid}' (Orig len: {original_len})")
        ...
        logger.critical(
            f"{Back.RED}{log_prefix} Unexpected error placing market order: {e}{Style.RESET_ALL}", exc_info=True
        )
        send_sms_alert(
            f"[{market_base}] ORDER FAIL ({side.upper()} {action}): Unexpected {type(e).__name__}.", config.sms_config
        )
        return None
...
def cancel_all_orders(
    ...
    symbol.split("/")[0]
    log_prefix = f"[{func_name}({symbol}, Filter:{order_filter or 'All'}, Reason:{reason})]"
    logger.info(f"{Fore.CYAN}{log_prefix} Attempting cancellation...{Style.RESET_ALL}")
    try:
        market = exchange.market(symbol)
        category = _get_v5_category(market)
        if not category:
            logger.error(f"{Fore.RED}{log_prefix} Cannot determine category. Aborting.{Style.RESET_ALL}")
            return False
    ...
    try:
        ...
        while request_count < max_requests:
            ...
            for attempt in range(max_retries + 1):
                try:
                    candles_chunk = exchange.fetch_ohlcv(
                        symbol, timeframe, since=current_since, limit=fetch_limit, params=params
                    )
                    ...
                    last_fetch_error = e
                    logger.error(f"{log_prefix} Unexpected fetch chunk #{request_count} err: {e}", exc_info=True)
                    break
            ...
            if not candles_chunk:
                logger.debug(f"{log_prefix} No more candles returned (Chunk #{request_count}).")
                break
            ...
            if num_fetched < fetch_limit:
                logger.debug(f"{log_prefix} Received fewer candles than requested. End of data likely reached.")
                break
...
def _process_ohlcv_list(
    candle_list: list[list], parent_func_name: str, symbol: str, timeframe: str, max_candles: int | None = None
) -> pd.DataFrame | list[list] | None:
    ...
    try:
        ...
        if df.empty:
            logger.error(f"{Fore.RED}{log_prefix} Processed DataFrame is empty after cleaning.{Style.RESET_ALL}")
...
def place_limit_order_tif(
    ...
    try:
        ...
        if time_in_force == "PostOnly":
            ...
            logger.warning(f"[{func_name}] Unsupported TIF '{time_in_force}'. Using GTC.")
        ...
        if client_order_id:
            ...
            params["clientOrderId"] = valid_coid
            if len(valid_coid) < original_len:
                logger.warning(f"[{func_name}] Client OID truncated: '{valid_coid}' (Orig len: {original_len})")
        ...
        logger.error(f"{Fore.RED}{log_prefix}: API Error: {type(e).__name__} - {e}{Style.RESET_ALL}")
        send_sms_alert(
            f"[{symbol.split('/')[0]}] ORDER FAIL (Limit {side.upper()}): {type(e).__name__}", config.sms_config
        )
        if isinstance(e, ccxt.NetworkError):
            raise e  # Allow retry
        return None
...
def get_current_position_bybit_v5(exchange: ccxt.bybit, symbol: str, config: Config) -> dict[str, Any]:
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category or category not in ["linear", "inverse"]:
            logger.error(f"{Fore.RED}{log_prefix} Not a contract symbol.{Style.RESET_ALL}")
            return default_position
        if not exchange.has.get("fetchPositions"):
            logger.error(f"{Fore.RED}{log_prefix} fetchPositions not available.{Style.RESET_ALL}")
            return default_position
        ...
        if active_position_data:
            try:
                ...
                position_side = (
                    api_conf.pos_long
                    if side_std == "long"
                    else (api_conf.pos_short if side_std == "short" else api_conf.pos_none)
                )
                ...
                if position_side == api_conf.pos_none:
                    ...
                    position_side = (
                        api_conf.pos_long
                        if side_info == "Buy"
                        else (api_conf.pos_short if side_info == "Sell" else api_conf.pos_none)
                    )

                # Final check
                if position_side == api_conf.pos_none or quantity <= api_conf.position_qty_epsilon:
                    logger.info(f"{log_prefix} Position found but size/side negligible after parsing.")
                    return default_position
...
def close_position_reduce_only(
    ...
    try:
        ...
        if err_code in [110025, 110045, 30086] or any(
            ...
            send_sms_alert(
                f"[{market_base}] CLOSE FAIL ({live_side}): ExchangeError Code {err_code}", config.sms_config
            )
            return None
        ...
        logger.critical(
            f"{Back.RED}{log_prefix} Close Order Unexpected Error ({reason}): {e}{Style.RESET_ALL}", exc_info=True
        )
...
# --- fetch_funding_rate ---
@retry_api_call()
def fetch_funding_rate(exchange: ccxt.bybit, symbol: str, config: Config) -> dict[str, Any] | None:
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category or category not in ["linear", "inverse"]:
            logger.error(f"{Fore.RED}{log_prefix} Invalid category '{category}' for funding rate.{Style.RESET_ALL}")
            return None
        ...
        if processed_fr["nextFundingTime"]:
            try:
                processed_fr["nextFundingDatetime"] = (
                    pd.to_datetime(processed_fr["nextFundingTime"], unit="ms", utc=True).strftime(
                        "%Y-%m-%d %H:%M:%S %Z"
                    )
                    if PANDAS_AVAILABLE
                ...
                logger.warning(f"{log_prefix} Could not format next funding datetime: {dt_err}")
        ...
        logger.error(
            f"{Fore.RED}{log_prefix} Unexpected error fetching funding rate: {e}{Style.RESET_ALL}", exc_info=True
        )
        return None
...
def set_position_mode_bybit_v5(
    ...
    if target_mode_code is None:
        logger.error(f"{Fore.RED}{log_prefix} Invalid mode '{mode}'. Use 'one-way' or 'hedge'.{Style.RESET_ALL}")
        return False
    ...
    if symbol_or_category.lower() in ["linear", "inverse"]:
        ...
        try:
            ...
            logger.warning(f"{log_prefix} Could not get market/category for '{symbol_or_category}': {e}")
    ...
    try:
        ...
        # Code 34036: Already in the target mode (specific to UTA?)
            ...
            logger.info(f"{Fore.CYAN}{log_prefix} Mode already set to '{mode}' for {target_category}.{Style.RESET_ALL}")
            return True
...
def fetch_l2_order_book_validated(
    ...
    if not exchange.has.get("fetchOrderBook"):
        logger.error(f"{Fore.RED}{log_prefix} fetchOrderBook not supported.{Style.RESET_ALL}")
        return None
    ...
    try:
        ...
        if limit > max_limit:
            logger.warning(f"{log_prefix} Clamping limit {limit} to {max_limit} for category '{category}'.")
            limit = max_limit
        ...
        if conversion_errors > 0:
            logger.warning(f"{log_prefix} Skipped {conversion_errors} invalid OB entries.")
        if not validated_bids or not validated_asks:
            logger.warning(f"{log_prefix} Empty validated bids/asks.")  # Return potentially empty lists
...
def place_native_stop_loss(
    ...
    if amount <= api_conf.position_qty_epsilon or stop_price <= Decimal("0"):
        logger.error(f"{Fore.RED}{log_prefix}: Invalid amount/stop price.{Style.RESET_ALL}")
        return None
    ...
    try:
        ...
        if client_order_id:
            ...
            if len(valid_coid) < len(client_order_id):
                logger.warning(f"{log_prefix} Client OID truncated to orderLinkId: '{valid_coid}'")
        ...
        client_oid_resp = sl_order.get("info", {}).get("orderLinkId", params.get("orderLinkId", "N/A"))
        ...
        send_sms_alert(
            f"[{symbol.split('/')[0]}] SL PLACE FAIL ({side.upper()}): Unexpected {type(e).__name__}", config.sms_config
        )
        return None
...
def fetch_open_orders_filtered(
    ...
    try:
        ...
        if order_filter:
            ...
            if any(k in norm_type for k in ["stop", "trigger", "take", "tpsl", "conditional"]):
        ...
        logger.error(
            f"{Fore.RED}{log_prefix} Unexpected error fetching open orders: {e}{Style.RESET_ALL}", exc_info=True
        )
        return None
...
def calculate_margin_requirement(
    ...
    if amount <= 0 or price <= 0 or leverage <= 0:
        logger.error(f"{Fore.RED}{log_prefix} Invalid inputs (amount/price/leverage must be > 0).{Style.RESET_ALL}")
        return None, None
    ...
    try:
        ...
        if is_inverse:
            ...
            if price <= 0:
                raise ValueError("Price must be positive for inverse value calculation.")
        ...
        try:
            ...
            if mmr_rate_str is not None:
                ...
                if mmr_rate is not None and mmr_rate >= 0:
                    ...
                    logger.debug(f"{log_prefix} Could not parse valid MMR rate from '{mmr_rate_str}'.")
            else:
        ...
        logger.error(
            f"{Fore.RED}{log_prefix} Unexpected error during margin calculation: {e}{Style.RESET_ALL}", exc_info=True
        )
        return None, None
...
def fetch_ticker_validated(
    ...
    try:
        ...
        if timestamp_ms is None:
            raise ValueError("Ticker data is missing timestamp.")  # Fail early if TS missing
        ...
        if age_seconds > max_age_seconds:
            raise ValueError(f"Ticker data stale (Age: {age_seconds:.1f}s > Max: {max_age_seconds}s).")
        ...
        if bid_price and ask_price:
            ...
            logger.warning(f"{log_prefix} Cannot calculate spread due to missing bid/ask.")
    ...
    try:
        ...
        if isinstance(trailing_offset, str) and trailing_offset.endswith("%"):
            ...
            if not (percent_val and min_pct <= percent_val <= max_pct):
                raise ValueError(f"Percentage trail '{trailing_offset}' out of range ({min_pct}%-{max_pct}%).")
            ...
            if trailing_offset <= Decimal("0"):
                raise ValueError(f"Absolute trail delta must be positive: {trailing_offset}")
        ...
        activation_price_str = (
            format_price(exchange, symbol, activation_price) if activation_price is not None else None
        )
        if activation_price is not None and (activation_price_str is None or activation_price_str == "Error"):
        ...
        client_oid_resp = tsl_order.get("info", {}).get("orderLinkId", params.get("orderLinkId", "N/A"))
        status = tsl_order.get("status", "?")
        returned_trail = tsl_order.get("info", {}).get("trailingStop")
        returned_act = safe_decimal_conversion(tsl_order.get("info", {}).get("activePrice"))
...
@retry_api_call(max_retries_override=2, initial_delay_override=1.0)
def fetch_account_info_bybit_v5(exchange: ccxt.bybit, config: Config) -> dict[str, Any] | None:
    ...
    try:
        if hasattr(exchange, endpoint):
            ...
            logger.debug(f"{log_prefix} Raw Account Info response: {str(account_info_raw)[:400]}...")
            ...
            if ret_code == 0 and "result" in account_info_raw:
                ...
                raise ccxt.ExchangeError(f"Failed fetch/parse account info. Code={ret_code}, Msg='{ret_msg}'")
        else:
            ...
            if accounts:
                logger.info(f"{log_prefix} Fallback fetch_accounts(): {str(accounts[0])[:200]}...")
                return accounts[0]
            else:
                logger.error(f"{Fore.RED}{log_prefix} Fallback fetch_accounts() returned no data.{Style.RESET_ALL}")
                return None
    except (ccxt.NetworkError, ccxt.ExchangeError, ccxt.AuthenticationError) as e:
...
# --- validate_market ---
def validate_market(exchange: ccxt.bybit, symbol: str, config: Config, check_active: bool = True) -> dict | None:
    ...
    try:
        ...
        if not exchange.markets:
            logger.error(f"{Fore.RED}{log_prefix} Failed to load markets.{Style.RESET_ALL}")
            return None
        ...
        logger.error(f"{Fore.RED}{log_prefix} Validation Failed: Symbol not found. Error: {e}{Style.RESET_ALL}")
        return None
    except ccxt.NetworkError as e:
        logger.error(f"{Fore.RED}{log_prefix} Network error during market validation: {e}{Style.RESET_ALL}")
        return None  # Network errors are usually critical for validation
    except Exception as e:
        logger.error(f"{Fore.RED}{log_prefix} Unexpected error validating market: {e}{Style.RESET_ALL}", exc_info=True)
        return None
...
def fetch_recent_trades(
    exchange: ccxt.bybit, symbol: str, config: Config, limit: int = 100, min_size_filter: Decimal | None = None
) -> list[dict] | None:
    ...
    try:
        ...
        for trade in trades_raw:
            try:
                ...
                if (
                    not all([trade.get("id"), trade.get("timestamp"), trade.get("side"), price, amount])
                    or price <= 0
                    or amount <= 0
                ):
                    conversion_errors += 1
        ...
        if filtered_out_count > 0:
            logger.debug(f"{log_prefix} Filtered {filtered_out_count} trades smaller than {min_size_filter}.")
...
def update_limit_order(
    ...
    if all(v is None for v in [new_amount, new_price, new_trigger_price, new_client_order_id]):
        logger.warning(f"{log_prefix} No changes provided (amount, price, trigger, client ID). Aborting update.")
        return None
    ...
    if new_trigger_price is not None and new_trigger_price <= Decimal("0"):
        logger.error(f"{Fore.RED}{log_prefix}: Invalid new trigger price ({new_trigger_price}).")
        return None
    ...
    if new_trigger_price is not None:
        log_changes.append(f"TrigPx:{format_price(exchange, symbol, new_trigger_price)}")
    ...
    try:
        ...
        if new_client_order_id:
            ...
            if len(valid_coid) < len(new_client_order_id):
                logger.warning(f"{log_prefix} New Client OID truncated to orderLinkId: '{valid_coid}'")
...
@retry_api_call(max_retries_override=3, initial_delay_override=1.0)
def fetch_position_risk_bybit_v5(exchange: ccxt.bybit, symbol: str, config: Config) -> dict[str, Any] | None:
    ...
    try:
        ...
        if exchange.has.get("fetchPositionsRisk"):
            try:
                ...
                logger.warning(f"{log_prefix} fetch_positions_risk failed ({type(e).__name__}). Falling back.")
                position_data = None
        else:
        ...
        if position_data is None:
            if exchange.has.get("fetchPositions"):
                ...
                logger.error(f"{Fore.RED}{log_prefix} No position fetch methods available.{Style.RESET_ALL}")
                return default_risk

        if position_data is None:
            logger.error(f"{Fore.RED}{log_prefix} Failed fetch position data ({fetch_method_used}).{Style.RESET_ALL}")
            return default_risk
        ...
        for pos in position_data:
            ...
            if pos_symbol == market_id and pos_v5_side != "None" and pos_idx == 0:
                ...
                if pos_size is not None and abs(pos_size) > api_conf.position_qty_epsilon:
                    active_pos_risk = pos
                    logger.debug(f"{log_prefix} Found active One-Way pos risk data ({fetch_method_used}).")
                    break
        ...
        try:
            ...
            position_side = (
                ...
                else (api_conf.pos_short if side_std == "short" or side_info == "Sell" else api_conf.pos_none)
            )
...
def set_isolated_margin_bybit_v5(exchange: ccxt.bybit, symbol: str, leverage: int, config: Config) -> bool:
    ...
    if leverage <= 0:
        logger.error(f"{Fore.RED}{log_prefix} Leverage must be positive.{Style.RESET_ALL}")
        return False
    try:
        market = exchange.market(symbol)
        category = _get_v5_category(market)
        if not category or category not in ["linear", "inverse"]:
            logger.error(f"{Fore.RED}{log_prefix} Not a contract symbol ({category}).{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            exchange.set_margin_mode(
                marginMode="isolated", symbol=symbol, params={"category": category, "leverage": leverage}
            )
            logger.success(
        ...
        if not hasattr(exchange, endpoint):
            logger.error(f"{Fore.RED}{log_prefix} CCXT lacks '{endpoint}'.{Style.RESET_ALL}")
            return False
        ...
        logger.error(
            f"{Fore.RED}{log_prefix} Unexpected error setting isolated margin: {e}{Style.RESET_ALL}", exc_info=True
        )
        return False
...
if __name__ == "__main__":
    ...
    all_funcs = [
        ...
        if callable(obj) and not name.startswith("_") and name not in ["Config", "AppConfig"]
    ]
