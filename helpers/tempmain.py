...
async def main():
    ...
    test_exchange = None
    try:
        test_exchange = await bybit.initialize_bybit(full_config["API_CONFIG"])  # Pass only API config
        if test_exchange:
        ...
        logger.error(f"Isolated initialize_bybit failed with exception: {e}", exc_info=True)
    finally:
        # Cleanup
