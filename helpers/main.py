...
async def handle_signal(sig: signal.Signals):
    ...
    if shutdown_requested:
        ...
        if logger: logger.warning("Shutdown already requested, ignoring duplicate signal.")
        return
    ...
    if strategy_instance and getattr(strategy_instance, 'is_running', False):  # Check if running
         ...
         if logger: logger.info("Strategy instance exists but is not running.")
    else:
         if logger: logger.info("Strategy instance not found.")
    ...
    if main_task and not main_task.done():
        ...
        try:
             ...
             if logger: logger.info("Main task cancellation processed.")
        except asyncio.CancelledError:
              if logger: logger.info("Main task successfully cancelled.")
        except TimeoutError:
              if logger: logger.error("Timeout waiting for main task to cancel. Forcing exit might be needed.")
        except Exception as e:
             if logger: logger.error(f"Error during main task cancellation: {e}")
...
async def main():
    ...
    try:
        ...
        if not isinstance(log_third_party_level, int):
             print(f"Warning: Invalid THIRD_PARTY_LOG_LEVEL '{log_third_party_level_str}'. Defaulting to WARNING.", file=sys.stderr)
    ...
    try:
        ...
        logger.info(f"{Style.BRIGHT}--- Main Execution Block Finalizing (Run Success: {run_success}) ---{Style.RESET_ALL}")
        ...
        if exchange_instance_ref and hasattr(exchange_instance_ref, 'close') and callable(exchange_instance_ref.close):
             ...
             logger.warning("Exchange instance reference not available for final cleanup or close method missing (may be normal if init failed).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"{Back.RED}{Fore.WHITE}FATAL UNHANDLED ERROR during asyncio execution: {top_level_exc}{Style.RESET_ALL}", file=sys.stderr)
        ...
        print(f"{Fore.CYAN}--- Application Shutdown Complete (Total Runtime: {total_runtime:.2f}s) ---{Style.RESET_ALL}")
        if logger:
