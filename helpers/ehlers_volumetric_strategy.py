...
except ImportError: print("Warning: python-dotenv not found. Cannot load .env file.", file=sys.stderr); load_dotenv = lambda *args, **kwargs: False  # Dummy function
...
class EhlersStrategy:
    ...
    def _initialize(self) -> bool:
        ...
        try:
            ...
            if self.strategy_config.default_margin_mode == 'isolated':
                 ...
                 # Pass AppConfig
                 margin_set = self.bybit_helpers.set_isolated_margin_bybit_v5(self.exchange, self.symbol, self.strategy_config.leverage, self.app_config)
                 if not margin_set:
                      logger.warning(f"{Fore.YELLOW}Could not set ISOLATED margin mode. Ensure account allows it and no conflicting settings.{Style.RESET_ALL}")
                 else:
    ...
    def _update_state(self) -> bool:
        ...
        if not self.exchange: return False  # Ensure exchange is initialized
        try:
    ...
    def _verify_open_sl_tp(self):
        ...
        if self.sl_order_id:
            sl_order = self.bybit_helpers.fetch_order(self.exchange, self.symbol, self.sl_order_id, self.app_config, order_filter=sl_filter)
            if not sl_order or sl_order.get('status') not in ['open', 'new', 'untriggered']:  # Check for open/pending states
                 logger.warning(f"Tracked SL order {self.sl_order_id} no longer active (Status: {sl_order.get('status') if sl_order else 'Not Found'}). Clearing ID.")
                 self.sl_order_id = None
            else: logger.debug(f"Tracked SL order {self.sl_order_id} status: {sl_order.get('status')}")

        if self.tp_order_id:
             tp_order = self.bybit_helpers.fetch_order(self.exchange, self.symbol, self.tp_order_id, self.app_config, order_filter=tp_filter)
             if not tp_order or tp_order.get('status') not in ['open', 'new']:  # TP is limit, check open/new
                 logger.warning(f"Tracked TP order {self.tp_order_id} no longer active (Status: {tp_order.get('status') if tp_order else 'Not Found'}). Clearing ID.")
                 self.tp_order_id = None
             else: logger.debug(f"Tracked TP order {self.tp_order_id} status: {tp_order.get('status')}")

    def _fetch_data(self) -> tuple[pd.DataFrame | None, Decimal | None]:
        """Fetches OHLCV data and the latest ticker price."""
        if not self.exchange: return None, None
    ...
    def _calculate_sl_tp(self, df_ind: pd.DataFrame, side: Literal['buy', 'sell'], entry_price: Decimal) -> tuple[Decimal | None, Decimal | None]:
    ...
    def _cancel_all_open_orders(self, reason: str = "Strategy Action") -> bool:
        ...
        try:
            cancelled_reg = self.bybit_helpers.cancel_all_orders(self.exchange, self.symbol, self.app_config, reason=f"{reason} (Regular)", order_filter='Order')
            if not cancelled_reg: logger.warning("Failed to cancel all regular orders."); success = False
            else: logger.info("Regular orders cancelled successfully or none found.")
        except Exception as e:
            logger.error(f"Error cancelling regular orders: {e}", exc_info=True); success = False

        # Cancel stop/conditional orders
        try:
            cancelled_stop = self.bybit_helpers.cancel_all_orders(self.exchange, self.symbol, self.app_config, reason=f"{reason} (Stop)", order_filter='StopOrder')
            if not cancelled_stop: logger.warning("Failed to cancel all stop orders."); success = False
            else: logger.info("Stop orders cancelled successfully or none found.")
        except Exception as e:
            logger.error(f"Error cancelling stop orders: {e}", exc_info=True); success = False
    ...
    def _close_position(self, reason: str) -> bool:
        ...
        if close_order_result:
            ...
            if self.current_side == self.api_config.pos_none:
                 ...
                 logger.warning(f"Position still showing active after close order submission (Side: {self.current_side}, Qty: {self.current_qty}). Manual check may be needed.")
        ...
        try:
            ...
            if not should_exit:
                 if self.sl_order_id is None and self.current_side != self.api_config.pos_none:
                      should_exit = True; exit_reason = "Stop Loss Order likely hit (ID missing)"
                      logger.warning(f"{Fore.YELLOW}Assuming SL hit for {self.current_side} position as SL order ID is missing.{Style.RESET_ALL}")
                 elif self.tp_order_id is None and self.strategy_config.take_profit_atr_multiplier > 0 and self.current_side != self.api_config.pos_none:
                      # Only consider TP hit if TP was actually configured
                      should_exit = True; exit_reason = "Take Profit Order likely hit (ID missing)"
                      logger.warning(f"{Fore.YELLOW}Assuming TP hit for {self.current_side} position as TP order ID is missing.{Style.RESET_ALL}")

            # --- Execute Exit ---
            if should_exit:
                logger.warning(f"{Fore.YELLOW}EXIT Triggered for {self.current_side} {self.current_qty} {self.symbol}. Reason: {exit_reason}{Style.RESET_ALL}")
                ...
                if close_success:
                    ...
                    send_sms_alert(f"CRITICAL: Failed to close {self.symbol} position ({exit_reason}). Manual check needed!", self.sms_config)
    ...
    def _place_sl_tp(self, position_qty: Decimal, position_side: Literal['buy', 'sell'], stop_loss_price: Decimal, take_profit_price: Decimal | None) -> bool:
        ...
        order_side = self.api_config.side_sell if position_side == self.api_config.pos_long else self.api_config.side_buy
        ...
        if stop_loss_price is not None:
            try:
                ...
                if self.strategy_config.place_tpsl_as_limit:
                    logger.info(f"Placing SL as Reduce-Only LIMIT Order: Side={order_side}, Qty={position_qty}, Px={stop_loss_price}")
                    sl_order_details = self.bybit_helpers.place_limit_order_tif(
                        exchange=self.exchange, symbol=self.symbol, side=order_side,
                        amount=position_qty, price=stop_loss_price, config=self.app_config,
                        is_reduce_only=True, client_order_id=sl_client_oid, time_in_force='GTC'  # GTC is typical for SL/TP limit
                    )
                else:  # Native Stop Order
                    logger.info(f"Placing SL as NATIVE Stop Market Order: Side={order_side}, Qty={position_qty}, TriggerPx={stop_loss_price}")
                    sl_order_details = self.bybit_helpers.place_native_stop_loss(
                        exchange=self.exchange, symbol=self.symbol, side=order_side,
                        amount=position_qty, stop_price=stop_loss_price, config=self.app_config,
        ...
        if take_profit_price is not None:
            try:
                ...
                logger.info(f"Placing TP as Reduce-Only LIMIT Order: Side={order_side}, Qty={position_qty}, Px={take_profit_price}")
                tp_order_details = self.bybit_helpers.place_limit_order_tif(
                    exchange=self.exchange, symbol=self.symbol, side=order_side,
                    amount=position_qty, price=take_profit_price, config=self.app_config,
                    is_reduce_only=True, client_order_id=tp_client_oid, time_in_force='GTC'
                )
        ...
        if not tp_success:
             logger.warning(f"{Fore.YELLOW}Failed to place Take Profit order. Position remains open with SL only.{Style.RESET_ALL}")
    ...
    def _handle_entry(self, entry_signal: Literal['buy', 'sell'], current_price: Decimal, df_ind: pd.DataFrame):
        ...
        if not self.exchange: return
        if self.current_side != self.api_config.pos_none:
            logger.info(f"Entry signal '{entry_signal}' received, but already in {self.current_side} position. Ignoring.")
            return

        logger.warning(f"{Style.BRIGHT}--- Attempting {entry_signal.upper()} Entry for {self.symbol} ---{Style.RESET_ALL}")
        ...
        entry_order = self.bybit_helpers.place_market_order_slippage_check(
            exchange=self.exchange, symbol=self.symbol, side=entry_signal,
            amount=position_size, config=self.app_config, client_order_id=entry_client_oid
        )
        ...
        if not self._update_state():  # Fetch position again
             ...
             send_sms_alert(f"CRITICAL: Failed state update after {self.symbol} entry. SL/TP NOT PLACED!", self.sms_config)
        ...
        if self.current_side == self.api_config.pos_none or self.current_qty <= Decimal(0):
            logger.error(f"Entry FAILED: Position not detected after market order {format_order_id(entry_order_id)} submission.")
        ...
        if self.current_side != (self.api_config.pos_long if entry_signal == 'buy' else self.api_config.pos_short):
             logger.error(f"Entry MISMATCH: Intended {entry_signal}, but position is {self.current_side}. Aborting SL/TP.")
        ...
        logger.info(f"Confirmed Entry: Side={self.current_side}, Qty={actual_position_qty}, AvgPrice={actual_entry_price}")
    ...
    def run_loop(self):
        ...
        logger.info(f"{Fore.GREEN}{Style.BRIGHT}>>> Strategy Loop Started ({self.symbol} / {self.timeframe}) <<< {Style.RESET_ALL}")
        ...
        while self.is_running:
            ...
            try:
                ...
                logger.critical(f"{Back.RED}CRITICAL UNHANDLED EXCEPTION in main loop: {e}{Style.RESET_ALL}", exc_info=True)
...
if __name__ == "__main__":
    ...
    logger.info(f"Using Project Config: Testnet={app_config.api_config.testnet_mode}, Symbol={app_config.api_config.symbol}")
    logger.info("=" * 60)
