...
try:
    ...
    print(f"{warn_fore}Ensure bybit_helpers.py, indicators.py, neon_logger.py, and bybit_utils.py are accessible, compatible, and contain necessary async functions.{reset_all}")
    sys.exit(1)
...
class Config:
    ...
    def __init__(self):
        ...
        # Order Settings
        self.DEFAULT_SLIPPAGE_PCT: Decimal = Decimal(os.getenv("DEFAULT_SLIPPAGE_PCT", "0.005"))  # Allowed slippage for market orders
        ...
        self.EVT_MULTIPLIER: float = float(os.getenv("EVT_MULTIPLIER", 2.5))  # Note: float, might be better as Decimal if precision critical in indicator
        ...
        self.POSITION_CONFIRM_DELAY_SECONDS: int = int(os.getenv("POSITION_CONFIRM_DELAY", 7))  # Wait after entry order before checking position
        ...
        if self.ENABLE_SMS_ALERTS and not all([self.SMS_RECIPIENT_NUMBER, self.TWILIO_ACCOUNT_SID, self.TWILIO_AUTH_TOKEN, self.TWILIO_PHONE_NUMBER]):
            print(f"{Fore.YELLOW}Warning: SMS alerts enabled, but one or more Twilio config variables (SID, TOKEN, NUMBER) or recipient number are missing.{Style.RESET_ALL}")
...
def calculate_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame | None:
    ...
    try:
        ...
        if atr_col_name in missing_cols and config.analysis_flags.get("use_atr", False):
            logger.warning(f"{Fore.YELLOW}ATR column '{atr_col_name}' missing. Attempting fallback calculation...{Style.RESET_ALL}")
            if all(c in df_with_indicators for c in ['high', 'low', 'close']):
                try:
                    ...
                    if atr_result is not None and not atr_result.empty:
                        ...
                        logger.info(f"{Fore.CYAN}Calculated missing ATR column '{atr_result.name}' using pandas_ta.{Style.RESET_ALL}")
                    ...
                    logger.error(f"{Fore.RED}Cannot perform fallback ATR calculation: 'pandas_ta' not installed. Run 'pip install pandas_ta'.{Style.RESET_ALL}")
                except Exception as atr_err:
                    logger.error(f"{Fore.RED}Error during fallback ATR calculation: {atr_err}{Style.RESET_ALL}", exc_info=True)
            else:
                logger.error(f"{Fore.RED}Cannot calculate fallback ATR: Missing 'high', 'low', or 'close' columns.{Style.RESET_ALL}")

        # --- Final Check for Missing Columns ---
        if missing_cols:  # Check again after potential fallback
            logger.error(f"{Fore.RED}Required indicator columns remain missing after calculation: {', '.join(missing_cols)}. Cannot proceed with signal generation or SL calculation.{Style.RESET_ALL}")
            return None

        logger.debug(f"Indicators calculated. DataFrame shape: {df_with_indicators.shape}. Columns: {', '.join(df_with_indicators.columns)}")
...
def generate_signals(df_ind: pd.DataFrame, config: Config) -> str | None:
    ...
    try:
        if len(df_ind) == 0:
             logger.warning(f"{Fore.YELLOW}Cannot generate signals: DataFrame has 0 rows after indicator calculation.{Style.RESET_ALL}")
             return None
        ...
        if not all(col in latest.index for col in required_evt_cols):
            ...
            logger.warning(f"{Fore.YELLOW}EVT signal columns missing ({', '.join(missing_evt_cols)}) in latest data row (Index: {latest.name}). Cannot generate EVT signal.{Style.RESET_ALL}")
            return None
        ...
        if any(pd.isna(latest[col]) for col in required_evt_cols):
            ...
            logger.debug(f"Latest EVT data contains NaN values. Trend={trend_val}, Buy={buy_val}, Sell={sell_val}. No signal.")
            return None
        ...
        try:
            ...
            logger.error(f"{Fore.RED}Error converting EVT signal data to expected types: {conv_err}. Data: Trend={latest.get(trend_col)}, Buy={latest.get(buy_col)}, Sell={latest.get(sell_col)}.{Style.RESET_ALL}")
            return None

        logger.debug(f"Signal Check: Index={latest.name}, Close={latest.get('close', 'N/A'):.4f}, "
        ...
        if buy_signal and not sell_signal:  # Explicit buy signal
            ...
            logger.warning(f"{Fore.YELLOW}Conflicting signals: Both Buy and Sell flags are active in latest data. No signal generated.{Style.RESET_ALL}")
            return None
        else:
        ...
        logger.warning(f"{Fore.YELLOW}Could not access latest indicator data (IndexError), DataFrame might be too short ({len(df_ind)} rows).{Style.RESET_ALL}")
        return None
    except (KeyError, ValueError, TypeError) as e:  # Catch potential issues if columns exist but data is wrong type
        logger.error(f"{Fore.RED}Error processing latest data for signal generation: {e}. Data: {latest.to_dict()}{Style.RESET_ALL}")
        return None
    except Exception as e:
...
def calculate_stop_loss(current_exchange: ccxt_async.bybit, df_ind: pd.DataFrame, side: str, entry_price: Decimal, config: Config) -> Decimal | None:
    ...
    if not config.analysis_flags.get("use_atr", False):
        logger.warning(f"{Fore.YELLOW}ATR usage is disabled in config, cannot calculate ATR-based stop loss.{Style.RESET_ALL}")
        return None  # Or implement alternative SL logic (e.g., fixed percentage)

    try:
        market = current_exchange.market(config.SYMBOL)
        if not market:
            logger.error(f"{Fore.RED}Cannot calculate stop-loss: Market details for {config.SYMBOL} not loaded.{Style.RESET_ALL}")
            return None

        # Determine tick size for price formatting
        price_precision_str = market.get('precision', {}).get('price')
        if price_precision_str is None:
            logger.warning(f"{Fore.YELLOW}Could not determine price precision (tick size) for {config.SYMBOL}. Using fallback 1e-8. SL accuracy might be affected.{Style.RESET_ALL}")
            ...
            try:
                ...
                tick_size = Decimal(str(price_precision_str))
                 ...
                 try:
                     ...
                     logger.warning(f"{Fore.YELLOW}Could not parse price precision '{price_precision_str}' for {config.SYMBOL}. Using fallback 1e-8.{Style.RESET_ALL}")
                     tick_size = Decimal("1e-8")
        ...
        if pd.isna(latest_atr_raw):
            logger.warning(f"{Fore.YELLOW}Latest ATR value is NaN (Index: {df_ind.index[-1]}), cannot calculate stop-loss.{Style.RESET_ALL}")
            return None

        latest_atr = safe_decimal_conversion(latest_atr_raw)
        if latest_atr is None or latest_atr <= Decimal(0):
            logger.warning(f"{Fore.YELLOW}Invalid ATR value ({latest_atr_raw} -> {latest_atr}), cannot calculate stop-loss accurately.{Style.RESET_ALL}")
            return None
        ...
        if side == config.SIDE_BUY:
            if stop_loss_price >= entry_price:
                logger.warning(f"{Fore.YELLOW}Calculated Buy SL ({format_price(current_exchange, config.SYMBOL, stop_loss_price)}) is >= Entry ({format_price(current_exchange, config.SYMBOL, entry_price)}). Adjusting one tick lower.{Style.RESET_ALL}")
            ...
            if stop_loss_price <= Decimal(0):
                logger.error(f"{Fore.RED}Adjusted Buy SL resulted in zero or negative price ({stop_loss_price}). Cannot set SL.{Style.RESET_ALL}")
                return None
        elif side == config.SIDE_SELL:
            if stop_loss_price <= entry_price:
                logger.warning(f"{Fore.YELLOW}Calculated Sell SL ({format_price(current_exchange, config.SYMBOL, stop_loss_price)}) is <= Entry ({format_price(current_exchange, config.SYMBOL, entry_price)}). Adjusting one tick higher.{Style.RESET_ALL}")
                stop_loss_price = (stop_loss_price + tick_size).quantize(tick_size, rounding=ROUND_UP)

        logger.info(f"Calculated SL for {side.upper()} at {format_price(current_exchange, config.SYMBOL, stop_loss_price)} "
...
async def calculate_position_size(current_exchange: ccxt_async.bybit, symbol: str, entry_price: Decimal, stop_loss_price: Decimal, config: Config) -> Decimal | None:
    ...
    try:
        ...
        if available_balance is None or available_balance <= Decimal("0"):
            logger.error(f"{Fore.RED}Cannot calculate position size: Zero, None, or invalid available balance ({available_balance}).{Style.RESET_ALL}")
            return None
        ...
        if price_diff <= config.POSITION_QTY_EPSILON:
            logger.error(f"{Fore.RED}Cannot calculate position size: Entry price ({format_price(current_exchange, symbol, entry_price)}) and SL price ({format_price(current_exchange, symbol, stop_loss_price)}) are identical or too close (Diff: {price_diff}).{Style.RESET_ALL}")
            return None
        ...
        if not market:
            logger.error(f"{Fore.RED}Market {symbol} not loaded, cannot get limits/precision for position size.{Style.RESET_ALL}")
            return None
        ...
        if qty_precision_value is not None:
            try:
                ...
                if step_size <= 0:
                    logger.warning(f"{Fore.YELLOW}Market precision 'amount' is zero or negative ({qty_precision_value}). Using fallback 1e-8.{Style.RESET_ALL}")
                 ...
                 try:
                    ...
                    if num_digits == 0:
                        ...
                        step_size = Decimal('1') / (Decimal('10') ** num_digits)
                         ...
                         logger.warning(f"{Fore.YELLOW}Market precision 'amount' is negative ({num_digits}). Using fallback 1e-8.{Style.RESET_ALL}")
                         step_size = Decimal("1e-8")
                 except (ValueError, TypeError):
                    logger.warning(f"{Fore.YELLOW}Could not determine valid quantity step size from precision '{qty_precision_value}' for {symbol}. Using fallback 1e-8.{Style.RESET_ALL}")
                    step_size = Decimal("1e-8")
        else:
            logger.warning(f"{Fore.YELLOW}Could not determine quantity precision/step size for {symbol}. Using fallback 1e-8.{Style.RESET_ALL}")
        ...
        if position_size_adjusted <= Decimal(0):
            logger.warning(f"{Fore.YELLOW}Calculated position size is zero or negative ({position_size_adjusted}) after adjusting for step size {step_size}. "
        ...
        if min_qty is not None and position_size_adjusted < min_qty:
            logger.warning(f"{Fore.YELLOW}Calculated position size ({format_amount(current_exchange, symbol, position_size_adjusted)}) is below exchange minimum ({format_amount(current_exchange, symbol, min_qty)}). Cannot place trade.{Style.RESET_ALL}")
            return None

        max_qty = safe_decimal_conversion(max_qty_str, default=None)
        if max_qty is not None and position_size_adjusted > max_qty:
            logger.warning(f"{Fore.YELLOW}Calculated position size ({format_amount(current_exchange, symbol, position_size_adjusted)}) exceeds exchange maximum ({format_amount(current_exchange, symbol, max_qty)}). Capping at maximum.{Style.RESET_ALL}")
            ...
            if position_size_adjusted <= Decimal(0) or (min_qty is not None and position_size_adjusted < min_qty):
                 logger.error(f"{Fore.RED}Position size after capping at max ({format_amount(current_exchange, symbol, position_size_adjusted)}) became invalid (zero or below min). Cannot place trade.{Style.RESET_ALL}")
                 return None
        ...
        logger.info(f"Calculated position size: {format_amount(current_exchange, symbol, position_size_adjusted)} {base_currency} "
        ...
        logger.warning(f"{Fore.YELLOW}Network error during position size calculation (fetching balance?): {e}{Style.RESET_ALL}")
        return None
    except ccxt.ExchangeError as e:
        logger.error(f"{Fore.RED}Exchange error during position size calculation (fetching balance?): {e}{Style.RESET_ALL}")
        return None
    except (KeyError, ValueError, TypeError, InvalidOperation) as e:
        logger.error(f"{Fore.RED}Error processing data/market info during position size calculation: {e}{Style.RESET_ALL}")
        return None
    except Exception as e:
...
async def run_strategy(config: Config, current_exchange: ccxt_async.bybit):
    ...
    if logger is None:
        ...
        return
    if not current_exchange:
        logger.critical(f"{Back.RED}{Fore.WHITE}Strategy cannot run: Invalid exchange object received.{Style.RESET_ALL}")
        return

    logger.info(f"{Fore.MAGENTA}{Style.BRIGHT}--- Starting Ehlers Volumetric Strategy Loop for {config.SYMBOL} on {config.TIMEFRAME} ---{Style.RESET_ALL}")
    logger.info(f"Risk: {config.RISK_PER_TRADE:.2%}, Leverage: {config.DEFAULT_LEVERAGE}x, Margin: {config.DEFAULT_MARGIN_MODE}, Position: {config.DEFAULT_POSITION_MODE}")
    logger.info(f"EVT Params: Enabled={config.EVT_ENABLED}, Length={config.EVT_LENGTH}, Multiplier={config.EVT_MULTIPLIER}")
    logger.info(f"ATR Params: Period={config.STOP_LOSS_ATR_PERIOD}, Multiplier={config.STOP_LOSS_ATR_MULTIPLIER}")
    if config.ENABLE_SMS_ALERTS: logger.info(f"SMS Alerts: ENABLED for {config.SMS_RECIPIENT_NUMBER}")
    ...
    while True:
        try:
            ...
            logger.info(f"{Fore.BLUE}{Style.BRIGHT}" + "-" * 25 + f" Cycle Start: {cycle_start_time.isoformat()} " + "-" * 25 + f"{Style.RESET_ALL}")
            ...
            if current_position is None:
                logger.warning(f"{Fore.YELLOW}Failed to get current position state for {config.SYMBOL}. Retrying next cycle.{Style.RESET_ALL}")
            ...
            logger.info(f"Current Position ({config.SYMBOL}): Side={current_side}, "
            ...
            ohlcv_df = await bybit.fetch_ohlcv_paginated(
                current_exchange, config.SYMBOL, config.TIMEFRAME,
            ...
            if ohlcv_df is None or ohlcv_df.empty or len(ohlcv_df) < min_required_candles:
                logger.warning(f"{Fore.YELLOW}Could not fetch sufficient OHLCV data for {config.SYMBOL} "
            ...
            if ticker is None or ticker.get('last') is None:
                logger.warning(f"{Fore.YELLOW}Could not fetch valid ticker data for {config.SYMBOL}. Skipping cycle.{Style.RESET_ALL}")
                await asyncio.sleep(config.LOOP_DELAY_SECONDS)
                continue
            current_price = safe_decimal_conversion(ticker['last'])
            if current_price is None:
                logger.error(f"{Fore.RED}Could not convert ticker price '{ticker['last']}' to Decimal for {config.SYMBOL}. Skipping cycle.{Style.RESET_ALL}")
                await asyncio.sleep(config.LOOP_DELAY_SECONDS)
                continue
            logger.debug(f"Current Price ({config.SYMBOL}): {format_price(current_exchange, config.SYMBOL, current_price)}")
            ...
            if df_with_indicators is None:
                logger.warning(f"{Fore.YELLOW}Failed to calculate indicators for {config.SYMBOL}. Skipping cycle.{Style.RESET_ALL}")
                await asyncio.sleep(config.LOOP_DELAY_SECONDS)
                continue
            ...
            if current_side != config.POS_NONE and current_qty > config.POSITION_QTY_EPSILON:
                ...
                if config.EVT_ENABLED:
                    try:
                        ...
                        if not pd.isna(latest_trend_val):
                            ...
                            logger.warning(f"{Fore.YELLOW}Latest EVT trend is NaN for {config.SYMBOL}, cannot use for exit signal.{Style.RESET_ALL}")
                    except (IndexError, KeyError, ValueError, TypeError) as trend_err:
                         logger.warning(f"{Fore.YELLOW}Could not evaluate EVT trend for exit condition: {trend_err}.{Style.RESET_ALL}")
                ...
                if not should_exit and signal:  # Only check if not already exiting based on trend
                    if current_side == config.POS_LONG and signal == config.SIDE_SELL or current_side == config.POS_SHORT and signal == config.SIDE_BUY:
                        should_exit = True
                ...
                if should_exit:
                    logger.warning(f"{Fore.YELLOW}{Style.BRIGHT}Exit condition met for {current_side} {config.SYMBOL} position: {exit_reason}. Attempting to close.{Style.RESET_ALL}")

                    # --- Cancel Existing SL Order FIRST ---
                    sl_order_id = stop_loss_orders.pop(config.SYMBOL, None)  # Get and remove ID if found
                    if sl_order_id:
                        try:
                            logger.info(f"Attempting to cancel existing SL order {format_order_id(sl_order_id)} for {config.SYMBOL} before closing position...")
                            logger.debug(f"{Fore.CYAN}# Awaiting cancellation of SL order {sl_order_id}...{Style.RESET_ALL}")
                            # ASSUMPTION: cancel_order IS async
                            cancelled = await bybit.cancel_order(current_exchange, config.SYMBOL, sl_order_id, config=config)
                            if cancelled:
                                logger.info(f"{Fore.GREEN}Successfully cancelled SL order {format_order_id(sl_order_id)} for {config.SYMBOL}.{Style.RESET_ALL}")
                            else:
                                # This might happen if the order was already filled/cancelled. Log as warning.
                                logger.warning(f"{Fore.YELLOW}Attempt to cancel SL order {format_order_id(sl_order_id)} returned non-True or None. It might have already executed or been cancelled.{Style.RESET_ALL}")
                        except ccxt.OrderNotFound:
                            logger.warning(f"{Fore.YELLOW}SL order {format_order_id(sl_order_id)} not found when trying to cancel (already filled/cancelled?). Proceeding with position close.{Style.RESET_ALL}")
                        except ccxt.NetworkError as e:
                            logger.warning(f"{Fore.YELLOW}Network error cancelling SL order {format_order_id(sl_order_id)}: {e}. Proceeding with position close.{Style.RESET_ALL}")
                        except ccxt.ExchangeError as e:
                            # Log error but proceed with closing the position - better to close than be stuck.
                            logger.error(f"{Fore.RED}Exchange error cancelling SL order {format_order_id(sl_order_id)}: {e}. Proceeding with position close with caution.{Style.RESET_ALL}")
                        except Exception as e:
                            logger.error(f"{Fore.RED}Unexpected error cancelling SL order {format_order_id(sl_order_id)}: {e}{Style.RESET_ALL}", exc_info=True)
                            # Proceeding is usually better than being stuck.
                    else:
                        logger.warning(f"{Fore.YELLOW}No tracked SL order ID found to cancel for the existing {current_side} {config.SYMBOL} position. Ensure SL was placed and tracked correctly.{Style.RESET_ALL}")

                    # --- Close the Position (Market Order, Reduce Only) ---
                    logger.info(f"Attempting to close {current_side} position ({format_amount(current_exchange, config.SYMBOL, current_qty)} {config.SYMBOL}) via market order...")
                    logger.debug(f"{Fore.CYAN}# Awaiting position close market order for {config.SYMBOL}...{Style.RESET_ALL}")
                    ...
                    if close_order and close_order.get('id'):
                        ...
                        close_price_formatted = format_price(current_exchange, config.SYMBOL, avg_close_price) if avg_close_price else 'N/A'

                        logger.success(f"{Fore.GREEN}{Style.BRIGHT}Position ({config.SYMBOL}) CLOSE order {close_id_short} submitted. "
                        ...
                        logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: FAILED TO SUBMIT POSITION CLOSE ORDER for {config.SYMBOL} ({current_side}, {format_amount(current_exchange, config.SYMBOL, current_qty)})! Reason: {exit_reason}. Manual intervention likely required!{Style.RESET_ALL}")
                        if config.ENABLE_SMS_ALERTS:
            ...
            # Only enter if we are flat (no position) and there is a valid signal
                ...
                logger.info(f"{Fore.CYAN}{Style.BRIGHT}Attempting to enter {signal.upper()} position for {config.SYMBOL} based on signal...{Style.RESET_ALL}")
                ...
                cancelled_count = await bybit.cancel_all_orders(current_exchange, config.SYMBOL, config, reason="Pre-Entry Cleanup")
                if cancelled_count is not None and cancelled_count > 0:
                    logger.info(f"Pre-entry cleanup ({config.SYMBOL}): Cancelled {cancelled_count} potential lingering order(s).")
                ...
                stop_loss_price = calculate_stop_loss(current_exchange, df_with_indicators, signal, current_price, config)
                if not stop_loss_price or stop_loss_price <= 0:  # Check if None or invalid (zero/negative)
                    logger.error(f"{Fore.RED}Could not calculate a valid stop-loss for {config.SYMBOL} based on current data (Result: {stop_loss_price}). Cannot enter trade.{Style.RESET_ALL}")
                ...
                position_size = await calculate_position_size(current_exchange, config.SYMBOL, current_price, stop_loss_price, config)
                if not position_size or position_size <= 0:  # Check if None or zero/negative
                    logger.error(f"{Fore.RED}Could not calculate a valid position size for {config.SYMBOL} (Result: {position_size}). Cannot enter trade.{Style.RESET_ALL}")
                    await asyncio.sleep(config.LOOP_DELAY_SECONDS)  # Wait before retrying cycle
                    continue

                # --- Place Market Entry Order (Asynchronous) ---
                base_currency = config.SYMBOL.split('/')[0]
                logger.info(f"Placing {signal.upper()} market entry order for {format_amount(current_exchange, config.SYMBOL, position_size)} {base_currency}...")
                logger.debug(f"{Fore.CYAN}# Awaiting market entry order placement for {config.SYMBOL}...{Style.RESET_ALL}")
                # ASSUMPTION: place_market_order_slippage_check IS async and returns order dict or None
                entry_order = await bybit.place_market_order_slippage_check(current_exchange, config.SYMBOL, signal, position_size, config)
                ...
                if entry_order and entry_order.get('id'):
                    ...
                    avg_fill_price_order = safe_decimal_conversion(entry_order.get('average'))  # Might be None immediately
                    filled_qty_order = safe_decimal_conversion(entry_order.get('filled', 0))  # Might be 0 immediately

                    logger.success(f"{Fore.GREEN}Entry market order {order_id_short} submitted for {config.SYMBOL} ({signal.upper()} {format_amount(current_exchange, config.SYMBOL, position_size)}). "
                    ...
                    logger.info(f"Waiting {config.POSITION_CONFIRM_DELAY_SECONDS}s for position update after entry order {order_id_short}...")
                    await asyncio.sleep(config.POSITION_CONFIRM_DELAY_SECONDS)

                    logger.debug(f"{Fore.CYAN}# Awaiting position confirmation after entry ({config.SYMBOL})...{Style.RESET_ALL}")
                    ...
                    if (pos_after_entry and
                            pos_after_entry.get('side') == expected_pos_side and
                            pos_after_entry.get('qty', Decimal(0)) > config.POSITION_QTY_EPSILON):
                        ...
                        logger.info(f"{Fore.GREEN}{Style.BRIGHT}Position confirmed OPEN ({config.SYMBOL}): {pos_after_entry['side']} {pos_qty_formatted} @ ~{entry_price_formatted}{Style.RESET_ALL}")
                        ...
                        logger.info(f"Placing {sl_side.upper()} native stop-loss order for {pos_qty_formatted} {config.SYMBOL} at {sl_price_formatted}...")
                        logger.debug(f"{Fore.CYAN}# Awaiting native stop-loss placement for {config.SYMBOL}...{Style.RESET_ALL}")
                        # ASSUMPTION: place_native_stop_loss IS async
                        sl_order = await bybit.place_native_stop_loss(
                            current_exchange, config.SYMBOL, sl_side,
                        ...
                        if sl_order and sl_order.get('id'):
                            ...
                            logger.success(f"{Fore.GREEN}Native stop-loss order {sl_id_short} placed successfully for {pos_qty_formatted} {config.SYMBOL} at {sl_price_formatted}.{Style.RESET_ALL}")
                            ...
                            logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: Failed to place stop-loss order for {config.SYMBOL} after entering {signal.upper()} position ({pos_qty_formatted})! Attempting EMERGENCY CLOSE!{Style.RESET_ALL}")
                            if config.ENABLE_SMS_ALERTS:
                                alert_msg = f"[{config.SYMBOL}] URGENT: Failed SL place after {signal.upper()} entry ({pos_qty_formatted})! Closing pos NOW."
                                send_sms_alert(alert_msg, config)

                            logger.debug(f"{Fore.CYAN}# Awaiting emergency position close for {config.SYMBOL} due to failed SL placement...{Style.RESET_ALL}")
                            ...
                            if emergency_close_order and emergency_close_order.get('id'):
                                logger.warning(f"{Fore.YELLOW}Position ({config.SYMBOL}) emergency closed via order {format_order_id(emergency_close_order['id'])} due to failed SL placement.{Style.RESET_ALL}")
                            else:
                                logger.critical(f"{Back.RED}{Fore.WHITE}EMERGENCY FAILED: FAILED TO CLOSE POSITION ({pos_qty_formatted} {config.SYMBOL}) AFTER FAILED SL PLACEMENT! MANUAL INTERVENTION REQUIRED IMMEDIATELY!{Style.RESET_ALL}")
                                if config.ENABLE_SMS_ALERTS:
                                    send_sms_alert(f"[{config.SYMBOL}] !!! CRITICAL MANUAL ACTION: Failed emergency close pos {pos_qty_formatted} after failed SL place !!!", config)
                        ...
                        pos_qty_report = format_amount(current_exchange, config.SYMBOL, pos_after_entry.get('qty', 0)) if pos_after_entry else 'N/A'
                        logger.error(f"{Fore.RED}Entry order {order_id_short} for {config.SYMBOL} submitted, but position confirmation failed or quantity is zero/insufficient. "
                    ...
                    logger.error(f"{Fore.RED}Entry market order placement FAILED for {config.SYMBOL}. No valid order dictionary received from helper. Check exchange status and API logs.{Style.RESET_ALL}")
            ...
            logger.info(f"Cycle complete ({config.SYMBOL}). Duration: {cycle_duration:.2f}s. Waiting {wait_time:.2f}s...")
            await asyncio.sleep(wait_time)

        # --- Exception Handling for the Main Loop ---
        except ccxt.NetworkError as e:
            logger.warning(f"{Fore.YELLOW}Network Error in main loop ({config.SYMBOL}): {e}. Retrying after {config.RETRY_DELAY_SECONDS * 5:.1f}s delay...{Style.RESET_ALL}")
            await asyncio.sleep(config.RETRY_DELAY_SECONDS * 5)  # Longer delay for network issues
        except ccxt.ExchangeNotAvailable as e:
            logger.error(f"{Fore.RED}Exchange Not Available ({config.SYMBOL}): {e}. Possibly maintenance. Waiting longer ({config.LOOP_DELAY_SECONDS * 5}s)...{Style.RESET_ALL}")
            if config.ENABLE_SMS_ALERTS:
            ...
            logger.warning(f"{Fore.YELLOW}Rate Limit Exceeded ({config.SYMBOL}): {e}. Waiting {config.RETRY_DELAY_SECONDS * 10:.1f}s...{Style.RESET_ALL}")  # Longer delay for rate limits
            await asyncio.sleep(config.RETRY_DELAY_SECONDS * 10)
        except ccxt.AuthenticationError as e:
            logger.critical(f"{Back.RED}{Fore.WHITE}Authentication Error during runtime ({config.SYMBOL}): {e}. API keys might be invalid or expired. Stopping strategy.{Style.RESET_ALL}", exc_info=True)
            if config.ENABLE_SMS_ALERTS:
            ...
            logger.warning(f"{Fore.YELLOW}OrderNotFound error encountered in loop: {e}. This might be okay if cancelling an order that already executed/cancelled.{Style.RESET_ALL}")
            ...
            if order_id_in_error and order_id_in_error in stop_loss_orders.values():
                logger.info(f"OrderNotFound likely relates to tracked SL order {order_id_in_error}. Removing from tracking.")
            ...
            logger.error(f"{Back.RED}{Fore.WHITE}Insufficient Funds Error ({config.SYMBOL}): {e}. Check balance and open orders/positions. Pausing loop significantly.{Style.RESET_ALL}")
            if config.ENABLE_SMS_ALERTS:
            ...
            logger.error(f"{Fore.RED}Unhandled Exchange Error in main loop ({config.SYMBOL}): {e}. Retrying after {config.LOOP_DELAY_SECONDS}s delay...{Style.RESET_ALL}", exc_info=True)
            # Optional: Add SMS for specific error codes if needed (e.g., margin call)
            await asyncio.sleep(config.LOOP_DELAY_SECONDS)
        except KeyboardInterrupt:
            logger.warning(f"{Fore.YELLOW}{Style.BRIGHT}Keyboard interrupt received. Initiating graceful shutdown...{Style.RESET_ALL}")
            break  # Exit the loop
        except NameError as e:
            logger.critical(f"{Back.RED}{Fore.WHITE}A NameError occurred: {e}. Check imports (e.g., colorama, pandas_ta) and variable names.{Style.RESET_ALL}", exc_info=True)
            if not COLORAMA_AVAILABLE and ("Fore" in str(e) or "Back" in str(e) or "Style" in str(e)):
                logger.critical(f"{Fore.YELLOW}Suggestion: Ensure {Style.BRIGHT}'pip install colorama'{Style.RESET_ALL}{Fore.YELLOW} is installed and the import succeeded.{Style.RESET_ALL}")
            if "pandas_ta" in str(e):
                 logger.critical(f"{Fore.YELLOW}Suggestion: Ensure {Style.BRIGHT}'pip install pandas_ta'{Style.RESET_ALL}{Fore.YELLOW} is installed for fallback ATR calculation.{Style.RESET_ALL}")
            break  # Often critical, stop the strategy
        except Exception as e:
            # Catch-all for any other unexpected errors during the loop
            logger.critical(f"{Back.RED}{Fore.WHITE}!!! UNEXPECTED CRITICAL ERROR IN MAIN LOOP ({config.SYMBOL}) !!!{Style.RESET_ALL}", exc_info=True)
    ...
    logger.info(f"{Fore.MAGENTA}{Style.BRIGHT}--- Ehlers Volumetric Strategy Loop Stopped for {config.SYMBOL} ---{Style.RESET_ALL}")
...
async def main():
    ...
    try:
        ...
        logger.critical(f"{Back.RED}{Fore.WHITE}Failed to load or initialize configuration: {e}{Style.RESET_ALL}", exc_info=True)
        sys.exit(exit_code)  # Exit with error

    # --- 2. Validate Core Config (API Keys) ---
    if not CONFIG.API_KEY or not CONFIG.API_SECRET:
        logger.critical(f"{Back.RED}{Fore.WHITE}API Key or Secret not found in environment variables (BYBIT_API_KEY, BYBIT_API_SECRET). Grant the script access! Exiting.{Style.RESET_ALL}")
        sys.exit(exit_code)  # Exit with error
    logger.info(f"Strategy: {CONFIG.strategy['name']}, Symbol: {CONFIG.SYMBOL}, Timeframe: {CONFIG.TIMEFRAME}, Testnet: {CONFIG.TESTNET_MODE}")
    if CONFIG.ENABLE_SMS_ALERTS:
    ...
    try:
        ...
        if not exchange:
            logger.warning(f"{Fore.YELLOW}Helper 'initialize_bybit' did not return an exchange object. Attempting direct ccxt_async initialization...{Style.RESET_ALL}")
            try:
                ...
                if CONFIG.TESTNET_MODE:
                 ...
                 logger.critical(f"{Back.RED}{Fore.WHITE}Direct ccxt_async initialization failed: {direct_init_err}{Style.RESET_ALL}", exc_info=True)
                 raise ConnectionError("Failed to initialize Bybit async exchange object via helper or directly.") from direct_init_err

        if not exchange:  # Check again after fallback attempt
             raise ConnectionError("Failed to initialize Bybit async exchange object.")

        logger.success(f"{Fore.GREEN}Async Exchange connection initialized successfully ({'Testnet' if CONFIG.TESTNET_MODE else 'Mainnet'}).{Style.RESET_ALL}")
        ...
        market_logic = 'linear' if market_details.get('linear', False) else ('inverse' if market_details.get('inverse', False) else 'N/A')
        logger.success(f"{Fore.GREEN}Market {CONFIG.SYMBOL} validated: Type={market_type}, Logic={market_logic}{Style.RESET_ALL}")
        ...
        try:
            ...
            if not leverage_set_result:  # Adjust condition based on helper's success/failure return convention
                ...
                logger.critical(f"{Back.RED}{Fore.WHITE}Helper function 'set_leverage' indicated failure (returned: {leverage_set_result}). Check previous logs for API errors.{Style.RESET_ALL}")
            ...
            logger.success(f"{Fore.GREEN}Leverage successfully set to {CONFIG.DEFAULT_LEVERAGE}x for {CONFIG.SYMBOL}.{Style.RESET_ALL}")
            ...
            logger.critical(f"{Back.RED}{Fore.WHITE}Failed to set leverage due to exchange error: {leverage_error}{Style.RESET_ALL}", exc_info=True)
            # Log specific details if available in the error (e.g., error code, message from Bybit)
            logger.critical(f"Check if leverage {CONFIG.DEFAULT_LEVERAGE}x is valid for {CONFIG.SYMBOL} and if API keys have trade permissions.")
            ...
            logger.critical(f"{Back.RED}{Fore.WHITE}Unexpected error during set_leverage call: {e}{Style.RESET_ALL}", exc_info=True)
            raise  # Re-raise
    ...
    # --- Catch Specific Setup Errors ---
        ...
        logger.critical(f"{Back.RED}{Fore.WHITE}Authentication Error during setup: {e}. Check API keys/permissions. Exiting.{Style.RESET_ALL}", exc_info=True)
        if CONFIG and CONFIG.ENABLE_SMS_ALERTS: send_sms_alert(f"[{CONFIG.SYMBOL if CONFIG else 'N/A'}] EXIT: Auth error setup.", CONFIG)
    except (ccxt.ExchangeError, ccxt.NetworkError, ConnectionError, ValueError) as e:
        logger.critical(f"{Back.RED}{Fore.WHITE}Exchange/Network/Configuration Error during setup: {e}. Exiting.{Style.RESET_ALL}", exc_info=True)
        if CONFIG and CONFIG.ENABLE_SMS_ALERTS: send_sms_alert(f"[{CONFIG.SYMBOL if CONFIG else 'N/A'}] EXIT: Setup fail ({type(e).__name__}). Logs.", CONFIG)
    except Exception as e:
        # Catch any other unexpected errors during setup
        logger.critical(f"{Back.RED}{Fore.WHITE}Unexpected critical error during setup phase: {e}. Exiting.{Style.RESET_ALL}", exc_info=True)
        if CONFIG and CONFIG.ENABLE_SMS_ALERTS: send_sms_alert(f"[{CONFIG.SYMBOL if CONFIG else 'N/A'}] EXIT: Unexpected critical setup error ({type(e).__name__}). Logs.", CONFIG)
    ...
    if setup_success and exchange and CONFIG:
        try:
            ...
            logger.critical(f"{Back.RED}{Fore.WHITE}Strategy execution loop terminated abnormally by unhandled exception: {e}{Style.RESET_ALL}", exc_info=True)
            if CONFIG.ENABLE_SMS_ALERTS: send_sms_alert(f"[{CONFIG.SYMBOL}] CRITICAL FAILURE: Loop terminated ({type(e).__name__}). Logs!", CONFIG)
    ...
    if exchange and hasattr(exchange, 'close') and callable(exchange.close):
        try:
            ...
            logger.error(f"{Fore.RED}Error occurred while closing the exchange connection: {e}{Style.RESET_ALL}", exc_info=True)
            # Don't change exit code here, primary error was likely earlier
    else:
        logger.warning(f"{Fore.YELLOW}Exchange object was not initialized or has no close method; skipping explicit close.{Style.RESET_ALL}")
    ...
    if exit_code == 0:
        ...
        logger.warning(f"{Fore.YELLOW}{Style.BRIGHT}{final_message} (Indicates setup failure or abnormal termination){Style.RESET_ALL}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"{Back.RED}{Fore.WHITE}Fatal error during asyncio setup or top-level execution: {e}{Style.RESET_ALL}", file=sys.stderr)
