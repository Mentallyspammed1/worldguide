...
logger.setLevel(
    logging.DEBUG
)  # Set logger to lowest level to allow handlers to control verbosity
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any | None:
            ...
            while retries <= max_retries:
                try:
                    ...
                    logger.debug(
                        f"Attempting API call: {func_name} (Retry {retries}/{max_retries})"
                    )
    ...
    **Disclaimer:** Trading cryptocurrencies involves significant risk. This bot
    ...
    def __init__(
        ...
        self.daily_pnl: float = (
            0.0  # Simple daily PnL tracker (reset manually or add logic)
        )
        ...
        self.market_info: dict[str, Any] | None = (
            None  # Cache for market details (precision, limits)
        )
        self.price_decimals: int = (
            DEFAULT_PRICE_DECIMALS  # Calculated/retrieved price decimal places
        )
        self.amount_decimals: int = (
            DEFAULT_AMOUNT_DECIMALS  # Calculated/retrieved amount decimal places
        )
    ...
    def _load_state(self) -> None:
        ...
        try:
                ...
                with open(self.state_file, encoding="utf-8") as f:
                    ...
                    if isinstance(saved_state, list):
                        ...
                        required_keys = {
                            ...
                            "side",
                            "status",
                        }  # Example minimal required keys
                        for i, pos in enumerate(saved_state):
                            if isinstance(pos, dict) and required_keys.issubset(
                                pos.keys()
                            ):
                        ...
                        if os.path.exists(state_backup_file):
                            try:
                                ...
                                logger.debug(
                                    f"Removed old state backup file: {state_backup_file}"
                                )
            ...
            if os.path.exists(state_backup_file):
                ...
                try:
                    shutil.copy2(
                        state_backup_file, self.state_file
            ...
            logger.critical(
                f"{Fore.RED}Fatal error loading state from {self.state_file}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            logger.warning(
    ...
    def _save_state(self) -> None:
        ...
        if not hasattr(self, "open_positions"):
            logger.error(
                "Cannot save state: 'open_positions' attribute does not exist."
            )
            return
        ...
        try:
            ...
            logger.error(
                f"{Fore.RED}Could not scribe state to {self.state_file} (IO/OS Error): {e}{Style.RESET_ALL}"
            )
    ...
    def create_default_config(self, config_file: str) -> None:
        ...
        default_config = {
            ...
            "trading": {
                ...
                "symbol": os.getenv(
                    "TRADING_SYMBOL", "DOT/USDT:USDT"
                ),  # Changed default to DOT based on logs
        ...
        if os.getenv("ORDER_SIZE_PERCENTAGE"):
            try:
                ...
                if 0 < override_val <= 1:
                    ...
                    logger.info(
                        f"Overrode order_size_percentage from environment variable to {override_val}."
                    )
                else:
                    logger.warning(
                ...
                logger.warning(
                    "Invalid ORDER_SIZE_PERCENTAGE in environment (not a number), using default."
                )
    ...
    def validate_config(self) -> None:
        ...
        logger.debug(
            "Scrutinizing the configuration spellbook for potency and validity..."
        )
        try:
            ...
            for section in required_top_level_keys:
                _check(
                    isinstance(self.config[section], dict),
                    section,
            ...
            log_level = _get_nested(
                cfg_log, ["level"], default="INFO", req_type=str
            ).upper()
            _check(
            ...
            _check(
                ex_id and isinstance(ex_id, str),
            ...
            _check(
                symbol and isinstance(symbol, str),
                "trading.symbol",
                "Must be a non-empty string.",
            )
            timeframe = _get_nested(cfg_tr, ["timeframe"], req_type=str)
            _check(
                timeframe and isinstance(timeframe, str),
                "trading.timeframe",
                "Must be a non-empty string.",
            )
            ...
            offset_buy = _get_nested(
                cfg_tr, ["limit_order_offset_buy"], req_type=(int, float)
            )
            _check(
            ...
            offset_sell = _get_nested(
                cfg_tr, ["limit_order_offset_sell"], req_type=(int, float)
            )
            _check(
            ...
            imbalance = _get_nested(
                cfg_ob, ["imbalance_threshold"], req_type=(int, float)
            )
            _check(
            ...
            vol_mult = _get_nested(
                cfg_ind, ["volatility_multiplier"], req_type=(int, float)
            )
            _check(
            ...
            _check(
                isinstance(use_atr, bool),
                "risk_management.use_atr_sl_tp",
                "Must be true or false.",
            )
            if use_atr:
                atr_sl_mult = _get_nested(
                    cfg_rm, ["atr_sl_multiplier"], req_type=(int, float)
                )
                _check(
                ...
                atr_tp_mult = _get_nested(
                    cfg_rm, ["atr_tp_multiplier"], req_type=(int, float)
                )
                _check(
                ...
                sl_pct = _get_nested(
                    cfg_rm, ["stop_loss_percentage"], req_type=(int, float)
                )
                tp_pct = _get_nested(
                    cfg_rm, ["take_profit_percentage"], req_type=(int, float)
                )
                _check(
                    sl_pct is not None
                ...
                _check(
                    tp_pct is not None
                    and isinstance(tp_pct, (int, float))
                    and 0 < tp_pct < 1,
            ...
            enable_tsl = _get_nested(
                cfg_rm, ["enable_trailing_stop_loss"], req_type=bool
            )
            _check(
                isinstance(enable_tsl, bool),
                "risk_management.enable_trailing_stop_loss",
                "Must be true or false.",
            )
            if enable_tsl:
                tsl_pct = _get_nested(
                    cfg_rm, ["trailing_stop_loss_percentage"], req_type=(int, float)
                )
                _check(
                    tsl_pct is not None
                    and isinstance(tsl_pct, (int, float))
                    and 0 < tsl_pct < 1,
            ...
            order_size_pct = _get_nested(
                cfg_rm, ["order_size_percentage"], req_type=(int, float)
            )
            _check(
            ...
            strong_adj = _get_nested(
                cfg_rm, ["strong_signal_adjustment_factor"], req_type=(int, float)
            )
            _check(
            ...
            weak_adj = _get_nested(
                cfg_rm, ["weak_signal_adjustment_factor"], req_type=(int, float)
            )
            _check(
    ...
    def _initialize_exchange(self) -> ccxt.Exchange:
        ...
        self.api_key = os.getenv("BYBIT_API_KEY") or self.config.get(
            "exchange", {}
        ).get("api_key")
        self.api_secret = os.getenv("BYBIT_API_SECRET") or self.config.get(
            "exchange", {}
        ).get("api_secret")
        ...
        try:
            ...
            if not self.simulation_mode and self.testnet_mode:
                try:
                    logger.info(
                        "Attempting to switch exchange instance to Testnet mode..."
                    )
            ...
            if not market_details.get(
                "active", True
            ):  # Default to True if 'active' key is missing
                logger.warning(
            ...
            if not self.simulation_mode:
                logger.debug(
                    "Performing initial API connectivity test (fetching time)..."
                )
                ...
                if server_time_ms is not None:
                    server_time_str = pd.to_datetime(
                        server_time_ms, unit="ms", utc=True
                    ).isoformat()
                    logger.debug(
                        f"Exchange time crystal synchronized: {server_time_str}"
                    )
                    logger.info(
                        f"{Fore.GREEN}API connection and authentication successful.{Style.RESET_ALL}"
                    )
                else:
    ...
    def _calculate_decimal_places(self, tick_size: float | int | None) -> int:
        ...
        if tick_size is None:
            logger.debug(
                "Tick size is None, cannot calculate decimal places. Returning default."
            )
            return fallback_decimals
        if not isinstance(tick_size, (float, int)) or tick_size <= 0:
            logger.warning(
                f"Invalid tick size type or value for decimal calculation: {tick_size}. Returning default."
            )
            return fallback_decimals

        try:
            # Handle integer tick sizes (e.g., 1, 10) -> 0 decimals
            if isinstance(tick_size, int) or (
                isinstance(tick_size, float) and tick_size.is_integer()
            ):
                return 0

            # Use logarithm for simple powers of 10 (e.g., 0.1, 0.001)
            log_val = math.log10(tick_size)
            if log_val == math.floor(
                log_val
            ):  # Check if it's an integer (negative for < 1)
                decimals = max(0, int(-log_val))
                logger.debug(
                    f"Calculated decimals for power-of-10 tick size {tick_size}: {decimals}"
                )
                return decimals
            else:
                ...
                if "." in s:
                    ...
                    logger.debug(
                        f"Calculated decimals via string format for tick size {tick_size}: {num_decimals}"
                    )
                    return num_decimals
            ...
            ValueError
        ) as e:  # Handles log10(negative) which is already checked, but good practice
            logger.error(
                f"Math error calculating decimal places for tick size {tick_size}: {e}. Returning default."
            )
            return fallback_decimals
        except Exception as e:
            logger.error(
    ...
    def _load_market_info(self) -> None:
        ...
        try:
            ...
            min_amount = limits.get("amount", {}).get(
                "min"
            )  # Minimum order size in base currency
            min_cost = limits.get("cost", {}).get(
                "min"
            ...
            min_amount_str = (
                f"{min_amount:.{self.amount_decimals}f}"
                if min_amount is not None
                else "N/A"
            )
            min_cost_str = (
                f"{min_cost:.{self.price_decimals}f}" if min_cost is not None else "N/A"
            )
            logger.info(
    ...
    def fetch_market_price(self) -> float | None:
        ...
        if ticker and ticker.get("last") is not None:
            try:
                ...
                logger.debug(
                    f"Current market price ({self.symbol}): {price:.{self.price_decimals}f}"
                )
                return price
            except (ValueError, TypeError) as e:
                logger.error(
    ...
    def fetch_order_book(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if bid_volume > epsilon:
                ...
                logger.debug(
                    f"Order Book ({self.symbol}) Imbalance (Ask Vol / Bid Vol): {imbalance_ratio:.3f}"
                )
            elif ask_volume > epsilon:  # Bids are near zero, asks are not
                result["imbalance"] = float(
                    "inf"
                )  # Represent infinite pressure upwards
                logger.debug(
                    f"Order Book ({self.symbol}) Imbalance: Infinity (Near-Zero Bid Volume)"
                )
            else:  # Both bid and ask volumes are effectively zero
                result["imbalance"] = None  # Undefined imbalance
                logger.debug(
                    f"Order Book ({self.symbol}) Imbalance: N/A (Near-Zero Bid & Ask Volume)"
                )
    ...
    def fetch_historical_data(self, limit: int | None = None) -> pd.DataFrame | None:
        ...
        if limit is None:
            ...
            required_periods = [
                ...
                + self.macd_signal_period,  # Needs ~long+signal for good MACD hist
        ...
        try:
            ...
            if final_len < initial_len:
                logger.debug(
                    f"Dropped {initial_len - final_len} rows containing NaNs in OHLC from historical data."
                )
    ...
    def fetch_balance(self, currency_code: str | None = None) -> float:
        ...
        try:
            ...
            if (
                ...
                logger.debug(
                    f"Found standard 'free' balance for {quote_currency}: {available_balance}"
                )
                    ...
                    try:
                        ...
                        if result_list and isinstance(result_list[0], dict):
                            ...
                            account_type = account_info.get(
                                "accountType"
                            )  # e.g., 'UNIFIED', 'CONTRACT', 'SPOT'
                            logger.debug(
                                f"Parsing Bybit 'info': Account Type = {account_type}"
                            )
                            ...
                            if account_type in ["UNIFIED", "CONTRACT"]:
                                ...
                                unified_avail_str = account_info.get(
                                ...
                                if unified_avail_str is not None:
                                    ...
                                    for coin_entry in coin_list:
                                        if (
                                            isinstance(coin_entry, dict)
                                            and coin_entry.get("coin") == quote_currency
                                        ):
                            ...
                            logger.debug(
                                "Bybit 'info' structure not as expected (result.list missing or empty)."
                            )
            ...
            logger.info(
                f"Fetched available balance: {final_balance:.{self.price_decimals}f} {quote_currency}"
            )
            ...
            logger.error(
                f"{Fore.RED}Unexpected error fetching balance for {quote_currency}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return 0.0
    ...
    @retry_api_call(
        max_retries=1
    )  # Don't retry 'fetch_order' aggressively if it fails initially
    def fetch_order_status(
        ...
        if self.simulation_mode:
            ...
            for pos in self.open_positions:
                if pos["id"] == order_id:
                    logger.debug(
                        f"[SIMULATION] Returning cached/simulated status for order {order_id}."
                    )
                    ...
                    sim_timestamp_ms = (
                        int(entry_time_sec * 1000)
                        if entry_time_sec
                        else int(time.time() * 1000)
                    )
                    sim_datetime = pd.to_datetime(
                        sim_timestamp_ms, unit="ms", utc=True
                    ).isoformat()
                    ...
                    return {
                        ...
                        "stopLossPrice": pos.get(
                            "stop_loss_price"
                        ),  # Reflects internal state SL/TP
            ...
            logger.warning(
                f"[SIMULATION] Simulated order {order_id} not found in current state for status check."
            )
            ...
            try:
                ...
                if self.exchange_id.lower() == "bybit":
                    ...
                    if market_type in ["swap", "future"]:
                        params["category"] = (
                            "linear"
                            if self.exchange.market(target_symbol).get("linear")
                            else "inverse"
                        )
                    elif market_type == "spot":
                        # Spot order history might not need category or might use 'spot'
                        params["category"] = (
                            "spot"  # Example, check Bybit API if needed
                        )
                ...
                amount_str = (
                    f"{amount:.{self.amount_decimals}f}"
                    if amount is not None
                    else "N/A"
                )
                filled_str = (
                    f"{filled:.{self.amount_decimals}f}"
                    if filled is not None
                    else "N/A"
                )
                avg_price_str = (
                    f"{avg_price:.{self.price_decimals}f}"
                    if avg_price is not None
                    else "N/A"
                )
                remaining_str = (
                    f"{remaining:.{self.amount_decimals}f}"
                ...
                err_str = str(e).lower()
                if (
                    "order is finished" in err_str
                    or "order has been filled" in err_str
                    or "already closed" in err_str
    ...
    def calculate_volatility(close_prices: pd.Series, window: int) -> float | None:
        ...
        if len(close_prices) < min_len:
            logger.debug(
                f"Insufficient data for volatility (need {min_len}, got {len(close_prices)})."
            )
            return None
        try:
            ...
            if (close_prices <= 0).any():
                logger.warning(
                    "Non-positive prices found, cannot calculate log returns for volatility."
                )
                return None
            ...
            logger.error(
                f"Error calculating volatility: {e}", exc_info=False
            )  # Avoid spamming logs
            return None
    ...
    def calculate_ema(close_prices: pd.Series, period: int) -> float | None:
        ...
        if len(close_prices) < min_len:
            logger.debug(
                f"Insufficient data for EMA {period} (need {min_len}, got {len(close_prices)})."
            )
            return None
        try:
    ...
    def calculate_rsi(close_prices: pd.Series, period: int) -> float | None:
        ...
        if len(close_prices) < min_len:
            logger.debug(
                f"Insufficient data for RSI {period} (need {min_len}, got {len(close_prices)})."
            )
            return None
        try:
    ...
    def calculate_macd(
        ...
        if close_prices is None or not all(
            p > 0 for p in [short_period, long_period, signal_period]
        ):
            return None, None, None
        if short_period >= long_period:
            logger.error(
                f"MACD short_period ({short_period}) must be less than long_period ({long_period})."
            )
            return None, None, None
        ...
        try:
            ema_short = close_prices.ewm(
                span=short_period, adjust=False, min_periods=short_period
            ).mean()
            ema_long = close_prices.ewm(
                span=long_period, adjust=False, min_periods=long_period
            ).mean()
            ...
            if pd.isna(macd_val) or pd.isna(signal_val) or pd.isna(hist_val):
                logger.debug(
                    "MACD calculation resulted in NaN (likely insufficient data for period)."
                )
                return None, None, None
            return float(macd_val), float(signal_val), float(hist_val)
        except Exception as e:
            logger.error(
                f"Error calculating MACD ({short_period},{long_period},{signal_period}): {e}",
                exc_info=False,
            )
            return None, None, None
    ...
    def calculate_stoch_rsi(
        ...
        if close_prices is None or not all(
            p > 0 for p in [rsi_period, stoch_period, k_period, d_period]
        ):
            return None, None
        ...
        try:
            ...
            if rsi_series is None or rsi_series.isna().all():
                logger.debug(
                    "RSI series calculation failed or returned all NaNs for StochRSI."
                )
                return None, None
            ...
            min_rsi = rsi_series.rolling(
                window=stoch_period, min_periods=stoch_period
            ).min()
            ...
            if pd.isna(k_val) or pd.isna(d_val):
                logger.debug(
                    "StochRSI K or D is NaN (likely insufficient smoothed data)."
                )
                return None, None
            ...
            logger.error(
                f"Error calculating StochRSI ({rsi_period},{stoch_period},{k_period},{d_period}): {e}",
                exc_info=False,
            )
            return None, None
    ...
    def calculate_atr(
        ...
        if not (
            len(high_prices) >= min_len
            and len(low_prices) >= min_len
            and len(close_prices) >= min_len
        ):
            logger.debug(
                f"Insufficient data for ATR {period} (need {min_len}, got {len(close_prices)})."
            )
            return None
        try:
            ...
            tr = tr_df.max(
                axis=1, skipna=False
            )  # skipna=False propagates NaN if any component is NaN
            ...
            return (
                float(atr) if pd.notna(atr) and atr >= 0 else None
            )  # Ensure ATR is non-negative
        except Exception as e:
    ...
    def calculate_order_size(self, current_price: float) -> float:
        ...
        if current_price <= 1e-12:  # Avoid division by zero / nonsensical prices
            logger.error(
                f"Current price ({current_price}) is zero or negative, cannot calculate order size."
            )
            return 0.0
        ...
        amount_tick_size = self.market_info.get("precision", {}).get(
            "amount"
        ...
        if base_order_size_quote <= 0:
            logger.warning(
                "Calculated base order size (quote) is zero or negative. Cannot proceed."
            )
            return 0.0
        ...
        if self.volatility_multiplier is not None and self.volatility_multiplier > 0:
            ...
            if volatility is not None and volatility > 1e-9:
                ...
                )  # Simplified inverse scaling
                size_factor = max(
                    0.1, min(2.0, size_factor)
                )  # Clamp adjustment factor (e.g., 0.1x to 2.0x)
        ...
        if current_price <= 1e-12:  # Re-check after potential adjustments
            logger.error(
                "Current price is zero/negative after adjustments. Cannot convert to base amount."
            )
            return 0.0
    ...
    def _calculate_sl_tp_prices(
        ...
        if self.market_info is None or entry_price <= 1e-12:
            logger.error(
                f"Market info missing or invalid entry price ({entry_price}) for SL/TP calc."
            )
            return None, None
        ...
        if self.use_atr_sl_tp:
            ...
            logger.debug(
                f"Calculating SL/TP using Fixed %: SL={sl_pct * 100:.2f}%, TP={tp_pct * 100:.2f}%"
            )
            if side == "buy":
    ...
    def compute_trade_signal_score(
        ...
        EMA_THRESHOLD_MULTIPLIER = (
            0.0002  # Price must be 0.02% above/below EMA to trigger
        )
        ...
        if orderbook_imbalance is not None:
            ...
            elif orderbook_imbalance == float(
                "inf"
            ):  # Handle infinite imbalance (zero bids)
        ...
        logger.debug(
            f"Signal score calculation complete. Raw Score: {score:.2f}, Final Integer Score: {final_score}"
        )

        return final_score, reasons

    @retry_api_call(
        max_retries=2, initial_delay=2
    )  # Retry order placement a couple of times
    def place_entry_order(
        ...
        if order_size_base <= 0:
            logger.error(
                f"Order size ({order_size_base}) is zero or negative. Cannot place entry order."
            )
            return None
        ...
        try:
            ...
            if order_type == "limit":
                ...
                logger.debug(
                    f"Calculated precise limit price: {limit_price:.{self.price_decimals}f} {quote_currency}"
                )
            ...
            if stop_loss_price is not None:
                ...
                logger.debug(
                    f"Adding SL param: stopLoss={params['stopLoss']}, slTriggerBy={params.get('slTriggerBy')}"
                )
            ...
            if self.exchange_id.lower() == "bybit":
                market_type = self.exchange.market(self.symbol).get(
                    "type", "swap"
                )  # Assume swap if not specified
                is_linear = self.exchange.market(self.symbol).get(
                    "linear", True
                )  # Assume linear if not specified
                if market_type in ["swap", "future"]:
            ...
            logger.error(
                f"{Fore.RED}Error preparing entry order values/parameters: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
        except Exception as e:
            logger.error(
                f"{Fore.RED}Unexpected error preparing entry order: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
        ...
        if self.simulation_mode:
            ...
            simulated_order = {
                ...
                "average": sim_entry_price
                if sim_status == "closed"
                else None,  # Fill price
                "filled": sim_filled,
            ...
            simulated_order["bot_custom_info"] = {
                "confidence": confidence_level,
        ...
        # --- Live Trading Mode ---
            ...
            logger.info(
                f" -> Size: {amount_precise:.{self.amount_decimals}f} {base_currency}"
            )
            logger.info(f" -> Price: {limit_price_info}")
            logger.info(
                f" -> Value: ~{estimated_value_quote:.{self.price_decimals}f} {quote_currency}"
            )
            ...
            try:
                if order_type == "market":
                    ...
                    if limit_price is None:  # Should not happen due to earlier checks
                        logger.error(
                            f"{Fore.RED}Limit price is None. Cannot place live limit order.{Style.RESET_ALL}"
                        )
                        return None
                    ...
                    logger.error(
                        f"Unsupported order type '{order_type}' for live trading."
                    )
                    return None
                ...
                if order:
                    ...
                    oamt_f = (
                        f"{oamt:.{self.amount_decimals}f}"
                        if oamt is not None
                        else "N/A"
                    )
                    ofilled_f = (
                        f"{ofilled:.{self.amount_decimals}f}"
                        if ofilled is not None
                        else "N/A"
                    )
                    oprice_f = (
                        f"{oprice:.{self.price_decimals}f}"
                        if oprice is not None
                        else "N/A"
                    )
                    oavg_f = (
                        f"{oavg:.{self.price_decimals}f}" if oavg is not None else "N/A"
                    )
                    ocost_f = (
                        f"{ocost:.{self.price_decimals}f}"
            ...
            # Handle specific, potentially non-retryable errors based on decorator logic
                ...
                logger.error(
                    f"{Fore.RED}LIVE {action_desc} Order Failed (ExchangeError): {e}{Style.RESET_ALL}"
                )
                logger.error(f" -> Params Sent: {params}")
                return None
            except Exception as e:
    ...
    def cancel_order_by_id(self, order_id: str, symbol: str | None = None) -> bool:
        ...
        if self.simulation_mode:
            ...
            for i, pos in enumerate(self.open_positions):
                ...
                if pos["id"] == order_id and pos["status"] == STATUS_PENDING_ENTRY:
                    ...
                    logger.info(
                        f"[SIMULATION] Marked simulated order {order_id} as canceled."
                    )
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                logger.warning(
                    f"Network error cancelling order {order_id}: {e}. Retrying if possible."
                )
    ...
    def cancel_all_symbol_orders(self, symbol: str | None = None) -> int:
        ...
        if self.simulation_mode:
            logger.info(
                f"[SIMULATION] Simulating cancellation of all PENDING orders for {target_symbol}."
            )
            sim_cancelled = 0
            needs_save = False
            for i, pos in enumerate(self.open_positions):
                if (
                    pos.get("symbol") == target_symbol
                    and pos.get("status") == STATUS_PENDING_ENTRY
                ):
                    self.open_positions[i]["status"] = STATUS_CANCELED
                    self.open_positions[i]["last_update_time"] = time.time()
                    sim_cancelled += 1
            ...
            if sim_cancelled > 0:
                logger.info(
                    f"[SIMULATION] Marked {sim_cancelled} pending simulated orders as canceled."
                )
                if needs_save:
                    self._save_state()
            else:
                logger.info(
                    f"[SIMULATION] No pending simulated orders found for {target_symbol} to cancel."
                )
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                if self.exchange.has["fetchOpenOrders"]:
                    logger.debug(
                        f"Fetching open orders for {target_symbol} to cancel individually..."
                    )
                    params = {}
                    if self.exchange_id.lower() == "bybit":
                        market_type = self.exchange.market(target_symbol).get(
                    ...
                    if not open_orders:
                        logger.info(
                            f"No currently open orders found for {target_symbol} via fetchOpenOrders."
                        )
                        return 0
                    ...
                    for order in open_orders:
                        ...
                        if not order_id:
                            logger.warning(
                                f"Found open order without ID: {str(order)[:100]}... Skipping cancellation."
                            )
                            continue
                ...
                # Fallback to cancelAllOrders if fetchOpenOrders is not supported or preferred
                    ...
                    logger.info(
                        f"'cancelAllOrders' for {target_symbol} response snippet: {str(response)[:100]}..."
                    )
                    # We don't know the exact count from this response typically
                    cancelled_count = (
                        -1
                    )  # Indicate unknown count, but action was attempted
                else:
                ...
                final_msg = (
                    f"Bulk order cancellation process finished for {target_symbol}. "
                )
                ...
                return (
                    cancelled_count  # Return count successfully cancelled before error
                )
    ...
    def _check_pending_entries(self, indicators: dict) -> None:
        ...
        logger.debug(
            f"Checking status of {len(pending_positions)} pending entry order(s)..."
        )
        ...
        for position in pending_positions:
            ...
            if not entry_order_id:
                logger.error(
                    f"Found pending position with no ID: {str(position)[:100]}... Skipping."
                )
                continue
            ...
            if order_status == "closed" and filled_amount > 1e-12:
                ...
                updated_pos["size"] = (
                    filled_amount  # Update size to actual filled amount
                )
                ...
                if current_price_for_check:
                    atr_value = indicators.get(
                        "atr"
                    )  # Get current ATR from passed dict
            ...
            # Case 3: Order Failed (canceled, rejected, expired)
                ...
                reason = order_info.get("info", {}).get(
                    "rejectReason",
                    order_info.get("info", {}).get("cancelType", "No reason provided"),
                )  # Try Bybit fields
                logger.warning(
    ...
    def _manage_active_positions(self, current_price: float, indicators: dict) -> None:
        ...
        for position in active_positions:
            ...
            entry_price = position.get(
                "entry_price"
            )  # Should be set when status became ACTIVE
            ...
            if order_info is None:
                ...
                if abs(order_filled - position_size) > max(
                    position_size * 0.01, 1e-9
                ):  # Allow 1% tolerance
                    ...
                    logger.info(
                        f"{log_color}{exit_reason} at ~{exit_price:.{self.price_decimals}f}{Style.RESET_ALL}"
                    )
                else:
    ...
    def _infer_exit_reason(
        ...
        bybit_stop_order_type = info.get(
            "stopOrderType", ""
        ).lower()  # e.g., 'StopLoss', 'TakeProfit'
    ...
    def _update_trailing_stop_price(
        ...
        try:
            potential_tsl_price = float(
                self.exchange.price_to_precision(self.symbol, potential_tsl_price_raw)
            )
            if potential_tsl_price <= 0:
                potential_tsl_price = None  # Safety
        except Exception as format_err:
            logger.error(
                f"Error formatting potential TSL price {potential_tsl_price_raw}: {format_err}"
            )
        ...
        # --- TSL Update Check ---
            ...
            update_needed = (
                side == "buy" and potential_tsl_price > trailing_sl_price
            ) or (side == "sell" and potential_tsl_price < trailing_sl_price)
    ...
    def _attempt_edit_order_for_tsl(self, position: dict, new_tsl_price: float) -> bool:
        ...
        **EXPERIMENTAL - Use with caution!**.
        ...
        stored_tp_price = position.get(
            "take_profit_price"
        )  # Get existing TP to potentially resend
        ...
        try:
            ...
            if not fresh_order_info:
                logger.warning(
                    f"Cannot fetch fresh order info for {pos_id} ({symbol}). Skipping TSL edit attempt."
                )
                return False
            ...
            if order_type == "limit" and order_price is None:
                logger.error(
                    f"Fetched fresh order info for limit order {pos_id} is missing price field. Cannot edit."
                )
                return False

            logger.debug(
                f"Attempting edit_order for {pos_id} with new SL: {new_tsl_price}. Params: {edit_params}"
            )

            # Use retry decorator for the edit call itself

            @retry_api_call(max_retries=1, initial_delay=1)  # Retry edit once
            def _edit_order_attempt(
                exchange, order_id, sym, o_type, o_side, o_amount, o_price, params
            ):
                return exchange.edit_order(
                    order_id, sym, o_type, o_side, o_amount, o_price, params
                )
    ...
    def _log_position_pnl(
        ...
        try:
            ...
            logger.info(
                f"Daily PnL Updated: {self.daily_pnl:.{self.price_decimals}f} {quote_ccy}"
            )

        except ZeroDivisionError:
            logger.error(
                f"PnL calculation failed for {pos_id}: Division by zero (entry price was likely zero)."
            )
        except Exception as e:
            logger.error(
                f"Unexpected error calculating PnL for {pos_id}: {e}", exc_info=True
            )
    ...
    def _place_market_close_order(
        ...
        if size is None or size <= 1e-12:
            logger.error(
                f"Cannot place market close for position {pos_id}: Size is invalid ({size})."
            )
            return None
        ...
        # --- Live Trading Mode ---
            ...
            try:
                ...
                reduce_only_supported = self.exchange.has.get(
                    "reduceOnly", False
                )  # Check explicit support flag
                if reduce_only_supported:
                    params["reduceOnly"] = True
                    logger.debug(
                        "Using 'reduceOnly=True' parameter for market close order."
                    )
                else:
                    # Attempt sending it anyway for exchanges like Bybit V5 that use it in params
                    if (
                        self.exchange_id.lower() == "bybit"
                    ):  # Example: Assume Bybit needs it in params
                        params["reduceOnly"] = True
                        logger.debug(
                ...
                if order:
                    ...
                    oavg_f = (
                        f"{oavg:.{self.price_decimals}f}"
                        if oavg is not None
                        else "N/A (Market Fill)"
                    )
    ...
    def _fetch_market_data(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            )  # Fetches required lookback automatically
            ...
            if order_book_data is None:
                logger.debug(
                    "Order book data fetch failed or returned None. Proceeding without imbalance signal."
                )
            else:
                order_book_imbalance = order_book_data.get(
                    "imbalance"
                )  # Can be None, float, or inf
            ...
            logger.error(
                f"{Fore.RED}Unexpected error during market data fetching phase: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
    ...
    def _calculate_indicators(self, historical_data: pd.DataFrame) -> dict[str, Any]:
        ...
        if not isinstance(historical_data, pd.DataFrame) or historical_data.empty:
            logger.error(
                "Invalid or empty historical data provided for indicator calculation."
            )
            return {}
        ...
        if len(historical_data) < 2:
            logger.warning(
                f"Insufficient historical data rows ({len(historical_data)}) for most indicators."
            )
    ...
    def _process_signals_and_entry(self, market_data: dict, indicators: dict) -> None:
        ...
        if (
            logger.isEnabledFor(logging.DEBUG) or signal_score != 0
        ):  # Log reasons if debug or non-zero score
            for reason in reasons:
                logger.debug(
                    f"  -> {reason}"
                )  # Log reasons as debug unless score triggers action
        ...
        if entry_side:
            ...
            if sl_tp_required and sl_tp_failed:
                ...
                sl_tp_failed
            ):  # SL/TP not strictly required, but calculation failed anyway
                logger.warning(
            ...
            if entry_order:
                ...
                if order_status == "open":
                    ...
                    logger.info(
                        f"Entry order {order_id} placed with status 'open'. Added as PENDING."
                    )
                elif (
                    order_status == "closed"
                ):  # Assumed filled for market or immediate limit fills
                    ...
                    initial_pos_status = (
                        STATUS_PENDING_ENTRY  # Treat unknowns as pending
                    )
                ...
                entry_price_for_state = (
                    avg_fill_price if is_filled_immediately and avg_fill_price else None
                )
                entry_time_for_state = (
                    order_timestamp_ms / 1000
                    if is_filled_immediately and order_timestamp_ms
                    else None
                )
                size_for_state = (
                    filled_amount if is_filled_immediately else requested_amount
                )
        ...
        while True:
            ...
            try:
                ...
                if market_data is None:
                    logger.warning(
                        "Essential market data missing, pausing before next iteration."
                    )
                ...
                logger.info(
                    f"{loop_prefix} Current Price: {format_price(current_price)} OB Imbalance: {ob_imb_str}"
                )
                ...
                if not indicators:  # Check if indicator calculation failed critically
                    logger.error(
                        "Indicator calculation failed critically. Skipping trading logic."
                    )
                    time.sleep(DEFAULT_SLEEP_INTERVAL_SECONDS)
                    continue
                ...
                logger.debug(
                    f"{loop_prefix} Iteration took {execution_time:.2f}s. Waiting {wait_time:.2f}s..."
                )
                ...
                logger.warning(
                    f"SystemExit called with code {e.code}. Exiting main loop..."
                )
    ...
    def shutdown(self) -> None:
        ...
        if not pending_orders_to_cancel:
            ...
            logger.info(
                f"Attempting to cancel {len(pending_orders_to_cancel)} PENDING entry order(s)..."
            )
        ...
        if self.close_positions_on_exit and active_positions_to_manage:
            ...
            if current_price_for_close is None:
                ...
                for position in list(active_positions_to_manage):  # Iterate copy
                    ...
                    logger.info(
                        f"Attempting market close for active position {pos_id} ({pos_symbol})..."
                    )
                    close_order = self._place_market_close_order(
                        position, current_price_for_close
                    )
                    ...
                    if close_order:
                        ...
                        close_fill_price = (
                            close_order.get("average") or current_price_for_close
                        )
                        self._log_position_pnl(
                            position,
                            close_fill_price,
                            f"Closed on Exit (Market Order {close_order.get('id')})",
                        )
                    else:
                ...
                if failed_close_ids:
                    logger.error(
                        f"Failed close order IDs: {failed_close_ids}. MANUAL ACTION REQUIRED."
                    )
...
def format_price(
    price: float | int | str | None, decimals: int = DEFAULT_PRICE_DECIMALS
) -> str:
...
def format_amount(
    amount: float | int | str | None, decimals: int = DEFAULT_AMOUNT_DECIMALS
) -> str:
...
if __name__ == "__main__":
    ...
    try:
        ...
        bot_instance.format_price = lambda p: format_price(
            p, bot_instance.price_decimals
        )
        bot_instance.format_amount = lambda a: format_amount(
            a, bot_instance.amount_decimals
        )
        ...
        if exit_code not in [0, 130]:  # Log if not clean exit or Ctrl+C
            logger.error(
                f"Bot exited via SystemExit with unexpected code: {exit_code}."
            )
        else:
