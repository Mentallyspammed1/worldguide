            ...
            while retries <= max_retries:
                ...
                # --- Exchange Errors - Distinguish Retryable vs Non-Retryable ---
                    ...
                    if any(phrase in err_str for phrase in [
                        ...
                        "order is finished", "order has been filled", "already closed",  # Variations of closed states
                        ...
                        logger.warning(f"{Fore.YELLOW}Order vanished, unknown, or final state encountered for {func_name}: {e}. Returning None (no retry).{Style.RESET_ALL}")
                    ...
                    # Funding / Margin Related
                        ...
                        "available balance insufficient", "insufficient funds"  # Added variations
                         ...
                         logger.error(f"{Fore.RED}Insufficient essence (funds/margin) for {func_name}: {e}. Aborting operation (no retry).{Style.RESET_ALL}")
                    ...
                    # Invalid Request / Parameters / Limits / Config
                        ...
                        "leverage not modified",  # Config/setup error
                         ...
                         logger.error(f"{Fore.RED}Invalid parameters, limits, auth, or config for {func_name}: {e}. Aborting operation (no retry). Review logic/config/permissions.{Style.RESET_ALL}")
                    ...
                    elif any(phrase in err_str for phrase in ["nonce", "timeout", "service unavailable", "internal error", "busy"]):
                        logger.warning(
    ...
    **Disclaimer:** Trading cryptocurrencies involves significant risk. This bot
    ...
    def _load_state(self) -> None:
        ...
        try:
                    ...
                    if isinstance(saved_state, list):
                        ...
                        for i, pos in enumerate(saved_state):
                            if isinstance(pos, dict) and required_keys.issubset(pos.keys()):
                                ...
                                for key in ['size', 'original_size', 'entry_price', 'entry_time', 'stop_loss_price', 'take_profit_price', 'trailing_stop_price', 'last_update_time']:
                                     if key in pos and isinstance(pos[key], str):
                                          try: pos[key] = float(pos[key])
                                          except (ValueError, TypeError): pass  # Keep as string if conversion fails
                                valid_positions.append(pos)
                            else:
                                logger.warning(f"Ignoring invalid entry #{i + 1} in state file (not a valid position dict or missing keys): {str(pos)[:100]}...")  # Log truncated entry
            ...
            logger.error(f"{Fore.RED}Error decoding/validating state file {self.state_file}: {e}. Attempting recovery...{Style.RESET_ALL}")
        ...
        try:
            ...
            if os.path.exists(self.state_file):
                try:
                    ...
                    logger.warning(f"Could not create state backup {state_backup_file}: {backup_err}. Proceeding with overwrite cautiously.")
            ...
            if os.path.exists(temp_state_file):
                with contextlib.suppress(OSError): os.remove(temp_state_file)
        except TypeError as e:
             logger.error(f"{Fore.RED}Could not serialize state data to JSON: {e}. Check for non-serializable types in open_positions.{Style.RESET_ALL}", exc_info=True)
             # Clean up temporary file if it exists after an error
             if os.path.exists(temp_state_file):
                with contextlib.suppress(OSError): os.remove(temp_state_file)
        except Exception as e:
    ...
    def validate_config(self) -> None:
        ...
        try:
            ...
            _check(isinstance(offset_buy, (int, float)) and offset_buy >= 0, "trading.limit_order_offset_buy", "Must be a non-negative number.")
            offset_sell = _get_nested(cfg_tr, ["limit_order_offset_sell"], req_type=(int, float))
            _check(isinstance(offset_sell, (int, float)) and offset_sell >= 0, "trading.limit_order_offset_sell", "Must be a non-negative number.")
            ...
            _check(isinstance(imbalance, (int, float)) and imbalance > 0, "order_book.imbalance_threshold", "Must be a positive number.")
            ...
            _check(isinstance(vol_mult, (int, float)) and vol_mult >= 0, "indicators.volatility_multiplier", "Must be a non-negative number.")
            ...
            if use_atr:
                 ...
                 _check(isinstance(atr_sl_mult, (int, float)) and atr_sl_mult > 0, "risk_management.atr_sl_multiplier", "Must be a positive number when use_atr_sl_tp is true.")
                 atr_tp_mult = _get_nested(cfg_rm, ["atr_tp_multiplier"], req_type=(int, float))
                 _check(isinstance(atr_tp_mult, (int, float)) and atr_tp_mult > 0, "risk_management.atr_tp_multiplier", "Must be a positive number when use_atr_sl_tp is true.")
            else:
                 ...
                 _check(sl_pct is not None and isinstance(sl_pct, (int, float)) and 0 < sl_pct < 1, "risk_management.stop_loss_percentage", "Must be a number between 0 and 1 (exclusive, e.g., 0.01 for 1%) when use_atr_sl_tp is false.")
            ...
            _check(isinstance(order_size_pct, (int, float)) and 0 < order_size_pct <= 1, "risk_management.order_size_percentage", "Must be a number between 0 (exclusive) and 1 (inclusive, e.g., 0.05 for 5%).")
            max_pos = _get_nested(cfg_rm, ["max_open_positions"], req_type=int)
            _check(isinstance(max_pos, int) and max_pos > 0, "risk_management.max_open_positions", "Must be a positive integer.")
            ...
            _check(isinstance(strong_adj, (int, float)) and strong_adj > 0, "risk_management.strong_signal_adjustment_factor", "Must be a positive number.")
            weak_adj = _get_nested(cfg_rm, ["weak_signal_adjustment_factor"], req_type=(int, float))
            _check(isinstance(weak_adj, (int, float)) and weak_adj > 0, "risk_management.weak_signal_adjustment_factor", "Must be a positive number.")
    ...
    def _initialize_exchange(self) -> ccxt.Exchange:
        ...
        if not self.simulation_mode and not creds_found:
             logger.critical(f"{Fore.RED}API Key/Secret essence missing (check environment variables BYBIT_API_KEY/SECRET or config file). Cannot trade live or on testnet. Aborting.{Style.RESET_ALL}")
             sys.exit(1)
        ...
        try:
            ...
            if not self.simulation_mode and self.testnet_mode:
                try:
                    ...
                    logger.info(f"{Fore.YELLOW}Exchange sandbox mode explicitly enabled via CCXT.{Style.RESET_ALL}")
                     ...
                     logger.warning(f"{Fore.YELLOW}Exchange {self.exchange_id} does not support unified set_sandbox_mode via CCXT. Testnet functionality depends on API key type and exchange base URL (check Bybit key settings).{Style.RESET_ALL}")
            ...
            if not self.simulation_mode:
                ...
                if server_time_ms is not None:
                    ...
                    logger.critical(f"{Fore.RED}Initial API connectivity test (fetch_time) failed. Check credentials, permissions, network, and exchange status. Aborting.{Style.RESET_ALL}")
                    sys.exit(1)
        ...
        try:
            ...
            if log_val == math.floor(log_val):  # Check if it's an integer (negative for < 1)
                ...
                if '.' in s:
                    ...
                    logger.warning(f"Unexpected float tick size format: {tick_size}. Could not find decimal point in formatted string '{s}'. Returning default.")
                    return fallback_decimals
            ...
            logger.error(f"Unexpected error calculating decimal places for tick size {tick_size}: {e}. Returning default.")
            return fallback_decimals
    ...
    def _load_market_info(self) -> None:
        ...
        try:
            ...
            if amount_tick is None:
                logger.warning(f"{Fore.YELLOW}Amount precision (tick size) info missing for {self.symbol}. Using default amount decimals: {self.amount_decimals}. Amount formatting/checks may be inaccurate.{Style.RESET_ALL}")
            if price_tick is None:
                logger.warning(f"{Fore.YELLOW}Price precision (tick size) info missing for {self.symbol}. Using default price decimals: {self.price_decimals}. Price formatting/checks may be inaccurate.{Style.RESET_ALL}")
            if min_amount is None:
                 logger.warning(f"{Fore.YELLOW}Minimum order amount limit missing for {self.symbol}. Cannot enforce minimum amount check effectively.{Style.RESET_ALL}")
            if min_cost is None:
                 logger.warning(f"{Fore.YELLOW}Minimum order cost limit missing for {self.symbol}. Cannot enforce minimum cost check effectively.{Style.RESET_ALL}")
            ...
            logger.debug("Market details loaded and cached.")
             ...
             logger.critical(f"{Fore.RED}KeyError accessing market info for {self.symbol}: {e}. Market structure might be unexpected or incomplete. Market Info: {self.market_info}. Aborting.{Style.RESET_ALL}", exc_info=False)
             sys.exit(1)
        except ValueError as e:  # Catch specific value errors from validation/retrieval
             logger.critical(f"{Fore.RED}ValueError loading market info for {self.symbol}: {e}. Aborting.{Style.RESET_ALL}", exc_info=False)
             sys.exit(1)
        except Exception as e:
    ...
    def fetch_order_book(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not valid_bids or not valid_asks:
                 logger.warning(f"{Fore.YELLOW}Order book data invalid or incomplete for {self.symbol}. Valid Bids: {len(valid_bids)}, Valid Asks: {len(valid_asks)}. Raw: bids={len(bids)}, asks={len(asks)}{Style.RESET_ALL}")
            ...
            logger.error(f"{Fore.RED}Error fetching or processing order book for {self.symbol}: {e}{Style.RESET_ALL}", exc_info=False)  # Reduce log spam for common OB issues
    ...
    def fetch_historical_data(self, limit: int | None = None) -> pd.DataFrame | None:
        ...
        if limit is None:
             ...
             required_periods = [
                 ...
                 self.stoch_rsi_period + max(self.stoch_rsi_k_period, self.stoch_rsi_d_period) + self.rsi_period,  # StochRSI needs RSI series first
                 self.atr_period + 1
             ]
             ...
             required_limit = min(required_limit, 1000)  # Max limit depends on exchange (e.g., Bybit often 1000 or 1500 per request)
        else:
        ...
        try:
            ...
            if len(ohlcv) < 5:  # Arbitrary small number, need *some* data
                 logger.warning(f"{Fore.YELLOW}Very few historical OHLCV data points returned ({len(ohlcv)}). Insufficient for most indicators.{Style.RESET_ALL}")
                 return None
            ...
            if final_len < min_required_len:
                 logger.warning(f"{Fore.YELLOW}Insufficient valid historical data after cleaning for {self.symbol}. Fetched/Returned {initial_len}, valid {final_len}, needed ~{min_required_len} for full indicator calc. Indicators might be inaccurate or fail.{Style.RESET_ALL}")
    ...
    def fetch_balance(self, currency_code: str | None = None) -> float:
        ...
        if not quote_currency:
             logger.error(f"{Fore.RED}Cannot determine quote currency to fetch balance (market_info: {self.market_info}). Using 0.0{Style.RESET_ALL}")
             return 0.0
        ...
        try:
            ...
            if is_free_balance_zero:
                logger.debug(f"Standard 'free' balance for {quote_currency} is zero or missing. Checking 'info' for alternatives...")
                ...
                if self.exchange_id.lower() == 'bybit' and isinstance(info_data, dict):
                    try:
                        ...
                        if result_list and isinstance(result_list[0], dict):
                            ...
                            if account_type in ['UNIFIED', 'CONTRACT']:
                                ...
                                if unified_avail_str is not None:
                                    logger.debug(f"Using Bybit {account_type} 'totalAvailableBalance': {unified_avail_str}")
                                    ...
                                    for coin_entry in coin_list:
                                        if isinstance(coin_entry, dict) and coin_entry.get('coin') == quote_currency:
                                            ...
                                            coin_avail = coin_entry.get('availableBalance')  # This seems most relevant for margin trading
                                            if coin_avail is None: coin_avail = coin_entry.get('availableToWithdraw')  # Fallback

                                            if coin_avail is not None:
                                                 logger.debug(f"Using coin-specific '{quote_currency}' balance ('availableBalance' or fallback) within {account_type} account: {coin_avail}")
                            ...
                            # SPOT Account Check
                                ...
                                for coin_entry in coin_list:
                                     if isinstance(coin_entry, dict) and coin_entry.get('coin') == quote_currency:
                                         ...
                                         if spot_avail is None: spot_avail = coin_entry.get('walletBalance')  # Fallback

                                         if spot_avail is not None:
                                              logger.debug(f"Using Bybit SPOT balance ('free' or 'walletBalance') for {quote_currency}: {spot_avail}")
                        ...
                        logger.warning(f"{Fore.YELLOW}Could not parse expected Bybit structure in balance 'info': {bybit_info_err}. Raw info snippet: {str(info_data)[:200]}...{Style.RESET_ALL}")
            ...
            if is_available_balance_zero and quote_currency in balance_data:
                 ...
                 if total_balance is not None:
                     logger.warning(f"{Fore.YELLOW}Available balance is zero/missing. Using 'total' balance ({total_balance}) as a fallback for {quote_currency}. This includes used margin/collateral.{Style.RESET_ALL}")
            ...
            if available_balance is not None:
                try:
                    ...
                    if final_balance < 0:
                        logger.warning(f"{Fore.YELLOW}Fetched balance for {quote_currency} is negative ({final_balance}). Treating as 0.0.{Style.RESET_ALL}")
                        final_balance = 0.0
                except (ValueError, TypeError) as conv_err:
                    logger.error(f"{Fore.RED}Could not convert final balance value '{available_balance}' to float for {quote_currency}: {conv_err}. Returning 0.0{Style.RESET_ALL}")
                    final_balance = 0.0
            else:
                logger.warning(f"{Fore.YELLOW}Could not determine available balance for {quote_currency} after checking standard, info, and total fields. Returning 0.0.{Style.RESET_ALL}")
    ...
    def fetch_order_status(self, order_id: str, symbol: str | None = None) -> dict[str, Any] | None:
            ...
            for pos in self.open_positions:
                 if pos['id'] == order_id:
                    ...
                    ccxt_status = 'open' if sim_status == STATUS_PENDING_ENTRY else \
                                  ...
                                  sim_status  # Pass others like 'unknown'
                    ...
                    return {
                        'id': order_id, 'clientOrderId': order_id,
                        'symbol': target_symbol, 'status': ccxt_status,
                        ...
                        'average': sim_avg, 'timestamp': sim_timestamp_ms, 'datetime': sim_datetime,
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                logger.debug(f"Order {order_id} ({order_type} {order_side}): Status={status}, Amount={amount_str}, Filled={filled_str}, AvgPrice={avg_price_str}, Remaining={remaining_str}")

                return order_info

            except ccxt.OrderNotFound as e:
                # This is common, treat as non-error, indicates order is final or never existed
                logger.warning(f"{Fore.YELLOW}Order {order_id} not found on exchange ({target_symbol}). Assumed closed, cancelled, or invalid ID. Error: {e}{Style.RESET_ALL}")
                return None  # Return None, let the calling function decide how to interpret this
                 ...
                 err_str = str(e).lower()
                 if "order is finished" in err_str or "order has been filled" in err_str or "already closed" in err_str:  # Common variations for closed orders
                      logger.warning(f"{Fore.YELLOW}Order {order_id} not found (reported as finished/filled/closed). Assuming closed/cancelled. Error: {e}{Style.RESET_ALL}")
                      return None
                 else:
    ...
    def calculate_macd(close_prices: pd.Series, short_period: int, long_period: int, signal_period: int) -> tuple[float | None, float | None, float | None]:
        ...
        if len(close_prices) < min_len:
            logger.debug(f"Insufficient data for MACD ({short_period},{long_period},{signal_period}) (need ~{min_len}, got {len(close_prices)}).")
            return None, None, None
        try:
    ...
    def calculate_stoch_rsi(close_prices: pd.Series, rsi_period: int, stoch_period: int, k_period: int, d_period: int) -> tuple[float | None, float | None]:
        ...
        if len(close_prices) < min_len:
            logger.debug(f"Insufficient data for StochRSI ({rsi_period},{stoch_period},{k_period},{d_period}) (need ~{min_len}, got {len(close_prices)}).")
            return None, None
        try:
            ...
            if len(rsi_series) < stoch_period:
                logger.debug(f"Insufficient non-NaN RSI series length for StochRSI window (need {stoch_period}, got {len(rsi_series)}).")
                return None, None
            ...
            logger.error(f"Error calculating StochRSI ({rsi_period},{stoch_period},{k_period},{d_period}): {e}", exc_info=False)
            return None, None
    ...
    def calculate_atr(high_prices: pd.Series, low_prices: pd.Series, close_prices: pd.Series, period: int) -> float | None:
        ...
        try:
            ...
            tr = tr.fillna(0)  # Fill initial NaN with 0 (or handle differently if desired, e.g., start ATR after first TR)
    ...
    def calculate_order_size(self, current_price: float) -> float:
        ...
        if balance <= 0:
            ...
            logger.info(f"Insufficient balance ({balance:.{self.price_decimals}f} {quote_currency}) for new order. Need more {quote_currency}.")
            return 0.0

        # --- 2. Calculate Base Order Size (Quote Value) ---
        base_order_size_quote = balance * self.order_size_percentage
        logger.debug(f"Initial calculated order size (quote): {base_order_size_quote:.{self.price_decimals}f} {quote_currency} (Balance: {balance:.{self.price_decimals}f}, Pct: {self.order_size_percentage * 100:.2f}%)")
        ...
        if self.volatility_multiplier is not None and self.volatility_multiplier > 0:
            ...
            if volatility is not None and volatility > 1e-9:
                ...
                logger.debug(f"Volatility adjustment skipped: Volatility N/A ({volatility}) or multiplier is zero/invalid.")
        else:
        ...
        logger.debug(f"Calculated raw base amount: {order_size_base_raw:.{self.amount_decimals + 4}f} {base_currency}")  # Log with extra precision before rounding
        ...
        try:
            ...
            logger.debug(f"Amount after applying exchange precision: {amount_precise:.{self.amount_decimals}f} {base_currency}")

            # Check if precision resulted in zero amount
            if amount_precise <= 1e-12:  # Use epsilon
                 logger.warning(f"{Fore.YELLOW}Order amount became zero after applying precision ({amount_tick_size}). Original raw amount: {order_size_base_raw:.{self.amount_decimals + 4}f}. Cannot place order.{Style.RESET_ALL}")
                 return 0.0
            ...
            if min_amount is not None and amount_precise < min_amount:
                ...
                logger.warning(f"{Fore.YELLOW}Calculated precise order amount {amount_precise:.{self.amount_decimals}f} {base_currency} is below exchange minimum required: {min_amount}. Cannot place order.{Style.RESET_ALL}")
                return 0.0
            ...
            if min_cost is not None and estimated_cost < min_cost:
                logger.warning(f"{Fore.YELLOW}Estimated order cost {estimated_cost:.{self.price_decimals}f} {quote_currency} is below exchange minimum required: {min_cost}. Cannot place order. (Amt: {amount_precise}, Price: {price_precise}){Style.RESET_ALL}")
                return 0.0
            ...
            logger.error(f"{Fore.RED}Error applying precision or checking limits (ccxt.InvalidOrder): {e}. Raw amount: {order_size_base_raw:.{self.amount_decimals + 4}f}. Cannot place order.{Style.RESET_ALL}")
            return 0.0
        except (ccxt.ExchangeError, ValueError, TypeError, KeyError) as e:
            # Catch other potential errors during precision/limit handling
            logger.error(f"{Fore.RED}Error applying precision/limits: {type(e).__name__} - {e}. Raw amount: {order_size_base_raw:.{self.amount_decimals + 4}f}.{Style.RESET_ALL}")
            return 0.0
    ...
    def _calculate_sl_tp_prices(self, entry_price: float, side: str, current_price: float, atr: float | None) -> tuple[float | None, float | None]:
        ...
        if self.use_atr_sl_tp:
            if atr is None or atr <= 1e-12:
                ...
                return None, None
            if not (self.atr_sl_multiplier and self.atr_tp_multiplier and self.atr_sl_multiplier > 0 and self.atr_tp_multiplier > 0):
                 logger.warning(f"{Fore.YELLOW}ATR SL/TP enabled, but multipliers invalid (SL={self.atr_sl_multiplier}, TP={self.atr_tp_multiplier}). Cannot calculate SL/TP.{Style.RESET_ALL}")
                 return None, None

            logger.debug(f"Calculating SL/TP using ATR={format_price(atr)}, SL Mult={self.atr_sl_multiplier}, TP Mult={self.atr_tp_multiplier}")
            ...
            if side == "buy":
                ...
                stop_loss_price_raw = entry_price + sl_delta
                take_profit_price_raw = entry_price - tp_delta
            else: logger.error(f"Invalid side '{side}' for SL/TP calculation."); return None, None
            ...
            if not (sl_pct and tp_pct and 0 < sl_pct < 1 and 0 < tp_pct < 1):
                 logger.warning(f"{Fore.YELLOW}Fixed % SL/TP enabled, but percentages invalid/missing (SL={sl_pct}, TP={tp_pct}). Cannot calculate SL/TP.{Style.RESET_ALL}")
                 return None, None
            ...
            if stop_loss_price_raw is not None and stop_loss_price_raw > 1e-12:
                 ...
                 if (side == 'buy' and stop_loss_price_final >= entry_price) or \
                    (side == 'sell' and stop_loss_price_final <= entry_price):
                      logger.warning(f"{Fore.YELLOW}Calculated SL {format_price(stop_loss_price_final)} is on wrong side of entry {format_price(entry_price)} after precision. Setting SL to None.{Style.RESET_ALL}")
                      stop_loss_price_final = None
            elif stop_loss_price_raw is not None:  # If raw SL was zero or negative
                 logger.warning(f"Raw calculated stop loss price ({format_price(stop_loss_price_raw)}) is zero or negative. Setting SL to None.")
                 stop_loss_price_final = None
            ...
            if take_profit_price_raw is not None and take_profit_price_raw > 1e-12:
                 ...
                 if (side == 'buy' and take_profit_price_final <= entry_price) or \
                    (side == 'sell' and take_profit_price_final >= entry_price):
                      logger.warning(f"{Fore.YELLOW}Calculated TP {format_price(take_profit_price_final)} is on wrong side of entry {format_price(entry_price)} after precision. Setting TP to None.{Style.RESET_ALL}")
                      take_profit_price_final = None
            elif take_profit_price_raw is not None:  # If raw TP was zero or negative
                 logger.warning(f"Raw calculated take profit price ({format_price(take_profit_price_raw)}) is zero or negative. Setting TP to None.")
                 take_profit_price_final = None

        except (ccxt.ExchangeError, ValueError, TypeError) as e:
            logger.error(f"{Fore.RED}Error applying precision to SL/TP prices: {e}. Raw SL={format_price(stop_loss_price_raw)}, TP={format_price(take_profit_price_raw)}. Setting both to None.{Style.RESET_ALL}")
            return None, None
        except Exception as e:
        ...
        if stop_loss_price_final is not None and take_profit_price_final is not None:
             if (side == "buy" and stop_loss_price_final >= take_profit_price_final) or \
                (side == "sell" and stop_loss_price_final <= take_profit_price_final):
                  logger.warning(f"{Fore.YELLOW}Final check: Calculated SL {format_price(stop_loss_price_final)} conflicts with TP {format_price(take_profit_price_final)} for {side.upper()} position. Setting TP to None to avoid conflict.{Style.RESET_ALL}")
                  take_profit_price_final = None
        ...
        logger.debug(f"Final calculated SL={format_price(stop_loss_price_final)}, TP={format_price(take_profit_price_final)} for {side} entry near {format_price(entry_price)}")
        return stop_loss_price_final, take_profit_price_final

    def compute_trade_signal_score(self, price: float, indicators: dict[str, float | tuple[float | None, ...] | None], orderbook_imbalance: float | None) -> tuple[int, list[str]]:
        ...
        if orderbook_imbalance is not None:
            if self.imbalance_threshold <= 0:
                ...
                reason_str = f"{Fore.RED}[-1.0] OB Sell Pressure (Imb: Inf > {self.imbalance_threshold:.2f}){Style.RESET_ALL}"
                 ...
                 if imb < imb_buy_thresh:
                     score += 1.0
                     reason_str = f"{Fore.GREEN}[+1.0] OB Buy Pressure (Imb: {imb:.2f} < {imb_buy_thresh:.2f}){Style.RESET_ALL}"
        ...
        if rsi is not None:
            ...
            if rsi < RSI_OVERSOLD:
                ...
                reason_str = f"{Fore.WHITE}[ 0.0] RSI Neutral ({RSI_OVERSOLD} <= {rsi_f} <= {RSI_OVERBOUGHT}){Style.RESET_ALL}"
        else:
        ...
        if stoch_k is not None and stoch_d is not None:
            ...
            if stoch_k < STOCH_OVERSOLD and stoch_d < STOCH_OVERSOLD:
                score += 1.0
                reason_str = f"{Fore.GREEN}[+1.0] StochRSI Oversold (K={k_f}, D={d_f} < {STOCH_OVERSOLD}){Style.RESET_ALL}"
            # Condition for Overbought: Both K and D above threshold
            elif stoch_k > STOCH_OVERBOUGHT and stoch_d > STOCH_OVERBOUGHT:
                score -= 1.0
                reason_str = f"{Fore.RED}[-1.0] StochRSI Overbought (K={k_f}, D={d_f} > {STOCH_OVERBOUGHT}){Style.RESET_ALL}"
    ...
    def place_entry_order(
        ...
        try:
            ...
            logger.debug(f"Using pre-validated precise entry amount: {amount_precise:.{self.amount_decimals}f} {base_currency}")
            ...
            if order_type == "limit":
                ...
                if (side == 'buy' and limit_price >= current_price) or \
                   (side == 'sell' and limit_price <= current_price):
                    logger.warning(f"{Fore.YELLOW}Calculated limit price {format_price(limit_price)} is not favorable compared to current price {format_price(current_price)} for {side.upper()} order. Proceeding, but check offset.{Style.RESET_ALL}")
        ...
        if self.simulation_mode:
            ...
            simulated_order = {
                ...
                "timestamp": sim_timestamp_ms, "datetime": sim_datetime,
                ...
                "status": sim_status,
                "fee": None, "fees": [],  # Simulate no fees for simplicity
        ...
        # --- Live Trading Mode ---
            ...
            try:
                ...
                if order:
                    ...
                    if oavg: log_price_details += f", AvgFill: {oavg_f}"
                    ...
                    logger.error(f"{Fore.RED}LIVE {action_desc} order placement potentially failed: API call succeeded but returned None or empty response. Check exchange status/order history manually.{Style.RESET_ALL}")
    ...
    def cancel_order_by_id(self, order_id: str, symbol: str | None = None) -> bool:
        ...
        if self.simulation_mode:
            ...
            if not found_and_updated:
                 logger.warning(f"[SIMULATION] Simulated order {order_id} not found or not in cancellable state ({STATUS_PENDING_ENTRY}). Assuming cancellation is not needed or already done.")
            # In simulation, return True signifies the intent was processed or unnecessary
            if needs_save: self._save_state()
            return True
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                logger.info(f"{Fore.YELLOW}---> Successfully initiated cancellation for order {order_id}. Response snippet: {str(response)[:100]}...{Style.RESET_ALL}")
                return True
                ...
                return True
                 ...
                 if any(phrase in err_str for phrase in ["order has been filled", "order is finished", "already closed", "already cancelled", "order status error", "cannot be cancelled"]):
                      logger.warning(f"{Fore.YELLOW}Cannot cancel order {order_id}: Already filled, finished, or in non-cancellable state. Error: {e}{Style.RESET_ALL}")
    ...
    def cancel_all_symbol_orders(self, symbol: str | None = None) -> int:
        ...
        """
        target_symbol = symbol or self.symbol
        logger.info(f"{Fore.YELLOW}Checking for and attempting to cancel all OPEN orders for {target_symbol}...{Style.RESET_ALL}")
        cancelled_count = 0
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                if self.exchange.has['fetchOpenOrders']:
                    ...
                    for order in open_orders:
                        ...
                        if self.cancel_order_by_id(order_id, target_symbol):
                            ...
                            logger.error(f"Failed to cancel order {order_id} during bulk cancellation attempt. Continuing...")
                    logger.info(f"Individual cancellation attempt finished. Successfully cancelled: {cancelled_count}/{len(open_orders)}.")

                # Fallback to cancelAllOrders if fetchOpenOrders is not supported or preferred
                elif self.exchange.has['cancelAllOrders']:
                     logger.warning(f"{Fore.YELLOW}Exchange lacks 'fetchOpenOrders' or it failed, attempting unified 'cancelAllOrders' for {target_symbol}...{Style.RESET_ALL}")
                     params = {}
                     if self.exchange_id.lower() == 'bybit':
                         market_type = self.exchange.market(target_symbol).get('type', 'swap')
                         is_linear = self.exchange.market(target_symbol).get('linear', True)
                     ...
                     logger.error(f"{Fore.RED}Exchange {self.exchange_id} does not support 'fetchOpenOrders' or 'cancelAllOrders' for {target_symbol}. Cannot cancel all orders automatically.{Style.RESET_ALL}")
                ...
                logger.error(f"{Fore.RED}Error during bulk order cancellation process for {target_symbol}: {e}{Style.RESET_ALL}", exc_info=True)
    ...
    def _check_pending_entries(self, indicators: dict) -> None:
        ...
        for position in pending_positions:
            ...
            if order_info is None:
                logger.warning(f"{Fore.YELLOW}Pending entry order {entry_order_id} ({pos_symbol}) status unknown/not found. Assuming closed/cancelled externally. Removing from pending state.{Style.RESET_ALL}")
            ...
            if order_status == 'closed' and filled_amount > 1e-12:
                if entry_price is None or float(entry_price) <= 1e-12:
                    ...
                    logger.error(f"{Fore.RED}Entry order {entry_order_id} ({pos_symbol}) is 'closed' but has invalid/missing avg fill price ({entry_price}). Cannot activate reliably. Removing.{Style.RESET_ALL}")
                    positions_to_remove_ids.add(entry_order_id)
                    needs_state_save = True
                    continue
                ...
                if orig_size is not None and abs(filled_amount - orig_size) > max(orig_size * 0.01, 1e-9):  # Allow 1% tolerance
                     logger.warning(f"{Fore.YELLOW}Entry order {entry_order_id} filled amount ({filled_amount:.{self.amount_decimals}f}) differs from requested ({orig_size:.{self.amount_decimals}f}). Using actual filled amount.{Style.RESET_ALL}")

                logger.info(f"{Fore.GREEN}---> Pending entry order {entry_order_id} ({pos_symbol}) FILLED! Amount: {filled_amount:.{self.amount_decimals}f}, Avg Price: {entry_price:.{self.price_decimals}f}{Style.RESET_ALL}")
                ...
                if current_price_for_check:
                    ...
                    logger.info(f"Stored internal SL={format_price(sl_price)}, TP={format_price(tp_price)} for activated pos {entry_order_id} based on actual entry {entry_price:.{self.price_decimals}f}.")
                     ...
                     logger.warning(f"{Fore.YELLOW}Could not fetch current price. SL/TP recalculation skipped after entry fill for {entry_order_id}. Internal SL/TP might be based on original estimate.{Style.RESET_ALL}")
            ...
            # Case 3: Order Failed (canceled, rejected, expired)
                ...
                reason = order_info.get('info', {}).get('rejectReason', order_info.get('info', {}).get('cancelType', 'No reason provided'))  # Try Bybit fields
                logger.warning(f"{Fore.YELLOW}Pending entry order {entry_order_id} ({pos_symbol}) failed (Status: {order_status}, Reason: {reason}). Removing from state.{Style.RESET_ALL}")
                positions_to_remove_ids.add(entry_order_id)
                needs_state_save = True
            ...
            # Case 4: Order Still Open or Partially Filled (remains pending)
                 ...
                 logger.debug(f"Pending entry order {entry_order_id} ({pos_symbol}) is still 'open'. Filled: {filled_f}. Waiting...")
                 ...
                 logger.warning(f"{Fore.YELLOW}Pending entry order {entry_order_id} ({pos_symbol}) has status 'closed' but zero filled amount. Assuming cancelled/rejected pre-fill. Removing.{Style.RESET_ALL}")
                 positions_to_remove_ids.add(entry_order_id)
                 needs_state_save = True
            else:
                 # Unexpected status for a pending order
                 logger.warning(f"Pending entry order {entry_order_id} ({pos_symbol}) has unexpected status: {order_status}. Filled: {filled_amount}. Leaving as pending for now.")
        ...
        if needs_state_save:
            ...
            logger.debug(f"Pending checks complete. Filled/Activated: {updated_count}, Removed/Failed: {removed_count}. Current total positions: {len(self.open_positions)}")
    ...
    def _manage_active_positions(self, current_price: float, indicators: dict) -> None:
        ...
        for position in active_positions:
            ...
            if not pos_id: logger.error(f"Active position missing ID: {position}"); continue
            ...
            if not all([symbol, side, entry_price, position_size, entry_time]):
                 logger.error(f"Active position {pos_id} ({symbol}) has incomplete essential data: entry={entry_price}, size={position_size}, time={entry_time}. Skipping management.")
                 continue
            if position_size <= 1e-12:
                 logger.warning(f"{Fore.YELLOW}Active position {pos_id} ({symbol}) has size ~0 ({position_size}). Assuming closed. Removing.{Style.RESET_ALL}")
            ...
            if order_info is None:
                ...
                logger.warning(f"{Fore.YELLOW}Active position's main order {pos_id} ({symbol}) not found. Assuming closed/cancelled externally. Removing position.{Style.RESET_ALL}")
                ...
                exit_price = current_price  # Best guess is current price
                 ...
                 if abs(order_filled - position_size) > max(position_size * 0.01, 1e-9):  # Allow 1% tolerance
                    ...
                    logger.info(f"Order {pos_id} ({symbol}) status is 'closed', filled amount ({order_filled}) differs from position size ({position_size}). Inferring external closure.")
                    exit_price_raw = order_info.get('average') or order_info.get('price') or current_price  # Use average if available (unlikely for closure)
                    ...
                    logger.info(f"{log_color}{exit_reason} at ~{exit_price:.{self.price_decimals}f}{Style.RESET_ALL}")
                      ...
                      logger.debug(f"Order {pos_id} status 'closed' matches filled size. Position likely still active. No exit action taken based on this.")
                 ...
                 logger.error(f"{Fore.RED}State inconsistency! Active position {pos_id} ({symbol}) linked to order with status '{order_info.get('status')}'. Assuming closed. Removing position.{Style.RESET_ALL}")
                ...
                if self.time_based_exit_minutes is not None and self.time_based_exit_minutes > 0:
                    ...
                    if time_elapsed_minutes >= self.time_based_exit_minutes:
                        ...
                        if market_close_order:
                            ...
                            logger.info(f"Market close order for time exit placed successfully for {pos_id}. Exit Price ~{format_price(exit_price)}")
                        else:
                ...
                if not exit_reason and self.enable_trailing_stop_loss:
                    if not self.trailing_stop_loss_percentage or not (0 < self.trailing_stop_loss_percentage < 1):
                        ...
                        if new_tsl_price is not None:
                            logger.warning(f"{Fore.YELLOW}[EXPERIMENTAL] Attempting TSL update via edit_order for {pos_id} to SL={format_price(new_tsl_price)}. VERIFY EXCHANGE/CCXT SUPPORT!{Style.RESET_ALL}")
                            ...
                            if edit_success:
                                ...
                                needs_state_save = True
                                logger.info(f"{Fore.MAGENTA}Internal TSL state for {pos_id} updated to {format_price(new_tsl_price)} after edit attempt.{Style.RESET_ALL}")
                            else:
                                 logger.error(f"{Fore.RED}TSL update via edit_order for {pos_id} failed or was skipped. TSL state NOT updated.{Style.RESET_ALL}")
                # --- End TSL Logic ---
        ...
        if needs_state_save:
            ...
            if removed_count > 0 or updated_count > 0:
                 logger.debug(f"Active position management complete. Updated: {updated_count}, Removed/Closed: {removed_count}. Current total: {len(self.open_positions)}")
    ...
    def _update_trailing_stop_price(self, position: dict, current_price: float) -> float | None:
        ...
        try:
            ...
            if potential_tsl_price <= 0: potential_tsl_price = None  # Safety
        except Exception as format_err:
             logger.error(f"Error formatting potential TSL price {potential_tsl_price_raw}: {format_err}")
             potential_tsl_price = None

        if potential_tsl_price is None: return None  # Cannot proceed if formatting failed
        ...
        if not is_tsl_active:
            ...
            if activate_tsl:
                ...
                initial_trail_price = max(initial_trail_price, activation_price) if side == 'buy' else min(initial_trail_price, activation_price)
                ...
                if initial_trail_price != stored_sl_price:
                    ...
                    logger.info(f"{Fore.MAGENTA}Trailing Stop ACTIVATING for {side.upper()} {position['id']} at {format_price(new_tsl_price)} (Price: {format_price(current_price)}){Style.RESET_ALL}")
                else:
                     logger.debug(f"TSL activation condition met for {position['id']}, but potential TSL ({format_price(initial_trail_price)}) is same as current SL. No activation needed yet.")
        ...
        # --- TSL Update Check ---
            ...
            if update_needed:
                 ...
                 logger.info(f"{Fore.MAGENTA}Trailing Stop UPDATING for {side.upper()} {position['id']} from {format_price(trailing_sl_price)} to {format_price(new_tsl_price)} (Price: {format_price(current_price)}){Style.RESET_ALL}")
        ...
        **EXPERIMENTAL - Use with caution!**.
        ...
        try:
            ...
            if fresh_order_info.get('status') != 'open':
                 logger.warning(f"Order {pos_id} status is not 'open' ({fresh_order_info.get('status')}). Cannot edit for TSL.")
                 return False  # Cannot edit closed/canceled orders

            order_type = fresh_order_info.get('type')
            order_side = fresh_order_info.get('side')
            order_amount = fresh_order_info.get('amount')  # Use original amount? Or remaining? Check CCXT/exchange docs. Usually original amount.
            order_price = fresh_order_info.get('price')  # Required for limit orders

            if not all([order_type, order_side, order_amount]):
                 logger.error(f"Fetched fresh order info for {pos_id} is missing essential fields (type/side/amount). Cannot edit.")
                 return False
            ...
            if edited_order:
                ...
                logger.info(f"{Fore.MAGENTA}---> Attempted modify order {pos_id} via edit_order. Exchange Response SL: {confirmed_sl}, TP: {confirmed_tp}{Style.RESET_ALL}")
                ...
                if confirmed_sl and abs(float(confirmed_sl) - new_tsl_price) < 1e-9:
                     logger.info(f"{Fore.GREEN}---> TSL update via edit_order appears successful for {pos_id}.{Style.RESET_ALL}")
                     return True  # Signal success to update internal state
                else:
                     logger.warning(f"{Fore.YELLOW}---> TSL update via edit_order response SL ({confirmed_sl}) does not match target ({new_tsl_price}). Edit might have failed partially or confirmation is delayed.{Style.RESET_ALL}")
                     return False  # Signal potential failure
            else:
                # This case means edit_order call returned None after potential retries
                logger.error(f"{Fore.RED}TSL update via edit_order for {pos_id} failed (API returned None).{Style.RESET_ALL}")
                return False

        except ccxt.NotSupported as e:
            logger.error(f"{Fore.RED}TSL update FAILED: edit_order for SL/TP modification is NOT SUPPORTED by CCXT/{self.exchange_id} or for this order type/state. Error: {e}. Consider disabling TSL ('enable_trailing_stop_loss: false') or using a different TSL method.{Style.RESET_ALL}")
            ...
            return False
             ...
             logger.error(f"{Fore.RED}TSL update via edit_order failed for {pos_id} ({type(e).__name__}): {e}. Params: {edit_params}{Style.RESET_ALL}")
             return False
        except Exception as e:
            logger.error(f"{Fore.RED}Unexpected error attempting TSL order modification for {pos_id}: {e}{Style.RESET_ALL}", exc_info=True)
            return False
    ...
    def _log_position_pnl(self, position: dict, exit_price: float | None, reason: str) -> None:
        ...
        if not all([pos_id, side, entry_price, position_size, exit_price, symbol]):
            logger.warning(f"{Fore.YELLOW}---> Position {pos_id} ({symbol}) Closed. Reason: {reason}. "
    ...
    def _place_market_close_order(self, position: dict[str, Any], current_price: float) -> dict[str, Any] | None:
        ...
        logger.warning(f"{log_color}Initiating MARKET CLOSE for position {pos_id} ({symbol}). "
        ...
        if self.simulation_mode:
            ...
            simulated_close_order = {
                ...
                "price": None, "amount": size, "cost": sim_cost,
                "average": sim_avg_close_price, "filled": size, "remaining": 0.0,
                "status": 'closed', "fee": None, "fees": [],
        ...
        # --- Live Trading Mode ---
            ...
            try:
                ...
                if reduce_only_supported:
                    ...
                    logger.debug("Using 'reduceOnly=True' parameter for market close order.")
                     ...
                     if self.exchange_id.lower() == 'bybit':  # Example: Assume Bybit needs it in params
                          params['reduceOnly'] = True
                          logger.debug("Attempting 'reduceOnly=True' in params (exchange.has['reduceOnly'] is False/missing).")
                     else:
                          logger.warning(f"{Fore.YELLOW}Exchange {self.exchange_id} might not support 'reduceOnly'. Close order might open a new position if state is wrong.{Style.RESET_ALL}")
                ...
                logger.debug(f"Placing live market close order for {pos_id}. Side: {close_side}, Size: {size}, Params: {params}")
                ...
                if order:
                 ...
                 logger.error(f"{Fore.RED}LIVE Market Close Order Failed for {pos_id} ({type(e).__name__}): {e}. Position might already be closed or size/params incorrect.{Style.RESET_ALL}")
                 logger.error(f" -> Params Sent: {params}")
                 return None
            except ccxt.ExchangeError as e:
                 ...
                 if "order cost not meet" in err_str or "position size is zero" in err_str or "reduce-only" in err_str:
                      logger.warning(f"{Fore.YELLOW}Market close order for {pos_id} failed, likely because position is already closed or reduce-only conflict. Error: {e}{Style.RESET_ALL}")
    ...
    def _calculate_indicators(self, historical_data: pd.DataFrame) -> dict[str, Any]:
        ...
        if not all(col in historical_data.columns for col in required_cols):
             logger.error(f"Historical data DataFrame is missing required columns ({required_cols}). Cannot calculate indicators.")
             return {}
        ...
        macd_tuple = self.calculate_macd(
        ...
        def format_ind(key: str, precision: int | None = None) -> str:
            ...
            try:
                ...
                if precision is not None: return f"{val:.{precision}f}"
                # Default formatting: 1 decimal for RSI/Stoch, price decimals for EMA/MACD/ATR
                if key in ['rsi', 'stoch_k', 'stoch_d']: return f"{val:.1f}"
                if key in ['ema', 'atr', 'macd_line', 'macd_signal']: return f"{val:.{self.price_decimals}f}"
        ...
        if nones:
             logger.warning(f"{Fore.YELLOW}Indicator calculation issue: {', '.join(nones)} resulted in None. Check data sufficiency or period settings.{Style.RESET_ALL}")
    ...
    def _process_signals_and_entry(self, market_data: dict, indicators: dict) -> None:
        ...
        if active_or_pending_count >= self.max_open_positions:
             logger.info(f"{Fore.CYAN}Max open/pending positions ({self.max_open_positions}) reached. Skipping new entry evaluation.{Style.RESET_ALL}")
             return
        ...
        if order_size_base <= 0:
             ...
             logger.warning(f"{Fore.YELLOW}Cannot evaluate entry signal: Calculated order size is zero or invalid.{Style.RESET_ALL}")
             return
        ...
        if signal_score >= ENTRY_SIGNAL_THRESHOLD_ABS:
            entry_side = 'buy'
            logger.info(f"{Fore.GREEN}Buy signal score ({signal_score}) meets/exceeds threshold ({ENTRY_SIGNAL_THRESHOLD_ABS}). Preparing entry...{Style.RESET_ALL}")
        elif signal_score <= -ENTRY_SIGNAL_THRESHOLD_ABS:
            entry_side = 'sell'
            logger.info(f"{Fore.RED}Sell signal score ({signal_score}) meets/exceeds threshold ({-ENTRY_SIGNAL_THRESHOLD_ABS}). Preparing entry...{Style.RESET_ALL}")
        else:
            # Log only if score was calculated (avoids spamming neutral messages)
            if signal_score is not None:
                 logger.info(f"Neutral signal score ({signal_score}). Threshold ({ENTRY_SIGNAL_THRESHOLD_ABS}) not met. No entry action.")
        ...
        if entry_side:
            ...
            if sl_tp_required and sl_tp_failed:
                 logger.error(f"{Fore.RED}SL/TP calculation failed but is required by config. Cannot place entry order. Check ATR/percentage settings and logs.{Style.RESET_ALL}")
                 return  # Abort entry
            elif sl_tp_failed:  # SL/TP not strictly required, but calculation failed anyway
                 logger.warning(f"{Fore.YELLOW}SL/TP calculation failed or resulted in None (check logs). Proceeding without SL/TP parameters.{Style.RESET_ALL}")
            ...
            if entry_order:
                ...
                if not order_id:
                     logger.error(f"{Fore.RED}Entry order placed but received no order ID! Cannot track position. Manual check required! Order response: {str(entry_order)[:200]}...{Style.RESET_ALL}")
                ...
                if order_status == 'open':
                    ...
                    logger.info(f"Entry order {order_id} placed with status 'closed'. Assuming FILLED. Added as ACTIVE.")
                elif order_status in ['canceled', 'rejected', 'expired']:
                    logger.warning(f"{Fore.YELLOW}Entry order {order_id} failed immediately on placement (Status: {order_status}). Not adding position.{Style.RESET_ALL}")
                    return  # Don't add failed orders to state
                else:  # e.g., 'triggered', partial fills reported as 'open' or 'closed' depending on exchange
                    logger.warning(f"Entry order {order_id} has unusual initial status: '{order_status}'. Treating as PENDING for safety.")
                ...
                entry_time_for_state = order_timestamp_ms / 1000 if is_filled_immediately and order_timestamp_ms else None
                ...
                logger.info(f"{Fore.CYAN}---> Position {order_id} added to state. Status: {initial_pos_status}, Entry Price: {entry_price_f}{Style.RESET_ALL}")
                self._save_state()  # Save state after adding the new position
            else:
                 # Order placement failed (place_entry_order returned None)
                 logger.error(f"{Fore.RED}Entry order placement failed (API call error or validation). No position added.{Style.RESET_ALL}")
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"{loop_prefix} Current Price: {format_price(current_price)} "
    ...
    def shutdown(self) -> None:
        ...
        if not pending_orders_to_cancel:
            ...
            for pos in pending_orders_to_cancel:
                ...
                if self.cancel_order_by_id(pos_id, symbol=pos_symbol):
                    ...
                    logger.error(f"{Fore.RED}Failed to cancel pending order {pos_id} ({pos_symbol}) during shutdown. Manual check recommended.{Style.RESET_ALL}")
            logger.info(f"Attempted cancellation of {len(pending_orders_to_cancel)} pending orders. Success/Already Gone: {cancelled_pending_count}.")
        ...
        if self.close_positions_on_exit and active_positions_to_manage:
             logger.warning(f"{Fore.YELLOW}Configuration 'close_positions_on_exit' is TRUE. Attempting to close {len(active_positions_to_manage)} active position(s) via market order...{Style.RESET_ALL}")
             ...
             if current_price_for_close is None:
                 logger.critical(f"{Fore.RED}{Style.BRIGHT}CRITICAL: Cannot fetch current price for market close during shutdown. {len(active_positions_to_manage)} position(s) WILL REMAIN OPEN! Manual intervention required!{Style.RESET_ALL}")
             else:
                 closed_count = 0
                 failed_close_ids = []
                 needs_state_save = False
                 for position in list(active_positions_to_manage):  # Iterate copy
                    ...
                    if close_order:
                        closed_count += 1
                        logger.info(f"{Fore.YELLOW}---> Market close order successfully placed for position {pos_id}. Marking as '{STATUS_CLOSED_ON_EXIT}' in state.{Style.RESET_ALL}")
                        ...
                        self._log_position_pnl(position, close_fill_price, f"Closed on Exit (Market Order {close_order.get('id')})")
                    else:
                        failed_close_ids.append(pos_id)
                        logger.error(f"{Fore.RED}{Style.BRIGHT}CRITICAL: Failed to place market close order for {pos_id} ({pos_symbol}) during shutdown. POSITION REMAINS OPEN! Manual check required!{Style.RESET_ALL}")

                 logger.info(f"Attempted to close {len(active_positions_to_manage)} active positions. Market orders placed for: {closed_count}. Failed for: {len(failed_close_ids)}.")
                 if failed_close_ids:
                 ...
                 if needs_state_save: self._save_state()

        elif active_positions_to_manage:
            # Log remaining active positions if not closing them
            logger.warning(f"{Fore.YELLOW}{len(active_positions_to_manage)} position(s) remain active ('close_positions_on_exit' is false). Manual management may be required:{Style.RESET_ALL}")
            for pos in active_positions_to_manage:
                ...
                logger.warning(f" -> ID: {pos_id} ({symbol}), Side: {side}, Size: {format_amount(size)}, Entry: {format_price(entry)}")
        else:
