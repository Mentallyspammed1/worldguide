...
from utils import (validate_market, safe_decimal_conversion, format_price,
                   format_amount, format_order_id, send_sms_alert, retry_api_call) # Import placeholders/utils
...
try:
    ...
    main_logger.info(f"Config Loaded: Testnet={config.TESTNET_MODE}, Symbol={config.SYMBOL}, Leverage={config.DEFAULT_LEVERAGE}")

    # Validate essential config like API keys
    if not config.API_KEY or not config.API_SECRET:
        main_logger.error(f"{Back.RED}{Fore.WHITE}FATAL: Bybit API Key or Secret not found in environment variables or .env file.{Style.RESET_ALL}")
...
try:
    ...
    main_logger.info(f"{Fore.GREEN}Market Validation OK for {config.SYMBOL}. Type: {market_info.get('type')}, Logic: {market_info.get('linear') or market_info.get('inverse')}{Style.RESET_ALL}")
    ...
    if equity is None or available is None:
        ...
        main_logger.warning(f"{Fore.YELLOW}Fetch Balance failed after retries (returned None). Proceeding cautiously.{Style.RESET_ALL}")
        # Set defaults to avoid crashing later tests, but log the issue
        equity, available = Decimal("0.0"), Decimal("0.0")
    else:
        main_logger.info(f"{Fore.GREEN}Fetch Balance OK: Equity={equity:.4f} USDT, Available={available:.4f} USDT{Style.RESET_ALL}")
    ...
    if not lev_ok:
        ...
        main_logger.warning(f"{Fore.YELLOW}Set Leverage to {config.DEFAULT_LEVERAGE}x potentially failed or was unnecessary. Check logs for details.{Style.RESET_ALL}")
    else:
        main_logger.info(f"{Fore.GREEN}Set Leverage OK (or already set) to {config.DEFAULT_LEVERAGE}x for {config.SYMBOL}.{Style.RESET_ALL}")
    ...
    if not ticker:
        ...
        main_logger.info(f"{Fore.GREEN}Fetch Ticker OK: Last={ticker.get('last')}, Bid={ticker.get('bid')}, Ask={ticker.get('ask')}, Spread={ticker.get('spread_pct'):.4f}%{Style.RESET_ALL}")
    ...
    if position['side'] == config.POS_NONE:
        ...
        main_logger.info(f"{Fore.GREEN}Fetch Position OK: Side={position['side']}, Qty={position['qty']}, Entry={position['entry_price']}{Style.RESET_ALL}")
    ...
    if position['side'] != config.POS_NONE:
        ...
        if not risk:
            ...
            main_logger.info(f"{Fore.GREEN}Fetch Position Risk OK: Liq={risk['liq_price']}, IMR={risk['imr']:.4%}, MMR={risk['mmr']:.4%}, Value={risk['position_value']}{Style.RESET_ALL}")
    else:
    ...
    if run_advanced_tests:
        ...
        if acc_info:
            main_logger.info(f"{Fore.GREEN}Fetch Account Info OK: UTA Status={acc_info.get('unifiedMarginStatus')}, MarginMode={acc_info.get('marginMode')}{Style.RESET_ALL}")

        # Fetch OHLCV
        main_logger.info("\n--- Adv 2. Fetching OHLCV Data ---")
        ohlcv_df = fetch_ohlcv_paginated(exchange_instance, config.SYMBOL, '1h', config, max_total_candles=100) # Fetch last 100 hourly candles
        if ohlcv_df is not None and not ohlcv_df.empty:
            main_logger.info(f"{Fore.GREEN}Fetch OHLCV OK: Fetched {len(ohlcv_df)} candles. Last Close: {ohlcv_df['close'].iloc[-1]}{Style.RESET_ALL}")
        ...
        if run_order_tests and config.TESTNET_MODE:
            ...
            if initial_pos['side'] != config.POS_NONE:
                 main_logger.warning(f"Existing {initial_pos['side']} position found ({initial_pos['qty']}). Attempting to close...")
                 close_result = close_position_reduce_only(exchange_instance, config.SYMBOL, config, position_to_close=initial_pos, reason="Test Prep Close")
                 ...
                 if current_pos_check['side'] != config.POS_NONE:
                      main_logger.error(f"{Back.RED}Failed to close pre-existing position. Aborting order tests.{Style.RESET_ALL}")
            ...
            if current_ticker and current_ticker.get('bid'):
                ...
                limit_order = place_limit_order_tif(exchange_instance, config.SYMBOL, 'buy', limit_qty, limit_buy_price, config, time_in_force='GTC', is_post_only=True)
                if limit_order:
                    main_logger.info(f"{Fore.GREEN}Limit Order (PostOnly) placed OK: ID ...{format_order_id(limit_order.get('id'))}, Status: {limit_order.get('status')}{Style.RESET_ALL}")
                    limit_order_id = limit_order.get('id')
                    time.sleep(1)

                    # Test Fetch Open Orders (Filtered)
                    main_logger.info("\n--- Test 8b. Fetch Open Limit Order ---")
                    open_limit_orders = fetch_open_orders_filtered(exchange_instance, config.SYMBOL, config, side='buy', order_type='limit', order_filter='Order')
                    found_order = False
                    if open_limit_orders:
                        for o in open_limit_orders:
                            if o.get('id') == limit_order_id:
                                main_logger.info(f"{Fore.GREEN}Successfully fetched the open limit order: ID ...{format_order_id(o.get('id'))}{Style.RESET_ALL}")
                                found_order = True
                                break
                    if not found_order:
                         main_logger.warning(f"{Fore.YELLOW}Could not fetch the specific open limit order (might have filled or filter failed).{Style.RESET_ALL}")
                    ...
                    if cancel_ok:
                         ...
                         main_logger.warning(f"{Fore.YELLOW}Cancel All Orders reported potential issues. Check logs.{Style.RESET_ALL}")
                    time.sleep(1)
                    # Verify cancellation
                    open_orders_after_cancel = fetch_open_orders_filtered(exchange_instance, config.SYMBOL, config, order_filter='Order')
                    if not open_orders_after_cancel:
                         main_logger.info(f"{Fore.GREEN}Verified no open regular orders after cancellation.{Style.RESET_ALL}")
                    else:
                         main_logger.warning(f"{Fore.YELLOW}Found {len(open_orders_after_cancel)} open orders after cancel call!{Style.RESET_ALL}")

                else:
                     main_logger.warning(f"{Fore.YELLOW}Limit Order placement failed or was rejected (e.g., PostOnly failed).{Style.RESET_ALL}")
            ...
            market_entry_order = place_market_order_slippage_check(exchange_instance, config.SYMBOL, 'buy', entry_qty, config)
            if not market_entry_order:
                ...
                main_logger.info(f"{Fore.GREEN}Market Entry Order OK: ID ...{format_order_id(market_entry_order.get('id'))}, Status: {market_entry_order.get('status')}{Style.RESET_ALL}")
                ...
                if pos_after_entry['side'] != config.POS_LONG or pos_after_entry['qty'] < entry_qty * Decimal("0.9"):
                    main_logger.error(f"{Back.RED}Position check after market buy failed! Side: {pos_after_entry['side']}, Qty: {pos_after_entry['qty']}. Aborting.{Style.RESET_ALL}")
                else:
                    ...
                    sl_order = place_native_stop_loss(exchange_instance, config.SYMBOL, 'sell', pos_after_entry['qty'], sl_price, config, trigger_by='MarkPrice')
                    if sl_order:
                        main_logger.info(f"{Fore.GREEN}Native SL Order OK: ID ...{format_order_id(sl_order.get('id'))}, Status: {sl_order.get('status')}, Trigger: {sl_price}{Style.RESET_ALL}")
                        sl_order_id = sl_order.get('id')

                        # Fetch Stop Orders to verify
                        open_stops = fetch_open_orders_filtered(exchange_instance, config.SYMBOL, config, order_filter='StopOrder')
                        found_sl = any(o.get('id') == sl_order_id for o in open_stops) if open_stops else False
                        if found_sl: main_logger.info(f"{Fore.GREEN}Verified SL order exists via fetch (StopOrder filter).{Style.RESET_ALL}")
                        else: main_logger.warning(f"{Fore.YELLOW}Could not verify SL order via fetch (StopOrder filter).{Style.RESET_ALL}")
                        ...
                        try:
                             ...
                             main_logger.info(f"{Fore.GREEN}Specific SL order cancellation request sent.{Style.RESET_ALL}")
                    ...
                    if pos_before_close['side'] == config.POS_NONE:
                         main_logger.info(f"{Fore.CYAN}Position already closed before final close attempt. Skipping.{Style.RESET_ALL}")
                    else:
                        close_order = close_position_reduce_only(exchange_instance, config.SYMBOL, config, position_to_close=pos_before_close, reason="Test Close")
                        if close_order:
                            main_logger.info(f"{Fore.GREEN}Close Position Order OK: ID ...{format_order_id(close_order.get('id'))}{Style.RESET_ALL}")
                        else:
                            main_logger.warning(f"{Fore.YELLOW}Close Position attempt failed or position was already closed.{Style.RESET_ALL}")
                    ...
                    if final_pos_check['side'] == config.POS_NONE:
                        main_logger.info(f"{Fore.GREEN}{Style.BRIGHT}Position Confirmed Flat After All Tests.{Style.RESET_ALL}")
                         ...
                         main_logger.error(f"{Back.RED}Position is NOT flat after tests! Side: {final_pos_check['side']}, Qty: {final_pos_check['qty']}{Style.RESET_ALL}")
