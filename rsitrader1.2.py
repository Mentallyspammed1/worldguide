...
def log_debug(msg: str) -> None: logger.debug(f"{Fore.WHITE}{msg}{RESET}") # Use standard white for debug
...
def display_position_status(position: Dict[str, Any], price_precision: int, amount_precision: int) -> None:
    ...
    tsl_str = f" | Active TSL: {tsl_active_price:.{price_precision}f}" if isinstance(tsl_active_price, (float, int)) else ""
    ...
    protection_info = []
    if oco_id: protection_info.append(f"OCO ID: {oco_id}")
    else:
        if sl_id: protection_info.append(f"SL ID: {sl_id}")
        if tp_id: protection_info.append(f"TP ID: {tp_id}")

    if protection_info:
        print(f"    Protection Orders: {', '.join(protection_info)}")
    elif status is not None: # Position is active but no protection tracked
         print(f"    {NEON_YELLOW}Warning: Position active but no protection order IDs are currently tracked in state.{RESET}")
...
def neon_sleep_timer(seconds: int) -> None:
    ...
    for i in range(steps, -1, -1):
        ...
        if remaining_seconds <= 5 and i % 2 == 0: color = NEON_RED # Flashing red
...
def retry_api_call(max_retries: int = 3, initial_delay: float = 5.0, backoff_factor: float = 2.0) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            raise RuntimeError(f"API call '{func.__name__}' failed unexpectedly after exhausting retries without raising.") # Should not happen
        return wrapper
    return decorator
...
def load_config(filename: str = CONFIG_FILE) -> Dict[str, Any]:
    ...
    try:
        ...
        if "rsi_oversold" in cfg and "rsi_overbought" in cfg and cfg.get("rsi_oversold", 0) >= cfg.get("rsi_overbought", 100):
        ...
        if "stoch_oversold" in cfg and "stoch_overbought" in cfg and cfg.get("stoch_oversold", 0) >= cfg.get("stoch_overbought", 100):
        ...
        if needs_atr: validate("atr_length", int, lambda x: x > 0, required=True) # Required if ATR features used
        ...
        if cfg.get("enable_trailing_stop"):
             ...
             validate("trailing_stop_activation_atr_multiplier", (float, int), lambda x: x >= 0, required=True) # Can be 0 to activate immediately
        ...
        log_error(f"CRITICAL: Error decoding JSON from '{filename}'. Check for syntax errors (e.g., trailing commas): {e}")
        sys.exit(1)
    except Exception as e:
...
if not api_key or not secret:
    log_error(f"CRITICAL: API Key (expected env var: '{api_key_env}') or Secret Key (expected env var: '{secret_key_env}') not found in environment variables or .env file.")
...
try:
    ...
    log_error(f"CRITICAL: Authentication failed for {exchange_id}. Check API Key, Secret, and Passphrase (if required). Error: {e}")
    sys.exit(1)
...
try:
    ...
    if price_prec is not None:
        ...
        price_precision_digits = int(exchange.decimal_to_precision(price_prec, ccxt.ROUND, counting_mode=exchange.precisionMode))
    if amount_prec is not None:
        amount_precision_digits = int(exchange.decimal_to_precision(amount_prec, ccxt.ROUND, counting_mode=exchange.precisionMode))
    ...
    is_contract_market = market_info.get('swap', False) or \
                         market_info.get('future', False) or \
                         market_info.get('contract', False) or \
                         market_info.get('type') in ['swap', 'future']

    if not base_currency or not quote_currency:
        log_warning(f"Could not reliably determine base/quote currency for {symbol}. Base: {base_currency}, Quote: {quote_currency}")

    # Log extracted parameters
    log_info(f"Market Info | Base: {base_currency}, Quote: {quote_currency}, Contract Market: {is_contract_market}")
    log_info(f"Precision   | Price: {price_precision_digits} decimals (Tick Size: {min_tick:.{price_precision_digits+2}f}), Amount: {amount_precision_digits} decimals") # Show tick size better
    if min_amount is not None: log_info(f"Limits      | Min Amount: {min_amount} {base_currency or ''}")
    if min_cost is not None: log_info(f"Limits      | Min Cost: {min_cost} {quote_currency or ''}")

except KeyError as e:
    log_error(f"CRITICAL: Error accessing market info for {symbol}. Market data might be incomplete. Key: {e}", exc_info=True)
    sys.exit(1)
except Exception as e:
...
if SIMULATION_MODE:
    ...
    try:
        ...
        if sys.stdin.isatty():
            confirm = input(f"{NEON_RED}>>> Type 'LIVE' to confirm live trading, or press Enter/Ctrl+C to exit: {RESET}")
...
def load_position_state(filename: str = POSITION_STATE_FILE) -> None:
    ...
    try:
            ...
            if key in validated_state:
                ...
                if key == 'entry_time' and isinstance(loaded_value, str):
                    try:
                        ...
                        issues_found.append(f"Key '{key}': Invalid timestamp format ('{loaded_value}'). Resetting to None.")
                ...
                if loaded_value is not None and expected_type is not None:
                    ...
                    if not isinstance(loaded_value, expected_type) and not allow_int_for_float:
                        issues_found.append(f"Key '{key}': Type mismatch. Expected {expected_type.__name__}, got {type(loaded_value).__name__}. Resetting to default.")
        ...
        if validated_state.get('status') is not None:
            if not isinstance(validated_state.get('entry_price'), (float, int)) or \
               not isinstance(validated_state.get('quantity'), (float, int)) or \
               not isinstance(validated_state.get('stop_loss'), (float, int)) or \
               not isinstance(validated_state.get('take_profit'), (float, int)):
                issues_found.append(f"Active position ('{validated_state.get('status')}') found, but essential data (entry_price, quantity, stop_loss, take_profit) is missing or invalid. Resetting status to None.")
                validated_state['status'] = None # Invalidate the position if core data is bad

        # Report issues and update global state
        if issues_found:
            log_warning(f"Issues found during state file validation:\n - " + "\n - ".join(issues_found))
        ...
        log_error(f"Error decoding JSON from state file '{filename}': {e}. Starting with a fresh state.", exc_info=False)
        position = position_default_structure.copy()
    except Exception as e:
        log_error(f"Unexpected error loading or validating state file '{filename}': {e}. Starting with a fresh state.", exc_info=True)
        position = position_default_structure.copy()
...
def calculate_technical_indicators(
    ...
    if calc_vol_ma: required_cols.append('volume')
    if not all(col in df.columns for col in required_cols):
        missing = [col for col in required_cols if col not in df.columns]
        log_error(f"Cannot calculate indicators: Missing required columns: {missing}")
        return None

    log_debug(f"Calculating indicators (RSI, Stoch{' + ATR' if calc_atr else ''}{' + VolMA' if calc_vol_ma else ''})...")
    try:
        ...
        df_ind.ta.stoch(k=stoch_p['k'], d=stoch_p['d'], smooth_k=stoch_p['smooth_k'], append=True, col_names=(stoch_k_col, stoch_d_col))
...
def identify_potential_order_block(
    ...
    try:
        ...
        if avg_volume <= 0 and len(df_completed) > 0:
            ...
            if pd.notna(mean_vol): avg_volume = mean_vol
        ...
        log_debug(f"OB Analysis | Lookback: {lookback}, Avg Volume (completed candles): {avg_volume:.2f}, Volume Threshold: {volume_threshold:.2f}")
        ...
        for i in range(search_start_index, search_end_index -1, -1): # Iterate backwards
            if i < 1: break # Need index i and i-1
            ...
            is_bullish_imbalance = imbalance_candle['close'] > imbalance_candle['open'] and \
                                   imbalance_candle['close'] > ob_candidate_candle['high']
            # Bearish Imbalance: Imbalance candle is red, closes below OB low
            is_bearish_imbalance = imbalance_candle['close'] < imbalance_candle['open'] and \
                                   imbalance_candle['close'] < ob_candidate_candle['low']
            ...
            if not bullish_ob and ob_is_bearish and is_bullish_imbalance and is_high_volume_imbalance and imbalance_sweeps_ob_low:
                bullish_ob = {
                ...
                log_debug(f"Potential Bullish OB found at {ob_candidate_candle.name.strftime('%Y-%m-%d %H:%M')} (Low: {bullish_ob['low']:.{price_precision_digits}f}, High: {bullish_ob['high']:.{price_precision_digits}f})")
            ...
            # Needs: Bullish OB candle, followed by strong Bearish Imbalance candle (high vol, sweeps high)
            elif not bearish_ob and ob_is_bullish and is_bearish_imbalance and is_high_volume_imbalance and imbalance_sweeps_ob_high:
                bearish_ob = {
                    'high': ob_candidate_candle['high'],
                    'low': ob_candidate_candle['low'],
                    'time': ob_candidate_candle.name, # Timestamp of the OB candle
                ...
                log_debug(f"Potential Bearish OB found at {ob_candidate_candle.name.strftime('%Y-%m-%d %H:%M')} (Low: {bearish_ob['low']:.{price_precision_digits}f}, High: {bearish_ob['high']:.{price_precision_digits}f})")
...
def calculate_position_size(
    ...
    global base_currency, quote_currency, is_contract_market, min_amount, min_cost, min_tick, price_precision_digits, amount_precision_digits
    ...
    if price_difference < min_tick:
        log_error(f"Stop loss ({sl:.{price_precision_digits}f}) is too close to entry price ({entry:.{price_precision_digits}f}). Minimum distance required: {min_tick:.{price_precision_digits}f}.")
        return None

    log_debug(f"Calculating position size for {sym}: Entry={entry:.{price_precision_digits}f}, SL={sl:.{price_precision_digits}f}, Risk={risk_pct*100:.2f}%")
    ...
    try:
        ...
        if quote_currency:
            if quote_currency in balance_info and isinstance(balance_info[quote_currency], dict):
                ...
                if available_balance_quote == 0.0 and balance_info[quote_currency].get('total', 0.0) > 0:
                    ...
                    log_warning(f"Using 'total' {quote_currency} balance ({available_balance_quote}), as 'free' was zero.")
            elif 'free' in balance_info and isinstance(balance_info['free'], dict) and quote_currency in balance_info['free']:
                ...
                balance_source_info = f"['free']['{quote_currency}']"
                 ...
                 if 'result' in balance_info['info'] and 'list' in balance_info['info']['result'] and balance_info['info']['result']['list']:
                     for asset_info in balance_info['info']['result']['list']:
                         if asset_info.get('coin') == quote_currency:
                              available_balance_quote = float(asset_info.get('availableToWithdraw', 0.0)) # Or 'walletBalance'? Check Bybit docs
        ...
        if available_balance_quote <= 0:
            log_error(f"Insufficient available balance ({available_balance_quote:.{price_precision_digits}f}) in quote currency ({quote_currency}). Source checked: {balance_source_info}.")
            log_debug(f"Full balance details: {balance_info.get(quote_currency, balance_info.get('info', 'Balance info structure not recognized'))}")
            return None
        log_info(f"Available balance ({quote_currency}): {available_balance_quote:.{price_precision_digits}f} (Source: {balance_source_info})")
        ...
        try:
            ...
            log_warning(f"ccxt amount_to_precision failed ('{fmt_e}'). Using manual rounding to {amount_precision_digits} decimals.")
            quantity_adjusted = round(quantity_base, amount_precision_digits)

        if quantity_adjusted <= 0:
            log_error(f"Calculated quantity ({quantity_base:.{amount_precision_digits+4}f}) resulted in zero or negative ({quantity_adjusted}) after adjusting for precision.")
            return None

        # --- Check Exchange Limits ---
        # 1. Minimum Amount (Base Currency)
        if min_amount is not None and quantity_adjusted < min_amount:
            log_error(f"Calculated quantity {quantity_adjusted:.{amount_precision_digits}f} {base_currency} is below the minimum required amount of {min_amount} {base_currency}.")
            return None
        ...
        if min_cost is not None and estimated_cost_quote < min_cost:
             log_error(f"Estimated order cost {estimated_cost_quote:.{price_precision_digits}f} {quote_currency} is below the minimum required cost of {min_cost} {quote_currency}.")
             # Optionally suggest the required quantity:
             required_qty_for_min_cost = min_cost / entry
             log_info(f"  (Minimum quantity required to meet min cost at current price: ~{required_qty_for_min_cost:.{amount_precision_digits}f} {base_currency})")
             return None
        ...
        if estimated_cost_quote > available_balance_quote * cost_buffer_factor:
             log_error(f"Estimated order cost ({estimated_cost_quote:.{price_precision_digits}f} {quote_currency}) exceeds {cost_buffer_factor*100:.1f}% of available balance ({available_balance_quote:.{price_precision_digits}f}).")
             return None

        # --- Success ---
        log_info(f"{NEON_GREEN}Calculated position size: {quantity_adjusted:.{amount_precision_digits}f} {base_currency}{RESET} (Risking approx. {risk_amount_quote:.{price_precision_digits}f} {quote_currency})")
...
def place_market_order(
    ...
    if reduce_only:
        if is_contract_market:
            ...
            if exch.id in ['bybit', 'binance', 'kucoinfutures', 'okx', 'gateio']: # Add others known to use params
                 ...
                 log_warning(f"Uncertain how {exch.id} handles 'reduceOnly'. Assuming it's a top-level argument or handled implicitly. Check ccxt/exchange docs if closure fails.")
    ...
    if reduce_only_applied: log_info("  Type: Reduce-Only")
    ...
    if SIMULATION_MODE:
        ...
        try: # Attempt to fetch current price for simulation realism
            ...
            def fetch_sim_ticker(e: ccxt.Exchange, s: str) -> Dict: return e.fetch_ticker(s)
            ticker = fetch_sim_ticker(exch, sym)
            sim_price = ticker.get('last', ticker.get('close', 0.0)) # Use last or close price
            if sim_price <= 0: sim_price = ticker.get('bid', 0.0) if side == 'sell' else ticker.get('ask', 0.0) # Fallback to bid/ask
        except Exception as e:
    ...
    if order_result:
        ...
        log_info(f"Order Result | ID: {o_id}, Status: {o_status}, Avg Fill Px: {p_str}, Filled Qty: {f_str} {base_currency or ''}, Cost: {c_str} {quote_currency or ''}, ReduceOnly: {o_reduce}")
        ...
        if not SIMULATION_MODE and o_status == 'open':
            log_warning(f"Market order {o_id} status is 'open'. It might not have filled immediately. Monitoring required.")
...
def place_sl_tp_orders(
    ...
    if pos_side not in ['long', 'short']:
        log_error(f"Invalid position side '{pos_side}' for SL/TP placement."); return None, None, None
    if not (isinstance(qty, (float, int)) and qty > 0):
        log_error(f"Invalid quantity '{qty}' for SL/TP placement."); return None, None, None
    if not (isinstance(sl_pr, (float, int)) and sl_pr > 0 and isinstance(tp_pr, (float, int)) and tp_pr > 0):
        log_error(f"Invalid SL ({sl_pr}) or TP ({tp_pr}) price for placement."); return None, None, None
    ...
    if (pos_side == 'long' and sl_pr >= tp_pr):
        log_error(f"Invalid SL/TP logic for LONG position: SL ({sl_pr}) must be < TP ({tp_pr})."); return None, None, None
    if (pos_side == 'short' and sl_pr <= tp_pr):
        log_error(f"Invalid SL/TP logic for SHORT position: SL ({sl_pr}) must be > TP ({tp_pr})."); return None, None, None
    ...
    try:
        ...
        if qty_formatted <= 0: raise ValueError("Quantity formatted to zero or less.")
        ...
        if (pos_side == 'long' and sl_price_formatted >= tp_price_formatted) or \
         ...
         log_error(f"SL/TP input validation failed after formatting: {ve}"); return None, None, None
    except Exception as fmt_e:
        log_error(f"Failed to format SL/TP quantity or prices using exchange rules: {fmt_e}"); return None, None, None
    ...
    else: log_debug("Not applying reduceOnly (either spot market or assuming implicit).")
    ...
    if has_stop_market or has_generic_stop:
        ...
        limit_price_raw = sl_price_formatted - limit_offset if close_side == 'sell' else sl_price_formatted + limit_offset
        ...
        log_warning(f"Using fallback SL order type: {sl_order_type}. Trigger: {sl_price_formatted}, Limit: {sl_limit_price}. Fill depends on market liquidity.")
    else:
        # No supported stop order type found
        log_error(f"Exchange {exch.id} does not appear to support Stop Market or Stop Limit orders via ccxt. Cannot place Stop Loss.")
        return None, None, None
    ...
    if oco_supported:
        ...
        log_info(f"  Qty: {qty_formatted}, TP Limit: {tp_price_formatted}, SL Trigger: {sl_price_formatted} ({sl_order_type}{f', SL Limit: {sl_limit_price}' if sl_limit_price else ''})")
        ...
        try:
            ...
            if SIMULATION_MODE:
                ...
                log_warning(f"!!! VERIFY this structure works for {exch.id} before relying on it! Type='{oco_order_type_guess}', Params={oco_params}")
                ...
                raise ccxt.NotSupported(f"Generic OCO structure used for {exch.id}. Implement exchange-specific OCO logic in place_sl_tp_orders() or rely on separate orders.")
            ...
            log_warning(f"OCO not supported by {exch.id} or structure used is incorrect: {e}. Falling back to separate SL/TP orders.")
        except (ccxt.InvalidOrder, ccxt.ExchangeError) as e:
    ...
    try:
        ...
        if sl_limit_price: log_info(f"  Limit Price: {sl_limit_price}")
        if params: log_info(f"  Params: {params}")
        ...
        if SIMULATION_MODE:
            ...
            if sl_order and sl_order.get('id'):
                ...
                log_error(f"Separate SL ({sl_order_type}) placement failed or did not return an ID. Response: {sl_order}")
    ...
    try:
        log_info(f"Attempting separate TP (Limit):")
        log_info(f"  Side: {close_side.upper()}, Qty: {qty_formatted}")
...
def check_position_and_orders(exch: ccxt.Exchange, sym: str) -> bool:
    ...
    try:
        ...
        if has_tracked_orders:
            ...
            log_debug(f"Found {len(open_order_ids)} open order IDs for {sym}: {open_order_ids if open_order_ids else 'None'}")
            ...
            if oco_id:
                ...
                if sl_id and not sl_is_open and tp_id and not tp_is_open:
                    ...
                    log_info(f"{NEON_GREEN}Closure detected: {closure_reason}. Assuming TP was hit.{RESET}") # Usually green event
        ...
        should_check_position = is_contract_market and exch.has.get('fetchPositions') and \
                                (not has_tracked_orders or not protection_order_still_open)

        if should_check_position:
            log_info("Order check suggests closure or orders untracked. Verifying position status via fetchPositions...")
            try:
                ...
                if positions_data and isinstance(positions_data, list):
                    for pos_info in positions_data:
                        ...
                        if isinstance(pos_info, dict) and pos_info.get('symbol') == sym:
                            ...
                            size_val = pos_info.get('contracts', # Common field
                                                pos_info.get('info', {}).get('size', # Binance often uses info.size
                                                pos_info.get('info', {}).get('positionAmt', # Binance sometimes info.positionAmt
                                                pos_info.get('info', {}).get('qty', # Bybit often uses info.qty
                                                0)))) # Default to 0 if not found
                            try:
                                current_pos_size = float(size_val) if size_val is not None else 0.0
                            except (ValueError, TypeError):
                                log_warning(f"Could not parse position size ('{size_val}') from fetchPositions data for {sym}. Assuming 0.")
                                current_pos_size = 0.0

                            # Determine position side from data (handle variations)
                            pos_side_on_exchange = pos_info.get('side', # Standard field
                                                              pos_info.get('info', {}).get('side', # Bybit uses info.side
                                                              'unknown')).lower()
                            # Binance sometimes uses positionAmt sign instead of 'side' field
                            if pos_side_on_exchange == 'unknown' and 'info' in pos_info and 'positionAmt' in pos_info['info']:
                                pos_amt_sign = float(pos_info['info'].get('positionAmt', 0.0))
                                if pos_amt_sign > 0: pos_side_on_exchange = 'long'
                                elif pos_amt_sign < 0: pos_side_on_exchange = 'short'

                            log_debug(f"fetchPositions found entry for {sym}: Size={current_pos_size}, Side='{pos_side_on_exchange}'")

                            # Check if size is effectively non-zero (use min_amount or small epsilon)
                            size_threshold = min_amount if min_amount is not None else 1e-9 # Use min_amount if known, else small value
                            if abs(current_pos_size) > size_threshold:
                                position_found_on_exchange = True
                                log_info(f"fetchPositions confirms an active {pos_side_on_exchange} position exists for {sym} (Size: {current_pos_size}).")
                                ...
                                if position['status'] == pos_side_on_exchange:
                                    ...
                                    if reset_required:
                                        log_warning(f"Position check confirms active {position['status']} position. Overriding closure detected by order check. State remains active.")
                                    ...
                                    log_error(f"!!! STATE MISMATCH DETECTED !!!")
                ...
                log_warning("fetchPositions is not supported by this exchange. Cannot use it for position confirmation.")
            except Exception as e:
                log_error(f"Error during fetchPositions check: {e}. Relying solely on order status check.", exc_info=False) # Decorator logs full trace if needed
        ...
        if reset_required:
            ...
            if order_to_cancel and cancel_label:
                 ...
                 try:
                     ...
                     if cancelled:
                         log_info(f"Cancellation request for lingering order {order_to_cancel} sent or order already closed.")
                     else:
                         log_warning(f"Failed to cancel lingering {cancel_label} order {order_to_cancel}. Manual check might be needed.")
                 except Exception as e:
                     # Catch errors during the cancellation attempt itself
                     log_error(f"Error occurred while trying to cancel lingering order {order_to_cancel}: {e}", exc_info=True)
            ...
            display_position_status(position, price_precision_digits, amount_precision_digits) # Display the cleared state
        ...
        log_error(f"Error checking position/orders due to exchange communication issue: {e}. State remains unchanged.", exc_info=False)
        return False # Cannot determine state, do not reset prematurely
    except Exception as e:
        # Catch any other unexpected errors during the check
        if not isinstance(e, RETRYABLE_EXCEPTIONS): # Avoid double logging
            log_error(f"Unexpected error during position/order consistency check: {e}. State remains unchanged.", exc_info=True)
...
def update_trailing_stop(exch: ccxt.Exchange, sym: str, current_price: float, current_atr: Optional[float]) -> None:
    ...
    # --- Pre-checks ---
    if not enable_trailing_stop: return # TSL feature disabled
    if position.get('status') is None: return # No active position
    ...
    log_debug(f"TSL Check ({pos_side}): Current Price={current_price:.{price_precision_digits}f}, Eff SL={effective_sl_price:.{price_precision_digits}f}, ATR={current_atr:.{price_precision_digits}f}")

    # --- Update Peak Prices Reached Since Entry ---
    new_peak_reached = False
    ...
    if pos_side == 'long':
        ...
        if current_high > activation_threshold:
            ...
            log_debug(f"Long TSL potentially active. High ({current_high:.{price_precision_digits}f}) > Threshold ({activation_threshold:.{price_precision_digits}f}).")
        ...
        if current_low < activation_threshold:
            ...
            log_debug(f"Short TSL potentially active. Low ({current_low:.{price_precision_digits}f}) < Threshold ({activation_threshold:.{price_precision_digits}f}).")
            log_debug(f"  Potential New TSL Price: {potential_new_tsl_price:.{price_precision_digits}f}")
    ...
    if tsl_activated and potential_new_tsl_price is not None:
        ...
        if should_update_tsl:
            ...
            log_debug(f"TSL Update Condition Check: New TSL ({new_tsl_price_formatted:.{price_precision_digits}f if new_tsl_price_formatted else 'N/A'}) vs Effective SL ({effective_sl_price:.{price_precision_digits}f}) vs Price ({current_price:.{price_precision_digits}f}). No update needed.")
    ...
    if should_update_tsl and new_tsl_price_formatted is not None:
        ...
        if position.get('oco_order_id'): ids_to_cancel.append((position['oco_order_id'], "OCO"))
        else:
            if position.get('sl_order_id'): ids_to_cancel.append((position['sl_order_id'], "SL"))
            if position.get('tp_order_id'): ids_to_cancel.append((position['tp_order_id'], "TP"))

        if not ids_to_cancel:
            log_error("TSL CRITICAL: Update triggered, but no existing protection order IDs found in state to cancel. Aborting update.")
        ...
        for order_id, label in ids_to_cancel:
            ...
            if not cancel_order_with_retry(exch, order_id, sym):
                 log_error(f"TSL CRITICAL: Failed to cancel existing {label} order {order_id}. Aborting TSL update to avoid inconsistent state.")
        ...
        log_info(f"TSL: Placing new protection orders: SL @ {new_tsl_price_formatted:.{price_precision_digits}f}, Original TP @ {original_tp_price:.{price_precision_digits}f}")
        ...
        if protection_placed:
            ...
            log_error(f"  BUT FAILED to place new protection orders (SL={new_tsl_price_formatted}, TP={original_tp_price}).")
...
def run_bot():
    ...
    if enable_atr_sl_tp: log_info(f"    ATR SL: {atr_sl_multiplier}x, ATR TP: {atr_tp_multiplier}x (ATR Length: {atr_length})")
    else: log_info(f"    Fixed SL: {stop_loss_percentage*100:.2f}%, Fixed TP: {take_profit_percentage*100:.2f}%")
    log_info(f"  Trailing Stop Loss: {'Enabled' if enable_trailing_stop else 'Disabled'}")
    if enable_trailing_stop: log_info(f"    TSL Activation: {trailing_stop_activation_atr_multiplier}x ATR, TSL Distance: {trailing_stop_atr_multiplier}x ATR")
    ...
    if entry_volume_confirmation_enabled: log_info(f"    Vol MA Length: {entry_volume_ma_length}, Vol Multiplier: {entry_volume_multiplier}x")
    log_info("-" * 70)
    ...
    while True:
        try:
            ...
            try:
                ...
                current_high = float(latest_candle['high']) # Needed for OB context
                current_low = float(latest_candle['low'])   # Needed for OB context
                ...
                if needs_atr and atr_col_name and atr_col_name in latest_candle and pd.notna(latest_candle[atr_col_name]):
                    latest_atr = float(latest_candle[atr_col_name])
                    if latest_atr <= 0: latest_atr = None # Treat non-positive ATR as invalid

                latest_vol_ma: Optional[float] = None
                if entry_volume_confirmation_enabled and vol_ma_col_name and vol_ma_col_name in latest_candle and pd.notna(latest_candle[vol_ma_col_name]):
                    latest_vol_ma = float(latest_candle[vol_ma_col_name])
                    if latest_vol_ma <= 0: latest_vol_ma = None # Treat non-positive MA as invalid
                ...
                if any(math.isnan(v) for v in [current_price, latest_rsi, latest_stoch_k, latest_stoch_d]):
                 ...
                 log_error(f"Error extracting latest indicator data at {latest_timestamp}: {e}. Check indicator calculation and column names.", exc_info=False)
                 neon_sleep_timer(sleep_interval_seconds)
            ...
            if needs_atr and latest_atr is None:
                log_warning(f"ATR calculation needed (ATR SL/TP or TSL enabled) but result is invalid or missing for timestamp {latest_timestamp}.")
            if entry_volume_confirmation_enabled and latest_vol_ma is None:
                log_debug(f"Volume MA needed for entry confirmation but result is invalid or missing for timestamp {latest_timestamp}.") # Debug level might be sufficient

            # Display current market stats
            display_market_stats(current_price, latest_rsi, latest_stoch_k, latest_stoch_d, latest_atr, price_precision_digits)
            ...
            if position.get('status') is not None:
                log_info(f"Monitoring active {position['status'].upper()} position entered at {position.get('entry_price', 'N/A'):.{price_precision_digits}f}...")
            ...
            # --- B. If NOT in a Position: Check for Entry Signals ---
                ...
                if entry_volume_confirmation_enabled:
                    if current_volume is not None and latest_vol_ma is not None:
                        ...
                        log_debug(f"Volume Check: Current Vol={current_volume:.2f}, Vol MA={latest_vol_ma:.2f}, Threshold={latest_vol_ma * entry_volume_multiplier:.2f} -> {'OK' if volume_condition_met else 'FAILED'}")
                    else:
                ...
                base_long_signal = latest_rsi < rsi_oversold and latest_stoch_k < stoch_oversold and latest_stoch_d < stoch_oversold
                base_short_signal = latest_rsi > rsi_overbought and latest_stoch_k > stoch_overbought and latest_stoch_d > stoch_overbought
                if base_long_signal: log_debug("Base Signal: LONG (RSI & Stoch Oversold)")
                if base_short_signal: log_debug("Base Signal: SHORT (RSI & Stoch Overbought)")
                ...
                if base_long_signal and bullish_ob:
                    ...
                    else: log_debug(f"OB Confirmation: LONG Signal FAILED (Price {current_price:.{price_precision_digits}f} outside Bullish OB)")
                    ...
                    else: log_debug(f"OB Confirmation: SHORT Signal FAILED (Price {current_price:.{price_precision_digits}f} outside Bearish OB)")
                ...
                if base_long_signal and ob_confirmation_met and volume_condition_met and entry_order_block:
                    ...
                    if entry_volume_confirmation_enabled: entry_reason += ", Volume Confirmed"
                    ...
                    reasons = []
                    if not ob_confirmation_met: reasons.append("OB Confirmation Failed")
                    if not volume_condition_met: reasons.append("Volume Confirmation Failed")
                    log_info(f"Entry blocked for potential {'LONG' if base_long_signal else 'SHORT'} signal. Reason(s): {', '.join(reasons)}")
                ...
                if entry_side and entry_reason and entry_order_block:
                    ...
                    if enable_atr_sl_tp:
                        ...
                        sl_multiplier = 1.0 - stop_loss_percentage if entry_side == 'long' else 1.0 + stop_loss_percentage
                        tp_multiplier = 1.0 + take_profit_percentage if entry_side == 'long' else 1.0 - take_profit_percentage
                    ...
                    log_info(f"Initial Calculated SL: {initial_sl_price:.{price_precision_digits}f}, TP: {initial_tp_price:.{price_precision_digits}f}")
                    ...
                    if entry_side == 'long':
                        ...
                        if potential_refined_sl < initial_sl_price and potential_refined_sl < current_price:
                            ...
                            log_info(f"Refining SL based on Bullish OB low ({ob_low:.{price_precision_digits}f}) to: {refined_sl_price:.{price_precision_digits}f}")
                    else: # Short entry
                        ...
                        if potential_refined_sl > initial_sl_price and potential_refined_sl > current_price:
                            refined_sl_price = potential_refined_sl
                            log_info(f"Refining SL based on Bearish OB high ({ob_high:.{price_precision_digits}f}) to: {refined_sl_price:.{price_precision_digits}f}")
                    ...
                    try:
                        ...
                        if (entry_side == 'long' and (final_sl_price_fmt >= current_price or final_tp_price_fmt <= current_price)) or \
                           (entry_side == 'short' and (final_sl_price_fmt <= current_price or final_tp_price_fmt >= current_price)):
                            raise ValueError(f"Invalid final SL/TP logic after formatting: SL={final_sl_price_fmt}, TP={final_tp_price_fmt}, Entry={current_price}")

                    except ValueError as ve:
                        log_error(f"Final SL/TP validation failed: {ve}. Aborting entry.")
                        continue
                    except Exception as fmt_e:
                        log_error(f"Failed to format final SL ({final_sl_price}) or TP ({final_tp_price}): {fmt_e}. Aborting entry.")
                        continue

                    log_info(f"Final Target Prices: SL={final_sl_price_fmt:.{price_precision_digits}f}, TP={final_tp_price_fmt:.{price_precision_digits}f}")
                    ...
                    if entry_order_result and entry_order_result.get('id'):
                        ...
                        if actual_entry_price is None or actual_entry_price <= 0:
                            log_warning(f"Entry order {entry_order_id} filled, but average price not available. Using initial target price {current_price} for state.")
                        ...
                        if actual_filled_quantity is None or actual_filled_quantity <= 0:
                             log_warning(f"Entry order {entry_order_id} filled, but filled quantity not available or zero. Using requested quantity {entry_quantity} for state.")
                             actual_filled_quantity = entry_quantity
                        else:
                             actual_filled_quantity = float(actual_filled_quantity) # Ensure float

                        log_info(f"Entry market order placed/filled (ID: {entry_order_id}, Status: {entry_status}). Actual Entry Px: ~{actual_entry_price:.{price_precision_digits}f}, Filled Qty: {actual_filled_quantity:.{amount_precision_digits}f}")
                        ...
                        if oco_id or sl_id: # Success if at least OCO or separate SL was placed
                            ...
                            log_error(f"!!! CRITICAL ENTRY FAILURE !!!")
                            log_error(f"  Entry order {entry_order_id} filled for {actual_filled_quantity:.{amount_precision_digits}f} {base_currency}")
                            log_error(f"  BUT FAILED to place required SL/TP/OCO protection orders.")
                            log_error(f"  !!! POSITION IS ACTIVE BUT UNPROTECTED !!!")
                            ...
                            if emergency_order and emergency_order.get('id'):
                                log_warning(f"Emergency market close order placed (ID: {emergency_order['id']}). Status: {emergency_order.get('status')}. State NOT updated to active.")
                            else:
                                log_error(f"!!! EMERGENCY CLOSE ORDER FAILED TO PLACE !!! MANUAL INTERVENTION REQUIRED IMMEDIATELY FOR UNPROTECTED POSITION !!!")
                            # Do NOT update the state to active, as the position should ideally be closed now.

                    else:
                        log_error(f"Entry market order placement failed or did not return a valid result ({entry_order_result}). No position taken.")
        ...
        except (ccxt.RateLimitExceeded, ccxt.NetworkError, ccxt.ExchangeNotAvailable, ccxt.RequestTimeout, ccxt.DDoSProtection) as e:
            log_error(f"Recoverable CCXT Error encountered: {type(e).__name__}: {e}. Waiting and retrying...", exc_info=False)
            # Wait longer after rate limit errors
            wait_time = sleep_interval_seconds + 60 if isinstance(e, ccxt.RateLimitExceeded) else sleep_interval_seconds + 15
            log_warning(f"Waiting for {wait_time} seconds before next cycle due to error.")
            neon_sleep_timer(wait_time)
        except ccxt.AuthenticationError as e:
             log_error(f"!!! CRITICAL AUTHENTICATION ERROR: {e} !!! Check API credentials. Stopping bot.", exc_info=False)
