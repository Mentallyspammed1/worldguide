        ...
        if isinstance(value, str):
            ...
            dct[key] = [
                ...
                if isinstance(item, dict)
                    ...
                    and item.replace(".", "", 1).replace("-", "", 1).isdigit()
                    else item
                )
                for item in value
            ]
...
def load_config(config_path: Path, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    try:
        with open(config_path, "r") as f:
            ...
            config = json.load(
                f, object_pairs_hook=lambda pairs: decimal_decoder(dict(pairs))
            )
...
def validate_config(config: Dict[str, Any], logger: logging.Logger) -> bool:
    ...
    if "ema_short_period" in indicators and "ema_long_period" in indicators:
        if not (
            isinstance(indicators["ema_short_period"], int)
            and indicators["ema_short_period"] > 0
        ):
            logger.error(
            ...
            is_valid = False
        if not (
            isinstance(indicators["ema_long_period"], int)
            and indicators["ema_long_period"] > 0
        ):
            logger.error(
    ...
    if not isinstance(risk_percent, Decimal) or not (
        Decimal(0) < risk_percent <= Decimal(100)
    ):
        logger.error(
...
class BybitV5Wrapper:
        ...
        try:
            ...
            if market:
                ...
                if "precision" not in market or not market["precision"]:
                    self.logger.warning(
                        f"Precision info missing for market {symbol}. Reloading markets."
                    )
                    self.exchange.load_markets(reload=True)
                    market = self.exchange.market(symbol)
                    if "precision" not in market or not market["precision"]:
                        self.logger.error(
                            f"Failed to load precision info for {symbol} even after reload."
                        )
            ...
            self.logger.error(
                f"Error fetching market data for {symbol}: {e}", exc_info=True
            )
            return None
    ...
    def safe_ccxt_call(self, method_name: str, *args, **kwargs) -> Optional[Any]:
        ...
        while retries <= self.max_retries:
            try:
                ...
                self.logger.debug(
                    f"CCXT call {method_name} successful. Result snippet: {str(result)[:150]}..."
                )
                return result
            ...
            # --- Specific CCXT/Bybit Error Handling ---
                ...
                self.logger.error(
                    f"Permission Denied: {e}. Check API key permissions. Non-retryable."
                )
                return None  # Non-retryable
                ...
                self.logger.error(
                    f"Bad Request: {e}. Check parameters. May be non-retryable."
                )
                ...
                msg = str(e).lower()
                if (
                    "margin" in msg or "position idx" in msg
                ):  # Example non-retryable conditions
    ...
    def fetch_ohlcv(
        ...
        try:
            ...
            if df[["open", "high", "low", "close"]].isnull().values.any():
                self.logger.warning(
                    f"NaN values found in fetched OHLCV data for {symbol}."
                )
            if (df[["open", "high", "low", "close"]] <= Decimal(0)).values.any():
                self.logger.warning(
                    f"Zero or negative values found in fetched OHLCV prices for {symbol}."
                )

            self.logger.info(
                f"Successfully fetched and processed {len(df)} OHLCV candles."
            )
            return df
        except Exception as e:
            self.logger.error(
    ...
    def fetch_balance(self) -> Optional[Dict[str, Decimal]]:
        ...
        try:
            ...
            if self.exchange_id == "bybit" and self.category in ["linear", "inverse"]:
                    ...
                    if not relevant_account and target_account_type:
                        relevant_account = next(
                            (
                                acc
                                for acc in account_list
                                if acc.get("accountType") == target_account_type
                            ),
                            None,
                        )
                ...
                if relevant_account:
                    ...
                    for coin_info in coin_data:
                        ...
                        total_balance_str = coin_info.get(
                            "equity"
                        )  # Use equity as the base for risk %
            ...
            if not balances:
                ...
                self.logger.info(
                    f"Balance fetched successfully. Assets: {list(balances.keys())}"
                )
            return balances
    ...
    def fetch_positions(
        ...
        )  # Pass symbol in list for unified fetch_positions
        ...
        try:
            for pos in positions_data:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not parse position size '{size_str}' as Decimal. Skipping position."
                    )
                    continue
    ...
    def format_value_for_api(
        self, symbol: str, value_type: str, value: Decimal
    ) -> Union[str, float]:
    ...
    def create_order(
        ...
        if order_result:
            ...
            self.logger.info(
                f"Order creation request sent successfully. Order ID: {order_id}"
            )
    ...
    def set_leverage(self, symbol: str, leverage: Decimal) -> bool:
        ...
        if not (
            isinstance(leverage, Decimal) and leverage > 0 and leverage.is_finite()
        ):
            self.logger.error(
        ...
        result = self.safe_ccxt_call(
            "set_leverage", float(leverage), symbol, params=params
        )
        ...
        if result is not None:
            ...
            self.logger.info(
                f"Leverage for {symbol} set to {leverage}x successfully (or request accepted)."
            )
            ...
            if isinstance(result, dict) and result.get("info", {}).get("retCode") == 0:
                ...
                self.logger.warning(
                    "set_leverage call returned non-empty dict but no 'info'. Assuming success."
                )
                return True
            elif result is None:  # Can happen if ccxt method returns None on success
                self.logger.info(
                    "set_leverage call returned None. Assuming success based on lack of error."
                )
                return True
    ...
    def set_protection(
        ...
        self.logger.info(
            f"Attempting to set protection for {symbol}: {' / '.join(action)}"
        )
        ...
        try:
            ...
            if trailing_stop:
                ...
                if ts_value is not None:
                    if ts_value <= 0:
                        raise ValueError(
                            "Trailing stop distance/value must be positive."
                        )
                    ...
                    params["trailingStop"] = str(
                        ts_value
                    )  # Pass TSL distance/value as string
                ...
                if ts_active_price is not None:
                    if ts_active_price <= 0:
                        raise ValueError(
                            "Trailing stop activation price must be positive."
                        )
...
class TradingAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger):
        ...
        if abs(total_weight - Decimal("1.0")) > Decimal(
            "1e-9"
        ):  # Use tolerance for float conversion issues
            self.logger.warning(
    ...
    def calculate_indicators(self, ohlcv_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        if not all(isinstance(ohlcv_df[col].iloc[0], Decimal) for col in required_cols):
            self.logger.error(
                "OHLCV columns must be Decimal type for indicator calculation."
            )
            return None
        ...
        try:
            ...
            if isinstance(atr_period, int) and atr_period > 0:
                df.ta.atr(
                    length=atr_period, append=True, mamode="ema"
                )  # Use EMA for ATR smoothing typically
    ...
    def generate_signal(
        ...
        if not isinstance(indicators_df.index, pd.DatetimeIndex):
            self.logger.warning(
                "Indicators DataFrame index is not DatetimeIndex. Using iloc[-1]."
            )
            try:
        ...
        if rsi_weight > 0 and rsi_col and rsi_col in latest_data:
            ...
            if isinstance(rsi_value, Decimal) and rsi_value.is_finite():
                ...
                self.logger.warning(
                    f"Invalid RSI value for signal generation: {rsi_value}"
                )
        ...
        if macd_weight > 0 and macdh_col and macdh_col in latest_data:
            ...
            if isinstance(macd_hist, Decimal) and macd_hist.is_finite():
                ...
                hist_threshold = Decimal(
                    str(self.config.get("macd_hist_threshold", "0"))
                )
                ...
                self.logger.warning(
                    f"Invalid MACD Histogram value for signal generation: {macd_hist}"
                )
        ...
        if (
            ...
            if (
                ...
                self.logger.warning(
                    f"Invalid EMA values for signal generation: Short={ema_short}, Long={ema_long}"
                )
...
class PositionManager:
    ...
    def calculate_position_size(
        ...
        """
        risk_percent = self.risk_config.get(
        ...
        if not base_asset or not market_quote_asset:
            self.logger.error(
                f"Could not determine base/quote asset for {self.symbol}."
            )
            return None
        ...
        if stop_loss_distance <= Decimal(0):
            self.logger.error(
                f"Stop loss distance is zero or negative ({stop_loss_distance}). Check SL price."
            )
            return None
        ...
        if (
            ...
            self.logger.error(
                f"Calculated position size is invalid: {position_size_base}"
            )
            return None
        ...
        try:
            ...
            self.logger.debug(
                f"Position size after applying precision: {final_size_base} {base_asset}"
            )
            ...
            if max_amount_dec is not None and final_size_base > max_amount_dec:
                ...
                formatted_size_str = self.exchange.format_value_for_api(
                    self.symbol, "amount", final_size_base
                )
                final_size_base = Decimal(formatted_size_str)
                self.logger.info(
                    f"Position size capped to: {final_size_base} {base_asset}"
                )

            if final_size_base <= 0:
                self.logger.error(
                    "Final position size is zero or negative after adjustments. Cannot open position."
                )
                return None
    ...
    def quantize_price(
        self, price: Decimal, side: Optional[PositionSide] = None
    ) -> Optional[Decimal]:
        ...
        if (
            not market
            or "precision" not in market
            or "price" not in market["precision"]
        ):
            self.logger.error(
                f"Cannot quantize price: Market or price precision not found for {self.symbol}."
            )
        ...
        try:
            ...
            if tick_size_str is not None:
                ...
                if tick_size > 0:  # Ensure tick_size is valid
                    if side == PositionSide.LONG and price == self.state.get(
                        "stop_loss_price"
                    ):  # Round down Long SL
                        quantized_price = (price // tick_size) * tick_size
                    elif side == PositionSide.SHORT and price == self.state.get(
                        "stop_loss_price"
                    ):  # Round up Short SL
    ...
    def calculate_stop_loss(
        ...
        fixed_percent = self.risk_config.get(
        ...
        if atr_col and atr_col not in latest_indicators.index:
            ...
            if atr_col_alt in latest_indicators.index:
                ...
                self.logger.error(
                    f"ATR column ({atr_col} or {atr_col_alt}) not found in indicators."
                )
                return None
        ...
        if sl_method == "atr":
            ...
            if (
                ...
                self.logger.error(
                    f"Cannot calculate ATR stop loss: Invalid ATR value ({atr_value})."
                )
                return None
        ...
        if side == PositionSide.LONG and quantized_sl >= entry_price:
            self.logger.error(
                f"Quantized SL {quantized_sl} is >= entry price {entry_price}. Cannot set SL."
            )
    ...
    def check_ma_cross_exit(
        ...
        if (
            ...
            self.logger.warning(
                f"Invalid EMA values for MA cross exit check: Short={ema_short}, Long={ema_long}"
            )
            return False
    ...
    def manage_stop_loss(
        ...
        entry_price = position.get(
            "entryPrice"
        )  # Should be Decimal from fetch_positions
        current_sl_state = current_state.get(
            "stop_loss_price"
        )  # Get SL from our state (should be Decimal)
        position_side = PositionSide(
            position["side"]
        )  # Convert string 'long'/'short' to Enum
        mark_price = position.get("markPrice")  # Should be Decimal

        # Validate necessary data
        if not all(
            isinstance(val, Decimal) and val.is_finite()
            for val in [entry_price, current_sl_state, mark_price]
        ):
            self.logger.warning(
        ...
        if atr_value is None:
            self.logger.warning(
                "ATR value not available or invalid. Cannot perform ATR-based SL management (BE, TSL)."
            )
        ...
        if (
            ...
            if position_side == PositionSide.LONG:
                ...
                be_price_with_offset = (
                    entry_price + offset_distance
                )  # Target SL price slightly above entry
                if mark_price >= profit_target_price:
                    # Quantize potential BE price
                    quantized_be_price = self.quantize_price(
                        be_price_with_offset, side=PositionSide.LONG
                    )
                    if (
                        quantized_be_price is not None
                        ...
                        current_state["break_even_achieved"] = (
                            True  # Mark BE achieved in state
                        )
                ...
                be_price_with_offset = (
                    entry_price - offset_distance
                )  # Target SL price slightly below entry
                if mark_price <= profit_target_price:
                    # Quantize potential BE price
                    quantized_be_price = self.quantize_price(
                        be_price_with_offset, side=PositionSide.SHORT
                    )
                    if (
                        quantized_be_price is not None
                        and quantized_be_price < current_sl_state
        ...
        if use_tsl and atr_value is not None and not state_updated_this_cycle:
            ...
            if position_side == PositionSide.LONG:
                ...
                quantized_tsl_price = self.quantize_price(
                    potential_tsl_price, side=PositionSide.LONG
                )
                if (
                    quantized_tsl_price is not None
                ...
                quantized_tsl_price = self.quantize_price(
                    potential_tsl_price, side=PositionSide.SHORT
                )
                if (
                    quantized_tsl_price is not None
                    and quantized_tsl_price < current_sl_state
            ...
            if (
                new_sl_price is not None and new_sl_price != current_sl_state
            ):  # Check if TSL actually updated the price
                self.logger.info(
                    f"Trailing Stop Loss Update: New SL Price calculated = {new_sl_price}"
                )
                ...
                if current_state.get("break_even_achieved", False):
                    ...
                    if be_price_check is not None:
                        if (
                            ...
                            and new_sl_price < be_price_check
                        ):
                            self.logger.warning(
                        ...
                        self.logger.warning(
                            "Could not quantize entry price for TSL vs BE check."
                        )
        ...
        if new_sl_price is not None:
            ...
            if not new_sl_price.is_finite() or new_sl_price <= 0:
                self.logger.warning(
                    f"Calculated new SL price ({new_sl_price}) is invalid. Ignoring update."
                )
                return None
            ...
            price_tick_size = (
                self.exchange.get_market(self.symbol).get("precision", {}).get("price")
            )
            tolerance = (
                Decimal(str(price_tick_size)) / 2
                if price_tick_size
                else Decimal("1e-8")
            )
...
class TradingBot:
    ...
    def __init__(self, config_path: Path, state_path: Path):
        ...
        self.leverage = self.config["trading_settings"][
            "leverage"
        ]  # Should be Decimal from loader
        self.quote_asset = self.config["trading_settings"]["quote_asset"]
        self.poll_interval = self.config["trading_settings"].get(
        ...
        while self.is_running:
            try:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Indicators DataFrame is empty after calculation. Skipping cycle."
                    )
                    self._wait_for_next_cycle(start_time)
                    continue
                ...
                self.sync_bot_state_with_position(
                    current_position
                )  # Update self.state['active_position'] etc.
                ...
                self.logger.error(
                    f"Network error in main loop: {e}. Retrying after delay.",
                    exc_info=True,
                )
                time.sleep(
                    self.poll_interval
                )  # Wait standard interval after network error
            except Exception as e:
                self.logger.error(
    ...
    def get_current_position(self) -> Optional[Dict[str, Any]]:
        ...
        active_positions = [
            p
            for p in positions
            if p.get("contracts", Decimal(0)) != Decimal(0)
            or p.get("info", {}).get("size", Decimal(0)) != Decimal(0)
        ]
    ...
    def sync_bot_state_with_position(
        ...
        if current_position_on_exchange:
            ...
            exchange_pos_size = current_position_on_exchange.get(
                "contracts"
            ) or current_position_on_exchange.get("info", {}).get("size")
            ...
            exchange_sl = current_position_on_exchange.get("info", {}).get(
                "stopLoss"
            )  # SL from exchange 'info'

            # Convert exchange SL to Decimal if present
            exchange_sl_dec = None
            if (
                exchange_sl is not None and exchange_sl != "" and exchange_sl != "0"
            ):  # Check for valid SL string
                try:
            ...
            if not bot_thinks_has_position:
                ...
                self.logger.warning(
                    f"Found unexpected active position on exchange for {self.symbol}. Syncing state."
                )
                ...
                if self.hedge_mode:
                    if (
                        state_side.value == exchange_pos_side_str
                        and state_idx == exchange_pos_idx
                    ):
                        match = True
                ...
                if match:
                    ...
                    self.logger.debug(
                        "Exchange position matches state. Updating details."
                    )
                    ...
                    if exchange_sl_dec is not None and exchange_sl_dec > 0:
                        if current_state_sl is None or abs(
                            ...
                            if not self.state.get("break_even_achieved", False):
                                ...
                                if (
                                    state_side == PositionSide.LONG
                                    and exchange_sl_dec >= entry
                                ):
    ...
    def manage_existing_position(
        ...
        if not self.state.get("active_position"):
            self.logger.warning(
                "manage_existing_position called but state has no active position. Sync issue?"
            )
            return
        if not live_position_data:
            self.logger.error(
                "manage_existing_position called without live position data. Cannot proceed."
            )
            return
        ...
        if self.state.get("stop_loss_price") is None:
            self.logger.warning(
                "Cannot manage position: Stop loss price is missing from bot state."
            )
        ...
        if new_sl_price is not None:
            ...
            if (
                ...
                pos_idx = (
                    position_state.get("position_idx") if self.hedge_mode else None
                )
                # Pass only the SL price to set_protection
                if self.exchange.set_protection(
                    self.symbol, stop_loss=new_sl_price, position_idx=pos_idx
                ):
                    self.logger.info(
                        f"Stop loss updated successfully on exchange to {new_sl_price}."
                    )
                    ...
                    self.logger.error(
                        f"Failed to update stop loss on exchange to {new_sl_price}."
                    )
        ...
        exit_on_signal = self.config["trading_settings"].get(
        ...
        self.logger.debug(
            "No exit conditions met in this cycle. Position remains open."
        )
    ...
    def attempt_new_entry(
        ...
        if (
            ...
            self.logger.warning(
                "Could not get valid close price from indicators. Fetching ticker..."
            )
            try:
        ...
        if stop_loss_price is None:
            self.logger.error(
                "Failed to calculate stop loss. Cannot determine position size or place entry order."
            )
            return
        ...
        if not balance_info or self.quote_asset not in balance_info:
            self.logger.error(
                f"Failed to fetch sufficient balance info for {self.quote_asset}. Cannot size position."
            )
            return
        ...
        if order_result and order_result.get("id"):
            ...
            self.logger.info(
                f"Bot state updated for new {target_side.name} position. Intended SL: {stop_loss_price}."
            )

            # Optional: Short delay and verification step
            time.sleep(
                self.config["trading_settings"].get(
                    "post_order_verify_delay_seconds", 3
                )
            )  # Configurable delay
            ...
            if final_pos:
                ...
                if exchange_sl_dec is not None and abs(
                    exchange_sl_dec - stop_loss_price
                ) < Decimal("1e-8"):
                    self.logger.info(
                        f"Stop loss {stop_loss_price} confirmed on exchange position."
                    )
                    ...
                    self.state["stop_loss_price"] = (
                        exchange_sl_dec  # Trust exchange value after entry
                    )
                else:
                    self.logger.warning(
                    ...
                    pos_idx_retry = (
                        self.state["active_position"].get("position_idx")
                        if self.hedge_mode
                        else None
                    )
                    if not self.exchange.set_protection(
                        ...
                        self.logger.info(
                            "Successfully set stop loss via set_protection after initial attempt failed."
                        )
                        self.state["stop_loss_price"] = (
    ...
    def close_position(self, reason: str):
        ...
        if self.hedge_mode:
            ...
            if pos_idx is None:
                self.logger.error(
                    "Cannot close hedge mode position: positionIdx missing from state. Aborting close."
                )
        ...
        if order_result and order_result.get("id"):
            order_id = order_result["id"]
            self.logger.info(
                f"Position close order placed successfully. Order ID: {order_id}. Reason: {reason}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            logging.getLogger("TradingBot").critical(
