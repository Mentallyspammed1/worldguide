    ...
    for key, value in dct.items():
        if isinstance(value, str):
            ...
            # Recursively decode lists
             dct[key] = [
                 decimal_decoder(item) if isinstance(item, dict)
                 else (Decimal(item) if isinstance(item, str) and item.replace('.', '', 1).replace('-', '', 1).isdigit() else item)
                 for item in value
             ]
...
def validate_config(config: Dict[str, Any], logger: logging.Logger) -> bool:
    ...
    if not isinstance(leverage, Decimal) or leverage <= 0:
        logger.error("Config validation failed: 'trading_settings.leverage' must be a positive number (loaded as Decimal).")
        is_valid = False
    ...
    if not isinstance(risk_percent, Decimal) or not (Decimal(0) < risk_percent <= Decimal(100)):
         logger.error("Config validation failed: 'risk_per_trade_percent' must be a number between 0 (exclusive) and 100 (inclusive).")
         is_valid = False
...
class BybitV5Wrapper:
        ...
        while retries <= self.max_retries:
            try:
                ...
                if self.exchange_id == 'bybit' and 'category' not in params and self.category:
                     ...
                     if method_name in ['create_order', 'edit_order', 'cancel_order',
                                        'fetch_positions', 'fetch_balance', 'set_leverage',
                                        'set_margin_mode', 'fetch_open_orders', 'fetch_closed_orders',
                                        'fetch_my_trades', 'private_post_position_trading_stop']: # Added specific private call
            ...
            # --- Specific CCXT/Bybit Error Handling ---
                ...
                self.logger.error(f"Invalid Order parameters: {e}. Check order details (size, price, etc.). Non-retryable.")
                return None # Non-retryable
            ...
            if self.exchange_id == 'bybit' and self.category in ['linear', 'inverse']:
                ...
                if account_list:
                     ...
                     if not relevant_account and target_account_type:
                          relevant_account = next((acc for acc in account_list if acc.get('accountType') == target_account_type), None)
                ...
                if relevant_account:
                    ...
                    for coin_info in coin_data:
                     ...
                     self.logger.warning(f"Could not find relevant account type ('UNIFIED' or '{target_account_type}') in balance response for Bybit V5.")
    ...
    def fetch_positions(self, symbol: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            for pos in positions_data:
                ...
                decimal_fields_std = ['contracts', 'contractSize', 'entryPrice', 'leverage',
                ...
                if side == 'none' and size_dec != Decimal(0):
                     self.logger.warning(f"Position for {processed.get('symbol')} has size {size_dec} but side is 'None'. Check exchange data.")
    ...
    def create_order(self, symbol: str, order_type: str, side: OrderSide, amount: Decimal,
        ...
        try:
            ...
            price_str = self.format_value_for_api(symbol, 'price', price) if price is not None and order_type != 'market' else None
    ...
    def set_leverage(self, symbol: str, leverage: Decimal) -> bool:
        ...
        if result is not None:
             ...
             if isinstance(result, dict) and result.get('info', {}).get('retCode') == 0:
                  ...
                  self.logger.warning(f"set_leverage call returned unexpected result: {result}. Assuming success based on lack of error.")
                  return True
    ...
    def set_protection(self, symbol: str, stop_loss: Optional[Decimal] = None,
        ...
        action = []
        if stop_loss: action.append(f"SL={stop_loss}")
        if take_profit: action.append(f"TP={take_profit}")
        if trailing_stop: action.append(f"TSL={trailing_stop}")
        if not action:
        ...
        try:
            ...
            if trailing_stop:
                ...
                if ts_value is not None:
                     if ts_value <= 0: raise ValueError("Trailing stop distance/value must be positive.")
                ...
                if ts_active_price is not None:
                     if ts_active_price <= 0: raise ValueError("Trailing stop activation price must be positive.")
        ...
        if result is not None:
            ...
            if ret_code == 0:
                 self.logger.info(f"Protection levels set successfully for {symbol} (positionIdx: {position_idx if self.hedge_mode else 'N/A'}).")
                 return True
            else:
        ...
        try:
            ...
            for col in new_cols:
                 if pd.api.types.is_numeric_dtype(df[col]):
                     ...
                     df[col] = df[col].apply(lambda x: Decimal(str(x)) if pd.notna(x) and pd.api.types.is_number(x) and math.isfinite(x) else Decimal('NaN'))
...
class PositionManager:
    ...
    def calculate_position_size(self, entry_price: Decimal, stop_loss_price: Decimal,
        ...
        if not all(isinstance(val, Decimal) and val.is_finite() and val > 0 for val in [entry_price, stop_loss_price, available_equity]):
            self.logger.error(f"Invalid inputs for position size calculation: entry={entry_price}, sl={stop_loss_price}, equity={available_equity}. Must be positive finite Decimals.")
            return None
        ...
        self.logger.info(f"Risk per trade: {risk_percent:.2%}, Total Equity ({quote_asset}): {available_equity:.{DECIMAL_PRECISION}f}, Risk Amount: {risk_amount_quote:.{DECIMAL_PRECISION}f} {quote_asset}")
        ...
        contract_type = 'linear' # Default assumption, adjust if needed
        if market.get('inverse', False):
             ...
             self.logger.debug(f"Calculating size for INVERSE contract.")
        else: # Linear or Spot
             ...
             self.logger.debug(f"Calculating size for LINEAR/SPOT contract.")
        ...
        try:
            ...
            if min_amount_dec is not None and position_size_base < min_amount_dec:
                self.logger.warning(f"Calculated position size {position_size_base:.{DECIMAL_PRECISION}f} {base_asset} is below minimum order size {min_amount_dec} {base_asset}. Cannot open position.")
                return None
            ...
            if min_amount_dec is not None and final_size_base < min_amount_dec:
                self.logger.warning(f"Position size {final_size_base} after precision is below minimum {min_amount_dec}. Cannot open position.")
                return None
            ...
            if final_size_base <= 0:
                 self.logger.error(f"Final position size is zero or negative after adjustments. Cannot open position.")
                 return None
    ...
    def calculate_stop_loss(self, entry_price: Decimal, side: PositionSide, latest_indicators: pd.Series) -> Optional[Decimal]:
        ...
        atr_col = f"ATRr_{atr_period}" if isinstance(atr_period, int) else None # pandas_ta default ATR name might be ATR_{period} or ATRr_{period} - check output
        ...
        if sl_method == "atr":
            ...
            if fixed_percent <= 0 or fixed_percent >= 1:
                 self.logger.error(f"Invalid fixed_stop_loss_percent: {fixed_percent*100}%. Must be between 0 and 100.")
                 return None
            if side == PositionSide.LONG:
        ...
        quantized_sl = self.quantize_price(stop_loss_price, side=side) # Pass side for conservative rounding if implemented
    ...
    def manage_stop_loss(self, position: Dict[str, Any], latest_indicators: pd.Series, current_state: Dict[str, Any]) -> Optional[Decimal]:
        ...
        if not all(isinstance(val, Decimal) and val.is_finite() for val in [entry_price, current_sl_state, mark_price]):
            self.logger.warning(f"Missing or invalid data for SL management: Entry={entry_price}, CurrentSL={current_sl_state}, MarkPrice={mark_price}")
            return None
        if entry_price <= 0 or mark_price <= 0:
             self.logger.warning(f"Entry price or mark price is non-positive: Entry={entry_price}, Mark={mark_price}. Cannot manage SL.")
             return None
        ...
        if atr_col and atr_col not in latest_indicators.index:
             atr_col_alt = f"ATR_{atr_period}"
             if atr_col_alt in latest_indicators.index: atr_col = atr_col_alt
             else: atr_col = None # ATR not available
        ...
        if atr_value is None:
             self.logger.warning(f"ATR value not available or invalid. Cannot perform ATR-based SL management (BE, TSL).")
        ...
        if use_be and atr_value is not None and not current_state.get('break_even_achieved', False):
            ...
            be_price = entry_price # Default BE is entry price
            ...
            if position_side == PositionSide.LONG:
                ...
                if mark_price >= profit_target_price:
                    ...
                    if quantized_be_price is not None and quantized_be_price > current_sl_state:
                        self.logger.info(f"Break-Even Triggered (Long): Mark Price {mark_price} >= Target {profit_target_price}. Moving SL from {current_sl_state} to {quantized_be_price}")
                        ...
                        state_updated_this_cycle = True
                         ...
                         self.logger.debug(f"BE triggered (Long), but proposed BE price {quantized_be_price} is not better than current SL {current_sl_state}.")
                ...
                if mark_price <= profit_target_price:
                    ...
                    if quantized_be_price is not None and quantized_be_price < current_sl_state:
                        self.logger.info(f"Break-Even Triggered (Short): Mark Price {mark_price} <= Target {profit_target_price}. Moving SL from {current_sl_state} to {quantized_be_price}")
                        new_sl_price = quantized_be_price
                        current_state['break_even_achieved'] = True # Mark BE achieved in state
                        state_updated_this_cycle = True
                    elif quantized_be_price is None:
                         ...
                         self.logger.debug(f"BE triggered (Short), but proposed BE price {quantized_be_price} is not better than current SL {current_sl_state}.")
        ...
        if use_tsl and atr_value is not None and not state_updated_this_cycle:
             ...
             if position_side == PositionSide.LONG:
                 ...
                 if quantized_tsl_price is not None and quantized_tsl_price > current_sl_state:
                     self.logger.debug(f"Trailing SL Update (Long): Potential TSL {quantized_tsl_price} > Current SL {current_sl_state}")
                 ...
                 if quantized_tsl_price is not None and quantized_tsl_price < current_sl_state:
                     self.logger.debug(f"Trailing SL Update (Short): Potential TSL {quantized_tsl_price} < Current SL {current_sl_state}")
                     new_sl_price = quantized_tsl_price # Update SL if TSL is better
                 elif quantized_tsl_price is None:
             ...
             if new_sl_price is not None and new_sl_price != current_sl_state: # Check if TSL actually updated the price
                 ...
                 if current_state.get('break_even_achieved', False):
                      be_price_check = self.quantize_price(entry_price, side=position_side) # Re-quantize entry for comparison
                      if be_price_check is not None:
                           if position_side == PositionSide.LONG and new_sl_price < be_price_check:
                                self.logger.warning(f"TSL calculation resulted in SL ({new_sl_price}) below quantized entry ({be_price_check}) after BE. Clamping SL to entry.")
                                new_sl_price = be_price_check
                           elif position_side == PositionSide.SHORT and new_sl_price > be_price_check:
                                self.logger.warning(f"TSL calculation resulted in SL ({new_sl_price}) above quantized entry ({be_price_check}) after BE. Clamping SL to entry.")
                                new_sl_price = be_price_check
                      else:
        ...
        if new_sl_price is not None:
            ...
            if position_side == PositionSide.LONG and new_sl_price >= mark_price:
                 self.logger.warning(f"New SL price {new_sl_price} is >= current mark price {mark_price}. Invalid SL. Ignoring update.")
                 return None
            ...
            if abs(new_sl_price - current_sl_state) > tolerance:
                 ...
                 self.logger.debug(f"Calculated new SL {new_sl_price} is not significantly different from current {current_sl_state}. No update needed.")
...
class TradingBot:
    ...
    def __init__(self, config_path: Path, state_path: Path):
        ...
        if isinstance(log_level_enum, int):
             ...
             self.logger.warning(f"Invalid log level '{log_level_str}' in config. Using default {logging.getLevelName(LOG_LEVEL)}.")
    ...
    def run(self):
        ...
        self.logger.info(f"Symbol: {self.symbol}, Timeframe: {self.timeframe}, Leverage: {self.leverage}x, Hedge Mode: {self.hedge_mode}")
    ...
    def initialize_exchange_settings(self) -> bool:
        ...
        if not self.exchange.set_leverage(self.symbol, self.leverage):
             self.logger.error(f"Failed to set initial leverage to {self.leverage}x for {self.symbol}. Check permissions/settings.")
    ...
    def get_current_position(self) -> Optional[Dict[str, Any]]:
        ...
        active_positions = [p for p in positions if p.get('contracts', Decimal(0)) != Decimal(0) or p.get('info', {}).get('size', Decimal(0)) != Decimal(0)]
        ...
        if self.hedge_mode:
            ...
            if state_pos:
                 ...
                 if found_match:
                      self.logger.info(f"Found active hedge mode position matching state: Side {found_match.get('side')}, Idx {found_match.get('positionIdx')}")
                      return found_match
                 else:
                      # Bot state has a position, but no matching one found on exchange. State is likely stale.
                      self.logger.warning(f"Bot state indicates hedge position (Idx: {target_idx}, Side: {target_side}), but no matching active position found on exchange.")
                 ...
                 self.logger.warning(f"Found {len(active_positions)} active hedge mode position(s) on exchange, but bot state has no active position. Ignoring external positions.")
                 return None # Don't return external positions if bot thinks it has none

        else: # Non-hedge mode
            if len(active_positions) > 1:
                 self.logger.warning(f"Expected only one active position for {self.symbol} in non-hedge mode, but found {len(active_positions)}. Using the first one found.")
                 # Potentially refine this: choose largest, or one matching state if state exists?
            pos = active_positions[0]
            self.logger.info(f"Found active non-hedge position: Side {pos.get('side')}, Size {pos.get('contracts') or pos.get('info', {}).get('size')}")
            return pos
    ...
    def sync_bot_state_with_position(self, current_position_on_exchange: Optional[Dict[str, Any]]):
        ...
        if current_position_on_exchange:
            ...
            exchange_pos_size = current_position_on_exchange.get('contracts') or current_position_on_exchange.get('info', {}).get('size')
            ...
            if not bot_thinks_has_position:
                ...
                self.state['active_position'] = {
                    ...
                    'side': PositionSide(exchange_pos_side_str) if exchange_pos_side_str in ['long', 'short'] else PositionSide.NONE,
                ...
                if exchange_sl_dec is not None and exchange_sl_dec > 0:
                     ...
                     self.logger.warning("No valid stop loss found on the unexpected exchange position. SL state is None.")
                ...
                if match:
                     ...
                     self.logger.debug(f"Exchange position matches state. Updating details.")
                     ...
                     if exchange_sl_dec is not None and exchange_sl_dec > 0:
                          if current_state_sl is None or abs(exchange_sl_dec - current_state_sl) > Decimal('1e-8'):
                               ...
                               if not self.state.get('break_even_achieved', False):
                                    ...
                                    if state_side == PositionSide.LONG and exchange_sl_dec >= entry:
                                         ...
                                         self.logger.info("Marking Break-Even as achieved based on updated SL from exchange.")
                          ...
                          self.logger.warning(f"Bot state has SL {current_state_sl}, but no SL found on exchange position. Clearing state SL.")
                     ...
                     self.logger.warning(f"Found active position on exchange ({exchange_pos_side_str}, Idx: {exchange_pos_idx}) that does NOT match bot state ({state_side.value}, Idx: {state_idx}). Clearing bot state.")
            ...
            if bot_thinks_has_position:
                ...
                self.logger.info(f"Position for {self.symbol} no longer found on exchange (likely closed/stopped/liquidated). Clearing bot state.")
    ...
    def manage_existing_position(self, latest_indicators: pd.Series, signal: Signal, live_position_data: Dict[str, Any]):
        """
        ...
        if new_sl_price is not None:
            ...
            if (position_side == PositionSide.LONG and new_sl_price >= live_position_data.get('markPrice', new_sl_price)) or \
               (position_side == PositionSide.SHORT and new_sl_price <= live_position_data.get('markPrice', new_sl_price)):
                 self.logger.warning(f"Proposed new SL {new_sl_price} is invalid relative to mark price {live_position_data.get('markPrice')}. Aborting SL update.")
            else:
    ...
    def attempt_new_entry(self, signal: Signal, latest_indicators: pd.Series, signal_details: Dict[str, Any]):
        ...
        if not isinstance(available_equity, Decimal) or not available_equity.is_finite() or available_equity <= 0:
             self.logger.error(f"Invalid available equity for {self.quote_asset}: {available_equity}. Cannot size position.")
             return
        ...
        # --- Place Entry Order ---
        self.logger.info(f"Attempting to place {order_side.value} market order for {position_size} {self.symbol} with calculated SL {stop_loss_price}")
        ...
        if order_result and order_result.get('id'):
            ...
            self.logger.info(f"Entry order placed successfully. Order ID: {order_id}. Waiting briefly for fill/confirmation...")
            ...
            if final_pos:
                 ...
                 if exchange_sl_str and exchange_sl_str != '0':
                      try: exchange_sl_dec = Decimal(exchange_sl_str)
                      except: pass
                 ...
                 if exchange_sl_dec is not None and abs(exchange_sl_dec - stop_loss_price) < Decimal('1e-8'):
                      ...
                      self.logger.warning(f"Stop loss on exchange ({exchange_sl_dec}) differs from intended ({stop_loss_price}). State SL updated to exchange value.")
                      self.state['stop_loss_price'] = exchange_sl_dec # Trust exchange value after entry
                 else:
                      self.logger.warning("Stop loss NOT found or is zero on exchange position after placing order with SL parameter. Attempting to set SL via set_protection.")
                      ...
                      if not self.exchange.set_protection(self.symbol, stop_loss=stop_loss_price, position_idx=pos_idx_retry):
                           ...
                           self.state['stop_loss_price'] = stop_loss_price # Update state SL to intended value after successful set
                 ...
                 self.logger.error("Position not found on exchange shortly after placing entry order. Clearing potentially stale state.")
    ...
    def close_position(self, reason: str):
        ...
        if size_to_close <= 0:
             self.logger.error(f"Cannot close position: Size in state is zero or negative ({size_to_close}). Clearing state.")
             self._clear_position_state(f"Invalid size in state during close: {size_to_close}")
             save_state(self.state, self.state_path, self.logger)
             return

        self.logger.info(f"Attempting to close {current_side.name} position of size {size_to_close} for reason: {reason}")
