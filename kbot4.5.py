...
def fetch_with_retries(fetch_function, *args, max_retries=3, delay_seconds=3, **kwargs) -> Any:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.critical(
                f"{Fore.RED + Style.BRIGHT}Authentication error: {e}. Check API keys. Halting.", exc_info=False
            )
            ...
            if attempt < max_retries:
                ...
                logger.error(f"Max retries reached after exchange error for {fetch_function.__name__}.")
                break
    ...
    def _get_env(
        ...
        if value_str is None or value_str.strip() == "":
            ...
            if is_secret and default is None:
                logger.critical(f"{Fore.RED + Style.BRIGHT}Required secret '{key}' not found. Halting.")
                sys.exit(1)
        ...
        if value_str_for_cast is None:
            logger.critical(f"{Fore.RED + Style.BRIGHT}Config '{key}' has no value or default. Halting.")
            sys.exit(1)
        ...
        try:
            ...
            if cast_type == bool:
                ...
                casted_value = int(Decimal(val_to_cast))  # Use Decimal intermediary for float strings
            else:
            ...
            try:  # Re-cast default
                ...
                if default_str is None:
                    ...
                    casted_value = default_str.lower() in ["true", "1", "yes", "y", "on"]
                elif cast_type == Decimal:
...
class ExchangeManager:
    ...
    def _initialize_exchange(self):
        ...
        try:
            ...
            logger.critical(
                f"{Fore.RED + Style.BRIGHT}Authentication failed: {e}. Check API keys/permissions.", exc_info=False
            )
            sys.exit(1)
    ...
    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if not pos_info:
                logger.debug(f"No position found for symbol {market_id} and posIdx {self.config.position_idx}.")
                return positions_dict
    ...
    def generate_signals(
        ...
        try:
            ...
            if current_price.is_nan() or current_price <= 0:
                return {"long": False, "short": False, "reason": "Invalid price (NaN or <= 0)"}
...
class OrderManager:
    ...
    def _calculate_trade_parameters(
        ...
        try:
            ...
            if sl_distance_price <= self.market_info["tick_size"]:  # Use minimum tick size for check
                logger.error(f"SL distance ({sl_distance_price}) too small or zero. Cannot calculate quantity.")
                return None
            ...
            min_order_size = safe_decimal(
                self.market_info.get("limits", {}).get("amount", {}).get("min"), default=Decimal("0")
            )
    ...
    def _set_position_protection(
        ...
        try:
            ...
            if not hasattr(self.exchange, path):
                ...
                if not hasattr(self.exchange, path):
                    raise NotImplementedError("CCXT method for V5 SetTradingStop not found.")
        ...
        try:
            ...
            if should_activate_tsl:
                ...
                if self._set_position_protection(
                    position_side, is_tsl=True, tsl_distance=tsl_distance_price, tsl_activation_price=activation_price
                ):
    ...
    def close_position(self, position_side: str, qty_to_close: Decimal, reason: str = "Signal") -> bool:
        ...
        if final_positions is not None:
            if (
                ...
                or final_positions.get(position_side, {}).get("qty", Decimal("NaN")).copy_abs()
    ...
    def log_trade_exit_to_journal(
        self, side: str, qty: Decimal, avg_price: Decimal, order_id: Optional[str], reason: str
    ):
        ...
        try:
            ...
            logger.error(f"Unexpected error writing exit to journal: {e}", exc_info=True)
    ...
    def print_status_panel_with_market_info(
        ...
        if indicators:
            ...
            def fmt_ind(key, prec=1):
                return self._format_decimal(indicators.get(key), precision=prec, add_commas=False)
...
class TradingBot:
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        try:  # Extract Price & Timestamp
            ...
            logger.error(f"{Fore.RED}Cycle Failed: Price/Timestamp error: {e}", exc_info=False)
        ...
        if indicators is None:
            logger.warning(f"{Fore.YELLOW}Indicator calculation failed. Logic may be impacted.")
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
            logger.error(f"{Fore.RED}Failed fetching valid equity. Trading logic skipped.")
        ...
        if can_run_trade_logic:
            ...
            if positions_after_tsl is None:
                logger.error(f"{Fore.RED}Failed re-fetching positions after TSL check. Assuming previous state.")
    ...
    def graceful_shutdown(self) -> None:
        ...
        try:  # --- 1. Cancel All Open Orders ---
            ...
            try:
                ...
                logger.error(f"{Fore.RED}Error cancelling active orders: {e}", exc_info=True)
        ...
        try:  # --- 2. Close Any Lingering Positions ---
            ...
            if positions is not None:
                ...
                for side, pos_data in positions.items():
                    if (
                        pos_data
                        and safe_decimal(pos_data.get("qty", "0")).copy_abs() >= self.config.position_qty_epsilon
                    ):
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(
            f"{Fore.RED + Style.BRIGHT}Critical error during bot execution: {main_exception}", exc_info=True
        )
