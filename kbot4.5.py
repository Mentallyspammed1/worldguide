...
try:
    ...
    colorama_init(autoreset=True); missing_pkg = e.name
    ...
    else: print(f"{Style.BRIGHT}pip install {' '.join(COMMON_PACKAGES)}{Style.RESET_ALL}")
    sys.exit(1)
...
if not hasattr(logging.Logger, 'trade'):
    ...
    def trade_log(self, message, *args, **kws):
        if self.isEnabledFor(TRADE_LEVEL_NUM): self._log(TRADE_LEVEL_NUM, message, args, **kws) # pylint: disable=protected-access
...
def termux_notify(title: str, content: str) -> None:
    ...
    if platform.system() == "Linux" and "com.termux" in os.environ.get("PREFIX", ""):
        try:
            ...
            subprocess.run(["termux-toast", content], check=False, timeout=5, capture_output=True, text=True)
            # logger.debug(f"Termux toast sent: '{content}' (Title '{title}' ignored by toast)")
        except FileNotFoundError: logger.warning("Termux notify failed: 'termux-toast' not found.")
        except subprocess.TimeoutExpired: logger.warning("Termux notify failed: command timed out.")
        except Exception as e: logger.warning(f"Termux notify failed: {e}")
...
def fetch_with_retries(fetch_function, *args, max_retries=3, delay_seconds=3, **kwargs) -> Any:
    ...
    for attempt in range(max_retries + 1):
        try:
        ...
        except (ccxt.DDoSProtection, ccxt.RequestTimeout, ccxt.ExchangeNotAvailable, ccxt.NetworkError, requests.exceptions.RequestException) as e:
            last_exception = e
            ...
            if attempt < max_retries: logger.warning(retry_msg); time.sleep(delay_seconds)
            else: logger.error(f"{Fore.RED}Max retries ({max_retries + 1}) reached for {fetch_function.__name__}. Last error: {e}"); break
        except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Authentication error: {e}. Check API keys. Halting.", exc_info=False); sys.exit(1) # Less verbose exc_info for auth
        except ccxt.InsufficientFunds as e: logger.error(f"{Fore.RED}Insufficient funds: {e}"); break # Don't retry InsufficientFunds
        except ccxt.InvalidOrder as e: logger.error(f"{Fore.RED}Invalid order parameters: {e}"); break # Don't retry InvalidOrder
        except ccxt.OrderNotFound as e: logger.warning(f"{Fore.YELLOW}Order not found: {e}"); break # Often not retryable in context
        except ccxt.PermissionDenied as e: logger.error(f"{Fore.RED}Permission denied: {e}. Check API permissions/IP whitelisting."); break # Not retryable
        except ccxt.ExchangeError as e: # Catch other specific exchange errors
            logger.error(f"{Fore.RED}Exchange error during {fetch_function.__name__}: {e}")
            last_exception = e
            if attempt < max_retries: logger.warning(f"Retrying in {delay_seconds}s..."); time.sleep(delay_seconds)
            else: logger.error(f"Max retries reached after exchange error for {fetch_function.__name__}."); break
    ...
    else: raise RuntimeError(f"Function {fetch_function.__name__} failed after unexpected issue.")
...
class TradingConfig:
    ...
    def __init__(self):
        ...
        self.market_type = self._get_env("MARKET_TYPE", "linear", Style.DIM, allowed_values=['linear', 'inverse', 'swap']).lower()
        ...
        self.tp_atr_multiplier = self._get_env("TP_ATR_MULTIPLIER", "3.0", Fore.YELLOW, cast_type=Decimal, min_val=Decimal("0.0"), max_val=Decimal("50.0"))
        ...
        self.sl_trigger_by = self._get_env("SL_TRIGGER_BY", "LastPrice", Style.DIM, allowed_values=["LastPrice", "MarkPrice", "IndexPrice"])
        self.tsl_trigger_by = self._get_env("TSL_TRIGGER_BY", "LastPrice", Style.DIM, allowed_values=["LastPrice", "MarkPrice", "IndexPrice"]) # TSL trigger usually same as SL
        ...
        self.trend_ema_period = self._get_env("TREND_EMA_PERIOD", "12", Style.DIM, cast_type=int, min_val=5, max_val=500)
        ...
        self.stoch_oversold_threshold = self._get_env("STOCH_OVERSOLD_THRESHOLD", "30", Fore.CYAN, cast_type=Decimal, min_val=Decimal("0"), max_val=Decimal("45"))
        self.stoch_overbought_threshold = self._get_env("STOCH_OVERBOUGHT_THRESHOLD", "70", Fore.CYAN, cast_type=Decimal, min_val=Decimal("55"), max_val=Decimal("100"))
        self.trend_filter_buffer_percent = self._get_env("TREND_FILTER_BUFFER_PERCENT", "0.5", Fore.CYAN, cast_type=Decimal, min_val=Decimal("0"), max_val=Decimal("5"))
        self.atr_move_filter_multiplier = self._get_env("ATR_MOVE_FILTER_MULTIPLIER", "0.5", Fore.CYAN, cast_type=Decimal, min_val=Decimal("0"), max_val=Decimal("5"))
        self.min_adx_level = self._get_env("MIN_ADX_LEVEL", "20", Fore.CYAN, cast_type=Decimal, min_val=Decimal("0"), max_val=Decimal("90"))
        ...
        self.order_check_delay_seconds = self._get_env("ORDER_CHECK_DELAY_SECONDS", "2", Style.DIM, cast_type=int, min_val=1)
        self.order_fill_timeout_seconds = self._get_env("ORDER_FILL_TIMEOUT_SECONDS", "20", Style.DIM, cast_type=int, min_val=5) # Longer for fills
        self.max_fetch_retries = self._get_env("MAX_FETCH_RETRIES", "3", Style.DIM, cast_type=int, min_val=1, max_val=10)
    ...
    def _determine_v5_category(self) -> str:
        ...
        try:
             ...
             settle_curr = parts[-1].upper()
             category = ''
             if self.market_type == 'inverse': category = 'inverse'
             elif self.market_type in ['linear', 'swap']: category = 'linear' # Linear typically USDT or USDC settle
             else: raise ValueError(f"Unsupported MARKET_TYPE '{self.market_type}'")
             logger.info(f"Determined Bybit V5 API category: '{category}' for symbol '{self.symbol}' and type '{self.market_type}'")
             return category
        except (ValueError, IndexError) as e: logger.critical(f"Could not parse symbol '{self.symbol}' for category: {e}. Halting.", exc_info=True); sys.exit(1)

    def _validate_config(self):
        """Performs post-load validation."""
        if self.fast_ema_period >= self.slow_ema_period: logger.critical(f"{Fore.RED+Style.BRIGHT}FAST_EMA ({self.fast_ema_period}) must be < SLOW_EMA ({self.slow_ema_period}). Halting."); sys.exit(1)
        ...
        if self.stoch_oversold_threshold >= self.stoch_overbought_threshold: logger.critical(f"{Fore.RED+Style.BRIGHT}STOCH_OVERSOLD ({self.stoch_oversold_threshold.normalize()}) must be < STOCH_OVERBOUGHT ({self.stoch_overbought_threshold.normalize()}). Halting."); sys.exit(1)
    ...
    def _get_env(self, key: str, default: Any, color: str, cast_type: type = str, min_val=None, max_val=None, allowed_values=None) -> Any:
        ...
        if value_str is None or value_str.strip() == "":
            if default is None and not is_secret: logger.critical(f"{Fore.RED+Style.BRIGHT}Required config '{key}' not found. Halting."); sys.exit(1)
            if is_secret and default is None: logger.critical(f"{Fore.RED+Style.BRIGHT}Required secret '{key}' not found. Halting."); sys.exit(1)
        ...
        if value_str_for_cast is None:
             logger.critical(f"{Fore.RED+Style.BRIGHT}Config '{key}' has no value or default. Halting."); sys.exit(1)

        casted_value = None
        try:
            val_to_cast = str(value_str_for_cast).strip() # Ensure string and strip whitespace
            if cast_type == bool: casted_value = val_to_cast.lower() in ['true', '1', 'yes', 'y', 'on']
            elif cast_type == Decimal: casted_value = Decimal(val_to_cast)
            elif cast_type == int: casted_value = int(Decimal(val_to_cast)) # Use Decimal intermediary for float strings
            else: casted_value = cast_type(val_to_cast)
            ...
            try: # Re-cast default
                ...
                if default_str is None: casted_value = None
                elif cast_type == bool: casted_value = default_str.lower() in ['true', '1', 'yes', 'y', 'on']
                elif cast_type == Decimal: casted_value = Decimal(default_str)
                elif cast_type == int: casted_value = int(Decimal(default_str))
                else: casted_value = cast_type(default_str)
            except (ValueError, TypeError, InvalidOperation) as cast_default_err: logger.critical(f"{Fore.RED+Style.BRIGHT}Default '{default}' for {key} invalid ({cast_type.__name__}): {cast_default_err}. Halting."); sys.exit(1)

        # --- Validation ---
        if allowed_values:
            comp_value = str(casted_value).lower() if isinstance(casted_value, str) else casted_value
            lower_allowed = [str(v).lower() for v in allowed_values] if isinstance(allowed_values[0], str) else allowed_values
            if comp_value not in lower_allowed:
                 logger.error(f"{Fore.RED}Validation failed for {key}: Invalid value '{casted_value}'. Allowed: {allowed_values}. Using default: {default}")
                 # Recast default on validation failure (duplicate code, consider refactoring)
                 try:
                    default_str = str(default).strip() if default is not None else None
                    if default_str is None: return None
                    if cast_type == bool: return default_str.lower() in ['true', '1', 'yes', 'y', 'on']
                    elif cast_type == Decimal: return Decimal(default_str)
                    elif cast_type == int: return int(Decimal(default_str))
                    else: return cast_type(default_str)
                 except (ValueError, TypeError, InvalidOperation) as cast_default_err: logger.critical(f"{Fore.RED+Style.BRIGHT}Default '{default}' for {key} invalid on fallback: {cast_default_err}. Halting."); sys.exit(1)

        if min_val is not None and casted_value < min_val: logger.critical(f"{Fore.RED+Style.BRIGHT}Validation failed for {key}: {casted_value} < min {min_val}. Halting."); sys.exit(1)
        if max_val is not None and casted_value > max_val: logger.critical(f"{Fore.RED+Style.BRIGHT}Validation failed for {key}: {casted_value} > max {max_val}. Halting."); sys.exit(1)
...
class ExchangeManager:
    ...
    def _initialize_exchange(self):
        ...
        logger.info(f"Initializing Bybit exchange interface (V5)...")
        try:
            exchange_params = {
        ...
        except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Authentication failed: {e}. Check API keys/permissions.", exc_info=False); sys.exit(1)
        except ccxt.NetworkError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Network error initializing exchange: {e}. Check connection.", exc_info=True); sys.exit(1)
        except Exception as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Unexpected error initializing exchange: {e}", exc_info=True); sys.exit(1)
    ...
    def _load_market_info(self) -> Optional[Dict]:
        ...
        try:
            ...
            if not market: raise ccxt.ExchangeError(f"Market {self.config.symbol} not found.")
            # Extract precision safely
            amount_precision = safe_decimal(market.get('precision', {}).get('amount'), default=Decimal('8')) # Base precision
            price_precision = safe_decimal(market.get('precision', {}).get('price'), default=Decimal('8'))   # Price precision
            # Convert precision decimals to number of decimal places (integer)
            amount_dp = abs(amount_precision.as_tuple().exponent) if not amount_precision.is_nan() else 8
            price_dp = abs(price_precision.as_tuple().exponent) if not price_precision.is_nan() else 8
            ...
            min_amt = safe_decimal(market.get('limits', {}).get('amount', {}).get('min'), default=Decimal('NaN'))
            logger.info(f"Market info loaded: ID={market.get('id')}, Precision(AmtDP={amount_dp}, PriceDP={price_dp}), Limits(MinAmt={min_amt.normalize() if not min_amt.is_nan() else 'N/A'})")
            return market
        except (ccxt.ExchangeError, KeyError, Exception) as e:
            logger.critical(f"{Fore.RED+Style.BRIGHT}Failed to load market info for {self.config.symbol}: {e}. Halting.", exc_info=True)
            sys.exit(1)
    ...
    def format_price(self, price: Union[Decimal, str, float, int]) -> str:
        ...
        if self.market_info and 'precision_dp' in self.market_info and 'price' in self.market_info['precision_dp']:
            ...
            if price_decimal.is_nan(): return 'NaN' # Return NaN string if input was bad
            quantizer = Decimal('1e-' + str(precision))
            return str(price_decimal.quantize(quantizer, rounding=ROUND_HALF_EVEN))
        else: logger.warning("Market info/price precision unavailable, using default formatting."); return str(safe_decimal(price))
    ...
    def format_amount(self, amount: Union[Decimal, str, float, int], rounding_mode=ROUND_DOWN) -> str:
        ...
        if self.market_info and 'precision_dp' in self.market_info and 'amount' in self.market_info['precision_dp']:
            ...
            if amount_decimal.is_nan(): return 'NaN' # Return NaN string if input was bad
            quantizer = Decimal('1e-' + str(precision))
            return str(amount_decimal.quantize(quantizer, rounding=rounding_mode))
        else: logger.warning("Market info/amount precision unavailable, using default formatting."); return str(safe_decimal(amount))
    ...
    def fetch_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not ohlcv: logger.error("fetch_ohlcv returned empty list."); return None
            ...
            return df
        except Exception as e: logger.error(f"Failed to fetch OHLCV data: {e}", exc_info=True); return None
    ...
    def get_balance(self) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        if not self.exchange or not self.market_info: return None, None
        settle_currency = self.market_info.get('settle')
        if not settle_currency: logger.error("Settle currency not found in market info."); return None, None
        logger.debug(f"Fetching balance for {settle_currency} (Cat: {self.config.bybit_v5_category})...")
        try:
            # V5 balance needs category and accountType
            params = {'category': self.config.bybit_v5_category, 'accountType': 'UNIFIED'}
            ...
            total_equity = Decimal('NaN'); available_balance = Decimal('NaN')
                     ...
                     if unified_acc and 'coin' in unified_acc:
                          for coin_info in unified_acc['coin']:
                              if coin_info.get('coin') == settle_currency:
                                  ...
                                  available_balance = safe_decimal(coin_info.get('availableToWithdraw')) # Use 'availableToWithdraw' for free balance
                                  break
            ...
            if total_equity.is_nan() and settle_currency in balance_data['total']:
                ...
                available_balance = safe_decimal(balance_data[settle_currency].get('free'))

            if total_equity.is_nan(): logger.error(f"Could not extract valid total equity for {settle_currency}. Raw balance response snippet: {str(balance_data)[:500]}"); return None, None
            ...
            logger.debug(f"Balance Fetched: Equity={total_equity.normalize()}, Available={available_balance.normalize()} {settle_currency}")
            return total_equity, available_balance
        except Exception as e: logger.error(f"Failed to fetch balance: {e}", exc_info=True); return None, None

    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        """Fetches current position details for the symbol using V5 API."""
        if not self.exchange or not self.market_info: return None
        market_id = self.market_info.get('id')
        logger.debug(f"Fetching position for {self.config.symbol} (ID: {market_id}, Cat: {self.config.bybit_v5_category})...")
        ...
        try:
            ...
            if not position_data: logger.debug("No position data returned."); return positions_dict
            ...
            for p in position_data:
                ...
                if info.get('symbol') == market_id and int(info.get('positionIdx', -1)) == self.config.position_idx:
                    pos_info = info
                    break

            if not pos_info: logger.debug(f"No position found for symbol {market_id} and posIdx {self.config.position_idx}."); return positions_dict
            ...
            if side == 'buy' and position_details['qty'].copy_abs() >= self.config.position_qty_epsilon:
                positions_dict["long"] = position_details
                logger.debug(f"Found LONG position: Qty={position_details['qty'].normalize()}, Entry={position_details['entry_price'].normalize() if not position_details['entry_price'].is_nan() else 'N/A'}")
            elif side == 'sell' and position_details['qty'].copy_abs() >= self.config.position_qty_epsilon:
                positions_dict["short"] = position_details
                logger.debug(f"Found SHORT position: Qty={position_details['qty'].normalize()}, Entry={position_details['entry_price'].normalize() if not position_details['entry_price'].is_nan() else 'N/A'}")
            elif side != 'none':
                logger.debug(f"Position found but size negligible or NaN. Side: {side}, Qty: {position_details['qty']}")
            else:
                 logger.debug("Position side is 'None'. Considered flat.")

            return positions_dict

        except Exception as e: logger.error(f"Failed to fetch/parse positions: {e}", exc_info=True); return None
...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> Optional[Dict[str, Union[Decimal, bool, int]]]:
        ...
        if df is None or df.empty: logger.error(f"{Fore.RED}No DataFrame for indicators."); return None
        try:
            req_cols = ["open", "high", "low", "close", "volume"] # Volume needed? No, but keep standard columns
            if not all(c in df.columns for c in req_cols): logger.error(f"{Fore.RED}DataFrame missing columns: {[c for c in req_cols if c not in df.columns]}"); return None
            ...
            close = df_calc["close"].astype(float).values; high = df_calc["high"].astype(float).values
            low = df_calc["low"].astype(float).values; index = df_calc.index

            # Check Data Length
            min_required_len = max(self.config.slow_ema_period, self.config.trend_ema_period,
                                   self.config.stoch_period + self.config.stoch_smooth_k + self.config.stoch_smooth_d - 2,
                                   self.config.atr_period, self.config.adx_period * 2) + 1 # ADX needs more data + buffer
            if len(df_calc) < min_required_len: logger.error(f"{Fore.RED}Insufficient data ({len(df_calc)} < {min_required_len}) for indicators."); return None

            # Calculations (float using pandas/numpy)
            close_s = pd.Series(close, index=index); high_s = pd.Series(high, index=index); low_s = pd.Series(low, index=index)
            ...
            # Stochastic
            low_min = low_s.rolling(window=self.config.stoch_period).min(); high_max = high_s.rolling(window=self.config.stoch_period).max()
            stoch_range = high_max - low_min
            stoch_k_raw = np.where(stoch_range > 1e-12, 100 * (close_s - low_min) / stoch_range, 50.0) # Avoid NaN, default 50
            ...
            # ATR (Wilder's)
            prev_close = close_s.shift(1); tr1 = high_s - low_s; tr2 = (high_s - prev_close).abs(); tr3 = (low_s - prev_close).abs()
            ...
            def get_latest_decimal(series: pd.Series, name: str) -> Decimal:
                if series.empty or series.iloc[-1] is None or pd.isna(series.iloc[-1]): return Decimal("NaN")
                try: return Decimal(str(series.iloc[-1]))
                except (InvalidOperation, TypeError): logger.error(f"Failed converting {name} value {series.iloc[-1]} to Decimal."); return Decimal("NaN")
            ...
            if not any(v.is_nan() for v in [k_last, d_last, k_prev, d_prev]):
                 ...
                 prev_oversold = (k_prev <= self.config.stoch_oversold_threshold) or (d_prev <= self.config.stoch_oversold_threshold)
                 prev_overbought = (k_prev >= self.config.stoch_overbought_threshold) or (d_prev >= self.config.stoch_overbought_threshold)
                 if crossed_above and prev_oversold: stoch_kd_bullish = True
                 if crossed_below and prev_overbought: stoch_kd_bearish = True
            ...
            if failed: logger.error(f"{Fore.RED}Critical indicators failed (NaN): {', '.join(failed)}."); return None

            logger.info(f"{Fore.GREEN}Indicator patterns woven successfully.")
            return indicators_out

        except Exception as e: logger.error(f"{Fore.RED}Failed weaving indicator patterns: {e}", exc_info=True); return None
    ...
    def _calculate_adx(self, high_s: pd.Series, low_s: pd.Series, close_s: pd.Series, atr_s: pd.Series, period: int) -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        # Calculate +DM, -DM
        move_up = high_s.diff(); move_down = low_s.diff().mul(-1) # diff is current - previous
...
class SignalGenerator:
    ...
    def generate_signals(self, df_last_candles: pd.DataFrame, indicators: Dict[str, Union[Decimal, bool, int]]) -> Dict[str, Union[bool, str]]:
        ...
        long_signal, short_signal = False, False; signal_reason = "No signal - Conditions not met"
        if not indicators: return {"long": False, "short": False, "reason": "Indicators missing"}
        if df_last_candles is None or len(df_last_candles) < 2: return {"long": False, "short": False, "reason": "Insufficient candle data (<2)"}

        try:
            latest = df_last_candles.iloc[-1]; current_price = safe_decimal(latest["close"])
            prev_close = safe_decimal(df_last_candles.iloc[-2]["close"]) if not pd.isna(df_last_candles.iloc[-2]["close"]) else Decimal("NaN")
            if current_price.is_nan() or current_price <= 0: return {"long": False, "short": False, "reason": "Invalid price (NaN or <= 0)"}

            # Get indicator values safely
            k=indicators.get('stoch_k', Decimal('NaN')); fast_ema=indicators.get('fast_ema', Decimal('NaN'))
            slow_ema=indicators.get('slow_ema', Decimal('NaN')); trend_ema=indicators.get('trend_ema', Decimal('NaN'))
            atr=indicators.get('atr', Decimal('NaN')); kd_bull=indicators.get('stoch_kd_bullish', False)
            kd_bear=indicators.get('stoch_kd_bearish', False); adx=indicators.get('adx', Decimal('NaN'))
            pdi=indicators.get('pdi', Decimal('NaN')); mdi=indicators.get('mdi', Decimal('NaN'))

            req_vals = {'k':k, 'fast_ema':fast_ema, 'slow_ema':slow_ema, 'trend_ema':trend_ema, 'atr':atr, 'adx':adx, 'pdi':pdi, 'mdi':mdi}
            nan_keys = [n for n, v in req_vals.items() if isinstance(v, Decimal) and v.is_nan()]
            if nan_keys: return {"long": False, "short": False, "reason": f"Required indicator(s) NaN: {', '.join(nan_keys)}"}

            # --- Define Conditions ---
            ema_bullish_cross = fast_ema > slow_ema; ema_bearish_cross = fast_ema < slow_ema
            ...
            if self.config.atr_move_filter_multiplier > 0:
                 if atr <= 0: sig_move, atr_reason = False, f"Move Filter Skipped (Invalid ATR: {atr.normalize()})"
                 elif prev_close.is_nan(): sig_move, atr_reason = False, "Move Filter Skipped (Prev Close NaN)"
                 else: move = (current_price - prev_close).copy_abs(); thresh = atr * self.config.atr_move_filter_multiplier; sig_move = move > thresh; atr_reason = f"Move({move.normalize()}) {' > ' if sig_move else ' <= '} {self.config.atr_move_filter_multiplier.normalize()}xATR({thresh.normalize()})"
            # ADX Filter
            adx_trending = adx > self.config.min_adx_level; adx_long_confirm = adx_trending and pdi > mdi; adx_short_confirm = adx_trending and mdi > pdi
            adx_reason = f"ADX({adx:.1f})>{self.config.min_adx_level.normalize()} & " + (f"+DI({pdi:.1f})>-DI({mdi:.1f})" if adx_long_confirm else f"-DI({mdi:.1f})>+DI({pdi:.1f})" if adx_short_confirm else f"DI Invalid") if adx_trending else f"ADX({adx:.1f})<={self.config.min_adx_level.normalize()}"
            ...
            if potential_long:
                ...
                if trend_check:
                    long_signal = True; reason_parts = ["EMA Bull", f"Stoch Long ({k:.1f})", adx_reason, atr_reason]
                    if self.config.trade_only_with_trend: reason_parts.insert(2, "Trend OK")
                    signal_reason = "Long Signal: " + " | ".join(p for p in reason_parts if p)
                elif self.config.trade_only_with_trend: signal_reason = f"Long Blocked (Trend): P({current_price.normalize()}) !> T({trend_ema.normalize()})-{self.config.trend_filter_buffer_percent.normalize()}%"
            elif potential_short:
                trend_check = price_below_trend if self.config.trade_only_with_trend else True
                if trend_check:
                    short_signal = True; reason_parts = ["EMA Bear", f"Stoch Short ({k:.1f})", adx_reason, atr_reason]
                    if self.config.trade_only_with_trend: reason_parts.insert(2, "Trend OK")
                    signal_reason = "Short Signal: " + " | ".join(reason_parts)
                elif self.config.trade_only_with_trend: signal_reason = f"Short Blocked (Trend): P({current_price.normalize()}) !< T({trend_ema.normalize()})+{self.config.trend_filter_buffer_percent.normalize()}%"
            else: # Build reason for no signal
                 if "Blocked" not in signal_reason:
                     parts = []; base_cond = ema_bullish_cross and stoch_long_cond or ema_bearish_cross and stoch_short_cond
                     if not ema_bullish_cross and not ema_bearish_cross: parts.append(f"EMA ({fast_ema:.1f}/{slow_ema:.1f})")
                     elif ema_bullish_cross and not stoch_long_cond: parts.append(f"Stoch !Long ({k:.1f})")
                     elif ema_bearish_cross and not stoch_short_cond: parts.append(f"Stoch !Short ({k:.1f})")
                     if base_cond and not sig_move and self.config.atr_move_filter_multiplier > 0: parts.append(atr_reason)
                     if base_cond and sig_move and not adx_trending: parts.append(f"ADX !Trend ({adx:.1f})")
                     elif base_cond and sig_move and adx_trending and potential_long and not adx_long_confirm: parts.append(f"ADX !Conf Long ({pdi:.1f}/{mdi:.1f})")
                     elif base_cond and sig_move and adx_trending and potential_short and not adx_short_confirm: parts.append(f"ADX !Conf Short ({pdi:.1f}/{mdi:.1f})")
        ...
        except Exception as e: logger.error(f"{Fore.RED}Error generating signals: {e}", exc_info=True); return {"long": False, "short": False, "reason": f"Exception: {e}"}
    ...
    def check_exit_signals(self, position_side: str, indicators: Dict[str, Union[Decimal, bool, int]]) -> Optional[str]:
        ...
        if position_side == "long":
            if ema_bear_cross: exit_reason = "EMA Bearish Cross"
            elif stoch_exit_long: exit_reason = f"Stoch Overbought ({stoch_k:.1f} > {self.config.stoch_overbought_threshold})"
        elif position_side == "short":
            if ema_bull_cross: exit_reason = "EMA Bullish Cross"
            elif stoch_exit_short: exit_reason = f"Stoch Oversold ({stoch_k:.1f} < {self.config.stoch_oversold_threshold})"

        if exit_reason: logger.trade(f"{Fore.YELLOW}Exit Signal vs {position_side.upper()}: {exit_reason}.")
        return exit_reason
...
class OrderManager:
    ...
    def _calculate_trade_parameters(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> Optional[Dict]:
        """Calculates SL price, TP price, and quantity based on risk and ATR."""
        if atr.is_nan() or atr <= 0: logger.error("Invalid ATR for parameter calculation."); return None
        if total_equity.is_nan() or total_equity <= 0: logger.error("Invalid equity for parameter calculation."); return None
        if current_price.is_nan() or current_price <= 0: logger.error("Invalid price for parameter calculation."); return None
        ...
        try:
            ...
            if side == "buy":
                ...
                sl_price = current_price + sl_distance_atr
                tp_price = current_price - tp_distance_atr if tp_distance_atr > 0 else None
            else: logger.error(f"Invalid side '{side}' for calculation."); return None

            # Validate SL price > 0 and TP price > 0 if set
            if sl_price <= 0: logger.error(f"Calculated SL price ({sl_price}) is invalid (<=0)."); return None
            if tp_price is not None and tp_price <= 0: logger.warning(f"Calculated TP price ({tp_price}) is invalid (<=0). Setting TP to None."); tp_price = None

            sl_distance_price = (current_price - sl_price).copy_abs()
            if sl_distance_price <= self.market_info['tick_size']: # Use minimum tick size for check
                logger.error(f"SL distance ({sl_distance_price}) too small or zero. Cannot calculate quantity."); return None
            ...
            tp_price_str = self.exchange_manager.format_price(tp_price) if tp_price is not None else "0" # Use '0' string for API if no TP
            quantity_str = self.exchange_manager.format_amount(quantity, rounding_mode=ROUND_DOWN)

            # Minimum order size check
            min_order_size = safe_decimal(self.market_info.get('limits', {}).get('amount', {}).get('min'), default=Decimal('0'))
            if not min_order_size.is_nan() and safe_decimal(quantity_str) < min_order_size:
                 logger.error(f"Calculated quantity {quantity_str} is below minimum {min_order_size.normalize()}. Cannot place order."); return None
            ...
            tsl_distance_str = self.exchange_manager.format_price(tsl_distance_price) # Format as price difference

            logger.info(f"Trade Params: Side={side.upper()}, Qty={quantity_str}, Entry~={current_price.normalize()}, SL={sl_price_str}, TP={tp_price_str if tp_price else 'None'}, TSLDist~={tsl_distance_str}, RiskAmt={risk_amount.normalize()}, ATR={atr.normalize()}")
            return {
                "qty": safe_decimal(quantity_str),
                "sl_price": safe_decimal(sl_price_str),
                "tp_price": safe_decimal(tp_price_str) if tp_price else None,
                "tsl_distance": safe_decimal(tsl_distance_str) # Store calculated TSL distance
            }
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict]:
        ...
        if safe_decimal(qty_str) <= 0: logger.error(f"Attempted market order with zero/invalid quantity: {qty_str}"); return None
        ...
        try:
            ...
            logger.trade(f"{Fore.GREEN}Market order submitted: ID {order_id}, Side {side.upper()}, Qty {qty_str}")
            ...
            logger.info(f"Waiting {self.config.order_check_delay_seconds}s for order {order_id} to potentially propagate...")
    ...
    def _set_position_protection(self, position_side: str, sl_price: Optional[Decimal] = None, tp_price: Optional[Decimal] = None, is_tsl: bool = False, tsl_distance: Optional[Decimal] = None, tsl_activation_price: Optional[Decimal] = None) -> bool:
        ...
        if is_tsl and tsl_distance and tsl_distance > 0 and tsl_activation_price and tsl_activation_price > 0:
            ...
            params['activePrice'] = self.exchange_manager.format_price(tsl_activation_price) # Price to start trailing
        ...
        try:
            ...
            if not hasattr(self.exchange, path):
                ...
                if not hasattr(self.exchange, path):
                    raise NotImplementedError(f"CCXT method for V5 SetTradingStop not found.")
            ...
            if response and response.get('retCode') == 0:
                ...
                ret_code = response.get('retCode', 'N/A'); ret_msg = response.get('retMsg', 'Unknown Error')
                logger.error(f"{Fore.RED}{action_desc} failed for {position_side.upper()} {symbol}. Code: {ret_code}, Msg: {ret_msg}")
                # Clear tracker on failure to avoid inconsistent state only if it wasn't a clearing action
                if action_desc != "CLEAR SL/TP/TSL (No values provided)" and action_desc != "CLEAR SL/TP":
                     self.protection_tracker[tracker_key] = None
                termux_notify(f"{symbol} Protection FAILED", f"{action_desc[:30]} {position_side.upper()} failed: {ret_msg[:50]}")
                return False
        except Exception as e:
            logger.error(f"{Fore.RED}Error during {action_desc} for {position_side.upper()} {symbol}: {e}", exc_info=True)
    ...
    def place_risked_market_order(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> bool:
        ...
        if not params: logger.error("Failed to calculate trade parameters. Cannot place order."); return False

        qty = params['qty']; sl_price = params['sl_price']; tp_price = params['tp_price']
        position_side = "long" if side == "buy" else "short"

        # --- 1. Execute Market Order ---
        order_info = self._execute_market_order(side, qty)
        if not order_info: logger.error("Market order execution failed. Aborting entry."); return False
        ...
        if positions_after_entry is None:
            logger.error(f"{Fore.RED}Position check FAILED after market order. Manual check required!")
            self._handle_entry_failure(side, qty) # Attempt cleanup
            return False
        if not positions_after_entry.get(position_side):
             logger.error(f"{Fore.RED}Position {position_side.upper()} not found after market order. Manual check required! Potential partial fill or delay.")
             self._handle_entry_failure(side, qty)
             return False

        filled_qty = safe_decimal(positions_after_entry[position_side].get('qty', '0'))
        avg_entry_price = safe_decimal(positions_after_entry[position_side].get('entry_price', 'NaN'))
        logger.info(f"Position {position_side.upper()} confirmed: Qty={filled_qty.normalize()}, AvgEntry={avg_entry_price.normalize() if not avg_entry_price.is_nan() else 'N/A'}")
        ...
        if (filled_qty - qty).copy_abs() > self.config.position_qty_epsilon * Decimal('10'): # Allow slight discrepancy
            logger.warning(f"{Fore.YELLOW}Partial fill detected? Intended: {qty.normalize()}, Filled: {filled_qty.normalize()}. Proceeding with filled amount.")
        ...
        if not set_stops_ok:
            ...
            self.close_position(position_side, filled_qty, reason="CloseDueToFailedStopSet") # Close immediately if stops fail
            return False
    ...
    def manage_trailing_stop(self, position_side: str, entry_price: Decimal, current_price: Decimal, atr: Decimal) -> None:
        """Checks and activates TSL if conditions are met using V5 position TSL."""
        if not self.exchange or not self.market_info: return
        if atr.is_nan() or atr <= 0: logger.debug("Invalid ATR, cannot manage TSL."); return
        if entry_price.is_nan() or entry_price <= 0: logger.debug("Invalid entry price, cannot manage TSL."); return
        if current_price.is_nan() or current_price <= 0: logger.debug("Invalid current price, cannot manage TSL."); return
        tracker_key = position_side.lower()
        ...
        if self.protection_tracker.get(tracker_key) is None:
             logger.debug(f"No active protection tracked for {position_side.upper()}, skipping TSL activation check.")
             return
        ...
        try:
            ...
            if tsl_distance_price < self.market_info['tick_size']:
                 logger.warning(f"Calculated TSL distance ({tsl_distance_price}) too small, using min tick size {self.market_info['tick_size']}.")
            ...
            if position_side == "long":
                ...
                if current_price >= activation_price: should_activate_tsl = True
            elif position_side == "short":
                activation_price = entry_price - activation_distance_atr
                if current_price <= activation_price: should_activate_tsl = True
            ...
            if should_activate_tsl:
                ...
                logger.trade(f"  Entry={entry_price.normalize()}, Current={current_price.normalize()}, Activation Target~={activation_price.normalize()}")
                logger.trade(f"Attempting to activate TSL (Dist ~{tsl_distance_price.normalize()}, ActPx {activation_price.normalize()})...")

                # Activate TSL using _set_position_protection
                # Pass calculated distance and activation price
                if self._set_position_protection(position_side, is_tsl=True, tsl_distance=tsl_distance_price, tsl_activation_price=activation_price):
            ...
            else: logger.debug(f"TSL activation condition not met for {position_side.upper()} (Current: {current_price.normalize()}, Target: ~{activation_price.normalize()})")

        except Exception as e: logger.error(f"Error managing trailing stop for {position_side.upper()}: {e}", exc_info=True)
    ...
    def close_position(self, position_side: str, qty_to_close: Decimal, reason: str = "Signal") -> bool:
        ...
        if not self.exchange or not self.market_info: return False
        if qty_to_close.is_nan() or qty_to_close.copy_abs() < self.config.position_qty_epsilon:
             logger.warning(f"Close requested for zero/negligible qty ({qty_to_close}). Skipping."); return True
        symbol = self.config.symbol
        side_to_close = "sell" if position_side == "long" else "buy"
        tracker_key = position_side.lower()

        logger.trade(f"{Fore.YELLOW}Attempting to close {position_side.upper()} position ({qty_to_close.normalize()} {symbol}) Reason: {reason}...")

        # --- 1. Clear existing SL/TP/TSL from the position ---
        logger.info(f"Clearing any existing position protection (SL/TP/TSL) for {position_side.upper()} before closing...")
        # Call set_trading_stop with zeros/clearing values
        clear_stops_ok = self._set_position_protection(position_side, sl_price=Decimal('0'), tp_price=Decimal('0'), is_tsl=False) # Explicitly clear fixed and TSL
        if not clear_stops_ok:
            logger.warning(f"{Fore.YELLOW}Failed to confirm clearing of position protection. Proceeding with close order anyway...")
        else: logger.info("Position protection cleared successfully.")
        ...
        if not close_order_info:
            logger.error(f"{Fore.RED}Failed to submit closing market order for {position_side.upper()}. MANUAL INTERVENTION REQUIRED!")
            termux_notify(f"{symbol} CLOSE FAILED", f"Market {side_to_close.upper()} order failed!")
            return False

        close_order_id = close_order_info.get('id', 'N/A')
        avg_close_price = safe_decimal(close_order_info.get('average', 'NaN')) # Try to get avg price from order
        ...
        if final_positions is not None:
             if not final_positions.get(position_side) or final_positions.get(position_side, {}).get('qty', Decimal('NaN')).copy_abs() < self.config.position_qty_epsilon:
                  logger.trade(f"{Fore.GREEN}Position {position_side.upper()} confirmed closed.")
                  is_closed = True
             else: # Position still exists
                  lingering_qty = final_positions.get(position_side, {}).get('qty', Decimal('NaN'))
                  logger.error(f"{Fore.RED}Position {position_side.upper()} still shows qty {lingering_qty.normalize()} after close attempt. MANUAL CHECK REQUIRED!")
                  termux_notify(f"{symbol} CLOSE VERIFY FAILED", f"{position_side.upper()} still has position!")
        else: # Failed to fetch final positions
             logger.warning(f"{Fore.YELLOW}Could not verify position closure for {position_side.upper()} (failed fetch). Assuming closed based on order submission.")
             is_closed = True # Assume success if order submitted but verification failed
        ...
        if is_closed and self.config.enable_journaling:
             ...
             if final_positions and not avg_close_price.is_nan(): close_price_for_log = avg_close_price # Use order avg if available
             else: close_price_for_log = Decimal('NaN') # Fallback
             self.log_trade_exit_to_journal(position_side, qty_to_close, close_price_for_log, close_order_id, reason)
    ...
    def _handle_entry_failure(self, failed_entry_side: str, attempted_qty: Decimal):
        ...
        if positions and positions.get(position_side):
            ...
            if current_qty.copy_abs() >= self.config.position_qty_epsilon:
                 logger.error(f"{Fore.RED}Detected lingering {position_side.upper()} position (Qty: {current_qty.normalize()}) after entry failure. Attempting emergency close.")
                 close_success = self.close_position(position_side, current_qty, reason="EmergencyCloseEntryFail")
                 if close_success: logger.info("Emergency close order submitted.")
                 else: logger.critical(f"{Fore.RED+Style.BRIGHT}EMERGENCY CLOSE FAILED for {position_side.upper()}. MANUAL INTERVENTION URGENT!")
            else: logger.info("No significant lingering position found after entry failure.")
        elif positions is None: logger.error("Could not fetch positions during entry failure handling.")
        else: logger.info("No lingering position detected after entry failure.")
    ...
    def log_trade_entry_to_journal(self, side: str, qty: Decimal, avg_price: Decimal, order_id: Optional[str]):
        ...
        try:
            with open(file_path, 'a', newline='', encoding='utf-8') as csvfile:
                ...
                if not file_exists or os.path.getsize(file_path) == 0: writer.writeheader() # Write header only if file is new or empty
                writer.writerow({
        ...
        except IOError as e: logger.error(f"Failed to write entry to journal {file_path}: {e}")
        except Exception as e: logger.error(f"Unexpected error writing entry to journal: {e}", exc_info=True)

    def log_trade_exit_to_journal(self, side: str, qty: Decimal, avg_price: Decimal, order_id: Optional[str], reason: str):
        ...
        except IOError as e: logger.error(f"Failed to write exit to journal {file_path}: {e}")
        except Exception as e: logger.error(f"Unexpected error writing exit to journal: {e}", exc_info=True)
...
class StatusDisplay:
    ...
    def _format_decimal(self, value: Optional[Decimal], precision: int = 2, add_commas: bool = True) -> str:
    ...
    def print_status_panel_with_market_info(self, cycle: int, timestamp: Optional[pd.Timestamp], price: Optional[Decimal], indicators: Optional[Dict], positions: Optional[Dict], equity: Optional[Decimal], signals: Dict, protection_tracker: Dict, market_info: Optional[Dict]):
        ...
        if position_data:
            qty = self._format_decimal(position_data.get('qty'), precision=amount_dp, add_commas=False)
            entry = self._format_decimal(position_data.get('entry_price'), precision=price_dp)
            pnl = self._format_decimal(position_data.get('unrealized_pnl'), precision=4)
            sl = self._format_decimal(position_data.get('stop_loss_price'), precision=price_dp)
            tp = self._format_decimal(position_data.get('take_profit_price'), precision=price_dp)
            active_protection = protection_tracker.get(position_side_str)
            prot_str = f"[magenta]{active_protection}[/]" if active_protection else "[dim]None[/]"
            pos_str = (f"[bold {pos_str_color}]{position_side_str.upper()}:[/] Qty={qty} | Entry={entry} | "
                       f"PnL={pnl} | Prot: {prot_str} (SL:{sl} TP:{tp})")
        ...
        # Print Panel
        console.print(Panel(Text.from_markup(panel_content), title=f"[bold bright_magenta]{title}[/]", border_style="bright_blue"))
...
class TradingBot:
    ...
    def __init__(self):
        logger.info(f"{Fore.MAGENTA+Style.BRIGHT}--- Initializing Pyrmethus v2.4.1 ---")
        ...
        self.status_display = StatusDisplay(self.config) # Display now only needs config
    ...
    def _signal_handler(self, sig, frame):
        ...
        else: logger.warning("Shutdown already in progress.")
    ...
    def run(self):
        ...
        while not self.shutdown_requested:
            ...
            except KeyboardInterrupt: logger.warning("\nCtrl+C detected in main loop. Shutting down."); self.shutdown_requested = True; break
            except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}CRITICAL AUTH ERROR: {e}. Halting."); self.shutdown_requested = True; break # Exit loop to shutdown
            ...
            else: sleep_time = self.config.loop_sleep_seconds # Normal sleep

            if self.shutdown_requested: logger.info("Shutdown requested, breaking main loop."); break
            # Interruptible Sleep
            logger.debug(f"Cycle {cycle_count} finished. Sleeping for {sleep_time} seconds...")
            sleep_end_time = time.time() + sleep_time
            try:
                while time.time() < sleep_end_time and not self.shutdown_requested: time.sleep(0.5)
            except KeyboardInterrupt: logger.warning("\nCtrl+C during sleep."); self.shutdown_requested = True; break
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        start_time = time.time(); cycle_status = "OK" # Track cycle status for logging
        ...
        try: # Extract Price & Timestamp
            ...
            if current_price.is_nan() or current_price <= 0: raise ValueError("Invalid latest close price")
        ...
        if indicators is None: logger.warning(f"{Fore.YELLOW}Indicator calculation failed. Logic may be impacted."); cycle_status = "WARN_INDICATORS"
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
             logger.error(f"{Fore.RED}Failed fetching valid equity. Trading logic skipped."); cycle_status = "FAIL_EQUITY"
             can_run_trade_logic = False
        else: can_run_trade_logic = True # Equity is OK

        positions = self.exchange_manager.get_current_position()
        if positions is None:
            logger.error(f"{Fore.RED}Failed fetching positions. Trading logic skipped."); cycle_status = "FAIL_POSITIONS"
            can_run_trade_logic = False
        ...
        signals: Dict[str, Union[bool, str]] = {"long": False, "short": False, "reason": "Skipped: Initial State"} # Default
        ...
        if can_run_trade_logic:
            ...
            if has_long_pos and indicators: self.order_manager.manage_trailing_stop("long", long_entry, current_price, current_atr)
            elif has_short_pos and indicators: self.order_manager.manage_trailing_stop("short", short_entry, current_price, current_atr)
                 ...
                 if self.order_manager.protection_tracker["long"] or self.order_manager.protection_tracker["short"]:
                      logger.debug("Position flat, clearing protection tracker."); self.order_manager.protection_tracker = {"long": None, "short": None}
            protection_tracker_snapshot = copy.deepcopy(self.order_manager.protection_tracker) # Update snapshot after TSL logic

            # --- Re-fetch position AFTER TSL check (important if TSL hit or stops cleared) ---
            logger.debug("Re-fetching position state after TSL/clear check..."); positions_after_tsl = self.exchange_manager.get_current_position()
            if positions_after_tsl is None:
                logger.error(f"{Fore.RED}Failed re-fetching positions after TSL check. Assuming previous state."); cycle_status="WARN_POS_REFETCH_TSL"
                ...
                final_positions_for_panel = positions
                 ...
                 long_qty=safe_decimal(active_long_pos.get('qty','0')); short_qty=safe_decimal(active_short_pos.get('qty','0'))
                 has_long_pos=long_qty.copy_abs()>=self.config.position_qty_epsilon; has_short_pos=short_qty.copy_abs()>=self.config.position_qty_epsilon
                 is_flat = not has_long_pos and not has_short_pos; logger.debug(f"State After TSL Check: Flat={is_flat}, Long={long_qty.normalize()}, Short={short_qty.normalize()}")
                 final_positions_for_panel = positions_after_tsl # Update panel data
                 if is_flat and (self.order_manager.protection_tracker["long"] or self.order_manager.protection_tracker["short"]): # Clear again if became flat
                      logger.debug("Position became flat after TSL logic, clearing protection tracker."); self.order_manager.protection_tracker = {"long": None, "short": None}; protection_tracker_snapshot = copy.deepcopy(self.order_manager.protection_tracker)

                 # --- 5. Generate Trading Signals (Entry) ---
                 can_gen_signals = indicators is not None and not current_price.is_nan() and len(df) >= 2
                 if can_gen_signals: signals = self.signal_generator.generate_signals(df.iloc[-2:], indicators)
                 else: signals["reason"] = "Skipped Signal Gen: " + ("Indicators missing" if indicators is None else f"Need >=2 candles ({len(df)} found)"); logger.warning(signals['reason'])

                 # --- 6. Check for Exits (Signal-Based) ---
                 exit_triggered = False; exit_side = None
                 if can_gen_signals and not is_flat:
                     ...
                     if exit_reason:
                         ...
                         exit_triggered = self.order_manager.close_position(exit_side, qty_to_close, reason=exit_reason)
                         if not exit_triggered: cycle_status="FAIL_EXIT_CLOSE" # Mark failure if close command failed

                 # --- Re-fetch state AGAIN if an exit was triggered ---
                 if exit_triggered:
                      logger.debug(f"Re-fetching state after signal exit attempt ({exit_side})..."); positions_after_exit = self.exchange_manager.get_current_position()
                      if positions_after_exit is None:
                          logger.error(f"{Fore.RED}Failed re-fetching positions after signal exit."); cycle_status="WARN_POS_REFETCH_EXIT"
                          final_positions_for_panel = positions_after_tsl # Use previous state if re-fetch fails
                      else: # Update live state variables
                          active_long_pos=positions_after_exit.get('long',{}); active_short_pos=positions_after_exit.get('short',{})
                          long_qty=safe_decimal(active_long_pos.get('qty','0')); short_qty=safe_decimal(active_short_pos.get('qty','0'))
                          has_long_pos=long_qty.copy_abs()>=self.config.position_qty_epsilon; has_short_pos=short_qty.copy_abs()>=self.config.position_qty_epsilon
                          is_flat = not has_long_pos and not has_short_pos; logger.debug(f"State After Signal Exit: Flat={is_flat}, Long={long_qty.normalize()}, Short={short_qty.normalize()}")
                          final_positions_for_panel = positions_after_exit # Update panel data
                          if is_flat: logger.debug("Position became flat after signal exit, clearing protection tracker."); self.order_manager.protection_tracker = {"long": None, "short": None}; protection_tracker_snapshot = copy.deepcopy(self.order_manager.protection_tracker)
                 ...
                 if is_flat and can_gen_signals and not current_atr.is_nan() and (signals.get("long") or signals.get("short")):
                     if current_atr <= 0: logger.warning("Cannot enter trade: Invalid ATR (<= 0)")
                     elif total_equity <= 0: logger.warning("Cannot enter trade: Invalid Equity (<= 0)") # Should have been caught earlier but double check
                     else:
                         entry_side = "buy" if signals.get("long") else "sell"
                         logger.info(f"{Fore.GREEN+Style.BRIGHT if entry_side=='buy' else Fore.RED+Style.BRIGHT}{entry_side.upper()} signal! {signals.get('reason', '')}. Attempting entry...")
                         ...
                         else: logger.error(f"{Fore.RED}Entry process failed."); cycle_status = "FAIL_ENTRY"
                         # Re-fetch for panel accuracy after entry attempt
                         logger.debug("Re-fetching state after entry attempt..."); positions_after_entry = self.exchange_manager.get_current_position()
                         if positions_after_entry is not None: final_positions_for_panel = positions_after_entry; protection_tracker_snapshot = copy.deepcopy(self.order_manager.protection_tracker)
                         else: logger.warning("Failed re-fetching positions after entry attempt. Panel may be stale."); cycle_status="WARN_POS_REFETCH_ENTRY"
        ...
        self.status_display.print_status_panel_with_market_info(cycle_count, last_timestamp, current_price, indicators, final_positions_for_panel, total_equity, signals, protection_tracker_snapshot, self.exchange_manager.market_info)

        end_time = time.time(); logger.info(f"{Fore.MAGENTA}--- Cycle {cycle_count} Status: {cycle_status} (Duration: {end_time - start_time:.2f}s) ---")
    ...
    def graceful_shutdown(self) -> None:
        ...
        if not self.exchange_manager or not self.exchange_manager.exchange or not self.exchange_manager.market_info:
             logger.error(f"{Fore.RED}Cannot shutdown cleanly: Exchange/Market Info missing."); termux_notify("Shutdown Warning!", f"{self.config.symbol} Cannot shutdown cleanly."); return

        exchange = self.exchange_manager.exchange
        symbol = self.config.symbol

        try: # --- 1. Cancel All Open Orders ---
            logger.info(f"{Fore.CYAN}Attempting to cancel all open orders for {symbol} using V5 params...");
            try:
                ...
                if isinstance(cancel_resp, list): logger.info(f"Cancelled {len(cancel_resp)} active orders.")
            except Exception as e: logger.error(f"{Fore.RED}Error cancelling active orders: {e}", exc_info=True)

            logger.info("Clearing local protection tracker."); self.order_manager.protection_tracker = {"long": None, "short": None}
        ...
        logger.info("Waiting after order cancellation attempt..."); time.sleep(max(self.config.order_check_delay_seconds, 2))

        try: # --- 2. Close Any Lingering Positions ---
            logger.info(f"{Fore.CYAN}Checking for lingering positions to close..."); closed_count = 0
            positions = self.exchange_manager.get_current_position()
            if positions is not None:
                positions_to_close = {}
                for side, pos_data in positions.items():
                     if pos_data and safe_decimal(pos_data.get('qty', '0')).copy_abs() >= self.config.position_qty_epsilon:
                          positions_to_close[side] = pos_data
                if not positions_to_close: logger.info(f"{Fore.GREEN}No significant positions found requiring closure.")
                else:
                    logger.warning(f"{Fore.YELLOW}Found {len(positions_to_close)} positions requiring closure: {list(positions_to_close.keys())}")
                    for side, pos_data in positions_to_close.items():
                         ...
                         if self.order_manager.close_position(side, qty, reason="GracefulShutdown"):
                             closed_count += 1
                             logger.info(f"{Fore.GREEN}Closure initiated for {side.upper()}.")
                         else: logger.error(f"{Fore.RED}Closure failed for {side.upper()}. MANUAL INTERVENTION REQUIRED.")
                    if closed_count == len(positions_to_close): logger.info(f"{Fore.GREEN}All detected positions closed successfully.")
                    else: logger.warning(f"{Fore.YELLOW}Attempted {len(positions_to_close)} closures, {closed_count} closure orders submitted. MANUAL VERIFICATION REQUIRED.")
            else: logger.error(f"{Fore.RED}Failed fetching positions during shutdown check. MANUAL CHECK REQUIRED.")
    ...
    def _display_startup_info(self):
        ...
        console.print(f"[bold bright_cyan] summoning Pyrmethus [magenta]v2.4.1[/]...")
        console.print(f"[yellow]Trading Symbol: [white]{self.config.symbol}[/] | Interval: [white]{self.config.interval}[/] | Category: [white]{self.config.bybit_v5_category}[/]")
        console.print(f"[yellow]Risk: [white]{self.config.risk_percentage:.3%}[/] | SL Mult: [white]{self.config.sl_atr_multiplier.normalize()}x[/] | TP Mult: [white]{self.config.tp_atr_multiplier.normalize()}x[/]")
        console.print(f"[yellow]TSL Act Mult: [white]{self.config.tsl_activation_atr_multiplier.normalize()}x[/] | TSL %: [white]{self.config.trailing_stop_percent.normalize()}%[/]")
        console.print(f"[yellow]Trend Filter: [white]{'ON' if self.config.trade_only_with_trend else 'OFF'}[/] | ATR Move Filter: [white]{self.config.atr_move_filter_multiplier.normalize()}x[/] | ADX Filter: [white]>{self.config.min_adx_level.normalize()}[/]")
        console.print(f"[yellow]Journaling: [white]{'Enabled' if self.config.enable_journaling else 'Disabled'}[/] ([dim]{self.config.journal_file_path}[/])")
        console.print(f"[dim]Using V5 Position Stops (SLTrig:{self.config.sl_trigger_by}, TSLTrig:{self.config.tsl_trigger_by}, PosIdx:{self.config.position_idx})[/]")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"{Fore.RED+Style.BRIGHT}Critical error during bot execution: {main_exception}", exc_info=True)
