...
def calculate_margin_requirement(
    ...
    logger.debug(
        f"[{func_name}] Calculating margin requirement: {order_side} {format_amount(exchange, symbol, amount)} {symbol} "
    ...
    if amount <= Decimal("0") or price <= Decimal("0") or leverage <= Decimal("0"):
        logger.error(
            f"{Fore.RED}[{func_name}] Invalid input: Amount ({amount}), Price ({price}), or Leverage ({leverage}) must be positive.{Style.RESET_ALL}"
        )
        return None, None
    ...
    try:
        ...
        if not market.get("contract"):
            logger.error(
                f"{Fore.RED}[{func_name}] Cannot calculate margin for non-contract symbol: {symbol}.{Style.RESET_ALL}"
            )
            return None, None
        ...
        logger.debug(
            f"[{func_name}] Estimated Order Value: {format_price(exchange, quote_currency, order_value)} {quote_currency}"
        )
        ...
        logger.debug(
            f"[{func_name}] Base Initial Margin (Value / Leverage): {format_price(exchange, quote_currency, initial_margin_base)} {quote_currency}"
        )
        ...
        logger.debug(
            f"[{func_name}] Estimated Fee (Rate: {fee_rate:.4%}): {format_price(exchange, quote_currency, estimated_fee)} {quote_currency}"
        )
        ...
        logger.info(
            f"[{func_name}] Estimated TOTAL Initial Margin Required (Base IM + Est. Open Fee): "
        ...
        try:
            ...
            mmr_rate_str = (
                ...
                or market.get("info", {}).get("maintainMargin")
                or market.get("info", {}).get("mmr")
                or market.get("info", {}).get("maintenanceMarginRate")
            ...
            if mmr_rate_str:
                ...
                if mmr_rate and mmr_rate >= 0:  # MMR should be non-negative
                    ...
                    logger.debug(
                        f"[{func_name}] Basic Maintenance Margin Estimate (using base MMR {mmr_rate:.4%}): "
        ...
        logger.error(
            f"{Fore.RED}[{func_name}] Unexpected error during margin calculation for {symbol}: {e}{Style.RESET_ALL}",
            exc_info=True,
        )
        return None, None
