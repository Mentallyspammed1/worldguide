...
try:
    ...
    print(
        "Warning: 'colorama' library not found. Logs will not be colored. Install using: pip install colorama"
    )
...
try:
    ...
    if load_dotenv():
        print(
            f"{Fore.CYAN}Successfully loaded environment variables from '.env' file.{Style.RESET_ALL}"
        )
    else:
        print(
            f"{Fore.CYAN}No '.env' file found or it's empty. Relying on system environment variables.{Style.RESET_ALL}"
        )
except ImportError:
    print(
        f"{Fore.YELLOW}Warning: 'python-dotenv' library not found. Relying on system environment variables only. Install using: pip install python-dotenv{Style.RESET_ALL}"
    )
...
try:
    ...
    print(
        f"{Back.RED}{Fore.WHITE}FATAL ERROR: Failed to import local helper module: {e}{Style.RESET_ALL}"
    )
    print(
        "Ensure all required Python files (config.py, utils.py, etc.) are present in the same directory or accessible in PYTHONPATH."
    )
    sys.exit(1)
...
try:
    ...
    main_logger.warning(
        f"{Back.YELLOW}{Fore.BLACK}ENSURING TESTNET MODE IS ACTIVE: {config.TESTNET_MODE}{Style.RESET_ALL}"
    )
    ...
    config.DEFAULT_LEVERAGE = int(
        os.getenv("BYBIT_DEMO_LEVERAGE", 5)
    )  # Lower leverage for safety
    config.ENABLE_SMS_ALERTS = (
        os.getenv("ENABLE_SMS_ALERTS", "False").lower() == "true"
    )  # Keep SMS off by default

    main_logger.info(
        f"Configuration Loaded: Testnet={config.TESTNET_MODE}, Symbol={config.SYMBOL}, Default Leverage={config.DEFAULT_LEVERAGE}, SMS Alerts={config.ENABLE_SMS_ALERTS}"
    )

    # Validate essential credentials
    if not config.API_KEY or not config.API_SECRET:
        main_logger.critical(
            f"{Back.RED}{Fore.WHITE}FATAL: Bybit API Key or Secret not found.{Style.RESET_ALL}"
        )
        main_logger.critical(
            "Please set BYBIT_API_KEY and BYBIT_API_SECRET environment variables or in a '.env' file."
        )
        sys.exit(1)
...
def _get_v5_category(market_info: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    main_logger.warning(
        f"Could not determine V5 category for market type '{market_type}' (Linear: {is_linear}, Inverse: {is_inverse})."
    )
...
try:
    ...
    if not market_info:
        raise BadSymbol(
            f"Market validation failed for symbol {config.SYMBOL}. Ensure it's valid and active on Bybit."
        )
    main_logger.info(
        f"{Fore.GREEN}Market Validation OK for {config.SYMBOL}. Type: {market_info.get('type')}, Contract Type: {market_info.get('contractType', 'N/A')}, Linear/Inverse: {'Linear' if market_info.get('linear') else ('Inverse' if market_info.get('inverse') else 'N/A')}{Style.RESET_ALL}"
    )
    # Determine V5 Category early
    v5_category = _get_v5_category(market_info)
    if not v5_category:
        main_logger.warning(
            f"{Fore.YELLOW}Could not determine V5 API category for {config.SYMBOL}. Some API calls might require manual adjustment.{Style.RESET_ALL}"
        )
    else:
    ...
    if equity is None or available is None:
        ...
        main_logger.warning(
            f"{Fore.YELLOW}Fetch Balance failed after potential retries (returned None). Proceeding cautiously, but balance-dependent actions might fail.{Style.RESET_ALL}"
        )
        # Set defaults to avoid crashing later tests, but log the issue
        equity, available = Decimal("0.0"), Decimal("0.0")
    else:
        main_logger.info(
            f"{Fore.GREEN}Fetch Balance OK: Total Equity={format_price(equity, config.QUOTE_PRECISION)} USDT, Available Balance={format_price(available, config.QUOTE_PRECISION)} USDT{Style.RESET_ALL}"
        )
    ...
    if v5_category in ["linear", "inverse"]:  # Leverage applies to derivatives
        ...
        if not lev_ok:
            ...
            main_logger.warning(
                f"{Fore.YELLOW}Set Leverage to {config.DEFAULT_LEVERAGE}x potentially failed or was unnecessary (e.g., UTA Isolated Mode). Check logs.{Style.RESET_ALL}"
            )
        else:
            main_logger.info(
                f"{Fore.GREEN}Set Leverage request successful (or leverage already set) to {config.DEFAULT_LEVERAGE}x for {config.SYMBOL}.{Style.RESET_ALL}"
            )
    else:
        main_logger.info(
    ...
    ticker = fetch_ticker_validated(
        exchange_instance, config.SYMBOL, config, market_info=market_info
    )
    if not ticker:
        main_logger.warning(
            f"{Fore.YELLOW}Fetch Ticker failed after potential retries.{Style.RESET_ALL}"
        )
    else:
        spread = ticker.get("ask", 0) - ticker.get("bid", 0)
        spread_pct = (
            (spread / ticker["last"] * 100)
            if ticker.get("last") and ticker["last"] > 0
            else 0
        )
        main_logger.info(
            f"{Fore.GREEN}Fetch Ticker OK: Last={ticker.get('last')}, Bid={ticker.get('bid')}, Ask={ticker.get('ask')}, Spread={spread:.{config.PRICE_PRECISION}f} ({spread_pct:.4f}%){Style.RESET_ALL}"
        )
    ...
    if v5_category in ["linear", "inverse"]:
        ...
        position = get_current_position_bybit_v5(
            exchange_instance, config.SYMBOL, config, market_info=market_info
        )
        ...
        if position["side"] == config.POS_NONE:
            ...
            main_logger.info(
                f"{Fore.GREEN}Fetch Position OK: Side={position['side']}, Qty={position['qty']}, Entry Price={position['entry_price']}, Liq Price={position.get('liq_price', 'N/A')}{Style.RESET_ALL}"
            )
        ...
        if position["side"] != config.POS_NONE:
            ...
            risk = fetch_position_risk_bybit_v5(
                exchange_instance, config.SYMBOL, config, market_info=market_info
            )
            if not risk:
                main_logger.warning(
                ...
                imr_pct = (
                    f"{risk['imr'] * 100:.2f}%"
                    if risk.get("imr") is not None
                    else "N/A"
                )
                mmr_pct = (
                    f"{risk['mmr'] * 100:.2f}%"
                    if risk.get("mmr") is not None
                    else "N/A"
                )
                main_logger.info(
                    f"{Fore.GREEN}Fetch Position Risk OK: Liq Price={risk.get('liq_price', 'N/A')}, IMR={imr_pct}, MMR={mmr_pct}, Value={risk.get('position_value', 'N/A')}{Style.RESET_ALL}"
                )
        else:
            main_logger.info(
        ...
        position = {
            ...
            "qty": Decimal(0),
        }  # Define dummy position for later checks
    ...
    if run_advanced_tests:
        ...
        if acc_info:
            ...
            uta_status = acc_info.get(
                "unifiedMarginStatus", "N/A"
            )  # 1: Regular, 2: UTA Pro, 3/4: UTA Gradual
            main_logger.info(
                f"{Fore.GREEN}Fetch Account Info OK: UTA Status={uta_status}, Margin Mode={margin_mode}{Style.RESET_ALL}"
            )
        else:
            main_logger.warning(
                f"{Fore.YELLOW}Fetch Account Info failed.{Style.RESET_ALL}"
            )
        ...
        if ohlcv_df is not None and not ohlcv_df.empty:
            main_logger.info(
                f"{Fore.GREEN}Fetch OHLCV OK: Fetched {len(ohlcv_df)} candles. Last Close: {ohlcv_df['close'].iloc[-1]} @ {ohlcv_df.index[-1]}{Style.RESET_ALL}"
            )
        elif ohlcv_df is not None and ohlcv_df.empty:
            main_logger.info(
                f"{Fore.CYAN}Fetch OHLCV OK: No candles returned for the specified period/symbol.{Style.RESET_ALL}"
            )
        else:
        ...
        if v5_category in ["linear", "inverse"] and market_info.get("type") == "swap":
            ...
            funding_rate_info = fetch_funding_rate(
                exchange_instance, config.SYMBOL, config, market_info=market_info
            )
            if funding_rate_info:
                ...
                dt_object = (
                    time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(timestamp / 1000))
                    if timestamp
                    else "N/A"
                )
                main_logger.info(
                    f"{Fore.GREEN}Fetch Funding Rate OK: Rate={rate:.8f}, Timestamp={dt_object} UTC{Style.RESET_ALL}"
                )
            else:
                main_logger.warning(
                    f"{Fore.YELLOW}Fetch Funding Rate failed.{Style.RESET_ALL}"
                )
        else:
            main_logger.info(
    ...
    if run_order_tests and not config.TESTNET_MODE:
        main_logger.error(
            f"{Back.RED}{Fore.WHITE}CRITICAL SAFETY CHECK FAILED: Order tests enabled but TESTNET_MODE is FALSE.{Style.RESET_ALL}"
        )
        ...
        main_logger.warning(
            "These tests will place and cancel orders. Ensure sufficient testnet funds."
        )
        time.sleep(3)  # Pause for user awareness

        if not v5_category:
            main_logger.error(
                f"{Back.RED}Cannot run order tests: V5 category for {config.SYMBOL} could not be determined. Aborting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        ...
        if initial_pos["side"] != config.POS_NONE:
            main_logger.warning(
                f"Existing {initial_pos['side']} position found (Qty: {initial_pos['qty']}). Attempting market close..."
            )
            ...
            if close_result:
                main_logger.info(
                    f"Market close order placed (ID: ...{format_order_id(close_result.get('id'))}). Waiting for position update..."
                )
                time.sleep(5)  # Wait for closure to likely process
            else:
                main_logger.error(
                    f"{Back.RED}Failed to place market close order for pre-existing position.{Style.RESET_ALL}"
                )
            ...
            if current_pos_check["side"] != config.POS_NONE:
                main_logger.error(
                    f"{Back.RED}Failed to close pre-existing position (still {current_pos_check['side']} {current_pos_check['qty']}). Aborting order tests.{Style.RESET_ALL}"
                )
                sys.exit(1)
            else:
                main_logger.info(
        ...
        if current_ticker and current_ticker.get("bid") and current_ticker["bid"] > 0:
            ...
            limit_qty = safe_decimal_conversion(
                config.MIN_ORDER_QTY_BTC
            )  # Use configured minimum quantity
            if not limit_qty:
                limit_qty = Decimal("0.001")  # Fallback if config missing
            main_logger.info(
                f"Attempting to place LIMIT BUY: Qty={limit_qty}, Price={format_price(limit_buy_price, config.PRICE_PRECISION)}"
            )
            ...
            if limit_order:
                ...
                main_logger.info(
                    f"{Fore.GREEN}Limit Order (PostOnly) placed OK: ID ...{format_order_id(limit_order_id)}, Status: {limit_order.get('status')}{Style.RESET_ALL}"
                )
                ...
                if open_limit_orders:
                    for o in open_limit_orders:
                        if o.get("id") == limit_order_id:
                            main_logger.info(
                                f"{Fore.GREEN}Successfully fetched the specific open limit order: ID ...{format_order_id(o.get('id'))}{Style.RESET_ALL}"
                            )
                            found_order = True
                            break
                if not found_order:
                    main_logger.warning(
                        f"{Fore.YELLOW}Could not fetch the specific open limit order (might have filled instantly, been rejected post-placement, or filter failed).{Style.RESET_ALL}"
                    )
                ...
                if cancel_ok:
                    main_logger.info(
                        f"{Fore.GREEN}Cancel All Orders request executed successfully.{Style.RESET_ALL}"
                    )
                else:
                    main_logger.warning(
                        f"{Fore.YELLOW}Cancel All Orders reported potential issues or no orders to cancel. Check logs.{Style.RESET_ALL}"
                    )
                ...
                if not open_orders_after_cancel:
                    main_logger.info(
                        f"{Fore.GREEN}Verified no open regular orders remain after cancellation.{Style.RESET_ALL}"
                    )
                else:
                    main_logger.warning(
                        f"{Fore.YELLOW}Found {len(open_orders_after_cancel)} open regular orders after cancel call! Order IDs: {[o.get('id') for o in open_orders_after_cancel]}{Style.RESET_ALL}"
                    )

            else:
                main_logger.warning(
                    f"{Fore.YELLOW}Limit Order placement failed or was rejected (e.g., PostOnly failed due to price crossing spread). Check logs.{Style.RESET_ALL}"
                )
        else:
            main_logger.error(
                "Cannot get valid ticker bid price for limit order test. Skipping."
            )

        # --- Test 9a: Place Market Order (Entry) ---
        main_logger.info("\n--- Test 9a. Place Market Order (Entry) ---")
        entry_qty = safe_decimal_conversion(
            config.MIN_ORDER_QTY_BTC
        )  # Use configured minimum
        if not entry_qty:
            entry_qty = Decimal("0.001")  # Fallback
        ...
        if not market_entry_order:
            ...
            main_logger.info(
                f"{Fore.GREEN}Market Entry Order OK: ID ...{format_order_id(entry_order_id)}, Status: {market_entry_order.get('status')}, Avg Fill Price: {market_entry_order.get('average')}{Style.RESET_ALL}"
            )
            ...
            if pos_after_entry["side"] != config.POS_LONG or pos_after_entry[
                ...
                main_logger.error(
                    f"{Back.RED}Position check after market buy failed! Expected LONG ~{entry_qty}, Got: Side={pos_after_entry['side']}, Qty={pos_after_entry['qty']}. Aborting further tests.{Style.RESET_ALL}"
                )
            else:
                main_logger.info(
                    f"{Fore.GREEN}Position Confirmed: LONG {pos_after_entry['qty']} @ Entry ~{pos_after_entry['entry_price']}{Style.RESET_ALL}"
                )
                entry_price_decimal = safe_decimal_conversion(
                    pos_after_entry["entry_price"]
                )
                ...
                if entry_price_decimal and entry_price_decimal > 0:
                    ...
                    main_logger.info(
                        f"Attempting to place NATIVE STOP LOSS (SELL): Qty={sl_qty}, Trigger Price={format_price(sl_price, config.PRICE_PRECISION)}"
                    )
                    ...
                    if sl_order:
                        ...
                        main_logger.info(
                            f"{Fore.GREEN}Native SL Order OK: ID ...{format_order_id(sl_order_id)}, Status: {sl_order.get('status')}, Trigger: {format_price(sl_price, config.PRICE_PRECISION)}{Style.RESET_ALL}"
                        )
                        time.sleep(1)
                        ...
                        if found_sl:
                            ...
                            main_logger.warning(
                                f"{Fore.YELLOW}Could not verify SL order via fetch (StopOrder filter). Might have triggered or API delay.{Style.RESET_ALL}"
                            )
                        ...
                        try:
                            ...
                            main_logger.info(
                                f"Attempting exchange.cancel_order with ID: {sl_order_id}, Symbol: {config.SYMBOL}, Params: {cancel_params}"
                            )
                            cancel_response = exchange_instance.cancel_order(
                                sl_order_id, config.SYMBOL, params=cancel_params
                            )
                            main_logger.info(
                                f"{Fore.GREEN}Specific SL order cancellation request sent. Response: {cancel_response}{Style.RESET_ALL}"
                            )
                            # Add verification step if possible
                            time.sleep(2)
                            open_stops_after_cancel = fetch_open_orders_filtered(
                                exchange_instance,
                                config.SYMBOL,
                                config,
                                order_filter="StopOrder",
                            ...
                            found_sl_after_cancel = (
                                any(
                                    o.get("id") == sl_order_id
                                    for o in open_stops_after_cancel
                                )
                                if open_stops_after_cancel
                                else False
                            )
                            if not found_sl_after_cancel:
                                main_logger.info(
                                    f"{Fore.GREEN}Verified SL order no longer exists after cancellation attempt.{Style.RESET_ALL}"
                                )
                            else:
                                main_logger.warning(
                                    f"{Fore.YELLOW}SL order ...{format_order_id(sl_order_id)} still found after cancellation attempt!{Style.RESET_ALL}"
                                )

                        except OrderNotFound:
                            main_logger.warning(
                                f"{Fore.YELLOW}Specific SL order ID ...{format_order_id(sl_order_id)} not found during cancellation (already triggered, cancelled, or incorrect ID?).{Style.RESET_ALL}"
                            )
                        except (InvalidOrder, ccxt.ExchangeError) as cancel_err:
                            main_logger.error(
                                f"Failed to cancel specific SL order ID ...{format_order_id(sl_order_id)}: {cancel_err}",
                                exc_info=True,
                            )
                        except Exception as generic_cancel_err:
                            main_logger.error(
                                f"Unexpected error cancelling specific SL order: {generic_cancel_err}",
                                exc_info=True,
                            )

                    else:
                        main_logger.warning(
                            f"{Fore.YELLOW}Native Stop Loss Placement Failed. Check logs.{Style.RESET_ALL}"
                        )
                else:
                    main_logger.error(
                        "Could not place Stop Loss: Invalid entry price detected after market buy."
                    )
                ...
                if pos_before_close["side"] == config.POS_NONE:
                    main_logger.info(
                        f"{Fore.CYAN}Position appears already closed before final close attempt. Skipping.{Style.RESET_ALL}"
                    )
                else:
                    main_logger.info(
                        f"Attempting to close {pos_before_close['side']} position of Qty {pos_before_close['qty']} via Market ReduceOnly order."
                    )
                    ...
                    if close_order:
                        main_logger.info(
                            f"{Fore.GREEN}Close Position Order OK: ID ...{format_order_id(close_order.get('id'))}, Status: {close_order.get('status')}{Style.RESET_ALL}"
                        )
                    else:
                        # The function logs details on failure
                        main_logger.warning(
                            f"{Fore.YELLOW}Close Position attempt failed or reported issues. Position might still be open or was already closed.{Style.RESET_ALL}"
                        )
                ...
                if final_pos_check["side"] == config.POS_NONE:
                    ...
                    main_logger.error(
                        f"{Back.RED}{Fore.WHITE}POSITION IS NOT FLAT AFTER TESTS! Side: {final_pos_check['side']}, Qty: {final_pos_check['qty']}{Style.RESET_ALL}"
                    )
                    # Attempt one last forceful cleanup
                    main_logger.warning(
                        "Attempting final market close and order cancellation..."
                    )
                    close_position_reduce_only(
                        exchange_instance,
                        config.SYMBOL,
                        config,
                        position_to_close=final_pos_check,
                        reason="Final Cleanup Close",
                        market_info=market_info,
                    )
                    time.sleep(1)
                    cancel_all_orders(
...
# --- Exception Handling ---
    ...
    main_logger.critical(
        "Ensure keys are correctly set in environment variables or '.env' file and have necessary API permissions (read/trade)."
    )
    sys.exit(1)
    ...
    main_logger.critical(
        f"Could not connect to Bybit or the exchange is unavailable/timed out: {e}"
    )
    ...
    main_logger.critical(
        f"Invalid, inactive, or incorrectly formatted symbol used: '{config.SYMBOL}'. Error: {e}"
    )
    main_logger.critical(
        "Verify the symbol exists on Bybit (Testnet/Mainnet) and matches the required format (e.g., BTC/USDT:USDT for linear swaps)."
    )
    sys.exit(1)
    ...
    main_logger.critical(
        f"Check available balance ({available:.4f} USDT reported earlier) and order costs/margin requirements."
    )
    # Optionally send SMS alert here if configured
    send_sms_alert(
        f"[BybitApp] CRITICAL: Insufficient Funds Error for {config.SYMBOL}", config
    )
    sys.exit(1)
except (
    InvalidOrder,
    OrderNotFound,
    ccxt.ExchangeError,
) as e:  # Catch more specific CCXT errors
    main_logger.critical(
        f"\n{Back.RED}{Fore.WHITE}--- CRITICAL: Exchange Logic Error ---{Style.RESET_ALL}"
    )
    main_logger.critical(
        f"An error occurred related to order placement, cancellation, or position management: {type(e).__name__} - {e}",
        exc_info=True,
    )
    main_logger.critical(
        "This could be due to invalid parameters, rate limits, margin issues, or other exchange rules."
    )
    send_sms_alert(
        f"[BybitApp] CRITICAL: Exchange Logic Error - {type(e).__name__}", config
    )
    sys.exit(1)
except Exception as e:
    ...
    main_logger.critical(
        f"An unexpected error occurred during main script execution: {e}", exc_info=True
    )  # Log traceback
    ...
    if exchange_instance:
        main_logger.debug(
            "Exchange instance cleanup (if necessary - typically handled by CCXT for HTTP)."
        )
