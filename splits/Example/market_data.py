...
try:
    ...
    print("WARNING: pandas library not found. OHLCV data will be returned as lists, not DataFrames. Install: pip install pandas")
    ...
    class DummyDataFrame:
        def __init__(self, data=None, columns=None, index=None): pass
        def is_empty(self): return True # Simplification
        def dropna(self, **kwargs): return self
        def ffill(self, **kwargs): return self
        def sort_index(self, **kwargs): return self
        def iloc(self, *args, **kwargs): return self
        def isnull(self): return self # Needs more sophisticated dummy if used
    ...
    class DummyDateTimeIndex:
        name = None
        def tz_localize(self, *args, **kwargs): return self
    ...
    class DummyPandasModule:
        ...
        def to_datetime(*args, **kwargs):
            ...
            return None
        @staticmethod
        def isnull(val): return val is None
...
def fetch_usdt_balance(exchange: ccxt.bybit, config: Config) -> Tuple[Optional[Decimal], Optional[Decimal]]:
    ...
    try:
            ...
            if unified_account_info:
                ...
                if usdt_coin_info:
                    ...
                    if available_v5 is None:
                        logger.debug(f"[{func_name}] 'availableToWithdraw' not found or invalid ({avail_val_withdraw}), trying 'availableBalance' ({avail_val_balance}).")
                        available_v5 = safe_decimal_conversion(avail_val_balance)

                    if available_v5 is None:
                        logger.warning(f"{Fore.YELLOW}[{func_name}] Found USDT entry in UNIFIED account, but could not parse available balance from V5 fields ('availableToWithdraw', 'availableBalance'): {usdt_coin_info}{Style.RESET_ALL}")
                        available_v5 = Decimal("0.0") # Default to 0 if parsing fails within USDT entry
                else:
                    logger.info(f"[{func_name}] {usdt_symbol} coin data not found within the UNIFIED account details. Assuming 0 available {usdt_symbol}.")
                    available_v5 = Decimal("0.0") # Assume zero if USDT entry is missing

            else:
                # Fallback if UNIFIED account type is not explicitly found
                logger.warning(f"[{func_name}] 'UNIFIED' account type not explicitly found in V5 balance response list ({len(result_list)} accounts found). Attempting fallback using first account.")
                if result_list: # Check again if list is not empty
                     ...
                     if usdt_coin_info:
                         ...
                         if available_v5 is None: available_v5 = Decimal("0.0")
                     else:
                         logger.warning(f"[{func_name}] {usdt_symbol} coin data not found within the fallback account ({account_type_found}). Assuming 0 available {usdt_symbol}.")
                         available_v5 = Decimal("0.0")
                else:
                     logger.error(f"{Fore.RED}[{func_name}] Balance response list is unexpectedly empty after initial check. Cannot determine balance.{Style.RESET_ALL}")
                     # Keep equity_v5 and available_v5 as None to trigger standard CCXT fallback

        else:
            logger.warning(f"[{func_name}] V5 balance response 'result.list' is empty or missing. Attempting standard CCXT fallback.")
        ...
        if final_equity is None or final_available is None:
            logger.info(f"[{func_name}] V5 structure parsing was incomplete (Equity Found: {final_equity is not None}, Available Found: {final_available is not None}). Trying standard CCXT balance keys...")
            ...
            if parsed_from_std:
                ...
                logger.warning(f"[{func_name}] Used standard CCXT balance keys as fallback for missing values.")
                 ...
                 logger.error(f"{Fore.RED}[{func_name}] Failed to parse USDT balance from both V5 structure (Source: {account_type_found}) and Standard CCXT keys. Raw USDT data: {usdt_balance_std}{Style.RESET_ALL}")
                 raise ValueError(f"Failed to parse required USDT balance fields (Equity: {final_equity}, Available: {final_available}) after all attempts.")
        ...
        logger.info(f"{Fore.GREEN}[{func_name}] USDT Balance Fetched (Source: {account_type_found}): "
        ...
        logger.warning(f"{Fore.YELLOW}[{func_name}] Network/Exchange Error fetching balance: {e}. Retrying if possible...{Style.RESET_ALL}")
...
def fetch_ohlcv_paginated(
    ...
    if not isinstance(limit_per_req, int) or limit_per_req <= 0:
         logger.warning(f"[{func_name}] Invalid limit_per_req ({limit_per_req}). Setting to default: {bybit_v5_max_limit}.")
         limit_per_req = bybit_v5_max_limit
    elif limit_per_req > bybit_v5_max_limit:
        logger.warning(f"[{func_name}] Requested limit_per_req ({limit_per_req}) exceeds Bybit V5 max ({bybit_v5_max_limit}). Clamping.")
        limit_per_req = bybit_v5_max_limit
    ...
    try:
        ...
        if not category:
            ...
            logger.warning(f"{Fore.YELLOW}[{func_name}] Could not determine market category for {symbol}. Assuming 'linear' for OHLCV fetch. This might fail for Spot/Inverse markets.{Style.RESET_ALL}")
            category = 'linear' # Bybit default for many operations if unspecified, but risky

        params = {'category': category}

        since_dt_str = pd.to_datetime(since, unit='ms', utc=True).strftime('%Y-%m-%d %H:%M:%S UTC') if since and PANDAS_AVAILABLE else (str(since) if since else 'Most Recent')
        max_total_str = str(max_total_candles) if max_total_candles is not None else 'Unlimited'
        logger.info(f"{Fore.BLUE}[{func_name}] Starting OHLCV fetch for {symbol} ({timeframe}). "
        ...
        if max_total_candles is not None and max_total_candles > 0:
            ...
            if max_requests <= 0: max_requests = 1 # Ensure at least one request if max_total > 0

        # --- Pagination Loop ---
        while True:
            # Check stopping conditions
            if max_total_candles is not None and len(all_candles) >= max_total_candles:
                logger.info(f"[{func_name}] Reached or exceeded max_total_candles limit ({max_total_candles}). Fetch complete.")
                break
            ...
            for attempt in range(max_chunk_retries + 1): # 0 to max_retries
                try:
                    ...
                    candles_chunk = exchange.fetch_ohlcv(symbol, timeframe, since=fetch_since, limit=fetch_limit, params=params)
                    ...
                    if attempt < max_chunk_retries:
                        ...
                        logger.warning(f"{Fore.YELLOW}[{func_name}] API Error chunk #{request_count} (Try {attempt + 1}/{max_chunk_retries + 1}): {type(e).__name__}. Retrying in {retry_delay:.2f}s...{Style.RESET_ALL}")
                        time.sleep(retry_delay)
                    else:
                        logger.error(f"{Fore.RED}[{func_name}] API Error chunk #{request_count} failed after {max_chunk_retries + 1} attempts: {e}. Aborting chunk fetch.{Style.RESET_ALL}")

                except ccxt.ExchangeError as e: # Includes BadSymbol, AuthenticationError etc. that shouldn't be retried usually
                    last_fetch_error = e
                    logger.error(f"{Fore.RED}[{func_name}] Non-retryable ExchangeError on chunk #{request_count}: {e}. Aborting chunk fetch.{Style.RESET_ALL}")
                    break # Don't retry logical exchange errors

                except Exception as e: # Catch unexpected errors during fetch
                    last_fetch_error = e
                    logger.error(f"{Fore.RED}[{func_name}] Unexpected error fetching chunk #{request_count}: {e}{Style.RESET_ALL}", exc_info=True)
                    break # Don't retry unknown errors
            # --- End Internal Retry Loop ---

            # If an error occurred and wasn't resolved by retries
            if last_fetch_error:
                logger.error(f"{Fore.RED}[{func_name}] Failed to fetch chunk #{request_count} after {max_chunk_retries + 1} attempts. Last Error: {last_fetch_error}{Style.RESET_ALL}")
            ...
            if not candles_chunk:
                logger.info(f"[{func_name}] No more candles returned by exchange (Chunk #{request_count}). End of data for this range.")
            ...
            if all_candles and candles_chunk:
                ...
                if first_new_ts <= last_old_ts:
                    ...
                    if filtered_count > 0:
                        logger.debug(f"[{func_name}] Removed {filtered_count} overlapping candles from chunk #{request_count}.")

                    if not candles_chunk:
                        logger.info(f"[{func_name}] Entire chunk #{request_count} was overlap or empty after filtering. Assuming end of useful data.")
                        break # Exit main loop

            # --- Add valid chunk data ---
            num_new_candles = len(candles_chunk)
            logger.debug(f"[{func_name}] Fetched {num_new_candles} new, non-overlapping candles (Chunk #{request_count}). Total collected: {len(all_candles) + num_new_candles}")
            ...
            if num_new_candles < fetch_limit:
                logger.info(f"[{func_name}] Received fewer candles ({num_new_candles}) than requested ({fetch_limit}) in chunk #{request_count}. Assuming end of available data for this range.")
                break # Exit main loop
            ...
            logger.trace(f"[{func_name}] Applying rate limit delay: {delay_seconds:.3f}s") # Use trace level if too verbose
        ...
        return _process_ohlcv_data(all_candles, func_name, symbol, timeframe, max_total_candles)
         ...
         logger.error(f"{Fore.RED}[{func_name}] Exchange error during initial OHLCV setup ({symbol}, {timeframe}): {e}{Style.RESET_ALL}")
         return None
    except Exception as e:
        # Catch-all for unexpected errors during setup or loop logic (not chunk fetching)
        logger.critical(f"{Back.RED}[{func_name}] Unexpected critical error during OHLCV pagination setup/loop: {e}{Style.RESET_ALL}", exc_info=True)
        return None
...
def _process_ohlcv_data(
    ...
    if not candle_list:
        logger.warning(f"{Fore.YELLOW}[{func_name}] No candles were collected for {symbol} ({timeframe}). Returning empty result.{Style.RESET_ALL}")
        if PANDAS_AVAILABLE:
    ...
    if invalid_entries > 0:
        logger.warning(f"{Fore.YELLOW}[{func_name}] Removed {invalid_entries} invalid entries during initial validation.{Style.RESET_ALL}")
    if duplicate_timestamps > 0:
        logger.warning(f"{Fore.YELLOW}[{func_name}] Removed {duplicate_timestamps} duplicate timestamp entries.{Style.RESET_ALL}")

    if not validated_list:
        logger.error(f"{Fore.RED}[{func_name}] No valid candles remaining after initial validation for {symbol} ({timeframe}). Returning empty result.{Style.RESET_ALL}")
        if PANDAS_AVAILABLE: return _get_empty_ohlcv_df() # Return standard empty DF
        else: return []
    ...
    if max_candles is not None and len(validated_list) > max_candles:
        logger.debug(f"[{func_name}] Trimming collected candles from {len(validated_list)} to the latest {max_candles}.")
    ...
    if not PANDAS_AVAILABLE:
        logger.info(f"{Fore.GREEN}[{func_name}] Successfully processed {len(validated_list)} valid candles for {symbol} ({timeframe}). Returning as list.{Style.RESET_ALL}")
    ...
    try:
        ...
        if df.empty:
             logger.error(f"{Fore.RED}[{func_name}] DataFrame became empty after timestamp conversion/dropna for {symbol}.{Style.RESET_ALL}")
        ...
        if total_nans > 0:
            logger.warning(f"{Fore.YELLOW}[{func_name}] Found {total_nans} NaN values in OHLCV columns after conversion. Attempting forward fill... (Counts: {nan_counts[nan_counts > 0].to_dict()}){Style.RESET_ALL}")
            ...
            if rows_dropped > 0:
                 logger.warning(f"{Fore.YELLOW}[{func_name}] Dropped {rows_dropped} rows with persistent NaNs (likely at the beginning).{Style.RESET_ALL}")

            # Final check if df became empty after NaN handling
            if df.empty:
                logger.error(f"{Fore.RED}[{func_name}] DataFrame became empty after NaN handling for {symbol}.{Style.RESET_ALL}")
                return _get_empty_ohlcv_df()
        ...
        if df.empty: # Check again after potential trimming/dropna
            logger.error(f"{Fore.RED}[{func_name}] Processed DataFrame is empty after all cleaning steps for {symbol} ({timeframe}).{Style.RESET_ALL}")
            return _get_empty_ohlcv_df()

        logger.info(f"{Fore.GREEN}[{func_name}] Successfully processed {len(df)} valid candles into DataFrame for {symbol} ({timeframe}).{Style.RESET_ALL}")
        return df
...
def _get_empty_ohlcv_df() -> pd.DataFrame:
    ...
    if not PANDAS_AVAILABLE: return [] # Should not happen if called from pandas block
...
def fetch_funding_rate(exchange: ccxt.bybit, symbol: str, config: Config) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not category or category not in ['linear', 'inverse']:
            ...
            logger.error(f"{Fore.RED}[{func_name}] Invalid or unsupported category '{category}' determined for funding rate fetch ({symbol}). Expected 'linear' or 'inverse'.{Style.RESET_ALL}")
        ...
        if not funding_rate_info or not isinstance(funding_rate_info, dict):
             logger.error(f"{Fore.RED}[{func_name}] Received invalid or empty funding rate response for {symbol}.{Style.RESET_ALL}")
             return None
        ...
        next_ts_val = funding_rate_info.get(next_ts_key) or funding_rate_info.get('nextFundingTime') # Check alternative if needed
        ...
        if next_ts_val and isinstance(next_ts_val, (int, float)) and next_ts_val > 0:
            try:
                ...
                if PANDAS_AVAILABLE:
                    ...
                    processed_fr['nextFundingDatetime'] = next_dt.strftime('%Y-%m-%d %H:%M:%S %Z') # e.g., "2023-10-27 16:00:00 UTC"
                else:
                ...
                logger.warning(f"[{func_name}] Could not format next funding datetime from timestamp {next_ts_val}: {dt_err}")
                processed_fr['nextFundingDatetime'] = "Error formatting"
        elif next_ts_val:
             logger.warning(f"[{func_name}] Found next funding time key ('{next_ts_key}'), but value is invalid: {next_ts_val}")
        ...
        if errors:
            ...
            logger.error(f"{Fore.RED}[{func_name}] Failed to process funding rate response for {symbol}. Errors: {'; '.join(errors)}. Raw: {funding_rate_info}{Style.RESET_ALL}")
            return None
        ...
        logger.info(f"{Fore.GREEN}[{func_name}] Funding Rate Fetched {symbol}: Rate = {rate_str}, Next = {next_dt_str}{Style.RESET_ALL}")
        ...
        logger.warning(f"{Fore.YELLOW}[{func_name}] API Error fetching funding rate for {symbol}: {e}. Retrying if possible...{Style.RESET_ALL}")
        raise # Re-raise for retry decorator
    except Exception as e:
...
def fetch_l2_order_book_validated(
    ...
    try:
        ...
        if category:
            ...
            logger.warning(f"[{func_name}] Cannot determine category for {symbol}. Fetching OB without category param, max limit assumed {default_max_limit}.")
        ...
        if effective_limit > max_limit:
            logger.warning(f"[{func_name}] Requested limit {effective_limit} exceeds max {max_limit} for category '{category or 'Unknown'}'. Clamping to {max_limit}.")
        ...
        if not isinstance(order_book, dict) or 'bids' not in order_book or 'asks' not in order_book:
            logger.error(f"{Fore.RED}[{func_name}] Invalid order book structure received: Keys missing or not a dict.{Style.RESET_ALL}")
        ...
        if not isinstance(order_book['bids'], list) or not isinstance(order_book['asks'], list):
             logger.error(f"{Fore.RED}[{func_name}] Bids or asks are not lists: bids_type={type(order_book['bids'])}, asks_type={type(order_book['asks'])}{Style.RESET_ALL}")
        ...
        for i, entry in enumerate(order_book.get('bids', [])):
            ...
            if price is None or amount is None or price <= Decimal("0") or amount < Decimal("0"):
                conversion_errors += 1
                logger.debug(f"[{func_name}] Skipping invalid bid values at index {i}: Price={entry[0]}, Amount={entry[1]}")
                continue
        ...
        for i, entry in enumerate(order_book.get('asks', [])):
            ...
            if price is None or amount is None or price <= Decimal("0") or amount < Decimal("0"):
                conversion_errors += 1
                logger.debug(f"[{func_name}] Skipping invalid ask values at index {i}: Price={entry[0]}, Amount={entry[1]}")
                continue
        ...
        if validated_bids and validated_asks:
            ...
            if best_bid >= best_ask:
                ...
                logger.error(f"{Back.RED}[{func_name}] CRITICAL: Order book is CROSSED for {symbol}! Best Bid ({best_bid}) >= Best Ask ({best_ask}). Data is unreliable.{Style.RESET_ALL}")
        ...
        if not validated_bids and not validated_asks and (order_book['bids'] or order_book['asks']):
             logger.warning(f"{Fore.YELLOW}[{func_name}] All bid/ask entries failed validation for {symbol}. Returning empty book.{Style.RESET_ALL}")
             # Fall through to return empty lists

        logger.info(f"{Fore.GREEN}[{func_name}] Successfully fetched and validated L2 Order Book for {symbol}. Bids: {len(validated_bids)}, Asks: {len(validated_asks)}{Style.RESET_ALL}")
        ...
        logger.log(log_level, f"{Fore.YELLOW if log_level == logging.WARNING else Fore.RED}[{func_name}] API/Validation Error fetching L2 OB for {symbol}: {e}. Retrying if possible...{Style.RESET_ALL}")
        raise # Re-raise for retry decorator
    except Exception as e:
...
def fetch_ticker_validated(
    ...
    try:
        ...
        if not ticker or not isinstance(ticker, dict):
            logger.error(f"{Fore.RED}[{func_name}] Received invalid or empty ticker response for {symbol}.{Style.RESET_ALL}")
        ...
        if timestamp_ms is None or not isinstance(timestamp_ms, (int, float)) or timestamp_ms <= 0:
             logger.warning(f"{Fore.YELLOW}[{func_name}] Ticker for {symbol} has missing or invalid timestamp: {timestamp_ms}. Proceeding cautiously.{Style.RESET_ALL}")
        ...
        numeric_fields = {
            'bidVolume': 'bidVolume', 'askVolume': 'askVolume',
            'baseVolume': 'baseVolume', 'quoteVolume': 'quoteVolume',
            'high': 'high', 'low': 'low', 'open': 'open',
            'change': 'change', 'percentage': 'percentage', 'average': 'average'
        }
        ...
        logger.info(f"{Fore.GREEN}[{func_name}] Ticker validation OK for {symbol}: Last={format_price(exchange, symbol, last_price)}, "
        ...
        log_level = logging.ERROR if isinstance(e, ValueError) else logging.WARNING
        logger.log(log_level, f"{Fore.YELLOW if log_level == logging.WARNING else Fore.RED}[{func_name}] Failed to fetch or validate ticker for {symbol}: {e}. Retrying if possible...{Style.RESET_ALL}")
        raise # Re-raise for retry decorator
    except Exception as e:
...
def fetch_recent_trades(
    ...
    filter_log = f"(MinSize: {format_amount(exchange, symbol, min_size_filter) if min_size_filter else 'None'})" if min_size_filter else ""
    ...
    try:
        ...
        # --- Fetch Trades ---
        logger.debug(f"[{func_name}] Calling fetch_trades with symbol='{symbol}', limit={effective_limit}, params={params}")
        ...
        if trades_raw is None:
            ...
            logger.error(f"{Fore.RED}[{func_name}] fetch_trades returned None, indicating a potential issue with the request or CCXT implementation.{Style.RESET_ALL}")
        ...
        required_keys = ['id', 'timestamp', 'datetime', 'symbol', 'side', 'price', 'amount'] # Essential keys from CCXT structure
        ...
        for i, trade in enumerate(trades_raw):
            ...
            if not all(key in trade and trade[key] is not None for key in required_keys):
                ...
                conversion_errors += 1
                logger.debug(f"[{func_name}] Skipping trade with missing essential keys ({missing}) at index {i}: {trade}")
                continue
            ...
            try:
                ...
                if price is None or amount is None or price <= Decimal("0") or amount <= Decimal("0"):
                    conversion_errors += 1
                    logger.debug(f"[{func_name}] Skipping trade with invalid price/amount at index {i}: Price={trade['price']}, Amount={trade['amount']}")
                    continue
                ...
                if cost is None or abs(cost - calculated_cost) > (calculated_cost * Decimal("0.001")):
                    if cost is not None:
                         logger.trace(f"[{func_name}] Trade cost mismatch (Provided: {cost}, Calculated: {calculated_cost}) for trade {trade_id}. Using calculated cost.")
                ...
                conversion_errors += 1
                logger.warning(f"{Fore.YELLOW}[{func_name}] Error processing individual trade at index {i}: {proc_err}. Data: {trade}{Style.RESET_ALL}")
            except Exception as E: # Catch any other unexpected error during processing
                conversion_errors += 1
                logger.error(f"{Fore.RED}[{func_name}] Unexpected error processing trade at index {i}: {E}. Data: {trade}{Style.RESET_ALL}", exc_info=True)
        ...
        if filtered_out_count > 0:
            logger.info(f"[{func_name}] Filtered out {filtered_out_count} trades smaller than {min_size_filter} for {symbol}.")
        ...
        logger.info(f"{Fore.GREEN}[{func_name}] Fetched and processed {len(processed_trades)} valid trades for {symbol} {filter_log} (Limit: {effective_limit}).{Style.RESET_ALL}")
        ...
        logger.warning(f"{Fore.YELLOW}[{func_name}] API Error fetching trades for {symbol}: {e}. Retrying if possible...{Style.RESET_ALL}")
        raise # Re-raise for retry decorator
    except Exception as e:
