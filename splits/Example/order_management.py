...
from utils import (retry_api_call, _get_v5_category, safe_decimal_conversion,
                   format_price, format_amount, format_order_id, send_sms_alert)
...
def place_market_order_slippage_check(
    ...
    logger.info(f"{Fore.BLUE}{log_prefix} Init for {format_amount(exchange, symbol, amount)} {symbol}. "
    ...
    try:
        ...
        if not category:
             logger.error(f"{Fore.RED}[{func_name}] Cannot determine market category for {symbol}. Aborting market order.{Style.RESET_ALL}")
             return None
        ...
        if ob_data and ob_data.get('bids') and ob_data.get('asks'):
            ...
            if spread > effective_max_slippage:
                ...
                send_sms_alert(f"[{market_base}] ORDER ABORT ({side.upper()} {action}): High Slippage {spread:.4%}", config)
                return None
        else:
        ...
        if client_order_id:
            ...
            if len(valid_coid) < len(client_order_id):
                 logger.warning(f"[{func_name}] Client Order ID '{client_order_id}' truncated to 36 chars: '{valid_coid}'")
        ...
        logger.info(f"{Fore.GREEN}{log_prefix} Market order submitted successfully. "
...
def cancel_all_orders(exchange: ccxt.bybit, symbol: str, config: Config, reason: str = "Cleanup") -> bool:
    ...
    try:
        ...
        if not open_orders:
            logger.info(f"{Fore.CYAN}{log_prefix} No open regular orders found for {symbol} to cancel.{Style.RESET_ALL}")
        ...
        logger.warning(f"{Fore.YELLOW}{log_prefix} Found {len(open_orders)} open regular order(s) for {symbol}. Attempting cancellation...{Style.RESET_ALL}")

        success_count = 0
        fail_count = 0
        # Calculate a small delay based on rate limit to avoid hammering
        cancel_delay = max(0.05, 1.0 / (exchange.rateLimit if exchange.rateLimit and exchange.rateLimit > 0 else 20)) # e.g., 50ms
        ...
        for order in open_orders:
            ...
            try:
                ...
                logger.warning(f"{Fore.YELLOW}{log_prefix} Order {order_info_log} already cancelled or filled (Not Found). Considered OK.{Style.RESET_ALL}")
                ...
                logger.warning(f"{Fore.YELLOW}{log_prefix} Network/RateLimit error cancelling {order_info_log}: {e_cancel}. Loop continues...{Style.RESET_ALL}")
                fail_count += 1 # Count as failure for this attempt
            except ccxt.ExchangeError as e_cancel:
                 # Log persistent exchange errors
                 logger.error(f"{Fore.RED}{log_prefix} FAILED to cancel order {order_info_log}: {e_cancel}{Style.RESET_ALL}")
                 fail_count += 1
            except Exception as e_cancel:
                logger.error(f"{Fore.RED}{log_prefix} Unexpected error cancelling order {order_info_log}: {e_cancel}{Style.RESET_ALL}", exc_info=True)
                fail_count += 1
        ...
        if fail_count > 0:
            ...
            # Use logger.info for success
            logger.info(f"{Fore.GREEN}[{func_name}] {log_prefix}: Successfully cancelled or confirmed gone "
        ...
        logger.error(f"{Fore.RED}[{func_name}] {log_prefix}: API error during 'cancel all' setup/fetch for {symbol}: {e}{Style.RESET_ALL}")
...
def place_limit_order_tif(
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category:
            logger.error(f"{Fore.RED}[{func_name}] Cannot determine market category for {symbol}. Aborting limit order.{Style.RESET_ALL}")
            return None
        ...
        logger.info(f"{Fore.CYAN}{log_prefix} Placing -> Amount:{amount_float}, Price:{price_float}, Params:{params}{Style.RESET_ALL}")
        ...
        # Use logger.info for success
        logger.info(f"{Fore.GREEN}{log_prefix} Limit order placed successfully. ID:...{format_order_id(order_id)}, "
        ...
        return order
         ...
         if params.get('postOnly'):
             ...
             logger.error(f"{Fore.RED}{log_prefix} Unexpected OrderImmediatelyFillable without PostOnly flag: {e}{Style.RESET_ALL}")
        ...
        logger.critical(f"{Back.RED}[{func_name}] Unexpected critical error placing limit order: {e}{Style.RESET_ALL}", exc_info=True)
...
def place_native_stop_loss(
    ...
    logger.info(f"{Fore.CYAN}{log_prefix} Init: {format_amount(exchange, symbol, amount)} {symbol}, Trigger @ {format_price(exchange, symbol, stop_price)} "
    ...
    try:
        ...
        if not category or category not in ['linear', 'inverse']:
            logger.error(f"{Fore.RED}[{func_name}] Cannot place stop loss for non-contract symbol: {symbol} (Category: {category}).{Style.RESET_ALL}")
            return None
        ...
        logger.warning(f"{bg}{fg}{Style.BRIGHT}{log_prefix} Placing NATIVE Stop Loss (Market exec) -> "
        ...
        # Use logger.info for success
        logger.info(f"{Fore.GREEN}{log_prefix} Native SL trigger order placed successfully. "
                       f"ID:...{format_order_id(order_id)}, ClientOID:{client_oid_resp}, Status:{status}, "
        ...
        logger.critical(f"{Back.RED}[{func_name}] Unexpected critical error placing native SL: {e}{Style.RESET_ALL}", exc_info=True)
...
def fetch_open_orders_filtered(
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category:
            logger.error(f"{Fore.RED}[{func_name}] Cannot determine market category for {symbol}. Cannot fetch orders.{Style.RESET_ALL}")
            return None
        ...
        if order_type:
            ...
            def check_type(order: Dict) -> bool:
                ...
                has_trigger_price = order.get('stopPrice') or \
                                    order.get('info', {}).get('stopLoss') or \
                                    order.get('info', {}).get('takeProfit') or \
                                    order.get('info', {}).get('triggerPrice')
                ...
                if ('stop' in norm_type_filter or 'takeprofit' in norm_type_filter) and has_trigger_price:
                    ...
                    if 'market' in norm_type_filter and o_type == 'market': return True
                    # If filter is 'stoplimit', check if order type is 'limit' and has trigger
                    if 'limit' in norm_type_filter and o_type == 'limit': return True
                    # If filter is just 'stop' or 'takeprofit', match any order with a trigger
                    if norm_type_filter in ['stop', 'takeprofit']: return True

                return False

            filtered_orders = [o for o in filtered_orders if check_type(o)]
            logger.debug(f"[{func_name}] Filtered by type='{order_type}'. Count: {count_before_type_filter} -> {len(filtered_orders)}.")
...
def place_native_trailing_stop(
    ...
    if isinstance(trailing_offset, str) and trailing_offset.endswith('%'):
        try:
            ...
            params['trailingStop'] = str(percent_val.quantize(Decimal("0.01"))) # Format to 2 decimal places for percentage string
        ...
        try:
            ...
            raise ValueError(f"{log_prefix} Cannot format trailing offset {trailing_offset} using market precision: {fmt_e}") from fmt_e
    else:
        raise ValueError(f"{log_prefix} Invalid trailing_offset type: {type(trailing_offset)}. Must be Decimal (absolute) or str ('x.y%').")

    # --- Log Initialization ---
    act_px_log = format_price(exchange, symbol, activation_price) if activation_price else 'Immediate'
    logger.info(f"{Fore.CYAN}{log_prefix} Init: {format_amount(exchange, symbol, amount)} {symbol}, Trail:{trail_log_str}, "
    ...
    try:
        ...
        category = _get_v5_category(market)
        if not category or category not in ['linear', 'inverse']:
            raise ValueError(f"{log_prefix} Cannot place trailing stop for non-contract symbol: {symbol} (Category: {category}).")
        ...
        # --- Place Order ---
        bg = Back.YELLOW; fg = Fore.BLACK
        logger.warning(f"{bg}{fg}{Style.BRIGHT}{log_prefix} Placing NATIVE TSL (Market exec) -> "
        ...
        # Use logger.info for success
        logger.info(f"{Fore.GREEN}{log_prefix} Native TSL trigger order placed successfully. "
                       f"ID:...{format_order_id(order_id)}, ClientOID:{client_oid_resp}, Status:{status}, "
    ...
    except (ccxt.InsufficientFunds, ccxt.InvalidOrder, ccxt.ExchangeError, ccxt.NetworkError, ccxt.BadSymbol, ValueError) as e:
        logger.error(f"{Fore.RED}{log_prefix} API/Input Error placing native TSL: {type(e).__name__} - {e}{Style.RESET_ALL}")
        return None # Do not raise, TSL placement usually not retried
    # Catch unexpected errors
    except Exception as e:
        logger.critical(f"{Back.RED}[{func_name}] Unexpected critical error placing native TSL: {e}{Style.RESET_ALL}", exc_info=True)
...
def update_limit_order(
    ...
    logger.info(f"{Fore.CYAN}{log_prefix} Init update for {symbol}: NewAmount:{amount_log}, NewPrice:{price_log}, NewCoid:{coid_log}{Style.RESET_ALL}")

    try:
        # Check if exchange supports editing orders
        if not exchange.has.get('editOrder'):
            logger.error(f"{Fore.RED}{log_prefix} Exchange '{exchange.id}' via CCXT does not support 'editOrder'. Cannot update.{Style.RESET_ALL}")
            return None
        ...
        if order_type != 'limit':
            ...
            raise ccxt.InvalidOrder(f"{log_prefix} Cannot update order. Current type is '{order_type}' (must be 'limit').")
        ...
        if not allow_partial_fill_update and filled_qty > config.POSITION_QTY_EPSILON:
            logger.warning(f"{Fore.YELLOW}[{func_name}] Update aborted: Order ...{format_order_id(order_id)} is partially filled "
        ...
        logger.info(f"{Fore.CYAN}[{func_name}] Submitting update via edit_order -> "
        ...
        if updated_order:
            ...
            # Use logger.info for success
            logger.info(f"{Fore.GREEN}[{func_name}] Update successful. New/Current ID:...{format_order_id(new_id)}, "
            ...
            logger.warning(f"{Fore.YELLOW}[{func_name}] edit_order call succeeded but returned no data. Check order status manually.{Style.RESET_ALL}")
    ...
    # Catch specific, expected errors during update
        ...
        logger.error(f"{Fore.RED}[{func_name}] Failed to update order ...{format_order_id(order_id)}: {type(e).__name__} - {e}{Style.RESET_ALL}")
        return None # Return None on failure, don't raise as edits not retried
    # Catch unexpected errors
    except Exception as e:
        logger.critical(f"{Back.RED}[{func_name}] Unexpected critical error updating order ...{format_order_id(order_id)}: {e}{Style.RESET_ALL}", exc_info=True)
        return None
