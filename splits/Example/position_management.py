...
from order_management import place_market_order_slippage_check # Or a simpler market order function if preferred
...
def get_current_position_bybit_v5(exchange: ccxt.bybit, symbol: str, config: Config) -> Dict[str, Any]:
    ...
    try:
        ...
        if not category or category not in ['linear', 'inverse']:
            logger.error(f"{Fore.RED}[{func_name}] Cannot fetch position for non-contract symbol: {symbol} (Category: {category}).{Style.RESET_ALL}")
        ...
        if not hasattr(exchange, 'fetch_positions') or not exchange.has.get('fetchPositions'):
            logger.error(f"{Fore.RED}[{func_name}] Exchange object does not support 'fetchPositions'. Cannot get position.{Style.RESET_ALL}")
            return default_position
        ...
        if active_position_data:
            try:
                ...
                position_side = config.POS_LONG if pos_side_str == 'Buy' else (config.POS_SHORT if pos_side_str == 'Sell' else config.POS_NONE)
                ...
                if position_side == config.POS_NONE or quantity <= config.POSITION_QTY_EPSILON:
                    logger.info(f"[{func_name}] Parsed position for {symbol} has negligible size or side 'None'. Treating as flat.")
                    return default_position
                ...
                logger.warning(f"{Fore.YELLOW}[{func_name}] Error parsing details from active position data: {parse_err}. Data: {str(active_position_data)[:300]}{Style.RESET_ALL}")
...
def close_position_reduce_only(
    ...
    if position_to_close and isinstance(position_to_close, dict) and 'side' in position_to_close and 'qty' in position_to_close:
        ...
        try:
            ...
            logger.error(f"{Fore.RED}{log_prefix} Failed to fetch current position state for {symbol}: {fetch_err}. Aborting close.{Style.RESET_ALL}")
            return None
    ...
    if live_side == config.POS_NONE or live_qty <= config.POSITION_QTY_EPSILON:
        logger.warning(f"{Fore.YELLOW}[{func_name}] No active position found or quantity is zero for {symbol}. Nothing to close.{Style.RESET_ALL}")
    ...
    if live_side == config.POS_LONG:
        ...
        logger.error(f"{Fore.RED}{log_prefix} Invalid live position side '{live_side}' detected. Aborting close.{Style.RESET_ALL}")
        return None
    ...
    try:
        ...
        logger.warning(f"{Fore.YELLOW}{log_prefix} Attempting to close {live_side} position of {format_amount(exchange, symbol, live_qty)} {symbol} "
        ...
        logger.warning(f"{bg}{fg}{Style.BRIGHT}[{func_name}] Executing CLOSE {live_side} ({reason}): "
        ...
        logger.info(f"{Fore.GREEN}{Style.BRIGHT}[{func_name}] Close Order ({reason}) submitted for {symbol}. "
    ...
    # 6. Handle Specific Errors Gracefully
        ...
        already_closed_indicators = ["110025", "110045", "30086", "position is closed", "order would not reduce", "position size is zero"]
        if any(indicator in error_str for indicator in already_closed_indicators):
            logger.warning(f"{Fore.YELLOW}[{func_name}] Close Order ({reason}) for {symbol}: Exchange indicates position already closed or order would not reduce: {e}. Assuming closed.{Style.RESET_ALL}")
        ...
        logger.error(f"{Fore.RED}[{func_name}] Close Order Network/Setup Error ({reason}) for {symbol}: {e}{Style.RESET_ALL}")
        raise e # Re-raise for potential retry
    except Exception as e:
        # Catch-all for unexpected issues
        logger.critical(f"{Back.RED}[{func_name}] Close Order Unexpected Error ({reason}) for {symbol}: {e}{Style.RESET_ALL}", exc_info=True)
...
def fetch_position_risk_bybit_v5(exchange: ccxt.bybit, symbol: str, config: Config) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not category or category not in ['linear', 'inverse']:
            logger.error(f"{Fore.RED}[{func_name}] Cannot fetch position risk for non-contract symbol: {symbol} (Category: {category}).{Style.RESET_ALL}")
            return None
        ...
        if exchange.has.get('fetchPositionsRisk'):
            try:
                ...
                logger.warning(f"[{func_name}] fetch_positions_risk failed ({type(e).__name__}: {e}). Falling back to fetch_positions.")
             ...
             if exchange.has.get('fetchPositions'):
                 ...
                 try:
                     ...
                     fetch_method_used = "fetchPositions (Fallback)"
                      ...
                      logger.error(f"[{func_name}] Fallback fetch_positions also failed ({type(e).__name__}: {e}). Cannot get position data.")
                      position_data = None # Ensure it remains None
             else:
                 logger.error(f"{Fore.RED}[{func_name}] Neither fetchPositionsRisk nor fetchPositions are available. Cannot get position risk.{Style.RESET_ALL}")
                 return None # Cannot proceed

        if position_data is None:
            # This means both fetch attempts (if applicable) failed without raising exceptions handled by retry
            logger.error(f"{Fore.RED}[{func_name}] Failed to fetch position data using available methods ({fetch_method_used}).{Style.RESET_ALL}")
            return None
        ...
        for pos in position_data:
            ...
            if pos_symbol == market_id and pos_v5_side != 'None' and pos_idx == 0:
                ...
                if pos_size is not None and abs(pos_size) > config.POSITION_QTY_EPSILON:
                    active_pos_risk = pos
                    logger.debug(f"[{func_name}] Found active One-Way position risk data for {symbol} using {fetch_method_used}.")
                    break
        ...
        try:
            ...
            # Margins
            initial_margin = safe_decimal_conversion(active_pos_risk.get('initialMargin', info.get('positionIM'))) # positionIM in V5
            maint_margin = safe_decimal_conversion(active_pos_risk.get('maintenanceMargin', info.get('positionMM'))) # positionMM in V5
            # PNL
            ...
            mmr = safe_decimal_conversion(active_pos_risk.get('maintenanceMarginPercentage', info.get('mmr'))) # mmr in V5
            # Other info
            pos_value = safe_decimal_conversion(active_pos_risk.get('contractsValue', info.get('positionValue'))) # positionValue in V5
            ...
            if position_side == config.POS_NONE or quantity <= config.POSITION_QTY_EPSILON:
                logger.info(f"[{func_name}] Parsed position risk for {symbol} resulted in negligible size or side 'None'.")
                return None
            ...
            logger.info(f"  Initial Margin (IM): {format_price(exchange, quote_curr, initial_margin)} {quote_curr} (IMR: {imr:.4% if imr else 'N/A'})")
            logger.info(f"  Maint. Margin (MM): {format_price(exchange, quote_curr, maint_margin)} {quote_curr} (MMR: {mmr:.4% if mmr else 'N/A'})")
            ...
            logger.warning(f"{Fore.YELLOW}[{func_name}] Error parsing position risk details: {parse_err}. Data: {str(active_pos_risk)[:300]}{Style.RESET_ALL}")
