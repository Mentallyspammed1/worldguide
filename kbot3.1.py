...
try:
    ...
    if os.getenv("TERMUX_VERSION"):
         print(f"{Style.BRIGHT}pkg install python && pip install {' '.join(COMMON_PACKAGES)}{Style.RESET_ALL}")
    else:
    ...
    def _get_env(self, key: str, default: Any, color: str, cast_type: type = str,
        ...
        try:
            if cast_type == bool:
                ...
                casted_value = value_str.lower() in ['true', '1', 'yes', 'y', 'on'] if value_str is not None else bool(default)
...
def format_price(symbol: str, price: Union[Decimal, str, float, int]) -> str:
    ...
    if MARKET_INFO is None or EXCHANGE is None:
        ...
        try:
            ...
            return str(price_dec.quantize(Decimal("1E-8"), rounding=ROUND_HALF_EVEN)) # Use ROUND_HALF_EVEN for price
        except Exception:
    ...
    try:
        ...
        return EXCHANGE.price_to_precision(symbol, price_float)
         ...
         try:
             price_dec = Decimal(str(price))
             return str(price_dec.quantize(Decimal("1E-8"), rounding=ROUND_HALF_EVEN)) # Use ROUND_HALF_EVEN
         except Exception:
              return str(price)
    except Exception as e:
...
def calculate_indicators(df: pd.DataFrame) -> Optional[Dict[str, Decimal]]:
    ...
    try:
        ...
        if len(tr_series.iloc[1:].dropna()) >= CONFIG.atr_period: # Check non-NaN count after removing first
             ...
             atr_series = pd.Series([np.nan], index=[tr_series.index[0]]).append(atr_series_calc)
             # Need to reindex to match the original df_calc index in case of dropped rows earlier
            ...
            if ret_code == 0: # Bybit V5 success code
                 ...
                 if cum_exec_qty_raw is not None and avg_price_raw is not None:
                     try:
                         ...
                         fill_tolerance = qty.copy_abs() * Decimal("0.001") + min_qty * Decimal("0.1") # e.g., 0.1% + 10% of min_qty
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    if not can_trade_logic:
        ...
        if (has_long_pos or has_short_pos) and indicators is not None and not current_price.is_nan() and not current_atr.is_nan():
             ...
             if (has_long_pos or has_short_pos) and (indicators is None or current_price.is_nan() or current_atr.is_nan()):
                  ...
                  if current_price.is_nan(): skip_reason_parts.append("Current price missing")
    ...
    print_status_panel(
        ...
        final_positions_for_panel, current_equity, signals, final_order_tracker_state_for_panel # Use the final snapshots/state
    )
        ...
        if positions:
            ...
            if positions is not None: # Check if positions fetch itself succeeded
                for side, pos_data in positions.items():
                     ...
                     # Check qty is not None before abs
                     if qty is not None and qty.copy_abs() >= CONFIG.position_qty_epsilon:
            ...
            if not fetched_positions_to_process:
                ...
                for side, pos_data in fetched_positions_to_process.items():
                     ...
                     try:
                         ...
                         # Ensure qty is not None before taking abs
                         if qty is None:
                              logger.critical(Fore.RED + f"Closure failed: Quantity for {side.upper()} position is None. MANUAL CLOSURE REQUIRED!")
                              termux_notify("EMERGENCY!", f"{symbol} {side.upper()} POS QTY NONE! Close manually!")
                         ...
                         close_params = {'reduceOnly': True, 'positionIdx': 0} # Crucial: Only close, don't open new position + one-way mode. Category added by wrapper.
                         # Use unified symbol string for create_market_order
            ...
            if total_positions_to_close == 0: # Case where fetch found no significant positions
                 ...
                 logger.warning(Fore.YELLOW + f"Placed closure orders for {closed_count} out of {total_positions_to_close} detected positions. Some closures may have failed. MANUAL CHECK REQUIRED.")
                 termux_notify("Shutdown Warning!", f"{symbol} Some closures failed. Check logs/positions!")
            else: # closed_count is 0 but total_positions_to_close > 0
                 logger.critical(Fore.RED + Style.BRIGHT + f"Closure attempts failed for all {total_
