# --*- coding: utf-8 -*-
# pylint: disable=too-many-instance-attributes, too-many-arguments, too-many-locals, too-many-public-methods, invalid-name, unused-argument, too-many-lines, unnecessary-pass, line-too-long, broad-except, logging-fstring-interpolation, consider-using-f-string
# fmt: off
#   ____        _       _   _                  _            _         _
...
#         |___/                                                |_|                      |_|
# Pyrmethus v4.5.9 - Neon Nexus Enhanced (VolumaticTrend Integration)
# fmt: on
"""
Pyrmethus - Termux Trading Spell (v4.5.9 - Neon Nexus Enhanced)
...
stop-loss, take-profit, and trailing-stop features. Enhanced for robustness,
clarity, and detailed status reporting.
"""
...
class ExchangeManager:
    ...
    def _load_market_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Market {self.config.symbol} (API ID: {market.get('id', 'N/A')}): Decimals(Amount={market['precision_dp']['amount']}, Price={market['precision_dp']['price']}), "
                f"Steps(TickSize={tick_size_str}, AmountStep={amount_step_str}), Limits(MinOrderAmount={min_amt_str}), "
                f"ContractSize={market['contract_size'].normalize()}, SettleCurrency: {market.get('settle', 'N/A')}, "
                f"Base: {market.get('base', 'N/A')}, Quote: {market.get('quote', 'N/A')}"
            )
            return market
    ...
    def _format_v5_param(self, value: Optional[Union[Decimal, str, float, int]], param_type: Literal["price", "amount", "distance"] = "price", allow_zero: bool = False) -> Optional[str]:
        ...
        "0" is returned if allow_zero is True and value is zero.
        """
        if value is None:
            return None
    ...
    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if position_side_key:
                ...
                return positions_summary # type: ignore
...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> Optional[Dict[str, Union[Decimal, bool, int]]]:
        ...
        try:
            ...
            indicators_out["stoch_d_prev"] = stoch_d_prev_val # Store for potential use
            ...
            if not any(v.is_nan() for v in [k_now, d_now, k_prev, stoch_d_prev_val]): # type: ignore[union-attr]
                if (k_prev <= stoch_d_prev_val) and (k_now > d_now): # type: ignore[operator]
...
class SignalGenerator:
    ...
    def generate_signals(self, df_last_candles: pd.DataFrame, indicators: Dict[str, Union[Decimal, bool, int]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log(
                log_level, "Signal Check: %s | Orig: %s | VT: %s", summary_status, orig_reason, vt_reason)
    ...
    def check_exit_signals(self, position_side: str, indicators: Dict[str, Union[Decimal, bool, int]]) -> Optional[str]:
        ...
        stoch_d_curr, stoch_d_prev = indicators.get("stoch_d"), indicators.get("stoch_d_prev") # Current and previous %D
        ...
        orig_req_decs = {"fast_ema": fast_ema_val, "slow_ema": slow_ema_val,
                         ...
                         "stoch_d": stoch_d_curr, "stoch_d_prev": stoch_d_prev}
        orig_inds_valid = all(isinstance(v, Decimal) and not v.is_nan() # type: ignore[union-attr]
                              for v in orig_req_decs.values())

        if orig_inds_valid:
            # type: ignore[assignment]
            fast_ema, slow_ema, k_curr, k_prev, d_curr, d_prev = fast_ema_val, slow_ema_val, stoch_k_curr, stoch_k_prev, stoch_d_curr, stoch_d_prev
            ema_bullish_cross, ema_bearish_cross = fast_ema > slow_ema, fast_ema < slow_ema # type: ignore[operator]
            ...
            if position_side == "long":
                if ema_bearish_cross:  # EMA bearish crossover
                    orig_exit_reason = f"Orig Exit (Long): EMA Bearish Cross (FastEMA {fast_ema.normalize()} < SlowEMA {slow_ema.normalize()})" # type: ignore[union-attr]
                elif k_prev >= ob_lvl and k_curr < ob_lvl:  # Stochastic %K crosses down from overbought # type: ignore[operator]
                    orig_exit_reason = f"Orig Exit (Long): Stoch Reversal from Overbought (PrevK {k_prev.normalize():.1f} >= OB -> CurrK {k_curr.normalize():.1f} < OB)" # type: ignore[union-attr]
                elif k_prev >= d_prev and k_curr < d_curr: # K crosses D downwards (Bearish K/D cross) # type: ignore[operator]
                    orig_exit_reason = f"Orig Exit (Long): Stoch K/D Bearish Cross (PrevK {k_prev.normalize():.1f} >= PrevD {d_prev.normalize():.1f} -> CurrK {k_curr.normalize():.1f} < CurrD {d_curr.normalize():.1f})" # type: ignore[union-attr]

            elif position_side == "short":
                if ema_bullish_cross:  # EMA bullish crossover
                    orig_exit_reason = f"Orig Exit (Short): EMA Bullish Cross (FastEMA {fast_ema.normalize()} > SlowEMA {slow_ema.normalize()})" # type: ignore[union-attr]
                elif k_prev <= os_lvl and k_curr > os_lvl:  # Stochastic %K crosses up from oversold # type: ignore[operator]
                    orig_exit_reason = f"Orig Exit (Short): Stoch Reversal from Oversold (PrevK {k_prev.normalize():.1f} <= OS -> CurrK {k_curr.normalize():.1f} > OS)" # type: ignore[union-attr]
                elif k_prev <= d_prev and k_curr > d_curr: # K crosses D upwards (Bullish K/D cross) # type: ignore[operator]
                    orig_exit_reason = f"Orig Exit (Short): Stoch K/D Bullish Cross (PrevK {k_prev.normalize():.1f} <= PrevD {d_prev.normalize():.1f} -> CurrK {k_curr.normalize():.1f} > CurrD {d_curr.normalize():.1f})" # type: ignore[union-attr]
        ...
        if self.config.vt_enable:
            ...
            vt_decs_valid = all(isinstance(v, Decimal) and not v.is_nan() # type: ignore[union-attr]
            ...
            if vt_decs_valid and vt_bools_valid:
                ...
                if position_side == "long":
                    ...
                    if price < vt_trend_ema: # type: ignore[operator]
                        # type: ignore[union-attr]
                    ...
                    elif price < vt_vwma and is_vol_spike and is_red_cdl: # type: ignore[operator]
                        # type: ignore[union-attr]
                    ...
                    elif price > vt_vwma and is_vol_spike and is_green_cdl: # type: ignore[operator]
                        # type: ignore[union-attr]
...
class OrderManager:
    ...
    def _calculate_trade_parameters(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> Optional[Dict[str, Optional[Decimal]]]:
        ...
        if not self.market_info or any(v.is_nan() for v in [mkt_tick_size, mkt_contract_size, mkt_min_order_size]):
            ...
            return None
        if mkt_tick_size <= Decimal("0"): # type: ignore[operator]
            logger.error("Market tick_size is invalid (zero or negative). Cannot calculate trade parameters.")
            return None
        ...
        try:
            ...
            if sl_dist_abs < mkt_tick_size: # type: ignore[operator]
                logger.warning(
                    f"Calculated SL distance ({sl_dist_abs.normalize()}) is less than market tick size ({mkt_tick_size.normalize()}). Adjusting SL distance to one tick size.") # type: ignore[union-attr]
                sl_dist_abs = mkt_tick_size # type: ignore[assignment]
            ...
            if self.config.market_type == "inverse":
                ...
                if current_price <= Decimal("0"): # Should be caught earlier
                    logger.error(
                ...
                risk_per_contract_settle_ccy = sl_dist_abs * mkt_contract_size # type: ignore[operator]
            ...
            if qty_final_dec.is_nan() or qty_final_dec <= Decimal("0"):
                ...
                return None
            if qty_final_dec < mkt_min_order_size: # type: ignore[operator]
                logger.error(
                    f"Calculated quantity {qty_final_dec.normalize()} is less than minimum order size {mkt_min_order_size.normalize()}. Cannot place trade.") # type: ignore[union-attr]
                return None
            ...
            if tsl_dist_pts_calc < mkt_tick_size:  # Ensure TSL distance is at least one tick # type: ignore[operator]
                logger.debug(
                    f"Calculated TSL distance ({tsl_dist_pts_calc.normalize()}) is less than market tick size ({mkt_tick_size.normalize()}). Adjusting to tick size.") # type: ignore[union-attr]
                tsl_dist_pts_calc = mkt_tick_size # type: ignore[assignment]
            ...
            logger.info(
                f"Calculated Trade Parameters ({side.upper()}): Qty={params_out['qty'].normalize() if params_out['qty'] else 'N/A'} {base_ccy}, " # type: ignore[union-attr]
                f"EntryPx (approx)={current_price.normalize():.{DEFAULT_PRICE_DP}f}, SL={params_out['sl_price'].normalize() if params_out['sl_price'] else 'N/A'}, " # type: ignore[union-attr]
                f"TP={'Disabled' if not params_out['tp_price'] else params_out['tp_price'].normalize()}, " # type: ignore[union-attr]
                f"TSL_Dist={'N/A' if not params_out['tsl_distance'] else params_out['tsl_distance'].normalize()}, " # type: ignore[union-attr]
    ...
    def _set_position_protection(self, position_side: str, sl_price: Optional[Decimal] = None, tp_price: Optional[Decimal] = None, is_tsl: bool = False, tsl_distance: Optional[Decimal] = None, tsl_activation_price: Optional[Decimal] = None) -> bool:
        ...
        if is_tsl:
            if tsl_dist_api_str != "0" and tsl_act_px_api_str != "0":
                api_params.update({
                    ...
                    "stopLoss": "0",  # Clear fixed SL
                    "takeProfit": "0"  # Clear fixed TP
                })
            ...
            if tp_api_str != "0":
                ...
                api_params["tpTriggerBy"] = self.config.sl_trigger_by # Bybit uses tpTriggerBy
            api_params.update({"trailingStop": "0", "activePrice": "0"}) # Clear TSL
        ...
        # Common CCXT pattern for method names
        private_method_name_fallback = "privatePostPositionTradingStop"

        method_to_call_actual_name = private_method_name
        if not hasattr(self.exchange, private_method_name):
            if hasattr(self.exchange, private_method_name_fallback):
                logger.warning(f"CCXT method '{private_method_name}' not found, using fallback '{private_method_name_fallback}'. Consider updating CCXT.")
                method_to_call_actual_name = private_method_name_fallback
            else:
                logger.critical(f"{Style.BRIGHT}{Fore.RED}Fatal Error: CCXT method for setting trading stops ('{private_method_name}' or '{private_method_name_fallback}') not found. Cannot manage position protection. Update CCXT or check method name.{Style.RESET_ALL}")
                return False  # Critical failure

        method_to_call = getattr(self.exchange, method_to_call_actual_name)

        try:
            response = fetch_with_retries(
                method_to_call,
                params=api_params,  # Use the prepared api_params
                max_retries=self.config.max_fetch_retries,
                delay_seconds=self.config.retry_delay_seconds
            )

            if response and response.get("retCode") == V5_SUCCESS_RETCODE:
                logger.trade(f"{Style.BRIGHT}{Fore.GREEN}Successfully {action_desc} for {position_side.upper()} position.{Style.RESET_ALL}")
                termux_notify(f"{self.config.symbol} Protection Update",
                ...
                logger.error(f"{Fore.RED}Failed to {action_desc} for {position_side.upper()}. API Response: Code={ret_code}, Msg='{ret_msg}'.{Style.RESET_ALL}")
                logger.debug(
                    f"Full API response from failed {method_to_call_actual_name}: {str(response)[:500]}")
                termux_notify(f"{self.config.symbol} Protection FAILED",
            ...
            termux_notify(f"{self.config.symbol} Protection ERROR",
    ...
    def place_risked_market_order(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> bool:
        ...
        qty_to_order = trade_params.get("qty") if trade_params else None
        if not qty_to_order or qty_to_order <= Decimal("0"): # type: ignore[operator]
            logger.error(
                "Entry Aborted: Failed to calculate valid trade parameters (e.g., quantity is zero or invalid).")
            return False

        # type: ignore[union-attr]
        initial_sl_price, initial_tp_price = trade_params.get("sl_price"), trade_params.get("tp_price") # type: ignore[assignment]
        ...
        if not market_order_response:
            logger.error(
                f"Entry Aborted: Market order execution failed for {side.upper()} {qty_to_order.normalize()}.") # type: ignore[union-attr]
            self._handle_entry_failure(side, qty_to_order) # type: ignore[arg-type]
            return False
        ...
        min_expected_fill_qty = qty_to_order * Decimal("0.90") # type: ignore[operator]
        ...
        active_position_details = final_pos_state_summary.get( # type: ignore[union-attr]
        ...
        if actual_filled_qty < qty_to_order * Decimal("0.99"): # type: ignore[operator]
            logger.warning(
                f"Filled quantity {actual_filled_qty.normalize()} is notably less than ordered quantity {qty_to_order.normalize()}. This may indicate slippage or partial fill.{Style.RESET_ALL}") # type: ignore[union-attr]

        V5_SUCCESS_RETCODE = 0  # Bybit V5 API success code

def fetch_with_retries(self, method, params, max_retries, delay_seconds):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"Retry {attempt + 1}/{max_retries} after {delay_seconds}s due to {e}")
                time.sleep(delay_seconds)
            else:
...
def _set_position_protection(self, position_side, sl_price, tp_price, trailing_stop=None, order_id=None):
    """
    Set SL/TP/TSL for a position using Bybit V5 API with CCXT.
    Replaces the original _set_position_protection to fix response handling and CCXT compatibility.
    
    Args:
        position_side (str): 'LONG' or 'SHORT'
        sl_price (float): Stop-loss price
        tp_price (float): Take-profit price
        trailing_stop (float, optional): Trailing stop distance
        order_id (str, optional): Order ID for cancellation on failure
    
    Returns:
        bool: True if protection set successfully, False otherwise
    """
    # Standardized and fallback method names for Bybit V5 trading stop
    private_method_name = "private_post_v5_position_trading_stop"
    private_method_name_fallback = "privatePostPositionTradingStop"
    ...
    if not hasattr(self.ccxt_exchange, private_method_name):
        if hasattr(self.ccxt_exchange, private_method_name_fallback):
            ...
            if order_id:
                try:
                    self.ccxt_exchange.cancel_order(order_id, self.config.symbol)
                    logger.info(f"Order {order_id} canceled due to missing CCXT method.")
    ...
    api_params = {
        'category': 'linear',
        'symbol': self.config.symbol,
        'stopLoss': str(sl_price) if sl_price else '',
    ...
    action_desc = f"Set SL={sl_price}, TP={tp_price}, TSL={trailing_stop or 'None'}"
    tracker_key = f"{self.config.symbol}_{position_side.lower()}"
    new_tracker_state = {'sl': sl_price, 'tp': tp_price, 'tsl': trailing_stop}
    ...
    try:
        ...
        if response and response.get("retCode", -1) == V5_SUCCESS_RETCODE:
            logger.info(
            ...
            ret_code = response.get("retCode", "[No RetCode]") if response else "[No Response]"
            ret_msg = response.get("retMsg", "[No RetMsg]") if response else "[No Response]"
            logger.error(
                f"{Fore.RED}Failed to {action_desc} for {position_side.upper()}. "
                f"API Response: Code={ret_code}, Msg='{ret_msg}'.{Style.RESET_ALL}"
            )
            logger.debug(f"Full API response from failed {private_method_name}: {str(response)[:500]}")
            if order_id:
                try:
                    self.ccxt_exchange.cancel_order(order_id, self.config.symbol)
                    logger.info(f"Order {order_id} canceled due to failed protection setting.")
                except Exception as cancel_error:
                    logger.error(f"Failed to cancel order {order_id}: {cancel_error}")
            return False
    except Exception as e:
    ...
    def manage_trailing_stop(self, position_side: str, entry_price: Decimal, current_market_price: Decimal, current_atr: Decimal) -> None:
        ...
        if any(val is None or val.is_nan() or val <= Decimal("0") for val in [entry_price, current_market_price, current_atr]): # type: ignore[attr-defined]
            logger.debug(
        ...
        try:
            ...
            if tsl_activation_target_price.is_nan() or tsl_activation_target_price <= Decimal("0"): # type: ignore[union-attr]
                logger.warning(
                    f"Invalid TSL activation target price calculated ({tsl_activation_target_price.normalize() if not tsl_activation_target_price.is_nan() else 'NaN'}). Skipping TSL check for {position_side.upper()}.") # type: ignore[union-attr]
                return
            ...
            if not min_tick_size.is_nan() and min_tick_size > Decimal("0") and tsl_distance_to_set_pts < min_tick_size: # type: ignore[union-attr]
                logger.debug(
                    f"Calculated TSL distance ({tsl_distance_to_set_pts.normalize()}) is less than min tick size ({min_tick_size.normalize()}). Adjusting TSL distance to one tick size.") # type: ignore[union-attr]
                tsl_distance_to_set_pts = min_tick_size # type: ignore[union-attr]

            if tsl_distance_to_set_pts <= Decimal("0"): # type: ignore[union-attr]
                logger.warning(
                    f"Invalid TSL distance calculated ({tsl_distance_to_set_pts.normalize()}). Skipping TSL activation for {position_side.upper()}.") # type: ignore[union-attr]
                return
            ...
            if should_activate_tsl:
                ...
                logger.trade(
                    f"  Details: EntryPx={entry_price.normalize():.{DEFAULT_PRICE_DP}f}, CurrPx={current_market_price.normalize():.{DEFAULT_PRICE_DP}f}, " # type: ignore[union-attr]
                    f"TSL_ActivationTargetPx≈{tsl_activation_target_price.normalize():.{DEFAULT_PRICE_DP}f}, TSL_DistanceToSet≈{tsl_distance_to_set_pts.normalize():.{DEFAULT_PRICE_DP}f}" # type: ignore[union-attr]
                )
initial_tsl_distance = atr * self.config.tsl_activation_atr_multiplier  # e.g., 0.5 * ATR
order_id = market_order['id']  # From create_market_ord
                ...
                logger.debug(
                    f"TSL Check ({position_side.upper()}): Activation condition NOT MET. (CurrentPrice: {current_market_price.normalize():.{DEFAULT_PRICE_DP}f}, TargetActivationPrice: ~{tsl_activation_target_price.normalize():.{DEFAULT_PRICE_DP}f})") # type: ignore[union-attr]
        except Exception as e:
            logger.error(
...
class StatusDisplay:
    ...
    def print_status_panel(self, cycle_num: int, current_timestamp: Optional[datetime], current_market_price: Optional[Decimal], indicators_data: Optional[Dict[str, Any]], current_positions_summary: Optional[Dict[str, Dict[str, Any]]], account_equity: Optional[Decimal], signal_check_result: Dict[str, Any], protection_status_tracker: Dict[str, Optional[str]], market_specific_info: Optional[Dict[str, Any]]) -> None:
        ...
        if indicators_data:
            ...
            adx_strength_style = "yellow" if not adx_val.is_nan( # type: ignore[union-attr]
            ) and adx_val > self.config.min_adx_level else "dim yellow" # type: ignore[operator]
...
class TradingBot:
    ...
    def __init__(self):
        logger.info(
            f"{Style.BRIGHT}{Fore.MAGENTA}--- Initializing Pyrmethus v4.5.9 (Neon Nexus Enhanced - VolumaticTrend Edition) ---{Style.RESET_ALL}")
    ...
    def _signal_handler_callback(self, sig_num: int, _frame: Optional[Any]) -> None:
        """Callback function for OS signals to initiate graceful shutdown."""
        # `_frame` is unused but required by the signal.signal signature
    ...
    def run(self) -> None:
        ...
        while not self.shutdown_requested:
            ...
            # Handle sys.exit calls from within the cycle (e.g., config issues)
                ...
                self.shutdown_requested = True
                # Propagate the exit code if it's non-zero, otherwise let graceful_shutdown handle it
                if se.code != 0:
                    sys.exit(se.code)
                break
            ...
            if not self.shutdown_requested:
                ...
                if sleep_needed_seconds > 0:
                    logger.debug(
                        f"Sleeping for {sleep_needed_seconds:.2f} seconds until the next trading cycle...")
                    ...
                    try:
                        while time.monotonic() < sleep_end_time_monotonic and not self.shutdown_requested:
                            ...
                            time.sleep(min(0.5, sleep_end_time_monotonic - time.monotonic()))
    ...
    def trading_spell_cycle(self, cycle_num: int) -> None:
        ...
        if current_positions_summary.get("long", {}): # Check if long position dict is non-empty
            long_pos_data = current_positions_summary["long"]
            if safe_decimal(long_pos_data.get("qty")).copy_abs() >= POSITION_QTY_EPSILON:
                active_pos_side, active_pos_details_dict = "long", long_pos_data
        # If not long, check for short (cannot be both with default positionIdx=0 or hedge mode focus)
        if not active_pos_side and current_positions_summary.get("short", {}):
            short_pos_data = current_positions_summary["short"]
            if safe_decimal(short_pos_data.get("qty")).copy_abs() >= POSITION_QTY_EPSILON:
                active_pos_side, active_pos_details_dict = "short", short_pos_data
        ...
        if active_pos_side and active_pos_details_dict:
            ...
            current_atr_val = indicators.get("atr", Decimal("NaN")) # type: ignore[assignment]
            ...
            if (self.order_manager.protection_tracker.get(active_pos_side) == PROTECTION_STATE_SLTP and
                    not any(v is None or v.is_nan() or v <= Decimal("0") for v in [pos_entry_px, current_price, current_atr_val])): # type: ignore[attr-defined]
                self.order_manager.manage_trailing_stop(
                    active_pos_side, pos_entry_px, current_price, current_atr_val) # type: ignore[arg-type]
                ...
                if self.order_manager.protection_tracker.get(active_pos_side) == PROTECTION_STATE_TSL:
                    ...
                    current_positions_summary_after_tsl = self.exchange_manager.get_current_position()
                    if current_positions_summary_after_tsl: # Update if fetch was successful
                        current_positions_summary = current_positions_summary_after_tsl
                        # Re-evaluate active_pos_details_dict with potentially updated TSL info from exchange
                        if current_positions_summary.get(active_pos_side, {}):
                             active_pos_details_dict = current_positions_summary[active_pos_side]
                        else: # Position might have closed or changed unexpectedly
                             active_pos_side, active_pos_details_dict = None, None
                             logger.warning("Position side %s became None after TSL management and re-fetch. Assuming flat.", active_pos_side)


            # Check for Strategy-based Exit Signals (only if not already managed by TSL on exchange)
            if self.order_manager.protection_tracker.get(active_pos_side) != PROTECTION_STATE_TSL:
                exit_reason = self.signal_generator.check_exit_signals(
                    active_pos_side, indicators)
                if exit_reason:
                    logger.trade(
                        f"{Fore.YELLOW}Strategy Exit Signal for {active_pos_side.upper()} position: {exit_reason}{Style.RESET_ALL}")
                    if self.order_manager.close_position(active_pos_side, pos_qty, reason=exit_reason):
                        current_status_dict.update(
                            {"summary": f"CLOSED_{active_pos_side.upper()}:{exit_reason[:50]}"})
                        # Position closed, reset active_pos_side for this cycle
                        active_pos_side = None
                    else:
                        logger.error(
                            f"{Fore.RED}Failed to close {active_pos_side.upper()} position based on exit signal. MANUAL INTERVENTION MAY BE NEEDED.{Style.RESET_ALL}")
                        current_status_dict.update(
                            {"summary": f"FAIL_CLOSE_{active_pos_side.upper()}:{exit_reason[:40]}"})
                else: # No exit signal
                    current_status_dict.update(
                        {"summary": f"HOLDING_{active_pos_side.upper()}:NoStratExitSignal"})
            else: # Position is under TSL management by exchange
                 current_status_dict.update(
                    {"summary": f"HOLDING_{active_pos_side.upper()}:ManagedByExchangeTSL"})


        # 6. Entry Signal Check (if not in a position or if position just closed this cycle)
        if not active_pos_side:  # If flat (either initially or after a close this cycle)
            entry_signals = self.signal_generator.generate_signals(
                ohlcv_df, indicators)
            current_status_dict.update(entry_signals)  # Update with detailed signal reasons

            atr_for_entry = indicators.get("atr", Decimal("NaN")) # type: ignore[assignment]
            if atr_for_entry.is_nan() or atr_for_entry <= Decimal("0"): # type: ignore[union-attr]
                logger.warning(
                    f"ATR for entry is invalid ({atr_for_entry}). Cannot calculate risk for new orders.") # type: ignore[union-attr]
                if entry_signals.get("long") or entry_signals.get("short"):
                     current_status_dict["summary"] = f"Signal Blocked: Invalid ATR ({atr_for_entry.normalize() if not atr_for_entry.is_nan() else 'NaN'}) for risk calculation." # type: ignore[union-attr]
            elif entry_signals.get("long"):
                if self.order_manager.place_risked_market_order("buy", atr_for_entry, total_equity, current_price): # type: ignore[arg-type]
                    current_status_dict["summary"] = f"ENTERED_long:{entry_signals.get('summary', 'LongSignal')[:50]}"
                else:
                    current_status_dict["summary"] = f"FAIL_ENTRY_long:{entry_signals.get('summary', 'LongSignalFail')[:45]}"
            elif entry_signals.get("short"):
                if self.order_manager.place_risked_market_order("sell", atr_for_entry, total_equity, current_price): # type: ignore[arg-type]
                    current_status_dict["summary"] = f"ENTERED_short:{entry_signals.get('summary', 'ShortSignal')[:48]}"
                else:
                    current_status_dict["summary"] = f"FAIL_ENTRY_short:{entry_signals.get('summary', 'ShortSignalFail')[:43]}"
            # If no entry signal, current_status_dict already contains "No Signal" or similar from generate_signals
        elif active_pos_side and not current_status_dict.get("summary", "").startswith(("HOLDING_", "CLOSED_")):
            # If still in position and no specific holding/closed status set, default to holding
             current_status_dict["summary"] = f"HOLDING_{active_pos_side.upper()}:AwaitingNextSignal"


        # 7. Display Status Panel
        # Re-fetch position summary if an entry or close occurred this cycle to ensure display is current
        if "ENTERED_" in current_status_dict.get("summary", "") or "CLOSED_" in current_status_dict.get("summary", ""):
            logger.debug("Re-fetching position summary post-trade action for status display.")
            current_positions_summary = self.exchange_manager.get_current_position()

        self.status_display.print_status_panel(
            cycle_num, last_candle_timestamp, current_price, indicators,
            current_positions_summary, total_equity, current_status_dict,
            self.order_manager.protection_tracker, self.exchange_manager.market_info
        )

    def graceful_shutdown(self) -> None:
        """Handles graceful shutdown procedures, like closing open positions if configured."""
        console.print(
            "\n[bold yellow]Pyrmethus is preparing to return to the void...[/bold yellow]")
        logger.warning("Graceful shutdown initiated.")

        if self.config.close_positions_on_shutdown:
            logger.info(
                "Attempting to close open positions as per configuration...")
            try:
                current_pos_summary = self.exchange_manager.get_current_position()
                if current_pos_summary:
                    active_pos_side_to_close: Optional[str] = None
                    qty_to_close_on_shutdown: Optional[Decimal] = None

                    long_pos_data = current_pos_summary.get("long", {})
                    if long_pos_data and safe_decimal(long_pos_data.get("qty", "0")).copy_abs() >= POSITION_QTY_EPSILON:
                        active_pos_side_to_close = "long"
                        qty_to_close_on_shutdown = safe_decimal(
                            long_pos_data.get("qty")).copy_abs()

                    if not active_pos_side_to_close: # Check short only if not long
                        short_pos_data = current_pos_summary.get("short", {})
                        if short_pos_data and safe_decimal(short_pos_data.get("qty", "0")).copy_abs() >= POSITION_QTY_EPSILON:
                            active_pos_side_to_close = "short"
                            qty_to_close_on_shutdown = safe_decimal(
                                short_pos_data.get("qty")).copy_abs()

                    if active_pos_side_to_close and qty_to_close_on_shutdown:
                        logger.warning(
                            f"Found open {active_pos_side_to_close.upper()} position (Qty: {qty_to_close_on_shutdown.normalize()}). Attempting to close...")
                        termux_notify("Pyrmethus Shutdown",
                                      f"Closing open {active_pos_side_to_close.upper()} position.")
                        if self.order_manager.close_position(active_pos_side_to_close, qty_to_close_on_shutdown, reason="ShutdownClose"):
                            logger.info(
                                f"Successfully closed {active_pos_side_to_close.upper()} position during shutdown.")
                        else:
                            logger.error(
                                f"{Fore.RED}Failed to close {active_pos_side_to_close.upper()} position during shutdown. MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
                            termux_notify(
                                "Pyrmethus URGENT", f"Failed to close {active_pos_side_to_close.upper()} position on shutdown!")
                    else:
                        logger.info("No open positions found to close during shutdown.")
                else:
                    logger.error(
                        "Failed to fetch current position during shutdown. Cannot determine if positions need closing. MANUAL CHECK REQUIRED.")
            except Exception as e:
                logger.error(
                    f"Error during position closure on shutdown: {e}", exc_info=True)
                termux_notify(
                    "Pyrmethus Error", "Error closing positions on shutdown. Check logs.")
        else:
            logger.info(
                "CLOSE_POSITIONS_ON_SHUTDOWN is False. Any open positions will remain.")

        logger.info(
            f"{Style.BRIGHT}{Fore.MAGENTA}--- Pyrmethus Shutdown Sequence Complete ---{Style.RESET_ALL}")
        termux_notify("Pyrmethus Shutdown", "Pyrmethus has shut down.")
...
if __name__ == "__main__":
    ...
    try:
        bot = TradingBot()
        bot.run()
    except ccxt.AuthenticationError as auth_e: # Catch auth errors that might occur during bot init itself
        logger.critical(f"{Style.BRIGHT}{Fore.RED}CRITICAL AUTHENTICATION ERROR during Pyrmethus startup: {auth_e}. Halting.{Style.RESET_ALL}", exc_info=False)
        termux_notify("Pyrmethus CRITICAL ERROR", f"Startup Auth failed: {str(auth_e)[:100]}")
        exit_code = 1
    except SystemExit as e: # Catch sys.exit from config or other early critical points
        logger.info(f"Pyrmethus terminated with SystemExit code: {e.code}")
        # Use the exit code from SystemExit if non-zero
        exit_code = e.code if e.code is not None and e.code != 0 else 0
    except KeyboardInterrupt:
        logger.warning("\nKeyboardInterrupt received during Pyrmethus startup or very early execution. Shutting down...")
        if bot and not bot.shutdown_requested: # If bot instance exists and shutdown not already handled
             bot.shutdown_requested = True # Ensure flag is set
             bot.graceful_shutdown() # Attempt cleanup
        else: # Bot instance might not exist or shutdown already in progress
             console.print("[bold yellow]Pyrmethus interrupted early. Minimal cleanup performed.[/bold yellow]")
        exit_code = 0 # Typically 0 for Ctrl+C unless an error state
    except Exception as main_e:
        logger.critical(f"{Style.BRIGHT}{Fore.RED}Unhandled top-level exception in Pyrmethus: {main_e}{Style.RESET_ALL}", exc_info=True)
        termux_notify("Pyrmethus FATAL ERROR", f"Pyrmethus crashed: {str(main_e)[:100]}")
        if bot and not bot.shutdown_requested: # If bot instance exists and shutdown not already handled
             bot.shutdown_requested = True
             logger.info("Attempting emergency graceful shutdown after fatal error...")
             bot.graceful_shutdown()
        exit_code = 1
    finally:
        logger.info(f"Pyrmethus exiting with code {exit_code}.")
        sys.exit(exit_code)
