...
def setup_logger(name: str) -> logging.Logger:
    ...
    if logger.hasHandlers():
        return logger  # Avoid duplicate handlers if called again
...
def load_config(filepath: str) -> dict[str, Any]:
    ...
    if not os.path.exists(filepath):  # Create default if not found
        init_logger.warning(
            f"{NEON_YELLOW}Config file '{filepath}' not found. Creating a default configuration file.{RESET}"
        )
        try:
    ...
    try:  # Load existing config
        ...
        init_logger.error(
            f"{NEON_RED}Error decoding JSON from config file '{filepath}': {e}. Attempting to recreate with defaults.{RESET}"
        )
    ...
    try:  # Validate and merge loaded config with defaults
        ...
        def validate_numeric(
            ...
            try:
                ...
                init_logger.warning(
                    f"Config validation internal error: Key path '{key_path}' invalid after ensuring keys."
                )
            ...
            try:
                ...
                if (
                    ...
                    if type(final_value) is not type(original_value):
                        init_logger.info(
                            f"{NEON_YELLOW}Config: Corrected type for '{key_path}' from '{type(original_value).__name__}' to '{type(final_value).__name__}'. Value: {final_value}{RESET}"
                        )
                    corrected = True  # Mark as corrected if type or value changed

            except (ValueError, InvalidOperation, TypeError):
                init_logger.warning(
                    f"{NEON_YELLOW}Config: Invalid value '{original_value}' for '{key_path}'. Expected {'integer' if is_int else 'number'} in range {'(' if is_strict_min else '['}{min_val}, {max_val}{']'}{' (or 0)' if allow_zero else ''}. Using default: {default_value}{RESET}"
                )
        ...
        if updated_config.get("interval") not in VALID_INTERVALS:
            init_logger.error(
                f"{NEON_RED}Invalid interval '{updated_config.get('interval')}' in config. Using default '{default_config['interval']}'. Valid: {VALID_INTERVALS}{RESET}"
            )
        ...
        if config_needs_saving:
            try:
                ...
                )  # Simple way to convert Decimals if any remained
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if attempt < MAX_API_RETRIES:
                    ...
                    lg.critical(
                        f"{NEON_RED}Max retries reached while loading markets due to network errors: {e}. Exiting.{RESET}"
                    )
                    return None
            except Exception as e:
                lg.critical(
        ...
        try:
            ...
            if balance_val is not None:
                ...
                if CONFIG.get("enable_trading", False):
                    ...
                    lg.warning(
                        f"{NEON_YELLOW} Trading is disabled, proceeding cautiously despite balance fetch failure.{RESET}"
                    )
        except ccxt.AuthenticationError as auth_err:
            lg.critical(
                f"{NEON_RED}Authentication Error fetching initial balance: {auth_err}. Check API keys/permissions.{RESET}"
            )
            return None
...
def fetch_klines_ccxt(
    ...
    if limit > BYBIT_API_KLINE_LIMIT:
        lg.debug(
            f"Requested kline limit {limit} exceeds API limit {BYBIT_API_KLINE_LIMIT}. Requesting {BYBIT_API_KLINE_LIMIT}."
        )

    for attempt in range(MAX_API_RETRIES + 1):
        try:
            lg.debug(
                f"Fetching klines for {symbol}, {timeframe}, limit={actual_request_limit} (Attempt {attempt + 1}/{MAX_API_RETRIES + 1})"
            )
            # Fetch OHLCV data
            ...
            if (
                ...
                lg.debug(
                    f"Exchange returned fewer candles ({returned_count}) than requested ({actual_request_limit}). This might be expected near the start of market history."
                )
            ...
            if ohlcv and returned_count > 0:
                ...
                try:
                    ...
                    if lag_seconds < max_lag:
                        ...
                        lg.warning(
                            f"{NEON_YELLOW}Last kline timestamp {last_ts} seems old (Lag: {lag_seconds:.1f}s > Max allowed: {max_lag}s). Retrying fetch...{RESET}"
                        )
            ...
            if bybit_code in [10001, 110013]:  # Example Bybit parameter error codes
                lg.error(
                    f" >> Hint: Check if symbol '{symbol}' exists and is valid for the specified category (Linear/Inverse/Spot)."
                )
            return pd.DataFrame()
        except Exception as e:
            lg.error(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if market:
                ...
                def fmt_val(v):
                    return str(Decimal(str(v)).normalize()) if v is not None else "N/A"
                ...
                lg.debug(
                    f"Market Info Retrieved: {symbol} (ID={market.get('id')}, Type={market.get('type')}, Contract Type={market['contract_type_str']})"
                )
                lg.debug(
                    f"  Precision: Price={fmt_val(precision.get('price'))}, Amount={fmt_val(precision.get('amount'))}"
                )
                lg.debug(
                    f"  Limits (Amount): Min={fmt_val(amount_limits.get('min'))}, Max={fmt_val(amount_limits.get('max'))}"
                )
                lg.debug(
                ...
                if precision.get("price") is None or precision.get("amount") is None:
                    lg.error(
                        f"{NEON_RED}CRITICAL: Market '{symbol}' is missing required price or amount precision information! Trading may fail or be inaccurate.{RESET}"
                    )
            ...
            if attempts < MAX_API_RETRIES:
                ...
                lg.error(
                    f"{NEON_RED}Max retries reached getting market info for {symbol} due to network errors: {e}{RESET}"
                )
                return None
        except ccxt.ExchangeError as e:
            lg.error(
            ...
            for acc_type in account_types_to_try:
                try:
                    ...
                    # Check Bybit V5 specific 'info' structure (list of accounts)
                                ...
                                for coin_data in account["coin"]:
                                    if coin_data.get("coin") == currency:
                                        ...
                                        if free is not None:
                                            ...
                                            lg.debug(
                                                f"Found balance in V5 list structure for {acc_type}: {available_balance_str} (field: {'availableToWithdraw' if coin_data.get('availableToWithdraw') else 'availableBalance' if coin_data.get('availableBalance') else 'walletBalance'})"
                                            )
                                            found_structure = True
                        ...
                        if found_structure:
                            break  # Found in the list for this acc_type
            ...
            if found_structure and available_balance_str is not None:
                try:
                    ...
                    raise ccxt.ExchangeError(
                        f"Failed to convert fetched balance string '{available_balance_str}' to Decimal for {currency}: {conv_err}"
                    )
            else:
                # Raise an error if balance wasn't found after trying all methods
                raise ccxt.ExchangeError(
                    f"Balance information not found for currency '{currency}' in any expected structure."
                )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                ...
                lg.warning(
                    f"Could not get market info to determine category for position fetch ({market_err}). Assuming 'linear'."
                )
            ...
            for pos in positions:
                ...
                try:
                    ...
                    if abs(position_size) > size_threshold:
                        lg.debug(
                            f"Found potential active position with size {position_size} (Threshold: {size_threshold})."
                        )
                        active_position = pos
            ...
            if active_position:
                ...
                if side not in ["long", "short"]:
                    ...
                    # Fallback: Infer side from size if standard/V5 side is missing/invalid
                        ...
                        lg.warning(
                            f"Position size {std_pos['size_decimal']} is near zero or side is ambiguous. Treating as no position."
                        )
                        return None
                std_pos["side"] = side
                ...
                if tp_price_str is not None and str(tp_price_str) != "0":
                    std_pos["takeProfitPrice"] = str(tp_price_str)
                if tsl_distance_str is not None and str(tsl_distance_str) != "0":
                    std_pos["trailingStopLoss"] = str(tsl_distance_str)
                if tsl_activation_str is not None and str(tsl_activation_str) != "0":
                ...
                def fmt_log(val_str, p_type="price", p_def=4):
                    ...
                    if s_val == "0":
                        return "0"  # Avoid formatting "0" strangely
                    try:
                        d = Decimal(s_val)
                            ...
                            if prec_val is not None:
                                try:
                                    ...
                                    prec = (
                                        ...
                                        else abs(step.normalize().as_tuple().exponent)
                                    )
                                except Exception:
                                    pass  # Use default prec if calculation fails
                        # Quantize and normalize for clean output
                        exp = Decimal("1e-" + str(prec))
                        return str(
                            d.quantize(exp, rounding=ROUND_DOWN).normalize()
                        )  # Use normalize() to remove trailing zeros
                    except Exception:
                        return s_val  # Return original string if formatting fails
        ...
        # --- Exception Handling for the outer loop ---
            ...
            if bybit_code in [110004]:  # Account not found / key linking issue?
                lg.critical(
                    f"{NEON_RED}Possible Bybit Account Error ({bybit_code}): {e}. Check API key link/permissions. Stopping.{RESET}"
                )
                return None
            if bybit_code in [110013]:  # Parameter error
                lg.error(
                    f"{NEON_RED}Bybit Parameter Error ({bybit_code}) fetching position: {e}. Check symbol/category validity. Stopping.{RESET}"
                )
                return None
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                )  # Determine category from market info
            ...
            if ret_code is not None:  # Check if it looks like a Bybit V5 response
                if ret_code == 0:
                    ...
                    lg.info(
                        f"{NEON_YELLOW}Leverage for {symbol} already set to {leverage}x (Bybit Code: 110045 - Not modified).{RESET}"
                    )
                    return True
                else:
                ...
                lg.info(
                    f"{NEON_GREEN}Leverage set successfully for {symbol} to {leverage}x (Standard CCXT success).{RESET}"
                )
                return True
...
def calculate_position_size(
    ...
    if initial_stop_loss_price <= Decimal("0") or entry_price <= Decimal("0"):
        lg.error(
            f"Position sizing failed for {symbol}: Entry price ({entry_price}) or Stop Loss price ({initial_stop_loss_price}) must be positive."
        )
        return None
    ...
    try:
        ...
        try:
            ...
            lg.warning(
                f"Invalid contract size '{contract_size_str}' found in market info for {symbol}, using default '1': {e}"
            )
            contract_size = Decimal("1")

    except (KeyError, ValueError, InvalidOperation, TypeError) as e:
        lg.error(
            f"Position sizing failed for {symbol}: Error accessing/converting market precision/limits/contractSize: {e}"
        )
        return None
    ...
    try:
        if not is_inverse:
            ...
            lg.debug(
                f"  Raw Linear/Spot Size Calc: {risk_amt_quote.normalize()} / ({sl_dist_price.normalize()} * {contract_size.normalize()}) = {calculated_size.normalize()}"
            )
            ...
            if entry_price > 0 and initial_stop_loss_price > 0:
                ...
                lg.debug(
                    f"  Raw Inverse Size Calc: RiskPerContract = {contract_size.normalize()} * |1/{entry_price.normalize()} - 1/{initial_stop_loss_price.normalize()}| = {risk_per_contract_quote.normalize()}"
                )
                lg.debug(
    ...
    if calculated_size <= Decimal("0"):
        lg.error(
            f"Position sizing failed for {symbol}: Initial calculated size is zero or negative ({calculated_size.normalize()}). Check inputs and risk settings."
        )
        return None
    ...
    if min_amt > 0 and adj_size < min_amt:
        lg.warning(
            f"{NEON_YELLOW}Calculated size {adj_size.normalize()} is below minimum amount {min_amt.normalize()}. Adjusting size UP to minimum.{RESET}"
        )
        adj_size = min_amt
    if max_amt < Decimal("inf") and adj_size > max_amt:
        lg.warning(
            f"{NEON_YELLOW}Calculated size {adj_size.normalize()} exceeds maximum amount {max_amt.normalize()}. Adjusting size DOWN to maximum.{RESET}"
        )
        adj_size = max_amt
    ...
    try:
        if entry_price > 0:
            ...
            if not is_inverse:
                ...
                if is_contract:
                    est_cost = (
                        adj_size * entry_price * contract_size
                    )  # Need confirmation on linear contractSize definition
            else:  # Inverse
        ...
        if min_cost > 0 and est_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {est_cost.normalize()} is below minimum cost {min_cost.normalize()}. Attempting to increase size.{RESET}"
            )
            ...
            if max_amt < Decimal("inf") and required_size_for_min_cost > max_amt:
                lg.error(
                    f"{NEON_RED}Cannot meet minimum cost {min_cost.normalize()} without exceeding maximum amount limit {max_amt.normalize()}. Aborted.{RESET}"
                )
                return None
        ...
        # Check Max Cost
            ...
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {est_cost.normalize()} exceeds maximum cost {max_cost.normalize()}. Attempting to reduce size.{RESET}"
            )
            ...
            try:
                if entry_price > 0 and contract_size > 0:
                    if not is_inverse:
                        max_size_for_max_cost = max_cost / (
                            entry_price * contract_size
                        )  # Adjust if linear contractSize != 1
                    else:
            ...
            adj_size = max(
                min_amt, min(adj_size, max_size_for_max_cost)
            )  # Take the smaller of current adj_size and max_size_for_cost, ensuring it's >= min_amt
            cost_adjusted = True
    ...
    try:
        ...
        lg.debug(
            f"Applied amount precision using ccxt.amount_to_precision: {adj_size.normalize()} -> {final_size.normalize()}"
        )
        # Double check if rounding up occurred unexpectedly (some exchanges might?)
        if final_size > adj_size:
            lg.warning(
                f"ccxt.amount_to_precision rounded UP? {adj_size} -> {final_size}. Attempting manual round down."
            )
        ...
        try:
            ...
            lg.error(
                f"{NEON_RED}Manual amount precision failed: {manual_err}. Using limit-adjusted size without precision: {adj_size.normalize()}{RESET}"
            )
            final_size = adj_size  # Fallback to size before precision step
    except Exception as generic_fmt_err:
        lg.error(
            f"{NEON_RED}Unexpected error applying amount precision: {generic_fmt_err}. Using unrounded size.{RESET}",
            exc_info=True,
        )
        final_size = adj_size
    ...
    if final_size <= Decimal("0"):
        lg.error(
            f"{NEON_RED}Final position size is zero or negative ({final_size.normalize()}) after applying limits and precision. Aborted.{RESET}"
        )
        return None
    # Re-check against minimum amount limit after precision rounding
    if min_amt > 0 and final_size < min_amt:
        lg.error(
            f"{NEON_RED}Final size {final_size.normalize()} is less than minimum amount {min_amt.normalize()} after precision rounding. Aborted.{RESET}"
        )
    ...
    try:
        ...
        if min_cost > 0 and final_cost < min_cost:
            lg.debug(
                f"Final size {final_size.normalize()} results in cost {final_cost.normalize()} < min cost {min_cost.normalize()}."
            )
            ...
            try:
                ...
                valid_next_step = (
                    ...
                    and (max_amt == Decimal("inf") or next_step_size <= max_amt)
                    and (max_cost == Decimal("inf") or next_step_cost <= max_cost)
                )

                if valid_next_step:
                    lg.warning(
                        f"{NEON_YELLOW}Final size cost is below minimum. Bumping size by one step to {next_step_size.normalize()} to meet minimum cost.{RESET}"
                    )
                    final_size = next_step_size
                else:
                    lg.error(
                        f"{NEON_RED}Final size cost is below minimum ({min_cost.normalize()}), and increasing size by one step ({next_step_size.normalize()}) is invalid (violates limits or still too low). Aborted.{RESET}"
                    )
                    return None
            except Exception as bump_err:
                lg.error(
                    f"{NEON_RED}Error attempting to bump size by one step for min cost: {bump_err}. Aborted.{RESET}"
                )
                return None
...
def place_trade(
    ...
    if side is None:
        lg.error(
            f"Trade placement failed: Invalid trade_signal '{trade_signal}'. Must be BUY, SELL, EXIT_LONG, or EXIT_SHORT."
        )
        return None
...
def _set_position_protection(
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"Protection setting failed for {symbol}: Invalid position side ('{pos_side}') or missing entry price ('{entry_price_str}')."
        )
        return False
    try:
    ...
    try:
        ...
        if min_tick <= 0:
            raise ValueError("Invalid price precision step")
                ...
                if (
                    ...
                    if not valid_act_price:
                        ...
                        if fmt_tsl_dist and fmt_act_price:
                            ...
                            lg.error(
                                f"Failed to format valid TSL parameters (Distance: {fmt_tsl_dist}, Activation: {fmt_act_price}). Cannot set TSL."
                            )
        ...
        if not set_tsl and isinstance(stop_loss_price, Decimal):
            ...
            if stop_loss_price > 0:
                ...
                if not valid_sl:
                    lg.error(
                        f"Stop Loss Price {stop_loss_price.normalize()} is not beyond entry price {entry_price.normalize()} for {pos_side} position. Cannot set SL."
                    )
                else:
        ...
        if isinstance(take_profit_price, Decimal):
            ...
            if take_profit_price > 0:
                ...
                if not valid_tp:
                    lg.error(
                        f"Take Profit Price {take_profit_price.normalize()} is not beyond entry price {entry_price.normalize()} for {pos_side} position. Cannot set TP."
                    )
                else:
    ...
    if not params_to_set:
        if any_change_requested:
            lg.warning(
                "Protection change requested, but no valid parameters could be formatted/validated. No API call made."
            )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if ret_code == 0:
                ...
                if is_fatal:
                    lg.error(
                        " >> Hint: Non-retryable error code received. Check parameters, prices relative to entry, and permissions."
                    )
                    return False  # Stop retrying
                else:
...
def set_trailing_stop_loss(
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"TSL calculation failed for {symbol}: Invalid position side ('{pos_side}') or missing entry price ('{entry_price_str}')."
        )
        return False
    ...
    try:
        ...
        lg.error(
            f"{NEON_RED}Invalid TSL configuration or market/position info for {symbol}: {e}. Cannot calculate TSL.{RESET}"
        )
        return False
...
class VolumaticOBStrategy:
    ...
    def __init__(
        ...
        if self.min_data_len > BYBIT_API_KLINE_LIMIT:
            self.logger.error(
                f"{NEON_RED}{BRIGHT}CONFIGURATION ERROR:{RESET} Strategy requires {self.min_data_len} candles, "
                f"but the API limit per fetch is {BYBIT_API_KLINE_LIMIT}."
            )
            self.logger.error(
                f"{NEON_YELLOW} >> ACTION REQUIRED: Reduce lookback periods in 'config.json' (e.g., 'vt_vol_ema_length', 'vt_atr_period') "
    ...
    def _ema_swma(self, series: pd.Series, length: int) -> pd.Series:
        ...
        # Define SWMA weights
        weights = np.array([1.0, 2.0, 2.0, 1.0]) / 6.0  # Normalized weights
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        if len(df) < self.min_data_len:
            self.logger.warning(
                f"{NEON_YELLOW}Insufficient data length provided ({len(df)} candles) for full analysis "
        ...
        try:
            ...
            if missing_cols:
                self.logger.error(
                    f"Input DataFrame is missing required columns: {', '.join(missing_cols)}. Aborting analysis."
                )
                return empty_results
        ...
        try:
                            ...
                            if (
                                ...
                                if not any(
                                    ...
                                    if (
                                        ...
                                        new_boxes_count += 1
                                        self.logger.debug(
                                            f"  New Bear OB: {new_box['id']} @ {piv_idx} [{new_box['bottom'].normalize()}-{new_box['top'].normalize()}]"
                                        )
                                    else:
                                        self.logger.debug(
                                            f"Skipping Bear OB @ {piv_idx}: Invalid candle data (Top={top}, Bot={bot})."
                                        )
                            ...
                            if piv_loc >= 0:
                                ...
                                if not any(
                                    ...
                                    if (
                                        ...
                                        new_box = OrderBlock(
                                            id=f"L_{piv_idx.strftime('%y%m%d%H%M%S')}",
                                            type="bull",
                                            left_idx=piv_idx,
                                        ...
                                        new_boxes_count += 1
                                        self.logger.debug(
                                            f"  New Bull OB: {new_box['id']} @ {piv_idx} [{new_box['bottom'].normalize()}-{new_box['top'].normalize()}]"
                                        )
                                    else:
                                        self.logger.debug(
                                            f"Skipping Bull OB @ {piv_idx}: Invalid candle data (Top={top}, Bot={bot})."
                                        )
                    except Exception as e:
            ...
            if new_boxes_count > 0:
                self.logger.debug(
                    f"Found {new_boxes_count} new Order Block(s). Total counts before prune: Bull={len(self.bull_boxes)}, Bear={len(self.bear_boxes)}."
                )
                ...
                for box in self.bull_boxes:
                    if box["active"]:
                        ...
                        if last_close < box["bottom"]:
                            ...
                            self.logger.debug(
                                f"Bull OB {box['id']} VIOLATED by close {last_close.normalize()} < {box['bottom'].normalize()} at {last_idx}."
                            )
                ...
                for box in self.bear_boxes:
                    if box["active"]:
                        ...
                        if last_close > box["top"]:
                            ...
                            box["right_idx"] = last_idx
                            self.logger.debug(
                                f"Bear OB {box['id']} VIOLATED by close {last_close.normalize()} > {box['top'].normalize()} at {last_idx}."
                            )
                        elif self.ob_extend:
                            box["right_idx"] = last_idx
            else:
        ...
        self.logger.debug(
            f"  Bands: Lower={results['lower_band'].normalize() if results['lower_band'] else 'N/A'}, Upper={results['upper_band'].normalize() if results['upper_band'] else 'N/A'}"
        )
        self.logger.debug(
            f"  Active OBs: Bull={len(results['active_bull_boxes'])}, Bear={len(results['active_bear_boxes'])}"
        )
...
class SignalGenerator:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"{NEON_RED}Error initializing SignalGenerator with config values: {e}. Using hardcoded defaults.{RESET}",
                exc_info=True,
            )
    ...
    def generate_signal(
        ...
        if (
            ...
            or analysis_results.get("last_close") is None
            or analysis_results["last_close"] <= 0
            or analysis_results.get("atr") is None
            or analysis_results["atr"] <= 0
        ):
            lg.warning(
                f"{NEON_YELLOW}Invalid or incomplete strategy analysis results provided to signal generator. Holding.{RESET}"
            )
            ...
            # Exit Condition 2: Price hits a Bearish OB (using exit proximity)
                ...
                try:
                    ...
                    if close >= exit_threshold:
                        signal = "EXIT_LONG"
                        lg.warning(
                            f"{NEON_YELLOW}{BRIGHT}EXIT LONG Signal: Price {close.normalize()} >= Bear OB Exit Threshold {exit_threshold.normalize()} "
            ...
            # Exit Condition 2: Price hits a Bullish OB (using exit proximity)
                ...
                try:
                    ...
                    if close <= exit_threshold:
                        signal = "EXIT_SHORT"
                        lg.warning(
                            f"{NEON_YELLOW}{BRIGHT}EXIT SHORT Signal: Price {close.normalize()} <= Bull OB Exit Threshold {exit_threshold.normalize()} "
            ...
            if trend_up is True and bull_obs:
                for ob in bull_obs:
                    ...
                    if entry_zone_bottom <= close <= entry_zone_top:
                        ...
                        lg.info("  Trend is UP.")
                        lg.info(
                            f"  Price {close.normalize()} is within Bull OB Entry Zone [{entry_zone_bottom.normalize()} - {entry_zone_top.normalize()}]"
                        )
                        lg.info(
                            f"  (OB ID: {ob['id']}, Base Range: [{ob['bottom'].normalize()}-{ob['top'].normalize()}])"
                        )
            ...
            # Entry Condition 2: Trend is DOWN and price is inside/near an active Bearish OB
                ...
                for ob in bear_obs:
                    ...
                    if entry_zone_bottom <= close <= entry_zone_top:
                        ...
                        lg.info(
                            f"  Price {close.normalize()} is within Bear OB Entry Zone [{entry_zone_bottom.normalize()} - {entry_zone_top.normalize()}]"
                        )
                        lg.info(
                            f"  (OB ID: {ob['id']}, Base Range: [{ob['bottom'].normalize()}-{ob['top'].normalize()}])"
                        )
    ...
    def calculate_initial_tp_sl(
        ...
        if price_prec_str is None:
            lg.error(
                f"Missing price precision in market info for symbol {market_info['symbol']}. Cannot calculate TP/SL."
            )
            return None, None
        ...
        try:
            ...
            if tp_formatted is not None:
                if (signal == "BUY" and tp_formatted <= entry_price) or (
                    ...
                    lg.warning(
                        f"Calculated TP {tp_formatted.normalize()} is not strictly beyond entry price {entry_price.normalize()}. Disabling initial TP."
                    )
            ...
            if sl_formatted is None:
                lg.error(
                    f"{NEON_RED}Stop Loss calculation failed or resulted in an invalid level after formatting/adjustment. Cannot proceed with trade.{RESET}"
                )
...
def analyze_and_trade_symbol(
    ...
    if klines_df.empty or fetched_count < min_req_data:
        ...
        if api_limit_hit_but_insufficient:
            lg.error(
                f"{NEON_RED}CRITICAL DATA SHORTFALL:{RESET} Fetched {fetched_count} candles (hit API limit {BYBIT_API_KLINE_LIMIT}), "
                f"but strategy requires {min_req_data}."
            )
            lg.error(
                f"{NEON_YELLOW} >> ACTION REQUIRED: Reduce strategy lookback periods (e.g., 'vt_vol_ema_length') in config.json "
                f"so the minimum data needed is less than {BYBIT_API_KLINE_LIMIT}. Skipping cycle.{RESET}"
            )
        elif klines_df.empty:
            lg.error(
                f"Failed to fetch any kline data for {symbol}. Check connection or symbol validity. Skipping cycle."
            )
        else:  # Got some data, but less than required (and didn't hit API limit)
            lg.error(
                f"Failed to fetch sufficient kline data ({fetched_count}/{min_req_data}). "
    ...
    if (
        ...
        lg.error(
            f"{NEON_RED}Strategy analysis completed but produced invalid/incomplete results. Skipping cycle.{RESET}"
        )
    ...
    if current_price is None:
        lg.warning(
            f"{NEON_YELLOW}Failed to fetch real-time price. Using last kline close ({latest_close.normalize()}) for position management checks.{RESET}"
        )
    ...
    if not trading_enabled:
        ...
        if (
            ...
            or open_position
            and signal in ["EXIT_LONG", "EXIT_SHORT"]
        ):
            lg.info(f"  (Would attempt to {signal} if trading was enabled)")
        else:
    ...
    # --- Trading Enabled ---
    lg.debug(
        f"Trading is enabled. Signal: {signal}. Current Position: {'Yes (' + open_position['side'] + ')' if open_position else 'No'}"
    )
    ...
    if open_position is None:
        if signal in ["BUY", "SELL"]:
            lg.info(
                f"{BRIGHT}*** {signal} Signal received and NO current position. Initiating Entry Sequence... ***{RESET}"
            )
            ...
            tp_initial_calc, sl_initial_calc = signal_generator.calculate_initial_tp_sl(
            ...
            if pos_size is None or pos_size <= Decimal("0"):
                lg.error(
                    f"{NEON_RED}Trade Aborted ({signal}): Position size calculation failed or resulted in zero/negative size ({pos_size}).{RESET}"
                )
                return
            ...
            if trade_order and trade_order.get("id"):
                ...
                if confirmed_pos:
                    try:
                        ...
                        lg.info(
                            f"{NEON_GREEN}Position Confirmed! Actual Avg Entry Price: ~{entry_actual.normalize()}{RESET}"
                        )
                        ...
                        if sl_prot_calc is None:
                            lg.error(
                                f"{NEON_RED}CRITICAL: Failed to recalculate Stop Loss based on actual entry price! Position is unprotected!{RESET}"
                            )
                            ...
                            if prot_cfg.get("enable_trailing_stop", True):
                                lg.info(
                                    f"Setting Initial Trailing Stop Loss (based on Entry: {entry_actual.normalize()})..."
                                )
                            ...
                            if protection_set_successfully:
                                lg.info(
                                    f"{NEON_GREEN}{BRIGHT}=== TRADE ENTRY & INITIAL PROTECTION SETUP COMPLETE ({symbol} {signal}) ==={RESET}"
                                )
                            else:
                                lg.error(
                                    f"{NEON_RED}{BRIGHT}=== TRADE PLACED, BUT FAILED TO SET INITIAL PROTECTION ({symbol} {signal}). MANUAL MONITORING REQUIRED! ==={RESET}"
                                )

                    except Exception as post_trade_err:
                        lg.error(
                            f"{NEON_RED}Error during post-trade setup (protection): {post_trade_err}{RESET}",
                            exc_info=True,
                        )
                        lg.warning(
                            f"{NEON_YELLOW}Position confirmed, but may lack protection! Manual check required!{RESET}"
                        )
                else:
                    # Order placed, but position not found after delay
                    lg.error(
                        f"{NEON_RED}Order (ID: {trade_order['id']}) was placed, but FAILED TO CONFIRM open position after {confirm_delay}s! "
    ...
    # === Scenario 2: Existing Position ===
        ...
        try:
            pos_size = open_position["size_decimal"]  # Use pre-parsed Decimal size
        except KeyError:
        ...
        if exit_triggered:
            lg.warning(
                f"{NEON_YELLOW}{BRIGHT}*** {signal} Signal received! Initiating Position Close Sequence... ***{RESET}"
            )
            try:
                ...
                if size_to_close <= Decimal("0"):
                    lg.warning(
                        f"Attempting to close position, but recorded size is zero or negative ({pos_size}). Position might already be closed or data is inconsistent."
                    )
                    return  # Avoid placing zero size order

                lg.info(
                    f"===> Placing {close_signal} MARKET Order (Reduce Only) | Size: {size_to_close.normalize()} <==="
                )
                ...
                if close_order and close_order.get("id"):
                    ...
                    lg.error(
                        f"{NEON_RED}Failed to place CLOSE order for {symbol} position. Manual check required!{RESET}"
                    )
        ...
        # --- No Exit Signal: Perform Position Management (BE, TSL checks) ---
            ...
            tsl_active = False
            with contextlib.suppress(Exception):
                tsl_active = tsl_dist_str and Decimal(str(tsl_dist_str)) > 0
            ...
            lg.debug(
                f"  Current Protections: TSL Active={tsl_active} (Dist='{tsl_dist_str}'), Fixed SL={sl_curr.normalize() if sl_curr else 'None'}, Fixed TP={tp_curr.normalize() if tp_curr else 'None'}"
            )
            ...
            if (
                ...
                try:
                    ...
                    if be_offset_ticks < 0:
                        raise ValueError("BE offset ticks cannot be negative")
                    ...
                    if profit_in_atr >= be_trig_atr_mult:
                        lg.info(
                            f"{NEON_PURPLE}{BRIGHT}Break-Even profit target REACHED! (Profit {profit_in_atr:.3f} >= {be_trig_atr_mult.normalize()} ATRs){RESET}"
                        )
                        ...
                        if be_sl_price and be_sl_price > 0:
                            lg.debug(
                                f"  Calculated BE SL Price: {be_sl_price.normalize()} (Entry {entry_price.normalize()} +/- {offset_value.normalize()})"
                            )
                            ...
                            if sl_curr is None:
                                ...
                                pos_side == "long"
                                and be_sl_price > sl_curr
                                or pos_side == "short"
                                and be_sl_price < sl_curr
                            ):
                                update_sl = True
                                lg.info(
                                    f"  New BE SL {be_sl_price.normalize()} is better than current SL {sl_curr.normalize()}."
                                )
                            else:
                                lg.debug(
                                    f"  Current SL {sl_curr.normalize()} is already at or better than the calculated BE SL {be_sl_price.normalize()}. No SL update needed."
                                )

                            # If update is needed, call the protection function
                            if update_sl:
                                lg.warning(
                                    f"{NEON_PURPLE}{BRIGHT}*** Moving Stop Loss to Break-Even at {be_sl_price.normalize()} ***{RESET}"
                                )
            ...
            if tsl_enabled and not tsl_active and entry_price and current_atr:
                lg.warning(
                    f"{NEON_YELLOW}Trailing Stop Loss is enabled but not currently active on the position. Attempting to set/recover TSL...{RESET}"
                )
    ...
    cycle_end_time = time.monotonic()
    lg.info(
        f"{BRIGHT}---== Analysis Cycle End ({symbol}, Duration: {cycle_end_time - cycle_start_time:.2f}s) ==---{RESET}\n"
    )
...
def main() -> None:
    ...
    init_logger.info(
        f"{BRIGHT}--- Starting Pyrmethus Volumatic Bot v1.1.3 ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{RESET}"
    )
    init_logger.info(
        f"Loaded Configuration: Quote={QUOTE_CURRENCY}, Trading Enabled={CONFIG['enable_trading']}, Sandbox Mode={CONFIG['use_sandbox']}"
    )
    try:
        # Log dependency versions for easier debugging
        init_logger.info(
            f"Using versions: Python={os.sys.version.split()[0]}, CCXT={ccxt.__version__}, Pandas={pd.__version__}, Pandas-TA={getattr(ta, 'version', 'N/A')}, Numpy={np.__version__}, Requests={requests.__version__}"
        )
    except Exception as e:
        ...
        try:
            ...
            if m_info:
                ...
                if (
                    ...
                    init_logger.critical(
                        f"{NEON_RED}CRITICAL ERROR: Market '{target_symbol}' is missing price or amount precision information in its market data. Cannot trade safely. Exiting.{RESET}"
                    )
                    # Close log handlers
                    logging.shutdown()
                    for handler in init_logger.handlers[:]:
    ...
    while selected_interval is None:
        ...
        interval_input = input(
            f"{NEON_YELLOW}Enter timeframe {VALID_INTERVALS} (default: {default_int}): {RESET}"
        ).strip()
    ...
    prot_cfg = CONFIG["protection"]
    symbol_logger.info(
        f"Key Settings: Risk={CONFIG['risk_per_trade']:.2%}, Leverage={CONFIG['leverage']}x, "
    ...
    try:
        ...
        symbol_logger.critical(
            f"{NEON_RED}Failed to initialize strategy engine or signal generator: {engine_err}. Exiting.{RESET}",
            exc_info=True,
        )
        # Close log handlers
        logging.shutdown()
    ...
    try:
        while True:
            ...
            loop_count += 1
            symbol_logger.debug(
                f">>> Starting Loop Cycle #{loop_count} at {datetime.now(TIMEZONE).strftime('%H:%M:%S %Z')}"
            )
            ...
            # --- Robust Exception Handling for the Loop ---
                ...
                symbol_logger.error(
                    f"{NEON_RED}Network error encountered in main loop: {e}. Waiting {RETRY_DELAY_SECONDS * 3}s...{RESET}"
                )
