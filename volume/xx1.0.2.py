...
class PositionInfo(TypedDict):
    ...
    contracts: float | None  # Number of contracts (may be deprecated, use size_decimal)
    contractSize: Any | None  # Size of one contract for this position
    side: str | None  # 'long' or 'short'
    notional: Any | None  # Position value in quote currency
    leverage: Any | None  # Position leverage
    unrealizedPnl: Any | None  # Unrealized profit/loss
    realizedPnl: Any | None  # Realized profit/loss
    collateral: Any | None  # Margin used for the position
    entryPrice: Any | None  # Average entry price
    markPrice: Any | None  # Current mark price
    liquidationPrice: Any | None  # Estimated liquidation price
    marginMode: str | None  # 'isolated' or 'cross'
    hedged: bool | None  # Is hedging enabled for this position?
    ...
    stopLossPrice: str | None  # Current stop loss price (as string from exchange)
    takeProfitPrice: str | None  # Current take profit price (as string from exchange)
    trailingStopLoss: str | None  # Current trailing stop distance/price (as string)
    tslActivationPrice: str | None  # Trailing stop activation price (as string)
    # --- Bot State Tracking ---
...
def setup_logger(name: str) -> logging.Logger:
    ...
    try:
        ...
        )  # Format defined within the class
...
def _validate_and_correct_numeric(
    ...
    try:
        ...
        if isinstance(
            original_val, bool
        ):  # Explicitly disallow bools for numeric fields
            ...
            if isinstance(original_val, int):
                ...
                not isinstance(original_val, float)
                or isinstance(original_val, float)
        ...
        if needs_type_correction:
            init_logger.info(
                f"{NEON_YELLOW}Config Update: Corrected type/value for '{key_path}' from {repr(original_val)} to {repr(converted_val)}.{RESET}"
            )
    ...
    try:
        ...
        def get_nested_levels(
            ...
            try:
                ...
                init_logger.error(
                    f"Config validation error: Cannot access path '{path}'."
                )
                return None, None, None
        ...
        if not isinstance(pairs, list) or not all(
            ...
            init_logger.warning(
                f"Invalid 'trading_pairs' format. Using default: {default_config['trading_pairs']}."
            )
        ...
        if updated_config.get("interval") not in VALID_INTERVALS:
            init_logger.warning(
                f"Invalid 'interval' value '{updated_config.get('interval')}'. Must be one of {VALID_INTERVALS}. Using default '{default_config['interval']}'."
            )
        ...
        if not isinstance(updated_config.get("enable_trading"), bool):
            init_logger.warning(
                f"Invalid 'enable_trading' value. Must be true or false. Using default '{default_config['enable_trading']}'."
            )
        ...
        if not isinstance(updated_config.get("use_sandbox"), bool):
            init_logger.warning(
                f"Invalid 'use_sandbox' value. Must be true or false. Using default '{default_config['use_sandbox']}'."
            )
        ...
        )  # e.g., 1.0 to 1.1 range
        validate_numeric(
            updated_config, "strategy_params.ob_exit_proximity_factor", 1.0, 1.1
        )  # e.g., 1.0 to 1.1 range

        if updated_config.get("strategy_params", {}).get("ob_source") not in [
            "Wicks",
            "Body",
        ]:
            init_logger.warning(
                f"Invalid strategy_params.ob_source. Must be 'Wicks' or 'Body'. Using default '{DEFAULT_OB_SOURCE}'."
            )
            if "strategy_params" not in updated_config:
                updated_config["strategy_params"] = {}  # Ensure dict exists
            updated_config["strategy_params"]["ob_source"] = DEFAULT_OB_SOURCE
            config_needs_saving = True

        if not isinstance(
            updated_config.get("strategy_params", {}).get("ob_extend"), bool
        ):
            init_logger.warning(
                f"Invalid strategy_params.ob_extend. Must be true or false. Using default '{DEFAULT_OB_EXTEND}'."
            )
            if "strategy_params" not in updated_config:
                updated_config["strategy_params"] = {}
            updated_config["strategy_params"]["ob_extend"] = DEFAULT_OB_EXTEND
            config_needs_saving = True

        # Protection Params
        if not isinstance(
            updated_config.get("protection", {}).get("enable_trailing_stop"), bool
        ):
            init_logger.warning(
                "Invalid protection.enable_trailing_stop. Using default."
            )
            if "protection" not in updated_config:
                updated_config["protection"] = {}
        ...
        if not isinstance(
            updated_config.get("protection", {}).get("enable_break_even"), bool
        ):
            init_logger.warning("Invalid protection.enable_break_even. Using default.")
            if "protection" not in updated_config:
                updated_config["protection"] = {}
        ...
        if config_needs_saving:
            init_logger.info(
                f"Configuration requires updates. Saving changes to '{filepath}'..."
            )
            try:
...
def _format_price(
    ...
    try:
        ...
        if price_decimal <= Decimal("0"):
            init_logger.warning(
                f"Attempted to format non-positive price '{price}' for {symbol}. Returning None."
            )
            return None
        ...
        if Decimal(formatted_str) <= Decimal("0"):
            init_logger.warning(
                f"Price '{price}' for {symbol} formatted to non-positive value '{formatted_str}'. Returning None."
            )
            return None
        ...
        init_logger.warning(
            f"Error converting price '{price}' to Decimal/float for formatting ({symbol}): {e}"
        )
        return None
    except Exception as e:
        ...
        try:
            ...
            if price:
                ...
                if bid and ask:
                    ...
                    source = (
                        f"mid-price (Bid: {bid.normalize()}, Ask: {ask.normalize()})"
                    )
                elif ask:
            ...
            if price:
                ...
                last_exception = ValueError(
                    "No valid price source (last/bid/ask) found in ticker response."
                )
                lg.warning(
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        lg.debug(
            f"Using API parameters: category='{category}', market ID='{market_id}'."
        )
    ...
    while remaining_limit > 0 and chunk_num < max_chunks:
        ...
        while attempts <= MAX_API_RETRIES:
            ...
            attempts += 1
            if (
                attempts <= MAX_API_RETRIES and chunk_data is None
    ...
    try:
        ...
        for col in numeric_cols:
            if col in df.columns:
                ...
                lg.warning(
                    f"Expected column '{col}' not found in fetched data for {symbol}."
                )
        ...
        if rows_dropped > 0:
            lg.debug(
                f"Dropped {rows_dropped} rows with invalid OHLCV data for {symbol}."
            )
...
def get_market_info(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                ...
                lg.error(
                    f"{NEON_RED}Symbol '{symbol}' is invalid or not supported by {exchange.id}.{RESET}"
                )
                return None  # Non-retryable
            ...
            if market:
                ...
                std_market["contract_size_decimal"] = _safe_market_decimal(
                    contract_size_val, f"{symbol} contractSize", allow_zero=False
                ) or Decimal("1")
                ...
                if (
                    ...
                    lg.critical(
                        f"{NEON_RED}CRITICAL VALIDATION FAILED ({symbol}): Missing essential precision data! "
                ...
                min_a = (
                    std_market["min_amount_decimal"].normalize()
                    if std_market["min_amount_decimal"]
                    else "N/A"
                )
                max_a = (
                    std_market["max_amount_decimal"].normalize()
                    if std_market["max_amount_decimal"]
                    else "N/A"
                )
                min_c = (
                    std_market["min_cost_decimal"].normalize()
                    if std_market["min_cost_decimal"]
                    else "N/A"
                )
                max_c = (
                    std_market["max_cost_decimal"].normalize()
                    if std_market["max_cost_decimal"]
                    else "N/A"
                )
                ...
                try:
                    ...
                    lg.error(
                        f"Internal error casting market dictionary to MarketInfo type ({symbol}): {cast_err}. Returning raw dict."
                    )
                ...
                if attempts < MAX_API_RETRIES:
                    ...
                    lg.error(
                        f"{NEON_RED}Failed to retrieve market information for '{symbol}' after {MAX_API_RETRIES + 1} attempts. "
                        f"Last error: {last_exception}{RESET}"
                    )
                    return None
    ...
    lg.error(
        f"{NEON_RED}Failed to get market info for {symbol} after exhausting attempts. Last error: {last_exception}{RESET}"
    )
    return None
        ...
        try:
                                ...
                                for coin_data in account_details["coin"]:
                                    if coin_data.get("coin") == currency:
                                        ...
                                        if val is None:
                                            val = coin_data.get(
                                                "walletBalance"
                                            )  # Less ideal, might include frozen
                                            src = "walletBalance"
            ...
            if found and balance_str is not None:
                try:
                    ...
                    lg.debug(
                        f"Successfully parsed balance ({currency}) from {balance_source}: {final_bal.normalize()}"
                    )
                    return final_bal
                except (ValueError, InvalidOperation, TypeError) as e:
                    # If conversion fails despite finding a string, treat as an exchange error
                    raise ccxt.ExchangeError(
                        f"Failed to convert balance string '{balance_str}' to Decimal for {currency}: {e}"
                    )
            elif not found:
                # Currency not found in any checked structure
                raise ccxt.ExchangeError(
                    f"Balance information for currency '{currency}' not found in the response structure. "
...
def get_open_position(
    ...
    ).lower()  # 'linear', 'inverse', or 'spot'/'unknown'

    if not market_id or category not in ["linear", "inverse"]:
        lg.error(
            f"Cannot check position for {symbol}: Invalid market ID ('{market_id}') or category ('{category}') in market_info."
        )
        return None

    lg.debug(
        f"Checking for open position for {symbol} (Market ID: '{market_id}', Category: '{category}')..."
    )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                ...
                if (
                    exchange.has.get("fetchPositions")
                    and exchange.has["fetchPositions"] != "emulated"
                ):
                ...
                if exchange.has.get("fetchPositions"):
                    ...
                    )  # Request specific symbol if possible
                    ...
                    lg.debug(
                        f"fetchPosition returned: {'Position found' if positions else 'No position found'}"
                    )
                else:
                ...
                is_bybit_no_pos = code_str and any(
                    str(c) == code_str for c in bybit_no_pos_codes
                )
                is_common_no_pos = any(msg in err_str for msg in common_no_pos_msgs)

                if is_bybit_no_pos or is_common_no_pos:
                    lg.info(
                        f"No open position found for {symbol} (API indicated no position: Code='{code_str}', Msg='{err_str[:50]}...')."
                    )
            ...
            try:
                ...
                lg.warning(
                    f"Could not determine precise size threshold for {symbol} from market info: {e}. Using default {size_threshold.normalize()}"
                )
            lg.debug(
            ...
            for pos_data in positions:
                ...
                size_str = str(
                    pos_data.get("info", {}).get("size", pos_data.get("contracts", ""))
                ).strip()
                if not size_str:
                    lg.debug(
                        f"Skipping position data with missing size field ({symbol}). Data: {pos_data.get('info', {})}"
                    )
                    continue
                ...
                try:
                    ...
                    lg.warning(
                        f"Could not parse position size string '{size_str}' for {symbol}: {e}. Skipping this entry."
                    )
                    continue
            ...
            if active_raw_position:
                ...
                std_pos["entryPrice"] = _safe_market_decimal(
                    ...
                    or info_dict.get("entryPrice"),  # Bybit uses avgPrice in info
                    f"{symbol} pos.entry",
                    allow_zero=False,
                )
                ...
                std_pos["liquidationPrice"] = _safe_market_decimal(
                    ...
                    or info_dict.get("liqPrice"),  # Bybit uses liqPrice in info
                    f"{symbol} pos.liq",
                    allow_zero=False,
                )
                std_pos["unrealizedPnl"] = _safe_market_decimal(
                    std_pos.get("unrealizedPnl")
                    or info_dict.get("unrealisedPnl"),  # Bybit uses unrealisedPnl
                    f"{symbol} pos.pnl",
                    allow_zero=True,
                    allow_negative=True,
                ...
                std_pos["be_activated"] = (
                    False  # Break-even not yet activated by bot logic
                )
                ...
                try:
                    ...
                    lg.error(
                        f"Internal error casting position dictionary to PositionInfo type ({symbol}): {cast_err}. Returning raw dict."
                    )
...
def set_leverage_ccxt(
    ...
    if not hasattr(exchange, "set_leverage") or not exchange.has.get("setLeverage"):
        lg.error(
            f"Leverage setting failed: Exchange {exchange.id} does not support setLeverage method."
        )
        return False
    ...
    lg.info(
        f"Attempting to set leverage for {symbol} (Market ID: {market_id}, Category: {category}) to {leverage}x..."
    )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.debug(
                f"set_leverage call attempt {attempts + 1} for {market_id} to {leverage}x..."
            )
            params = {}
            ...
            if ret_code_str == "0":
                lg.info(
                    f"{NEON_GREEN}Leverage successfully set for {market_id} to {leverage}x (Code: 0).{RESET}"
                )
                return True
            elif ret_code_str == "110045":
                lg.info(
                    f"{NEON_YELLOW}Leverage for {market_id} is already {leverage}x (Code: 110045 - Not Modified). Success.{RESET}"
                )
                return True
                ...
                lg.info(
                    f"{NEON_GREEN}Leverage set/confirmed for {market_id} to {leverage}x (No specific code in response, assumed success).{RESET}"
                )
                return True
            else:
        ...
        # --- Error Handling for set_leverage call ---
            ...
            lg.error(
                f"{NEON_RED}Exchange error setting leverage ({market_id} to {leverage}x): {e} (Code: {err_code_str}){RESET}"
            )
            ...
            if err_code_str == "110045" or "leverage not modified" in err_str_lower:
                lg.info(
                    f"{NEON_YELLOW}Leverage already set to {leverage}x (confirmed via error response). Success.{RESET}"
                )
                return True
            ...
            if is_fatal_code or is_fatal_message:
                ...
                if "position exists" in err_str_lower:
                    ...
                    lg.error(
                        " >> Leverage change might conflict with current margin mode (cross/isolated)."
                    )
            ...
            last_exception = e
            lg.warning(
                f"{NEON_YELLOW}Network error setting leverage ({market_id}): {e}. Retry {attempts + 1}...{RESET}"
            )
            if attempts >= MAX_API_RETRIES:
                lg.error(
            ...
            last_exception = e
            lg.error(
                f"{NEON_RED}Unexpected error setting leverage ({market_id}): {e}{RESET}",
                exc_info=True,
            )
    ...
    # If loop finishes without success
    lg.error(
        f"{NEON_RED}Failed to set leverage for {market_id} to {leverage}x after {MAX_API_RETRIES + 1} attempts. "
        f"Last error: {last_exception}{RESET}"
    )
    return False
...
def calculate_position_size(
    ...
    if initial_stop_loss_price <= Decimal("0") or entry_price <= Decimal("0"):
        lg.error(
            f"Sizing failed ({symbol}): Entry price ({entry_price.normalize()}) or Stop Loss price ({initial_stop_loss_price.normalize()}) is non-positive."
        )
        return None
    ...
    try:
        ...
        min_amount = market_info["min_amount_decimal"] or Decimal(
            "0"
        )  # Treat None as 0
        max_amount = market_info["max_amount_decimal"] or Decimal(
            "inf"
        ...
        if not (amount_step and amount_step > 0):
            raise ValueError("Amount precision step is missing or invalid.")
        if not (price_step and price_step > 0):
            raise ValueError("Price precision step is missing or invalid.")
        if not (contract_size and contract_size > 0):
            raise ValueError("Contract size is missing or invalid.")

        lg.debug(f"  Market Constraints ({symbol}):")
        lg.debug(
            f"    Amount Step: {amount_step.normalize()}, Min Amount: {min_amount.normalize()}, Max Amount: {max_amount.normalize()}"
        )
        lg.debug(f"    Price Step : {price_step.normalize()}")
        lg.debug(
            f"    Cost Min   : {min_cost.normalize()}, Cost Max: {max_cost.normalize()}"
        )
        lg.debug(
    ...
    try:
        if not is_inverse:
            ...
            if value_change_per_unit <= Decimal("1e-18"):  # Avoid division by near-zero
                lg.error(
                    f"Sizing failed ({symbol}, Linear/Spot): Calculated value change per unit is near zero. Check prices/contract size."
                )
                return None
            calculated_size = risk_amount_quote / value_change_per_unit
            lg.debug(
                f"  Linear/Spot Calculation: Size = {risk_amount_quote} / ({stop_loss_distance} * {contract_size}) = {calculated_size}"
            )
        else:
            # --- Inverse Contract ---
            ...
            if entry_price <= 0 or initial_stop_loss_price <= 0:
                lg.error(
                    f"Sizing failed ({symbol}, Inverse): Entry or SL price is non-positive, cannot calculate inverse factor."
                )
                return None
            inverse_factor = abs(
            ...
            if inverse_factor <= Decimal("1e-18"):  # Avoid division by near-zero
                lg.error(
                    f"Sizing failed ({symbol}, Inverse): Calculated inverse factor is near zero. Check prices."
                )
                return None
            ...
            if risk_per_contract_unit <= Decimal(
                ...
                lg.error(
                    f"Sizing failed ({symbol}, Inverse): Calculated risk per contract unit is near zero."
                )
                return None
            calculated_size = risk_amount_quote / risk_per_contract_unit
            lg.debug(
                f"  Inverse Calculation: Size = {risk_amount_quote} / ({contract_size} * {inverse_factor}) = {calculated_size}"
            )
    ...
    def estimate_cost(size: Decimal, price: Decimal) -> Decimal | None:
        ...
        try:
            ...
            )  # Round up cost estimate slightly
    ...
    if min_amount > 0 and adjusted_size < min_amount:
        lg.warning(
            f"{NEON_YELLOW}Sizing ({symbol}): Initial size {adjusted_size.normalize()} < Min Amount {min_amount.normalize()}. Adjusting UP to Min Amount.{RESET}"
        )
        adjusted_size = min_amount
    if max_amount < Decimal("inf") and adjusted_size > max_amount:
        lg.warning(
            f"{NEON_YELLOW}Sizing ({symbol}): Initial size {adjusted_size.normalize()} > Max Amount {max_amount.normalize()}. Adjusting DOWN to Max Amount.{RESET}"
        )
    ...
    if estimated_cost is not None:
        ...
        if min_cost > 0 and estimated_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Sizing ({symbol}): Estimated cost {estimated_cost.normalize()} < Min Cost {min_cost.normalize()}. Attempting to increase size.{RESET}"
            )
            cost_adjusted = True
            try:
                ...
                if not is_inverse:
                    if entry_price * contract_size <= 0:
                        raise ZeroDivisionError("Entry price * contract size is zero.")
                    required_size_for_min_cost = min_cost / (
                        entry_price * contract_size
                    )
                else:
                    if contract_size <= 0:
                        raise ZeroDivisionError("Contract size is zero.")
                ...
                if target_size > max_amount:
                    lg.error(
                        f"{NEON_RED}Sizing failed ({symbol}): Cannot meet Min Cost ({min_cost.normalize()}). "
        ...
        # Check Max Cost
            ...
            lg.warning(
                f"{NEON_YELLOW}Sizing ({symbol}): Estimated cost {estimated_cost.normalize()} > Max Cost {max_cost.normalize()}. Attempting to reduce size.{RESET}"
            )
            cost_adjusted = True
            try:
                ...
                if max_size_for_max_cost <= 0:
                    raise ValueError("Calculated max size is non-positive.")
                ...
                )  # Take the smaller of current or max allowed
                adjusted_size = max(
                    min_amount, target_size
                ...
                if adjusted_size < target_size:
                    lg.warning(
                        f"{NEON_YELLOW}Sizing ({symbol}): Size reduction for Max Cost resulted in {adjusted_size.normalize()}, which is capped by Min Amount {min_amount.normalize()}.{RESET}"
                    )
                elif (
                    adjusted_size == target_size and target_size < calculated_size
                ):  # Check if we actually reduced it
                    lg.info(
                        f"  Adjusted size DOWN to meet Max Cost (respecting limits): {adjusted_size.normalize()} {size_unit}"
                    )
        ...
        lg.warning(
            f"Could not estimate position cost accurately for {symbol}. Cost limit checks (Min: {min_cost.normalize()}, Max: {max_cost.normalize()}) will be skipped."
        )
    ...
    try:
        ...
        lg.error(
            f"{NEON_RED}Error applying amount precision (step size) for {symbol}: {e}. Using unrounded size {adjusted_size.normalize()} cautiously.{RESET}"
        )
    ...
    if min_amount > 0 and final_size < min_amount:
        lg.error(
            f"{NEON_RED}Sizing failed ({symbol}): Final size {final_size.normalize()} is less than Min Amount {min_amount.normalize()} after applying precision.{RESET}"
        )
        ...
        if next_step_size >= min_amount and next_step_size <= max_amount:
            ...
            if (
                final_cost_next_step is not None
                and final_cost_next_step >= min_cost
                and final_cost_next_step <= max_cost
            ):
                lg.warning(
                    f"{NEON_YELLOW}Final size was below min amount. Bumping UP one step to {next_step_size.normalize()} {size_unit} as it meets all limits.{RESET}"
                )
                final_size = next_step_size
            else:
                lg.error(
                    f"{NEON_RED}Cannot bump size to meet Min Amount as next step ({next_step_size.normalize()}) violates other limits (MaxAmt/MinCost/MaxCost). Failing sizing.{RESET}"
                )
                return None
        else:
            lg.error(
                f"{NEON_RED}Cannot bump size to meet Min Amount as next step ({next_step_size.normalize()}) violates Max Amount limit ({max_amount.normalize()}). Failing sizing.{RESET}"
            )
            return None
    ...
    if max_amount < Decimal("inf") and final_size > max_amount:
        lg.error(
            f"{NEON_RED}Sizing failed ({symbol}): Final size {final_size.normalize()} is greater than Max Amount {max_amount.normalize()} after applying precision (unexpected!).{RESET}"
        )
        return None
    ...
    if final_cost is not None:
        ...
        if min_cost > 0 and final_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Sizing ({symbol}): Final cost {final_cost.normalize()} < Min Cost {min_cost.normalize()} after precision adjustment.{RESET}"
            )
            ...
            try:
                ...
                can_bump = (
                    ...
                    and next_size <= max_amount
                    and next_cost >= min_cost  # Check if bump meets min cost
                    and next_cost <= max_cost
                )

                if can_bump:
                    lg.info(
                        f"{NEON_YELLOW}Bumping final size by one step ({symbol}) to {next_size.normalize()} to meet Min Cost limit.{RESET}"
                    )
                    ...
                    lg.debug(
                        f"  Final Cost after bump to meet Min Cost: {final_cost.normalize() if final_cost else 'N/A'}"
                    )
                else:
                    lg.error(
                        f"{NEON_RED}Sizing failed ({symbol}): Cannot meet Min Cost. Final size {final_size.normalize()} has cost {final_cost.normalize()}, "
                ...
                lg.error(
                    f"{NEON_RED}Sizing failed ({symbol}): Error occurred while attempting to bump size for Min Cost: {e}.{RESET}"
                )
                return None
        ...
        # Check Max Cost again (unlikely to be violated by rounding down, but check)
            ...
            lg.error(
                f"{NEON_RED}Sizing failed ({symbol}): Final cost {final_cost.normalize()} > Max Cost {max_cost.normalize()} after precision adjustment (unexpected!).{RESET}"
            )
            return None
    elif min_cost > 0 or max_cost < Decimal("inf"):
...
def cancel_order(
    ...
    if "bybit" in exchange.id.lower():
        try:
            ...
            lg.warning(
                f"Could not get market details to determine category/market_id for cancel ({symbol}): {e}. Proceeding without category."
            )
...
def place_trade(
    ...
    params: dict | None = None,  # Allow passing extra params
) -> dict | None:
    ...
    try:
        ...
        lg.error(
            f"Failed to convert position size {position_size.normalize()} to valid float for order placement ({symbol}): {e}"
        )
        return None
    ...
    if "bybit" in exchange.id.lower() and is_contract:
        try:
            ...
            lg.error(
                f"Failed to set Bybit V5 specific parameters for {symbol}: {e}. Proceeding with base params."
            )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            avg_price_raw = order_result.get(
                "average"
            )  # Price at which the order was filled (avg)
            filled_raw = order_result.get("filled")  # Amount filled

            avg_price_dec = _safe_market_decimal(
                avg_price_raw, "order.average", allow_zero=True
            )  # Avg price can be 0 if not filled?
            filled_dec = _safe_market_decimal(
                filled_raw, "order.filled", allow_zero=True
            )  # Filled can be 0 initially
        ...
        # --- Error Handling for create_order ---
            ...
            if any(
                s in err_lower for s in ["minimum", "too small", "less than minimum"]
            ):
                lg.error(
                    f"  >> Hint: Check order size ({position_size.normalize()}) against Min Amount ({min_a}) and potentially Min Cost ({min_c})."
                )
            elif any(
                s in err_lower
                for s in ["precision", "lot size", "step size", "size precision"]
            ):
                lg.error(
                    f"  >> Hint: Check order size ({position_size.normalize()}) precision against Amount Step ({amt_s})."
                )
            elif any(
                s in err_lower for s in ["exceed", "too large", "greater than maximum"]
            ):
                lg.error(
                    f"  >> Hint: Check order size ({position_size.normalize()}) against Max Amount ({max_a}) and potentially Max Cost ({max_c})."
                )
            elif "reduce only" in err_lower or "reduceonly" in err_lower:
                lg.error(
                    "  >> Hint: Reduce-only order failed. Ensure there's an open position to reduce and the size is appropriate."
                )
            elif "position size" in err_lower:
                lg.error(
                    "  >> Hint: Check if order size conflicts with existing position or leverage limits."
                )
            ...
            if match:
                ...
                err_code = str(getattr(e, "code", "") or getattr(e, "retCode", ""))
            ...
            fatal_msgs = [
                ...
                "trigger liquidation",
                "account not unified",
                "unified account function",  # Errors related to account type mismatch
            ]
            ...
            if is_fatal_code or is_fatal_message:
                lg.error(
                    f"{NEON_RED} >> Hint: This appears to be a NON-RETRYABLE order placement error for {symbol}.{RESET}"
                )
...
def _set_position_protection(
    ...
    | None = None,  # Distance from activation/current price
    tsl_activation_price: Decimal | None = None,  # Price at which TSL should activate
) -> bool:
    ...
    ** THIS IS A PLACEHOLDER - FULL IMPLEMENTATION IS REQUIRED **
    ...
    lg.debug(
        f"  Attempting to set: SL={stop_loss_price}, TP={take_profit_price}, TSL Dist={trailing_stop_distance}, TSL Act={tsl_activation_price}"
    )
    ...
    if "bybit" in exchange.id.lower():
        ...
        if take_profit_price:
            ...
            if tp_str:
                ...
                lg.error(f"Invalid TP price format for {symbol}: {take_profit_price}")
                return False
        ...
        if trailing_stop_distance:
            ...
            lg.warning(
                "Trailing stop distance setting not fully implemented in placeholder."
            )
            pass
        if tsl_activation_price:
            ...
            lg.warning(
                "Trailing stop activation price setting not fully implemented in placeholder."
            )
            pass
    ...
    lg.error(
        f"Protection setting not implemented for exchange {exchange.id} in placeholder."
    )
...
def set_trailing_stop_loss(
    ...
    ** THIS IS A PLACEHOLDER - FULL IMPLEMENTATION IS REQUIRED **
    ...
    if not entry_price or not current_price or not side:
        lg.error(
            f"Cannot set TSL for {symbol}: Missing entry price, current price, or side."
        )
        return False
    ...
    if side == "long":
        activation_price_calc = entry_price_dec * (
            Decimal("1") + Decimal(str(activation_perc))
        )
    else:  # short
        activation_price_calc = entry_price_dec * (
            Decimal("1") - Decimal(str(activation_perc))
    ...
    lg.info(
        f"Calculated TSL params for {symbol}: ActivationPrice={activation_price_calc.normalize()}, Distance={tsl_distance_calc.normalize()} (based on current price)"
    )

    # Format for API (placeholder - assumes distance is absolute price points)
    tsl_distance_str = exchange.price_to_precision(
        symbol, float(tsl_distance_calc)
    )  # Example formatting
    ...
    success = _set_position_protection(
        ...
        ),  # Pass calculated activation price
    )
    ...
    if success:
        ...
        lg.error(
            f"Failed to set up trailing stop loss for {symbol} via _set_position_protection."
        )
...
class VolumaticOBStrategy:
    ...
    ** THIS IS A PLACEHOLDER - FULL IMPLEMENTATION IS REQUIRED **
    ...
    def __init__(
        ...
        self.price_tick = market_info["price_precision_step_decimal"] or Decimal(
            "0.00000001"
        )  # Miniscule default
        ...
        self.vt_vol_ema_len = self.params.get(
            "vt_vol_ema_length", DEFAULT_VT_VOL_EMA_LENGTH
        )
        ...
        self.ob_source = self.params.get(
            "ob_source", DEFAULT_OB_SOURCE
        )  # "Wicks" or "Body"
    ...
    def _find_pivots(
        ...
        if series.empty or left < 0 or right < 0:
    ...
    def update(self, df: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        try:
            # ATR
            atr_series = ta.atr(
                df_analysis["high"].astype(float),
                df_analysis["low"].astype(float),
                df_analysis["close"].astype(float),
                length=self.vt_atr_period,
            )
            df_analysis["atr"] = atr_series.apply(
                lambda x: Decimal(str(x)) if pd.notna(x) else Decimal("NaN")
            )
            current_atr = (
                df_analysis["atr"].iloc[-1]
                if pd.notna(df_analysis["atr"].iloc[-1])
                else None
            )

            # Vol EMA (Placeholder - using close price EMA)
            vol_ema = self._ema_swma(
                df_analysis["close"], self.vt_vol_ema_len
            )  # Placeholder using close
            df_analysis["vol_ema"] = vol_ema

            # VT Bands (Placeholder - using simple +/- ATR)
            mid_band = (
                df_analysis["close"].rolling(self.vt_len).mean()
            )  # Simple MA placeholder
            ...
            upper_band = (
                ...
                if pd.notna(df_analysis["vt_upper"].iloc[-1])
                else None
            )
            lower_band = (
                df_analysis["vt_lower"].iloc[-1]
                if pd.notna(df_analysis["vt_lower"].iloc[-1])
                else None
            )
            ...
            current_trend_up = (
                ...
                if pd.notna(df_analysis["trend_up"].iloc[-1])
                else None
            )
            trend_just_changed = (
            ...
            df_analysis["vol_norm"] = (
                (df_analysis["volume"] - df_analysis["volume"].min())
                / (df_analysis["volume"].max() - df_analysis["volume"].min())
                * 100
                if not df_analysis["volume"].empty
                and df_analysis["volume"].max() > df_analysis["volume"].min()
                else 0
            )
            vol_norm_int = (
                int(df_analysis["vol_norm"].iloc[-1])
                if pd.notna(df_analysis["vol_norm"].iloc[-1])
                else None
            )
        ...
        self.lg.debug(
            f"Strategy update complete for {self.symbol}. Last Close: {last_close}, TrendUp: {current_trend_up}, ATR: {current_atr}"
        )
...
class SignalGenerator:
    ...
    def __init__(
        ...
        self.lg.debug(
            f"  Params: Entry Prox={self.entry_prox_factor}, Exit Prox={self.exit_prox_factor}, "
    ...
    def _calculate_initial_sl_tp(
        ...
        if atr <= 0:
            self.lg.warning(
                f"Cannot calculate SL/TP for {self.symbol}: ATR is zero or negative ({atr})."
            )
            return None, None
        ...
        if initial_sl is not None and initial_sl <= 0:
            self.lg.warning(
                f"Calculated initial SL ({initial_sl.normalize()}) is non-positive for {self.symbol}. Setting SL to None."
            )
            initial_sl = None
        if initial_tp is not None and initial_tp <= 0:
            self.lg.warning(
                f"Calculated initial TP ({initial_tp.normalize()}) is non-positive for {self.symbol}. Setting TP to None."
            )
            initial_tp = None
        ...
        if initial_sl is not None:
            ...
            initial_sl = (initial_sl / self.price_tick).quantize(
                Decimal("1"), rounding
            ) * self.price_tick
            # Ensure SL didn't cross entry after rounding
            if side == "long" and initial_sl >= entry_price:
                initial_sl = entry_price - self.price_tick
            if side == "short" and initial_sl <= entry_price:
                initial_sl = entry_price + self.price_tick
            if initial_sl <= 0:
                initial_sl = None  # Final check after rounding

        if initial_tp is not None:
            rounding = ROUND_UP if side == "long" else ROUND_DOWN
            initial_tp = (initial_tp / self.price_tick).quantize(
                Decimal("1"), rounding
            ) * self.price_tick
            # Ensure TP didn't cross entry after rounding
            if side == "long" and initial_tp <= entry_price:
                initial_tp = entry_price + self.price_tick
            if side == "short" and initial_tp >= entry_price:
                initial_tp = entry_price - self.price_tick
            if initial_tp <= 0:
                initial_tp = None  # Final check after rounding

        self.lg.debug(
            f"Calculated SL/TP for {self.symbol} ({side}): Entry={entry_price.normalize()}, ATR={atr.normalize()} -> SL={initial_sl.normalize() if initial_sl else 'None'}, TP={initial_tp.normalize() if initial_tp else 'None'}"
        )
        ...
        if current_position is None:  # No position open, look for entry
            if trend_up is True and atr and last_close:
                ...
                if initial_sl is None:  # Cannot enter without SL
                    signal = "HOLD"
                    reason = "Placeholder: Trend up, but failed to calculate SL."
                ...
                if initial_sl is None:  # Cannot enter without SL
                    signal = "HOLD"
                    reason = "Placeholder: Trend down, but failed to calculate SL."
            else:
    ...
    ** THIS IS A PLACEHOLDER - FULL IMPLEMENTATION IS REQUIRED **
    ...
    try:
        ...
        if df.empty or len(df) < strategy_engine.min_data_len:
            lg.warning(
                f"Insufficient kline data for {symbol} (got {len(df)}, need ~{strategy_engine.min_data_len}). Skipping analysis."
            )
            return
        ...
        if current_position:
            ...
            if current_position.get(
                "tsl_activated"
            ):  # Use the flag we added in get_open_position
                if not position_state["tsl_activated"]:
                    lg.info(
                        f"Detected active TSL on exchange for {symbol}. Syncing internal state."
                    )
    ...
    ** THIS IS A PLACEHOLDER - FULL IMPLEMENTATION IS REQUIRED **
        ...
        if (
            ...
            if side == "long":
                ...
                if last_close >= be_trigger_price:
                    lg.info(
                        f"{NEON_YELLOW}Break-Even Triggered for {symbol} (Long)! Price {last_close} >= Trigger {be_trigger_price.normalize()}{RESET}"
                    )
                    lg.info(
                        f"  Moving SL to BE Price: {be_target_price.normalize()} (Entry {entry_price} + {be_offset_ticks} ticks)"
                    )
                    # --- Call API to move SL ---
                    success = _set_position_protection(
                        exchange,
                    ...
                    if success:
                        position_state["be_activated"] = (
                            True  # Mark BE as done for this position
                        )
                ...
                if last_close <= be_trigger_price:
                    lg.info(
                        f"{NEON_YELLOW}Break-Even Triggered for {symbol} (Short)! Price {last_close} <= Trigger {be_trigger_price.normalize()}{RESET}"
                    )
                    lg.info(
                        f"  Moving SL to BE Price: {be_target_price.normalize()} (Entry {entry_price} - {be_offset_ticks} ticks)"
                    )
    ...
    if (
        ...
        if entry_price_str and last_close and activation_perc > 0:
            ...
            if (
                side == "long"
                and last_close >= entry_price * (Decimal("1") + activation_perc)
                or side == "short"
                and last_close <= entry_price * (Decimal("1") - activation_perc)
            ):
            ...
            if activation_threshold_met:
                lg.info(
                    f"{NEON_YELLOW}Trailing Stop Activation Threshold Met for {symbol}! Activating TSL...{RESET}"
                )
                ...
                if success:
                    position_state["tsl_activated"] = (
                        True  # Mark TSL as activated internally
                    )
    ...
    if (
        ...
        if close_size > 0:
            ...
            if order_result:
                ...
                lg.error(
                    f"Failed to place closing order for {symbol}. State not reset."
                )
        else:
            lg.error(
                f"Cannot close {symbol}: Invalid position size ({close_size}) in position info."
            )
        ...
        if not enable_trading:
            ...
            return
        close_size = abs(
            current_position["size_decimal"]
        )  # Use positive size for closing order amount
        if close_size > 0:
            order_result = place_trade(
                exchange, symbol, signal, close_size, market_info, lg, reduce_only=True
            )
            ...
            lg.error(
                f"Cannot close {symbol}: Invalid position size ({current_position['size_decimal']}) in position info."
            )

    # --- Entry Logic ---
    elif signal in ["BUY", "SELL"] and current_position is None:
        lg.warning(
            f"{NEON_GREEN}>>> Opening {signal} position for {symbol} due to: {reason}{RESET}"
        )
        if not enable_trading:
        ...
        if current_active_positions >= max_positions:
            lg.warning(
                f"Skipping entry for {symbol}: Max concurrent positions ({max_positions}) reached."
            )
            return
        ...
        if balance is None or initial_sl is None or entry_price_approx is None:
            lg.error(
                f"Cannot calculate position size for {symbol}: Missing balance, initial SL, or entry price."
            )
            return
        ...
        if pos_size is None or pos_size <= 0:
            lg.error(
                f"Position size calculation failed or resulted in zero/negative size for {symbol}. Cannot place order."
            )
            return
        ...
        if leverage > 0 and market_info.get("is_contract"):
            ...
            set_leverage_success = set_leverage_ccxt(
                exchange, symbol, leverage, market_info, lg
            )
            if not set_leverage_success:
                lg.error(
                    f"Failed to set leverage {leverage}x for {symbol}. Aborting entry."
                )
                return
        else:
            lg.debug(
                f"Leverage setting skipped for {symbol} (leverage={leverage}, is_contract={market_info.get('is_contract')})"
            )

        # --- Place Entry Order ---
        entry_order_result = place_trade(
        ...
        if entry_order_result and entry_order_result.get("id"):
            ...
            lg.info(
                f"Waiting {confirm_delay}s after entry order for position confirmation..."
            )
            ...
            if new_position:
                ...
                lg.info(
                    f"Setting initial protection for new {symbol} position: SL={initial_sl.normalize()}, TP={initial_tp.normalize() if initial_tp else 'None'}"
                )
                ...
                if protection_success:
                    ...
                    lg.error(
                        f"Failed to set initial SL/TP for {symbol} after entry! Manual intervention may be required."
                    )
                    # Consider closing the position immediately if SL setup fails? Risky.
            else:
                lg.error(
                    f"Could not confirm new position for {symbol} after entry order to set SL/TP."
                )
        else:
        ...
        lg.info(
            f"Holding position or waiting for signal for {symbol}. Reason: {reason}"
        )
    else:
        lg.debug(
            f"Signal '{signal}' not resulting in action for {symbol} (e.g., trying to enter while position exists)."
        )
...
def main() -> None:
    ...
    signal.signal(
        signal.SIGTERM, _handle_shutdown_signal
    )  # Termination signal (e.g., systemctl stop)

    # --- Initialize Exchange ---
    exchange = initialize_exchange(main_logger)
    if not exchange:
        main_logger.critical(
            "Exchange initialization failed. Bot cannot start. Shutting down."
        )
        sys.exit(1)
    ...
    for pair in trading_pairs_config:
        pair_logger = get_logger_for_symbol(
            pair
        )  # Get logger early for validation messages
        ...
        if market_info and market_info.get("active"):
            pair_logger.info(
                f" -> {NEON_GREEN}{pair} is valid and active on {exchange.id}.{RESET}"
            )
            ...
            try:
                strategy_engines[pair] = VolumaticOBStrategy(
                    CONFIG, market_info, pair_logger
                )
                signal_generators[pair] = SignalGenerator(
                    CONFIG, market_info, pair_logger
                )
                pair_logger.info(
                    f" -> Strategy and Signal Generator initialized for {pair}."
                )
            except ValueError as init_err:
                # Catch specific initialization errors if raised by constructors
                pair_logger.error(
                    f" -> {NEON_RED}Initialization error for {pair}: {init_err}. Skipping this pair.{RESET}"
                )
                ...
                pair_logger.error(
                    f" -> {NEON_RED}Unexpected error initializing strategy/signal generator for {pair}: {init_err}. Skipping.{RESET}",
                    exc_info=True,
                )
                all_pairs_valid = False
                valid_pairs.remove(pair)
            ...
            pair_logger.error(
                f" -> {NEON_RED}{pair} is invalid, inactive, or details could not be fetched. Skipping.{RESET}"
            )
            all_pairs_valid = False

    if not valid_pairs:
        main_logger.critical(
            "No valid trading pairs found after validation. Cannot start trading loop. Exiting."
        )
        sys.exit(1)

    if not all_pairs_valid:
        main_logger.warning(
            f"Some configured pairs were invalid. Proceeding with valid pairs: {valid_pairs}"
        )
    else:
    ...
    if not CONFIG.get("enable_trading", False):
        main_logger.warning(
            f"{NEON_YELLOW}{BRIGHT}--- TRADING IS DISABLED --- (Set 'enable_trading': true in config.json to enable){RESET}"
        )
    else:
        main_logger.warning(
            f"{NEON_RED}{BRIGHT}--- LIVE TRADING IS ENABLED --- Verify configuration carefully!{RESET}"
        )
    ...
    while not _shutdown_requested:
        ...
        for symbol in valid_pairs:
            if _shutdown_requested:
                break  # Check flag before processing each symbol
            ...
            try:
                ...
                symbol_logger.critical(
                    f"{NEON_RED}Authentication Error during main loop for {symbol}: {e}. Stopping bot.{RESET}"
                )
                ...
                symbol_logger.error(
                    f"{NEON_RED}!! Unhandled Exception in main loop for {symbol}: {symbol_err} !!{RESET}",
                    exc_info=True,
                )
            # finally:
            ...
            if _shutdown_requested:
                break  # Check flag again after processing symbol
        ...
        main_logger.info(
            f"Cycle {loop_count} completed in {cycle_duration:.2f}s. Waiting {wait_time:.2f}s for next cycle..."
        )
    ...
    # --- Shutdown Sequence ---
    main_logger.info(
        f"{Fore.MAGENTA}{BRIGHT}--- Pyrmethus Bot Shutting Down Gracefully ---{Style.RESET_ALL}"
    )
...
if __name__ == "__main__":
    try:
        ...
        init_logger.critical(
            f"{NEON_RED}{BRIGHT}FATAL UNHANDLED EXCEPTION in __main__:{RESET} {global_err}",
            exc_info=True,
        )
