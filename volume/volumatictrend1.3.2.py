# --- START OF FILE volumatictrend1.1.6.py ---
...
init(autoreset=True) # Initialize Colorama for console colors
...
DEFAULT_FETCH_LIMIT: int = 750 # Default config value if user doesn't set it (used if less than min_data_len)
...
def setup_logger(name: str) -> logging.Logger:
    ...
    Applies Neon color scheme to console output.
    ...
    """
    safe_name = name.replace('/', '_').replace(':', '-') # Sanitize for filename/logger name
    ...
    # File Handler (DEBUG level, rotating, no colors)
    try:
        ...
        ff = SensitiveFormatter("%(asctime)s - %(levelname)-8s - [%(name)s:%(lineno)d] - %(message)s", datefmt='%Y-%m-%d %H:%M:%S')
        fh.setFormatter(ff)
        fh.setLevel(logging.DEBUG)
        logger.addHandler(fh)
    except Exception as e:
        # Use print for errors during logger setup itself
        print(f"{NEON_RED}Error setting up file logger '{log_filename}': {e}{RESET}")

    # Console Handler (Level from ENV or INFO default, Neon colors)
    try:
        ...
        class NeonConsoleFormatter(SensitiveFormatter):
            def format(self, record):
                level_color = level_colors.get(record.levelno, NEON_BLUE) # Default to blue
                log_fmt = f"{NEON_BLUE}%(asctime)s{RESET} - {level_color}%(levelname)-8s{RESET} - {NEON_PURPLE}[%(name)s]{RESET} - %(message)s"
                formatter = logging.Formatter(log_fmt, datefmt='%Y-%m-%d %H:%M:%S')
                formatter.converter = lambda *args: datetime.now(TIMEZONE).timetuple() # Use timezone
...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    Applies type/range checks to ensure configuration integrity.
    ...
    try: # Validate and merge loaded config with defaults
        ...
        def validate_numeric(cfg, key_path, min_val, max_val, is_strict_min=False, is_int=False, allow_zero=False):
            ...
            if original_val is None: init_logger.warning(f"Config validation error: Key missing '{key_path}'."); return False
            ...
            try:
                ...
                if type(final_val) is not type(original_val) or final_val != original_val:
                     if not isinstance(original_val, (float, int)) or abs(float(original_val) - float(final_val)) > 1e-9:
                        init_logger.info(f"{NEON_YELLOW}Config: Corrected type/value for '{key_path}' from {repr(original_val)} to {repr(final_val)}.{RESET}")
                        corrected = True
            except (ValueError, InvalidOperation, TypeError):
                init_logger.warning(f"{NEON_YELLOW}Config '{key_path}': Invalid value '{original_val}'. Using default {default_val}. Range: {'(' if is_strict_min else '['}{min_val}, {max_val}{']'}{' (or 0)' if allow_zero else ''}{RESET}")
        ...
        validate_numeric(updated_config, "fetch_limit", 100, MAX_DF_LEN, is_int=True) # Allow high fetch_limit request
        validate_numeric(updated_config, "risk_per_trade", 0, 1, is_strict_min=True)
        validate_numeric(updated_config, "leverage", 0, 200, is_int=True)
        ...
        validate_numeric(updated_config, "protection.initial_take_profit_atr_multiple", 0, 100.0, allow_zero=True)

        # Save if corrections/additions were made
        if config_needs_saving:
             try:
                 config_to_save = json.loads(json.dumps(updated_config)) # Convert back to standard types for JSON
    ...
    try:
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if attempt < MAX_API_RETRIES: lg.warning(f"Net error loading markets (Attempt {attempt+1}): {e}. Retrying..."); time.sleep(RETRY_DELAY_SECONDS)
...
# ... (fetch_current_price_ccxt, fetch_klines_ccxt, get_market_info, fetch_balance - unchanged from v1.1.5) ...
...
def fetch_klines_ccxt(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int, logger: logging.Logger) -> pd.DataFrame:
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        ...
        # Correctly access local variable 'e' defined in the except block
        if 'e' in locals() and not isinstance(e, ccxt.RateLimitExceeded): attempts += 1
        if attempts <= MAX_API_RETRIES: time.sleep(RETRY_DELAY_SECONDS) # Delay before next attempt
...
# ... (Rest of functions/classes: get_open_position, set_leverage_ccxt, calculate_position_size, place_trade, _set_position_protection, set_trailing_stop_loss, OrderBlock, StrategyAnalysisResults, VolumaticOBStrategy, SignalGenerator, analyze_and_trade_symbol, main) ...
# --- START COPY FROM v1.1.5 ---
# ... (get_open_position function - with SyntaxError fix applied) ...
        ...
        try:
            ...
            try:
                ...
                lg.debug(f"Fetch positions params: {params}"); positions = exchange.fetch_positions([symbol], params=params)
                 ...
                 else: raise e # Re-raise other exchange errors for retry
            ...
            for pos in positions:
                ...
                try:
                    ...
                    if abs(size) > threshold:
                         active_pos = pos; break # Found active position
                except (ValueError, InvalidOperation, TypeError) as parse_err: # Catch specific parsing errors
                     lg.warning(f"Could not parse/check position size '{size_str}': {parse_err}. Skipping entry.")
                     continue # Skip to the next position entry if parsing fails
                # --- END CORRECTION ---

            if active_pos: # Format and log the found position
                std_pos = active_pos.copy(); info = std_pos.get('info', {})
                std_pos['size_decimal'] = size # Use already parsed size
...
def calculate_position_size(balance: Decimal, risk_per_trade: float, initial_stop_loss_price: Decimal, entry_price: Decimal,
    ...
    if min_amt > 0 and adj_size < min_amt: lg.warning(f"{NEON_YELLOW}Size {adj_size} < min {min_amt}. Adjusting UP.{RESET}"); adj_size = min_amt
    if max_amt < Decimal('inf') and adj_size > max_amt: lg.warning(f"{NEON_YELLOW}Size {adj_size} > max {max_amt}. Adjusting DOWN.{RESET}"); adj_size = max_amt
    ...
    if min_cost > 0 and est_cost < min_cost:
        lg.warning(f"{NEON_YELLOW}Est cost {est_cost} < min {min_cost}. Increasing size.{RESET}"); req_size = None
        try: req_size = (min_cost/(entry_price*contract_size)) if not is_inverse else ((min_cost*entry_price)/contract_size)
        except Exception: pass
        if req_size is None or req_size <= 0: lg.error(f"{NEON_RED}Cannot meet min cost {min_cost}. Abort.{RESET}"); return None
        if max_amt < Decimal('inf') and req_size > max_amt: lg.error(f"{NEON_RED}Cannot meet min cost without exceeding max amt. Abort.{RESET}"); return None
        ...
        lg.warning(f"{NEON_YELLOW}Est cost {est_cost} > max {max_cost}. Reducing size.{RESET}"); adj_size_cost = None
        try: adj_size_cost = (max_cost/(entry_price*contract_size)) if not is_inverse else ((max_cost*entry_price)/contract_size)
        except Exception: pass
        if adj_size_cost is None or adj_size_cost <= 0: lg.error(f"{NEON_RED}Cannot reduce size for max cost {max_cost}. Abort.{RESET}"); return None
    ...
    try: fmt_str = exchange.amount_to_precision(symbol, float(adj_size)); final_size = Decimal(fmt_str); lg.info(f"Applied amt precision (ccxt): {adj_size} -> {final_size}")
        ...
        lg.warning(f"{NEON_YELLOW}ccxt precision error: {fmt_err}. Manual round down.{RESET}");
        ...
        except Exception as manual_err: lg.error(f"{NEON_RED}Manual precision fail: {manual_err}. Using unrounded.{RESET}"); final_size = adj_size
    if final_size <= 0: lg.error(f"{NEON_RED}Final size zero/neg ({final_size}). Abort.{RESET}"); return None
    if min_amt > 0 and final_size < min_amt: lg.error(f"{NEON_RED}Final size {final_size} < min {min_amt}. Abort.{RESET}"); return None
    ...
    if min_cost > 0 and final_cost < min_cost:
         lg.debug(f"Final cost {final_cost} < min cost {min_cost}.")
         try:
             ...
             if valid: lg.warning(f"{NEON_YELLOW}Bumping size to {next_size} for min cost.{RESET}"); final_size = next_size
             else: lg.error(f"{NEON_RED}Final cost below min, next step invalid. Abort.{RESET}"); return None
         except Exception as bump_err: lg.error(f"{NEON_RED}Error bumping size: {bump_err}. Abort.{RESET}"); return None
...
def place_trade(exchange: ccxt.Exchange, symbol: str, trade_signal: str, position_size: Decimal, market_info: Dict,
    ...
    while attempts <= MAX_API_RETRIES:
        ...
        if 'e' in locals() and not isinstance(e, ccxt.RateLimitExceeded): attempts += 1 # Increment only if not rate limited
        if attempts <= MAX_API_RETRIES: time.sleep(RETRY_DELAY_SECONDS * attempts) # Delay before next attempt

    lg.error(f"Failed place {action} order {symbol} after retries."); return None

# ... (_set_position_protection function) ...
def _set_position_protection(exchange: ccxt.Exchange, symbol: str, market_info: Dict, position_info: Dict, logger: logging.Logger,
    ...
    attempts = 0
    while attempts <= MAX_API_RETRIES: # API Call with Retries...
        try:
...
# ... (set_trailing_stop_loss function) ...
...
# ... (VolumaticOBStrategy Class - with manual pivot logic) ...
class VolumaticOBStrategy:
    ...
    def __init__(self, config: Dict[str, Any], market_info: Dict[str, Any], logger: logging.Logger):
        ...
        strategy_cfg = config["strategy_params"]; protection_cfg = config["protection"]
        ...
        self.lg.info(f"{NEON_CYAN}Initializing VolumaticOB Strategy...{RESET}")
        self.lg.info(f"  VT Params: Len={self.vt_length}, ATRLen={self.vt_atr_period}, VolLen={self.vt_vol_ema_length}, ATRMult={self.vt_atr_multiplier.normalize()}")
        self.lg.info(f"  OB Params: Src={self.ob_source}, PH={self.ph_left}/{self.ph_right}, PL={self.pl_left}/{self.pl_right}, Extend={self.ob_extend}, MaxBoxes={self.ob_max_boxes}")
        self.lg.info(f"  Min Data Required: {self.min_data_len} candles")
        if self.min_data_len > BYBIT_API_KLINE_LIMIT + 10: self.lg.error(f"{NEON_RED}{BRIGHT}CONFIG WARNING:{RESET} Strategy requires {self.min_data_len} candles > API limit {BYBIT_API_KLINE_LIMIT}. Reduce lookbacks.")
    ...
    def _find_pivots(self, series: pd.Series, left: int, right: int, high: bool) -> pd.Series:
        """Finds pivot highs (high=True) or lows (high=False) using rolling window comparison."""
        if not isinstance(series, pd.Series) or series.empty or left < 1 or right < 1: return pd.Series(False, index=series.index)
        num_series = pd.to_numeric(series, errors='coerce');
        if num_series.isnull().all(): return pd.Series(False, index=series.index)
        pivots = pd.Series(True, index=series.index)
        for i in range(1, left + 1): pivots &= (num_series > num_series.shift(i)) if high else (num_series < num_series.shift(i))
        for i in range(1, right + 1): pivots &= (num_series > num_series.shift(-i)) if high else (num_series < num_series.shift(-i))
        return pivots.fillna(False)
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        if df_input.empty: self.lg.error("Strategy update: empty DF."); return empty_results
        df = df_input.copy()
        if not isinstance(df.index, pd.DatetimeIndex) or not df.index.is_monotonic_increasing: self.lg.error("DF index invalid."); return empty_results
        if len(df) < self.min_data_len: self.lg.warning(f"Insufficient data ({len(df)} < {self.min_data_len}). Results may be inaccurate.")
        self.lg.debug(f"Starting analysis on {len(df)} candles (min req: {self.min_data_len}).")

        try: # Convert to float for TA Libs
            df_float = pd.DataFrame(index=df.index)
            for col in ['open', 'high', 'low', 'close', 'volume']:
                if col not in df.columns: self.lg.error(f"Missing column '{col}'. Abort."); return empty_results
                df_float[col] = pd.to_numeric(df[col], errors='coerce')
            df_float.dropna(subset=['open', 'high', 'low', 'close'], inplace=True)
            if df_float.empty: self.lg.error("DF empty after float conversion."); return empty_results
        except Exception as e: self.lg.error(f"Error converting to float: {e}", exc_info=True); return empty_results
        ...
        try: # Indicator Calculations
            ...
            # Assign results back to avoid FutureWarnings on slices
            trend_up_series = (df_float['ema2'] > df_float['ema1'].shift(1)).ffill().fillna(False)
            df_float['trend_up'] = trend_up_series
            trend_changed_series = (df_float['trend_up'].shift(1) != df_float['trend_up']) & df_float['trend_up'].notna() & df_float['trend_up'].shift(1).notna()
        ...
        except Exception as e: self.lg.error(f"Error during indicator calc: {e}", exc_info=True); return empty_results
        ...
        try: # Copy results back to Decimal DF
            ...
            for col in cols:
                if col in df_float.columns:
                    src = df_float[col].reindex(df.index);
                    if src.dtype=='bool': df[col] = src.astype(bool)
                    elif pd.api.types.is_object_dtype(src): df[col] = src
                    else: df[col] = src.apply(lambda x: Decimal(str(x)) if pd.notna(x) and np.isfinite(x) else Decimal('NaN'))
        except Exception as e: self.lg.error(f"Error converting results to Decimal: {e}", exc_info=True);

        # Clean Decimal DF
        init_len = len(df); req_cols = ['close', 'atr', 'trend_up', 'upper_band', 'lower_band', 'is_ph', 'is_pl']
        ...
        if df.empty: self.lg.warning("DF empty after indicator calcs."); return empty_results
        self.lg.debug("Indicators calculated. Processing OBs...")

        # --- Order Block Management ---
        try:
            new_ob_count = 0; last_candle_idx = df.index[-1] if not df.empty else None
            if not df.empty:
                 for piv_idx in df.index: # Iterate through candles
                     try:
                         is_ph = df.loc[piv_idx, 'is_ph']; is_pl = df.loc[piv_idx, 'is_pl']
                         if is_ph: # Bearish OB from Pivot High
                             if not any(b['left_idx'] == piv_idx and b['type']=='bear' for b in self.bear_boxes):
                                 c = df.loc[piv_idx]; t,b = (c['high'],c['open']) if self.ob_source=='Wicks' else (max(c['open'],c['close']), min(c['open'],c['close']))
                                 if pd.notna(t) and pd.notna(b) and isinstance(t, Decimal) and isinstance(b, Decimal) and t > b:
                                     nb = OrderBlock(id=f"B_{piv_idx.strftime('%y%m%d%H%M')}", type='bear', left_idx=piv_idx, right_idx=last_candle_idx, top=t, bottom=b, active=True, violated=False)
                                     self.bear_boxes.append(nb); new_ob_count+=1; self.lg.debug(f"  + New Bear OB: {nb['id']} @ {piv_idx.strftime('%H:%M')} [{b.normalize()}-{t.normalize()}]")
                         if is_pl: # Bullish OB from Pivot Low
                             if not any(b['left_idx'] == piv_idx and b['type']=='bull' for b in self.bull_boxes):
                                 c = df.loc[piv_idx]; t,b = (c['open'],c['low']) if self.ob_source=='Wicks' else (max(c['open'],c['close']), min(c['open'],c['close']))
                                 if pd.notna(t) and pd.notna(b) and isinstance(t, Decimal) and isinstance(b, Decimal) and t > b:
                                     nb = OrderBlock(id=f"L_{piv_idx.strftime('%y%m%d%H%M')}", type='bull', left_idx=piv_idx, right_idx=last_candle_idx, top=t, bottom=b, active=True, violated=False)
                                     self.bull_boxes.append(nb); new_ob_count+=1; self.lg.debug(f"  + New Bull OB: {nb['id']} @ {piv_idx.strftime('%H:%M')} [{b.normalize()}-{t.normalize()}]")
                     except Exception as e: self.lg.warning(f"Error processing pivot at {piv_idx}: {e}", exc_info=True)
            if new_ob_count > 0: self.lg.debug(f"Identified {new_ob_count} new OBs.")
            ...
            if last is not None and pd.notna(last.get('close')) and isinstance(last['close'], Decimal):
                ...
                for box in self.bull_boxes:
                    if box['active']:
                        if last_close < box['bottom']: box['active']=False; box['violated']=True; box['right_idx']=last_idx; self.lg.debug(f"Bull OB {box['id']} VIOLATED.")
                        elif self.ob_extend: box['right_idx'] = last_idx
                for box in self.bear_boxes:
                    if box['active']:
                        if last_close > box['top']: box['active']=False; box['violated']=True; box['right_idx']=last_idx; self.lg.debug(f"Bear OB {box['id']} VIOLATED.")
                        elif self.ob_extend: box['right_idx'] = last_idx
            else: self.lg.warning("Cannot check OB violations: Invalid last close.")

            # Prune OBs (Keep N most recent non-violated)
            self.bull_boxes = sorted([b for b in self.bull_boxes if b['active']], key=lambda b: b['left_idx'], reverse=True)[:self.ob_max_boxes] # Filter active here
            self.bear_boxes = sorted([b for b in self.bear_boxes if b['active']], key=lambda b: b['left_idx'], reverse=True)[:self.ob_max_boxes] # Filter active here
            self.lg.debug(f"Pruned OBs. Kept Active: Bull={len(self.bull_boxes)}, Bear={len(self.bear_boxes)}.")
        except Exception as e: self.lg.error(f"Error during OB processing: {e}", exc_info=True);
        ...
        results = StrategyAnalysisResults(
            ...
            active_bull_boxes=self.bull_boxes, active_bear_boxes=self.bear_boxes, # Return pruned active lists
        ...
        self.lg.debug(f"Strategy Results ({time_str}): Close={results['last_close'].normalize()}, Trend={trend_str}, TrendChg={results['trend_just_changed']}, ATR={atr_str}, VolNorm={results['vol_norm_int']}, Active OBs (B/B): {len(results['active_bull_boxes'])}/{len(results['active_bear_boxes'])}")
        return results
        ...
        if pos_side is None:
            if trend_up is True and bull_obs:
                for ob in bull_obs: lb=ob['bottom']; ub=ob['top']*self.ob_entry_proximity_factor
                    if lb <= close <= ub: signal="BUY"; lg.info(f"{NEON_GREEN}{BRIGHT}BUY Signal: Trend UP & Price {close.normalize()} in Bull OB {ob['id']} [{lb.normalize()}-{ub.normalize()}]{RESET}"); break
                ...
                for ob in bear_obs: lb=ob['bottom']/self.ob_entry_proximity_factor if self.ob_entry_proximity_factor > 0 else ob['bottom']; ub=ob['top']
                    if lb <= close <= ub: signal="SELL"; lg.info(f"{NEON_RED}{BRIGHT}SELL Signal: Trend DOWN & Price {close.normalize()} in Bear OB {ob['id']} [{lb.normalize()}-{ub.normalize()}]{RESET}"); break
...
# ... (analyze_and_trade_symbol function - no major changes needed from v1.1.3) ...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger,
    ...
    fetch_limit_request = min(fetch_limit_needed, BYBIT_API_KLINE_LIMIT) # Cap at API limit
    ...
    if klines_df.empty or fetched_count < min_req_data:
        ...
        elif klines_df.empty: lg.error(f"Failed fetch any kline data for {symbol}. Skipping cycle.")
    ...
    lg.info(f"Strategy Analysis: Trend={analysis_results['current_trend_up']}, Last Close={latest_close.normalize()}, ATR={current_atr.normalize()}")

    # --- 5. Get Current Market State ---
    lg.debug("Fetching current price and position...")
    ...
    lg.debug("Generating trading signal...")
    ...
    if not trading_enabled: # Analysis-only mode
        lg.info(f"{NEON_YELLOW}Trading disabled.{RESET} Analysis complete.")
        if open_position is None and signal in ["BUY","SELL"]: lg.info(f"  (Action: Would attempt {signal})")
        elif open_position and signal in ["EXIT_LONG","EXIT_SHORT"]: lg.info(f"  (Action: Would attempt {signal})")
        else: lg.info("  (Action: No entry/exit indicated)")
        cycle_end_time = time.monotonic(); lg.debug(f"---== Analysis Cycle End ({symbol}, {cycle_end_time - cycle_start_time:.2f}s) ==---\n")
        return

    # --- Trading Enabled ---
    lg.debug(f"Trading enabled. Processing signal '{signal}'...")
    ...
    if open_position is None and signal in ["BUY", "SELL"]:
        ...
        if balance is None or balance <= 0: lg.error(f"{NEON_RED}Trade Abort {signal}: No balance {balance}.{RESET}"); return
        tp_calc, sl_calc = signal_generator.calculate_initial_tp_sl(latest_close, signal, current_atr, market_info, exchange)
        if sl_calc is None: lg.error(f"{NEON_RED}Trade Abort {signal}: Initial SL calc failed.{RESET}"); return
        leverage_ok = True
        ...
        if not leverage_ok: lg.error(f"{NEON_RED}Trade Abort {signal}: Leverage set failed.{RESET}"); return
        pos_size = calculate_position_size(balance, config["risk_per_trade"], sl_calc, latest_close, market_info, exchange, lg)
        if pos_size is None or pos_size <= 0: lg.error(f"{NEON_RED}Trade Abort {signal}: Invalid size {pos_size}.{RESET}"); return
        ...
        if trade_order and trade_order.get('id'): # Post-Trade
            ...
            if confirmed_pos:
                try:
                    ...
                    if sl_prot is None: lg.error(f"{NEON_RED}CRITICAL: Failed recalculate SL! Position vulnerable!{RESET}")
                    else:
                        prot_success = False
...
def main() -> None:
    ...
    init_logger.info(f"{BRIGHT}--- Starting Pyrmethus Volumatic OB Bot v1.1.5 ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{RESET}")
    ...
    if CONFIG["enable_trading"]:
        ...
        init_logger.warning(f"  Risk Per Trade: {CONFIG['risk_per_trade']:.2%}, Leverage: {CONFIG['leverage']}x")
        init_logger.warning(f"  Trailing Stop (TSL): {'ENABLED' if prot_cfg['enable_trailing_stop'] else 'DISABLED'}")
        if prot_cfg['enable_trailing_stop']: init_logger.warning(f"    - Callback Rate: {prot_cfg['trailing_stop_callback_rate']:.3%}, Activation Pct: {prot_cfg['trailing_stop_activation_percentage']:.3%}")
        init_logger.warning(f"  Break Even (BE): {'ENABLED' if prot_cfg['enable_break_even'] else 'DISABLED'}")
        if prot_cfg['enable_break_even']: init_logger.warning(f"    - Trigger ATRs: {prot_cfg['break_even_trigger_atr_multiple']}, Offset Ticks: {prot_cfg['break_even_offset_ticks']}")
        init_logger.warning(f"  Initial SL ATR Mult: {prot_cfg['initial_stop_loss_atr_multiple']}")
        tp_mult = prot_cfg.get('initial_take_profit_atr_multiple', 0); init_logger.warning(f"  Initial TP ATR Mult: {tp_mult} {'(Disabled)' if tp_mult == 0 else ''}")
    ...
    else: init_logger.info(f"{NEON_YELLOW}Trading is disabled. Running in analysis-only mode.{RESET}")
    ...
    prot_cfg = CONFIG["protection"]
    symbol_logger.info(f"Key Settings: Risk={CONFIG['risk_per_trade']:.2%}, Lev={CONFIG['leverage']}x, TSL={'ON' if prot_cfg['enable_trailing_stop'] else 'OFF'}, BE={'ON' if prot_cfg['enable_break_even'] else 'OFF'}")
    try:
    ...
    # Main Loop
    symbol_logger.info(f"{BRIGHT}Entering main loop... Press Ctrl+C to stop gracefully.{RESET}")
    loop_count = 0
    try:
        while True:
            ...
            except ccxt.AuthenticationError as e: symbol_logger.critical(f"{NEON_RED}CRITICAL Auth Error: {e}. Stop bot.{RESET}"); break
            ...
            # -----------------------------
            sleep = max(0, loop_delay - elapsed)
        ...
        if 'exchange' in locals() and exchange and hasattr(exchange, 'close'):
            try: init_logger.info("Closing exchange connection..."); init_logger.info("Exchange closed.") # exchange.close() often not needed for sync
        ...
        try: # Close handlers manually just in case
            ...
            loggers_to_close.extend([logging.getLogger(), init_logger]) # Add root and init
            if 'symbol_logger' in locals(): loggers_to_close.append(symbol_logger)
            unique_handlers = set()
            for logger in loggers_to_close:
                 if hasattr(logger, 'handlers'):
                     for handler in logger.handlers: unique_handlers.add(handler)
            for handler in unique_handlers:
                 try: handler.close()
                 except Exception as e: print(f"{NEON_YELLOW} Minor error closing handler {handler}: {e}{RESET}")
        except Exception as log_err: print(f"{NEON_RED}Error shutting down logging system: {log_err}{RESET}")
        print(f"{NEON_YELLOW}{BRIGHT}Bot stopped.{RESET}")

if __name__ == "__main__":
    main()

--- START OF FILE volumatictrend1.3.2.py ---

# --- START OF FILE volumatictrend1.1.5.py ---
...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    try: # Validate and merge loaded config with defaults
        ...
        validate_numeric(updated_config, "strategy_params.vt_vol_ema_length", 1, MAX_DF_LEN, is_int=True) # Validate but allow user setting > API limit (handled later)
        validate_numeric(updated_config, "strategy_params.vt_atr_multiplier", 0.1, 20.0); validate_numeric(updated_config, "strategy_params.ph_left", 1, 100, is_int=True)
        ...
        validate_numeric(updated_config, "protection.trailing_stop_callback_rate", 0.0001, 0.5, is_strict_min=True)
        validate_numeric(updated_config, "protection.trailing_stop_activation_percentage", 0, 0.5, allow_zero=True)
        validate_numeric(updated_config, "protection.break_even_trigger_atr_multiple", 0.1, 10.0); validate_numeric(updated_config, "protection.break_even_offset_ticks", 0, 1000, is_int=True, allow_zero=True)
        validate_numeric(updated_config, "protection.initial_stop_loss_atr_multiple", 0.1, 100.0); validate_numeric(updated_config, "protection.initial_take_profit_atr_multiple", 0, 100.0, allow_zero=True)
...
class VolumaticOBStrategy:
    ...
    def __init__(self, config: Dict[str, Any], market_info: Dict[str, Any], logger: logging.Logger):
        ...
        self.logger.info(f"  Min Data Required: {self.min_data_len} candles")
        if self.min_data_len > BYBIT_API_KLINE_LIMIT + 10: self.lg.error(f"{NEON_RED}{BRIGHT}CONFIG WARNING:{RESET} Strategy requires {self.min_data_len} candles > API limit {BYBIT_API_KLINE_LIMIT}. Reduce lookbacks.")
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        try: # Indicator Calculations
            ...
            df_float['ema1'] = self._ema_swma(df_float['close'], length=self.vt_length)
            df_float['ema2'] = ta.ema(df_float['close'], length=self.vt_length, fillna=np.nan)
            # Address FutureWarning by assigning result back
            trend_up_series = (df_float['ema2'] > df_float['ema1'].shift(1)).ffill().fillna(False)
            df_float['trend_up'] = trend_up_series
            trend_changed_series = (df_float['trend_up'].shift(1) != df_float['trend_up']) & df_float['trend_up'].notna() & df_float['trend_up'].shift(1).notna()
            df_float['trend_changed'] = trend_changed_series.fillna(False)
            ...
            # Pivot Calculation using corrected method
            if self.ob_source.lower() == "wicks": high_s = df_float['high']; low_s = df_float['low']
            else: high_s = df_float[['open', 'close']].max(axis=1); low_s = df_float[['open', 'close']].min(axis=1)
            df_float['is_ph'] = self._find_pivots(high_s, self.ph_left, self.ph_right, high=True)
            df_float['is_pl'] = self._find_pivots(low_s, self.pl_left, self.pl_right, high=False)
        ...
        try:
                     ...
                     try:
                         ...
                         if is_ph: # Bearish OB from Pivot High
                             if not any(b['left_idx'] == piv_idx and b['type']=='bear' for b in self.bear_boxes):
                                 c = df.loc[piv_idx]; t,b = (c['high'],c['open']) if self.ob_source=='Wicks' else (max(c['open'],c['close']), min(c['open'],c['close']))
                                 if pd.notna(t) and pd.notna(b) and isinstance(t, Decimal) and isinstance(b, Decimal) and t > b:
                                     nb = OrderBlock(id=f"B_{piv_idx.strftime('%y%m%d%H%M%S')}", type='bear', left_idx=piv_idx, right_idx=last_candle_idx, top=t, bottom=b, active=True, violated=False)
                                     self.bear_boxes.append(nb); new_ob_count+=1; self.lg.debug(f"  + New Bear OB: {nb['id']} @ {piv_idx.strftime('%H:%M')} [{b.normalize()}-{t.normalize()}]")
                         if is_pl: # Bullish OB from Pivot Low
                             if not any(b['left_idx'] == piv_idx and b['type']=='bull' for b in self.bull_boxes):
                                 c = df.loc[piv_idx]; t,b = (c['open'],c['low']) if self.ob_source=='Wicks' else (max(c['open'],c['close']), min(c['open'],c['close']))
                                 if pd.notna(t) and pd.notna(b) and isinstance(t, Decimal) and isinstance(b, Decimal) and t > b:
                                     nb = OrderBlock(id=f"L_{piv_idx.strftime('%y%m%d%H%M%S')}", type='bull', left_idx=piv_idx, right_idx=last_candle_idx, top=t, bottom=b, active=True, violated=False)
                                     self.bull_boxes.append(nb); new_ob_count+=1; self.lg.debug(f"  + New Bull OB: {nb['id']} @ {piv_idx.strftime('%H:%M')} [{b.normalize()}-{t.normalize()}]")
                     except Exception as e: self.lg.warning(f"Error processing pivot at {piv_idx}: {e}", exc_info=True)
            if new_ob_count > 0: self.lg.debug(f"Identified {new_ob_count} new OBs.")
            ...
            self.bear_boxes = sorted([b for b in self.bear_boxes if b['active']], key=lambda b: b['left_idx'], reverse=True)[:self.ob_max_boxes] # Filter active here
            self.lg.debug(f"Pruned OBs. Kept Active: Bull={len(self.bull_boxes)}, Bear={len(self.bear_boxes)}.")
        except Exception as e: self.lg.error(f"Error during OB processing: {e}", exc_info=True); # Continue
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger,
    ...
    if not trading_enabled:
        ...
        if open_position is None and signal in ["BUY", "SELL"]: lg.info(f"  (Action: Would attempt to {signal} if trading was enabled)")
        elif open_position and signal in ["EXIT_LONG", "EXIT_SHORT"]: lg.info(f"  (Action: Would attempt to {signal} current {open_position['side']} position if trading were enabled)")
        else: lg.info("  (Action: No entry/exit action indicated)")
    ...
    if open_position is None and signal in ["BUY", "SELL"]:
        ...
        tp_calc, sl_calc = signal_generator.calculate_initial_tp_sl(latest_close, signal, current_atr, market_info, exchange)
        if sl_calc is None: lg.error(f"{NEON_RED}Trade Abort {signal}: Initial SL calc failed.{RESET}"); return
        if tp_calc is None: lg.warning(f"{NEON_YELLOW}Initial TP calc failed/disabled.{RESET}")
        leverage_ok = True
        if market_info['is_contract']: leverage=int(config['leverage']); leverage_ok = set_leverage_ccxt(exchange, symbol, leverage, market_info, lg) if leverage > 0 else True
        if not leverage_ok: lg.error(f"{NEON_RED}Trade Abort {signal}: Leverage set failed.{RESET}"); return
        pos_size = calculate_position_size(balance, config["risk_per_trade"], sl_calc, latest_close, market_info, exchange, lg)
    ...
    # --- Scenario 2: Existing Position -> Consider Exit or Manage ---
        ...
        if exit_triggered: # Handle Exit Signal
            ...
            try:
                close_sig = "SELL" if pos_side == 'long' else "BUY"; size_close = abs(pos_size)
                if size_close <= 0: lg.warning("Close size zero/neg. Already closed?"); return
                lg.info(f"===> Placing {signal} Market Order (Reduce Only) | Size: {size_close.normalize()} <===") # Use exit signal in log
                close_order = place_trade(exchange, symbol, signal, size_close, market_info, lg, reduce_only=True) # Pass exit signal
                if close_order and close_order.get('id'): lg.info(f"{NEON_GREEN}Position CLOSE order ({close_order['id']}) placed successfully.{RESET}")
                else: lg.error(f"{NEON_RED}Failed place CLOSE order. Manual check!{RESET}")
            except Exception as close_err: lg.error(f"{NEON_RED}Error closing position: {close_err}{RESET}", exc_info=True); lg.warning(f"{NEON_YELLOW}Manual close may be needed!{RESET}")
        else: # Handle Position Management (BE, TSL)
