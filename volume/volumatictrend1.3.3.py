...
def _ensure_config_keys(
    ...
    for key, default_value in default_config.items():
        ...
        if key not in updated_config:
            ...
            changed = True
            init_logger.info(
                f"{NEON_YELLOW}Config: Added missing key '{full_key_path}' with default: {repr(default_value)}{RESET}"
            )
...
def load_config(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:  # Attempt to recreate the file with defaults if corrupted
            ...
            init_logger.error(
                f"{NEON_RED}Error recreating default config file after decode error: {e_create}. Using internal defaults.{RESET}"
            )
            return default_config
    except Exception as e:
        init_logger.error(
            f"{NEON_RED}Unexpected error loading config file '{filepath}': {e}{RESET}",
            exc_info=True,
        )
        init_logger.warning("Using internal default configuration values.")
        return default_config
    ...
    try:
        ...
        def validate_numeric(
            ...
            if original_val is None:
                init_logger.warning(
                    f"Config validation error: Key missing at '{key_path}'. Should have been added by _ensure_config_keys."
                )
            ...
            try:
                ...
                # Check range
                min_check = (
                    num_val > Decimal(str(min_val))
                    if is_strict_min
                    else num_val >= Decimal(str(min_val))
                )
                ...
                if allow_zero:
                    range_str += " or 0"
                init_logger.warning(
                    f"{NEON_YELLOW}Config '{key_path}': Invalid value '{repr(original_val)}'. Using default: {repr(default_val)}. Expected range: {range_str}{RESET}"
                )
        ...
        if updated_config.get("interval") not in VALID_INTERVALS:
            init_logger.error(
                f"{NEON_RED}Invalid config interval '{updated_config.get('interval')}'. Valid: {VALID_INTERVALS}. Using default '{default_config['interval']}'.{RESET}"
            )
            updated_config["interval"] = default_config["interval"]
            config_needs_saving = True
        validate_numeric(updated_config, "retry_delay", 1, 60, is_int=True)
        validate_numeric(
            updated_config, "fetch_limit", 100, MAX_DF_LEN, is_int=True
        )  # Allow high fetch limit request, capped internally later
        validate_numeric(
            updated_config, "risk_per_trade", 0, 1, is_strict_min=True
        )  # Risk must be > 0 and <= 1
        validate_numeric(
            updated_config, "leverage", 0, 200, is_int=True
        )  # Leverage 0 means no leverage setting attempt
        ...
        validate_numeric(
            updated_config, "protection.break_even_trigger_atr_multiple", 0.1, 10.0
        )
        validate_numeric(
            updated_config,
        ...
        validate_numeric(
            updated_config, "protection.initial_stop_loss_atr_multiple", 0.1, 100.0
        )
        validate_numeric(
            updated_config,
        ...
        init_logger.error(
            f"{NEON_RED}Unexpected error processing configuration: {e}. Using internal defaults.{RESET}",
            exc_info=True,
        )
...
def initialize_exchange(logger: logging.Logger) -> ccxt.Exchange | None:
    ...
    try:
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if exchange.markets and len(exchange.markets) > 0:
                    lg.info(
                        f"Markets loaded successfully ({len(exchange.markets)} symbols found)."
                    )
                    markets_loaded = True
                ...
                if attempt >= MAX_API_RETRIES:
                    lg.critical(
                        f"{NEON_RED}Maximum retries exceeded while loading markets due to network errors. Exiting.{RESET}"
                    )
                    return None
        ...
        if not markets_loaded:
            lg.critical(
                f"{NEON_RED}Failed to load markets for {exchange.id} after all retries. Exiting.{RESET}"
            )
            return None
        ...
        try:
            ...
            if balance_val is not None:
                lg.info(
                    f"{NEON_GREEN}Initial available balance: {balance_val.normalize()} {QUOTE_CURRENCY}{RESET}"
                )
            else:
                # Balance fetch failed after retries within fetch_balance function
                lg.critical(
                    f"{NEON_RED}Initial balance fetch FAILED for {QUOTE_CURRENCY}. Cannot proceed safely.{RESET}"
                )
                if CONFIG.get("enable_trading", False):
                    lg.critical(
                        f"{NEON_RED}Trading is enabled, but balance check failed. Exiting to prevent errors.{RESET}"
                    )
                    return None
                else:
                    lg.warning(
            ...
            if CONFIG.get("enable_trading", False):
                ...
                lg.warning(
                    f"{NEON_YELLOW}Trading is disabled. Proceeding despite balance check error.{RESET}"
                )
...
def fetch_current_price_ccxt(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            def safe_decimal_from_ticker(
                value_str: Any | None, field_name: str
            ) -> Decimal | None:
                ...
                try:
                    ...
                    lg.warning(
                        f"Could not parse ticker field '{field_name}' value '{value_str}' to Decimal."
                    )
                    return None
            ...
            if price is None:
                ...
                if bid and ask:
                    ...
                    price = ask
                    lg.warning(
                        f"{NEON_YELLOW}Using 'ask' price as fallback: {price.normalize()}{RESET}"
                    )
                elif bid:
                    # Use bid price if only bid is valid
                    price = bid
                    lg.warning(
                        f"{NEON_YELLOW}Using 'bid' price as fallback: {price.normalize()}{RESET}"
                    )
            ...
            if price:
                ...
                lg.warning(
                    f"No valid price ('last', 'mid', 'ask', 'bid') found in ticker data (Attempt {attempts + 1}). Ticker: {ticker}"
                )
        ...
        # --- Error Handling with Retries ---
            ...
            lg.critical(
                f"{NEON_RED}Authentication Error fetching price: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
    ...
    lg.error(
        f"{NEON_RED}Failed to fetch current price for {symbol} after {MAX_API_RETRIES + 1} attempts.{RESET}"
    )
    return None
...
def fetch_klines_ccxt(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.debug(
                f"Fetching klines for {symbol} | Timeframe: {timeframe} | Limit: {actual_request_limit} (Attempt {attempts + 1}/{MAX_API_RETRIES + 1})"
            )
            params = {}
            ...
            if ohlcv and fetched_count > 0:
                ...
                if (
                    fetched_count == BYBIT_API_KLINE_LIMIT
                    and limit > BYBIT_API_KLINE_LIMIT
                ):
                    lg.warning(
                        f"{NEON_YELLOW}Hit API kline limit ({BYBIT_API_KLINE_LIMIT}). Strategy might require more data than fetched in a single request.{RESET}"
                    )
                ...
                try:
                    ...
                    if actual_lag_seconds < max_allowed_lag_seconds:
                        lg.debug(
                            f"Last kline timestamp {last_ts} seems recent (Lag: {actual_lag_seconds:.1f}s <= Max: {max_allowed_lag_seconds}s). Data OK."
                        )
                        break  # Successful fetch and basic validation passed, exit retry loop
                    else:
                        lg.warning(
                            f"{NEON_YELLOW}Last kline timestamp {last_ts} is too old (Lag: {actual_lag_seconds:.1f}s > Max: {max_allowed_lag_seconds}s). Data might be stale. Retrying...{RESET}"
                        )
                        ohlcv = None  # Discard potentially stale data and retry
                except Exception as ts_err:
                    lg.warning(
                        f"Could not validate kline timestamp lag: {ts_err}. Proceeding with fetched data."
                    )
                    break  # Proceed even if validation fails, but log warning
            else:
                lg.warning(
                    f"API returned no kline data (Attempt {attempts + 1}). Retrying..."
                )
        ...
        # --- Error Handling with Retries ---
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error fetching klines: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            ...
            if "invalid timeframe" in str(e).lower():
                lg.critical(
                    f"{NEON_RED}Invalid timeframe '{timeframe}' specified for {exchange.id}. Exiting.{RESET}"
                )
                return pd.DataFrame()
        ...
        attempts += 1
        if (
            attempts <= MAX_API_RETRIES and ohlcv is None
        ):  # Only sleep if fetch failed and retries remain
            time.sleep(RETRY_DELAY_SECONDS * attempts)  # Exponential backoff
...
def get_market_info(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if symbol not in exchange.markets:
                if attempts == 0:
                    ...
                    lg.warning(
                        f"Symbol '{symbol}' still not found after market reload (Attempt {attempts + 1}). Retrying check..."
                    )
                    # No need to sleep here yet, retry immediately
                else:
                    # Subsequent attempts failed, assume symbol is invalid
                    lg.error(
                        f"{NEON_RED}Market '{symbol}' not found on {exchange.id} after reload and retries.{RESET}"
                    )
            ...
            if market:
                ...
                if precision.get("price") is None or precision.get("amount") is None:
                    lg.error(
                        f"{NEON_RED}CRITICAL VALIDATION FAILED:{RESET} Market '{symbol}' is missing essential precision data (price or amount). Trading calculations will likely fail. Cannot proceed safely."
                    )
                ...
                lg.error(
                    f"Market dictionary is unexpectedly None for '{symbol}' even though it was found in exchange.markets."
                )
                return None
        ...
        # --- Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded retrieving market info for {symbol} due to network errors.{RESET}"
                )
                return None
        except ccxt.AuthenticationError as e:
            lg.critical(
                f"{NEON_RED}Authentication Error getting market info: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return None  # Fatal error
        except ccxt.ExchangeError as e:
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded retrieving market info for {symbol} due to exchange errors.{RESET}"
                )
                return None
        except Exception as e:
            lg.error(
    ...
    lg.error(
        f"{NEON_RED}Failed to retrieve market info for {symbol} after all attempts.{RESET}"
    )
    return None
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            account_types_to_check = (
                ["UNIFIED", "CONTRACT"] if "bybit" in exchange.id.lower() else [""]
            )  # Check default if not Bybit

            for acc_type in account_types_to_check:
                try:
                    params = {"accountType": acc_type} if acc_type else {}
                    lg.debug(
                        f"Fetching balance for {currency} (Account Type: {acc_type or 'Default'}, Attempt {attempts + 1}/{MAX_API_RETRIES + 1})..."
                    )
                    ...
                    # 2. Bybit V5 structure (often nested in 'info') - Unified/Contract specific
                        ...
                        for account_details in balance_info["info"]["result"]["list"]:
                            ...
                            if (
                                account_details.get("accountType") == acc_type
                                or not acc_type
                                ...
                                if found:
                                    break  # Found in account details
                        if found:
                    ...
                    lg.warning(
                        f"Unexpected error fetching balance for type '{acc_type}': {e}. Trying next type..."
                    )
                    continue  # Try the next account type
            ...
            if (
                not found and "" not in account_types_to_check
            ):  # Only run if default wasn't checked
                try:
                    lg.debug(
                        f"Fetching balance for {currency} (Default/Fallback, Attempt {attempts + 1})..."
                    )
            ...
            if found and balance_str is not None:
                ...
                raise ccxt.ExchangeError(
                    f"Could not find balance information for currency '{currency}' in the response."
                )
        ...
        # --- Error Handling with Retries ---
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error fetching balance: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return None  # Fatal error
        except ccxt.ExchangeError as e:
            last_exception = e
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                ...
                lg.warning(
                    f"Exchange requires fetching all positions ({e}). Filtering locally (potentially slower)."
                )
                params = {
                    "category": category or "linear"
                }  # Default to linear if category unknown
                ...
                if code in no_pos_codes or any(
                    msg in err_str for msg in no_pos_messages
                ):
                    lg.info(
            ...
            if active_position:
                ...
                if not side:
                    lg.error(
                        f"Could not determine side for active position {symbol}. Data: {info}"
                    )
                ...
                std_pos["unrealizedPnl"] = std_pos.get("unrealizedPnl") or info.get(
                    "unrealisedPnl"
                )
                ...
                if sl is not None and str(sl).strip() and Decimal(str(sl)) != 0:
                    std_pos["stopLossPrice"] = str(sl)
                if tp is not None and str(tp).strip() and Decimal(str(tp)) != 0:
                    std_pos["takeProfitPrice"] = str(tp)
                if (
                    tsl_dist is not None
                    and str(tsl_dist).strip()
                    and Decimal(str(tsl_dist)) != 0
                ):
                    std_pos["trailingStopLoss"] = str(tsl_dist)
                if (
                    tsl_act is not None
                    and str(tsl_act).strip()
                    and Decimal(str(tsl_act)) != 0
                ):
                    std_pos["tslActivationPrice"] = str(tsl_act)
                ...
                lg.info(
                    f"{NEON_GREEN}Active {side.upper()} Position Found ({symbol}): "
        ...
        # --- Error Handling with Retries ---
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error fetching positions: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return None  # Fatal error
        except ccxt.ExchangeError as e:
            last_exception = e
...
def set_leverage_ccxt(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                category = (
                    "linear" if market_info.get("linear", True) else "inverse"
                )  # Default linear if unsure
                params = {
                    "category": category,
            ...
            if ret_code is not None:  # Bybit V5 response structure detected
                if ret_code == 0:
                    lg.info(
                        f"{NEON_GREEN}Leverage successfully set for {symbol} to {leverage}x (Bybit Code: 0).{RESET}"
                    )
                    return True
                elif ret_code == 110045:  # "Leverage not modified"
                    lg.info(
                        f"{NEON_YELLOW}Leverage for {symbol} is already {leverage}x (Bybit Code: 110045).{RESET}"
                    )
                    return True
                else:
                ...
                lg.info(
                    f"{NEON_GREEN}Leverage set/confirmed for {symbol} to {leverage}x (No specific Bybit code in response).{RESET}"
                )
                return True
        ...
        # --- Error Handling with Retries ---
            ...
            err_str = str(e).lower()
            lg.error(
                f"{NEON_RED}Exchange error setting leverage for {symbol}: {e} (Code: {err_code}){RESET}"
            )
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for ExchangeError setting leverage.{RESET}"
                )
                return False
            ...
            last_exception = e
            lg.warning(
                f"{NEON_YELLOW}Network error setting leverage for {symbol}: {e}. Retry {attempts + 1}/{MAX_API_RETRIES + 1}...{RESET}"
            )
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for NetworkError setting leverage.{RESET}"
                )
                return False
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error setting leverage: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return False  # Fatal error
    ...
    lg.error(
        f"{NEON_RED}Failed to set leverage for {symbol} to {leverage}x after {MAX_API_RETRIES + 1} attempts. Last error: {last_exception}{RESET}"
    )
    return False
...
def calculate_position_size(
    ...
    if balance <= Decimal("0"):
        lg.error(
            f"Sizing failed: Invalid balance ({balance} {quote_currency}). Must be positive."
        )
        return None
    try:
    ...
    if initial_stop_loss_price <= Decimal("0") or entry_price <= Decimal("0"):
        lg.error(
            f"Sizing failed: Entry price ({entry_price}) and Stop Loss price ({initial_stop_loss_price}) must be positive."
        )
        return None
    if initial_stop_loss_price == entry_price:
        lg.error(
            f"Sizing failed: Stop Loss price ({initial_stop_loss_price}) cannot be equal to Entry price ({entry_price})."
        )
        return None
    ...
    try:
        ...
        if amount_precision_step <= Decimal("0"):
            raise ValueError(
                f"Invalid amount precision step size: {amount_precision_step}"
            )
        ...
        )  # Minimum order value (size * price)
        max_cost = (
        ...
        contract_size = (
            Decimal(str(contract_size_str)) if contract_size_str else Decimal("1")
        )
    ...
    if stop_loss_distance_price <= Decimal("0"):
        ...
        lg.error(
            f"Sizing failed: Stop Loss distance is zero or negative ({stop_loss_distance_price})."
        )
        return None
    ...
    try:
        if not is_inverse:  # Linear Contracts or Spot
            ...
            lg.debug(
                f"  Linear/Spot Calc: RiskAmt={risk_amount_quote} / (SLDist={stop_loss_distance_price} * ContrSize={contract_size}) = {calculated_size}"
            )
            ...
            lg.debug(
                f"  Inverse Calc: RiskAmt={risk_amount_quote} / (ContrSize={contract_size} * InvFactor={inverse_factor}) = {calculated_size}"
            )
    ...
    if calculated_size <= Decimal("0"):
        lg.error(
            f"Sizing failed: Initial calculated size is zero or negative ({calculated_size.normalize()}). Check inputs/risk."
        )
        return None
    ...
    try:
        if entry_price > 0:
            ...
            estimated_cost = (
                (adjusted_size * entry_price * contract_size)
                if not is_inverse
                else ((adjusted_size * contract_size) / entry_price)
            )
            lg.debug(
                f"  Estimated Cost for adjusted size: {estimated_cost.normalize()} {quote_currency}"
            )
    ...
    if min_cost > 0 and estimated_cost > 0 and estimated_cost < min_cost:
        lg.warning(
            f"{NEON_YELLOW}Estimated cost {estimated_cost.normalize()} is below minimum cost {min_cost.normalize()}. Attempting to increase size.{RESET}"
        )
        ...
        try:
            ...
            if required_size_for_min_cost is None or required_size_for_min_cost <= 0:
                raise ValueError("Invalid required size calc")
            lg.info(
            ...
            if max_amount < Decimal("inf") and required_size_for_min_cost > max_amount:
                lg.error(
                    f"{NEON_RED}Cannot meet minimum cost ({min_cost.normalize()}) without exceeding maximum amount limit ({max_amount.normalize()}). Aborting sizing.{RESET}"
                )
                return None
            ...
            lg.error(
                f"{NEON_RED}Failed to calculate size needed for minimum cost: {cost_calc_err}. Aborting sizing.{RESET}"
            )
            return None

    elif max_cost < Decimal("inf") and estimated_cost > 0 and estimated_cost > max_cost:
        lg.warning(
            f"{NEON_YELLOW}Estimated cost {estimated_cost.normalize()} exceeds maximum cost {max_cost.normalize()}. Attempting to reduce size.{RESET}"
        )
        ...
        try:
            ...
            if max_size_for_max_cost is None or max_size_for_max_cost <= 0:
                raise ValueError("Invalid max size calc")
            lg.info(
            ...
            lg.error(
                f"{NEON_RED}Failed to calculate maximum size allowed by maximum cost: {cost_calc_err}. Aborting sizing.{RESET}"
            )
            return None
    ...
    try:
        ...
        if final_size != adjusted_size:
            lg.info(
                f"Applied amount precision (CCXT): {adjusted_size.normalize()} -> {final_size.normalize()} {size_unit}"
            )
    except Exception as fmt_err:
        lg.warning(
            f"{NEON_YELLOW}CCXT amount_to_precision failed: {fmt_err}. Attempting manual rounding down to step size.{RESET}"
        )
        try:
            ...
            if amount_precision_step > 0:
                ...
                if final_size != adjusted_size:
                    lg.info(
                        f"Applied manual amount precision (floor): {adjusted_size.normalize()} -> {final_size.normalize()} {size_unit}"
                    )
            else:
                # Should not happen based on earlier check, but handle defensively
                lg.error(
                    "Amount precision step is zero or negative, cannot apply manual precision."
                )
                final_size = adjusted_size  # Use unrounded
        except Exception as manual_err:
            lg.error(
                f"{NEON_RED}Manual precision application failed: {manual_err}. Using unrounded size.{RESET}"
            )
    ...
    if final_size <= Decimal("0"):
        lg.error(
            f"{NEON_RED}Final size after precision is zero or negative ({final_size.normalize()}). Aborting sizing.{RESET}"
        )
        return None
    if min_amount > 0 and final_size < min_amount:
        lg.error(
            f"{NEON_RED}Final size {final_size.normalize()} is below minimum amount {min_amount.normalize()} after precision adjustment. Aborting sizing.{RESET}"
        )
    ...
    if max_amount < Decimal("inf") and final_size > max_amount:
        lg.error(
            f"{NEON_RED}Final size {final_size.normalize()} exceeds maximum amount {max_amount.normalize()} after precision. Aborting sizing.{RESET}"
        )
    ...
    try:
        if entry_price > 0:
            final_cost = (
                ...
                if not is_inverse
                else ((final_size * contract_size) / entry_price)
            )
            lg.debug(
                f"  Final Estimated Cost: {final_cost.normalize()} {quote_currency}"
            )
    except Exception:
        pass
    ...
    if min_cost > 0 and final_cost > 0 and final_cost < min_cost:
        ...
        try:
            ...
            if entry_price > 0:
                next_step_cost = (
                    (next_step_size * entry_price * contract_size)
                    if not is_inverse
                    else ((next_step_size * contract_size) / entry_price)
                )

            # Check if bumping up is valid (meets min cost, doesn't exceed max amount/cost)
            can_bump_up = (
                (next_step_cost >= min_cost)
                and (max_amount == Decimal("inf") or next_step_size <= max_amount)
            ...
            if can_bump_up:
                ...
                lg.error(
                    f"{NEON_RED}Cannot meet minimum cost even by bumping size one step due to other limits. Aborting sizing.{RESET}"
                )
                return None
        except Exception as bump_err:
            lg.error(
                f"{NEON_RED}Error trying to bump size for min cost: {bump_err}. Aborting sizing.{RESET}"
            )
            return None
...
def place_trade(
    ...
    if position_size <= Decimal("0"):
        lg.error(
            f"Invalid position size provided to place_trade: {position_size}. Must be positive."
        )
        return None
    ...
    settle_currency = (
        market_info.get("settle", quote_currency) if is_contract else base_currency
    )
    ...
    lg.info(
        f"===> Attempting {action_desc} | {side.upper()} {order_type.upper()} Order | {symbol} | Size: {position_size.normalize()} {size_unit} <==="
    )
    if order_params:
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if avg_price_str != "N/A":
                log_msg += f", Avg Fill Price: ~{avg_price_str}"
            if filled_amount_str != "N/A":
                log_msg += f", Filled Amount: {filled_amount_str}"
            lg.info(log_msg)
        ...
        # --- Error Handling with Retries ---
            ...
            last_exception = e
            lg.error(
                f"{NEON_RED}Order Failed: Insufficient funds for {symbol} {side} {position_size}. Error: {e}{RESET}"
            )
            return None  # Non-retryable
        except ccxt.InvalidOrder as e:
            last_exception = e
            lg.error(
                f"{NEON_RED}Order Failed: Invalid order parameters for {symbol}. Error: {e}{RESET}"
            )
            ...
            lg.error(
                f"{NEON_RED}Order Failed: Exchange error placing order for {symbol}. Error: {e} (Code: {err_code}){RESET}"
            )
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for ExchangeError placing order.{RESET}"
                )
                return None
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for NetworkError placing order.{RESET}"
                )
                return None
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error placing order: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return None  # Fatal error
...
def _set_position_protection(
    ...
    **Important:** This uses a direct API call (`private_post`) and relies on Bybit's specific V5 endpoint and parameters.
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"Protection setting failed for {symbol}: Invalid position side ('{pos_side}') or entry price ('{entry_price_str}')."
        )
        return False
    try:
        ...
        lg.error(
            f"Protection setting failed for {symbol}: Invalid entry price format or value '{entry_price_str}': {e}"
        )
        return False
    ...
    try:
        ...
        if min_tick <= 0:
            raise ValueError("Invalid price precision (tick size)")
        ...
        def format_price_param(
            ...
            if price_decimal is None:
                return None
            if price_decimal == 0:
                return "0"  # Allow "0" to clear protection
            if price_decimal < 0:
                lg.warning(
                    f"Invalid negative price {price_decimal} provided for {param_name}. Ignoring."
                )
                return None
            try:
                ...
                lg.error(
                    f"Failed to format {param_name} value {price_decimal} to exchange precision: {e}."
                )
                return None
        ...
        if isinstance(trailing_stop_distance, Decimal):
            ...
            if trailing_stop_distance > 0:  # Setting an active TSL
                if (
                    ...
                    lg.error(
                        f"TSL request failed: Valid activation price ({tsl_activation_price}) is required when TSL distance ({trailing_stop_distance}) > 0."
                    )
                else:
                    ...
                    if not is_valid_activation:
                        ...
                        if fmt_distance and fmt_activation:
                            ...
                            lg.error(
                                f"TSL request failed: Could not format TSL parameters (Distance: {fmt_distance}, Activation: {fmt_activation})."
                            )
        ...
        if not (
            set_tsl and trailing_stop_distance > 0
        ):  # Allow SL clear even if TSL clear requested
    ...
    if not params_to_set:
        if any_protection_requested:
            lg.warning(
                f"No valid protection parameters to set for {symbol} after formatting/validation. No API call made."
            )
            # Return False because the requested action couldn't be fulfilled
            return False
        else:
            lg.debug(
                f"No protection changes requested for {symbol}. No API call needed."
            )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if ret_code == 0:
                ...
                fatal_protect_codes = [
                    ...
                    10002,
                    110043,
                ]  # 110043 = Position closed
                is_fatal = (
                    ret_code in fatal_protect_codes
                    or "invalid" in ret_msg.lower()
                    or "parameter" in ret_msg.lower()
                    or "cannot be" in ret_msg.lower()
                )  # General phrases indicating invalid input

                if is_fatal:
                    lg.error(
                        f"{NEON_RED} >> Hint: This appears to be a non-retryable protection setting error.{RESET}"
                    )
        ...
        # --- Standard CCXT Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for NetworkError setting protection.{RESET}"
                )
                return False
            ...
            last_exception = e
            lg.critical(
                f"{NEON_RED}Authentication Error setting protection: {e}. Check API Key/Secret/Permissions. Stopping.{RESET}"
            )
            return False  # Fatal error

        except (
            ccxt.ExchangeError
        ) as e:  # Catch re-raised Bybit errors or other CCXT exchange errors
            last_exception = e
            lg.warning(
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Maximum retries exceeded for ExchangeError setting protection.{RESET}"
                )
                return False
...
def set_trailing_stop_loss(
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"TSL calculation failed for {symbol}: Invalid position side ('{pos_side}') or entry price ('{entry_price_str}')."
        )
        return False
    ...
    try:
        ...
        if not (
            ...
            raise ValueError(
                "Invalid input values (entry, callback rate, activation %, or tick size)."
            )

    except (KeyError, ValueError, InvalidOperation, TypeError) as e:
        lg.error(
            f"TSL calculation failed for {symbol}: Invalid configuration or market info: {e}"
        )
        return False
    ...
    try:
        ...
        if tsl_activation_price <= 0:
            lg.error(
                f"TSL calculation failed: Calculated Activation Price ({tsl_activation_price.normalize()}) is zero or negative."
            )
            return False
        ...
        tsl_trailing_distance = max(
            raw_trailing_distance.quantize(price_tick, ROUND_UP), price_tick
        )
...
class VolumaticOBStrategy:
    ...
    def __init__(
        ...
        if (
            ...
            self.lg.error(
                f"{NEON_RED}{BRIGHT}CONFIGURATION WARNING:{RESET} Strategy requires {self.min_data_len} candles, which may exceed the API fetch limit ({BYBIT_API_KLINE_LIMIT}). "
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        if (
            ...
            self.lg.error(
                "Strategy update failed: DataFrame index is not a monotonic DatetimeIndex."
            )
            return empty_results
        if len(df) < self.min_data_len:
            # Proceed but warn if data is insufficient
            self.lg.warning(
                f"Strategy update: Insufficient data ({len(df)} candles < {self.min_data_len} required). Results may be inaccurate or incomplete."
            )
        self.lg.debug(
        ...
        try:
            ...
            for col in required_cols:
                if col not in df.columns:
                    self.lg.error(
                        f"Strategy update failed: Missing required column '{col}' in input DataFrame."
                    )
                    return empty_results
                df_float[col] = pd.to_numeric(
        ...
        try:
            ...
            trend_up_series = (df_float["ema2"] > df_float["ema1"].shift(1)).fillna(
                False
            )
            ...
            df_float["vol_norm"] = np.where(
                ...
                0.0,
            )  # Assign 0 if max volume is zero
        ...
        try:
                ...
                try:
                    ...
                    if is_pivot_high:
                        ...
                        if not any(
                            ...
                            for ob in self.bear_boxes
                        ):
                            # Determine top/bottom based on OB source config
                            if self.ob_source.lower() == "wicks":
                                ob_top = candle.get("high")
                                ob_bottom = candle.get(
                                    "open"
                                )  # Bearish OB uses high and open of PH candle wick
                            else:  # Body
                            ...
                            if (
                                ...
                                new_ob_count += 1
                                self.lg.debug(
                                    f"  + New Bearish OB identified: {ob_id} @ {timestamp.strftime('%H:%M')} [Top:{ob_top.normalize()}, Bot:{ob_bottom.normalize()}]"
                                )
                            else:
                                self.lg.warning(
                                    f"Could not create Bear OB at {timestamp}: Invalid top/bottom values (Top: {ob_top}, Bottom: {ob_bottom})."
                                )
                    ...
                    if is_pivot_low:
                        if not any(
                            ...
                            for ob in self.bull_boxes
                        ):
                            if self.ob_source.lower() == "wicks":
                                ob_top = candle.get(
                                    "open"
                                )  # Bullish OB uses open and low of PL candle wick
                            ...
                            if (
                                ...
                                new_ob_count += 1
                                self.lg.debug(
                                    f"  + New Bullish OB identified: {ob_id} @ {timestamp.strftime('%H:%M')} [Top:{ob_top.normalize()}, Bot:{ob_bottom.normalize()}]"
                                )
                            else:
                                self.lg.warning(
                                    f"Could not create Bull OB at {timestamp}: Invalid top/bottom values (Top: {ob_top}, Bottom: {ob_bottom})."
                                )
            ...
            if new_ob_count > 0:
                self.lg.debug(
                    f"Identified {new_ob_count} new Order Blocks in this update."
                )
            ...
            if isinstance(last_close, Decimal) and pd.notna(last_close):
                for box in self.bull_boxes:
                    if box["active"]:
                        ...
                        if last_close < box["bottom"]:
                            ...
                            self.lg.debug(
                                f"Bullish OB {box['id']} VIOLATED by close {last_close.normalize()} < {box['bottom'].normalize()}"
                            )
                ...
                for box in self.bear_boxes:
                    if box["active"]:
                        ...
                        if last_close > box["top"]:
                            ...
                            box["right_idx"] = last_candle_idx
                            self.lg.debug(
                                f"Bearish OB {box['id']} VIOLATED by close {last_close.normalize()} > {box['top'].normalize()}"
                            )
                        # Extend active OB
                        elif self.ob_extend:
                            box["right_idx"] = last_candle_idx
                ...
                self.lg.warning(
                    "Cannot check Order Block violations: Invalid last close price."
                )
        ...
        analysis_results = StrategyAnalysisResults(
            ...
            last_close=safe_decimal_from_candle(last_candle_final, "close")
            or Decimal("0"),
            current_trend_up=bool(last_candle_final["trend_up"])
            if last_candle_final is not None
            and isinstance(last_candle_final.get("trend_up"), (bool, np.bool_))
            else None,
            trend_just_changed=bool(last_candle_final["trend_changed"])
            if last_candle_final is not None
            and isinstance(last_candle_final.get("trend_changed"), (bool, np.bool_))
            else False,
            active_bull_boxes=self.bull_boxes,  # Return the pruned list of active OBs
            active_bear_boxes=self.bear_boxes,
            vol_norm_int=int(v)
            if (v := safe_decimal_from_candle(last_candle_final, "vol_norm"))
            is not None
            else None,
        ...
        self.lg.debug(
            f"  Trend: {trend_str} (Changed: {analysis_results['trend_just_changed']})"
        )
...
class SignalGenerator:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger) -> None:
        ...
        try:
            ...
            self.lg.info(
                f"  OB Entry Proximity Factor: {self.ob_entry_proximity_factor:.3f}"
            )
            self.lg.info(
                f"  OB Exit Proximity Factor: {self.ob_exit_proximity_factor:.3f}"
            )
            self.lg.info(
            ...
            self.lg.error(
                f"{NEON_RED}Error initializing SignalGenerator parameters from config: {e}. Using hardcoded defaults.{RESET}",
                exc_info=True,
            )
    ...
    def generate_signal(
        ...
        if (
            ...
            lg.warning(
                f"{NEON_YELLOW}Signal Generation: Invalid or incomplete strategy analysis results provided. Defaulting to HOLD.{RESET}"
            )
            return "HOLD"
        ...
        lg.debug(
            f"  Input: Close={last_close.normalize()}, TrendUp={trend_is_up}, TrendChanged={trend_changed}, Position={position_side or 'None'}"
        )
        lg.debug(
        ...
        if position_side == "long":
            ...
            # Exit Long if price nears a bearish OB
                ...
                signal == "HOLD" and active_bear_obs
            ):  # Check only if not already exiting
                try:
    ...
    def calculate_initial_tp_sl(
        ...
        if entry_price <= 0 or atr <= 0:
            lg.error(
                f"TP/SL Calc Failed: Entry price ({entry_price}) and ATR ({atr}) must be positive."
            )
            return None, None
        try:
            price_tick_str = market_info["precision"]["price"]
            ...
            def format_level(
                ...
                try:
                    ...
                    if formatted_decimal > 0:
                        ...
                        lg.warning(
                            f"Formatted {level_name} ({formatted_str}) is zero or negative. Ignoring."
                        )
                        return None
                except Exception as e:
                    lg.error(
                        f"Error formatting {level_name} value {price_decimal} to exchange precision: {e}."
                    )
                    return None
...
def analyze_and_trade_symbol(
    ...
    if not ccxt_interval:
        lg.critical(
            f"Invalid interval '{config['interval']}' configured. Cannot map to CCXT timeframe. Skipping cycle."
        )
        return
    ...
    if (
        ...
        lg.error(
            f"{NEON_RED}Strategy analysis did not produce valid results (missing trend, close price, or ATR). Skipping cycle.{RESET}"
        )
    ...
    if current_market_price is None:
        lg.warning(
            f"{NEON_YELLOW}Failed to fetch live market price. Using last kline close price ({latest_close.normalize()}) for protection checks.{RESET}"
        )
    ...
    if open_position is None and signal in ["BUY", "SELL"]:
        ...
        if balance is None or balance <= 0:
            lg.error(
                f"{NEON_RED}Trade Aborted ({signal}): Cannot fetch valid balance ({balance}) for {QUOTE_CURRENCY}.{RESET}"
            )
            return
        ...
        if market_info["is_contract"]:
            ...
            if leverage_to_set > 0:
                ...
                lg.info(
                    "Leverage setting skipped (config leverage is 0 or invalid). Using exchange default."
                )
            ...
            if confirmed_position:
                try:
                    ...
                    if prot_sl_calc is None:
                        ...
                        lg.error(
                            f"{NEON_RED}{BRIGHT}CRITICAL ERROR: Position entered, but failed to recalculate SL based on entry price {entry_price_actual.normalize()}! POSITION IS UNPROTECTED!{RESET}"
                        )
                    else:
                        ...
                        if prot_cfg.get("enable_trailing_stop", True):
                            ...
                            lg.info(
                                "No protection (TSL or Fixed SL/TP) enabled in config. Position entered without protection."
                            )
                    ...
                    lg.warning(
                        f"{NEON_YELLOW}Position is confirmed open for {symbol}, but may lack protection! Manual check recommended!{RESET}"
                    )
            else:
                # Order placed but position not found after delay
                lg.error(
                    f"{NEON_RED}Order {trade_order['id']} was placed, but FAILED TO CONFIRM open position for {symbol} after {confirm_delay}s delay! Manual check required!{RESET}"
                )
        else:
    ...
    # --- Scenario 2: Existing Position -> Consider Exit or Manage ---
        ...
        if exit_triggered:
            ...
            try:
                ...
                if size_to_close <= 0:
                    lg.warning(
                        f"Attempting to close {pos_side} position, but parsed size is {pos_size_decimal}. Position might already be closed or data is stale."
                    )
            ...
            try:
                tsl_active = (
                    open_position.get("trailingStopLoss")
                    and Decimal(str(open_position["trailingStopLoss"])) > 0
                )
            ...
            try:
                current_tp_str = open_position.get("takeProfitPrice")
                current_tp = (
                    Decimal(str(current_tp_str))
            ...
            if (
                ...
                lg.debug(
                    f"  BE Check Inputs: Entry={entry_price.normalize()}, CurrentPrice={price_for_checks.normalize()}, ATR={current_atr.normalize()}, CurrentSL={current_sl}"
                )
                try:
                    ...
                    if not (
                        be_trigger_atr_multiple > 0
                        and be_offset_ticks >= 0
                        and price_tick > 0
                    ):
                        raise ValueError(
                            "Invalid BE config parameters (trigger multiple, offset ticks, or price tick)."
                        )
                    ...
                    if profit_in_atr >= be_trigger_atr_multiple:
                        ...
                        if pos_side == "long":
                            be_stop_loss = be_stop_loss_raw.quantize(
                                price_tick, ROUND_UP
                            )
                        else:  # Short
                            be_stop_loss = be_stop_loss_raw.quantize(
                        ...
                        if be_stop_loss and be_stop_loss > 0:
                            ...
                            if current_sl is None:
                                ...
                                lg.debug(
                                    f"  Current SL ({current_sl.normalize()}) is already at or better than calculated BE SL ({be_stop_loss.normalize()}). No update needed."
                                )
                            ...
                            lg.error(
                                f"{NEON_RED}Break-Even triggered, but calculated BE Stop Loss level ({be_stop_loss}) is invalid.{RESET}"
                            )
                    else:
                ...
                lg.debug(
                    f"Break-Even check skipped: {'TSL is active' if tsl_active else 'Missing required data (entry price, ATR, or current price)'}."
                )
            else:  # BE disabled
            ...
            if tsl_enabled and not tsl_active and entry_price and current_atr:
                ...
                lg.debug(
                    f"TSL setup/recovery check skipped: {'TSL already active' if tsl_active else 'Missing required data (entry price or ATR)'}."
                )
            else:  # TSL disabled
    ...
    # --- Scenario 4: Unhandled State (Should not normally happen) ---
        ...
        lg.error(
            f"Unhandled state encountered: Signal='{signal}', Position Side='{open_position.get('side') if open_position else None}'. No action defined."
        )
...
def main() -> None:
    ...
    init_logger.info(
        f"{BRIGHT}--- Starting Pyrmethus Volumatic OB Bot v1.1.6 ---{RESET}"
    )
    init_logger.info(
        f"Timestamp: {datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}"
    )
    init_logger.info(
        f"Loaded Configuration: Quote Currency={QUOTE_CURRENCY}, Trading Enabled={CONFIG['enable_trading']}, Sandbox Mode={CONFIG['use_sandbox']}"
    )
    try:
        ...
        ta_version = getattr(
            ta, "version", "N/A"
        )  # pandas_ta version attribute might vary
        np_version = getattr(np, "__version__", "N/A")
        init_logger.info(
            f"Versions: Python={py_version}, CCXT={ccxt_version}, Pandas={pd_version}, Numpy={np_version}, Pandas-TA={ta_version}"
        )
    except Exception as e:
    ...
    if CONFIG.get("enable_trading", False):
        ...
        init_logger.warning(
            f"║ {Fore.RED}{BRIGHT}!!!           LIVE TRADING IS ENABLED           !!!{RESET}{NEON_YELLOW}{BRIGHT} ║"
        )
        init_logger.warning(
            f"╚═════════════════════════════════════════════════════════════╝{RESET}"
        )
        mode_str = (
            f"{NEON_RED}!!! LIVE (REAL FUNDS AT RISK) !!!"
            if not CONFIG.get("use_sandbox")
            else f"{NEON_GREEN}SANDBOX (Testnet Funds)"
        )
        ...
        prot_cfg = CONFIG.get("protection", {})
        init_logger.warning(
            f"{BRIGHT}--- Please Review Key Trading Settings ---{RESET}"
        )
        init_logger.warning(
            f"  Risk Per Trade: {CONFIG.get('risk_per_trade', 0.01):.2%}"
        )
        init_logger.warning(
            f"  Leverage: {CONFIG.get('leverage', 0)}x (0 uses exchange default)"
        )
        init_logger.warning(
            f"  Trailing Stop (TSL): {'ENABLED' if prot_cfg.get('enable_trailing_stop', False) else 'DISABLED'}"
        )
        if prot_cfg.get("enable_trailing_stop", False):
            init_logger.warning(
                f"    - Callback Rate: {prot_cfg.get('trailing_stop_callback_rate', 0):.3%}"
            )
            init_logger.warning(
                f"    - Activation Pct: {prot_cfg.get('trailing_stop_activation_percentage', 0):.3%}"
            )
        init_logger.warning(
            f"  Break Even (BE): {'ENABLED' if prot_cfg.get('enable_break_even', False) else 'DISABLED'}"
        )
        if prot_cfg.get("enable_break_even", False):
            init_logger.warning(
                f"    - Trigger ATRs: {prot_cfg.get('break_even_trigger_atr_multiple', 0)}"
            )
            init_logger.warning(
                f"    - Offset Ticks: {prot_cfg.get('break_even_offset_ticks', 0)}"
            )
        init_logger.warning(
            f"  Initial SL ATR Multiple: {prot_cfg.get('initial_stop_loss_atr_multiple', 0)}"
        )
        tp_mult = prot_cfg.get("initial_take_profit_atr_multiple", 0)
        init_logger.warning(
            f"  Initial TP ATR Multiple: {tp_mult} {'(Disabled)' if tp_mult == 0 else ''}"
        )
        ...
        init_logger.info(
            f"{NEON_YELLOW}Trading is DISABLED in config.json. Running in analysis-only mode.{RESET}"
        )
    ...
    if not exchange:
        init_logger.critical(
            "Exchange initialization failed. Cannot continue. Exiting."
        )
        logging.shutdown()
    ...
    while target_symbol is None:
        try:
            ...
            symbol_input = (
                input(
                    f"{NEON_YELLOW}Enter the trading symbol (e.g., BTC/USDT, ETH/USD:ETH): {RESET}"
                )
                .strip()
                .upper()
            )
            ...
            if m_info:
                target_symbol = m_info[
                    "symbol"
                ]  # Use the standardized symbol from CCXT
                market_info = m_info
                init_logger.info(
                ...
                if (
                    ...
                    init_logger.critical(
                        f"{NEON_RED}CRITICAL ERROR:{RESET} Market '{target_symbol}' is missing essential price or amount precision data after validation! Cannot trade safely. Exiting."
                    )
                    logging.shutdown()
                    return
                ...
                init_logger.error(
                    f"{NEON_RED}Symbol '{symbol_input}' could not be validated or market info retrieval failed. Please try again.{RESET}"
                )
                init_logger.info(
                    "Common formats: BASE/QUOTE (e.g., BTC/USDT), BASE/QUOTE:SETTLE (e.g., BTC/USDT:USDT for linear, ETH/USD:ETH for inverse)"
                )
            ...
            init_logger.error(
                f"Unexpected error during symbol validation: {e}", exc_info=True
            )
    ...
    if market_info is None:
        init_logger.critical(
            "Market info is unexpectedly None after symbol selection. Exiting."
        )
        logging.shutdown()
        return
    ...
    while selected_interval is None:
        ...
        interval_input = input(
            f"{NEON_YELLOW}Enter timeframe {VALID_INTERVALS} (default: {default_tf}): {RESET}"
        ).strip()
        ...
        if interval_input in VALID_INTERVALS:
            ...
            init_logger.error(
                f"{NEON_RED}Invalid timeframe '{interval_input}'. Please choose from: {VALID_INTERVALS}{RESET}"
            )
    ...
    symbol_logger.info(
        f"Key Protections: TSL={'ON' if prot_cfg_log.get('enable_trailing_stop') else 'OFF'}, BE={'ON' if prot_cfg_log.get('enable_break_even') else 'OFF'}"
    )
    ...
    try:
        ...
        symbol_logger.critical(
            f"Failed to initialize strategy engine or signal generator: {engine_err}. Exiting.",
            exc_info=True,
        )
        logging.shutdown()
        return
    ...
    try:
        while True:
            ...
            loop_count += 1
            symbol_logger.debug(
                f">>> Loop #{loop_count} Start: {datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}"
            )
            ...
            # --- Handle Specific Exceptions within the Loop ---
                ...
                symbol_logger.warning(
                    f"{NEON_YELLOW}Rate limit exceeded during main loop: {e}. Waiting 60 seconds...{RESET}"
                )
                time.sleep(60)
            except (
                ccxt.NetworkError,
                ...
                symbol_logger.error(
                    f"{NEON_RED}Network error encountered in main loop: {e}. Waiting {delay} seconds...{RESET}"
                )
                time.sleep(delay)
            except ccxt.AuthenticationError as e:
                # Critical auth errors should stop the bot
                symbol_logger.critical(
                    f"{NEON_RED}CRITICAL AUTHENTICATION ERROR in main loop: {e}. Stopping bot.{RESET}"
                )
                break  # Exit the while loop
            except ccxt.ExchangeNotAvailable as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange not available (e.g., maintenance): {e}. Waiting 60 seconds...{RESET}"
                )
                time.sleep(60)
            except ccxt.OnMaintenance as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange is on maintenance: {e}. Waiting 5 minutes...{RESET}"
                )
                time.sleep(300)
            except ccxt.ExchangeError as e:
            ...
            symbol_logger.debug(
                f"<<< Loop #{loop_count} completed in {elapsed_time:.2f}s. Sleeping for {sleep_duration:.2f}s..."
            )
            if sleep_duration > 0:
        ...
        init_logger.critical(
            f"CRITICAL UNHANDLED ERROR outside main loop: {critical_err}", exc_info=True
        )
        if "symbol_logger" in locals():  # Log to symbol logger if available
            symbol_logger.critical(
                f"CRITICAL UNHANDLED ERROR outside main loop: {critical_err}",
                exc_info=True,
            )
    ...
    # --- Shutdown Sequence ---
        ...
        try:
            ...
            if "symbol_logger" in locals():
                loggers_to_close.append(symbol_logger)
