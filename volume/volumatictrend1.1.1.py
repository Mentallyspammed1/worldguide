...
try:
    ...
    print(f"{Fore.RED}Failed to initialize timezone. Please install 'tzdata' (`pip install tzdata`). Using UTC as fallback.{Style.RESET_ALL}")
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            # Allow retry for other potentially temporary exchange errors
                 ...
                 lg.error(f"Max retries reached for ExchangeError setting leverage.")
                 return False

        except (ccxt.NetworkError, ccxt.RequestTimeout, requests.exceptions.RequestException) as e:
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"Max retries reached for NetworkError setting leverage.")
                 return False
...
def calculate_position_size(
    ...
    try:
        ...
        try:
            ...
            price_precision_step = Decimal(str(price_precision_step_str))
...
class SignalGenerator:
    ...
    def calculate_initial_tp_sl(
        ...
        if market_info.get('precision', {}).get('price') is None:
             lg.error(f"Missing market price precision info."); return None, None
...
def analyze_and_trade_symbol(
    ...
    # === Scenario 2: Existing Open Position ===
        ...
        # --- Manage Existing Position (HOLD signal or compatible signal) ---
            ...
            try: current_sl_price = Decimal(str(open_position.get('stopLossPrice'))) if open_position.get('stopLossPrice') and str(open_position.get('stopLossPrice')) != '0' else None
            except Exception: pass
            try: current_tp_price = Decimal(str(open_position.get('takeProfitPrice'))) if open_position.get('takeProfitPrice') and str(open_position.get('takeProfitPrice')) != '0' else None
            except Exception: pass

            entry_price_actual = None
            try: entry_price_actual = Decimal(str(open_position.get('entryPrice'))) if open_position.get('entryPrice') else None
            except Exception: pass
            ...
            if tsl_enabled and not is_tsl_active_on_pos and entry_price_actual and current_atr:
                 ...
                 if tsl_set_success: lg.info(f"TSL setup/recovery attempt successful.")
                 else: lg.error(f"TSL setup/recovery attempt failed.")
