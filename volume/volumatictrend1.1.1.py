...
try:
    TIMEZONE = ZoneInfo(
        "America/Chicago"
    )  # Example: Use 'UTC' for Coordinated Universal Time
except Exception:
...
POSITION_CONFIRM_DELAY_SECONDS = (
    8  # Wait time after placing order before confirming position state
)
LOOP_DELAY_SECONDS = (
    15  # Min time between the end of one cycle and the start of the next
)
...
VALID_INTERVALS = [
    ...
    "W",
    "M",
]  # Intervals supported by the bot logic
...
def load_config(filepath: str) -> dict[str, Any]:
    ...
    if not os.path.exists(filepath):
        ...
        try:
            ...
            init_logger.error(
                f"{NEON_RED}Error creating default config file {filepath}: {e}. Using default values.{RESET}"
            )
            return default_config
    ...
    try:
        ...
        if interval_from_config not in VALID_INTERVALS:
            init_logger.error(
                f"{NEON_RED}Invalid interval '{interval_from_config}' in config. Using default '{default_config['interval']}'.{RESET}"
            )
        ...
        numeric_keys_to_validate = {
            "risk_per_trade": (
                0,
                1,
            ),  # Range: > 0 and <= 1 (allow up to 100% risk, though high)
        ...
        def validate_and_convert(cfg, key_path, valid_range):
            ...
            try:
                for key in keys:
                    ...
                    if value is None:
                        return False  # Key not found, handled by _ensure_config_keys
            ...
            try:
                ...
                if key_path in [
                    ...
                    if not (min_val < dec_value <= max_val):
                        raise ValueError(
                            f"Value {dec_value} out of range ({min_val}, {max_val}] for {key_path}"
                        )
                else:  # >= min_val
                    if not (min_val <= dec_value <= max_val):
                        raise ValueError(
                ...
                if isinstance(original_value, str):
                    ...
                    if isinstance(default_val, int):
                        cfg_to_update = cfg
                        for key in keys[:-1]:
                            cfg_to_update = cfg_to_update[key]
                        ...
                        for key in keys[:-1]:
                            cfg_to_update = cfg_to_update[key]
                        cfg_to_update[keys[-1]] = float(
                            dec_value
                        )  # Store as float for JSON compatibility if needed later
                        is_changed = True
            except (ValueError, InvalidOperation, TypeError):
                # Conversion failed or out of range, use default
                init_logger.warning(
                    f"{NEON_YELLOW}Config: Invalid numeric value '{original_value}' for '{key_path}'. Using default: {default_val}{RESET}"
                )
                cfg_to_update = cfg
                for key in keys[:-1]:
                    cfg_to_update = cfg_to_update[key]
        ...
        if any_numeric_corrected:
            config_needs_saving = True
            init_logger.warning(
                f"{NEON_YELLOW}Config file values corrected based on expected numeric types/ranges.{RESET}"
            )
        ...
        if config_needs_saving:
            try:
                ...
                init_logger.info(
                    f"{NEON_GREEN}Saved updated configuration to: {filepath}{RESET}"
                )
            except OSError as e:
                init_logger.error(
                ...
                init_logger.error(
                    f"{NEON_RED}Error preparing/saving updated config: {save_err}{RESET}",
                    exc_info=True,
                )
        ...
        init_logger.error(
            f"{NEON_RED}Error decoding config file {filepath}: {e}. Attempting to recreate default config.{RESET}"
        )
        try:
            ...
            init_logger.error(
                f"{NEON_RED}Error creating default config file after load error: {e_create}. Using default values.{RESET}"
            )
            return default_config
    except Exception as e:
        init_logger.error(
            f"{NEON_RED}Unexpected error loading config: {e}. Using default values.{RESET}",
            exc_info=True,
        )
        return default_config
    ...
    try:
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if exchange.markets:
                    lg.info(
                        f"Markets loaded successfully for {exchange.id} ({len(exchange.markets)} symbols)."
                    )
                    break  # Success
                else:
                    lg.warning(
                        f"load_markets returned empty markets list (Attempt {attempt + 1}/{MAX_API_RETRIES + 1}). Retrying..."
                    )
            except (
                ccxt.NetworkError,
                ccxt.RequestTimeout,
                ...
                if attempt < MAX_API_RETRIES:
                    ...
                    lg.critical(
                        f"{NEON_RED}Max retries reached loading markets for {exchange.id}. Error: {e}. Exiting.{RESET}"
                    )
                    return None  # Critical failure
            except ccxt.AuthenticationError as e:
                lg.critical(
                    f"{NEON_RED}Authentication error loading markets: {e}. Check API keys.{RESET}"
                )
                return None
            except ccxt.ExchangeError as e:
                lg.critical(
                    f"{NEON_RED}Exchange error loading markets: {e}. Exiting.{RESET}"
                )
                return None
            except Exception as e:
                lg.critical(
        ...
        if not exchange.markets:  # Double check if markets actually loaded after loop
            lg.critical(
                f"{NEON_RED}Market loading failed, exchange.markets is empty after retries. Exiting.{RESET}"
            )
            return None
        ...
        try:
            ...
            if balance_val is not None:
                lg.info(
                    f"{NEON_GREEN}Successfully connected and fetched initial balance.{RESET} ({QUOTE_CURRENCY} available: {balance_val.normalize()})"
                )
            else:
                # fetch_balance returning None after retries usually indicates a persistent issue
                lg.critical(
                    f"{NEON_RED}Initial balance fetch failed after retries. Check API key permissions, network, or account type.{RESET}"
                )
                # Decide if this is fatal. If trading is enabled, it likely is.
                if CONFIG.get("enable_trading", False):
                    lg.critical(
                        f"{NEON_RED}Trading enabled, but initial balance check failed. This is critical. Exiting.{RESET}"
                    )
                    return None
                else:
                    lg.warning(
                        f"{NEON_YELLOW}Trading disabled, but initial balance check failed. Proceeding, but trading functions will likely fail if enabled later.{RESET}"
                    )
            ...
            lg.warning(
                f"{NEON_YELLOW}Could not perform initial balance fetch: {balance_err}. Check API permissions/account type if trading fails.{RESET}",
                exc_info=True,
            )
            # Proceed cautiously if trading is disabled, otherwise consider fatal?
            if CONFIG.get("enable_trading", False):
                lg.critical(
                    f"{NEON_RED}Trading enabled, initial balance fetch failed unexpectedly. Exiting.{RESET}"
                )
                return None
...
def fetch_current_price_ccxt(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
                ...
                if val_str is not None and str(val_str).strip() != "":
                    try:
                        ...
                        if p > Decimal("0"):
                            ...
                            lg.debug(
                                f"Ticker {name} price '{val_str}' is not positive."
                            )
                            return None
                    except (InvalidOperation, TypeError) as conv_err:
                        lg.warning(
                            f"Invalid ticker {name} price format '{val_str}': {conv_err}"
                        )
                        return None
                return None
            ...
            if price:
                lg.debug(f"Using 'last' price: {price.normalize()}")
            ...
            if price is None:
                ...
                if bid and ask:
                    if ask >= bid:  # Sanity check: ask >= bid
                        ...
                        lg.debug(
                            f"Using bid/ask midpoint: {price.normalize()} (Bid: {bid.normalize()}, Ask: {ask.normalize()})"
                        )
                    else:
                        lg.warning(
                            f"Invalid bid/ask values: Bid={bid.normalize()}, Ask={ask.normalize()}"
                        )
                else:
            ...
            if price is not None:
                ...
                lg.warning(
                    f"Failed to get a valid positive price from ticker (Attempt {attempts + 1}). Ticker data: {ticker}"
                )
            ...
            lg.warning(
                f"{NEON_YELLOW}Rate limit exceeded fetching price: {e}. Waiting {wait_time}s...{RESET}"
            )
            time.sleep(
                wait_time - RETRY_DELAY_SECONDS
            )  # Account for standard delay below
        except ccxt.ExchangeError as e:
            lg.error(
...
def fetch_klines_ccxt(
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        try:
            ...
            if ohlcv is not None and len(ohlcv) > 0:
                ...
                try:
                    ...
                    interval_seconds = (
                        ...
                        and exchange.parse_timeframe(timeframe)
                        else None
                    )
                    # Allow a lag, e.g., 5 intervals, or 300 seconds if interval duration unknown
                    max_allowed_lag_seconds = (
                        (interval_seconds * 5) if interval_seconds else 300
                    )
                    # Allow extra buffer for lower frequency timeframes
                    if timeframe in ["1d", "1w", "1M"]:
                        max_allowed_lag_seconds = max(
                            max_allowed_lag_seconds, 3600
                        )  # Allow 1 hour lag for daily+

                    lag_seconds = (now_utc - last_ts).total_seconds()
                    if lag_seconds < max_allowed_lag_seconds:
                        lg.debug(
                            f"Received {len(ohlcv)} klines. Last timestamp: {last_ts} (Lag: {lag_seconds:.1f}s)"
                        )
                        break  # Success
                    else:
                        lg.warning(
                            f"{NEON_YELLOW}Received {len(ohlcv)} klines, but last timestamp {last_ts} seems too old (Lag: {lag_seconds:.1f}s > {max_allowed_lag_seconds}s). Retrying...{RESET}"
                        )
                        # Potentially stale data, force retry
                        ohlcv = None  # Clear ohlcv to force retry
                except Exception as ts_err:
                    lg.warning(
                        f"Error validating timestamp: {ts_err}. Proceeding cautiously."
                    )
                    break  # Proceed even if timestamp check fails
            else:
                lg.warning(
                    f"fetch_ohlcv returned None or empty list for {symbol} (Attempt {attempt + 1}). Retrying..."
                )
            ...
            if attempt < MAX_API_RETRIES:
                lg.warning(
                    f"{NEON_YELLOW}Network error fetching klines for {symbol} (Attempt {attempt + 1}): {e}. Retrying in {RETRY_DELAY_SECONDS}s...{RESET}"
                )
                time.sleep(RETRY_DELAY_SECONDS)
            else:
                lg.error(
                    f"{NEON_RED}Max retries reached fetching klines for {symbol} after network errors: {e}{RESET}"
                )
                return pd.DataFrame()  # Return empty DF on persistent network failure
        except ccxt.RateLimitExceeded as e:
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
                f"{NEON_YELLOW}Rate limit exceeded fetching klines for {symbol}: {e}. Retrying in {wait_time}s... (Attempt {attempt + 1}){RESET}"
            )
    ...
    try:
        ...
        if rows_dropped > 0:
            lg.debug(
                f"Dropped {rows_dropped} rows with NaN/invalid price/volume data for {symbol}."
            )
        ...
        lg.error(
            f"{NEON_RED}Error processing kline data for {symbol}: {e}{RESET}",
            exc_info=True,
        )
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        try:
            ...
            if not exchange.markets or symbol not in exchange.markets:
                lg.info(
                    f"Market info for {symbol} not loaded or missing. Reloading markets (Attempt {attempt + 1})..."
                )
            ...
            if symbol not in exchange.markets:
                ...
                if attempt == 0:
                    continue  # Allow one reload attempt before failing
                lg.error(
                    f"{NEON_RED}Market '{symbol}' still not found after reloading markets.{RESET}"
                )
                return None
            ...
            if market:
                ...
                contract_type = (
                    "Linear" if is_linear else "Inverse" if is_inverse else "Spot/Other"
                )
                ...
                if (
                    market.get("precision", {}).get("price") is None
                    or market.get("precision", {}).get("amount") is None
                ):
                    lg.error(
                        f"{NEON_RED}Market info for {symbol} is missing essential precision data (price or amount). Trading may fail.{RESET}"
                    )
                ...
                if market.get("limits", {}).get("amount", {}).get("min") is None:
                    lg.warning(
                        f"{NEON_YELLOW}Market info for {symbol} is missing minimum amount limit. Assuming 0, but orders might fail.{RESET}"
                    )
                ...
                lg.error(
                    f"{NEON_RED}Market dictionary unexpectedly None for '{symbol}' even though key exists in markets.{RESET}"
                )
                return None  # Treat as failure

        except ccxt.BadSymbol as e:
            lg.error(
                f"{NEON_RED}Symbol '{symbol}' not supported or invalid on {exchange.id}: {e}{RESET}"
            )
            ...
            if attempt < MAX_API_RETRIES:
                lg.warning(
                    f"{NEON_YELLOW}Network error getting market info for {symbol} (Attempt {attempt + 1}): {e}. Retrying...{RESET}"
                )
                time.sleep(RETRY_DELAY_SECONDS)
            else:
                lg.error(
                    f"{NEON_RED}Max retries reached getting market info for {symbol} after network errors: {e}{RESET}"
                )
                return None
        except ccxt.ExchangeError as e:
            lg.warning(
                f"{NEON_YELLOW}Exchange error getting market info for {symbol}: {e}. Retrying...{RESET}"
            )
            ...
            if attempt < MAX_API_RETRIES:
                ...
                lg.error(
                    f"{NEON_RED}Max retries reached getting market info for {symbol} after exchange errors: {e}{RESET}"
                )
                return None
        except Exception as e:
            lg.error(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            for acc_type in account_types_to_try:
                try:
                    lg.debug(
                        f"Fetching balance using params={{'accountType': '{acc_type}'}} for {currency}..."
                    )
                    ...
                    if (
                        ...
                        lg.debug(
                            f"Found balance via standard ['{currency}']['free'] structure: {available_balance_str} (Type: {acc_type})"
                        )
                    ...
                    # Structure 2: Bybit V5 Unified/Contract ('info' field)
                        ...
                        for account in balance_info["info"]["result"]["list"]:
                            ...
                            if (
                                account_type_in_response is None
                                or account_type_in_response == acc_type
                            ) and isinstance(account.get("coin"), list):
                                for coin_data in account["coin"]:
                                    if coin_data.get("coin") == currency:
                                        ...
                                        if free is not None:
                                            ...
                                            lg.debug(
                                                f"Found balance via V5 info.result.list[].coin[] ({acc_type}): {available_balance_str}"
                                            )
                                            found_structure = True
                                            break
                                if found_structure:
                        ...
                        if found_structure:
                            break  # Exit account type loop if found
                        lg.debug(
                            f"Currency '{currency}' not found within V5 info.result.list structure for type '{acc_type}'."
                        )
                    else:
                        lg.debug(
                            f"V5 structure not found for type '{acc_type}'. Response structure: {balance_info.keys()}"
                        )  # Log keys found
                    ...
                    lg.debug(
                        f"API error fetching balance for type '{acc_type}': {e}. Trying next type."
                    )
                    continue  # Try next account type
                except Exception as e:
                    lg.warning(
                        f"Unexpected error fetching balance type '{acc_type}': {e}. Trying next type.",
                        exc_info=True,
                    )
                    continue
            ...
            if not found_structure:
                lg.debug(
                    f"Fetching balance using default parameters for {currency} (fallback)..."
                )
                try:
                    balance_info = exchange.fetch_balance()
                    ...
                    if (
                        ...
                        available_balance_str = str(balance_info[currency]["free"])
                        lg.debug(
                            f"Found balance via standard ['{currency}']['free'] (default fetch): {available_balance_str}"
                        )
                        found_structure = True
                    # Check top-level 'free' dict (older CCXT versions?)
                        ...
                        lg.debug(
                            f"Found balance via top-level 'free' dict (default fetch): {available_balance_str}"
                        )
                        found_structure = True
                            ...
                            if isinstance(account.get("coin"), list):
                                for coin_data in account["coin"]:
                                    if coin_data.get("coin") == currency:
                                        ...
                                        if free is not None:
                                            available_balance_str = str(free)
                                            acc_type_str = account.get(
                                                "accountType", "N/A"
                                            )
                                            lg.debug(
                                                f"Found balance via V5 nested structure (default fetch, type {acc_type_str}): {available_balance_str}"
                                            )
                                            found_structure = True
                                            break  # Found balance for currency in *any* account type list
                                if found_structure:
                                    break  # Exit inner account loop
                            if found_structure:
                                break  # Exit list loop
                        if not found_structure:
                            lg.debug(
                                f"Currency '{currency}' not found within V5 nested structure from default fetch."
                            )
            ...
            if found_structure and available_balance_str is not None:
                try:
                    ...
                    if final_balance >= Decimal("0"):
                        lg.info(
                            f"Available {currency} balance: {final_balance.normalize()}"
                        )
                        return final_balance  # Success
                    else:
                        lg.error(
                            f"Parsed balance for {currency} is negative ({final_balance.normalize()}). Check account."
                        )
                        # Treat negative balance as an issue, maybe retry? For now, raise error.
                        raise ccxt.ExchangeError(
                            f"Negative balance detected for {currency}"
                        )
                    ...
                    raise ccxt.ExchangeError(
                        f"Balance conversion failed for {currency}"
                    )
            else:
                # If still no balance found after all attempts
                lg.error(
                    f"{NEON_RED}Could not determine available balance for {currency} after checking known structures.{RESET}"
                )
                # lg.debug(f"Last balance_info structure checked: {balance_info}") # Too verbose possibly
                # Raise error to trigger retry
                raise ccxt.ExchangeError(
                    f"Balance not found for {currency} in response"
                )
        ...
        # --- Retry Logic for Handled Exceptions ---
            ...
            lg.warning(
                f"{NEON_YELLOW}Network error fetching balance for {currency}: {e}. Retrying...{RESET}"
            )
        except ccxt.RateLimitExceeded as e:
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
            ...
            lg.critical(
                f"{NEON_RED}Authentication Error fetching balance: {e}. Check API keys/permissions.{RESET}"
            )
            ...
            lg.warning(
                f"{NEON_YELLOW}Exchange error fetching balance for {currency}: {e}. Retrying...{RESET}"
            )
            ...
            if bybit_code in [
                10001,
                10002,
            ]:  # API key invalid/expired/no permission or IP restricted
                lg.critical(
                    f"{NEON_RED}Bybit API Key/Auth Error (Code {bybit_code}) during balance fetch: {e}. Stopping.{RESET}"
                )
                return None  # Treat as fatal
        except Exception as e:
            lg.error(
                f"{NEON_RED}Unexpected error during balance fetch for {currency}: {e}{RESET}",
                exc_info=True,
            )
...
def get_open_position(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.debug(
                f"Fetching positions for symbol: {symbol} (Attempt {attempts + 1})"
            )
            positions: list[dict] = []
            ...
            try:
                ...
                lg.warning(
                    "fetch_positions requires fetching all positions (exchange doesn't support single symbol filter). This may be slower."
                )
                params = {
                    "category": category or "linear"
                }  # Use determined category if possible
                ...
                err_str = str(e).lower()
                if (
                    (hasattr(e, "code") and e.code in no_pos_codes_v5)
                    or "position not found" in err_str
                    or "no position found" in err_str
                    or "invalid symbol" in err_str
                ):  # Could be an error if symbol/category combination is wrong
                    lg.info(
                        f"No position or invalid symbol/category for {symbol} (Exchange confirmation: {e})."
                    )
            ...
            try:
                ...
                if amount_precision_step_str:
                    ...
                    size_threshold = Decimal(str(amount_precision_step_str)) * Decimal(
                        "0.1"
                    )
            except Exception as market_err:
                lg.debug(
                    f"Could not get market precision for position size threshold ({market_err}), using default {size_threshold}."
                )
            ...
            for pos in positions:
                ...
                ):  # Standard CCXT field as fallback
                ...
                if pos_size_str is None:
                    lg.debug(
                        f"Skipping position entry, could not determine size: {pos.get('info', {})}"
                    )
                    continue
                ...
                try:
                    ...
                    if abs(position_size) > size_threshold:
                        ...
                        lg.debug(
                            f"Found potential active position entry for {symbol} with size {position_size.normalize()}. Details: {pos.get('info', {})}"
                        )
                    ...
                    lg.warning(
                        f"Could not parse position size '{pos_size_str}' to Decimal: {parse_err}. Skipping entry."
                    )
                    continue
            ...
            if active_position:
                ...
                if side not in ["long", "short"]:
                    pos_side_v5 = info_dict.get(
                        "side", ""
                    ).lower()  # Bybit V5 'Buy'/'Sell'
                    if pos_side_v5 == "buy":
                        side = "long"
                    elif pos_side_v5 == "sell":
                        side = "short"
                    # Fallback: derive from size if side is missing/ambiguous
                    elif std_pos["size_decimal"] > size_threshold:
                        side = "long"
                    elif std_pos["size_decimal"] < -size_threshold:
                        side = "short"
                    else:
                        lg.warning(
                            f"Position size {std_pos['size_decimal'].normalize()} near zero or side ambiguous. Cannot determine side."
                        )
                ...
                std_pos["entryPrice"] = (
                    entry_price_str  # Store standardized field (string)
                )
                ...
                # Liquidation Price
                liq_price_str = std_pos.get("liquidationPrice") or info_dict.get(
                    "liqPrice"
                )
                std_pos["liquidationPrice"] = (
                    liq_price_str  # Store standardized field (string)
                )
                ...
                sl_price_str = info_dict.get(
                    "stopLoss"
                )  # Prefer 'info' field for V5 accuracy
                if not sl_price_str:
                    sl_price_str = std_pos.get("stopLossPrice")  # Fallback to standard

                tp_price_str = info_dict.get("takeProfit")  # Prefer 'info' field
                if not tp_price_str:
                    tp_price_str = std_pos.get("takeProfitPrice")

                tsl_distance_str = info_dict.get(
                    "trailingStop"
                )  # Bybit V5: Distance/Value
                tsl_activation_str = info_dict.get(
                    "activePrice"
                )  # Bybit V5: Activation price for TSL
                ...
                if sl_price_str is not None:
                    std_pos["stopLossPrice"] = str(sl_price_str)
                if tp_price_str is not None:
                    std_pos["takeProfitPrice"] = str(tp_price_str)
                if tsl_distance_str is not None:
                    std_pos["trailingStopLoss"] = str(tsl_distance_str)
                if tsl_activation_str is not None:
                    std_pos["tslActivationPrice"] = str(tsl_activation_str)
                ...
                def format_log_val(val_str, precision_type="price", default_prec=4):
                    ...
                    try:
                            ...
                            if prec_val is not None:
                                try:
                                    ...
                                    if prec_step_dec == prec_step_dec.to_integral():
                                        ...
                                        prec = abs(
                                            prec_step_dec.normalize()
                                            .as_tuple()
                                            .exponent
                                        )
                                except Exception:
                                    pass  # Stick with default precision if parsing fails
                        ...
                        return str(
                            d_val.quantize(exponent, rounding=ROUND_DOWN).normalize()
                        )
                ...
                tsl_dist_fmt = format_log_val(
                    std_pos.get("trailingStopLoss"), "price"
                )  # TSL distance often uses price precision
                ...
                logger.info(
                    f"No active open position found for {symbol} (checked {len(positions)} entries)."
                )
        ...
        # --- Retry Logic for Handled Exceptions ---
            ...
            lg.warning(
                f"{NEON_YELLOW}Network error fetching position for {symbol}: {e}. Retrying...{RESET}"
            )
        except ccxt.RateLimitExceeded as e:
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
            ...
            lg.critical(
                f"{NEON_RED}Authentication Error fetching position: {e}. Stopping checks.{RESET}"
            )
            return None  # Fatal
        except ccxt.ExchangeError as e:
            ...
            lg.warning(
                f"{NEON_YELLOW}Exchange error fetching position for {symbol}: {e}. Retrying...{RESET}"
            )
            bybit_code = getattr(e, "code", None)
            ...
            if bybit_code in [110004]:
                lg.critical(
                    f"{NEON_RED}Bybit Account Error (Code {bybit_code}) during position fetch: {e}. Is API key linked to Unified or Derivatives account? Stopping checks.{RESET}"
                )
                return None  # Fatal
            elif bybit_code in [110013]:
                lg.error(
                    f"{NEON_RED}Bybit Parameter Error (Code {bybit_code}) fetching position: {e}. Check symbol/category. Stopping checks.{RESET}"
                )
...
def set_leverage_ccxt(
    ...
    if leverage <= 0:
        lg.warning(
            f"Leverage setting skipped for {symbol}: Invalid leverage ({leverage}). Must be > 0."
        )
        return False

    if not exchange.has.get("setLeverage"):
        lg.error(
            f"{NEON_RED}Exchange {exchange.id} does not support setLeverage via CCXT.{RESET}"
        )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.info(
                f"Attempting to set leverage for {symbol} to {leverage}x (Attempt {attempts + 1}/{MAX_API_RETRIES + 1})..."
            )
            params = {}
            ...
            lg.debug(
                f"Set leverage raw response for {symbol}: {response}"
            )  # Contains exchange-specific response
            ...
            if ret_code is not None:
                if ret_code == 0:
                    lg.info(
                        f"{NEON_GREEN}Leverage set/requested successfully for {symbol} to {leverage}x (Bybit retCode 0).{RESET}"
                    )
                    return True  # Success based on Bybit code
                elif ret_code == 110045:  # Bybit: Leverage not modified (already set)
                    lg.info(
                        f"{NEON_YELLOW}Leverage for {symbol} already set to {leverage}x (Bybit retCode 110045).{RESET}"
                    )
                    ...
                    raise ccxt.ExchangeError(
                        f"Bybit API error setting leverage: {response.get('retMsg', 'Unknown error')} (Code: {ret_code})"
                    )
            else:
                # If no specific retCode, assume success if no exception raised and response isn't obviously an error
                lg.info(
                    f"{NEON_GREEN}Leverage set/requested successfully for {symbol} to {leverage}x.{RESET}"
                )
                return True
            ...
            bybit_code = getattr(
                e, "code", None
            )  # Try to extract Bybit code if available
            lg.error(
            ...
            if bybit_code == 110045 or "leverage not modified" in err_str:
                lg.info(
                    f"{NEON_YELLOW}Leverage for {symbol} already set to {leverage}x (Exchange confirmation).{RESET}"
                )
                return True  # Already set is considered success
            elif (
                bybit_code in [110028, 110009, 110055, 110043]
                or "margin mode" in err_str
                or "position exists" in err_str
            ):
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Cannot change leverage/margin mode. Check Margin Mode (Isolated/Cross), open orders, or existing position for {symbol}.{RESET}"
                )
                return False  # Unrecoverable state for automated setting
            elif bybit_code == 110044 or "risk limit" in err_str:
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Leverage {leverage}x may exceed risk limit tier for current/potential position size. Check Bybit Risk Limits documentation.{RESET}"
                )
                return False  # Configuration issue, not temporary
            elif (
                bybit_code == 110013
                or "parameter error" in err_str
                or "leverage value invalid" in err_str
            ):
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Leverage value {leverage}x might be invalid for {symbol} or other params incorrect. Check allowed range on Bybit.{RESET}"
                )
            ...
            if attempts >= MAX_API_RETRIES:
                ...
                return False
            lg.warning(
                f"{NEON_YELLOW}Network error setting leverage for {symbol} (Attempt {attempts + 1}): {e}. Retrying..."
            )
        except Exception as e:
            lg.error(
    ...
    lg.error(
        f"{NEON_RED}Failed to set leverage for {symbol} after {MAX_API_RETRIES + 1} attempts.{RESET}"
    )
...
def calculate_position_size(
    ...
    if (
        ...
        lg.error(
            f"Position sizing failed ({symbol}): Invalid entry ({entry_price.normalize() if entry_price else 'N/A'}) or SL ({initial_stop_loss_price.normalize() if initial_stop_loss_price else 'N/A'})."
        )
        return None
    if initial_stop_loss_price == entry_price:
        lg.error(
            f"Position sizing failed ({symbol}): SL price cannot equal entry price."
        )
        return None
    ...
    if amount_precision_step_str is None or price_precision_step_str is None:
        lg.error(
            f"Position sizing failed ({symbol}): Missing amount or price precision in market info."
        )
        return None
    ...
    try:
        ...
        lg.info(
            f"Position Sizing ({symbol}): Balance={balance.normalize()} {quote_currency}, Risk={risk_per_trade_dec:.2%}, RiskAmt={risk_amount_quote.normalize()} {quote_currency}"
        )
        ...
        if sl_distance_price <= Decimal("0"):
            lg.error(
                f"Position sizing failed ({symbol}): SL distance is zero/negative ({sl_distance_price.normalize()})."
            )
            return None
        lg.info(
            f"  Entry={entry_price.normalize()}, SL={initial_stop_loss_price.normalize()}, SL Dist={sl_distance_price.normalize()}"
        )
        ...
        if contract_size_str is None:
            contract_size = Decimal(
                "1"
            )  # Assume 1 if not specified (common for spot/basic futures)
            ...
            try:
                ...
                lg.warning(
                    f"{NEON_YELLOW}Invalid contract size '{contract_size_str}' for {symbol}, using 1. Error: {e}{RESET}"
                )
                contract_size = Decimal("1")  # Fallback

        lg.info(
            f"  ContractSize={contract_size.normalize()}, Type={'Linear/Spot' if not is_inverse else 'Inverse'}"
        )
        ...
        if not is_inverse:  # Linear Contract or Spot
            ...
            if sl_distance_price > Decimal("0"):
                ...
                lg.error(
                    f"Sizing failed ({symbol}): Linear/Spot SL distance is zero/negative."
                )
                return None
            ...
            lg.info(
                f"Inverse contract detected ({symbol}). Using specific inverse sizing formula."
            )
            if entry_price > Decimal("0") and initial_stop_loss_price > Decimal("0"):
                try:
                    ...
                    if inverse_factor <= Decimal("0"):
                        lg.error(
                            f"Sizing failed ({symbol}): Inverse factor calculation resulted in zero/negative value ({inverse_factor.normalize()}). Check entry/SL prices."
                        )
                        return None
                    ...
                    if risk_per_contract_quote <= Decimal("0"):
                        lg.error(
                            f"Sizing failed ({symbol}): Inverse contract risk per contract is zero/negative ({risk_per_contract_quote.normalize()})."
                        )
                        return None
                    ...
                    lg.error(
                        f"Sizing failed ({symbol}): Error during inverse calculation ({calc_err}). Check entry/SL prices."
                    )
                    return None
            else:
                lg.error(
                    f"Sizing failed ({symbol}): Invalid entry/SL price for inverse contract calculation."
                )
                return None

        lg.info(
            f"  Initial Calculated Size = {calculated_size.normalize()} {size_unit}"
        )
        ...
        amount_limits = limits.get(
            "amount", {}
        )  # Limits on position size (in contracts or base currency)
        cost_limits = limits.get(
            "cost", {}
        )  # Limits on position value (in quote currency)
        ...
        try:
            ...
            min_amount = Decimal(
                str(amount_limits.get("min", "0"))
            )  # Default min to 0 if missing
            max_amount = (
            ...
            lg.error(
                f"Sizing failed ({symbol}): Error converting market limits/precision to Decimal: {conv_err}"
            )
            return None
        ...
        if min_amount > Decimal("0") and adjusted_size < min_amount:
            lg.warning(
                f"{NEON_YELLOW}Calculated size {adjusted_size.normalize()} < min amount {min_amount.normalize()}. Adjusting to min amount.{RESET}"
            )
            adjusted_size = min_amount
        if max_amount < Decimal("inf") and adjusted_size > max_amount:
            lg.warning(
                f"{NEON_YELLOW}Calculated size {adjusted_size.normalize()} > max amount {max_amount.normalize()}. Adjusting to max amount.{RESET}"
            )
        ...
        try:
            if entry_price > Decimal("0"):
                ...
                lg.warning(
                    f"Cannot estimate cost for {symbol} due to zero entry price."
                )  # Skip cost check?
        except Exception as cost_est_err:
            lg.error(
                f"Error estimating cost for size {adjusted_size.normalize()}: {cost_est_err}"
            )
            # Cannot check cost limits if cost estimation fails
            min_cost, max_cost = (
                Decimal("0"),
                Decimal("inf"),
            )  # Effectively disable cost limits
        ...
        if min_cost > Decimal("0") and estimated_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {estimated_cost.normalize()} < min cost {min_cost.normalize()}. Attempting to increase size.{RESET}"
            )
            ...
            except Exception as cost_calc_err:
                lg.error(
                    f"Error calculating required size for min cost: {cost_calc_err}"
                )

            if (
                required_size_for_min_cost is None
                or required_size_for_min_cost <= Decimal("0")
            ):
                lg.error(
                    f"{NEON_RED}Cannot meet min cost {min_cost.normalize()}. Calculation failed or resulted in zero/negative size. Aborted.{RESET}"
                )
                return None
            lg.info(
                f"  Required size for min cost: {required_size_for_min_cost.normalize()}"
            )

            if max_amount < Decimal("inf") and required_size_for_min_cost > max_amount:
                lg.error(
                    f"{NEON_RED}Cannot meet min cost {min_cost.normalize()} without exceeding max amount {max_amount.normalize()}. Aborted.{RESET}"
                )
                return None
            ...
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {estimated_cost.normalize()} > max cost {max_cost.normalize()}. Reducing size.{RESET}"
            )
            ...
            try:
                if entry_price > Decimal("0") and contract_size > Decimal("0"):
                    if not is_inverse:  # Linear / Spot
                        ...
                        adjusted_size_for_max_cost = (
                            max_cost * entry_price
                        ) / contract_size
            except Exception as cost_calc_err:
                lg.error(f"Error calculating max size for max cost: {cost_calc_err}")

            if (
                adjusted_size_for_max_cost is None
                or adjusted_size_for_max_cost <= Decimal("0")
            ):
                lg.error(
                    f"{NEON_RED}Cannot reduce size to meet max cost {max_cost.normalize()}. Calculation failed or resulted in zero/negative size. Aborted.{RESET}"
                )
                return None
            lg.info(
                f"  Max size allowed by max cost: {adjusted_size_for_max_cost.normalize()}"
            )

            # Reduce size to meet max cost, ensuring it doesn't fall below min amount
            adjusted_size = max(
                min_amount, min(adjusted_size, adjusted_size_for_max_cost)
            )
            cost_adjusted = True

        if cost_adjusted:
            lg.info(
                f"  Size after Cost Limits: {adjusted_size.normalize()} {size_unit}"
            )
        ...
        try:
            ...
            lg.info(
                f"Applied amount precision/step (via ccxt, typically truncated): {adjusted_size.normalize()} -> {final_size.normalize()} {size_unit}"
            )
        except Exception as fmt_err:
            # Fallback or error if ccxt's method fails - attempt manual rounding down
            lg.warning(
                f"{NEON_YELLOW}Error applying ccxt amount precision for {symbol}: {fmt_err}. Attempting manual rounding down.{RESET}",
                exc_info=True,
            )
            try:
                if amount_precision_step > Decimal("0"):
                    ...
                    lg.info(
                        f"Applied manual amount step size ({amount_precision_step.normalize()}): {adjusted_size.normalize()} -> {final_size.normalize()} {size_unit}"
                    )
                else:
                    raise ValueError("Manual step size must be positive")
            except Exception as manual_err:
                lg.error(
                    f"{NEON_RED}Invalid amount precision value '{amount_precision_step_str}' ({manual_err}). Using size adjusted for limits only.{RESET}",
                    exc_info=True,
                )
        ...
        if final_size <= Decimal("0"):
            lg.error(
                f"{NEON_RED}Position size became zero/negative ({final_size.normalize()}) after adjustments. Aborted.{RESET}"
            )
            return None
        ...
        if min_amount > Decimal("0") and final_size < min_amount:
            ...
            lg.error(
                f"{NEON_RED}Final size {final_size.normalize()} is below minimum amount {min_amount.normalize()} after precision formatting. Aborted.{RESET}"
            )
            return None
        ...
        try:
            ...
            lg.warning("Could not recalculate final cost.")

        if min_cost > Decimal("0") and final_cost < min_cost:
            lg.debug(
                f"Final size {final_size.normalize()} results in cost {final_cost.normalize()} which is below min cost {min_cost.normalize()}."
            )
            ...
            try:
                ...
                if step_size <= Decimal("0"):
                    raise ValueError("Step size must be positive to bump.")
                ...
                if entry_price > Decimal("0"):
                    if not is_inverse:
                        next_step_cost = next_step_size * entry_price * contract_size
                    else:
                        next_step_cost = (next_step_size * contract_size) / entry_price

                # Check if next step is valid
                valid_next_step = True
                if next_step_cost < min_cost:
                    valid_next_step = False  # Still doesn't meet min cost
                if max_amount < Decimal("inf") and next_step_size > max_amount:
                    valid_next_step = False
                if max_cost < Decimal("inf") and next_step_cost > max_cost:
                    valid_next_step = False

                if valid_next_step:
                    lg.warning(
                        f"{NEON_YELLOW}Final size cost {final_cost.normalize()} < min cost {min_cost.normalize()}. Bumping size to next step {next_step_size.normalize()} ({size_unit}) to meet minimums.{RESET}"
                    )
                    final_size = next_step_size
                else:
                    lg.error(
                        f"{NEON_RED}Final size {final_size.normalize()} cost is below minimum, and next step size {next_step_size.normalize()} is invalid (violates min cost or max limits). Aborted.{RESET}"
                    )
                    return None
            except Exception as bump_err:
                lg.error(
                    f"{NEON_RED}Final size cost is below minimum. Error trying to bump size: {bump_err}. Aborted.{RESET}"
                )
                return None

        lg.info(
            f"{NEON_GREEN}Final calculated position size for {symbol}: {final_size.normalize()} {size_unit}{RESET}"
        )
        return final_size
...
def place_trade(
    ...
    size_unit = (
        market_info.get("settle", base_currency) if is_contract else base_currency
    )  # Settled in quote/settle for contracts
    ...
    lg.info(
        f"Attempting to place {action_desc} {side.upper()} {order_type} order for {symbol}:"
    )
    lg.info(f"  Side: {side.upper()}, Size: {position_size.normalize()} {size_unit}")
    lg.debug(
        f"  Category: {category}, PositionIdx: {position_idx}, ReduceOnly: {reduce_only}"
    )
    ...
    try:
        ...
        if amount_float <= 0:
            lg.error(
                f"Trade aborted ({symbol} {side} {action_desc}): Invalid position size ({position_size.normalize()}). Size must be positive."
            )
            return None
    except ValueError:
        lg.error(
            f"Trade aborted ({symbol} {side} {action_desc}): Could not convert size {position_size.normalize()} to float."
        )
        return None
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.debug(
                f"Executing create_order API call (Attempt {attempts + 1}/{MAX_API_RETRIES + 1})..."
            )
            ...
            if avg_fill_price is not None:
                lg.info(
                    f"  Avg Fill Price: ~{Decimal(str(avg_fill_price)).normalize() if avg_fill_price else 'N/A'}"
                )
            if filled_amount is not None:
                lg.info(
                    f"  Filled Amount: {Decimal(str(filled_amount)).normalize() if filled_amount else '0'}"
                )
        ...
        # --- Specific Error Handling (Non-retryable errors) ---
            ...
            lg.error(
                f"{NEON_RED}Insufficient funds to place {action_desc} {side} order ({symbol}): {e}{RESET}"
            )
            return None  # Non-retryable
        except ccxt.InvalidOrder as e:
            lg.error(
                f"{NEON_RED}Invalid order parameters placing {action_desc} {side} order ({symbol}): {e}{RESET}"
            )
            bybit_code = getattr(e, "code", None)
            if reduce_only and (
                bybit_code == 110014
                or "reduce-only" in str(e).lower()
                or "no position" in str(e).lower()
            ):
                lg.error(
                    f"{NEON_YELLOW} >> Hint (Reduce-Only Fail): Position might be closed, size incorrect, or wrong side specified? Check active position.{RESET}"
                )
            elif (
                bybit_code == 110007
                or "order quantity" in str(e).lower()
                or "amount is invalid" in str(e).lower()
            ):
                lg.error(
                    f"{NEON_YELLOW} >> Hint (Quantity Error): Check order size ({amount_float}) against market precision (step size) and amount limits (min/max).{RESET}"
                )
            elif bybit_code == 110040 or "order cost" in str(e).lower():
                lg.error(
                    f"{NEON_YELLOW} >> Hint (Cost Error): Check estimated order value ({position_size.normalize()} * price) against market cost limits (min/max).{RESET}"
                )
            elif bybit_code == 110013:
                lg.error(
                    f"{NEON_YELLOW} >> Hint (Parameter Error): Review order parameters: {order_params}. Ensure they are valid for Bybit V5.{RESET}"
                )
            return None  # Invalid order is generally not retryable
        except ccxt.ExchangeError as e:
            bybit_code = getattr(e, "code", None)
            lg.error(
                f"{NEON_RED}Exchange error placing {action_desc} order ({symbol}): {e} (Code: {bybit_code}){RESET}"
            )
            if reduce_only and bybit_code == 110025:
                lg.warning(
                    f"{NEON_YELLOW} >> Hint (Position Not Found): Position might have been closed already when trying to place reduce-only order.{RESET}"
                )
                return None  # Don't retry if position is gone
            elif bybit_code == 30086 or "risk limit" in str(e).lower():
                lg.error(
                    f"{NEON_YELLOW} >> Hint (Risk Limit): Order size + existing position size may exceed the risk limit tier for the current leverage. Reduce size/leverage or check Bybit docs.{RESET}"
                )
        ...
        # --- Retryable Error Handling ---
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Max retries reached placing order after network errors: {e}{RESET}"
                )
                return None
            lg.warning(
                f"{NEON_YELLOW}Network error placing order (Attempt {attempts + 1}): {e}. Retrying..."
            )
        except ccxt.RateLimitExceeded as e:
            # Don't count rate limit waits against retries, just wait and try again
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
                f"{NEON_YELLOW}Rate limit exceeded placing order: {e}. Waiting {wait_time}s..."
            )
            time.sleep(wait_time)
            ...
            lg.error(
                f"{NEON_RED}Unexpected error placing {action_desc} order ({symbol}) (Attempt {attempts + 1}): {e}{RESET}",
                exc_info=True,
            )
...
def _set_position_protection(
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"Cannot set protection for {symbol}: Invalid position side ('{pos_side}') or missing entry price."
        )
        return False
    try:
        ...
        lg.error(
            f"Invalid entry price format ('{entry_price_str}') for protection check: {e}"
        )
        return False
    ...
    log_parts = [
        f"Attempting to set protection for {symbol} ({pos_side.upper()} @ {entry_price.normalize()}):"
    ]
    any_protection_requested = (
        False  # Track if any valid protection level was provided (non-zero Decimal)
    )
    ...
    try:
        ...
        if price_prec_str is None:
            raise ValueError("Missing price precision in market info")
        min_tick_size = Decimal(str(price_prec_str))
        if min_tick_size <= Decimal("0"):
            raise ValueError(f"Invalid tick size: {min_tick_size}")
        ...
        def format_price(price_decimal: Decimal | None) -> str | None:
            ...
            if price_decimal < Decimal("0"):
                lg.warning(
                    f"Cannot format negative price ({price_decimal.normalize()}). Returning None."
                )
                return None
            if price_decimal == Decimal("0"):
            ...
            try:
                ...
                formatted = exchange.price_to_precision(
                    symbol=symbol,
                    price=float(price_decimal),
                    rounding_mode=exchange.ROUND,
                )
                if Decimal(formatted) <= Decimal("0"):  # Double-check after formatting
                    lg.warning(
                        f"Formatted price {formatted} became non-positive for input {price_decimal.normalize()}. Returning None."
                    )
                    return None
                return formatted
            except Exception as e:
                lg.error(
                    f"Failed to format price {price_decimal.normalize()} using exchange precision: {e}. Returning None."
                )
                return None
        ...
        if isinstance(trailing_stop_distance, Decimal):
            if trailing_stop_distance > Decimal("0"):
                ...
                if not isinstance(
                ...
                ) or tsl_activation_price <= Decimal("0"):
                    lg.error(
                        f"{NEON_RED}TSL requested ({trailing_stop_distance.normalize()}), but activation price is missing or invalid ({tsl_activation_price}). Cannot set TSL.{RESET}"
                    )
                else:
                    ...
                    if (
                        pos_side == "long"
                        and tsl_activation_price > entry_price
                        or pos_side == "short"
                        and tsl_activation_price < entry_price
                    ):
                    ...
                    if not activation_valid:
                        lg.error(
                            f"{NEON_RED}TSL Activation Price ({tsl_activation_price.normalize()}) is not strictly beyond entry price ({entry_price.normalize()}) for {pos_side} position. Cannot set TSL.{RESET}"
                        )
                    else:
                        # Format TSL distance (must be positive, >= 1 tick)
                        min_dist = max(
                            trailing_stop_distance, min_tick_size
                        ...
                        if formatted_tsl_distance and formatted_activation_price:
                            ...
                            log_parts.append(
                                "  (Fixed SL will be ignored/removed as TSL is active)"
                            )
                        else:
                            lg.error(
                                f"Failed to format valid TSL parameters (Dist: {formatted_tsl_distance}, Act: {formatted_activation_price}). TSL not set."
                            )
        ...
        if not set_tsl and isinstance(stop_loss_price, Decimal):
            if stop_loss_price > Decimal("0"):
                ...
                if not sl_valid:
                    lg.error(
                        f"{NEON_RED}Stop Loss Price ({stop_loss_price.normalize()}) is not strictly beyond entry price ({entry_price.normalize()}) for {pos_side} position. Cannot set SL.{RESET}"
                    )
                else:
                    ...
                    if formatted_sl:
                        ...
                        lg.error(
                            f"Failed to format valid SL price: {stop_loss_price.normalize()}. Fixed SL not set."
                        )
        ...
        if isinstance(take_profit_price, Decimal):
            if take_profit_price > Decimal("0"):
                ...
                if not tp_valid:
                    lg.error(
                        f"{NEON_RED}Take Profit Price ({take_profit_price.normalize()}) is not strictly beyond entry price ({entry_price.normalize()}) for {pos_side} position. Cannot set TP.{RESET}"
                    )
                else:
                    ...
                    if formatted_tp:
                        ...
                        lg.error(
                            f"Failed to format valid TP price: {take_profit_price.normalize()}. Fixed TP not set."
                        )
        ...
        lg.error(
            f"Validation error processing protection parameters for {symbol}: {ve}",
            exc_info=False,
        )
        return False
    except (InvalidOperation, TypeError) as conv_err:
        lg.error(
            f"Conversion error processing protection parameters for {symbol}: {conv_err}",
            exc_info=True,
        )
        return False
    except Exception as fmt_err:
    ...
    if not params_to_set:
        if any_protection_requested:
            lg.warning(
                f"No valid protection parameters to set for {symbol} after validation/formatting. No API call made."
            )
            return False  # Requested action failed validation/formatting
        else:
            lg.info(
                f"No protection parameters provided or cleared for {symbol}. No API call needed."
            )
    ...
    try:
        ...
        lg.warning(
            f"Could not parse positionIdx from position info, using default {position_idx}."
        )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            lg.debug(
                f"Executing set-trading-stop API call for {symbol} (Attempt {attempts + 1}/{MAX_API_RETRIES + 1})..."
            )
            response = exchange.private_post(
                "/v5/position/set-trading-stop", params=final_api_params
            )
            ...
            if ret_code == 0:
                ...
                if any(msg in ret_msg.lower() for msg in no_change_msgs):
                    lg.info(
                        f"{NEON_YELLOW}Position protection already set to target values or no change needed for {symbol}. Response: {ret_msg}{RESET}"
                    )
                else:
                    lg.info(
                        f"{NEON_GREEN}Position protection (SL/TP/TSL) set/updated successfully for {symbol}.{RESET}"
                    )
                return True  # Success

            else:
                lg.error(
                    f"{NEON_RED}Failed to set protection for {symbol}: {ret_msg} (Code: {ret_code}){RESET}"
                )  # Ext: {ret_ext}
                ...
                non_retryable_codes = [
                    ...
                    10001,
                    10002,
                ]  # Parameter, Logic, Auth errors
                if bybit_code in non_retryable_codes:
                    lg.error(
                        f" >> Hint: Error code {bybit_code} indicates a non-retryable issue (parameter/logic/auth). Check parameters and configuration."
                    )
                    return False  # Fatal error
                # If not explicitly fatal, raise to trigger retry
                raise ccxt.ExchangeError(
                    f"Bybit Exchange Error setting protection: {ret_msg} (Code: {ret_code})"
                )
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Max retries reached setting protection after network errors: {e}{RESET}"
                )
                return False
            lg.warning(
            ...
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
                f"{NEON_YELLOW}Rate limit exceeded setting protection: {e}. Retrying in {wait_time}s... (Attempt {attempts + 1}){RESET}"
            )
            time.sleep(wait_time)
            ...
            lg.critical(
                f"{NEON_RED}Authentication Error setting protection: {e}. Stopping bot.{RESET}"
            )
            ...
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Max retries reached setting protection after exchange errors: {e}{RESET}"
                )
                return False
            lg.warning(
            ...
            lg.error(
                f"{NEON_RED}Unexpected error during protection API call (Attempt {attempts + 1}): {e}{RESET}",
                exc_info=True,
            )
            if attempts >= MAX_API_RETRIES:
                lg.error(
                    f"{NEON_RED}Max retries reached after unexpected error setting protection: {e}{RESET}"
                )
                return False
            # Allow retry for unexpected errors
...
def set_trailing_stop_loss(
    ...
    if pos_side not in ["long", "short"] or not entry_price_str:
        lg.error(
            f"{NEON_RED}Missing required position info (side, entryPrice) for TSL calculation ({symbol}).{RESET}"
        )
        return False
    ...
    try:
        ...
        activation_percentage = Decimal(
            str(protection_cfg.get("trailing_stop_activation_percentage"))
        )
        ...
        if activation_percentage < Decimal("0"):
            raise ValueError("Activation percentage cannot be negative")
        ...
        lg.error(
            f"{NEON_RED}Invalid TSL parameter format or position/market info ({symbol}): {ve}. Cannot calculate TSL.{RESET}"
        )
        return False
    except Exception as e:
        lg.error(
            f"{NEON_RED}Error parsing TSL parameters ({symbol}): {e}. Cannot calculate TSL.{RESET}",
            exc_info=True,
        )
        return False
    ...
    try:
        ...
        if activation_price is None or activation_price <= Decimal("0"):
            lg.error(
                f"{NEON_RED}Calculated TSL activation price ({activation_price}) is zero/negative for {symbol}. Cannot set TSL.{RESET}"
            )
            return False
        if (pos_side == "long" and activation_price <= entry_price) or (
            ...
            lg.error(
                f"{NEON_RED}Calculated and rounded TSL Activation Price ({activation_price.normalize()}) is not strictly beyond entry price ({entry_price.normalize()}) for {pos_side} position. Cannot set TSL.{RESET}"
            )
            return False
        ...
        trailing_distance = trailing_distance_raw.quantize(
            min_tick_size, rounding=ROUND_UP
        )
        if trailing_distance < min_tick_size:
            lg.debug(
                f"Calculated TSL distance {trailing_distance_raw.normalize()} rounded to {trailing_distance.normalize()}, which is less than min tick {min_tick_size.normalize()}. Setting distance to one tick."
            )
            trailing_distance = min_tick_size

        if trailing_distance <= Decimal("0"):
            lg.error(
                f"{NEON_RED}Calculated TSL distance zero/negative ({trailing_distance.normalize()}) for {symbol}. Cannot set TSL.{RESET}"
            )
            return False
        ...
        lg.info(
            f"  Entry={entry_price.normalize()}, Act%={activation_percentage:.3%}, Callback%={callback_rate:.3%}"
        )
...
class OrderBlock(TypedDict):
    ...
    right_idx: (
        pd.Timestamp
    )  # Timestamp of the last bar OB is considered valid for (or violated)
...
class StrategyAnalysisResults(TypedDict):
    ...
    dataframe: (
        pd.DataFrame
    )  # DataFrame with all indicator calculations (Decimal columns)
...
class VolumaticOBStrategy:
    ...
    def __init__(
        ...
        self.vt_step_atr_multiplier = Decimal(
            str(
                strategy_cfg.get(
        ...
        self.min_data_len = (
            max(
                ...
                self.ph_left
                + self.ph_right
                + 1,  # Pivot needs left+pivot+right bars (total points)
                self.pl_left + self.pl_right + 1,
            )
            + 10
        )  # Add a general safety buffer

        self.logger.info(
            f"{NEON_CYAN}Initializing VolumaticOB Strategy Engine...{RESET}"
        )
        self.logger.info(
            f"  VT Params: Len={self.vt_length}, ATRLen={self.vt_atr_period}, VolLen={self.vt_vol_ema_length}, ATRMult={self.vt_atr_multiplier.normalize()}, StepMult={self.vt_step_atr_multiplier.normalize()}"
        )
        self.logger.info(
            f"  OB Params: Src={self.ob_source}, PH={self.ph_left}/{self.ph_right}, PL={self.pl_left}/{self.pl_right}, Extend={self.ob_extend}, MaxBoxes={self.ob_max_boxes}"
        )
        self.logger.info(
            f"  Minimum historical data points recommended: {self.min_data_len}"
        )
    ...
    def _ema_swma(self, series: pd.Series, length: int) -> pd.Series:
        ...
        weights = np.array([1.0, 2.0, 2.0, 1.0]) / 6.0  # Ensure weights are float
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        if (
            ...
            self.logger.error(
                "DataFrame index is invalid (not DatetimeIndex or not sorted). Analysis aborted."
            )
            return empty_results

        if len(df) < self.min_data_len:
            self.logger.warning(
                f"{NEON_YELLOW}Insufficient data ({len(df)} rows, need >= {self.min_data_len}) for full strategy analysis. Results may be incomplete or inaccurate.{RESET}"
            )
        ...
        try:
            ...
            for col in float_cols_to_copy:
                if col in df.columns:
                    ...
                    self.logger.error(
                        f"Required column '{col}' not found in DataFrame. Analysis aborted."
                    )
                    return empty_results
            ...
            if df_float.empty:
                self.logger.error(
                    "DataFrame became empty after converting to float and dropping NaNs."
                )
                return empty_results
        except Exception as e:
            self.logger.error(
                f"Error converting DataFrame columns to float for TA calculations: {e}",
                exc_info=True,
            )
            return empty_results
        ...
        try:
            ...
            df_float["trend_changed"] = (
                (df_float["trend_up"].shift(1) != df_float["trend_up"])
            ...
            self.logger.error(
                f"Error during Volumatic Trend indicator calculation: {e}",
                exc_info=True,
            )
            return empty_results
        ...
        try:
            for col in cols_to_copy:
                if col in df_float.columns:
                    ...
                    if source_series.dtype == "bool" or pd.api.types.is_object_dtype(
                        source_series
                    ):
            ...
            self.logger.error(
                f"Error converting calculated float columns back to Decimal: {e}",
                exc_info=True,
            )
            return empty_results
        ...
        if rows_dropped > 0:
            self.logger.debug(
                f"Dropped {rows_dropped} initial rows lacking essential indicator values after calculation."
            )

        if df.empty:
            self.logger.warning(
                f"{NEON_YELLOW}DataFrame empty after calculating indicators and dropping NaNs. Insufficient data or no stable trend/ATR established yet.{RESET}"
            )
            return empty_results

        self.logger.debug(
            "Volumatic Trend calculations complete. Proceeding to Order Blocks."
        )
                                ...
                                if not any(
                                    ...
                                    if pd.notna(box_top) and pd.notna(box_bottom):
                                        ...
                                        if (
                                            ...
                                            self.logger.debug(
                                                f"{NEON_RED}New Bearish OB created at {pivot_bar_idx} [{box_bottom.normalize()} - {box_top.normalize()}]{RESET}"
                                            )
                                ...
                                if not any(
                                    ...
                                    if pd.notna(box_top) and pd.notna(box_bottom):
                                        ...
                                        if (
                                            ...
                                            self.logger.debug(
                                                f"{NEON_GREEN}New Bullish OB created at {pivot_bar_idx} [{box_bottom.normalize()} - {box_top.normalize()}]{RESET}"
                                            )
                                            new_boxes_found_count += 1

                    except KeyError:
                        continue  # Should not happen with aligned index
                    except Exception as e:
                        self.logger.warning(
                            f"Error processing pivot signal at {confirmation_idx}: {e}",
                            exc_info=True,
                        )

            if new_boxes_found_count > 0:
                self.logger.debug(
                    f"Found {new_boxes_found_count} new OB(s). Total counts before pruning: {len(self.bull_boxes)} Bull, {len(self.bear_boxes)} Bear."
                )
                ...
                for box in self.bull_boxes:
                    if box["active"]:
                        if last_close < box["bottom"]:
                            ...
                            self.logger.debug(
                                f"Bull Box {box['id']} ({box['bottom'].normalize()}-{box['top'].normalize()}) VIOLATED by close {last_close.normalize()} at {last_bar_idx}."
                            )
                        elif self.ob_extend:
                            box["right_idx"] = last_bar_idx
                ...
                for box in self.bear_boxes:
                    if box["active"]:
                        if last_close > box["top"]:
                            ...
                            box["right_idx"] = last_bar_idx
                            self.logger.debug(
                                f"Bear Box {box['id']} ({box['bottom'].normalize()}-{box['top'].normalize()}) VIOLATED by close {last_close.normalize()} at {last_bar_idx}."
                            )
                        elif self.ob_extend:
                            box["right_idx"] = last_bar_idx
            else:
                self.logger.warning(
                    "Cannot check OB violations: Last close price or DataFrame is invalid."
                )

            # --- Prune Older Order Blocks ---
            # Keep the N most recent boxes overall (active or inactive)
            all_bull = sorted(
                self.bull_boxes, key=lambda b: b["left_idx"], reverse=True
            )
            all_bear = sorted(
                self.bear_boxes, key=lambda b: b["left_idx"], reverse=True
            )
            self.bull_boxes = all_bull[: self.ob_max_boxes]
            self.bear_boxes = all_bear[: self.ob_max_boxes]

            active_bull_count_after_prune = sum(
                1 for b in self.bull_boxes if b["active"]
            )
            active_bear_count_after_prune = sum(
                1 for b in self.bear_boxes if b["active"]
            )
            self.logger.debug(
                f"Pruned OBs. Total Kept: {len(self.bull_boxes)} Bull ({active_bull_count_after_prune} active), {len(self.bear_boxes)} Bear ({active_bear_count_after_prune} active). (Max {self.ob_max_boxes} total of each type)"
            )

        except Exception as e:
            self.logger.error(
                f"Error during Pivot Order Block processing: {e}", exc_info=True
            )
        ...
        final_close = (
            last_row.get("close", Decimal("NaN"))
            if last_row is not None
            else Decimal("NaN")
        )
        final_atr = (
            last_row.get("atr", Decimal("NaN"))
            if last_row is not None
            else Decimal("NaN")
        )
        final_upper_band = (
            last_row.get("upper_band", Decimal("NaN"))
            if last_row is not None
            else Decimal("NaN")
        )
        final_lower_band = (
            last_row.get("lower_band", Decimal("NaN"))
            if last_row is not None
            else Decimal("NaN")
        )
        final_vol_norm = (
            last_row.get("vol_norm", Decimal("NaN"))
            if last_row is not None
            else Decimal("NaN")
        )
        latest_trend_up = last_row.get("trend_up") if last_row is not None else None
        latest_trend_changed = (
            last_row.get("trend_changed", False) if last_row is not None else False
        )
        ...
        def sanitize_decimal(val, positive_only=False):
            if pd.notna(val) and isinstance(val, Decimal) and np.isfinite(float(val)):
                if positive_only and val <= Decimal("0"):
                    return None
                return val
            return None

        final_trend_up_bool = (
            bool(latest_trend_up)
            if isinstance(latest_trend_up, (bool, np.bool_))
            else None
        )
        final_vol_norm_int = (
            int(final_vol_norm)
            if sanitize_decimal(final_vol_norm) is not None
            else None
        )
        ...
        final_close_dec = sanitize_decimal(final_close) or Decimal(
            "0"
        )  # Default to 0 if close is invalid
        ...
        trend_str = (
            ...
            if results["current_trend_up"] is True
            else f"{NEON_RED}DOWN{RESET}"
            if results["current_trend_up"] is False
        ...
        self.logger.debug(
            f"Strategy Results ({last_idx_time_str}): "
...
class SignalGenerator:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger) -> None:
        ...
        try:
            ...
            if self.ob_entry_proximity_factor < Decimal("1"):
                self.ob_entry_proximity_factor = Decimal("1.0")
            if self.ob_exit_proximity_factor < Decimal("1"):
                self.ob_exit_proximity_factor = Decimal("1.0")

            # ATR Multipliers for initial TP/SL (validated during config load)
            self.initial_tp_atr_multiple = Decimal(
                str(protection_cfg.get("initial_take_profit_atr_multiple"))
            )
            self.initial_sl_atr_multiple = Decimal(
                str(protection_cfg.get("initial_stop_loss_atr_multiple"))
            )
            # Ensure SL multiple is strictly positive
            if self.initial_sl_atr_multiple <= Decimal("0"):
                self.logger.error(
                    f"{NEON_RED}Config Error: initial_stop_loss_atr_multiple must be positive. Using default 1.8.{RESET}"
                )
                self.initial_sl_atr_multiple = Decimal("1.8")

        except (ValueError, TypeError, KeyError, InvalidOperation) as e:
            self.logger.error(
                f"{NEON_RED}Error initializing SignalGenerator with config values: {e}. Using defaults.{RESET}",
                exc_info=True,
            )
        ...
        self.logger.info(
            f"  OB Entry Proximity Factor: {self.ob_entry_proximity_factor.normalize()}, OB Exit Proximity Factor: {self.ob_exit_proximity_factor.normalize()}"
        )
        self.logger.info(
            f"  Initial TP ATR Multiple: {self.initial_tp_atr_multiple.normalize()}, Initial SL ATR Multiple: {self.initial_sl_atr_multiple.normalize()}"
        )
    ...
    def generate_signal(
        ...
        # --- Input Validation ---
        if (
            not isinstance(analysis_results, dict)
            or analysis_results.get("dataframe") is None
            or analysis_results["dataframe"].empty
            or analysis_results.get("current_trend_up") is None
            or not isinstance(analysis_results.get("last_close"), Decimal)
            or analysis_results["last_close"] <= Decimal("0")
            or analysis_results.get("atr") is None
            or analysis_results["atr"] <= Decimal("0")
        ):  # atr checked positive by sanitize
            self.logger.warning(
                f"{NEON_YELLOW}Incomplete or invalid strategy analysis results received. Cannot generate signal. Holding.{RESET}"
            )
            self.logger.debug(
        ...
        self.logger.debug(
            f"Signal Check: Close={latest_close.normalize()}, Trend Up={is_trend_up}, Trend Changed={trend_just_changed}, Position={current_pos_side or 'None'}"
        )
        self.logger.debug(
        ...
        if current_pos_side == "long":
            ...
            if is_trend_up is False:
                if trend_just_changed:
                    signal = "EXIT_LONG"
                    self.logger.warning(
                        f"{NEON_YELLOW}{BRIGHT}EXIT LONG Signal: Trend flipped DOWN.{RESET}"
                    )
            ...
            if signal == "HOLD" and active_bear_obs:
                try:
                    closest_bear_ob = min(
                        active_bear_obs, key=lambda ob: abs(ob["top"] - latest_close)
                    )
                    ...
                    self.logger.debug(
                        f"  Bear OB Exit Check: Closest Bear OB Top={closest_bear_ob['top'].normalize()}, Exit Threshold={exit_threshold.normalize()}"
                    )
                    if latest_close >= exit_threshold:
                        signal = "EXIT_LONG"
                        self.logger.warning(
                            f"{NEON_YELLOW}{BRIGHT}EXIT LONG Signal: Price ({latest_close.normalize()}) >= Bear OB Exit Threshold ({exit_threshold.normalize()}) [OB ID: {closest_bear_ob['id']}]{RESET}"
                        )
                except Exception as e:
            ...
            if is_trend_up is True:
                if trend_just_changed:
                    signal = "EXIT_SHORT"
                    self.logger.warning(
                        f"{NEON_YELLOW}{BRIGHT}EXIT SHORT Signal: Trend flipped UP.{RESET}"
                    )
            ...
            if signal == "HOLD" and active_bull_obs:
                try:
                    closest_bull_ob = min(
                        active_bull_obs, key=lambda ob: abs(ob["bottom"] - latest_close)
                    )
                    ...
                    if self.ob_exit_proximity_factor > Decimal("0"):
                        ...
                        exit_threshold = closest_bull_ob["bottom"]  # Fallback
                    self.logger.debug(
                        f"  Bull OB Exit Check: Closest Bull OB Bottom={closest_bull_ob['bottom'].normalize()}, Exit Threshold={exit_threshold.normalize()}"
                    )
                    if latest_close <= exit_threshold:
                        signal = "EXIT_SHORT"
                        self.logger.warning(
                            f"{NEON_YELLOW}{BRIGHT}EXIT SHORT Signal: Price ({latest_close.normalize()}) <= Bull OB Exit Threshold ({exit_threshold.normalize()}) [OB ID: {closest_bull_ob['id']}]{RESET}"
                        )
                except Exception as e:
        ...
        if current_pos_side is None:
            ...
            if is_trend_up is True and active_bull_obs:
                ...
                if entry_ob:
                    signal = "BUY"
                    self.logger.info(
                        f"{NEON_GREEN}{BRIGHT}BUY Signal: Trend UP & Price ({latest_close.normalize()}) in/near Bull OB {entry_ob['id']} [{entry_ob['bottom'].normalize()}-{entry_ob['top'].normalize()}]. Entry Zone: [{lower_bound.normalize()}, {upper_bound.normalize()}]{RESET}"
                    )
            ...
            # Check for SELL (Short Entry): Trend DOWN and Price interacts with Bear OB
                ...
                for ob in active_bear_obs:
                    if self.ob_entry_proximity_factor > Decimal("0"):
                        ...
                        lower_bound = ob["bottom"]  # Fallback
                    upper_bound = ob["top"]
                    if lower_bound <= latest_close <= upper_bound:
                        entry_ob = ob
                        break
                if entry_ob:
                    signal = "SELL"
                    self.logger.info(
                        f"{NEON_RED}{BRIGHT}SELL Signal: Trend DOWN & Price ({latest_close.normalize()}) in/near Bear OB {entry_ob['id']} [{entry_ob['bottom'].normalize()}-{entry_ob['top'].normalize()}]. Entry Zone: [{lower_bound.normalize()}, {upper_bound.normalize()}]{RESET}"
                    )
        ...
        if signal == "HOLD":
            ...
            if current_pos_side:
                self.logger.debug(
                    f"HOLD Signal: Trend ({trend_status}) compatible with existing {current_pos_side} position. No exit condition met."
                )
            else:
                ob_status = (
                    "No relevant active OBs found for current trend or no trend."
                )
                if is_trend_up is True and active_bull_obs:
                    ob_status = f"Price ({latest_close.normalize()}) not within entry zone of active Bull OBs."
                elif is_trend_up is False and active_bear_obs:
                    ob_status = f"Price ({latest_close.normalize()}) not within entry zone of active Bear OBs."
                self.logger.debug(
                    f"HOLD Signal: No position open. Trend is {trend_status}. {ob_status}"
                )
    ...
    def calculate_initial_tp_sl(
        ...
        if signal not in ["BUY", "SELL"]:
            lg.error(f"Invalid signal '{signal}'.")
            return None, None
        if entry_price <= Decimal("0"):
            lg.error(f"Invalid entry price ({entry_price}).")
            return None, None
        if atr <= Decimal("0"):
            lg.error(f"Invalid ATR value ({atr}).")
            return None, None
        if market_info.get("precision", {}).get("price") is None:
            lg.error("Missing market price precision info.")
            return None, None
        ...
        try:
            ...
            def format_level(
                price_decimal: Decimal | None, level_type: str
            ) -> Decimal | None:
                if price_decimal is None:
                    return None
                if price_decimal <= Decimal("0"):
                    lg.warning(
                        f"Calculated {level_type} is zero/negative ({price_decimal.normalize()}). Setting to None."
                    )
                    return None
                try:
                    ...
                    if formatted_decimal <= Decimal("0"):
                        lg.warning(
                            f"Formatted {level_type} became zero/negative ({formatted_decimal.normalize()}). Setting to None."
                        )
                        return None
                    return formatted_decimal
                except Exception as e:
                    lg.error(
                        f"Error formatting {level_type} price {price_decimal.normalize()}: {e}. Setting to None."
                    )
                    return None
            ...
            if stop_loss is not None:
                if (signal == "BUY" and stop_loss >= entry_price) or (
                    ...
                    lg.warning(
                        f"Formatted {signal} SL ({stop_loss.normalize()}) not strictly beyond entry ({entry_price.normalize()}). Adjusting by one tick."
                    )
                    stop_loss = (
                        (entry_price - min_tick_size)
                        if signal == "BUY"
                        else (entry_price + min_tick_size)
                    )
                    stop_loss = format_level(
                        stop_loss, "SL"
            ...
            if take_profit is not None:
                if (signal == "BUY" and take_profit <= entry_price) or (
                    ...
                    lg.warning(
                        f"Formatted {signal} TP ({take_profit.normalize()}) not strictly beyond entry ({entry_price.normalize()}). Setting TP to None."
                    )
                    take_profit = None
            ...
            if stop_loss is None:
                lg.error(
                    f"{NEON_RED}Stop Loss calculation failed or resulted in an invalid level. Cannot proceed with sizing.{RESET}"
                )
            ...
            lg.error(
                f"{NEON_RED}Error calculating initial TP/SL: {ve}{RESET}", exc_info=True
            )
            return None, None
        except Exception as e:
            lg.error(
...
def analyze_and_trade_symbol(
    ...
    if fetch_limit < required_fetch_limit:
        lg.warning(
            f"{NEON_YELLOW}Configured fetch_limit ({fetch_limit}) < recommended ({required_fetch_limit}). Increasing to {required_fetch_limit}.{RESET}"
        )
        fetch_limit = required_fetch_limit

    klines_df = fetch_klines_ccxt(
        exchange, symbol, ccxt_interval, limit=fetch_limit, logger=lg
    )
    if klines_df.empty or len(klines_df) < min_strategy_len:
        lg.error(
            f"Failed to fetch sufficient kline data for {symbol} (fetched {len(klines_df)}, need >= {min_strategy_len}). Skipping cycle."
        )
        return
    ...
    try:
        ...
        lg.error(
            f"{NEON_RED}Error during strategy analysis update for {symbol}: {analysis_err}{RESET}",
            exc_info=True,
        )
        return  # Skip cycle
    ...
    if (
        ...
        lg.error(
            f"{NEON_RED}Strategy analysis produced incomplete/invalid results. Cannot proceed. Skipping cycle.{RESET}"
        )
    ...
    if price_for_checks <= Decimal("0"):
        lg.error(
            f"{NEON_RED}Cannot get valid price (current or last close) for {symbol}. Skipping cycle.{RESET}"
        )
        return
    if current_market_price is None:
        lg.warning(
            f"{NEON_YELLOW}Using last close price {latest_close.normalize()} for protection checks as current price fetch failed.{RESET}"
        )
    ...
    try:
        ...
        lg.error(
            f"{NEON_RED}Error during signal generation for {symbol}: {signal_err}{RESET}",
            exc_info=True,
        )
        return  # Skip cycle
    ...
    if not trading_enabled:
        lg.info(
            f"{NEON_YELLOW}Trading disabled. Generated Signal: {signal}. Analysis complete for {symbol}.{RESET}"
        )
    ...
    # ===========================================
    lg.debug(
        f"Trading enabled. Signal: {signal}. Position: {'Yes' if open_position else 'No'}"
    )
    ...
    if open_position is None:
        if signal in ["BUY", "SELL"]:
            lg.info(
                f"*** {NEON_GREEN if signal == 'BUY' else NEON_RED}{BRIGHT}{signal} Signal & No Position: Initiating Trade Entry Sequence for {symbol}{RESET} ***"
            )
            ...
            if balance is None or balance <= Decimal("0"):
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Cannot fetch balance or balance is zero/negative ({balance}).{RESET}"
                )
                return

            lg.debug(
                f"Calculating initial TP/SL for sizing (Est. Entry={latest_close.normalize()}, ATR={current_atr.normalize()})"
            )
            ...
            if initial_sl_calc is None:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Initial SL calculation failed. Cannot size.{RESET}"
                )
                return
            if initial_tp_calc is None:
                lg.warning(
                    f"{NEON_YELLOW}Initial TP calculation failed/disabled. Proceeding without initial fixed TP.{RESET}"
                )
            ...
            if market_info.get("is_contract", False):
                ...
                if leverage > 0:
                    leverage_set_success = set_leverage_ccxt(
                        exchange, symbol, leverage, market_info, lg
                    )
                    if not leverage_set_success:
                        lg.error(
                            f"{NEON_RED}Trade Aborted ({symbol} {signal}): Failed to set leverage {leverage}x.{RESET}"
                        )
                        return
            ...
            if position_size is None or position_size <= Decimal("0"):
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Invalid position size ({position_size}). Check balance, risk, SL, limits.{RESET}"
                )
                return

            # --- Place Entry Trade ---
            lg.info(
                f"==> Placing {signal} market order | Size: {position_size.normalize()} {market_info.get('base', '')} <=="
            )
            ...
            if trade_order and trade_order.get("id"):
                ...
                lg.info(
                    f"Order {order_id} placed. Waiting {confirm_delay}s for confirmation..."
                )
                ...
                if confirmed_position:
                    try:
                        ...
                        entry_price_actual = (
                            Decimal(str(entry_price_actual_str))
                            if entry_price_actual_str
                            else latest_close
                        )  # Fallback
                        if entry_price_actual <= Decimal("0"):
                            entry_price_actual = latest_close  # Ensure positive
                        lg.info(
                            f"{NEON_GREEN}Position Confirmed! Side: {confirmed_position.get('side')}, Actual Entry: ~{entry_price_actual.normalize()}{RESET}"
                        )

                        # --- Set Protection based on Actual Entry ---
                        protection_cfg = config.get("protection", {})
                        tp_for_protection, sl_for_protection = (
                        ...
                        if sl_for_protection is None:
                            lg.error(
                                f"{NEON_RED}Failed to recalculate SL for setting protection. Position vulnerable!{RESET}"
                            )
                        ...
                        if protection_cfg.get("enable_trailing_stop", True):
                            lg.info(
                                f"Setting Trailing Stop Loss (using Entry={entry_price_actual.normalize()})..."
                            )
                            protection_set_success = set_trailing_stop_loss(
                                ...
                                config,
                                lg,
                                take_profit_price=tp_for_protection,
                            )  # Pass recalculated TP
                        elif not protection_cfg.get("enable_trailing_stop", True) and (
                            protection_cfg.get("initial_stop_loss_atr_multiple", 0) > 0
                            or protection_cfg.get("initial_take_profit_atr_multiple", 0)
                            > 0
                        ):
                            lg.info(
                                f"Setting Fixed SL/TP (using Entry={entry_price_actual.normalize()})..."
                            )
                            if sl_for_protection or tp_for_protection:
                                protection_set_success = _set_position_protection(
                                    ...
                                    confirmed_position,
                                    lg,
                                    stop_loss_price=sl_for_protection,
                                    take_profit_price=tp_for_protection,
                                )
                            else:
                                lg.warning(
                                    f"{NEON_YELLOW}No valid fixed SL/TP levels calculated based on actual entry. No fixed protection set.{RESET}"
                                )
                                protection_set_success = True  # No action needed
                        else:
                            lg.info(
                                "Neither TSL nor Fixed SL/TP enabled. No protection set."
                            )
                        ...
                        if protection_set_success:
                            ...
                            lg.error(
                                f"{NEON_RED}=== TRADE placed BUT FAILED TO SET PROTECTION ({symbol} {signal}). MANUAL MONITORING REQUIRED! ==="
                            )

                    except Exception as post_trade_err:
                        lg.error(
                            f"{NEON_RED}Critical error during post-trade setup ({symbol}): {post_trade_err}{RESET}",
                            exc_info=True,
                        )
                        lg.warning(
                            f"{NEON_YELLOW}Position may be open without protection. Manual check needed!{RESET}"
                        )
                else:
                    lg.error(
                        f"{NEON_RED}Trade order {order_id} placed, but FAILED TO CONFIRM position after {confirm_delay}s delay! Manual investigation required!{RESET}"
                    )
            else:
                lg.error(
                    f"{NEON_RED}=== TRADE EXECUTION FAILED ({symbol} {signal}). See previous logs. ==="
                )
        else:  # signal == HOLD and no position
            lg.info(
                f"Signal is HOLD and no open position for {symbol}. No entry action."
            )
    ...
    # === Scenario 2: Existing Open Position ===
        ...
        lg.info(
            f"Existing {pos_side.upper()} position found for {symbol} (Size: {pos_size_fmt}). Signal: {signal}"
        )
        ...
        if exit_signal_triggered:
            lg.warning(
                f"{NEON_YELLOW}{BRIGHT}*** {signal} Signal Triggered! Initiating Close Sequence for {pos_side} position on {symbol}... ***{RESET}"
            )
            try:
                ...
                if size_to_close <= Decimal("0"):
                    lg.warning(
                        f"Position size to close is zero/negative ({size_to_close.normalize()}). Already closed? Skipping."
                    )
                else:
                    lg.info(
                        f"==> Placing {close_side_signal} MARKET order (reduceOnly=True) | Size: {size_to_close.normalize()} <=="
                    )
                    ...
                    if close_order and close_order.get("id"):
                        lg.info(
                            f"{NEON_GREEN}Position CLOSE order placed successfully. Order ID: {close_order.get('id', 'N/A')}{RESET}"
                        )
                    else:
                        lg.error(
                            f"{NEON_RED}Failed to place CLOSE order. Manual check required!{RESET}"
                        )
            except Exception as close_err:
                lg.error(
                    f"{NEON_RED}Unexpected error closing position {symbol}: {close_err}{RESET}",
                    exc_info=True,
                )
        ...
        # --- Manage Existing Position (HOLD signal or compatible signal) ---
            ...
            lg.debug(
                f"Signal ({signal}) allows holding. Managing protections for {pos_side} position..."
            )
            protection_cfg = config.get("protection", {})
            ...
            try:
                if tsl_dist_on_pos_str and Decimal(str(tsl_dist_on_pos_str)) > Decimal(
                    "0"
                ):
            ...
            with contextlib.suppress(Exception):
                current_sl_price = (
                    Decimal(str(open_position.get("stopLossPrice")))
                    if open_position.get("stopLossPrice")
                    and str(open_position.get("stopLossPrice")) != "0"
                    else None
                )
            with contextlib.suppress(Exception):
                current_tp_price = (
                    Decimal(str(open_position.get("takeProfitPrice")))
                    if open_position.get("takeProfitPrice")
                    and str(open_position.get("takeProfitPrice")) != "0"
                    else None
                )
            ...
            with contextlib.suppress(Exception):
                entry_price_actual = (
                    Decimal(str(open_position.get("entryPrice")))
                    if open_position.get("entryPrice")
                    else None
                )
            ...
            if (
                be_enabled
                and not is_tsl_active_on_pos
                and entry_price_actual
                and current_atr
                and price_for_checks > Decimal("0")
            ):
                lg.debug(
                    f"Checking Break-Even (Entry: {entry_price_actual.normalize()}, Price: {price_for_checks.normalize()}, ATR: {current_atr.normalize()})..."
                )
                try:
                    ...
                    be_offset_ticks = int(
                        protection_cfg.get("break_even_offset_ticks", 2)
                    )
                    price_diff = (
                        (price_for_checks - entry_price_actual)
                        if pos_side == "long"
                        else (entry_price_actual - price_for_checks)
                    )
                    profit_in_atr = price_diff / current_atr
                    lg.debug(
                        f"  BE Check: Profit ATRs = {profit_in_atr:.2f}, Trigger = {be_trigger_atr_mult.normalize()}"
                    )

                    if profit_in_atr >= be_trigger_atr_mult:
                        lg.info(
                            f"{NEON_PURPLE}{BRIGHT}BE Profit target ({be_trigger_atr_mult.normalize()} ATRs) REACHED!{RESET}"
                        )
                        min_tick_size = Decimal(str(market_info["precision"]["price"]))
                        tick_offset_value = min_tick_size * Decimal(
                            str(be_offset_ticks)
                        )
                        be_stop_price = (
                            (entry_price_actual + tick_offset_value).quantize(
                                min_tick_size, rounding=ROUND_UP
                            )
                            if pos_side == "long"
                            else (entry_price_actual - tick_offset_value).quantize(
                                min_tick_size, rounding=ROUND_DOWN
                            )
                        )
                        ...
                        if be_stop_price and be_stop_price > Decimal("0"):
                            ...
                            if current_sl_price is None:
                                update_be = True
                                lg.info("  No current SL, setting BE.")
                            elif (
                                pos_side == "long" and be_stop_price > current_sl_price
                            ):
                                update_be = True
                                lg.info(
                                    f"  BE SL {be_stop_price.normalize()} > Current SL {current_sl_price.normalize()}."
                                )
                            elif (
                                ...
                                lg.debug(
                                    f"  Current SL ({current_sl_price.normalize()}) already >= BE target ({be_stop_price.normalize()})."
                                )

                            if update_be:
                                lg.warning(
                                    f"{NEON_PURPLE}{BRIGHT}*** Moving SL to Break-Even for {symbol} at {be_stop_price.normalize()} ***{RESET}"
                                )
                            ...
                            lg.error(
                                f"{NEON_RED}BE triggered, but calculated BE stop price ({be_stop_price}) invalid.{RESET}"
                            )
                    else:
                        lg.debug("BE Profit target not reached.")
                except Exception as be_err:
                    lg.error(
                        f"{NEON_RED}Error during BE check: {be_err}{RESET}",
                        exc_info=True,
                    )
            elif be_enabled and is_tsl_active_on_pos:
                lg.debug("BE check skipped: TSL is active.")
            elif not be_enabled:
                lg.debug("BE check skipped: Disabled in config.")
            else:
                lg.debug("BE check skipped: Missing data (entry/price/ATR).")
            ...
            if (
                ...
                lg.warning(
                    f"{NEON_YELLOW}TSL enabled but not active. Attempting TSL setup/recovery...{RESET}"
                )
                tp_recalc, _ = signal_generator.calculate_initial_tp_sl(
                    entry_price=entry_price_actual,
                    signal=pos_side.upper(),
                    atr=current_atr,
                    market_info=market_info,
                    exchange=exchange,
                )
                ...
                if tsl_set_success:
                    lg.info("TSL setup/recovery attempt successful.")
                else:
                    lg.error("TSL setup/recovery attempt failed.")
...
def main() -> None:
    ...
    init_logger.info(
        f"{BRIGHT}--- Starting Pyrmethus Volumatic Bot ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{RESET}"
    )

    # Config loaded globally, validation and defaults applied

    init_logger.info(
        f"Config Loaded: Quote={QUOTE_CURRENCY}, Trading={CONFIG.get('enable_trading')}, Sandbox={CONFIG.get('use_sandbox')}"
    )
    try:
        init_logger.info(
            f"Versions: Python={os.sys.version.split()[0]}, CCXT={ccxt.__version__}, Pandas={pd.__version__}, NumPy={np.__version__}, PandasTA={getattr(ta, 'version', 'N/A')}"
        )
    except Exception as e:
        init_logger.warning(f"Could not get library versions: {e}")
    ...
    if trading_enabled_cfg:
        init_logger.warning(
            f"{NEON_YELLOW}{BRIGHT}!!! LIVE TRADING IS ENABLED !!!{RESET}"
        )
        if sandbox_mode_cfg:
            init_logger.warning(
                f"{NEON_YELLOW}Mode: SANDBOX (Testnet) Environment{RESET}"
            )
        else:
            init_logger.warning(
                f"{NEON_RED}{BRIGHT}Mode: LIVE (Real Money) Environment{RESET}"
            )

        protection_cfg = CONFIG.get("protection", {})
        init_logger.warning(f"{BRIGHT}--- Key Settings Review ---{RESET}")
        init_logger.warning(
            f"  Risk Per Trade: {CONFIG.get('risk_per_trade', 0) * 100:.2f}%"
        )
        init_logger.warning(f"  Leverage: {CONFIG.get('leverage', 0)}x")
        init_logger.warning(
            f"  Trailing SL: {'ENABLED' if protection_cfg.get('enable_trailing_stop') else 'DISABLED'} "
        ...
        init_logger.warning(
            f"  Break Even: {'ENABLED' if protection_cfg.get('enable_break_even') else 'DISABLED'} "
        ...
        try:
            input(
                f"{BRIGHT}>>> Review settings CAREFULLY. Press {NEON_GREEN}Enter{RESET}{BRIGHT} to continue, or {NEON_RED}Ctrl+C{RESET}{BRIGHT} to abort... {RESET}"
            )
        ...
        init_logger.info(
            f"{NEON_YELLOW}Trading is disabled in config. Running in analysis-only mode.{RESET}"
        )
    ...
    if not exchange:
        init_logger.critical(
            f"{NEON_RED}Failed to initialize exchange connection. Exiting.{RESET}"
        )
        return
    ...
    while target_symbol is None:  # Loop until a valid symbol is confirmed
        try:
            symbol_input_raw = (
                input(
                    f"{NEON_YELLOW}Enter the trading symbol (e.g., BTC/USDT, ETH/USDT:USDT): {RESET}"
                )
                .strip()
                .upper()
            )
            ...
            if "/" in symbol_input_raw and ":" not in symbol_input_raw:
                symbols_to_try.append(
                    f"{symbol_input_raw}:{QUOTE_CURRENCY}"
                )  # e.g., BTC/USDT -> BTC/USDT:USDT
            elif (
                ":" in symbol_input_raw
                and "/" not in symbol_input_raw
                and symbol_input_raw[-1] != ":"
            ):
            ...
            for symbol_attempt in symbols_to_try:
                ...
                market_info_attempt = get_market_info(
                    exchange, symbol_attempt, init_logger
                )

                if market_info_attempt:
                    target_symbol = market_info_attempt[
                        "symbol"
                    ]  # Use the validated symbol from CCXT
                    market_info = market_info_attempt  # Store the validated market info
                    market_type_desc = market_info.get(
                        "contract_type_str", "Unknown Type"
                    )
                    init_logger.info(
                    ...
                    if (
                        market_info.get("precision", {}).get("price") is None
                        or market_info.get("precision", {}).get("amount") is None
                    ):
                        init_logger.error(
                            f"{NEON_RED}CRITICAL: Validated market '{target_symbol}' is missing required price/amount precision! Cannot continue.{RESET}"
                        )
                ...
                if (
                    market_info is None
                ):  # If loop finished without finding a valid symbol
                    init_logger.error(
                        f"{NEON_RED}Symbol '{symbol_input_raw}' (and variations {symbols_to_try}) not found or invalid on {exchange.id}. Please check the symbol and try again.{RESET}"
                    )
    ...
    while selected_interval is None:  # Loop until valid interval
        ...
        interval_input = input(
            f"{NEON_YELLOW}Enter analysis interval {VALID_INTERVALS} (default: {default_interval}): {RESET}"
        ).strip()
        ...
        if interval_input in VALID_INTERVALS:
            ...
            init_logger.info(
                f"Using interval: {selected_interval} (CCXT Timeframe: {ccxt_tf})"
            )
            break
        else:
            init_logger.error(
                f"{NEON_RED}Invalid interval '{interval_input}'. Please choose from {VALID_INTERVALS}.{RESET}"
            )

    # --- Setup Symbol-Specific Logger ---
    symbol_logger = setup_logger(
        target_symbol
    )  # Use the validated symbol for logger name
    symbol_logger.info(
        f"---=== {BRIGHT}Starting Trading Loop for {target_symbol} ({CONFIG['interval']}){RESET} ===---"
    )
    symbol_logger.info(
        f"Trading Enabled: {trading_enabled_cfg}, Sandbox Mode: {sandbox_mode_cfg}"
    )
    protection_cfg = CONFIG.get("protection", {})  # Re-get for symbol logger
    symbol_logger.info(
        f"Settings: Risk={CONFIG['risk_per_trade']:.2%}, Leverage={CONFIG['leverage']}x, TSL={'ON' if protection_cfg.get('enable_trailing_stop') else 'OFF'}, BE={'ON' if protection_cfg.get('enable_break_even') else 'OFF'}"
    )
    symbol_logger.debug(
        f"Strategy Params: {json.dumps(CONFIG.get('strategy_params', {}))}"
    )
    ...
    try:
        ...
        symbol_logger.critical(
            f"{NEON_RED}Failed to initialize strategy engine or signal generator: {engine_init_err}. Exiting.{RESET}",
            exc_info=True,
        )
        return
    ...
    # --- Main Trading Loop ---
    symbol_logger.info(
        f"{BRIGHT}Entering main trading loop... Press Ctrl+C to stop.{RESET}"
    )
    try:
        while True:
            loop_start_time = time.time()
            symbol_logger.debug(
                f">>> New Loop Cycle Start: {datetime.now(TIMEZONE).strftime('%H:%M:%S %Z')}"
            )
            ...
            # --- Robust Error Handling for Common Issues within the Loop ---
                ...
                symbol_logger.warning(
                    f"{NEON_YELLOW}Rate limit exceeded: {e}. Waiting 60 seconds...{RESET}"
                )
                time.sleep(60)
            except (
                ccxt.NetworkError,
                ...
                symbol_logger.error(
                    f"{NEON_RED}Network error encountered: {e}. Waiting {RETRY_DELAY_SECONDS * 3}s before next cycle...{RESET}"
                )
                time.sleep(RETRY_DELAY_SECONDS * 3)
            except ccxt.AuthenticationError as e:
                symbol_logger.critical(
                    f"{NEON_RED}CRITICAL Authentication Error: {e}. API keys may be invalid/expired/permissions revoked. Stopping bot.{RESET}"
                )
                break  # Exit the main trading loop
            except ccxt.ExchangeNotAvailable as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange not available: {e}. Waiting 60 seconds...{RESET}"
                )
                time.sleep(60)
            except ccxt.OnMaintenance as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange under maintenance: {e}. Waiting 5 minutes...{RESET}"
                )
                time.sleep(300)
            except ccxt.ExchangeError as e:
                symbol_logger.error(
                    f"{NEON_RED}Unhandled Exchange Error in main loop: {e}{RESET}",
                    exc_info=True,
                )
                symbol_logger.warning(
                    f"{NEON_YELLOW}Pausing for 10 seconds before next cycle.{RESET}"
                )
                time.sleep(10)  # Short pause
            except Exception as loop_error:
                symbol_logger.error(
                    f"{NEON_RED}Critical unexpected error in main trading loop: {loop_error}{RESET}",
                    exc_info=True,
                )
                symbol_logger.warning(
                    f"{NEON_YELLOW}Pausing for 15 seconds due to unexpected error.{RESET}"
                )
                time.sleep(15)
            ...
            symbol_logger.debug(
                f"<<< Loop cycle processed in {elapsed_time:.2f}s. Sleeping for {sleep_time:.2f}s..."
            )
            if sleep_time > 0:
        ...
        init_logger.critical(
            f"{NEON_RED}CRITICAL UNHANDLED ERROR outside main loop: {critical_error}{RESET}",
            exc_info=True,
        )
    finally:
        # --- Shutdown Procedures ---
        shutdown_msg = (
            f"--- Pyrmethus Volumatic Bot for {target_symbol or 'N/A'} Stopping ---"
        )
