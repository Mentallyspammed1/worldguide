...
class VolumaticOBStrategy:
    ...
    def __init__(self, config: Dict[str, Any], market_info: Dict[str, Any], logger: logging.Logger):
        ...
        if self.min_data_len > BYBIT_API_KLINE_LIMIT + 10: self.logger.error(f"{NEON_RED}{BRIGHT}CONFIG WARNING:{RESET} Strategy requires {self.min_data_len} candles > API limit {BYBIT_API_KLINE_LIMIT}. Reduce lookbacks.")
    ...
    def _find_pivots(self, series: pd.Series, left: int, right: int, high: bool) -> pd.Series:
        ...
        if not isinstance(series, pd.Series) or series.empty or left < 1 or right < 1:
        ...
        pivots = pd.Series(True, index=series.index) # Assume pivot initially
        for i in range(1, left + 1):
            shifted = num_series.shift(i)
            condition = num_series > shifted if high else num_series < shifted
            pivots &= condition # Must be greater/less than all left candles
        for i in range(1, right + 1):
            shifted = num_series.shift(-i)
            condition = num_series > shifted if high else num_series < shifted
            pivots &= condition # Must be greater/less than all right candles

        return pivots.fillna(False) # Fill NaNs resulting from shifts
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        try:
            ...
            if not df.empty:
                 ...
                 for pivot_idx in df.index: # Use index directly
                     try:
                         ...
                         if is_pivot_high:
                             # Check if OB from this pivot already exists
                             if not any(b['left_idx'] == pivot_idx and b['type'] == 'bear' for b in self.bear_boxes):
                                 ...
                                 if pd.notna(top) and pd.notna(bot) and isinstance(top, Decimal) and isinstance(bot, Decimal) and top > bot:
                                     ...
                                     self.logger.debug(f"  New Bear OB: {new_box['id']} @ {piv_idx.strftime('%H:%M')} [{bot.normalize()}-{top.normalize()}]")
                         ...
                         if is_pivot_low:
                              if not any(b['left_idx'] == pivot_idx and b['type'] == 'bull' for b in self.bull_boxes):
                                 ...
                                 if pd.notna(top) and pd.notna(bot) and isinstance(top, Decimal) and isinstance(bot, Decimal) and top > bot:
                                     ...
                                     self.logger.debug(f"  New Bull OB: {new_box['id']} @ {piv_idx.strftime('%H:%M')} [{bot.normalize()}-{top.normalize()}]")
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger,
    ...
    if klines_df.empty or fetched_count < min_req_data:
        ...
        if api_limit_hit_insufficient: lg.error(f"{NEON_RED}CRITICAL DATA SHORTFALL:{RESET} Fetched {fetched_count} (API limit), need {min_req_data}. {NEON_YELLOW}Reduce lookbacks in config! Skipping.{RESET}")
...
def main() -> None:
    ...
    init_logger.info(f"{BRIGHT}--- Starting Pyrmethus Bot v1.1.5 ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---{RESET}")
