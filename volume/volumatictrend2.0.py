...
-   Robust API interaction with retries and error handling (Network, Rate Limit, Auth, Exchange errors with specific code checks).
...
# Set Decimal precision globally for high-accuracy financial calculations
getcontext().prec = 28
# Initialize Colorama for console colors, resetting after each print statement
...
if not API_KEY or not API_SECRET:
    # Critical error if API keys are missing - raise immediately
...
# Default timezone if not set in .env, fallback to UTC if tzdata is not installed or invalid
TIMEZONE_STR: str = os.getenv("TIMEZONE", "UTC") # Default to UTC if not specified or America/Chicago fails
try:
    TIMEZONE = ZoneInfo(TIMEZONE_STR)
    # Validate if the timezone string is valid by creating ZoneInfo
    # If it fails, the except block will catch it and set to UTC
    str(datetime.now(TIMEZONE).tzname()) # Quick check if tzname is accessible
except Exception:
...
LOOP_DELAY_SECONDS: int = 15    # Default delay between trading cycles for all symbols
BYBIT_API_KLINE_LIMIT: int = 1000  # Maximum number of Klines Bybit V5 API returns per request

# Timeframes Mapping
# Bybit API expects specific strings for intervals, CCXT has its own standard
...
DEFAULT_OB_ENTRY_PROXIMITY_FACTOR: float = 1.005 # Price must be <= OB top * factor (long) or >= OB bottom / factor (short)
DEFAULT_OB_EXIT_PROXIMITY_FACTOR: float = 1.001  # Exit if price >= Bear OB top * factor or <= Bull OB bottom / factor

# Protection Defaults
DEFAULT_ENABLE_TSL: bool = True      # Use trailing stop loss
DEFAULT_TSL_CALLBACK_RATE: float = 0.005  # TSL distance as % of activation price (e.g., 0.005 = 0.5%)
DEFAULT_TSL_ACTIVATION_PERCENTAGE: float = 0.003  # Activate TSL when price moves this % from entry (e.g., 0.003 = 0.3%)
DEFAULT_ENABLE_BE: bool = True         # Move SL to entry + offset when profit target hit
DEFAULT_BE_TRIGGER_ATR_MULTIPLE: float = 1.0  # Profit needed (in ATR multiples) to trigger BE
DEFAULT_BE_OFFSET_TICKS: int = 2       # Move SL this many ticks beyond entry for BE
DEFAULT_INITIAL_SL_ATR_MULTIPLE: float = 1.8  # Initial SL distance in ATR multiples
DEFAULT_INITIAL_TP_ATR_MULTIPLE: float = 0.7  # Initial TP distance in ATR multiples (0 to disable)
...
class MarketInfo(TypedDict):
    ...
    # Standard CCXT fields (may vary slightly by exchange) - using Any for fields not directly used as Decimal/bool
    id: str
    symbol: str
    base: str
    quote: str
    ...
    strike: float | None
    optionType: str | None
    precision: dict[str, Any]  # {'amount': Any, 'price': Any, 'cost': Any, 'base': float, 'quote': float}
    limits: dict[str, Any]  # {'leverage': dict, 'amount': dict, 'price': dict, 'cost': dict}
    info: dict[str, Any]  # Exchange-specific raw info
    # Custom added fields for convenience (parsed to Decimal)
    is_contract: bool
    is_linear: bool
    is_inverse: bool
...
class PositionInfo(TypedDict):
    ...
    # Standard CCXT fields (may vary slightly by exchange) - using Any for fields not directly used as Decimal/str/bool
    id: str | None
    symbol: str
    timestamp: int | None
    datetime: str | None
    ...
    tslActivationPrice: str | None  # Parsed from info if available (Bybit TSL activation price)
...
def setup_logger(name: str) -> logging.Logger:
    ...
    """
    # Sanitize name for filenames and logger registry keys
    ...
    try:
        ...
        print(f"ERROR: Failed to set up file logger for '{name}': {e}", file=sys.stderr)
    ...
    try:
        ...
        class NeonConsoleFormatter(SensitiveFormatter):
            ...
            def format(self, record: logging.LogRecord) -> str:
                ...
                # Using UTC for log time to avoid ambiguity across machines/systems,
                # unless TIMEZONE is explicitly set and loaded.
                tz_abbr = datetime.now(TIMEZONE).astimezone(TIMEZONE).strftime('%Z') if TIMEZONE else 'UTC'
                log_fmt = (
                    f"{NEON_BLUE}%(asctime)s{RESET} {DIM}{tz_abbr}{RESET} - "
                ...
                formatter.converter = lambda *args: datetime.now(TIMEZONE).timetuple() if TIMEZONE else time.gmtime() # Fallback to GMT if ZoneInfo fails
        ...
        print(f"ERROR: Failed to set up console logger for '{name}': {e}", file=sys.stderr)
...
init_logger.info(f"Logging initialized. Console level: {logging.getLevelName(init_logger.handlers[1].level)}")
...
def load_config(filepath: str) -> dict[str, Any]:
    ...
    default_config = {
        ...
        "strategy_params": {
            ...
            # DEFAULT_VT_STEP_ATR_MULTIPLIER is currently unused but kept in code constants for reference
            ...
            "ob_entry_proximity_factor": DEFAULT_OB_ENTRY_PROXIMITY_FACTOR,
            "ob_exit_proximity_factor": DEFAULT_OB_EXIT_PROXIMITY_FACTOR
        },
        # Position Protection Parameters
        "protection": {
             "enable_trailing_stop": DEFAULT_ENABLE_TSL,
             "trailing_stop_callback_rate": DEFAULT_TSL_CALLBACK_RATE,
             "trailing_stop_activation_percentage": DEFAULT_TSL_ACTIVATION_PERCENTAGE,
             "enable_break_even": DEFAULT_ENABLE_BE,
             "break_even_trigger_atr_multiple": DEFAULT_BE_TRIGGER_ATR_MULTIPLE,
             "break_even_offset_ticks": DEFAULT_BE_OFFSET_TICKS,
             "initial_stop_loss_atr_multiple": DEFAULT_INITIAL_SL_ATR_MULTIPLE,
             "initial_take_profit_atr_multiple": DEFAULT_INITIAL_TP_ATR_MULTIPLE
        }
    }
    ...
    try:
        ...
        def validate_numeric(cfg: dict, key_path: str, min_val: int | float | Decimal, max_val: int | float | Decimal,
            ...
            Args:
                cfg: The configuration dictionary (or nested dict) being validated.
                key_path: The dot-separated path to the key (e.g., "strategy_params.vt_length").
                min_val: The minimum allowed value (inclusive, unless is_strict_min=True).
                max_val: The maximum allowed value (inclusive).
                is_strict_min: If True, the value must be strictly greater than min_val.
                is_int: If True, the value must be an integer.
                allow_zero: If True, the value 0 is allowed regardless of min_val.
            ...
            keys = key_path.split('.')
            current_level = cfg
            default_level = default_config # Use the original default_config for getting default values
            try:
                ...
                default_val = default_level.get(leaf_key) # Get default value from original defaults
            ...
            try:
                # Attempt to convert to Decimal for comparison
                num_val_dec = Decimal(str(original_val))
                ...
                zero_ok = allow_zero and num_val_dec == Decimal('0')

                # Check range
                range_check = (num_val_dec > min_dec if is_strict_min else num_val_dec >= min_dec) and num_val_dec <= max_dec

                if not range_check and not zero_ok:
                    range_str = f"{'(' if is_strict_min else '['}{min_val}, {max_val}{']'}"
                    if allow_zero: range_str += " or 0"
                    raise ValueError(f"Value {num_val_dec.normalize()} out of allowed range: {range_str}")

                # Check type and convert if necessary
                needs_type_conversion = False
                if is_int:
                    # Check if it's an integer or a float that's numerically an integer
                    if not isinstance(original_val, int) and not (isinstance(original_val, float) and original_val.is_integer()):
                         needs_type_conversion = True
                    final_val = int(num_val_dec) # Convert to int for consistency
                else: # Expected float
                     if not isinstance(original_val, (int, float)): # Allow int input for float fields
                         needs_type_conversion = True
                     final_val = float(num_val_dec) # Convert to float for consistency

                if needs_type_conversion:
                    init_logger.info(f"{NEON_YELLOW}Config: Corrected type for '{key_path}' from {type(original_val).__name__} to {type(final_val).__name__}. Value: {repr(original_val)} -> {repr(final_val)}.{RESET}")
                    corrected = True
                # Check if the value itself changed significantly after conversion/quantization if needed?
                # For now, rely on range check and type conversion.
        ...
        if not isinstance(updated_config.get("trading_pairs"), list) or \
           not all(isinstance(s, str) and s and '/' in s for s in updated_config.get("trading_pairs", [])):
            init_logger.warning(f"{NEON_YELLOW}Invalid config 'trading_pairs' value '{updated_config.get('trading_pairs')}'. Must be a non-empty list of non-empty strings with '/'. Using default {default_config['trading_pairs']}.{RESET}")
        ...
        # Ensure fetch_limit is at least enough for basic strategy calculations
        min_fetch_limit_for_strategy = max(DEFAULT_VT_LENGTH, DEFAULT_VT_ATR_PERIOD, DEFAULT_VT_VOL_EMA_LENGTH, DEFAULT_PH_LEFT + DEFAULT_PH_RIGHT + 1, DEFAULT_PL_LEFT + DEFAULT_PL_RIGHT + 1) + 20 # Add buffer
        validate_numeric(updated_config, "fetch_limit", min_fetch_limit_for_strategy, MAX_DF_LEN, is_int=True)
        ...
        # Strategy Params
        strat_cfg_dict = updated_config.get("strategy_params", {})
        if not isinstance(strat_cfg_dict, dict):
             init_logger.warning(f"Invalid 'strategy_params' value '{updated_config.get('strategy_params')}'. Must be a dictionary. Using default strategy parameters.")
             updated_config["strategy_params"] = default_config["strategy_params"]
             strat_cfg_dict = updated_config["strategy_params"]
             config_needs_saving = True

        validate_numeric(strat_cfg_dict, "vt_length", 1, 500, is_int=True)
        validate_numeric(strat_cfg_dict, "vt_atr_period", 1, MAX_DF_LEN, is_int=True)  # Allow long ATR period
        validate_numeric(strat_cfg_dict, "vt_vol_ema_length", 1, MAX_DF_LEN, is_int=True)  # Allow long Vol EMA
        validate_numeric(strat_cfg_dict, "vt_atr_multiplier", 0.1, 20.0)
        validate_numeric(strat_cfg_dict, "ph_left", 1, 100, is_int=True)
        validate_numeric(strat_cfg_dict, "ph_right", 1, 100, is_int=True)
        validate_numeric(strat_cfg_dict, "pl_left", 1, 100, is_int=True)
        validate_numeric(strat_cfg_dict, "pl_right", 1, 100, is_int=True)
        validate_numeric(strat_cfg_dict, "ob_max_boxes", 1, 200, is_int=True)
        validate_numeric(strat_cfg_dict, "ob_entry_proximity_factor", 1.0, 1.1)  # e.g., 1.005 = 0.5% proximity
        validate_numeric(strat_cfg_dict, "ob_exit_proximity_factor", 1.0, 1.1)  # e.g., 1.001 = 0.1% proximity
        if strat_cfg_dict.get("ob_source") not in ["Wicks", "Body"]:
             init_logger.warning(f"Invalid strategy_params.ob_source '{strat_cfg_dict.get('ob_source')}'. Must be 'Wicks' or 'Body'. Using default '{DEFAULT_OB_SOURCE}'.")
             strat_cfg_dict["ob_source"] = DEFAULT_OB_SOURCE
             config_needs_saving = True
        if not isinstance(strat_cfg_dict.get("ob_extend"), bool):
             init_logger.warning(f"Invalid strategy_params.ob_extend value '{strat_cfg_dict.get('ob_extend')}'. Must be true or false. Using default '{DEFAULT_OB_EXTEND}'.")
             strat_cfg_dict["ob_extend"] = DEFAULT_OB_EXTEND
             config_needs_saving = True

        # Protection Params
        prot_cfg_dict = updated_config.get("protection", {})
        if not isinstance(prot_cfg_dict, dict):
             init_logger.warning(f"Invalid 'protection' value '{updated_config.get('protection')}'. Must be a dictionary. Using default protection parameters.")
             updated_config["protection"] = default_config["protection"]
             prot_cfg_dict = updated_config["protection"]
             config_needs_saving = True

        if not isinstance(prot_cfg_dict.get("enable_trailing_stop"), bool):
             init_logger.warning(f"Invalid protection.enable_trailing_stop value '{prot_cfg_dict.get('enable_trailing_stop')}'. Must be true or false. Using default '{default_config['protection']['enable_trailing_stop']}'.")
             prot_cfg_dict["enable_trailing_stop"] = default_config["protection"]["enable_trailing_stop"]
             config_needs_saving = True
        if not isinstance(prot_cfg_dict.get("enable_break_even"), bool):
             init_logger.warning(f"Invalid protection.enable_break_even value '{prot_cfg_dict.get('enable_break_even')}'. Must be true or false. Using default '{default_config['protection']['enable_break_even']}'.")
             prot_cfg_dict["enable_break_even"] = default_config["protection"]["enable_break_even"]
             config_needs_saving = True
        validate_numeric(prot_cfg_dict, "trailing_stop_callback_rate", Decimal('0.0001'), Decimal('0.5'), is_strict_min=True)  # Must be > 0
        validate_numeric(prot_cfg_dict, "trailing_stop_activation_percentage", Decimal('0'), Decimal('0.5'), allow_zero=True)  # 0 means activate immediately
        validate_numeric(prot_cfg_dict, "break_even_trigger_atr_multiple", Decimal('0.1'), Decimal('10.0'))
        validate_numeric(prot_cfg_dict, "break_even_offset_ticks", 0, 1000, is_int=True, allow_zero=True)  # 0 means move SL exactly to entry
        validate_numeric(prot_cfg_dict, "initial_stop_loss_atr_multiple", Decimal('0.1'), Decimal('100.0'), is_strict_min=True) # Must be > 0
        validate_numeric(prot_cfg_dict, "initial_take_profit_atr_multiple", Decimal('0'), Decimal('100.0'), allow_zero=True)  # 0 disables initial TP

        # --- Save Updated Config if Necessary ---
        if config_needs_saving:
             try:
                 # Convert potentially corrected values back to standard types for JSON
                 # (e.g., if defaults were used which might be Decimals internally, though validate_numeric converts to float/int)
    ...
    try:
        ...
        exchange_options = {
            ...
            'options': {
                ...
                'fetchMarketsTimeout': 60000,    # Timeout for loading markets
            }
        }
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if attempt >= MAX_API_RETRIES:
                 ...
                 last_market_error = e
                 lg.error(f"{NEON_RED}Exchange error loading markets: {e}. Retrying...{RESET}")
                 if attempt >= MAX_API_RETRIES:
                      lg.critical(f"{NEON_RED}Maximum retries exceeded while loading markets due to exchange errors. Last error: {last_market_error}. Exiting.{RESET}")
                      return None
            except Exception as e:
                last_market_error = e
            ...
            # Apply delay before retrying (only if not successful)
...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Decimal | None:
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            def safe_decimal_from_ticker(value: Any | None, field_name: str) -> Decimal | None:
                ...
                try:
                    ...
                    # lg.debug(f"Could not parse ticker field '{field_name}' value '{value}' to Decimal.") # Too verbose
                    return None
            ...
            if price is None:
                ...
                if bid is not None and ask is not None: # Check if both bid and ask were successfully parsed
            ...
            if price and price > Decimal('0'): # Ensure price is positive
                lg.debug(f"Current price successfully fetched for {symbol}: {price.normalize()}")
                return price.normalize()  # Ensure normalization
            else:
                last_exception = ValueError(f"No valid positive price ('last', 'mid', 'ask', 'bid') found in ticker data. Ticker: {ticker}")
        ...
        # --- Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for NetworkError fetching price. Last error: {last_exception}{RESET}")
                 return None
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for ExchangeError fetching price. Last error: {last_exception}{RESET}")
                 return None
        except Exception as e:
            last_exception = e
...
def fetch_klines_ccxt(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int, logger: logging.Logger) -> pd.DataFrame:
    ...
    Args:
        ...
        limit: The desired number of klines. Will be capped by `BYBIT_API_KLINE_LIMIT`
               ...
               (currently single request, may need multiple requests in future).
        logger: The logger instance for status messages.
    ...
    # Attempt to get market info once before retries
    try:
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            # Fetch OHLCV data
            # Use market_id for Bybit if available, as fetch_ohlcv uses it internally anyway
            fetch_symbol = market_id if market_id else symbol
            ohlcv_data = exchange.fetch_ohlcv(fetch_symbol, timeframe=timeframe, limit=actual_request_limit, params=params)
            ...
            if ohlcv_data and fetched_count > 0:
                ...
                try:
                    ...
                    # Allow a lag of up to 2 intervals (generous) + a small buffer
                    ...
                    if actual_lag_seconds < 0:  # Clock skew issue?
                        ...
                        last_exception = ValueError(f"Kline data potentially stale. Last candle timestamp {last_ts.strftime('%Y-%m-%d %H:%M:%S %Z')} (Lag: {actual_lag_seconds:.1f}s) > max allowed ({max_allowed_lag_seconds:.1f}s). Current UTC: {now_utc.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        ...
        # --- Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for NetworkError fetching klines. Last error: {last_exception}{RESET}")
                 return pd.DataFrame()
        except ccxt.RateLimitExceeded as e:
            last_exception = e
            wait_time = RETRY_DELAY_SECONDS * 3  # Longer wait for rate limits
            ...
            err_str = str(e).lower()
            if "invalid timeframe" in err_str or "interval is not supported" in err_str:
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for ExchangeError fetching klines. Last error: {last_exception}{RESET}")
                 return pd.DataFrame()
        except Exception as e:
            last_exception = e
    ...
    try:
        ...
        # Take only up to the number of columns returned by fetch_ohlcv
...
def _safe_market_decimal(value: Any | None, field_name: str, allow_zero: bool = True) -> Decimal | None:
    """Converts a market info value to Decimal, handling None and invalid formats."""
    if value is None:
        return None
    try:
        # Ensure value is treated as string to avoid float precision issues
        s_val = str(value).strip()
        if not s_val: return None
        d_val = Decimal(s_val)
...
def get_market_info(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> MarketInfo | None:
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                # Use .safe_market to handle potential KeyError gracefully
                market = exchange.safe_market(symbol)
            ...
            if market is None:
                ...
                price_limits = limits.get('price', {}) # Not used for sizing directly, but good to have
                ...
                try:
                    ...
                    # Simpler approach: Trust the structure matches and cast (with a type ignore)
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if 'bybit' in exchange.id.lower():
                 # Try common Bybit account types first
                 account_types_to_check = ['UNIFIED', 'CONTRACT', 'SPOT']
            account_types_to_check.append('')  # Always check default/unspecified type last
            ...
            for acc_type in account_types_to_check:
                try:
                    ...
                    # Use exchange.fetch_balance directly, CCXT handles params mapping
                    balance_info = exchange.fetch_balance(params=params)

                    # --- Try different ways to extract balance from CCXT result ---
                    # 1. Standard CCXT structure ('free' field within currency dict)
                    ...
                    # 2. Bybit V5 specific structure (often nested in 'info')
                        ...
                        for account_details in balance_info['info']['result']['list']:
                            ...
                            # Note: CCXT might standardize 'accountType' in the outer dict, but check inner info as well
                    ...
                    # Errors like "account type does not exist" or "invalid account type" are expected when checking multiple types
                    err_str = str(e).lower()
                    ...
                    else: # Re-raise other exchange errors for main retry loop
                         raise e
                except Exception as e:
            ...
            if found and balance_str is not None:
                ...
                # Use safe_balance if available to see if CCXT found anything standardized
                safe_bal = exchange.safe_balance(balance_info)
                currency_bal = safe_bal.get(currency)
                if currency_bal and currency_bal.get('free') is not None:
                     try:
                          balance_decimal = Decimal(str(currency_bal['free']))
                          final_balance = max(balance_decimal, Decimal('0'))
                          lg.debug(f"Found balance for {currency} in safe_balance: {final_balance.normalize()}")
                          return final_balance # Success via safe_balance
                     except (ValueError, InvalidOperation, TypeError):
                         pass # Failed to parse safe_balance, continue to error below
        ...
        # --- Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for NetworkError fetching balance. Last error: {last_exception}{RESET}")
                 return None
        except ccxt.RateLimitExceeded as e:
            last_exception = e
            ...
            # Re-raise the exception to be caught by the caller (initialize_exchange) for critical handling
            raise e
        except ccxt.ExchangeError as e:
            last_exception = e
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for ExchangeError fetching balance. Last error: {last_exception}{RESET}")
                 return None
        except Exception as e:
            last_exception = e
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> PositionInfo | None:
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            try:
                ...
                params = {'category': category}
                # Use CCXT's standard fetchPositions, it should handle the symbol/category mapping for Bybit V5
                positions = exchange.fetch_positions([symbol], params=params)
                lg.debug(f"Fetched {len(positions)} potential position entries for {symbol}.")

            except ccxt.ExchangeError as e:
                 # Bybit often returns specific codes for "no position" or related issues
                 no_pos_codes = [110025, 3400041, 3400042] # e.g., "position idx not match", "position not found"
                 ...
                 if code_match or any(msg in err_str for msg in no_pos_messages):
                     lg.debug(f"No open position found for {symbol} (Exchange message suggests no position: {e}).")
            ...
            # Get amount precision from market info if available
            size_threshold = Decimal('1e-9') # Default small value if precision not found
            try:
                 if market_info and market_info.get('amount_precision_step_decimal') is not None:
                      ...
                      if amount_precision_step > 0:
                           size_threshold = amount_precision_step * Decimal('0.01') # 1% of step size
                 # Ensure threshold is positive
                 if size_threshold <= 0: size_threshold = Decimal('1e-9')
            except Exception as prec_err:
                lg.warning(f"Could not get amount precision for {symbol} to set size threshold: {prec_err}. Using default: {size_threshold.normalize()}")
            ...
            # Note: Bybit V5 in one-way mode usually returns only one entry per symbol/side,
            # but handle potential multiple entries defensively.
            for pos in positions:
                # Extract size from 'info' (exchange-specific) or standard 'contracts' field
                # Bybit V5 uses 'size' in info, CCXT might map to 'contracts' or use 'contracts' directly
            ...
            if active_position_raw:
                ...
                if not side:
                    lg.error(f"Could not determine side for active position {symbol}. Size: {size.normalize()}. Data: {info}")
                ...
                def get_protection_field(field_name: str) -> str | None:
                    ...
                    try:
                         # Check if it's a valid number string and not zero (as string "0" often means not set)
                ...
                def format_decimal_log(value: Any | None) -> str:
                ...
                tsl_log = f"TSL(Dist:{tsl_dist_str}/Act:{tsl_act_str})" if tsl_dist_str != 'N/A' or tsl_act_str != 'N/A' else "TSL(N/A)"
                ...
                try:
                    ...
                    return final_position_info
                     ...
                     return std_pos  # type: ignore[return-value] # Return raw dict if cast fails
            else:
                # No position with size > threshold was found after filtering
                lg.debug(f"No active position found for {symbol} (checked {len(positions)} filtered entries).")
                return None
        ...
        # --- Error Handling with Retries ---
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for NetworkError fetching positions. Last error: {last_exception}{RESET}")
                 return None
        except ccxt.RateLimitExceeded as e:
            last_exception = e
            wait_time = RETRY_DELAY_SECONDS * 3
            ...
            if attempts >= MAX_API_RETRIES:
                 lg.error(f"{NEON_RED}Maximum retries exceeded for ExchangeError fetching positions. Last error: {last_exception}{RESET}")
                 return None
        except Exception as e:
            last_exception = e
...
def set_leverage_ccxt(exchange: ccxt.Exchange, symbol: str, leverage: int, market_info: MarketInfo, logger: logging.Logger) -> bool:
    ...
    # Use the exchange-specific market ID for the API call
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            # Pass market_id as the symbol argument to CCXT set_leverage
        ...
        # --- Error Handling with Retries ---
            ...
            fatal_codes = [
                ...
                '3400151', # Unified account is not in portfolio margin mode (if setting higher leverage)
            ]
            # Common fatal message fragments
            fatal_messages = ["margin mode", "position exists", "risk limit", "parameter error", "insufficient balance", "invalid leverage", "does not support this leverage"]
...
def calculate_position_size(balance: Decimal, risk_per_trade: float, initial_stop_loss_price: Decimal, entry_price: Decimal,
                            market_info: MarketInfo, exchange: ccxt.Exchange, logger: logging.Logger) -> Decimal | None:
    ...
    try:
        amount_precision_step = market_info['amount_precision_step_decimal']
        price_precision_step = market_info['price_precision_step_decimal'] # Not directly used for size, but good context
    ...
    try:
        if not is_inverse:  # Linear Contracts or Spot
            ...
            # Add tolerance for zero check
            if value_change_per_unit <= Decimal('1e-18'):
            ...
            lg.debug(f"  Linear/Spot Calc: RiskAmt={risk_amount_quote.normalize()} / ValueChangePerUnit={value_change_per_unit.normalize()} = {calculated_size.normalize()}")
            ...
            except InvalidOperation as inv_err:
                 lg.error(f"Sizing failed (Inverse): Invalid operation during inverse price calculation: {inv_err}")
                 return None
            ...
            lg.debug(f"  Inverse Calc: RiskAmt={risk_amount_quote.normalize()} / RiskPerContract={risk_per_contract_quote.normalize()} = {calculated_size.normalize()}")
    ...
    # Helper function to estimate cost (value of the order)
    def estimate_cost(size: Decimal, price: Decimal) -> Decimal | None:
        ...
        try:
             ...
             # For Spot, contractSize is typically 1
             if not is_inverse:
                 return (size * price * contract_size)
             else:
                 # Avoid division by zero if price is zero (already checked above, but defensive)
    ...
    try:
        ...
        # Use quantize with ROUND_DOWN for robustness
        final_size_manual_floor = (adjusted_size / amount_precision_step).quantize(Decimal('1'), ROUND_DOWN) * amount_precision_step

        # Choose the manually floored value as it's generally safer (avoids rounding up)
        final_size = final_size_manual_floor

        if abs(final_size - adjusted_size) > Decimal('1e-18'): # Check if precision adjustment made a change
    ...
    if min_amount > 0 and final_size < min_amount:
        ...
        return None
    # Check Max Amount again (should be fine if rounded down, but check defensively)
    ...
    if final_cost is not None:
        ...
        if min_cost > 0 and final_cost < min_cost:
             lg.warning(f"{NEON_YELLOW}Final cost {final_cost.normalize()} is slightly below min cost {min_cost.normalize()} after precision rounding. Checking if one step up helps...{RESET}")
...
def place_trade(exchange: ccxt.Exchange, symbol: str, trade_signal: str, position_size: Decimal, market_info: MarketInfo,
    ...
    # Note: EXIT_SHORT means BUY (to close a short), EXIT_LONG means SELL (to close a long)
    ...
    # Determine the unit for logging (e.g., Contracts, BTC, ETH)
    size_log_unit = market_info.get('settle') if is_contract and market_info.get('settle') else market_info.get('base') if market_info.get('base') else 'Units'
    ...
    try:
         # Use amount_to_precision to format the Decimal size correctly before converting to float
         # This handles lot size, minimum amount, etc.
         formatted_size_str = exchange.amount_to_precision(symbol=market_info['symbol'], amount=float(position_size))
         amount_float = float(formatted_size_str)
         lg.debug(f"Formatted position size {position_size.normalize()} to {amount_float} ({formatted_size_str}) for API.")
    except Exception as float_err:
         lg.error(f"Failed to convert/format position size {position_size.normalize()} to float for API call: {float_err}")
         return None
    ...
    lg.info(f"{BRIGHT}===> Attempting {action_desc} | {side.upper()} {order_type.upper()} Order | {symbol} | Size: {position_size.normalize()} {size_log_unit} <==={RESET}")
    ...
    while attempts <= MAX_API_RETRIES:
        ...
        # --- Error Handling with Retries ---
            ...
            last_exception = e
            lg.error(f"{NEON_RED}Order Failed ({action_desc}): Insufficient funds for {symbol} {side} {position_size.normalize()}. Error: {e}{RESET}")
            ...
            elif "exceed" in err_str or "too large" in err_str:
                 lg.error(f"  >> Hint: Check position size ({position_size.normalize()}) against market maximum amount/cost limits.")
            elif "reduce only check failed" in err_str:
                 lg.error(f"  >> Hint: reduceOnly order failed. Ensure TimeInForce is IOC or FOK, and you actually have an open position to reduce.")
            return None  # Non-retryable
        except ccxt.ExchangeError as e:
            last_exception = e
            ...
            fatal_order_codes = [
                ...
                '3400155',  # Position is in closing status (cannot place new order)
            ]
            # Common fatal message fragments
            fatal_messages = ["invalid parameter", "precision", "exceed limit", "risk limit", "invalid symbol", "reduce only check failed", "position in closing status"]
...
def _set_position_protection(exchange: ccxt.Exchange, symbol: str, market_info: MarketInfo, position_info: PositionInfo, logger: logging.Logger,
    ...
    **Important:** This uses a direct API call (`private_post`) and relies on Bybit's specific
    ...
    in terms of which parameters are sent, but the API handles which one is 'active'.
    ...
    Args:
        ...
        stop_loss_price: Desired fixed SL price (Decimal). Set to 0 or None to clear/ignore.
                         If TSL is being set (distance > 0), this parameter is ignored.
    ...
    if pos_side not in ['long', 'short']:
        ...
        return False
    try:
        if entry_price_str is None: raise ValueError("Missing entry price string in position info")
    ...
    set_tsl_active_params = False # Flag if TSL distance > 0 parameters are being prepared
    ...
    try:
        ...
        def format_price_param(price_decimal: Decimal | None, param_name: str) -> str | None:
            ...
            # Allow "0" to clear protection - return string "0" directly
        ...
        if isinstance(trailing_stop_distance, Decimal):
            ...
            if trailing_stop_distance > 0:  # Setting an active TSL
                if not isinstance(tsl_activation_price, Decimal) or tsl_activation_price <= 0:
                    ...
                    if not is_valid_activation:
                        ...
                        if fmt_distance and fmt_activation:
                            ...
                            set_tsl_active_params = True  # Mark TSL parameters as being actively prepared
                        else:
                ...
                # set_tsl_active_params remains False

        # --- Fixed Stop Loss (SL) ---
        # Can set fixed SL. Bybit API allows setting both TSL and fixed SL simultaneously.
        # The "active" one depends on the activePrice relative to the market price
        # and whether the TSL has been triggered. We will send both if requested.
        if isinstance(stop_loss_price, Decimal):
            ...
            if stop_loss_price > 0:  # Setting an active SL
                ...
                if not is_valid_sl:
                 ...
                 # Only send "0" if SL field isn't already set by TSL logic (which it won't be)
                 if 'stopLoss' not in params_to_set: # Defensive check, should always be true here
        ...
        if isinstance(take_profit_price, Decimal):
            ...
            if take_profit_price > 0:  # Setting an active TP
                ...
                if not is_valid_tp:
                 ...
                 if 'takeProfit' not in params_to_set: # Defensive check, should always be true here
    ...
    if not params_to_set:
        if any_protection_requested:
            # If parameters were requested but none were valid/formatted, it's a failure
            ...
            # If no parameters were requested at all, it's a success (no action needed)
    ...
    while attempts <= MAX_API_RETRIES:
        ...
        # --- Standard CCXT Error Handling with Retries ---
            ...
            fatal_protect_codes = [
                ...
                '3400155', # Position is in closing status
            ]
            fatal_messages = ["invalid parameter", "invalid price", "cannot be higher", "cannot be lower", "position status", "position is in closing status"]
...
def set_trailing_stop_loss(exchange: ccxt.Exchange, symbol: str, market_info: MarketInfo, position_info: PositionInfo, config: dict[str, Any],
    ...
    try:
        ...
        if entry_price_str is None: raise ValueError("Missing entry price string in position info")
        entry_price = Decimal(str(entry_price_str))
        # Config values were validated in load_config, but ensure they are treated as Decimal here
        callback_rate = Decimal(str(prot_cfg.get("trailing_stop_callback_rate", DEFAULT_TSL_CALLBACK_RATE)))
        activation_percentage = Decimal(str(prot_cfg.get("trailing_stop_activation_percentage", DEFAULT_TSL_ACTIVATION_PERCENTAGE)))
        price_tick = market_info.get('price_precision_step_decimal') # Get Decimal tick size

        # Validate parameters
        if not (entry_price > 0 and callback_rate > 0 and activation_percentage >= 0):
             raise ValueError("Invalid input values (entry, callback rate > 0, activation % >= 0).")
        if price_tick is None or price_tick <= 0:
             raise ValueError("Invalid price tick size from market info.")
    ...
    try:
        ...
        # If activation_percentage is 0, the raw_activation_price is the entry price.
        # In this case, the activation price should be set just beyond the entry price (by one tick)
        if activation_percentage == Decimal('0'):
             if pos_side == 'long':
                  tsl_activation_price = entry_price + price_tick
             else: # Short position
                  tsl_activation_price = entry_price - price_tick
        else:
            if pos_side == 'long':
                # Round up for long activation to ensure it's strictly beyond entry
                # Ensure result is strictly greater than entry + tick
                ...
                # Ensure result is strictly less than entry - tick
        ...
        # Use abs() in case activation price somehow became negative (shouldn't happen after validation)
        ...
        # Using ROUND_UP to ensure the distance is sufficient
        ...
        # Pass None for fixed stop_loss_price here as TSL is the primary protection being set.
        # Bybit API allows setting both, but logic here prioritizes TSL if enabled.
        return _set_position_protection(
            ...
            logger=lg,
            stop_loss_price=None,  # We are setting TSL, so don't attempt to set/clear fixed SL here
...
class VolumaticOBStrategy:
    ...
    def __init__(self, config: dict[str, Any], market_info: MarketInfo, logger: logging.Logger) -> None:
        ...
        try:
            ...
            self.ob_source = str(strategy_cfg.get("ob_source", DEFAULT_OB_SOURCE))  # "Wicks" or "Body"
    ...
    def _ema_swma(self, series: pd.Series, length: int) -> pd.Series:
        ...
        Operates on float series. Handles NaN inputs gracefully.

        Args:
            series: The input Pandas Series (typically 'close' prices as float).
            length: The length for the final EMA calculation (integer > 0).
        ...
        if not isinstance(series, pd.Series) or len(series) < 4 or length <= 0:
            self.lg.debug(f"_ema_swma: Invalid input series length ({len(series)}) or length ({length}). Returning NaNs.")
        ...
        if numeric_series.isnull().all():  # Check if all values are NaN after conversion
            self.lg.debug(f"_ema_swma: Input series is all NaN after numeric conversion. Returning NaNs.")
        ...
        # Raw=True passes numpy array for performance
        ...
        # dropna=False keeps original index size
        ema_of_swma = ta.ema(swma, length=length, fillna=np.nan, dropna=False)
    ...
    def _find_pivots(self, series: pd.Series, left_bars: int, right_bars: int, is_high: bool) -> pd.Series:
        ...
        and `right_bars` values to its right.
        Uses strict inequality ('>' or '<'). Operates on float series. Handles NaNs.
        ...
        Returns:
            ...
            Returns a Series of False if input is invalid or empty.
        """
        if not isinstance(series, pd.Series) or series.empty or left_bars < 1 or right_bars < 1:
            self.lg.debug(f"_find_pivots: Invalid input series or lookback periods ({left_bars}/{right_bars}). Returning all False.")
            return pd.Series(False, index=series.index, dtype=bool)  # Return all False if input invalid

        # Ensure numeric type
        num_series = pd.to_numeric(series, errors='coerce')
        if num_series.isnull().all():
            self.lg.debug(f"_find_pivots: Input series is all NaN after numeric conversion. Returning all False.")
            return pd.Series(False, index=series.index, dtype=bool)

        # Initialize conditions mask
        # Ensure the mask has the same index as the series
        mask = pd.Series(True, index=num_series.index, dtype=bool)

        # Check left bars
        for i in range(1, left_bars + 1):
            shifted = num_series.shift(i)
            if is_high:
                mask &= (num_series > shifted)
            else:
                mask &= (num_series < shifted)
        ...
        for i in range(1, right_bars + 1):
            # shift(-i) requires values *after* the current point, which will be NaN for the latest bars
            # This is correct behavior - pivots cannot be identified on the rightmost bars
            shifted = num_series.shift(-i)
            if is_high:
                mask &= (num_series > shifted)
            else:
        ...
        # The last `right_bars` entries will always be False because shift(-i) introduces NaNs there.
        # This is correct pivot logic.
        return mask.fillna(False) # Final fillna to ensure no remaining NaNs turn into True
    ...
    def update(self, df_input: pd.DataFrame) -> StrategyAnalysisResults:
        ...
        6. Identifies new Order Blocks based on pivots on *all* historical candles.
        ...
        try:
            ...
            # dropna=False keeps original index size, fillna=np.nan ensures NaNs at start
            df_float['atr'] = ta.atr(df_float['high'], df_float['low'], df_float['close'], length=self.vt_atr_period, fillna=np.nan, dropna=False)

            # Volumatic Trend EMAs
            df_float['ema1'] = self._ema_swma(df_float['close'], length=self.vt_length)  # EMA(SWMA(close))
            df_float['ema2'] = ta.ema(df_float['close'], length=self.vt_length, fillna=np.nan, dropna=False)  # Standard EMA(close)

            # Determine Trend Direction (ema2 crosses ema1 of the *previous* bar)
            # Ensure comparison happens only when both values are valid
            # Use .shift(1) to compare current ema2 with previous ema1
            valid_comparison_mask = df_float['ema2'].notna() & df_float['ema1'].shift(1).notna()

            # Initialize trend_up_series with None (object dtype)
            trend_up_series = pd.Series(None, index=df_float.index, dtype=object)
            # Assign boolean trend only where comparison is valid
            trend_up_series[valid_comparison_mask] = df_float['ema2'][valid_comparison_mask] > df_float['ema1'].shift(1)[valid_comparison_mask]

            # Identify Trend Changes (where trend differs from previous bar's trend)
            # Compare current boolean trend to previous boolean trend. Handle None/NaN.
            # A change occurs if current is True and prev is False, OR current is False and prev is True.
            # Also requires both current and previous trend values to be determined (not None/NaN).
            prev_trend_up = trend_up_series.shift(1)
            trend_changed_series = ((trend_up_series == True) & (prev_trend_up == False)) | \
                                   ((trend_up_series == False) & (prev_trend_up == True))
            # Fill NaNs introduced by shift or where trend is None/NaN
            df_float['trend_changed'] = trend_changed_series.fillna(False).astype(bool) # Store as pure boolean

            # Store the boolean trend (potentially with None if undetermined)
            df_float['trend_up'] = trend_up_series

            # Capture EMA1 and ATR values at the exact point the trend changed
            # Use np.where for conditional assignment, preserving NaNs where condition is False
            ...
            # Use limit=None to fill all NaNs from the first valid value
            df_float['ema1_for_bands'] = df_float['ema1_at_change'].ffill(limit=None)
            df_float['atr_for_bands'] = df_float['atr_at_change'].ffill(limit=None)
            ...
            volume_numeric = df_float['volume'].fillna(0.0)  # Use float volume, handle NaNs as 0 for max calc
            # Use a reasonable min_periods for the rolling max to avoid NaN at the start
            # Min periods should be at least 1, up to the length, but not more than series length
            min_periods_vol = max(1, min(self.vt_vol_ema_length, len(df_float)))
            # Calculate rolling max, backfill NaNs at the start using the first non-NaN value
            df_float['vol_max'] = volume_numeric.rolling(window=self.vt_vol_ema_length, min_periods=min_periods_vol).max().fillna(method='bfill')
            # Calculate normalized volume (0-100 range, potentially > 100)
            # Use small epsilon to avoid division by zero/near-zero when vol_max is 0 or close to 0
            epsilon = 1e-9
            df_float['vol_norm'] = np.where(df_float['vol_max'] > epsilon,
                                            (volume_numeric / df_float['vol_max'] * 100.0),
                                            0.0)  # Assign 0.0 if max volume is zero or negative
            # Handle potential NaNs (shouldn't happen after fillna(0) and np.where) and clip unreasonable values (e.g., > 200%)
            ...
            # Return empty results if indicator calculation fails critically
            return empty_results
        ...
        try:
            ...
            # List of columns to convert from float/object back to Decimal/bool/None
            cols_to_copy = {
                'atr': Decimal, 'ema1': Decimal, 'ema2': Decimal,
                'upper_band': Decimal, 'lower_band': Decimal, 'vol_norm': Decimal,
                'trend_changed': bool, 'is_ph': bool, 'is_pl': bool,
                'trend_up': object # Keep as object to allow None
            }

            for col, dtype in cols_to_copy.items():
                if col in df_float.columns:
                    ...
                    if dtype == Decimal:
                         ...
                         # Convert boolean series, filling NaNs as False
                         df[col] = source_series.fillna(False).astype(bool)
                    elif dtype == object: # For 'trend_up'
                         # Convert potential boolean/None series, preserving None
                         df[col] = source_series.apply(lambda x: bool(x) if pd.notna(x) else None)
                    else:
                         # Should not happen with the explicit list above
                         df[col] = source_series # Copy as is

                else:
                     self.lg.warning(f"Expected calculated column '{col}' not found in float DataFrame.")


        except Exception as e:
            self.lg.error(f"Strategy update failed: Error converting calculated indicators back to Decimal: {e}", exc_info=True)
            # Return empty results if this crucial step fails
            return empty_results

        # --- Clean Final Decimal DataFrame ---
        # Drop rows where essential indicators might be NaN (e.g., at the start of the series)
        # ATR, Bands are crucial. Close price is also essential.
        initial_len_final = len(df)
        essential_indicator_cols = ['close', 'atr', 'upper_band', 'lower_band'] # is_ph/is_pl are optional for basic analysis results
        df.dropna(subset=essential_indicator_cols, inplace=True)
        ...
        self.lg.debug(f"Indicators finalized in Decimal DataFrame ({len(df)} candles). Processing Order Blocks...")
        ...
        try:
            ...
            # Keep only active boxes from previous runs for the update process
            current_active_bull_boxes = [b for b in self.bull_boxes if b['active'] and not b['violated']]
            current_active_bear_boxes = [b for b in self.bear_boxes if b['active'] and not b['violated']]

            # 1. Identify New Order Blocks from Pivots on *all* historical candles in the df
            # Rebuilding OBs based on the current lookback settings is simpler than
            # managing a complex state machine across runs, but might recreate old ones.
            # Filter out duplicates based on timestamp and type when adding.
            new_bull_candidates: list[OrderBlock] = []
            new_bear_candidates: list[OrderBlock] = []

            # Get existing OB timestamps to quickly check for duplicates
            existing_ob_timestamps = {(ob['left_idx'], ob['type']) for ob in self.bull_boxes + self.bear_boxes}

            for timestamp, candle in df.iterrows():
                try:
                    # Ensure essential candle data is present and Decimal before using
                    open_p = candle.get('open')
                    high_p = candle.get('high')
                    low_p = candle.get('low')
                    close_p = candle.get('close')

                    if not all(isinstance(p, Decimal) and pd.notna(p) for p in [open_p, high_p, low_p, close_p]):
                        self.lg.debug(f"Skipping pivot check for candle at {timestamp}: Missing or invalid price data.")
                        continue

                    # Create Bearish OB from Pivot High
                    if candle.get('is_ph') and (timestamp, 'bear') not in existing_ob_timestamps:
                        ob_top = high_p if self.ob_source == "Wicks" else max(open_p, close_p)
                        ob_bottom = open_p if self.ob_source == "Wicks" else min(open_p, close_p)
                        # Ensure valid Decimal values and top > bottom
                        if ob_top > ob_bottom:
                            ob_id = f"B_{timestamp.strftime('%y%m%d%H%M%S')}"  # Unique ID based on creation time
                            ...
                            existing_ob_timestamps.add((timestamp, 'bear')) # Add to check set
                            new_ob_count += 1
                        # else: self.lg.debug(f"Skipped Bear OB at {timestamp}: Invalid top/bottom ({ob_top.normalize()}/{ob_bottom.normalize()}).") # Too verbose

                    # Create Bullish OB from Pivot Low
                    if candle.get('is_pl') and (timestamp, 'bull') not in existing_ob_timestamps:
                        ob_top = open_p if self.ob_source == "Wicks" else max(open_p, close_p)
                        ob_bottom = low_p if self.ob_source == "Wicks" else min(open_p, close_p)
                        if ob_top > ob_bottom:
                            ...
                            existing_ob_timestamps.add((timestamp, 'bull')) # Add to check set
                            new_ob_count += 1
                        # else: self.lg.debug(f"Skipped Bull OB at {timestamp}: Invalid top/bottom ({ob_top.normalize()}/{ob_bottom.normalize()}).") # Too verbose

                except Exception as pivot_proc_err:
                    self.lg.warning(f"Error processing potential pivot at {timestamp.strftime('%y%m%d%H%M%S')}: {pivot_proc_err}", exc_info=False)  # Less verbose exc_info
            ...
            # Iterate through candles *after* the box was formed up to the latest candle in the DF.
            # This ensures violations are detected even if they happened several candles ago within the fetched data.
            # Process all boxes (including newly added ones) that are currently active.
            all_active_boxes = [b for b in self.bull_boxes + self.bear_boxes if b['active']]
            self.lg.debug(f"Checking violations/extensions for {len(all_active_boxes)} currently active OBs...")

            for box in all_active_boxes:
                 # Find candles strictly after the box's creation time (left_idx)
                 ...
                 for ts, candle in relevant_candles.iterrows():
                      ...
                      if isinstance(close_price, Decimal) and pd.notna(close_price):
                           # Violation check: Close below the bottom of a bull OB OR above the top of a bear OB
                           if (box['type'] == 'bull' and close_price < box['bottom']) or \
                              (box['type'] == 'bear' and close_price > box['top']):
                                box['active'] = False
                                box['violated'] = True
                                box['right_idx'] = ts  # Mark violation time
                                violated_ob_count += 1
                                self.lg.debug(f"{box['type'].capitalize()}ish OB {box['id']} VIOLATED at {ts.strftime('%H:%M')} by close {close_price.normalize()}")
                                violation_detected = True
                                break  # Stop checking this box once violated
                           else:
                                # Update right_idx if still active and not violated by this candle
                                box['right_idx'] = ts

                 # If not violated by any candle in the relevant_candles range, ensure right_idx is updated to the last candle if extend is true
                 if not violation_detected and self.ob_extend:
                     box['right_idx'] = last_candle_idx # Extend to the very last candle processed
        ...
        def safe_decimal_from_candle(candle_data: pd.Series | None, col_name: str, positive_only: bool = False) -> Decimal | None:
            ...
            if isinstance(value, Decimal) and pd.notna(value):  # Check isnan before potential comparison
                 ...
                 if not positive_only or value > Decimal('0'):
                     return value
            # lg.debug(f"safe_decimal: Value for '{col_name}' is invalid or failed check: {value}") # Too verbose
            return None

        # Helper to safely extract Boolean values (handling None/NaN)
        def safe_bool_from_candle(candle_data: pd.Series | None, col_name: str) -> bool | None:
        ...
        last_ts_log = last_candle_final.name.strftime('%Y-%m-%d %H:%M:%S %Z') if last_candle_final is not None else "N/A"

        self.lg.debug(f"--- Strategy Analysis Results ({last_ts_log}) ---")
        ...
        self.lg.debug(f"  Bands (Upper/Lower): {analysis_results['upper_band'].normalize() if analysis_results['upper_band'] is not None else 'N/A'} / {analysis_results['lower_band'].normalize() if analysis_results['lower_band'] is not None else 'N/A'}")
        ...
        # for ob in analysis_results['active_bull_boxes']: self.lg.debug(f"    Bull OB: {ob['id']} [{ob['bottom'].normalize()} - {ob['top'].normalize()}] created {ob['left_idx'].strftime('%H:%M')}, active to {ob['right_idx'].strftime('%H:%M')}")
        # for ob in analysis_results['active_bear_boxes']: self.lg.debug(f"    Bear OB: {ob['id']} [{ob['bottom'].normalize()} - {ob['top'].normalize()}] created {ob['left_idx'].strftime('%H:%M')}, active to {ob['right_idx'].strftime('%H:%M')}")
        self.lg.debug("---------------------------------------------")
...
class SignalGenerator:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger) -> None:
        ...
        try:
            ...
            # Config values were validated in load_config, but ensure they are treated as Decimal here
            self.ob_entry_proximity_factor = Decimal(str(strategy_cfg.get("ob_entry_proximity_factor", DEFAULT_OB_ENTRY_PROXIMITY_FACTOR)))
            self.ob_exit_proximity_factor = Decimal(str(strategy_cfg.get("ob_exit_proximity_factor", DEFAULT_OB_EXIT_PROXIMITY_FACTOR)))
            self.initial_tp_atr_multiple = Decimal(str(protection_cfg.get("initial_take_profit_atr_multiple", DEFAULT_INITIAL_TP_ATR_MULTIPLE)))
            self.initial_sl_atr_multiple = Decimal(str(protection_cfg.get("initial_stop_loss_atr_multiple", DEFAULT_INITIAL_SL_ATR_MULTIPLE)))

            # Basic validation (redundant if load_config is perfect, but defensive)
    ...
    def generate_signal(self, analysis_results: StrategyAnalysisResults, open_position: PositionInfo | None) -> str:
        ...
        pos_log = f"{position_side.upper()} (Size: {open_position.get('size_decimal', Decimal(0)).normalize()})" if open_position else 'None'
        lg.debug(f"  Input: Close={last_close.normalize()}, Trend={trend_log}, TrendChanged={trend_changed}, Position={pos_log}")
        ...
        if position_side == 'long':
            ...
            # Exit Long if price nears a bearish OB (use exit proximity factor)
                ...
                try:
                    ...
                    # Filter out OBs whose top is already below the current price (should be rare for bear OB near long pos)
                    relevant_bear_obs = [ob for ob in active_bear_obs if ob['top'] >= last_close]
                    if relevant_bear_obs:
                        closest_bear_ob = min(relevant_bear_obs, key=lambda ob: ob['top']) # Closest by top edge price
            ...
            # Exit Short if price nears a bullish OB (use exit proximity factor)
                ...
                try:
                    ...
                    # Filter out OBs whose bottom is already above the current price (should be rare for bull OB near short pos)
                    relevant_bull_obs = [ob for ob in active_bull_obs if ob['bottom'] <= last_close]
                    if relevant_bull_obs:
                        closest_bull_ob = max(relevant_bull_obs, key=lambda ob: ob['bottom']) # Closest by bottom edge price
            ...
            if trend_is_up is True and active_bull_obs:
                for ob in active_bull_obs:
                    try:
                        ...
                        # Ensure valid range before checking price
                        if entry_zone_bottom < entry_zone_top and entry_zone_bottom >= 0: # Ensure bottom is positive
                        ...
                        # else: lg.debug(f"Skipping Bull OB {ob.get('id')} entry check due to invalid zone: [{entry_zone_bottom.normalize()} - {entry_zone_top.normalize()}]") # Too verbose
                    ...
                    try:
                        ...
                        if entry_zone_bottom < entry_zone_top and entry_zone_bottom >= 0: # Ensure bottom is positive
                             if entry_zone_bottom <= last_close <= entry_zone_top:
                                 ...
                                 break  # Take the first valid entry signal found
                        # else: lg.debug(f"Skipping Bear OB {ob.get('id')} entry check due to invalid zone: [{entry_zone_bottom.normalize()} - {entry_zone_top.normalize()}]") # Too verbose
                    except (ZeroDivisionError, InvalidOperation, Exception) as e:
    ...
    def calculate_initial_tp_sl(self, entry_price: Decimal, signal: str, atr: Decimal, market_info: MarketInfo, exchange: ccxt.Exchange) -> tuple[Decimal | None, Decimal | None]:
        ...
        try:
            ...
            lg.debug(f"  Raw Levels: TP={take_profit_raw.normalize() if take_profit_raw is not None else 'N/A'}, SL={stop_loss_raw.normalize()}")
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: dict[str, Any], logger: logging.Logger,
    ...
    if klines_df.empty:
        ...
        return
    if fetched_count < min_required_data:
        ...
        if hit_api_limit_but_insufficient:
            lg.error(f"{NEON_RED}CRITICAL DATA ISSUE:{RESET} Fetched maximum {fetched_count} klines allowed by API, "
                     ...
                     f"{NEON_YELLOW}ACTION REQUIRED: Reduce long lookback periods in strategy config (e.g., vt_atr_period, vt_vol_ema_length)! Skipping cycle.{RESET}")
        else:  # Fetched some data, but not enough for strategy minimum
    ...
    # Fetch position info *before* generating signal, as signal depends on it
    open_position: PositionInfo | None = get_open_position(exchange, symbol, lg) # Returns standardized dict or None
    ...
    if current_market_price is not None and current_market_price > 0: # Explicit check for None
    ...
    if open_position is None and signal in ["BUY", "SELL"]:
        ...
        if initial_sl_calc is None:
            ...
            return
        if initial_tp_calc is None and config.get('protection', {}).get('initial_take_profit_atr_multiple', DEFAULT_INITIAL_TP_ATR_MULTIPLE) > 0:
            lg.warning(f"{NEON_YELLOW}Initial Take Profit calculation failed despite positive multiplier. Proceeding without initial TP.{RESET}")
        elif initial_tp_calc is None:
             lg.info(f"Initial Take Profit disabled (TP Mult = 0) or calculation resulted in None. Proceeding without initial TP.")
        ...
        # Place Market Order
        lg.warning(f"{BRIGHT}===> PLACING {signal} MARKET ORDER | Size: {position_size.normalize()} {market_info.get('base', 'Units')} <==={RESET}") # Use base currency for spot, default for contracts
            ...
            if confirmed_position:
                try:
                    ...
                    if prot_sl_calc is None:
                        ...
                        tsl_enabled_in_config = prot_cfg.get("enable_trailing_stop", DEFAULT_ENABLE_TSL)

                        if tsl_enabled_in_config:
                            ...
                            if not protection_set_success:
                                 lg.error(f"{NEON_RED}Attempted to set TSL but failed.{RESET}")

                        # If TSL is not enabled in config OR TSL setting failed, attempt to set fixed SL/TP
                        # Only attempt fixed SL/TP if TSL was NOT enabled OR if setting TSL failed AND fixed SL/TP is valid
                        if not tsl_enabled_in_config or (tsl_enabled_in_config and not protection_set_success):
                             if prot_sl_calc is not None or prot_tp_calc is not None:
                                  lg.info(f"Attempting to set Fixed Stop Loss / Take Profit based on entry {entry_price_actual.normalize()}...")
                                  fixed_protection_success = _set_position_protection(
                                       exchange, symbol, market_info, confirmed_position, lg,
                                       stop_loss_price=prot_sl_calc,
                                       take_profit_price=prot_tp_calc,
                                       trailing_stop_distance=Decimal('0') if tsl_enabled_in_config else None, # Clear TSL if it failed to set/recover
                                       tsl_activation_price=Decimal('0') if tsl_enabled_in_config else None
                                  )
                                  if fixed_protection_success:
                                       lg.info(f"{NEON_GREEN}Fixed Stop Loss / Take Profit set successfully.{RESET}")
                                       protection_set_success = True # Overall protection setup is now successful
                                  else:
                                       lg.error(f"{NEON_RED}Failed to set Fixed Stop Loss / Take Profit via API.{RESET}")
                             else:
                                  # Neither TSL was enabled/set nor fixed SL/TP was calculated
                    ...
                    lg.warning(f"{NEON_YELLOW}Position is
