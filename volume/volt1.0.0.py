...
class VolumaticOBStrategy:
    ...
    def __init__(self, config: dict[str, Any], market_info: dict, logger: StructuredLogger) -> None:
        ...
        self.min_data_len = (
            max(
                ...
                self.ph_left + self.ph_right,
                self.pl_left + self.pl_right,
            )
            + 50
        )
    ...
    def update(self, df: pd.DataFrame) -> dict:
        ...
        df["trend_changed"] = (df["trend_up"] != df["trend_up"].shift(1)) | (
            df["trend_down"] != df["trend_down"].shift(1)
        )
        ...
        if signal in ["BUY", "SELL"]:
                ...
                if size:
                    ...
                    if order:
                        set_trailing_stop_loss(
                            ...
                            symbol,
                            market_info,
                            {"entryPrice": analysis_results["last_close"]},
                            config,
                            logger,
                            tp,
                        )
            ...
            if position:
                place_trade(
                    exchange,
                    symbol,
                    "SELL" if signal == "EXIT_LONG" else "BUY",
                    abs(position["size_decimal"]),
                    market_info,
                    logger,
                    reduce_only=True,
                )
