...
class TradingConfig:
    ...
    def _get_env(
        ...
        if value_str_for_cast is None and default is None:
            logger.critical(
                f"{Fore.RED + Style.BRIGHT}Required config '{key}' missing, no default. Halting."
            )
            sys.exit(1)
        casted_value = None
        try:
            if cast_type == bool:
                casted_value = str(value_str_for_cast).lower() in [
                    "true",
                    "1",
                    "yes",
                    "y",
                ...
                casted_value = int(
                    Decimal(str(value_str_for_cast))
                )  # Use Decimal intermediary
            else:
                casted_value = cast_type(
                    str(value_str_for_cast)
            ...
            try:  # Re-cast default
                if default is None:
                    ...
                    casted_value = str(default).lower() in [
                        "true",
                        "1",
                        "yes",
                        "y",
...
def termux_notify(title: str, content: str) -> None:
    ...
    if platform.system() == "Linux" and "com.termux" in os.environ.get("PREFIX", ""):
        try:
            subprocess.run(
                ["termux-toast", "-t", title, content], check=False, timeout=5
            )
...
class ExchangeManager:
    ...
    def format_price(self, price: Union[Decimal, str, float, int]) -> str:
        ...
        if (
            self.market_info
            and "precision" in self.market_info
            and "price" in self.market_info["precision"]
        ):
    ...
    def format_amount(
        ...
        if (
            self.market_info
            and "precision" in self.market_info
            and "amount" in self.market_info["precision"]
        ):
    ...
    def get_balance(self) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        try:
            ...
            if total_equity.is_nan() and settle_currency in balance_data["total"]:
                total_equity = Decimal(
                    str(balance_data[settle_currency].get("total", "NaN"))
                )
                available_balance = Decimal(
                    str(balance_data[settle_currency].get("free", "NaN"))
                )
            ...
            if available_balance.is_nan():
                logger.warning(
                    f"Could not extract available balance for {settle_currency}."
                )
    ...
    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        try:
            ...
            entry_price = (
                ...
                if entry_price_str != "0"
                else Decimal("NaN")
            )
            liq_price_str = pos_info.get("liqPrice", "0")
            liq_price = (
                Decimal(str(liq_price_str)) if liq_price_str != "0" else Decimal("NaN")
            )
            ...
            position_details = {
                ...
                "stop_loss_price": Decimal(str(sl_price_str))
                if sl_price_str and sl_price_str != "0"
                else None,
                "take_profit_price": Decimal(str(tp_price_str))
                if tp_price_str and tp_price_str != "0"
                else None,
                "is_tsl_active": bool(
                    tsl_active_price_str and tsl_active_price_str != "0"
                ),
            }
            ...
            if side == "buy" and qty.copy_abs() >= self.config.position_qty_epsilon:
                ...
                logger.debug(
                    "Position size is zero or side is 'None'. Considered flat."
                )
...
class OrderManager:
    ...
    def _calculate_trade_parameters(
        ...
        try:
            ...
            if sl_distance_price <= 0:
                logger.error(
                    f"SL distance is zero or negative ({sl_distance_price}). Cannot calculate quantity."
                )
                return None
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict]:
        ...
        try:
            ...
            termux_notify(
                f"{symbol} Order Failed", f"Market {side.upper()} failed: {e}"
            )
            return None
        except Exception as e:
            logger.error(
    ...
    def _set_position_protection(
        ...
        if is_tsl and tsl_distance and tsl_distance > 0:
            ...
            logger.warning(
                "No valid SL, TP, or TSL provided to _set_position_protection."
            )
            return False
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                termux_notify(
                    ...
                    f"{action_desc} {position_side.upper()} failed: {ret_msg[:50]}",
                )
                return False
        except Exception as e:
            logger.error(
            ...
            termux_notify(
                ...
                f"{action_desc} {position_side.upper()} error.",
            )
            return False
    ...
    def place_risked_market_order(
        ...
        if positions_after_entry is None or not positions_after_entry.get(
            position_side
        ):
            logger.error(
        ...
        avg_entry_price = positions_after_entry[position_side].get(
            ...
            if should_activate_tsl:
                ...
                if (
                    ...
                    tsl_distance_price = Decimal(
                        str(
                            self.exchange.markets[self.config.symbol]["precision"][
                                "price"
                            ]
                        )
                    )  # Use market min tick
                ...
                if self._set_position_protection(
                    position_side, is_tsl=True, tsl_distance=tsl_distance_price
                ):
                    logger.trade(
    ...
    def close_position(self, position_side: str, qty_to_close: Decimal) -> bool:
        ...
        logger.info(
            f"Clearing any existing position protection (SL/TP/TSL) for {position_side.upper()}..."
        )
        # Call set_trading_stop with zeros
        clear_stops_ok = self._set_position_protection(
            position_side, sl_price=Decimal("0"), tp_price=Decimal("0")
        )
...
class StatusDisplay:
    ...
    def print_status_panel(
        ...
        equity_str = (
            f"{equity.normalize():,.2f}"
            if equity and not equity.is_nan()
            else "[dim]N/A[/]"
        )
        ...
        if indicators:
            ...
            ind_parts.append(
                f"ATR({indicators.get('atr_period', '?')}): {fmt_ind(indicators.get('atr'), 4)}"
            )
            ind_parts.append(
        ...
        if (
            long_pos
            and long_pos.get("qty", Decimal(0)).copy_abs()
            ...
            short_pos
            and short_pos.get("qty", Decimal(0)).copy_abs()
            >= self.config.position_qty_epsilon
        ):
...
class TradingBot:
    ...
    def __init__(self):
        logger.info(
            f"{Fore.MAGENTA + Style.BRIGHT}--- Initializing Pyrmethus v2.4.0 ---"
        )
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        try:  # Extract Price & Timestamp
            ...
            logger.error(
                f"{Fore.RED}Cycle Aborted: Price/Timestamp error: {e}", exc_info=True
            )
            end_time = time.time()
            logger.info(
        ...
        if indicators is None:
            logger.error(
                f"{Fore.RED}Indicator calculation failed. Logic may be skipped."
            )
            cycle_success = False
        current_atr = (
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0:
            logger.error(
                f"{Fore.RED}Failed fetching valid equity. Logic may be skipped."
            )
            cycle_success = False
        positions = self.exchange_manager.get_current_position()
        ...
        if can_run_trade_logic:
            ...
            signals["reason"] = (
                "Skipped: Critical data missing (Equity/Position fetch failed)"
            )
            logger.warning(signals["reason"])
        ...
        logger.info(
            f"{Fore.MAGENTA}--- Cycle {cycle_count} {status_log} (Duration: {end_time - start_time:.2f}s) ---"
        )
    ...
    def graceful_shutdown(self) -> None:
        ...
        try:  # --- 1. Cancel All Open Orders ---
            logger.info(
                f"{Fore.CYAN}Attempting to cancel all open orders for {symbol}..."
            )
            cancelled = False
            try:  # Try V5 cancel all first
                ...
                if response and response.get("retCode") == 0:
                    ...
                    logger.warning(
                        f"V5 Cancel all orders response unclear/failed: {response}"
                    )
            except (NotImplementedError, Exception) as e:
                logger.warning(
                    f"V5 cancel_all exception: {e}. Trying generic cancel..."
                )

            if (
                not cancelled
            ):  # Fallback to generic cancel (might not work well for V5 position stops)
                try:
                    ...
                    logger.error(
                        f"{Fore.RED}Generic cancelAllOrders fallback error: {e}",
                        exc_info=True,
                    )
        ...
        try:  # --- 2. Close Any Lingering Positions ---
            ...
            if positions is not None:
                ...
                for side, pos_data in positions.items():
                    if (
                        pos_data
                        and pos_data.get("qty", Decimal("0")).copy_abs()
                        >= self.config.position_qty_epsilon
                    ):
