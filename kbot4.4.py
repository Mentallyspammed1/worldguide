...
class TradingConfig:
    ...
    def __init__(self):
        ...
        self.tsl_trigger_by = self._get_env("TSL_TRIGGER_BY", "LastPrice", Style.DIM, allowed_values=["LastPrice", "MarkPrice", "IndexPrice"])
    ...
    def _determine_v5_category(self) -> str:
        ...
        try:
             ...
             elif self.market_type in ['linear', 'swap']: category = 'linear' # Linear uses USDT or USDC settle
    ...
    def _get_env(self, key: str, default: Any, color: str, cast_type: type = str, min_val=None, max_val=None, allowed_values=None) -> Any:
        ...
        if value_str is None or value_str.strip() == "":
            if default is None and key not in ['BYBIT_API_KEY', 'BYBIT_API_SECRET']: logger.critical(f"{Fore.RED+Style.BRIGHT}Required config '{key}' not found. Halting."); sys.exit(1)
        ...
        if value_str_for_cast is None and default is None: logger.critical(f"{Fore.RED+Style.BRIGHT}Required config '{key}' missing, no default. Halting."); sys.exit(1)
        casted_value = None
        try:
            if cast_type == bool: casted_value = str(value_str_for_cast).lower() in ['true', '1', 'yes', 'y', 'on']
            elif cast_type == Decimal: casted_value = Decimal(str(value_str_for_cast)) # Ensure str conversion
            elif cast_type == int: casted_value = int(Decimal(str(value_str_for_cast))) # Use Decimal intermediary
            else: casted_value = cast_type(str(value_str_for_cast)) # Ensure str conversion
            ...
            try: # Re-cast default
                ...
                else: casted_value = cast_type(str(default))
        ...
        if allowed_values:
            ...
            if comp_value not in lower_allowed:
                 ...
                 try:
                    ...
                    else: return cast_type(str(default))
...
def termux_notify(title: str, content: str) -> None:
    ...
    if platform.system() == "Linux" and "com.termux" in os.environ.get("PREFIX", ""):
        try: subprocess.run(["termux-toast", "-t", title, content], check=False, timeout=5)
        except (FileNotFoundError, subprocess.TimeoutExpired, Exception) as e: logger.warning(f"Termux notify failed: {e}")
...
def fetch_with_retries(fetch_function, *args, max_retries=3, delay_seconds=3, **kwargs) -> Any:
    ...
    for attempt in range(max_retries + 1):
        ...
        except (ccxt.DDoSProtection, ccxt.RequestTimeout, ccxt.ExchangeNotAvailable, ccxt.NetworkError, ccxt.ExchangeError, requests.exceptions.RequestException) as e:
            last_exception = e
            if attempt < max_retries:
                logger.warning(f"{Fore.YELLOW}Retryable error ({type(e).__name__}) on attempt {attempt + 1}/{max_retries + 1} for {fetch_function.__name__}: {e}. Retrying in {delay_seconds}s...")
                time.sleep(delay_seconds)
            else:
                logger.error(f"{Fore.RED}Max retries ({max_retries + 1}) reached for {fetch_function.__name__}. Last error: {e}")
                break # Exit loop after max retries
        except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Authentication error: {e}. Check API keys. Halting.", exc_info=True); sys.exit(1)
...
class ExchangeManager:
    ...
    def _initialize_exchange(self):
        ...
        except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}Authentication failed: {e}. Check API keys/permissions.", exc_info=True); sys.exit(1)
    ...
    def _load_market_info(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Market info loaded: ID={market.get('id')}, Precision(Amt={market.get('precision', {}).get('amount')}, Price={market.get('precision', {}).get('price')}), Limits(MinAmt={market.get('limits',{}).get('amount',{}).get('min')})")
            # Pre-calculate Decimal precision values if needed
            market['precision']['amount_decimal'] = Decimal(str(10 ** -int(market.get('precision', {}).get('amount', 8)))) # Default 8 if missing
            market['precision']['price_decimal'] = Decimal(str(10 ** -int(market.get('precision', {}).get('price', 8))))  # Default 8 if missing
            return market
    ...
    def format_price(self, price: Union[Decimal, str, float, int]) -> str:
        ...
        else: logger.warning("Market info/price precision unavailable, using default formatting."); return str(Decimal(str(price))) # Fallback
    ...
    def format_amount(self, amount: Union[Decimal, str, float, int], rounding_mode=ROUND_DOWN) -> str:
        ...
        else: logger.warning("Market info/amount precision unavailable, using default formatting."); return str(Decimal(str(amount))) # Fallback
    ...
    def get_balance(self) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        try:
            ...
            params = {'accountType': 'UNIFIED', 'coin': settle_currency} if self.config.bybit_v5_category in ['linear', 'inverse'] else {}
                     ...
                     if unified_acc and 'totalEquity' in unified_acc and 'coin' in unified_acc:
                          for coin_info in unified_acc['coin']:
                              if coin_info.get('coin') == settle_currency:
                                  ...
                                  available_balance = Decimal(str(coin_info.get('availableToWithdraw', 'NaN'))) # Or 'availableToBorrow' depending on need
                                  break
            ...
            if total_equity.is_nan(): logger.error(f"Could not extract total equity for {settle_currency}."); return None, None
            if available_balance.is_nan(): logger.warning(f"Could not extract available balance for {settle_currency}."); available_balance = Decimal('0') # Default to 0 if unavailable
    ...
    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if not position_data: logger.debug("No position data returned."); return positions_dict # Return empty if no positions
            ...
            if not pos_info: logger.debug(f"No position found for symbol {market_id} in response."); return positions_dict
            ...
            tsl_active_price_str = pos_info.get('trailingStop', '0') # Trailing stop trigger price in V5 often means it's active
            ...
            if side == 'buy' and qty.copy_abs() >= self.config.position_qty_epsilon:
                ...
                logger.debug(f"Found LONG position: Qty={qty.normalize()}, Entry={entry_price.normalize() if not entry_price.is_nan() else 'N/A'}")
                ...
                logger.debug(f"Found SHORT position: Qty={qty.normalize()}, Entry={entry_price.normalize() if not entry_price.is_nan() else 'N/A'}")
            else:
                 logger.debug("Position size is zero or side is 'None'. Considered flat.")

            return positions_dict

        except Exception as e: logger.error(f"Failed to fetch positions: {e}", exc_info=True); return None
...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> Optional[Dict[str, Union[Decimal, bool, int]]]:
        ...
        try:
            ...
            k_prev=get_latest_decimal(stoch_k_s.iloc[-2:-1], "stoch_k_prev"); d_prev=get_latest_decimal(stoch_d_s.iloc[-2:-1], "stoch_d_prev")
...
class SignalGenerator:
    ...
    def generate_signals(self, df_last_candles: pd.DataFrame, indicators: Dict[str, Union[Decimal, bool, int]]) -> Dict[str, Union[bool, str]]:
        ...
        try:
            latest = df_last_candles.iloc[-1]; current_price = Decimal(str(latest["close"]))
            prev_close = Decimal(str(df_last_candles.iloc[-2]["close"])) if not pd.isna(df_last_candles.iloc[-2]["close"]) else Decimal("NaN")
            if current_price <= 0: return {"long": False, "short": False, "reason": "Invalid price (<= 0)"}
...
class OrderManager:
    ...
    def _calculate_trade_parameters(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> Optional[Dict]:
        ...
        try:
            ...
            if sl_distance_price <= 0: logger.error(f"SL distance is zero or negative ({sl_distance_price}). Cannot calculate quantity."); return None
            ...
            tp_price_str = self.exchange_manager.format_price(tp_price) if tp_price is not None else "0" # Use '0' for API if no TP
            ...
            if min_order_size_str:
                 ...
                 if Decimal(quantity_str) < min_order_size: logger.error(f"Calculated quantity {quantity_str} is below minimum {min_order_size}. Cannot place order."); return None

            logger.info(f"Trade Params: Side={side.upper()}, Qty={quantity_str}, Entry~={current_price.normalize()}, SL={sl_price_str}, TP={tp_price_str if tp_price else 'None'}, RiskAmt={risk_amount.normalize()}, ATR={atr.normalize()}")
            return {"qty": Decimal(quantity_str), "sl_price": Decimal(sl_price_str), "tp_price": Decimal(tp_price_str) if tp_price else None}
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict]:
        ...
        try:
            ...
            logger.trade(f"{Fore.GREEN}Market order submitted: ID {order.get('id', 'N/A')}, Side {side.upper()}, Qty {qty_str}")
            ...
            logger.info(f"Waiting {self.config.order_check_delay_seconds}s for order {order.get('id')} to potentially fill...")
    ...
    def _set_position_protection(self, position_side: str, sl_price: Optional[Decimal] = None, tp_price: Optional[Decimal] = None, is_tsl: bool = False, tsl_distance: Optional[Decimal] = None) -> bool:
        ...
        if is_tsl and tsl_distance and tsl_distance > 0:
            ...
            params['activePrice'] = '0' # Let Bybit activate based on current price movement (or specify activation price if needed)
        ...
        try:
            ...
            if not method: raise NotImplementedError(f"CCXT method {path} not found for Bybit V5.")
            ...
            if response and response.get('retCode') == 0:
                ...
                self.protection_tracker[tracker_key] = None
                termux_notify(f"{symbol} Protection FAILED", f"{action_desc} {position_side.upper()} failed: {ret_msg[:50]}")
                return False
        except Exception as e:
    ...
    def manage_trailing_stop(self, position_side: str, entry_price: Decimal, current_price: Decimal, atr: Decimal) -> None:
        ...
        try:
            ...
            if should_activate_tsl:
                ...
                if tsl_distance_price <= self.market_info['precision']['price_decimal']: # Avoid zero/too small distance
                    logger.warning(f"Calculated TSL distance ({tsl_distance_price}) too small, using minimum tick size.")
                    tsl_distance_price = Decimal(str(self.exchange.markets[self.config.symbol]['precision']['price'])) # Use market min tick

                logger.trade(f"Attempting to activate {self.config.trailing_stop_percent}% TSL (Distance ~{tsl_distance_price.normalize()})...")
    ...
    def close_position(self, position_side: str, qty_to_close: Decimal) -> bool:
        ...
        if not self.exchange or not self.market_info: return False
        if qty_to_close.copy_abs() < self.config.position_qty_epsilon: logger.warning(f"Close requested for zero/negligible qty ({qty_to_close}). Skipping."); return True
        symbol = self.config.symbol
        market_id = self.market_info.get('id')
        side_to_close = "sell" if position_side == "long" else "buy"
        tracker_key = position_side.lower()

        logger.trade(f"{Fore.YELLOW}Attempting to close {position_side.upper()} position ({qty_to_close.normalize()} {symbol})...")
    ...
    def log_trade_entry_to_journal(self, side: str, qty: Decimal, avg_price: Decimal, order_id: Optional[str]):
        ...
        try:
            with open(file_path, 'a', newline='', encoding='utf-8') as csvfile:
                ...
                if not file_exists: writer.writeheader() # Write header only if file is new
                writer.writerow({
...
class StatusDisplay:
    ...
    def print_status_panel(self, cycle: int, timestamp: Optional[pd.Timestamp], price: Optional[Decimal], indicators: Optional[Dict], positions: Optional[Dict], equity: Optional[Decimal], signals: Dict, protection_tracker: Dict):
        ...
        if indicators:
            ind_parts = []
            def fmt_ind(val, prec=1): return f"{val:.{prec}f}" if isinstance(val, Decimal) and not val.is_nan() else "[dim]N/A[/]"
            ind_parts.append(f"EMA(F/S/T): {fmt_ind(indicators.get('fast_ema'))}/{fmt_ind(indicators.get('slow_ema'))}/{fmt_ind(indicators.get('trend_ema'))}")
            ind_parts.append(f"Stoch(K/D): {fmt_ind(indicators.get('stoch_k'))}/{fmt_ind(indicators.get('stoch_d'))} {'[bold green]BULL[/]' if indicators.get('stoch_kd_bullish') else ''}{'[bold red]BEAR[/]' if indicators.get('stoch_kd_bearish') else ''}")
            ind_parts.append(f"ATR({indicators.get('atr_period', '?')}): {fmt_ind(indicators.get('atr'), 4)}")
            ind_parts.append(f"ADX({self.config.adx_period}): {fmt_ind(indicators.get('adx'))} [+DI:{fmt_ind(indicators.get('pdi'))} -DI:{fmt_ind(indicators.get('mdi'))}]")
        ...
        if long_pos and long_pos.get('qty', Decimal(0)).copy_abs() >= self.config.position_qty_epsilon:
            entry = long_pos.get('entry_price'); qty = long_pos.get('qty'); pnl = long_pos.get('unrealized_pnl', Decimal('NaN'))
            sl = long_pos.get('stop_loss_price'); tp = long_pos.get('take_profit_price')
            active_protection = protection_tracker.get("long")
            pos_str = (f"[bold green]LONG:[/] Qty={qty.normalize()} | Entry={entry.normalize() if entry else '[dim]N/A[/]'} | "
            ...
            entry = short_pos.get('entry_price'); qty = short_pos.get('qty'); pnl = short_pos.get('unrealized_pnl', Decimal('NaN'))
            sl = short_pos.get('stop_loss_price'); tp = short_pos.get('take_profit_price')
            active_protection = protection_tracker.get("short")
            pos_str = (f"[bold red]SHORT:[/] Qty={qty.normalize()} | Entry={entry.normalize() if entry else '[dim]N/A[/]'} | "
                       f"PnL={pnl.normalize() if not pnl.is_nan() else '[dim]N/A[/]'} | "
                       f"Prot: {'[magenta]' + active_protection + '[/]' if active_protection else '[dim]None[/]'} "
                       f"(SL:{sl.normalize() if sl else 'N/A'} TP:{tp.normalize() if tp else 'N/A'})")
...
class TradingBot:
    ...
    def run(self):
        ...
        while not self.shutdown_requested:
            ...
            except ccxt.AuthenticationError as e: logger.critical(f"{Fore.RED+Style.BRIGHT}CRITICAL AUTH ERROR: {e}. Halting."); break # Exit loop to shutdown
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        start_time = time.time(); cycle_success = True

        # --- 1. Fetch Data & Basic Info ---
        df = self.exchange_manager.fetch_ohlcv()
        if df is None or df.empty: logger.error(f"{Fore.RED}Cycle Aborted: Market data fetch failed."); end_time = time.time(); logger.info(f"{Fore.MAGENTA}--- Cycle {cycle_count} ABORTED (Data Fetch) ({end_time - start_time:.2f}s) ---"); return
        ...
        except (IndexError, KeyError, ValueError, InvalidOperation, TypeError) as e: logger.error(f"{Fore.RED}Cycle Aborted: Price/Timestamp error: {e}", exc_info=True); end_time = time.time(); logger.info(f"{Fore.MAGENTA}--- Cycle {cycle_count} ABORTED (Price Proc) ({end_time - start_time:.2f}s) ---"); return

        # --- 2. Calculate Indicators ---
        indicators = self.indicator_calculator.calculate_indicators(df)
        if indicators is None: logger.error(f"{Fore.RED}Indicator calculation failed. Logic may be skipped."); cycle_success = False
        ...
        if total_equity is None or total_equity.is_nan() or total_equity <= 0: logger.error(f"{Fore.RED}Failed fetching valid equity. Logic may be skipped."); cycle_success = False
        positions = self.exchange_manager.get_current_position()
        if positions is None: logger.error(f"{Fore.RED}Failed fetching positions. Logic may be skipped."); cycle_success = False
        ...
        if can_run_trade_logic:
            ...
            long_qty = active_long_pos.get('qty', Decimal('0.0')); short_qty = active_short_pos.get('qty', Decimal('0.0'))
            long_entry = active_long_pos.get('entry_price', Decimal('NaN')); short_entry = active_short_pos.get('entry_price', Decimal('NaN'))
            ...
            logger.debug("Re-fetching position state after TSL check..."); positions_after_tsl = self.exchange_manager.get_current_position()
            if positions_after_tsl is None: logger.error(f"{Fore.RED}Failed re-fetching positions after TSL check."); cycle_success = False; signals["reason"] = "Skipped: Pos re-fetch failed"
                 ...
                 if is_flat and can_gen_signals and not current_atr.is_nan() and (signals.get("long") or signals.get("short")):
                     ...
                     else: logger.error(f"{Fore.RED}Entry process failed."); cycle_success = False
        ...
        self.status_display.print_status_panel(cycle_count, last_timestamp, current_price, indicators, final_positions_for_panel, total_equity, signals, protection_tracker_snapshot)
    ...
    def graceful_shutdown(self) -> None:
        ...
        try: # --- 1. Cancel All Open Orders ---
            logger.info(f"{Fore.CYAN}Attempting to cancel all open orders for {symbol}..."); cancelled = False
            try: # Try V5 cancel all first
                 logger.debug("Using V5 cancel_all...");
                 params={'category': self.config.bybit_v5_category, 'symbol': market_id, 'orderFilter': 'Order'} # Cancel active orders
                 path = 'privatePostOrderCancelAll'; method = getattr(exchange, path, None)
                 if not method: raise NotImplementedError(f"CCXT method {path} not found.")
                 response = fetch_with_retries(method, params=params, max_retries=1, delay_seconds=1) # Less retry on shutdown
                 if response and response.get('retCode') == 0:
                     logger.info(f"{Fore.GREEN}V5 Cancel all orders successful."); cancelled = True
                     list_cancelled = response.get('result', {}).get('list', [])
                     if list_cancelled: logger.debug(f"Cancelled {len(list_cancelled)} orders.")
                     else: logger.debug("No active orders found to cancel via V5.")
                 else: logger.warning(f"V5 Cancel all orders response unclear/failed: {response}")
            except (NotImplementedError, Exception) as e: logger.warning(f"V5 cancel_all exception: {e}. Trying generic cancel...")
            ...
            if not cancelled: # Fallback to generic cancel (might not work well for V5 position stops)
                 try:
                     ...
                     cancel_resp = fetch_with_retries(exchange.cancel_all_orders, symbol=symbol, max_retries=1, delay_seconds=1)
                     logger.info(f"Generic cancelAllOrders response: {cancel_resp}")
                 except Exception as e: logger.error(f"{Fore.RED}Generic cancelAllOrders fallback error: {e}", exc_info=True)
        ...
        logger.info("Waiting after order cancellation..."); time.sleep(max(self.config.order_check_delay_seconds, 2))
    ...
    def _display_startup_info(self):
        ...
        console.print(f"[bold bright_cyan] summoning Pyrmethus [magenta]v2.4.0[/]...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"{Fore.RED+Style.BRIGHT}Critical error during bot initialization or final shutdown: {main_exception}", exc_info=True)
