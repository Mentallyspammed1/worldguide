...
color_log_formatter = logging.Formatter(
    Fore.CYAN + "%(asctime)s " + Style.BRIGHT + "[%(levelname)s] " + Style.RESET_ALL + Fore.WHITE + "%(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
...
class TradingConfig:
    ...
    def __init__(self) -> None:
        ...
        self.sl_trigger_by = self._get_env(
            "SL_TRIGGER_BY", "LastPrice", Fore.YELLOW, allowed_values=allowed_trigger_prices
        )
        self.tsl_trigger_by = self._get_env(
            "TSL_TRIGGER_BY", "LastPrice", Fore.YELLOW, allowed_values=allowed_trigger_prices
        )
    ...
    def _get_env(
        self, key: str, default: Any, color: str, cast_type: type = str, allowed_values: Optional[list] = None
    ) -> Any:
        value = os.getenv(key)
...
try:
    ...
    if CONFIG.symbol not in exchange.markets:
        ...
        with contextlib.suppress(Exception):  # Avoid crashing if market data is weird
            available_symbols = [
                ...
                and exchange.markets[s].get("subType") == CONFIG.market_type  # Match linear/inverse
                and exchange.markets[s].get("quote") == quote_currency  # Match quote currency
        ...
        if available_symbols:
            ...
            logger.info(
                Fore.CYAN + f"Could not find similar active {CONFIG.market_type} symbols with {quote_currency} quote."
            )
        sys.exit(1)
    else:
    ...
    logger.critical(
        Fore.RED + Style.BRIGHT + f"Network disturbance during Nexus initialization: {e}. Check connection. Halting."
    )
    sys.exit(1)
...
def fetch_market_data(
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(
                Fore.YELLOW + f"Network disturbance fetching data (Attempt {attempt + 1}/{retries}): {e}. Retrying..."
            )
            if attempt < retries - 1:
...
def get_current_position(symbol: str) -> Optional[Dict[str, Dict[str, Decimal]]]:
    ...
    try:
        ...
        logger.warning(
            Fore.YELLOW + f"Network disturbance consulting position spirits: {e}. Cannot reliably get position."
        )
...
def place_risked_market_order(symbol: str, side: str, risk_percentage: Decimal, atr: Decimal) -> bool:
    ...
    try:
        if CONFIG.market_type == "linear":
            ...
            if current_price.is_zero():
                raise ValueError("Current price cannot be zero for inverse calculation.")
            if stop_distance_quote.is_zero():
                raise ValueError("Stop distance cannot be zero.")  # Already checked above
    ...
    logger.info(
        Fore.YELLOW + f"Calculated Order: Side={side.upper()}, Qty={qty}, Entry≈{current_price:.4f}, SL={sl_price:.4f}"
    )
    ...
    try:
        ...
        logger.error(
            Fore.RED + Style.BRIGHT + f"Insufficient funds to place {side.upper()} market order for {qty} {symbol}: {e}"
        )
        return False
    ...
    if order_status_data:
        ...
        if order_final_status == "closed":  # 'closed' usually means fully filled for market orders
            logger.info(
                Fore.GREEN + Style.BRIGHT + f"Order {order_id} confirmed filled: {filled_qty} @ {average_price:.4f}"
            )
        ...
        logger.error(
            Fore.RED + f"Could not determine status for order {order_id}. Assuming failure. Aborting SL placement."
        )
    ...
    if sl_qty_decimal < CONFIG.position_qty_epsilon or (min_qty is not None and sl_qty_decimal < min_qty):
        ...
        try:
            ...
            logger.critical(
                Fore.RED + Style.BRIGHT + f"EMERGENCY CLOSURE FAILED: {close_err}. MANUAL INTERVENTION REQUIRED!"
            )
        return False
    ...
    try:
        ...
        logger.error(
            Fore.RED + Style.BRIGHT + f"Insufficient funds to place stop-loss order: {e}. Position is UNPROTECTED."
        )
        ...
        # Trigger emergency closure
        logger.warning(
            Fore.YELLOW + "Attempting emergency closure of unprotected position due to unexpected SL error..."
        )
        try:
            close_qty_str = format_amount(symbol, filled_qty)
            exchange.create_market_order(symbol, sl_order_side, float(close_qty_str), params={"reduceOnly": True})
...
def print_status_panel(
    ...
    panel_data.append(
        [Fore.CYAN + "Market", f"{Style.BRIGHT}{CONFIG.symbol}", f"{price_color}{price:.4f}{Style.RESET_ALL}"]
    )
    ...
    long_qty_str = (
        f"{Fore.GREEN}{long_pos['qty']}" if long_pos["qty"] > CONFIG.position_qty_epsilon else f"{Fore.WHITE}0.0"
    )
    short_qty_str = (
        f"{Fore.RED}{short_pos['qty']}" if short_pos["qty"] > CONFIG.position_qty_epsilon else f"{Fore.WHITE}0.0"
    )
...
def generate_signals(indicators: Dict[str, Decimal], current_price: Decimal) -> Dict[str, bool]:
    ...
    try:
        ...
        long_conditions_met = (
            ema_bullish_cross and stoch_bullish_cross and stoch_oversold_level  # Require K to be low when crossing D
        )
        ...
        short_conditions_met = (
            ema_bearish_cross and stoch_bearish_cross and stoch_overbought_level  # Require K to be high when crossing D
        )
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    if current_equity is None:
        ...
        logger.warning(
            Fore.YELLOW + f"Failed to fetch current {quote_currency} balance/equity. Status panel may be incomplete."
        )
    ...
    if current_atr.is_nan():
        ...
        if long_pos["qty"].copy_abs() >= CONFIG.position_qty_epsilon:
            manage_trailing_stop(
                CONFIG.symbol, "long", long_pos["qty"], long_pos["entry_price"], current_price, current_atr
            )
        # Manage short TSL if short position exists
        if short_pos["qty"].copy_abs() >= CONFIG.position_qty_epsilon:
            manage_trailing_stop(
                CONFIG.symbol, "short", short_pos["qty"], short_pos["entry_price"], current_price, current_atr
            )
    ...
    order_tracker_snapshot = {"long": order_tracker["long"].copy(), "short": order_tracker["short"].copy()}
...
def graceful_shutdown() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                Fore.YELLOW + f"Could not fetch open orders before cancelling: {fetch_err}. Proceeding with cancel all."
            )
        ...
        if open_orders:
            ...
            try:
                ...
                logger.info(
                    Fore.GREEN + f"Cancel all orders command sent. Exchange response: {response}"
                )  # Response varies
                # Add a small delay for cancellations to process
                time.sleep(2)
            except (ccxt.ExchangeError, ccxt.NetworkError) as cancel_err:
                logger.error(
                    Fore.RED + f"Error sending cancel all orders command: {cancel_err}. MANUAL ORDER CHECK REQUIRED."
                )
        ...
        logger.error(
            Fore.RED + f"Unexpected error during order cancellation phase: {e}. MANUAL CHECK REQUIRED.", exc_info=True
        )
            ...
            if qty.copy_abs() >= CONFIG.position_qty_epsilon:
                ...
                try:
                    ...
                    if close_qty_decimal < CONFIG.position_qty_epsilon or (
                        ...
                        logger.critical(
                            Fore.RED + Style.BRIGHT + f"MANUAL INTERVENTION REQUIRED TO CLOSE {side} POSITION."
                        )
...
if __name__ == "__main__":
    logger.info(
        Back.MAGENTA + Fore.WHITE + Style.BRIGHT + "*** Pyrmethus Termux Trading Spell Activated (v2 Precision) ***"
    )
    ...
    if MARKET_INFO:  # Check if market info loaded successfully during init
        ...
        logger.critical(
            Fore.RED + Style.BRIGHT + "Market info failed to load during initialization. Cannot start trading loop."
        )
        sys.exit(1)
