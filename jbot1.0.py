...
try:
    ...
    missing_pkg = e.name
    print(Fore.RED + Style.BRIGHT + f"Error: Missing required enchantment '{missing_pkg}'.")
    ...
    if missing_pkg == 'numpy':
        print(Fore.YELLOW + "Note: If numpy fails to install on Termux, you might need additional dependencies:")
...
try:
    ...
    logger.warning(Fore.YELLOW + f"Permission denied writing log file '{LOG_FILENAME}'. Log file disabled.")
except Exception as e:
    logger.warning(Fore.YELLOW + f"Failed to initialize log file handler: {e}. Log file disabled.")
...
class TradingConfig:
    ...
    def __init__(self) -> None:
        ...
        self.market_type = self._get_env("MARKET_TYPE", "linear", Fore.YELLOW, allowed_values=['linear', 'inverse'])
        ...
        self.sl_trigger_by = self._get_env("SL_TRIGGER_BY", "LastPrice", Fore.YELLOW, allowed_values=allowed_trigger_prices)
        self.tsl_trigger_by = self._get_env("TSL_TRIGGER_BY", "LastPrice", Fore.YELLOW, allowed_values=allowed_trigger_prices)

        # Threshold for considering position closed (as Decimal)
        # Adjust based on minimum contract size or typical dust amounts
        self.position_qty_epsilon = self._get_env("POSITION_QTY_EPSILON", "0.000001", Fore.YELLOW, cast_type=Decimal)
        self.api_key = self._get_env("BYBIT_API_KEY", None, Fore.YELLOW) # Use Yellow for warning maybe?
        ...
        self.loop_sleep_seconds = self._get_env("LOOP_SLEEP_SECONDS", 15, Fore.YELLOW, cast_type=int)
        self.order_check_delay_seconds = self._get_env("ORDER_CHECK_DELAY_SECONDS", 2, Fore.YELLOW, cast_type=int)
        self.order_check_timeout_seconds = self._get_env("ORDER_CHECK_TIMEOUT_SECONDS", 10, Fore.YELLOW, cast_type=int) # Max time to wait for order status check
        self.max_fetch_retries = self._get_env("MAX_FETCH_RETRIES", 3, Fore.YELLOW, cast_type=int)
        ...
        if self.risk_percentage <= 0 or self.risk_percentage >= 1:
            logger.warning(f"{Fore.YELLOW}RISK_PERCENTAGE ({self.risk_percentage}) seems unusual. Ensure it's a decimal (e.g., 0.01 for 1%).")
        if self.sl_atr_multiplier <= 0 or self.tsl_activation_atr_multiplier <= 0 or self.trailing_stop_percent <= 0:
             logger.warning(f"{Fore.YELLOW}ATR/TSL multipliers and percentages should be positive.")

    def _get_env(self, key: str, default: Any, color: str, cast_type: type = str, allowed_values: Optional[list] = None) -> Any:
        value = os.getenv(key)
        ...
        if value is None:
            ...
            if default is not None:
                logger.warning(f"{color}Using default value for {key}: {'****' if is_secret else default}")
        else:
        ...
        try:
            ...
            logger.error(f"{Fore.RED}Could not cast {key} ('{log_value}') to {cast_type.__name__}: {e}. Using default: {'****' if is_secret else default}")
        ...
        if allowed_values is not None and casted_value not in allowed_values:
             ...
             if default in allowed_values:
                 ...
                 logger.critical(f"{Fore.RED + Style.BRIGHT}Default value '{default}' for {key} is also not in allowed values {allowed_values}. Halting.")
                 sys.exit(1)
...
MARKET_INFO: Optional[Dict[str, Any]] = None  # Global to store market details after connection
    ...
    if CONFIG.symbol not in exchange.markets:
         ...
         with contextlib.suppress(Exception): # Avoid crashing if market data is weird
             available_symbols = [
                 ...
                 if exchange.markets[s].get('active') and
                    exchange.markets[s].get('type') == 'future' and # Ensure it's a future
                    exchange.markets[s].get('subType') == CONFIG.market_type and # Match linear/inverse
                    exchange.markets[s].get('quote') == quote_currency # Match quote currency
             ][:15] # Limit suggestions
         if available_symbols:
              logger.info(Fore.CYAN + f"Available active {CONFIG.market_type} symbols with {quote_currency} quote (sample): " + ", ".join(available_symbols))
         else:
              logger.info(Fore.CYAN + f"Could not find similar active {CONFIG.market_type} symbols with {quote_currency} quote.")
         sys.exit(1)
    else:
        ...
        logger.debug(f"Raw Market Precision: Price={price_prec_raw}, Amount={amount_prec_raw}")
        logger.debug(f"Raw Market Limits: Min Amount={min_amount_raw}, Max Amount={max_amount_raw}")
        ...
        try:
            ...
            if min_amount_raw is not None: Decimal(str(min_amount_raw))
            # Max amount can sometimes be None/null
            if max_amount_raw is not None: Decimal(str(max_amount_raw))
            Decimal(str(contract_size_raw))
            logger.info("Successfully parsed market precision, limits, and contract size.")
        except (TypeError, ValueError, Exception) as e:
             logger.critical(f"{Fore.RED + Style.BRIGHT}Failed to parse critical market info (precision/limits/size) as numbers: {e}. Market Info: {MARKET_INFO}. Halting.")
             sys.exit(1)


except ccxt.AuthenticationError:
    logger.critical(Fore.RED + Style.BRIGHT + "Authentication failed! Check your API keys in the .env scroll. Halting.")
    sys.exit(1)
except ccxt.ExchangeNotAvailable:
    logger.critical(Fore.RED + Style.BRIGHT + "Exchange Nexus is currently unavailable (e.g., maintenance). Halting.")
    sys.exit(1)
except ccxt.NetworkError as e:
    logger.critical(Fore.RED + Style.BRIGHT + f"Network disturbance during Nexus initialization: {e}. Check connection. Halting.")
    sys.exit(1)
except ccxt.ExchangeError as e:
    logger.critical(Fore.RED + Style.BRIGHT + f"Exchange Nexus Error during initialization: {e}. Halting.")
    sys.exit(1)
except Exception as e:
...
def termux_notify(title: str, content: str) -> None:
    ...
    try:
        result = subprocess.run(['command', '-v', 'termux-toast'], capture_output=True, text=True, check=False)
        if result.returncode != 0:
            logger.debug("termux-toast command not found or not executable. Skipping notification.")
            return
    ...
    try:
        ...
        logger.warning(Fore.YELLOW + "termux-toast command not found when trying to notify.")
    except subprocess.TimeoutExpired:
        logger.warning(Fore.YELLOW + "Termux notification command timed out.")
    except subprocess.CalledProcessError as e:
        logger.warning(Fore.YELLOW + f"Termux notification command failed with error: {e.stderr}")
    except Exception as e:
...
def fetch_market_data(symbol: str, timeframe: str, limit: int, retries: int = CONFIG.max_fetch_retries) -> Optional[pd.DataFrame]:
    ...
    logger.info(Fore.CYAN + f"Channeling market whispers for {symbol} ({timeframe}, limit={limit})...")
    for attempt in range(retries):
        try:
            # Check if exchange object is valid and has the method
            if not hasattr(exchange, 'fetch_ohlcv'):
                 logger.error(Fore.RED + "Exchange object not properly initialized or lacks fetch_ohlcv.")
                 return None

            ohlcv: list[list[Union[int, float]]] = exchange.fetch_ohlcv(symbol, timeframe, limit=limit)

            if not ohlcv:
                # It's possible to get an empty list if the market has no recent trades
                logger.warning(Fore.YELLOW + f"Received empty OHLCV data (Attempt {attempt + 1}/{retries}). Market might be inactive?")
                # Continue retrying in case it was a temporary glitch
                if attempt < retries - 1:
                    time.sleep(1 * (attempt + 1))  # Simple backoff
                    continue
                else:
                    logger.error(Fore.RED + f"Received empty OHLCV data after {retries} attempts. Cannot proceed.")
                    return None
            ...
            if df.empty:
                logger.error(Fore.RED + "DataFrame is empty after processing OHLCV data (possibly all rows had NaNs).")
                return None
...
def calculate_indicators(df: pd.DataFrame) -> Optional[Dict[str, Decimal]]:
    ...
    try:
        ...
        trend_ema_series = close.ewm(span=50, adjust=False).mean() # Using 50 for trend is common
        ...
        if len(df) < stoch_period:
             logger.warning(f"Not enough data ({len(df)}) for Stochastic period {stoch_period}. Using default values.")
        else:
        ...
        if len(df) < atr_period + 1:  # Need at least period+1 for shift() and initial calculation
             logger.warning(f"Not enough data ({len(df)}) for ATR period {atr_period}. Using default value 0.")
        else:
            ...
            atr_series = tr.ewm(alpha=1/atr_period, adjust=False).mean() # Pandas EWMA with alpha=1/N approximates Wilder's for large N
            ...
            if pd.isna(atr): atr = 0.0  # Handle potential NaN at start
        ...
        def safe_decimal(value, quantum):
            if pd.isna(value): return Decimal("NaN") # Or Decimal(0)? NaN might be clearer.
        ...
        if any(val.is_nan() for val in indicators_dec.values()):
             logger.warning(f"{Fore.YELLOW}Some indicators resulted in NaN: {indicators_dec}")
...
def get_current_position(symbol: str) -> Optional[Dict[str, Dict[str, Decimal]]]:
    ...
    try:
        ...
        symbol_positions = [
            p for p in positions_raw if p.get('symbol') == symbol and p.get('info') and p.get('contracts') is not None
        ]

        if not symbol_positions:
            logger.info(Fore.BLUE + f"No detailed position data found for {symbol} in fetched list.")
            return pos_dict
            ...
            if side in pos_dict and contracts_str is not None:
                try:
                    ...
                    if contracts.copy_abs() < CONFIG.position_qty_epsilon:
                        logger.debug(f"Ignoring effectively zero size position entry: Side={side}, Qty={contracts}")
                        continue
                    ...
                    if current_qty.is_zero(): # First entry for this side
                        ...
                        if not total_qty.is_zero():
                             weighted_entry = ((current_qty * current_entry) + (contracts * entry_price)) / total_qty
                             pos_dict[side]["entry_price"] = weighted_entry # Keep precision
                    ...
                    if pos_dict[side]["qty"].copy_abs() >= CONFIG.position_qty_epsilon:
                         logger.info(Fore.YELLOW + f"Detected active {side} position: Qty={pos_dict[side]['qty']}, Avg Entry={pos_dict[side]['entry_price']:.6f}")
                         active_positions_found += 1
                    else:
                         # If aggregation resulted in zero, reset entry price
                         logger.info(Fore.BLUE + f"{side} position is now effectively zero after aggregation.")
                         pos_dict[side]["qty"] = Decimal("0.0")
                         pos_dict[side]["entry_price"] = Decimal("0.0")


                except (TypeError, ValueError) as e:
                    logger.error(f"{Fore.RED}Error parsing position data for side {side}: {e}. Data: {pos}")
                    # Continue to next position entry if possible
                    continue
            elif side not in pos_dict:
                 logger.warning(f"{Fore.YELLOW}Received position data with unexpected side '{side}'. Ignoring.")


        if active_positions_found == 0:
             logger.info(Fore.BLUE + f"No active non-zero positions found for {symbol} after processing.")

        logger.info(Fore.GREEN + "Position spirits consulted.")
        return pos_dict

    except (ccxt.NetworkError, ccxt.RequestTimeout, ccxt.DDoSProtection) as e:
        logger.warning(Fore.YELLOW + f"Network disturbance consulting position spirits: {e}. Cannot reliably get position.")
        return None  # Indicate failure
    except ccxt.AuthenticationError:
        logger.error(Fore.RED + "Authentication error fetching positions. Check API keys.")
        return None # Indicate failure
    except ccxt.ExchangeError as e:
...
def get_balance(currency: str = "USDT") -> Tuple[Optional[Decimal], Optional[Decimal]]:
    ...
    try:
        ...
        if not isinstance(free_balance_data, dict) or not isinstance(total_balance_data, dict):
             logger.error(f"{Fore.RED}Unexpected balance structure received. Cannot parse {currency} balance.")
        ...
        try:
            ...
            logger.warning(f"{Fore.YELLOW}Could not parse free balance '{free_balance_str}' to Decimal: {e}. Using 0.")
        try:
            if total_balance_str is not None:
                total_balance = Decimal(str(total_balance_str))
        except Exception as e:
            logger.warning(f"{Fore.YELLOW}Could not parse total balance '{total_balance_str}' to Decimal: {e}. Using 0.")
        ...
        logger.error(Fore.RED + "Authentication error fetching balance. Check API keys.")
        return None, None
    except ccxt.ExchangeError as e:
...
def check_order_status(order_id: str, symbol: str, timeout: int = CONFIG.order_check_timeout_seconds) -> Optional[Dict[str, Any]]:
    """
    ...
    """
    logger.info(Fore.CYAN + f"Verifying status of order {order_id} for {symbol} (timeout: {timeout}s)...")
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            if order_status and isinstance(order_status, dict):
                ...
                logger.info(f"Order {order_id} status check attempt {attempt}: Status '{status}'")
                ...
                logger.warning(f"fetch_order returned empty or unexpected structure for {order_id} (Attempt {attempt}). Retrying...")
            ...
            logger.warning(Fore.YELLOW + f"Order {order_id} not found by exchange (Attempt {attempt}).")
            # Returning None indicates it's not fetchable / doesn't exist in an open/pending state.
            return None

        except (ccxt.NetworkError, ccxt.RequestTimeout, ccxt.DDoSProtection) as e:
            logger.warning(f"Network issue checking order {order_id} (Attempt {attempt}): {e}. Retrying...")
        except ccxt.AuthenticationError:
            logger.error(Fore.RED + f"Authentication error checking order {order_id}. Check API keys.")
            ...
            logger.error(f"Exchange error checking order {order_id} (Attempt {attempt}): {e}. Retrying...")
            # Consider if specific exchange errors should terminate the check (e.g., invalid order ID format)
        except Exception as e:
            logger.error(f"Unexpected error checking order {order_id} (Attempt {attempt}): {e}", exc_info=True)
    ...
    logger.error(Fore.RED + f"Timed out or failed to determine status for order {order_id} after {timeout} seconds and {attempt} attempts.")
...
def place_risked_market_order(symbol: str, side: str, risk_percentage: Decimal, atr: Decimal) -> bool:
    ...
    if free_balance is None or free_balance <= Decimal("0"):
        logger.error(Fore.RED + f"Cannot place order: Invalid or zero available {quote_currency} balance.")
        return False
    ...
    logger.debug(f"ATR: {atr:.6f}, SL Multiplier: {CONFIG.sl_atr_multiplier}, SL Distance Pts: {sl_distance_points:.6f}")
    ...
    if side == "buy" and sl_price >= current_price:
        logger.error(Fore.RED + f"Calculated SL price ({sl_price}) is >= current price ({current_price}). Risk too high or ATR too large? Aborting.")
        return False
    ...
    risk_amount_quote = free_balance * risk_percentage # Risk amount in quote currency (e.g., USDT)
    # Stop distance in quote currency (absolute difference between entry and SL price)
    stop_distance_quote = abs(current_price - sl_price) # Use current price as estimated entry
    ...
    try:
        if CONFIG.market_type == 'linear':
            ...
            if contract_size.is_zero(): raise ValueError("Contract size cannot be zero.")
            qty_raw = risk_amount_quote / (stop_distance_quote * contract_size)
            logger.debug(f"Linear Sizing: Risk={risk_amount_quote:.4f}, StopDist={stop_distance_quote:.4f}, ContractSize={contract_size}")
            ...
            if current_price.is_zero(): raise ValueError("Current price cannot be zero for inverse calculation.")
            if stop_distance_quote.is_zero(): raise ValueError("Stop distance cannot be zero.") # Already checked above
            ...
            if contract_size == Decimal('1'): # Assuming $1 contract size for inverse
                 ...
                 logger.debug(f"Inverse Sizing ($1 Contract): Risk={risk_amount_quote:.4f}, StopDist={stop_distance_quote:.4f}, Price={current_price}")
            else:
                 ...
                 logger.warning(f"{Fore.YELLOW}Inverse contract size is not 1 ({contract_size}). Sizing calculation might need adjustment based on exact contract specs. Using simplified formula.")
                 ...
                 qty_raw = (risk_amount_quote / stop_distance_quote) * current_price # Stick to $1 assumption with warning.
         ...
         logger.error(f"{Fore.RED}Error during quantity calculation: {e}. Check inputs (price, stop distance, contract size).")
         return False
    ...
    if qty.is_zero() or qty < CONFIG.position_qty_epsilon:
        logger.error(Fore.RED + f"Calculated quantity ({qty}) is zero or too small after precision formatting. Risk amount, price movement, or ATR might be too small.")
        return False
    if min_qty is not None and qty < min_qty:
        logger.error(Fore.RED + f"Calculated quantity {qty} is below minimum {min_qty}. Cannot place order. Increase risk or adjust strategy.")
    ...
    logger.info(Fore.YELLOW + f"Calculated Order: Side={side.upper()}, Qty={qty}, Entry≈{current_price:.4f}, SL={sl_price:.4f}")
    ...
    try:
        ...
        order = exchange.create_market_order(symbol, side, float(qty), params={}) # No extra params needed for basic market order
        order_id = order.get('id')
        if not order_id:
            logger.error(Fore.RED + "Market order submission failed to return an ID. Order might have failed.")
        ...
        logger.debug(f"Failed order details: Symbol={symbol}, Side={side}, Qty={float(qty)}")
        return False
    except Exception as e:
    ...
    # 6. Verify Order Fill
    logger.info(f"Waiting {CONFIG.order_check_delay_seconds}s before checking order status...")
    ...
    if order_status_data:
        ...
        try:
            ...
            if average_str is not None:
             ...
             logger.warning(f"{Fore.YELLOW}Could not parse filled quantity or average price from order status: {e}. Using defaults.")
             logger.debug(f"Order Status Data: {order_status_data}")


        if order_final_status == 'closed': # 'closed' usually means fully filled for market orders
            ...
            if abs(filled_qty - qty) > CONFIG.position_qty_epsilon * Decimal("10"): # Allow slightly larger tolerance for fill vs request
                 logger.warning(f"{Fore.YELLOW}Filled quantity {filled_qty} differs significantly from requested {qty}.")
             ...
             logger.warning(Fore.YELLOW + f"Order {order_id} status is '{order_final_status}'. Filled: {filled_qty}. SL will be based on filled amount.")
             if filled_qty < CONFIG.position_qty_epsilon:
                  ...
                  with contextlib.suppress(Exception): exchange.cancel_order(order_id, symbol)
                  return False
    ...
    if sl_qty_decimal < CONFIG.position_qty_epsilon or (min_qty is not None and sl_qty_decimal < min_qty):
         logger.error(Fore.RED + f"Stop loss quantity ({sl_qty_decimal}) is invalid (zero, too small, or below min {min_qty}) after formatting filled amount {filled_qty}. Cannot place SL.")
         ...
         try:
             ...
             close_qty_str = format_amount(symbol, filled_qty) # Format again just in case
             exchange.create_market_order(symbol, sl_order_side, float(close_qty_str), params={'reduceOnly': True})
             logger.info(Fore.GREEN + "Emergency closure order placed for unprotected position.")
    ...
    logger.info(Fore.CYAN + f"Placing Initial SL order: Side={sl_order_side}, Qty={sl_qty_str_for_order}, Trigger={sl_price_str_for_order}, TriggerBy={CONFIG.sl_trigger_by}")
    ...
    try:
        ...
        if not sl_order_id:
            ...
            raise ccxt.ExchangeError(f"Stop loss order placement did not return an ID. Response: {sl_order}")

        # Store the SL order ID
        order_tracker[position_side]["sl_id"] = sl_order_id
        order_tracker[position_side]["tsl_id"] = None # Ensure TSL is cleared on new entry
        ...
        termux_notify("Trade Entry", f"{side.upper()} {symbol} @ {average_price:.4f} | SL ID: ...{sl_order_id[-6:]}")
        ...
        logger.error(Fore.RED + Style.BRIGHT + f"Failed to place initial SL order (ID: {sl_order_id if sl_order_id else 'N/A'}): {e}. Position might be UNPROTECTED.")
        ...
        # Trigger emergency closure
        logger.warning(Fore.YELLOW + "Attempting emergency closure of unprotected position due to unexpected SL error...")
        try:
             close_qty_str = format_amount(symbol, filled_qty)
             exchange.create_market_order(symbol, sl_order_side, float(close_qty_str), params={'reduceOnly': True})
             logger.info(Fore.GREEN + "Emergency closure order placed.")
...
def manage_trailing_stop(
    ...
    if position_qty < CONFIG.position_qty_epsilon or entry_price <= Decimal("0"):
        ...
        if order_tracker[position_side]["tsl_id"] or order_tracker[position_side]["sl_id"]:
             logger.debug(f"Position {position_side} closed or invalid, clearing any stale order trackers.")
    ...
    if has_tsl:
        logger.debug(f"{position_side.upper()} TSL (ID: {tsl_id}) is already active. Exchange managing trail.")
    ...
    if not has_initial_sl:
        logger.debug(f"No initial SL tracked for {position_side} position. Cannot activate TSL.")
        return

    if atr is None or atr.is_nan() or atr <= Decimal("0"):
        logger.warning(Fore.YELLOW + "Cannot evaluate TSL activation: Invalid ATR value ({atr}).")
        return
    ...
    logger.debug(f"{position_side.upper()} Profit: {profit:.4f}, TSL Activation Threshold (Points): {activation_threshold_points:.4f} (ATR={atr:.4f})")
    ...
    if profit > activation_threshold_points:
        ...
        # --- 1. Cancel Initial SL ---
        logger.info(Fore.CYAN + f"Attempting to cancel initial SL (ID: {initial_sl_id}) before placing TSL...")
        try:
            ...
            order_tracker[position_side]["sl_id"] = None # Mark as cancelled locally immediately
        except ccxt.OrderNotFound:
            logger.warning(Fore.YELLOW + f"Initial SL (ID: {initial_sl_id}) not found when trying to cancel. Might have been triggered or already cancelled/rejected.")
            order_tracker[position_side]["sl_id"] = None # Assume it's gone
        except (ccxt.ExchangeError, ccxt.NetworkError) as e:
            logger.error(Fore.RED + f"Failed to cancel initial SL (ID: {initial_sl_id}): {e}.")
            logger.warning(Fore.YELLOW + "Proceeding with TSL placement attempt, but risk of duplicate orders exists if SL cancellation failed silently.")
        ...
        min_qty_str = MARKET_INFO.get('limits', {}).get('amount', {}).get('min')
        min_qty = Decimal(str(min_qty_str)) if min_qty_str is not None else None
        if tsl_qty_decimal < CONFIG.position_qty_epsilon or (min_qty is not None and tsl_qty_decimal < min_qty):
             logger.error(Fore.RED + f"Trailing stop loss quantity ({tsl_qty_decimal}) is invalid (zero, too small, or below min {min_qty}) using position qty {position_qty}. Cannot place TSL.")
             logger.warning(Fore.YELLOW + "Position is now UNPROTECTED as initial SL may have been cancelled. MANUAL INTERVENTION MAY BE NEEDED.")
        ...
        logger.info(Fore.CYAN + f"Placing TSL order: Side={tsl_order_side}, Qty={tsl_qty_str}, Trail%={trail_percent_value_str}, TriggerBy={CONFIG.tsl_trigger_by}")
        ...
        try:
            ...
            if not new_tsl_id:
                 raise ccxt.ExchangeError(f"Trailing stop order placement did not return an ID. Response: {tsl_order}")

            # Update tracker
            order_tracker[position_side]["tsl_id"] = new_tsl_id
            order_tracker[position_side]["sl_id"] = None # Ensure initial SL ID is cleared
            logger.info(Fore.GREEN + Style.BRIGHT + f"Trailing Stop Loss activated for {position_side.upper()}: ID {new_tsl_id}, Trail: {trail_percent_value_str}%")
            termux_notify("TSL Activated", f"{position_side.upper()} {symbol} TSL ID: ...{new_tsl_id[-6:]}")

        except (ccxt.ExchangeError, ccxt.InvalidOrder) as e:
            logger.error(Fore.RED + Style.BRIGHT + f"Failed to place TSL order (ID: {new_tsl_id if new_tsl_id else 'N/A'}, tried type '{order_type}'): {e}")
            logger.warning(Fore.YELLOW + "Position might be UNPROTECTED as initial SL was likely cancelled and TSL placement failed.")
            ...
            logger.critical(Fore.RED + Style.BRIGHT + "MANUAL INTERVENTION RECOMMENDED: Check position and place a stop loss manually if needed.")
            ...
            logger.warning(Fore.YELLOW + "Position might be unprotected after unexpected TSL placement error.")
            order_tracker[position_side]["tsl_id"] = None
            logger.critical(Fore.RED + Style.BRIGHT + "MANUAL INTERVENTION RECOMMENDED due to unexpected TSL error.")


def print_status_panel(
    cycle: int, timestamp: pd.Timestamp, price: Decimal, indicators: Dict[str, Decimal],
    positions: Dict[str, Dict[str, Decimal]], equity: Optional[Decimal], signals: Dict[str, bool],
    ...
    # Time & Cycle
    panel_data.append([Fore.MAGENTA + "Cycle", f"{Style.BRIGHT}{cycle}", f"{timestamp.strftime('%Y-%m-%d %H:%M:%S')}"])
    ...
    panel_data.append([Fore.CYAN + "Market", f"{Style.BRIGHT}{CONFIG.symbol}", f"{price_color}{price:.4f}{Style.RESET_ALL}"])
    ...
    ema_status = f"{Fore.GREEN}Bullish" if ema_cross_color == Fore.GREEN else f"{Fore.RED}Bearish" if ema_cross_color == Fore.RED else "Neutral"
    panel_data.append([Fore.CYAN + "EMA (8/12)", f"{ema_cross_color}{fast_ema:.4f} / {slow_ema:.4f}", ema_status])

    trend_status = f"{price_color}(Above Trend)" if price > trend_ema else f"{price_color}(Below Trend)" if price < trend_ema else "(At Trend)"
    panel_data.append([Fore.CYAN + "EMA Trend (50)", f"{Fore.WHITE}{trend_ema:.4f}", trend_status])

    stoch_k = indicators.get('stoch_k', Decimal('NaN'))
    stoch_d = indicators.get('stoch_d', Decimal('NaN'))
    stoch_color = Fore.GREEN if stoch_k < Decimal(25) else Fore.RED if stoch_k > Decimal(75) else Fore.YELLOW
    stoch_status = f"{Fore.GREEN}Oversold (<25)" if stoch_color == Fore.GREEN else f"{Fore.RED}Overbought (>75)" if stoch_color == Fore.RED else "Neutral"
    panel_data.append([Fore.CYAN + "Stoch %K/%D", f"{stoch_color}{stoch_k:.2f} / {stoch_d:.2f}", stoch_status])

    # Account & Positions
    equity_str = f"{equity:.2f} {MARKET_INFO.get('quote', 'USD')}" if equity is not None and not equity.is_nan() else f"{Fore.YELLOW}N/A"
    ...
    long_qty_str = f"{Fore.GREEN}{long_pos['qty']}" if long_pos['qty'] > CONFIG.position_qty_epsilon else f"{Fore.WHITE}0.0"
    short_qty_str = f"{Fore.RED}{short_pos['qty']}" if short_pos['qty'] > CONFIG.position_qty_epsilon else f"{Fore.WHITE}0.0"
    panel_data.append([Fore.CYAN + "Position Qty", long_qty_str + " (Long)", short_qty_str + " (Short)"])

    long_entry_str = f"{long_pos['entry_price']:.4f}" if long_pos['qty'] > CONFIG.position_qty_epsilon else "-"
    short_entry_str = f"{short_pos['entry_price']:.4f}" if short_pos['qty'] > CONFIG.position_qty_epsilon else "-"
    panel_data.append([Fore.CYAN + "Entry Price", f"{Fore.GREEN}{long_entry_str}", f"{Fore.RED}{short_entry_str}"])
    ...
    # Signals
    long_signal_color = Fore.GREEN + Style.BRIGHT if signals.get('long', False) else Fore.WHITE + Style.DIM
    short_signal_color = Fore.RED + Style.BRIGHT if signals.get('short', False) else Fore.WHITE + Style.DIM
    panel_data.append([Fore.CYAN + "Signals", f"{long_signal_color}LONG", f"{short_signal_color}SHORT"])

    # Format with tabulate
    # Using 'fancy_grid' for better visual separation
    output = tabulate(panel_data, headers=headers, tablefmt="fancy_grid", stralign="left", numalign="left")
    print("\n" + output)
...
def generate_signals(indicators: Dict[str, Decimal], current_price: Decimal) -> Dict[str, bool]:
    ...
    try:
        ...
        if any(ind.is_nan() for ind in [k, d, fast_ema, slow_ema, trend_ema]):
             logger.warning(f"{Fore.YELLOW}Cannot generate signals: One or more indicators are NaN. Indicators: {indicators}")
             return signals
        ...
        logger.debug(f"Signal Inputs: Price={current_price:.4f}, FastEMA={fast_ema:.4f}, SlowEMA={slow_ema:.4f}, TrendEMA={trend_ema:.4f}, StochK={k:.2f}, StochD={d:.2f}")
        logger.debug(f"Conditions: EMA Bullish={ema_bullish_cross}, EMA Bearish={ema_bearish_cross}, Price Above Trend={price_above_trend}, Stoch Oversold={stoch_oversold_level}, Stoch Overbought={stoch_overbought_level}, Stoch Bull Cross={stoch_bullish_cross}")
        ...
        long_conditions_met = (
            ema_bullish_cross and
            stoch_bullish_cross and
            stoch_oversold_level # Require K to be low when crossing D
        )
        if long_conditions_met:
            if CONFIG.trade_only_with_trend:
                if price_above_trend:
                    signals["long"] = True
                    logger.debug("Long Signal Criteria Met: EMA Cross Bullish, Stoch Bullish Cross in Oversold, Price Above Trend EMA")
                else:
                    logger.debug("Long Signal Blocked: Price Below Trend EMA (Trend Filter ON)")
            else: # Trend filter off
                signals["long"] = True
                logger.debug("Long Signal Criteria Met: EMA Cross Bullish, Stoch Bullish Cross in Oversold (Trend Filter OFF)")
        ...
        short_conditions_met = (
            ema_bearish_cross and
            stoch_bearish_cross and
            stoch_overbought_level # Require K to be high when crossing D
        )
        if short_conditions_met:
             if CONFIG.trade_only_with_trend:
                 if price_below_trend:
                     signals["short"] = True
                     logger.debug("Short Signal Criteria Met: EMA Cross Bearish, Stoch Bearish Cross in Overbought, Price Below Trend EMA")
                 else:
                     logger.debug("Short Signal Blocked: Price Above Trend EMA (Trend Filter ON)")
             else: # Trend filter off
                 signals["short"] = True
                 logger.debug("Short Signal Criteria Met: EMA Cross Bearish, Stoch Bearish Cross in Overbought (Trend Filter OFF)")
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    try:
        ...
        logger.error(Fore.RED + "Failed to get current price from DataFrame (IndexError).")
        return
    except Exception as e:
    ...
    if indicators is None:
        ...
        return
    current_atr = indicators.get('atr', Decimal('NaN')) # Keep as Decimal, check for NaN later
    ...
    if current_equity is None:
        ...
        logger.warning(Fore.YELLOW + f"Failed to fetch current {quote_currency} balance/equity. Status panel may be incomplete.")
    ...
    is_flat = long_pos['qty'].copy_abs() < CONFIG.position_qty_epsilon and short_pos['qty'].copy_abs() < CONFIG.position_qty_epsilon
    logger.debug(f"Position Status: Flat = {is_flat} (Long Qty: {long_pos['qty']}, Short Qty: {short_pos['qty']})")
    ...
    if is_flat:
        ...
        if current_atr.is_nan():
            ...
            trade_executed = place_risked_market_order(CONFIG.symbol, "buy", CONFIG.risk_percentage, current_atr)
        elif signals.get("short"):
            logger.info(Fore.RED + Style.BRIGHT + "Short signal detected! Attempting entry...")
            trade_executed = place_risked_market_order(CONFIG.symbol, "sell", CONFIG.risk_percentage, current_atr)
        else:
        ...
        if trade_executed:
            ...
            time.sleep(CONFIG.order_check_delay_seconds + 1) # Small pause slightly longer than order check delay
    ...
    print_status_panel(
        ...
        positions, current_equity, signals, order_tracker_snapshot # Use the snapshot
    )
...
def graceful_shutdown() -> None:
    ...
    termux_notify("Shutdown Started", f"Closing orders/positions for {CONFIG.symbol}...")

    # Ensure exchange object is available
    if 'exchange' not in globals() or not hasattr(exchange, 'cancel_all_orders'):
        logger.error(Fore.RED + "Exchange object not available for shutdown. Cannot manage orders/positions.")
        return
    ...
    try:
        ...
        if open_orders:
            ...
            logger.info(f"Found {len(open_orders)} open orders to cancel: {', '.join(order_ids)}")
            try:
                ...
                logger.info(Fore.GREEN + f"Cancel all orders command sent. Exchange response: {response}") # Response varies
                # Add a small delay for cancellations to process
                time.sleep(2)
            except (ccxt.ExchangeError, ccxt.NetworkError) as cancel_err:
                 logger.error(Fore.RED + f"Error sending cancel all orders command: {cancel_err}. MANUAL ORDER CHECK REQUIRED.")
            except Exception as cancel_exc:
                 logger.error(Fore.RED + f"Unexpected error sending cancel all orders command: {cancel_exc}. MANUAL ORDER CHECK REQUIRED.")
        else:
        ...
        logger.error(Fore.RED + f"Unexpected error during order cancellation phase: {e}. MANUAL CHECK REQUIRED.", exc_info=True)
    ...
    try:
        ...
        if positions is None:
             logger.error(Fore.RED + "Could not fetch final positions during shutdown. MANUAL POSITION CHECK REQUIRED.")
        ...
        for side, pos_data in positions.items():
             ...
             if qty.copy_abs() >= CONFIG.position_qty_epsilon:
                 ...
                 try:
                     ...
                     close_qty_str = format_amount(CONFIG.symbol, qty.copy_abs()) # Use absolute value for amount
                     close_qty_decimal = Decimal(close_qty_str)

                     # Final check on close quantity validity
                     min_qty_str = MARKET_INFO.get('limits', {}).get('amount', {}).get('min') if MARKET_INFO else None
                     min_qty = Decimal(str(min_qty_str)) if min_qty_str is not None else None
                     if close_qty_decimal < CONFIG.position_qty_epsilon or (min_qty is not None and close_qty_decimal < min_qty):
                          logger.error(Fore.RED + f"Cannot close {side} position: Formatted close quantity {close_qty_decimal} is invalid (zero, too small, or below min {min_qty}).")
                          logger.critical(Fore.RED + Style.BRIGHT + f"MANUAL INTERVENTION REQUIRED TO CLOSE {side} POSITION.")
                     ...
                     logger.info(Fore.GREEN + f"Position closure order placed: Side={close_side}, Qty={close_qty_str}, ID={close_order_id}")
                     closed_count += 1
                     ...
                     time.sleep(CONFIG.order_check_delay_seconds)
                      ...
                      logger.critical(Fore.RED + Style.BRIGHT + f"Insufficient funds to close {side} position ({qty}). MANUAL INTERVENTION REQUIRED!")
                 except (ccxt.ExchangeError, ccxt.InvalidOrder) as e:
        ...
        logger.error(Fore.RED + f"Error during position closure check phase: {e}. Manual check advised.", exc_info=True)
...
if __name__ == "__main__":
    ...
    if MARKET_INFO: # Check if market info loaded successfully during init
         ...
         logger.info(Fore.GREEN + Style.BRIGHT + "Nexus connection stable. Awaiting market whispers...")
    else:
         # Error should have been logged during init, this is a final check
         logger.critical(Fore.RED + Style.BRIGHT + "Market info failed to load during initialization. Cannot start trading loop.")
         sys.exit(1)
