...
class TradingConfig:
    ...
    DEFAULT_CONFIG = {
        ...
        "ORDER_CHECK_DELAY_SECONDS": "3",
        "ORDER_CHECK_TIMEOUT_SECONDS": "15",
        "MAX_FETCH_RETRIES": "3",
    ...
    TYPE_VALIDATIONS = {
        "SYMBOL": str,
        "MARKET_TYPE": (str, ["linear", "inverse"]),
        "INTERVAL": (
            str,
        ...
        "USE_TESTNET": bool,
        "ENABLE_TRADING": bool,
        "LOG_LEVEL": (str, list(logging._nameToLevel.keys())),
        "RISK_PERCENTAGE": (Decimal, Decimal("0.0001"), Decimal("0.5")),
        "LEVERAGE": (int, 1, 200),
        "SL_ATR_MULTIPLIER": (Decimal, Decimal("0.1"), Decimal("10.0")),
        "TP_ATR_MULTIPLIER": (Decimal, Decimal("0.1"), Decimal("20.0")),
        "ENABLE_TRAILING_STOP": bool,
        "TSL_ACTIVATION_ATR_MULTIPLIER": (Decimal, Decimal("0.0"), Decimal("10.0")),
        "TRAILING_STOP_DISTANCE_ATR_MULTIPLIER": (
            Decimal,
            Decimal("0.1"),
            Decimal("10.0"),
        ),
        ...
        "BREAK_EVEN_TRIGGER_ATR_MULTIPLE": (Decimal, Decimal("0.0"), Decimal("10.0")),
        ...
        "ACTIVE_WEIGHT_SET": str,
        "OHLCV_LIMIT": (int, 50, 1000),
        "INDICATOR_ATR_PERIOD": (int, 2, 500),
        "INDICATOR_EMA_SHORT": (int, 2, 500),
        "INDICATOR_EMA_LONG": (int, 3, 1000),
        "INDICATOR_TREND_EMA": (int, 5, 1000),
        "INDICATOR_STOCH_RSI_WINDOW": (int, 5, 500),
        "INDICATOR_STOCH_RSI_K": (int, 1, 500),
        "INDICATOR_STOCH_RSI_D": (int, 1, 500),
        "INDICATOR_RSI_WINDOW": (int, 2, 500),
        "INDICATOR_CCI_WINDOW": (int, 2, 500),
        "TRADE_ONLY_WITH_TREND": bool,
        "LOOP_SLEEP_SECONDS": (int, 5, 600),
        "ORDER_CHECK_DELAY_SECONDS": (int, 1, 30),
        "ORDER_CHECK_TIMEOUT_SECONDS": (int, 5, 120),
        "MAX_FETCH_RETRIES": (int, 1, 10),
        "QUOTE_CURRENCY": str,
    }
    ...
    def __init__(self) -> None:
        ...
        if not self.ccxt_interval:
            logger.critical(
                f"Invalid INTERVAL '{self.interval}'. Valid: {list(self.CCXT_INTERVAL_MAP.keys())}. Halting."
            )
            sys.exit(1)
    ...
    def _get_and_validate_env(self, key: str, default: Any) -> Any:
        ...
        if (
            value_str is None and default is not None
        ):  # Required parameter missing, fatal
            logger.critical(
                f"Required configuration key {key} not found in .env and has no default! Halting."
            )
            sys.exit(1)

        validation_info = self.TYPE_VALIDATIONS.get(key)
        if not validation_info:
            logger.warning(f"No validation defined for '{key}'.")
            return value_str  # Treat as string
        ...
        try:
            ...
            logger.error(
                f"Cast Error for {key}='{value_str}' to {target_type.__name__}: {e}. Using default: '{default}'."
            )
            # Re-cast default
            try:
                if default is None:
                ...
                logger.critical(
                    f"Default '{default}' for {key} also fails casting. Halting."
                )
                sys.exit(1)
        ...
        if isinstance(validation_info, tuple):
            ...
            if isinstance(constraint, list):  # Allowed values
                if casted_value not in constraint:
                    validation_passed = False
                    logger.error(
                        f"{key} '{casted_value}' not in allowed list {constraint}."
                    )
            elif constraint is not None:  # Min value
                if casted_value < constraint:
                    validation_passed = False
                    logger.error(f"{key} '{casted_value}' < min '{constraint}'.")
            if max_val is not None:  # Max value
                if casted_value > max_val:
                    validation_passed = False
                    logger.error(f"{key} '{casted_value}' > max '{max_val}'.")
        ...
        try:
            ...
            if len(base_quote) == 2:
                ...
                self.settle_currency = (
                    parts[1] if len(parts) > 1 else self.quote_currency
                )
                logger.info(
                    f"Parsed Symbol '{self.symbol}': Base={self.base_currency}, Quote={self.quote_currency}, Settle={self.settle_currency}"
                )
                # Override default QUOTE_CURRENCY with parsed value
                self.default_quote_currency = (
                    self.quote_currency
                )  # Keep original default if needed
                self.quote_currency = (
                    self.settle_currency
                )  # Use settle currency for balance/futures
            else:
                raise ValueError("Incorrect symbol format")
        except Exception as e:
            logger.warning(
                f"Could not parse Base/Quote/Settle from SYMBOL '{self.symbol}': {e}. Using default Quote: {self.quote_currency}."
            )
            ...
            try:
                ...
                self.quote_currency = (
                    base_quote[1]
                    if len(base_quote) > 1
                    else self.DEFAULT_CONFIG["QUOTE_CURRENCY"]
                )
                self.settle_currency = self.quote_currency
                logger.info(
                    f"Partial Parse '{self.symbol}': Base={self.base_currency}, Quote/Settle={self.quote_currency}"
                )
            except:
    ...
    def _validate_dependent_params(self) -> None:
        ...
        # TSL Method Priority
        if (
            self.trailing_stop_distance_atr_multiplier is not None
            and self.trailing_stop_callback_rate is not None
        ):
            logger.warning(
                "Both TSL ATR Multiplier and Callback Rate are set. Prioritizing ATR Multiplier."
            )
            self.trailing_stop_callback_rate = None
        elif (
            self.trailing_stop_distance_atr_multiplier is None
            and self.trailing_stop_callback_rate is None
            and self.enable_trailing_stop
        ):
            logger.error(
                "TSL is enabled, but neither ATR Multiplier nor Callback Rate is set for distance. Disabling TSL."
            )
        ...
        if self.indicator_ema_long <= self.indicator_ema_short:
            logger.warning(
                f"EMA Long ({self.indicator_ema_long}) <= EMA Short ({self.indicator_ema_short}). Setting Long = Short + 1."
            )
            self.indicator_ema_long = self.indicator_ema_short + 1
        if self.indicator_trend_ema <= self.indicator_ema_long:
            logger.warning(
                f"Trend EMA ({self.indicator_trend_ema}) <= EMA Long ({self.indicator_ema_long}). Trend filter might be less effective."
            )
    ...
    def _log_key_parameters(self) -> None:
        ...
        logger.info(
            f"Symbol: {self.symbol} ({self.market_type.capitalize()}) | Timeframe: {self.interval} ({self.ccxt_interval})"
        )
        logger.info(
            f"Trading: {'LIVE' if self.enable_trading else 'DISABLED'} | Environment: {'REAL MONEY' if not self.use_testnet else 'SANDBOX (Testnet)'}"
        )
        logger.info(
            f"Risk: {self.risk_percentage * 100:.2f}% | Leverage: {self.leverage}x | Quote: {self.quote_currency}"
        )
        logger.info(
            f"Initial SL/TP (ATR Mult): {self.sl_atr_multiplier} / {self.tp_atr_multiplier}"
        )
        tsl_mode = "DISABLED"
        if self.enable_trailing_stop:
            dist_method = ""
            if self.trailing_stop_distance_atr_multiplier:
                dist_method = f"Dist:{self.trailing_stop_distance_atr_multiplier}*ATR"
            elif self.trailing_stop_callback_rate:
                dist_method = f"Callback:{self.trailing_stop_callback_rate:.3%}"
            else:
                dist_method = "ERROR:NoDistanceMethod"
            tsl_mode = (
                f"ENABLED ({dist_method}, Act:{self.tsl_activation_atr_multiplier}*ATR)"
            )
        logger.info(f"Trailing SL: {tsl_mode}")
        be_mode = (
            f"ENABLED (Trig:{self.break_even_trigger_atr_multiple}*ATR, Offset:{self.break_even_offset_ticks} ticks)"
            if self.enable_break_even
            else "DISABLED"
        )
        logger.info(f"Break-Even SL: {be_mode}")
        logger.info(
            f"Trend Filter (EMA {self.indicator_trend_ema}): {'ON' if self.trade_only_with_trend else 'OFF'}"
        )
...
def termux_notify(title: str, content: str) -> None:
    ...
    try:
        ...
        logger.warning(f"Termux notification failed: {e}")


# --- Precision Formatting ---
def format_price(symbol: str, price: Any) -> str | None:
    global MARKET_INFO, EXCHANGE
    if not EXCHANGE or not MARKET_INFO or price is None:
        return str(price)
    try:
...
def fetch_with_retries(fetch_function, *args, **kwargs) -> Any:
    global EXCHANGE
    if not EXCHANGE:
        logger.critical("Exchange object is None!")
        return None
    last_exception = None
    ...
    if "bybit" in EXCHANGE.id.lower():
        ...
        if "category" not in params:
            params["category"] = CONFIG.market_type
    ...
    for attempt in range(CONFIG.max_fetch_retries + 1):
        try:
            ...
            wait_time = 2**attempt
            logger.warning(
                f"{fetch_function.__name__}: Network issue (Attempt {attempt + 1}). Retrying in {wait_time}s... Error: {e}"
            )
        except ccxt.RateLimitExceeded as e:
            last_exception = e
            wait_time = 5 * (attempt + 1)
            logger.warning(
                f"{fetch_function.__name__}: Rate limit hit. Retrying in {wait_time}s... Error: {e}"
            )
        except ccxt.ExchangeNotAvailable as e:
            last_exception = e
            logger.error(f"{fetch_function.__name__}: Exchange unavailable: {e}. Stop.")
            break
        except ccxt.AuthenticationError as e:
            last_exception = e
            logger.critical(f"{fetch_function.__name__}: Auth error: {e}. Halt.")
            sys.exit(1)
        except (ccxt.OrderNotFound, ccxt.InsufficientFunds, ccxt.InvalidOrder) as e:
            last_exception = e
            logger.warning(
                f"{fetch_function.__name__}: Encountered {type(e).__name__}: {e}. Propagating."
            )
            raise e
            ...
            if error_code in non_retryable_codes:
                logger.error(
                    f"{fetch_function.__name__}: Non-retryable error (Code:{error_code}): {e}. Stop."
                )
                break
            else:
                wait_time = 2 * (attempt + 1)
                logger.warning(
                    f"{fetch_function.__name__}: Exchange error (Code:{error_code}). Retrying in {wait_time}s... Error: {e}"
                )
        except Exception as e:
            last_exception = e
            logger.error(
                f"{fetch_function.__name__}: Unexpected error: {e}", exc_info=True
            )
            break
        ...
        if attempt < CONFIG.max_fetch_retries:
            ...
            logger.error(
                f"{fetch_function.__name__} failed after {CONFIG.max_fetch_retries + 1} attempts. Last exception: {last_exception}"
            )
            break
...
def fetch_balance() -> Decimal | None:
    ...
    try:
        ...
        )  # Let fetch_with_retries handle params if needed
        if not balance_data:
            return None
        ...
        if currency in balance_data:
            ...
            if equity is not None:
                balance_val_str = str(equity)
                logger.debug("Using 'equity' for balance.")
            elif total is not None:
                balance_val_str = str(total)
                logger.debug("Using 'total' for balance.")
            elif free is not None:
                balance_val_str = str(free)
                logger.debug("Using 'free' for balance.")

        # Add Bybit V5 parsing as fallback
        elif "info" in balance_data and isinstance(
            balance_data["info"].get("result", {}).get("list"), list
        ):
            for account in balance_data["info"]["result"]["list"]:
                if (
                    account.get("accountType") == "CONTRACT"
                    or account.get("accountType") == "UNIFIED"
                ):  # Look in relevant account types
                    equity = account.get("equity")  # V5 equity
                    wallet = account.get("walletBalance")
                    available = account.get("availableToWithdraw")
                    if equity is not None:
                        balance_val_str = str(equity)
                        logger.debug("Using V5 'equity' for balance.")
                        break
                    if wallet is not None:
                        balance_val_str = str(wallet)
                        logger.debug("Using V5 'walletBalance' for balance.")
                        break
                    if available is not None:
                        balance_val_str = str(available)
                        logger.debug("Using V5 'availableToWithdraw' for balance.")
                        break
            if balance_val_str:
                pass  # Found it
            else:
                logger.warning(
                    f"Could not find {currency} balance in Bybit V5 structure."
                )
        ...
        if balance_val_str:
            ...
            logger.error(f"Could not parse balance for {currency}.")
            return None
    except Exception as e:
...
def get_market_info(symbol: str) -> dict | None:
    ...
    try:
        ...
        if symbol not in EXCHANGE.markets:
            logger.error(f"Symbol {symbol} not found after reload.")
            return None
        MARKET_INFO = EXCHANGE.market(symbol)
        MARKET_INFO["is_contract"] = MARKET_INFO.get(
            "contract", False
        ) or MARKET_INFO.get("type") in ["swap", "future"]
        logger.info(
            f"Market info loaded for {symbol}. Type: {MARKET_INFO.get('type')}, Contract: {MARKET_INFO['is_contract']}"
        )
...
def get_current_position(symbol: str) -> dict | None:
    ...
    try:
        ...
        if not isinstance(positions, list):
            logger.error("fetch_positions returned non-list.")
            return None
        ...
        if size_str is None:
            logger.error("Could not determine position size.")
            return None
        ...
        if side not in ["long", "short"]:
            if standardized_pos["size"] > Decimal("1e-9"):
                side = "long"
            elif standardized_pos["size"] < Decimal("-1e-9"):
                side = "short"
            else:
                logger.error("Position size near zero, cannot determine side.")
                return None
        ...
        if entry_price_str:
            ...
            logger.warning("Missing entry price.")
        ...
        for key, info_key in [
            ...
            standardized_pos[key] = (
                Decimal(str(val_str)) if val_str is not None else None
            )
        ...
        if sl_str and str(sl_str) != "0":
            active_protection_state["stopLossPrice"] = Decimal(str(sl_str))
        if tp_str and str(tp_str) != "0":
            active_protection_state["takeProfitPrice"] = Decimal(str(tp_str))
        if tsl_val_str and str(tsl_val_str) != "0":
            active_protection_state["trailingStopValue"] = Decimal(str(tsl_val_str))
        if tsl_act_str and str(tsl_act_str) != "0":
            active_protection_state["trailingStopActivationPrice"] = Decimal(
                str(tsl_act_str)
            )
        ...
        # Timestamp & Index
        standardized_pos["timestamp_ms"] = pos.get("timestamp") or info_dict.get(
            "updatedTime"
        )
        standardized_pos["positionIdx"] = int(info_dict.get("positionIdx", 0))

        # Margin Mode
        standardized_pos["marginMode"] = str(
            info_dict.get("tradeMode", "cross")
        ).lower()

        if standardized_pos["entryPrice"] is None:
            logger.error("Processed position lacks valid entry price.")
            return None

        logger.info(
            f"Active {side.upper()} position found for {symbol}. Size: {standardized_pos['size']}"
        )
        return standardized_pos

    except Exception as e:
        logger.error(f"Error fetching/processing position: {e}", exc_info=True)
        return None
...
class TradingAnalyzer:
    ...
    def __init__(
        ...
        self.weights = self.config.get("weight_sets", {}).get(
        ...
        if not self.weights:
            logger.error(f"Weight set '{self.active_weight_set_name}' empty/missing.")
    ...
    def get_price_precision(self) -> int:
        ...
        try:
            ...
            if isinstance(price_precision_val, (float, str)):
                ...
                if tick > 0:
                    return abs(tick.normalize().as_tuple().exponent)
            # Add other fallbacks if needed (e.g., limits.price.min)
        except Exception as e:
            logger.warning(f"Error getting price precision: {e}. Using default.")
        return 4  # Default
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            logger.warning(f"Error getting tick size: {e}. Using default.")
        return Decimal(
            "1e-" + str(self.get_price_precision())
        )  # Fallback based on precision
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        expected_patterns = {
            ...
            "Trend_EMA": [
                f"EMA_{self.config.get('indicator_trend_ema')}"
            ],  # Specific key for trend EMA
            "StochRSI_K": [
                f"STOCHRSIk_{self.config.get('indicator_stoch_rsi_window')}_"
            ],  # Partial pattern
            "StochRSI_D": [
                f"STOCHRSId_{self.config.get('indicator_stoch_rsi_window')}_"
            ],  # Partial pattern
            "RSI": [f"RSI_{self.config.get('indicator_rsi_window')}"],
            "CCI": [
                f"CCI_{self.config.get('indicator_cci_window')}_"
            ],  # Partial pattern
        ...
        for col in result_df.columns:
            for pattern in patterns:
                if col.startswith(pattern):
                    return col  # Use startswith for flexibility
        # Basic fallback
        for col in result_df.columns:
            if base_name.lower() in col.lower():
                return col
        logger.warning(f"Could not map TA col name for '{base_name}'")
        return None

    def _calculate_all_indicators(self) -> None:
        if self.df.empty:
            logger.warning("DataFrame empty, skipping indicator calc.")
            return
        ...
        if len(self.df) < required_len:
            logger.warning(
                f"Data length {len(self.df)} < {required_len}, indicator accuracy may suffer."
            )
        ...
        try:
            ...
            atr_p = cfg["indicator_atr_period"]
            df_calc.ta.atr(length=atr_p, append=True)
            self.ta_column_names["ATR"] = self._get_ta_col_name("ATR", df_calc)

            # EMA Alignment & Trend
            ema_s = cfg["indicator_ema_short"]
            df_calc.ta.ema(length=ema_s, append=True)
            self.ta_column_names["EMA_Short"] = self._get_ta_col_name(
                "EMA_Short", df_calc
            )
            ema_l = cfg["indicator_ema_long"]
            df_calc.ta.ema(length=ema_l, append=True)
            self.ta_column_names["EMA_Long"] = self._get_ta_col_name(
                "EMA_Long", df_calc
            )
            ema_t = cfg["indicator_trend_ema"]
            df_calc.ta.ema(length=ema_t, append=True)
            self.ta_column_names["Trend_EMA"] = self._get_ta_col_name(
                "Trend_EMA", df_calc
            )

            # Stochastic RSI
            st_win = cfg["indicator_stoch_rsi_window"]
            st_k = cfg["indicator_stoch_rsi_k"]
            st_d = cfg["indicator_stoch_rsi_d"]
            stochrsi_df = df_calc.ta.stochrsi(
                length=st_win, rsi_length=cfg["indicator_rsi_window"], k=st_k, d=st_d
            )  # Use rsi_window here
            if stochrsi_df is not None:
                df_calc = pd.concat([df_calc, stochrsi_df], axis=1)
            ...
            # Standard RSI
            rsi_p = cfg["indicator_rsi_window"]
            df_calc.ta.rsi(length=rsi_p, append=True)
            self.ta_column_names["RSI"] = self._get_ta_col_name("RSI", df_calc)

            # CCI
            cci_p = cfg["indicator_cci_window"]
            df_calc.ta.cci(length=cci_p, append=True)
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        if self.df.empty or self.df.iloc[-1].isnull().all():
            logger.warning(
                "Cannot update latest indicator values: DataFrame empty or last row is all NaN."
            )
        ...
        for key, col_name in self.ta_column_names.items():
            if col_name and col_name in latest.index and pd.notna(latest[col_name]):
                ...
                updated_values[key] = (
                    None  # Store None if column missing or value is NaN
                )
        ...
        for base_col in ["open", "high", "low", "close", "volume"]:
            if base_col in latest.index and pd.notna(latest[base_col]):
                try:
                    updated_values[base_col.capitalize()] = Decimal(
                        str(latest[base_col])
                    )
                except:
                    updated_values[base_col.capitalize()] = None
            else:
                updated_values[base_col.capitalize()] = None
    ...
    def calculate_fibonacci_levels(
        ...
        try:
            ...
            if diff > 0:
                ...
                levels = {
                    f"Fib_{pct * 100:.1f}%": high
                    for pct in [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]
                }
            self.fib_levels_data = levels
            logger.debug(
                f"Fibonacci Levels: { {k: f'{v:.{self.get_price_precision()}f}' for k, v in levels.items()} }"
            )
            return levels
        except Exception as e:
    ...
    def get_nearest_fibonacci_levels(
        ...
        try:
            dists = [
                {"n": n, "l": l, "d": abs(current_price - l)}
                for n, l in self.fib_levels_data.items()
                if isinstance(l, Decimal)
            ]
            dists.sort(key=lambda x: x["d"])
            return [(i["n"], i["l"]) for i in dists[:num_levels]]
        except Exception as e:
            logger.error(f"Error finding nearest Fibs: {e}")
            return []
    ...
    def calculate_entry_tp_sl(
        ...
        if not isinstance(atr_val, Decimal) or atr_val <= 0:
            logger.warning("Cannot calc TP/SL: Invalid ATR.")
            return entry_price_estimate, None, None
        if not isinstance(entry_price_estimate, Decimal) or entry_price_estimate <= 0:
            logger.warning("Cannot calc TP/SL: Invalid entry estimate.")
            return entry_price_estimate, None, None
        ...
        try:
            ...
            tp, sl = None, None
            if signal == "BUY":
                tp = (entry_price_estimate + atr_val * tp_mult).quantize(
                    rounding_factor, rounding=ROUND_UP
                )
                sl = (entry_price_estimate - atr_val * sl_mult).quantize(
                    rounding_factor, rounding=ROUND_DOWN
                )
                # Validation
                if sl >= entry_price_estimate:
                    sl = (entry_price_estimate - min_tick).quantize(
                        rounding_factor, rounding=ROUND_DOWN
                    )
                if tp <= entry_price_estimate:
                    tp = None  # Invalidate non-profitable TP
            else:  # SELL
                tp = (entry_price_estimate - atr_val * tp_mult).quantize(
                    rounding_factor, rounding=ROUND_DOWN
                )
            ...
            if sl is not None and sl <= 0:
                logger.error("Stop loss calc resulted in non-positive price!")
                sl = None
            if tp is not None and tp <= 0:
                logger.warning("Take profit calc resulted in non-positive price!")
                tp = None

            logger.debug(
                f"Calculated TP/SL: Entry={entry_price_estimate:.{price_prec}f}, TP={tp}, SL={sl}, ATR={atr_val:.{price_prec + 1}f}"
            )
            return entry_price_estimate, tp, sl
        except Exception as e:
            logger.error(f"Error calculating TP/SL: {e}", exc_info=True)
            return entry_price_estimate, None, None
    ...
    def generate_trading_signal(
        ...
        if not all(
            isinstance(v, Decimal) for v in [ema_s, ema_l, trend_ema, current_price]
        ):
        ...
        if bullish_cross:
            if self.config.get("trade_only_with_trend", True):
                if price_above_trend:
                    final_signal = "BUY"
                    reason = "EMA Bull Cross + Above Trend"
                else:
                    reason = "Long Blocked: Below Trend"
            else:
                final_signal = "BUY"
                reason = "EMA Bull Cross (Trend Filter OFF)"
        elif bearish_cross:
            if self.config.get("trade_only_with_trend", True):
                if price_below_trend:
                    final_signal = "SELL"
                    reason = "EMA Bear Cross + Below Trend"
                else:
                    reason = "Short Blocked: Above Trend"
            else:
                final_signal = "SELL"
                reason = "EMA Bear Cross (Trend Filter OFF)"

        logger.info(
            f"Signal Check ({self.symbol} @ {current_price:.{self.get_price_precision()}f}): {reason} ==> {final_signal}"
        )
...
def set_leverage_ccxt(
    ...
    if not is_contract:
        lg.info(f"Leverage skipped for {symbol} (Spot).")
        return True
    if leverage <= 0:
        lg.warning(f"Leverage skipped: Invalid value ({leverage}).")
        return False
    if not exchange.has.get("setLeverage"):
        lg.error(f"{exchange.id} does not support setLeverage.")
        return False
    ...
    try:
        ...
        lg.info(
            f"{NEON_GREEN}Leverage set/requested to {leverage}x for {symbol}.{RESET}"
        )
        return True
        ...
        if code == 110045 or "not modified" in err_str:
            lg.info("Leverage already set.")
            return True  # Treat as success
        # Add hints for other codes if needed (risk limit, margin mode etc.)
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected leverage error ({symbol}): {e}{RESET}", exc_info=True
        )
    return False
...
def calculate_position_size(
    ...
    if (
        not all(
            isinstance(v, Decimal) and v > 0
            for v in [balance, entry_price, initial_stop_loss_price]
        )
        or not (0 < risk_per_trade < 1)
        or initial_stop_loss_price == entry_price
    ):
        lg.error(f"Sizing fail ({symbol}): Invalid inputs.")
        return None
    if "limits" not in market_info or "precision" not in market_info:
        lg.error(f"Sizing fail ({symbol}): Market info missing.")
        return None
    ...
    try:
        ...
        if sl_dist <= 0:
            lg.error(f"Sizing fail ({symbol}): SL dist zero/neg.")
            return None
        ...
        if is_linear or not is_contract:
            ...
            if denominator <= 0:
                lg.error(f"Sizing fail ({symbol}): Zero denominator.")
                return None
            calculated_size = risk_amount / denominator
        else:  # Placeholder for Inverse - needs specific exchange logic
            lg.error(
                f"Inverse contract sizing logic required but not fully implemented for {symbol}. Cannot size."
            )
            return None

        lg.info(
            f"Sizing ({symbol}): RiskAmt={risk_amount:.4f} {quote}, SLDist={sl_dist:.{MARKET_INFO['precision'].get('price', 4)}f}, InitialSize={calculated_size:.8f} {size_unit}"
        )
        ...
        if adj_size != calculated_size:
            lg.warning(
                f"Size adjusted by Amount Limits: {calculated_size:.8f} -> {adj_size:.8f}"
            )

        # Cost Check (Simplified for Linear/Spot)
        est_cost = adj_size * entry_price * contract_size
        if min_cost > 0 and est_cost < min_cost:
            lg.error(f"Cost {est_cost:.4f} < MinCost {min_cost}. Cannot meet limit.")
            return None
        if max_cost > 0 and est_cost > max_cost:
            ...
            if entry_price * contract_size > 0:
                ...
                if adj_size < min_amount:
                    lg.error("Size reduced below Min Amount by Max Cost.")
                    return None
            else:
                lg.error("Cannot adjust for Max Cost (zero price/contract size).")
                return None

        # Apply Amount Precision (Format and convert back)
        final_size_str = format_amount(symbol, adj_size, ROUND_DOWN)
        if final_size_str is None:
            lg.error("Failed to format amount.")
            return None
        final_size = Decimal(final_size_str)

        # Final Validations
        if final_size <= 0:
            lg.error("Final size zero/negative.")
            return None
        if final_size < min_amount:
            lg.error(f"Final size {final_size} < Min Amount {min_amount}.")
            return None
        final_cost = final_size * entry_price * contract_size  # Recheck cost
        if min_cost > 0 and final_cost < min_cost:
            lg.error(f"Final cost {final_cost:.4f} < Min Cost {min_cost}.")
            return None

        lg.info(f"{NEON_GREEN}Final Size ({symbol}): {final_size} {size_unit}{RESET}")
        return final_size
    except Exception as e:
        lg.error(f"Unexpected sizing error ({symbol}): {e}", exc_info=True)
        return None
...
def place_trade(
    ...
    try:  # Size validation
        ...
        if amount_float <= 0 or position_size < min_amount:
            raise ValueError(f"Invalid size {position_size}")
    except Exception as e:
        lg.error(f"Trade Aborted ({symbol}): Invalid size {e}")
        return None
    if order_type == "limit" and (
        not isinstance(limit_price, Decimal) or limit_price <= 0
    ):
        lg.error(f"Trade Aborted ({symbol}): Invalid limit price {limit_price}")
        return None
    ...
    try:
        order_fn = (
            ...
            else exchange.create_limit_order
        )
        price_arg = None if order_type == "market" else float(limit_price)
        order = fetch_with_retries(
            order_fn, symbol, side, amount_float, price_arg, order_params
        )

        if order:
            lg.info(
                f"{NEON_GREEN}Order Request Sent ({symbol}): ID={order.get('id', 'N/A')}, Status={order.get('status', 'N/A')}{RESET}"
            )
            lg.debug(f"Raw Order Response: {order}")
            return order
        else:
            lg.error(
                f"{NEON_RED}Order placement failed after retries ({symbol} {side}).{RESET}"
            )
            return None
    except (ccxt.InsufficientFunds, ccxt.InvalidOrder, ccxt.ExchangeError) as e:
        lg.error(
            f"{NEON_RED}Trade Placement Error ({symbol} {side}): {type(e).__name__} - {e}{RESET}"
        )
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected trade error ({symbol} {side}): {e}{RESET}",
            exc_info=True,
        )
    return None
...
def _set_position_protection(  # Internal helper for Bybit V5 protection endpoint
    ...
    if "bybit" not in exchange.id.lower():
        lg.error("Protection setting currently Bybit V5 specific.")
        return False
    if not market_info.get("is_contract", False):
        lg.warning("Protection skipped (not contract).")
        return False
    if not position_info:
        lg.error("Missing position info.")
        return False

    pos_side = position_info.get("side")
    pos_idx = position_info.get("positionIdx", 0)
    if pos_side not in ["long", "short"]:
        lg.error(f"Invalid pos side '{pos_side}'.")
        return False
    ...
    if not has_sl and not has_tp and not has_tsl:
        lg.info("No valid protection provided.")
        return True
    ...
    try:  # Formatting
        ...
        def fmt_p(p):
            return format_price(symbol, p) if p and p > 0 else None
        ...
        if has_tsl:
            fmt_d = fmt_tsl_d(trailing_stop_distance)
            fmt_a = fmt_p(tsl_activation_price)
            if fmt_d and fmt_a:
                params["trailingStop"] = fmt_d
                params["activePrice"] = fmt_a
                ...
                lg.error("Failed to format TSL params.")
                has_tsl = False

        if has_sl:
            fmt_sl = fmt_p(stop_loss_price)
            if fmt_sl:
                params["stopLoss"] = fmt_sl
                params["slTriggerBy"] = CONFIG.sl_trigger_by
                log_parts.append(f"  SL: {fmt_sl}")
            else:
                has_sl = False
        ...
        if has_tp:
            ...
            if fmt_tp:
                params["takeProfit"] = fmt_tp
                params["tpTriggerBy"] = CONFIG.tp_trigger_by
                log_parts.append(f"  TP: {fmt_tp}")
            else:
                has_tp = False
        ...
        if (
            ...
            lg.error("No valid protection params remained after formatting.")
            return False

    except Exception as fmt_err:
        lg.error(f"Error formatting protection params: {fmt_err}", exc_info=True)
        return False
    ...
    try:
        response = fetch_with_retries(
            exchange.private_post, "/v5/position/set-trading-stop", params=params
        )
        if response is None:
        ...
        if ret_code == 0:
            if "not modified" in ret_msg.lower():
                lg.info(
                    f"{NEON_YELLOW}Protection already set/partially modified ({symbol}): {ret_msg}{RESET}"
                )
            else:
                lg.info(
                    f"{NEON_GREEN}Protection set/updated successfully ({symbol}).{RESET}"
                )
            return True
        else:
            lg.error(
                f"{NEON_RED}Failed to set protection ({symbol}): {ret_msg} (Code:{ret_code}){RESET}"
            )
...
def set_trailing_stop_loss(  # Calculates parameters based on config, calls internal helper
    ...
    try:  # Get position/analysis data
        ...
        if (
            ...
            raise ValueError("Invalid position info")
        if not isinstance(current_atr, Decimal) or current_atr <= 0:
            raise ValueError(f"Invalid ATR {current_atr}")
        ...
        act_price = (
            entry_price + act_offset if side == "long" else entry_price - act_offset
        )
        act_price = act_price.quantize(
            price_rounding, rounding=ROUND_UP if side == "long" else ROUND_DOWN
        )
        if side == "long" and act_price <= entry_price:
            act_price = (entry_price + min_tick).quantize(price_rounding, ROUND_UP)
        if side == "short" and act_price >= entry_price:
            act_price = (entry_price - min_tick).quantize(price_rounding, ROUND_DOWN)
        if act_price <= 0:
            raise ValueError(f"Invalid activation price {act_price}")

        # Calculate Trailing Distance
        trail_dist: Decimal | None = None
        if dist_atr_mult is not None:  # Prioritize ATR distance
            if not isinstance(dist_atr_mult, Decimal):
                dist_atr_mult = Decimal(str(dist_atr_mult))  # Ensure Decimal
            ...
            if not isinstance(callback_rate, Decimal):
                callback_rate = Decimal(str(callback_rate))  # Ensure Decimal
            if callback_rate <= 0:
                raise ValueError("Invalid callback rate")
            trail_dist_raw = (
                act_price * callback_rate
            )  # Distance based on activation price
            lg.debug(f"TSL Dist Method: Callback Rate ({callback_rate:.3%})")
        else:
            raise ValueError("No TSL distance method configured")

        # Round distance UP to nearest tick, ensure > min_tick
        trail_dist = (trail_dist_raw / min_tick).quantize(
            Decimal("1"), rounding=ROUND_UP
        ) * min_tick
        if trail_dist < min_tick:
            trail_dist = min_tick
        if trail_dist <= 0:
            raise ValueError(f"Invalid trailing distance {trail_dist}")

        lg.info(
            f"Calculated TSL: Activate @ {act_price:.{price_prec}f}, Trail Distance ≈ {trail_dist:.{price_prec}f}"
        )
        ...
        lg.error(f"Error calculating/setting TSL ({symbol}): {e}", exc_info=True)
        return False
...
def set_breakeven_stop_loss(  # Checks conditions, calls internal helper
    ...
    lg = logger
    config = config_dict
    if not config.get("enable_break_even", False):
    ...
    if active_protection_state.get("trailingStopValue") is not None:
        lg.debug("BE check skipped: Trailing Stop is already active.")
        return False

    try:  # Get position and analysis data
        entry_price = position_info.get("entryPrice")
        side = position_info.get("side")
        current_sl = active_protection_state.get(
            "stopLossPrice"
        )  # Get current SL from global state
        current_atr = analyzer.indicator_values.get("ATR")
        current_price = analyzer.indicator_values.get("Close")  # Current close price
        if not all(
            isinstance(v, Decimal) and v > 0
            for v in [entry_price, current_atr, current_price]
        ) or side not in [
            "long",
            "short",
        ]:
            raise ValueError(
                f"Invalid inputs for BE check (entry={entry_price}, atr={current_atr}, price={current_price}, side={side})"
            )
    except Exception as e:
        lg.error(f"BE Check Error ({symbol}): Invalid data - {e}")
        return False

    try:  # Check trigger condition
        trigger_atr_mult = Decimal(
        ...
        price_prec = analyzer.get_price_precision()
        lg.debug(
            f"BE Check ({symbol}): Profit Pts={current_profit_points:.{price_prec}f}, Target Pts={profit_target_points:.{price_prec}f}"
        )
        ...
        if current_profit_points >= profit_target_points:
            ...
            be_stop_price = (
                entry_price + tick_offset
                if side == "long"
                else entry_price - tick_offset
            )
            be_stop_price = be_stop_price.quantize(
                min_tick, rounding=ROUND_UP if side == "long" else ROUND_DOWN
            )
            if be_stop_price <= 0:
                raise ValueError("Invalid BE stop price calculated")

            # Determine if update needed
            update_needed = False
            if current_sl is None:
                update_needed = True
                lg.info("BE trigger: No current SL.")
            elif side == "long" and be_stop_price > current_sl:
                update_needed = True
                lg.info(f"BE trigger: Moving SL UP {current_sl} -> {be_stop_price}.")
            elif side == "short" and be_stop_price < current_sl:
                update_needed = True
                lg.info(f"BE trigger: Moving SL DOWN {current_sl} -> {be_stop_price}.")
            else:
                lg.debug("BE Triggered, but current SL already better.")
                return False  # No action needed

            if update_needed:
                lg.warning(
                    f"{NEON_PURPLE}*** Moving SL to Break-Even ({symbol}) at {be_stop_price} ***{RESET}"
                )
                existing_tp = active_protection_state.get(
                    "takeProfitPrice"
                ...
                if success:
                    lg.info(f"{NEON_GREEN}BE SL set successfully.{RESET}")
                else:
                    lg.error(f"{NEON_RED}Failed to set BE SL.{RESET}")
                return success  # Return result of setting protection
        else:
            lg.debug("BE trigger condition not met.")
            return False  # Condition not met
    except Exception as e:
        lg.error(f"Error during BE check/set ({symbol}): {e}", exc_info=True)
        return False
...
def print_status_panel(  # Uses tabulate for better formatting
    ...
    price_str = (
        f"{price:.{price_prec}f}" if isinstance(price, Decimal) else f"{Fore.YELLOW}N/A"
    )
    atr = indicators.get("ATR")
    atr_str = (
        f"{atr:.{price_prec + 1}f}" if isinstance(atr, Decimal) else f"{Fore.YELLOW}N/A"
    )
    trend_ema_key = f"EMA_{CONFIG.indicator_trend_ema}"  # Construct key
    trend_ema = indicators.get(trend_ema_key)
    trend_ema_str = (
        f"{trend_ema:.{price_prec}f}"
        if isinstance(trend_ema, Decimal)
    ...
    if isinstance(price, Decimal) and isinstance(trend_ema, Decimal):
        if price > trend_ema:
            price_color, trend_desc = Fore.GREEN, f"{Fore.GREEN}(Above Trend)"
        elif price < trend_ema:
            price_color, trend_desc = Fore.RED, f"{Fore.RED}(Below Trend)"
        else:
            price_color, trend_desc = Fore.YELLOW, f"{Fore.YELLOW}(At Trend)"

    stoch_k = indicators.get("StochRSI_K")
    stoch_d = indicators.get("StochRSI_D")  # Floats or None
    ...
    if pd.notna(stoch_k):
        if stoch_k < 25:
            stoch_color, stoch_desc = Fore.GREEN, f"{Fore.GREEN}Oversold"
        elif stoch_k > 75:
            stoch_color, stoch_desc = Fore.RED, f"{Fore.RED}Overbought"
        else:
            stoch_color, stoch_desc = Fore.YELLOW, f"{Fore.YELLOW}Neutral"

    [
        [
            section_color + "Market",
            value_color + market_info.get("symbol", "N/A"),
            f"{price_color}{price_str}",
        ],
        [
            section_color + f"Trend EMA ({CONFIG.indicator_trend_ema})",
            f"{value_color}{trend_ema_str}",
            trend_desc,
        ],
        [
            section_color + f"ATR ({CONFIG.indicator_atr_period})",
            f"{value_color}{atr_str}",
            "",
        ],
        [
            section_color + "Stoch K/D",
    ...
    def fmt_prot(val, prefix="") -> str:
        return f"{prefix}{val:.{price_prec}f}" if val else f"{Fore.RED}NONE{reset}"

    tsl_status = (
        f"{Fore.GREEN}ACTIVE{reset} (Dist:{fmt_prot(tsl_dist, '')}, Act:{fmt_prot(tsl_act)})"
        if tsl_dist
        else f"{Fore.YELLOW}INACTIVE{reset}"
    )

    pos_size_str = (
        f"{pos_size:.8f}".rstrip("0").rstrip(".") if pos_size != Decimal(0) else "0"
    )
    ...
    pos_pnl_str = (
        f"{pnl_color}{pos_pnl:+.{price_prec}f}{value_color}"
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    if not EXCHANGE or not MARKET_INFO:
        logger.critical("Exchange/Market Info missing!")
        return
    ...
    if df is None or df.empty or len(df) < 50:
        logger.error("Market data failed. Skip cycle.")
        return
    ...
    position_snapshot = (
        current_position.copy() if current_position else None
    )  # Snapshot for panel
    ...
    if current_position:
        ...
        # a. Check Exit Signal
        if (pos_side == "long" and signal_action == "SELL") or (
            pos_side == "short" and signal_action == "BUY"
        ):
            logger.warning(
                f"*** EXIT Signal ({signal_action}) for {pos_side} position. Closing... ***"
            )
            if pos_size:
                place_trade(
                    EXCHANGE,
        ...
        if CONFIG.time_based_exit_minutes and current_position.get("timestamp_ms"):
            elapsed_mins = (
                time.time() * 1000 - current_position["timestamp_ms"]
            ) / 60000
            if elapsed_mins >= CONFIG.time_based_exit_minutes:
                logger.warning(
                    f"*** TIME EXIT triggered ({elapsed_mins:.1f} >= {CONFIG.time_based_exit_minutes}m). Closing... ***"
                )
                if pos_size:
                    place_trade(
                        EXCHANGE,
        ...
        if not active_protection_state.get(
            "trailingStopValue"
        ):  # Only check BE if TSL inactive
            set_breakeven_stop_loss(
                EXCHANGE,
                symbol,
                MARKET_INFO,
                current_position,
        ...
        set_trailing_stop_loss(
            ...
            analyzer,
            logger,
            active_protection_state.get("takeProfitPrice"),
        )

    elif signal_action in ["BUY", "SELL"]:  # No Position & Entry Signal
        logger.info(
            f"*** {signal_action} Signal & No Position: Initiating Entry for {symbol} ***"
        )
        if current_equity is None or current_equity <= 0:
            logger.error("Cannot enter: Invalid equity.")
            return
        if not isinstance(current_atr, Decimal) or current_atr <= 0:
            logger.error("Cannot enter: Invalid ATR.")
            return

        # Set Leverage
        if MARKET_INFO.get("is_contract", False):
            if not set_leverage_ccxt(
                EXCHANGE, symbol, CONFIG.leverage, MARKET_INFO, logger
            ):
                logger.error("Entry aborted: Leverage set failed.")
                return

        # Calculate SL for sizing
        _, _, initial_sl_price = analyzer.calculate_entry_tp_sl(
            current_price, signal_action
        )
        if not initial_sl_price:
            logger.error("Entry aborted: Initial SL calc failed.")
            return
        ...
        if not pos_size:
            logger.error("Entry aborted: Sizing failed.")
            return

        # Place Entry Order
        entry_order = place_trade(
            EXCHANGE,
            symbol,
            signal_action,
            pos_size,
        ...
        if entry_order and entry_order.get("id"):
            ...
            logger.info(
                f"Entry order {order_id} placed. Waiting {CONFIG.order_check_delay_seconds}s for confirmation..."
            )
            ...
            if confirmed_position:
                entry_price_actual = (
                    confirmed_position.get("entryPrice") or current_price
                )  # Use actual or estimate
                logger.info(
                    f"Position Confirmed! Entry: ~{entry_price_actual:.{analyzer.get_price_precision()}f}"
                )
                _, tp_final, sl_final = analyzer.calculate_entry_tp_sl(
                    entry_price_actual, signal_action
                )

                protection_set = False
                if CONFIG.enable_trailing_stop:
                    logger.info(
                        f"Setting Initial Trailing Stop Loss (TP target: {tp_final})..."
                    )
                    ...
                    logger.info(
                        f"Setting Initial Fixed SL ({sl_final}) and TP ({tp_final})..."
                    )
                    if sl_final or tp_final:
                        ...
                        logger.warning(
                            "Initial SL/TP invalid, no fixed protection set."
                        )

                if protection_set:
                    logger.info(
                        f"{NEON_GREEN}=== ENTRY & INITIAL PROTECTION COMPLETE ({symbol} {signal_action}) ==="
                    )
                else:
                    logger.error(
                        f"{NEON_RED}=== ENTRY OK BUT PROTECTION FAILED! ({symbol} {signal_action}) ==="
                    )
                    termux_notify(
                        "PROTECTION FAILED!",
                        f"{symbol} entry OK, SL/TP/TSL set failed!",
                    )
            else:
                logger.error(
                    f"{NEON_RED}Entry order {order_id} placed, but FAILED TO CONFIRM position! Manual check needed!{RESET}"
                )
                termux_notify(
                    "CONFIRM FAILED!", f"{symbol} entry {order_id} status unknown!"
                )
        else:
            logger.error(
                f"{NEON_RED}=== ENTRY FAILED ({symbol} {signal_action}). Order placement issue. ==="
            )
    ...
    logger.info(
        f"---== Cycle {cycle_count} End ({symbol}, Duration: {cycle_end_time - start_time:.2f}s) ==---"
    )
...
if __name__ == "__main__":
    ...
    if not EXCHANGE:
        logger.critical("Exchange init failed. Exiting.")
        sys.exit(1)

    MARKET_INFO = get_market_info(CONFIG.symbol, logger)
    if not MARKET_INFO:
        logger.critical(f"Market info load failed for {CONFIG.symbol}. Exiting.")
        sys.exit(1)
    ...
    if CONFIG.enable_trading:
        ...
        if CONFIG.use_testnet:
            logger.warning(f"{Fore.YELLOW}Using SANDBOX (Testnet).{Style.RESET_ALL}")
        else:
            logger.warning(f"{Fore.RED}!!! USING REAL MONEY !!!{Style.RESET_ALL}")
            time.sleep(5)
    else:
        logger.info(
            f"{Fore.YELLOW}Trading is DISABLED. Analysis only.{Style.RESET_ALL}"
        )
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.critical(
                    f"!! UNHANDLED CYCLE {cycle} ERROR !!: {cycle_err}", exc_info=True
                )
                time.sleep(60)  # Pause after crash
            logger.info(
                f"Cycle {cycle} finished. Sleeping {CONFIG.loop_sleep_seconds}s..."
            )
            time.sleep(CONFIG.loop_sleep_seconds)
    except KeyboardInterrupt:
        logger.warning("\nCtrl+C detected! Shutting down...")
    except Exception as main_err:
        logger.critical(f"!! FATAL MAIN LOOP ERROR !!: {main_err}", exc_info=True)
    finally:
