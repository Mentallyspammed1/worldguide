    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if signal == "BUY":
                ...
                take_profit = take_profit_raw.quantize(
...
def fetch_balance(
    ...
    try:
        ...
        # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
            ...
            balance_list = balance_info["info"]["result"]["list"]
            for account in balance_list:
                # Check if this account is the relevant type (e.g., CONTRACT)
                # This helps avoid picking up balances from SPOT accounts if we're trading CONTRACT
                # Let's refine this based on which type succeeded earlier, or default to CONTRACT
                succeeded_acc_type = "CONTRACT"  # Default assumption if type-specific fetch failed earlier
                if balance_info.get("params", {}).get("type"):
                    succeeded_acc_type = balance_info["params"]["type"]

                if account.get("accountType") == succeeded_acc_type:
                    coin_list = account.get("coin")
                    if isinstance(coin_list, list):
                        for coin_data in coin_list:
...
def get_open_position(
    ...
    try:
        ...
        if active_position:
            ...
            is_tsl_active_log = (
                Decimal(str(active_position.get("trailingStopLoss", "0"))) > 0
            )
