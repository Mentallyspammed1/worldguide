...
MIN_POSITION_SIZE_DIFF_PCT = Decimal(
    "0.05"
)  # Max allowed difference (5%) between ordered and confirmed size (Used in main loop logic)
...
def load_config(filepath: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    if "trailing_stop_callback_rate" in default_config:
        ...
        lg.debug(
            "Removed 'trailing_stop_callback_rate' from default config structure (incompatible with Bybit V5 API used)."
        )
    if "trailing_stop_activation_percentage" in default_config:
        del default_config["trailing_stop_activation_percentage"]
        lg.debug(
            "Removed 'trailing_stop_activation_percentage' from default config structure (incompatible with Bybit V5 API used)."
        )
    ...
    try:
        ...
        if loaded_config != initial_state_to_compare:
            ...
            try:
                ...
                TypeError
            ):  # If direct comparison failed and json fails, assume change
                config_changed_needs_save = True
                lg.info(
                    "Configuration values likely modified during validation/conversion (comparison check inconclusive)."
                )
            ...
            with open(filepath, encoding="utf-8") as f:
                ...
                for symbol, symbol_data_raw in state_raw.items():
                    if symbol == "global":  # Handle global state entries if needed
                        state[symbol] = (
                            symbol_data_raw  # Copy global state directly for now
                        )
                        continue
                    ...
                    if isinstance(symbol_data_raw, dict):
                        ...
                        for key in expected_symbol_keys:
                            ...
                            if key in [
                                ...
                                try:
                                    ...
                                    if dec_val.is_finite():
                                        ...
                                        lg.warning(
                                            f"State for '{symbol}': Found non-finite Decimal string '{raw_value}' for key '{key}'. Setting to None."
                                        )
                                except (InvalidOperation, ValueError):
                                    lg.warning(
                                        f"State for '{symbol}': Found invalid Decimal string '{raw_value}' for key '{key}'. Setting to None."
                                    )
                            # Handle boolean conversion (if needed, though JSON should handle bools)
                                ...
                                if isinstance(raw_value, bool):
                                    ...
                                    processed_value = (
                                        False  # Default if missing or wrong type
                                    )
                            # Handle strings (like position_side)
                            elif key == "position_side":
                                if isinstance(raw_value, str) and raw_value in [
                                    "long",
                                    "short",
                                ]:
                                    processed_value = raw_value
                                    ...
                                    processed_value = (
                                        None  # Default if missing or invalid
                                    )
                            else:
                            ...
                            if processed_value is not None:
                                ...
                                if key == "break_even_triggered":
                                    ...
                                    symbol_data[key] = None  # Default for price/size
                        ...
                        state[symbol] = {
                            k: (False if k == "break_even_triggered" else None)
                            for k in expected_symbol_keys
                        }
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict]) -> Optional[float]:
        ...
        try:
            ...
            if invalid_level_found:
                self.logger.warning(
                    f"Skipped invalid level(s) in OBI calculation for {self.symbol}."
                )
    ...
    try:
        ...
        if result and isinstance(result, dict):
            ...
            if ret_code == 0:
                ...
                lg.error(
                    f"{NEON_RED}Failed to set protection for {symbol} (Idx: {position_idx}). Code={ret_code}, Msg='{ret_msg}'.{RESET}"
                )
                lg.debug(f"Failed Protection Result: {result}")
                # Provide hints for common errors
                if ret_code == 110041 or ret_code == 170140:
                    lg.warning(
                        f"Hint: Failed because no active position found (Code {ret_code})."
                    )
                elif ret_code == 110042 or ret_code == 170131:
                    lg.warning(
                        f"Hint: Failed because TP/SL price is invalid (Code {ret_code}). Check distance from entry/liquidation."
                    )
                elif ret_code == 170132:
                    lg.warning(
                        f"Hint: Failed because TP/SL would trigger immediate liquidation (Code {ret_code})."
                    )
                elif ret_code == 110025:
                    lg.warning(
                        f"Hint: Failed due to positionIdx mismatch (Code {ret_code}). Ensure Hedge Mode params are correct."
                    )
                return False
        else:
            # safe_ccxt_call returned None or unexpected type
            lg.error(
                f"Protection API call failed or returned unexpected result type for {symbol}. Result: {result}"
            )
            return False
...
if __name__ == "__main__":
    ...
    if args.debug:
        print(
            f"{NEON_YELLOW}Debug mode enabled. Console log level set to DEBUG.{RESET}"
        )
    ...
    try:
        ...
        while True:
            ...
            for symbol in config.get("symbols", []):
                ...
                if not market_info:
                    symbol_logger.warning(
                        f"Skipping {symbol} due to missing market info"
                    )
                    continue
                ...
                async def fetch_symbol_data():
                    ...
                    kline, price, orderbook = await asyncio.gather(
                        kline_task, price_task, orderbook_task
                    )
                ...
                signal = analyzer.generate_trading_signal(
                    kline_data, price_data, orderbook
                )
                if not signal:
            ...
            main_logger.info(
                f"Cycle finished in {cycle_duration:.2f}s. Sleeping for {sleep_time:.2f}s..."
            )
