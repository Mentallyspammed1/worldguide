    ...
    try:
        ...
        if market_info.get("inverse", False):
            ...
            risk_per_contract = abs(
                contract_size * (Decimal("1") / entry_price - Decimal("1") / initial_stop_loss_price)
            )
...
def analyze_and_trade_symbol(
    ...
    current_total_open_positions = len(
        [
            ...
            if p.get("contractsDecimal") and abs(p["contractsDecimal"]) > Decimal("1e-9")
        ]
    )
            ...
            try:
                ...
                for category in categories_to_check:
                    try:
                        ...
                        if (
                            "account is not supported" in str(e_cat).lower() or getattr(e_cat, "code", None) == 170001
