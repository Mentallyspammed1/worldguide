...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        try:
            ...
            for key in self.ta_column_names:
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        self.indicator_values["close"] = (
            float(latest_data["close"]) if pd.notna(latest_data["close"]) else None
        )
        self.indicator_values["high"] = (
            float(latest_data["high"]) if pd.notna(latest_data["high"]) else None
        )
        self.indicator_values["low"] = (
            float(latest_data["low"]) if pd.notna(latest_data["low"]) else None
        )
        self.indicator_values["open"] = (
            float(latest_data["open"]) if pd.notna(latest_data["open"]) else None
        )
        self.indicator_values["volume"] = (
            float(latest_data["volume"]) if pd.notna(latest_data["volume"]) else None
        )
    ...
    def get_price_precision(self) -> int:
        ...
        try:
            precision = int(
                self.market_info.get("precision", {}).get("price", 8)
            ...
            self.logger.warning(
                f"Could not determine price precision for {self.symbol}. Using default 8."
            )
            return 8
    ...
    def get_min_tick_size(self) -> Decimal | None:
        ...
        try:
            tick_size_str = str(
                self.market_info.get("precision", {}).get("price", "0.00000001")
            ...
            if tick_size > 0:
                ...
                self.logger.warning(
                    f"Parsed tick size is not positive ({tick_size_str} -> {tick_size}) for {self.symbol}. Returning None."
                )
                return None
        except (InvalidOperation, ValueError, TypeError, KeyError, AttributeError) as e:
            self.logger.error(
                f"Error determining min tick size for {self.symbol}: {e}. Market info: {self.market_info.get('precision')}"
            )
            return None
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        if volume is None or vol_ma is None or vol_ma == 0:
            return 0.0
        return (
            1.0 if volume > vol_ma * multiplier else 0.0
    ...
    def _check_stoch_rsi(self) -> float:
        ...
        if k > d and k < oversold:
            return 1.0  # Bullish divergence / potential reversal up
        if k < d and k > overbought:
            return -1.0  # Bearish divergence / potential reversal down
        if k > d and k > d + 5:
    ...
    def _check_rsi(self) -> float:
        ...
        if rsi is None:
            return 0.0
        if rsi < 30:
            return 0.5  # Oversold, potential buy
        if rsi > 70:
            return -0.5  # Overbought, potential sell
        return 0.0
    ...
    def _check_bollinger_bands(self, current_price: Decimal) -> float:
        ...
        if bbl is None or bbu is None or current_price is None:
            return 0.0
        price_f = float(current_price)
        if price_f < bbl:
            return 1.0  # Below lower band, potential bounce (buy)
        if price_f > bbu:
            return -1.0  # Above upper band, potential pullback (sell)
        return 0.0
    ...
    def _check_psar(self, current_price: Decimal) -> float:
        ...
        psar_long = self.indicator_values.get(
            "PSARl"
        )  # Appears when trend is potentially UP
        psar_short = self.indicator_values.get(
            "PSARs"
        )  # Appears when trend is potentially DOWN
        ...
        if psar_long is not None and not np.isnan(psar_long):
            if price_f > psar_long:
                return 1.0  # Price above PSAR dot -> Bullish signal
            else:
                return -0.5  # Price crossed below PSAR dot -> Potential reversal signal
        ...
        # If PSAR Short has a value, it's above the price (potential downtrend)
            ...
            if price_f < psar_short:
                return -1.0  # Price below PSAR dot -> Bearish signal
            else:
                return 0.5  # Price crossed above PSAR dot -> Potential reversal signal
    ...
    def generate_trading_signal(
        ...
        final_score_to_use = (
            total_score  # Using non-normalized score based on original logic
        )
            ...
            if (
                ...
                for item in balance_list:
                    if item.get("coin") == currency_code:
                        ...
                        balance_str = item.get(
                            "walletBalance", item.get("availableToWithdraw")
                        )
                        if balance_str:
                            try:
...
def get_market_info(
    ...
    try:
        ...
        if (
            ...
            or time.time() - exchange.last_load_markets_timestamp > 3600
        ):  # Reload every hour
...
def get_price_precision_digits_from_market(
    ...
    try:
        ...
        if price_prec is None:
            ...
            if tick_size_str:
                ...
                if "." in tick_size_str:
                    ...
                    try:
                        return int(tick_size_str.split("e-")[-1])
                    except:
                        pass
                elif Decimal(tick_size_str) == 1:
                    return 0  # Tick size 1 means 0 decimal places
            # Fallback if tick size not found or unusable
            logger.warning(
                f"Could not find price precision digits directly for {market.get('symbol')}. Trying tick size inference."
            )
        ...
        if price_prec_dec < 1 and price_prec_dec > 0:  # Likely a tick size
        ...
        elif price_prec_dec >= 1 and "." not in str(
            price_prec
            ...
            logger.warning(
                f"Unclear format for price precision '{price_prec}' for {market.get('symbol')}. Attempting inference."
            )
            min_tick = get_min_tick_size_from_market(market, logger)
            if min_tick is not None and min_tick > 0:
                return abs(min_tick.normalize().as_tuple().exponent)
...
def get_amount_precision_digits_from_market(
    ...
    try:
        ...
        if amount_prec is None:
            ...
            if step_size_str:
                ...
                if step_size > 0:
                    ...
                    return 0  # Default if step size is invalid
            else:
                # Fallback for spot?
                ...
                if step_size_str:
                    try:
                        ...
                        pass
                logger.warning(
                    f"Could not find amount precision/step size for {market.get('symbol')}. Using default 3."
                )
        ...
        elif amount_prec_dec >= 1 and "." not in str(
            amount_prec
        ):  # Likely integer digits
            return int(amount_prec_dec)
        elif (
            amount_prec_dec == 0
        ):  # Precision 0 usually means integer amount (0 digits)
            return 0
        else:  # Unclear format
            logger.warning(
                f"Unclear format for amount precision '{amount_prec}' for {market.get('symbol')}. Using default 3."
            )
            return 3  # Default
    ...
    try:
        ...
        if min_amount_raw is not None:
            ...
            if min_amount > 0:
                ...
                logger.debug(
                    f"Minimum order amount is zero or negative ({min_amount_raw}) for {market.get('symbol')}. Returning small default."
                )
    ...
    try:
        ...
        if min_cost_raw is not None:
            ...
            if min_cost > 0:
                ...
                logger.debug(
                    f"Minimum order cost is zero or negative ({min_cost_raw}) for {market.get('symbol')}. Returning None."
                )
...
def calculate_position_size(
    ...
    try:
        ...
        amount_precision_digits = market_info.get(
            "amount_precision_digits", 8
        )  # Use derived value
        min_order_amount = market_info.get(
            "min_order_amount", Decimal("1e-8")
        )  # Use derived value
        min_order_cost = market_info.get(
            "min_order_cost"
        )  # Use derived value (can be None)
        min_amount_step = (
            Decimal("1") / (Decimal("10") ** amount_precision_digits)
        ...
        if market_info.get("inverse", False):
            ...
            lg.warning(
                f"Position sizing for INVERSE contracts is simplified. Accuracy may vary for {symbol}."
            )
            ...
            if risk_per_contract <= 0:
                lg.error(
                    f"Cannot calculate position size for {symbol} (Inverse): Invalid risk per contract ({risk_per_contract})."
                )
                return None
            ...
            if risk_per_unit <= 0:
                lg.error(
                    f"Cannot calculate position size for {symbol} (Linear/Spot): Invalid risk per unit ({risk_per_unit})."
                )
                return None
        ...
        lg.debug(
            f"  ContractSize={contract_size}, RiskPerUnit/Contract={risk_per_unit if not market_info.get('inverse') else risk_per_contract:.8f}"
        )
...
def _manual_amount_rounding(
    ...
    try:
        ...
        if amount_precision_digits is None:
            lg.warning(
                f"Amount precision digits not found for {symbol}, cannot perform manual rounding. Returning original."
            )
            return amount
...
def get_open_position(
    ...
    try:
        ...
        if open_position:
            ...
            try:
                entry_price_str = enhanced_pos.get(
                    "entryPrice", info_dict.get("avgPrice", "0")
                )
                enhanced_pos["entryPriceDecimal"] = (
                    Decimal(str(entry_price_str)) if entry_price_str else None
                )
            except (InvalidOperation, ValueError, TypeError):
                lg.warning(
            ...
            try:
                contracts_str = enhanced_pos.get(
                    "contracts", info_dict.get("size", "0")
                )
                ...
                if enhanced_pos["side"] == "long" and size_dec < 0:
                    size_dec = abs(size_dec)
                elif enhanced_pos["side"] == "short" and size_dec > 0:
                    size_dec = -abs(size_dec)
                elif enhanced_pos["side"] == "short" and size_dec == 0:
                    size_dec = Decimal(0)  # Ensure zero if side is short but size is 0
            ...
            try:
                sl_price_str = enhanced_pos.get(
                    "stopLossPrice", info_dict.get("stopLoss")
                )
                # Bybit V5 info might have 'stopLoss' as "" when inactive
                if (
                    sl_price_str
                    and str(sl_price_str) != "0"
                    and str(sl_price_str) != ""
                ):
            ...
            try:
                tp_price_str = enhanced_pos.get(
                    "takeProfitPrice", info_dict.get("takeProfit")
                )
                if (
                    tp_price_str
                    and str(tp_price_str) != "0"
                    and str(tp_price_str) != ""
                ):
            ...
            if (
                ...
                lg.info(
                    f"Confirmed open {enhanced_pos['side']} position for {symbol}. Size: {enhanced_pos['contractsDecimal']}"
                )
                return enhanced_pos
            else:
                lg.debug(
...
def place_trade(
    ...
    try:
        ...
        try:
            ...
            lg.warning(
                f"exchange.amount_to_precision failed ({fmt_err}). Using manual rounding for {symbol}."
            )
        ...
        try:
            if order_type.lower() == "market":
                order = exchange.create_market_order(
                    symbol, side, amount_final, params=params
                )
            ...
            if bybit_code == 110043 or (
                ...
                lg.error(
                    f"{NEON_RED}{order_tag} Order FAILED ({symbol} {side}): Invalid Order - Reduce-only would increase position. Check position state. Details: {e}{RESET}"
                )
            elif (
                "min order value" in err_str
                or "order cost" in err_str
                or bybit_code == 110014
            ):
                lg.error(
                    f"{NEON_RED}{order_tag} Order FAILED ({symbol} {side}): Invalid Order - Minimum order cost not met. Check size/price. Details: {e}{RESET}"
                )
            elif (
                "order quantity" in err_str or "size" in err_str or bybit_code == 110007
            ):
                lg.error(
                    f"{NEON_RED}{order_tag} Order FAILED ({symbol} {side}): Invalid Order - Quantity issue (too small, step size?). Check size. Details: {e}{RESET}"
                )
            else:
                lg.error(
                    f"{NEON_RED}{order_tag} Order FAILED ({symbol} {side}): Invalid Order. Details: {e} (Code: {bybit_code}){RESET}"
                )
        except ccxt.RateLimitExceeded as e:
            lg.error(
...
def _set_position_protection(
    ...
    category = _determine_category(market_info)
    position_idx = int(
        position_info.get("info", {}).get("positionIdx", 0)
    ...
    try:
        ...
        if stop_loss_price is not None and stop_loss_price > 0:
            ...
            lg.warning(
                f"Invalid SL price ({stop_loss_price}) provided for {symbol}. Skipping SL setting."
            )
        else:
        ...
        if take_profit_price is not None and take_profit_price > 0:
            ...
            lg.warning(
                f"Invalid TP price ({take_profit_price}) provided for {symbol}. Skipping TP setting."
            )
        else:
            ...
            if (
                ...
                if tsl_activation_price is not None:
                    if (
                        ...
                        params["activePrice"] = price_format_str.format(
                            tsl_activation_price
                        )
                        log_parts.append(f"TSL_ActAt={params['activePrice']}")
                    else:
                        lg.warning(
                            f"Invalid TSL activation price ({tsl_activation_price}) provided for {symbol}. Using default (immediate)."
                        )
                        params["activePrice"] = "0"
                else:
                    params["activePrice"] = (
                        "0"  # Default to immediate if distance is set
                    )
                ...
                lg.warning(
                    f"Invalid TSL distance ({trailing_stop_distance}) provided for {symbol}. Skipping TSL setting."
                )

        # Only call API if there's something to set/modify
        if "stopLoss" in params or "takeProfit" in params or "trailingStop" in params:
            lg.info(
                f"{' '.join(log_parts)} for {symbol} ({category}, Idx={position_idx}). Params: {params}"
            )
            ...
            if hasattr(
            ...
            ):  # Check for a likely related V5 method name
                ...
                try:
                    ...
                    lg.debug(
                        "No specific modify_position/set_sl_tp method found, using privatePostPositionTradingStop."
                    )
            ...
            if response and response.get("retCode") == 0:
                ...
                if ret_code == 110026:
                    lg.error(
                        f"{NEON_RED}>> Error suggests SL/TP price is invalid (e.g., wrong side of current price, too close). Check calculations.{RESET}"
                    )
                elif ret_code == 110042:
                    lg.error(
                        f"{NEON_RED}>> Error suggests Trailing Stop parameters (distance/activation) are invalid. Check calculations.{RESET}"
                    )
                return False
        else:
            lg.info(
                f"No protection parameters provided to set for {symbol}. Skipping API call."
            )
...
def analyze_and_trade_symbol(
    ...
    kline_limit = max(
        250, required_kline_history
    )  # Fetch reasonable minimum or required amount
    ...
    if klines_df.empty or len(klines_df) < max(
        50, required_kline_history // 2
    ):  # Relax requirement slightly
        lg.error(
            f"{NEON_RED}Failed to fetch sufficient kline data for {symbol} (fetched {len(klines_df)}, needed >{max(50, required_kline_history // 2)}). Skipping analysis.{RESET}"
        )
        return
    ...
    open_position_raw = all_open_positions.get(
        symbol
    )  # Get raw position for this symbol
    open_position = None
    if open_position_raw:
        ...
        if (
            ...
            and pos_size_dec is not None
            and abs(pos_size_dec) > Decimal("1e-9")
            ...
            lg.debug(
                f"Position data found for {symbol} in global fetch but failed validation. Treating as closed."
            )
    ...
    current_total_open_positions = len(
        [
            p
            for p in all_open_positions.values()
            if p.get("contractsDecimal")
            and abs(p["contractsDecimal"]) > Decimal("1e-9")
        ]
    )
                ...
                if confirmed_position:
                    ...
                    try:
                        ...
                        pos_size_actual = confirmed_position.get(
                            "contractsDecimal"
                        )  # Use Decimal version (should match side)
                        valid_entry = False
                        ...
                        if (
                            ...
                            lg.error(
                                f"Confirmed position has invalid entry price ({entry_price_actual}) or size ({pos_size_actual})."
                            )
        ...
        if not exit_signal_triggered and config.get("enable_ma_cross_exit", False):
            ...
            if pd.notna(ema_short) and pd.notna(ema_long):
                ...
                if pos_side == "long" and ema_short < ema_long:
                    ma_cross_exit = True
                    lg.warning(
                        f"{NEON_YELLOW}*** MA CROSS EXIT (Bearish): Short EMA ({ema_short:.{price_precision_digits}f}) crossed below Long EMA ({ema_long:.{price_precision_digits}f}). Closing LONG position. ***{RESET}"
                    )
                elif pos_side == "short" and ema_short > ema_long:
                    ma_cross_exit = True
                    lg.warning(
                        f"{NEON_YELLOW}*** MA CROSS EXIT (Bullish): Short EMA ({ema_short:.{price_precision_digits}f}) crossed above Long EMA ({ema_long:.{price_precision_digits}f}). Closing SHORT position. ***{RESET}"
                    )
            else:
            ...
            try:
                ...
                if close_order:
                    ...
                    if (
                        ...
                        lg.info(
                            f"{NEON_GREEN}Position for {symbol} confirmed already closed (or closing order unnecessary).{RESET}"
                        )
                ...
                try:
                    ...
                    if trigger_met:
                        if be_stop_price and be_stop_price > 0:
                            ...
                            lg.info(
                                f"  Current Price: {current_price:.{price_precision_digits}f}, Trigger Price: {trigger_price:.{price_precision_digits}f} (Profit >= {profit_target_offset:.{price_precision_digits}f})"
                            )
                            lg.info(
                                f"  Current SL: {sl_log_str}, Current TP: {tp_log_str}, TSL Active: {is_tsl_active}"
                            )
                            lg.info(
                                f"  Target Break-Even SL: {be_stop_price:.{price_precision_digits}f} (Entry {entry_price_dec:.{price_precision_digits}f} +/- {offset_ticks} ticks)"
                            )
                            ...
                            if is_tsl_active:
                                if force_fixed_sl:
                                    lg.info(
                                        "  TSL is active, but break_even_force_fixed_sl=True. Will replace TSL with fixed BE SL."
                                    )
                                    needs_sl_update = True
                                else:
                                    ...
                                    lg.warning(
                                        "BE triggered with TSL active and break_even_force_fixed_sl=False - Keeping TSL. (Verify Behavior!)"
                                    )
                                    needs_sl_update = False  # Keep TSL as per config
                                    # Mark BE as 'triggered' logically, even if SL isn't modified
                                    if (
                                        not analyzer.break_even_triggered
                                    ):  # Only log/set once
                            ...
                            if needs_sl_update and current_sl_price_dec:
                                if (
                                    ...
                                    lg.info(
                                        f"  Existing Fixed SL ({sl_log_str}) is already at or better than break-even target ({be_stop_price:.{price_precision_digits}f}). No SL modification needed."
                                    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Bybit Trading Bot (Whale 2.1 - Multi-Symbol, V5, BE)"
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    for symbol in symbols_to_trade:
        ...
        if market_info:
            ...
            if (
                market_info.get("min_tick_size") is None
                or market_info.get("min_order_amount") is None
            ):
                logger.error(
                    f"Market info for {symbol} is missing critical precision/limit data (tick size or min amount). Skipping symbol."
                )
            else:
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                for category in categories_to_check:
                    try:
                        ...
                        if (
                            "account is not supported" in str(e_cat).lower()
                            or getattr(e_cat, "code", None) == 170001
                        ):  # Bybit code for unified error on classic
                            main_logger.debug(
                ...
                if position_mode == "One-Way":
                    for pos_raw in raw_positions_list:
                        ...
                        if (
                            ...
                            and symbol not in symbols_with_positions
                        ):
                            ...
                            exchange.fetch_positions = (
                                lambda symbols=None, params=None: temp_pos_list
                            )
                            try:
                                ...
                                exchange.fetch_positions = (
                                    original_fetch  # Restore original method
                                )
                    ...
                    for symbol, symbol_raw_positions in raw_positions_by_symbol.items():
                        if symbol not in symbols_with_positions:
                            ...
                            original_fetch = exchange.fetch_positions
                            exchange.fetch_positions = (
                                lambda symbols=None, params=None: symbol_raw_positions
                            )
                            try:
                                enhanced_pos = get_open_position(
                                    exchange,
                ...
                main_logger.critical(
                    f"{NEON_RED}CRITICAL: Authentication Error fetching positions: {e_pos_auth}. Check API Key/Secret/Permissions. Bot cannot manage positions.{RESET}"
                )
            ...
            main_logger.critical(
                f"{NEON_RED}CRITICAL: Authentication Error during main loop: {e}. Bot stopped. Check API keys/permissions.{RESET}"
            )
            ...
            main_logger.warning(
                f"{NEON_YELLOW}Rate Limit Exceeded in main loop: {e}. CCXT should handle, but consider increasing loop delay if frequent. Waiting {wait_time:.1f}s.{RESET}"
            )
            time.sleep(wait_time)
            ...
            if bybit_code == 10016 or "system maintenance" in err_str:
                ...
                main_logger.error(
                    f"{NEON_RED}Parameter error from exchange (Code: {bybit_code}). Review API calls and parameters. Error: {e}{RESET}"
                )
