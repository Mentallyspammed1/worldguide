...
class TradingAnalyzer:
        ...
        try:
            ...
            if diff > 0:
                ...
                for level_pct in FIB_LEVELS:
                    ...
                    # Quantize the result to the market's price precision (using tick size)
    ...
    def get_price_precision(self) -> int:
        ...
        try:
            ...
            if price_precision_val is not None:
                # If it's numeric (float/int/str), assume it's tick size
                try:
                    ...
                    if tick_size > 0:
                        ...
                        # self.logger.debug(f"Derived price precision {precision} from tick size {tick_size} for {self.symbol}")
                        return precision
                except Exception:
                    self.logger.warning(
                        f"Could not parse precision.price '{price_precision_val}' as Decimal for {self.symbol}."
                    )

            # Fallback 1: Use limits.price.min (sometimes this represents tick size)
            ...
            if min_price_val is not None:
                try:
                    ...
                    if min_price_tick > 0:
                        ...
                        self.logger.debug(
                            f"Inferring price precision {precision} from limits.price.min ({min_price_tick}) for {self.symbol}."
                        )
                        return precision
                except Exception:
                    self.logger.warning(
            ...
            if last_close and pd.notna(last_close) and last_close > 0:
                try:
                    ...
                    self.logger.warning(
                        f"Error inferring precision from close price {last_close}: {e}"
                    )
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        # Fallback: Use precision decimal places to calculate tick size (less accurate but better than nothing)
        ...
        if (
            ...
            if entry_price > 0:
                risk_per_contract_quote = (
                    sl_distance_per_unit * contract_value_quote / entry_price
                )
    ...
    try:
        ...
        for pos in positions:
            ...
            try:
                ...
                min_contract_size = Decimal("1e-9")  # Default threshold
                if market:
                    try:
                        min_amt = market.get("limits", {}).get("amount", {}).get("min")
                        if min_amt is not None:
                            min_contract_size = max(
                                min_contract_size,
                ...
                if abs(position_size) > min_contract_size:
        ...
        if active_position:
            ...
            if side not in ["long", "short"]:
                ...
                # If still not determined, use size sign
                    ...
                    # If size_decimal is negative here, it likely came from 'info' or another field.
                    # We should ensure 'contracts' field holds the absolute value if we modify it.
                    # active_position['contracts'] = abs(size_decimal) # Optional: Standardize 'contracts' field
                else:
            ...
            try:
                active_position["trailingStopLossDistanceDecimal"] = (
                    Decimal(tsl_dist_str) if tsl_dist_str else Decimal("0")
                )
            except Exception:
                active_position["trailingStopLossDistanceDecimal"] = Decimal("0")
            try:
                active_position["tslActivationPriceDecimal"] = (
                    Decimal(tsl_act_str) if tsl_act_str else Decimal("0")
                )
            except Exception:
            ...
            def format_log_value(
                ...
                try:
                    ...
                    if d_value > 0:
                        ...
                        return "0.0"  # Show inactive TSL as 0.0
                    elif d_value == 0:
                        return "N/A"  # Treat 0 as N/A for SL/TP/Entry etc. unless TSL field
                    else:  # Negative values
                        # For PnL, format negative. For others, treat as invalid?
                        if "pnl" in key.lower():
                            return f"{d_value:.{precision}f}"
                        else:
                            return "Invalid (<0)"
            ...
            amount_precision = (
                market_for_log["precision"]["amount"] if market_for_log else 8
            )
            if isinstance(
                amount_precision, float
            ):  # If precision is step size, calculate decimals
                try:
                    amount_precision = abs(
                        Decimal(str(amount_precision)).normalize().as_tuple().exponent
                    )
                except Exception:
                    amount_precision = 8
...
def analyze_and_trade_symbol(
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        sl_log_str = f"{current_sl_str}" if current_sl_str else "N/A"
        tp_log_str = f"{current_tp_str}" if current_tp_str else "N/A"
        lg.info(
    ...
    try:
        while True:
            ...
            # --- Handle Specific Errors within the loop ---
                ...
                wait_time_str = (
                    str(e).split("try again in")[-1].split("seconds")[0].strip()
                )
                try:
                    wait_time = max(1, int(wait_time_str))  # Ensure positive wait time
                except ValueError:
                    wait_time = 60  # Default wait if parsing fails
...
if __name__ == "__main__":
    ...
    # to a file named 'livexy.py'. This is highly unusual for a production script
    # and generally bad practice. It assumes the script is run directly using
    # `python livexy.py` and may cause issues if imported or run differently.
    # It's kept here only because it was part of the original input structure.
    # Consider removing this block for deployment.
