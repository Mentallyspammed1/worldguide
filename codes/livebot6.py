            ...
            if price_precision_val is not None:
                ...
                try:
                    ...
                    if tick_size > 0:
                     ...
                     self.logger.warning(f"Could not parse precision.price '{price_precision_val}' as Decimal for {self.symbol}.")
            ...
            if min_price_val is not None:
                try:
                    ...
                    if min_price_tick > 0:
                        ...
                        self.logger.debug(f"Inferring price precision {precision} from limits.price.min ({min_price_tick}) for {self.symbol}.")
                        return precision
                except Exception:
                    self.logger.warning(f"Could not parse limits.price.min '{min_price_val}' as Decimal for {self.symbol}.")
        ...
        try:
            ...
            if price_precision_val is not None:
                try:
                    ...
                    self.logger.warning(f"Could not parse precision.price '{price_precision_val}' as Decimal for {self.symbol}. Trying other methods.")
            ...
            if amount_precision_val is not None:
                ...
                if isinstance(amount_precision_val, int):
                    ...
                    try:
                        ...
                        else: num_decimals = abs(step_size.normalize().as_tuple().exponent)  # Infer decimals from step
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> dict | None:
    ...
    try:
                ...
                if market:
                    try:
                        ...
                        if min_amt is not None:
                            min_contract_size = max(min_contract_size, Decimal(str(min_amt)) * Decimal('0.01'))  # Use a fraction of min size
        ...
        if active_position:
            ...
            try:
                 active_position['trailingStopLossDistanceDecimal'] = Decimal(tsl_dist_str) if tsl_dist_str else Decimal('0')
            except Exception:
            ...
            if isinstance(amount_precision, float):  # If precision is step size, calculate decimals
                 ...
                 except Exception: amount_precision = 8
