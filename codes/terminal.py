...
try:
    ...
    print(
        f"{Fore.YELLOW}Warning: 'pandas_ta' library not found. Technical Analysis features will be limited.{Style.RESET_ALL}"
    )
        ...
        if "indicator_periods" in config_dict and isinstance(config_dict["indicator_periods"], dict):
            ...
            for key, default_value in default_periods.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid value '{current_value}' for indicator period '{key}'. Using default: {default_value}"
                    )
                    config_dict["indicator_periods"][key] = default_value
        else:
            config_dict["indicator_periods"] = self.DEFAULT_CONFIG[
                "indicator_periods"
            ].copy()  # Use default if missing/invalid
    ...
    def _setup_theme_colors(self) -> Dict:
        ...
        if theme == "dark":
            ...
            return {
                ...
                "neutral": Fore.BLACK,
                "dim": Style.DIM,
                "reset": Style.RESET_ALL,  # Keep dim/reset standard
            }
...
class BybitFuturesCCXTClient:
        ...
        if self.credentials.testnet:
            ...
            if hasattr(self.exchange, "set_sandbox_mode") and callable(self.exchange.set_sandbox_mode):
                try:
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}Falling back to manually setting testnet API URL due to set_sandbox_mode error.{Style.RESET_ALL}"
                    )
                    self.exchange.urls["api"] = self.exchange.urls["test"]
            else:
                # If method doesn't exist, fallback to setting URL
                logger.warning(
                    f"{Fore.YELLOW}exchange.set_sandbox_mode() not available. Manually setting testnet API URL.{Style.RESET_ALL}"
                )
                self.exchange.urls["api"] = self.exchange.urls["test"]
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if self.exchange.markets:
                        ...
                        logger.warning(
                            f"{Fore.YELLOW}Market loading attempt {attempt + 1} returned no markets. Retrying...{Style.RESET_ALL}"
                        )
                        await asyncio.sleep(1.5**attempt)  # Exponential backoff
                except (ccxt.NetworkError, asyncio.TimeoutError) as net_err:
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"{Fore.YELLOW}Network error loading markets (Attempt {attempt + 1}/{max_retries}): {net_err}. Retrying...{Style.RESET_ALL}"
                        )
                        await asyncio.sleep(1.5**attempt)
                    else:
                        logger.error(
                            f"{Fore.RED}Failed to load markets after {max_retries} attempts due to network errors.{Style.RESET_ALL}"
                        )
            ...
            if not self.exchange.markets:
                ...
                usdt_swap_markets = {
                    ...
                    and v.get("active")  # Ensure market is active
                }
                if not usdt_swap_markets:
                    logger.warning(
                        f"{Fore.YELLOW}No ACTIVE USDT Perpetual Swap markets found after loading. Check API key permissions or market availability.{Style.RESET_ALL}"
                    )
                else:
                    logger.info(
                        f"{Fore.GREEN}Loaded {len(self.exchange.markets)} total markets, found {len(usdt_swap_markets)} active USDT Perpetual Swaps.{Style.RESET_ALL}"
                    )
            ...
            logger.error(
                f"{Fore.RED}CCXT Authentication Sigil Rejected: Invalid API keys or permissions. Check .env file and Bybit API settings. Ensure key has CONTRACT TRADE permissions. {e}{Style.RESET_ALL}"
            )
    ...
    async def fetch_balance(self) -> Dict:
        ...
        try:
                        ...
                        if isinstance(raw_list, list):
                            for item in raw_list:
                                if (
                                    ...
                                    and item.get("accountType") == "CONTRACT"
            ...
            logger.warning(
                f"{Fore.YELLOW}Standard 'USDT' balance structure not found. Attempting direct parse from 'info'.{Style.RESET_ALL}"
            )
            if (
                "info" in balance
                    ...
                    for asset_info in raw_list:
                        ...
                        if (
                            ...
                            parsed_balance = {
                                ...
                                ),  # Or 'availableBalance'? Check API docs. 'availableToWithdraw' seems safer.
                                "used": float(
                                    asset_info.get("usedMargin", 0)
                                ),  # This might be order margin + position margin
                                "total": float(
                            ...
                            logger.info(
                                f"{Fore.YELLOW}Successfully parsed V5 balance directly from info.{Style.RESET_ALL}"
                            )
    ...
    async def place_order(
        ...
        try:
            ...
            if order_type == "market" and price is not None:
                logger.warning(
                    f"{Fore.YELLOW}Price ({price}) provided for Market order, ignoring price.{Style.RESET_ALL}"
                )
                price = None
            ...
            logger.error(
                f"{Fore.RED}Order weaving failed: Insufficient magical essence (funds). Check available balance and margin requirements. {e}{Style.RESET_ALL}"
            )
            raise
            ...
            if "order quantity" in err_msg or "order qty" in err_msg or "size" in err_msg or "lot size" in err_msg:
                ...
                logger.error(
                    f"{Fore.RED}Order weaving failed: Invalid Trailing Stop parameters. Check percentage value (e.g., 0.1 to 10), activation price (if applicable), or if feature is enabled. {e}{Style.RESET_ALL}"
                )
            else:
                logger.error(
                    f"{Fore.RED}Order weaving failed: Flawed incantation (invalid order parameters). Review all order details. {e}{Style.RESET_ALL}"
                )
            raise
        except ccxt.BadSymbol as e:
            logger.error(
                f"{Fore.RED}Order weaving failed: Invalid symbol sigil '{symbol}'. Does it exist and is it active on Bybit USDT Perpetual market? {e}{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if not ohlcv:
                ...
                if symbol not in self.exchange.markets:
                    logger.error(
                        f"{Fore.RED}Symbol '{symbol}' not found in loaded markets for OHLCV fetch.{Style.RESET_ALL}"
                    )
                    raise ValueError(f"Invalid symbol for OHLCV: {symbol}")
                else:
                    logger.warning(
                        f"{Fore.YELLOW}No OHLCV data returned for {symbol} ({timeframe}). May be a new pair, inactive market, or requested range has no data.{Style.RESET_ALL}"
                    )
            ...
            if (
                "interval" in err_msg
                or "candle type" in err_msg
                or "invalid timeframe" in err_msg
                or "period" in err_msg
            ):
                logger.error(
                    f"{Fore.RED}Invalid or unsupported time-shard '{timeframe}' for {symbol}. Check available timeframes on Bybit. {e}{Style.RESET_ALL}"
                )
...
class TechnicalAnalysis:
    ...
    def __init__(self, config: Dict):
        ...
        self.indicator_periods = config.get(
            "indicator_periods", ConfigManager.DEFAULT_CONFIG["indicator_periods"]
        ).copy()
        if ta is None:
            logger.warning(
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(
                f"{Fore.RED}Invalid indicator period type in configuration: {e}. Using defaults.{Style.RESET_ALL}"
            )
            # Reset periods to defaults if conversion failed
            periods = ConfigManager.DEFAULT_CONFIG["indicator_periods"].copy()
            sma_short_len, sma_long_len, ema_len, rsi_len = (
                periods["sma_short"],
                periods["sma_long"],
                periods["ema"],
                periods["rsi"],
        ...
        try:
            ...
            if not added_cols:
                ...
                if len(df_out) < min_rows_needed:
                    logger.warning(
                        f"{Fore.YELLOW}Pandas TA strategy ran but added no new indicator columns. Insufficient data length ({len(df_out)} rows) for configured lookback periods (longest requires ~{min_rows_needed}).{Style.RESET_ALL}"
                    )
                else:
                    # If enough data, maybe some other issue
                    logger.warning(
                        f"{Fore.YELLOW}Pandas TA strategy ran but added no new indicator columns despite sufficient data ({len(df_out)} rows). Check indicator configurations or library behavior.{Style.RESET_ALL}"
                    )
            else:
                logger.debug(
            ...
            if "'DataFrame' object has no attribute 'ta'" in str(
                e
            ) or "module 'pandas' has no attribute 'DataFrame'" in str(e):
                logger.error(
                    f"{Fore.RED}Pandas TA extension not found or failed to attach. Is pandas_ta installed and imported correctly? Is pandas working?{Style.RESET_ALL}",
                    exc_info=True,
                )
            else:
    ...
    def calculate_pivot_points(self, df_period: pd.DataFrame) -> Optional[Dict[str, float]]:
        ...
        if len(df_period_copy) < 1:
            logger.warning(
                f"{Fore.YELLOW}Insufficient rows in pivot DataFrame (need at least 1 row for last completed period).{Style.RESET_ALL}"
            )
            return None
    ...
    def determine_trend(self, df: pd.DataFrame) -> str:
        ...
        if ta is None:
            return f"{colors['dim']}Unknown (pandas_ta missing){colors['reset']}"
        if df.empty:
            return f"{colors['dim']}Unknown (No Data){colors['reset']}"
        ...
        if not sma_short_col or not sma_long_col:
            ...
            missing_cols = []
            if not sma_short_col:
                missing_cols.append(f"SMA_{sma_short_len} (or sma_{sma_short_len})")
            if not sma_long_col:
                missing_cols.append(f"SMA_{sma_long_len} (or sma_{sma_long_len})")
            logger.warning(
                f"Required SMA columns for trend ({', '.join(missing_cols)}) not found in DataFrame. Columns available: {list(df.columns)}"
            )
        ...
        try:
            ...
            logger.error(
                f"KeyError accessing SMA columns '{sma_short_col}' or '{sma_long_col}' during trend calculation."
            )
        ...
        try:
            ...
            logger.error(
                f"TypeError during SMA comparison for trend: {e}. Check SMA column data types. Last values: {last_row.to_dict()}, Prev values: {prev_row.to_dict()}",
                exc_info=True,
            )
    ...
    def generate_ta_summary(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        try:
            rsi_len = int(self.indicator_periods.get("rsi", 14))
        except (ValueError, TypeError):
            rsi_len = 14
        ...
        if isinstance(hist, (float, int, np.number)):
            ...
            if isinstance(macd_line, (float, int, np.number)) and isinstance(signal, (float, int, np.number)):
                if macd_line > signal:
                    ...
                    macd_str += f" {colors['negative']}(M<S){colors['reset']}"
        ...
        if isinstance(k, (float, int, np.number)) and isinstance(d, (float, int, np.number)):
            ...
            # Check for crossover
                ...
                stoch_str = f"{colors['neutral']}{stoch_val} {colors['negative']}(K<D){colors['reset']}"
            else:
                stoch_str = f"{colors['neutral']}{stoch_val}{colors['reset']}"
        summary["Stochastic"] = stoch_str

        # ATR (Volatility - e.g., ATR_14 or atr_14)
        try:
            atr_len = int(self.indicator_periods.get("atr", 14))
        except (ValueError, TypeError):
            atr_len = 14
...
class TerminalUI:
        ...
        while True:
            try:
                ...
                if choice.isdigit() and 1 <= int(choice) <= len(options):
                    ...
                    print(
                        f"{self.colors['warning']}Warning: Invalid input. Please enter a number between 1 and {len(options)}.{self.colors['reset']}"
                    )
            except EOFError:
    ...
    def get_input(
        ...
        required: bool = True,
        input_type: type = str,
        validation_func: Optional[Callable[[Any], Union[bool, str]]] = None,
    ) -> Any:
        """
        ...
        while True:
            ...
            try:
                ...
                if input_type == float or input_type == int:
                    print(
                        f"{self.colors['error']}Error: Invalid input. Expected a number ({input_type.__name__}). Details: {e}{self.colors['reset']}"
                    )
                else:
                    print(
    ...
    def print_table(
        ...
        if (
            data is None
            or (isinstance(data, (list, dict)) and not data)
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                if contains_color:
                    logger.debug("Detected ANSI color codes in list of dicts.")
            ...
            if tabulate and table_data is not None and not contains_color:
                ...
                print(
                    f"{self.colors['neutral']}{tabulate(formatted_table_data, headers=headers, tablefmt=table_fmt, numalign='right', stralign='left', missingval='N/A')}{self.colors['reset']}"
                )
            ...
            # Use Pandas built-in string conversion if data is DataFrame (handles color better than tabulate)
                ...
                try:
                    ...
                    term_width = 120  # Wider default fallback width
                ...
                with pd.option_context(
                    ...
                    "display.width",
                    term_width,  # Use detected width
                    "display.expand_frame_repr",
                    True,  # Prevent line wrapping if possible
    ...
    def display_chart(self, data: List[float], title: str):
        ...
        try:
            ...
            color_map = {
                ...
                "91": asciichart.red,
                "92": asciichart.green,
                "93": asciichart.yellow,
                "94": asciichart.blue,
                "95": asciichart.magenta,
                "96": asciichart.cyan,
                "97": asciichart.white,
                # Handle black if used in a light theme
                "30": asciichart.default,  # Map black to default as asciichart might not have black
            }
            chart_color = color_map.get(
                chart_color_code_str, asciichart.default
            )  # Use default if code not found or invalid
...
class TradingTerminal:
    ...
    async def setup_credentials(self) -> bool:
        ...
        if key_invalid or secret_invalid:
            error_parts = []
            if key_invalid:
                error_parts.append("BYBIT_API_KEY")
            if secret_invalid:
            ...
            try:
                ...
                if shutdown_wait_task in done:
                    ...
                    if menu_task in pending:
                        ...
                        try:
                            await asyncio.wait_for(menu_task, timeout=0.5)
                ...
                if choice is None:
                    if not self._running:
                        break  # Break if shutdown flag set explicitly
                    logger.debug(
                        "Choice is None after wait, continuing loop (likely due to handled menu error or shutdown)."
                    )
    ...
    async def handle_action(self, action_coro: Callable[[], Any], action_name: str):
        ...
        # --- Specific CCXT Error Handling (Catch most specific first) ---
            ...
            logger.error(
                f"InvalidOrder during '{action_name}': {e}", exc_info=False
            )  # Log less verbose stack for common errors
            ...
            self.ui.print_warning(
                "Ensure symbol exists on Bybit USDT Perpetual market and is formatted correctly (e.g., BTC/USDT)."
            )
            await self.ui_wait_for_enter_async()
        ...
        if self.exchange_client and self.exchange_client._markets_loaded:
            ...
            if ccxt_symbol not in self.exchange_client.exchange.markets:
                ...
                usdt_swap_symbols = sorted(
                    [
                        s
                        for s, m in self.exchange_client.exchange.markets.items()
                        if isinstance(s, str)
                        and m.get("swap")
                        and m.get("linear")
                ...
                if not (
                    market_info.get("swap")
                    and market_info.get("linear")
                    and market_info.get("quote") == "USDT"
                    and market_info.get("active")
                ):
    ...
    def _get_ccxt_symbol(self, user_input: str) -> str:
        ...
        if (
            ...
            for market_symbol, market_data in markets.items():
                if market_data.get("id", "").upper() == symbol:
                    ...
                    if (
                        market_data.get("swap")
                        and market_data.get("linear")
                        and market_data.get("quote") == "USDT"
                        and market_data.get("active")
                    ):
                        logger.debug(
                            f"Normalized symbol '{symbol}' to '{market_symbol}' based on matching market ID (Active USDT Swap)."
                        )
                        return market_symbol
                    else:
                        logger.debug(
                            f"Symbol '{symbol}' matched market ID '{market_symbol}' but it's not an active USDT Perpetual Swap. Skipping ID match."
                        )
            ...
            if len(possible_matches) == 1:
                ...
                logger.debug(
                    f"Normalized symbol '{symbol}' to '{formatted}' based on concatenated BASEQUOTE (Active USDT Swap)."
                )
                return formatted
            elif len(possible_matches) > 1:
                logger.warning(
                    f"Ambiguous symbol '{symbol}'. Multiple active USDT swaps match concatenated form: {possible_matches}. Returning first match: {possible_matches[0]}"
                )
        ...
        if symbol.endswith("USDT") and len(symbol) > 4:
            ...
            logger.debug(
                f"Normalized symbol '{symbol}' to '{formatted}' (generic USDT guess - market check recommended)."
            )
            return formatted
        ...
        logger.warning(
            f"{Fore.YELLOW}Could not reliably normalize symbol '{symbol}' to BASE/USDT swap format using market data or patterns. Using as is.{Style.RESET_ALL}"
        )
        return symbol
    ...
    def _validate_positive_float(self, value: Any) -> Union[bool, str]:
        ...
        try:
            ...
            if not np.isfinite(num_value):
                return "Value must be a finite number (not infinity or NaN)."
            # Use a small tolerance for comparison > 0 if needed, but direct check is usually fine
            if num_value <= 0:
                return "Value must be positive (greater than 0)."
            return True
        except (ValueError, TypeError):
    ...
    def _validate_percentage(self, value: Any) -> Union[bool, str]:
        ...
        if result is not True:
            return result  # Return error message from positive float check
    ...
    def _validate_timeframe(self, timeframe: str) -> Union[bool, str]:
        ...
        if self.exchange_client and self.exchange_client._markets_loaded and self.exchange_client.exchange:
            ...
            if hasattr(self.exchange_client.exchange, "timeframes") and isinstance(
                ...
                logger.warning(
                    "Exchange timeframes data not available or not in expected format. Skipping timeframe support check."
                )
    ...
    async def place_order_menu(self):
        ...
        amount = await self.ui_get_input_async(
            "Enter quantity (in base currency, e.g., BTC amount)",
            input_type=float,
        ...
        sl_price = await self.ui_get_input_async(
            "Stop Loss trigger price (0 for none)",
            default="0",
            required=False,
            input_type=float,
        ...
        tp_price = await self.ui_get_input_async(
            "Take Profit trigger price (0 for none)",
            default="0",
            required=False,
            input_type=float,
            validation_func=self._validate_non_negative_float,
        ...
        trail_percent = await self.ui_get_input_async(
            "Trailing Stop percentage (e.g., 0.5 for 0.5%, min ~0.01, 0 for none)",
            default="0",
            required=False,
            input_type=float,
        ...
        if trail_percent is not None and trail_percent > 0:
            ...
            self.ui.print_info(
                f"Trailing Stop set at {trail_percent}%. Activation details depend on exchange defaults if not specified."
            )

        # Reduce Only
        # Use more direct boolean input helper if available or simple yes/no
        reduce_only_input = await self.ui_get_input_async(
            "Set as Reduce Only? (yes/no)", default="no", required=False, input_type=str
        )  # Keep as string for simple check
        ...
        if not price and order_type == "limit":
            confirm_parts.append(
                f"{self.ui.colors['error']}(ERROR: No price for Limit order!){self.ui.colors['reset']}"
            )  # Should not happen
        ...
        if "stopLossPrice" in params:
            display_keys.append("stopLossPrice")  # CCXT standard field name
        if "takeProfitPrice" in params:
            display_keys.append("takeProfitPrice")  # CCXT standard field name
        if "trailingPercent" in params:
            display_keys.append("trailingPercent")  # CCXT standard field name
        if "reduceOnly" in params:
            display_keys.append("reduceOnly")  # CCXT standard field name
        ...
        if "info" in result and isinstance(result["info"], dict):
            ...
            if (
                "Trailingpercent" not in display_result
                and info_data.get("trailingStop")
                and info_data["trailingStop"] != "0"
            ):
        ...
        if "Id" not in display_result:
            display_result["Id"] = result.get("id", "N/A")
        if "Status" not in display_result:
            display_result["Status"] = result.get("status", "N/A")
        if "Symbol" not in display_result:
            display_result["Symbol"] = result.get("symbol", "N/A")
        if "Side" not in display_result:
            display_result["Side"] = str(result.get("side", "N/A")).capitalize()
        if "Type" not in display_result:
            display_result["Type"] = str(result.get("type", "N/A")).capitalize()
        ...
        try:
            ...
            if unrealized_pnl is not None and display_data["Unrealized PNL"] == "N/A":
                display_data["Unrealized PNL"] = (
                    f"{self.ui.colors['error']}Error: {unrealized_pnl}{self.ui.colors['reset']}"
                )
            if realized_pnl is not None and display_data["Realized PNL"] == "N/A":
                display_data["Realized PNL"] = (
                    f"{self.ui.colors['error']}Error: {realized_pnl}{self.ui.colors['reset']}"
                )
    ...
    async def view_open_orders(self):
        ...
        if symbol_input is None and symbol_input != "":
            return  # Exit if cancelled/error, allow blank
        ...
        self.ui.print_info(
            f"{self.ui.colors['primary']}# Fetching active order scrolls {action_desc}...{self.ui.colors['reset']}"
        )
        ...
        if not open_orders:
            ...
            for order in open_orders:
                ...
                if trail_percent is not None and trail_percent > 0:
                    ...
                    if bybit_active_price:
                        trail_info_str += f" (Act: {bybit_active_price})"
    ...
    async def view_open_positions(self):
        ...
        action_desc = f"for {symbol}" if symbol else "for all symbols"
        self.ui.print_info(
            f"{self.ui.colors['primary']}# Fetching open position essences {action_desc}...{self.ui.colors['reset']}"
        )
        ...
        if not positions:
            ...
            for pos in positions:
                ...
                if (
                    ...
                    and not np.isclose(float(initial_margin), 0.0)
    ...
    async def check_price_menu(self):
        ...
        if bid is not None and ask is not None:
            try:
                display_data["Spread"] = abs(float(ask) - float(bid))
            except (ValueError, TypeError):
    ...
    async def technical_analysis_menu(self):
                ...
                if pivot_ohlcv and len(pivot_ohlcv) >= 1:
                    ...
                    if not df_pivot_raw.empty:
                        ...
                        if pivot_points:
                            ...
                            logger.info(
                                f"Calculated {pivot_period_str} pivots based on candle ending {candle_time_str}"
                            )
                        else:
                            # Calc function already logged NaN/other issues
                            logger.warning(
                                f"Pivot calculation returned None for {pivot_period_str}. Check HLC data for the period."
                            )
                    else:
                        logger.warning(
                            f"Could not use fetched data for {pivot_period_str} pivots after cleaning (e.g., NaN HLC)."
                        )
                else:
                    logger.warning(
                        f"Could not fetch sufficient data (need >= 1 completed candle) for {pivot_period_str} pivot points."
                    )
        ...
        ta_summary = self.ta_analyzer.generate_ta_summary(
            df_indicators if df_indicators is not df else df
        )  # Pass df with indicators if available
        self.ui.print_table(
        ...
        if "close" in df_to_use.columns:
            ...
            if close_prices:
                points_to_chart = min(
                    len(close_prices), chart_points
                )  # Use configured number of points or less if not enough data
    ...
    async def settings_menu(self):
        ...
        while settings_running and self._running:  # Check global running flag too
                ...
                if choice == "1":  # Change Theme
                    ...
                    if symbol_input:
                        ...
                        if validation_result is True:
                            ...
                            self.ui.print_error(
                                f"Validation failed for symbol '{normalized_symbol}': {validation_result}"
                            )
                    await asyncio.sleep(0.8)

                elif choice == "3":  # Set Default Timeframe
                    tf_input = await self.ui_get_input_async(
                        "Enter default timeframe (e.g., 1h, 1d, W)",
                    ...
                    if tf_input:
                        ...
                        tf_map = {"d": "1d", "w": "1w", "m": "1M"}
                        if new_tf_norm.upper() in tf_map:
                    ...
                    level_input = await self.ui_get_input_async(
                        ...
                        validation_func=lambda L: str(L).upper() in log_levels
                        or f"Must be one of: {', '.join(log_levels)}",
                    )
                    if level_input:
                    ...
                    if pivot_tf_input:
                        ...
                        tf_map = {"d": "1d", "w": "1w", "m": "1M"}
                        if new_pivot_tf_norm.upper() in tf_map:
            ...
            # Handle interruptions during input *within* the settings loop
                ...
                logger.warning(
                    f"Interruption '{type(e).__name__}' during settings modification. Exiting settings menu."
                )
    ...
    try:
        with open(env_path, "r", encoding="utf-8") as f:
            ...
            if "your_api_key_here" in content or "your_api_secret_here" in content:
                print(
                    f"{Fore.YELLOW}Warning: Found placeholder values ('your_api_key_here' or 'your_api_secret_here') in '{env_path}'.{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.YELLOW}>>> Please edit '{env_path}' and replace them with your actual Bybit API credentials before running authenticated actions. <<<"
                )
                if created_now:
                    # Give extra warning if just created
                    print(
                        f"{Fore.YELLOW}The terminal will likely run in limited mode until credentials are added.{Style.RESET_ALL}"
                    )
...
async def main():
    ...
    print(
        f"{Fore.RED}{Style.BRIGHT}Security Charm: Ensure your .env file (containing API keys) is listed in your .gitignore file to prevent accidental commits to public repositories!{Style.RESET_ALL}"
    )
    ...
    try:
        ...
        if terminal and not terminal._shutdown_event.is_set():
            logger.warning(
                "Main task cancelled but shutdown sequence may not have completed. Attempting final shutdown steps."
            )
...
if __name__ == "__main__":
    ...
    if sys.version_info < (3, 8):
        ...
        print(
            f"{Fore.YELLOW}Warning: This script is optimized for Python 3.8+ (uses features like named tasks). You are using {sys.version}. Proceeding, but some features might be limited or behave unexpectedly.{Style.RESET_ALL}",
            file=sys.stderr,
        )
