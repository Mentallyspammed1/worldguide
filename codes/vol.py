...
tion loaded and validated successfully.                         Traceback (most recent call last):                                File "/data/data/com.termux/files/home/worldguide/codes/vol.py", line 514, in <module>                                            retry=retry_if_exception_type(RETRYABLE_CCXT_EXCEPTIONS) | _should_retry_ccxt # Combine standard types and custom logic
          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^~~~~~~~~~~~~~~~~~~~                                                 File "/data/data/com.termux/files/usr/lib/python3.12/site-packages/tenacity/retry.py", line 39, in __or__
    return other.__ror__(self)                                             ^^^^^^^^^^^^^                                        AttributeError: 'function' object has no attribute '__ror__'. Did you mean: '__dir__'?
...
def _should_retry_ccxt(exception: BaseException) -> bool:
    """Determines if a CCXT exception is retryable. (Used by decorator)"""
    ...
    if isinstance(exception, ccxt.ExchangeError):
        ...
        if "server error" in error_str or "busy" in error_str or "try again later" in error_str or "request timeout" in error_str:
             base_logger.warning(f"Retryable ExchangeError encountered: {exception}. Retrying...")
             return True
    return False

# Define retry decorator using the custom function
ccxt_retry_decorator = retry(
    ...
    retry=retry_if_exception_type(RETRYABLE_CCXT_EXCEPTIONS) | _should_retry_ccxt # Combine standard types and custom logic
)
...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    try:
        ...
        raise # Reraise to allow tenacity to handle retries
         ...
         logger.error(f"Error converting price data for {symbol}: {e}. Ticker: {ticker if 'ticker' in locals() else 'N/A'}")
         return None
    except Exception as e:
...
def get_market_info(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        # exchange.load_markets(True) # Force reload if needed, but often unnecessary per cycle
        ...
        if market_details['limits']['amount']['min'] is None:
             ...
             # Set a default? Or let sizing handle it? Let sizing handle potential None.
             # market_details['limits']['amount']['min'] = Decimal('0')
    ...
    try:
        ...
        if len(open_positions) > 1:
            ...
            for i, p in enumerate(open_positions):
                 logger.debug(f"Position {i+1} Info: {p.get('info')}") # Log raw info
        ...
        # Use safe_decimal and check multiple possible keys
        ...
        position_details = {
            ...
            # Protection levels (use None if not set or invalid)
...
def format_value_for_exchange(
    ...
    market_info: Dict,
    logger: logging.Logger, # Removed exchange instance, formatting is independent
    rounding_mode: str = ROUND_DOWN # Default rounding mode
) -> Optional[Decimal]:
...
def calculate_position_size(
    ...
    Returns:
        The calculated position size as a Decimal, or None if calculation fails.
    """
    symbol = market_info.get('symbol', 'UNKNOWN')
    lg = logger
    ...
    try:
        ...
        if config.get('atr_position_sizing', False) and current_atr is not None and current_atr > 0:
            ...
            # A simple approach: scale size inversely with volatility.
            # Higher ATR -> smaller size, Lower ATR -> larger size for same $ risk.
            # Calculate a reference ATR (e.g., average over 100 periods)
            # This requires passing the dataframe or pre-calculating avg ATR.
            # Simpler approach: Normalize SL distance by ATR? Already done via ATR-based SL.
            # Let's adjust the *risk amount* slightly based on current ATR vs entry price.
            # Higher relative ATR suggests higher risk, maybe slightly reduce position size.
        ...
        if min_amount is not None and raw_size < min_amount:
            ...
            # Check if minimum size risk exceeds allowed risk significantly
            max_allowed_risk = balance * Decimal(str(risk_per_trade)) * Decimal('1.5') # Allow 50% overshoot? Be stricter?
...
def place_trade(
    ...
    limit_price: Optional[Decimal] = None # Pass limit price if order_type is 'limit'
) -> Optional[Dict]:
    ...
    Args:
        ...
        position_size: The size of the order in base currency units (already formatted).
        market_info: Market information dictionary.
        config: Bot configuration dictionary.
        logger: Logger instance.
    ...
    try:
        ...
        try:
             ...
             lg.error(f"Invalid position size type for float conversion: {amount_decimal} ({type(amount_decimal)})")
             return None

        if amount_float <= 0:
            lg.error(f"Invalid position size {amount_float} ({amount_decimal}) for {symbol}. Cannot place trade.")
            return None
        ...
        if order_type == 'limit':
            ...
            # BUY: Round DOWN (more likely to fill, less aggressive) or UP (less likely, more aggressive)
            # SELL: Round UP (more likely to fill, less aggressive) or DOWN (less likely, more aggressive)
            # Defaulting to less aggressive (round towards market)
            ...
            try:
                 ...
                 lg.error(f"Invalid limit price type for float conversion: {formatted_limit_price}")
                 return None
        ...
        if "margin" in str(e).lower() or "insufficient balance" in str(e).lower():
             ...
             lg.warning("Order failed possibly due to cost limits. Check position size and price.")
...
def _set_position_protection(
    ...
    Uses the `v5/position/trading-stop` endpoint.
    ...
    Args:
        ...
        market_info: Market information dictionary.
        logger: Logger instance.
        stop_loss_price: The desired stop loss price.
        take_profit_price: The desired take profit price.
        trailing_stop_distance: The desired trailing stop distance (in price units).
        tsl_activation_price: The price at which the trailing stop should activate.
    ...
    if stop_loss_price is not None:
        if stop_loss_price > 0:
            ...
            sl_rounding = ROUND_DOWN if stop_loss_price < market_info.get('last_price', stop_loss_price) else ROUND_UP # Heuristic: assumes SL is away from current price
        ...
        else: # Cancel SL if price is 0 or less
            params['stopLoss'] = '0'
    ...
    if take_profit_price is not None:
        if take_profit_price > 0:
            ...
            tp_rounding = ROUND_UP if take_profit_price > market_info.get('last_price', take_profit_price) else ROUND_DOWN # Heuristic
    ...
    if trailing_stop_distance is not None:
        if trailing_stop_distance > 0:
            # Format distance (usually treated like a price difference, use price precision)
            # Rounding for distance: depends on context, usually floor/ceiling based on tick size
            ...
            if tsl_dist_formatted and tsl_dist_formatted > 0:
                ...
                if tsl_activation_price is not None and tsl_activation_price > 0:
                    ...
                    act_rounding = ROUND_UP if tsl_activation_price > market_info.get('last_price', tsl_activation_price) else ROUND_DOWN
                ...
                lg.error(f"Invalid TSL distance formatting or zero value for {symbol}: {trailing_stop_distance}. Cannot set TSL.")
...
def set_trailing_stop_loss(
    ...
    if use_atr_tsl and current_atr is not None and current_atr > 0:
        ...
        lg.info(f"Using ATR-based TSL for {symbol}: Distance={distance:.4f} ({trailing_atr_multiple}x ATR), Activation Price={activation_price:.4f} ({activation_pct*100}%)")
        ...
        distance = current_price * callback_rate
        activation_offset = entry_price * activation_pct
        activation_price = entry_price + activation_offset if side == 'long' else entry_price - activation_offset
        lg.info(f"Using Percentage-based TSL for {symbol}: Distance={distance:.4f} ({callback_rate*100}% of current price), Activation Price={activation_price:.4f} ({activation_pct*100}% of entry)")
    ...
    if protection_cfg.get("dynamic_tsl_callback", True) and distance is not None and current_atr is not None and current_atr > 0:
        ...
        if profit_atr > Decimal('2.0'):
            ...
            lg.info(f"Dynamic TSL: Profit {profit_atr:.2f}x ATR > 2.0. Tightening TSL distance from {original_distance:.4f} to {distance:.4f}")
        # Example: Loosen slightly if profit barely positive? (More complex)


    # --- Validation and Final Checks ---
    if distance is None or distance <= 0:
        lg.error(f"Calculated invalid TSL distance ({distance}) for {symbol}. Cannot set TSL.")
        return False
    if activation_price is None or activation_price <= 0:
         lg.error(f"Calculated invalid TSL activation price ({activation_price}) for {symbol}. Cannot set TSL.")
         return False
    ...
    if min_tick is None or min_tick <= 0: # Safety check for tick size
        ...
        lg.warning(f"TSL Activation Price ({activation_price:.4f}) for LONG is not above current price ({current_price:.4f}). Adjusting slightly above current.")
        activation_price = current_price + min_tick # Set slightly above current
    elif side == 'short' and activation_price >= current_price:
        lg.warning(f"TSL Activation Price ({activation_price:.4f}) for SHORT is not below current price ({current_price:.4f}). Adjusting slightly below current.")
        activation_price = current_price - min_tick # Set slightly below current
    ...
    return _set_position_protection(
        ...
        stop_loss_price=current_sl, # Preserve existing SL unless BE logic overrides later
...
class VolumaticOBStrategy:
    ...
    def _calculate_min_data_length(self) -> int:
        ...
        # Add a buffer for stability and calculations needing prior data points
        # Buffer should be at least max(ph_right, pl_right) + a few more
...
class SignalGenerator:
    ...
    def _check_ob_entry(self, analysis_results: StrategyAnalysisResults) -> str:
        ...
        if self.strategy_params.dynamic_ob_proximity and current_atr is not None and current_atr > 0:
            ...
            dynamic_factor = Decimal('1') + (relative_atr * Decimal('2')) # Example scaling factor 2
            ...
            if is_trend_up:
                ...
                if relevant_bear_obs:
                    ...
                    if nearest_bear_ob.get('top') and latest_close > nearest_bear_ob['top'] and prev_close <= nearest_bear_ob['top']:
                         self.logger.info(f"BUY signal (Breakout): Price broke above Bear OB {nearest_bear_ob.get('id','N/A')} Top ({nearest_bear_ob['top']:.4f})")
                         signal = "BUY"
                ...
                if relevant_bull_obs:
                     ...
                     if nearest_bull_ob.get('bottom') and latest_close < nearest_bull_ob['bottom'] and prev_close >= nearest_bull_ob['bottom']:
                          self.logger.info(f"SELL signal (Breakout): Price broke below Bull OB {nearest_bull_ob.get('id','N/A')} Bottom ({nearest_bull_ob['bottom']:.4f})")
                          signal = "SELL"
    ...
    def _check_exit_conditions(self, analysis_results: StrategyAnalysisResults, open_position: Dict) -> str:
        ...
        if current_pos_side == 'long':
            ...
            if relevant_bear_obs:
                nearest_bear_ob = relevant_bear_obs[0]
                ob_bottom = nearest_bear_ob.get('bottom')
                if ob_bottom:
            ...
            if relevant_bull_obs:
                nearest_bull_ob = relevant_bull_obs[0]
                ob_top = nearest_bull_ob.get('top')
                if ob_top:
    ...
    def calculate_initial_tp_sl(
        ...
        market_info: Dict
        # Removed exchange instance, formatting happens later or uses format_value_for_exchange directly
        ...
        if use_partial_tp and not use_fib_tp:
            ...
            for i, level in enumerate(self.protection_config.partial_tp_levels):
                try:
                    level_multiple = base_tp_multiple * safe_decimal(level.multiple, default=Decimal('1.0')) # Use base TP multiple adjusted by level's multiple
                    ...
                    self.logger.error(f"Error processing partial TP level {i+1} ({level}): {e}")
...
def analyze_and_trade_symbol(
    ...
    try:
        ...
        if open_position is None and signal in ["BUY", "SELL"]:
            ...
            # current_positions = exchange.fetch_positions() # Fetch all positions (can be slow)
            # active_symbols = {p['symbol'] for p in current_positions if abs(safe_decimal(p.get('contracts', 0))) > 1e-9}
            ...
            if config.get('order_type') == 'limit':
                 ...
                 price_tick = market_info['precision']['price']
                 if price_tick is None or price_tick <= 0:
                      lg.error("Cannot calculate limit entry price: Invalid price tick size.")
                      return # Cannot place limit order without tick size
                 offset = price_tick * Decimal('2') # Example: 2 ticks away from current price
            ...
            trade_order = place_trade(
                ...
                position_size=position_size, # Already formatted by calculate_position_size
                market_info=market_info,
                config=config,
                logger=lg,
                reduce_only=False,
                limit_price=limit_entry_price_raw # Pass raw limit price
            )
                ...
                if confirmed_position:
                    ...
                    if sl_final_raw is None or not tp_levels_final_raw:
                        ...
                        if config.get("protection", {}).get("enable_trailing_stop", False):
                            ...
                            # Calculate TSL distance and activation based on current state
                            ...
                            if use_atr_tsl and current_atr and current_atr > 0:
                                ...
                                tsl_dist = entry_price_actual * tsl_rate # Base distance on actual entry
                            ...
                            if tsl_dist and tsl_dist > 0:
                                ...
                                if tsl_act and tsl_act > 0:
                                     protection_params['tsl_activation_price'] = tsl_act
                                else:
                        ...
                        lg.info(f"Attempting to set initial protection: SL={sl_final_raw:.4f}, TP={first_tp_raw:.4f if first_tp_raw else 'N/A'}, TSL_Dist={protection_params.get('trailing_stop_distance')}, TSL_Act={protection_params.get('tsl_activation_price')}")
                        ...
                        # This is complex and potentially risky if main protection fails. Keep it simple for now.
                        # Consider only setting the *first* TP via the main protection call,
                        # and placing subsequent TPs as separate limit orders if needed.
                        if protection_success and len(tp_levels_final_raw) > 1 and config.get("protection", {}).get("partial_tp_levels"):
                            ...
                            placed_percentage = Decimal('0.0') # Track placed percentage for size calculation
                            ...
                            for i, tp_level in enumerate(tp_levels_final_raw[1:], start=1):
                                ...
                                if size_for_this_level > 0:
                                    ...
                                    lg.info(f"Placing Partial TP {i+1}: Side={tp_side}, Size%={level_percentage:.1%}, Price={tp_price_raw:.4f}")
                                    try:
                                        ...
                                        if formatted_tp_size is None or formatted_tp_size <= 0:
                                             lg.warning(f"Skipping partial TP {i+1}: Formatted size is zero or invalid ({formatted_tp_size}).")
                                             continue
                 ...
                 lg.error(f"Order placement failed for {signal} {symbol}. No order returned.")
        ...
        # ========================================
            ...
            protection_cfg = config.get("protection", {})
            current_sl = open_position.get('stopLossPrice')
            current_tp = open_position.get('takeProfitPrice') # Needed for preserving when setting SL/TSL
            current_tsl_dist = open_position.get('tslDistance')
            current_tsl_active_price = open_position.get('tslActivationPrice')
            is_tsl_set = current_tsl_dist is not None and current_tsl_dist > 0 # Check if TSL parameters are set
            ...
            if protection_cfg.get("enable_break_even", False) and current_atr is not None and current_atr > 0:
                ...
                if profit >= be_trigger_threshold:
                    ...
                    if min_tick is None or min_tick <= 0:
                        ...
                        if current_sl is None: # No SL exists yet
                            sl_needs_update_to_be = True
                        elif pos_side == 'long' and be_sl_price_raw > current_sl: # Move SL up to BE
                            sl_needs_update_to_be = True
                        elif pos_side == 'short' and be_sl_price_raw < current_sl: # Move SL down to BE
                            sl_needs_update_to_be = True
                        ...
                        if sl_needs_update_to_be:
                            ...
                            # Pass existing TP/TSL values to _set_position_protection to *attempt* preservation.
                            update_success = _set_position_protection(
                                exchange, symbol, market_info, lg,
                                stop_loss_price=be_sl_price_raw, # Pass raw price
                                take_profit_price=current_tp, # Try to keep existing TP
                                trailing_stop_distance=current_tsl_dist, # Try to keep existing TSL
                                tsl_activation_price=current_tsl_active_price,
                                position_idx=pos_idx
                            )
                            if not update_success:
                                 lg.warning("Failed to update SL to Break-Even via API.")
                            # else:
                                 # If BE moved SL, maybe disable TSL trailing to OB? Or let TSL take over later?
                                 # Current logic allows other trailing methods to potentially override BE SL later.
            ...
            if protection_cfg.get("enable_trailing_stop", False):
                ...
                # else: handled inside set_trailing_stop_loss


            # --- SL Trailing to OB Boundaries (Snippet 24) ---
            # This logic runs if enabled AND if TSL is NOT currently active (to avoid conflicts)
            # Re-fetch position state to see if TSL was successfully activated in the previous step
            pos_after_tsl_check = get_open_position(exchange, symbol, lg) # Fetch fresh state
            is_tsl_now_active = False
            if pos_after_tsl_check:
                 tsl_dist_check = pos_after_tsl_check.get('tslDistance')
                 is_tsl_now_active = tsl_dist_check is not None and tsl_dist_check > 0

            if protection_cfg.get("sl_trail_to_ob", False) and not is_tsl_now_active:
                ...
                if new_sl_target_raw and nearest_protective_ob:
                    # Get the current SL from the latest position check
                    current_sl_check = pos_after_tsl_check.get('stopLossPrice')
                    sl_needs_update_to_ob = False
                    if current_sl_check is None:
                        sl_needs_update_to_ob = True
                    elif pos_side == 'long' and new_sl_target_raw > current_sl_check:
                        sl_needs_update_to_ob = True
                    elif pos_side == 'short' and new_sl_target_raw < current_sl_check:
                        sl_needs_update_to_ob = True
                    ...
                    if sl_needs_update_to_ob:
                        ...
                        update_success = _set_position_protection(
                            ...
                            take_profit_price=pos_after_tsl_check.get('takeProfitPrice'), # Preserve current TP
        ...
        if "margin" in str(e).lower() or "liquidation" in str(e).lower():
            lg.critical(f"Potential critical exchange issue detected: {e}")
            # Consider pausing or stopping the bot based on error severity
    except Exception as e:
...
def main() -> None:
    ...
    global CONFIG, QUOTE_CURRENCY # Allow main to update global config if needed
    ...
    while True:
        try:
            interval_prompt = f"Enter interval [{'/'.join(VALID_INTERVALS)}] (Press Enter for default: {selected_interval}): "
            ...
            if not interval_input: # User pressed Enter, use default
                 ...
                 init_logger.info(f"Using default interval: {validated_interval}")
                 break
            else:
    ...
    symbol_logger.info(f"Initializing strategy for {target_symbol} on {CONFIG['interval']} interval...")
    ...
    try:
        while True:
            ...
            if check_config_reload(symbol_logger):
                ...
                market_info = get_market_info(exchange, target_symbol, symbol_logger)
                if not market_info:
                     symbol_logger.error("Failed to get market info after config reload. Skipping cycle.")
        ...
        if CONFIG.get("enable_trading") and exchange and market_info: # Check if resources are available
            try:
                ...
                if open_pos:
                    ...
                    if pos_size:
                        lg.warning(f"Closing open {pos_side} position for {target_symbol} (Size: {pos_size})...")
                        close_sig = "SELL" if pos_side == 'long' else "BUY"
                        close_size_formatted = format_value_for_exchange(abs(pos_size), 'amount', market_info, lg, ROUND_DOWN)
                        if close_size_formatted and close_size_formatted > 0:
                             place_trade(exchange, target_symbol, close_sig, close_size_formatted, market_info, CONFIG, lg, reduce_only=True)
                             time.sleep(2) # Brief pause for order processing
                        else:
                             lg.error("Could not format close size during shutdown.")
                    else:
                        lg.warning("Position found but size is invalid, cannot close.")
                else:
                     lg.info("No open position found for {target_symbol} during shutdown.")

                # Cancel remaining open orders (TPs, potentially pending entries)
                lg.info(f"Cancelling any remaining open orders for {target_symbol}...")
                ...
                lg.debug(f"Cancel all orders response: {cancel_response}")
                lg.info("Open orders cancellation attempt completed.")
