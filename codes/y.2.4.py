...
# Bot Identity
BOT_VERSION = "1.1.2" # Updated version after review
    ...
    if os.path.exists(filepath):
        try:
            with open(filepath, encoding='utf-8') as f:
                ...
                expected_symbol_keys = ['break_even_triggered', 'last_entry_price', 'position_size', 'position_side']
                for symbol, symbol_data_raw in state_raw.items():
                    ...
                    if isinstance(symbol_data_raw, dict):
                        ...
                        for key in expected_symbol_keys:
                            ...
                            if key in ['last_entry_price', 'position_size'] and isinstance(raw_value, str):
                                try:
...
if __name__ == "__main__":
    ...
    for sym in config.get("symbols", []):
        if sym not in bot_state:
            bot_state[sym] = {} # Initialize symbol state if missing
    ...
    try:
        ...
        while True:
            ...
            # --- Check and Reload Markets Periodically ---
            now = time.time()
            ...
            if (now - last_load_time) > MARKET_RELOAD_INTERVAL_SECONDS:
                ...
                try:
                    ...
                    exchange.last_load_markets_timestamp = now
                    main_logger.info("Markets reloaded successfully.")
                except Exception as e:
                    main_logger.error(f"Failed to reload markets: {e}", exc_info=True)
                    # Continue with potentially stale market data, but log error

            # --- Fetch Balance (once per cycle) ---
            # Use quote currency from config for main balance check
            current_balance = fetch_balance(exchange, QUOTE_CURRENCY, main_logger)
            if current_balance is None:
                 main_logger.error(f"Failed to fetch {QUOTE_CURRENCY} balance this cycle. Skipping trading actions.")
                 # Optionally skip the rest of the cycle or handle differently
                 # For now, proceed but trading actions requiring balance will fail

            # --- Trading Logic per Symbol ---
            active_positions_count = 0 # Count active positions this cycle
            # Get max allowed concurrent positions
            max_positions = config.get("max_concurrent_positions_total", 1)

            # First pass: Count existing active positions managed by the bot state
            for symbol in config.get("symbols", []):
                symbol_state = bot_state.get(symbol, {})
                # Check if state indicates an active position (non-zero size and valid side)
                if symbol_state.get("position_size") and symbol_state.get("position_side"):
                    active_positions_count += 1

            main_logger.info(f"Active positions (from state): {active_positions_count}/{max_positions}")

            # Second pass: Process each symbol
            for symbol in config.get("symbols", []):
                symbol_logger = setup_logger(symbol, is_symbol_logger=True)
                symbol_state = bot_state.get(symbol, {}) # Get mutable state for this symbol

                try:
                    # --- Get Market Info ---
                    market_info = get_market_info(exchange, symbol, symbol_logger)
                    if not market_info:
                        symbol_logger.warning(f"Skipping {symbol}: Could not get market info.")
                        continue

                    # --- Fetch Data ---
                    # Fetch Klines for analysis
                    kline_limit = config.get("fibonacci_window", 100) + 50 # Ensure enough data for indicators
                    kline_tf = config.get("interval", "5")
                    kline_data_df = fetch_klines_ccxt(exchange, symbol, kline_tf, kline_limit, symbol_logger, market_info)
                    if kline_data_df.empty:
                        symbol_logger.warning(f"Skipping {symbol}: Failed to fetch or process kline data.")
                        continue

                    # --- Analyze Data ---
                    try:
                        analyzer = TradingAnalyzer(kline_data_df, symbol_logger, config, market_info, symbol_state)
                    except ValueError as e:
                         symbol_logger.error(f"Failed to initialize TradingAnalyzer for {symbol}: {e}")
                         continue

                    # Fetch current price and orderbook for signal generation
                    current_price = fetch_current_price_ccxt(exchange, symbol, symbol_logger, market_info)
                    if current_price is None:
                        symbol_logger.warning(f"Skipping {symbol}: Failed to fetch current price.")
                        continue

                    # Fetch orderbook only if the indicator is enabled
                    orderbook = None
                    if config.get("indicators", {}).get("orderbook"):
                        ...
                        # Don't skip if orderbook fetch fails, just log it; OBI score will be None
                        if orderbook is None:
                             symbol_logger.warning(f"Failed to fetch orderbook for {symbol}, OBI signal will be unavailable.")

                    # Generate signal
                    signal = analyzer.generate_trading_signal(current_price, orderbook)

                    # --- Check Position & Execute Logic ---
                    # Fetch actual position from exchange
                    position = fetch_positions_ccxt(exchange, symbol, symbol_logger, market_info)
                    position_size = position.get('contracts') if position else Decimal('0')
                    position_side = position.get('side') if position else None
                    position_entry_price = position.get('entryPrice') if position else None
                    position_idx = position.get('positionIdx', 0) if position else 0 # Get index for hedge mode

                    # Update state with actual position details (more reliable than just tracking intent)
                    symbol_state['position_size'] = position_size
                    symbol_state['position_side'] = position_side
                    symbol_state['last_entry_price'] = position_entry_price
                    # Note: break_even_triggered needs logic to be set based on price movement
