...
getcontext().prec = (
    28  # Increased precision for calculations, especially intermediate ones
)
init(autoreset=True)
load_dotenv()
...
def initialize_exchange(logger: logging.Logger) -> ccxt.Exchange | None:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"{NEON_YELLOW}Could not perform initial balance fetch: {balance_err}. Check API permissions/account type.{RESET}"
            )
        ...
        try:
            ...
            if last_price is not None:
                try:
                    ...
                    lg.warning(f"Invalid 'last' price: {last_price}")
            ...
            if price is None and bid_price is not None and ask_price is not None:
                try:
                    ...
                    lg.warning(f"Invalid bid/ask: {bid_price}, {ask_price}")
            ...
            if price is None and ask_price is not None:  # Fallback to ask
                try:
                    ...
                    lg.warning(f"Invalid 'ask' price: {ask_price}")
            ...
            if price is None and bid_price is not None:  # Fallback to bid
                try:
                    ...
                    lg.warning(f"Invalid 'bid' price: {bid_price}")

            if price is not None and price > 0:
                return price
            else:
                lg.warning(
                    f"Failed to get a valid price from ticker attempt {attempts + 1}."
                )
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if ohlcv is not None:  # Basic check if data was returned
                    ...
                    lg.warning(
                        f"fetch_ohlcv returned None for {symbol} (Attempt {attempt + 1}). Retrying..."
                    )
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            if not orderbook:
                ...
                lg.warning(
                    f"{NEON_YELLOW}Invalid orderbook structure for {symbol}. Attempt {attempts + 1}. Response: {orderbook}{RESET}"
                )
            ...
            lg.warning(
                f"Rate limit exceeded fetching orderbook for {symbol}. Retrying in {RETRY_DELAY_SECONDS * 5}s..."
            )
            time.sleep(RETRY_DELAY_SECONDS * 5)
            attempts += 1
            continue
...
class TradingAnalyzer:
    ...
    def __init__(
        ...
        self.indicator_values: dict[
            str, Any
        ] = {}  # Stores latest indicator values (float or Decimal)
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        expected_patterns = {
            ...
            "StochRSI_K": [
                f"STOCHRSIk_{self.config.get('stoch_rsi_window', DEFAULT_STOCH_RSI_WINDOW)}"
            ],  # Simpler pattern
            "StochRSI_D": [
                f"STOCHRSId_{self.config.get('stoch_rsi_window', DEFAULT_STOCH_RSI_WINDOW)}"
            ],  # Simpler pattern
            "RSI": [f"RSI_{self.config.get('rsi_period', DEFAULT_RSI_WINDOW)}"],
            "BB_Lower": [
                f"BBL_{self.config.get('bollinger_bands_period', DEFAULT_BOLLINGER_BANDS_PERIOD)}"
            ],  # Simpler pattern
            "BB_Middle": [
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        if len(self.df) < min_required_data:
            self.logger.warning(
                f"{NEON_YELLOW}Insufficient data ({len(self.df)} points) for {self.symbol} to calculate all indicators (min recommended: {min_required_data}).{RESET}"
            )
        ...
        try:
            ...
            self.logger.error(
                f"{NEON_RED}AttributeError calculating indicators for {self.symbol}: {e}{RESET}",
                exc_info=True,
            )
        except Exception as e:
            self.logger.error(
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        if self.df.empty or self.df.iloc[-1].isnull().all():
            self.logger.warning(
                f"Cannot update latest values: DataFrame empty or last row is all NaN for {self.symbol}."
            )
        ...
        try:
                ...
                if col_name and col_name in latest.index:
                    ...
                    if pd.notna(value):
                        try:
                            ...
                            self.logger.warning(
                                f"Could not convert value for {key} ('{col_name}': {value}) for {self.symbol}."
                            )
                    ...
                    if key in self.ta_column_names:  # Log only if calc was attempted
                        self.logger.debug(
                            f"Indicator column '{col_name}' for key '{key}' not found in latest data for {self.symbol}. Storing NaN."
                        )
                    updated_values[key] = np.nan
            ...
            for base_col in ["open", "high", "low", "close", "volume"]:
                ...
                if pd.notna(value):
                    try:
                        ...
                        self.logger.warning(
                            f"Could not convert base value for '{base_col}' ({value}) to Decimal for {self.symbol}."
                        )
            ...
            self.logger.error(
                f"Error accessing latest row (iloc[-1]) for {self.symbol}. DataFrame might be empty/short."
            )
            self.indicator_values = dict.fromkeys(
                list(self.ta_column_names.keys())
                + ["Close", "Volume", "High", "Low", "Open"],
    ...
    def calculate_fibonacci_levels(
        ...
        if len(self.df) < window:
            self.logger.debug(
                f"Not enough data ({len(self.df)}) for Fibonacci window ({window}) on {self.symbol}."
            )
        ...
        try:
            ...
            if pd.isna(high_price_raw) or pd.isna(low_price_raw):
                self.logger.warning(
                    f"Could not find valid high/low for Fibonacci on {self.symbol}."
                )
                self.fib_levels_data = {}
                return {}
            ...
            if diff > 0:
                ...
                self.logger.debug(
                    f"Fibonacci range is zero (High={high}, Low={low}) for {self.symbol}."
                )
    ...
    def get_price_precision(self) -> int:
        ...
        try:
            ...
            if last_close and isinstance(last_close, Decimal) and last_close > 0:
                ...
                self.logger.debug(
                    f"Inferring price precision from last close price ({last_close}) as {precision} for {self.symbol}."
                )
                return precision

        except Exception as e:
            self.logger.warning(
                f"Could not determine price precision for {self.symbol}: {e}. Falling back to default."
            )
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            precision_info = self.market_info.get("precision", {})
            price_precision_val = precision_info.get("price")
            if price_precision_val is not None and isinstance(
                price_precision_val, (float, str)
            ):
                tick_size = Decimal(str(price_precision_val))
                if tick_size > 0:
                    return tick_size
            ...
            if min_price_val is not None:
                ...
                if min_tick_from_limit > 0:
                    ...
                    if min_tick_from_limit < Decimal("0.1"):
                        ...
                        self.logger.debug(
                            f"limits.price.min ({min_tick_from_limit}) seems too large for tick size, potentially min order price."
                        )
            ...
            if price_precision_val is not None and isinstance(price_precision_val, int):
                ...
                self.logger.debug(
                    f"Calculated tick size from precision.price (decimals): {tick_size} for {self.symbol}"
                )
                return tick_size
        ...
        try:
            ...
            for name, level_price in self.fib_levels_data.items():
                if isinstance(level_price, Decimal):
                    ...
                    self.logger.warning(
                        f"Non-decimal value found in fib_levels_data: {name}={level_price}"
                    )
    ...
    def calculate_ema_alignment_score(self) -> float:
        ...
        current_price_float = float(
            self.indicator_values.get("Close", np.nan)
    ...
    def generate_trading_signal(
        ...
        if not self.indicator_values:
            self.logger.warning(
                f"{NEON_YELLOW}Cannot generate signal for {self.symbol}: Indicator values empty.{RESET}"
            )
            return "HOLD"
        core_indicators_present = any(
            pd.notna(v)
            for k, v in self.indicator_values.items()
            if k not in ["Close", "Volume", "High", "Low", "Open"]
        )
        ...
        if not active_weights:
            self.logger.error(
                f"Active weight set '{self.active_weight_set_name}' missing or empty for {self.symbol}. Cannot generate signal."
            )
            return "HOLD"
        ...
        for indicator_key, enabled in self.config.get("indicators", {}).items():
            ...
            try:
                ...
                self.logger.warning(
                    f"Invalid weight format '{weight_str}' for indicator '{indicator_key}'. Skipping."
                )
                continue
            ...
            if hasattr(self, check_method_name) and callable(
                ...
                self.logger.warning(
                    f"Check method '{check_method_name}' not found for enabled/weighted indicator: {indicator_key} ({self.symbol})"
                )

        if total_weight_applied == 0:
            self.logger.warning(
                f"No indicators contributed to the signal score for {self.symbol}. Defaulting to HOLD."
            )
            final_signal = "HOLD"
        else:
        ...
        self.logger.debug(
            f"  Indicator Scores: {debug_scores}"
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        if pd.isna(current_volume) or pd.isna(volume_ma_float) or volume_ma_float <= 0:
            return np.nan
        volume_ma = Decimal(
            str(volume_ma_float)
        ...
        try:
            if current_volume > volume_ma * Decimal(str(multiplier)):
                return 0.7  # High volume significance
            elif current_volume < volume_ma / Decimal(str(multiplier)):
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if last_close_float > bb_middle:
            proximity_to_upper = (
                (last_close_float - bb_middle) / (bb_upper - bb_middle)
        ...
        if last_close_float < bb_middle:
            proximity_to_lower = (
                (bb_middle - last_close_float) / (bb_middle - bb_lower)
    ...
    def _check_orderbook(
        ...
        try:
            ...
            self.logger.debug(
                f"Orderbook check ({self.symbol}): Top {num_levels_to_check}: BidVol={bid_volume_sum:.4f}, AskVol={ask_volume_sum:.4f}, OBI={obi:.4f}, Score={score:.4f}"
            )
            return score
    ...
    def calculate_entry_tp_sl(
        ...
        if not isinstance(atr_val, Decimal) or pd.isna(atr_val) or atr_val <= 0:
            self.logger.warning(
                f"{NEON_YELLOW}Cannot calculate TP/SL for {self.symbol}: Invalid ATR ({atr_val}).{RESET}"
            )
            return entry_price_estimate, None, None
        if (
            ...
            self.logger.warning(
                f"{NEON_YELLOW}Cannot calculate TP/SL for {self.symbol}: Invalid entry price estimate ({entry_price_estimate}).{RESET}"
            )
            return entry_price_estimate, None, None
        ...
        try:
            ...
            if signal == "BUY" and take_profit <= entry_price:
                self.logger.warning(
                    f"{NEON_YELLOW}BUY TP calculation non-profitable (TP {take_profit} <= Entry {entry_price}). Setting TP to None.{RESET}"
                )
                take_profit = None
            elif signal == "SELL" and take_profit >= entry_price:
                self.logger.warning(
                    f"{NEON_YELLOW}SELL TP calculation non-profitable (TP {take_profit} >= Entry {entry_price}). Setting TP to None.{RESET}"
                )
                take_profit = None
            ...
            if stop_loss is not None and stop_loss <= 0:
                self.logger.error(
                    f"{NEON_RED}Stop loss calc resulted in non-positive price ({stop_loss}). Setting SL to None.{RESET}"
                )
                stop_loss = None
            if take_profit is not None and take_profit <= 0:
                self.logger.warning(
                    f"{NEON_YELLOW}Take profit calc resulted in non-positive price ({take_profit}). Setting TP to None.{RESET}"
                )
                take_profit = None

            self.logger.debug(
                f"Calculated TP/SL for {self.symbol} {signal}: Entry={entry_price:.{price_precision}f}, TP={take_profit}, SL={stop_loss}, ATR={atr_val:.{price_precision + 1}f}"
            )
    ...
    try:
            ...
            try:
                ...
                if (
                    ...
                    if found_structure:
                        break
                lg.debug(
                    f"Currency '{currency}' not directly found using type '{acc_type}'."
                )
        ...
        # 2. Bybit V5 Nested
            ...
            if available_balance_str:
                lg.debug(f"Found balance via Bybit V5 nested: {available_balance_str}")
            else:
                lg.warning(
                    f"{currency} not found within V5 'info.result.list[].coin[]'."
                )
        ...
        if available_balance_str is None:
            ...
            if total_balance is not None:
                lg.warning(
                    f"{NEON_YELLOW}Using 'total' balance ({total_balance}) as fallback for {currency}.{RESET}"
                )
                available_balance_str = str(total_balance)
            else:
                lg.error(
                    f"{NEON_RED}Could not determine any balance for {currency}.{RESET}"
                )
        ...
        Exception
    ) as e:  # Catch errors from the initial try/except blocks if fetch failed entirely
        lg.error(
            f"{NEON_RED}Critical error during balance fetch for {currency}: {e}{RESET}",
            exc_info=True,
        )
        return None
...
def calculate_position_size(
    ...
    if not (0 < risk_per_trade < 1):
        lg.error(
            f"Position sizing failed ({symbol}): Invalid risk_per_trade ({risk_per_trade})."
        )
        return None
    if (
        ...
        lg.error(
            f"Position sizing failed ({symbol}): Invalid entry ({entry_price}) or SL ({initial_stop_loss_price})."
        )
        return None
    ...
    if "limits" not in market_info or "precision" not in market_info:
        lg.error(
            f"Position sizing failed ({symbol}): Market info missing limits/precision."
        )
        return None
    ...
    try:
        ...
        try:
            ...
            lg.warning(f"Invalid contract size '{contract_size_str}', using 1.")
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            lg.warning(
                f"Inverse contract sizing for {symbol} using placeholder logic - VERIFY!"
            )
            calculated_size = risk_amount_quote / (sl_distance_per_unit * contract_size)

        lg.info(
            f"Position Sizing ({symbol}): Balance={balance:.2f}, Risk={risk_per_trade:.2%}, RiskAmt={risk_amount_quote:.4f}"
        )
        lg.info(
        ...
        if adjusted_size != calculated_size:
            lg.warning(
                f"{NEON_YELLOW}Size adjusted by amount limits: {calculated_size:.8f} -> {adjusted_size:.8f} {size_unit}{RESET}"
            )

        # 2. Check COST limits (Linear/Spot assumed for cost calc)
        current_cost = adjusted_size * entry_price * contract_size
        lg.debug(
            f"  Cost Check: Adjusted Size={adjusted_size:.8f}, Estimated Cost={current_cost:.4f} {quote_currency}"
        )

        if min_cost > 0 and current_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {current_cost:.4f} below min cost {min_cost:.4f}. Attempting to increase size.{RESET}"
            )
            required_size_for_min_cost = (
                min_cost / (entry_price * contract_size)
            ...
            if required_size_for_min_cost > max_amount:
                lg.error(
                    f"{NEON_RED}Cannot meet min cost {min_cost:.4f} without exceeding max amount {max_amount:.8f}. Aborted.{RESET}"
                )
                return None
            elif required_size_for_min_cost < min_amount:
                lg.error(
                    f"{NEON_RED}Conflicting limits: Min cost requires size {required_size_for_min_cost:.8f}, but min amount is {min_amount:.8f}. Aborted.{RESET}"
                )
                return None
            else:
                lg.info(
                    f"  Adjusting size to meet min cost: {required_size_for_min_cost:.8f}"
                )
            ...
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {current_cost:.4f} exceeds max cost {max_cost:.4f}. Reducing size.{RESET}"
            )
            adjusted_size_for_max_cost = (
                max_cost / (entry_price * contract_size)
                if entry_price > 0 and contract_size > 0
                else None
            )
            if adjusted_size_for_max_cost is None:
                ...
                return None
            lg.info(
                f"  Reduced size to meet max cost: {adjusted_size_for_max_cost:.8f}"
            )
            if adjusted_size_for_max_cost < min_amount:
                lg.error(
                    f"{NEON_RED}Size reduced for max cost ({adjusted_size_for_max_cost:.8f}) is below min amount {min_amount:.8f}. Aborted.{RESET}"
                )
                return None
            else:
        ...
        try:
            ...
            lg.info(
                f"Applied amount precision/step (Truncated): {adjusted_size:.8f} -> {final_size} {size_unit}"
            )
        except Exception as fmt_err:
            lg.warning(
                f"{NEON_YELLOW}Could not use exchange.amount_to_precision ({fmt_err}). Using manual rounding.{RESET}"
            )
            ...
            if amount_step > 0:
                final_size = (
                    adjusted_size // amount_step
                ) * amount_step  # Round down to step size
                lg.info(
                    f"Applied manual amount step size ({amount_step}): {adjusted_size:.8f} -> {final_size} {size_unit}"
                )
            else:
                lg.warning(
                    "Could not determine amount step size, using unrounded adjusted size."
                )
        ...
        if final_size <= 0:
            lg.error(
                f"{NEON_RED}Position size became zero/negative ({final_size}) after adjustments. Aborted.{RESET}"
            )
            return None
        if final_size < min_amount:
            lg.error(
                f"{NEON_RED}Final size {final_size} is below minimum amount {min_amount}. Aborted.{RESET}"
            )
            return None
        # Final cost check
        ...
        if min_cost > 0 and final_cost < min_cost:
            lg.error(
                f"{NEON_RED}Final size {final_size} results in cost {final_cost:.4f} below minimum cost {min_cost:.4f}. Aborted.{RESET}"
            )
            return None
...
def get_open_position(
    ...
    try:
        ...
        try:
            ...
            if "symbol not found" in str(e).lower() or (
        ...
        if fetch_all:
            try:
                ...
                lg.debug(
                    f"Fetched {len(all_positions)} total, found {len(positions)} matching {symbol}."
                )
            except Exception as e:
                lg.error(
        ...
        if active_position:
            ...
            def format_log_val(val, precision=6, is_size=False):
                if val is None or str(val).strip() == "" or str(val) == "0":
                    return "N/A"  # Treat '0' string as N/A for SL/TP/TSL
                try:
                    ...
                    if is_size:
                        ...
                        try:
                            amount_prec = abs(
                                get_market_info(exchange, symbol, lg)["precision"][
                                    "amount"
                                ]
                                .normalize()
                                .as_tuple()
                            ...
                            amount_prec = 8  # Default precision for size
                        return f"{abs(d_val):.{amount_prec}f}"  # Show absolute size
                    # Price/Value formatting
                    else:
                        # Use market price precision
                        try:
                            price_prec = TradingAnalyzer(
                                pd.DataFrame(),
                                lg,
                                CONFIG,
                                get_market_info(exchange, symbol, lg),
                            ).get_price_precision()
                        except:
                            price_prec = precision  # Fallback precision
                        return f"{d_val:.{price_prec}f}"
                except Exception:
                    return str(val)

            entry_price = format_log_val(
                active_position.get("entryPrice", info_dict.get("avgPrice"))
            )
            contracts = format_log_val(
                active_position.get("contracts", info_dict.get("size")), is_size=True
            )
            ...
            tsl_dist = format_log_val(
                active_position.get("trailingStopLoss")
            )  # Distance (value)
            tsl_act = format_log_val(
...
def set_leverage_ccxt(
    ...
    if leverage <= 0:
        lg.warning(
            f"Leverage setting skipped for {symbol}: Invalid leverage ({leverage})."
        )
        return False
    if not exchange.has.get("setLeverage"):
        lg.error(
            f"{NEON_RED}Exchange {exchange.id} does not support set_leverage via CCXT.{RESET}"
        )
        return False
    ...
    try:
        ...
        if bybit_code == 110045 or "leverage not modified" in err_str:
            ...
            lg.error(
                f"{NEON_YELLOW} >> Hint: Check Margin Mode (Isolated/Cross) compatibility with leverage setting.{RESET}"
            )
            ...
            lg.error(
                f"{NEON_YELLOW} >> Hint: Leverage value {leverage}x might be invalid for {symbol}. Check allowed range.{RESET}"
            )
    except Exception as e:
        lg.error(
...
def place_trade(
    ...
    try:
        ...
        if amount_float <= 0:
            lg.error(
                f"Trade aborted ({symbol} {side} {action}): Invalid position size ({amount_float})."
            )
            return None
    except Exception as e:
        lg.error(
            f"Trade aborted ({symbol} {side} {action}): Failed to convert size {position_size} to float: {e}"
        )
        return None
    ...
    if reduce_only:  # Closing order specific parameters
        params["timeInForce"] = (
            "IOC"  # ImmediateOrCancel recommended for closing market orders
        )
        lg.info(
            f"Attempting to place {action} {side.upper()} {order_type} order for {symbol}:"
        )
    else:  # Opening order
        lg.info(
            f"Attempting to place {action} {side.upper()} {order_type} order for {symbol}:"
    ...
    try:
        ...
        lg.info(
            f"{NEON_GREEN}{action} Trade Placed Successfully! Order ID: {order_id}, Initial Status: {order_status}{RESET}"
        )
        ...
        lg.error(
            f"{NEON_RED}Insufficient funds to {action} {side} order ({symbol}): {e}{RESET}"
        )
        # ... (balance logging and hints remain similar) ...
    except ccxt.InvalidOrder as e:
        lg.error(
            f"{NEON_RED}Invalid order parameters to {action} {side} order ({symbol}): {e}{RESET}"
        )
        # ... (parameter logging and hints remain similar) ...
        bybit_code = getattr(e, "code", None)
        if reduce_only and bybit_code == 110014:  # Reduce-only error
            lg.error(
                f"{NEON_YELLOW} >> Hint (110014): Reduce-only order failed. Position might already be closed, size incorrect, or API issue?{RESET}"
            )
    except ccxt.NetworkError as e:
        lg.error(
            f"{NEON_RED}Network error placing {action} order ({symbol}): {e}{RESET}"
        )
    except ccxt.ExchangeError as e:
        bybit_code = getattr(e, "code", None)
        lg.error(
            f"{NEON_RED}Exchange error placing {action} order ({symbol}): {e} (Code: {bybit_code}){RESET}"
        )
        # ... (hints for common codes remain similar) ...
        if (
            reduce_only and bybit_code == 110025
        ):  # Position not found/zero on close attempt
            lg.warning(
                f"{NEON_YELLOW} >> Hint (110025): Position might have been closed already when trying to place reduce-only order.{RESET}"
            )
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected error placing {action} order ({symbol}): {e}{RESET}",
            exc_info=True,
        )
...
def _set_position_protection(
    ...
    trailing_stop_distance: Decimal | None = None,  # Distance in price points (Decimal)
    ...
    if pos_side not in ["long", "short"]:
        lg.error(
            f"Cannot set protection for {symbol}: Invalid position side ('{pos_side}')."
        )
        return False
    ...
    if not has_sl and not has_tp and not has_tsl:
        lg.info(
            f"No valid protection parameters provided for {symbol}. No protection set/updated."
        )
    ...
    try:
        ...
        lg.warning(f"Could not parse positionIdx, using {position_idx}.")
    ...
    try:
        ...
        def format_price(price_decimal: Decimal | None) -> str | None:
            ...
            try:
                ...
                lg.warning(f"Failed to format price {price_decimal}: {e}")
                return None
        ...
        if has_tsl:
            ...
            try:
                ...
                if Decimal(formatted_tsl_distance) < min_tick:
                    formatted_tsl_distance = str(
                        min_tick
                    )  # Ensure minimum distance is at least one tick
            except Exception as e:
                lg.warning(
                    f"Failed to format TSL distance {trailing_stop_distance}: {e}"
                )
    ...
    try:
        ...
        if ret_code == 0:
            if "not modified" in ret_msg.lower():
                lg.info(
                    f"{NEON_YELLOW}Position protection already set to target values or partially modified for {symbol}. Response: {ret_msg}{RESET}"
                )
            else:
                lg.info(
            ...
            if ret_code == 110013 and "parameter error" in ret_msg.lower():
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110013): Check SL/TP prices vs entry, TSL dist/act prices, tick size compliance.{RESET}"
                )
            elif ret_code == 110036:  # TSL Active price invalid
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110036): TSL Activation price {params.get('activePrice')} likely invalid (already passed, wrong side, too close?).{RESET}"
                )
            elif ret_code == 110086:  # SL=TP
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110086): SL price cannot equal TP price.{RESET}"
                )
            elif "trailing stop value invalid" in ret_msg.lower():
                lg.error(
                    f"{NEON_YELLOW} >> Hint: TSL distance {params.get('trailingStop')} invalid (too small/large/violates tick?).{RESET}"
                )
...
def set_trailing_stop_loss(
    ...
    config: dict[str, Any],
    logger: logging.Logger,
    take_profit_price: Decimal | None = None,  # Optional TP to set alongside TSL
) -> bool:
    ...
    if not config.get("enable_trailing_stop", False):
        lg.info(
            f"Trailing Stop Loss disabled in config for {symbol}. Skipping TSL setup."
        )
    ...
    if callback_rate <= 0:
        lg.error(
            f"{NEON_RED}Invalid trailing_stop_callback_rate ({callback_rate}) for {symbol}.{RESET}"
        )
        return False
    if activation_percentage < 0:
        lg.error(
            f"{NEON_RED}Invalid trailing_stop_activation_percentage ({activation_percentage}) for {symbol}.{RESET}"
        )
        return False

    try:
        entry_price_str = position_info.get("entryPrice") or position_info.get(
            "info", {}
        ).get("avgPrice")
        side = position_info.get("side")
        if entry_price_str is None or side not in ["long", "short"]:
            lg.error(
                f"{NEON_RED}Missing required position info (entryPrice, side) for TSL calc ({symbol}).{RESET}"
            )
            return False
        entry_price = Decimal(str(entry_price_str))
        if entry_price <= 0:
            lg.error(
                f"{NEON_RED}Invalid entry price ({entry_price}) for TSL calc ({symbol}).{RESET}"
            )
            return False
    except Exception as e:
        lg.error(
            f"{NEON_RED}Error parsing position info for TSL calculation ({symbol}): {e}.{RESET}"
        )
        return False
    ...
    try:
        ...
        if activation_price is None or activation_price <= 0:
            lg.error(
                f"{NEON_RED}Calculated TSL activation price ({activation_price}) invalid for {symbol}.{RESET}"
            )
            return False
        ...
        if trailing_distance < min_tick_size:
            trailing_distance = min_tick_size  # Enforce minimum tick size
        if trailing_distance <= 0:
            lg.error(
                f"{NEON_RED}Calculated TSL distance zero/negative ({trailing_distance}) for {symbol}.{RESET}"
            )
            return False
...
def analyze_and_trade_symbol(
    ...
    if not ccxt_interval:
        lg.error(
            f"Invalid interval '{config['interval']}'. Cannot map to CCXT timeframe."
        )
        return
    ...
    if klines_df.empty or len(klines_df) < 50:
        lg.error(
            f"{NEON_RED}Failed sufficient kline data for {symbol} (fetched {len(klines_df)}). Skipping cycle.{RESET}"
        )
        return
    ...
    if current_price is None:
        lg.warning(
            f"{NEON_YELLOW}Failed current price for {symbol}. Using last close fallback.{RESET}"
        )
        try:
            ...
            if pd.notna(last_close_val) and last_close_val > 0:
                ...
                lg.error(f"{NEON_RED}Last close price invalid. Cannot proceed.{RESET}")
                return
        except Exception as e:
            lg.error(f"{NEON_RED}Error getting last close: {e}. Cannot proceed.{RESET}")
            return
    ...
    if not analyzer.indicator_values:
        lg.error(
            f"{NEON_RED}Indicator calculation failed for {symbol}. Skipping signal.{RESET}"
        )
        return

    signal = analyzer.generate_trading_signal(current_price, orderbook_data)
    _, tp_calc, sl_calc = analyzer.calculate_entry_tp_sl(
        current_price, signal
    )  # Based on current price estimate
    price_precision = analyzer.get_price_precision()
    min_tick_size = analyzer.get_min_tick_size()
    ...
    lg.info(
        f"TSL: {'Enabled' if tsl_enabled else 'Disabled'} | BE: {'Enabled' if be_enabled else 'Disabled'}"
    )
    ...
    if open_position is None:
        if signal in ["BUY", "SELL"]:
            ...
            if sl_calc is None:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Initial SL calculation failed (ATR invalid?).{RESET}"
                )
                return
            ...
            if market_info.get("is_contract", False):
                ...
                if leverage > 0:
                    if not set_leverage_ccxt(
                        ...
                        lg.error(
                            f"{NEON_RED}Trade Aborted ({symbol} {signal}): Failed to set leverage.{RESET}"
                        )
                        return
            else:
                lg.info("Leverage setting skipped (Spot).")
            ...
            if position_size is None or position_size <= 0:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Invalid position size ({position_size}).{RESET}"
                )
                return
            ...
            if trade_order and trade_order.get("id"):
                ...
                lg.info(
                    f"Attempting confirmation for {symbol} after order {order_id}..."
                )
                ...
                if confirmed_position:
                    try:
                        entry_price_actual_str = confirmed_position.get(
                            "entryPrice"
                        ) or confirmed_position.get("info", {}).get("avgPrice")
                        entry_price_actual = (
                        ...
                        lg.info(
                            f"{NEON_GREEN}Position Confirmed! Actual Entry: ~{entry_price_actual:.{price_precision}f}{RESET}"
                        )
                        ...
                        if config.get("enable_trailing_stop", False):
                            lg.info(
                                f"Setting Trailing Stop Loss (TP target: {tp_final})..."
                            )
                            ...
                            if sl_final or tp_final:
                                ...
                                lg.warning(
                                    f"{NEON_YELLOW}Fixed SL/TP calculation failed based on actual entry. No fixed protection set.{RESET}"
                                )
                        ...
                        if protection_set_success:
                            ...
                            lg.warning(
                                f"{NEON_YELLOW}MANUAL MONITORING REQUIRED!{RESET}"
                            )
                    ...
                    lg.error(
                        f"{NEON_RED}Trade order {order_id} placed, but FAILED TO CONFIRM position after {confirm_delay}s delay!{RESET}"
                    )
                    lg.warning(
                        f"{NEON_YELLOW}Order may have failed/rejected, or API delay. Manual investigation required!{RESET}"
                    )
            else:
                lg.error(
                    f"{NEON_RED}=== TRADE EXECUTION FAILED ({symbol} {signal}). See logs. ==="
                )
        else:  # signal == HOLD
    ...
    # --- Scenario 2: Existing Open Position ---
        ...
        if exit_signal_triggered:
            lg.warning(
                f"{NEON_YELLOW}*** EXIT Signal Triggered: New signal ({signal}) opposes existing {pos_side} position. Closing... ***{RESET}"
            )
            try:
                close_side = "sell" if pos_side == "long" else "buy"
                size_to_close_str = open_position.get("contracts") or open_position.get(
                    "info", {}
                ).get("size")
                if size_to_close_str is None:
                    raise ValueError("Cannot determine position size to close.")
                size_to_close = abs(Decimal(str(size_to_close_str)))
                if size_to_close <= 0:
                    raise ValueError(f"Position size {size_to_close_str} invalid.")

                lg.info(
                    f"==> Placing {close_side.upper()} MARKET order (reduceOnly=True) | Size: {size_to_close} <=="
                )
                ...
                lg.error(
                    f"{NEON_RED}Error closing position {symbol}: {close_err}{RESET}",
                    exc_info=True,
                )
                lg.warning(
            ...
            try:
                is_tsl_active = (
                    Decimal(str(open_position.get("trailingStopLoss", "0"))) > 0
                )
            except:
                pass  # Ignore parsing error for TSL check
            ...
            if config.get("enable_break_even", False) and not is_tsl_active:
                ...
                try:
                    entry_price = Decimal(
                        str(
                            ...
                            or open_position.get("info", {}).get("avgPrice")
                        )
                    )
                    if not isinstance(current_atr, Decimal) or current_atr <= 0:
                        ...
                        if profit_in_atr >= profit_target_atr:
                            ...
                            be_stop_price = (
                                (entry_price + tick_offset).quantize(
                                    min_tick_size, rounding=ROUND_UP
                                )
                                if pos_side == "long"
                                else (entry_price - tick_offset).quantize(
                            ...
                            if be_stop_price > 0:
                                if current_sl_price is None:
                                    ...
                                    pos_side == "long"
                                    and be_stop_price > current_sl_price
                                ):
                                    update_be = True
                                    lg.info(
                                        f"BE triggered: Target {be_stop_price} > Current SL {current_sl_price}."
                                    )
                                elif (
                                    pos_side == "short"
                                    and be_stop_price < current_sl_price
                                    ...
                                    lg.debug(
                                        f"BE Triggered, but current SL ({current_sl_price}) already better than target ({be_stop_price})."
                                    )
                            ...
                            if update_be:
                                ...
                                if success:
                                    lg.info(
                                        f"{NEON_GREEN}Break-Even SL set successfully.{RESET}"
                                    )
                                else:
                                    lg.error(
                ...
                lg.info("Break-even check skipped: Trailing Stop Loss is active.")
            else:  # BE disabled
...
def main() -> None:
    ...
    init_logger.info(
        f"--- Starting LiveXY Bot ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---"
    )
    CONFIG = load_config(CONFIG_FILE)
    QUOTE_CURRENCY = CONFIG.get("quote_currency", "USDT")
    init_logger.info(f"Config loaded. Quote Currency: {QUOTE_CURRENCY}")
    init_logger.info(
        f"Versions: CCXT={ccxt.__version__}, Pandas={pd.__version__}, PandasTA={ta.version if hasattr(ta, 'version') else 'N/A'}"
    )
    ...
    if CONFIG.get("enable_trading"):
        ...
        if CONFIG.get("use_sandbox"):
            ...
            init_logger.warning(
                f"{NEON_RED}!!! USING REAL MONEY ENVIRONMENT !!!{RESET}"
            )
        init_logger.warning(
            f"Settings: Risk={CONFIG.get('risk_per_trade', 0) * 100:.2f}%, Lev={CONFIG.get('leverage', 0)}x, TSL={'ON' if CONFIG.get('enable_trailing_stop') else 'OFF'}, BE={'ON' if CONFIG.get('enable_break_even') else 'OFF'}"
        )
        try:
            input(
                f">>> Review settings. Press {NEON_GREEN}Enter{RESET} to continue, or {NEON_RED}Ctrl+C{RESET} to abort... "
            )
            init_logger.info("User confirmed live trading settings.")
        except KeyboardInterrupt:
            init_logger.info("User aborted.")
            return
    else:
        init_logger.info(
    ...
    if not exchange:
        init_logger.critical(
            f"{NEON_RED}Failed exchange initialization. Exiting.{RESET}"
        )
        return
    ...
    while True:
        try:
            symbol_input_raw = (
                input(f"{NEON_YELLOW}Enter symbol (e.g., BTC/USDT): {RESET}")
                .strip()
                .upper()
            )
            ...
            if market_info:
                ...
                if "/" in symbol_input:
                    ...
                    variations_to_try.append(
                        f"{base}/{quote}:{quote}"
                    )  # Common linear perp format

                found_variation = False
                if variations_to_try:
                    init_logger.info(
                        f"Symbol '{symbol_input}' not found. Trying variations: {variations_to_try}"
                    )
                ...
                if found_variation:
                    ...
                    init_logger.error(
                        f"{NEON_RED}Symbol '{symbol_input_raw}' and variations not validated.{RESET}"
                    )

        except Exception as e:
            init_logger.error(f"Error validating symbol: {e}", exc_info=True)

    selected_interval = None
    while True:
        interval_input = input(
            f"{NEON_YELLOW}Enter interval [{'/'.join(VALID_INTERVALS)}] (default: {CONFIG['interval']}): {RESET}"
        ).strip()
        ...
        if interval_input in VALID_INTERVALS and interval_input in CCXT_INTERVAL_MAP:
            ...
            init_logger.error(
                f"{NEON_RED}Invalid interval '{interval_input}'. Choose from {VALID_INTERVALS}.{RESET}"
            )
    ...
    symbol_logger.info(
        f"Config: Risk={CONFIG['risk_per_trade']:.2%}, Lev={CONFIG['leverage']}x, TSL={'ON' if CONFIG['enable_trailing_stop'] else 'OFF'}, BE={'ON' if CONFIG['enable_break_even'] else 'OFF'}, Trading={'ENABLED' if CONFIG['enable_trading'] else 'DISABLED'}"
    )
    ...
    try:
        while True:
            ...
            symbol_logger.debug(
                f">>> New Loop Cycle Start: {datetime.now(TIMEZONE).strftime('%H:%M:%S')}"
            )
            try:
                ...
                symbol_logger.warning(
                    f"{NEON_YELLOW}Rate limit: {e}. Waiting 60s...{RESET}"
                )
                time.sleep(60)
            except (ccxt.NetworkError, requests.exceptions.RequestException) as e:
                symbol_logger.error(
                    f"{NEON_RED}Network error: {e}. Waiting {RETRY_DELAY_SECONDS * 3}s...{RESET}"
                )
                time.sleep(RETRY_DELAY_SECONDS * 3)
            except ccxt.AuthenticationError as e:
                symbol_logger.critical(
                    f"{NEON_RED}CRITICAL Auth Error: {e}. Stopping.{RESET}"
                )
                break
            except ccxt.ExchangeNotAvailable as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange unavailable: {e}. Waiting 60s...{RESET}"
                )
                time.sleep(60)
            except ccxt.OnMaintenance as e:
                symbol_logger.error(
                    f"{NEON_RED}Exchange Maintenance: {e}. Waiting 5m...{RESET}"
                )
                time.sleep(300)
            except Exception as loop_error:
                symbol_logger.error(
                    f"{NEON_RED}Uncaught loop error: {loop_error}{RESET}", exc_info=True
                )
                time.sleep(15)
            ...
            symbol_logger.debug(
                f"<<< Loop cycle finished in {elapsed_time:.2f}s. Sleeping {sleep_time:.2f}s..."
            )
            if sleep_time > 0:
                time.sleep(sleep_time)

    except KeyboardInterrupt:
        symbol_logger.info("Keyboard interrupt. Shutting down...")
    except Exception as critical_error:
        init_logger.critical(
            f"{NEON_RED}Critical unhandled error: {critical_error}{RESET}",
            exc_info=True,
        )
    finally:
        ...
        if "exchange" in locals() and exchange and hasattr(exchange, "close"):
            try:
                ...
                exchange.close()
                init_logger.info("Connection closed.")
            except Exception as close_err:
                init_logger.error(f"Error closing connection: {close_err}")
        logging.shutdown()
