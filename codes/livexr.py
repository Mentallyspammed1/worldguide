...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"{NEON_RED}AttributeError calculating indicators for {self.symbol}: {e}{RESET}", exc_info=True
            )
        except Exception as e:
            self.logger.error(
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        if self.df.empty or self.df.iloc[-1].isnull().all():
            ...
            self.indicator_values = dict.fromkeys(
                list(self.ta_column_names.keys()) + ["Close", "Volume", "High", "Low", "Open"], np.nan
            )
            return
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: dict | None) -> str:
        ...
        core_indicators_present = any(
            pd.notna(v) for k, v in self.indicator_values.items() if k not in ["Close", "Volume", "High", "Low", "Open"]
        )
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if last_close_float > bb_middle:
            proximity_to_upper = (
                (last_close_float - bb_middle) / (bb_upper - bb_middle) if (bb_upper - bb_middle) > 0 else 0
            )
            return 0.3 * (1 - proximity_to_upper)
        if last_close_float < bb_middle:
            proximity_to_lower = (
                (bb_middle - last_close_float) / (bb_middle - bb_lower) if (bb_middle - bb_lower) > 0 else 0
            )
    ...
    try:
        ...
        # 2. Bybit V5 Nested
            ...
            if available_balance_str:
                ...
                lg.warning(f"{currency} not found within V5 'info.result.list[].coin[]'.")
...
def calculate_position_size(
    ...
    if not (0 < risk_per_trade < 1):
        ...
        return None
    if initial_stop_loss_price is None or initial_stop_loss_price <= 0 or entry_price is None or entry_price <= 0:
    ...
    try:
        ...
        if min_cost > 0 and current_cost < min_cost:
            ...
            required_size_for_min_cost = (
                min_cost / (entry_price * contract_size) if entry_price > 0 and contract_size > 0 else None
            )
            ...
            adjusted_size_for_max_cost = (
                max_cost / (entry_price * contract_size) if entry_price > 0 and contract_size > 0 else None
            )
        ...
        try:
            ...
            )  # Assuming amount step is same as price tick for simplicity - **CHECK THIS**
            ...
            def format_log_val(val, precision=6, is_size=False):
                ...
                try:
                    ...
                    if is_size:
                        ...
                        try:
                            amount_prec = abs(
                                get_market_info(exchange, symbol, lg)["precision"]["amount"]
                                .normalize()
                                .as_tuple()
                                .exponent
                            )
                    ...
                    # Price/Value formatting
                        ...
                        try:
                            price_prec = TradingAnalyzer(
                                pd.DataFrame(), lg, CONFIG, get_market_info(exchange, symbol, lg)
                            ).get_price_precision()
                        except:
    ...
    try:
        ...
        if has_tsl:
            ...
            try:
                ...
                lg.warning(f"Failed to format TSL distance {trailing_stop_distance}: {e}")
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        if side == "long":
            ...
            if activation_percentage > 0 and activation_price <= entry_price or activation_percentage == 0:
...
def analyze_and_trade_symbol(
        ...
        if signal in ["BUY", "SELL"]:
            ...
            if trade_order and trade_order.get("id"):
                ...
                if confirmed_position:
                    try:
                        entry_price_actual_str = confirmed_position.get("entryPrice") or confirmed_position.get(
                            "info", {}
                        ).get("avgPrice")
                        entry_price_actual = (
    ...
    # --- Scenario 2: Existing Open Position ---
        ...
        if exit_signal_triggered:
            ...
            is_tsl_active = False
            try:
                is_tsl_active = Decimal(str(open_position.get("trailingStopLoss", "0"))) > 0
            except:
            ...
            if config.get("enable_break_even", False) and not is_tsl_active:
                ...
                try:
                    entry_price = Decimal(
                        str(open_position.get("entryPrice") or open_position.get("info", {}).get("avgPrice"))
                    )
                    if not isinstance(current_atr, Decimal) or current_atr <= 0:
                        ...
                        if profit_in_atr >= profit_target_atr:
                            ...
                            be_stop_price = (
                                (entry_price + tick_offset).quantize(min_tick_size, rounding=ROUND_UP)
                                if pos_side == "long"
                            ...
                            if update_be:
                                ...
                                current_tp_str = open_position.get("takeProfitPrice") or open_position.get(
                                    "info", {}
                                ).get("takeProfit")
                                ...
                                if success:
                                    lg.info(f"{NEON_GREEN}Break-Even SL set successfully.{RESET}")
                                else:
...
def main() -> None:
    ...
    if not exchange:
        init_logger.critical(f"{NEON_RED}Failed exchange initialization. Exiting.{RESET}")
        return
        ...
        try:
            ...
            if market_info:
                ...
                if found_variation:
                    ...
                    init_logger.error(f"{NEON_RED}Symbol '{symbol_input_raw}' and variations not validated.{RESET}")
    ...
    while True:
        ...
        if interval_input in VALID_INTERVALS and interval_input in CCXT_INTERVAL_MAP:
            ...
            init_logger.error(f"{NEON_RED}Invalid interval '{interval_input}'. Choose from {VALID_INTERVALS}.{RESET}")
    ...
    try:
        while True:
            ...
            try:
                ...
                symbol_logger.warning(f"{NEON_YELLOW}Rate limit: {e}. Waiting 60s...{RESET}")
                time.sleep(60)
            except (ccxt.NetworkError, requests.exceptions.RequestException) as e:
                symbol_logger.error(f"{NEON_RED}Network error: {e}. Waiting {RETRY_DELAY_SECONDS * 3}s...{RESET}")
                time.sleep(RETRY_DELAY_SECONDS * 3)
            except ccxt.AuthenticationError as e:
                symbol_logger.critical(f"{NEON_RED}CRITICAL Auth Error: {e}. Stopping.{RESET}")
                break
            except ccxt.ExchangeNotAvailable as e:
                symbol_logger.error(f"{NEON_RED}Exchange unavailable: {e}. Waiting 60s...{RESET}")
                time.sleep(60)
            except ccxt.OnMaintenance as e:
                symbol_logger.error(f"{NEON_RED}Exchange Maintenance: {e}. Waiting 5m...{RESET}")
                time.sleep(300)
            except Exception as loop_error:
                symbol_logger.error(f"{NEON_RED}Uncaught loop error: {loop_error}{RESET}", exc_info=True)
                time.sleep(15)
        ...
        init_logger.critical(f"{NEON_RED}Critical unhandled error: {critical_error}{RESET}", exc_info=True)
    finally:
