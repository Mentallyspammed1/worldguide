...
# Ensure the timezone name is valid according to the tz database (IANA standard)
try:
    TIMEZONE = ZoneInfo("America/Chicago") # e.g., "America/New_York", "Europe/London", "Asia/Tokyo"
except Exception:
    print(f"{NEON_RED}Warning: Could not load specified timezone 'America/Chicago'. Falling back to UTC.{RESET}")
...
def _ensure_config_keys(config: Dict[str, Any], default_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, default_value in default_config.items():
        ...
        # Optional: Handle type mismatches if needed, e.g., ensure numbers are numbers
        elif isinstance(default_value, (int, float)) and not isinstance(updated_config.get(key), (int, float)):
            try:
                # Attempt to convert loaded value to the default type
                updated_config[key] = type(default_value)(updated_config[key])
            except (ValueError, TypeError):
                print(f"{NEON_YELLOW}Warning: Config value for '{key}' ({updated_config.get(key)}) is not the expected type ({type(default_value)}). Using default value: {default_value}{RESET}")
                updated_config[key] = default_value
    ...
    try:
        ...
        try:
            ...
            if balance_decimal is not None:
                 ...
                 logger.warning(f"{NEON_YELLOW}Initial balance fetch returned None. Check API permissions/account type ({account_type_to_test}) if trading fails.{RESET}")
            ...
            if "account type" in str(balance_err).lower():
                logger.warning(f"{NEON_YELLOW} >> Hint: The error might indicate the wrong account type ('{account_type_to_test}') was specified for the balance check. Try changing between 'CONTRACT' and 'UNIFIED'.{RESET}")
...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        try:
            ...
            def parse_price(price_str: Any, field_name: str) -> Optional[Decimal]:
...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> Optional[str]:
        ...
        for col in result_df.columns:
            ...
            if col.lower() == base_name.lower():
                 self.logger.debug(f"Found column '{col}' for base '{base_name}' using exact match fallback search.")
                 return col
    ...
    def _update_latest_indicator_values(self):
        ...
        nan_placeholder = {k: np.nan for k in list(self.ta_column_names.keys()) + ["Close", "Volume", "High", "Low"]}
        if self.df.empty:
        ...
        try:
            ...
            if diff > 0:
                ...
                for level_pct in FIB_LEVELS:
                    ...
                    if min_tick:
                        # Round towards zero (truncate) for fib levels? Or round normally? Let's round normally (nearest).
                        # Or maybe round down for support, up for resistance? Let's stick to nearest for now.
                        level_price_quantized = (level_price / min_tick).quantize(Decimal('1'), rounding=ROUND_DOWN) * min_tick # Use ROUND_DOWN for consistency?
                    else:
    ...
    def get_price_precision(self) -> int:
        ...
        # Fallback: Infer from ccxt market precision.price if it's an integer (representing decimal places)
        try:
        ...
        try:
            ...
            if last_close and pd.notna(last_close) and last_close > 0:
                try:
                    ...
                    if '.' in s_close:
                        precision = len(s_close.split('.')[-1].rstrip('0')) # Count digits after removing trailing zeros
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            # 1. Check market_info['precision']['price'] (ccxt standard, often the tick size as float/str)
            precision_info = self.market_info.get('precision', {})
            ...
            # 2. Check Bybit specific 'tickSize' in info (V5)
            ...
            if min_price_val is not None:
                try:
                    ...
                    if min_tick_from_limit > 0 and min_tick_from_limit < 1:
    ...
    def generate_trading_signal(
        ...
        if not self.indicator_values:
            ...
            return "HOLD"
        # Check if *any* score-contributing indicator value is non-NaN
            ...
            for key, weight_str in active_weights.items():
                try:
                    ...
                    if weight != 0 and self.config.get("indicators", {}).get(key, False):
                         # Map indicator key to indicator_values key(s)
                         # This mapping needs to be more robust based on how indicator_values keys are named
                         # Simple example: assume key maps directly or to capitalized version for price/vol
                         value_keys_to_check = [key.capitalize()] # e.g., 'Close'
                         ...
                         elif key == 'orderbook': value_keys_to_check = [] # Orderbook checked separately
                         ...
                         if key == 'orderbook' and orderbook_data:
                             core_indicators_present = True
                             break
        ...
        if not core_indicators_present:
             self.logger.warning(f"{NEON_YELLOW}Cannot generate signal for {self.symbol}: All score-contributing indicator values are NaN or orderbook data missing.{RESET}")
             return "HOLD"
        ...
        for indicator_key, enabled in self.config.get("indicators", {}).items():
            ...
            if hasattr(self, check_method_name) and callable(getattr(self, check_method_name)):
                ...
                try:
                    ...
                    if indicator_key == "orderbook":
                        if orderbook_data: # Only call if data exists
                            ...
                            # Only log if orderbook weight is non-zero
                            if weight != 0:
                ...
                if pd.notna(indicator_score):
                    try:
                        # Ensure score is a valid float before converting to Decimal
                        if not isinstance(indicator_score, (float, int)):
                            raise TypeError(f"Indicator score for {indicator_key} is not a float: {indicator_score} ({type(indicator_score)})")
    ...
    def _check_momentum(self) -> float:
        ...
        # Avoid division by zero if momentum is exactly zero
    ...
    def _check_stoch_rsi(self) -> float:
        ...
        try:
            ...
            if oversold >= overbought:
                self.logger.warning(f"Stoch RSI oversold threshold ({oversold}) >= overbought ({overbought}). Check config.")
                # Set reasonable defaults if config is bad
                oversold = 25
                overbought = 75
        except ValueError:
            self.logger.error(f"Invalid Stoch RSI threshold format in config. Using defaults (25/75).")
            oversold = 25
            overbought = 75
        ...
        if oversold <= k <= overbought: # Inside normal range
            ...
            if range_width > 0:
                 mid_point = oversold + range_width / 2
                 # Avoid division by zero if range_width/2 is zero (though checked above)
                 half_range_width = range_width / 2
                 if half_range_width > 0:
                     mid_range_score = (k - mid_point) / half_range_width # Scales -1 to 1 within the range
    ...
    def _check_rsi(self) -> float:
        ...
        if lower_mid <= rsi <= upper_mid:
            # Linear scale from +0.5 (at lower_mid) to -0.5 (at upper_mid)
            mid_range = upper_mid - lower_mid
            if mid_range > 0:
                return 0.5 - (rsi - lower_mid) * (1.0 / mid_range)
    ...
    def _check_cci(self) -> float:
        ...
        if cci <= strong_oversold: return 1.0 # Strong Oversold -> Bullish
        if cci >= strong_overbought: return -1.0 # Strong Overbought -> Bearish
        if cci < mod_oversold: return 0.6 # Moderately Oversold
        if cci > mod_overbought: return -0.6 # Moderately Overbought
        # Trend confirmation near zero line - scale based on value
        if mod_oversold <= cci <= mod_overbought:
            # Linear scale from +0.6 (at mod_oversold) to -0.6 (at mod_overbought)
            mid_range = mod_overbought - mod_oversold
            if mid_range > 0:
                 # Scale from -0.6 to +0.6 (reversed from RSI example)
                 # Score = -0.6 + (CCI - LowerBound) * (TotalScoreRange / ValueRange)
                 return -0.6 + (cci - mod_oversold) * (1.2 / mid_range)
    ...
    def _check_wr(self) -> float: # Williams %R
        ...
        if wr <= oversold: return 1.0 # Oversold -> Bullish
        if wr >= overbought: return -1.0 # Overbought -> Bearish
        # Scale linearly in the middle range (-80 to -20)
        if oversold < wr < overbought:
            # Scale from +1.0 (at -80) to -1.0 (at -20)
            mid_range = overbought - oversold # Should be 60
            if mid_range > 0:
                return 1.0 - (wr - oversold) * (2.0 / mid_range)
    ...
    def _check_mfi(self) -> float:
        ...
        if mfi <= oversold: return 1.0 # Oversold -> Bullish potential
        if mfi >= overbought: return -1.0 # Overbought -> Bearish potential
        if mfi < lower_mid: return 0.4 # Leaning oversold/accumulation
        if mfi > upper_mid: return -0.4 # Leaning overbought/distribution
        # Scale linearly in the middle range (lower_mid to upper_mid)
        if lower_mid <= mfi <= upper_mid:
            # Scale from +0.4 (at lower_mid) to -0.4 (at upper_mid)
            mid_range = upper_mid - lower_mid
            if mid_range > 0:
                 return 0.4 - (mfi - lower_mid) * (0.8 / mid_range)
            else: # Avoid division by zero
                 return 0.0
        return 0.0 # Fallback
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if last_close > bb_middle:
            ...
            else: # If middle == upper, price is above middle => price is above upper (handled above) or exactly on band
                return 0.0 # Or perhaps slightly bullish? Let's stick to 0.0
            ...
            if lower_range > 0:
                ...
                return max(-0.5, min(0.5, score)) # Clamp result
            else: # If middle == lower, price is below middle => price is below lower (handled above) or exactly on band
                return 0.0
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict], current_price: Decimal) -> float:
        ...
        try:
            ...
            # Clamp score just in case of floating point issues
        ...
        try:
            ...
            for acc_type in account_types_to_try:
                try:
                    ...
                    # Add 'accountType' for V5 clarity, although 'type' might work too
                    ...
                    if currency in balance_info:
                        ...
                        # Fallback: Check if 'info' structure has the currency data (Bybit V5 case)
                            ...
                            if isinstance(balance_list, list):
                                ...
                                for account in balance_list:
                                    # Match the accountType we queried for
                        ...
                        if found_in_nested:
                            break # Break outer loop if found in nested structure
                        else:
                    ...
                    if balance_info is None: # If not found in this type, try next
            ...
            if not balance_info:
                ...
                try:
                    ...
                    # Still raise to trigger retry logic below if applicable
                    raise e
            ...
            # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
                ...
                if 'params' in balance_info and balance_info.get('params',{}).get('accountType') in account_types_to_try:
                     ...
                     lg.debug("Default fetch balance used, will check all account types in response.")
                        ...
                        if isinstance(coin_list, list):
                            for coin_data in coin_list:
                                if coin_data.get('coin') == currency:
                                    # Prefer Bybit V5 'availableToWithdraw' or 'availableBalance' (more accurate than walletBalance for trading)
                                    free = coin_data.get('availableBalance') # Bybit's preferred field for available trading balance
                                    if free is None: free = coin_data.get('availableToWithdraw') # Alternative 'free' balance
                                    ...
                                    if free is not None:
                                        ...
                                        lg.debug(f"Found balance via Bybit V5 nested structure: {available_balance_str} {currency} (Field Used: {'availableBalance' if coin_data.get('availableBalance') else 'availableToWithdraw' if coin_data.get('availableToWithdraw') else 'walletBalance'}, Account: {current_account_type or 'N/A'})")
            ...
            if available_balance_str is None:
                ...
                if total_balance is not None:
                    ...
                    return None # Failed to find balance
...
def get_market_info(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        ...
        if market:
            ...
            if market.get('precision', {}).get('price') is None or market.get('precision', {}).get('tick') is None:
                 ...
                 analyzer_temp = TradingAnalyzer(dummy_df, lg, {}, market) # Pass minimal config
                 ...
                 if min_tick > 0:
                     ...
                     # Store tick size as string for consistency with potential API responses
                     market['precision']['tick'] = str(min_tick)
                     # If precision.price wasn't set, use tick size
                     if market['precision'].get('price') is None:
                         market['precision']['price'] = str(min_tick)
                     lg.debug(f"Added/updated derived min tick size {min_tick} (precision {price_prec_digits} digits) to market info precision.")

            # Ensure limits/precision values are usable numbers if present
            for limit_type in ['amount', 'cost', 'price']:
                if limit_type in market.get('limits', {}):
                    for bound in ['min', 'max']:
                        val = market['limits'][limit_type].get(bound)
                        if val is not None:
                            try: market['limits'][limit_type][bound] = Decimal(str(val))
                            except (InvalidOperation, TypeError):
                                lg.warning(f"Could not parse market limit {limit_type}.{bound} ('{val}') as Decimal.")
                                market['limits'][limit_type][bound] = None # Invalidate bad limit

            for prec_type in ['amount', 'price', 'cost', 'tick']:
                 if prec_type in market.get('precision', {}):
                      val = market['precision'][prec_type]
                      if val is not None:
                           try:
                               # Store precision values (like tick size or step size) as Decimal
                               market['precision'][prec_type] = Decimal(str(val))
                           except (InvalidOperation, TypeError):
                                lg.warning(f"Could not parse market precision {prec_type} ('{val}') as Decimal.")
            ...
            lg.debug(
                ...
                f"Contract Size: {market.get('contractSize', 'N/A')}, Active: {market.get('active')}"
            )
...
def calculate_position_size(
    ...
    try:
        ...
        if market_info.get('linear', True) or not is_contract: # Treat spot as linear type
            ...
            contract_value_quote = contract_size # Assumed value of 1 contract in QUOTE currency
            if entry_price > 0:
        ...
        amount_step_size = precision.get('amount')

        # Get min/max amount limits (size limits, should be Decimal)
        min_amount = amount_limits.get('min', Decimal('0'))
        max_amount = amount_limits.get('max', Decimal('inf'))

        # Get min/max cost limits (value limits in quote currency, should be Decimal)
        min_cost = cost_limits.get('min', Decimal('0'))
        max_cost = cost_limits.get('max', Decimal('inf'))

        # Ensure limits are valid Decimals
        if min_amount is None: min_amount = Decimal('0')
        if max_amount is None: max_amount = Decimal('inf')
        ...
        if amount_step_size is not None and isinstance(amount_step_size, Decimal) and amount_step_size > 0:
            try:
                ...
                final_size = adjusted_size # Fallback
        else:
        ...
        if final_size < min_amount:
            ...
            if not (final_size >= min_amount or math.isclose(float(final_size), float(min_amount), rel_tol=1e-9)):
        ...
        if min_cost > 0 and final_cost < min_cost:
             ...
             # Compare Decimals directly
            if not (final_cost >= min_cost or math.isclose(float(final_cost), float(min_cost), rel_tol=1e-6)):
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            ...
            if 'bybit' in exchange.id.lower():
                 try:
                     ...
                     lg.warning(f"Cannot fetch market info for {symbol} within get_open_position. Assuming 'linear'.")
                     market = {'linear': True, 'symbol': symbol} # Assume linear if symbol fails temporarily
                 except Exception as e:
                     lg.warning(f"Error fetching market info for {symbol} within get_open_position: {e}. Assuming 'linear'.")
                     market = {'linear': True, 'symbol': symbol} # Assume linear on other errors

                 # Determine category based on market info or assumption
                 category = 'linear' if market and market.get('linear', True) else 'inverse'
                 params['category'] = category
                 # Use the symbol passed to the function for the API call
                 params['symbol'] = symbol
                 lg.debug(f"Using params for fetch_positions: {params}")
                 # Fetch positions for the specific symbol(s) using params
                 positions = exchange.fetch_positions(symbols=[symbol], params=params)

            else:
                 # For other exchanges, try fetching single symbol without specific params first
            ...
            # Bybit V5 might use specific codes or messages
            err_str = str(e).lower()
            bybit_code = getattr(e, 'code', None)
            no_pos_codes_v5 = [110025, 110021] # Position idx not match / Position is closed or not exist
            not_found_msgs = ["symbol not found", "instrument not found", "position does not exist"]

            if any(msg in err_str for msg in not_found_msgs) or bybit_code in no_pos_codes_v5:
        ...
        for pos in positions:
            ...
            # Bybit V5 uses 'size' in info dict
            elif pos.get('info', {}).get('size') is not None: pos_size_str = str(pos['info']['size'])
            # Binance uses 'positionAmt' in info dict
            elif pos.get('info', {}).get('positionAmt') is not None: pos_size_str = str(pos['info']['positionAmt'])
        ...
        if active_position:
            ...
            if side not in ['long', 'short']:
                ...
                # Note: Binance uses signed 'positionAmt', Bybit uses unsigned 'size' + 'side' field
                elif info_side == 'None' and 'positionAmt' in active_position.get('info',{}): # Check Binance style signed size
                    try:
                        signed_size = Decimal(str(active_position['info']['positionAmt']))
                        if signed_size >= min_size_threshold: side = 'long'
                        elif signed_size <= -min_size_threshold: side = 'short'
                    except (InvalidOperation, ValueError, TypeError): pass
                # Fallback using unsigned size (less reliable if side field missing)
                elif size_decimal >= min_size_threshold:
                    side = 'long'
                    lg.debug(f"Inferred side as 'long' from positive size {size_decimal} for {symbol} (may be inaccurate if exchange uses side field).")
                elif size_decimal <= -min_size_threshold: # Should only happen if size field was signed
                    side = 'short'
                    ...
                    lg.warning(f"Position size {size_decimal} is close to zero or side ambiguous, could not determine side for {symbol}. Treating as no position.")
            ...
            def get_valid_decimal_price_from_info(key: str) -> Optional[Decimal]:
                ...
                # Bybit uses '0' or '0.0' or '' for inactive, check non-empty and not just '0' variants
                if val_str and str(val_str).strip() and str(val_str).strip() != '0' and str(val_str).strip() != '0.0':
                    try:
            ...
            def format_log_value(key: str, value: Union[str, float, Decimal, None], precision: int = 6) -> str:
                ...
                try:
                    # Handle potential scientific notation in strings from API
                    str_value = format(Decimal(str(value)), 'f') if isinstance(value, (float, str)) else str(value)
                    d_value = Decimal(str_value)

                    # Allow zero only for specific fields like TSL distance/activation if inactive
                    is_tsl_field = 'trailingstop' in key.lower() or 'tslactivation' in key.lower()

                    if d_value > 0:
                         return f"{d_value:.{precision}f}"
                    elif d_value == 0 and is_tsl_field:
                        ...
                        if 'pnl' in key.lower():
                             return f"{d_value:.{precision}f}"
                        elif key.lower() in ['size', 'contracts', 'positionamt']:
            ...
            log_precision = 8 # Default precision if market info fails
            amount_precision = 8 # Default amount precision
            if market:
                try:
                    price_prec_val = market.get('precision', {}).get('price') # This is often tick size
                    if price_prec_val is not None:
                         log_precision = abs(Decimal(str(price_prec_val)).normalize().as_tuple().exponent)
                except Exception: pass
                try:
                    # Handle amount precision being Decimal (step size)
                    amount_precision_val = market.get('precision', {}).get('amount')
                    if isinstance(amount_precision_val, Decimal) and amount_precision_val > 0:
                        amount_precision = abs(amount_precision_val.normalize().as_tuple().exponent)
                    elif isinstance(amount_precision_val, int): # Handle case where it's digits
        ...
        # Should this be retried? For now, return None.
    except ccxt.ExchangeError as e:
    ...
    try:
        ...
        if 'bybit' in exchange.id.lower():
            ...
            try:
                ...
                # Use a simplified fetch here to avoid recursion depth if get_open_position fails
                pos_data = exchange.fetch_positions(symbols=[symbol], params={'category': 'linear' if market_info.get('linear', True) else 'inverse'})
                if pos_data:
                    pos_info = pos_data[0] # Assume first entry is relevant
...
def place_trade(
    ...
    params = {
        ...
        'positionIdx': 0, # 0 for one-way mode (most common setup)
        'reduceOnly': reduce_only, # Use the passed flag
        # 'timeInForce': 'IOC', # Optional: Market orders usually fill quickly anyway (ImmediateOrCancel)
    ...
    # --- Error Handling ---
        ...
        elif bybit_code == 110071 and "minimum order value" in str(e).lower(): # Order value less than minimum
             lg.error(f"{NEON_YELLOW} >> Hint (Code {bybit_code}): Order cost (Size * Price / Leverage) is below the minimum required value for {symbol}.{RESET}")
        ...
        elif bybit_code == 110043: # Set order tpsl/sl/tp failed (less likely for market order placement itself, but possible)
            lg.error(f"{NEON_YELLOW} >> Hint (Code {bybit_code}): Problem related to SL/TP settings? Ensure they are not sent with market order if not supported.{RESET}")
...
def _set_position_protection(
    ...
    # Check if values are valid Decimals and non-negative (or zero for activation price/distance if canceling), or '0' string
    def is_valid_or_cancel(val: Optional[Union[Decimal, str]]) -> bool:
        if val is None: return False # No intent to set/cancel this parameter
        if isinstance(val, str) and val.strip() == '0': return True # Explicit cancel signal
        if isinstance(val, Decimal):
            return val >= 0 # Valid price/distance must be non-negative (0 is ok for TSL activation/distance)
        # Handle numeric strings that aren't just '0'
        if isinstance(val, str):
            try:
                return Decimal(val.strip()) >= 0
            except (InvalidOperation, ValueError, TypeError):
    ...
    has_tsl_intent = trailing_stop_distance is not None or tsl_activation_price is not None # Intent if either is provided

    is_sl_valid_input = has_sl_intent and is_valid_or_cancel(stop_loss_price)
    is_tp_valid_input = has_tp_intent and is_valid_or_cancel(take_profit_price)
    # TSL requires both distance and activation to be valid (or explicitly '0') if setting,
    # but can cancel individually if needed (though API might require both)
    is_tsl_dist_valid = is_valid_or_cancel(trailing_stop_distance)
    is_tsl_act_valid = is_valid_or_cancel(tsl_activation_price)
    ...
    if not is_sl_valid_input and not is_tp_valid_input and not is_tsl_valid_input:
        # Check if only one part of TSL was provided - this is ambiguous
        if has_tsl_intent and (not is_tsl_dist_valid or not is_tsl_act_valid):
             lg.warning(f"TSL intent detected but requires both valid distance ('{trailing_stop_distance}') and activation ('{tsl_activation_price}'). Skipping TSL.")
        else:
             lg.info(f"No valid protection parameters (SL, TP, TSL >= 0 or '0' for cancel) provided for {symbol}. No protection set/modified.")
        # Consider this success as no API call was intended/needed, or input was invalid for setting
    ...
    try:
        ...
        if pos_idx_val is not None:
            ...
            if position_idx not in [0, 1, 2]: # Validate index (0=One-Way, 1=Buy Hedge, 2=Sell Hedge)
                 lg.warning(f"Invalid positionIdx '{pos_idx_val}' found. Defaulting to 0 (One-Way).")
                 position_idx = 0
    except (ValueError, TypeError):
    ...
    try:
        ...
        def format_api_value(value: Optional[Union[Decimal, str]], precision_type: str = 'price') -> Optional[str]:
            if value is None: return None # Not provided
            if isinstance(value, str) and value.strip() == '0': return '0' # Pass cancel signal directly

            try:
                val_dec = Decimal(str(value).strip())
                if val_dec < 0:
                    lg.warning(f"Invalid negative value '{val_dec}' provided for {precision_type}. Skipping.")
                    return None
                if val_dec == 0: return '0' # Treat Decimal 0 as cancel signal

                # Use appropriate formatting helper
                if precision_type == 'price':
                    return exchange.price_to_precision(symbol, float(val_dec))
                elif precision_type == 'amount': # Distance is also a price difference
                    return exchange.price_to_precision(symbol, float(val_dec)) # Use price precision for distance too
                else:
                     lg.warning(f"Unknown precision type '{precision_type}' for formatting. Using price.")
                     return exchange.price_to_precision(symbol, float(val_dec))
            except (InvalidOperation, ValueError, TypeError) as fmt_err:
                lg.warning(f"Could not format value '{value}' using exchange precision ({precision_type}): {fmt_err}. Skipping this parameter.")
                return None

        # Trailing Stop Loss (Set first, as it might override fixed SL)
        # Only add TSL params if a valid TSL action is intended
        if is_tsl_valid_input:
            formatted_tsl_distance = format_api_value(trailing_stop_distance, 'amount') # Distance uses amount/price precision
            formatted_activation_price = format_api_value(tsl_activation_price, 'price') # Activation is a price
            ...
            if formatted_tsl_distance is not None and formatted_activation_price is not None:
                ...
                # Mark TSL intent as failed if formatting failed
                is_tsl_valid_input = False


        # Fixed Stop Loss - Only add if SL intent exists AND TSL was NOT successfully added OR if TSL is being cancelled ('0')
        if is_sl_valid_input:
            # Check if TSL is active and not being cancelled
            ...
            if not tsl_is_active_and_not_cancelling:
                ...
                if formatted_sl is not None:
            ...
            elif has_sl_intent: # TSL is active, SL intent ignored
                 ...
                 is_sl_valid_input = False # Mark fixed SL as not set
    ...
    if not sl_added_to_params and not tp_added_to_params and not tsl_added_to_params:
        ...
        # Check original intent vs final valid state
        ...
        if original_intent_present and not valid_params_remain:
            return False # Intent was there, but all failed validation/formatting
        else:
            return True # Either no intent, or some valid intent remains even if not added (e.g., SL ignored due to TSL)
    ...
    try:
        ...
        if ret_code == 0:
            ...
            # Example message structure: "StopLoss:NotModified, TakeProfit:OK"
            # Check if all requested modifications were successful or "NotModified"
            success_msg = "OK"
            no_change_msg = "notmodified" # Bybit often uses this format
            failed = False
            log_details = []
            if sl_added_to_params and no_change_msg not in ret_msg.lower() and success_msg not in ret_msg: log_details.append(f"SL ({params.get('stopLoss')})")
            if tp_added_to_params and no_change_msg not in ret_msg.lower() and success_msg not in ret_msg: log_details.append(f"TP ({params.get('takeProfit')})")
            if tsl_added_to_params and no_change_msg not in ret_msg.lower() and success_msg not in ret_msg: log_details.append(f"TSL (Dist={params.get('trailingStop')}/Act={params.get('activePrice')})")

            if "not modified" in ret_msg.lower() or ret_msg == "OK": # Success or no change needed
                 if no_change_msg in ret_msg.lower().replace(" ",""):
                 ...
                 # Unexpected success message? Log it but treat as success.
                 lg.warning(f"Protection call returned success code 0 but unexpected message: '{ret_msg}'. Assuming success.")
                 return True
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        return _set_position_protection(
            ...
            logger=lg,
            stop_loss_price='0', # Explicitly set fixed SL to '0' (cancel) when setting TSL
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger) -> None:
        ...
        if signal in ["BUY", "SELL"]:
            ...
            if market_info.get('is_contract', False):
                try:
                    ...
                    lg.error(f"Invalid leverage format in config: {config.get('leverage', 1)}. Using 1x.")
                    leverage = 1
                    ...
                    try:
                        ...
                        if entry_price_actual_str and pos_size_actual_str:
                            try:
                                ...
                                # Use a small threshold for size validation based on step size
                                min_size_threshold = Decimal('1e-9')
                                if market_info:
                                    try:
                                        min_amt_step = market_info.get('precision', {}).get('amount')
                                        if isinstance(min_amt_step, Decimal) and min_amt_step > 0:
                                             min_size_threshold = min_amt_step
                                        else:
                                             min_amt_limit = market_info.get('limits', {}).get('amount', {}).get('min')
                                             if isinstance(min_amt_limit, Decimal) and min_amt_limit > 0:
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        sl_log_str = f"{current_sl_price_val:.{price_precision}f}" if current_sl_price_val and float(current_sl_price_val) > 0 else 'N/A'
        tp_log_str = f"{current_tp_price_val:.{price_precision}f}" if current_tp_price_val and float(current_tp_price_val) > 0 else 'N/A'
            ...
            if config.get("enable_break_even", False) and not is_tsl_active:
                ...
                try:
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        if current_sl_price_val is not None:
                            try:
                                sl_val = float(current_sl_price_val)
                                if sl_val > 0:
                                    current_sl_price_dec = Decimal(str(sl_val))
                        ...
                        if update_be:
                            ...
                            if current_tp_price_val is not None:
                                try:
                                    tp_val = float(current_tp_price_val)
                                    if tp_val > 0:
                                        current_tp_price_dec = Decimal(str(tp_val))
