...
class TradingAnalyzer:
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            price_precision_val = precision_info.get("tick", precision_info.get("price"))
...
def fetch_balance(exchange: ccxt.Exchange, currency: str, logger: logging.Logger) -> Decimal | None:
    ...
    try:
        ...
        # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
            ...
            if balance_info.get("params", {}).get("accountType") in account_types_to_try:
                ...
                try:
                    ...
                    if min_tick > 0:
                        if "precision" not in market:
                            market["precision"] = {}
                        market["precision"]["tick"] = float(min_tick)  # Store as float for consistency
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                params["symbol"] = market["id"]  # Use exchange ID for Bybit V5 symbol param
        ...
        for pos in positions:
            ...
            try:
                ...
                if abs(position_size) > min_size_threshold:
                    ...
                    lg.debug(f"Position entry size {position_size} is too small for {symbol}. Skipping.")
        ...
        if active_position:
            ...
            def format_log_value(key: str, value: str | float | Decimal | None, precision: int = 6) -> str:
                ...
                try:
                    ...
                    if price_prec_val is not None:
                        ...
                        if isinstance(price_prec_val, int):
                            ...
                            log_precision = abs(Decimal(str(price_prec_val)).normalize().as_tuple().exponent)
                except Exception:
                    pass
            ...
            tsl_dist_log = format_log_value(
                "trailingStopLossDistance", active_position.get("trailingStopLossDistance"), log_precision
            )
            tsl_act_log = format_log_value(
                "tslActivationPrice", active_position.get("tslActivationPrice"), log_precision
            )
...
def _set_position_protection(
    ...
    has_tsl_intent = trailing_stop_distance is not None and tsl_activation_price is not None
    ...
    is_tsl_valid = (
        has_tsl_intent and is_valid_or_cancel(trailing_stop_distance) and is_valid_or_cancel(tsl_activation_price)
    )
                            ...
                            try:
                                ...
                                if market_info:
                                    try:
                                        ...
                                        if min_amt is not None:
                                            min_size_threshold = Decimal(str(min_amt)) * Decimal(
                                                "0.1"
                                            )  # e.g., 10% of min size
                                    except Exception:
                                        pass
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        if be_stop_price is not None and be_stop_price > 0:  # Ensure target BE is valid
                            if current_sl_price_dec is None:  # No current SL, set BE SL
                                update_be = True
                                lg.info("Profit target hit. No current SL found. Setting BE SL.")
