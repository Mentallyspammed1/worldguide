...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        try:
            ...
            self.logger.warning(
                f"Invalid bollinger_bands_std_dev '{cfg.get('bollinger_bands_std_dev')}' in config. Using default {DEFAULT_BOLLINGER_BANDS_STD_DEV}."
            )
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            # Prefer 'tick' if available, otherwise 'price'
            price_precision_val = precision_info.get(
                "tick", precision_info.get("price")
            )
            ...
            if price_precision_val is not None:
                try:
                    ...
                    if tick_size > 0:
                        # self.logger.debug(f"Using tick size from precision.tick/price: {tick_size} for {self.symbol}")
                        return tick_size
                except Exception:
                    self.logger.debug(
                        f"Could not parse precision.tick/price '{price_precision_val}' as Decimal for {self.symbol}. Trying other methods."
                    )
            ...
            if min_price_val is not None:
                try:
                    ...
                    if min_tick_from_limit > 0:
                        self.logger.debug(
                            f"Using tick size from limits.price.min: {min_tick_from_limit} for {self.symbol}"
                        )
            ...
            if bybit_tick_size is not None:
                try:
                    ...
                    if tick_size_bybit > 0:
                        self.logger.debug(
                            f"Using tick size from market_info['info']['tickSize']: {tick_size_bybit} for {self.symbol}"
                        )
    ...
    def generate_trading_signal(
        ...
        core_indicators_present = any(
            pd.notna(v)
            for k, v in self.indicator_values.items()
            if k not in ["Close", "Volume", "High", "Low", "EMA_Short", "EMA_Long"]
        )  # Exclude EMAs if only used for exit
        if not core_indicators_present and not self.config.get("indicators", {}).get(
            "ema_alignment", False
        ):  # Only require core if EMA align not used for score
            self.logger.warning(
                f"{NEON_YELLOW}Cannot generate signal for {self.symbol}: All score-contributing indicator values are NaN.{RESET}"
            )
            return "HOLD"
...
def fetch_balance(
    ...
    try:
        ...
        for acc_type in account_types_to_try:
            try:
                ...
                if currency in balance_info:
                    lg.debug(
                        f"Found currency '{currency}' in balance structure using accountType '{acc_type}'."
                    )
                                ...
                                if account.get("accountType") == acc_type:
                                    ...
                                    if isinstance(coin_list, list):
                                        if any(
                                            ...
                                            lg.debug(
                                                f"Currency '{currency}' confirmed in nested V5 balance structure (accountType: {acc_type})."
                                            )
                    ...
                    lg.debug(
                        f"Currency '{currency}' found in balance structure (accountType '{acc_type}'), but missing 'free' or V5 nested data. Trying next type."
                    )
                            ...
                            if account.get("accountType") == acc_type:
                                ...
                                if isinstance(coin_list, list):
                                    if any(
                                        ...
                                        lg.debug(
                                            f"Found currency '{currency}' in nested V5 balance structure using accountType '{acc_type}'."
                                        )
                                        found_in_nested = True
                                        break  # Found in this account's coin list
                        if found_in_nested:
                            lg.debug(
                                f"Using balance_info from successful nested check (accountType: {acc_type})"
                            )
                            break  # Break outer loop too

                lg.debug(
                    f"Currency '{currency}' not found in balance structure using accountType '{acc_type}'. Trying next."
                )
        ...
        # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
            ...
            if (
                balance_info.get("params", {}).get("accountType")
                in account_types_to_try
            ):
                successful_acc_type = balance_info["params"]["accountType"]
            else:
                        ...
                        for coin_data in coin_list:
                            if coin_data.get("coin") == currency:
                                ...
                                if free is None:
                                    free = coin_data.get(
                                        "availableBalance"
                                    )  # Bybit's preferred field for V5 Contract/Unified
                ...
                try:
                    ...
                    if min_tick > 0:
                        if "precision" not in market:
                            market["precision"] = {}
                        market["precision"]["tick"] = float(
                            min_tick
                        )  # Store as float for consistency
                        lg.debug(
                            f"Added derived min tick size {min_tick} to market info precision."
                        )
                except ValueError as ve:  # Catch error if market_info was invalid
                    lg.warning(
                        f"Could not derive min tick size due to issue with market info: {ve}"
                    )
...
def calculate_position_size(
    ...
    lg = logger or logging.getLogger(__name__)
    if not market_info:
        lg.error("Position sizing failed: Missing market_info.")
        return None
    ...
    try:
        ...
        if (
            ...
            lg.warning(
                f"{NEON_YELLOW}Inverse contract detected for {symbol}. Sizing calculation assumes `contractSize` ({contract_size}) is the Quote value per contract. VERIFY THIS LOGIC FOR {exchange.id}!{RESET}"
            )
            ...
            if entry_price > 0:
                ...
                size_of_1_contract_in_base = contract_value_quote / entry_price
                risk_per_contract_quote = (
                    size_of_1_contract_in_base * sl_distance_per_unit
                )

                if risk_per_contract_quote > 0:
                    calculated_size = risk_amount_quote / risk_per_contract_quote
                    lg.debug(
                        f"  Inverse Sizing: Size1Base={size_of_1_contract_in_base}, RiskPerContQuote={risk_per_contract_quote}"
                    )
                    lg.debug(
        ...
        amount_precision_val = precision.get(
            "amount"
        )  # Can be int (decimals) or float (step size)
        ...
        try:
            ...
            lg.warning(
                f"{NEON_YELLOW}Could not use exchange.amount_to_precision for {symbol} ({fmt_err}). Using manual rounding (ROUND_DOWN) based on precision.{RESET}"
            )
            ...
            if amount_precision_val is not None:
                ...
                ):  # Assume it's step size
                    try:
                        step_size = Decimal(str(amount_precision_val))
                        if step_size <= 0:
                            step_size = None  # Invalidate non-positive step size
                        else:
                            # Infer decimals from step size for quantization
                            num_decimals = abs(
                                step_size.normalize().as_tuple().exponent
                            )
                    except Exception:
                ...
                if step_size is not None and step_size > 0:
                    ...
                    lg.info(
                        f"Applied manual amount step size ({step_size}), rounded down: {adjusted_size:.8f} -> {final_size} {size_unit}"
                    )
                    ...
                    final_size = adjusted_size.quantize(
                        rounding_factor, rounding=ROUND_DOWN
                    )
                    lg.info(
                        f"Applied manual amount precision ({num_decimals} decimals), rounded down: {adjusted_size:.8f} -> {final_size} {size_unit}"
                    )
                else:
                    lg.warning(
...
def get_open_position(
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                try:
                    market = get_market_info(
                        exchange, symbol, lg
                    )  # Use helper to get market info
                    if not market:
                        raise ValueError("Failed to get market info")
                except Exception as e:
                    lg.warning(
                        f"Error fetching market info for {symbol} within get_open_position: {e}. Assuming 'linear'."
                    )
                    market = {
                        "linear": True,
                        "symbol": symbol,
                        "id": symbol.replace("/", ""),
                    }  # Create dummy market info

                category = (
                    "linear" if market and market.get("linear", True) else "inverse"
                )
                params["category"] = category
                params["symbol"] = market[
                    "id"
                ]  # Use exchange ID for Bybit V5 symbol param
            ...
            # Providing both might be redundant but safer.
            positions = exchange.fetch_positions(symbols=[symbol], params=params)

        except ccxt.ArgumentsRequired:
            lg.warning(
                f"Exchange {exchange.id} requires fetching all positions, then filtering. Fetching all..."
            )
            fetch_all = True
        except ccxt.ExchangeError as e:
            ...
            if (
                ...
                lg.warning(
                    f"Symbol {symbol} not found when fetching position: {e}. Assuming no position."
                )
                return None
            ...
            if bybit_code in no_pos_codes_v5:
                lg.info(
                    f"No position found for {symbol} (Exchange error code: {bybit_code} - {e})."
                )
                return None
        ...
        for pos in positions:
            ...
            try:
                ...
                if abs(position_size) > min_size_threshold:
                    ...
                    lg.debug(
                        f"Position entry size {position_size} is too small for {symbol}. Skipping."
                    )
        ...
        if active_position:
            ...
            try:
                ...
                size_str_for_side = active_position.get(
                    "contracts", active_position.get("info", {}).get("size", "0")
                )
            ...
            if side not in ["long", "short"]:
                ...
                # If still not determined, use size sign (less reliable for V5)
                    ...
                    side = "long"
                    lg.debug(
                        f"Inferred side as 'long' from positive size {size_decimal} for {symbol}."
                    )
                elif size_decimal < -size_threshold:  # Use threshold again
                    side = "short"
                    lg.debug(
                        f"Inferred side as 'short' from negative size {size_decimal} for {symbol}."
                    )
                    ...
                    if "contracts" in active_position:
                        active_position["contracts"] = abs(
                            size_decimal
                        )  # Standardize 'contracts' field to positive
                else:
            ...
            def get_valid_decimal_price_from_info(key: str) -> Decimal | None:
                val_str = info_dict.get(key)
                # Bybit uses '0' or '0.0' for inactive, check non-empty and not just '0' variants
                if (
                    val_str
                    and str(val_str).strip()
                    and Decimal(str(val_str).strip()) != 0
                ):
                    try:
            ...
            if active_position.get("stopLossPrice") is None:
                ...
                if sl_val_dec:
                    active_position["stopLossPrice"] = float(
                        sl_val_dec
                    )  # CCXT expects float
            ...
            if active_position.get("takeProfitPrice") is None:
                ...
                if tp_val_dec:
                    active_position["takeProfitPrice"] = float(
                        tp_val_dec
                    )  # CCXT expects float
            ...
            active_position["trailingStopLossDistance"] = (
                tsl_dist_str  # Store raw string
            )
            ...
            try:
                ...
                active_position["trailingStopLossDistanceDecimal"] = max(
                    Decimal(0), tsl_dist_val
                )
            except Exception:
            ...
            try:
                ...
                active_position["tslActivationPriceDecimal"] = max(
                    Decimal(0), tsl_act_val
                )
            except Exception:
            ...
            def format_log_value(
                key: str, value: str | float | Decimal | None, precision: int = 6
            ) -> str:
                ...
                try:
                    ...
                    if d_value > 0:
                        ...
                        "trailingstop" in key.lower() or "tslactivation" in key.lower()
                    ):
            ...
            if market:
                try:
                    ...
                    if price_prec_val is not None:
                        ...
                        if isinstance(price_prec_val, int):
                            log_precision = price_prec_val
                        else:  # Assume float/string tick size
                            log_precision = abs(
                                Decimal(str(price_prec_val))
                                .normalize()
                                .as_tuple()
                                .exponent
                            )
                    ...
                    if amount_precision_val is not None:
                        if isinstance(amount_precision_val, int):
                            ...
                            amount_precision = abs(
                                Decimal(str(amount_precision_val))
                                .normalize()
                                .as_tuple()
                                .exponent
                            )
            ...
            liq_price = format_log_value(
                "liquidationPrice",
                active_position.get("liquidationPrice"),
                log_precision,
            )
            ...
            sl_price = format_log_value(
                "stopLossPrice", active_position.get("stopLossPrice"), log_precision
            )
            tp_price = format_log_value(
                "takeProfitPrice", active_position.get("takeProfitPrice"), log_precision
            )
            ...
            tsl_dist_log = format_log_value(
                ...
                active_position.get("trailingStopLossDistance"),
                log_precision,
            )
            tsl_act_log = format_log_value(
                "tslActivationPrice",
                active_position.get("tslActivationPrice"),
                log_precision,
            )
    ...
    try:
        ...
        if "bybit" in exchange.id.lower():
            ...
            try:
                ...
                pos_info = get_open_position(
                    exchange, symbol, lg
                )  # Use our enhanced function
                if pos_info:
                    margin_mode = pos_info.get("marginMode")  # Standard ccxt field
                    if margin_mode is None:
                        trade_mode_val = pos_info.get("info", {}).get(
                            "tradeMode"
                        )  # Bybit V5 'tradeMode' (0: Cross, 1: Isolated)
                        if trade_mode_val == 0:
                            margin_mode = "cross"
                        elif trade_mode_val == 1:
                            margin_mode = "isolated"
                    lg.debug(
                        f"Detected margin mode from existing position for {symbol}: {margin_mode}"
                    )
                else:
                    ...
                    lg.debug(
                        f"No existing position for {symbol}, cannot detect margin mode automatically. Assuming Isolated/Cross based on account default."
                    )

            except Exception as e:
                lg.warning(
                    f"Could not reliably fetch margin mode for {symbol} before setting leverage: {e}. Proceeding cautiously."
                )
        ...
        if response is not None:
            if isinstance(response, dict):
                ...
                )  # Check top level or nested info
                if ret_code == 0:
                    lg.debug(
...
def place_trade(
    ...
    try:
        ...
        lg.debug(
            f"Raw order response ({symbol} {side} reduce={reduce_only}): {order}"
    ...
    # --- Error Handling ---
        ...
        if (
            ...
            bybit_code == 110025 and reduce_only
        ):  # Position not found when trying to close
            lg.warning(
                f"{NEON_YELLOW} >> Hint (Code 110025): Position not found when attempting to close (reduceOnly=True). Might have already been closed by SL/TP or manually.{RESET}"
            )
...
def _set_position_protection(
    ...
    is_tsl_valid = (
        ...
        and is_valid_or_cancel(tsl_activation_price)
    )
    ...
    try:
        ...
        def format_price(price_val: Decimal | str | None) -> str | None:
            if price_val is None:
                return None
            if isinstance(price_val, str) and price_val == "0":
            ...
            if isinstance(price_val, Decimal):
                ...
                try:
                    ...
                    lg.warning(
                        f"Could not format price {price_val} using exchange.price_to_precision: {fmt_err}. Skipping this parameter."
                    )
                    return None
        ...
        # Handles Decimal input and '0' string for cancellation
        def format_distance(dist_val: Decimal | str | None) -> str | None:
            if dist_val is None:
                return None
            if isinstance(dist_val, str) and dist_val == "0":
                return "0"  # Pass cancel signal directly
            if isinstance(dist_val, Decimal):
                ...
                try:
                    ...
                    formatted = exchange.price_to_precision(
                        symbol, float(abs(dist_val))
                    )
                    lg.debug(f"Formatted distance {dist_val} -> {formatted}")
                    return formatted
                except Exception as fmt_err:
                    lg.warning(
                        f"Could not format distance {dist_val} using exchange.price_to_precision: {fmt_err}. Skipping this parameter."
                    )
                    return None
            return None  # Invalid type
        ...
        if is_sl_valid and (
            ...
            has_sl_intent and tsl_added_to_params and params.get("trailingStop") != "0"
        ):
            lg.warning(
    ...
    try:
        ...
        if ret_code == 0:
            ...
            if ret_code == 110043:  # Set tpsl failed
                ...
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110025): Position might have closed or changed size unexpectedly. Or `positionIdx` ({params.get('positionIdx')}) mismatch?{RESET}"
                )
                ...
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110013): Check parameters: Is SL/TP/TSL value valid (non-zero, respects tick size)? Is `activePrice` valid for TSL? Are SL/TP on the wrong side of entry?{RESET}"
                )
        ...
        ccxt.ExchangeError
    ) as e:  # Catch potential errors from the call itself if not parsed above
        lg.error(
        ...
        lg.error(
            f"{NEON_RED}CCXT BaseError setting protection for {symbol}: {e}{RESET}"
        )
    except KeyError as e:
        lg.error(
...
def analyze_and_trade_symbol(
    ...
    try:
        ...
        lg.error(
            f"{NEON_RED}Failed to initialize TradingAnalyzer for {symbol}: {e_analyzer}. Skipping analysis cycle.{RESET}"
        )
        return
                ...
                if confirmed_position:
                    ...
                    try:
                        ...
                        if entry_price_actual_str and pos_size_actual_str:
                                    ...
                                    try:
                                        ...
                                        if min_amt is not None:
                                            min_size_threshold = Decimal(
                                                str(min_amt)
                                            ) * Decimal("0.1")  # e.g., 10% of min size
                                    except Exception:
                                        pass

                                if (
                                    entry_price_actual > 0
                                    and abs(pos_size_actual) >= min_size_threshold
                                ):
                                    valid_entry = True
                                else:
                                    lg.error(
                                        f"Confirmed position has invalid entry price ({entry_price_actual}) or size ({pos_size_actual} < {min_size_threshold})."
                                    )
                            ...
                            lg.error(
                                "Confirmed position missing entryPrice or size information."
                            )
        ...
        if not exit_signal_triggered and config.get("enable_ma_cross_exit", False):
            ...
            if pd.notna(ema_short) and pd.notna(ema_long):
                ...
                if pos_side == "long" and ema_short < ema_long:
                    ma_cross_exit = True
                    lg.warning(
                        f"{NEON_YELLOW}*** MA CROSS EXIT (Bearish): Short EMA ({ema_short:.{price_precision}f}) crossed below Long EMA ({ema_long:.{price_precision}f}). Closing LONG position. ***{RESET}"
                    )
                elif pos_side == "short" and ema_short > ema_long:
                    ma_cross_exit = True
                    lg.warning(
                        f"{NEON_YELLOW}*** MA CROSS EXIT (Bullish): Short EMA ({ema_short:.{price_precision}f}) crossed above Long EMA ({ema_long:.{price_precision}f}). Closing SHORT position. ***{RESET}"
                    )
            else:
            ...
            try:
                ...
                if close_order:
                    ...
                    if (
                        ...
                        lg.info(
                            f"{NEON_GREEN}Position CLOSE for {symbol} confirmed (was likely already closed).{RESET}"
                        )
                    else:
                        lg.info(
                ...
                try:
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        if (
                            ...
                            and float(current_sl_price_val) > 0
                        ):
                            try:
                                current_sl_price_dec_temp = Decimal(
                                    str(current_sl_price_val)
                                )
                        ...
                        if (
                            ...
                            if current_sl_price_dec is None:  # No current SL, set BE SL
                                update_be = True
                                lg.info(
                                    "Profit target hit. No current SL found. Setting BE SL."
                                )
                            elif (
                                pos_side == "long"
                                ...
                                update_be = True
                                lg.info(
                                    f"Profit target hit. Current SL {current_sl_price_dec} < Target BE SL {be_stop_price}. Moving SL to BE."
                                )
                            elif (
                                pos_side == "short"
                                and be_stop_price < current_sl_price_dec
                        ...
                        if update_be:
                            ...
                            if (
                                ...
                                and float(current_tp_price_val) > 0
                            ):
                                try:
                                    current_tp_price_dec_temp = Decimal(
                                        str(current_tp_price_val)
                                    )
...
def main() -> None:
    ...
    init_logger.info(
        f"--- Starting LiveBot 7.1 ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---"
    )
    ...
    if CONFIG.get("enable_trading"):
        ...
        init_logger.warning(
            f"Key Settings: Risk Per Trade: {CONFIG.get('risk_per_trade', 0) * 100:.2f}%, Leverage: {CONFIG.get('leverage', 0)}x, TSL: {'Enabled' if CONFIG.get('enable_trailing_stop') else 'Disabled'}, BE: {'Enabled' if CONFIG.get('enable_break_even') else 'Disabled'}, MA Exit: {'Enabled' if CONFIG.get('enable_ma_cross_exit') else 'Disabled'}"
        )
    ...
    symbol_logger.info(
        f"Using Configuration: Risk={CONFIG['risk_per_trade']:.2%}, Lev={CONFIG['leverage']}x, TSL={'ON' if CONFIG['enable_trailing_stop'] else 'OFF'}, BE={'ON' if CONFIG['enable_break_even'] else 'OFF'}, MA Exit={'ON' if CONFIG['enable_ma_cross_exit'] else 'OFF'}, Trading={'ENABLED' if CONFIG['enable_trading'] else 'DISABLED'}"
    )
    ...
    try:
        ...
        shutdown_msg = f"--- LiveBot 7.1 for {target_symbol if target_symbol else 'N/A'} Stopping ---"
