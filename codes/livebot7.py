    ...
    try:
        ...
        if market:
            ...
            if market.get("precision", {}).get("tick") is None:
                analyzer_temp = TradingAnalyzer(pd.DataFrame(), lg, {}, market)  # Temp instance for helper
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> dict | None:
    ...
    try:
        ...
        if active_position:
            ...
            log_precision = market["precision"]["price"] if market and market.get("precision", {}).get("price") else 6
            # Handle amount precision being int (decimals) or float (step size)
            amount_precision_val = market.get("precision", {}).get("amount") if market else None
                        ...
                        if current_sl_price_val is not None and float(current_sl_price_val) > 0:
                            try:
                                ...
                                if current_sl_price_dec <= 0:
                                    current_sl_price_dec = None  # Treat invalid SL as None
                            except Exception:
                                lg.warning(
