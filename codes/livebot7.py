...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        expected_patterns = {
            ...
            "BB_Lower": [
                f"BBL_{cfg.get('bollinger_bands_period', DEFAULT_BOLLINGER_BANDS_PERIOD)}_{float(cfg.get('bollinger_bands_std_dev', DEFAULT_BOLLINGER_BANDS_STD_DEV)):.1f}"
            ],
            "BB_Middle": [
                f"BBM_{cfg.get('bollinger_bands_period', DEFAULT_BOLLINGER_BANDS_PERIOD)}_{float(cfg.get('bollinger_bands_std_dev', DEFAULT_BOLLINGER_BANDS_STD_DEV)):.1f}"
            ],
            "BB_Upper": [
                f"BBU_{cfg.get('bollinger_bands_period', DEFAULT_BOLLINGER_BANDS_PERIOD)}_{float(cfg.get('bollinger_bands_std_dev', DEFAULT_BOLLINGER_BANDS_STD_DEV)):.1f}"
            ],
            "Volume_MA": [
...
def fetch_balance(
    ...
    try:
        ...
        for acc_type in account_types_to_try:
            try:
                lg.debug(
                    f"Fetching balance using params={{'type': '{acc_type}'}} for {currency}..."
                )
        ...
        if market:
            ...
            if market.get("precision", {}).get("tick") is None:
                analyzer_temp = TradingAnalyzer(
                    pd.DataFrame(), lg, {}, market
                )  # Temp instance for helper
                ...
                if min_tick > 0:
                    ...
                    market["precision"]["tick"] = float(
                        min_tick
                    )  # Store as float for consistency? Or keep Decimal? Let's use float for ccxt style.
                    lg.debug(
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                try:
                    ...
                    lg.warning(
                        f"Cannot fetch market info for {symbol} within get_open_position. Assuming 'linear'."
                    )
                    market = {
                        "linear": True
                    }  # Assume linear if symbol fails temporarily
                except Exception as e:
                    lg.warning(
                        f"Error fetching market info for {symbol} within get_open_position: {e}. Assuming 'linear'."
                    )
                    market = {"linear": True}  # Assume linear on other errors
            ...
            no_pos_codes_v5 = [
                110025,
                110021,
            ]  # Position idx not match / Position is closed or not exist
        ...
        if active_position:
            ...
            log_precision = (
                ...
                if market and market.get("precision", {}).get("price")
                else 6
            )
            # Handle amount precision being int (decimals) or float (step size)
            amount_precision_val = (
                market.get("precision", {}).get("amount") if market else None
            )
            amount_precision = 8  # Default
            if isinstance(amount_precision_val, int):
                amount_precision = amount_precision_val
            elif isinstance(amount_precision_val, float):
...
def _set_position_protection(
    ...
    | None = None,  # Trailing distance in price points (Decimal)
    ...
    try:
        ...
        if ret_code == 0:
            ...
            if processed_ret_msg == no_change_msg:
                ...
                lg.info(
                    f"{NEON_YELLOW}Position protection partially modified or already set for {symbol}. Response: {ret_msg}{RESET}"
                )
            else:
                lg.info(
                ...
                try:
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        if (
                            ...
                            try:
                                current_sl_price_dec = Decimal(
                                    str(current_sl_price_val)
                                )
                                if current_sl_price_dec <= 0:
                                    current_sl_price_dec = (
                                        None  # Treat invalid SL as None
                                    )
                            except Exception:
                                lg.warning(
