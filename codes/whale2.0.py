...
# --- Execute Position Close if Exit Condition Met ---
        ...
        # --- Check Break-Even Condition (Only if NOT Exiting) ---
            ...
            try:
                # Use the entry_price_str defined earlier in Scenario 2
                entry_price_dec = Decimal(str(entry_price_str))
            except (InvalidOperation, ValueError, TypeError): pass
            # Use current_atr_float defined earlier in the function
            ...
            if entry_price_dec and entry_price_dec > 0 and current_atr_dec and current_atr_dec > 0:
                try:
                    ...
                    # Use min_tick_size defined earlier in the function
                    ...
                    if min_tick > 0:
                        ...
                        if trigger_met and be_stop_price and be_stop_price > 0:
                            ...
                            modify_sl_needed = True # Assume we need to modify SL unless checked otherwise
                            # Use current_sl_price_dec defined earlier in Scenario 2
                            if current_sl_price_dec:
                                if (pos_side == 'long' and current_sl_price_dec >= be_stop_price) or \
                                   (pos_side == 'short' and current_sl_price_dec <= be_stop_price):
                                     lg.info(f"  Stop loss ({current_sl_price_dec:.{price_precision}f}) is already at or beyond break-even target ({be_stop_price:.{price_precision}f}). No SL modification needed.")
                                     analyzer.break_even_triggered = True # Mark as triggered even if no modification needed
                                     modify_sl_needed = False # Explicitly mark that no API call is needed
                            # If no current_sl_price_dec, modify_sl_needed remains True

                            # --- Perform modification ONLY if needed ---
                            if modify_sl_needed:
                                ...
                                be_success = _set_position_protection(
                                    ...
                                    # Use current_tp_price_dec defined earlier in Scenario 2
            ...
            # Proceed only if entry price, ATR, and min_tick are valid
            if entry_price_dec and entry_price_dec > 0 and \
               current_atr_dec and current_atr_dec > 0 and \
               min_tick_size and min_tick_size > 0: # Use min_tick_size fetched earlier

                # <<< START TRY BLOCK (for calculations/potential errors) >>>
                try:
                    trigger_multiple = Decimal(str(config.get("break_even_trigger_atr_multiple", 1.0)))
                    offset_ticks = int(config.get("break_even_offset_ticks", 2))
                    min_tick = min_tick_size # Assign to local variable for clarity

                    # --- Calculations ---
                    profit_target_offset = current_atr_dec * trigger_multiple
                    be_stop_price = None
                    trigger_met = False
                    trigger_price = None # Initialize trigger_price
                    ...
                    # --- Process Trigger (outside the calculation part of try, but could be inside if API call is here) ---
                    if trigger_met:
                        if be_stop_price and be_stop_price > 0:
                            # --- Valid BE Stop Price Logic ---
                            lg.warning(f"{NEON_PURPLE}*** BREAK-EVEN Triggered for {symbol} {pos_side.upper()} position! ***")
                            trigger_price_log = f"{trigger_price:.{price_precision}f}" if trigger_price is not None else "N/A"
                            ...
                            if modify_sl_needed:
                                lg.info(f"  Modifying protection: Setting Fixed SL to {be_stop_price:.{price_precision}f}, keeping TP {tp_log_str}, cancelling TSL (if active).")
                                # Note: _set_position_protection call could also be inside the try block if desired
                                be_success = _set_position_protection(
                                    exchange=exchange,
                                    symbol=symbol,
                                    market_info=market_info,
                                    ...
                                    take_profit_price=current_tp_price_dec,
                                ...
                                if be_success:
                                    ...
                                    lg.error(f"{NEON_RED}Failed to set break-even stop loss for {symbol}. Original protection may still be active.{RESET}")
                            # --- End Valid BE Stop Price Logic ---
                        else: # be_stop_price is invalid or zero
                            lg.error(f"{NEON_RED}Break-even triggered, but calculated BE stop price ({be_stop_price}) is invalid. Cannot modify SL.{RESET}")
                    # --- End Process Trigger ---

                # <<< END TRY BLOCK >>>
                except (InvalidOperation, ValueError, TypeError, KeyError) as be_err: # Correctly placed except
                    lg.error(f"{NEON_RED}Error calculating or applying break-even logic for {symbol}: {be_err}{RESET}", exc_info=True)
            # <<< END IF entry_price_dec/current_atr_dec/min_tick_size > 0 BLOCK >>>
            else:
                 # Log reason if BE check skipped (because entry, ATR, or min_tick was invalid)
                 if not entry_price_dec or entry_price_dec <= 0: lg.debug(f"Break-even check skipped: Invalid entry price ({entry_price_str}).")
                 elif not current_atr_dec or current_atr_dec <= 0: lg.debug(f"Break-even check skipped: Invalid ATR ({current_atr_float}).")
                 elif not min_tick_size or min_tick_size <= 0: lg.debug(f"Break-even check skipped: Invalid min_tick_size ({min_tick_size}).")
        ...
        # --- If no exit or BE modification, log HOLD ---
             ...
             lg.info(f"Signal is {signal}. Holding existing {pos_side.upper()} position for {symbol}. No management action taken this cycle.")
...
def main():
    ...
    # Determine the symbol to trade from command-line arguments or default
    import argparse
    parser = argparse.ArgumentParser(description="Enhanced Bybit Trading Bot (Whale 2.0)")
    parser.add_argument("symbol", help="Trading symbol in CCXT format (e.g., BTC/USDT:USDT)")
    ...
    # Initialize logger for the specified symbol
    # Setup logger *after* potentially setting console_log_level from args
    logger = setup_logger(symbol)

    logger.info(f"*** Live Trading Bot Whale 2.0 Initializing for {symbol} ***")
    logger.info(f"Using config file: {CONFIG_FILE}")
    logger.info(f"Logging to directory: {LOG_DIRECTORY}")
    logger.info(f"Configured Timezone: {TIMEZONE}")
    logger.info(f"Trading Enabled: {CONFIG.get('enable_trading', False)}")
    logger.info(f"Using Sandbox: {CONFIG.get('use_sandbox', True)}")
    ...
    if CONFIG.get("interval") not in VALID_INTERVALS:
        logger.error(f"{NEON_RED}Invalid 'interval' ({CONFIG.get('interval')}) in {CONFIG_FILE}. Must be one of {VALID_INTERVALS}. Exiting.{RESET}")
        return
    ...
    # Validate symbol exists on the exchange and get market info
    ...
    # --- Bot Main Loop ---
    logger.info(f"Starting main trading loop for {symbol}...")
    while True:
        loop_start_utc = datetime.now(ZoneInfo("UTC")) # Use UTC for internal timing
        loop_start_local = loop_start_utc.astimezone(TIMEZONE)
        logger.debug(f"--- New loop cycle starting at {loop_start_local.strftime('%Y-%m-%d %H:%M:%S %Z')} ---")
        ...
        try:
            ...
            # close_open_positions(exchange, symbol, logger) # Example function call
            ...
            # Implement exponential backoff or longer fixed delay for persistent network issues
            time.sleep(RETRY_DELAY_SECONDS * 10) # e.g., 50 seconds sleep
        except ccxt.RateLimitExceeded as e:
             logger.warning(f"{NEON_YELLOW}Rate Limit Exceeded in main loop: {e}. CCXT should handle internally, but consider increasing loop delay if frequent.{RESET}")
             time.sleep(RETRY_DELAY_SECONDS * 2) # Short delay to allow rate limit reset
        except ccxt.ExchangeNotAvailable as e:
            logger.error(f"{NEON_RED}Exchange Not Available: {e}. Retrying after significant delay...{RESET}")
            time.sleep(LOOP_DELAY_SECONDS * 5) # Long delay if exchange reports unavailable
        except ccxt.ExchangeError as e:
            # Handle potentially recoverable vs non-recoverable exchange errors
            bybit_code = getattr(e, 'code', None)
            err_str = str(e).lower()
            logger.error(f"{NEON_RED}Exchange Error encountered in main loop: {e} (Code: {bybit_code}){RESET}")
            # Example: Stop on critical errors like invalid account status, permission issues
            # Continue/retry on potentially transient errors like margin checks, temporary order rejections
            # If Bybit maintenance codes appear (e.g., 10016), wait longer.
            if bybit_code == 10016 or "system maintenance" in err_str:
                logger.warning(f"{NEON_YELLOW}Exchange likely in maintenance (Code: {bybit_code}). Waiting longer...{RESET}")
            ...
            logger.error("Bot encountered a potentially fatal error. For safety, the bot will stop.")
            # Consider sending a notification here (email, Telegram, etc.)
            break # Stop the bot on unexpected errors to prevent unintended behavior

        # --- Loop Delay ---
        # Calculate delay based on config
        try:
             # Use 'retry_delay' key from config, fallback to LOOP_DELAY_SECONDS
             loop_delay = int(CONFIG.get("retry_delay", LOOP_DELAY_SECONDS))
             if loop_delay < 1:
                 loop_delay = 1 # Ensure minimum 1 second delay
        except (ValueError, TypeError):
            loop_delay = LOOP_DELAY_SECONDS # Default if config value is invalid

        logger.debug(f"Loop cycle finished. Sleeping for {loop_delay} seconds...")
        time.sleep(loop_delay)

    # --- End of Main Loop ---
    logger.info(f"*** Live Trading Bot for {symbol} has stopped. ***")

# --- Entry Point ---
if __name__ == "__main__":
    # Set default timezone for the process if needed (might affect system calls)
    # os.environ['TZ'] = TIMEZONE.key # Not always reliable, use ZoneInfo objects directly
    try:
        main()
    except Exception as e:
        ...
        try:
            root_logger = logging.getLogger() # Get root or specific logger if available
            if root_logger.hasHandlers():
                root_logger.critical("Unhandled exception caused script termination.", exc_info=True)
            else: # Fallback to print if logger not set up
                import traceback
                traceback.print_exc()
