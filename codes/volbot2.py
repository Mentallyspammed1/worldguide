...
DEFAULT_VOLBOT_VOLUME_NORMALIZATION_PERCENTILE = (
    100  # Use 100th percentile (max) for normalization
)
DEFAULT_VOLBOT_OB_SOURCE = (
    "Wicks"  # "Wicks" or "Bodys" - determines candle parts for OB calculation
)
...
DEFAULT_ATR_PERIOD = (
    14  # Default ATR period for SL/TP/BE calculations (Risk Management ATR)
)
...
def initialize_exchange(logger: logging.Logger) -> ccxt.Exchange | None:
    ...
    try:
        ...
        lg.info(
            f"Markets loaded. CCXT exchange initialized ({exchange.id}). Sandbox: {CONFIG.get('use_sandbox')}"
        )
        ...
        if price is None:
            ...
            if bid_price is not None and ask_price is not None:
                try:
                    ...
                    lg.warning(
                        f"Could not parse bid '{bid_price}' or ask '{ask_price}' for midpoint."
                    )
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        if not ohlcv:
            lg.warning(
                f"{NEON_YELLOW}No kline data returned for {symbol} {timeframe} after all retries.{RESET}"
            )
            return pd.DataFrame()
...
def ema_swma(series: pd.Series, length: int, logger: logging.Logger) -> pd.Series:
    ...
    if len(series) < 4:
        lg.warning(
            f"Series length ({len(series)}) is less than 4. SWMA requires 4 periods. Returning standard EMA."
        )
        # Fallback to standard EMA if series is too short
        return ta.ema(
            series, length=length, adjust=False
        )  # Use adjust=False for TV-like behavior
...
def calculate_volatility_levels(
    ...
    df["trend_up_strat"] = (
        df["ema1_strat"] > df["ema2_strat"]
    )  # Corrected logic: ema1 > ema2 for uptrend
    ...
    for i in range(1, len(df)):
        ...
        if df.loc[current_index, "trend_changed_strat"]:
            ...
            if pd.notna(ema1_val) and pd.notna(atr_val) and atr_val > 0:
                ...
                df.loc[current_index:, "last_trend_change_index_strat"] = (
                    last_trend_change_idx
                )
            else:
                # If EMA or ATR is NaN at trend change, propagate previous levels
                lg.debug(
                    f"NaN encountered for EMA/ATR at trend change index {i}. Propagating previous levels."
                )
    ...
    df["percentile_vol_strat"] = (
        df["volume"].rolling(window=volume_percentile_lookback, min_periods=1).max()
    )
    df["vol_norm_strat"] = np.where(
        df["percentile_vol_strat"] > 0,
        (
            df["volume"] / df["percentile_vol_strat"] * 100
        ),  # Scale volume 0-100 based on max in lookback
    ...
    df["cum_vol_delta_since_change_strat"] = df.groupby(trend_block_group)[
        "volume_delta_strat"
    ].cumsum()
    df["cum_vol_total_since_change_strat"] = df.groupby(trend_block_group)[
        "volume_total_strat"
    ].cumsum()
...
def manage_order_blocks(
    ...
    )  # Bars from PH back to OB candle
    right_l = config.get(
        "volbot_pivot_right_len_l", DEFAULT_VOLBOT_PIVOT_RIGHT_LEN_L
    )  # Bars from PL back to OB candle
    max_boxes = config.get("volbot_max_boxes", DEFAULT_VOLBOT_MAX_BOXES)

    # Lists to store OB dictionaries
    bull_boxes: list[
        dict
    ] = []  # Stores active bullish OBs: [ {id, type, start_idx, end_idx, top, bottom, state} ]
    bear_boxes: list[
        dict
    ] = []  # Stores active bearish OBs: [ {id, type, start_idx, end_idx, top, bottom, state} ]
    ...
    df["active_bear_ob_strat"] = pd.Series(
        dtype="object"
    )  # Changed from pd.series to pd.Series
    ...
    for i in range(len(df)):
        ...
        if pd.notna(df.loc[current_idx, "ph_strat"]):
            ...
            if ob_candle_iloc >= 0:
                ob_candle_idx = df.index[
                    ob_candle_iloc
                ]  # Timestamp index of the OB candle
        ...
        for box in bull_boxes:
            if box["state"] == "active":
                ...
                if current_close < box["bottom"]:
                    ...
                    lg.debug(
                        f"Closed Bullish OB: {box['id']} (Close {current_close:.5f} < Bottom {box['bottom']:.5f})"
                    )
                else:
        ...
        for box in bear_boxes:
            if box["state"] == "active":
                ...
                if current_close > box["top"]:
                    ...
                    box["end_idx"] = current_idx  # Mark the closing bar
                    lg.debug(
                        f"Closed Bearish OB: {box['id']} (Close {current_close:.5f} > Top {box['top']:.5f})"
                    )
                else:
                    # If still active, update its end_idx to the current bar
                    box["end_idx"] = current_idx
        ...
        if len(active_bull_boxes) > max_boxes:
            ...
            bull_boxes = [
                b
                for b in bull_boxes
                if b["state"] == "closed" or b["id"] not in ids_to_remove
            ]
            lg.debug(
                f"Removed {num_to_remove} oldest active Bull OBs to maintain max count ({max_boxes})."
            )
        ...
        if len(active_bear_boxes) > max_boxes:
            ...
            lg.debug(
                f"Removed {num_to_remove} oldest active Bear OBs to maintain max count ({max_boxes})."
            )

    lg.info(
        f"Order Block management complete. Total Bull OBs: {len(bull_boxes)}, Bear OBs: {len(bear_boxes)}"
    )
...
class TradingAnalyzer:
    ...
    def _update_latest_strategy_state(self) -> None:
        ...
        if self.df_processed.empty:
            self.logger.warning(
                f"Cannot update latest state: Processed DataFrame is empty for {self.symbol}."
            )
            self.strategy_state = {}
        ...
        try:
            ...
            if self.config.get("volbot_enabled", True):
                ...
                updated_state["is_in_active_bull_ob"] = (
                    self.latest_active_bull_ob is not None
                )
                updated_state["is_in_active_bear_ob"] = (
            ...
            self.logger.debug(
                f"Latest strategy state updated for {self.symbol}: {valid_state}"
            )
            ...
            self.logger.error(
                f"Unexpected error updating latest strategy state for {self.symbol}: {e}",
                exc_info=True,
            )
            self.strategy_state = {}
            self.latest_active_bull_ob = None
    ...
    def get_price_precision(self) -> int:
        ...
        try:
            ...
            price_precision_val = precision_info.get(
                "price"
            )  # This is often the tick size (e.g., 0.01)
            ...
            self.logger.warning(
                f"Could not reliably determine price precision for {self.symbol}: {e}. Using default."
            )
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            self.logger.warning(
                f"Could not reliably determine tick size for {self.symbol}: {e}. Using fallback."
            )
    ...
    def generate_trading_signal(self) -> str:
        ...
        if not self.strategy_state or not self.config.get("volbot_enabled", True):
            self.logger.debug(
                "Cannot generate Volbot signal: State is empty or strategy disabled."
            )
            return signal

        # --- Extract latest state variables ---
        is_trend_up = self.strategy_state.get(
            "trend_up_strat", None
        )  # Boolean (True/False) or None if undetermined
        ...
        if is_trend_up is None:
            self.logger.debug(
                "Volbot signal: HOLD (Trend state could not be determined)"
            )
            return "HOLD"
        ...
        if signal_on_flip and trend_changed:
            if is_trend_up:
                signal = "BUY"
                self.logger.info(
                    f"{COLOR_UP}Volbot Signal: BUY (Reason: Trend flipped to UP){RESET}"
                )
            else:  # Trend flipped down
                signal = "SELL"
                self.logger.info(
                    f"{COLOR_DN}Volbot Signal: SELL (Reason: Trend flipped to DOWN){RESET}"
                )
    ...
    def calculate_entry_tp_sl(
        ...
        atr_val_float = self.strategy_state.get(
            "atr_risk"
        )  # Use the separate Risk Management ATR
...
def fetch_balance(
    ...
    try:
        ...
        account_types_to_try = [
            "CONTRACT",
            "UNIFIED",
        ]  # Try derivatives first, then unified
            ...
            try:
                ...
                if currency in balance_info:
                    ...
                    if balance_info[currency].get("free") is not None:
                        lg.debug(
                            f"Found '{currency}' in balance (type '{acc_type}') with 'free' key."
                        )
                ...
                if (
                    balance_info is None
                ):  # Check if we reset it or it wasn't found nested
                    lg.debug(
                        f"Currency '{currency}' not found in balance structure (type '{acc_type}'). Trying next."
                    )
                ...
                if (
                    ...
                    lg.debug(
                        f"Account type '{acc_type}' not supported/invalid. Trying next."
                    )
                    continue
                else:
                    # Log other exchange errors but continue trying other types/default
                    lg.warning(
                        f"Exchange error fetching balance (type {acc_type}): {e}. Trying next."
                    )
                    continue
            except Exception as e:
                lg.warning(
        ...
        if (
            ...
            lg.debug(
                f"Parsing balance via standard ccxt structure ['{currency}']['free']: {available_balance_str}"
            )
        ...
        # 2. Check Bybit V5 nested structure ('info.result.list') if standard failed
            ...
            if available_balance_str:
                ...
                lg.warning(
                    f"{currency} not found within Bybit V5 'info.result.list[].coin[]'."
                )
        ...
        # 3. Check older/alternative top-level 'free' dictionary structure
            ...
            lg.debug(
                f"Parsing balance via top-level 'free' dictionary: {available_balance_str} {currency}"
            )
        ...
        try:
            ...
            lg.error(
                f"Failed to convert final balance string '{available_balance_str}' to Decimal: {e}"
            )
            return None
...
def get_market_info(
    ...
    try:
        ...
        if market:
            ...
            contract_type = (
                "Linear" if is_linear else "Inverse" if is_inverse else "N/A"
            )

            # Add a convenient boolean flag to check if it's a contract market
            is_contract_market = market.get("contract", False) or market_type in [
                "swap",
                "future",
            ]
            ...
            lg.error(
                f"{NEON_RED}Market dictionary could not be retrieved for {symbol} despite being listed.{RESET}"
            )
            return None

    except ccxt.BadSymbol as e:
        lg.error(
            f"{NEON_RED}Symbol '{symbol}' is not supported or invalid on {exchange.id}: {e}{RESET}"
        )
        return None
    except ccxt.NetworkError as e:
        lg.error(
            f"{NEON_RED}Network error loading markets or getting info for {symbol}: {e}{RESET}"
        )
        return None
    except ccxt.ExchangeError as e:
        lg.error(
            f"{NEON_RED}Exchange error loading markets or getting info for {symbol}: {e}{RESET}"
        )
        return None
    except Exception as e:
        lg.error(
...
def calculate_position_size(
    ...
    if not (0 < risk_per_trade < 1):  # Risk should be between 0 and 1 (exclusive)
        lg.error(
            f"Position sizing failed ({symbol}): Invalid risk_per_trade ({risk_per_trade}). Must be > 0 and < 1."
        )
        return None
    ...
    if "limits" not in market_info or "precision" not in market_info:
        lg.error(
            f"Position sizing failed ({symbol}): Missing market 'limits' or 'precision' info."
        )
        return None
    ...
    try:
        ...
        if sl_distance_per_unit <= 0:
            lg.error(
                f"Position sizing failed ({symbol}): SL distance is zero or negative ({sl_distance_per_unit})."
            )
            return None
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            if value_per_unit > 0:
                ...
                lg.error(
                    f"Pos sizing failed (linear/spot {symbol}): Risk per unit zero/negative ({value_per_unit})."
                )
                return None
        ...
        # Contract size here is typically the quote value of 1 contract (e.g., 1 USD)
            ...
            if entry_price > 0:
                ...
                lg.error(
                    f"Pos sizing failed (inverse {symbol}): Entry price is zero or negative."
                )
                return None
        ...
        lg.info(
            f"  ContractSize={contract_size}, IsLinear/Spot={market_info.get('linear', True) or not is_contract}"
        )
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            contract_value_quote = (
                contract_size  # Value of 1 contract in quote currency
            )
        ...
        # Adjust for Max Cost
            ...
            lg.info(
                f"  Reduced size required to meet max cost: {adjusted_size_for_max_cost:.8f} {size_unit}"
            )
        ...
        try:
            ...
            lg.info(
                f"Applied exchange amount precision (Truncated): {adjusted_size:.8f} -> {final_size} {size_unit}"
            )
            ...
            if amount_precision_val is not None:
                if isinstance(amount_precision_val, (float, str)):
                    ...
                    isinstance(amount_precision_val, int) and amount_precision_val >= 0
                ):
            ...
            if step_size is not None:
                ...
                lg.info(
                    f"Applied manual step size ({step_size}): {adjusted_size:.8f} -> {final_size}"
                )
                ...
                lg.info(
                    f"Applied manual precision ({num_decimals} decimals): {adjusted_size:.8f} -> {final_size}"
                )
            else:
                lg.warning(
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                try:
                    ...
                    lg.warning(
                        f"Market info not found for {symbol} during position check. Assuming 'linear'."
                    )
            ...
            lg.debug(
                f"Exchange {exchange.id} requires fetching all positions. Fetching all..."
            )
            fetch_all = True
        except ccxt.ExchangeError as e:
            ...
            if err_code in no_pos_codes_v5 or any(
                msg in err_str for msg in no_pos_msgs
            ):
                lg.info(
                    f"No position found for {symbol} (Exchange code/message: {err_code}/{err_str})."
                )
            ...
            lg.error(
                f"Unexpected error fetching single position for {symbol}: {e}",
                exc_info=True,
            )
        ...
        if fetch_all:
            try:
                ...
                lg.error(
                    f"Error fetching all positions after single fetch failed for {symbol}: {e}",
                    exc_info=True,
                )
                return None
        ...
        for pos in positions:
            ...
            try:
                ...
                size_threshold = Decimal(
                    "1e-9"
                )  # Threshold to consider a position non-zero
        ...
        if active_position:
            ...
            if side not in ["long", "short"]:
                ...
                active_position["side"] = (
                    side  # Update the dictionary with standardized side
                )
            ...
            def format_log_val(key, value, precision=6):
                ...
                try:
                    ...
                    if d_val > 0 or (
                        d_val == 0 and key in ["trailingStopLoss", "tslActivationPrice"]
                    ):
                        ...
                        return str(
                            value
                        )  # Return original if not positive (or not TSL zero)
                except Exception:
            ...
            pos_details = {
                ...
                "Size": format_log_val(
                    "size", active_position.get("contracts", info_dict.get("size"))
                ),
                "EntryPrice": format_log_val(
                    "entryPrice",
                    active_position.get("entryPrice", info_dict.get("avgPrice")),
                ),
                "MarkPrice": format_log_val(
                    "markPrice",
                    active_position.get("markPrice", info_dict.get("markPrice")),
                ),
                "LiqPrice": format_log_val(
                    "liquidationPrice",
                    active_position.get("liquidationPrice", info_dict.get("liqPrice")),
                    precision=2,
                ),
                "uPnL": format_log_val(
                    "unrealizedPnl",
                ...
                "Collateral": format_log_val(
                    "collateral",
                    active_position.get("collateral", info_dict.get("positionIM")),
                    precision=4,
                ),  # Initial Margin as collateral proxy
                "Leverage": format_log_val(
                    "leverage",
                    active_position.get("leverage", info_dict.get("leverage")),
                    precision=1,
                ),
                "TP": format_log_val(
                    "takeProfitPrice",
                    active_position.get("takeProfitPrice"),
                    precision=4,
                ),
                "SL": format_log_val(
                    "stopLossPrice", active_position.get("stopLossPrice"), precision=4
                ),
                "TSL": format_log_val(
                    "trailingStopLoss",
        ...
        lg.error(
            f"{NEON_RED}Unexpected error checking positions for {symbol}: {e}{RESET}",
            exc_info=True,
        )
        return None
...
def place_market_order(
    ...
    try:
        ...
        lg.info(
            f"{NEON_GREEN}Successfully placed market order: {order_desc}. Order ID: {order.get('id')}{RESET}"
        )
        lg.debug(f"Market order details: {order}")
        return order
    except ccxt.InsufficientFunds as e:
        lg.error(
            f"{NEON_RED}Insufficient funds to place market order {order_desc}: {e}{RESET}"
        )
    except ccxt.InvalidOrder as e:
        lg.error(
            f"{NEON_RED}Invalid market order parameters for {order_desc}: {e}. Check size/limits/params.{RESET}"
        )
        ...
        lg.error(
            f"{NEON_RED}Exchange error placing market order {order_desc}: {e}{RESET}",
            exc_info=True,
        )
    except ccxt.NetworkError as e:
        lg.error(
            f"{NEON_RED}Network error placing market order {order_desc}: {e}{RESET}"
        )
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected error placing market order {order_desc}: {e}{RESET}",
            exc_info=True,
        )
...
def _set_position_protection(
    ...
    | None = None,  # Pass current position to avoid redundant fetches
    ...
    if not CONFIG.get("enable_trading", False):
        lg.warning(
            f"{NEON_YELLOW}TRADING DISABLED. Skipping setting protection for {symbol}.{RESET}"
        )
        return False
    ...
    if abs(position_size) <= Decimal("1e-9"):
        lg.info(
            f"Position size for {symbol} is zero or negligible. No protection needed."
        )
    ...
    if stop_loss_price is not None:
        sl_float = (
            float(stop_loss_price) if stop_loss_price > 0 else 0
        )  # Use 0 to remove SL on some exchanges
        ...
        sl_tp_set_parts.append(
            f"SL={stop_loss_price:.{market_info['precision'].get('price', 4)}f}"
            if sl_float > 0
            else "SL=Remove"
        )
    ...
    if take_profit_price is not None:
        ...
        sl_tp_set_parts.append(
            f"TP={take_profit_price:.{market_info['precision'].get('price', 4)}f}"
            if tp_float > 0
            else "TP=Remove"
        )
        ...
        if "bybit" in exchange.id.lower():
            ...
            if tsl_distance is not None:
                ...
                if Decimal(tsl_distance_str) <= 0:
                    params["tpslMode"] = (
                        "Full"  # Required when setting TP/SL without TSL
                    )
                    ...
                    if tsl_activation is not None:
                        if isinstance(tsl_activation, (Decimal, float)):
                            tsl_activation_str = f"{Decimal(str(tsl_activation)):.{market_info['precision'].get('price', 4)}f}"
                        else:
                    ...
                    if stop_loss_price or take_profit_price:
                        ...
                        params["tpslMode"] = (
                            "Full"  # Assumed for TSL only, verify if needed
                        )
            ...
            # Ensure tpslMode is set if TP or SL is set, even without TSL change
                ...
                if Decimal(str(existing_tsl)) > 0:
                    ...
                    existing_act_price = position.get("info", {}).get(
                        "activePrice", "0"
                    )
                    if Decimal(str(existing_act_price)) > 0:
                        params["activePrice"] = str(existing_act_price)
                    lg.debug(
                        "Setting tpslMode=Partial and resubmitting existing TSL due to TP/SL change."
                    )
                else:
    ...
    if not params or len(sl_tp_set_parts) == 0:
        lg.info(
            f"No protection parameters provided or changed for {symbol}. No action taken."
        )
    ...
    try:
        ...
        # Fallback to set_leverage (Bybit uses this endpoint for SL/TP/TSL)
            ...
            current_leverage = position.get(
                "leverage", position.get("info", {}).get("leverage")
            )
            if current_leverage is None:
                lg.error(
                    "Could not determine current leverage for Bybit setLeverage call."
                )
                return False
            # Note: setLeverage requires leverage argument, even if not changing it.
            response = exchange.set_leverage(
                leverage=float(current_leverage), symbol=symbol, params=params
            )
        ...
        lg.info(
            f"{NEON_GREEN}Successfully executed protection setting request for {symbol}.{RESET}"
        )
        ...
        if isinstance(response, dict):
            ...
            )  # Check common keys for status code
        ...
        lg.error(
            f"{NEON_RED}Insufficient funds error during protection setting for {symbol}: {e}{RESET}"
        )
    except ccxt.InvalidOrder as e:
        ...
        lg.error(
            f"{NEON_RED}Order not found error setting protection for {symbol}: {e}{RESET}"
        )
    except ccxt.ExchangeError as e:
...
def analyze_and_trade_symbol(
    ...
    if not ccxt_timeframe:
        lg.error(
            f"Invalid interval '{interval_str}' in config. Check VALID_INTERVALS. Skipping cycle."
        )
        return
    ...
    if (
        df_klines.empty or len(df_klines) < 50
    ):  # Check for minimum reasonable data points
        lg.warning(
    ...
    if analyzer.df_processed.empty or not analyzer.strategy_state:
        lg.error(
            f"Failed to calculate indicators or update state for {symbol}. Skipping further analysis."
        )
        return
    ...
    position_entry_price_str = (
        current_position.get(
            "entryPrice", current_position.get("info", {}).get("avgPrice")
        )
        if has_open_position
        else None
    )
    position_size_str = (
        current_position.get("contracts", current_position.get("info", {}).get("size"))
        if has_open_position
        else None
    )
    position_entry_price = (
        Decimal(str(position_entry_price_str)) if position_entry_price_str else None
    )
    position_size = (
        Decimal(str(position_size_str)) if position_size_str else Decimal("0")
    )
    ...
    if has_open_position:
        ...
        if position_side == "long" and signal == "SELL":
            lg.info(
                f"{COLOR_DN}Exit Signal: SELL signal received while in LONG position. Closing position.{RESET}"
            )
            exit_signal = True
        elif position_side == "short" and signal == "BUY":
            lg.info(
                f"{COLOR_UP}Exit Signal: BUY signal received while in SHORT position. Closing position.{RESET}"
            )
            exit_signal = True
        ...
        if exit_signal:
            if trading_enabled:
                ...
                if close_order:
                    lg.info(
                        f"{NEON_GREEN}Successfully closed {position_side} position based on exit signal.{RESET}"
                    )
                    ...
                    try:
                        ...
                        lg.info(
                            f"Cancelled pending Stop/TP orders for {symbol} after closing position."
                        )
                    except Exception as cancel_err:
                        lg.warning(
                            f"Could not cancel stop orders for {symbol} after closing: {cancel_err}"
                        )
                else:
                    lg.error(
                        f"{NEON_RED}Failed to place market order to close {position_side} position.{RESET}"
                    )
        ...
        if not exit_signal:
            ...
            current_sl = (
                ...
                if current_sl_str and Decimal(str(current_sl_str)) > 0
                else None
            )
            current_tp = (
                Decimal(str(current_tp_str))
                if current_tp_str and Decimal(str(current_tp_str)) > 0
                else None
            )
            ...
            if (
                enable_be and current_sl != position_entry_price
            ):  # Only trigger BE if SL isn't already at entry
                be_trigger_multiple = Decimal(
                ...
                if (
                    ...
                    if current_profit >= profit_target_for_be:
                        ...
                        be_price_rounded = (be_price / min_tick).quantize(
                        ...
                        if (
                            position_side == "long"
                            and be_price_rounded > sl_price_to_compare
                        ) or (
                            position_side == "short"
                            and be_price_rounded < sl_price_to_compare
                        ):
                            ...
                            lg.info(
                                f"  Old SL: {current_sl}. New BE SL: {be_price_rounded}"
                            )
                    ...
                    lg.warning(
                        f"Cannot calculate BE: Invalid Risk ATR ({risk_atr_val}) or Min Tick ({min_tick})."
                    )
            ...
            if (
                enable_tsl and not has_current_tsl
            ):  # Check if TSL should be set but isn't
                tsl_callback_rate = config.get(
                    "trailing_stop_callback_rate", 0.005
                )  # e.g., 0.5%
                tsl_activation_perc = config.get(
                ...
                if current_profit_perc >= Decimal(str(tsl_activation_perc)):
                    ...
                    if tsl_activation_perc > 0:
                        if position_side == "long":
                            act_price_raw = position_entry_price * (
                                1 + Decimal(str(tsl_activation_perc))
                            )
                            act_price = (
                                act_price_raw / analyzer.get_min_tick_size()
                            ).quantize(
                                Decimal("1"), rounding=ROUND_UP
                    ...
                    tsl_value_str = (
                        str(tsl_callback_rate * 100)
                        if tsl_callback_rate < 1
                        else str(tsl_callback_rate)
            ...
            if needs_update and trading_enabled:
                ...
                final_tp_to_set = (
                    current_tp  # TP generally doesn't change unless strategy dictates
                )
                ...
                if success:
                    lg.info(
                        f"{NEON_GREEN}Successfully updated position protection.{RESET}"
                    )
                else:
        ...
        if signal in ["BUY", "SELL"]:
            ...
            if trading_enabled:
                ...
                if market_info.get("is_contract", False):
                    ...
                    try:
                        ...
                        if (
                            ...
                            lg.info(
                                f"Leverage for {symbol} already set to {desired_leverage}x."
                            )
                            ...
                            lg.error(
                                f"{NEON_RED}Failed to set leverage for {symbol}: {e}{RESET}"
                            )
                ...
                entry_order = place_market_order(
                    exchange, symbol, signal.lower(), position_size, market_info, lg
                )
                ...
                if entry_order and entry_order.get("id"):
                    ...
                    if protection_success:
                        lg.info(
                            f"{NEON_GREEN}Successfully set initial protection for new {signal} position.{RESET}"
                        )
                    else:
                        lg.error(
                    ...
                    lg.error(
                        f"{NEON_RED}Failed to place entry market order for {signal}.{RESET}"
                    )
            else:
                lg.warning(
...
def main() -> None:
    ...
    if not exchange:
        init_logger.critical(
            f"{NEON_RED}Exchange initialization failed. Exiting.{RESET}"
        )
        return
    ...
    while True:
        ...
        if not symbol_input and symbols_to_trade:
            init_logger.info(
                f"Starting analysis with symbols: {', '.join(symbols_to_trade)}"
            )
            break
    ...
    try:
        while True:
            ...
            init_logger.info(
                f"*** New Cycle Start: {now.strftime('%Y-%m-%d %H:%M:%S %Z')} ***"
            )
            ...
            for symbol in symbols_to_trade:
                ...
                try:
                    ...
                    symbol_logger.error(
                        f"{NEON_RED}Exchange Error during analysis for {symbol}: {e}.{RESET}",
                        exc_info=True,
                    )
                except Exception as e:
                    symbol_logger.error(
                        f"{NEON_RED}Unhandled Exception during analysis for {symbol}: {e}{RESET}",
                        exc_info=True,
                    )
            ...
            init_logger.info(
                f"Cycle duration: {cycle_duration:.2f}s. Sleeping for {sleep_time:.2f}s..."
            )
            time.sleep(sleep_time)

    except KeyboardInterrupt:
        init_logger.info(
            f"{NEON_YELLOW}KeyboardInterrupt received. Shutting down gracefully...{RESET}"
        )
        # Optional: Add cleanup logic here (e.g., close open positions, cancel orders)
        if CONFIG.get("enable_trading", False):
            init_logger.warning(
                "Trading was enabled. Consider manually checking open positions/orders."
            )
    except Exception as e:
        init_logger.critical(
            f"{NEON_RED}--- FATAL UNHANDLED ERROR IN MAIN LOOP ---{RESET}",
            exc_info=True,
        )
