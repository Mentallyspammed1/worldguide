        ...
        if active_position:
            ...
            def format_log_price(price_val, precision=6):
                ...
                try:
                     ...
                     if d_price > 0 or ('tsl' in key.lower() and price_val == '0'):  # Check if key indicates TSL if price is 0
...
def main() -> None:
    ...
    init_logger.info(f"--- Starting KBotTo Trading Bot ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---")  # CHANGED BOT NAME
