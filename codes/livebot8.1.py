...
class SensitiveFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        for secret in SensitiveFormatter._secrets:
            msg = msg.replace(secret, f"***{secret[:3]}...{secret[-3:]}***") # Redact partially
        return msg
    ...
    try:
        ...
        if market:
            ...
            if min_tick > 0:
                 ...
                 if market['precision'].get('tick') is None or float(market['precision']['tick']) <= 0:
    ...
    if precision_val is not None:
        ...
        if step_size is not None and step_size > 0:
            ...
            if rounding_mode == ROUND_DOWN:
                ...
                rounded_value = ((value + step_size - Decimal('1e-18')) // step_size) * step_size # Add epsilon for ceiling
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            ...
            if isinstance(bybit_code, str): bybit_code = int(bybit_code) # Convert code to int if string
    ...
    try:
        ...
        if 'bybit' in exchange.id.lower():
            ...
            try:
                ...
                if positions:
                 ...
                 bybit_code = getattr(e, 'code', None)
                 if isinstance(bybit_code, str): bybit_code = int(bybit_code)
                 if bybit_code in no_pos_codes_v5:
        ...
        if response is not None:
            if isinstance(response, dict):
                 ...
                 if isinstance(ret_code, str): ret_code = int(ret_code) # Convert code to int if string
...
def place_trade(
    ...
    try:
        ...
        if amount_float <= 0:
            lg.error(f"{NEON_RED}Trade aborted ({symbol} {side} reduce={reduce_only}): Invalid position size for order amount ({amount_float}). Must be positive.{RESET}")
            return None
    except Exception as e:
        lg.error(f"{NEON_RED}Trade aborted ({symbol} {side} reduce={reduce_only}): Failed to convert position size {position_size} to float: {e}{RESET}")
        return None

    # --- Prepare Order Parameters ---
    params = {
        # Bybit V5: Specify position index for One-Way vs Hedge mode
        # Assuming One-Way mode (positionIdx=0) is default/required for linear/inverse perps/futures
        'positionIdx': 0,
        ...
        # 'timeInForce': 'GTC', # GoodTilCancelled (Default for Market on Bybit?) - Market orders fill or kill usually.
        'timeInForce': 'FOK', # FillOrKill - Ensures entire order fills immediately or cancels. Safer for market orders.
        'closeOnTrigger': False, # Default is False
    }
    # Add category for Bybit V5 if it's a contract market
    ...
    try:
        ...
        if avg_fill_price: lg.info(f"  Avg Fill Price (reported): {avg_fill_price}")
        if filled_amount: lg.info(f"  Filled Amount (reported): {filled_amount}")
    ...
    # --- Error Handling ---
        ...
        if bybit_code == 10001 and "parameter error" in str(e).lower():
            lg.error(f"{NEON_YELLOW} >> Hint (Code {bybit_code}): Check if size/price violates precision or limits. Ensure TIF ('{params.get('timeInForce')}') is valid.{RESET}")
...
def _set_position_protection(
    ...
    is_contract = market_info.get('is_contract', False)
    if not is_contract:
        lg.warning(f"{NEON_YELLOW}Position protection (SL/TP/TSL) skipped for {symbol} (Not a contract market).{RESET}")
        return False # Not applicable, not a failure

    # --- Validate Inputs ---
    if not position_info:
        lg.error(f"{NEON_RED}Cannot set protection for {symbol}: Missing position information.{RESET}")
        return False
    ...
    if pos_side not in ['long', 'short'] or pos_size_str is None:
        lg.error(f"{NEON_RED}Cannot set protection for {symbol}: Invalid or missing position side ('{pos_side}') or size in position_info.{RESET}")
        return False
    ...
    def is_valid_or_cancel(val: Optional[Union[Decimal, str]], is_price: bool) -> bool:
        if isinstance(val, str) and val.strip() == '0': return True # Explicit cancel
        if isinstance(val, Decimal):
            if is_price: return val > 0 # Prices must be positive
            else: return val >= 0 # Distance/Activation can be zero
        return False
    ...
    is_sl_valid = has_sl_intent and is_valid_or_cancel(stop_loss_price, is_price=True)
    is_tp_valid = has_tp_intent and is_valid_or_cancel(take_profit_price, is_price=True)
    # TSL is valid if distance is valid (>=0 or '0') and activation price is valid (>=0 or '0')
    is_tsl_dist_valid = has_tsl_intent and is_valid_or_cancel(trailing_stop_distance, is_price=False)
    is_tsl_act_valid = has_tsl_act_intent and is_valid_or_cancel(tsl_activation_price, is_price=False)
    # Overall TSL validity requires *both* distance and activation price being provided (even if '0') and valid
    is_tsl_valid = is_tsl_dist_valid and is_tsl_act_valid

    # Check if ANY protection is being attempted
    if not (has_sl_intent or has_tp_intent or has_tsl_intent):
        lg.info(f"No protection parameters (SL, TP, TSL) provided for {symbol}. No protection set/modified.")
        return True # Success as no API call was intended/needed
    ...
    try:
        ...
        lg.warning(f"{NEON_YELLOW}Could not parse positionIdx '{pos_idx_val}' from position info for {symbol}. Defaulting to {position_idx}.{RESET}")
    ...
    try:
        ...
        def format_param(value: Optional[Union[Decimal, str]], param_type: str) -> Optional[str]:
            if value is None: return None
            if isinstance(value, str) and value.strip() == '0': return '0' # Pass cancel signal directly
            if isinstance(value, Decimal):
                # Prices must be > 0, distance/activation >= 0
                is_price = param_type in ['sl', 'tp', 'tsl_act']
                if (is_price and value <= 0) or (not is_price and value < 0):
                    lg.warning(f"{NEON_YELLOW}Invalid {param_type} value ({value}) for formatting. Must be > 0 for prices, >= 0 otherwise. Skipping.{RESET}")
                    return None
                if value == 0 and not is_price: return '0' # Allow explicit zero for non-prices

                try:
                    # Use price_to_precision (expects float input)
                    return exchange.price_to_precision(symbol, float(value))
                except Exception as fmt_err:
                    lg.warning(f"{NEON_YELLOW}Could not format {param_type} {value} using exchange.price_to_precision: {fmt_err}. Skipping this parameter.{RESET}")
                    return None
            lg.warning(f"{NEON_YELLOW}Invalid type for {param_type} parameter: {type(value)}. Skipping.{RESET}")
            return None

        # Trailing Stop Loss (Set first, as it might override fixed SL)
        if has_tsl_intent: # Check intent first
            if is_tsl_valid: # Check if both parts are valid or '0'
                formatted_tsl_distance = format_param(trailing_stop_distance, 'tsl_dist')
                formatted_activation_price = format_param(tsl_activation_price, 'tsl_act')
                ...
                if formatted_tsl_distance is not None and formatted_activation_price is not None:
                    ...
                    lg.error(f"{NEON_RED}Failed to format valid TSL parameters for {symbol} (Dist: {trailing_stop_distance}, Act: {tsl_activation_price}). Cannot set TSL.{RESET}")
                    formatting_failed = True # Mark that formatting failed
            else:
                 lg.warning(f"{NEON_YELLOW}TSL intent present, but parameters invalid (Dist valid: {is_tsl_dist_valid}, Act valid: {is_tsl_act_valid}). Skipping TSL.{RESET}")


        # Fixed Stop Loss - Only add if TSL was NOT successfully added OR if TSL is being cancelled ('0')
        if has_sl_intent:
            if is_sl_valid:
                # Only proceed if TSL wasn't set or is being cancelled
                if not tsl_added_to_params or params.get('trailingStop') == '0':
                    formatted_sl = format_param(stop_loss_price, 'sl')
                    if formatted_sl is not None:
                        params['stopLoss'] = formatted_sl
                        log_parts.append(f"  Fixed SL: {formatted_sl}")
                        sl_added_to_params = True
                    else:
                        lg.error(f"{NEON_RED}Failed to format valid SL parameter for {symbol} ({stop_loss_price}). Cannot set SL.{RESET}")
                        formatting_failed = True
                else: # TSL was added and is active
                    lg.warning(f"{NEON_YELLOW}Both valid 'stopLoss' and active 'trailingStop' provided for {symbol}. Prioritizing TSL. Fixed 'stopLoss' parameter ignored.{RESET}")
            else:
                 lg.warning(f"{NEON_YELLOW}Fixed SL intent present, but price invalid ({stop_loss_price}). Skipping SL.{RESET}")


        # Fixed Take Profit
        if has_tp_intent:
            if is_tp_valid:
                formatted_tp = format_param(take_profit_price, 'tp')
                if formatted_tp is not None:
                    params['takeProfit'] = formatted_tp
                    log_parts.append(f"  Fixed TP: {formatted_tp}")
                    tp_added_to_params = True
                else:
                    lg.error(f"{NEON_RED}Failed to format valid TP parameter for {symbol} ({take_profit_price}). Cannot set TP.{RESET}")
                    formatting_failed = True
            else:
                 lg.warning(f"{NEON_YELLOW}Fixed TP intent present, but price invalid ({take_profit_price}). Skipping TP.{RESET}")


    except Exception as fmt_err:
        lg.error(f"{NEON_RED}Error processing/formatting protection parameters for {symbol}: {fmt_err}{RESET}", exc_info=True)
        return False # Failure if error during formatting
    ...
    if not sl_added_to_params and not tp_added_to_params and not tsl_added_to_params:
        # If formatting failed for an intended parameter, return False
        if formatting_failed:
            lg.error(f"{NEON_RED}Protection setting failed due to parameter formatting issues for {symbol}. No API call made.{RESET}")
            return False
        else:
            # Otherwise, nothing valid was intended or provided
            lg.warning(f"{NEON_YELLOW}No valid protection parameters could be formatted or remain after adjustments for {symbol}. No API call made.{RESET}")
            return True # Treat as success if no action was needed/possible
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        if not market_info:
             lg.error(f"{NEON_RED}Cannot calculate TSL: Market info not available.{RESET}")
             return False
        ...
        if min_tick_size <= 0:
            lg.error(f"{NEON_RED}Cannot calculate TSL: Invalid min tick size ({min_tick_size}) for {symbol}.{RESET}")
            return False
        ...
        # Ensure activation price is non-negative (can be zero) and positive if not immediate
        if activation_price is None or activation_price < 0 or (activation_percentage > 0 and activation_price <= 0):
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger) -> None:
    ...
    if not ccxt_interval:
        lg.error(f"{NEON_RED}Invalid interval '{config['interval']}' in config. Cannot map to CCXT timeframe for {symbol}.{RESET}")
        return
    ...
    # Example of logging Fib levels if needed
    # nearest_fibs = analyzer.get_nearest_fibonacci_levels(current_price, 3)
    # if nearest_fibs:
    #     lg.info(f"Nearest Fib Levels: " + ", ".join([f"{name}={level:.{price_precision}f}" for name, level in nearest_fibs]))
        ...
        if signal in ["BUY", "SELL"]:
            ...
            if market_info.get('is_contract', False):
                ...
                if leverage > 0:
                    ...
                    lg.warning(f"{NEON_YELLOW}Leverage setting skipped for {symbol}: Configured leverage is zero or negative ({leverage}).{RESET}")
            else:
            ...
            if trade_order and trade_order.get('id'):
                ...
                if confirmed_position:
                    ...
                    try:
                        ...
                        if entry_price_actual_str and pos_size_actual_str:
                            try:
                                ...
                                # Use a small threshold for size validation (re-calculate for context)
                                ...
                                if entry_price_actual > 0 and abs(pos_size_actual) >= min_size_threshold:
                                    ...
                                    lg.error(f"{NEON_RED}Confirmed position has invalid entry price ({entry_price_actual}) or size ({pos_size_actual} < {min_size_threshold}).{RESET}")
                            except Exception as parse_err:
                                lg.error(f"{NEON_RED}Error parsing confirmed position entry/size: {parse_err}{RESET}")
                        else:
                            lg.error(f"{NEON_RED}Confirmed position missing entryPrice or size information.{RESET}")
                    ...
                    try:
                        lg.info(f"Fetching status for order {order_id}...")
                        # Use fetch_order with the correct symbol and potential params
                        order_params = {}
                        if 'bybit' in exchange.id.lower() and market_info.get('is_contract', False):
                            order_params['category'] = 'linear' if market_info.get('linear', True) else 'inverse'
                        order_status_info = exchange.fetch_order(order_id, symbol, params=order_params)
                        lg.info(f"Status of order {order_id}: Status='{order_status_info.get('status')}', Filled='{order_status_info.get('filled')}', AvgPrice='{order_status_info.get('average')}'")
                        lg.debug(f"Full order status info: {order_status_info}")
                    except ccxt.OrderNotFound:
                         lg.error(f"Order {order_id} not found on exchange.")
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        try:
               ...
               (pos_side == 'short' and signal == "BUY"):
                ...
                if config.get("enable_break_even", False) and not is_tsl_active:
                    ...
                    try:
                        ...
                        if not entry_price_str or str(entry_price_str).lower() == 'n/a' or str(entry_price_str).strip() == '':
                        ...
                        if current_atr_float is None or pd.isna(current_atr_float) or current_atr_float <= 0:
                            lg.warning(f"{NEON_YELLOW}Cannot check break-even: Invalid ATR value ({current_atr_float}).{RESET}")
                        ...
                        if price_diff >= profit_target_price_diff:
                            ...
                            if current_sl_price_val is not None:
                                try:
                                    sl_val_float = float(current_sl_price_val)
                                    if sl_val_float > 0: # Only consider positive SL values
                                    ...
                                    lg.warning(f"{NEON_YELLOW}Could not parse current SL price '{current_sl_price_val}' for BE comparison.{RESET}")
                            ...
                            if be_stop_price is not None and be_stop_price > 0: # Ensure target BE is valid
                                if current_sl_price_dec is None: # No current SL, set BE SL
                                    ...
                                    update_be = True
                                    lg.info(f"Profit target hit. Current SL {current_sl_price_dec:.{price_precision}f} < Target BE SL {be_stop_price:.{price_precision}f}.")
                                ...
                                lg.warning(f"{NEON_YELLOW}Calculated BE Stop Price ({be_stop_price}) is invalid. Cannot update.{RESET}")


                            # Execute the update if needed
                            if update_be:
                                lg.warning(f"{NEON_PURPLE}*** Moving Stop Loss to Break-Even for {symbol} at {be_stop_price:.{price_precision}f} ***{RESET}")
                                # Fetch current TP to preserve it when setting SL
                                current_tp_price_dec = None
                                if current_tp_price_val is not None:
                                    try:
                                        tp_val_float = float(current_tp_price_val)
                                        if tp_val_float > 0:
                        ...
                        lg.warning(f"{NEON_YELLOW}Skipping BE check due to invalid data: {ve}{RESET}")
            ...
            lg.error(f"{NEON_RED}Error during position management phase for {symbol}: {manage_err}{RESET}", exc_info=True)
...
def main() -> None:
    ...
    try:
        init_logger.info(f"Using Pandas TA Version: {ta.version() if hasattr(ta, 'version') and callable(ta.version) else getattr(ta, '__version__', 'N/A')}")
    except Exception:
    ...
    if CONFIG.get("enable_trading"):
        ...
        try:
            ...
            print("-" * 60)
            if confirm != "": # Allow pressing Enter only
