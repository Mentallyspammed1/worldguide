...
try:
    ...
    try:
        ...
        print("FATAL ERROR: 'zoneinfo' (Python 3.9+) or 'pytz' package required for timezone handling.")
        print("If using Python < 3.9, please install pytz: pip install pytz")
        sys.exit(1)
...
try:
    ...
    print("FATAL ERROR: 'ccxt' package not found. Please install it: pip install ccxt")
    sys.exit(1)
try:
    import numpy as np
except ImportError:
    print("FATAL ERROR: 'numpy' package not found. Please install it: pip install numpy")
    sys.exit(1)
try:
    import pandas as pd
except ImportError:
    print("FATAL ERROR: 'pandas' package not found. Please install it: pip install pandas")
    sys.exit(1)
try:
    import pandas_ta as ta
except ImportError:
    print("FATAL ERROR: 'pandas_ta' package not found. Please install it: pip install pandas_ta")
    sys.exit(1)
...
try:
    ...
    print("Install it for enhanced logging: pip install colorama")
...
try:
    ...
    print("Warning: 'python-dotenv' package not found. Cannot load .env file.")
    print("Install it for environment variable management: pip install python-dotenv")
...
if not API_KEY or not API_SECRET:
    print(f"{NEON_RED}FATAL ERROR: BYBIT_API_KEY and BYBIT_API_SECRET must be set in your environment variables or a .env file.{RESET}")
...
try:
    ...
    print(f"{NEON_RED}FATAL ERROR: Could not create log directory '{LOG_DIRECTORY}': {e}{RESET}")
    sys.exit(1)
...
try:
    ...
    print(f"Using Timezone: {TZ_NAME} (via {'zoneinfo' if _has_zoneinfo else 'pytz'})")
except Exception as tz_err:
    print(f"{NEON_YELLOW}Warning: Could not load timezone '{TZ_NAME}': {tz_err}. Defaulting to {_FALLBACK_TIMEZONE}.{RESET}")
    ...
    try:
        ...
        print(f"{NEON_RED}FATAL ERROR: Could not load fallback timezone '{_FALLBACK_TIMEZONE}': {fallback_tz_err}. Exiting.{RESET}")
        sys.exit(1)
...
def _ensure_config_keys(config: Dict[str, Any], default_config: Dict[str, Any], parent_key: str = "") -> Tuple[Dict[str, Any], bool]:
    ...
    for key, default_value in default_config.items():
        ...
        if key not in updated_config:
            ...
            print(f"{NEON_YELLOW}Config Warning: Missing key '{full_key_path}'. Added default value: {repr(default_value)}{RESET}")
            ...
            if not is_promoting_num and not is_str_bool_candidate:
                print(f"{NEON_YELLOW}Config Warning: Type mismatch for key '{full_key_path}'. Expected {type(default_value).__name__}, got {type(loaded_value).__name__}. Using loaded value: {repr(loaded_value)}.{RESET}")
...
def _check_account_type_and_balance(exchange: ccxt.Exchange, currency: str, logger: logging.Logger) -> Tuple[Optional[bool], Optional[Decimal]]:
    ...
    try:
        ...
        if parsed_balance is not None:
            lg.info(f"Detected UNIFIED account structure (Balance check successful).")
...
def get_market_info(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        def get_digits_from_precision(precision_val: Optional[Union[float, int, str]], default_digits: int = 8) -> int:
    ...
    def _define_ta_strategy(self) -> None:
        ...
        if indi_cfg.get("volume_confirmation") and vol_ma_p > 0:
            ...
            if 'volume' in self.df.columns and pd.api.types.is_float_dtype(self.df['volume']):
                ...
                self.logger.warning(f"Cannot calculate Volume SMA: 'volume' column missing or not float in TA DataFrame.")
...
def calculate_position_size(
    ...
    if amount_step_size is None or amount_step_size <= 0:
        lg.error(f"Size Calc Error ({symbol}): Amount step size missing or invalid ({amount_step_size})"); return None
    ...
    try:
        ...
        if is_contract:
            if is_inverse:
                ...
                # More accurate: Risk = Contracts * ContractSize * |1/Entry - 1/SL| * ValueAtEntryPrice? No.
                # Risk per contract (in Quote) = Contracts * ContractSize * |(1/SL - 1/Entry)| * EntryPrice? No.
                # Risk per contract (in Quote) = ContractSize * |Entry/SL - 1| ? No.
                # Correct: Value per contract = ContractSize / Price. Risk = dValue = |Value(Entry) - Value(SL)|
                # Risk (Quote) = | (ContractSize / SL) - (ContractSize / Entry) | * SL? No.
                # Risk (Base) = | (ContractSize / SL) - (ContractSize / Entry) | * ContractSize? No.
                # Let's stick to the simple approximation used widely:
                # Risk per contract (Quote) ≈ ContractSize * |Entry - SL| / Entry  <- This seems wrong too.
                # Back to basics: Position value = contracts * contract_size / price (in Base).
                # Value in Quote = contracts * contract_size (since contract size is in Base units).
                # Change in Quote value = contracts * contract_size * d(1/price) * price? No.
                # Let's use the standard formula: Size (Contracts) = Risk Amount (Quote) / (Contract Size (Base/Contract) * |1/SL - 1/Entry| * Entry Price)
                # So, Risk Per Contract (Quote) = Contract Size * |1/SL - 1/Entry| * Entry Price
        ...
        quantized_size = self.quantize_amount(size_unquantized, rounding=ROUND_DOWN)
        ...
        if max_order_amount is not None and quantized_size > max_order_amount:
            ...
            quantized_size = self.quantize_amount(max_order_amount, rounding=ROUND_DOWN)
        ...
        if is_contract:
             if is_inverse:
                 ...
                 order_value_quote = quantized_size * contract_size # Value in Base currency terms
                 # Margin for inverse = (Contracts * ContractSize) / Leverage (in Base currency)
                 margin_required_base = order_value_quote / Decimal(leverage)
                 # Convert margin required to Quote currency for comparison with balance
                 margin_required = margin_required_base * entry_price # Approx margin in quote terms
                 # Recalculate order value in quote for min cost check
                 order_value_quote = quantized_size * contract_size * entry_price # Approx value in quote
             else: # Linear Contract
...
def attempt_new_entry(
    ...
    entry_ref, tp_price, sl_price = analyzer.calculate
