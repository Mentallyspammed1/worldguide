    ...
    try:
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                wait_time = (
                    int(e.args[0].split(" ")[-2])
                    if "try again in" in e.args[0]
                    else RETRY_DELAY_SECONDS * 5
                )
                lg.warning(
...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        expected_patterns = {
            ...
            "CCI": [
                f"CCI_{self.config.get('cci_window', DEFAULT_CCI_WINDOW)}_0.015"
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        try:
            ...
            price_precision_val = precision_info.get(
                "price"
            )  # This might be tick size or decimal places
        ...
        self.logger.debug(
            f"Using fallback tick size based on precision places for {self.symbol}: {fallback_tick}"
        )
    ...
    def _check_wr(self) -> float:  # Williams %R
        ...
        return 0.0  # Should only happen if exactly -50?
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            price_precision = self.get_price_precision()
            # Get minimum price increment (tick size) for validation and adjustments
            ...
            if signal == "BUY":
                ...
                # Quantize to the nearest tick size
...
def calculate_position_size(
    ...
    try:
        ...
        # Size (in Contracts) = Risk Amount (Quote) / (SL Distance per Unit (Quote/Base) / Entry Price (Quote/Base) * Contract Value (Base/Contract)) - This is complex!
        # We assume Linear/Spot based on default config. Inverse requires careful validation.
        ...
        if (
            ...
            lg.warning(
                f"{NEON_YELLOW}Inverse contract detected for {symbol}. Sizing calculation assumes 1 contract = 1 Base unit value, which might be incorrect. VERIFY INVERSE SIZING LOGIC!{RESET}"
            )
            # Placeholder using simplified linear logic - **THIS NEEDS CORRECTION FOR REAL INVERSE TRADING**
            # A more correct inverse calculation depends on contract value specification (e.g., 1 USD per contract, or 100 BTC per contract etc.)
            # Example for value = 1 USD per contract (like Bybit inverse):
            # Value per contract in Quote = 1 USD (or USDT equivalent)
            # Value per contract in Base = 1 / Entry Price (Quote/Base)
            # Risk per Contract (Quote) = SL Distance (Quote) * Value per contract (Base) = SL Distance * (1 / Entry Price)
            # Size (Contracts) = Risk Amount (Quote) / Risk per Contract (Quote) = Risk Amount / (SL Distance / Entry Price)
            if entry_price > 0:
                # Assume contract_size represents the Quote value per contract (e.g., 1 for USD)
                # Risk per contract (Quote) = SL_distance(Quote/Base) * ContractSize(Quote/Contract) / EntryPrice(Quote/Base)
                risk_per_contract_quote = (
                    sl_distance_per_unit * contract_size / entry_price
                )
                if risk_per_contract_quote > 0:
                    calculated_size = risk_amount_quote / risk_per_contract_quote
                else:
                    lg.error(
                        f"Position sizing failed for inverse contract {symbol}: Risk per contract calculation is zero or negative."
                    )
                    return None
            else:
                lg.error(
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            # Use contract_size as the value per contract in Quote (e.g., 1 for USD)
        ...
        try:
            ...
            # Use ROUND_DOWN explicitly when formatting amount via truncate/floor
            # Use exchange.amount_to_precision with TRUNCATE (equivalent to floor for positive numbers)
            ...
            if amount_precision_val is not None:
                ...
                if step_size is not None and step_size > 0:
                    ...
                    lg.info(
                        f"Applied manual amount step size ({step_size}): {adjusted_size:.8f} -> {final_size} {size_unit}"
                    )
        ...
        if final_size < min_amount:
            ...
            # Use a small tolerance relative to min_amount? Or strict check? Be strict for now.
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                # Determine category based on market info (need market info passed or fetched here)
                market = None
                try:
                    market = exchange.market(symbol)
                ...
                # Bybit V5 fetch_positions/fetch_position *might* not require symbol in params if symbols list is used,
                # but adding it can sometimes help or be required by specific endpoints called internally.
                # params['symbol'] = symbol
                lg.debug(f"Using params for fetch_positions: {params}")

            positions = exchange.fetch_positions(
                symbols=[symbol], params=params
            )  # Pass symbol in list and params
            ...
            if hasattr(e, "code") and e.code in no_pos_codes_v5:
                lg.info(
                    f"No position found for {symbol} (Exchange error code: {e.code} - {e})."
                )
                return None
        ...
        for pos in positions:
            ...
            try:
                ...
                # Use a small threshold (e.g., based on minimum contract size if available)
        ...
        if active_position:
            ...
            # Helper to extract and validate price from info dict
            def get_valid_price_from_info(key: str) -> str | None:
                ...
                if (
                    ...
                    try:
                        # Basic validation: can it be converted to Decimal > 0?
                        if Decimal(str(val_str)) > 0:
                            return str(val_str)  # Return as string
                    except Exception:
            ...
            if active_position.get("stopLossPrice") is None:
                sl_val = get_valid_price_from_info("stopLoss")
                if sl_val:
                    active_position["stopLossPrice"] = sl_val

            if active_position.get("takeProfitPrice") is None:
                tp_val = get_valid_price_from_info("takeProfit")
                if tp_val:
                    active_position["takeProfitPrice"] = tp_val
            ...
            active_position["trailingStopLoss"] = info_dict.get(
                "trailingStop", "0"
            )  # TSL Distance/Value
            active_position["tslActivationPrice"] = info_dict.get(
                "activePrice", "0"
            ...
            def format_log_price(key, price_val, precision=6):
                ...
                try:
                    ...
                    if d_price > 0 or (
                        d_price == 0
                        and ("trailingStop" in key or "tslActivation" in key)
                    ):
                        return f"{d_price:.{precision}f}"
                    elif d_price == 0 and key not in [
                        "trailingStopLoss",
                        "tslActivationPrice",
                    ]:
                        return "N/A"  # Treat 0 as N/A for SL/TP/Entry etc.
                    else:
                        return "Invalid"  # Treat negative as invalid
                except Exception:
                    return str(price_val)  # Return raw if conversion fails

            # Log details of the confirmed active position
            market_for_log = (
                exchange.market(symbol) if symbol in exchange.markets else None
            )
            log_precision = (
                market_for_log["precision"]["price"] if market_for_log else 6
            )
            ...
            contracts = format_log_price(
                "contracts", contracts_str, 8
            )  # Use more precision for size
            liq_price = format_log_price(
                "liquidationPrice",
            ...
            leverage = (
                f"{Decimal(leverage_str):.1f}x" if leverage_str is not None else "N/A"
            )
            pnl_str = active_position.get("unrealizedPnl")
            pnl = format_log_price("unrealizedPnl", pnl_str, 4)
            sl_price = format_log_price(
                "stopLossPrice", active_position.get("stopLossPrice"), log_precision
            )
            tp_price = format_log_price(
                "takeProfitPrice", active_position.get("takeProfitPrice"), log_precision
            )
            tsl_dist = format_log_price(
                "trailingStopLoss",
                active_position.get("trailingStopLoss"),
                log_precision,
            )
            tsl_act = format_log_price(
                "tslActivationPrice",
            ...
            try:
                # Check if TSL distance > 0 to determine if TSL is active
                if Decimal(str(active_position.get("trailingStopLoss", "0"))) > 0:
                    is_tsl_active_log = True
            except:
                pass
            ...
            if isinstance(response, dict):
                ...
                if ret_code == 0:
                    ...
                    if ret_code == 110045:
                        ...
                        lg.warning(
                            f"Set leverage call for {symbol} returned non-zero retCode {ret_code}. Treating as failure despite no exception."
                        )
...
def place_trade(
    ...
    try:
        ...
        lg.error(
            f"Trade aborted ({symbol} {side}): Failed to convert position size {position_size} to float: {e}"
        )
        return None
    ...
    params = {
        ...
        # 'timeInForce': 'IOC', # ImmediateOrCancel for market might ensure quick fill or fail, but usually default is fine
        "reduceOnly": False,  # Explicitly set to False for opening/increasing position
    ...
    try:
        ...
        lg.info(
            f"{NEON_GREEN}Trade Placed Successfully! Order ID: {order_id}, Initial Status: {order_status}{RESET}"
        )
        lg.debug(
            f"Raw order response ({symbol} {side}): {order}"
    ...
    # --- Error Handling ---
        ...
        lg.error(
            f"{NEON_RED}Insufficient funds to place {side} order for {symbol}: {e}{RESET}"
        )
        ...
        err_str = str(e).lower()
        lg.error(
            f"{NEON_RED}Exchange error placing order for {symbol}: {e} (Code: {bybit_code}){RESET}"
        )
...
def _set_position_protection(
    ...
    # Check if values are valid Decimals and positive (or zero for activation price)
    ...
    # TSL activation price can be 0 for immediate activation
    has_tsl = (
    ...
    if not has_sl and not has_tp and not has_tsl:
        lg.info(
            f"No valid protection parameters (SL, TP, TSL > 0) provided for {symbol}. No protection set."
        )
    ...
    try:
        ...
        def format_distance(dist_decimal: Decimal | None) -> str | None:
            if dist_decimal is None or dist_decimal <= 0:
                return None
            try:
                # Use price_to_precision, as distance is formatted like a price
                # Important: Bybit API expects TSL distance as a positive value
                return exchange.price_to_precision(symbol, float(abs(dist_decimal)))
            except Exception as fmt_err:
                lg.warning(
                    f"Could not format distance {dist_decimal} using exchange.price_to_precision: {fmt_err}. Skipping this parameter."
                )
                return None
        ...
        if has_tsl:
            ...
            # Activation price uses standard price formatting, allow zero for immediate
            formatted_activation_price = None
            if tsl_activation_price == 0:
                # Bybit API expects '0' as a string for immediate activation
                formatted_activation_price = "0"
            elif tsl_activation_price is not None and tsl_activation_price > 0:
                formatted_activation_price = format_price(tsl_activation_price)

            if (
                formatted_tsl_distance and formatted_activation_price is not None
            ):  # Check activation price formatted correctly
                ...
                lg.error(
                    f"Failed to format valid TSL parameters for {symbol} (Dist: {formatted_tsl_distance}, Act: {formatted_activation_price}). Cannot set TSL."
                )
                has_tsl = False  # Mark TSL as failed

        # Fixed Stop Loss - Only add if TSL was NOT successfully added
        if has_sl and not tsl_added_to_params:
        ...
        elif has_sl and tsl_added_to_params:
            lg.warning(
                f"Both valid 'stopLoss' and 'trailingStop' provided for {symbol}. Prioritizing TSL. Fixed 'stopLoss' parameter ignored."
            )
    ...
    if not sl_added_to_params and not tp_added_to_params and not tsl_added_to_params:
        ...
        if has_sl or has_tp or has_tsl:  # Check original intent
            return False
        else:
    ...
    try:
        ...
        if ret_code == 0:
            ...
            no_change_msg = "stop loss take profit and trailing stop were not modified"  # Updated based on potential API response format
            processed_ret_msg = (
                ret_msg.lower()
                .replace(",", "")
                .replace(".", "")
            ...
            if processed_ret_msg == no_change_msg.replace(" ", ""):
                lg.info(
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        lg.info(
            f"  => Activation Price (API): {activation_price:.{price_precision}f if activation_price > 0 else '0 (Immediate)'}"
        )
        lg.info(
            f"  => Trailing Distance (API): {trailing_distance:.{price_precision}f}"
        )
        if take_profit_price and take_profit_price > 0:
            lg.info(
                f"  Take Profit Price: {take_profit_price:.{price_precision}f} (Will be set alongside TSL)"
            )
        else:
...
def analyze_and_trade_symbol(
    ...
    lg.info(
        f"ATR: {f'{current_atr_float:.{price_precision + 1}f}' if current_atr_float and pd.notna(current_atr_float) else 'N/A'}"
    )
    lg.info(
        f"Potential Initial SL (for new trade sizing): {sl_potential if sl_potential else 'N/A'}"
    )
    lg.info(
        f"Potential Initial TP (for new trade): {tp_potential if tp_potential else 'N/A'}"
    )
    ...
    if open_position is None:
        if signal in ["BUY", "SELL"]:
            ...
            if trade_order and trade_order.get("id"):
                ...
                lg.info(
                    f"Order {order_id} placed for {symbol}. Waiting briefly for position confirmation..."
                )
                    ...
                    try:
                        ...
                        if entry_price_actual_str and pos_size_actual_str:
                            try:
                                ...
                                if entry_price_actual > 0 and abs(pos_size_actual) > 0:
                                    valid_entry = True
                                else:
                                    lg.error(
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        current_sl_str = open_position.get("stopLossPrice")  # May be None or '0'
        current_tp_str = open_position.get("takeProfitPrice")  # May be None or '0'
        tsl_distance_str = open_position.get(
            "trailingStopLoss"
        )  # String distance or '0'/'0.0'
        # Determine if TSL is active based on distance being > 0
        is_tsl_active = False
        if tsl_distance_str:
            try:
                # Use Decimal comparison for accuracy
                if Decimal(str(tsl_distance_str)) > 0:
                    is_tsl_active = True
            except Exception:
                pass  # Ignore parsing errors, assume inactive

        # Log current state including TSL status
        lg.info(
            f"Existing {pos_side.upper()} position found for {symbol}. Size: {pos_size_str}, Entry: {entry_price_str}, SL: {current_sl_str or 'N/A'}, TP: {current_tp_str or 'N/A'}, TSL Active: {is_tsl_active}"
        )
        ...
        if (pos_side == "long" and signal == "SELL") or (
            ...
            try:
                ...
                close_params = {
                    "positionIdx": open_position.get("info", {}).get(
                        "positionIdx", 0
                ...
                if "bybit" in exchange.id.lower():
                    ...
                    close_params["category"] = category
                ...
                # Bybit V5 generally handles SL/TP cancellation when position is closed via market/limit order.
                # Explicit cancellation might be needed if closing via API is slow or uncertain.
                # For now, rely on Bybit's behavior.
                lg.debug(
                    "Skipping explicit SL/TP cancellation before closing (assuming exchange handles it)..."
                )
            ...
            ) as e:  # Should not happen for reduceOnly usually unless fees are high
                lg.error(
                    f"{NEON_RED}Error closing position (InsufficientFunds?): {e}{RESET}"
                )
            except ccxt.InvalidOrder as e:
                ...
                lg.error(
                    f"{NEON_RED}Error closing position (NetworkError): {e}. Manual check required!{RESET}"
                )
            except ccxt.ExchangeError as e:
                bybit_code = getattr(e, "code", None)
                lg.error(
                    f"{NEON_RED}Error closing position (ExchangeError): {e} (Code: {bybit_code}){RESET}"
                )
            ...
            if config.get("enable_break_even", False) and not is_tsl_active:
                ...
                try:
                    ...
                    if not entry_price_str or entry_price_str == "N/A":
                        raise ValueError("Missing entry price for BE check")
                    entry_price = Decimal(str(entry_price_str))
                    if entry_price <= 0:
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        # Get current SL price as Decimal (handle None, '0', 'N/A', errors)
                        current_sl_price = None
                        if current_sl_str and str(current_sl_str).strip() not in [
                            "0",
                            "0.0",
                            "N/A",
                        ]:
                            try:
                                ...
                                if current_sl_price <= 0:
                                    current_sl_price = None  # Treat invalid SL as None
                            except Exception:
                                lg.warning(
                                    f"Could not parse current SL price '{current_sl_str}' for BE comparison."
                                )
                        ...
                        if (
                            ...
                            if current_sl_price is None:  # No current SL, set BE SL
                                ...
                                update_be = True
                                lg.info(
                                    f"Profit target hit. Current SL {current_sl_price} < Target BE SL {be_stop_price}."
                                )
                            elif (
                                ...
                                lg.debug(
                                    f"BE Triggered, but current SL ({current_sl_price}) is already at or better than target BE SL ({be_stop_price}). No update needed."
                                )
                        else:
                            lg.warning(
                        ...
                        if update_be:
                            ...
                            if current_tp_str and str(current_tp_str).strip() not in [
                                "0",
                                "0.0",
                                "N/A",
                            ]:
                                try:
                                    current_tp_price_dec = Decimal(str(current_tp_str))
                                    if current_tp_price_dec > 0:
                                        current_tp_price = current_tp_price_dec
                                except Exception:
                                    pass
                            ...
                            success = _set_position_protection(
                                ...
                                trailing_stop_distance=None,  # Ensure TSL is cancelled/overridden
    ...
    try:
        while True:
            ...
            # --- Handle Specific Errors within the loop ---
                ...
                wait_time = (
                    int(e.args[0].split(" ")[-2]) if "try again in" in e.args[0] else 60
                )
...
if __name__ == "__main__":
    ...
    # to a file named 'livexy.py'. This is unusual for a production script
    # but kept here as it was part of the original input structure.
    # Ideally, deployment involves copying the script file directly.
