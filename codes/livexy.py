    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if signal == "BUY":
                ...
                # Use min_tick_size for quantization
...
def fetch_balance(
    ...
    try:
                ...
                # Bybit V5 specific check (nested):
                    ...
                    if isinstance(balance_list, list):
                        ...
                        for account in balance_list:
                            # Check the account type matches where we expect the currency
                        ...
                        if found_in_nested:
                            # If found in nested, need to save the whole balance_info and break outer loop
                            lg.debug(
        ...
        # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
            ...
            balance_list = balance_info["info"]["result"]["list"]
            for account in balance_list:
                # Determine which account type was likely successful (or default)
                # This check might be complex if default fetch was used, assume CONTRACT if unsure.
                relevant_acc_type = "CONTRACT"  # Default assumption
                # If balance_info has params from a successful typed fetch, use that type
                if balance_info.get("params", {}).get("type") in account_types_to_try:
                    relevant_acc_type = balance_info["params"]["type"]

                if account.get("accountType") == relevant_acc_type:
                    coin_list = account.get("coin")
                    if isinstance(coin_list, list):
                        for coin_data in coin_list:
                            if coin_data.get("coin") == currency:
                                # Prefer Bybit V5 'availableToWithdraw' or 'availableBalance' (often same for CONTRACT)
            ...
            if available_balance_str is None:
                lg.warning(
                    f"{currency} not found within Bybit V5 'info.result.list[].coin[]' structure for relevant account type."
                )
    ...
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                params["symbol"] = symbol  # Bybit V5 often uses symbol in params too
        ...
        if active_position:
            ...
            def get_valid_price_from_info(key: str) -> str | None:
                ...
                if (
                    val_str and val_str != "0" and val_str != "0.0"
                ):  # Bybit uses '0' for inactive
                    try:
            ...
            entry_price = format_log_price("entryPrice", entry_price_str)
            ...
            liq_price = format_log_price(
                "liquidationPrice", active_position.get("liquidationPrice")
            )
            ...
            sl_price = format_log_price(
                "stopLossPrice", active_position.get("stopLossPrice")
            )
            tp_price = format_log_price(
                "takeProfitPrice", active_position.get("takeProfitPrice")
            )
            tsl_dist = format_log_price(
                "trailingStopLoss", active_position.get("trailingStopLoss")
            )
            tsl_act = format_log_price(
                "tslActivationPrice", active_position.get("tslActivationPrice")
            )
...
def _set_position_protection(
    ...
    try:
        ...
        if ret_code == 0:
            ...
            no_change_msg = "stop loss not modified take profit not modified trailing stop not modified"
            if ret_msg.lower().replace(",", "").replace(".", "") == no_change_msg:
                lg.info(
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        temp_analyzer = TradingAnalyzer(
            df=pd.DataFrame(), logger=lg, config=config, market_info=market_info
        )
        price_precision = temp_analyzer.get_price_precision()
        Decimal(
            "1e-" + str(price_precision)
        )  # For display/checks only, API uses price_to_precision
            ...
            if config.get("enable_break_even", False) and not is_tsl_active:
                ...
                try:
                    ...
                    if price_diff >= profit_target_price_diff:
                        ...
                        if current_sl_str and str(current_sl_str) not in [
                            "0",
                            "0.0",
                            "N/A",
                        ]:
                        ...
                        if update_be:
                            ...
                            if current_tp_str and str(current_tp_str) not in [
                                "0",
                                "0.0",
                                "N/A",
                            ]:
                            ...
                            success = _set_position_protection(
                                ...
                                if current_tp_price and current_tp_price > 0
                                else None,  # Preserve existing TP
