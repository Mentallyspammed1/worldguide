...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> str | None:
        ...
        for col in result_df.columns:
            if base_name.lower() in col.lower():
                ...
                self.logger.debug(
                    f"Found column '{col}' for base '{base_name}' using fallback search."
                )
                return col
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if last_close > bb_middle:
            ...
            proximity_to_upper = (
                (last_close - bb_middle) / (bb_upper - bb_middle)
        ...
        if last_close < bb_middle:
            ...
            proximity_to_lower = (
                (bb_middle - last_close) / (bb_middle - bb_lower)
                if (bb_middle - bb_lower) > 0
                else 0
            )
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if signal == "BUY" and stop_loss >= entry_price:
                ...
                stop_loss = (entry_price - min_sl_distance).quantize(
                    rounding_factor, rounding=ROUND_DOWN
                )
    ...
    try:
        ...
        # 2. Bybit V5 structure (often nested): Check 'info' -> 'result' -> 'list'
            ...
            if available_balance_str is None:
                lg.warning(
                    f"{currency} not found within Bybit V5 'info.result.list[].coin[]' structure."
                )
...
def calculate_position_size(
    ...
    try:
        ...
        if (
            ...
            if entry_price > 0:
                calculated_size = risk_amount_quote / (
                    sl_distance_per_unit / entry_price * contract_size
                )  # contract_size might be 1 here if value is defined differently
            else:
                lg.error(
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            # Determine Contract Value in Quote (e.g., 1 USD for Bybit Inverse)
            # This needs to be accurate for the specific contract! Assuming 1 Quote unit for placeholder.
            contract_value_quote = Decimal(
                "1"
            )  # Placeholder - **VERIFY FOR SPECIFIC INVERSE CONTRACT**
        ...
        if min_cost > 0 and current_cost < min_cost:
            ...
            if market_info.get("linear", True) or not is_contract:
                ...
                contract_value_quote = Decimal("1")  # Placeholder - **VERIFY**
...
def get_open_position(
    ...
    Ensures SL/TP/TSL info is included if available from exchange 'info' dict.
    """
    lg = logger
    try:
        ...
        try:
            ...
            if "bybit" in exchange.id.lower():
                ...
                # Simplified: assume 'linear' if not specified, might need adjustment
                # market = exchange.market(symbol) # Fetch market info if needed
        ...
        if active_position:
            ...
            if active_position.get("stopLossPrice") is None:
                sl_val = info_dict.get("stopLoss")
                # Ensure SL is a valid price (not 0 or empty string) before assigning
                if sl_val and Decimal(str(sl_val)) > 0:
                    active_position["stopLossPrice"] = sl_val
            if active_position.get("takeProfitPrice") is None:
                tp_val = info_dict.get("takeProfit")
                if tp_val and Decimal(str(tp_val)) > 0:
            ...
            active_position["trailingStopLoss"] = info_dict.get(
                "trailingStop"
            )  # TSL Distance/Value (string '0.0' if inactive)
            active_position["tslActivationPrice"] = info_dict.get(
                "activePrice"
            )  # TSL Activation Price (string '0.0' if inactive/not set)
            ...
            def format_log_price(key, price_val, precision=6):
                ...
                try:
                    ...
                    if d_price > 0 or (
                        ...
                        return "N/A"  # Treat non-positive as N/A for SL/TP/Entry etc.
                except Exception:
        ...
        if "bybit" in exchange.id.lower():
            ...
            try:
                ...
                if margin_mode is None:
                    margin_mode = pos_info.get("info", {}).get(
                        "tradeMode"
                ...
                # Handle "no position found" error specifically
                ...
                if hasattr(e, "code") and e.code in no_pos_codes_v5:
                    ...
                    # Cannot reliably determine mode here, proceed with setting both buy/sell leverage
                else:
                    lg.warning(
                        f"Could not reliably fetch margin mode for {symbol} before setting leverage (Error fetching position): {e}. Proceeding cautiously."
                    )
            except Exception as e:
                lg.warning(
        ...
        if response is not None:
            if isinstance(response, dict):
                ...
                if ret_code == 0:
                    ...
                    lg.warning(
                        f"Set leverage call for {symbol} returned non-zero retCode {ret_code}. Treating as potential failure despite no exception."
                    )
...
def _set_position_protection(
    ...
    # Check if values are valid Decimals and positive
    ...
    )  # Activation price can be 0 if immediate
    ...
    try:
        ...
        def format_distance(dist_decimal: Decimal | None) -> str | None:
            ...
            try:
                ...
                return exchange.price_to_precision(symbol, float(dist_decimal))
            except Exception as fmt_err:
                lg.warning(
        ...
        if has_tsl:
            ...
            if tsl_activation_price == 0:
                formatted_activation_price = (
                    "0"  # Use string '0' if activation is immediate/zero
                )
            ...
            if formatted_tsl_distance and formatted_activation_price:
                ...
                # Mark TSL as successfully added to params
                tsl_added = True
            else:
                lg.error(
    ...
    if (
        ...
        # Let's return False if any of the initial has_sl/tp/tsl flags were True but params are empty now.
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        # Trail distance = Activation Price * Callback Rate (common method if activation > 0)
        # Or Trail distance = Entry Price * Callback Rate (if activation is immediate)
        # Or fixed ATR multiple? Let's stick to callback rate based on entry/activation.
        base_price_for_distance = (
            entry_price if activation_price == 0 else activation_price
        )
        trailing_distance_raw = base_price_for_distance * callback_rate
        ...
        if trailing_distance < min_tick_size:
            ...
            lg.error(
                f"{NEON_RED}Calculated TSL distance is zero or negative ({trailing_distance}) for {symbol}. Check callback rate ({callback_rate}) and base price ({base_price_for_distance}).{RESET}"
            )
            return False
...
def analyze_and_trade_symbol(
    ...
    current_atr = analyzer.indicator_values.get("ATR")  # Get float ATR

    # --- 3. Log Analysis Summary ---
    # (Signal logging is handled within generate_trading_signal now)
    lg.info(
        f"ATR: {f'{current_atr:.{price_precision + 1}f}' if current_atr and pd.notna(current_atr) else 'N/A'}"
    )
            ...
            if trade_order and trade_order.get("id"):
                ...
                if confirmed_position:
                    ...
                    try:
                        ...
                        pos_size_actual = (
                            Decimal(str(pos_size_actual_str))
                            if pos_size_actual_str
                            else Decimal("0")
                        )
                        ...
                        if config.get("enable_trailing_stop", False):
                            ...
                            _, tp_recalc, _ = analyzer.calculate_entry_tp_sl(
                                entry_price_actual, signal
                            )
                            lg.info(
                                f"Setting Trailing Stop Loss for {symbol} (TP target: {tp_recalc})..."
                            )
                            ...
                            _, tp_recalc, sl_recalc = analyzer.calculate_entry_tp_sl(
                                entry_price_actual, signal
                            )
                            lg.info(
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        current_sl_str = open_position.get("stopLossPrice")  # May be None
        current_tp_str = open_position.get("takeProfitPrice")  # May be None
...
def main() -> None:
    ...
    console_log_level = logging.INFO  # Default console level
    if console_handler:
    ...
    try:
        init_logger.info(
            f"Using Pandas TA Version: {ta.version() if callable(ta.version) else getattr(ta, 'version', 'N/A')}"
        )
    except Exception:
        ...
        while True:
            ...
            # --- Handle Specific Errors within the loop ---
                ...
                symbol_logger.warning(
                    f"{NEON_YELLOW}Rate limit exceeded: {e}. Waiting 60s...{RESET}"
                )
                time.sleep(60)
            except (
                ccxt.NetworkError,
