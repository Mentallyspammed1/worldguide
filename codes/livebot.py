...
def calculate_position_size(
    ...
    try:
        ...
        if market_info.get('linear', True) or not is_contract:  # Treat spot as linear type
            ...
            if entry_price > 0:
                calculated_size = risk_amount_quote / (sl_distance_per_unit / entry_price * contract_size)  # contract_size might be 1 here if value is defined differently
            else:
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> dict | None:
    ...
    try:
        ...
        if active_position:
            ...
            active_position['trailingStopLoss'] = info_dict.get('trailingStop')  # TSL Distance/Value (string '0.0' if inactive)
            active_position['tslActivationPrice'] = info_dict.get('activePrice')  # TSL Activation Price (string '0.0' if inactive/not set)
            ...
            try:
                ...
                lg.debug(f"Detected margin mode from existing position for {symbol}: {margin_mode}")
                 ...
                 if hasattr(e, 'code') and e.code in no_pos_codes_v5:
                      ...
                      lg.warning(f"Could not reliably fetch margin mode for {symbol} before setting leverage (Error fetching position): {e}. Proceeding cautiously.")
            except Exception as e:
            ...
            if isinstance(response, dict):
                 ...
                 if ret_code == 0:
                     ...
                     verified = True
                      ...
                      lg.warning(f"Set leverage call for {symbol} returned non-zero retCode {ret_code}. Treating as potential failure despite no exception.")
...
def _set_position_protection(
    ...
    has_tsl = isinstance(trailing_stop_distance, Decimal) and trailing_stop_distance > 0 and \
              isinstance(tsl_activation_price, Decimal)  # Activation price can be 0 if immediate
...
def set_trailing_stop_loss(
    ...
    try:
        ...
        if trailing_distance < min_tick_size:
            ...
            lg.error(f"{NEON_RED}Calculated TSL distance is zero or negative ({trailing_distance}) for {symbol}. Check callback rate ({callback_rate}) and base price ({base_price_for_distance}).{RESET}")
            return False
...
def main() -> None:
    ...
    try:
        init_logger.info(f"Using Pandas TA Version: {ta.version() if callable(ta.version) else getattr(ta, 'version', 'N/A')}")
    except Exception:
