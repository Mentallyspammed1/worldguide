        ...
        try:
            ...
            if bybit_code in non_retryable_codes:
                ...
                if bybit_code == 10001:
                    ...
                    bybit_code == 110017 or bybit_code == 110005 or bybit_code == 110006
                ):
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        if len(df) < initial_len:
            lg.warning(
                f"Dropped {initial_len - len(df)} rows with NaN values in OHLC columns for {symbol}."
            )
                    ...
                    if isinstance(coin_list, list):
                        for coin_data in coin_list:
                            if (
                                ...
                                free = coin_data.get(
                                    "availableToWithdraw"
                                )  # UTA uses this primarily
                                if free is None or str(free).strip() == "":
                                    free = coin_data.get(
                                        "availableBalance"
                                    )  # Check this too, esp. for non-UTA
                                if free is None or str(free).strip() == "":
                                    lg.debug(
...
class TradingAnalyzer:
    ...
    def _define_ta_strategy(self) -> None:
        ...
        stochrsi_rsi_w = int(
            get_param("stoch_rsi_rsi_window", DEFAULT_STOCH_WINDOW)
        )  # Inner RSI
        ...
        if indi_cfg.get("psar"):
            ...
            self.ta_column_map["PSAR_Long"] = (
                l_col  # Value when SAR is below (long trend)
            )
            self.ta_column_map["PSAR_Short"] = (
                s_col  # Value when SAR is above (short trend)
            )
    ...
    def _check_stoch_rsi(self) -> float | None:
        ...
        if k < oversold and d < oversold:
            ...
            score = (
                -1.0 if k < d else -0.8
            )  # Bearish crossover deep OB (stronger if k<d)
    ...
    def _check_cci(self) -> float | None:
        ...
        if cci >= ob:
            ...
            score = (
                -cci / ob
            )  # Linear scale between -100 and 100 (cci=50 -> -0.5, cci=-50 -> 0.5)
        return max(-1.0, min(1.0, score))
    ...
    def _check_psar(self) -> float | None:
        ...
        psar_l = self.indicator_values.get(
            "PSAR_Long"
        )  # Decimal or None (value when SAR is long)
        psar_s = self.indicator_values.get(
            "PSAR_Short"
        )  # Decimal or None (value when SAR is short)
    ...
    def generate_trading_signal(
        ...
        if not self.weights:
            self.logger.debug(
                f"No weights found for active set '{self.active_weight_set_name}'. Holding."
            )
            return "HOLD"
        ...
        for indicator_key, is_enabled in self.config.get("indicators", {}).items():
            ...
            try:
                ...
                self.logger.warning(
                    f"Invalid weight '{weight_val}' for '{indicator_key}'. Skipping."
                )
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            tp_offset = (
                tp_offset_raw if tp_offset_raw > 0 else min_tick
            )  # Use min tick if calc is zero/negative
            ...
            if stop_loss is not None:
                ...
                if (
                    signal == "BUY"
                    and stop_loss
                    >= quantized_entry - min_dist_from_entry + (min_tick / Decimal("2"))
                ):
...
def calculate_position_size(
    ...
    if amount_digits is None:
        lg.error(
            f"Size Calc Error ({symbol}): Amount precision missing from market info"
        )
        return None
    if not (0 < risk_per_trade < 1):
        lg.error(
            f"Size Calc Error ({symbol}): Invalid risk_per_trade value {risk_per_trade}"
        )
        return None
    ...
    try:
        ...
        if not size_unquantized.is_finite() or size_unquantized <= 0:
            lg.error(
                f"Size Calc Error ({symbol}): Invalid unquantized size calculated ({size_unquantized})"
            )
            return None
        ...
        quantized_size = quantized_size.quantize(
            step_size
        )  # Ensure exact decimal places match step size
        ...
        if max_order_amount is not None and quantized_size > max_order_amount:
            ...
            if (
                quantized_size < min_order_amount if min_order_amount else Decimal("0")
        ...
        for pos in all_positions:
            ...
            try:
                ...
                if pos_size_str is None:
                    lg.debug(
                        f"Skipping position entry for {symbol} due to missing size field. Data: {pos}"
                    )
                    continue
                ...
                standardized_pos["contracts"] = (
                    abs_size  # Store absolute size as Decimal
                )
                ...
                lg.info(
                    f"Found active {pos_side} position for {symbol}: Size={abs_size}, Entry={entry_log}"
                )
...
def set_leverage_ccxt(
    ...
    try:
        ...
        if result is not None and isinstance(result, dict):
            ...
            lg.error(
                f"{NEON_RED}set_leverage call returned unexpected result for {symbol}: {result}{RESET}"
            )
            return False
...
def create_order_ccxt(
    ...
    if order_type_lower == "limit" and (
        ...
        lg.error(
            f"Valid positive price required for limit order ({symbol}, Price: {price})"
        )
        return None
...
def set_protection_ccxt(
    ...
    try:
        ...
        if not hasattr(exchange, method_to_call):
            ...
            lg.error(
                f"Cannot call {method_to_call}. Ensure it's mapped in exchange options."
            )
            return False
        ...
        if result and isinstance(result, dict) and result.get("retCode") == 0:
            ...
            if ret_code == 170140:
                lg.warning(
                    f"Hint: Set protection failed because position might not exist (Code {ret_code})."
                )
...
def close_position_ccxt(
    ...
    try:
        ...
        if pos_idx_to_close is not None:
            ...
            lg.debug(
                f"Setting positionIdx={pos_idx_to_close} for closing order ({symbol})."
            )
        else:
...
def run_bot(
    ...
    for symbol in symbols_to_trade:
        ...
        bot_state[symbol].setdefault(
            "last_entry_price", None
        )  # Stored as string, represents last known entry
    ...
    while True:
        ...
        active_positions: dict[
            str, dict
        ] = {}  # Stores standardized position data {symbol: position_dict}
        main_logger.debug("Fetching active positions for configured symbols...")
        for symbol in symbols_to_trade:
            temp_logger = get_logger(
                symbol, is_symbol_logger=True
            )  # Use symbol-specific logger for fetch
            try:
                market_info = get_market_info(exchange, symbol, temp_logger)
                if not market_info:
                    temp_logger.warning(
                        f"Skipping position check for {symbol}: Could not get market info."
                    )
                    continue
                if market_info.get(
                    "is_contract"
                ):  # Only fetch positions for derivatives
        ...
        for symbol in symbols_to_trade:
            ...
            symbol_state = bot_state[
                symbol
            ]  # Get reference to this symbol's state dict
            ...
            try:
                ...
                if not market_info:
                    symbol_logger.error(
                        f"Could not get market info for {symbol}. Skipping cycle for this symbol."
                    )
                    continue
                ...
                if (
                    df_raw.empty or len(df_raw) < base_limit // 2
                ):  # Check if significantly less than needed
                ...
                if config.get("indicators", {}).get("orderbook"):
                    try:
                        ...
                        symbol_logger.warning(
                            f"Failed to fetch order book for {symbol}: {ob_err}"
                        )
                ...
                try:
                    ...
                    symbol_logger.error(
                        f"Unexpected analyzer initialization error: {analyze_err}",
                        exc_info=True,
                    )
                    continue
                ...
                if current_position:
                    ...
                    if position_closed_in_manage:
                        ...
                        open_positions_count = max(
                            0, open_positions_count - 1
                        )  # Decrement count
                        symbol_logger.info(
                            f"Position for {symbol} closed during management routine."
                        )
                ...
                if not active_positions.get(symbol) and market_info.get("is_contract"):
                    if open_positions_count < max_allowed_positions:
                        ...
                        symbol_state["last_signal"] = (
                            signal  # Store latest signal regardless of entry
                        )
                                ...
                                if current_balance is not None and current_balance > 0:
                                    ...
                                    if opened_new:
                                        open_positions_count += (
                                            1  # Increment count if entry successful
                                        )
                    ...
                    symbol_logger.debug(
                        f"Symbol {symbol} is not a contract. Skipping position management/entry logic."
                    )
                ...
                if is_adverse_cross:
                    ...
                    if config.get("enable_trading"):
                        ...
                        if close_result:
                            ...
                            logger.error(
                                f"Failed to place MA Cross close order for {symbol}. Position remains open."
                            )
                            return False  # Retry next cycle
                    else:
                        logger.info(
                            f"MA Cross exit triggered for {symbol}, but trading is disabled."
                        )
        ...
        if (
            ...
            if atr_val and atr_val.is_finite() and atr_val > 0:
                try:
                        ...
                        if min_tick and offset_ticks >= 0:
                            ...
                            if (
                                ...
                                if config.get("enable_trading"):
                                    ...
                                    pos_idx = pos_info.get(
                                        "positionIdx", 0
                                    )  # Use index from current position
                                    ...
                                    analyzer.break_even_triggered = (
                                        True  # Update state even if not trading
                                    )
                            else:
                                logger.error(
                    ...
                    logger.error(
                        f"Error during Break-Even calculation for {symbol}: {be_calc_err}"
                    )
                except Exception as be_err:
                    logger.error(
                        f"Unexpected error during Break-Even check for {symbol}: {be_err}",
                        exc_info=True,
                    )
            else:
                logger.warning(
...
def attempt_new_entry(
    ...
    if not quantized_entry or not stop_loss_price:
        logger.error(
            f"Cannot enter {signal} ({symbol}): Failed to calculate valid Entry/SL price. Aborting entry."
        )
        return False
    ...
    if not entry_order or not entry_order.get("id"):
        logger.error(
            f"Failed to place entry market order for {signal} {symbol}. Aborting entry."
        )
        return False
    ...
    try:
        ...
        if updated_position and updated_position.get("entryPrice"):
            ...
            current_size_fetched = updated_position.get(
                "contracts"
            )  # Should be Decimal
            if (
            ...
            if abs(filled_size - position_size) / position_size > Decimal(
                ...
                logger.warning(
                    f"Filled size {filled_size} differs >5% from ordered {position_size} for {symbol}."
                )
        ...
        logger.error(
            f"Error confirming entry for {symbol}: {confirm_err}. Using calculated entry price.",
            exc_info=True,
        )
    ...
    if config.get("enable_trailing_stop"):
        try:
            cb_rate = Decimal(
                str(config.get("trailing_stop_callback_rate"))
            )  # Distance as % of price
            act_perc = Decimal(
                str(config.get("trailing_stop_activation_percentage"))
            )  # Activation profit %
            min_tick = analyzer.get_min_tick_size()
            ...
            if cb_rate > 0 and min_tick:
                ...
                )  # Ensure at least 1 tick distance
                ...
                if act_perc > 0:
                    ...
                    if tsl_activation_price and tsl_activation_price > 0:
                        if (
                            ...
                            adj_act_price = actual_entry_price + (
                                min_tick if signal == "BUY" else -min_tick
                            )
                            tsl_activation_price = analyzer.quantize_price(
                                adj_act_price, rounding
                            )
                            logger.debug(
                        ...
                        tsl_activation_price = (
                            None  # Invalid activation price calculated
                        )
                        logger.warning(
                    ...
                    tsl_activation_price = (
                        None  # Use None to signal immediate activation (API uses "0")
                    )
                ...
                logger.warning(
                    f"Cannot calc TSL distance for {symbol}: Min tick size unavailable."
                )
            ...
            logger.error(
                f"Unexpected error calculating TSL params for {symbol}: {tsl_err}",
                exc_info=True,
            )
    ...
    if not protection_set:
        ...
        if config.get("enable_trading"):
            ...
            if pos_to_close:
                ...
                if close_result:
                    logger.info(
                        f"Emergency close order placed successfully for {symbol}."
                    )
                else:
                    logger.error(
                        f"{NEON_RED}EMERGENCY CLOSE FAILED for {symbol}! Manual intervention required!{RESET}"
                    )
            else:
                logger.error(
    ...
    # --- Success ---
    logger.info(
        f"{NEON_GREEN}Successfully entered {signal} trade for {symbol} with initial protection set.{RESET}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable DEBUG console logging level."
    )
    ...
    if config.get("use_sandbox"):
        ...
        main_logger.warning(
            f"{NEON_RED}--- LIVE EXCHANGE MODE is ACTIVE --- Real funds are at risk! ---{RESET}"
        )
    ...
    if exchange:
        main_logger.info(
            f"{NEON_GREEN}Exchange initialized successfully. Starting main bot loop...{RESET}"
        )
        try:
            ...
            main_logger.critical(
                f"{NEON_RED}!!! BOT CRASHED UNEXPECTEDLY IN MAIN LOOP: {e} !!!{RESET}",
                exc_info=True,
            )
