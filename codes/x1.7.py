...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger, market_info: Dict) -> Optional[Decimal]:
    ...
    try:
        ...
        ticker = safe_ccxt_call(exchange, 'fetch_ticker', lg, symbol=symbol, params=params) # Pass symbol for CCXT standard, params for V5
        ...
        if isinstance(ticker, dict):
            ...
            if ('info' in ticker and isinstance(ticker['info'], dict) and
                'result' in ticker['info'] and isinstance(ticker['info']['result'], dict) and
                'list' in ticker['info']['result'] and isinstance(ticker['info']['result']['list'], list) and
                len(ticker['info']['result']['list']) > 0):
            ...
            # Check V5 structure (result -> item) - Less common?
                  ...
                  'result' in ticker['info'] and isinstance(ticker['info']['result'], dict)):
                    ...
                    if isinstance(coin_list, list):
                        for coin_data in coin_list:
                            if isinstance(coin_data, dict) and coin_data.get('coin') == currency:
                                ...
                                if free is None or str(free).strip() == "":
                                    lg.debug(f"'availableToWithdraw'/'availableBalance' missing/empty for {currency} in {account_type_checked}, trying 'walletBalance'")
                                    free = coin_data.get('walletBalance')

                                if free is not None and str(free).strip() != "":
                                    available_balance_str = str(free)
                                    field_used = 'unknown'
                                    if free == coin_data.get('availableToWithdraw'): field_used = 'availableToWithdraw'
                                    elif free == coin_data.get('availableBalance'): field_used = 'availableBalance'
                                    elif free == coin_data.get('walletBalance'): field_used = 'walletBalance'
...
class TradingAnalyzer:
    ...
    def calculate_fibonacci_levels(self, window: Optional[int] = None) -> Dict[str, Decimal]:
        ...
        try:
            ...
            if high <= low: # Use <= to handle cases where high == low
                 self.logger.warning(f"Invalid range (High <= Low): High={high}, Low={low} in window for Fib calculation on {self.symbol}. Cannot calculate levels.")
                 return {}
    ...
    def _check_volume_confirmation(self) -> Optional[float]:
        ...
        if vol is None or vol_ma is None or vol_ma <= 0: return None # Need MA > 0 for comparison
    ...
    def _check_rsi(self) -> Optional[float]:
        ...
        if rsi >= ob: score = max(-1.0, score * 1.1) # Increase bearishness above OB
        if rsi <= os: score = min(1.0, score * 1.1) # Increase bullishness below OS
        return max(-1.0, min(1.0, score))
    ...
    def _check_cci(self) -> Optional[float]:
        ...
        else: score = -cci / ob # Linear scale between -100 and 100 (cci=50 -> -0.5, cci=-50 -> 0.5)
        return max(-1.0, min(1.0, score))
    ...
    def _check_wr(self) -> Optional[float]:
        ...
        if wr >= ob: score = max(-1.0, score * 1.1) # Increase bearishness above OB (-20)
        if wr <= os: score = min(1.0, score * 1.1) # Increase bullishness below OS (-80)
        return max(-1.0, min(1.0, score))
    ...
    def _check_psar(self) -> Optional[float]:
        ...
        if psar_l_active and not psar_s_active: return 1.0  # SAR below price (bullish trend)
        if psar_s_active and not psar_l_active: return -1.0 # SAR above price (bearish trend)
    ...
    def _check_mfi(self) -> Optional[float]:
        ...
        score = (50.0 - mfi) / 50.0
        if mfi >= ob: score = max(-1.0, score * 1.1) # Boost bearishness
        if mfi <= os: score = min(1.0, score * 1.1) # Boost bullishness
        return max(-1.0, min(1.0, score))
    ...
    def _check_bollinger_bands(self) -> Optional[float]:
        ...
        close = self._get_indicator_float("Close")
        if bbl is None or bbu is None or close is None or bbu <= bbl: return None # Invalid bands
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict]) -> Optional[float]:
        ...
        try:
            ...
            if not isinstance(bids, list) or not isinstance(asks, list): return 0.0

            # Take top N levels
            top_bids = bids[:limit]
            top_asks = asks[:limit]
            if not top_bids or not top_asks: return 0.0
    ...
    def generate_trading_signal(self, current_price_dec: Decimal, orderbook_data: Optional[Dict]) -> str:
        ...
        for indicator_key, is_enabled in self.config.get("indicators", {}).items():
            ...
            try: weight = Decimal(str(weight_val))
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if sl_offset_raw < min_sl_offset_value:
                self.logger.warning(f"Calculated SL offset ({sl_offset_raw}) < minimum {MIN_TICKS_AWAY_FOR_SLTP} ticks ({min_sl_offset_value}). Adjusting SL offset.")
            ...
            if stop_loss is not None:
                ...
                elif signal == "SELL" and stop_loss <= quantized_entry + min_dist_from_entry - (min_tick / Decimal('2')):
...
def calculate_position_size(
    ...
    if amount_digits is None: lg.error(f"Size Calc Error ({symbol}): Amount precision missing from market info"); return None
    if not (0 < risk_per_trade < 1): lg.error(f"Size Calc Error ({symbol}): Invalid risk_per_trade value {risk_per_trade}"); return None
    ...
    try:
        ...
        if sl_distance_points <= 0: lg.error(f"Size Calc Error ({symbol}): SL distance points <= 0"); return None
        ...
        lg.debug(f"Size Calc ({symbol}): Bal={balance:.2f}, Risk={risk_per_trade*100:.2f}%, RiskAmt={risk_amount_quote:.4f}, SLDist={sl_distance_points}, RiskPerUnit={risk_per_unit_or_contract_quote:.8f}, UnquantSize={size_unquantized:.8f}")
        ...
        if min_order_amount is not None and quantized_size < min_order_amount:
            lg.warning(f"{NEON_YELLOW}Size Calc ({symbol}): Calculated size {quantized_size} < Min Amount {min_order_amount}. Cannot place order.{RESET}")
            return None
        if max_order_amount is not None and quantized_size > max_order_amount:
            lg.warning(f"{NEON_YELLOW}Size Calc ({symbol}): Calculated size {quantized_size} > Max Amount {max_order_amount}. Capping size.{RESET}")
            ...
            if quantized_size < min_order_amount if min_order_amount else Decimal('0'): # Check again after capping
                lg.error(f"Size Calc Error ({symbol}): Capped size {quantized_size} is below min amount {min_order_amount}.")
                return None
        ...
        if margin_required * buffer_factor > balance:
            lg.warning(f"{NEON_YELLOW}Size Calc ({symbol}): Required margin {margin_required:.4f} (incl. buffer) > Available balance {balance:.4f}. Cannot place order.{RESET}")
...
def fetch_positions_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger, market_info: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        if not isinstance(all_positions, list):
            lg.error(f"fetch_positions did not return a list for {symbol}. Type: {type(all_positions)}. Response: {all_positions}")
            return None
...
def set_leverage_ccxt(exchange: ccxt.Exchange, symbol: str, leverage: int, logger: logging.Logger, market_info: Dict) -> bool:
    ...
    try:
        ...
        if result is not None and isinstance(result, dict):
         ...
         lg.error(f"{NEON_RED}Failed to set leverage for {symbol} to {leverage}x: {e}{RESET}", exc_info=False) # Don't need full trace if safe_call logged it
         return False
    except Exception as e:
...
def create_order_ccxt(
    ...
    if order_type_lower == 'limit' and (price is None or not price.is_finite() or price <= 0):
        lg.error(f"Valid positive price required for limit order ({symbol}, Price: {price})"); return None
    ...
    try:
        ...
        lg.debug(f"CCXT Order Args: Symbol={symbol}, Type={order_type}, Side={side}, Amount={amount_float}, Price={price_float}, Params={order_params}")
...
def set_protection_ccxt(
    ...
    trailing_stop_price: Optional[Decimal] = None, # TSL distance/offset value
    ...
    try:
        ...
        if not hasattr(exchange, method_to_call):
            ...
            lg.warning(f"Method '{method_to_call}' not found directly on exchange object, attempting generic privatePost.")
        ...
        if result and isinstance(result, dict) and result.get('retCode') == 0:
            ...
            if ret_code == 170140: lg.warning(f"Hint: Set protection failed because position might not exist (Code {ret_code}).")
    ...
    try:
        ...
        if pos_idx_to_close is not None:
            ...
            lg.debug(f"Setting positionIdx={pos_idx_to_close} for closing order ({symbol}).")
             ...
             if config.get("position_mode") == "Hedge":
                 lg.warning(f"Position Index missing from position data for {symbol}, but Hedge Mode may be active. Defaulting close order to positionIdx=0. This might fail if not One-Way mode.")
...
def run_bot(exchange: ccxt.Exchange, config: Dict[str, Any], bot_state: Dict[str, Any]) -> None:
    ...
    main_logger.info(f"Position Mode: {config.get('position_mode', 'One-Way')} (Note: Hedge mode requires careful param setting)")
                    ...
                    if position:
                        ...
                        if entry_p_state is None and isinstance(entry_p_pos, Decimal):
                             try:
                                 ...
                                 temp_logger.info(f"Updated state's last_entry_price from fetched position: {entry_p_pos}")
        ...
        for symbol in symbols_to_trade:
            ...
            try:
                ...
                if config.get("indicators", {}).get("orderbook"):
                    ...
                    except Exception as ob_err: symbol_logger.warning(f"Failed to fetch order book for {symbol}: {ob_err}")
...
def manage_existing_position(
    ...
    if not all([symbol, position_side, isinstance(entry_price, Decimal), isinstance(pos_size, Decimal)]) or pos_size <= 0:
        logger.error(f"Invalid position data received for management: {position_data}"); return False
    if not current_price_dec.is_finite() or current_price_dec <= 0:
    ...
    try:
        ...
        if config.get("enable_ma_cross_exit"):
            ...
            if ema_s_f is not None and ema_l_f is not None:
                ...
                if is_adverse_cross:
                    logger.warning(f"{NEON_YELLOW}MA Cross Exit Triggered for {position_side} {symbol}! Attempting close.{RESET}")
                    if config.get("enable_trading"):
                        ...
                        if close_result:
                            ...
                            logger.error(f"Failed to place MA Cross close order for {symbol}. Position remains open."); return False # Retry next cycle
                    else:
            ...
            if atr_val and atr_val.is_finite() and atr_val > 0:
                try:
                    ...
                    if current_profit_points >= profit_target_points:
                        logger.info(f"{NEON_GREEN}Break-Even Trigger Met for {symbol}! (Profit: {current_profit_points:.4f} >= Target: {profit_target_points:.4f}){RESET}")
                        ...
                        if min_tick and offset_ticks >= 0:
                            ...
                            rounding_mode = ROUND_DOWN if position_side == 'long' else ROUND_UP # Round towards safety zone
                            ...
                            if be_stop_price:
                                if position_side == 'long' and be_stop_price <= entry_price:
                                    be_stop_price = analyzer.quantize_price(entry_price + min_tick, ROUND_UP) # Move at least one tick beyond
                                elif position_side == 'short' and be_stop_price >= entry_price:
                                    be_stop_price = analyzer.quantize_price(entry_price - min_tick, ROUND_DOWN) # Move at least one tick beyond
                            ...
                            if be_stop_price and be_stop_price.is_finite() and be_stop_price > 0:
                                ...
                                if config.get("enable_trading"):
                                    ...
                                    logger.info(f"Setting BE SL={be_stop_price}, keeping TP={tp_to_set}, TSL={'Active' if tsl_to_set else 'Inactive'}")
                                    success = set_protection_ccxt(
                                        exchange, symbol, stop_loss_price=be_stop_price, take_profit_price=tp_to_set,
...
def attempt_new_entry(
    ...
    if not quantized_entry or not stop_loss_price:
        logger.error(f"Cannot enter {signal} ({symbol}): Failed to calculate valid Entry/SL price. Aborting entry."); return False
    ...
    if not position_size or position_size <= 0:
        logger.error(f"Cannot enter {signal} ({symbol}): Position size calculation failed or resulted in zero/negative size ({position_size}). Aborting entry."); return False
    ...
    if market_info.get('is_contract'):
        if not set_leverage_ccxt(exchange, symbol, leverage, logger, market_info):
            logger.error(f"Failed to set leverage {leverage}x for {symbol}. Aborting entry."); return False
    ...
    if not entry_order or not entry_order.get('id'):
        logger.error(f"Failed to place entry market order for {signal} {symbol}. Aborting entry."); return False
    order_id = entry_order['id']

    # --- 5. Confirm Entry Price & Position (Wait and Fetch) ---
    logger.info(f"Entry market order ({order_id}) placed for {symbol}. Waiting {POSITION_CONFIRM_DELAY}s for fill confirmation...")
    ...
    try:
        ...
        if updated_position and updated_position.get('entryPrice'):
             ...
             else: logger.warning(f"Fetched position for {symbol} has invalid entry price ({entry_p_fetched}). Using calculated: {actual_entry_price}.")
             if isinstance(current_size_fetched, Decimal) and current_size_fetched.is_finite() and current_size_fetched > 0:
                 filled_size = abs(current_size_fetched) # Use absolute size
             else: logger.warning(f"Fetched position for {symbol} has invalid size ({current_size_fetched}). Using ordered: {filled_size}.")

             # Get the actual position index from the confirmed position
             position_idx_filled = updated_position.get('info', {}).get('positionIdx', position_idx_filled) # Use fetched index if available

             logger.info(f"Position Confirmed ({symbol}): Actual Entry={actual_entry_price:.{analyzer.get_price_precision_digits()}f}, Filled Size={filled_size}, PosIdx={position_idx_filled}")
            ...
            logger.warning(f"Could not fetch or confirm position details for {symbol} after entry order {order_id}. Using calculated entry price for protection setup.")
    except Exception as confirm_err:
        logger.error(f"Error confirming entry for {symbol}: {confirm_err}. Using calculated entry price.", exc_info=True)
    ...
    if config.get("enable_trailing_stop"):
        try:
            ...
            if cb_rate > 0 and min_tick:
                ...
                if act_perc > 0:
                    ...
                    if tsl_activation_price and tsl_activation_price > 0:
                        if (signal == "BUY" and tsl_activation_price <= actual_entry_price) or \
                           ...
                           logger.debug(f"Adjusted TSL activation price for {symbol} to be beyond entry: {tsl_activation_price}")
                    else:
                        tsl_activation_price = None # Invalid activation price calculated
                        logger.warning(f"Could not calculate valid TSL activation price for {symbol}. TSL will activate immediately if set.")
                else:
                ...
                logger.debug(f"TSL Params Calculated for {symbol}: Dist={tsl_distance}, ActP={tsl_activation_price if tsl_activation_price else 'Immediate'}")
            elif not min_tick: logger.warning(f"Cannot calc TSL distance for {symbol}: Min tick size unavailable.")
        ...
        except Exception as tsl_err: logger.error(f"Unexpected error calculating TSL params for {symbol}: {tsl_err}", exc_info=True)
    ...
    if not protection_set:
        logger.error(f"{NEON_RED}CRITICAL: Failed to set initial protection (SL/TP/TSL) for {symbol} after entry order {order_id}! Position may be unprotected.{RESET}")
        ...
        if config.get("enable_trading"):
            ...
            if pos_to_close:
                ...
                if close_result: logger.info(f"Emergency close order placed successfully for {symbol}.")
                else: logger.error(f"{NEON_RED}EMERGENCY CLOSE FAILED for {symbol}! Manual intervention required!{RESET}")
            else:
                logger.error(f"{NEON_RED}Could not fetch position for emergency close of {symbol}. Manual intervention required!{RESET}")
        # --- End Emergency Close ---
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbols", type=str, default=None, help="Override symbols in config (comma-separated list, e.g., 'BTC/USDT:USDT,ETH/USDT:USDT').")
    parser.add_argument("--live", action="store_true", help="Enable LIVE trading. Overrides 'use_sandbox' and 'enable_trading' in config. USE WITH EXTREME CAUTION!")
    ...
    if args.live:
        main_logger.warning(f"{NEON_RED}--- LIVE TRADING ENABLED via --live flag! This overrides config settings for 'enable_trading' and 'use_sandbox'. ---{RESET}")
