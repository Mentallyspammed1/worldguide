...
def calculate_volatility_levels(df: pd.DataFrame, config: Dict[str, Any], logger: logging.Logger) -> pd.DataFrame:
    ...
    try:
        ...
        df_calc['vol_norm_strat'] = np.where(
            ...
            (df_calc['volume_float'].fillna(0) / max_vol_lookback * 100.0),
...
def calculate_pivot_order_blocks(df: pd.DataFrame, config: Dict[str, Any], logger: logging.Logger) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            # Rolling max for PH
            rolling_max = high_col_dec.rolling(window=ph_window_size, center=False).max().shift(-right_h)
            # Identify where the value at index `i` (shifted by left_h) is the max of its window
            is_ph = (high_col_dec == rolling_max)
            # Additional check: Ensure strict inequality with neighbors if required by definition
            # (Rolling max itself doesn't guarantee strictness if multiple bars have the same max)
            # This strict check is complex to vectorize efficiently with Decimal, loop might be clearer if needed.
            # For now, using rolling max equality:
            df_calc.loc[is_ph, 'ph_strat'] = high_col_dec[is_ph]

            # Rolling min for PL
            rolling_min = low_col_dec.rolling(window=pl_window_size, center=False).min().shift(-right_l)
            is_pl = (low_col_dec == rolling_min)
            df_calc.loc[is_pl, 'pl_strat'] = low_col_dec[is_pl]

        except TypeError as e:
             # Handle potential issues if rolling doesn't support Decimal directly in older pandas/numpy
             lg.warning(f"Vectorized pivot calculation with Decimal failed ({e}). Falling back to loop method. Performance may be impacted.")
             # Fallback Loop Implementation (from original code, ensure Decimal comparison)
             df_calc['ph_strat'] = pd.NA # Reset before loop
             df_calc['pl_strat'] = pd.NA
             for i in range(left_h, len(df_calc) - right_h):
                 pivot_val = high_col_dec.iloc[i]
                 if pd.isna(pivot_val): continue
                 # Check left: pivot_val > all left values (strict)
                 left_vals = high_col_dec.iloc[i-left_h : i]
                 if left_vals.isna().any() or not (pivot_val > left_vals).all(): continue
                 # Check right: pivot_val > all right values (strict)
                 right_vals = high_col_dec.iloc[i+1 : i+right_h+1]
                 if right_vals.isna().any() or not (pivot_val > right_vals).all(): continue
                 df_calc.loc[df_calc.index[i], 'ph_strat'] = pivot_val # Store Decimal

             for i in range(left_l, len(df_calc) - right_l):
                 pivot_val = low_col_dec.iloc[i]
                 if pd.isna(pivot_val): continue
                 # Check left: pivot_val < all left values (strict)
                 ...
                 if right_vals.isna().any() or not (pivot_val < right_vals).all(): continue
                 df_calc.loc[df_calc.index[i], 'pl_strat'] = pivot_val # Store Decimal
...
def manage_order_blocks(df: pd.DataFrame, config: Dict[str, Any], logger: logging.Logger) -> Tuple[pd.DataFrame, List[Dict], List[Dict]]:
    ...
    try:
        ...
        for i in range(len(df_calc)):
            ...
            current_close_dec = df_calc.at[current_idx, 'close_dec']
            # current_high_dec = df_calc.at[current_idx, 'high_dec'] # Not used in mitigation logic below
            # current_low_dec = df_calc.at[current_idx, 'low_dec'] # Not used in mitigation logic below
            ...
            if pd.notna(df_calc.at[current_idx, 'ph_strat']):
                ...
                if ob_candle_iloc >= 0:
                    ...
                    # Define Bearish OB range using Decimal prices
                    top_p_dec, bottom_p_dec = pd.NA, pd.NA
                    ob_open_dec = df_calc.at[ob_candle_idx, 'open_dec']
                    ob_close_dec = df_calc.at[ob_candle_idx, 'close_dec']
                    ob_high_dec = df_calc.at[ob_candle_idx, 'high_dec']
            ...
            if pd.notna(df_calc.at[current_idx, 'pl_strat']):
                ...
                if ob_candle_iloc >= 0:
                    ob_candle_idx = df_calc.index[ob_candle_iloc]
                    # Define Bullish OB range using Decimal prices
                    top_p_dec, bottom_p_dec = pd.NA, pd.NA
                    ob_open_dec = df_calc.at[ob_candle_idx, 'open_dec']
                    ob_close_dec = df_calc.at[ob_candle_idx, 'close_dec']
                    ob_low_dec = df_calc.at[ob_candle_idx, 'low_dec']
        ...
        # Clean up temporary Decimal columns
        for col in price_cols_to_convert:
             df_calc.drop(columns=[f'{col}_dec'], inplace=True, errors='ignore')
            ...
            for col in all_possible_cols:
                ...
                if col in latest.index and pd.notna(latest[col]):
                    ...
                    if col_name_in_state in ['active_bull_ob_strat', 'active_bear_ob_strat']:
                        ...
                        self.strategy_state[col_name_in_state] = bool(value) # Convert pd.BooleanNA to True/False
            ...
            if take_profit is not None and tp_multiple > 0:
                if signal == "BUY" and take_profit <= entry_price:
                    ...
                    take_profit = adjusted_tp
                     ...
                     # Adjust TP down by one tick if rounding put it at or above entry
                    adjusted_tp = take_profit - min_tick
...
def fetch_balance(exchange: ccxt.Exchange, currency: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    for acc_type in account_types_to_try:
        try:
            ...
            if "account type not support" in err_str or "invalid account type" in err_str or \
               "account type invalid" in err_str or "parameter error" in err_str: # Add common param errors
                lg.debug(f"Account type '{acc_type}' or params not supported/invalid for fetch_balance. Trying next.")
                continue
            else:
                        ...
                        if coin_data.get('coin') == currency:
                            ...
                            for key in keys_to_check:
                                ...
                                if free is not None:
                                    ...
                                    lg.debug(f"_parse_balance: Found {currency} balance candidate via {parse_source}: {available_balance_str}")
                                    # Check if the found value is non-zero
                                    temp_dec = safe_decimal(available_balance_str)
                                    if temp_dec is not None and temp_dec > 0:
                                         lg.debug("Candidate balance is positive.")
                                         break # Found usable positive balance
                                    elif temp_dec is not None and temp_dec == 0:
                                         lg.debug("Candidate balance is zero.")
                                         found_zero = True # Mark that we found an explicit zero
                                         # Continue checking other keys/accounts in case a non-zero balance exists elsewhere
                                    else:
                                         lg.debug("Candidate balance is invalid or negative.")
                                         available_balance_str = None # Reset if invalid
            ...
            if currency in balance_info and isinstance(balance_info.get(currency), dict) and 'free' in balance_info[currency]:
                ...
                if free_val is not None:
                    ...
                    if temp_dec is not None and temp_dec > 0:
                        ...
                        available_balance_str = temp_str # Store zero if found
                        parse_source = f"standard ['{currency}']['free']"
                        found_zero = True
    ...
    try:
        ...
        if has_open_position and isinstance(position_size, Decimal) and isinstance(position_entry_price, Decimal) and abs(position_size) > Decimal('1e-9'):
            ...
            # --- 7b. Risk Management (Only if NO exit signal) ---
                    ...
                    if not re.match(r"^\d+(\.\d+)?%?$", tsl_callback_rate_str) or safe_decimal(tsl_callback_rate_str.replace('%','')) <= 0:
                        ...
                        if activate_tsl:
                            ...
                            if new_sl_price is not None:
                                lg.warning(f"{COLOR_WARNING}Both Break-Even SL ({new_sl_price:{price_fmt}}) and TSL activation triggered. Sending combined update. Exchange behavior may vary (check Bybit docs on tpslMode='Partial').{RESET}")
                ...
                if needs_protection_update and trading_enabled:
                    ...
                    # Pass the analyzer instance
                    success = set_position_protection(
                        exchange, symbol, market_info, analyzer,
                        stop_loss_price=final_sl_to_set, # Can be Decimal or None
                        take_profit_price=final_tp_to_set, # Can be Decimal or None
                        trailing_stop_params=final_tsl_params_to_set, # Can be dict or None
                    ...
                    proposed_sl_str = f"{new_sl_price:{price_fmt}}" if new_sl_price else 'N/A'
                    lg.warning(f"{COLOR_WARNING}TRADING DISABLED: Would have updated protection. Proposed SL={proposed_sl_str}, Proposed TSL Params={new_tsl_params}{RESET}")
                else:
        ...
        # ==================================
            ...
            if signal in ["BUY", "SELL"]:
                ...
                # Allow entry even if balance is None (error fetching) or zero, position size calc will handle it
                if quote_balance is None:
                     lg.warning(f"{COLOR_WARNING}Could not fetch {QUOTE_CURRENCY} balance. Position sizing might fail or use zero balance.{RESET}")
                     quote_balance = Decimal('0') # Assume zero if fetch failed
                elif quote_balance <= 0:
                     lg.warning(f"{COLOR_WARNING}{QUOTE_CURRENCY} balance is {quote_balance}. Position sizing will likely fail.{RESET}")
                     # Proceed to sizing, which should return None
                ...
                # Pass the analyzer instance
                risk_per_trade_float = config.get("risk_per_trade", 0.01) # Config stores as float
                position_size = calculate_position_size(
                    balance=quote_balance,
                    risk_per_trade=risk_per_trade_float,
                ...
                if trading_enabled:
                    ...
                    if market_info.get('is_contract', False):
                        ...
                        try:
                            ...
                            # Prepare params, especially for Bybit V5
                            params={}
                            if 'bybit' in exchange.id.lower():
                                 params['category'] = market_info.get('contract_type', 'linear').lower() # linear/inverse/spot
                                 # Bybit V5 requires separate buy/sell leverage, set both
                                 params['buyLeverage'] = str(float(desired_leverage)) # API expects string float
                                 params['sellLeverage'] = str(float(desired_leverage))
                            # Use set_leverage method
                            # Note: ccxt might automatically convert leverage to float string if needed
                            exchange.set_leverage(desired_leverage, symbol, params=params)
                            lg.info(f"Leverage set/confirmed request sent for {desired_leverage}x for {symbol}.")
                        except (ccxt.ExchangeError, ccxt.NetworkError) as e:
                            err_str = str(e).lower()
                            # Non-fatal leverage errors (already set, positions exist, etc.)
                            if "leverage not modified" in err_str or "position open" in err_str or "same leverage" in err_str or "no need to modify" in err_str:
                                lg.warning(f"{COLOR_WARNING}Leverage not modified to {desired_leverage}x (Reason: {e}). Proceeding with existing leverage.{RESET}")
                    ...
                    # Pass analyzer instance for rounding
                    entry_order = place_market_order(
                        exchange, symbol, signal.lower(), # 'buy' or 'sell'
                        position_size, market_info, analyzer, lg
                    )
                    ...
                    if entry_order and entry_order.get('id') and entry_order.get('status', '').lower() in ['closed', 'filled']:
                        ...
                        if config.get("enable_trailing_stop", True):
                            ...
                            if re.match(r"^\d+(\.\d+)?%?$", tsl_callback_rate_str) and safe_decimal(tsl_callback_rate_str.replace('%','')) > 0:
                                ...
                                # Optional: Add activePrice based on activation_percentage config
                                tsl_activation_perc = safe_decimal(config.get("trailing_stop_activation_percentage", "0.003"), Decimal('0'))
                                if tsl_activation_perc > 0:
                                     # Estimate entry price (use order fill price if available, else current price)
                                     filled_price = safe_decimal(entry_order.get('average', entry_order.get('price', current_price))) or current_price
                                     act_price_raw = filled_price * (1 + tsl_activation_perc) if signal == "BUY" else filled_price * (1 - tsl_activation_perc)
                                     act_rounding = ROUND_UP if signal == "BUY" else ROUND_DOWN
                                     rounded_act_price = (act_price_raw / analyzer.get_min_tick_size()).quantize(Decimal('1'), rounding=act_rounding) * analyzer.get_min_tick_size()
                                     if rounded_act_price > 0:
                                         tsl_params_for_entry['activePrice'] = f"{rounded_act_price:{price_fmt}}"
                                         lg.info(f"Calculated initial TSL Activation Price: {tsl_params_for_entry['activePrice']}")

                                lg.info(f"Prepared initial TSL params for new position: {tsl_params_for_entry}")
                            else:
                                 lg.error(f"Invalid TSL callback rate format or value in config: '{tsl_callback_rate_str}'. Cannot set initial TSL.")


                        # Set initial SL, TP, and potentially TSL
                        lg.info(f"Attempting to set initial protection for new {signal} position...")
                        # Pass analyzer instance
                    ...
                    lg.warning(f"  Size = {position_size}, SL = {potential_sl:{price_fmt}}, TP = {potential_tp:{price_fmt} if potential_tp else 'None'}")
        ...
        raise # Re-raise critical auth errors to potentially stop the main loop
         ...
         # Log specific exchange errors that might indicate temporary issues vs critical ones
...
def main() -> None:
    ...
    """
    # Use a generic init logger for setup phase - level set globally before main()
    ...
    while True:
        current_list_str = f"Current list: {', '.join(symbols_to_trade)}" if symbols_to_trade else "Current list is empty"
        print(f"\n{current_list_str}")
        prompt = (
            f"Enter symbol (e.g., BTC/USDT), "
        ...
        if not symbol_input and symbols_to_trade:
            ...
            print(f"{COLOR_WARNING}List is empty. Please enter at least one symbol or 'all'.{RESET}")
            continue
        elif symbol_input.lower() == 'clear':
             symbols_to_trade = []
             print(f"{COLOR_YELLOW}Symbol list cleared.{RESET}")
             continue
            ...
            try:
                ...
                linear_swaps = [
                    ...
                    if mkt.get('active', False) # Explicitly check for active=True
                       and mkt.get('linear', False) # Check if linear contract
                       and mkt.get('swap', False) # Check if it's a perpetual swap
