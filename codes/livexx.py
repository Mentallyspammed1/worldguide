...
class TradingAnalyzer:
    ...
    def get_min_tick_size(self) -> Decimal:
        ...
        self.logger.debug(
            f"Using fallback tick size based on precision for {self.symbol}: {fallback_tick}"
        )
    ...
    try:
        ...
        if min_cost > 0 and current_cost < min_cost:
            ...
            if entry_price <= 0 or contract_size <= 0:  # Prevent division by zero
                lg.error(
                    "Cannot calculate size for min cost due to invalid entry price or contract size."
                )
                return None
            ...
            if entry_price <= 0 or contract_size <= 0:
                lg.error(
                    "Cannot calculate size for max cost due to invalid entry price or contract size."
                )
                return None
    ...
    try:
        ...
        if active_position:
            ...
            entry_price_str = active_position.get(
                "entryPrice", active_position.get("info", {}).get("avgPrice")
            )
            entry_price = (
                f"{Decimal(entry_price_str):.6f}"
                if entry_price_str is not None
                else "N/A"
            )
            contracts_str = active_position.get(
                "contracts", active_position.get("info", {}).get("size")
            )
            contracts = (
                f"{Decimal(contracts_str):.8f}" if contracts_str is not None else "N/A"
            )
            liq_price_str = active_position.get("liquidationPrice")
            liq_price = (
                f"{Decimal(liq_price_str):.6f}" if liq_price_str is not None else "N/A"
            )
            leverage_str = active_position.get(
                "leverage", active_position.get("info", {}).get("leverage")
            )
            leverage = (
            ...
            sl_str = active_position.get(
                "stopLossPrice", active_position.get("info", {}).get("stopLoss")
            )
            tp_str = active_position.get(
                "takeProfitPrice", active_position.get("info", {}).get("takeProfit")
            )
            sl_price = (
                f"{Decimal(sl_str):.6f}"
                if sl_str is not None and Decimal(sl_str) > 0
                else "N/A"
            )
            tp_price = (
                f"{Decimal(tp_str):.6f}"
                if tp_str is not None and Decimal(tp_str) > 0
                else "N/A"
            )
...
def _set_position_protection(
    ...
    params = {
        ...
        "positionIdx": position_info.get("info", {}).get(
            "positionIdx", 0
        ),  # Default to 0 (One-Way)
    }

    log_parts = [
        f"Attempting to set protection for {symbol} ({pos_side.upper()} position):"
    ]
    ...
    try:
        ...
        if has_tsl:
            ...
            if formatted_tsl_distance and formatted_activation_price:
                ...
                lg.error(
                    f"Failed to format TSL parameters for {symbol}. Cannot set TSL."
                )
...
def analyze_and_trade_symbol(
    ...
    # --- Scenario 2: Existing Open Position Found ---
        ...
        if (pos_side == "long" and signal == "SELL") or (
            ...
            lg.info(
                f"Signal is HOLD, maintaining existing {pos_side} position for {symbol}."
            )
            # Log current SL/TP from position info for reference
            sl_price = open_position.get(
                "stopLossPrice", open_position.get("info", {}).get("stopLoss", "N/A")
            )
            tp_price = open_position.get(
                "takeProfitPrice",
                open_position.get("info", {}).get("takeProfit", "N/A"),
            )
            tsl_active_dist = open_position.get("info", {}).get(
                "trailingStop", "0"
            )  # TSL distance if active
            ...
            lg.info(
                f"Signal ({signal}) matches existing {pos_side} position direction for {symbol}. Holding position."
            )
            # --- Advanced Logic Placeholder ---
...
def main() -> None:
    ...
    init_logger.info(
        f"--- Starting LiveXX Trading Bot ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---"
    )
    ...
    while True:
        try:
            ...
            if market_info:
                ...
                variations = []
                if (
                    ":" not in symbol_input and "/" in symbol_input
                ):  # e.g., BTC/USDT -> BTC/USDT:USDT
                    variations.append(
                        f"{symbol_input}:{market_info.get('quote', QUOTE_CURRENCY)}"
                    )
                    ...
                    variations.append(
                        f"{base}/{QUOTE_CURRENCY}:{QUOTE_CURRENCY}"
                    )  # For Bybit linear
