...
try:
    ...
    try:
        ...
        print(
            "Error: Missing timezone library. Please install 'pytz' (Python < 3.9) or ensure Python 3.9+ for 'zoneinfo'."
        )
        sys.exit(1)
...
try:
    ...
    print(
        "Error: Missing required library 'ccxt'. Please install it (`pip install ccxt`)."
    )
    sys.exit(1)
try:
    import numpy as np
    ...
    print(
        "Error: Missing required library 'numpy'. Please install it (`pip install numpy`)."
    )
    sys.exit(1)
try:
    import pandas as pd
    ...
    print(
        "Error: Missing required library 'pandas'. Please install it (`pip install pandas`)."
    )
    sys.exit(1)
try:
    ...
    print(
        "Error: Missing required library 'pandas_ta'. Please install it (`pip install pandas_ta`)."
    )
    sys.exit(1)
...
try:
    ...
    def load_dotenv(*args: Any, **kwargs: Any) -> bool:
        return False  # Return False to indicate not loaded
...
if load_dotenv():
    ...
    print(
        "Note: .env file not found or python-dotenv not installed. Relying on system environment variables."
    )
...
if not API_KEY or not API_SECRET:
    print(
        f"{NEON_RED}CRITICAL ERROR: BYBIT_API_KEY or BYBIT_API_SECRET environment variables not set.{RESET}"
    )
...
try:
    ...
    print(
        f"{NEON_RED}CRITICAL ERROR: Could not create log directory '{LOG_DIRECTORY}': {e}{RESET}"
    )
    sys.exit(1)
...
try:
    ...
    print(
        f"Warning: Could not load timezone '{TZ_NAME}' ({tz_err}). Defaulting to {_FALLBACK_TIMEZONE}."
    )
    ...
    try:
        ...
        print(
            f"{NEON_RED}CRITICAL ERROR: Could not load fallback timezone {_FALLBACK_TIMEZONE}: {fallback_tz_err}{RESET}"
        )
        sys.exit(1)
...
MIN_POSITION_SIZE_DIFF_PCT = Decimal(
    "0.05"
)  # Max allowed difference (5%) between ordered and confirmed size
...
class LocalTimeFormatter(SensitiveFormatter):
    ...
    converter = (
        lambda _, timestamp: datetime.fromtimestamp(timestamp, tz=TIMEZONE).timetuple()
        if TIMEZONE
        else time.localtime(timestamp)
    )
...
def setup_logger(name: str, is_symbol_logger: bool = False) -> logging.Logger:
    ...
    try:
        ...
        print(
            f"{NEON_RED}Error setting up file logger for '{logger_instance_name}': {e}{RESET}"
        )
    ...
    try:
        ...
        print(
            f"{NEON_RED}Error setting up stream logger for '{logger_instance_name}': {e}{RESET}"
        )
...
def _ensure_config_keys(
    ...
    for key, default_value in default_config.items():
        ...
        if key not in updated_config:
            ...
            is_str_num_candidate = isinstance(
                default_value, (int, float)
            ) and isinstance(loaded_value, str)

            if (
                not is_promoting_num
                and not is_str_bool_candidate
                and not is_str_num_candidate
            ):
...
def _validate_config_values(config: Dict[str, Any], logger: logging.Logger) -> bool:
    ...
    if interval is None or str(interval) not in CCXT_INTERVAL_MAP:
        lg.error(
            f"Config Error: Invalid or missing 'interval' value '{interval}'. Must be one of {VALID_INTERVALS}"
        )
        is_valid = False
    else:
    ...
    numeric_params: Dict[
        str, Tuple[type, Union[int, float], Union[int, float], bool]
    ] = {
    ...
    if (
        ...
        weight_sets.get(active_set_name), dict
    ):  # Use .get() for safety
        lg.error(
...
def load_config(filepath: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    default_config: Dict[str, Any] = {
        ...
        "trailing_stop_callback_rate": 0.005,  # TSL distance as fraction of price (float, e.g., 0.005 = 0.5%) - NOTE: Bybit V5 uses price distance, not rate for TSL! This needs adjustment.
        "trailing_stop_activation_percentage": 0.003,  # Profit % required to activate TSL (float, e.g., 0.003 = 0.3%) - NOTE: Bybit V5 uses activation price, not percentage! This needs adjustment.
    ...
    # Bybit V5 uses price distance for TSL, not callback rate.
    # Bybit V5 uses activation price, not activation percentage.
    # We need to remove the old config keys and potentially add new ones,
    # or adjust the bot logic to calculate the required values from these rates/percentages.
    # For now, let's remove the incompatible keys from the default config and validation.
    # The logic using these will need to be updated later if TSL is enabled.
    if "trailing_stop_callback_rate" in default_config:
        del default_config["trailing_stop_callback_rate"]
        lg.warning(
            "Removed 'trailing_stop_callback_rate' from default config. Bybit V5 uses price distance for TSL."
        )
    if "trailing_stop_activation_percentage" in default_config:
        del default_config["trailing_stop_activation_percentage"]
        lg.warning(
            "Removed 'trailing_stop_activation_percentage' from default config. Bybit V5 uses activation price for TSL."
        )
    # Remove them from validation as well
    if "trailing_stop_callback_rate" in numeric_params:
        del numeric_params["trailing_stop_callback_rate"]
    if "trailing_stop_activation_percentage" in numeric_params:
        del numeric_params["trailing_stop_activation_percentage"]
    ...
    if not os.path.exists(filepath):
        ...
        try:
            ...
            original_loaded_config = (
                default_config.copy()
            )  # Treat as if starting from defaults
    ...
    try:
        ...
        initial_state_to_compare = (
            original_loaded_config  # Use the state before ensure/validate
        )
        # Use json dumps for structural comparison, ignoring minor float differences by not using it
        # Direct comparison is safer if types were converted in place
        if loaded_config != initial_state_to_compare:
            # More robust check: compare JSON strings
            if json.dumps(loaded_config, sort_keys=True) != json.dumps(
                initial_state_to_compare, sort_keys=True
            ):
                config_changed_needs_save = True
                lg.info(
                    "Configuration values modified during validation/conversion (e.g., type changes)."
                )
                    ...
                    if isinstance(symbol_data, dict):
                        for key in expected_symbol_keys:
                            if key not in symbol_data:
                                lg.warning(
                                    f"State for symbol '{symbol}' missing key '{key}'. Initializing with default."
                                )
                                # Initialize with appropriate default (e.g., False for bool, None for price)
                                if key == "break_even_triggered":
                                    symbol_data[key] = False
                                else:
                                    symbol_data[key] = None
                    elif (
                        symbol != "global"
                    ):  # Allow non-dict global state entries if needed
                        lg.warning(
                            f"Invalid state data found for key '{symbol}' (expected dict). Resetting state for this symbol."
                        )
                        state[symbol] = {
    ...
    try:
        ...
        def json_serializer(obj: Any) -> Optional[str]:
            if isinstance(obj, Decimal):
                ...
                return (
                    str(obj) if obj.is_finite() else None
                )  # Avoid saving NaN/Infinity
            ...
            try:
                ...
                lg.warning(
                    f"Object of type {obj.__class__.__name__} is not JSON serializable. Saving as None."
                )
...
def _check_account_type_and_balance(
    ...
    try:
        ...
        is_account_type_error = (
            ...
            or (
                "10001" in error_str
                and ("accounttype" in error_str or "coin does not exist" in error_str)
            )
            or (
                "10005" in error_str
        ...
        if is_account_type_error:
            lg.debug(
                f"Fetching with UNIFIED failed with account type/coin mismatch error ({e}). Assuming Non-UTA."
            )
        ...
        try:
                ...
                try:
                    ...
                    if json_part_match:
                        ...
                        try:
                            ...
                            if code_match:
                                bybit_code = int(code_match.group(1))
                            if msg_match:
                                ret_msg = msg_match.group(1)
                except (
                    IndexError,
                    TypeError,
                    ValueError,
                ):  # Catch potential errors during regex/parsing
                    lg.debug(
            ...
            if bybit_code is not None and bybit_code in non_retryable_codes:
                ...
                if attempt < max_retries:
                    ...
                    lg.warning(
                        f"{NEON_YELLOW}Retryable/Unknown Exchange Error calling {method_name}: {ret_msg} (Code: {bybit_code}). Retrying in {wait_time:.1f}s... (Attempt {attempt + 1}/{max_retries}){RESET}"
                    )
                    time.sleep(wait_time)
...
def quantize_price_util(
    price: Union[Decimal, float, str],
    min_tick_size: Optional[Decimal],
    logger: logging.Logger,
    symbol: str = "N/A",
    ...
    if (
        min_tick_size is None
        or not isinstance(min_tick_size, Decimal)
        or not min_tick_size.is_finite()
        or min_tick_size <= 0
    ):
        logger.warning(
            f"Cannot quantize price for {symbol}: Invalid or missing minimum tick size ({min_tick_size})."
        )
        return None
    try:
        ...
        logger.error(
            f"Error quantizing price '{price}' with tick '{min_tick_size}' for {symbol}: {e}"
        )
        return None


def quantize_amount_util(
    amount: Union[Decimal, float, str],
    amount_step_size: Optional[Decimal],
    amount_precision_digits: Optional[int],
    logger: logging.Logger,
    symbol: str = "N/A",
    rounding: str = ROUND_DOWN,
) -> Optional[Decimal]:
    ...
    if (
        step_size is None
        or not isinstance(step_size, Decimal)
        or not step_size.is_finite()
        or step_size <= 0
    ):
        if (
            amount_precision_digits is not None
            and isinstance(amount_precision_digits, int)
            and amount_precision_digits >= 0
        ):
            step_size = Decimal("1") / (Decimal("10") ** amount_precision_digits)
            logger.debug(
                f"Amount step size missing/invalid for {symbol}, using precision digits ({amount_precision_digits}) -> derived step={step_size}"
            )
        else:
            logger.error(
                f"Cannot quantize amount for {symbol}: Amount step size and precision digits are missing or invalid."
            )
            return None

    if step_size <= 0:  # Double check after potential calculation from digits
        logger.error(
            f"Cannot quantize amount for {symbol}: Invalid step size ({step_size})."
        )
        return None
    ...
    try:
        ...
        final_quantized = quantized_amount.normalize().quantize(
            step_size.normalize(), rounding=rounding
        )  # Use same rounding as initial step calc
        return final_quantized
    except (InvalidOperation, ValueError, TypeError, DivisionByZeroError) as e:
        logger.error(
            f"Error quantizing amount '{amount}' with step '{step_size}' for {symbol}: {e}"
        )
        return None
    ...
    try:
        ...
        if not market:
            ...
            if ":" in symbol:
                ...
                lg.debug(
                    f"Symbol '{symbol}' not found directly, trying base symbol '{base_symbol_tried}'..."
                )
                market = exchange.market(base_symbol_tried)

            if not market:  # Still not found
                not_found_msg = f"Symbol '{symbol}'"
                if base_symbol_tried:
                    not_found_msg += f" (or base '{base_symbol_tried}')"
...
def fetch_current_price_ccxt(
    ...
    try:
        ...
        lg.error(
            f"Error fetching current price for {symbol}: {e}", exc_info=False
        )  # Reduce noise for common fetch errors
        lg.debug("Traceback for price fetch error:", exc_info=True)
        return None
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        lg.error(
            f"Error fetching/processing klines for {symbol}: {e}", exc_info=False
        )  # Reduce noise
        lg.debug("Traceback for kline fetch error:", exc_info=True)
...
def fetch_orderbook_ccxt(
    ...
    if limit not in valid_limits:
        ...
        if limit < 50:
            ...
            adjusted_limit = 200
        lg.warning(
            f"Invalid orderbook limit {limit} requested for {symbol}. Adjusted to nearest valid Bybit V5 limit: {adjusted_limit}"
        )
    ...
    try:
        ...
        if (
            ...
            for level in orderbook["bids"] + orderbook["asks"]:
                if not (
                    ...
                    isinstance(level[0], (str, int, float))
                    and isinstance(level[1], (str, int, float))
                ):
                    lg.warning(
                        f"Order book level for {symbol} has invalid format: {level}"
                    )
                    is_valid = False
                    break
            if not is_valid:
                lg.warning(
                    f"Order book for {symbol} contains invalid level format. Response snippet: bids={orderbook.get('bids', [])[:2]}, asks={orderbook.get('asks', [])[:2]}"
                )
                return None
        ...
        lg.debug("Traceback for orderbook fetch error:", exc_info=True)
        return None
        ...
        if (
            ...
            if len(balance_list) > 0 and isinstance(balance_list[0], dict):
                ...
                # Alternative structure: Sometimes the list contains coin details directly? Less common.
                    ...
                    lg.warning(
                        "Unexpected V5 balance structure: List contains coin data directly? Trying to parse..."
                    )
                    account_data_found = {
                        "coin": balance_list
                    }  # Wrap it to resemble expected structure
...
class TradingAnalyzer:
    ...
    def get_amount_step_size(self) -> Optional[Decimal]:
        ...
        if (
            ...
            self.logger.debug(
                f"Invalid or missing amount_step_size ({step}) for {self.symbol}. Amount quantization will use digits fallback."
            )
            return None
        return step
            ...
            if diff > 0:
                for level_pct_float in FIB_LEVELS:
                    ...
                    if level_price_quantized is not None:
                        ...
                        self.logger.warning(
                            f"Failed to quantize Fibonacci level {level_pct * 100:.1f}% (Raw: {level_price_raw}) for {self.symbol}. Skipping level."
                        )
    ...
    def generate_trading_signal(
        self, current_price_dec: Decimal, orderbook_data: Optional[Dict]
    ) -> str:
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            if signal == "BUY":
                ...
                stop_loss = quantize_price_util(
                    sl_raw, min_tick, self.logger, self.symbol, rounding=ROUND_DOWN
                )
                take_profit = quantize_price_util(
                    tp_raw, min_tick, self.logger, self.symbol, rounding=ROUND_UP
                )
            else:  # SELL
                sl_raw = quantized_entry + sl_offset
                tp_raw = quantized_entry - tp_offset
                ...
                stop_loss = quantize_price_util(
                    sl_raw, min_tick, self.logger, self.symbol, rounding=ROUND_UP
                )
                take_profit = quantize_price_util(
                    tp_raw, min_tick, self.logger, self.symbol, rounding=ROUND_DOWN
                )
    ...
    try:
        ...
        if is_contract:
            if is_inverse:
                ...
                if (
                    not risk_per_contract_base.is_finite()
                    or risk_per_contract_base <= 0
                ):
                    lg.error(
                        f"Size Calc Error ({symbol}): Invalid calculated risk per contract in BASE ({risk_per_contract_base})"
                    )
                    return None
                ...
                )  # Approx quote risk per contract
        ...
        lg.debug(
            f"Quantized Size ({symbol}): {quantized_size} {size_unit_name} (Step: {amount_step_size or 'Derived'})"
        )
        ...
        if required_margin_with_buffer > balance:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Required margin ~{margin_required_quote:.4f} {quote_currency} (Buffered: {required_margin_with_buffer:.4f}) > Available balance {balance:.4f}. Cannot place order.{RESET}"
            )
            ...
            # MaxValue = MaxMargin * Leverage; MaxSize = MaxValue / (ContractSize * EntryPrice) etc. (Linear)
            # MaxMarginBase = MaxMarginQuote / EntryPrice; MaxValueBase = MaxMarginBase * Lev; MaxSize = MaxValueBase / ContractSize (Inverse)
            # For now, just reject the trade if margin is insufficient.
            return None

        # --- Success ---
        lg.info(
            f"Calculated position size for {symbol}: {quantized_size} {size_unit_name} (Value: ~{cost_check_value:.2f} {quote_currency}, Margin: ~{margin_required_quote:.2f} {quote_currency})"
        )
    ...
    try:
        ...
        for pos_raw in all_positions_raw:
            ...
            try:
                ...
                def safe_decimal(
                    value_str: Optional[Union[str, float, int]],
                ) -> Optional[Decimal]:
        ...
        lg.debug("Traceback for position fetch error:", exc_info=True)
        return None
...
def set_protection_ccxt(
    ...
    try:
        ...
        # Use safe_ccxt_call to execute the method
        # Note: safe_ccxt_call expects method name as string, then args/kwargs.
        # Here, the parameters are passed in the 'params' keyword argument for the underlying CCXT call.
        ...
        if result and isinstance(result, dict):
            ...
            ret_code = result.get(
                "retCode"
            )  # V5 response often has retCode at top level
            ret_msg = result.get("retMsg", "Unknown Status")

            if ret_code == 0:
                lg.info(
                    f"{NEON_GREEN}Successfully set protection for {symbol} (Idx: {position_idx}).{RESET}"
                )
                lg.debug(f"Protection Result: {result}")
                return True
            else:
                lg.error(
                    f"{NEON_RED}Setting protection failed for {symbol} (Idx: {position_idx}). Code={ret_code}, Msg='{ret_msg}'.{RESET}"
                )
                lg.debug(f"Failed Protection Result: {result}")
                # Provide hints for common rejection codes
                if ret_code == 110041 or ret_code == 170140:
                    lg.warning(
                        f"Hint: Protection failed because no position exists (Code {ret_code})."
                    )
                elif ret_code == 110042 or ret_code == 170131 or ret_code == 170132:
                    lg.warning(
                        f"Hint: Protection failed due to invalid TP/SL price (Code {ret_code})."
                    )
                elif ret_code == 110025:
                    lg.warning(
                        f"Hint: Protection failed due to positionIdx mismatch (Code {ret_code})."
                    )
                return False
        else:
            # safe_ccxt_call failed or returned unexpected format
            lg.error(
                f"Setting protection API call failed or returned unexpected result ({symbol}). Result: {result}"
            )
            return False
        ...
        # Catch any unexpected error during the process
        lg.error(
            f"{NEON_RED}Unexpected error setting protection ({symbol}): {e}{RESET}",
            exc_info=True,
        )
        return False
...
# The main execution loop would go here, utilizing the functions and classes defined above.
# Example structure:
# if __name__ == "__main__":
#     # 1. Parse arguments (debug level, config file)
#     # 2. Setup main logger
#     # 3. Load configuration
#     # 4. Initialize exchange
#     # 5. Load state
#     # 6. Main loop:
#     #    - Check market reload timer
...
#     #        - Fetch balance
#     #        - Implement entry/exit logic based on signal, position, config flags
#     #            - Calculate position size
#     #            - Set leverage (if needed)
#     #            - Create entry/exit orders
#     #            - Set TP/SL/TSL protection
#     #        - Update symbol state (break-even, last entry price etc.)
#     #    - Save state periodically
#     #    - Handle loop delay
#     #    - Graceful shutdown (save state, cancel orders?)
#     pass # Replace with actual main loop implementation
