...
MIN_POSITION_SIZE_DIFF_PCT = Decimal("0.05")  # Max allowed difference (5%) between ordered and confirmed size
...
def load_config(filepath: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    if "trailing_stop_callback_rate" in default_config:
        ...
        lg.warning("Removed 'trailing_stop_callback_rate' from default config. Bybit V5 uses price distance for TSL.")
    ...
    if not os.path.exists(filepath):
        ...
        try:
            ...
            )  # Shallow copy is enough if ensure_keys doesn't modify nested dicts directly
...
def _check_account_type_and_balance(
    ...
    try:
        ...
        is_account_type_error = (
            ...
            or ("10001" in error_str and ("accounttype" in error_str or "coin does not exist" in error_str))
...
def _parse_balance_response(
    balance_info: Optional[Dict], currency: str, account_type_checked: str, logger: logging.Logger
) -> Optional[Decimal]:
        ...
        if (
            ...
            if len(balance_list) > 0 and isinstance(balance_list[0], dict):
                ...
                # Alternative structure: Sometimes the list contains coin details directly? Less common.
                    ...
                    lg.warning("Unexpected V5 balance structure: List contains coin data directly? Trying to parse...")
...
class TradingAnalyzer:
    ...
    def calculate_entry_tp_sl(
        ...
        quantized_entry = quantize_price_util(
            entry_price_signal, min_tick, self.logger, self.symbol, rounding=entry_rounding
        )
            ...
            if stop_loss is not None:
                ...
                if needs_sl_adjustment:
                    ...
                    adjusted_sl = quantize_price_util(
                        sl_boundary, min_tick, self.logger, self.symbol, rounding=rounding
                    )
            ...
            if take_profit is not None:
                ...
                if needs_tp_adjustment:
                    ...
                    adjusted_tp = quantize_price_util(
                        tp_boundary, min_tick, self.logger, self.symbol, rounding=rounding
                    )
                    # If quantizing the boundary didn't move it far enough, move one more tick
...
def set_protection_ccxt(
    ...
    try:
        ...
        if result and isinstance(result, dict):
            # Check Bybit's V5 response code within the 'info' field for confirmation
            ret_code = result.get("retCode")  # V5 response often has retCode at top level
            ...
            if ret_code == 0:
                ...
                if ret_code == 110041 or ret_code == 170140:
                    lg.warning(f"Hint: Protection failed because no position exists (Code {ret_code}).")
                elif ret_code == 110042 or ret_code == 170131 or ret_code == 170132:
                    lg.warning(f"Hint: Protection failed due to invalid TP/SL price (Code {ret_code}).")
                elif ret_code == 110025:
                    lg.warning(f"Hint: Protection failed due to positionIdx mismatch (Code {ret_code}).")
                return False
        else:
            # safe_ccxt_call failed or returned unexpected format
            lg.error(f"Setting protection API call failed or returned unexpected result ({symbol}). Result: {result}")
            return False
