...
def load_config(filepath: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    if "trailing_stop_activation_percentage" in default_config:
        ...
        lg.warning("Removed 'trailing_stop_activation_percentage' from default config. Bybit V5 uses activation price for TSL.")
    ...
    try:
        ...
        if result and isinstance(result, dict):
            ...
            if ret_code == 0:
                 ...
                 lg.error(f"{NEON_RED}Setting protection failed for {symbol} (Idx: {position_idx}). Code={ret_code}, Msg='{ret_msg}'.{RESET}")
                 lg.debug(f"Failed Protection Result: {result}")
                 # Provide hints for common rejection codes
                 if ret_code == 110041 or ret_code == 170140: lg.warning(f"Hint: Protection failed because no position exists (Code {ret_code}).")
                 elif ret_code == 110042 or ret_code == 170131 or ret_code == 170132: lg.warning(f"Hint: Protection failed due to invalid TP/SL price (Code {ret_code}).")
                 elif ret_code == 110025: lg.warning(f"Hint: Protection failed due to positionIdx mismatch (Code {ret_code}).")
                 return False
        else:
