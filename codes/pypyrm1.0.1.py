...
The issue is that the code is trying to catch an exception that is not defined in the code. The line:
...
is not valid Python syntax. The `except` block should be followed by a colon, and the code block should be indented. The way it is written currently, it is trying to execute the code after the `except` as if it were part of the `try` block.

To fix this, you can restructure the code to properly handle the `ImportError` exception:
...
try: # --- Pybit ---
    ...
    print("Error: 'pybit' not found. pip install pybit")
    sys.exit(1)
...
try: 
    ...
    print("Warning: 'colorama' not found. pip install colorama")
    class D:__getattr__=lambda s,n:"";Fore,Style=D(),D();init=lambda *a,**k:None
...
1. Moved the `try`/`except` block for the `colorama` import to its own block, with a proper `except` block.
2. Moved the `init(autoreset=True)` call for `colorama` inside the `try` block, so it's only executed if the import is successful.
3. Added an `except` block to handle the `ImportError` for the `pybit` import, and exit the script with a non-zero status code if the import fails.

This should resolve the syntax error and allow the script to run without crashing on the `colorama` import.
File "/data/data/com.termux/files/home/worldguide/codes/pypyrm1.0.1.py", line 54
    except ImportError: print("Warning: 'colorama' not found. pip install colorama"); class D:__getattr__=lambda s,n:"";Fore,Style=D(),D();init=lambda *a,**k:None
                                                                                      ^^^^^
SyntaxError: invalid syntax# -*- coding: utf-8 -*-
"""
Enhanced Multi-Symbol Trading Bot for Bybit (V5 API) - Pybit Version v1.0.1
...
def save_state(filepath: str, state: Dict[str, Any], logger: logging.Logger):
    """Saves bot state to a JSON file using an atomic write approach."""
    # Implementation uses write-to-temp then rename for atomicity.
    ...
    try:
        ...
        # Atomic rename (os.replace is atomic on most modern OS including POSIX and Windows)
        os.replace(temp_filepath, filepath)
        logger.debug(f"Saved current bot state to {filepath}")
    except Exception as e:
        logger.error(f"Error saving state to {filepath}: {e}", exc_info=True)
        # Clean up temp file if it exists after an error during write or replace
        if os.path.exists(temp_filepath):
            try: os.remove(temp_filepath)
            except OSError as rm_err: logger.error(f"Error removing temporary state file {temp_filepath}: {rm_err}")
        ...
        try:
            ...
            if response and response.get('retCode') == 0 and 'result' in response and 'list' in response['result']:
                ...
                for instrument in instruments:
                    ...
                    # ========================================================================
                    # === CRITICAL WARNING: Contract Size Assumption                       ===
                    # === The bot assumes contract_size = 1 base unit (like BTCUSDT).      ===
                    # === If trading INVERSE contracts (e.g., BTCUSD), contract size is    ===
                    # === often 1 USD. Verify this in Bybit's contract specifications.     ===
                    # === Failure to set correct contract_size will lead to INCORRECT      ===
                    # === position sizing and risk calculation.                            ===
                    # ========================================================================
                    contract_size = Decimal('1.0') # DEFAULT ASSUMPTION: 1 Base Unit (like linear perps)
                    if category == 'inverse':
                        # !!! USER VERIFICATION REQUIRED FOR INVERSE CONTRACTS !!!
                        # Bybit often uses 1 USD as contract size for inverse.
                        contract_size = Decimal('1.0') # <--- VERIFY AND CHANGE IF NEEDED FOR YOUR SYMBOL
                        lg.warning(f"{NEON_YELLOW}CRITICAL: Assuming contract size {contract_size} for INVERSE symbol {symbol}. VERIFY THIS in Bybit docs! Incorrect size leads to wrong risk.{RESET}")
                         ...
                         contract_size = None # Not applicable for spot
                    ...
                    try:
                        ...
                        parsed_info = {
                            ...
                            'contract_size': contract_size, # Store the determined/assumed contract size (Decimal or None) - CRITICAL!
                        ...
                        # Only add to cache if essential precision info is valid and status is TRADING
...
def get_market_info_pybit(client: UnifiedHTTP, symbol: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    if 'min_tick_size' not in market: # Check if enhancement is needed
        ...
        if enhanced_market['is_contract'] and enhanced_market['contract_size'] is None:
             lg.error(f"{NEON_RED}CONTRACT_SIZE IS NONE for {symbol} ({enhanced_market['category']}) - calculation error likely.{RESET}")
        elif enhanced_market['is_contract'] and enhanced_market['contract_size'] != Decimal('1.0'):
             lg.warning(f"{NEON_YELLOW}Using non-standard contract_size={enhanced_market['contract_size']} for {symbol} ({enhanced_market['category']}). Ensure risk/size calculations are correct!{RESET}")
        elif enhanced_market['is_contract']:
             lg.debug(f"Using contract_size={enhanced_market['contract_size']} for {symbol} ({enhanced_market['category']}).")
...
def fetch_positions_pybit(client: UnifiedHTTP, symbol: str, logger: logging.Logger, market_info: Dict) -> Optional[Dict]:
    ...
    NOTE: In Hedge Mode, this currently returns the *first* non-zero position found (either long or short).
    Enhancement needed for full Hedge Mode support to potentially return both.
    """
    lg = logger; category = market_info.get('category')
...
def create_order_pybit(
    ...
    if current_est_price and current_est_price > 0:
         ...
         if market_info.get('is_inverse') and contract_sz: # Value = Qty / Price * ContractSize (where Qty is in contracts)
             # !!! Check this formula carefully for inverse contracts !!!
             # If Qty is in contracts (USD value), and contract_size is USD value per contract (e.g., 1 USD), then value in Quote = Qty * ContractSize.
             estimated_cost = quantized_amount * contract_sz # Value in Quote (e.g., USD, assumed ~= USDT)
             lg.debug(f"Inverse est cost calc ({symbol}): Qty(contracts)={quantized_amount} * ContractSize={contract_sz} = CostQuote={estimated_cost}")
         elif market_info.get('is_linear') and contract_sz: # Value = Qty * Price * ContractSize (where Qty is in base asset, ContractSize=1 base)
             estimated_cost = quantized_amount * current_est_price * contract_sz
             lg.debug(f"Linear est cost calc ({symbol}): Qty(base)={quantized_amount} * Price={current_est_price} * ContractSize={contract_sz} = CostQuote={estimated_cost}")
         elif market_info.get('is_spot'): # Value = Qty * Price (where Qty is in base asset)
              estimated_cost = quantized_amount * current_est_price
              lg.debug(f"Spot est cost calc ({symbol}): Qty(base)={quantized_amount} * Price={current_est_price} = CostQuote={estimated_cost}")
         else: # Option or unknown
              estimated_cost = quantized_amount * current_est_price # Approx
              lg.debug(f"Unknown Category est cost calc ({symbol}): Qty={quantized_amount} * Price={current_est_price} = CostQuote={estimated_cost}")
...
def set_protection_pybit(
    ...
    params: Dict[str, Any] = {
        ...
        "symbol": symbol,
        # === Hedge Mode Note ===
        # positionIdx MUST be provided correctly for Hedge Mode.
        # 0 = One-Way Mode position
        # 1 = Hedge Mode BUY side position
        # 2 = Hedge Mode SELL side position
    ...
    # Only include tpslMode if TP or SL is being set (or modified from non-zero)
    ...
    # Default trigger prices (can be overridden if needed, e.g., LastPrice)
    ...
    if tsl_dist_str and tsl_dist_str != "0":
        ...
        # Pass active price only if it's validly formatted. If None/invalid, don't pass the key for immediate activation.
        if tsl_act_str:
            params['activePrice'] = tsl_act_str
            lg.debug(f"Setting TSL for {symbol} with distance {tsl_dist_str}, activation price {tsl_act_str}.")
        else:
             lg.debug(f"Setting TSL for {symbol} with distance {tsl_dist_str}, immediate activation (no activePrice passed).")
    ...
    if tsl_dist_str and tsl_dist_str != "0":
         ...
         if 'activePrice' in params: tsl_log += f", ActPrice={params['activePrice']}"
    ...
    if not log_parts:
        ...
        # Remove activePrice if cancelling TSL
        if params['trailingStop'] == "0" and 'activePrice' in params: del params['activePrice']
        # Ensure tpslMode is included if cancelling an existing TP/SL
        if params['stopLoss'] == "0" or params['takeProfit'] == "0": params['tpslMode'] = "Full"
        ...
        if params['stopLoss'] == "0" and params['takeProfit'] == "0" and params['trailingStop'] == "0":
             ...
             if params['trailingStop'] != "0":
                 act_log = ", Act=Immediate"
                 if 'activePrice' in params: act_log = f", Act={params['activePrice']}"
                 log_parts.append(f"TSL_Dist={params['trailingStop']}{act_log}")
    ...
    try:
        ...
        if response and response.get('retCode') == 0:
            ...
            if response.get('retCode') == 30067: lg.error("Reason: Position Idx mismatch (check Hedge Mode settings and provided positionIdx).")
            return False
        else: # safe_pybit_call returned None
...
def calculate_position_size(
    ...
    # =====================================================================
    # === CRITICAL: Contract Size is crucial for correct calculations! ===
    # =====================================================================
    ...
    if is_contract and (contract_size is None or contract_size <= 0):
        lg.error(f"{NEON_RED}Cannot calculate size for contract {symbol}: Invalid contract size ({contract_size}). Check market info fetch and contract spec!{RESET}")
        return None
    ...
    try:
        ...
        # Formula: Size = RiskAmount / (RiskPerUnit)
        # RiskPerUnit needs to account for contract size and type (linear/inverse)
        ...
        if market_info.get('is_linear'):
            ...
            # Risk per unit of Base Asset (e.g., per 1 BTC) = PriceDiff (USDT) * ContractSize (Base=1)
            risk_per_base_unit_quote = price_diff * contract_size # contract_size should be 1 for standard linear
            if risk_per_base_unit_quote <= 0: lg.error(f"Linear risk per base unit is zero/negative."); return None
            position_size_base_or_contracts = risk_amount_quote / risk_per_base_unit_quote
            lg.debug(f"Linear Size Calc ({symbol}): RiskQuote={risk_amount_quote:.4f}, PriceDiff={price_diff}, ContractSize={contract_size}, RiskPerBase={risk_per_base_unit_quote} -> SizeBase={position_size_base_or_contracts:.{amount_digits}f}")
            if contract_size != Decimal('1.0'): lg.warning(f"{NEON_YELLOW}Linear contract {symbol} has unusual contract_size={contract_size}. Verify size calculation.{RESET}")

        elif market_info.get('is_inverse'):
            # Inverse (e.g., BTCUSD): Qty is in Contracts (USD). Contract size = 1 Quote (USD) usually. Value = Qty(Contracts) * ContractSize(Quote). Risk is specified in Quote (USDT).
            # Risk per Contract (in Quote) = abs(Entry - SL) * ContractSize (Quote) / (Entry * SL)  -- More precise formula for inverse risk per contract
            # Simplified approach assumes risk_per_contract_quote = abs(1/SL - 1/Entry) * ContractSize(Quote) is close enough for non-huge price moves. Let's use the more precise one.
            risk_per_contract_quote = price_diff * contract_size / (entry_price * stop_loss_price)
            if risk_per_contract_quote <= 0: lg.error(f"Inverse risk per contract is zero/negative ({risk_per_contract_quote}). Check prices."); return None
            position_size_base_or_contracts = risk_amount_quote / risk_per_contract_quote
            lg.debug(f"Inverse Size Calc ({symbol}): RiskQuote={risk_amount_quote:.4f}, PriceDiff={price_diff}, ContractSize={contract_size}, Entry={entry_price}, SL={stop_loss_price}, RiskPerContractQuote={risk_per_contract_quote:.8f} -> SizeContracts={position_size_base_or_contracts:.{amount_digits}f}")
            if contract_size != Decimal('1.0'): lg.warning(f"{NEON_YELLOW}Inverse contract {symbol} has unusual contract_size={contract_size}. Verify size calculation and contract spec!{RESET}")
            ...
            risk_per_base_unit_quote = price_diff
            if risk_per_base_unit_quote <= 0: lg.error(f"Spot risk per base unit is zero/negative."); return None
            position_size_base_or_contracts = risk_amount_quote / risk_per_base_unit_quote
            lg.debug(f"Spot Size Calc ({symbol}): RiskQuote={risk_amount_quote:.4f}, PriceDiff={price_diff} -> SizeBase={position_size_base_or_contracts:.{amount_digits}f}")
        ...
        # Max position value (in Quote currency) = Balance * Leverage
        max_position_value_quote = balance * Decimal(leverage)
        calculated_position_value_quote = Decimal('NaN')

        if market_info.get('is_linear'):
            # Value = Size(Base) * EntryPrice * ContractSize(Base=1)
            calculated_position_value_quote = position_size_base_or_contracts * entry_price * contract_size
        elif market_info.get('is_inverse'):
             # Value = Size(Contracts) * ContractSize(Quote)
             calculated_position_value_quote = position_size_base_or_contracts * contract_size
        ...
        if is_contract and calculated_position_value_quote.is_finite() and calculated_position_value_quote > max_position_value_quote:
            ...
            if market_info.get('is_linear'):
                 if entry_price * contract_size > 0:
                      position_size_base_or_contracts = max_position_value_quote / (entry_price * contract_size)
                 else: lg.error("Cannot cap linear size by leverage: entry price or contract size is zero."); return None
            elif market_info.get('is_inverse'):
                 if contract_size > 0:
                      position_size_base_or_contracts = max_position_value_quote / contract_size # Max value / value per contract
                 else: lg.error("Cannot cap inverse size by leverage: contract size is zero."); return None
...
class TradingAnalyzer:
    ...
    def _calculate_all_indicators(self):
        ...
        if hasattr(self.ta_strategy, 'talib') or hasattr(self.ta_strategy, 'ta'): # Check if strategy has indicators
             # Simple heuristic: find the max length parameter used (can be improved)
             max_len_param = 0
             if hasattr(self.ta_strategy, 'ta') and isinstance(self.ta_strategy.ta, list):
                 for indi in self.ta_strategy.ta:
                     if isinstance(indi, dict):
                         ...
                         if isinstance(length_param, (int, float)) and length_param > max_len_param:
                             max_len_param = int(length_param)
    ...
    def _check_stoch_rsi(self) -> Optional[float]:
        ...
        elif d < oversold and k >= oversold: score = 0.5 # K crossing up out of oversold
        # Exiting overbought (bearish)
        elif d > overbought and k <= overbought: score = -0.5 # K crossing down out of overbought
        # General trend indication by K/D position outside extreme zones
    ...
    def _check_rsi(self) -> Optional[float]:
        ...
        if rsi <= 20: score = 1.0 # Strong Oversold -> Strong Buy Signal
             ...
             # Simple scale: Map 30 to 0.7, 70 to -0.7
            ...
            if stop_loss is not None:
                 ...
                 if signal == "BUY":
                      ...
                      if stop_loss >= required_sl:
                           ...
                           self.logger.info(f"Adjusting BUY Stop Loss {stop_loss} to {adjusted_sl} to ensure minimum distance ({MIN_TICKS_AWAY_FOR_SLTP} ticks) from entry {quantized_entry}.")
                      ...
                      if stop_loss <= required_sl:
                           ...
                           self.logger.info(f"Adjusting SELL Stop Loss {stop_loss} to {adjusted_sl} to ensure minimum distance ({MIN_TICKS_AWAY_FOR_SLTP} ticks) from entry {quantized_entry}.")
                           stop_loss = adjusted_sl
            ...
            if take_profit is not None:
                 ...
                 if signal == "BUY":
                      if take_profit <= quantized_entry: # TP must be > entry
                          ...
                          self.logger.info(f"Adjusting BUY Take Profit upwards to {take_profit} as it was <= entry.")
                 else: # SELL
                      if take_profit >= quantized_entry: # TP must be < entry
                           take_profit = self.quantize_price(quantized_entry - min_required_tp_diff, ROUND_DOWN)
                           self.logger.info(f"Adjusting SELL Take Profit downwards to {take_profit} as it was >= entry.")
    ...
    while True:
        ...
        for symbol in symbols_to_trade:
            ...
            if market_info.get('is_contract'):
                try:
                    # === Hedge Mode Note ===
                    # fetch_positions_pybit currently returns only the FIRST non-zero position found.
                    # For Hedge Mode, this means it might find the long OR the short, but not both if both exist.
            ...
            try:
                ...
                periods = [int(config.get(p, d)) for p, d in [
                    ...
                    ("volume_ma_period", DEFAULT_VOLUME_MA_PERIOD), ("fibonacci_window", DEFAULT_FIB_WINDOW)]
                     if config.get("indicators", {}).get(p.split('_')[0].replace('rsi_rsi','stoch_rsi'), False) or p in ["atr_period", "fibonacci_window"] # Include ATR always, handle stoch_rsi key diff
                ]
...
def manage_existing_position_pybit(
    ...
    # --- Validate Inputs ---
    if not all([symbol, side, entry_price_str, size_str is not None, market_info, state is not None]):
    ...
    if config.get("enable_ma_cross_exit", False):
        ...
        if ema_short is not None and ema_long is not None:
            ...
            tolerance = Decimal(str(ema_long)) * Decimal("0.0001") # 0.01% tolerance, adjust as needed

            if side == 'long' and Decimal(str(ema_short)) < (Decimal(str(ema_long)) - tolerance):
                crossed_against = True
                cross_details = f"Short EMA ({ema_short:.5f}) crossed BELOW Long EMA ({ema_long:.5f})"
            elif side == 'short' and Decimal(str(ema_short)) > (Decimal(str(ema_long)) + tolerance):
                crossed_against = True
...
def attempt_new_entry_pybit(
    ...
    if config.get("position_mode") == "Hedge":
        ...
        logger.info(f"{NEON_YELLOW}Hedge Mode Active: Using positionIdx={position_idx} for {signal} order.{RESET}")
        # REMINDER: fetch_positions_pybit needs enhancement for full hedge mode support.
    ...
    # Wait for order fill and position update. Consider using polling instead of fixed sleep for more robustness.
    ...
    for i in range(retries):
        try:
             ...
             if confirmed_position and confirmed_position.get('contracts', Decimal('0')) > 0:
                  # Ensure we are looking at the correct position index if in Hedge Mode
                  pos_idx_confirm = confirmed_position.get('positionIdx')
                  if config.get("position_mode") == "Hedge" and pos_idx_confirm != position_idx:
                       logger.warning(f"Confirmed position has Idx {pos_idx_confirm}, but expected {position_idx} for {signal}. Position data might be for the other side. Retrying...")
                       # This indicates fetch_positions needs improvement for Hedge mode. Continue retry loop.
                  elif config.get("position_mode") == "One-Way" and pos_idx_confirm != 0:
                       logger.warning(f"Confirmed position has Idx {pos_idx_confirm}, but expected 0 for One-Way mode. Retrying...")
                       # Continue retry loop.
                  else: # Correct position index found (or One-Way mode)
                       ...
                       if entry_str and size_str is not None:
                           ...
                           logger.warning(f"Position found for {symbol} (Idx: {pos_idx_confirm}), but entry price or size missing/invalid in confirmation data. Retrying...")
             else:
    ...
    if actual_entry_price is None or final_position_size is None or final_position_size <= 0:
        logger.error(f"{NEON_RED}Failed to confirm position entry for {symbol} after {retries} retries. Cannot set SL/TP. MANUAL INTERVENTION MAY BE REQUIRED if order filled!{RESET}")
        ...
        try:
            ...
            if callback_rate > 0 and tick_size:
                ...
                if activation_perc > 0:
                    ...
                    tsl_activation_price = None # Immediate activation (by not sending activePrice)
...
if __name__ == "__main__":
    ...
    if config.get("enable_trading"):
        ...
        if config.get("use_sandbox"):
            ...
            print(f"{NEON_RED}Starting live trading in 5 seconds... Press Ctrl+C to cancel.{RESET}")
            try: time.sleep(5)
