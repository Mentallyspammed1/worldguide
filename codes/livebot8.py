            ...
            if "bybit" in exchange.id.lower():
                try:
                    market = get_market_info(
                        exchange, symbol, lg
                    )  # Use helper to ensure markets loaded
                    if market:
                        ...
                        lg.warning(
                            f"Could not get market info for {symbol} within get_open_position. Params may be incomplete."
                        )
                except Exception as e:
                    lg.warning(
                        f"Error fetching market info for {symbol} within get_open_position: {e}. Assuming 'linear' for category."
                    )
                    params["category"] = (
                        "linear"  # Default assumption if market fetch fails
                    )
        ...
        if active_position:
            ...
            if side not in ["long", "short"]:
                ...
                # If still not determined, use size sign
                    ...
                    if (
                        "contracts" in active_position
                        and active_position["contracts"] is not None
                    ):
                        try:
                            active_position["contracts"] = abs(
                                Decimal(str(active_position["contracts"]))
            ...
            try:
                ...
                tsl_dist_val = (
                    ...
                    if tsl_dist_str and str(tsl_dist_str).strip()
                    else Decimal("0")
                )
            ...
            try:
                ...
                tsl_act_val = (
                    Decimal(tsl_act_str)
                    if tsl_act_str and str(tsl_act_str).strip()
                    else Decimal("0")
                )
            ...
            if market:
                with contextlib.suppress(Exception):
                    log_precision = market.get("precision", {}).get("price", 6)
                try:
...
def _set_position_protection(
    ...
    )  # Activation can be None/0 if distance is set
                                ...
                                if market_info:
                                    try:
                                        ...
                                        if min_amt is not None:
                                            ...
                                            min_size_threshold = max(
                                                min_size_threshold,
                                                Decimal(str(min_amt)) * Decimal("0.1"),
                                            )
                                    except Exception:
                                        pass
