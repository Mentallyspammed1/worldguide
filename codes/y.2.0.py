...
def _ensure_config_keys(config: dict[str, Any], default_config: dict[str, Any], parent_key: str = "") -> tuple[dict[str, Any], bool]:
    ...
    for key, default_value in default_config.items():
        ...
        if key not in updated_config:
            ...
            # Log addition of missing key (at DEBUG level)
            ...
            if not is_promoting_num and not is_str_bool_candidate:
                ...
                # get_logger("main").warning(f"Config Warning: Type mismatch for '{full_key_path}'. Expected {type(default_value).__name__}, found {type(loaded_value).__name__}. Using loaded value '{loaded_value}'.")
...
def save_state(filepath: str, state: dict[str, Any], logger: logging.Logger) -> None:
    ...
    try:
        ...
        def json_serializer(obj: Any) -> str | None:
...
def _get_precision_digits(precision_val: float | int | str | None, default_digits: int = 8) -> int:
...
def quantize_price_util(price: Decimal | float | str, min_tick_size: Decimal | None, logger: logging.Logger, symbol: str = "N/A", rounding: str = ROUND_DOWN) -> Decimal | None:
...
def quantize_amount_util(amount: Decimal | float | str, amount_step_size: Decimal | None, amount_precision_digits: int | None, logger: logging.Logger, symbol: str = "N/A", rounding: str = ROUND_DOWN) -> Decimal | None:
    ...
    try:
        ...
        final_quantized = quantized_amount.quantize(step_size, rounding=rounding) # Use same rounding as initial step calc
    ...
    try:
        ...
        if is_contract:
             if is_inverse:
                 ...
                 order_value_base = quantized_size * contract_size  # Value in Base
                 # Approximate Quote value for cost check:
                 order_value_quote = order_value_base * entry_price  # Rough quote value
                 margin_required = order_value_base / Decimal(leverage) # Margin in Base currency
                 margin_required_quote = margin_required * entry_price # Approx margin in quote terms
             else:  # Linear Contract
                 ...
                 margin_required_quote = margin_required # Margin is already in quote
        else:  # Spot
