...
async def place_limit_order_interactive_async(exchange_pro: ccxtpro.Exchange, symbol: str, side: str, ask_map: dict[int, decimal.Decimal], bid_map: dict[int, decimal.Decimal], market_info: dict, config: dict) -> None:
    ...
    if not target_map: print_color("OB side empty.", Fore.YELLOW); return
...
async def main_async() -> None:
    ...
    print_color("*" * 85, Fore.RED, Style.BRIGHT); print_color("   🔥 Pyrmethus Market Analyzer v3.1 ASYNC Stable 🔥", Fore.RED, Style.BRIGHT); print_color("   WebSockets Activated! Use responsibly.", Fore.YELLOW); print_color("*" * 85, Fore.RED, Style.BRIGHT)
    ...
    try:
        ...
        print_color("\n--- Critical Loop Error ---", Fore.RED, Style.BRIGHT); traceback.print_exc(); print_color(f"Error: {e}", Fore.RED); print_color("--- End Critical Error ---", Fore.RED, Style.BRIGHT); termux_toast("Pyrmethus CRITICAL Error", "long")
    finally:
