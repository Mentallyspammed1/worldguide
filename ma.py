...
CONFIG = {
    ...
    "REFRESH_INTERVAL": get_config_value("REFRESH_INTERVAL", 5, int),  # Display refresh rate
    ...
    "OHLCV_FETCH_INTERVAL": get_config_value("OHLCV_FETCH_INTERVAL", 300, int),
}
    ...
    while True:
        try:
            ...
            if market_info:
                ...
                await asyncio.sleep(0.5)  # Brief pause if market_info not ready
        except (TimeoutError, ccxt.NetworkError, ccxt.RequestTimeout) as e:
        ...
        try:
            ...
            if fetch_ohlcv:
                ...
                if isinstance(piv_res, list) and len(piv_res) > 0:
                    ...
                    latest_data["pivots"] = None  # Clear if fetch returned empty/None
...
async def calculate_and_store_indicators() -> None:
    ...
    try:
        ...
        indicators["sma1"]["value"] = calculate_sma(
            close_prices, CONFIG["SMA_PERIOD"]
        )  # ... (similar value assignments) ...
    ...
    if pivot_ohlcv and len(pivot_ohlcv) > 0:
        ...
        if isinstance(prev_candle, list) and len(prev_candle) >= 5:
            ...
            if calculated_pivots:
                ...
                latest_data["pivots"] = None
                verbose_print("Pivot calculation failed.")
        else:
            latest_data["pivots"] = None
            verbose_print("Invalid prev candle for pivots.")
    else:
        latest_data["pivots"] = None
        verbose_print("No pivot OHLCV data.")
...
def display_orderbook(
    ...
    ask_lines, bid_lines = [], []
    display_asks = list(reversed(analyzed_ob["asks"]))
    ...
    print_color(f"{'Asks'.center(col_w)}{'Bids'.center(col_w)}", Fore.LIGHTBLACK_EX)
    print_color(f"{'-' * col_w} {'-' * col_w}", Fore.LIGHTBLACK_EX)
    max_r = max(len(ask_lines), len(bid_lines))
    for _i in range(max_r):
        pass
    best_a, best_b = (
...
def display_volume_analysis(analyzed_ob: dict | None, market_info: dict, config: dict) -> None:
    ...
    if imb.is_infinite():
        imb_c, imb_s = Fore.LIGHTGREEN_EX, "Inf"
    elif imb.is_finite():
...
async def place_market_order_async(
    ...
    if confirm == "y" or confirm == "yes":
        ...
        try:
            ...
            termux_toast(f"{symbol} Market {side.upper()} Placed: {oid}")
        except ccxt.InsufficientFunds as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Funds Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Mkt Fail: Funds", "long")
        except ccxt.InvalidOrder as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Invalid Order: {e}", Fore.RED)
            termux_toast(f"{symbol} Mkt Fail: Invalid", "long")
        except ccxt.ExchangeError as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Exch Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Mkt Fail: ExchErr", "long")
        except Exception as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Place Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Mkt Fail: Error", "long")
    else:
        print_color("Market order cancelled.", Fore.YELLOW)
...
async def place_limit_order_async(
    ...
    try:
        ...
        min_amount, amount_prec, price_prec = (
            market_info["min_amount"],
            market_info["amount_precision"],
            market_info["price_precision"],
        )
        amount_step, price_tick = (
            market_info.get("amount_step", decimal.Decimal("0")),
            market_info.get("price_tick_size", decimal.Decimal("0")),
        )
        ...
        if price_tick > 0 and (price % price_tick) != 0:
            ...
            print_color(f"Price rounded: {original_price} -> {price}", Fore.YELLOW)
            if price <= 0:
    ...
    if confirm == "y" or confirm == "yes":
        ...
        try:
            ...
            if details:
                msg += f" ({', '.join(details)})"
            print_color(f"\n{msg}", Fore.GREEN, Style.BRIGHT)
            termux_toast(f"{symbol} Limit {side.upper()} Placed: {oid}")
        except ccxt.InsufficientFunds as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Funds Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Lim Fail: Funds", "long")
        except ccxt.InvalidOrder as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Invalid Ord: {e}", Fore.RED)
            termux_toast(f"{symbol} Lim Fail: Invalid", "long")
        except ccxt.ExchangeError as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Exch Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Lim Fail: ExchErr", "long")
        except Exception as e:
            sys.stdout.write("\033[K")
            print_color(f"\n❌ Async Place Err: {e}", Fore.RED)
            termux_toast(f"{symbol} Lim Fail: Error", "long")
    else:
        print_color("Limit order cancelled.", Fore.YELLOW)
    ...
    while selected_price is None:
        try:
            ...
            if selected_price is None:
                print_color(f"Index {index_str} not found.", Fore.YELLOW)
    ...
    while True:
        try:
            ...
            if qty_str.lower() == "c":
                print_color("Cancelled.", Fore.YELLOW)
                return
            if decimal.Decimal(qty_str) <= 0:
                print_color("Qty must be positive.", Fore.YELLOW)
                continue
            break
        except (decimal.InvalidOperation, ValueError):
            print_color("Invalid quantity.", Fore.YELLOW)
        except (EOFError, KeyboardInterrupt):
            print_color("\nCancelled.", Fore.YELLOW)
            return
...
async def fetch_and_recalculate(exchange_pro: ccxtpro.Exchange, symbol: str, config: dict) -> None:
    ...
    try:
        ...
        if isinstance(ind_ohlcv_res, list) and len(ind_ohlcv_res) >= min_ohlcv:
            ...
            print_color(f"# Recalc Ind OHLCV failed: {ind_ohlcv_res}", Fore.YELLOW)
        else:
            print_color(
                f"# Recalc Ind OHLCV insufficient ({len(ind_ohlcv_res if ind_ohlcv_res else [])}<{min_ohlcv})",
                Fore.YELLOW,
            )
            latest_data["indicator_ohlcv"] = None
        ...
        if isinstance(piv_ohlcv_res, list) and len(piv_ohlcv_res) > 0:
            ...
            print_color(f"# Recalc Piv OHLCV failed: {piv_ohlcv_res}", Fore.YELLOW)
            latest_data["pivots"] = None
        else:
            latest_data["pivots"] = None  # Clear if empty/None
    except Exception as e:
...
async def initial_calculations(exchange_pro: ccxtpro.Exchange, symbol: str, config: dict) -> None:
    ...
    try:
        ...
        print_color(f"# Error during initial bal/pos fetch: {e}", Fore.RED)
...
async def main_async() -> None:
    ...
    try:
        ...
        sys.stdout.write("\033[K")
        print_color("Async connection ok.", Fore.GREEN)
    except Exception as e:
        sys.stdout.write("\033[K")
        print_color(f"Connection failed: {e}", Fore.RED)
    ...
    while not market_info:
        ...
        try:
            ...
            print_color(f"Symbol input error: {e}", Fore.RED)
            await asyncio.sleep(1)

    latest_data["market_info"] = market_info  # Store globally
    print_color(
        f"Focus: {Fore.MAGENTA}{symbol}{Fore.CYAN} | P:{market_info['price_precision']} A:{market_info['amount_precision']}",
        Fore.CYAN,
    )
    print_color(
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if side in ["buy", "sell"]:
                    ...
                    verbose_print("Manual refresh (display updates continuously)")
                elif side == "exit":
                    print_color("Dispelling...", Fore.YELLOW)
                    break
                elif side != "unknown":
                    print_color("Unknown command.", Fore.YELLOW)

                # Reset error streak if user interacts successfully
                data_error_streak = 0

            except TimeoutError:  # No input, just continue loop for refresh
                pass
            except (EOFError, KeyboardInterrupt):
        ...
        print_color("\n--- Critical Loop Error ---", Fore.RED, Style.BRIGHT)
        traceback.print_exc()
        print_color(f"Error: {e}", Fore.RED)
        print_color("--- End Critical Error ---", Fore.RED, Style.BRIGHT)
        termux_toast("Pyrmethus CRITICAL Error", "long")
    finally:
        ...
        if exchange and hasattr(exchange, "close"):
            try:
                await exchange.close()
                print_color("Exchange connection closed.", Fore.YELLOW)
            except Exception as close_e:
                print_color(f"Error closing exchange: {close_e}", Fore.RED)
