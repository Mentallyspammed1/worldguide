        ...
        for key, user_value in user.items():
            if key in merged:
                ...
                # Allow merging if types match, or if user provides a number for a default float/int
                    ...
                    print(
                        Color.format(
                            f"Warning: Config type mismatch for key '{key}'. Expected {type(default_value)}, got {type(user_value)}. Using default value.",
                            Color.YELLOW,
                        )
                    )
            ...
            if name in data_dict:
                ...
                if hasattr(type_hint, "__dataclass_fields__") and isinstance(value, dict):
                    ...
                    try:
                        ...
                        print(
                            Color.format(
                                f"Warning: Could not convert config value '{value}' for key '{name}' to type {type_hint}. Using default: '{default_val}'. Error: {e}",
                                Color.YELLOW,
                            )
                        )
...
class BybitCCXTClient:
    ...
    def _cache_market_categories(self) -> None:
        ...
        for symbol, details in self.markets.items():
            ...
            if details is None:
                self.logger.warning(
                    f"Market details for symbol '{symbol}' are None. Skipping category caching for this symbol."
                )
                skipped_none += 1
                continue
            ...
            if market_type == "spot":
                ...
                # Fallback to contractType if settle is ambiguous or missing
                    ...
                    if quote == "USDT":
                        ...
                        category = "spot"  # Less likely for derivatives, but a fallback
                    self.logger.debug(
                        f"Guessed category '{category}' for derivative {symbol} based on quote '{quote}'. Market type: '{market_type}', Contract type: '{contract_type}', Settle: '{settle_currency}'."
                    )
                ...
                if quote == "USDT":
                    ...
                    category = "spot"
                self.logger.warning(
                    f"Unknown market type '{market_type}' for {symbol}. Guessed category '{category}' based on quote '{quote}'."
                )
    ...
    async def fetch_ticker(self, symbol: str) -> Optional[dict]:
        ...
        if tickers and isinstance(tickers, dict) and symbol in tickers:
            ...
            if isinstance(ticker_data, dict):
                ...
                self.logger.error(
                    f"Unexpected ticker data format for {symbol} in fetch_tickers response: {ticker_data}"
                )
                return None
        else:
    ...
    async def fetch_klines(self, symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
        ...
        klines = await self.fetch_with_retry(
            "fetch_ohlcv", symbol, timeframe=ccxt_timeframe, limit=limit, params=params
        )
        ...
        try:
            ...
            if dropped_ohlcv_rows > 0:
                self.logger.warning(
                    f"Dropped {dropped_ohlcv_rows} rows with missing/invalid essential OHLCV data from klines for {symbol}."
                )
    ...
    async def fetch_orderbook(self, symbol: str, limit: int) -> Optional[dict]:
        ...
        if category == "spot" and limit > 50:
            ...
            self.logger.warning(
                f"Requested orderbook limit {limit} exceeds Bybit V5 linear/inverse limit (200). Adjusting to 200."
            )
            bybit_limit = 200
        elif limit <= 0:
        ...
        if orderbook and isinstance(orderbook, dict):
            ...
            if isinstance(bids, list) and isinstance(asks, list):
                ...
                self.logger.warning(
                    f"Fetched order book data for {symbol} is missing bids/asks or has an unexpected format: {orderbook}"
                )
                return None
        else:
...
class TradingAnalyzer:
    ...
    def _generate_column_names(self) -> None:
        ...
        if is_.volume_ma_period > 0:
            self.col_names["vol_ma"] = f"VOL_MA_{is_.volume_ma_period}"  # Custom name
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            self.logger.error(
                f"Cannot calculate indicators: Input DataFrame missing required columns ({required_cols}). Found: {list(df.columns)}"
            )
            return df
        ...
        if dropped_rows > 0:
            self.logger.warning(
                f"Dropped {dropped_rows} rows due to non-numeric OHLCV data during indicator calculation prep."
            )
        ...
        strategy_ta = [
            ...
            {
                "kind": "stochrsi",
                "length": is_.stoch_rsi_period,
                "rsi_length": is_.rsi_period,
        ...
        try:
            ...
            def to_decimal_df(data: List[List[Union[str, float, int]]], columns: List[str]) -> pd.DataFrame:
                ...
                try:
                    ...
                    if "price" in columns:
                        df = df[df["price"] > DECIMAL_COMPARISON_THRESHOLD]
                    if "size" in columns:
    ...
    def _format_signal(self, label: str, value: Any, signal: SignalState, precision: int = 2, details: str = "") -> str:
        ...
        color_map = {
            SignalState.BULLISH: Color.GREEN,
            SignalState.STRONG_BULLISH: Color.GREEN,
            SignalState.OVERSOLD: Color.GREEN,
            SignalState.INCREASING: Color.GREEN,
            SignalState.ACCUMULATION: Color.GREEN,
            SignalState.FLIP_BULLISH: Color.GREEN,
            SignalState.ABOVE_SIGNAL: Color.GREEN,
            SignalState.BEARISH: Color.RED,
            SignalState.STRONG_BEARISH: Color.RED,
            SignalState.OVERBOUGHT: Color.RED,
            SignalState.DECREASING: Color.RED,
            SignalState.DISTRIBUTION: Color.RED,
            SignalState.FLIP_BEARISH: Color.RED,
            SignalState.BELOW_SIGNAL: Color.RED,
            SignalState.NEUTRAL: Color.YELLOW,
            SignalState.RANGING: Color.YELLOW,
            SignalState.WITHIN_BANDS: Color.YELLOW,
            SignalState.AVERAGE_VOLUME: Color.YELLOW,
            SignalState.FLAT: Color.YELLOW,
            SignalState.NONE: Color.YELLOW,
            ...
            SignalState.HIGH_VOLUME: Color.PURPLE,
    ...
    def _interpret_oscillators(
        ...
        if flags.rsi_threshold:
            ...
            if np.isfinite(rsi):
                signal = SignalState.NEUTRAL
                if rsi >= thresh.rsi_overbought:
                    signal = SignalState.OVERBOUGHT
                elif rsi <= thresh.rsi_oversold:
                    signal = SignalState.OVERSOLD
                signals[signal_key] = signal
        ...
        if flags.mfi_threshold:
            ...
            if np.isfinite(mfi):
                signal = SignalState.NEUTRAL
                if mfi >= thresh.mfi_overbought:
                    signal = SignalState.OVERBOUGHT
                elif mfi <= thresh.mfi_oversold:
                    signal = SignalState.OVERSOLD
                signals[signal_key] = signal
        ...
        if flags.cci_threshold:
            ...
            if np.isfinite(cci):
                signal = SignalState.NEUTRAL
                if cci >= thresh.cci_overbought:
                    signal = SignalState.OVERBOUGHT
                elif cci <= thresh.cci_oversold:
                    signal = SignalState.OVERSOLD
                signals[signal_key] = signal
        ...
        if flags.williams_r_threshold:
            ...
            if np.isfinite(wr):
                ...
                if wr >= thresh.williams_r_overbought:
                    signal = SignalState.OVERBOUGHT
                # Oversold is when value is *below* the OS threshold (closer to -100)
                elif wr <= thresh.williams_r_oversold:
                    signal = SignalState.OVERSOLD
                signals[signal_key] = signal
    ...
    def _interpret_macd(self, last_row: pd.Series, prev_row: pd.Series) -> Tuple[List[str], Dict[str, SignalState]]:
        ...
        if flags.macd_cross:
            ...
            summary_lines.append(
                self._format_signal(label, value, signal, precision=4, details=details)
            )  # Use higher precision for MACD
        ...
        try:
            ...
            for key, signal_enum in all_signals_enums.items():
                if key in interpretation["signals"]:  # Ensure we only update expected keys
                    ...
                    self.logger.warning(
                        f"Generated signal key '{key}' not in expected list {all_signal_keys}. Ignoring."
                    )
        ...
        try:
            ...
            if len(df_klines_decimal) >= 2 and "timestamp" in df_klines_decimal.columns:
                ...
                if not mode_diff.empty:
                    ...
                    if minutes in [1, 3, 5, 15, 30]:
                        interval_key = str(int(minutes))
                    elif hours in [1, 2, 4, 6, 12]:
                        interval_key = str(int(hours * 60))
                    elif days == 1:
                        interval_key = "D"
                    elif weeks == 1:
                        interval_key = "W"
            ...
            if not df_with_indicators.empty:
                ...
                analysis_result["raw_indicators"] = {
                    k: (
                        format_decimal(v, 4)
                        if isinstance(v, (float, np.floating, int, Decimal))
                        # Format numerics
                        else (
                            v.isoformat()
            ...
            if orderbook and isinstance(current_price, Decimal) and current_price.is_finite():
                ...
                self.logger.debug(
                    "Skipping orderbook analysis due to missing orderbook data or invalid/missing current price."
                )
    ...
    while True:
                ...
                if logger_instance.isEnabledFor(logging.DEBUG):
                    try:
                        ...
                        logger_instance.error(
                            f"Unexpected error preparing analysis result for JSON debug logging: {log_json_err}"
                        )
            else:
        ...
        # --- Specific Exception Handling for the Loop ---
            ...
            logger_instance.critical(
                Color.format(
                    f"Authentication Error during analysis cycle: {e}. Check API Key/Secret. Stopping analysis for {symbol}.",
                    Color.RED,
                )
            )
            ...
            logger_instance.critical(
                Color.format(
                    f"Invalid Nonce Error: {e}. Check system time sync with server. Stopping analysis for {symbol}.",
                    Color.RED,
                )
            )
        ...
        logger_instance.debug(
            f"Analysis cycle for {symbol} took {elapsed_time:.2f}s. Sleeping for {sleep_duration:.2f}s (+ jitter)."
        )
...
async def main() -> None:
    ...
    while True:
        try:
            ...
            print(
                Color.format(
                    f"Invalid interval '{interval_input}'. Please choose from the list or use standard CCXT format (e.g., 1h, 4h, 1d).",
                    Color.RED,
                )
            )
