...
class Config:
    ...
    def __init__(self) -> None:
        ...
        if self.strategy_name not in self.valid_strategies:
            raise ValueError(f"Invalid STRATEGY_NAME '{self.strategy_name}'. Valid options: {self.valid_strategies}")
        ...
        # --- CCXT / API Parameters ---
        self.default_recv_window: int = 10000  # milliseconds (adjust if needed for API timing issues)
        self.order_book_fetch_limit: int = max(25, self.order_book_depth)  # Min limit often 25 for L2, ensure we fetch enough
        ...
        self.position_qty_epsilon: Decimal = Decimal("1e-9")  # Small value to check against zero qty, avoid floating point issues
    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, color: str = Fore.WHITE) -> Any:
        ...
        if value is None:
            if required:
                logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: Required environment variable '{key}' not set.{Style.RESET_ALL}")
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"{Fore.RED}Default value '{default}' for {key} is also invalid for Decimal.{Style.RESET_ALL}")
                    value = None

                if required and value is None:
                    raise ValueError(f"Required Decimal env var '{key}' had invalid value and no valid default.")
        ...
        if value is None and required:
             logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: Required environment variable '{key}' has no value or valid default.{Style.RESET_ALL}")
             raise ValueError(f"Required environment variable '{key}' has no value or valid default.")
...
def format_amount(exchange: ccxt.Exchange, symbol: str, amount: float | Decimal | str) -> str:
    ...
    try:
        ...
        if precision_amount is not None:
            ...
            decimal_places = Decimal(str(precision_amount)).normalize().as_tuple().exponent * -1
            quantizer = Decimal('1e-' + str(decimal_places))
            rounded_amount = amount_decimal.quantize(quantizer, rounding=ROUND_DOWN)
            return f"{rounded_amount:.{decimal_places}f}" # Format to fixed decimal places
        else:
            # Fallback if precision info is missing (less ideal)
            logger.warning(f"Amount precision not found for {symbol}, using default formatting.")
    ...
    except (ValueError, TypeError, ccxt.ExchangeError, InvalidOperation, Exception) as e:
...
def send_sms_alert(message: str) -> bool:
    ...
    safe_message = message.replace('"', "'").replace('`', "'") # Basic quote replacement
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error(f"{Fore.RED}SMS command failed. RC: {result.returncode}, Stderr: {stderr_output}{Style.RESET_ALL}")
            return False
    except FileNotFoundError:
        logger.error(f"{Fore.RED}SMS failed: 'termux-sms-send' command not found. Ensure Termux:API package and app are installed and configured.{Style.RESET_ALL}")
        return False
...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    if not CONFIG.api_key or not CONFIG.api_secret:
        logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: API keys (BYBIT_API_KEY, BYBIT_API_SECRET) missing in environment.{Style.RESET_ALL}")
    ...
    try:
        exchange = ccxt.bybit({
            ...
            "options": {
                ...
                'verbose': LOGGING_LEVEL == logging.DEBUG, # Enable verbose CCXT logging if bot is in debug mode
        ...
        logger.success(f"{Fore.GREEN}{Style.BRIGHT}CCXT Bybit Session Initialized (Targeting V5 API - LIVE SCALPING MODE - EXTREME CAUTION!).{Style.RESET_ALL}")
        ...
        logger.critical(f"{Back.RED}{Fore.WHITE}Network error during initialization: {e}. Check connection and Bybit status.{Style.RESET_ALL}")
        send_sms_alert(f"[Pyrmethus] CRITICAL: Network Error on Init: {e}. Bot stopped.")
    except ccxt.ExchangeError as e:
        logger.critical(f"{Back.RED}{Fore.WHITE}Exchange error during initialization: {e}. Check Bybit status and API documentation.{Style.RESET_ALL}")
        send_sms_alert(f"[Pyrmethus] CRITICAL: Exchange Error on Init: {e}. Bot stopped.")
    except Exception as e:
        logger.critical(f"{Back.RED}{Fore.WHITE}Unexpected error during exchange initialization: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
...
def calculate_supertrend(df: pd.DataFrame, length: int, multiplier: Decimal, prefix: str = "") -> pd.DataFrame:
    ...
    long_flip_col = f"{col_prefix}st_long"  # Boolean: True if trend flipped to Long on this candle
    ...
    pt_long_col = f"SUPERTl_{length}_{float(multiplier)}" # Often represents the lower band value
    pt_short_col = f"SUPERTs_{length}_{float(multiplier)}" # Often represents the upper band value
    ...
    if df is None or df.empty or not all(c in df.columns for c in required_input_cols) or len(df) < min_len:
        logger.warning(f"{Fore.YELLOW}Indicator Calc ({col_prefix}ST): Insufficient data (Len: {len(df) if df is not None else 0}, Need: {min_len}). Setting cols to NA.{Style.RESET_ALL}")
        return df
    ...
    try:
        ...
        if not all(c in df.columns for c in [pt_st_col, pt_trend_col]):
            ...
            raise KeyError(f"pandas_ta failed to create expected raw columns: {missing}")
        ...
        df[trend_col] = df[pt_trend_col].apply(lambda x: True if pd.notna(x) and x == 1 else (False if pd.notna(x) and x == -1 else pd.NA))
        ...
        if pd.notna(last_st_val):
            ...
            last_trend_str = 'Up' if last_trend_val is True else ('Down' if last_trend_val is False else 'NA')
            signal = 'LONG_FLIP' if df[long_flip_col].iloc[-1] else ('SHORT_FLIP' if df[short_flip_col].iloc[-1] else 'No Flip')
            logger.debug(f"Indicator Calc ({col_prefix}ST({length},{multiplier})): Trend={last_trend_str}, Val={last_st_val:.4f}, Signal={signal}")
        else:
...
def analyze_volume_atr(df: pd.DataFrame, atr_len: int, vol_ma_len: int) -> Tuple[pd.DataFrame, Dict[str, Optional[Decimal]]]:
    ...
    if df is None or df.empty or not all(c in df.columns for c in required_cols) or len(df) < min_len:
        logger.warning(f"{Fore.YELLOW}Indicator Calc (Vol/ATR): Insufficient data (Len: {len(df) if df is not None else 0}, Need: {min_len}). Returning N/A.{Style.RESET_ALL}")
        return df, results
    ...
    try:
        ...
        if results["volume_ma"] is not None and results["volume_ma"] > CONFIG.position_qty_epsilon and results["last_volume"] is not None:
            try:
             ...
             results["volume_ratio"] = None # Cannot calculate ratio if MA is zero/NA or volume is NA
...
def calculate_stochrsi_momentum(df: pd.DataFrame, rsi_len: int, stoch_len: int, k: int, d: int, mom_len: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or not all(c in df.columns for c in required_input_cols) or len(df) < min_len:
        logger.warning(f"{Fore.YELLOW}Indicator Calc (StochRSI/Mom): Insufficient data (Len: {len(df) if df is not None else 0}, Need ~{min_len}). Setting cols to NA.{Style.RESET_ALL}")
        return df
    ...
    try:
        ...
        if k_col_pt in stochrsi_df.columns:
            ...
            logger.warning(f"StochRSI K column '{k_col_pt}' not found in pandas_ta output.")
        ...
        if d_col_pt in stochrsi_df.columns:
            ...
            logger.warning(f"StochRSI D column '{d_col_pt}' not found in pandas_ta output.")
        ...
        if mom_col_pt in df.columns:
            ...
            df.drop(columns=[mom_col_pt], errors='ignore', inplace=True) # Clean up raw column
        else:
            logger.warning(f"Momentum column '{mom_col_pt}' not found after calculation.")
        ...
        logger.debug(f"Indicator Calc (StochRSI({rsi_len},{stoch_len},{k},{d})/Mom({mom_len})): K={k_str}, D={d_str}, Mom={mom_str}")
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or not all(c in df.columns for c in required_input_cols) or len(df) < min_len:
        logger.warning(f"{Fore.YELLOW}Indicator Calc (EhlersFisher): Insufficient data (Len: {len(df) if df is not None else 0}, Need ~{min_len}). Setting cols to NA.{Style.RESET_ALL}")
        return df
    ...
    try:
        ...
        if fish_col_pt in fisher_df.columns:
            df[fish_col_internal] = fisher_df[fish_col_pt].apply(safe_decimal_conversion)
        else:
            logger.warning(f"Ehlers Fisher column '{fish_col_pt}' not found in pandas_ta output.")
        ...
        if signal > 0 and signal_col_pt in fisher_df.columns:
            df[signal_col_internal] = fisher_df[signal_col_pt].apply(safe_decimal_conversion)
        else:
             df[signal_col_internal] = pd.NA # Set to NA if signal length is 0 or column missing
        ...
        sig_str = f"{sig_val:.4f}" if pd.notna(sig_val) else ("NA" if signal > 0 else "N/A (SigLen=0)")
...
def calculate_ehlers_ma(df: pd.DataFrame, fast_len: int, slow_len: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or not all(c in df.columns for c in required_input_cols) or len(df) < min_len:
        logger.warning(f"{Fore.YELLOW}Indicator Calc (EhlersMA - PLACEHOLDER): Insufficient data (Len: {len(df) if df is not None else 0}, Need ~{min_len}). Setting cols to NA.{Style.RESET_ALL}")
        return df

    try:
        # --- PLACEHOLDER WARNING ---
        logger.warning(f"{Back.YELLOW}{Fore.BLACK}WARNING: Using standard EMA as placeholder for Ehlers Super Smoother MA. "
        ...
        logger.debug(f"Indicator Calc (EhlersMA Placeholder - EMA({fast_len},{slow_len})): Fast={fast_str}, Slow={slow_str}")

    except Exception as e:
        logger.error(f"{Fore.RED}Indicator Calc (EhlersMA Placeholder): Error: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        for col in target_cols: df[col] = pd.NA # Ensure reset on error
    return df
...
def analyze_order_book(exchange: ccxt.Exchange, symbol: str, depth: int, fetch_limit: int) -> Dict[str, Optional[Decimal]]:
    ...
    if not exchange.has.get('fetchL2OrderBook'):
        logger.warning(f"{Fore.YELLOW}Order Book: fetchL2OrderBook not supported by {exchange.id}. Cannot analyze.{Style.RESET_ALL}")
        return results
    try:
        ...
        if not bids or not asks:
            logger.warning(f"{Fore.YELLOW}Order Book: Empty bids or asks returned for {symbol}. OB analysis skipped.{Style.RESET_ALL}")
            return results

        # Get best bid/ask safely
        best_bid = safe_decimal_conversion(bids[0][0]) if len(bids) > 0 and len(bids[0]) > 0 else None
        best_ask = safe_decimal_conversion(asks[0][0]) if len(asks) > 0 and len(asks[0]) > 0 else None
...
def get_market_data(exchange: ccxt.Exchange, symbol: str, interval: str, limit: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not ohlcv:
            logger.warning(f"{Fore.YELLOW}Data Fetch: No OHLCV data returned for {symbol} ({interval}). Could be API issue, insufficient history, or incorrect symbol/interval.{Style.RESET_ALL}")
            return None
        ...
        if df.isnull().values.any():
            ...
            logger.warning(f"{Fore.YELLOW}Data Fetch: OHLCV contains NaNs after initial conversion:\n{initial_nan_counts[initial_nan_counts > 0]}\nAttempting forward fill...{Style.RESET_ALL}")
            df.ffill(inplace=True) # Forward fill first (carries last valid observation forward)
        ...
        if not all(pd.api.types.is_numeric_dtype(df[col]) for col in numeric_cols):
             logger.error(f"{Fore.RED}Data Fetch: Non-numeric data detected in OHLCV columns AFTER processing. Discarding fetch.{Style.RESET_ALL}")
             return None

        # Check if enough data was returned (sometimes API returns fewer than requested)
        if len(df) < limit // 2 : # Heuristic check for significantly less data than requested
            logger.warning(f"{Fore.YELLOW}Data Fetch: Received significantly fewer candles ({len(df)}) than requested ({limit}). Potential API issue or lack of history.{Style.RESET_ALL}")
            # Decide if this is critical - for now, proceed if some data exists

        logger.debug(f"Data Fetch: Successfully processed {len(df)} OHLCV candles for {symbol}. Last candle: {df.index[-1]}")
        return df
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
    ...
    try:
        if not exchange.has.get('fetchPositions'):
            logger.warning(f"{Fore.YELLOW}Position Check: Exchange capability 'fetchPositions' not reported by CCXT for {exchange.id}. Assuming no position.{Style.RESET_ALL}")
        ...
        category = 'linear' if market.get('linear', True) else 'inverse' # Default to linear if unsure
        ...
        for pos in fetched_positions:
            ...
            pos_side_v5 = str(pos_info.get('side', 'None')).strip().capitalize() # Normalize side
            size_str = pos_info.get('size') # Position size as string

            logger.debug(f"Processing fetched pos: MarketID={pos_market_id}, Idx={position_idx}, SideV5={pos_side_v5}, Size={size_str}")
            ...
            if pos_market_id == market_id and position_idx == 0 and pos_side_v5 in ['Buy', 'Sell']:
                ...
                if abs(size) > CONFIG.position_qty_epsilon:
                    ...
                    logger.debug(f"Found potential active One-Way position entry: {active_pos_data}")
        ...
        if active_pos_data:
            try:
                ...
                if side != CONFIG.pos_none and abs(size) > CONFIG.position_qty_epsilon and entry_price >= Decimal("0"): # Entry price shouldn't be negative
                    ...
                    logger.warning(f"{Fore.YELLOW}Position Check: Found position data but parsed as invalid/flat (Side:{side}, Qty:{size}, Entry:{entry_price}). Treating as flat.{Style.RESET_ALL}")
                    return default_pos

            except Exception as parse_err:
                 logger.warning(f"{Fore.YELLOW}Position Check: Error parsing active position data: {parse_err}. Raw data: {active_pos_data}{Style.RESET_ALL}")
                 return default_pos # Return default on parsing error
        else:
            # No position matched the strict criteria
            logger.info(f"Position Check: No active One-Way (Idx=0) position found for {market_id}.")
            return default_pos
        ...
        logger.error(f"{Fore.RED}Position Check: Unexpected error fetching/processing positions for {symbol}: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    try:
        ...
        if not market or not market.get('contract'):
            logger.error(f"{Fore.RED}Leverage Setting: Cannot set leverage for non-contract/non-futures market: {symbol}.{Style.RESET_ALL}")
            return False
    ...
    for attempt in range(CONFIG.retry_count):
        try:
            ...
            if "leverage not modified" in err_str or "leverage is same as requested" in err_str or "110044" in err_str:
                logger.info(f"{Fore.CYAN}Leverage Setting: Leverage already set to {leverage}x for {symbol}. No change needed.{Style.RESET_ALL}")
                return True
        ...
        if attempt < CONFIG.retry_count - 1:
            ...
            logger.error(f"{Fore.RED}Leverage Setting: Failed to set leverage for {symbol} to {leverage}x after {CONFIG.retry_count} attempts.{Style.RESET_ALL}")
            send_sms_alert(f"[{get_market_base_currency(symbol)}] CRITICAL: Failed to set leverage to {leverage}x for {symbol}!")
...
def close_position(exchange: ccxt.Exchange, symbol: str, position_to_close: Dict[str, Any], reason: str = "Signal") -> Optional[Dict[str, Any]]:
    ...
    logger.warning(f"{Back.YELLOW}{Fore.BLACK}Close Position: Initiated for {symbol}. Reason: {reason}. Expected state: {initial_side} Qty={initial_qty:.8f}{Style.RESET_ALL}")
    ...
    if live_position['side'] == CONFIG.pos_none:
        ...
        if initial_side != CONFIG.pos_none:
            logger.info(f"{Fore.CYAN}Close Position: Discrepancy detected (Bot expected {initial_side}, exchange reports None). State likely already updated.{Style.RESET_ALL}")
    ...
    try:
        ...
        if amount_to_close_precise <= CONFIG.position_qty_epsilon:
            logger.error(f"{Fore.RED}Close Position: Closing amount after precision ({amount_str}) is negligible or zero for {symbol}. Aborting closure. Manual check might be needed.{Style.RESET_ALL}")
        ...
        status = order.get('status', 'unknown') # Check status if available (market orders often fill immediately)
        ...
        fee_cost = safe_decimal_conversion(order.get('fee', {}).get('cost', '0.0')) # Fee cost if available
        ...
        return order # Return the order details
         ...
         logger.error(f"{Fore.RED}Close Position ({reason}): Failed for {symbol} - Insufficient Funds (Unexpected for reduceOnly): {e}{Style.RESET_ALL}")
         send_sms_alert(f"[{market_base}] ERROR Closing ({reason}): Insufficient Funds reported (Check Margin/State!).")
    except ccxt.ExchangeError as e:
        err_str = str(e).lower()
        ...
        if ("order would not reduce position size" in err_str or
            "position is zero" in err_str or "position has been closed" in err_str or
            "size is zero" in err_str or "qty exceeds available position size" in err_str or
            "110025" in err_str or "110045" in err_str or "30086" in err_str):
             logger.warning(f"{Fore.YELLOW}Close Position: Exchange indicates position likely already closed/closing or size mismatch ({e}). Assuming closure succeeded or wasn't needed.{Style.RESET_ALL}")
             # Send SMS indicating it was likely already closed if the bot initiated the closure
             send_sms_alert(f"[{market_base}] Close ({reason}): Exchange reported position likely already closed/zero.")
            ...
            logger.error(f"{Fore.RED}Close Position ({reason}): Unhandled Exchange error for {symbol}: {e}{Style.RESET_ALL}")
...
def calculate_position_size(equity: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, stop_loss_price: Decimal,
    ...
    if not (entry_price > 0 and stop_loss_price > 0):
        logger.error(f"{Fore.RED}Risk Calc: Invalid entry price ({entry_price}) or SL price ({stop_loss_price}). Must be positive.{Style.RESET_ALL}")
        return None, None
    if entry_price == stop_loss_price:
         logger.error(f"{Fore.RED}Risk Calc: Entry price cannot be exactly equal to SL price.{Style.RESET_ALL}")
         return None, None
    ...
    if not 0 < risk_per_trade_pct < 1:
        logger.error(f"{Fore.RED}Risk Calc: Invalid risk percentage: {risk_per_trade_pct:.4%}. Must be between 0 and 1 (exclusive).{Style.RESET_ALL}")
        return None, None
    if equity <= 0:
        logger.error(f"{Fore.RED}Risk Calc: Invalid equity: {equity:.4f}. Must be positive.{Style.RESET_ALL}")
        return None, None
    if leverage <= 0:
    ...
    try:
        ...
        quantity_precise_str = format_amount(exchange, symbol, quantity_raw) # format_amount handles rounding down
        quantity_precise = Decimal(quantity_precise_str)

    except (ValueError, InvalidOperation, ZeroDivisionError, Exception) as e:
        logger.error(f"{Fore.RED}Risk Calc: Error during quantity calculation or formatting for raw qty {quantity_raw if 'quantity_raw' in locals() else 'N/A'}: {e}{Style.RESET_ALL}")
        return None, None

    # Check if calculated quantity is valid after precision formatting
    if quantity_precise <= CONFIG.position_qty_epsilon:
        logger.warning(f"{Fore.YELLOW}Risk Calc: Calculated quantity negligible or zero after precision ({quantity_precise:.8f}). RiskAmt={risk_amount_usdt:.4f}, PriceDiff={price_diff:.4f}. Cannot place order.{Style.RESET_ALL}")
        return None, None
    ...
    try:
        ...
        required_margin_estimate = position_value_usdt / Decimal(leverage)
         ...
         logger.error(f"{Fore.RED}Risk Calc: Error calculating estimated margin: {e}{Style.RESET_ALL}")
         return None, None

    logger.debug(f"Risk Calc Result: Qty={quantity_precise:.8f}, RiskAmt={risk_amount_usdt:.4f}, EstValue={position_value_usdt:.4f}, EstMargin={required_margin_estimate:.4f}")
...
def wait_for_order_fill(exchange: ccxt.Exchange, order_id: str, symbol: str, timeout_seconds: int) -> Optional[Dict[str, Any]]:
    ...
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            if elapsed_time < 5.0: # Allow 5 seconds grace period
                ...
                logger.error(f"{Fore.RED}Order {order_id_short} not found after {elapsed_time:.1f}s grace period. Assuming failed/pruned.{Style.RESET_ALL}")
                return None
    ...
    try:
        ...
        logger.warning(f"Final status check for timed-out order {order_id_short}: Status='{final_order_check.get('status')}', Filled='{final_order_check.get('filled')}'")
...
def place_risked_market_order(exchange: ccxt.Exchange, symbol: str, side: str,
    ...
    logger.info(f"{Fore.BLUE}{Style.BRIGHT}Place Order: Initiating {action_verb} entry for {symbol} with Risk/SL/TSL...{Style.RESET_ALL}")

    # --- Pre-computation & Validation ---
    if current_atr is None or current_atr <= CONFIG.position_qty_epsilon:
        logger.error(f"{Fore.RED}Place Order ({action_verb}): Invalid or zero ATR ({current_atr}). Cannot calculate SL or place order.{Style.RESET_ALL}")
        return None
    ...
    try:
        ...
        logger.debug(f"Market Limits: MinQty={min_qty or 'N/A'}, MaxQty={max_qty or 'N/A'}, MinPrice={min_price or 'N/A'}")
        ...
        if usdt_equity <= Decimal("0"):
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Zero or invalid equity ({usdt_equity:.4f}). Cannot calculate risk or place order.{Style.RESET_ALL}")
            return None
        if usdt_free < Decimal("0"): # Free margin shouldn't be negative, but check defensively
             logger.warning(f"{Fore.YELLOW}Place Order ({action_verb}): Negative free margin detected ({usdt_free:.4f}). May indicate issues.{Style.RESET_ALL}")
        ...
        if side == CONFIG.side_buy and best_ask: entry_price_estimate = best_ask
            ...
            try:
                ...
                if not entry_price_estimate or entry_price_estimate <= 0:
                 ...
                 logger.error(f"{Fore.RED}Place Order ({action_verb}): Failed to fetch ticker or get valid price for estimation: {e}{Style.RESET_ALL}")
                 return None # Cannot proceed without a price estimate

        if not entry_price_estimate or entry_price_estimate <= 0:
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Failed to obtain a valid positive entry price estimate ({entry_price_estimate}). Aborting.{Style.RESET_ALL}")
            return None
        ...
        if min_price is not None and initial_sl_price_raw < min_price:
            logger.warning(f"{Fore.YELLOW}Calculated SL price estimate {initial_sl_price_raw:.4f} is below min price {min_price}. Adjusting SL estimate to min price.{Style.RESET_ALL}")
            initial_sl_price_raw = min_price
        if initial_sl_price_raw <= 0:
             logger.error(f"{Fore.RED}Place Order ({action_verb}): Invalid Initial SL price calculated ({initial_sl_price_raw:.4f}). Check ATR/multiplier/price data. Aborting.{Style.RESET_ALL}")
             return None
        ...
        if calc_qty is None or req_margin_estimate is None:
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Failed risk calculation. Cannot determine position size.{Style.RESET_ALL}")
            return None
        ...
        if pos_value_estimate > max_order_cap_usdt:
            logger.warning(f"{Fore.YELLOW}Calculated order value {pos_value_estimate:.4f} USDT exceeds cap {max_order_cap_usdt:.4f}. Capping quantity based on max value.{Style.RESET_ALL}")
        ...
        if final_quantity <= CONFIG.position_qty_epsilon:
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Final quantity ({final_quantity:.8f}) is negligible or zero after calculations/capping. Aborting.{Style.RESET_ALL}")
            return None
        if min_qty is not None and final_quantity < min_qty:
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Final quantity {final_quantity:.8f} is below market minimum {min_qty}. Aborting.{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Qty {final_quantity:.8f} < Min {min_qty}")
            return None
        if max_qty is not None and final_quantity > max_qty:
            logger.warning(f"{Fore.YELLOW}Final quantity {final_quantity:.8f} exceeds market maximum {max_qty}. Adjusting to max limit.{Style.RESET_ALL}")
        ...
        if usdt_free < req_margin_buffered:
            logger.error(f"{Fore.RED}Place Order ({action_verb}): Insufficient FREE margin. Need ~{req_margin_buffered:.4f} (incl. {margin_check_buffer:.1%} buffer), Have {usdt_free:.4f}{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Insufficient Free Margin (Need ~{req_margin_buffered:.2f})")
            return None

        logger.info(f"{Fore.GREEN}Pre-order Checks Passed. Final Qty: {final_quantity:.8f}, Est Margin: {req_margin_estimate:.4f}, Buffered Margin Need: {req_margin_buffered:.4f}{Style.RESET_ALL}")

        # === 7. Place Entry Market Order ===
        entry_order_details: Optional[Dict[str, Any]] = None
        try:
            qty_float = float(final_quantity) # CCXT requires float for amount in create_order
            logger.warning(f"{Back.GREEN}{Fore.BLACK}{Style.BRIGHT}*** Placing {action_verb} MARKET ENTRY: {qty_float:.8f} {symbol} ***{Style.RESET_ALL}")
            ...
            if not entry_order_id:
                ...
                logger.error(f"{Fore.RED}{Style.BRIGHT}Entry order placed but NO Order ID returned! Critical issue. Response: {entry_order_details}{Style.RESET_ALL}")
                ...
                if current_pos['side'] != CONFIG.pos_none:
                     logger.warning(f"{Fore.YELLOW}Position check reveals position exists despite missing entry ID! Qty: {current_pos['qty']}. Cannot reliably manage SL/TSL. Manual check required!{Style.RESET_ALL}")
                     send_sms_alert(f"[{market_base}] CRITICAL: Position opened ({action_verb}) but entry ID MISSING! Manual check needed!")
                ...
                raise ValueError("Entry order placement failed to return an ID and no position found.")
            ...
            logger.error(f"{Fore.RED}{Style.BRIGHT}FAILED TO PLACE ENTRY ORDER ({action_verb}): {e}{Style.RESET_ALL}")
            logger.debug(traceback.format_exc())
            send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Entry placement failed: {type(e).__name__}")
        ...
        if not filled_entry_order:
            logger.error(f"{Fore.RED}{Style.BRIGHT}Entry order {format_order_id(entry_order_id)} did NOT fill or failed within timeout.{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Entry {format_order_id(entry_order_id)} fill timeout/fail.")
            # Attempt to cancel the potentially stuck order (might fail if already filled/gone)
            try:
                logger.warning(f"Attempting to cancel potentially unfilled/failed entry order {format_order_id(entry_order_id)}...")
                exchange.cancel_order(entry_order_id, symbol)
                logger.info(f"Cancel request sent for {format_order_id(entry_order_id)}.")
            except ccxt.OrderNotFound:
                logger.warning(f"Could not cancel order {format_order_id(entry_order_id)} (likely already filled, cancelled, or rejected).")
            except (ccxt.ExchangeError, ccxt.NetworkError) as cancel_e:
                logger.warning(f"Error sending cancel request for order {format_order_id(entry_order_id)}: {cancel_e}")
            except Exception as cancel_e:
                logger.error(f"Unexpected error cancelling order {format_order_id(entry_order_id)}: {cancel_e}")
            ...
            current_pos = get_current_position(exchange, symbol)
            if current_pos['side'] != CONFIG.pos_none:
                 logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}CRITICAL STATE: POSITION OPENED ({current_pos['side']} Qty: {current_pos['qty']}) despite entry fill failure/timeout! Attempting emergency close!{Style.RESET_ALL}")
                 send_sms_alert(f"[{market_base}] CRITICAL: Position opened ({action_verb}) on FAILED entry fill! Closing NOW.")
        ...
        fee_cost = safe_decimal_conversion(filled_entry_order.get('fee', {}).get('cost', '0.0'))
        ...
        if avg_fill_price <= 0 or filled_qty <= CONFIG.position_qty_epsilon:
            ...
            send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Invalid fill details {format_order_id(entry_order_id)}.")
            ...
            if current_pos['side'] != CONFIG.pos_none:
                 ...
                 logger.warning("Position seems already closed or wasn't opened despite invalid fill data.")
        ...
        if min_price is not None and actual_sl_price_raw < min_price:
            logger.warning(f"{Fore.YELLOW}Adjusted SL price {actual_sl_price_raw:.4f} is below min price {min_price}. Adjusting SL to min price.{Style.RESET_ALL}")
        ...
        if actual_sl_price_raw <= 0:
            ...
            logger.error(f"{Fore.RED}{Style.BRIGHT}Invalid ACTUAL SL price calculated ({actual_sl_price_raw:.4f}) based on fill price {avg_fill_price:.4f}. CANNOT PLACE SL! Attempting emergency close!{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}] CRITICAL ({action_verb}): Invalid ACTUAL SL price ({actual_sl_price_raw:.4f})! Emergency Closing!")
            # Use filled details for emergency close attempt
            close_position(exchange, symbol, {'side': side, 'qty': filled_qty, 'entry_price': avg_fill_price}, reason="Emergency Close - Invalid SL Calc")
        ...
        try:
            ...
            if sl_qty_float <= 0: raise ValueError(f"Stop Loss quantity is zero or negative ({sl_qty_float})")
            ...
            if sl_order_id:
                ...
                logger.success(f"{Fore.GREEN}{sl_status}{Style.RESET_ALL}")
                 ...
                 send_sms_alert(f"[{market_base}] ERROR ({action_verb}): Failed initial SL placement (NO ID).")
            ...
            logger.debug(traceback.format_exc())
            send_sms_alert(f"[{market_base}] ERROR ({action_verb}): Failed initial SL placement: {type(e).__name__}")
        ...
        if tsl_percent > CONFIG.position_qty_epsilon: # Only place if TSL percentage is meaningful
            try:
                ...
                if act_price_raw <= 0: raise ValueError(f"Invalid TSL activation price calculated: {act_price_raw:.4f}")
                ...
                if tsl_qty_float <= 0: raise ValueError(f"Trailing Stop Loss quantity is zero or negative ({tsl_qty_float})")
                ...
                if tsl_order_id:
                    ...
                    send_sms_alert(f"[{market_base}] ERROR ({action_verb}): Failed TSL placement (NO ID).")
                ...
                logger.debug(traceback.format_exc())
                send_sms_alert(f"[{market_base}] ERROR ({action_verb}): Failed TSL placement: {type(e).__name__}")
        else:
            # TSL not configured
            tsl_status = "Not Configured (Percentage Zero)"
            logger.info(f"{Fore.CYAN}Trailing SL not configured (percentage is zero or less). Skipping TSL placement.{Style.RESET_ALL}")

        # === 13. Final Summary Log & SMS ===
        logger.info(f"{Back.BLUE}{Fore.WHITE}--- ORDER PLACEMENT SUMMARY ({action_verb} {symbol}) ---{Style.RESET_ALL}")
        logger.info(f"  Entry Order: {format_order_id(entry_order_id)} | Filled Qty: {filled_qty:.8f} @ Avg Price: {avg_fill_price:.4f}")
        ...
        tsl_summary = ('%' + tsl_trail_value_str if tsl_order_id else ('FAIL' if tsl_percent > 0 else 'OFF'))
        sms_summary = (f"[{market_base}] {action_verb} {filled_qty:.6f}@{avg_fill_price:.3f}. "
        ...
        logger.error(f"{Fore.RED}{Style.BRIGHT}Place Order ({action_verb}): Overall process failed before/during entry placement: {type(e).__name__} - {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        send_sms_alert(f"[{market_base}] ORDER FAIL ({action_verb}): Pre-entry setup/checks failed: {type(e).__name__}")
        ...
        if entry_order_id and not filled_entry_order:
             logger.warning("Checking position status after setup failure but before confirmed fill...")
             time.sleep(1.5)
             current_pos = get_current_position(exchange, symbol)
             if current_pos['side'] != CONFIG.pos_none:
                  logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}CRITICAL STATE: POSITION OPENED ({current_pos['side']} Qty: {current_pos['qty']}) despite order setup failure! Attempting emergency close!{Style.RESET_ALL}")
                  send_sms_alert(f"[{market_base}] CRITICAL: Position opened ({action_verb}) on FAILED setup! Closing NOW.")
...
def cancel_open_orders(exchange: ccxt.Exchange, symbol: str, reason: str = "Cleanup") -> None:
    ...
    try:
        ...
        if not exchange.has.get('fetchOpenOrders'):
            logger.warning(f"{Fore.YELLOW}Order Cancel: fetchOpenOrders capability not reported by CCXT for {exchange.id}. Cannot automatically cancel.{Style.RESET_ALL}")
            return
        if not exchange.has.get('cancelOrder'):
             logger.warning(f"{Fore.YELLOW}Order Cancel: cancelOrder capability not reported by CCXT for {exchange.id}. Cannot cancel.{Style.RESET_ALL}")
             return
        ...
        for order in open_orders:
            ...
            if order_stop_price: order_info += f" StopPx:{order_stop_price}"
            order_info += f" Status:{order_status})"

            if order_id and order_status == 'open': # Only attempt to cancel open orders with an ID
                try:
                    logger.debug(f"Cancelling order: {order_info}")
                    # Bybit V5 cancelOrder might need category, CCXT should handle it
                    exchange.cancel_order(order_id, symbol, params={'category': category})
                    logger.info(f"{Fore.CYAN}Order Cancel: Success for {order_info}{Style.RESET_ALL}")
                    cancelled_count += 1
                    time.sleep(0.15) # Small delay between cancellations to avoid potential rate limits
                ...
                logger.warning(f"Order Cancel: Found open order without ID: {order}") # Should not happen
            else:
                logger.debug(f"Skipping cancellation for non-open order: {order_info}") # Log skipped orders

        # Log summary of cancellation attempt
        log_level = logging.INFO if failed_count == 0 else logging.WARNING
        logger.log(log_level, f"{Fore.CYAN}Order Cancel: Finished for {symbol}. Attempted: {len(open_orders)}, Succeeded/Not Found: {cancelled_count}, Failed: {failed_count}.{Style.RESET_ALL}")
        if failed_count > 0:
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> Dict[str, Any]:
    ...
    if df is None or len(df) < required_rows:
        logger.warning(f"Signal Gen ({strategy_name}): Insufficient data ({len(df) if df is not None else 0} rows, need {required_rows})")
    ...
    try:
        ...
        if strategy_name == "DUAL_SUPERTREND":
            ...
            confirm_is_short = last.get('confirm_trend') is False # Explicitly check for False
        ...
        # --- StochRSI + Momentum Logic ---
            ...
            if not all(isinstance(val, Decimal) for val in [k_now, d_now, mom_now, k_prev, d_prev]):
                 logger.debug("Signal Gen (StochRSI/Mom): Skipping due to missing/invalid indicator data (NA values).")
        ...
        # --- Ehlers Fisher Logic ---
            ...
            if not isinstance(fish_now, Decimal) or not isinstance(fish_prev, Decimal):
                 logger.debug("Signal Gen (EhlersFisher): Skipping due to missing/invalid Fisher line data (NA values).")
                 return signals

            # Determine if signal line is valid and should be used for crosses
            use_signal_cross = isinstance(sig_now, Decimal) and isinstance(sig_prev, Decimal)

            if use_signal_cross:
                # Strategy: Fisher crossing Signal line
                logger.debug("Signal Gen (EhlersFisher): Using Fisher/Signal cross strategy.")
                ...
                logger.debug("Signal Gen (EhlersFisher): Signal line invalid/disabled, using Fisher Zero-Cross strategy.")
                zero = Decimal("0.0")
                # Entry Long: Fisher crosses above zero
                if fish_prev <= zero and fish_now > zero: signals['enter_long'] = True
                # Entry Short: Fisher crosses below zero
                if fish_prev >= zero and fish_now < zero: signals['enter_short'] = True
                # Exit Long: Fisher crosses below zero
                if fish_prev >= zero and fish_now < zero:
                    signals['exit_long'] = True; signals['exit_reason'] = "Ehlers Fisher crossed below Zero"
                # Exit Short: Fisher crosses above zero
                if fish_prev <= zero and fish_now > zero:
                    signals['exit_short'] = True; signals['exit_reason'] = "Ehlers Fisher crossed above Zero"
        ...
        # --- Ehlers MA Cross Logic (Placeholder EMA Cross) ---
            ...
            if not all(isinstance(val, Decimal) for val in [fast_ma_now, slow_ma_now, fast_ma_prev, slow_ma_prev]):
                 logger.debug("Signal Gen (EhlersMA Cross - Placeholder): Skipping due to missing/invalid MA data (NA values).")
                 return signals
        ...
        logger.error(f"{Fore.RED}Signal Generation Error: Missing expected indicator column in DataFrame: {e}. Strategy '{strategy_name}' cannot run correctly.{Style.RESET_ALL}")
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    required_lookbacks = [CONFIG.atr_calculation_period, CONFIG.volume_ma_period] # Base lookbacks needed by all
    if CONFIG.strategy_name == "DUAL_SUPERTREND":
        ...
        required_lookbacks.append(CONFIG.ehlers_fisher_length + CONFIG.ehlers_fisher_signal_length)
    elif CONFIG.strategy_name == "EHLERS_MA_CROSS":
        required_lookbacks.extend([CONFIG.ehlers_fast_period, CONFIG.ehlers_slow_period])

    # Need max lookback + 1 for current candle + buffer for calculation stability
    required_rows = max(required_lookbacks) + 2 # +2 = current candle + previous for comparisons

    if df is None or len(df) < required_rows:
        logger.warning(f"{Fore.YELLOW}Trade Logic: Insufficient data ({len(df) if df is not None else 0} rows, need ~{required_rows} for {CONFIG.strategy_name}). Skipping cycle.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        if current_price <= 0: # Check if price is valid
            logger.warning(f"{Fore.YELLOW}Trade Logic: Last candle close price is invalid or zero ({current_price}). Skipping cycle.{Style.RESET_ALL}")
            return
        ...
        if not can_calculate_sl:
            ...
            logger.warning(f"{Fore.YELLOW}Trade Logic: Invalid ATR ({current_atr}). Cannot calculate SL for new entries this cycle.{Style.RESET_ALL}")
        ...
        logger.info(f"State | Price: {current_price:.4f}, ATR({CONFIG.atr_calculation_period}): {atr_log}")
        vol_ratio_log = f"{vol_ratio:.2f}" if vol_ratio is not None else 'N/A'
        logger.info(f"State | Volume: Ratio={vol_ratio_log}, Spike={vol_spike} (EntryReq={CONFIG.require_volume_spike_for_entry})")
        ...
        try: # Add specific logging based on strategy, handle potential missing keys safely
            if CONFIG.strategy_name == "DUAL_SUPERTREND":
                ...
                k, d, mom = last.get('stochrsi_k'), last.get('stochrsi_d'), last.get('momentum')
        ...
        logger.info(f"State | OrderBook: Ratio={ob_ratio_log}, Spread={spread_log} (Fetched={ob_data is not None})")
        ...
        logger.debug(f"Strategy Signals ({CONFIG.strategy_name}): EnterLong={strategy_signals['enter_long']}, EnterShort={strategy_signals['enter_short']}, ExitLong={strategy_signals['exit_long']}, ExitShort={strategy_signals['exit_short']}, Reason='{strategy_signals['exit_reason']}'")
        ...
        if position_side != CONFIG.pos_none:
            ...
            if should_exit:
                ...
                logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}*** STRATEGY EXIT SIGNAL: Closing {position_side} position due to '{exit_reason}' ***{Style.RESET_ALL}")
                ...
                time.sleep(0.75) # Brief pause after cancel request before sending close order
                ...
                if close_result:
                    ...
                    time.sleep(CONFIG.post_close_delay_seconds) # Pause to allow state settlement
                else:
                    # This is a potential problem - the strategy indicated exit, but closure failed.
                    logger.error(f"{Fore.RED}{Style.BRIGHT}Failed to execute close order for {position_side} exit signal! Manual check required. Position might still be open.{Style.RESET_ALL}")
                    send_sms_alert(f"[{market_base}] CRITICAL: Failed to CLOSE {position_side} on signal '{exit_reason}'! Check position!")
                ...
                return
                 ...
                 logger.info(f"Holding {position_side} position. No strategy exit signal. Relying on exchange-managed SL/TSL.")
        ...
        if not can_calculate_sl:
             logger.warning(f"{Fore.YELLOW}Holding Cash. Cannot evaluate new entries because ATR is invalid for SL calculation this cycle.{Style.RESET_ALL}")
        ...
        if not CONFIG.fetch_order_book_per_cycle and potential_entry and ob_data is None:
            logger.debug("Potential entry signal detected, fetching Order Book for confirmation...")
            ob_data = analyze_order_book(exchange, symbol, CONFIG.order_book_depth, CONFIG.order_book_fetch_limit)
            ...
            ob_ratio_log = f"{bid_ask_ratio:.3f}" if bid_ask_ratio is not None else 'N/A'
            spread_log = f"{spread:.4f}" if spread is not None else 'N/A'
            logger.info(f"State Update | OrderBook (On Demand): Ratio={ob_ratio_log}, Spread={spread_log}")
        ...
        passes_long_ob_final = not ob_confirm_needed or (ob_available and passes_long_ob)
        passes_short_ob_final = not ob_confirm_needed or (ob_available and passes_short_ob)
        ...
        if entry_side:
            ...
            if place_result:
                ...
                logger.error(f"{Fore.RED}Entry placement failed for {entry_side.upper()} despite confirmed signal. See logs from place_risked_market_order.{Style.RESET_ALL}")
             ...
             if potential_entry and not (execute_long_entry or execute_short_entry):
                 reason = []
                 if enter_long_signal and not passes_long_ob_final: reason.append("OB Long")
                 if enter_short_signal and not passes_short_ob_final: reason.append("OB Short")
                 if not passes_volume and vol_confirm_needed: reason.append("Volume")
                 logger.warning(f"{Fore.YELLOW}Entry signal received but FAILED confirmation checks ({', '.join(reason)}). Holding cash.{Style.RESET_ALL}")
        ...
        logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}CRITICAL UNEXPECTED ERROR in trade_logic cycle {cycle_count}: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        send_sms_alert(f"[{market_base}] CRITICAL ERROR in trade_logic cycle {cycle_count}: {type(e).__name__}. Check logs!")
    finally:
...
def graceful_shutdown(exchange: Optional[ccxt.Exchange], symbol: Optional[str]) -> None:
    ...
    if not exchange or not symbol:
        logger.warning(f"{Fore.YELLOW}Shutdown: Exchange instance or symbol not available. Cannot perform API cleanup.{Style.RESET_ALL}")
        logger.info(f"{Fore.YELLOW}{Style.BRIGHT}--- Pyrmethus Shutdown Sequence Complete (No API Actions) ---{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        time.sleep(1.5) # Allow time for cancellations to be processed by the exchange API
        ...
        if position['side'] != CONFIG.pos_none:
            logger.warning(f"{Fore.YELLOW}Shutdown: Active {position['side']} position found (Qty: {position['qty']:.8f}). Attempting market close...{Style.RESET_ALL}")
            ...
            if close_result:
                ...
                time.sleep(CONFIG.post_close_delay_seconds * 2) # Wait longer to allow market closure and state update
                ...
                if final_pos['side'] == CONFIG.pos_none:
                    ...
                    logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}Shutdown Error: FAILED TO CONFIRM position closure after placing order. "
                ...
                logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}Shutdown Error: Failed to PLACE close order for active position ({position['side']} Qty: {position['qty']:.8f}). MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
            ...
            send_sms_alert(f"[{market_base}] No active position found during shutdown cleanup.")
        ...
        logger.debug(traceback.format_exc())
        send_sms_alert(f"[{market_base}] Error during shutdown cleanup: {type(e).__name__}. Manual check advised.")
    finally:
...
def main() -> None:
    ...
    logger.info(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bybit Scalping Spell v2.1.1 Initializing ({start_time_str}) ---{Style.RESET_ALL}")
    logger.info(f"{Fore.CYAN}--- Strategy Enchantment Selected: {CONFIG.strategy_name} ---{Style.RESET_ALL}")
    logger.info(f"{Fore.GREEN}--- Warding Runes Active: Initial ATR Stop Loss + Exchange Native Trailing Stop Loss ---{Style.RESET_ALL}")
    logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}--- !!! LIVE FUTURES SCALPING MODE ENGAGED - EXTREME RISK INVOLVED - USE CAUTION !!! ---{Style.RESET_ALL}")
    ...
    try:
        ...
        try:
            ...
            market = exchange.market(symbol_to_use) # Raises BadSymbol if not found/loaded
            ...
            if not market.get('contract'):
                raise ValueError(f"Market '{symbol}' is not a contract/futures market. Check SYMBOL config.")
            logger.info(f"{Fore.GREEN}Using Symbol: {symbol} (Type: {market.get('type', 'N/A')}, Exchange ID: {market.get('id')}){Style.RESET_ALL}")
            ...
            logger.critical(f"{Back.RED}{Fore.WHITE}Symbol/Leverage setup failed for '{CONFIG.symbol}': {e}{Style.RESET_ALL}")
            send_sms_alert(f"[Pyrmethus] CRITICAL: Symbol/Leverage setup FAILED ({e}). Exiting.")
            return # Exit if symbol/leverage setup fails
        except Exception as e:
            logger.critical(f"{Back.RED}{Fore.WHITE}Unexpected error during symbol/leverage setup: {e}{Style.RESET_ALL}")
            logger.debug(traceback.format_exc())
        ...
        logger.info(f"  Trailing SL: {CONFIG.trailing_stop_percentage:.2%} (Trail Dist), {CONFIG.trailing_stop_activation_offset_percent:.2%} (Activation Offset from Entry)")
        logger.info("  Confirmations:")
        logger.info(f"    Vol Spike: Required={CONFIG.require_volume_spike_for_entry} (MA={CONFIG.volume_ma_period}, Threshold={CONFIG.volume_spike_threshold}x)")
        logger.info(f"    Order Book: FetchPerCycle={CONFIG.fetch_order_book_per_cycle}, Depth={CONFIG.order_book_depth}, Long Ratio>={CONFIG.order_book_ratio_threshold_long}, Short Ratio<={CONFIG.order_book_ratio_threshold_short}")
        logger.info(f"  Timing: Sleep={CONFIG.sleep_seconds}s, FillTimeout={CONFIG.order_fill_timeout_seconds}s, PostCloseDelay={CONFIG.post_close_delay_seconds}s")
        logger.info(f"  Alerts: SMS Enabled={CONFIG.enable_sms_alerts} (To: {CONFIG.sms_recipient_number or 'Not Set'})")
        ...
        send_sms_alert(f"[{market_base}] Pyrmethus Bot started. Strategy: {CONFIG.strategy_name}. SL: ATR+TSL. Risk: {CONFIG.risk_per_trade_percentage:.2%}. Live Trading Active!")
        ...
        while run_bot:
            ...
            try:
                ...
                active_lookbacks = [CONFIG.atr_calculation_period, CONFIG.volume_ma_period]
                if CONFIG.strategy_name == "DUAL_SUPERTREND": active_lookbacks.extend([CONFIG.st_atr_length, CONFIG.confirm_st_atr_length])
                elif CONFIG.strategy_name == "STOCHRSI_MOMENTUM": active_lookbacks.extend([CONFIG.stochrsi_rsi_length + CONFIG.stochrsi_stoch_length + CONFIG.stochrsi_k_period + CONFIG.stochrsi_d_period, CONFIG.momentum_length])
                elif CONFIG.strategy_name == "EHLERS_FISHER": active_lookbacks.append(CONFIG.ehlers_fisher_length + CONFIG.ehlers_fisher_signal_length)
                elif CONFIG.strategy_name == "EHLERS_MA_CROSS": active_lookbacks.extend([CONFIG.ehlers_fast_period, CONFIG.ehlers_slow_period])
                # Fetch enough data for the longest lookback + buffer for calculations + previous candle access
                data_limit = max(active_lookbacks) + CONFIG.api_fetch_limit_buffer + 2 # +2 for prev candle access
                ...
                if df is not None and not df.empty:
                    ...
                    trade_logic(exchange, symbol, df) # df is already a copy if needed internally
                else:
                    # Handle case where data fetching failed
                    logger.warning(f"{Fore.YELLOW}No valid market data returned for {symbol} ({CONFIG.interval}) in cycle {cycle_count}. Skipping trade logic. Check connection or symbol validity.{Style.RESET_ALL}")
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                logger.warning(f"{Back.YELLOW}{Fore.BLACK}Rate Limit Exceeded: {e}. Sleeping for {CONFIG.sleep_seconds * 6}s...{Style.RESET_ALL}")
                ...
                logger.warning(f"{Fore.YELLOW}Network Error in main loop: {e}. Check connection. Will retry next cycle.{Style.RESET_ALL}")
                # Consider a slightly longer sleep or a connection check helper function if this persists
                time.sleep(CONFIG.sleep_seconds * 1.5) # Slightly longer sleep on network errors
            except ccxt.ExchangeNotAvailable as e:
                # Exchange maintenance or temporary outage
                logger.error(f"{Back.RED}{Fore.WHITE}Exchange Not Available: {e}. Sleeping for {CONFIG.sleep_seconds * 10}s... Check exchange status.{Style.RESET_ALL}")
                ...
                logger.critical(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}Authentication Error during operation: {e}. Invalid API keys or permissions? Stopping bot NOW.{Style.RESET_ALL}")
                run_bot = False # Signal loop termination
                send_sms_alert(f"[{market_base}] CRITICAL: Authentication Error! Bot stopping NOW.")
            except ccxt.ExchangeError as e: # Catch other specific exchange errors not handled above
                logger.error(f"{Fore.RED}Unhandled Exchange Error in main loop cycle {cycle_count}: {e}{Style.RESET_ALL}")
                ...
                logger.exception(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}!!! UNEXPECTED CRITICAL ERROR IN MAIN LOOP (Cycle {cycle_count}): {e} !!!{Style.RESET_ALL}")
            ...
            if run_bot: # Only sleep if the bot hasn't been signaled to stop
                ...
                logger.debug(f"Cycle {cycle_count} processed in {elapsed:.2f}s. Sleeping for {sleep_duration:.2f}s.")
                if sleep_duration > 0:
