...
class ScalpingBot:
    ...
    def load_config(self, config_file: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"{Fore.YELLOW}Created a default configuration file: '{config_file}'. "
    ...
    def validate_config(self) -> None:
        ...
        try:
            ...
            for key, type_ in [
                ...
                ("stoch_rsi_k_period", int),
                ("stoch_rsi_d_period", int),
    ...
    def _initialize_exchange(self) -> ccxt.Exchange:
        ...
        if not self.api_key or not self.api_secret:
            if not self.simulation_mode:
                logger.error(
                    f"{Fore.RED}API Key or Secret not found in environment variables (e.g., BYBIT_API_KEY). "
                    f"Cannot run in live mode. Exiting.{Style.RESET_ALL}"
                )
                sys.exit(1)
            else:
                logger.warning(
                    f"{Fore.YELLOW}API Key/Secret not found. Running in simulation mode only.{Style.RESET_ALL}"
                )
        ...
        try:
            ...
            if self.simulation_mode and self.exchange_id in [
                "bybit",
                "binance",
            ]:  # Add other exchanges supporting sandbox
            ...
            if self.symbol not in exchange.markets:
                logger.error(
                    f"{Fore.RED}Symbol '{self.symbol}' not found on {self.exchange_id.upper()}. "
                    f"Available symbols: {list(exchange.markets.keys())[:10]}... Exiting.{Style.RESET_ALL}"
                )
                sys.exit(1)

            logger.info(
                f"{Fore.GREEN}Successfully connected to {self.exchange_id.upper()} "
                f"({Fore.YELLOW}SIMULATION MODE{Style.RESET_ALL}"
                if self.simulation_mode
                else f"{Fore.GREEN}LIVE MODE{Style.RESET_ALL})."
            )
            return exchange

        except AttributeError:
            logger.error(
                f"{Fore.RED}Exchange ID '{self.exchange_id}' not found in ccxt library. Exiting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except ccxt.AuthenticationError as e:
            logger.error(
                f"{Fore.RED}Authentication failed for {self.exchange_id.upper()}: {e}. Check API keys. Exiting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except ccxt.ExchangeNotAvailable as e:
            logger.error(
                f"{Fore.RED}Exchange {self.exchange_id.upper()} is not available: {e}. Exiting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except ccxt.RequestTimeout as e:
            logger.error(
                f"{Fore.RED}Connection timed out while connecting to {self.exchange_id.upper()}: {e}. Exiting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except Exception as e:
            logger.error(
    ...
    def fetch_market_price(self) -> float | None:
        ...
        if ticker and "last" in ticker and ticker["last"] is not None:
            ...
            logger.warning(
                f"{Fore.YELLOW}Could not fetch last market price for {self.symbol}. Ticker data: {ticker}{Style.RESET_ALL}"
            )
            return None
    ...
    def fetch_order_book(self) -> float | None:
        ...
        if bids and asks:
            ...
            if bid_volume > 0:
                ...
                logger.warning(
                    f"{Fore.YELLOW}No bid volume found in the top {self.order_book_depth} levels for {self.symbol}.{Style.RESET_ALL}"
                )
                return float("inf")  # Return infinity to indicate extreme ask dominance
        else:
            logger.warning(
                f"{Fore.YELLOW}Order book data incomplete or unavailable for {self.symbol}. Bids: {len(bids)}, Asks: {len(asks)}{Style.RESET_ALL}"
            )
            return None
    ...
    def fetch_historical_data(self, limit: int | None = None) -> pd.DataFrame | None:
        ...
        if limit is None:
            ...
            limit = (
                max(
                    ...
                    - 2,  # Stoch RSI needs underlying RSI + smoothing
                )
                + 5
            )  # Add a small buffer
        ...
        try:
            ...
            logger.error(
                f"{Fore.RED}Error fetching or processing historical data for {self.symbol}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
    ...
    def fetch_balance(self, currency_code: str = "USDT") -> float:
        ...
        try:
            ...
            if free_balance is None:
                free_balance = 0.0  # Handle potential None value
    ...
    def calculate_macd(
        ...
        if close_prices is None or len(close_prices) < long_period + signal_period:
            logger.debug(
                f"Insufficient data for MACD calculation (need {long_period + signal_period}, got {len(close_prices)})."
            )
        ...
        logger.debug(
            f"Calculated MACD ({short_period},{long_period},{signal_period}): MACD={macd_val:.4f}, Signal={signal_val:.4f}, Hist={hist_val:.4f}"
        )
    ...
    def calculate_stoch_rsi(
        ...
        if len(stoch_rsi) < max(k_period, d_period):
            logger.debug(
                f"Insufficient Stoch RSI values for smoothing (need {max(k_period, d_period)}, got {len(stoch_rsi)})."
            )
            return None, None
        ...
        logger.debug(
            f"Calculated Stoch RSI ({rsi_period},{stoch_period},{k_period},{d_period}): %K={k_val:.2f}, %D={d_val:.2f}"
        )
        return k_val, d_val
    ...
    def calculate_order_size(self, base_currency: str = "USDT") -> float:
        ...
        if balance <= 0:
            logger.warning(
                f"{Fore.YELLOW}Insufficient balance ({balance} {base_currency}) to calculate order size.{Style.RESET_ALL}"
            )
            return 0.0
        ...
        if min_cost is not None and final_size < min_cost:
            logger.warning(
                f"{Fore.YELLOW}Calculated order size {final_size:.4f} {base_currency} is below minimum cost {min_cost} {base_currency}. Setting size to 0.{Style.RESET_ALL}"
            )
            return 0.0
        ...
        logger.info(
            f"{Fore.CYAN}Calculated order size: {final_size:.4f} {base_currency} (Balance: {balance:.2f} {base_currency}){Style.RESET_ALL}"
        )
        return final_size
    ...
    def compute_trade_signal_score(
        ...
        if orderbook_imbalance is not None:
            if orderbook_imbalance < (1 / self.imbalance_threshold):  # More bid volume
                score += 1
                reasons.append(
                    f"{Fore.GREEN}+1: Order book shows buy pressure (Imb: {orderbook_imbalance:.2f}){Style.RESET_ALL}"
                )
            elif orderbook_imbalance > self.imbalance_threshold:  # More ask volume
                score -= 1
                reasons.append(
                    f"{Fore.RED}-1: Order book shows sell pressure (Imb: {orderbook_imbalance:.2f}){Style.RESET_ALL}"
                )
            else:
                reasons.append(
                    f"{Fore.WHITE}0: Order book relatively balanced (Imb: {orderbook_imbalance:.2f}){Style.RESET_ALL}"
                )
        ...
        if ema is not None:
            if price > ema:
                score += 1
                reasons.append(
                    f"{Fore.GREEN}+1: Price ({price:.2f}) above EMA ({ema:.2f}) (bullish trend){Style.RESET_ALL}"
                )
            elif price < ema:
                score -= 1
                reasons.append(
        ...
        if macd_line is not None and macd_signal is not None:
            if macd_line > macd_signal:  # MACD line crossed above signal line
                score += 1
                reasons.append(
                    f"{Fore.GREEN}+1: MACD line ({macd_line:.4f}) above signal ({macd_signal:.4f}) (bullish momentum){Style.RESET_ALL}"
                )
            else:  # MACD line crossed below signal line
                score -= 1
                reasons.append(
                    f"{Fore.RED}-1: MACD line ({macd_line:.4f}) below signal ({macd_signal:.4f}) (bearish momentum){Style.RESET_ALL}"
                )
        ...
        if stoch_rsi_k is not None and stoch_rsi_d is not None:
            ...
            #     score -= 0.5
                ...
                reasons.append(
                    f"{Fore.WHITE}0: Stoch RSI K ({stoch_rsi_k:.2f}) & D ({stoch_rsi_d:.2f}) neutral{Style.RESET_ALL}"
                )
    ...
    def place_order(
        ...
        if min_amount is not None and order_size_base < min_amount:
            logger.warning(
                f"{Fore.YELLOW}Calculated order size {order_size_base:.8f} {market_info['base']} "
        ...
        # --- Live Trading Mode ---
            ...
            try:
                ...
                log_color = Fore.GREEN if side == "buy" else Fore.RED
                logger.info(
                    f"{log_color}Attempting to place LIVE {order_type.upper()} {side.upper()} order...{Style.RESET_ALL}"
                )
                ...
                order_price = order.get("price") or (
            ...
            # --- Specific Error Handling for Orders ---
                ...
                logger.error(
                    f"{Fore.RED}LIVE Order Failed (Insufficient Funds): Cannot place {side} order for {amount_precise} {market_info['base']}. {e}{Style.RESET_ALL}"
                )
                return None
            except ccxt.InvalidOrder as e:
                logger.error(
                    f"{Fore.RED}LIVE Order Failed (Invalid Order): Parameters likely incorrect. Amount={amount_precise}, Price={limit_price_precise}, SL={stop_loss_price}, TP={take_profit_price}. Error: {e}{Style.RESET_ALL}"
                )
                return None
    ...
    def manage_positions(self) -> None:
        ...
        if current_price is None:
            logger.warning(
                f"{Fore.YELLOW}Cannot manage positions: Failed to fetch current market price.{Style.RESET_ALL}"
            )
            return
        ...
        for position in list(self.open_positions):
            ...
            if exit_reason:
                logger.info(
                    f"Placing closing order for {position_side.upper()} position (ID: {position_id}) due to: {exit_reason}"
                )
                ...
                if closing_order:
                    ...
                    logger.error(
                        f"{Fore.RED}Failed to place closing order for position (ID: {position_id}). Position remains open. Manual intervention may be required.{Style.RESET_ALL}"
                    )
    ...
    def cancel_all_open_orders(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"{Fore.YELLOW}Found {len(open_orders)} open order(s) for {self.symbol}. Attempting cancellation...{Style.RESET_ALL}"
            )
            cancelled_count = 0
            failed_count = 0
            for order in open_orders:
                try:
                    ...
                    logger.info(
                        f"{Fore.YELLOW}Cancelled order: ID {order['id']}, Side: {order['side']}, Price: {order['price']}, Amount: {order['amount']}{Style.RESET_ALL}"
                    )
                    cancelled_count += 1
                    ...
                    logger.error(
                        f"{Fore.RED}Network error cancelling order {order['id']}: {e}. Will retry if decorator active.{Style.RESET_ALL}"
                    )
                    failed_count += 1
            ...
            logger.info(
                f"Order cancellation attempt finished. Cancelled: {cancelled_count}, Failed/Not Found: {failed_count}."
            )

        except Exception as e:
            logger.error(
                f"{Fore.RED}An error occurred during the order cancellation process: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                # Log indicator state
                logger.info(
                    f"Market State: Price={current_price:.2f}, Volatility={f'{volatility:.5f}' if volatility is not None else 'N/A'}"
                )
                logger.info(
                    f"Indicators: EMA={f'{indicators["ema"]:.2f}' if indicators['ema'] is not None else 'N/A'}, "
                ...
                logger.info(
                    f"Order Book Imbalance: {f'{orderbook_imbalance:.2f}' if orderbook_imbalance is not None else 'N/A'} "
                ...
                if not can_open_new_position:
                    ...
                    if signal_score >= ENTRY_SIGNAL_THRESHOLD_ABS:
                        logger.info(
                            f"{Fore.GREEN}Potential LONG entry signal detected (Score: {signal_score}){Style.RESET_ALL}"
                        )

                        # Adjust TP/SL based on confidence (signal score magnitude)
                        adjustment_factor = (
                            self.strong_signal_adjustment_factor
                            if abs(signal_score) >= STRONG_SIGNAL_THRESHOLD_ABS
                            else self.weak_signal_adjustment_factor
                        )
                        ...
                        limit_entry_price = (
                            current_price * (1 - self.limit_order_entry_offset_pct_buy)
                        ...
                        if entry_order:
                            ...
                            logger.info(
                                f"{Fore.GREEN}---> Entered LONG position. Size: {entry_order['amount']}, Entry: {actual_entry_price:.2f}, "
                    ...
                    # Check for SHORT entry signal
                        ...
                        logger.info(
                            f"{Fore.RED}Potential SHORT entry signal detected (Score: {signal_score}){Style.RESET_ALL}"
                        )
                        ...
                        take_profit_price = current_price * (1 - take_profit_pct)
                        limit_entry_price = (
                            current_price * (1 + self.limit_order_entry_offset_pct_sell)
                            if self.entry_order_type == "limit"
                            else None
                        )
                        ...
                        if entry_order:
                            ...
                            logger.info(
                                f"{Fore.RED}---> Entered SHORT position. Size: {entry_order['amount']}, Entry: {actual_entry_price:.2f}, "
                                f"SL: {stop_loss_price:.2f} ({stop_loss_pct * 100:.2f}%), TP: {take_profit_price:.2f} ({take_profit_pct * 100:.2f}%){Style.RESET_ALL}"
                            )
                ...
                logger.debug(
                    f"Iteration {self.iteration} complete. Waiting {DEFAULT_SLEEP_INTERVAL_SECONDS} seconds..."
                )
                time.sleep(DEFAULT_SLEEP_INTERVAL_SECONDS)
