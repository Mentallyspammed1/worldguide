    ...
    for attempt in range(CONFIG.max_fetch_retries + 1):  # +1 to allow logging final failure
        try:
            ...
            if hasattr(e, "args") and len(e.args) > 0 and isinstance(e.args[0], str):
                ...
                if (
                    "Invalid symbol" in e.args[0] or "Order quantity not specified" in e.args[0]
                ):  # Add more non-retryable patterns
                    logger.error(
                        Fore.RED + f"{fetch_function.__name__}: Non-retryable exchange error: {e}. Stopping retries."
                    )
                    should_retry = False
...
def place_risked_market_order(symbol: str, side: str, risk_percentage: Decimal, atr: Decimal) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                Fore.RED + Style.BRIGHT + f"Failed to place initial SL order: {e}. Position might be UNPROTECTED."
            )
            logger.warning(
                Fore.YELLOW
    ...
    if profit > activation_threshold_points:
        ...
        if not is_placeholder_sl:
            ...
            try:
                ...
                logger.error(
                    Fore.RED + Style.BRIGHT + f"Unexpected error cancelling initial SL order: {e}", exc_info=True
                )
        ...
        try:
            ...
            logger.error(
                Fore.RED + Style.BRIGHT + f"CRITICAL: Failed to set TSL after clearing initial SL/placeholder: {e}"
            )
            logger.warning(
                Fore.YELLOW + Style.BRIGHT + "Position may be UNPROTECTED. MANUAL INTERVENTION STRONGLY ADVISED."
            )
...
def print_status_panel(
    ...
    atr_str = (
        f"{indicators['atr']:.6f}"
        if indicators and indicators.get("atr") is not None and not indicators["atr"].is_nan()
    ...
    long_stop_status = (
        get_stop_status(long_sl_id, long_tsl_id) if long_qty >= CONFIG.position_qty_epsilon else f"{Fore.WHITE}-"
    )
    short_stop_status = (
        get_stop_status(short_sl_id, short_tsl_id) if short_qty >= CONFIG.position_qty_epsilon else f"{Fore.WHITE}-"
    )
    ...
    if not pos_avail:
        ...
        long_pnl_str = (
            f"{long_pnl_color}{long_pnl:+.4f}{Fore.WHITE}" if long_qty >= CONFIG.position_qty_epsilon else "-"
        )
        short_pnl_str = (
            f"{short_pnl_color}{short_pnl:+.4f}{Fore.WHITE}" if short_qty >= CONFIG.position_qty_epsilon else "-"
        )
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    is_flat_after_tsl = (
        long_qty_after_tsl < CONFIG.position_qty_epsilon and short_qty_after_tsl < CONFIG.position_qty_epsilon
    )
    logger.debug(
        ...
        if positions:
            ...
            for side, pos_data in positions.items():
                ...
                if qty.copy_abs() >= CONFIG.position_qty_epsilon:
                    ...
                    logger.warning(
                        Fore.YELLOW + f"Closing {side} position (Qty: {qty}, Entry: {entry_price}) with market order..."
                    )
                    try:
                        ...
                        if close_order and close_order.get("id"):
                            ...
                            close_order and close_order.get("info", {}).get("retCode") == 0
            ...
            if closed_count == 0 and any(
                p["qty"].copy_abs() >= CONFIG.position_qty_epsilon for p in positions.values()
            ):
                logger.warning(
                    Fore.YELLOW
            ...
            logger.error(
                Fore.RED + Style.BRIGHT + "Could not fetch final positions during shutdown. MANUAL CHECK REQUIRED."
            )

    except Exception as e:
        logger.error(
            Fore.RED + Style.BRIGHT + f"Error during position closure check: {e}. Manual check advised.", exc_info=True
        )
