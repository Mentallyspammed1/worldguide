    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, color: str = NEON["PARAM"], secret: bool = False) -> Any:
        ...
        try:
            ...
            if cast_type == bool: final_value = raw_value_str_for_cast.lower() in ["true", "1", "yes", "y"]
                ...
                _logger.warning(f"{NEON['WARNING']}Unsupported cast_type '{cast_type.__name__}' for key '{key}'. Returning raw.{NEON['RESET']}")
            ...
            if default is None: # If default is also None, and casting failed
                ...
                try:
                    default_str_for_cast = str(default) # Ensure default is string before type-specific casting
                    ...
                    else: final_value = default # Should not happen with supported cast_types
...
def load_persistent_state() -> bool:
    ...
    try:
        ...
        logger.debug(traceback.format_exc())
        try: os.remove(STATE_FILE_PATH) # Attempt to remove corrupted state file
        except OSError: pass
...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        if hasattr(exchange, 'sandbox') and exchange.sandbox: # Check if sandbox mode is active
            ...
            if "DEMO" in CONFIG.api_key.upper(): # Simple check
                 logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}!!! POTENTIAL DEMO/PAPER TRADING ACCOUNT DETECTED (via API Key) !!!{NEON['RESET']}")
            else:
...
class MockExchange: # For testing without real API keys
    ...
    def load_markets(self,reload=False): pass # `reload` is the modern ccxt param
    ...
    def fetch_positions(self,symbols=None,params=None):
        ...
        if qty > CONFIG.position_qty_epsilon:
            side = _active_trade_parts[0]['side'] if _active_trade_parts else CONFIG.pos_none
        ...
        exchange_side = "Buy" if side == CONFIG.pos_long else ("Sell" if side == CONFIG.pos_short else "None")
        
        return [{"info":{"symbol":self.markets[CONFIG.symbol]['id'],"positionIdx":0,"size":str(qty),"avgPrice":str(avg_px),"side":exchange_side}}] if qty > CONFIG.position_qty_epsilon else []
    ...
    def create_order(self,s,type,side,amt,price=None,params=None): # For SL/TSL
        ...
        if type.lower() == "stopmarket" and params and (params.get("stopPrice") or params.get("trailingStop")):
            return {"id":f"mock_stop_{int(time.time()*1000)}","status":"open", "params": params, "symbol": s, "type": type, "side": side, "amount": float(amt)}
    ...
    def fetch_order(self,id,s,params=None):
        ...
        if "lim_" in id: time.sleep(0.05); return {"id":id,"status":"closed","average": self.fetch_ticker(s)['last'],"filled":1.0,"timestamp":int(time.time()*1000)} # Assuming it fills with qty 1 for simplicity
    ...
    def fetch_open_orders(self,s,params=None): return [] # Assume no other open orders for simplicity
    def cancel_order(self,id,s,params=None): return {"id":id,"status":"canceled"}
    def set_leverage(self,leverage,s,params=None): return {"status":"ok", "info": {"leverage": str(leverage)}} # Mock response
    ...
    def parse_timeframe(self,tf_str): # Convert timeframe string to seconds
        if 'm' in tf_str: return int(tf_str.replace('m','')) * 60
        if 'h' in tf_str: return int(tf_str.replace('h','')) * 3600
        if 'd' in tf_str: return int(tf_str.replace('d','')) * 86400
        return 60 # Default to 1 minute if parsing fails
    has={"fetchOHLCV":True,"fetchL2OrderBook":True} # Advertise capabilities
    def fetch_ohlcv(self,s,tf,lim,since=None,params=None): # More structured mock OHLCV
        now_ms=int(time.time()*1000); tf_s=self.parse_timeframe(tf); data_points=[]
        start_ts = now_ms - (lim -1) * tf_s * 1000 # Calculate start timestamp based on limit
        if since is not None and since > start_ts : # If since is provided and more recent, adjust start
            start_ts = since
        ...
        for i in range(lim): # Still generate 'lim' candles for simplicity in mock
            ...
            if ts > now_ms + tf_s : # Don't generate candles too far into the future (allow one partial future candle)
                continue

            price_base = 30000 + (i - lim / 2) * 10 # Base price trend
            price_noise = (random.random() - 0.5) * 50 # Random noise component
            
            open_p = price_base + price_noise + (i%5-2)*3 # Add some deterministic variation
            ...
            if open_p > high_p: high_p = open_p
            if open_p < low_p: low_p = open_p
            if close_p > high_p: high_p = close_p
            if close_p < low_p: low_p = close_p
            
            volume = 100 + i + (ts % 50) + random.randint(0,50) # Volume with some trend and randomness
...
vol_atr_analysis_results_cache: Dict[str, Any] = {} # Cache for ATR/Volume analysis results
...
def calculate_supertrend(df: pd.DataFrame, length: int, multiplier: Decimal, prefix: str = "") -> pd.DataFrame:
    ...
    if not all(c in df.columns for c in ["high", "low", "close"]) or df.empty or len(df) < length + 1: # Min length for ATR
        logger.warning(f"{NEON['WARNING']}Scrying (Supertrend {prefix}): Insufficient data or missing HLC columns (Rows: {len(df)}, Min: {length+1}). Populating NAs.{NEON['RESET']}")
        for col in output_cols: df[col] = pd.NA
        return df

    try:
        temp_df_st = df[['high', 'low', 'close']].apply(pd.to_numeric, errors='coerce').copy() # Ensure numeric and use a copy
        
        st_df = temp_df_st.ta.supertrend(length=length, multiplier=float(multiplier), append=False)
        
        m_str = str(float(multiplier))
        pta_val_col_pattern = f"SUPERT_{length}_{m_str}"
        pta_dir_col_pattern = f"SUPERTd_{length}_{m_str}"

        if st_df is None or pta_val_col_pattern not in st_df.columns or pta_dir_col_pattern not in st_df.columns:
            logger.error(f"{NEON['ERROR']}Scrying (Supertrend {prefix}): pandas_ta did not return expected SuperTrend columns. Found: {st_df.columns if st_df is not None else 'None'}. Expected patterns like: {pta_val_col_pattern}, {pta_dir_col_pattern}{NEON['RESET']}")
            for col in output_cols: df[col] = pd.NA
            return df
        ...
        logger.error(f"{NEON['ERROR']}Scrying (Supertrend {prefix}): Error during calculation: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        for col in output_cols: df[col] = pd.NA
    return df
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal_length: int) -> pd.DataFrame:
    ...
    if not all(c in df.columns for c in required_ohlc_cols) or df.empty or len(df) < length + signal_length + 5: # Approx min length
        logger.warning(f"{NEON['WARNING']}Scrying (EhlersFisher): Insufficient data or missing HLC columns (Rows: {len(df)}, Min: {length + signal_length + 5}). Populating NAs.{NEON['RESET']}")
        for col in output_cols: df[col] = pd.NA
        return df

    try:
        temp_df_ef = df[required_ohlc_cols].apply(pd.to_numeric, errors='coerce').copy()
        fisher_df_pta = temp_df_ef.ta.fisher(length=length, signal=signal_length, append=False)
        ...
        if fisher_df_pta is not None and not fisher_df_pta.empty:
            df[fisher_col] = fisher_df_pta[pta_fisher_col_name].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if pta_fisher_col_name in fisher_df_pta else pd.NA
            df[signal_col] = fisher_df_pta[pta_signal_col_name].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if pta_signal_col_name in fisher_df_pta else pd.NA
        else:
            df[fisher_col], df[signal_col] = pd.NA, pd.NA

        if not df.empty and fisher_col in df.columns and signal_col in df.columns and \
           not pd.isna(df[fisher_col].iloc[-1]) and not pd.isna(df[signal_col].iloc[-1]):
...
def analyze_volume_atr(df: pd.DataFrame, short_atr_len: int, long_atr_len: int, vol_ma_len: int, dynamic_sl_enabled: bool) -> Dict[str, Union[Decimal, PandasNAType, None]]:
    ...
    try:
        ...
        if temp_df.empty or len(temp_df) < max(short_atr_len, long_atr_len if dynamic_sl_enabled else 0, vol_ma_len) +1 : # +1 for pandas_ta
        ...
        if 'volume' in temp_df.columns and len(temp_df) >= vol_ma_len: # Use temp_df (cleaned) for MA
            results["volume_ma"] = safe_decimal_conversion(temp_df["volume"].rolling(window=vol_ma_len, min_periods=1).mean().iloc[-1], pd.NA)

        # Volume Ratio
        if not pd.isna(results["last_volume"]) and not pd.isna(results["volume_ma"]) and \
           results["volume_ma"] is not None and results["volume_ma"] > CONFIG.position_qty_epsilon and \
           results["last_volume"] is not None:
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> Tuple[pd.DataFrame, Dict[str, Any]]: # Return type for dict
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
    ...
    if consolidated_qty <= CONFIG.position_qty_epsilon: # If bot thinks position is closed
        _active_trade_parts.clear()
    ...
    current_pos_side = _active_trade_parts[0]['side'] if _active_trade_parts else CONFIG.pos_none # Assuming all parts have same side
    ...
    if exchange_pos_data["side"] != current_pos_side or \
       abs(exchange_pos_data["qty"] - consolidated_qty) > CONFIG.position_qty_epsilon * Decimal('10'): # Increased tolerance
...
def _get_raw_exchange_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        category = "inverse" if market.get("inverse") else "linear" # Default to linear if not specified
        ...
        for pos_data in fetched_positions:
            ...
            if int(pos_info.get("positionIdx", -1)) == 0:
                ...
                if size_dec > CONFIG.position_qty_epsilon: # If position size is significant
                    ...
                    bybit_side_str = pos_info.get("side") # "Buy" for Long, "Sell" for Short
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    try:
        ...
        response = safe_api_call(exchange.set_leverage, leverage, symbol, params=params) # ccxt handles mapping leverage to buyLeverage/sellLeverage
...
def calculate_dynamic_risk() -> Decimal:
    ...
    if trend >= 0.5: # Winning more or breakeven
        scale_factor = (trend - 0.5) / 0.5 # Scale from 0 (at 0.5 trend) to 1 (at 1.0 trend)
        dynamic_risk = base_risk + (max_risk - base_risk) * Decimal(scale_factor)
    else: # Losing more
        scale_factor = (0.5 - trend) / 0.5 # Scale from 0 (at 0.5 trend) to 1 (at 0.0 trend)
        dynamic_risk = base_risk - (base_risk - min_risk) * Decimal(scale_factor)
    
    final_risk = max(min_risk, min(max_risk, dynamic_risk)) # Clamp within min/max bounds
...
def calculate_position_size(usdt_equity: Decimal, risk_pct: Decimal, entry_px: Decimal, sl_px: Decimal,
    ...
    if price_diff_per_unit < CONFIG.position_qty_epsilon: # Avoid division by zero or near-zero
...
def wait_for_order_fill(exchange: ccxt.Exchange, order_id: str, symbol: str, timeout_seconds: int, order_type: str = "market") -> Optional[Dict[str, Any]]:
    ...
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            time.sleep(1.0 if order_type == "limit" else 0.75) # Poll more frequently for market orders
...
def place_risked_order(exchange: ccxt.Exchange, symbol: str, side: str,
    ...
    try:
        ...
        signal_price = safe_decimal_conversion(ticker.get("last"), pd.NA) # Use last price as signal price
        ...
        if order_value_usdt > max_order_cap_usdt:
            logger.info(f"{NEON['INFO']}Order value ({_format_for_log(order_value_usdt,2)}) exceeds MAX_ORDER_USDT_AMOUNT ({_format_for_log(max_order_cap_usdt,2)}). Capping order size.{NEON['RESET']}")
            capped_qty_raw = max_order_cap_usdt / signal_price
            order_quantity = Decimal(format_amount(exchange, symbol, capped_qty_raw))
            logger.info(f"{NEON['INFO']}Order quantity capped to {_format_for_log(order_quantity, color=NEON['QTY'])}.{NEON['RESET']}")
            if order_quantity <= CONFIG.position_qty_epsilon: # Recheck after capping
                logger.error(f"{NEON['ERROR']}Capped order quantity is negligible. Cannot place order.{NEON['RESET']}")
                return None
            if order_quantity is not None: # Ensure mypy
                estimated_margin = (order_quantity * signal_price) / Decimal(leverage) # Recalculate margin
        ...
        limit_price_str_for_slippage: Optional[str] = None # For slippage calculation if limit order
        ...
        if actual_filled_quantity <= CONFIG.position_qty_epsilon or actual_fill_price <= CONFIG.position_qty_epsilon or entry_timestamp_ms is None:
        ...
        _active_trade_parts.append({
            "id": new_part_identifier, "entry_price": actual_fill_price, "entry_time_ms": int(entry_timestamp_ms), # Ensure int
            "side": side, "qty": actual_filled_quantity, "sl_price": Decimal(actual_sl_price_str) # Store precise SL
        })
        ...
        if Decimal(actual_sl_price_str) > 0:
            ...
            try:
                ...
                sl_order_response = safe_api_call(exchange.create_order, symbol, "StopMarket", sl_order_side, float(actual_filled_quantity), price=None, params=sl_params_v5)
            ...
            except ccxt.InvalidOrder as e_inv_ord: logger.error(f"{NEON['CRITICAL']}SL Placement Failed (Part {new_part_identifier}): Invalid Order! {e_inv_ord}{NEON['RESET']}") # Often param issues
        ...
        if not is_scale_in and CONFIG.trailing_stop_percentage > 0 and actual_filled_quantity > CONFIG.position_qty_epsilon:
            ...
            if Decimal(tsl_activation_price_str) > 0:
                ...
                try:
                    ...
                    tsl_order_response = safe_api_call(exchange.create_order, symbol, "StopMarket", tsl_order_side, float(actual_filled_quantity), price=None, params=tsl_params_specific_v5)
        ...
        if not sl_placed_successfully :
            ...
            close_position(exchange, symbol, {"side": side, "qty": actual_filled_quantity, "entry_price": actual_fill_price}, reason=f"EMERGENCY CLOSE - SL FAIL ({action_type} part {new_part_identifier})")
            # Remove the part that failed SL placement from _active_trade_parts
            _active_trade_parts = [p for p in _active_trade_parts if p["id"] != new_part_identifier]
        ...
        logger.debug(f"Params sent: symbol={symbol}, side={side}, qty={order_quantity if 'order_quantity' in locals() else 'N/A'}, entry_type={entry_type.value}")
...
def close_partial_position(exchange: ccxt.Exchange, symbol: str, close_qty: Optional[Decimal] = None, reason: str = "Scale Out") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"{NEON['WARNING']}ACTION REQUIRED: Manually cancel/adjust SL for partially closed part ID {oldest_part['id']}. Automated SL adjustment for partial closes is complex.{NEON['RESET']}")
...
def close_position(exchange: ccxt.Exchange, symbol: str, position_to_close_details: Dict[str, Any], reason: str = "Signal") -> Optional[Dict[str, Any]]:
    ...
    position_side_for_log = _active_trade_parts[0]['side'] # Assume all parts have same side
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, market_data_df: pd.DataFrame) -> None:
    ...
    # --- Max Drawdown Check ---
    if CONFIG.enable_max_drawdown_stop and (now_timestamp - _last_drawdown_check_time > 300): # Check every 5 mins
        try:
            balance = safe_api_call(exchange.fetch_balance, params={"category": "linear"}) # V5 specific
    ...
    df_with_indicators, current_vol_atr_data = calculate_all_indicators(market_data_df.copy(), CONFIG) # Use a copy for indicators
    current_atr_short = current_vol_atr_data.get("atr_short", Decimal("0")) # Default to 0 if not found
    ...
    if CONFIG.enable_scale_out and pos_side != CONFIG.pos_none and num_active_parts > 0:
        ...
        if current_atr_short > 0 and avg_pos_entry_price > 0: # Ensure valid ATR and entry price
        ...
        if profit_in_atr >= CONFIG.scale_out_trigger_atr:
            ...
            pos_side, total_pos_qty, avg_pos_entry_price = current_position["side"], current_position["qty"], current_position["entry_price"]
            num_active_parts = current_position.get("num_parts", 0)
            if pos_side == CONFIG.pos_none: return # If position fully closed by scale-out
    ...
    save_persistent_state(force_heartbeat=True) # Heartbeat save if no other major action triggered save
...
def graceful_shutdown(exchange_instance: Optional[ccxt.Exchange], trading_symbol: Optional[str]) -> None:
    ...
    if exchange_instance and trading_symbol:
        try:
            ...
            current_pos_on_exchange = _get_raw_exchange_position(exchange_instance, trading_symbol)
            if current_pos_on_exchange["side"] != CONFIG.pos_none and current_pos_on_exchange["qty"] > CONFIG.position_qty_epsilon:
                logger.warning(f"{NEON['WARNING']}Unweaving: Active {current_pos_on_exchange['side']} position found on exchange. Attempting final consolidated close...{NEON['RESET']}")
                # Use the position details fetched directly from exchange for closing
                close_position(exchange_instance, trading_symbol, current_pos_on_exchange, "Bot Shutdown Final Close")
            elif _active_trade_parts: # If bot *thinks* it has parts but exchange is flat
                 logger.warning(f"{NEON['WARNING']}Unweaving: Bot remembers active parts, but exchange reports FLAT. Clearing bot state.{NEON['RESET']}")
                 _active_trade_parts.clear()
                 save_persistent_state(force_heartbeat=True)
            else:
                logger.info(f"{NEON['INFO']}Unweaving: No active position found on exchange or in bot state for '{trading_symbol}'.{NEON['RESET']}")
...
_last_market_data_fetch_ts: Dict[str, float] = {} # Cache timestamp per symbol_timeframe
...
def get_market_data(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int = 150) -> Optional[pd.DataFrame]:
    ...
    if cache_key in _market_data_cache and cache_key in _last_market_data_fetch_ts:
        try:
            ...
            if (current_time_seconds - _last_market_data_fetch_ts[cache_key]) < (candle_duration_seconds * float(CONFIG.cache_candle_duration_multiplier)):
                ...
                if len(_market_data_cache[cache_key]) >= limit:
                    logger.debug(f"Data Fetch: Using CACHED market data for {NEON['VALUE']}{cache_key}{NEON['DEBUG']} ({len(_market_data_cache[cache_key])} candles).{NEON['RESET']}")
                    return _market_data_cache[cache_key].copy() # Return a copy to prevent modification
                else:
        ...
        except Exception as e_parse_tf: # Handle if parse_timeframe fails for some reason
    ...
    try:
        ...
        if df.isnull().values.any():
            ...
            if df[['close']].iloc[-1].isnull().any():
                 logger.critical(f"{NEON['CRITICAL']}Last 'close' price is NaN for {symbol}. Cannot proceed with this data.{NEON['RESET']}")
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            if not market_details.get("contract"): # Check if it's a futures/contract market
        ...
        if load_persistent_state():
            ...
            if _active_trade_parts: # If bot remembers active parts, verify with exchange
                ...
                elif exchange_pos['side'] != bot_side or abs(exchange_pos['qty'] - bot_qty) > CONFIG.position_qty_epsilon * Decimal('10'): # Use larger epsilon for sum of parts
        ...
        try: 
            balance = safe_api_call(current_exchange_instance.fetch_balance, params={"category":"linear"}) # V5 specific
        ...
        market_base_name = unified_trading_symbol.split("/")[0].split(":")[0] # e.g., BTC from BTC/USDT:USDT
        ...
        while should_run_bot:
            ...
            try: 
                ...
                if not current_exchange_instance.fetch_balance(params={"category":"linear"}): # V5 specific
            ...
            try:
                ...
                indicator_max_lookback = max(CONFIG.st_atr_length, CONFIG.confirm_st_atr_length, CONFIG.momentum_period, 
                                             CONFIG.ehlers_fisher_length, CONFIG.ehlers_fisher_signal_length, # Added Ehlers
                                             CONFIG.atr_short_term_period, CONFIG.atr_long_term_period, CONFIG.volume_ma_period,
                                             50) # General minimum
                data_limit_needed = indicator_max_lookback + CONFIG.api_fetch_limit_buffer + 20 # Buffer for NaNs and stability
    ...
    except Exception as startup_err: # Catch errors during initial setup before main loop
        ...
        if 'CONFIG' in globals() and hasattr(CONFIG, 'enable_sms_alerts') and CONFIG.enable_sms_alerts: # Check if CONFIG is available
