...
def _format_value(price, tick_size) -> Optional[str]:
    if price is None:
        return None
...
def send_batched_notifications(messages, config_notifications, notification_type) -> None:
...
async def main_websocket_listener():
...
async def trading_loop_iteration(price_queue, config, format_price):
