...
readonly MAX_OUTPUT_TOKENS=1000000 # Standard high limit, adjust if model supports more
readonly TEMPERATURE=0.6        # Lower value for more deterministic code output
readonly TOP_P=1.0
readonly TOP_K=1
...
API_RESPONSE=""
ENHANCED_CODE=""
OUTPUT_FILENAME="" # Variable to store user's desired output filename
...
# --- Get Input Filename and Content ---
# Prompts user for input filename, validates, and reads content.
...
get_input_filename_and_content() {
    ...
    if [[ ! -r "$INPUT_FILENAME" ]]; then
        error_exit "Cannot read input file '$INPUT_FILENAME'. Check permissions." 3
    fi
