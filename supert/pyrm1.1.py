...
def close_position(
    ...
    try:
        ...
        send_sms_alert(f"[{market_base}] ERROR Banishing ({reason}): {type(e).__name__}. Check logs.")
    ...
    try:
        ...
        if position["side"] != CONFIG.pos_none:
            ...
            if close_result:
                ...
                send_sms_alert(f"[{market_base}] CRITICAL ERROR: Failed PLACE close order on shutdown. MANUAL CHECK!")
        else:
...
def main() -> None:
    ...
    try:
        ...
        send_sms_alert(f"[{market_base}] Bot Configured ({CONFIG.strategy_name}). SL: ATR+TSL. Starting main loop.")
        ...
        while run_bot:
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                send_sms_alert(f"[{market_base}] ERROR: Exchange unavailable! Long pause.")
            except ccxt.AuthenticationError as e:
                ...
                logger.debug(traceback.format_exc())
                send_sms_alert(f"[{market_base}] ERROR: Unhandled Exchange error: {type(e).__name__}")
