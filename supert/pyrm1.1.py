...
def analyze_volume_atr(df: pd.DataFrame, atr_len: int, vol_ma_len: int) -> Dict[str, Optional[Decimal]]:
    ...
    try:
        ...
        logger.debug(traceback.format_exc())
        results = {key: None for key in results} # Nullify results on error
    return results
...
def place_risked_market_order(exchange: ccxt.Exchange, symbol: str, side: str,
    ...
    try:
        ...
        if avg_fill_price <= 0 or filled_qty <= CONFIG.position_qty_epsilon:
            ...
            send_sms_alert(f"[{market_base}] CRITICAL ORDER FAIL ({side.upper()}): Invalid fill details ...{format_order_id(order_id)}!")
        ...
        if position['side'] != CONFIG.pos_none:
            ...
            if close_result:
                ...
                if final_pos['side'] == CONFIG.pos_none:
                    ...
                    send_sms_alert(f"[{market_base}] CRITICAL ERROR: Failed confirm closure! Final: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK!")
            else:
...
def main() -> None:
    ...
    logger.info(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bybit Scalping Spell v2.1 Initializing ({start_time}) ---{Style.RESET_ALL}")
    ...
    symbol: Optional[str] = None # The specific market symbol (e.g., BTC/USDT:USDT)
