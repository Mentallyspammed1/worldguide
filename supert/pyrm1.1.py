...
# ██║        ██║   ███████║██║ ╚═╝ ██║███████║   ██║   ╚██████╔╝╚██████╔╝███████║
# ╚═╝        ╚═╝   ╚══════╝╚═╝     ╚═╝╚══════╝   ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝
# Pyrmethus - Unified Scalping Spell v2.1 (Enhanced Clarity & Arcane Weaving)
...
Version: 2.1.0 (Unified: Selectable Strategies + Precision + Native SL/TSL + Pyrmethus Enhancements).
...
- Termux Dependency: Requires Termux:API for SMS communication scrolls.
...
class Config:
    ...
    def _get_env(
        ...
        """Fetches env var, casts type, logs, handles defaults/errors with arcane grace."""
        value = os.getenv(key)
        log_value = (
            f"'{value}'"
...
try:
    ...
    # Error already logged within Config init
    sys.exit(1)
...
def send_sms_alert(message: str) -> bool:
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}SMS failed: 'termux-sms-send' command not found. Ensure Termux:API is installed and configured.{Style.RESET_ALL}"
        )
        return False
...
def initialize_exchange() -> ccxt.Exchange | None:
    ...
    try:
        ...
        send_sms_alert("[ScalpBot] Portal opened & authenticated successfully.")
        return exchange
...
def close_position(
    ...
    try:
        ...
        send_sms_alert(
            f"[{market_base}] BANISHED {live_position_side} {amount_str} @ ~{fill_price:.4f} ({reason}). ID:...{order_id_short}"
        )
        ...
        send_sms_alert(
            f"[{market_base}] ERROR Banishing ({reason}): {type(e).__name__}. Check logs."
        )
...
def place_risked_market_order(
    ...
    try:
        ...
        if usdt_free < req_margin_buffered:
            ...
            send_sms_alert(
                f"[{market_base}] ORDER FAIL ({side.upper()}): Insufficient Free Margin (Need ~{req_margin_buffered:.2f})"
            )
            return None
        logger.info(
        ...
        try:
            ...
            if not order_id:
                ...
                send_sms_alert(
                    f"[{market_base}] CRITICAL ORDER FAIL ({side.upper()}): Entry placed but NO ID received!"
                )
        ...
        if avg_fill_price <= 0 or filled_qty <= CONFIG.position_qty_epsilon:
            ...
            send_sms_alert(
                f"[{market_base}] CRITICAL ORDER FAIL ({side.upper()}): Invalid fill details ...{format_order_id(order_id)}!"
            )
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> dict[str, Any]:
    ...
    # Log generated signals for debugging
    if (
        signals["enter_long"]
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if any(strategy_signals.values()):
            logger.debug(
                f"Strategy Signals ({CONFIG.strategy_name}): {strategy_signals}"
            )
        ...
        if position["side"] != CONFIG.pos_none:
            ...
            if close_result:
                ...
                if final_pos["side"] == CONFIG.pos_none:
                    ...
                    send_sms_alert(
                        f"[{market_base}] CRITICAL ERROR: Failed confirm closure! Final: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK!"
                    )
            else:
                ...
                send_sms_alert(
                    f"[{market_base}] CRITICAL ERROR: Failed PLACE close order on shutdown. MANUAL CHECK!"
                )
        else:
            logger.info(
...
def main() -> None:
    ...
    logger.info(
        f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bybit Scalping Spell v2.1 Initializing ({start_time}) ---{Style.RESET_ALL}"
    )
    logger.info(
    ...
    try:
        ...
        send_sms_alert(
            f"[{market_base}] Bot Configured ({CONFIG.strategy_name}). SL: ATR+TSL. Starting main loop."
        )
        ...
        while run_bot:
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                send_sms_alert(
                    f"[{market_base}] ERROR: Exchange unavailable! Long pause."
                )
            except ccxt.AuthenticationError as e:
