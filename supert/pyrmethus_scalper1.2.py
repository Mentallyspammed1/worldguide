...
] ========== Cycle Weaving End: FARTCOIN/USDT:USDT ==========

2025-05-01 19:56:36 [INFO] ========== New Weaving Cycle: FARTCOIN/USDT:USDT (3m) | 2025-05-01 19:56:36 CDT ==========
2025-05-01 19:56:36 [INFO] Data Fetch: Gathering 11 3m candles for FARTCOIN/USDT:USDT...
2025-05-01 19:56:38 [INFO] Position Check: Position size for FARTCOINUSDT is zero/negligible (0). Treating as flat.
2025-05-01 19:56:38 [INFO] State | Position: Side=None, Qty=0.00000000, Entry=0.0000
2025-05-01 19:56:38 [INFO] State | Indicators: Price=1.1952, ATR=0.0043, ST Trend=-1, Confirm ST Trend=-1, VolRatio=0.90
2025-05-01 19:56:38 [INFO] ========== Cycle Weaving End: FARTCOIN/USDT:USDT ==========
#!/usr/bin/env python
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.order_book_fetch_limit: int = max(25, self.order_book_depth)  # Ensure sufficient depth fetched (Bybit V5 default is 50/200)
    ...
    def _get_env(self, var_name: str, default: Any, expected_type: type, required: bool = False, color: str = Fore.WHITE) -> Any:
        ...
        try:
            if expected_type == bool:
                ...
                return Decimal(str(value_to_process)) # Ensure input is string for Decimal
            else:
...
try:
    ...
    try:
        if CONFIG and CONFIG.enable_sms_alerts and CONFIG.sms_recipient_number:
             send_sms_alert("[ScalpBot] CRITICAL: Config validation FAILED. Bot stopped.")
    except NameError: # CONFIG object might not exist if failure was very early
        pass
...
def calculate_supertrend(
    df: pd.DataFrame, length: int, multiplier: float, prefix: str = ""
     ...
     try:
         ...
         cols_to_convert = [st_col_name, st_trend_col, st_long_col, st_short_col]
         ...
         df[target_cols[2]] = (prev_trend_direction == -1) & (df[target_cols[1]] == 1)  # Long flip (Trend changed from Down to Up)
         df[target_cols[3]] = (prev_trend_direction == 1) & (df[target_cols[1]] == -1)  # Short flip (Trend changed from Up to Down)
         # Ensure boolean type and fill NA (especially first row) with False
         df[[target_cols[2], target_cols[3]]] = df[[target_cols[2], target_cols[3]]].fillna(False).astype(bool)

         # Clean up intermediate columns generated by pandas_ta if they exist
         # Use a more robust way to identify potential intermediate columns
         intermediate_cols = [c for c in df.columns if c.startswith("SUPERT_") and c not in target_cols]
         df.drop(columns=list(set(intermediate_cols)), errors='ignore', inplace=True)

         # Log last candle result
         if not df.empty:
             last_trend_val = df[target_cols[1]].iloc[-1] if pd.notna(df[target_cols[1]].iloc[-1]) else None
             last_st_val = df[target_cols[0]].iloc[-1] if pd.notna(df[target_cols[0]].iloc[-1]) else float('nan')
...
def analyze_volume_atr(
     ...
     if df is None or df.empty or not all(c in df.columns for c in required_cols):
         ...
         return results
     min_len = max(atr_len, vol_ma_len, 1) # Need at least 1 row
     if len(df) < min_len:
     ...
     try:
         ...
         if pd.notna(df[volume_ma_col].iloc[-1]) and pd.notna(df['volume'].iloc[-1]):
            ...
            if results[VOLUME_MA_KEY] and results[VOLUME_MA_KEY] > CONFIG.position_qty_epsilon and results[LAST_VOLUME_KEY] is not None:
                try:
         ...
         try:
             ...
             if len(bids) > 0 and len(bids[0]) > 0 and len(asks) > 0 and len(asks[0]) > 0:
                 ...
                 if results[BEST_BID_KEY] > 0 and results[BEST_ASK_KEY] > 0:
                      ...
                      # Determine precision dynamically if possible, else use a reasonable default
                      ...
                      except Exception: pass # Ignore errors getting precision, use default
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
     ...
     try:
         ...
         # Determine category for V5 API call based on market info
         params = {}
         ...
         for pos in positions:
             ...
             position_idx_str = pos_info.get('positionIdx', '-1') # V5 returns as string
             try:
         ...
         if active_position_found:
             ...
             try:
                 ...
                 if abs(size) > CONFIG.position_qty_epsilon:
                     ...
                     entry_price_str = pos_info.get(AVG_PRICE_KEY) or active_position_found.get(ENTRY_PRICE_KEY) # Prefer raw V5 avgPrice
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    for attempt in range(CONFIG.retry_count):
        try:
            ...
            # Note: CCXT's set_leverage for Bybit V5 might implicitly set both buy/sell if not specified in params,
            # but explicitly setting them via params is safer if the behavior changes.
            # However, the unified method should handle this. Let's try without explicit params first,
            # as CCXT aims to abstract this. If it fails, we might need params={'buyLeverage': str(leverage), 'sellLeverage': str(leverage)}
            response = exchange.set_leverage(leverage=leverage, symbol=symbol) # Removed explicit params for now, rely on CCXT abstraction
            ...
            if attempt < CONFIG.retry_count - 1: time.sleep(CONFIG.retry_delay_seconds)
             ...
             logger.debug(traceback.format_exc())
             # Exit immediately on unexpected errors? Or retry? Let's fail fast here.
...
def close_position(
    ...
    try:
        ...
        if "110025" in str(e) or "110043" in str(e) or any(phrase in err_str_lower for phrase in ["order would not reduce position size", "position is zero", "position size is zero", "cannot be less than"]):
             logger.warning(f"{Fore.YELLOW}Banish Position ({reason}): Exchange indicates order would not reduce size or position is zero. Assuming already closed. Error: {e}{Style.RESET_ALL}")
...
def calculate_position_size(
    ...
    exchange: ccxt.Exchange
     ...
     """Calculates position size based on risk, checks limits. Returns floats for CCXT compatibility."""
     ...
     try:
         ...
         def get_limit_decimal(limit_dict: dict, key: str, default: str) -> Decimal:
         ...
         if position_value_usdt > max_cost:
             # If even the capped quantity's value exceeds max cost, it's an issue (though less likely after amount capping)
     ...
     # Return float for CCXT create_order compatibility, using the potentially capped quantity/margin
...
def confirm_order_fill(exchange: ccxt.Exchange, order_id: str, symbol: str) -> Optional[Dict[str, Any]]:
    ...
    if confirmed_order:
        ...
        try:
            filled_qty = Decimal(str(confirmed_order.get(FILLED_KEY, '0')))
            ...
            logger.error(f"{Fore.RED}{log_prefix}: Could not parse filled quantity from confirmed order {order_id}. Treating as FAILED.{Style.RESET_ALL}")
            return None
...
def place_risked_market_order(
    ...
    symbol: str,
    side: str,
    quantity: float, # Float received from calculate_position_size
    required_margin: float, # Float received from calculate_position_size
    stop_loss_price: Decimal,
    ...
    try:
        ...
        # Use 'free' balance for margin check
        ...
        last_price_raw = exchange.fetch_ticker(symbol).get(LAST_PRICE_KEY)
        ...
        # --- Pre-flight Checks ---
        quantity_decimal = Decimal(str(quantity)) # Convert input float to Decimal for checks
        ...
        if estimated_value > CONFIG.max_order_usdt_amount:
            ...
            quantity = float(quantity_str)  # Update float quantity for create_order
            ...
            required_margin = float(required_margin_decimal) # Update float margin estimate
            logger.warning(f"{Fore.YELLOW}{log_prefix}: Qty {original_quantity_str} (Val ~{estimated_value:.2f}) > Max {CONFIG.max_order_usdt_amount:.2f}. Capping to {quantity_str} (New Est. Margin ~{required_margin:.2f}).{Style.RESET_ALL}")
        else:
            # Use original calculated margin if not capped
            required_margin_decimal = Decimal(str(required_margin))
        ...
        entry_order = exchange.create_market_order(symbol, side, quantity) # Use the final float quantity
        ...
        try:
            ...
            actual_avg_price_str = confirmed_order.get(AVERAGE_KEY) or confirmed_order.get(PRICE_KEY) # Fallback to 'price'
        ...
        sl_params = {
            ...
            'tpslMode': 'Partial', # Bybit V5 might need this for partial TP/SL? Check docs. Default is Full. Let's assume Full is okay.
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, timeframe: str) -> None:
    ...
    # Check if columns exist and the last value is not NaN
    ...
    try:
        ...
        st_long_signal = last_candle['st_long']   # True only on the candle where trend flipped to long
        st_short_signal = last_candle['st_short'] # True only on the candle where trend flipped to short
        ...
        if pd.isna(st_long_signal) or pd.isna(st_short_signal):
             raise ValueError("Supertrend flip signals (st_long, st_short) are NaN.")
        ...
        if selected_side:
            ...
            if volume_confirmed and ob_confirmed:
                ...
                try:
                    ...
                    # Let's use standard rounding for simplicity first. Adjust if needed.
                ...
                if quantity_float is not None and margin_est_float is not None:
                    ...
                    # Don't return here immediately, let the cycle finish naturally unless place_risked_market_order handles everything
