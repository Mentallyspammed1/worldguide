...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not determine settle currency for {CONFIG.symbol}, defaulting to {USDT_SYMBOL} for balance check.")
        ...
        logger.debug(f"Using accountType='{account_type}' for balance fetch based on settle currency '{settle_currency}'.")
    ...
    df: pd.DataFrame, length: int, multiplier: float, prefix: str = ""
     ...
     try:
         ...
         if missing_pta_cols:
              ...
              logger.warning(f"{Fore.YELLOW}Scrying ({col_prefix}ST): pandas_ta did not create expected raw columns: {', '.join(missing_pta_cols)}. Calculation might be incomplete.{Style.RESET_ALL}")
         ...
         df.rename(columns={pta_st_col_name: target_st_val_col, pta_st_trend_col: target_st_trend_col}, inplace=True, errors='ignore')
         ...
         if active_position_found:
             ...
             try:
                 ...
                 if abs(size) > CONFIG.position_qty_epsilon:
                     ...
                     if entry_price_str is not None and entry_price_str != "" and entry_price_str != "0": # Also check for "0" string
                         try:
