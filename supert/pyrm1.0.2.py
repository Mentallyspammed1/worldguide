...
class Config:
    ...
    def __init__(self) -> None:
        ...
        )  # Default to 1 (often just the Fisher line itself)
        ...
        )  # Wait time for market order fill
    ...
    def _get_env(
        ...
        if final_value is None and required:
            ...
            logger.critical(
                f"{Back.RED}{Fore.WHITE}CRITICAL: Required configuration rune '{key}' has no value or default.{Style.RESET_ALL}"
            )
            raise ValueError(
                f"Required environment variable '{key}' has no value or default."
            )
        ...
        try:
            ...
            logger.error(
                f"{Fore.RED}Invalid type/value for {key}: '{final_value}' (from env or default). Expected {cast_type.__name__}. Error: {e}. Attempting default '{default}' again.{Style.RESET_ALL}"
            )
            if default is None:
                ...
                try:
                    if cast_type == bool:
                        final_value = str(default).lower() in ["true", "1", "yes", "y"]
                    elif cast_type == Decimal:
                        final_value = Decimal(str(default))
                    elif cast_type == int:
                        final_value = int(str(default))
                    elif cast_type == float:
                        final_value = float(str(default))
                    elif cast_type == str:
                    ...
                    logger.critical(
                        f"{Back.RED}{Fore.WHITE}CRITICAL: Failed to cast BOTH provided value and default value for {key}. Default='{default}', Type={cast_type.__name__}. Error: {e_default}{Style.RESET_ALL}"
                    )
                    raise ValueError(
        ...
        logger.debug(
            f"{color}Using final value for {key}: {final_value} (Type: {type(final_value).__name__}){Style.RESET_ALL}"
        )
        return final_value
...
def format_price(exchange: ccxt.Exchange, symbol: str, price: float | Decimal) -> str:
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}Error shaping price {price} for {symbol}: {e}{Style.RESET_ALL}"
        )
        return str(
            Decimal(str(price)).normalize()
        )  # Fallback to Decimal string representation
...
def format_amount(exchange: ccxt.Exchange, symbol: str, amount: float | Decimal) -> str:
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}Error shaping amount {amount} for {symbol}: {e}{Style.RESET_ALL}"
        )
        return str(
            Decimal(str(amount)).normalize()
        )  # Fallback to Decimal string representation
...
def send_sms_alert(message: str) -> bool:
    ...
    if not CONFIG.sms_recipient_number:
        logger.warning(
            f"{Fore.YELLOW}SMS alerts enabled, but SMS_RECIPIENT_NUMBER rune is missing.{Style.RESET_ALL}"
        )
        return False
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}SMS failed: 'termux-sms-send' vanished unexpectedly?{Style.RESET_ALL}"
        )
        ...
        logger.error(
            f"{Fore.RED}SMS failed: Unexpected disturbance: {e}{Style.RESET_ALL}"
        )
...
def initialize_exchange() -> ccxt.Exchange | None:
    ...
    try:
        ...
        send_sms_alert(
            f"[ScalpBot/{CONFIG.strategy_name}] Portal opened & authenticated."
        )  # Added strategy name
        return exchange
    except ccxt.AuthenticationError as e:
        logger.critical(
            f"{Back.RED}{Fore.WHITE}Authentication failed: {e}. Check keys/IP/permissions.{Style.RESET_ALL}"
        )
        send_sms_alert(
            f"[ScalpBot] CRITICAL: Authentication FAILED: {e}. Spell failed."
        )
    except ccxt.NetworkError as e:
        logger.critical(
            f"{Back.RED}{Fore.WHITE}Network disturbance during portal opening: {e}. Check connection/Bybit status.{Style.RESET_ALL}"
        )
        send_sms_alert(
            f"[ScalpBot] CRITICAL: Network Error on Init: {e}. Spell failed."
        )
    except ccxt.ExchangeError as e:
        logger.critical(
            f"{Back.RED}{Fore.WHITE}Exchange spirit rejected portal opening: {e}. Check Bybit status/API docs.{Style.RESET_ALL}"
        )
        send_sms_alert(
            f"[ScalpBot] CRITICAL: Exchange Error on Init: {e}. Spell failed."
        )
    except Exception as e:
        logger.critical(
        ...
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[ScalpBot] CRITICAL: Unexpected Init Error: {type(e).__name__}. Spell failed."
        )
    return None
...
def calculate_supertrend(
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying ({col_prefix}ST): Insufficient data (Len: {len(df) if df is not None else 0}, Need: {length + 1}).{Style.RESET_ALL}"
        )
    ...
    try:
        ...
        df[f"{col_prefix}st_long"] = (prev_trend == -1) & (
            df[st_trend_col] == 1
        )  # Boolean: Trend flipped to Long
        df[f"{col_prefix}st_short"] = (prev_trend == 1) & (
            df[st_trend_col] == -1
        )  # Boolean: Trend flipped to Short
        ...
        if pd.notna(last_st_val):
            ...
            logger.debug(
                f"Scrying ({col_prefix}ST({length},{multiplier})): Trend={Fore.GREEN if last_trend == 'Up' else Fore.RED}{last_trend}{Style.RESET_ALL}, Val={last_st_val:.4f}, Signal={signal}"
            )
        else:
            logger.debug(
...
def analyze_volume_atr(
    ...
    try:
        ...
        logger.debug(
            f"Scrying Results: ATR({atr_len})={Fore.CYAN}{atr_str}{Style.RESET_ALL}, VolMA({vol_ma_len})={vol_ma_str}, VolRatio={Fore.YELLOW}{vol_ratio_str}{Style.RESET_ALL}"
        )
...
def calculate_stochrsi_momentum(
    ...
    min_len = (
        max(rsi_len + stoch_len, mom_len) + 5
    )  # Add buffer for calculation stability
    if (
        df is None
        or df.empty
    ...
    try:
        ...
        if k_col in stochrsi_df.columns:
            ...
            logger.warning("StochRSI K column not found after calculation")
            df["stochrsi_k"] = pd.NA
        if d_col in stochrsi_df.columns:
            df["stochrsi_d"] = stochrsi_df[d_col].apply(safe_decimal_conversion)
        else:
            logger.warning("StochRSI D column not found after calculation")
            df["stochrsi_d"] = pd.NA
        ...
        if mom_col in df.columns:
            ...
            logger.warning("Momentum column not found after calculation")
            df["momentum"] = pd.NA
        ...
        if pd.notna(k_val) and pd.notna(d_val) and pd.notna(mom_val):
            ...
            mom_color = (
                Fore.GREEN if mom_val > 0 else (Fore.RED if mom_val < 0 else Fore.WHITE)
            )
            logger.debug(
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying (EhlersFisher): Insufficient data (Len: {len(df) if df is not None else 0}, Need {length + 1}).{Style.RESET_ALL}"
        )
        for col in target_cols:
            df[col] = pd.NA
        return df
    try:
        ...
        if fish_col in fisher_df.columns:
            ...
            logger.warning("Ehlers Fisher column not found after calculation")
        ...
        if signal_col in fisher_df.columns:
            ...
            logger.warning("Ehlers Signal column not found after calculation")
...
def calculate_ehlers_ma(df: pd.DataFrame, fast_len: int, slow_len: int) -> pd.DataFrame:
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying (EhlersMA): Insufficient data (Len: {len(df) if df is not None else 0}, Need ~{min_len}).{Style.RESET_ALL}"
        )
        for col in target_cols:
            df[col] = pd.NA
        return df
    try:
        ...
        logger.warning(
            f"{Fore.YELLOW}{Style.DIM}Scrying (EhlersMA): Using EMA as placeholder for Ehlers Super Smoother. Verify suitability.{Style.RESET_ALL}"
        )
        logger.debug(
        ...
        if pd.notna(fast_val) and pd.notna(slow_val):
            logger.debug(
                f"Scrying (EhlersMA({fast_len},{slow_len})): Fast={Fore.GREEN}{fast_val:.4f}{Style.RESET_ALL}, Slow={Fore.RED}{slow_val:.4f}{Style.RESET_ALL}"
            )
        else:
            logger.debug("Scrying (EhlersMA): Resulted in NA for last candle.")
    except Exception as e:
        logger.error(
            f"{Fore.RED}Scrying (EhlersMA): Error during calculation: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        for col in target_cols:
            df[col] = pd.NA  # Nullify results on error
...
def analyze_order_book(
    ...
    if not exchange.has.get("fetchL2OrderBook"):
        logger.warning(
            f"{Fore.YELLOW}Order Book Scrying: fetchL2OrderBook not supported by {exchange.id}. Cannot peer into depth.{Style.RESET_ALL}"
        )
        return results
    try:
        ...
        if (
            ...
            logger.debug(
                f"OB Scrying: Best Bid={Fore.GREEN}{best_bid:.4f}{Style.RESET_ALL}, Ask={Fore.RED}{best_ask:.4f}{Style.RESET_ALL}, Spread={Fore.YELLOW}{results['spread']:.4f}{Style.RESET_ALL}"
            )
        else:
            logger.debug(
                f"OB Scrying: Bid={best_bid or 'N/A'}, Ask={best_ask or 'N/A'} (Spread N/A)"
            )
        ...
        logger.debug(
            f"OB Scrying (Depth {depth}): BidVol={Fore.GREEN}{bid_vol:.4f}{Style.RESET_ALL}, AskVol={Fore.RED}{ask_vol:.4f}{Style.RESET_ALL}"
        )
        ...
        if ask_vol > CONFIG.position_qty_epsilon:
            try:
                ...
                logger.debug(
                    f"OB Scrying Ratio: {ratio_color}{results['bid_ask_ratio']:.3f}{Style.RESET_ALL}"
                )
            except Exception as e:
...
def get_market_data(
    ...
    try:
        ...
        if not ohlcv:
            logger.warning(
                f"{Fore.YELLOW}Data Fetch: No OHLCV data returned for {symbol} ({interval}). Market asleep?{Style.RESET_ALL}"
            )
            return None
        ...
        if df.isnull().values.any():
            ...
            logger.warning(
                f"{Fore.YELLOW}Data Fetch: OHLCV contains gaps (NaNs) after conversion:\n{nan_counts[nan_counts > 0]}\nAttempting forward fill...{Style.RESET_ALL}"
            )
            df.ffill(inplace=True)  # Attempt to fill gaps with previous values
            if df.isnull().values.any():  # Check again
                logger.warning(
                    f"{Fore.YELLOW}Gaps remain after ffill, attempting backward fill...{Style.RESET_ALL}"
                )
                df.bfill(
                    inplace=True
                )  # Attempt to fill remaining gaps with next values
                if df.isnull().values.any():
                    logger.error(
                        f"{Fore.RED}Data Fetch: Gaps persist after ffill/bfill. Cannot proceed with unreliable data.{Style.RESET_ALL}"
                    )
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        if not exchange.has.get("fetchPositions"):
            logger.warning(
                f"{Fore.YELLOW}Position Check: fetchPositions spell not available for {exchange.id}.{Style.RESET_ALL}"
            )
            return default_pos
        ...
        logger.debug(
            f"Position Check: Querying positions for {symbol} (MarketID: {market_id}) with params: {params}"
        )
        ...
        for pos in fetched_positions:
            ...
            if (
                ...
                if (
                    abs(size) > CONFIG.position_qty_epsilon
        ...
        if active_pos:
            try:
                ...
                logger.info(
                    f"{pos_color}Position Check: Found ACTIVE {side} position: Qty={abs(size):.8f} @ Entry={entry_price:.4f}{Style.RESET_ALL}"
                )
            ...
            logger.info(
                f"{Fore.BLUE}Position Check: No active One-Way position found for {market_id}. Currently Flat.{Style.RESET_ALL}"
            )
            return default_pos

    except (ccxt.NetworkError, ccxt.ExchangeError, Exception) as e:
        logger.warning(
            f"{Fore.YELLOW}Position Check: Disturbance querying positions for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    try:
        ...
        if not market.get("contract"):
            logger.error(
                f"{Fore.RED}Leverage Conjuring: Cannot set leverage for non-contract market: {symbol}.{Style.RESET_ALL}"
            )
            return False
    except Exception as e:
        logger.error(
            f"{Fore.RED}Leverage Conjuring: Failed to identify market structure for '{symbol}': {e}{Style.RESET_ALL}"
        )
        return False
    ...
    for attempt in range(CONFIG.retry_count):
        try:
            ...
            logger.success(
                f"{Fore.GREEN}Leverage Conjuring: Set to {leverage}x for {symbol}. Response: {response}{Style.RESET_ALL}"
            )
            return True
        except ccxt.ExchangeError as e:
            ...
            if (
                ...
                logger.info(
                    f"{Fore.CYAN}Leverage Conjuring: Already set to {leverage}x for {symbol}.{Style.RESET_ALL}"
                )
                return True
            logger.warning(
                f"{Fore.YELLOW}Leverage Conjuring: Exchange resistance (Attempt {attempt + 1}/{CONFIG.retry_count}): {e}{Style.RESET_ALL}"
            )
            if attempt < CONFIG.retry_count - 1:
                time.sleep(CONFIG.retry_delay_seconds)
            else:
                logger.error(
                    f"{Fore.RED}Leverage Conjuring: Failed after {CONFIG.retry_count} attempts.{Style.RESET_ALL}"
                )
        except (ccxt.NetworkError, Exception) as e:
            logger.warning(
                f"{Fore.YELLOW}Leverage Conjuring: Network/Other disturbance (Attempt {attempt + 1}/{CONFIG.retry_count}): {e}{Style.RESET_ALL}"
            )
            if attempt < CONFIG.retry_count - 1:
                time.sleep(CONFIG.retry_delay_seconds)
            else:
...
def close_position(
    ...
    if live_position["side"] == CONFIG.pos_none:
        logger.warning(
            f"{Fore.YELLOW}Banish Position: Re-validation shows NO active position for {symbol}. Aborting banishment.{Style.RESET_ALL}"
        )
        if initial_side != CONFIG.pos_none:
            logger.warning(
                f"{Fore.YELLOW}Banish Position: Discrepancy detected (was {initial_side}, now None).{Style.RESET_ALL}"
            )
    ...
    try:
        ...
        if amount_float <= float(CONFIG.position_qty_epsilon):
            logger.error(
                f"{Fore.RED}Banish Position: Closing amount negligible ({amount_str}) after precision shaping. Aborting.{Style.RESET_ALL}"
            )
            return None

        # Execute the market close order with reduceOnly flag
        logger.warning(
            f"{Back.YELLOW}{Fore.BLACK}Banish Position: Attempting to CLOSE {live_position_side} ({reason}): "
        ...
        close_color = (
            Fore.GREEN
            if (
                live_position_side == CONFIG.pos_long
                and side_to_execute_close == CONFIG.side_sell
            )
            or (
                live_position_side == CONFIG.pos_short
                and side_to_execute_close == CONFIG.side_buy
            )
            else Fore.YELLOW
        )  # Should always be green if logic is right

        logger.success(
            f"{close_color}{Style.BRIGHT}Banish Position: Order ({reason}) placed for {symbol}. "
            f"Filled: {filled_qty:.8f}/{amount_str}, AvgFill: {fill_price:.4f}, Cost: {cost:.2f} USDT. ID:...{order_id_short}{Style.RESET_ALL}"
        )
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] BANISHED {live_position_side} {amount_str} @ ~{fill_price:.4f} ({reason}). ID:...{order_id_short}"
        )
        ...
        logger.error(
            f"{Fore.RED}Banish Position ({reason}): Failed for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}"
        )
        ...
        if isinstance(e, ccxt.ExchangeError) and (
            ...
            or "cannot be less than" in err_str
        ):  # Added common size error
            logger.warning(
                f"{Fore.YELLOW}Banish Position: Exchange indicates position already closed/closing or zero size. Assuming banished.{Style.RESET_ALL}"
            )
            return None  # Treat as success (or non-actionable) in this case
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] ERROR Banishing ({reason}): {type(e).__name__}. Check logs."
        )
...
def calculate_position_size(
    ...
    if not (entry_price > 0 and stop_loss_price > 0):
        logger.error(
            f"{Fore.RED}Risk Calc: Invalid entry/SL price (<= 0).{Style.RESET_ALL}"
        )
        return None, None
    price_diff = abs(entry_price - stop_loss_price)
    if price_diff < CONFIG.position_qty_epsilon:
        logger.error(
            f"{Fore.RED}Risk Calc: Entry/SL prices too close ({price_diff:.8f}). Cannot calculate risk.{Style.RESET_ALL}"
        )
        return None, None
    if not 0 < risk_per_trade_pct < 1:
        logger.error(
            f"{Fore.RED}Risk Calc: Invalid risk percentage: {risk_per_trade_pct:.4%}. Must be between 0 and 1.{Style.RESET_ALL}"
        )
        return None, None
    if equity <= 0:
        logger.error(
    ...
    )  # Max USDT amount to risk on this trade
    ...
    try:
        ...
        logger.debug(
            f"Risk Calc: Raw Qty={quantity_raw:.8f}, Precise Qty={quantity_precise:.8f}"
        )
    except Exception as e:
        logger.warning(
            f"{Fore.YELLOW}Risk Calc: Failed precision shaping for quantity {quantity_raw:.8f}. Using raw with fallback quantization. Error: {e}{Style.RESET_ALL}"
        )
    ...
    logger.debug(
        f"Risk Calc Result: Qty={Fore.CYAN}{quantity_precise:.8f}{Style.RESET_ALL}, EstValue={pos_value_usdt:.4f}, EstMargin={required_margin:.4f}"
    )
...
def wait_for_order_fill(
    ...
    logger.info(
        f"{Fore.CYAN}Observing order ...{order_id_short} ({symbol}) for fill (Timeout: {timeout_seconds}s)...{Style.RESET_ALL}"
    )
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            if (
                ...
                logger.error(
                    f"{Fore.RED}Order ...{order_id_short} failed with status '{status}'.{Style.RESET_ALL}"
                )
            ...
            logger.warning(
                f"{Fore.YELLOW}Order ...{order_id_short} not found yet by exchange spirits. Retrying...{Style.RESET_ALL}"
            )
            time.sleep(1)  # Wait a bit longer if not found initially
        except (ccxt.NetworkError, ccxt.ExchangeError, Exception) as e:
            logger.warning(
                f"{Fore.YELLOW}Disturbance checking order ...{order_id_short}: {type(e).__name__} - {e}. Retrying...{Style.RESET_ALL}"
            )
...
def place_risked_market_order(
    ...
    if current_atr is None or current_atr <= Decimal("0"):
        logger.error(
            f"{Fore.RED}Place Order ({side.upper()}): Invalid ATR ({current_atr}). Cannot calculate SL distance.{Style.RESET_ALL}"
        )
        return None
    ...
    try:
        ...
        if usdt_equity <= Decimal("0"):
            logger.error(
                f"{Fore.RED}Place Order ({side.upper()}): Zero or Invalid equity ({usdt_equity:.4f}). Cannot proceed.{Style.RESET_ALL}"
            )
            return None
        if usdt_free < Decimal("0"):  # Free margin shouldn't be negative
            logger.error(
                f"{Fore.RED}Place Order ({side.upper()}): Invalid free margin ({usdt_free:.4f}). Cannot proceed.{Style.RESET_ALL}"
            )
            return None
        logger.debug(
            f"Resources: Equity={usdt_equity:.4f}, Free={usdt_free:.4f} {CONFIG.usdt_symbol}"
        )
        ...
        if side == CONFIG.side_buy and best_ask:
            ...
            try:
                ...
                logger.error(
                    f"{Fore.RED}Failed to fetch ticker price for estimate: {e}{Style.RESET_ALL}"
                )
                return None
        if not entry_price_estimate or entry_price_estimate <= 0:
            logger.error(
                f"{Fore.RED}Invalid entry price estimate ({entry_price_estimate}). Cannot proceed.{Style.RESET_ALL}"
            )
            return None
        ...
        if min_price is not None and initial_sl_price_raw < min_price:
            logger.warning(
                f"{Fore.YELLOW}Initial SL price {initial_sl_price_raw:.4f} below min price {min_price}. Adjusting SL to min price.{Style.RESET_ALL}"
            )
            initial_sl_price_raw = min_price
        if initial_sl_price_raw <= 0:
            logger.error(
                f"{Fore.RED}Invalid Initial SL price calculation resulted in {initial_sl_price_raw:.4f}. Cannot proceed.{Style.RESET_ALL}"
            )
            return None
        ...
        logger.info(
            f"Calculated Initial SL Price (Estimate) ~ {Fore.YELLOW}{initial_sl_price_estimate:.4f}{Style.RESET_ALL} (ATR Dist: {sl_distance:.4f})"
        )
        ...
        if calc_qty is None or req_margin is None:
            logger.error(
                f"{Fore.RED}Failed risk calculation. Cannot determine position size.{Style.RESET_ALL}"
            )
            return None
        ...
        if pos_value_estimate > max_order_cap_usdt:
            logger.warning(
                f"{Fore.YELLOW}Calculated order value {pos_value_estimate:.4f} > Cap {max_order_cap_usdt:.4f}. Capping quantity.{Style.RESET_ALL}"
            )
            ...
            logger.info(
                f"Capped Qty: {final_quantity:.8f}, New Est. Margin: {req_margin:.4f}"
            )

        # === 6. Check Limits & Margin Availability - Final Preparations ===
        if final_quantity <= CONFIG.position_qty_epsilon:
            logger.error(
                f"{Fore.RED}Final Quantity negligible after capping/formatting: {final_quantity:.8f}{Style.RESET_ALL}"
            )
            return None
        # Check against minimum order size
        if min_qty is not None and final_quantity < min_qty:
            logger.error(
                f"{Fore.RED}Final Quantity {final_quantity:.8f} < Min Allowed {min_qty}. Cannot place order.{Style.RESET_ALL}"
            )
            return None
        ...
        if max_qty is not None and final_quantity > max_qty:
            logger.warning(
                f"{Fore.YELLOW}Final Quantity {final_quantity:.8f} > Max Allowed {max_qty}. Capping to max.{Style.RESET_ALL}"
            )
        ...
        req_margin_buffered = (
            final_req_margin * margin_check_buffer
        )  # Add safety buffer
        logger.debug(
            f"Final Margin Check: Need ~{final_req_margin:.4f} (Buffered: {req_margin_buffered:.4f}), Have Free: {usdt_free:.4f}"
        )

        # Check if sufficient free margin is available
        if usdt_free < req_margin_buffered:
            logger.error(
                f"{Fore.RED}Insufficient FREE margin. Need ~{req_margin_buffered:.4f} (incl. buffer), Have {usdt_free:.4f}{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Insufficient Free Margin (Need ~{req_margin_buffered:.2f})"
            )
            return None
        logger.info(
            f"{Fore.GREEN}Final Order Details: Qty={final_quantity:.8f}, EstValue={final_quantity * entry_price_estimate:.4f}, EstMargin={final_req_margin:.4f}. Margin check OK.{Style.RESET_ALL}"
        )
        ...
        try:
            ...
            logger.warning(
                f"{entry_color}{Fore.BLACK}{Style.BRIGHT}*** Placing {side.upper()} MARKET ENTRY: {qty_float:.8f} {symbol} ***{Style.RESET_ALL}"
            )
            ...
            if not order_id:
                ...
                logger.error(
                    f"{Back.RED}{Fore.WHITE}Entry order placed but NO ID returned! Cannot track. Response: {entry_order}{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ORDER FAIL ({side.upper()}): Entry placed but NO ID received!"
                )
                # Attempt to find position manually? Difficult. Best to stop or handle manually.
                return None  # Cannot proceed without order ID
            logger.success(
                f"{Fore.GREEN}Market Entry Order submitted. ID: ...{format_order_id(order_id)}. Awaiting confirmation...{Style.RESET_ALL}"
            )
        except Exception as e:
            logger.error(
                f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}FAILED TO PLACE ENTRY ORDER: {e}{Style.RESET_ALL}"
            )
            logger.debug(traceback.format_exc())
            send_sms_alert(
        ...
        if not filled_entry:
            logger.error(
                f"{Fore.RED}Entry order ...{format_order_id(order_id)} did not fill or failed confirmation.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Entry ...{format_order_id(order_id)} fill timeout/fail."
            )
            ...
            try:
                ...
                logger.warning(
                    f"Could not cancel order ...{format_order_id(order_id)} (may be filled or already gone): {cancel_e}"
                )
        ...
        if avg_fill_price <= 0 or filled_qty <= CONFIG.position_qty_epsilon:
            logger.error(
                f"{Back.RED}{Fore.WHITE}Invalid fill details for ...{format_order_id(order_id)}: Price={avg_fill_price}, Qty={filled_qty}. Position state unknown!{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ORDER FAIL ({side.upper()}): Invalid fill details ...{format_order_id(order_id)}!"
            )
        ...
        logger.success(
            f"{fill_color}{Fore.BLACK}{Style.BRIGHT}ENTRY CONFIRMED: ...{format_order_id(order_id)}. Filled: {filled_qty:.8f} @ {avg_fill_price:.4f}, Cost: {cost:.4f} USDT{Style.RESET_ALL}"
        )
        ...
        if min_price is not None and actual_sl_price_raw < min_price:
            logger.warning(
                f"{Fore.YELLOW}Actual SL price {actual_sl_price_raw:.4f} below min price {min_price}. Adjusting SL to min price.{Style.RESET_ALL}"
            )
            actual_sl_price_raw = min_price
        if actual_sl_price_raw <= 0:
            logger.error(
                f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid ACTUAL SL price ({actual_sl_price_raw:.4f}) calculated based on fill price {avg_fill_price:.4f}. Cannot place SL!{Style.RESET_ALL}"
            )
            # Position is open without SL protection. Attempt emergency close.
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ({side.upper()}): Invalid ACTUAL SL price! Attempting emergency close."
            )
            close_position(
                exchange,
                symbol,
        ...
        try:
            ...
            logger.success(
                f"{Fore.GREEN}Initial Fixed SL ward placed. ID: ...{sl_order_id}, Trigger: {actual_sl_price_str}{Style.RESET_ALL}"
            )
        except Exception as e:
            logger.error(
            ...
            logger.debug(traceback.format_exc())
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ERROR ({side.upper()}): Failed initial SL placement: {type(e).__name__}"
            )
        ...
        try:
            ...
            if min_price is not None and act_price_raw < min_price:
                logger.warning(
                    f"{Fore.YELLOW}TSL activation price {act_price_raw:.4f} below min price {min_price}. Adjusting to min price.{Style.RESET_ALL}"
                )
                act_price_raw = min_price
            if act_price_raw <= 0:
                raise ValueError(f"Invalid TSL activation price {act_price_raw:.4f}")

            tsl_act_price_str = format_price(
                exchange, symbol, act_price_raw
            ...
            logger.success(
                f"{Fore.GREEN}Trailing SL shield placed. ID: ...{tsl_order_id}, Trail%: {tsl_trail_value_str}, ActPx: {tsl_act_price_str}{Style.RESET_ALL}"
            )
        ...
        logger.error(
            f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}Place Order Ritual ({side.upper()}): Overall process failed: {type(e).__name__} - {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Overall process failed: {type(e).__name__}"
        )
...
def cancel_open_orders(
    ...
    logger.info(
        f"{Fore.CYAN}Order Cleanup: Attempting for {symbol} (Reason: {reason})...{Style.RESET_ALL}"
    )
    try:
        if not exchange.has.get("fetchOpenOrders"):
            logger.warning(
                f"{Fore.YELLOW}Order Cleanup: fetchOpenOrders spell not available.{Style.RESET_ALL}"
            )
            return
        ...
        logger.warning(
            f"{Fore.YELLOW}Order Cleanup: Found {len(open_orders)} open orders for {symbol}. Cancelling...{Style.RESET_ALL}"
        )
        ...
        for order in open_orders:
            ...
            if order_id:
                try:
                    ...
                    exchange.cancel_order(order_id, symbol)
                    logger.info(
                        f"{Fore.CYAN}Order Cleanup: Success for {order_info}{Style.RESET_ALL}"
                    )
                    cancelled_count += 1
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}Order Cleanup: Not found (already closed/cancelled?): {order_info}{Style.RESET_ALL}"
                    )
                    cancelled_count += 1  # Treat as cancelled if not found
                except (ccxt.NetworkError, ccxt.ExchangeError, Exception) as e:
                    logger.error(
                        f"{Fore.RED}Order Cleanup: FAILED for {order_info}: {type(e).__name__} - {e}{Style.RESET_ALL}"
                    )
                    failed_count += 1
            else:
                logger.error(
                    f"{Fore.RED}Order Cleanup: Found order with no ID: {order}. Cannot cancel.{Style.RESET_ALL}"
                )
                failed_count += 1

        logger.info(
            f"{Fore.CYAN}Order Cleanup: Finished. Cancelled: {cancelled_count}, Failed: {failed_count}.{Style.RESET_ALL}"
        )
        if failed_count > 0:
            send_sms_alert(
                f"[{symbol.split('/')[0]}/{CONFIG.strategy_name}] WARNING: Failed to cancel {failed_count} orders during {reason}."
            )
    except (ccxt.NetworkError, ccxt.ExchangeError, Exception) as e:
        logger.error(
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}Signal Generation Error: Missing expected indicator column in DataFrame: {e}. Strategy: {strategy_name}{Style.RESET_ALL}"
        )
    except Exception as e:
        logger.error(
            f"{Fore.RED}Signal Generation Error: Unexpected disturbance: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    required_rows = (
        max(
            ...
            CONFIG.momentum_length,  # Estimate lookback needed
    ...
    if df is None or len(df) < required_rows:
        logger.warning(
            f"{Fore.YELLOW}Trade Logic: Insufficient data ({len(df) if df is not None else 0}, need ~{required_rows}). Skipping cycle.{Style.RESET_ALL}"
        )
        return
    ...
    try:
        ...
        if pd.isna(current_price) or current_price <= 0:
            logger.warning(
                f"{Fore.YELLOW}Last candle close price is invalid ({current_price}). Skipping cycle.{Style.RESET_ALL}"
            )
            return
        ...
        if not can_place_order:
            logger.warning(
                f"{Fore.YELLOW}Invalid ATR ({current_atr}). Cannot calculate SL or place new orders this cycle.{Style.RESET_ALL}"
            )
        ...
        logger.info(
            f"State | Volume: Ratio={Fore.YELLOW}{vol_ratio_str}{Style.RESET_ALL}, Spike={vol_spike_str} (Req={CONFIG.require_volume_spike_for_entry})"
        )
        ob_ratio_str = f"{bid_ask_ratio:.3f}" if bid_ask_ratio else "N/A"
        ob_spread_str = f"{spread:.4f}" if spread else "N/A"
        logger.info(
            f"State | OrderBook: Ratio={Fore.YELLOW}{ob_ratio_str}{Style.RESET_ALL}, Spread={ob_spread_str} (Fetched={ob_data is not None})"
        )
        # Log position state
        pos_color = (
            Fore.GREEN
        ...
        if position_side != CONFIG.pos_none:
            logger.info(
                f"Holding {pos_color}{position_side}{Style.RESET_ALL} position. Awaiting SL/TSL or Strategy Exit signal."
            )
            return  # Do nothing if already in a position
        if not can_place_order:
            logger.warning(
                f"{Fore.YELLOW}Holding Cash. Cannot enter: Invalid ATR ({current_atr}) prevents SL calculation.{Style.RESET_ALL}"
            )
        ...
        if ob_check_required and ob_data is None:
            logger.debug(
                "Potential entry signal and OB not fetched yet, fetching OB for confirmation..."
            )
            ob_data = analyze_order_book(
                exchange, symbol, CONFIG.order_book_depth, CONFIG.order_book_fetch_limit
            )
        ...
        if (
            strategy_signals["enter_long"] or strategy_signals["enter_short"]
        ):  # Only log if there was a base signal
            logger.debug(
                f"Final Entry Check (Long): Strategy={strategy_signals['enter_long']}, {ob_log}, {vol_log} => {Fore.GREEN if enter_long else Fore.RED}Enter={enter_long}{Style.RESET_ALL}"
            )
            logger.debug(
                f"Final Entry Check (Short): Strategy={strategy_signals['enter_short']}, {ob_log}, {vol_log} => {Fore.GREEN if enter_short else Fore.RED}Enter={enter_short}{Style.RESET_ALL}"
            )
        ...
        if enter_long:
            ...
            logger.success(
                f"{entry_side_color}{Fore.BLACK}{Style.BRIGHT}*** TRADE SIGNAL: CONFIRMED LONG ENTRY ({CONFIG.strategy_name}) for {symbol} ***{Style.RESET_ALL}"
            )
            ...
            logger.success(
                f"{entry_side_color}{Fore.WHITE}{Style.BRIGHT}*** TRADE SIGNAL: CONFIRMED SHORT ENTRY ({CONFIG.strategy_name}) for {symbol} ***{Style.RESET_ALL}"
            )
            # Cancel any stray orders before entering
            ...
            if potential_entry and not action_taken_this_cycle:
                logger.info(
                    "Strategy signal present but confirmation filters not met. Holding cash."
                )
        ...
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[{symbol.split('/')[0]}/{CONFIG.strategy_name}] CRITICAL ERROR in trade_logic: {type(e).__name__}. Check logs!"
        )
    finally:
...
def graceful_shutdown(exchange: ccxt.Exchange | None, symbol: str | None) -> None:
    ...
    logger.warning(
        f"{Fore.YELLOW}{Style.BRIGHT}Shutdown requested. Withdrawing arcane energies gracefully...{Style.RESET_ALL}"
    )
    ...
    if not exchange or not symbol:
        logger.warning(
            f"{Fore.YELLOW}Shutdown: Exchange portal or symbol not defined. Cannot perform cleanup.{Style.RESET_ALL}"
        )
        return
    ...
    try:
        ...
        if position["side"] != CONFIG.pos_none:
            ...
            logger.warning(
                f"{Fore.YELLOW}Shutdown: Active {pos_color}{position['side']}{Style.RESET_ALL} position found (Qty: {position['qty']:.8f}). Attempting banishment...{Style.RESET_ALL}"
            )
            ...
            if close_result:
                ...
                if final_pos["side"] == CONFIG.pos_none:
                    logger.success(
                        f"{Fore.GREEN}{Style.BRIGHT}Shutdown: Position confirmed BANISHED.{Style.RESET_ALL}"
                    )
                    send_sms_alert(
                    ...
                    logger.error(
                        f"{Back.RED}{Fore.WHITE}Shutdown: FAILED TO CONFIRM position closure. Final state: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK REQUIRED!{Style.RESET_ALL}"
                    )
                    send_sms_alert(
                        f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ERROR: Failed confirm closure! Final: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK!"
                    )
            else:
                # Close order placement failed
                logger.error(
                    f"{Back.RED}{Fore.WHITE}Shutdown: Failed to place close order. MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ERROR: Failed PLACE close order on shutdown. MANUAL CHECK!"
                )
        else:
            logger.info(
                f"{Fore.GREEN}Shutdown: No active position found. Clean exit.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] No active position found on shutdown."
            )
        ...
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] Error during shutdown cleanup: {type(e).__name__}"
        )

    logger.info(
        f"{Fore.YELLOW}{Style.BRIGHT}--- Scalping Spell Shutdown Complete ---{Style.RESET_ALL}"
    )
...
def main() -> None:
    ...
    logger.info(
        f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bybit Scalping Spell v2.2 Initializing ({start_time}) ---{Style.RESET_ALL}"
    )
    logger.info(
        f"{Fore.CYAN}--- Strategy Enchantment Selected: {CONFIG.strategy_name} ---{Style.RESET_ALL}"
    )
    logger.info(
        f"{Fore.GREEN}--- Protective Wards: Initial ATR-Stop + Exchange Trailing Stop ---{Style.RESET_ALL}"
    )
    logger.warning(
    ...
    try:
        ...
        try:
            ...
            sym_input = input(
                f"{Fore.YELLOW}Enter target symbol {Style.DIM}(Default [{CONFIG.symbol}]){Style.NORMAL}: {Style.RESET_ALL}"
            ).strip()
            ...
            logger.info(
                f"{Fore.GREEN}Focusing spell on Symbol: {symbol} (Type: {market.get('type')}, Linear: {market.get('linear')}){Style.RESET_ALL}"
            )
            ...
            send_sms_alert(
                f"[ScalpBot/{CONFIG.strategy_name}] CRITICAL: Symbol/Leverage setup FAILED ({e}). Exiting."
            )
            return
        except Exception as e:
            logger.critical(
                f"{Back.RED}{Fore.WHITE}Unexpected error during spell focus setup: {e}{Style.RESET_ALL}"
            )
            logger.debug(traceback.format_exc())
            send_sms_alert(
                f"[ScalpBot/{CONFIG.strategy_name}] CRITICAL: Unexpected setup error. Exiting."
            )
            return
        ...
        if CONFIG.strategy_name == "DUAL_SUPERTREND":
            ...
            logger.info(
                f"  Params: FastMA(EMA)={CONFIG.ehlers_fast_period}, SlowMA(EMA)={CONFIG.ehlers_slow_period}"
            )
        logger.info(
        ...
        logger.info(
            f"{Fore.YELLOW}Order Book Filter: {CONFIG.fetch_order_book_per_cycle} (Depth={CONFIG.order_book_depth}, L>={CONFIG.order_book_ratio_threshold_long}, S<={CONFIG.order_book_ratio_threshold_short})"
        )
        # *** THE FIX IS HERE: Ensure CONFIG.required_margin_buffer is Decimal before formatting ***
        logger.info(
            f"{Fore.WHITE}Timing: Sleep={CONFIG.sleep_seconds}s, Margin Buffer={CONFIG.required_margin_buffer:.1%}, SMS Alerts={CONFIG.enable_sms_alerts}{Style.RESET_ALL}"
        )
        logger.info(
        ...
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] Bot Configured. SL: ATR+TSL. Starting main loop."
        )
        ...
        while run_bot:
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    )  # Pass copy to avoid modifying original df in logic
                else:
                    logger.warning(
                        f"{Fore.YELLOW}No valid market data received for {symbol}. Skipping trade logic this cycle.{Style.RESET_ALL}"
                    )
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] WARNING: Rate limit hit! Pausing."
                )
            except ccxt.NetworkError as e:
                # Transient network issues, retry next cycle
                logger.warning(
                    f"{Fore.YELLOW}Network disturbance: {e}. Retrying next cycle.{Style.RESET_ALL}"
                )
                time.sleep(
                ...
                logger.error(
                    f"{Back.RED}{Fore.WHITE}Exchange unavailable: {e}. Portal temporarily closed. Sleeping much longer...{Style.RESET_ALL}"
                )
                time.sleep(CONFIG.sleep_seconds * 10)  # Wait a significant time
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] ERROR: Exchange unavailable! Long pause."
                )
            except ccxt.AuthenticationError as e:
                # API keys might have been revoked or expired
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}Authentication Error: {e}. Spell broken! Stopping NOW.{Style.RESET_ALL}"
                )
                run_bot = False  # Stop the bot immediately
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL: Authentication Error! Stopping NOW."
                )
                ...
                logger.exception(
                    f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}!!! UNEXPECTED CRITICAL CHAOS: {e} !!! Stopping spell!{Style.RESET_ALL}"
                )
                run_bot = False  # Stop the bot on unknown critical errors
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL UNEXPECTED ERROR: {type(e).__name__}! Stopping NOW."
                )
            ...
            if run_bot:
                ...
                logger.debug(
                    f"Cycle {cycle_count} duration: {elapsed:.2f}s. Sleeping for {sleep_dur:.2f}s."
                )
                if sleep_dur > 0:
