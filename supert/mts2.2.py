...
load_dotenv()
getcontext().prec = (
    18  # Set Union[Decimal, pd.NAType, None]precision for all calculations
)
...
class Config:
    ...
    def __init__(self) -> Union[Decimal, pd.NAType, None]:
        ...
        self.required_margin_buffer: Union[Decimal, pd.NAType, None] = self._get_env(
            ...
            "1.05",
            cast_type=Decimal,
            color=Fore.GREEN,  # e.g., 5% buffer
        )
        self.max_account_margin_ratio: Union[Decimal, pd.NAType, None] = (
        ...
        self.trailing_stop_activation_offset_percent: Union[
            Decimal, pd.NAType, None
        ] = self._get_env(
        ...
        self.ehlers_ssf_poles: int = self._get_env(
            ...
            2,
            cast_type=int,
            color=Fore.CYAN,  # 2 or 3 poles typical
        )
        ...
        self.order_book_depth: int = self._get_env(
            ...
            20,
            cast_type=int,
            color=Fore.YELLOW,  # Number of levels to sum volume from
        )
        self.order_book_ratio_threshold_long: Union[Decimal, pd.NAType, None] = (
            self._get_env(
        ...
        self.order_book_ratio_threshold_short: Union[Decimal, pd.NAType, None] = (
            self._get_env(
        ...
        self.sms_recipient_number: str | Union[Decimal, pd.NAType, None] = (
            self._get_env(
                "SMS_RECIPIENT_NUMBER",
                Union[Decimal, pd.NAType, None],
                cast_type=str,
                color=Fore.MAGENTA,  # Optional
            )
        )
        ...
        self.default_recv_window: int = (
            13000  # Bybit max is 20000, but default 5000 can be too short
        )
        ...
        self.api_fetch_limit_buffer: int = (
            10  # Extra candles to fetch beyond min requirement
        )
        self.position_qty_epsilon: Union[Decimal, pd.NAType, None] = Decimal(
            "1e-9"
        )  # For float comparisons with position sizes
        self.post_close_delay_seconds: int = (
            3  # Wait after closing a position before next action
        )
        self.cache_candle_duration_multiplier: Union[Decimal, pd.NAType, None] = (
            Decimal("0.95")
    ...
    def _get_env(
        ...
        ):  # Catches if default wasUnion[Decimal, pd.NAType, None] and env var not set
            if required:
                _pre_logger.critical(
        ...
        try:
            ...
            if (
                default is None
            ):  # If default is alsoUnion[Decimal, pd.NAType, None], and casting failed
                if required:
                    _pre_logger.critical(
                    ...
                    returnUnion[
                        Decimal, pd.NAType, None
                    ]  # ReturnUnion[Decimal, pd.NAType, None] if default is None and casting failed
            else:
...
class TradingStrategy(ABC):
    def __init__(
        self,
        config: Config,
        df_columns: list[str] | Union[Decimal, pd.NAType, None] = Union[
            Decimal, pd.NAType, None
        ],
    ):
        self.config = config
    ...
    def _validate_df(self, df: pd.DataFrame, min_rows: int = 2) -> bool:
        if df is None or df.empty or len(df) < min_rows:
            self.logger.debug(
                f"Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need: {min_rows})."
            )
            return False
...
class DualSupertrendStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        primary_long_flip = last.get(
            "st_long_flip", False
        )  # From calculate_supertrend with no prefix
        primary_short_flip = last.get(
            "st_short_flip", False
        )  # From calculate_supertrend with no prefix
        confirm_is_up = last.get(
            "confirm_trend", pd.NA
        )  # From calculate_supertrend with "confirm_" prefix
...
class StochRsiMomentumStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if (
            k_now_dec.is_nan()
            or mom_now_dec.is_nan()
            or k_prev_dec.is_nan()
            or d_prev_dec.is_nan()
            or d_now_dec.is_nan()
        ):
            self.logger.debug(
                "Skipping due to NA StochRSI/Mom Union[Decimal, pd.NAType, None]values after conversion."
            )
            return signals
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if any(v.is_nan() for v in [fish_now, sig_now, fish_prev, sig_prev]):
            self.logger.debug(
                "Skipping due to NA Ehlers Fisher Union[Decimal, pd.NAType, None]values after conversion."
            )
            return signals
...
class EhlersMaCrossStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if any(
            ...
            self.logger.debug(
                "Skipping due to NA Ehlers SSF MA Union[Decimal, pd.NAType, None]values after conversion."
            )
            return signals
...
_active_trade_details: dict[str, Any | Union[Decimal, pd.NAType, None]] = {
    "entry_price": Union[Decimal, pd.NAType, None],
    "entry_time_ms": Union[Decimal, pd.NAType, None],
    "side": Union[Decimal, pd.NAType, None],
    "qty": Union[Decimal, pd.NAType, None],
}
...
def safe_decimal_conversion(
    ...
    ):  # pd.isna handlesUnion[Decimal, pd.NAType, None], np.nan, pd.NaT, pd.NA
        return default
    try:
...
_termux_sms_command_exists: bool | Union[Decimal, pd.NAType, None] = Union[
    Decimal, pd.NAType, None
]
...
def send_sms_alert(message: str) -> bool:
    ...
    if _termux_sms_command_exists is None:  # Check only once
        _termux_sms_command_exists = (
            shutil.which("termux-sms-send") is notUnion[Decimal, pd.NAType, None]
        )
...
def initialize_exchange() -> ccxt.Exchange | Union[Decimal, pd.NAType, None]:
    ...
    try:
        ...
        )  # Not using safe_api_call here, fail fast on init
...
def calculate_supertrend(
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying ({col_prefix}ST): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed_for_pta}). Populating with NAs.{Style.RESET_ALL}"
        )
        if (
            df is notUnion[Decimal, pd.NAType, None]
        ):  # Ensure df exists to add NA columns
        ...
        )  # Return empty DF if input wasUnion[Decimal, pd.NAType, None]
    ...
    try:
        ...
        if not all(c in temp_df.columns for c in pta_cols_to_check):
            ...
            for col in target_cols:
                df[col] = Union[Decimal, pd.NAType, None]  # Assign to original df
            return df
...
def analyze_volume_atr(
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying (Vol/ATR): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}).{Style.RESET_ALL}"
        )
        return results
    ...
    try:
        ...
        if not temp_df.empty:
            ...
            if (
                not pd.isna(vol_ma)
                and not pd.isna(last_vol)
                and vol_ma is notUnion[Decimal, pd.NAType, None]
                and last_vol is notUnion[Decimal, pd.NAType, None]
                and vol_ma > CONFIG.position_qty_epsilon
            ):
                try:
...
def calculate_stochrsi_momentum(
    ...
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(
            f"{Fore.YELLOW}Scrying (StochRSI/Mom): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}"
        )
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols:
                df[col] = pd.NA
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying (EhlersFisher): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}"
        )
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols:
                df[col] = pd.NA
...
def calculate_ehlers_ma(
    ...
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(
            f"{Fore.YELLOW}Scrying (Ehlers SSF MA): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}"
        )
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols:
                df[col] = pd.NA
    ...
    try:
        ...
        df["ehlers_ssf_fast"] = (
            ...
            if ssf_fast_series is notUnion[Decimal, pd.NAType, None]
            and not ssf_fast_series.empty
            else pd.NA
        )
        ...
        df["ehlers_ssf_slow"] = (
            ...
            if ssf_slow_series is notUnion[Decimal, pd.NAType, None]
            and not ssf_slow_series.empty
            else pd.NA
        )
...
def analyze_order_book(
    ...
    try:
        ...
        if (
            ...
            and best_bid_val is notUnion[Decimal, pd.NAType, None]
            and best_ask_val is notUnion[Decimal, pd.NAType, None]
            and best_bid_val > 0
            and best_ask_val > 0
        ):
...
_last_market_data: pd.DataFrame | Union[Decimal, pd.NAType, None] = Union[
    Decimal, pd.NAType, None
]
...
def get_market_data(
...
) -> pd.DataFrame | Union[Decimal, pd.NAType, None]:
    ...
    if (
        _last_market_data is notUnion[Decimal, pd.NAType, None]
    ...
    try:
        ...
        if total_equity <= Decimal("0"):
            ...
            if used_margin > Decimal("0"):
                ...
                send_sms_alert(
                    f"[{config.symbol.split('/')[0].split(':')[0]}/{config.strategy_name.value}] CRITICAL: Zero/Neg Equity with Used Margin. BOT PAUSED."
                )
                return False
            return True
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    try:
        ...
        ):  # Bybit V5 "Set leverage not modified" code
            logger.info(
...
def close_position(
    ...
    reason: str = "Signal",
) -> dict[str, Any] | Union[Decimal, pd.NAType, None]:
    ...
    if (
        ...
        if (
            _active_trade_details.get("entry_price")
            is notUnion[Decimal, pd.NAType, None]
        ):
            logger.info(
    ...
    try:
        ...
        if (
            ...
            if (
                _active_trade_details.get("entry_price")
                is notUnion[Decimal, pd.NAType, None]
                and order_timestamp_ms is notUnion[Decimal, pd.NAType, None]
            ):
                if (
                    _active_trade_details.get("side")
                    and _active_trade_details.get("qty")
                    and _active_trade_details.get("entry_time_ms")
                ):
...
def calculate_position_size(
    ...
    Union[Decimal, pd.NAType, None] | Union[Decimal, pd.NAType, None],
    Union[Decimal, pd.NAType, None] | Union[Decimal, pd.NAType, None],
]:
...
def place_risked_market_order(
    ...
    current_atr: Union[Decimal, pd.NAType, None]
    | Union[Decimal, pd.NAType, None]
    | Union[Decimal, pd.NAType, None],
    ...
    try:
        ...
        if (
            not pd.isna(entry_px_est_orig)
            and entry_px_est_orig is notUnion[Decimal, pd.NAType, None]
            and entry_px_est_orig > 0
        ):
        ...
        avg_fill_px = safe_decimal_conversion(
            filled_entry_order.get("average"), Decimal("0")
        )  # Default to 0 to catch issues
        filled_qty_val = safe_decimal_conversion(
            filled_entry_order.get("filled"), Decimal("0")
        )
...
def cancel_open_orders(
    ...
    )  # Default to empty dict if not linear
    params_fetch = {"category": v5_category} if v5_category else {}

    try:
        open_orders = safe_api_call(
            exchange.fetch_open_orders, symbol, params=params_fetch
        )

        if not open_orders:
            logger.info(
                f"{Fore.CYAN}Order Cleanup: No open orders found for {symbol} (Category: {v5_category or 'N/A'}).{Style.RESET_ALL}"
            )
            return 0
        ...
        for order in open_orders:
            ...
            if order_id:
                try:
                    ...
                    safe_api_call(
                        exchange.cancel_order, order_id, symbol, params=params_cancel
                    )
                    cancelled_count += 1
                    logger.debug(
        ...
        if failed_count > 0:
            send_sms_alert(
                f"[{symbol.split('/')[0].split(':')[0]}/{CONFIG.strategy_name.value}] WARNING: Failed to cancel {failed_count} orders during cleanup ({reason}). Manual check recommended."
            )
...
def trade_logic(
    ...
    cycle_time_str = (
        ...
        if not df.empty and isinstance(df.index[-1], pd.Timestamp)
        else "N/A"
    )
    logger.info(
    ...
    if df is None or len(df) < required_rows_for_any_indicator:
        logger.warning(
            f"{Fore.YELLOW}Trade Logic: Insufficient data for {symbol} ({len(df) if df is notUnion[Decimal, pd.NAType, None] else 0} rows, need ~{required_rows_for_any_indicator}). Skipping logic cycle.{Style.RESET_ALL}"
        )
        return
    ...
    try:
        ...
        if (
            not pd.isna(current_atr_orig)
            and current_atr_orig is notUnion[Decimal, pd.NAType, None]
        ):
        ...
        if (
            not pd.isna(current_price_orig)
            and current_price_orig is notUnion[Decimal, pd.NAType, None]
        ):
        ...
        if pd.isna(current_price) or current_price is None or current_price <= 0:
            logger.warning(
                f"{Fore.YELLOW}Trade Logic: Invalid or missing last close price for {symbol}. Last candle data: {last_candle_data.to_dict() if not last_candle_data.empty else 'empty'}{Style.RESET_ALL}"
            )
            return

        can_place_order_with_atr_sl = (
            not pd.isna(current_atr)
            and current_atr is notUnion[Decimal, pd.NAType, None]
            and current_atr > 0
        )
        ...
        if (
            ...
            and volume_ratio_val is notUnion[Decimal, pd.NAType, None]
        ):
        ...
        if (
            ob_data
            and isinstance(ob_data, dict)
            and not pd.isna(ob_data.get("bid_ask_ratio"))
            and ob_data.get("bid_ask_ratio") is notUnion[Decimal, pd.NAType, None]
        ):
            ratio = ob_data["bid_ask_ratio"]
            if isinstance(
                ratio, Decimal
            ):  # Ensure ratio is Union[Decimal, pd.NAType, None]for comparison
            ...
            logger.warning(
                f"{Fore.YELLOW}Order book filter active, but no valid OB data or ratio. Entry blocked by OB filter.{Style.RESET_ALL}"
            )
...
def graceful_shutdown(
    exchange: ccxt.Exchange | Union[Decimal, pd.NAType, None],
    symbol: str | Union[Decimal, pd.NAType, None],
) -> Union[Decimal, pd.NAType, None]:
    logger.warning(
    ...
    if "trade_metrics" in globals() and hasattr(
        trade_metrics, "summary"
    ):  # Check trade_metrics exists
...
def main() -> Union[Decimal, pd.NAType, None]:
    ...
    exchange: ccxt.Exchange | Union[Decimal, pd.NAType, None] = Union[
        Decimal, pd.NAType, None
    ]
    symbol_unified: str | Union[Decimal, pd.NAType, None] = Union[
        Decimal, pd.NAType, None
    ]
    run_bot: bool = True
    cycle_count: int = 0
    ...
    try:
        ...
        while run_bot:
            ...
            try:
                ...
                if (
                    df_market_data is notUnion[Decimal, pd.NAType, None]
                    and not df_market_data.empty
                ):
