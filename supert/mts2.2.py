...
- Enhanced Precision: Uses Union[Decimal, pd.NAType, None]for critical financial calculations.
...
load_dotenv()
getcontext().prec = 18 # Set Union[Decimal, pd.NAType, None]precision for all calculations
...
class Config:
    ...
    def __init__(self) ->Union[Decimal, pd.NAType, None]:
        ...
        self.api_key: str |Union[Decimal, pd.NAType, None] = self._get_env(
            "BYBIT_API_KEY", required=True, color=Fore.RED
        )
        self.api_secret: str |Union[Decimal, pd.NAType, None] = self._get_env(
        ...
        self.risk_per_trade_percentage: Union[Decimal, pd.NAType, None]= self._get_env(
            "RISK_PER_TRADE_PERCENTAGE", "0.01", cast_type=Decimal, color=Fore.GREEN # 1% risk
        )
        self.atr_stop_loss_multiplier: Union[Decimal, pd.NAType, None]= self._get_env(
            "ATR_STOP_LOSS_MULTIPLIER", "1.3", cast_type=Decimal, color=Fore.GREEN
        )
        self.max_order_usdt_amount: Union[Decimal, pd.NAType, None]= self._get_env(
            "MAX_ORDER_USDT_AMOUNT", "50.0", cast_type=Decimal, color=Fore.GREEN # Max position size in USDT
        )
        self.required_margin_buffer: Union[Decimal, pd.NAType, None]= self._get_env(
            "REQUIRED_MARGIN_BUFFER", "1.05", cast_type=Decimal, color=Fore.GREEN # e.g., 5% buffer
        )
        self.max_account_margin_ratio: Union[Decimal, pd.NAType, None]= self._get_env( # For check_account_health
        ...
        self.trailing_stop_percentage: Union[Decimal, pd.NAType, None]= self._get_env(
            "TRAILING_STOP_PERCENTAGE", "0.005", cast_type=Decimal, color=Fore.GREEN # e.g., 0.5%
        )
        self.trailing_stop_activation_offset_percent: Union[Decimal, pd.NAType, None]= self._get_env(
        ...
        self.st_multiplier: Union[Decimal, pd.NAType, None]= self._get_env(
        ...
        self.confirm_st_multiplier: Union[Decimal, pd.NAType, None]= self._get_env(
        ...
        self.stochrsi_overbought: Union[Decimal, pd.NAType, None]= self._get_env(
            "STOCHRSI_OVERBOUGHT", "80.0", cast_type=Decimal, color=Fore.CYAN
        )
        self.stochrsi_oversold: Union[Decimal, pd.NAType, None]= self._get_env(
        ...
        self.ehlers_ssf_poles: int = self._get_env(
            "EHLERS_SSF_POLES", 2, cast_type=int, color=Fore.CYAN # 2 or 3 poles typical
        )
        ...
        self.volume_spike_threshold: Union[Decimal, pd.NAType, None]= self._get_env(
            "VOLUME_SPIKE_THRESHOLD", "1.5", cast_type=Decimal, color=Fore.YELLOW # e.g., 1.5x MA
        )
        ...
        self.order_book_depth: int = self._get_env(
            "ORDER_BOOK_DEPTH", 20, cast_type=int, color=Fore.YELLOW # Number of levels to sum volume from
        )
        self.order_book_ratio_threshold_long: Union[Decimal, pd.NAType, None]= self._get_env(
            "ORDER_BOOK_RATIO_THRESHOLD_LONG",
            "1.2", # Bid volume must be >= 1.2 * Ask volume
            cast_type=Decimal,
            color=Fore.YELLOW,
        )
        self.order_book_ratio_threshold_short: Union[Decimal, pd.NAType, None]= self._get_env(
            "ORDER_BOOK_RATIO_THRESHOLD_SHORT",
            "0.8", # Bid volume must be <= 0.8 * Ask volume (or Ask >= 1/0.8 * Bid)
            cast_type=Decimal,
            color=Fore.YELLOW,
        )
        ...
        self.sms_recipient_number: str |Union[Decimal, pd.NAType, None] = self._get_env(
            "SMS_RECIPIENT_NUMBER",Union[Decimal, pd.NAType, None], cast_type=str, color=Fore.MAGENTA # Optional
        )
        ...
        self.default_recv_window: int = 13000 # Bybit max is 20000, but default 5000 can be too short
        self.order_book_fetch_limit: int = max(25, self.order_book_depth) # How many levels to fetch from API for OB analysis
        self.shallow_ob_fetch_depth: int = 5 # For quick price estimation if needed
        ...
        self.api_fetch_limit_buffer: int = 10 # Extra candles to fetch beyond min requirement
        self.position_qty_epsilon: Union[Decimal, pd.NAType, None]= Decimal("1e-9") # For float comparisons with position sizes
        self.post_close_delay_seconds: int = 3 # Wait after closing a position before next action
        self.cache_candle_duration_multiplier: Union[Decimal, pd.NAType, None]= Decimal("0.95") # For data cache validity, e.g., 95% of candle duration
    ...
    def _get_env(
        self,
        key: str,
        default: Any =Union[Decimal, pd.NAType, None],
        cast_type: type = str,
        required: bool = False,
        ...
        source = "Env Var"
        value_to_cast: Any =Union[Decimal, pd.NAType, None]
        ...
        if value_to_cast is None: # Catches if default wasUnion[Decimal, pd.NAType, None] and env var not set
            if required:
                _pre_logger.critical(
                    f"{Back.RED}{Fore.WHITE}CRITICAL: Required config '{key}' has no value (env/default).{Style.RESET_ALL}"
                )
                raise ValueError(
                    f"Required environment variable '{key}' resolved toUnion[Decimal, pd.NAType, None]."
                )
            else:
                # This debug log is important for optionalUnion[Decimal, pd.NAType, None] values like SMS_RECIPIENT_NUMBER
                _pre_logger.debug(
                    f"{color}Final value for {key}:Union[Decimal, pd.NAType, None] (Type:Union[Decimal, pd.NAType, None]Type) (Source: {source}){Style.RESET_ALL}"
                )
                returnUnion[Decimal, pd.NAType, None]

        final_value: Any =Union[Decimal, pd.NAType, None]
        try:
            ...
            if default is None: # If default is alsoUnion[Decimal, pd.NAType, None], and casting failed
                if required:
                    _pre_logger.critical(
                    ...
                    _pre_logger.warning(
                        f"{Fore.YELLOW}Casting failed for {key}, default is None. Final value:Union[Decimal, pd.NAType, None]{Style.RESET_ALL}"
                    )
                    returnUnion[Decimal, pd.NAType, None] # ReturnUnion[Decimal, pd.NAType, None] if default is None and casting failed
            else:
...
def log_success(self: logging.Logger, message: str, *args: Any, **kwargs: Any) ->Union[Decimal, pd.NAType, None]:
...
class TradingStrategy(ABC):
    def __init__(self, config: Config, df_columns: list[str] |Union[Decimal, pd.NAType, None] =Union[Decimal, pd.NAType, None]):
        self.config = config
    ...
    def _validate_df(self, df: pd.DataFrame, min_rows: int = 2) -> bool:
        if df is None or df.empty or len(df) < min_rows:
            self.logger.debug(f"Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need: {min_rows}).")
            return False
        ...
        if self.required_columns and df.iloc[-1][self.required_columns].isnull().any():
             ...
             # Allow strategies to handle this; they should check forUnion[Decimal, pd.NAType, None]or use safe_decimal_conversion.
        return True
...
class DualSupertrendStrategy(TradingStrategy):
    def __init__(self, config: Config):
        super().__init__(config, df_columns=["st_long_flip", "st_short_flip", "confirm_trend"]) # Using internal names

    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        signals = self._get_default_signals()
        if not self._validate_df(df): return signals
        last = df.iloc[-1]
        primary_long_flip = last.get("st_long_flip", False) # From calculate_supertrend with no prefix
        primary_short_flip = last.get("st_short_flip", False) # From calculate_supertrend with no prefix
        confirm_is_up = last.get("confirm_trend", pd.NA) # From calculate_supertrend with "confirm_" prefix
...
class StochRsiMomentumStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        signals = self._get_default_signals()
        if not self._validate_df(df, min_rows=2): return signals # Need prev and current
        ...
        # Check forUnion[Decimal, pd.NAType, None]before conversion, as safe_decimal_conversion will turn them to Decimal('NaN') or default
        if any(pd.isna(v) for v in [k_now, d_now, mom_now, k_prev, d_prev]):
            self.logger.debug("Skipping due to initial NA StochRSI/Mom values.")
            return signals
        
        # Convert to Union[Decimal, pd.NAType, None]for precise comparison with config thresholds.
        # safe_decimal_conversion handles original float/Decimal/str values.
        k_now_dec = safe_decimal_conversion(k_now, Decimal('NaN')) # NaN if problematic
        mom_now_dec = safe_decimal_conversion(mom_now, Decimal('NaN'))
        
        # Also convert prev values if they are part of logic that needs Union[Decimal, pd.NAType, None]precision (not strictly needed here for comparison logic with k_now/d_now)
        k_prev_dec = safe_decimal_conversion(k_prev, Decimal('NaN'))
        d_prev_dec = safe_decimal_conversion(d_prev, Decimal('NaN'))
        d_now_dec = safe_decimal_conversion(d_now, Decimal('NaN'))


        if k_now_dec.is_nan() or mom_now_dec.is_nan() or k_prev_dec.is_nan() or d_prev_dec.is_nan() or d_now_dec.is_nan():
            self.logger.debug("Skipping due to NA StochRSI/Mom Union[Decimal, pd.NAType, None]values after conversion.")
            return signals
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if any(v.is_nan() for v in [fish_now, sig_now, fish_prev, sig_prev]):
            self.logger.debug("Skipping due to NA Ehlers Fisher Union[Decimal, pd.NAType, None]values after conversion.")
            return signals
...
class EhlersMaCrossStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if any(v.is_nan() for v in [fast_ma_now, slow_ma_now, fast_ma_prev, slow_ma_prev]):
            self.logger.debug("Skipping due to NA Ehlers SSF MA Union[Decimal, pd.NAType, None]values after conversion.")
            return signals
...
if StrategyClass:
    ...
    logger.critical(f"Failed to find strategy class for {CONFIG.strategy_name.value}. Exiting.")
    sys.exit(1)
...
class TradeMetrics:
    ...
    def log_trade(self, symbol: str, side: str, entry_price: Decimal, exit_price: Decimal, qty: Decimal, entry_time_ms: int, exit_time_ms: int, reason: str):
        ...
        if side.lower() == CONFIG.side_sell.lower() or side.lower() == CONFIG.pos_short.lower():
            profit_per_unit = entry_price - exit_price # For shorts, profit if exit_price < entry_price
        else: # Long trade
        ...
        self.logger.success( # Using custom success level
    ...
    def summary(self) -> str:
        ...
        win_rate = (wins / total_trades) * 100 if total_trades > 0 else Decimal(0)
        ...
        summary_str = (
            ...
            f"Avg P/L per Trade: {(Fore.GREEN if avg_profit > 0 else (Fore.RED if avg_profit < 0 else Fore.YELLOW))}{avg_profit:.2f} {CONFIG.usdt_symbol}{Style.RESET_ALL}\n"
...
_active_trade_details: dict[str, Any |Union[Decimal, pd.NAType, None]] = {"entry_price":Union[Decimal, pd.NAType, None], "entry_time_ms":Union[Decimal, pd.NAType, None], "side":Union[Decimal, pd.NAType, None], "qty":Union[Decimal, pd.NAType, None]}


# --- Helper Functions - Minor Cantrips ---
def safe_decimal_conversion(value: Any, default: Union[Decimal, pd.NAType, None]= Decimal("0.0")) -> Decimal:
    """Safely converts a value to Decimal, handlingUnion[Decimal, pd.NAType, None], pd.NA, and conversion errors.
    If default is Decimal('NaN'), returns Decimal('NaN') on error/None/NA."""
    if pd.isna(value) or value is None: # pd.isna handlesUnion[Decimal, pd.NAType, None], np.nan, pd.NaT, pd.NA
        return default
    try:
...
def format_order_id(order_id: str | int |Union[Decimal, pd.NAType, None]) -> str:
...
_termux_sms_command_exists: bool |Union[Decimal, pd.NAType, None] =Union[Decimal, pd.NAType, None]
def send_sms_alert(message: str) -> bool:
    ...
    if _termux_sms_command_exists is None: # Check only once
        _termux_sms_command_exists = shutil.which("termux-sms-send") is notUnion[Decimal, pd.NAType, None]
        if not _termux_sms_command_exists:
            logger.warning(f"{Fore.YELLOW}SMS: 'termux-sms-send' command not found. Install Termux:API and ensure termux-api package is installed in Termux.{Style.RESET_ALL}")
    ...
    try:
        ...
        if result.returncode == 0:
            logger.success(f"{Fore.MAGENTA}SMS dispatched successfully.{Style.RESET_ALL}"); return True
        else:
            logger.error(f"{Fore.RED}SMS dispatch failed. Code: {result.returncode}, Stderr: {result.stderr.strip()}{Style.RESET_ALL}"); return False
    except subprocess.TimeoutExpired:
        logger.error(f"{Fore.RED}SMS dispatch timed out after {CONFIG.sms_timeout_seconds}s.{Style.RESET_ALL}"); return False
    except Exception as e:
        logger.error(f"{Fore.RED}SMS dispatch failed with exception: {e}{Style.RESET_ALL}"); return False


# --- Exchange Initialization - Opening the Portal ---
def initialize_exchange() -> ccxt.Exchange |Union[Decimal, pd.NAType, None]:
    logger.info(f"{Fore.BLUE}Opening portal to Bybit via CCXT...{Style.RESET_ALL}")
    if not CONFIG.api_key or not CONFIG.api_secret:
        logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: API Key and/or Secret are missing in configuration.{Style.RESET_ALL}")
    ...
    try:
        ...
        exchange.load_markets(force_reload=True) # Force reload
        logger.debug(f"Loaded {len(exchange.markets)} market structures from Bybit.")
        
        logger.debug("Performing initial balance check to verify API key permissions...")
        ...
        exchange.fetch_balance(params=balance_params) # Not using safe_api_call here, fail fast on init
        
        logger.success(f"{Fore.GREEN}{Style.BRIGHT}Portal to Bybit Opened & Authenticated (V5 API).{Style.RESET_ALL}")
        if exchange.sandbox: # type: ignore
            logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}!!! TESTNET MODE ACTIVE !!!{Style.RESET_ALL}")
            send_sms_alert(f"[Pyrmethus/{CONFIG.strategy_name.value}] Portal opened (TESTNET).")
        else:
            logger.warning(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}!!! LIVE TRADING MODE ACTIVE - EXTREME CAUTION !!!{Style.RESET_ALL}")
            send_sms_alert(f"[Pyrmethus/{CONFIG.strategy_name.value}] Portal opened (LIVE).")
        return exchange
    except ccxt.AuthenticationError as e:
        logger.critical(f"{Back.RED}{Fore.WHITE}Portal opening FAILED (Authentication Error): {e}{Style.RESET_ALL}")
        send_sms_alert(f"[Pyrmethus] CRITICAL: Auth FAILED: {e}. Check API keys.")
    except ccxt.NetworkError as e:
        logger.critical(f"{Back.RED}{Fore.WHITE}Portal opening FAILED (Network Error): {e}{Style.RESET_ALL}")
...
def calculate_supertrend(df: pd.DataFrame, length: int, multiplier: Decimal, prefix: str = "") -> pd.DataFrame:
    ...
    # Internal column names for consistency within the bot
    ...
    min_len_needed_for_pta = length + 1 # ATR period + 1 for pandas_ta to start calculating

    if df is None or df.empty or not all(c in df.columns for c in ["high", "low", "close"]) or len(df) < min_len_needed_for_pta:
        logger.warning(f"{Fore.YELLOW}Scrying ({col_prefix}ST): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed_for_pta}). Populating with NAs.{Style.RESET_ALL}")
        if df is notUnion[Decimal, pd.NAType, None]: # Ensure df exists to add NA columns
            for col in target_cols: df[col] = pd.NA
        return df if df is notUnion[Decimal, pd.NAType, None] else pd.DataFrame(columns=target_cols) # Return empty DF if input wasUnion[Decimal, pd.NAType, None]
    ...
    try:
        ...
        if not all(c in temp_df.columns for c in pta_cols_to_check):
            ...
            logger.error(f"{Fore.RED}Scrying ({col_prefix}ST): pandas_ta failed to create expected SuperTrend columns: {missing_cols}. Populating with NAs.{Style.RESET_ALL}")
            for col in target_cols: df[col] =Union[Decimal, pd.NAType, None]# Assign to original df
            return df
        
        df[out_supertrend_val] = temp_df[pta_st_val_col].apply(lambda x: safe_decimal_conversion(x, pd.NA))
        
        df[out_trend_direction] =Union[Decimal, pd.NAType, None]# Default to NA
        ...
        df[out_long_flip] = (temp_df[pta_st_dir_col] == 1) & (prev_dir == -1)
        df[out_short_flip] = (temp_df[pta_st_dir_col] == -1) & (prev_dir == 1)
        
        # Fill NaNs in boolean flip columns with False after comparison, as NA means no flip
        ...
        logger.debug(traceback.format_exc())
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols: df[col] = pd.NA
    return df

def analyze_volume_atr(df: pd.DataFrame, atr_len: int, vol_ma_len: int) -> dict[str, Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]]:
    results: dict[str, Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]] = {"atr": pd.NA, "volume_ma": pd.NA, "last_volume": pd.NA, "volume_ratio": pd.NA}
    min_len_needed = max(atr_len, vol_ma_len) + 1 
    
    if df is None or df.empty or not all(c in df.columns for c in ["high", "low", "close", "volume"]) or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (Vol/ATR): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}).{Style.RESET_ALL}")
        return results
    ...
    try:
        ...
        if not temp_df.empty:
            results["volume_ma"] = safe_decimal_conversion(temp_df[volume_ma_col].iloc[-1], pd.NA)
            results["last_volume"] = safe_decimal_conversion(temp_df["volume"].iloc[-1], pd.NA)
            
            vol_ma = results["volume_ma"]
            last_vol = results["last_volume"]
            if not pd.isna(vol_ma) and not pd.isna(last_vol) and vol_ma is notUnion[Decimal, pd.NAType, None] and last_vol is notUnion[Decimal, pd.NAType, None] and vol_ma > CONFIG.position_qty_epsilon:
                try: 
                    results["volume_ratio"] = last_vol / vol_ma
                except (DivisionByZero, InvalidOperation): 
                    results["volume_ratio"] =Union[Decimal, pd.NAType, None]
        ...
        logger.error(f"{Fore.RED}Scrying (Vol/ATR): Error during calculation: {e}{Style.RESET_ALL}"); logger.debug(traceback.format_exc())
        results = {key:Union[Decimal, pd.NAType, None]for key in results} 
    return results
...
def calculate_stochrsi_momentum(df: pd.DataFrame, rsi_len: int, stoch_len: int, k: int, d: int, mom_len: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (StochRSI/Mom): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols: df[col] = pd.NA
        return df if df is notUnion[Decimal, pd.NAType, None] else pd.DataFrame(columns=target_cols)
    ...
    try:
        ...
        if stochrsi_df is notUnion[Decimal, pd.NAType, None] and not stochrsi_df.empty:
            df["stochrsi_k"] = stochrsi_df[k_col_ta].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if k_col_ta in stochrsi_df else pd.NA
            df["stochrsi_d"] = stochrsi_df[d_col_ta].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if d_col_ta in stochrsi_df else pd.NA
        else: 
            df["stochrsi_k"], df["stochrsi_d"] = pd.NA, pd.NA
        
        temp_df_mom = df[['close']].copy() 
        mom_col_ta = f"MOM_{mom_len}" 
        temp_df_mom.ta.mom(length=mom_len, append=True)
        df["momentum"] = temp_df_mom[mom_col_ta].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if mom_col_ta in temp_df_mom else pd.NA
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    min_len_needed = length + signal + 5 
    
    if df is None or df.empty or not all(c in df.columns for c in ["high", "low"]) or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (EhlersFisher): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols: df[col] = pd.NA
        return df if df is notUnion[Decimal, pd.NAType, None] else pd.DataFrame(columns=target_cols)
    ...
    try:
        ...
        if fisher_df is notUnion[Decimal, pd.NAType, None] and not fisher_df.empty:
            df["ehlers_fisher"] = fisher_df[fish_col_ta].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if fish_col_ta in fisher_df else pd.NA
            df["ehlers_signal"] = fisher_df[signal_col_ta].apply(lambda x: safe_decimal_conversion(x, pd.NA)) if signal_col_ta in fisher_df else pd.NA
        else:
...
def calculate_ehlers_ma(df: pd.DataFrame, fast_len: int, slow_len: int, poles: int) -> pd.DataFrame:
    ...
    min_len_needed = max(fast_len, slow_len) + poles + 5 
    
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (Ehlers SSF MA): Insufficient data (Rows: {len(df) if df is notUnion[Decimal, pd.NAType, None] else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols: df[col] = pd.NA
        return df if df is notUnion[Decimal, pd.NAType, None] else pd.DataFrame(columns=target_cols)
    ...
    try:
        ...
        df["ehlers_ssf_fast"] = ssf_fast_series.apply(lambda x: safe_decimal_conversion(x, pd.NA)) if ssf_fast_series is notUnion[Decimal, pd.NAType, None] and not ssf_fast_series.empty else pd.NA
        
        ssf_slow_series = df.ta.ssf(length=slow_len, poles=poles, append=False) # type: ignore
        df["ehlers_ssf_slow"] = ssf_slow_series.apply(lambda x: safe_decimal_conversion(x, pd.NA)) if ssf_slow_series is notUnion[Decimal, pd.NAType, None] and not ssf_slow_series.empty else pd.NA
        ...
        logger.error(f"{Fore.RED}Scrying (Ehlers SSF MA): Error during calculation: {e}{Style.RESET_ALL}"); logger.debug(traceback.format_exc())
        if df is notUnion[Decimal, pd.NAType, None]:
            for col in target_cols: df[col] = pd.NA
    return df

def analyze_order_book(exchange: ccxt.Exchange, symbol: str, depth: int, fetch_limit: int) -> dict[str, Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]]:
    results: dict[str, Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]] = {"bid_ask_ratio": pd.NA, "spread": pd.NA, "best_bid": pd.NA, "best_ask": pd.NA}
    if not exchange.has.get("fetchL2OrderBook"): 
        logger.warning(f"{Fore.YELLOW}OB Scrying: Exchange does not support fetchL2OrderBook for {symbol}.{Style.RESET_ALL}")
        return results
    ...
    try:
        ...
        if not bids or not asks:
            logger.warning(f"{Fore.YELLOW}OB Scrying: Empty bids or asks array for {symbol}. Market might be thin or API issue.{Style.RESET_ALL}")
            return results
        ...
        if not pd.isna(best_bid_val) and not pd.isna(best_ask_val) and best_bid_val is notUnion[Decimal, pd.NAType, None] and best_ask_val is notUnion[Decimal, pd.NAType, None] and best_bid_val > 0 and best_ask_val > 0:
        ...
        if ask_vol > CONFIG.position_qty_epsilon: 
            try: 
                ...
                results["bid_ask_ratio"] =Union[Decimal, pd.NAType, None]
        else:
            results["bid_ask_ratio"] =Union[Decimal, pd.NAType, None]
        ...
        if not pd.isna(results['bid_ask_ratio']): log_parts.append(f"Ratio(B/A)={Fore.CYAN}{_format_for_log(results['bid_ask_ratio'],3)}{Style.RESET_ALL}")
        logger.debug(f"OB Scrying (Depth {depth}, Fetched {fetch_limit}): {', '.join(log_parts)}")

    except Exception as e:
        logger.warning(f"{Fore.YELLOW}OB Scrying Error for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        results = {key:Union[Decimal, pd.NAType, None]for key in results}
    return results
...
_last_market_data: pd.DataFrame |Union[Decimal, pd.NAType, None] =Union[Decimal, pd.NAType, None]
_last_fetch_timestamp: float = 0.0

def get_market_data(exchange: ccxt.Exchange, symbol: str, interval: str, limit: int) -> pd.DataFrame |Union[Decimal, pd.NAType, None]:
    ...
    try:
        ...
        logger.warning(f"Could not parse timeframe '{interval}' for caching via exchange.parse_timeframe: {e}. Cache duration check might be affected.")
        candle_duration_seconds = 0 

    cache_is_valid = False
    if _last_market_data is notUnion[Decimal, pd.NAType, None] and not _last_market_data.empty and len(_last_market_data) >= limit:
    ...
    if cache_is_valid:
        logger.debug(f"Data Fetch: Using CACHED market data ({len(_last_market_data)} candles) for {symbol}. Last fetch: {time.strftime('%H:%M:%S', time.localtime(_last_fetch_timestamp))}")
    ...
    try:
        logger.debug(f"Data Fetch: Gathering {limit} OHLCV candles for {symbol} (Timeframe: {interval})...")
        ...
        if not ohlcv: 
            logger.warning(f"{Fore.YELLOW}Data Fetch: No OHLCV data returned for {symbol} ({interval}). Market may be inactive or API issue.{Style.RESET_ALL}")
            returnUnion[Decimal, pd.NAType, None] 
        ...
        if df.isnull().values.any():
            ...
            logger.warning(f"{Fore.YELLOW}Data Fetch: NaNs found in OHLCV data for columns: {nan_cols}. Applying ffill then bfill.{Style.RESET_ALL}")
            df.ffill(inplace=True) 
            df.bfill(inplace=True) 
            
            if df.isnull().values.any(): 
                logger.error(f"{Fore.RED}Data Fetch: Unfillable NaNs remain in OHLCV data after ffill/bfill. Data quality issue for {symbol}.{Style.RESET_ALL}")
                returnUnion[Decimal, pd.NAType, None] 
        
        _last_market_data = df.copy() 
        _last_fetch_timestamp = current_time
        logger.debug(f"Data Fetch: Successfully woven {len(df)} OHLCV candles for {symbol}. Data cached.")
        return df.copy() 
        
    except Exception as e:
        logger.error(f"{Fore.RED}Data Fetch: Error processing OHLCV for {symbol} ({interval}): {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
    returnUnion[Decimal, pd.NAType, None]
...
def check_account_health(exchange: ccxt.Exchange, config: Config) -> bool:
    ...
    try:
        ...
        if not usdt_balance: 
            logger.error(f"Account Health: '{config.usdt_symbol}' balance data not found in response. Full balance: {balance}")
            return False 

        total_equity = safe_decimal_conversion(usdt_balance.get("total")) 
        used_margin = safe_decimal_conversion(usdt_balance.get("used"))   

        if total_equity.is_nan() or used_margin.is_nan(): 
             logger.warning(f"Account Health: Could not determine equity or used margin. Total: {usdt_balance.get('total')}, Used: {usdt_balance.get('used')}")
             return False 
        ...
        if total_equity <= Decimal("0"):
            ...
            if used_margin > Decimal("0"):
                 logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL HEALTH: Zero/Negative Equity ({_format_for_log(total_equity,2)}) with Used Margin ({_format_for_log(used_margin,2)})! Halting trading logic.{Style.RESET_ALL}")
                 send_sms_alert(f"[{config.symbol.split('/')[0].split(':')[0]}/{config.strategy_name.value}] CRITICAL: Zero/Neg Equity with Used Margin. BOT PAUSED.")
                 return False 
            return True 

        margin_ratio = used_margin / total_equity if total_equity > 0 else Decimal("Infinity") 
        health_color = Fore.GREEN if margin_ratio <= config.max_account_margin_ratio else Fore.RED

        logger.info(f"Account Health: Equity={_format_for_log(total_equity,2)}, UsedMargin={_format_for_log(used_margin,2)}, MarginRatio={health_color}{margin_ratio:.2%}{Style.RESET_ALL} (Max: {config.max_account_margin_ratio:.0%})")

        if margin_ratio > config.max_account_margin_ratio:
            logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL HEALTH: High margin ratio {margin_ratio:.2%} exceeds max {config.max_account_margin_ratio:.0%}. Halting trading logic.{Style.RESET_ALL}")
        ...
        logger.error(f"Account health check failed with exception: {type(e).__name__} - {e}")
        logger.debug(traceback.format_exc())
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        if market.get("linear"): category = "linear"
            ...
            logger.warning(f"{Fore.YELLOW}Pos Check: Market type for {symbol} not explicitly linear/inverse. Assuming 'linear'.{Style.RESET_ALL}")
        ...
        if not fetched_positions: 
            logger.info(f"{Fore.BLUE}Pos Check: No positions returned for {market_id} with category '{category}'. Assumed Flat.{Style.RESET_ALL}")
            return default_pos
        ...
        for pos in fetched_positions:
            ...
            if int(pos_info.get("positionIdx", -1)) == 0: # One-Way mode position
                ...
                if size_dec > CONFIG.position_qty_epsilon: 
                    entry_price_str = pos_info.get("avgPrice") 
                    entry_price = safe_decimal_conversion(entry_price_str)
                    
                    bybit_side = pos_info.get("side")
                    side = CONFIG.pos_none
                    if bybit_side == "Buy": side = CONFIG.pos_long
                    elif bybit_side == "Sell": side = CONFIG.pos_short
                    else: 
                        logger.warning(f"{Fore.YELLOW}Pos Check: Unknown side '{bybit_side}' for {market_id}. Treating as flat.{Style.RESET_ALL}")
                        continue 

                    pos_color = Fore.GREEN if side == CONFIG.pos_long else Fore.RED
                    logger.info(f"{pos_color}Pos Check: ACTIVE {side} position found. Qty={_format_for_log(size_dec,8)} @ Entry={_format_for_log(entry_price,4)}{Style.RESET_ALL}")
                    return {"side": side, "qty": size_dec, "entry_price": entry_price}
        
        logger.info(f"{Fore.BLUE}Pos Check: No active One-Way position for {market_id} (category '{category}'). Flat.{Style.RESET_ALL}")
    except Exception as e:
        logger.error(f"{Fore.RED}Pos Check: Error fetching or processing position for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
    return default_pos

def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    logger.info(f"{Fore.CYAN}Leverage: Attempting to set {leverage}x for {symbol}...{Style.RESET_ALL}")
    try:
        market = exchange.market(symbol)
        if not market or not market.get("contract"):
            logger.error(f"{Fore.RED}Leverage: Market '{symbol}' is not a contract market or not found.{Style.RESET_ALL}"); return False
        ...
        logger.success(f"{Fore.GREEN}Leverage: Successfully set to {leverage}x for {symbol} (Category: {category}). Response: {response}{Style.RESET_ALL}")
        return True
    except ccxt.ExchangeError as e:
        err_str = str(e).lower()
        if any(sub in err_str for sub in ["leverage not modified", "same leverage", "110044"]): # Bybit V5 "Set leverage not modified" code
            logger.info(f"{Fore.CYAN}Leverage: Already set to {leverage}x for {symbol} (or no change needed).{Style.RESET_ALL}"); return True
        logger.error(f"{Fore.RED}Leverage: Exchange error setting leverage for {symbol} to {leverage}x: {e}{Style.RESET_ALL}")
    except Exception as e_unexp:
        logger.error(f"{Fore.RED}Leverage: Unexpected error setting leverage for {symbol}: {e_unexp}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
    return False

def close_position(exchange: ccxt.Exchange, symbol: str, position_to_close: dict[str, Any], reason: str = "Signal") -> dict[str, Any] |Union[Decimal, pd.NAType, None]:
    ...
    logger.info(f"{Fore.YELLOW}Banish Position: Attempting to close {symbol} ({reason}). Initial details: Side={initial_side}, Qty={_format_for_log(initial_qty, 8)}{Style.RESET_ALL}")
    ...
    if live_position["side"] == CONFIG.pos_none or live_position["qty"] <= CONFIG.position_qty_epsilon:
        logger.warning(f"{Fore.YELLOW}Banish Position: Re-validation shows NO active position for {symbol}. Aborting close attempt.{Style.RESET_ALL}")
        if _active_trade_details.get("entry_price") is notUnion[Decimal, pd.NAType, None]:
            logger.info("Clearing potentially stale active trade details as position is now confirmed flat.")
            _active_trade_details = {"entry_price":Union[Decimal, pd.NAType, None], "entry_time_ms":Union[Decimal, pd.NAType, None], "side":Union[Decimal, pd.NAType, None], "qty":Union[Decimal, pd.NAType, None]}
        returnUnion[Decimal, pd.NAType, None] 
    ...
    try:
        logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}Banish Position: Sending CLOSE order for {live_position['side']} ({reason}): {side_to_execute.upper()} MARKET {amount_to_close_str} {symbol}...{Style.RESET_ALL}")
        ...
        avg_fill_price_closed = safe_decimal_conversion(order.get("average", "0")) 
        order_timestamp_ms = order.get("timestamp") # Timestamp of the close order itself

        if status == "closed" and abs(filled_qty_closed - live_position["qty"]) < CONFIG.position_qty_epsilon:
            logger.success(f"{Fore.GREEN}{Style.BRIGHT}Banish Position: CONFIRMED FILLED for {symbol} ({reason}). Order ID: ...{format_order_id(order.get('id'))}{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] BANISHED {live_position['side']} {amount_to_close_str} ({reason}). ExitPx: {_format_for_log(avg_fill_price_closed,4)}")
            
            if _active_trade_details.get("entry_price") is notUnion[Decimal, pd.NAType, None] and order_timestamp_ms is notUnion[Decimal, pd.NAType, None]:
                if _active_trade_details.get("side") and _active_trade_details.get("qty") and _active_trade_details.get("entry_time_ms"):
                    trade_metrics.log_trade(
                        ...
                        qty=_active_trade_details["qty"], 
                        entry_time_ms=_active_trade_details["entry_time_ms"], 
                        exit_time_ms=order_timestamp_ms, 
                        reason=reason
                    )
                else:
                ...
                logger.info(f"TradeMetrics: Skipping log for {symbol} as no active entry details were found or exit order timestamp missing.")
            
            _active_trade_details = {"entry_price":Union[Decimal, pd.NAType, None], "entry_time_ms":Union[Decimal, pd.NAType, None], "side":Union[Decimal, pd.NAType, None], "qty":Union[Decimal, pd.NAType, None]}
            return order 
        else:
            logger.warning(f"{Fore.YELLOW}Banish Position: Fill status uncertain for {symbol}. Expected Qty: {live_position['qty']}, Filled: {filled_qty_closed}. Order ID: ...{format_order_id(order.get('id'))}, Status: {status}. Manual check may be needed.{Style.RESET_ALL}")
            return order 
            
    except Exception as e:
        logger.error(f"{Fore.RED}Banish Position ({reason}): Exception during close attempt for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}")
        send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] ERROR Banishing {initial_side} ({reason}): {type(e).__name__}.")
        logger.debug(traceback.format_exc())
    returnUnion[Decimal, pd.NAType, None] 

def calculate_position_size(equity: Decimal, risk_pct: Decimal, entry_px: Decimal, sl_px: Decimal, lev: int, sym: str, ex: ccxt.Exchange) -> tuple[Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None], Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]]:
    if not (entry_px > 0 and sl_px > 0 and 0 < risk_pct < 1 and equity > 0 and lev > 0):
        logger.error(f"{Fore.RED}RiskCalc: Invalid inputs for position sizing (Equity: {equity}, Risk%: {risk_pct}, Entry: {entry_px}, SL: {sl_px}, Lev: {lev}).{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None],Union[Decimal, pd.NAType, None]
    
    price_diff_per_unit = abs(entry_px - sl_px) 
    if price_diff_per_unit < CONFIG.position_qty_epsilon: 
        logger.error(f"{Fore.RED}RiskCalc: Entry price and Stop Loss price are too close or identical. Cannot calculate position size.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None],Union[Decimal, pd.NAType, None]
    ...
    try:
        ...
        if qty_prec <= CONFIG.position_qty_epsilon: 
            logger.warning(f"{Fore.YELLOW}RiskCalc: Calculated quantity ({_format_for_log(qty_prec,8)}) is negligible or zero after precision adjustment. Min risk not met or stop too wide.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None],Union[Decimal, pd.NAType, None]
        
        position_value_usdt = qty_prec * entry_px
        margin_required = position_value_usdt / Decimal(lev) 
        
        logger.debug(f"RiskCalc: Equity={_format_for_log(equity,2)}, Risk%={risk_pct:.2%}, RiskAmt={_format_for_log(risk_amt_usdt,2)}")
        logger.debug(f"RiskCalc: EntryPx={_format_for_log(entry_px,4)}, SLPx={_format_for_log(sl_px,4)}, PriceDiff={_format_for_log(price_diff_per_unit,4)}")
        logger.debug(f"RiskCalc: RawQty={_format_for_log(qty_raw,8)}, PreciseQty={Fore.CYAN}{_format_for_log(qty_prec,8)}{Style.RESET_ALL}, PosValue={_format_for_log(position_value_usdt,2)}, MarginReq={_format_for_log(margin_required,4)}")
        
        return qty_prec, margin_required
    except (DivisionByZero, InvalidOperation, Exception) as e:
        logger.error(f"{Fore.RED}RiskCalc: Error during position size calculation: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        returnUnion[Decimal, pd.NAType, None],Union[Decimal, pd.NAType, None]

def wait_for_order_fill(exchange: ccxt.Exchange, order_id: str, symbol: str, timeout_s: int) -> dict[str, Any] |Union[Decimal, pd.NAType, None]:
    ...
    while time.time() - start_time < timeout_s:
        try:
            ...
            if status == "closed": 
                logger.success(f"{Fore.GREEN}Order ...{oid_short} ({symbol}) confirmed FILLED/CLOSED.{Style.RESET_ALL}"); return order
            if status in ["canceled", "rejected", "expired"]:
                logger.error(f"{Fore.RED}Order ...{oid_short} ({symbol}) FAILED with status: {status}.{Style.RESET_ALL}"); return order
            
            time.sleep(0.75) 
            
        except ccxt.OrderNotFound:
            logger.warning(f"{Fore.YELLOW}Order ...{oid_short} ({symbol}) not found. Possible propagation delay. Retrying...{Style.RESET_ALL}")
            time.sleep(1.5) 
        except Exception as e: 
            logger.warning(f"{Fore.YELLOW}Error checking order ...{oid_short} ({symbol}): {type(e).__name__}. Retrying...{Style.RESET_ALL}")
            logger.debug(traceback.format_exc())
            time.sleep(2) 

    logger.error(f"{Fore.RED}Order ...{oid_short} ({symbol}) fill check TIMEOUT after {timeout_s}s.{Style.RESET_ALL}")
    try: 
        return safe_api_call(exchange.fetch_order, order_id, symbol, params=params)
    except Exception as e_final:
        logger.error(f"{Fore.RED}Final check for order ...{oid_short} ({symbol}) also failed: {type(e_final).__name__}{Style.RESET_ALL}")
        returnUnion[Decimal, pd.NAType, None]

def place_risked_market_order(exchange: ccxt.Exchange, symbol: str, side: str, risk_percentage: Decimal, current_atr: Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None], sl_atr_multiplier: Decimal, leverage: int, max_order_cap_usdt: Decimal, margin_check_buffer: Decimal, tsl_percent: Decimal, tsl_activation_offset_percent: Decimal) -> dict[str, Any] |Union[Decimal, pd.NAType, None]:
    global _active_trade_details
    market_base = symbol.split("/")[0].split(":")[0]
    ...
    if pd.isna(current_atr) or current_atr is None or current_atr <= 0:
        logger.error(f"{Fore.RED}Place Order Error: Invalid ATR ({current_atr}). Cannot calculate Stop Loss or position size.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]
    ...
    try:
        ...
        market = exchange.market(symbol)
        min_qty_allowed = safe_decimal_conversion(market.get("limits",{}).get("amount",{}).get("min"), Decimal("0"))
        ...
        if usdt_equity <= 0 or usdt_free < 0: 
            logger.error(f"{Fore.RED}Place Order Error: Invalid equity ({_format_for_log(usdt_equity,2)}) or free margin ({_format_for_log(usdt_free,2)}).{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]

        ob_data = analyze_order_book(exchange, symbol, CONFIG.shallow_ob_fetch_depth, CONFIG.shallow_ob_fetch_depth)
        entry_px_est_orig = ob_data.get("best_ask") if side == CONFIG.side_buy else ob_data.get("best_bid")
        entry_px_est: Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None] = pd.NA
        if not pd.isna(entry_px_est_orig) and entry_px_est_orig is notUnion[Decimal, pd.NAType, None] and entry_px_est_orig > 0:
             entry_px_est = entry_px_est_orig
        else:
            ticker = safe_api_call(exchange.fetch_ticker, symbol) 
            entry_px_est = safe_decimal_conversion(ticker.get("last"), pd.NA)
        
        if pd.isna(entry_px_est) or entry_px_est is None or entry_px_est <= 0:
            logger.error(f"{Fore.RED}Place Order Error: Failed to get a valid entry price estimate for {symbol}.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]

        sl_distance = current_atr * sl_atr_multiplier
        sl_px_raw_est = (entry_px_est - sl_distance) if side == CONFIG.side_buy else (entry_px_est + sl_distance)
        sl_px_est_str = format_price(exchange, symbol, sl_px_raw_est) 
        sl_px_est = Decimal(sl_px_est_str)
        if sl_px_est <= 0: 
            logger.error(f"{Fore.RED}Place Order Error: Invalid estimated SL price ({_format_for_log(sl_px_est,4)}) after calculation.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]
        ...
        if final_qty is None or margin_est is None or final_qty <= 0 or margin_est <=0:
            logger.error(f"{Fore.RED}Place Order Error: Position size calculation failed or resulted in zero/negative qty/margin.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]
        
        estimated_pos_value = final_qty * entry_px_est
        if estimated_pos_value > max_order_cap_usdt:
            logger.warning(f"{Fore.YELLOW}Place Order: Estimated position value {estimated_pos_value:.2f} USDT exceeds cap {max_order_cap_usdt:.2f} USDT. Adjusting quantity.{Style.RESET_ALL}")
            ...
            if final_qty <= 0: logger.error(f"{Fore.RED}Place Order: Quantity became zero after cap adjustment.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]
            logger.info(f"Place Order: Quantity adjusted to {_format_for_log(final_qty,8)} due to max cap.")

        if min_qty_allowed > 0 and final_qty < min_qty_allowed:
            logger.error(f"{Fore.RED}Place Order Error: Calculated qty {_format_for_log(final_qty,8)} is less than min allowed {_format_for_log(min_qty_allowed,8)} for {symbol}.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]
        if usdt_free < margin_est * margin_check_buffer:
            logger.error(f"{Fore.RED}Place Order Error: Insufficient FREE margin. Need ~{_format_for_log(margin_est * margin_check_buffer,2)}, Have {_format_for_log(usdt_free,2)}.{Style.RESET_ALL}"); returnUnion[Decimal, pd.NAType, None]

        # --- Place Entry Order ---
        entry_params = {"reduceOnly": False, "category": v5_category, "positionIdx": 0} 
        ...
        if not order_id: 
            logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: Entry order for {symbol} did NOT return an ID! Order details: {entry_order}{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] CRITICAL: Entry order NO ID for {symbol}!")
            returnUnion[Decimal, pd.NAType, None] 
        
        filled_entry_order = wait_for_order_fill(exchange, order_id, symbol, CONFIG.order_fill_timeout_seconds)
        if not filled_entry_order or filled_entry_order.get("status") != "closed":
            logger.error(f"{Fore.RED}Entry order ...{format_order_id(order_id)} for {symbol} not filled or failed. Status: {filled_entry_order.get('status') if filled_entry_order else 'timeout'}.{Style.RESET_ALL}")
            try: 
                if filled_entry_order and filled_entry_order.get("status") == "open":
                    ...
                    logger.info(f"Attempted to cancel unfilled/open entry order ...{format_order_id(order_id)}.")
            except Exception as e_cancel: 
                logger.warning(f"Could not cancel potentially unfilled order ...{format_order_id(order_id)}: {e_cancel}")
            returnUnion[Decimal, pd.NAType, None]

        avg_fill_px = safe_decimal_conversion(filled_entry_order.get("average"), Decimal('0')) # Default to 0 to catch issues
        filled_qty_val = safe_decimal_conversion(filled_entry_order.get("filled"), Decimal('0'))
        entry_timestamp_ms = filled_entry_order.get("timestamp")

        if filled_qty_val <= CONFIG.position_qty_epsilon or avg_fill_px <= CONFIG.position_qty_epsilon or entry_timestamp_ms is None:
            logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid fill data for entry order ...{format_order_id(order_id)}. Qty: {filled_qty_val}, Px: {avg_fill_px}, Ts: {entry_timestamp_ms}.{Style.RESET_ALL}")
            returnUnion[Decimal, pd.NAType, None] 

        logger.success(f"{order_side_color}{Style.BRIGHT}ENTRY CONFIRMED: ...{format_order_id(order_id)}. Filled: {_format_for_log(filled_qty_val,8)} {symbol.split('/')[0]} @ AvgPx: {_format_for_log(avg_fill_px,4)}{Style.RESET_ALL}")
        
        _active_trade_details = {"entry_price": avg_fill_px, "entry_time_ms": entry_timestamp_ms, "side": side, "qty": filled_qty_val}

        # --- Place Protection Orders (SL & TSL) ---
        sl_placed_successfully = False
        tsl_placed_successfully = False 

        actual_sl_px_raw = (avg_fill_px - sl_distance) if side == CONFIG.side_buy else (avg_fill_px + sl_distance)
        actual_sl_px_str = format_price(exchange, symbol, actual_sl_px_raw)
        if Decimal(actual_sl_px_str) <= 0:
            logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL: Actual SL price ({actual_sl_px_str}) is zero or negative! Cannot place SL.{Style.RESET_ALL}")
        else:
            ...
            sl_params = {
                ...
                "reduceOnly": True, 
                "positionIdx": 0, 
                # "tpslMode": "Full", # Bybit specific: Full or Partial for SL/TP scope
                # "slOrderType": "Market" # Bybit specific: default for stopMarket
            }
            try:
                logger.info(f"Placing Fixed SL order: Side={sl_order_side}, Qty={float(filled_qty_val)}, TriggerPx={actual_sl_px_str}")
                sl_order = safe_api_call(exchange.create_order, symbol, "StopMarket", sl_order_side, float(filled_qty_val), price=None, params=sl_params)
                logger.success(f"{Fore.GREEN}Fixed SL placed successfully. ID:...{format_order_id(sl_order.get('id'))}, TriggerPx: {actual_sl_px_str}{Style.RESET_ALL}")
                sl_placed_successfully = True
            except Exception as e_sl: 
                logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL: FAILED to place Fixed SL order: {e_sl}{Style.RESET_ALL}")
                logger.debug(traceback.format_exc())

        if CONFIG.trailing_stop_percentage > 0:
            tsl_activation_offset_val = avg_fill_px * tsl_activation_offset_percent
            tsl_activation_price_raw = (avg_fill_px + tsl_activation_offset_val) if side == CONFIG.side_buy else (avg_fill_px - tsl_activation_offset_val)
            tsl_activation_price_str = format_price(exchange, symbol, tsl_activation_price_raw)
            tsl_value_for_api = str((tsl_percent * Decimal("100")).normalize()) 
            
            if Decimal(tsl_activation_price_str) <= 0:
                logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL: TSL Activation Price ({tsl_activation_price_str}) is zero or negative! Cannot place TSL.{Style.RESET_ALL}")
            else:
                tsl_params_specific = { # Use a different name to avoid modifying sl_params if it was used
                    "category": v5_category,
                    "trailingStop": tsl_value_for_api, 
                    "activePrice": float(tsl_activation_price_str), 
                    "reduceOnly": True,
                    "positionIdx": 0, 
                }
                try:
                    tsl_order_side = CONFIG.side_sell if side == CONFIG.side_buy else CONFIG.side_buy # same side as fixed SL
                    logger.info(f"Placing Trailing SL order: Side={tsl_order_side}, Qty={float(filled_qty_val)}, TrailValue={tsl_value_for_api}%, ActivationPx={tsl_activation_price_str}")
                    # TSL might be placed as part of a market/limit order params, or as a separate conditional order.
                    # CCXT create_order with type 'StopMarket' and trailingStop params is one way.
                    tsl_order = safe_api_call(exchange.create_order, symbol, "StopMarket", tsl_order_side, float(filled_qty_val), price=None, params=tsl_params_specific)
                    logger.success(f"{Fore.GREEN}Trailing SL placed successfully. ID:...{format_order_id(tsl_order.get('id'))}, Trail: {tsl_value_for_api}%, ActivateAt: {tsl_activation_price_str}{Style.RESET_ALL}")
                    tsl_placed_successfully = True 
                except Exception as e_tsl:
                    logger.error(f"{Back.YELLOW}WARNING: FAILED to place Trailing SL order: {e_tsl}{Style.RESET_ALL}") 
                    logger.debug(traceback.format_exc())

        if not sl_placed_successfully: 
            logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL: ESSENTIAL Fixed Stop-Loss placement FAILED for new {side.upper()} position on {symbol}. Attempting EMERGENCY CLOSE of position.{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] CRITICAL: SL FAILED for {side.upper()} {filled_qty_val} @ {avg_fill_px}. ATTEMPTING EMERGENCY CLOSE.")
            
            emergency_close_reason = "EMERGENCY CLOSE - FIXED SL FAILED"
            # _active_trade_details is set, pass a copy as close_position might modify its own version or clear the global one
            current_pos_details_for_close = _active_trade_details.copy() 
            close_order_details = close_position(exchange, symbol, current_pos_details_for_close, reason=emergency_close_reason)
            
            if close_order_details and close_order_details.get("status") == "closed":
                logger.warning(f"{Fore.YELLOW}Emergency close successful for {symbol} due to SL failure.{Style.RESET_ALL}")
            else:
                logger.critical(f"{Back.RED}{Fore.WHITE}EMERGENCY CLOSE FAILED or status uncertain for {symbol}. MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
                send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] CRITICAL: EMERGENCY CLOSE FAILED for {symbol}. MANUAL CHECK!")
            returnUnion[Decimal, pd.NAType, None] 

        send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] ENTERED {side.upper()} {_format_for_log(filled_qty_val,8)} @ {_format_for_log(avg_fill_px,4)}. SL:~{actual_sl_px_str}, TSL:{tsl_value_for_api if CONFIG.trailing_stop_percentage > 0 else 'N/A'}%@~{tsl_activation_price_str if CONFIG.trailing_stop_percentage > 0 else 'N/A'}. EntryID:...{format_order_id(order_id)}")
        return filled_entry_order

    except Exception as e_main:
        logger.error(f"{Back.RED}{Fore.WHITE}Place Order Ritual FAILED with unhandled exception: {type(e_main).__name__} - {e_main}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] ORDER FAIL ({side.upper()}): {type(e_main).__name__}")
    returnUnion[Decimal, pd.NAType, None]


def cancel_open_orders(exchange: ccxt.Exchange, symbol: str, reason: str = "Cleanup") -> int:
    logger.info(f"{Fore.CYAN}Order Cleanup: Cancelling ALL open orders for {symbol} (Reason: {reason})...{Style.RESET_ALL}")
    cancelled_count, failed_count = 0, 0
    
    v5_category = "linear" if exchange.options.get("defaultType") == "linear" else {} # Default to empty dict if not linear
    params_fetch = {"category": v5_category} if v5_category else {}
    
    try:
        open_orders = safe_api_call(exchange.fetch_open_orders, symbol, params=params_fetch)
        
        if not open_orders:
            logger.info(f"{Fore.CYAN}Order Cleanup: No open orders found for {symbol} (Category: {v5_category or 'N/A'}).{Style.RESET_ALL}")
            return 0
            
        logger.warning(f"{Fore.YELLOW}Order Cleanup: Found {len(open_orders)} open orders for {symbol}. Attempting to cancel...{Style.RESET_ALL}")
        for order in open_orders:
            ...
            if order_id:
                try:
                    logger.info(f"Cancelling order ...{format_order_id(order_id)} ({order_type} {order_side} @ {order_price})")
                    params_cancel = {"category": v5_category} if v5_category else {}
                    safe_api_call(exchange.cancel_order, order_id, symbol, params=params_cancel)
                    cancelled_count += 1
                    logger.debug(f"Order ...{format_order_id(order_id)} cancellation request sent.")
                except ccxt.OrderNotFound:
                    logger.info(f"{Fore.GREEN}Order ...{format_order_id(order_id)} already gone (not found). Considered handled.{Style.RESET_ALL}")
                    cancelled_count += 1 
                except Exception as e_cancel:
                    logger.error(f"{Fore.RED}Order Cleanup: FAILED to cancel order ...{format_order_id(order_id)}: {e_cancel}{Style.RESET_ALL}")
                    failed_count +=1
            else:
                logger.error(f"{Fore.RED}Order Cleanup: Found an open order without an ID. Order data: {order}{Style.RESET_ALL}")
                failed_count +=1
                
        if failed_count > 0:
            send_sms_alert(f"[{symbol.split('/')[0].split(':')[0]}/{CONFIG.strategy_name.value}] WARNING: Failed to cancel {failed_count} orders during cleanup ({reason}). Manual check recommended.")
            
    except Exception as e_fetch:
        logger.error(f"{Fore.RED}Order Cleanup: Error fetching/processing open orders for {symbol}: {e_fetch}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
...
def generate_strategy_signals(df: pd.DataFrame, strategy_instance: TradingStrategy) -> dict[str, Any]:
    ...
    logger.error("Unknown or uninitialized strategy instance provided for signal generation.")
    # Fallback to a default "no signal" structure using a temporary base strategy instance
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) ->Union[Decimal, pd.NAType, None]:
    cycle_time_str = df.index[-1].strftime("%Y-%m-%d %H:%M:%S %Z") if not df.empty and isinstance(df.index[-1], pd.Timestamp) else "N/A"
    logger.info(f"{Fore.BLUE}{Style.BRIGHT}========== Cycle Start ({CONFIG.strategy_name.value}): {symbol} | Candle: {cycle_time_str} =========={Style.RESET_ALL}")

    required_rows_for_any_indicator = 100 
    if df is None or len(df) < required_rows_for_any_indicator:
        logger.warning(f"{Fore.YELLOW}Trade Logic: Insufficient data for {symbol} ({len(df) if df is notUnion[Decimal, pd.NAType, None] else 0} rows, need ~{required_rows_for_any_indicator}). Skipping logic cycle.{Style.RESET_ALL}")
        return

    try:
        df_with_indicators, vol_atr_data = calculate_all_indicators(df.copy(), CONFIG)
        
        current_atr_orig = vol_atr_data.get("atr") 
        current_atr: Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None] = pd.NA
        if not pd.isna(current_atr_orig) and current_atr_orig is notUnion[Decimal, pd.NAType, None]:
            current_atr = current_atr_orig


        last_candle_data = df_with_indicators.iloc[-1] if not df_with_indicators.empty else pd.Series(dtype='object')
        current_price_orig = last_candle_data.get("close")
        current_price: Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None]|Union[Decimal, pd.NAType, None] = pd.NA
        if not pd.isna(current_price_orig) and current_price_orig is notUnion[Decimal, pd.NAType, None]:
            current_price = safe_decimal_conversion(current_price_orig, pd.NA)


        if pd.isna(current_price) or current_price is None or current_price <= 0:
            logger.warning(f"{Fore.YELLOW}Trade Logic: Invalid or missing last close price for {symbol}. Last candle data: {last_candle_data.to_dict() if not last_candle_data.empty else 'empty'}{Style.RESET_ALL}"); return
        
        can_place_order_with_atr_sl = not pd.isna(current_atr) and current_atr is notUnion[Decimal, pd.NAType, None] and current_atr > 0

        position = get_current_position(exchange, symbol)
        pos_side, pos_qty, pos_entry = position["side"], position["qty"], position["entry_price"]

        ob_data =Union[Decimal, pd.NAType, None]
        if CONFIG.fetch_order_book_per_cycle or (pos_side == CONFIG.pos_none and can_place_order_with_atr_sl):
        ...
        logger.info(f"{Fore.MAGENTA}--- Indicator & Market Snapshot for {symbol} ---{Style.RESET_ALL}")
        ...
        is_vol_spike = False
        if not pd.isna(volume_ratio_val) and volume_ratio_val is notUnion[Decimal, pd.NAType, None]:
            is_vol_spike = volume_ratio_val > CONFIG.volume_spike_threshold
        logger.info(f"  Volume: Ratio={_format_for_log(volume_ratio_val, 2)}, SpikeThreshold={CONFIG.volume_spike_threshold}, IsSpike={is_vol_spike}")
        
        if ob_data and isinstance(ob_data, dict): 
            logger.info(f"  OrderBook: Ratio(B/A)={_format_for_log(ob_data.get('bid_ask_ratio'),3)}, Spread={_format_for_log(ob_data.get('spread'),4)}")
        
        strategy_logger = CONFIG.strategy_instance.logger 
        ...
        for col_name in CONFIG.strategy_instance.required_columns:
             if col_name in last_candle_data.index:
                 is_trend_col = "trend" in col_name.lower() 
                 strategy_logger.info(f"    {col_name}: {_format_for_log(last_candle_data[col_name], is_bool_trend=is_trend_col)}")
             else:
                 strategy_logger.info(f"    {col_name}: N/A (Not found in DataFrame's last row)")

        pos_color = Fore.GREEN if pos_side == CONFIG.pos_long else (Fore.RED if pos_side == CONFIG.pos_short else Fore.BLUE)
        logger.info(f"  Position: Side={pos_color}{pos_side}{Style.RESET_ALL}, Qty={_format_for_log(pos_qty,8)}, EntryPx={_format_for_log(pos_entry,4)}")
        logger.info(f"{Fore.MAGENTA}{'-'* (len(f'--- Indicator & Market Snapshot for {symbol} ---') -2)}{Style.RESET_ALL}") # Dynamic separator length
        # --- End Snapshot ---
        ...
        if should_exit_long or should_exit_short:
            ...
            logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}*** EXIT SIGNAL TRIGGERED: Attempting to close {pos_side} position for {symbol} (Reason: {exit_reason}) ***{Style.RESET_ALL}")
            ...
            if close_result: 
                logger.info(f"Position close order for {symbol} processed. Waiting for post-close delay.")
                time.sleep(CONFIG.post_close_delay_seconds)
            return 

        if pos_side != CONFIG.pos_none:
            logger.info(f"Holding {pos_color}{pos_side}{Style.RESET_ALL} position for {symbol}. Awaiting SL/TSL trigger or explicit exit signal.")
            return 

        if not can_place_order_with_atr_sl: 
            logger.warning(f"{Fore.YELLOW}Holding Cash for {symbol}. Cannot consider entry: Invalid ATR ({current_atr}) for SL calculation.{Style.RESET_ALL}"); return
        ...
        if ob_data and isinstance(ob_data, dict) and not pd.isna(ob_data.get("bid_ask_ratio")) and ob_data.get("bid_ask_ratio") is notUnion[Decimal, pd.NAType, None]:
            ratio = ob_data["bid_ask_ratio"] 
            if isinstance(ratio, Decimal): # Ensure ratio is Union[Decimal, pd.NAType, None]for comparison
            ...
            logger.warning(f"{Fore.YELLOW}Order book filter active, but no valid OB data or ratio. Entry blocked by OB filter.{Style.RESET_ALL}")
        ...
        entry_params_dict: dict[str, Any] = { # Explicitly type for clarity
            "exchange": exchange, "symbol": symbol, 
            "risk_percentage": CONFIG.risk_per_trade_percentage,
            "current_atr": current_atr, "sl_atr_multiplier": CONFIG.atr_stop_loss_multiplier, 
        ...
        if final_enter_long:
            ...
            logger.info(f"Filter status: OB Long Confirm: {ob_confirm_long}, Volume Confirm: {vol_confirm}")
            cancel_open_orders(exchange, symbol, "Pre-Long Entry"); time.sleep(0.5) 
            place_risked_market_order(side=CONFIG.side_buy, **entry_params_dict) 
            ...
            logger.info(f"Filter status: OB Short Confirm: {ob_confirm_short}, Volume Confirm: {vol_confirm}")
            cancel_open_orders(exchange, symbol, "Pre-Short Entry"); time.sleep(0.5)
            place_risked_market_order(side=CONFIG.side_sell, **entry_params_dict) 
        elif potential_enter_long or potential_enter_short: 
            logger.info(f"Holding Cash for {symbol}. Entry signal present but confirmation filters not met. Long Filter: OB({ob_confirm_long}) Vol({vol_confirm}). Short Filter: OB({ob_confirm_short}) Vol({vol_confirm}).")

    except Exception as e: 
        logger.error(f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}CRITICAL ERROR in trade_logic cycle for {symbol}: {e}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc()) 
        market_base = symbol.split("/")[0].split(":")[0]
        send_sms_alert(f"[{market_base}/{CONFIG.strategy_name.value}] CRITICAL trade_logic ERROR: {type(e).__name__}. Cycle skipped.")
    finally:
        logger.info(f"{Fore.BLUE}{Style.BRIGHT}========== Cycle End: {symbol} =========={Style.RESET_ALL}\n")


# --- Graceful Shutdown - Withdrawing the Arcane Energies ---
def graceful_shutdown(exchange: ccxt.Exchange |Union[Decimal, pd.NAType, None], symbol: str |Union[Decimal, pd.NAType, None]) ->Union[Decimal, pd.NAType, None]:
    logger.warning(f"\n{Fore.YELLOW}{Style.BRIGHT}Shutdown requested. Withdrawing Pyrmethus's arcane energies...{Style.RESET_ALL}")
    
    market_base = "Bot" 
    if symbol:
        parts = symbol.split("/")
        if parts: market_base = parts[0].split(":")[0]

    strat_name_val = "N/A"
    if 'CONFIG' in globals() and hasattr(CONFIG, 'strategy_name') and CONFIG.strategy_name: # Check CONFIG exists
        strat_name_val = CONFIG.strategy_name.value
        
    send_sms_alert(f"[{market_base}/{strat_name_val}] Shutdown initiated. Attempting cleanup...")

    if 'trade_metrics' in globals() and hasattr(trade_metrics, 'summary'): # Check trade_metrics exists
        logger.info("Attempting to log final trade metrics summary...")
        trade_metrics.summary() 

    if not exchange or not symbol:
        logger.warning(f"{Fore.YELLOW}Shutdown: Exchange instance or trading symbol not defined. Automated cleanup might be limited.{Style.RESET_ALL}")
    else:
        try:
            ...
            if position["side"] != CONFIG.pos_none and position["qty"] > CONFIG.position_qty_epsilon:
                logger.warning(f"{Fore.YELLOW}Shutdown: Active {position['side']} position found for {symbol}. Attempting to close...{Style.RESET_ALL}")
                close_result = close_position(exchange, symbol, position, "Bot Shutdown Emergency Close")
                
                if close_result: 
                    logger.info(f"{Fore.CYAN}Shutdown: Close order for {symbol} placed. Performing final position check after delay...{Style.RESET_ALL}")
                    ...
                    if final_pos_check["side"] == CONFIG.pos_none:
                        logger.success(f"{Fore.GREEN}Shutdown: Position for {symbol} confirmed CLOSED successfully.{Style.RESET_ALL}")
                        send_sms_alert(f"[{market_base}/{strat_name_val}] Position on {symbol} successfully closed during shutdown.")
                    else:
                        logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL SHUTDOWN ERROR: FAILED TO CONFIRM position closure for {symbol}! Final State: {final_pos_check['side']} Qty={final_pos_check['qty']}. MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
                        send_sms_alert(f"[{market_base}/{strat_name_val}] CRITICAL: FAILED to confirm {symbol} position closure on shutdown. MANUAL CHECK!")
                else: 
                    logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL SHUTDOWN ERROR: FAILED TO PLACE close order for active position on {symbol}! MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
                    send_sms_alert(f"[{market_base}/{strat_name_val}] CRITICAL: FAILED to place close order for {symbol} on shutdown. MANUAL CHECK!")
            else:
                logger.info(f"{Fore.GREEN}Shutdown: No active position found for {symbol}. Clean exit regarding positions.{Style.RESET_ALL}")
        except Exception as e_cleanup:
            logger.error(f"{Fore.RED}Shutdown Error: Exception during cleanup for {symbol}: {e_cleanup}{Style.RESET_ALL}")
            logger.debug(traceback.format_exc())
            send_sms_alert(f"[{market_base}/{strat_name_val}] ERROR during shutdown cleanup for {symbol}: {type(e_cleanup).__name__}. MANUAL CHECK MAY BE NEEDED.")
...
def main() ->Union[Decimal, pd.NAType, None]:
    ...
    logger.info(f"{Fore.CYAN}--- Active Strategy Path: {CONFIG.strategy_name.value} ---{Style.RESET_ALL}")
    logger.info(f"{Fore.GREEN}--- Primary Protections: ATR-based Fixed Stop Loss + Exchange-Native Trailing Stop Loss (Bybit V5) ---{Style.RESET_ALL}")
    ...
    exchange: ccxt.Exchange |Union[Decimal, pd.NAType, None] =Union[Decimal, pd.NAType, None] 
    symbol_unified: str |Union[Decimal, pd.NAType, None] =Union[Decimal, pd.NAType, None]    
    run_bot: bool = True; cycle_count: int = 0

    try:
        exchange = initialize_exchange()
        if not exchange: 
            logger.critical(f"{Back.RED}{Fore.WHITE}Exchange initialization failed. Pyrmethus cannot proceed. Exiting.{Style.RESET_ALL}")
            return 
            
        market = exchange.market(CONFIG.symbol) 
        if not market:
            raise ValueError(f"Market for symbol '{CONFIG.symbol}' not found. Ensure it's a valid symbol for {exchange.id}.")
        symbol_unified = market["symbol"] 

        if not market.get("contract", False): 
            raise ValueError(f"Market '{symbol_unified}' is not a contract market. This bot is for futures trading.")
        
        logger.info(f"{Fore.GREEN}Spell focused on symbol: {symbol_unified} (Type: {market.get('type', 'N/A')}, ID: {market.get('id', 'N/A')}){Style.RESET_ALL}")
        
        if not set_leverage(exchange, symbol_unified, CONFIG.leverage):
            raise RuntimeError(f"Failed to set leverage to {CONFIG.leverage}x for {symbol_unified}. Halting.")

        logger.info(f"{Fore.MAGENTA}--- Key Spell Configuration Summary ---{Style.RESET_ALL}")
        logger.info(f"Trading Symbol: {symbol_unified}, Candle Interval: {CONFIG.interval}, Leverage: {CONFIG.leverage}x")
        ...
        logger.info(f"{Fore.MAGENTA}{'-'*60}{Style.RESET_ALL}") # Adjusted separator length

        market_base_name = symbol_unified.split("/")[0].split(":")[0] 
        send_sms_alert(f"[{market_base_name}/{CONFIG.strategy_name.value}] Pyrmethus v2.3.0 Initialized. Symbol: {symbol_unified}. Starting trading loop.")

        while run_bot:
            cycle_start_time = time.monotonic()
            cycle_count += 1
            logger.debug(f"{Fore.CYAN}--- Cycle {cycle_count} Start ({time.strftime('%H:%M:%S %Z')}) ---{Style.RESET_ALL}")
            ...
            try:
                ...
                data_limit = max(100, max(indicator_lookbacks) if indicator_lookbacks else 100) + CONFIG.api_fetch_limit_buffer + 20 
                
                df_market_data = get_market_data(exchange, symbol_unified, CONFIG.interval, limit=data_limit)
                
                if df_market_data is notUnion[Decimal, pd.NAType, None] and not df_market_data.empty:
                    trade_logic(exchange, symbol_unified, df_market_data) 
                else:
                    logger.warning(f"{Fore.YELLOW}Skipping trade logic for cycle {cycle_count}: Invalid or missing market data for {symbol_unified}.{Style.RESET_ALL}")
            
            except ccxt.RateLimitExceeded as e_ratelimit:
                logger.warning(f"{Back.YELLOW}{Fore.BLACK}Rate Limit Exceeded: {e_ratelimit}. Sleeping for an extended duration...{Style.RESET_ALL}")
                time.sleep(CONFIG.sleep_seconds * 6) 
            except (ccxt.NetworkError, ccxt.ExchangeNotAvailable, ccxt.RequestTimeout) as e_network: 
                logger.warning(f"{Fore.YELLOW}Network/Exchange Connectivity Issue: {e_network}. Retrying after a pause.{Style.RESET_ALL}")
                sleep_multiplier = 6 if isinstance(e_network, ccxt.ExchangeNotAvailable) else 3
                time.sleep(CONFIG.sleep_seconds * sleep_multiplier)
            except ccxt.AuthenticationError as e_auth: 
                logger.critical(f"{Back.RED}{Fore.WHITE}FATAL: Authentication Error: {e_auth}. API keys may be invalid or revoked. Stopping bot.{Style.RESET_ALL}")
                send_sms_alert(f"[{market_base_name}/{CONFIG.strategy_name.value}] CRITICAL: Auth Error! Bot stopping. Check API keys.")
                run_bot = False 
            except Exception as e_mainloop: 
                logger.exception(f"{Back.RED}{Fore.WHITE}!!! UNEXPECTED CRITICAL ERROR in Main Loop (Cycle {cycle_count}): {e_mainloop} !!! Attempting to stop gracefully.{Style.RESET_ALL}")
                send_sms_alert(f"[{market_base_name}/{CONFIG.strategy_name.value}] CRITICAL UNEXPECTED ERROR: {type(e_mainloop).__name__}! Bot stopping."); 
                run_bot = False 

            if run_bot: 
                elapsed_cycle_time = time.monotonic() - cycle_start_time
                sleep_duration = max(0, CONFIG.sleep_seconds - elapsed_cycle_time)
                logger.debug(f"Cycle {cycle_count} processed in {elapsed_cycle_time:.2f}s. Sleeping for {sleep_duration:.2f}s until next cycle.")
