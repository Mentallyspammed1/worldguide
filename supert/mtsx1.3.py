...
class Config:
    def __init__(self) -> None:
        ...
        self.min_usdt_balance_for_trading: Decimal = self._get_env("MIN_USDT_BALANCE_FOR_TRADING", "20.0", cast_type=Decimal) # Pyrmethus: New Ward
        self.atr_stop_loss_multiplier: Decimal = self._get_env("ATR_STOP_LOSS_MULTIPLIER", "1.5", cast_type=Decimal)
        self.enable_take_profit: bool = self._get_env("ENABLE_TAKE_PROFIT", "true", cast_type=bool)
        self.atr_take_profit_multiplier: Decimal = self._get_env("ATR_TAKE_PROFIT_MULTIPLIER", "2.0", cast_type=Decimal)
        ...
        self.max_active_trade_parts: int = self._get_env("MAX_ACTIVE_TRADE_PARTS", 1, cast_type=int)
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.min_usdt_balance_for_trading < 0: errors.append("MIN_USDT_BALANCE_FOR_TRADING cannot be negative.") # Pyrmethus: New validation
...
if sys.stdout.isatty():
    ...
    for level, color_code in [(logging.DEBUG, NEON['DEBUG']), (logging.INFO, NEON['INFO']), (SUCCESS_LEVEL, NEON['SUCCESS']),
        ...
        if '\033' in level_name: 
            ...
            for part_str in base_name_parts: # Renamed `part` to `part_str` to avoid conflict
                if not (part_str.startswith('[') and part_str.endswith('m')): original_name = part_str; break
...
def load_persistent_state() -> bool:
    ...
    try:
        ...
        if saved_major_minor != current_major_minor:
            logger.warning(f"{NEON['WARNING']}Phoenix Feather: Scroll version '{saved_version}' (Major.Minor: {saved_major_minor}) "
             ...
             logger.info(f"{NEON['INFO']}Phoenix Feather: Scroll patch version '{saved_version}' differs from current '{PYRMETHUS_VERSION}'. Proceeding.{NEON['RESET']}")
        ...
        for part_data_str_values in loaded_parts_raw:
            ...
            for k, v_loaded in part_data_str_values.items():
                if k in decimal_keys_in_part:
                    ...
                    if restored_part[k] is None and v_loaded is not None and v_loaded != "None":
...
def calculate_position_size(balance: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, sl_price: Decimal, market_info: Dict) -> Optional[Decimal]:
    ...
    if quantity_base <= CONFIG.position_qty_epsilon: 
        logger.warning(f"{NEON['WARNING']}Calculated qty {quantity_base} is zero/negligible after step adjustment. No order.{NEON['RESET']}"); return None
    if min_qty is not None and quantity_base < min_qty: 
        logger.warning(f"{NEON['WARNING']}Calculated qty {NEON['QTY']}{quantity_base}{NEON['WARNING']} < exchange min {NEON['QTY']}{min_qty}{NEON['WARNING']}. No order.{NEON['RESET']}"); return None # Pyrmethus: Clearer log
    
    qty_display_precision = abs(qty_step.as_tuple().exponent) if qty_step and qty_step.as_tuple().exponent < 0 else 0
...
def _synchronize_part_sltp(new_sl: Optional[Decimal], new_tp: Optional[Decimal]):
    ...
    for p in _active_trade_parts:
        if new_sl is not None: p['sl_price'] = new_sl
        if new_tp is not None: p['tp_price'] = new_tp
        elif not CONFIG.enable_take_profit: p['tp_price'] = None # Ensure TP is cleared if disabled
...
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price_target: Decimal, sl_price: Decimal, tp_price: Optional[Decimal], atr_val: Optional[Decimal], df_for_entry_context: Optional[pd.DataFrame] = None) -> Optional[Dict]:
    ...
    if balance is None or balance < config.min_usdt_balance_for_trading: # Pyrmethus: Check against configurable min balance
        logger.error(f"{NEON['ERROR']}Insufficient treasury ({balance}, Min Required: {config.min_usdt_balance_for_trading}). Cannot order.{NEON['RESET']}"); return None
    ...
    try:
        ...
        if filled_order.get('status') == 'closed' or (order_type_str == 'Market' and filled_order.get('filled', 0) > 0):
            ...
            new_part = {
                ...
                "breakeven_set": False, "recent_pnls": deque(maxlen=config.profit_momentum_window), 
                "last_known_pnl": Decimal(0), "adverse_candle_closes": deque(maxlen=config.last_chance_consecutive_adverse_candles),
                "partial_tp_taken_flags": {}, "divergence_exit_taken": False,
            }
            ...
            _synchronize_part_sltp(position_sl_price, position_tp_price) # Pyrmethus: Sync all parts to new position SL/TP
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part_to_close: Dict, reason: str, close_price_target: Optional[Decimal] = None) -> bool:
    global _active_trade_parts
    part_id = part_to_close.get('part_id', 'UnknownPart'); part_qty_to_close = part_to_close.get('qty', Decimal(0)); part_side = part_to_close.get('side') # Renamed part_qty to part_qty_to_close
    logger.info(f"{NEON['ACTION']}Unraveling part {part_id} ({part_side} {NEON['QTY']}{part_qty_to_close}{NEON['RESET']}) for: {reason}{NEON['RESET']}")
    if not isinstance(part_qty_to_close, Decimal) or part_qty_to_close <= CONFIG.position_qty_epsilon:
        logger.warning(f"{NEON['WARNING']}Cannot unravel part {part_id}: qty {part_qty_to_close} invalid.{NEON['RESET']}")
    ...
    try:
        ...
        current_total_qty, _ = get_current_position_info() # Get total qty before this part is conceptually removed
        is_closing_entire_position = (current_total_qty - part_qty_to_close) <= CONFIG.position_qty_epsilon

        logger.info(f"Casting MARKET {close_side_verb.upper()} order for part {part_id}: Qty {NEON['QTY']}{part_qty_to_close}{NEON['RESET']}")
        close_order = exchange.create_order(config.symbol, 'market', close_side_verb, float(part_qty_to_close), None, params)
        ...
        if filled_close_order.get('status') == 'closed':
            ...
            trade_metrics.log_trade(symbol=config.symbol, side=part_side, entry_price=entry_price, exit_price=actual_exit_price,
                                    qty=part_qty_to_close, entry_time_ms=part_to_close['entry_time_ms'], exit_time_ms=exit_timestamp_ms,
            ...
            for p in _active_trade_parts:
                if p.get('part_id') == part_id:
                    p['qty'] -= part_qty_to_close # Reduce qty if it was a conceptual partial close of a part
            ...
            if not part_found_and_updated and any(p.get('part_id') == part_id for p in _active_trade_parts): # Should not happen if logic is correct
                 _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_id] # Fallback removal

            if not _active_trade_parts: # If all parts are now closed
                logger.info(f"{NEON['INFO']}All parts closed for position {config.symbol}. Exchange SL/TP should be auto-cancelled.{NEON['RESET']}")
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    if parts_to_close_copy:
        ...
        for part_data in parts_to_close_copy: # Use a different variable name
            if part_data.get('symbol') == config.symbol: 
                close_position_part(exchange, config, part_data, reason + " (Global Unraveling)")
    ...
    try:
        logger.info(f"Attempting to clear SL/TP for position {config.symbol} on exchange by setting to 0.")
        ...
        if hasattr(exchange, 'set_trading_stop'):
            ...
            logger.warning(f"Exchange {exchange.id} does not support set_trading_stop. SL/TP may need manual clearing or are auto-cleared with position.")
...
def modify_position_sl_tp(exchange: ccxt.Exchange, config: Config, new_sl: Optional[Decimal] = None, new_tp: Optional[Decimal] = None) -> bool:
    ...
    logger.info(f"{NEON['ACTION']}Modifying Position SL/TP: New SL {sl_to_set_on_exchange}, New TP {tp_to_set_on_exchange if config.enable_take_profit else 'Disabled'}{NEON['RESET']}")
    try:
        exchange.set_trading_stop(config.symbol, params=params_sl_tp)
        logger.success(f"{NEON['SUCCESS']}Position SL/TP modification successful. Exchange SL: {sl_to_set_on_exchange}, TP: {tp_to_set_on_exchange if config.enable_take_profit else 'N/A'}{NEON['RESET']}") # type: ignore [attr-defined]
        
        _synchronize_part_sltp(sl_to_set_on_exchange, tp_to_set_on_exchange if config.enable_take_profit else None)
        save_persistent_state(force_heartbeat=True)
        return True
    except ccxt.ExchangeError as e:
        logger.error(f"{NEON['ERROR']}Exchange error modifying Position SL/TP: {e}{NEON['RESET']}")
        if "Stop loss price is not valid" in str(e) or "Take profit price is not valid" in str(e):
             logger.warning(f"Position SL/TP rejected (likely too close to market or invalid). Original SL/TP may remain.")
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Unexpected error modifying Position SL/TP: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
    return False
        ...
        while True:
            ...
            try:
                ...
                if current_balance_iter is not None:
                    trade_metrics.set_initial_equity(current_balance_iter) # Also updates daily equity/trade counts
                    if current_balance_iter < config.min_usdt_balance_for_trading: # Pyrmethus: Prudence Ward
                        logger.warning(f"{NEON['WARNING']}Treasury ({current_balance_iter:.2f} {config.usdt_symbol}) below prudence ward ({config.min_usdt_balance_for_trading} {config.usdt_symbol}). Pausing trade considerations.{NEON['RESET']}")
                        time.sleep(config.sleep_seconds * 3); continue # Check less frequently if balance too low
                ...
                for part_iter in active_parts_copy: # Renamed part to part_iter
                    if part_iter.get('symbol') != config.symbol: continue 
                    if config.enable_trend_contradiction_cooldown and part_iter.get('side') and not part_iter.get('contradiction_checked', False):
                        time_since_entry_ms = (time.time() * 1000) - part_iter['entry_time_ms']
                        ...
                        if time_since_entry_ms < check_window_ms and time_since_entry_ms > 0: 
                            ...
                            if (part_iter['side'] == config.pos_long and confirm_trend_now is False) or \
                               (part_iter['side'] == config.pos_short and confirm_trend_now is True):
                                logger.warning(f"{NEON['WARNING']}Trend contradiction for part {part_iter['part_id']} shortly after entry. Cooldown activated.{NEON['RESET']}")
                    ...
                    if part_iter['side'] == config.pos_long: current_unrealized_pnl = (latest_close - part_iter['entry_price']) * part_iter['qty']
                    elif part_iter['side'] == config.pos_short: current_unrealized_pnl = (part_iter['entry_price'] - latest_close) * part_iter['qty']
                    part_iter.get('recent_pnls', deque()).append(current_unrealized_pnl) 
                    if config.enable_profit_momentum_sl_tighten and len(part_iter.get('recent_pnls',[])) == config.profit_momentum_window:
                        ...
                        if is_currently_profitable and is_profit_increasing and latest_atr > 0:
                            original_atr_at_entry = part_iter.get('atr_at_entry', latest_atr)
                            tightened_sl_dist = (original_atr_at_entry * config.atr_stop_loss_multiplier) * config.profit_momentum_sl_tighten_factor
                            new_sl_price_candidate = (latest_close - tightened_sl_dist) if part_iter['side'] == config.pos_long else (latest_close + tightened_sl_dist)
                            current_sl = part_iter['sl_price']
                            if (part_iter['side'] == config.pos_long and new_sl_price_candidate > current_sl and new_sl_price_candidate < latest_close) or \
                               (part_iter['side'] == config.pos_short and new_sl_price_candidate < current_sl and new_sl_price_candidate > latest_close):
                                logger.info(f"{NEON['ACTION']}Profit Momentum: Attempting SL tighten for part {part_iter['part_id']} to {new_sl_price_candidate:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}{NEON['RESET']}")
                                modify_position_sl_tp(exchange, config, new_sl=new_sl_price_candidate) 
                    if config.enable_breakeven_sl and not part_iter.get('breakeven_set', False) and latest_atr > 0:
                        current_profit_atr = (current_unrealized_pnl / part_iter['qty']) / latest_atr if part_iter['qty'] > 0 and latest_atr > 0 else Decimal(0)
                        ...
                        if atr_target_met and abs_pnl_target_met:
                            ...
                            current_sl = part_iter['sl_price']
                            if (part_iter['side'] == config.pos_long and new_sl_price_candidate > current_sl) or \
                               (part_iter['side'] == config.pos_short and new_sl_price_candidate < current_sl):
                                logger.info(f"{NEON['ACTION']}Breakeven SL: Attempting for part {part_iter['part_id']} to {new_sl_price_candidate}{NEON['RESET']}")
                                if modify_position_sl_tp(exchange, config, new_sl=new_sl_price_candidate):
                                    part_iter['breakeven_set'] = True 
                    part_iter.get('adverse_candle_closes', deque()).append(latest_close)
                    if config.enable_last_chance_exit and latest_atr > 0 and len(part_iter.get('adverse_candle_closes',[])) == config.last_chance_consecutive_adverse_candles:
                        adverse_closes_list = list(part_iter['adverse_candle_closes'])
                        is_cons_adverse = (all(adverse_closes_list[i] < adverse_closes_list[i-1] for i in range(1, len(adverse_closes_list))) if part_iter['side'] == config.pos_long else
                        ...
                        if is_cons_adverse:
                            ...
                            if dist_to_sl <= sl_prox_thresh_price:
                                logger.warning(f"{NEON['WARNING']}Last Chance Exit: Part {part_iter['part_id']} ({dist_to_sl:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}} <= {sl_prox_thresh_price:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}).{NEON['RESET']}")
                                if close_position_part(exchange, config, part_iter, "Last Chance Preemptive Exit"): continue 
                    if config.strategy_name == StrategyName.EHLERS_FISHER and config.ehlers_enable_divergence_scaled_exit and \
                       not part_iter.get('divergence_exit_taken', False) and 'entry_fisher_value' in part_iter and part_iter['entry_fisher_value'] is not None:
                        ...
                        if fisher_now is not pd.NA and signal_now is not pd.NA:
                            entry_fisher = part_iter['entry_fisher_value']; entry_signal = part_iter.get('entry_signal_value', pd.NA) 
                            if entry_fisher is not pd.NA and entry_signal is not pd.NA:
                                ...
                                if part_iter['side'] == config.pos_long and fisher_now > signal_now and fisher_now < entry_fisher and current_spread < (initial_spread * config.ehlers_divergence_threshold_factor):
                                    divergence_met = True
                                elif part_iter['side'] == config.pos_short and fisher_now < signal_now and fisher_now > entry_fisher and current_spread < (initial_spread * config.ehlers_divergence_threshold_factor):
                                    divergence_met = True
                                if divergence_met:
                                    qty_to_close_f = part_iter['qty'] * config.ehlers_divergence_exit_percentage
                                    ...
                                    if qty_to_close > config.position_qty_epsilon:
                                        logger.info(f"{NEON['ACTION']}Ehlers Divergence Exit ({config.ehlers_divergence_exit_percentage*100}%): Part {part_iter['part_id']}, Qty {qty_to_close}{NEON['RESET']}")
                                        ...
                                        if close_position_part(exchange, config, temp_part_to_close, "Ehlers Divergence Partial Exit"):
                                            ...
                                            if part_iter['qty'] <= config.position_qty_epsilon: 
                                                _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_iter['part_id']]
                                                logger.info(f"Part {part_iter['part_id']} fully closed due to negligible remainder after partial Ehlers div exit.")
                                            save_persistent_state(force_heartbeat=True)
                                            continue 
                                    else: logger.debug(f"Ehlers div exit: calculated qty_to_close {qty_to_close} too small.")
                    if (part_iter['side'] == config.pos_long and signals.get("exit_long")) or \
                       (part_iter['side'] == config.pos_short and signals.get("exit_short")):
                        exit_reason = signals.get('exit_reason', 'Strategy Exit Signal')
                        logger.info(f"{NEON['ACTION']}Omen to EXIT {part_iter['side']} for {config.symbol}: {exit_reason}{NEON['RESET']}")
                        if close_position_part(exchange, config, part_iter, exit_reason): continue 
                    if part_iter.get('sl_price') is not None:
                        if (part_iter['side'] == config.pos_long and latest_close <= part_iter['sl_price']) or \
                           (part_iter['side'] == config.pos_short and latest_close >= part_iter['sl_price']):
                            logger.warning(f"{NEON['WARNING']}Part {part_iter['part_id']} SL price {part_iter['sl_price']} potentially breached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}")
                    if part_iter.get('tp_price') is not None and config.enable_take_profit:
                        if (part_iter['side'] == config.pos_long and latest_close >= part_iter['tp_price']) or \
                           (part_iter['side'] == config.pos_short and latest_close <= part_iter['tp_price']):
                            logger.info(f"{NEON['INFO']}Part {part_iter['part_id']} TP price {part_iter['tp_price']} potentially reached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}")
