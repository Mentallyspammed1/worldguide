...
# ██║        ██║   ███████║██║ ╚═╝ ██║███████║   ██║   ╚██████╔╝╚██████╔╝███████║
# ╚═╝        ╚═╝   ╚══════╝╚═╝     ╚═╝╚══════╝   ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝
# Pyrmethus - Unified Scalping Spell v2.9.1 (Wards of Prudence & Precision)
# Enhancements: Min balance check, SL/TP state sync across parts, refined logging.
# Previous Weave: v2.9.0 (Aegis of Dynamic Wards)

"""
High-Frequency Trading Bot (Scalping) for Bybit USDT Futures
Version: 2.9.1 (Wards of Prudence & Precision) - Enhanced by Pyrmethus
...
Enhancements in v2.9.1 (by Pyrmethus):
- Minimum Balance Check:
    - Added `MIN_USDT_BALANCE_FOR_TRADING` configuration.
    - Bot pauses if current balance is below this threshold.
- SL/TP State Synchronization:
    - When SL/TP is modified for the position (either at order placement or through dynamic adjustments),
      the `sl_price` and `tp_price` fields are updated across all internally tracked `_active_trade_parts`
      to maintain consistent state reflecting the overall position's SL/TP.
- Refined Logging:
    - Clearer log messages for order placement failures due to insufficient balance or quantity issues.
- Configuration Validation:
    - Added validation for `MIN_USDT_BALANCE_FOR_TRADING`.
"""
...
PYRMETHUS_VERSION = "2.9.1"  # Pyrmethus: Version bump for new enhancements
...
class Config:
    def __init__(self) -> None:
        ...
        self.min_usdt_balance_for_trading: Decimal = self._get_env(
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.min_usdt_balance_for_trading < 0:
            errors.append(
                "MIN_USDT_BALANCE_FOR_TRADING cannot be negative."
            )  # Pyrmethus: New validation
        ...
        if self.enable_anti_martingale_risk:
            ...
            if not (0 < self.max_risk_pct_anti_martingale < 1):
                errors.append("MAX_RISK_PCT_ANTI_MARTINGALE must be between 0 and 1.")
        if errors:
            error_message = (
...
if sys.stdout.isatty():
    ...
    for level, color_code in [
        ...
        if "\033" in level_name:
            ...
            for (
                part_str
            ) in base_name_parts:  # Renamed `part` to `part_str` to avoid conflict
                if not (part_str.startswith("[") and part_str.endswith("m")):
                    original_name = part_str
                    break
            if not original_name:
                original_name = (
                    level_name.split("m")[-1] if "m" in level_name else level_name
                )
    ...
    def generate_signals(
        ...
        min_rows_needed = (
            max(
                ...
                self.config.atr_calculation_period,
            )
            + 10
        )
        ...
        if primary_long_flip and primary_short_flip:
            self.logger.warning(
                f"{NEON['WARNING']}Conflicting primary Supertrend flips. Resolving...{NEON['RESET']}"
            )
        ...
        if (
            ...
            self.logger.info(
                f"{NEON['SIDE_LONG']}DualST+Mom Signal: LONG Entry.{NEON['RESET']}"
            )
        elif (
            primary_short_flip
            ...
            self.logger.info(
                f"{NEON['SIDE_SHORT']}DualST+Mom Signal: SHORT Entry.{NEON['RESET']}"
            )
    ...
    def generate_signals(
        ...
        if not is_fisher_extreme:
            if fisher_prev <= signal_prev and fisher_now > signal_now:
                signals["enter_long"] = True
                self.logger.info(
                    f"{NEON['SIDE_LONG']}EhlersFisher Signal: LONG Entry.{NEON['RESET']}"
                )
            elif fisher_prev >= signal_prev and fisher_now < signal_now:
                signals["enter_short"] = True
                self.logger.info(
                    f"{NEON['SIDE_SHORT']}EhlersFisher Signal: SHORT Entry.{NEON['RESET']}"
                )
...
class TradeMetrics:
    ...
    def log_trade(
        ...
        if not all(
            ...
            self.logger.warning(
                f"{NEON['WARNING']}Trade log skipped due to flawed parameters for Part ID: {part_id}.{NEON['RESET']}"
            )
            return
    ...
    def load_trades_from_list(self, trades_list: List[Dict[str, Any]]):
        ...
        self.logger.info(
            f"{NEON['INFO']}TradeMetrics: Re-inked {len(self.trades)} sagas.{NEON['RESET']}"
        )
...
def load_persistent_state() -> bool:
    ...
    if not os.path.exists(STATE_FILE_PATH):
        logger.info(
            f"{NEON['INFO']}Phoenix Feather: No previous scroll.{NEON['RESET']}"
        )
        return False
    try:
        ...
        # Pyrmethus: More flexible version check (major.minor must match, patch can differ)
        ...
        if saved_major_minor != current_major_minor:
            logger.warning(
                f"{NEON['WARNING']}Phoenix Feather: Scroll version '{saved_version}' (Major.Minor: {saved_major_minor}) "
                f"differs significantly from current '{PYRMETHUS_VERSION}' (Major.Minor: {current_major_minor}). "
                f"Caution during reanimation or consider starting fresh.{NEON['RESET']}"
            )
        elif saved_version != PYRMETHUS_VERSION:
            logger.info(
                f"{NEON['INFO']}Phoenix Feather: Scroll patch version '{saved_version}' differs from current '{PYRMETHUS_VERSION}'. Proceeding.{NEON['RESET']}"
            )
        ...
        if (
            ...
            logger.warning(
                f"{NEON['WARNING']}Phoenix Scroll sigils mismatch! Current: {CONFIG.symbol}/{CONFIG.strategy_name.value}, Scroll: {state_data.get('config_symbol')}/{state_data.get('config_strategy')}. Ignoring scroll.{NEON['RESET']}"
            )
            return False
            ...
            for k, v_loaded in part_data_str_values.items():
                if k in decimal_keys_in_part:
                    ...
                    if (
                        ...
                        logger.warning(
                            f"Failed to restore Decimal {k} for part {part_data_str_values.get('part_id')}, value was '{v_loaded}'"
                        )
                elif k in deque_keys_in_part:
                    if isinstance(v_loaded, list):
                        restored_part[k] = deque(v_loaded, maxlen=deque_keys_in_part[k])
                    else:
                        restored_part[k] = deque(maxlen=deque_keys_in_part[k])
                        logger.warning(
                            f"Expected list for deque {k}, got {type(v_loaded)}"
                        )
                    ...
                    if isinstance(v_loaded, (int, float)):
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Malformed entry_time_ms '{v_loaded}'. Skipping part."
                                )
                                valid_part = False
                                break
                    else:
                        logger.warning(
                            f"Unexpected type for entry_time_ms '{v_loaded}'. Skipping part."
                        )
                        valid_part = False
                        break
                else:
        ...
        if trade_metrics.last_daily_reset_day is not None:
            try:
                ...
                logger.warning("Malformed last_daily_reset_day.")
        ...
        previous_day_high = (
            safe_decimal_conversion(prev_high_str, None) if prev_high_str else None
        )
        prev_low_str = state_data.get("previous_day_low_str")
        previous_day_low = (
            safe_decimal_conversion(prev_low_str, None) if prev_low_str else None
        )
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    if df.empty:
        logger.warning(f"{NEON['WARNING']}Market data scroll empty.{NEON['RESET']}")
        return df.copy()
    for col in ["close", "high", "low", "volume"]:
        if col in df.columns:
            ...
            logger.error(
                f"{NEON['ERROR']}Essential rune '{col}' missing.{NEON['RESET']}"
            )
    ...
    if config.enable_trap_filter:
        if "high" in df.columns and not df["high"].isnull().all():
            df[f"rolling_high_{config.trap_filter_lookback_period}"] = (
                df["high"]
                .rolling(window=config.trap_filter_lookback_period, min_periods=1)
                .max()
            )
        else:
            df[f"rolling_high_{config.trap_filter_lookback_period}"] = pd.NA
        if "low" in df.columns and not df["low"].isnull().all():
            df[f"rolling_low_{config.trap_filter_lookback_period}"] = (
                df["low"]
                .rolling(window=config.trap_filter_lookback_period, min_periods=1)
                .min()
            )
        else:
            df[f"rolling_low_{config.trap_filter_lookback_period}"] = pd.NA
    return df
...
def get_market_data(
    ...
    try:
        ...
        if not ohlcv:
            logger.warning(
                f"{NEON['WARNING']}No OHLCV runes for {symbol}.{NEON['RESET']}"
            )
            return None
        df = pd.DataFrame(
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Market realm rejects symbol '{symbol}': {e_bs}.{NEON['RESET']}"
        )
        return None
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error summoning market whispers: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        return None
...
def fetch_account_balance(
    ...
    try:
        ...
        if (
            ...
            if (
                balance_data["info"]["result"]["list"]
                and "totalEquity" in balance_data["info"]["result"]["list"][0]
            ):
                total_balance_val = balance_data["info"]["result"]["list"][0][
                    "totalEquity"
                ]
                logger.info(
        ...
        if total_balance is None:
            logger.warning(
                f"{NEON['WARNING']}{currency_code} balance rune unreadable.{NEON['RESET']}"
            )
            return None
        logger.info(
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    total_qty = sum(
        ...
        and part.get("side") == first_part.get("side")
    )
    if total_qty <= CONFIG.position_qty_epsilon:
        if total_qty < Decimal(0):
            logger.error(
                f"{NEON['ERROR']}Negative total quantity {total_qty}. State corrupted! Clearing parts.{NEON['RESET']}"
            )
        _active_trade_parts.clear()
    ...
    if current_side_str not in [CONFIG.pos_long, CONFIG.pos_short]:
        logger.warning(
            f"{NEON['WARNING']}Incoherent side '{current_side_str}'. Assuming None. Clearing parts.{NEON['RESET']}"
        )
        _active_trade_parts.clear()
        save_persistent_state(force_heartbeat=True)
        return CONFIG.pos_none, Decimal(0)
...
def calculate_position_size(
    ...
    if not all(
        [
            ...
            market_info
            and "precision" in market_info
            and "amount" in market_info["precision"]
            and "limits" in market_info
            and "amount" in market_info["limits"]
            and "min" in market_info["limits"]["amount"],
        ]
    ):
        logger.warning(
            f"{NEON['WARNING']}Invalid runes for sizing. Bal: {balance}, Entry: {entry_price}, SL: {sl_price}, MarketInfo: {'Present' if market_info else 'Absent'}{NEON['RESET']}"
        )
        return None
    risk_per_unit = abs(entry_price - sl_price)
    if risk_per_unit <= Decimal(0):
        logger.warning(
            f"{NEON['WARNING']}Risk per unit zero or negative. Cannot divine size.{NEON['RESET']}"
        )
        return None
    ...
    if position_usdt_value > CONFIG.max_order_usdt_amount:
        logger.info(
            f"Calculated position USDT value {position_usdt_value:.2f} exceeds MAX_ORDER_USDT_AMOUNT. Capping."
        )
    ...
    if qty_step is None or qty_step <= Decimal(0):
        logger.warning(
            f"{NEON['WARNING']}Qty step invalid for {market_info.get('symbol')}. Using raw quantity.{NEON['RESET']}"
        )
    else:
        quantity_base = (
            (quantity_base // qty_step) * qty_step
            if quantity_base >= qty_step
            else Decimal(0)
        )
    if quantity_base <= CONFIG.position_qty_epsilon:
        logger.warning(
            f"{NEON['WARNING']}Calculated qty {quantity_base} is zero/negligible after step adjustment. No order.{NEON['RESET']}"
        )
        return None
    if min_qty is not None and quantity_base < min_qty:
        logger.warning(
            f"{NEON['WARNING']}Calculated qty {NEON['QTY']}{quantity_base}{NEON['WARNING']} < exchange min {NEON['QTY']}{min_qty}{NEON['WARNING']}. No order.{NEON['RESET']}"
        )
        return None  # Pyrmethus: Clearer log

    qty_display_precision = (
        abs(qty_step.as_tuple().exponent)
        if qty_step and qty_step.as_tuple().exponent < 0
        else 0
    )
    logger.info(
        f"Calculated position size: {NEON['QTY']}{quantity_base:.{qty_display_precision}f}{NEON['RESET']} (USDT Value: {position_usdt_value:.2f})"
    )
    return quantity_base


def _synchronize_part_sltp(new_sl: Optional[Decimal], new_tp: Optional[Decimal]):
    # Pyrmethus: Helper to update all active parts with the new position-wide SL/TP
    global _active_trade_parts
    for p in _active_trade_parts:
        ...
        if new_tp is not None:
            p["tp_price"] = new_tp
        elif not CONFIG.enable_take_profit:
            p["tp_price"] = None  # Ensure TP is cleared if disabled
...
def place_risked_order(
    ...
    global \
        ...
        whipsaw_cooldown_active_until
    logger.info(
        f"{NEON['ACTION']}Attempting {side.upper()} order for {config.symbol}... Target: {NEON['PRICE']}{entry_price_target}{NEON['RESET']}, SL: {NEON['PRICE']}{sl_price}{NEON['RESET']}{f', TP: {NEON["PRICE"]}{tp_price}{NEON["RESET"]}' if tp_price else ''}"
    )
    ...
    if config.enable_anti_martingale_risk and trade_metrics.trades:
        ...
        if last_trade_pnl < 0:
            ...
            logger.info(
                f"{NEON['INFO']}Anti-Martingale: Risk reduced to {effective_risk_pct_for_trade * 100:.3f}% (prev loss).{NEON['RESET']}"
            )
            ...
            logger.info(
                f"{NEON['INFO']}Anti-Martingale: Risk adjusted to {effective_risk_pct_for_trade * 100:.3f}% (prev win).{NEON['RESET']}"
            )
    balance = fetch_account_balance(exchange, config.usdt_symbol)
    if (
        balance is None or balance < config.min_usdt_balance_for_trading
    ):  # Pyrmethus: Check against configurable min balance
        logger.error(
            f"{NEON['ERROR']}Insufficient treasury ({balance}, Min Required: {config.min_usdt_balance_for_trading}). Cannot order.{NEON['RESET']}"
        )
        return None
    if (
        ...
        logger.error(
            f"{NEON['ERROR']}Market runes (info, tick_size, qty_step) not divined. Cannot order.{NEON['RESET']}"
        )
        return None
    ...
    if quantity is None or quantity <= CONFIG.position_qty_epsilon:
        logger.warning(
            f"{NEON['WARNING']}Position sizing yielded no quantity.{NEON['RESET']}"
        )
        return None
        ...
        if config.enable_limit_order_price_improvement_check:
            try:
                ...
                if (
                    ...
                    logger.info(
                        f"{NEON['INFO']}Price Improve: Ask ({current_best_ask}) <= Limit ({calculated_limit_price}). Adjusting.{NEON['RESET']}"
                    )
                    ...
                    logger.info(
                        f"{NEON['INFO']}Price Improve: Bid ({current_best_bid}) >= Limit ({calculated_limit_price}). Adjusting.{NEON['RESET']}"
                    )
                    calculated_limit_price = current_best_bid - config.tick_size
            except Exception as e_ticker:
                logger.warning(
                    f"{NEON['WARNING']}Ticker fetch fail for price improve: {e_ticker}{NEON['RESET']}"
                )
    ...
    try:
        ...
        logger.success(
            f"{NEON['SUCCESS']}Entry Order {order['id']} ({order.get('status', 'N/A')}) cast.{NEON['RESET']}"
        )  # type: ignore [attr-defined]
        config.send_notification_method(
            f"Pyrmethus Order: {side.upper()}",
            f"{config.symbol} Qty: {quantity:.4f} @ {order_price_param_for_call if order_type_str == 'Limit' else 'Market'}, SL: {sl_price}{f', TP: {tp_price}' if tp_price else ''}",
        )
        time.sleep(config.order_fill_timeout_seconds / 3)
        filled_order = exchange.fetch_order(order["id"], config.symbol)
        if filled_order.get("status") == "closed" or (
            order_type_str == "Market" and filled_order.get("filled", 0) > 0
        ):
            ...
            # Pyrmethus: The SL/TP set here becomes the new position-wide SL/TP
            position_sl_price = sl_price
            position_tp_price = tp_price if config.enable_take_profit else None
            new_part = {
                "part_id": part_id,
                ...
                "sl_price": position_sl_price,
                "tp_price": position_tp_price,
            ...
            _synchronize_part_sltp(
                position_sl_price, position_tp_price
            )  # Pyrmethus: Sync all parts to new position SL/TP
            ...
            if config.enable_whipsaw_cooldown:
                ...
                if (
                    ...
                    logger.critical(
                        f"{NEON['CRITICAL']}Whipsaw detected! Pausing for {config.whipsaw_cooldown_seconds}s.{NEON['RESET']}"
                    )
                    config.send_notification_method(
                        "Pyrmethus Whipsaw",
                        f"Pausing for {config.whipsaw_cooldown_seconds}s.",
                    )
                    whipsaw_cooldown_active_until = (
            ...
            save_persistent_state(force_heartbeat=True)
            logger.success(
                f"{NEON['SUCCESS']}Order {order['id']} filled! Part {part_id}. Entry: {NEON['PRICE']}{actual_entry_price}{NEON['RESET']}, Qty: {NEON['QTY']}{actual_qty}{NEON['RESET']}"
            )  # type: ignore [attr-defined]
            config.send_notification_method(
            ...
            logger.warning(
                f"{NEON['WARNING']}Order {order['id']} not fully 'closed'. Status: {filled_order.get('status', 'N/A')}, Filled: {filled_order.get('filled', 0)}. Cancelling if open.{NEON['RESET']}"
            )
            if filled_order.get("status") == "open":
                try:
                    exchange.cancel_order(order["id"], config.symbol)
                    logger.info(
                        f"Cancelled unfilled/partially-filled limit order {order['id']}."
                    )
                except Exception as e_cancel:
                    logger.error(f"Error cancelling order {order['id']}: {e_cancel}")
            return None
    except ccxt.InsufficientFunds as e:
        logger.error(f"{NEON['ERROR']}Insufficient funds: {e}{NEON['RESET']}")
        return None
    except ccxt.ExchangeError as e:
        logger.error(f"{NEON['ERROR']}Exchange rejected order: {e}{NEON['RESET']}")
        return None
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Unexpected chaos weaving order: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        return None
...
def close_position_part(
    ...
    part_qty_to_close = part_to_close.get("qty", Decimal(0))
    part_side = part_to_close.get("side")  # Renamed part_qty to part_qty_to_close
    logger.info(
        f"{NEON['ACTION']}Unraveling part {part_id} ({part_side} {NEON['QTY']}{part_qty_to_close}{NEON['RESET']}) for: {reason}{NEON['RESET']}"
    )
    if (
        not isinstance(part_qty_to_close, Decimal)
        or part_qty_to_close <= CONFIG.position_qty_epsilon
    ):
        logger.warning(
            f"{NEON['WARNING']}Cannot unravel part {part_id}: qty {part_qty_to_close} invalid.{NEON['RESET']}"
        )
        _active_trade_parts = [
    ...
    if config.symbol.endswith(":USDT"):
        params["category"] = "linear"
    try:
        # Pyrmethus: If this close action results in closing the entire logical position,
        # the exchange will automatically cancel associated SL/TP orders.
        # If it's a partial close of a larger logical position (multiple parts),
        # the SL/TP on the exchange for the remaining position remains.
        current_total_qty, _ = (
            get_current_position_info()
        )  # Get total qty before this part is conceptually removed
        is_closing_entire_position = (
            current_total_qty - part_qty_to_close
        ) <= CONFIG.position_qty_epsilon

        logger.info(
            f"Casting MARKET {close_side_verb.upper()} order for part {part_id}: Qty {NEON['QTY']}{part_qty_to_close}{NEON['RESET']}"
        )
        close_order = exchange.create_order(
            config.symbol,
            "market",
            close_side_verb,
            float(part_qty_to_close),
            None,
            params,
        )
        logger.success(
            f"{NEON['SUCCESS']}Unraveling Order {close_order['id']} ({close_order.get('status', 'N/A')}) cast.{NEON['RESET']}"
        )  # type: ignore [attr-defined]
        ...
        if filled_close_order.get("status") == "closed":
            ...
            pnl = pnl_per_unit * part_qty_to_close
            trade_metrics.log_trade(
                symbol=config.symbol,
                side=part_side,
                entry_price=entry_price,
                exit_price=actual_exit_price,
                qty=part_qty_to_close,
            ...
            for p in _active_trade_parts:
                if p.get("part_id") == part_id:
                    p["qty"] -= (
                        part_qty_to_close  # Reduce qty if it was a conceptual partial close of a part
                    )
                    if p["qty"] > CONFIG.position_qty_epsilon:
                        temp_active_parts.append(p)  # Keep part if remaining qty
                    part_found_and_updated = True
                else:
                    temp_active_parts.append(p)
            _active_trade_parts = temp_active_parts

            if not part_found_and_updated and any(
                p.get("part_id") == part_id for p in _active_trade_parts
            ):  # Should not happen if logic is correct
                _active_trade_parts = [
                    p for p in _active_trade_parts if p.get("part_id") != part_id
                ]  # Fallback removal

            if not _active_trade_parts:  # If all parts are now closed
                logger.info(
                    f"{NEON['INFO']}All parts closed for position {config.symbol}. Exchange SL/TP should be auto-cancelled.{NEON['RESET']}"
                )
            ...
            logger.warning(
                f"{NEON['WARNING']}Unraveling order {close_order['id']} for part {part_id} not 'closed'. Status: {filled_close_order.get('status', 'N/A')}. Manual check.{NEON['RESET']}"
            )
            return False
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error unraveling part {part_id}: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        config.send_notification_method(
...
def cancel_all_symbol_orders(exchange: ccxt.Exchange, symbol: str):
    logger.info(
        f"{NEON['ACTION']}Dispelling all open orders (not position SL/TP) for {symbol}...{NEON['RESET']}"
    )
    try:
        params = {"category": "linear"} if symbol.endswith(":USDT") else {}
        ...
        for order in open_orders:
            # Pyrmethus: Be cautious not to cancel exchange-managed SL/TP orders unintentionally.
            # Bybit usually links SL/TP to the position rather than separate cancellable orders of type 'Stop' or 'TakeProfit'.
            # So, cancelling 'limit' orders should be safe.
            if order.get("type", "").lower() == "limit" and not order.get(
                "reduceOnly", False
            ):
        ...
        logger.warning(
            f"{NEON['WARNING']}Exchange {exchange.id} may not support cancel_all_orders or fetch_open_orders correctly.{NEON['RESET']}"
        )
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error dispelling enchantments: {e}{NEON['RESET']}"
        )
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    global _active_trade_parts
    logger.warning(
        f"{NEON['WARNING']}Forceful unraveling of ALL positions for {config.symbol} due to: {reason}{NEON['RESET']}"
    )
    ...
    if parts_to_close_copy:
        ...
        # Closing the first part (if MAX_ACTIVE_TRADE_PARTS=1) or all parts should trigger position close
        # For simplicity, if multiple parts form one logical position, closing them one by one via market reduceOnly orders
        # will eventually close the entire position.
        for part_data in parts_to_close_copy:  # Use a different variable name
            if part_data.get("symbol") == config.symbol:
                close_position_part(
                    exchange, config, part_data, reason + " (Global Unraveling)"
                )
    else:
        logger.info("No internally tracked trade parts to unravel via market orders.")
    logger.info(
        f"Safeguard scrying exchange for any residual {config.symbol} positions..."
    )
    try:
        params_fetch_pos = (
        ...
        for pos_data in positions:
            if pos_data and pos_data.get("symbol") == config.symbol:
                ...
                pos_side_str = pos_data.get("side")
                if pos_qty is not None and pos_qty > CONFIG.position_qty_epsilon:
                    ...
                    logger.warning(
                        f"Found residual exchange position: {pos_side_str} {pos_qty} {config.symbol}. Force market unravel."
                    )
    ...
    try:
        logger.info(
            f"Attempting to clear SL/TP for position {config.symbol} on exchange by setting to 0."
        )
        params_clear_sltp = {"positionIdx": 0, "stopLoss": "0", "takeProfit": "0"}
        if config.symbol.endswith(":USDT"):
            params_clear_sltp["category"] = "linear"
        # Only call if the exchange supports setTradingStop, which Bybit does.
        if hasattr(exchange, "set_trading_stop"):
            ...
            logger.warning(
                f"Exchange {exchange.id} does not support set_trading_stop. SL/TP may need manual clearing or are auto-cleared with position."
            )
    except Exception as e_clear_sl_tp:
        logger.warning(
            f"{NEON['WARNING']}Could not clear SL/TP on exchange for {config.symbol}: {e_clear_sl_tp}. May already be clear or not supported directly.{NEON['RESET']}"
        )
    _active_trade_parts.clear()
    save_persistent_state(force_heartbeat=True)
    logger.info(
...
def modify_position_sl_tp(
    ...
    # Pyrmethus: This function now modifies the position-wide SL/TP and syncs all internal parts.
    global _active_trade_parts
    if not _active_trade_parts:
    ...
    if not config.MARKET_INFO or not config.tick_size:
        logger.error(
            f"{NEON['ERROR']}Market info or tick_size not available. Cannot modify SL/TP.{NEON['RESET']}"
        )
        return False

    # Use the SL/TP from the first part as representative of the current position SL/TP, if not changing it.
    # This assumes all parts are synchronized.
    current_sl_on_part = _active_trade_parts[0].get("sl_price")
    current_tp_on_part = _active_trade_parts[0].get("tp_price")

    params_sl_tp = {"positionIdx": 0}
    if config.symbol.endswith(":USDT"):
        params_sl_tp["category"] = "linear"

    sl_to_set_on_exchange = new_sl if new_sl is not None else current_sl_on_part
    tp_to_set_on_exchange = new_tp if new_tp is not None else current_tp_on_part
    ...
    else:  # If no SL is desired or current_sl_on_part was None
    ...
    if config.enable_take_profit:
        ...
        else:  # If no TP is desired or current_tp_on_part was None
            params_sl_tp["takeProfit"] = "0"
    else:  # If TP is disabled in config, ensure it's set to 0 to cancel any existing.
        params_sl_tp["takeProfit"] = "0"
        tp_to_set_on_exchange = None  # Internal state should reflect TP disabled

    # Avoid API call if SL and TP are effectively '0' (cancel only) and were already None/0 internally.
    # This condition might be too complex; an API call to set to '0' is usually harmless.
    # Let's proceed if there's any change or explicit setting.

    logger.info(
        f"{NEON['ACTION']}Modifying Position SL/TP: New SL {sl_to_set_on_exchange}, New TP {tp_to_set_on_exchange if config.enable_take_profit else 'Disabled'}{NEON['RESET']}"
    )
    try:
        exchange.set_trading_stop(config.symbol, params=params_sl_tp)
        logger.success(
            f"{NEON['SUCCESS']}Position SL/TP modification successful. Exchange SL: {sl_to_set_on_exchange}, TP: {tp_to_set_on_exchange if config.enable_take_profit else 'N/A'}{NEON['RESET']}"
        )  # type: ignore [attr-defined]

        _synchronize_part_sltp(
            sl_to_set_on_exchange,
            tp_to_set_on_exchange if config.enable_take_profit else None,
        )
        save_persistent_state(force_heartbeat=True)
        return True
    except ccxt.ExchangeError as e:
        logger.error(
            f"{NEON['ERROR']}Exchange error modifying Position SL/TP: {e}{NEON['RESET']}"
        )
        if "Stop loss price is not valid" in str(
            e
        ) or "Take profit price is not valid" in str(e):
            logger.warning(
                f"Position SL/TP rejected (likely too close to market or invalid). Original SL/TP may remain."
            )
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Unexpected error modifying Position SL/TP: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
    return False
...
def update_daily_key_levels(exchange: ccxt.Exchange, config: Config):
    ...
    if last_key_level_update_day == today_utc_day:
        return
    logger.info("Attempting to update daily key levels (Prev Day H/L)...")
    try:
        two_days_ago_utc = datetime.now(pytz.utc).replace(
        ...
        daily_candles = exchange.fetch_ohlcv(
            config.symbol, "1d", since=since_timestamp_ms, limit=2, params=params
        )
        if daily_candles and len(daily_candles) >= 1:
            ...
            for candle_data in reversed(daily_candles):
                candle_ts_utc = datetime.fromtimestamp(
                    candle_data[0] / 1000, tz=pytz.utc
                )
                if candle_ts_utc.date() == yesterday_target_date:
                    ...
                    logger.info(
                        f"{NEON['INFO']}Updated key levels: Prev Day High {previous_day_high}, Prev Day Low {previous_day_low}{NEON['RESET']}"
                    )
            ...
            if not found_yesterday:
                logger.warning(
                    f"{NEON['WARNING']}Could not definitively find yesterday's candle data to update key levels.{NEON['RESET']}"
                )
        else:
            logger.warning(
...
def main_loop(exchange: ccxt.Exchange, config: Config) -> None:
    ...
    global contradiction_cooldown_active_until, consecutive_loss_cooldown_active_until
    logger.info(
        f"{NEON['HEADING']}=== Pyrmethus Spell v{PYRMETHUS_VERSION} Awakening on {exchange.id} ==={NEON['RESET']}"
    )
    ...
    try:
        while True:
            ...
            try:
                logger.debug(
                    f"{NEON['COMMENT']}# New cycle of observation ({loop_counter})...{NEON['RESET']}"
                )
                if (
                    config.enable_whipsaw_cooldown
                    and time.time() < whipsaw_cooldown_active_until
                ):
                    logger.warning(
                        f"{NEON['WARNING']}In whipsaw cooldown. Ends in {whipsaw_cooldown_active_until - time.time():.0f}s.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                if (
                    config.enable_trend_contradiction_cooldown
                    and time.time() < contradiction_cooldown_active_until
                ):
                    logger.warning(
                        f"{NEON['WARNING']}In Trend Contradiction cooldown. Ends in {contradiction_cooldown_active_until - time.time():.0f}s.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                if (
                    config.enable_daily_max_trades_rest
                    and trade_metrics.daily_trades_rest_active_until > time.time()
                ):
                    logger.warning(
                        f"{NEON['WARNING']}Resting (daily max trades). Resumes in {(trade_metrics.daily_trades_rest_active_until - time.time()) / 3600:.1f} hrs.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds * 10)
                    continue
                if (
                    config.enable_consecutive_loss_limiter
                    and time.time() < consecutive_loss_cooldown_active_until
                ):
                    logger.warning(
                        f"{NEON['WARNING']}In consecutive loss cooldown. Ends in {(consecutive_loss_cooldown_active_until - time.time()) / 60:.1f} mins.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                ...
                if current_balance_iter is not None:
                    ...
                    )  # Also updates daily equity/trade counts
                    if (
                        current_balance_iter < config.min_usdt_balance_for_trading
                    ):  # Pyrmethus: Prudence Ward
                        logger.warning(
                            f"{NEON['WARNING']}Treasury ({current_balance_iter:.2f} {config.usdt_symbol}) below prudence ward ({config.min_usdt_balance_for_trading} {config.usdt_symbol}). Pausing trade considerations.{NEON['RESET']}"
                        )
                        time.sleep(config.sleep_seconds * 3)
                        continue  # Check less frequently if balance too low
                ...
                if (
                    ...
                    if (
                        ...
                        logger.critical(
                            f"{NEON['SUCCESS']}SESSION PROFIT TARGET {config.session_profit_target_usdt} USDT REACHED! PNL: {current_session_pnl:.2f}.{NEON['RESET']}"
                        )
                        config.send_notification_method(
                            "Pyrmethus Session Goal!",
                            f"Profit target {config.session_profit_target_usdt} hit.",
                        )
                        close_all_symbol_positions(
                    ...
                    if (
                        ...
                        logger.critical(
                            f"{NEON['CRITICAL']}SESSION MAX LOSS {config.session_max_loss_usdt} USDT REACHED! PNL: {current_session_pnl:.2f}.{NEON['RESET']}"
                        )
                        config.send_notification_method(
                            "Pyrmethus Session Loss!",
                            f"Max session loss {config.session_max_loss_usdt} hit.",
                        )
                        close_all_symbol_positions(
                ...
                if (
                    ...
                    logger.critical(
                        f"{NEON['CRITICAL']}Daily max trades ({config.daily_max_trades_limit}) reached. Resting for {config.daily_max_trades_rest_hours} hrs.{NEON['RESET']}"
                    )
                    config.send_notification_method(
                        "Pyrmethus Daily Trades Rest",
                        f"Max {config.daily_max_trades_limit} trades. Resting.",
                    )
                    trade_metrics.daily_trades_rest_active_until = (
                        time.time() + config.daily_max_trades_rest_hours * 3600
                    )
                    close_all_symbol_positions(
                        exchange, config, "Daily Max Trades Limit Reached"
                    )
                    continue
                if (
                    config.enable_consecutive_loss_limiter
                    ...
                    logger.critical(
                        f"{NEON['CRITICAL']}{config.max_consecutive_losses} consecutive losses! Cooldown for {config.consecutive_loss_cooldown_minutes} mins.{NEON['RESET']}"
                    )
                    config.send_notification_method(
                        "Pyrmethus Cooldown",
                        f"{config.max_consecutive_losses} losses. Pausing.",
                    )
                    consecutive_loss_cooldown_active_until = (
                        time.time() + config.consecutive_loss_cooldown_minutes * 60
                    )
                    trade_metrics.consecutive_losses = 0
                    close_all_symbol_positions(
                        exchange, config, "Consecutive Loss Limit"
                    )
                    continue
                ...
                if ohlcv_df is None or ohlcv_df.empty:
                    logger.warning(
                        f"{NEON['WARNING']}Market whispers faint. Retrying.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                ...
                if (
                    ...
                    logger.warning(
                        f"{NEON['WARNING']}Indicator alchemy faint. Pausing.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                ...
                if (
                    ...
                    logger.warning(
                        f"{NEON['WARNING']}Missing Close/ATR or invalid ATR ({_format_for_log(latest_atr)}). No decisions.{NEON['RESET']}"
                    )
                    time.sleep(config.sleep_seconds)
                    continue
                ...
                if config.signal_persistence_candles <= 1:
                    ...
                    if current_long_sig:
                        ...
                        persistent_signal_counter["short"] = 0
                    logger.debug(
                        f"Signal persistence: Lcnt {persistent_signal_counter['long']}, Scnt {persistent_signal_counter['short']}. Confirmed L/S: {confirmed_enter_long}/{confirmed_enter_short}"
                    )
                ...
                if (
                    ...
                    if confirmed_enter_long or confirmed_enter_short:
                        ...
                        if config.enable_no_trade_zones and latest_close is not None:
                            ...
                            for level in key_levels_to_check:
                                ...
                                if (
                                    ...
                                    logger.debug(
                                        f"No-trade zone active around {level}. Entry suppressed."
                                    )
                                    break
                        trap_detected = False
                        if (
                            ...
                            if len(df_with_indicators) > abs(prev_candle_idx):
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        trap_detected = True
                                        logger.debug(
                                            f"Bull Trap Filter: Rejected from recent high {recent_high_val}. Long suppressed."
                                        )
                                elif (
                                    ...
                                    if (
                                        ...
                                        trap_detected = True
                                        logger.debug(
                                            f"Bear Trap Filter: Rejected from recent low {recent_low_val}. Short suppressed."
                                        )
                        if trade_allowed_by_zone and not trap_detected:
                            ...
                            if entry_side != config.pos_none:
                                ...
                                if entry_side == config.pos_long and (
                                    sl_price >= latest_close
                                    or (tp_price and tp_price <= latest_close)
                                ):
                                    logger.warning(
                                        f"Invalid SL/TP for LONG: Entry {latest_close}, SL {sl_price}, TP {tp_price}. Skipping entry."
                                    )
                                elif entry_side == config.pos_short and (
                                    sl_price <= latest_close
                                    or (tp_price and tp_price >= latest_close)
                                ):
                                    logger.warning(
                                        f"Invalid SL/TP for SHORT: Entry {latest_close}, SL {sl_price}, TP {tp_price}. Skipping entry."
                                    )
                                else:
                    ...
                    logger.debug(
                        f"{NEON['INFO']}Max active trade parts ({config.max_active_trade_parts}) reached or position exists. No new entries.{NEON['RESET']}"
                    )
                active_parts_copy = list(_active_trade_parts)
                for part_iter in active_parts_copy:  # Renamed part to part_iter
                    if part_iter.get("symbol") != config.symbol:
                        continue
                    if (
                        config.enable_trend_contradiction_cooldown
                        and part_iter.get("side")
                        and not part_iter.get("contradiction_checked", False)
                    ):
                        time_since_entry_ms = (time.time() * 1000) - part_iter[
                            "entry_time_ms"
                        ]
                        check_window_ms = (
                        ...
                        if (
                            time_since_entry_ms < check_window_ms
                            and time_since_entry_ms > 0
                        ):
                            confirm_trend_now = (
                            ...
                            if (
                                part_iter["side"] == config.pos_long
                                and confirm_trend_now is False
                            ) or (
                                part_iter["side"] == config.pos_short
                                and confirm_trend_now is True
                            ):
                                logger.warning(
                                    f"{NEON['WARNING']}Trend contradiction for part {part_iter['part_id']} shortly after entry. Cooldown activated.{NEON['RESET']}"
                                )
                                contradiction_cooldown_active_until = (
                                    time.time()
                                    + config.trend_contradiction_cooldown_seconds
                                )
                        part_iter["contradiction_checked"] = True
                    current_unrealized_pnl = Decimal(0)
                    if part_iter["side"] == config.pos_long:
                        current_unrealized_pnl = (
                            latest_close - part_iter["entry_price"]
                        ) * part_iter["qty"]
                    elif part_iter["side"] == config.pos_short:
                        current_unrealized_pnl = (
                            part_iter["entry_price"] - latest_close
                        ) * part_iter["qty"]
                    part_iter.get("recent_pnls", deque()).append(current_unrealized_pnl)
                    if (
                        config.enable_profit_momentum_sl_tighten
                        and len(part_iter.get("recent_pnls", []))
                        == config.profit_momentum_window
                    ):
                        recent_pnls_list = list(part_iter["recent_pnls"])
                        ...
                        if (
                            is_currently_profitable
                            and is_profit_increasing
                            and latest_atr > 0
                        ):
                            original_atr_at_entry = part_iter.get(
                            ...
                            current_sl = part_iter["sl_price"]
                            if (
                                part_iter["side"] == config.pos_long
                                and new_sl_price_candidate > current_sl
                                ...
                                logger.info(
                                    f"{NEON['ACTION']}Profit Momentum: Attempting SL tighten for part {part_iter['part_id']} to {new_sl_price_candidate:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}{NEON['RESET']}"
                                )
                                modify_position_sl_tp(
                                    exchange, config, new_sl=new_sl_price_candidate
                                )
                    if (
                        config.enable_breakeven_sl
                        and not part_iter.get("breakeven_set", False)
                        and latest_atr > 0
                    ):
                        current_profit_atr = (
                            (current_unrealized_pnl / part_iter["qty"]) / latest_atr
                            if part_iter["qty"] > 0 and latest_atr > 0
                            else Decimal(0)
                        )
                        atr_target_met = (
                        ...
                        if atr_target_met and abs_pnl_target_met:
                            new_sl_price_candidate = part_iter["entry_price"]
                            current_sl = part_iter["sl_price"]
                            if (
                                part_iter["side"] == config.pos_long
                                and new_sl_price_candidate > current_sl
                                ...
                                logger.info(
                                    f"{NEON['ACTION']}Breakeven SL: Attempting for part {part_iter['part_id']} to {new_sl_price_candidate}{NEON['RESET']}"
                                )
                                if modify_position_sl_tp(
                                    exchange, config, new_sl=new_sl_price_candidate
                                ):
                                    part_iter["breakeven_set"] = True
                    part_iter.get("adverse_candle_closes", deque()).append(latest_close)
                    if (
                        config.enable_last_chance_exit
                        and latest_atr > 0
                        and len(part_iter.get("adverse_candle_closes", []))
                        == config.last_chance_consecutive_adverse_candles
                    ):
                        adverse_closes_list = list(part_iter["adverse_candle_closes"])
                        is_cons_adverse = (
                            all(
                        ...
                        if is_cons_adverse:
                            dist_to_sl = abs(latest_close - part_iter["sl_price"])
                            sl_prox_thresh_price = (
                                latest_atr * config.last_chance_sl_proximity_atr
                            )
                            if dist_to_sl <= sl_prox_thresh_price:
                                logger.warning(
                                    f"{NEON['WARNING']}Last Chance Exit: Part {part_iter['part_id']} ({dist_to_sl:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}} <= {sl_prox_thresh_price:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}).{NEON['RESET']}"
                                )
                    ...
                    if (
                        ...
                        and not part_iter.get("divergence_exit_taken", False)
                        and "entry_fisher_value" in part_iter
                        and part_iter["entry_fisher_value"] is not None
                    ):
                        ...
                        if fisher_now is not pd.NA and signal_now is not pd.NA:
                            entry_fisher = part_iter["entry_fisher_value"]
                            entry_signal = part_iter.get("entry_signal_value", pd.NA)
                            if entry_fisher is not pd.NA and entry_signal is not pd.NA:
                                ...
                                if divergence_met:
                                    ...
                                    qty_to_close = (
                                        ...
                                        * config.qty_step
                                        if config.qty_step and config.qty_step > 0
                                        else qty_to_close_f
                                    )
                                    if qty_to_close > config.position_qty_epsilon:
                                        logger.info(
                                            f"{NEON['ACTION']}Ehlers Divergence Exit ({config.ehlers_divergence_exit_percentage * 100}%): Part {part_iter['part_id']}, Qty {qty_to_close}{NEON['RESET']}"
                                        )
                                        temp_part_to_close = part_iter.copy()
                                        ...
                                        if close_position_part(
                                            ...
                                            part_iter["qty"] -= qty_to_close
                                            part_iter["divergence_exit_taken"] = True
                                            if (
                                                part_iter["qty"]
                                                <= config.position_qty_epsilon
                                            ):
                                                ...
                                                logger.info(
                                                    f"Part {part_iter['part_id']} fully closed due to negligible remainder after partial Ehlers div exit."
                                                )
                                            save_persistent_state(force_heartbeat=True)
                                            continue
                                    else:
                                        logger.debug(
                                            f"Ehlers div exit: calculated qty_to_close {qty_to_close} too small."
                                        )
                    if (
                        part_iter["side"] == config.pos_long
                        and signals.get("exit_long")
                    ) or (
                        part_iter["side"] == config.pos_short
                        and signals.get("exit_short")
                    ):
                        exit_reason = signals.get("exit_reason", "Strategy Exit Signal")
                        logger.info(
                            f"{NEON['ACTION']}Omen to EXIT {part_iter['side']} for {config.symbol}: {exit_reason}{NEON['RESET']}"
                        )
                        if close_position_part(
                            exchange, config, part_iter, exit_reason
                        ):
                            continue
                    if part_iter.get("sl_price") is not None:
                        if (
                            part_iter["side"] == config.pos_long
                            and latest_close <= part_iter["sl_price"]
                        ) or (
                            part_iter["side"] == config.pos_short
                            and latest_close >= part_iter["sl_price"]
                        ):
                            logger.warning(
                                f"{NEON['WARNING']}Part {part_iter['part_id']} SL price {part_iter['sl_price']} potentially breached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}"
                            )
                    if (
                        part_iter.get("tp_price") is not None
                        and config.enable_take_profit
                    ):
                        if (
                            part_iter["side"] == config.pos_long
                            and latest_close >= part_iter["tp_price"]
                        ) or (
                            part_iter["side"] == config.pos_short
                            and latest_close <= part_iter["tp_price"]
                        ):
                            logger.info(
                                f"{NEON['INFO']}Part {part_iter['part_id']} TP price {part_iter['tp_price']} potentially reached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}"
                            )
                save_persistent_state()
                logger.debug(
                    f"{NEON['COMMENT']}# Cycle complete. Resting {config.sleep_seconds}s...{NEON['RESET']}"
                )
                time.sleep(config.sleep_seconds)
                ...
                logger.critical(
                    f"{NEON['CRITICAL']}Critical error in main loop iteration! Error: {e_iter}{NEON['RESET']}"
                )
                logger.debug(traceback.format_exc())
                if hasattr(CONFIG, "send_notification_method"):
                    CONFIG.send_notification_method(
                        "Pyrmethus Critical Error",
                        f"Loop iteration crashed: {str(e_iter)[:150]}",
                    )
                logger.info(
                    f"Resting longer ({config.sleep_seconds * 5}s) after critical error in iteration."
                )
                time.sleep(config.sleep_seconds * 5)
    finally:
        logger.info(
            f"{NEON['HEADING']}=== Pyrmethus Spell Concludes ==={NEON['RESET']}"
        )
        if exchange:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if CONFIG.tick_size is None or CONFIG.tick_size <= 0:
            logger.warning(
                f"{NEON['WARNING']}Could not determine valid tick size from market info. Using a small default if needed.{NEON['RESET']}"
            )
            CONFIG.tick_size = Decimal("1e-8")
        ...
        if CONFIG.qty_step is None or CONFIG.qty_step <= 0:
            logger.warning(
                f"{NEON['WARNING']}Could not determine valid quantity step from market info. Using a small default if needed.{NEON['RESET']}"
            )
            CONFIG.qty_step = Decimal("1e-8")
        logger.success(
        ...
        try:
            ...
            logger.info(
                f"Setting leverage to {CONFIG.leverage}x for {CONFIG.symbol} (Category: {category or 'inferred'})..."
            )
            response = exchange_instance.set_leverage(
                CONFIG.leverage, CONFIG.symbol, params=leverage_params
            )
            logger.success(
                f"{NEON['SUCCESS']}Leverage for {CONFIG.symbol} set/confirmed. Response (part): {json.dumps(response, default=str)[:150]}...{NEON['RESET']}"
            )  # type: ignore [attr-defined]
        except Exception as e_lev:
            logger.warning(
                f"{NEON['WARNING']}Could not set leverage (may be pre-set or issue): {e_lev}{NEON['RESET']}"
            )
            CONFIG.send_notification_method(
                "Pyrmethus Leverage Warn",
                f"Leverage issue for {CONFIG.symbol}: {str(e_lev)[:60]}",
            )
        logger.success(
            f"{NEON['SUCCESS']}Connected to {exchange_instance.id} for {CONFIG.symbol}.{NEON['RESET']}"
        )  # type: ignore [attr-defined]
        CONFIG.send_notification_method(
            "Pyrmethus Online",
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Exchange attribute error: {e_attr}. CCXT issue or invalid ID?{NEON['RESET']}"
        )
        sys.exit(1)
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Network Error: {e_net}. Check connection/DNS.{NEON['RESET']}"
        )
        CONFIG.send_notification_method(
            "Pyrmethus Network Error", f"Cannot connect: {str(e_net)[:80]}"
        )
        sys.exit(1)
    except ccxt.ExchangeError as e_exch:
        logger.critical(
            f"{NEON['CRITICAL']}Exchange API Error: {e_exch}. Check API/symbol/status.{NEON['RESET']}"
        )
        CONFIG.send_notification_method(
            "Pyrmethus API Error", f"API issue: {str(e_exch)[:80]}"
        )
        sys.exit(1)
    ...
    if exchange_instance:
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Exchange not initialized. Spell aborted.{NEON['RESET']}"
        )
        sys.exit(1)
