                                                              ...
                                                              ```python                                                              # Suggestion 11: Recolorize Neon (Alternative Bright Theme)                                                                                   # In Logger Setup section:                                             # Replace the existing logging.addLevelName calls with these:          if hasattr(sys.stdout, 'isatty') and sys.stdout.isatty():
    # Apply Alternative Bright colors                                      logging.addLevelName(logging.DEBUG, f"{Fore.BLUE}{Style.DIM}{logging.getLevelName(logging.DEBUG)}{Style.RESET_ALL}")    # Dim Blue for Debug                                                                         logging.addLevelName(logging.INFO, f"{Fore.CYAN}{logging.getLevelName(logging.INFO)}{Style.RESET_ALL}")        # Cyan for Info                logging.addLevelName(SUCCESS_LEVEL, f"{Fore.YELLOW}{Style.BRIGHT}{logging.getLevelName(SUCCESS_LEVEL)}{Style.RESET_ALL}") # Bright Yellow for Success                                                                logging.addLevelName(logging.WARNING, f"{Fore.MAGENTA}{Style.BRIGHT}{logging.getLevelName(logging.WARNING)}{Style.RESET_ALL}")# Bright Magenta for Warning                                                           logging.addLevelName(logging.ERROR, f"{Fore.RED}{Style.BRIGHT}{logging.getLevelName(logging.ERROR)}{Style.RESET_ALL}")      # Bright Red for Error                                                                   logging.addLevelName(logging.CRITICAL, f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}{logging.getLevelName(logging.CRITICAL)}{Style.RESET_ALL}")# White on Red for Critical
else:                                                                      # Avoid color codes if not a TTY                                       logging.addLevelName(SUCCESS_LEVEL, "SUCCESS") # Ensure level name exists without color                                                                                                                          # Additionally, search and replace inline color usage throughout the script.                                                                  # Example replacements:                                                # - Replace Fore.MAGENTA for config/SMS messages with Fore.WHITE or Fore.CYAN                                                                 # - Replace Fore.GREEN for success messages with Fore.YELLOW           # - Replace Fore.BLUE for info messages with Fore.CYAN                 # - Replace Back.YELLOW/Fore.BLACK for warnings/exits with Back.MAGENTA/Fore.WHITE                                                            # - Replace Back.GREEN/Fore.BLACK for buy entry with Back.CYAN/Fore.BLACK                                                                     # - Replace Back.RED/Fore.WHITE for sell entry/critical errors with Back.RED/Fore.WHITE (keep)                                                
# Example (replace in place_risked_market_order):                      # Original:                                                            # logger.warning(f"{entry_side_color}{text_color}{Style.BRIGHT}*** Placing {side.upper()} MARKET ENTRY: {quantity_float:.8f} {symbol} ***{Style.RESET_ALL}")                                                         # New (Conceptual - adjust colors as desired):                         # entry_side_color = Back.CYAN if side == SIDE_BUY else Back.RED # Cyan for Buy, Red for Sell                                                 # text_color = Fore.BLACK if side == SIDE_BUY else Fore.WHITE
...
# Example (replace in trade_logic exit):                               # Original:                                                            # logger.warning(f"{exit_side_color}{Fore.BLACK}{Style.BRIGHT}*** TRADE EXIT SIGNAL: Closing {position_side} due to {exit_reason} ***{Style.RESET_ALL}")                                                             # New:                                                                 # exit_side_color = Back.MAGENTA # Magenta background for exits
# logger.warning(f"{exit_side_color}{Fore.WHITE}{Style.BRIGHT}*** TRADE EXIT SIGNAL: Closing {position_side} due to {exit_reason} ***{Style.RESET_ALL}")                                                             
# Apply similar replacements systematically to all inline colorama codes based on the desired theme.
```                                                                                                                                           ```python                                                              # Suggestion 12: Bot Wrapper Class Structure                                                                                                  # Keep Config class, standalone functions (calculate_supertrend, analyze_volume_atr, etc.),
# and helper functions (send_sms_alert, initialize_exchange, set_leverage, etc.) mostly as they are.
# Modify them to accept exchange/config objects if they rely on global state currently.
                                                                       class TradingBot:
    def __init__(self) -> None:                                                self._configure_logging() # Setup logger instance variable             self.logger.info(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bot Wrapper Initializing ---{Style.RESET_ALL}")                         try:                                                                       self.config = Config() # Load configuration                        except ValueError as config_err:
            self.logger.critical(f"Configuration failed: {config_err}")            # Optional: Attempt emergency SMS if config allows partial load                                                                               # send_sms_alert(f"[ScalpBot] CRITICAL: Config validation FAILED: {config_err}. Bot stopped.")                                                raise  # Re-raise to prevent bot object creation on critical config error                                                                                                                                        self.exchange = None                                                   self._running = False                                                  self.symbol = self.config.symbol                                       self.interval = self.config.interval                           
    def _configure_logging(self) -> None:                                      """Sets up the logger for the bot instance."""
        # You can reuse the existing logger setup logic here,                  # assigning the logger to self.logger
        LOGGING_LEVEL_STR = os.getenv("LOGGING_LEVEL", "INFO").upper()         LOGGING_LEVEL = getattr(logging, LOGGING_LEVEL_STR, logging.INFO)                                                                             SUCCESS_LEVEL = 25                                                     logging.addLevelName(SUCCESS_LEVEL, "SUCCESS")                         def log_success(self_log, message, *args, **kwargs): # type: ignore                                                                               if self_log.isEnabledFor(SUCCESS_LEVEL): self_log._log(SUCCESS_LEVEL, message, args, **kwargs) # type: ignore
        logging.Logger.success = log_success # type: ignore[attr-defined]
                                                                               logging.basicConfig(
            level=LOGGING_LEVEL,                                                   format="%(asctime)s [%(levelname)s] (Bot) %(message)s", # Added (Bot) tag                                                                     datefmt="%Y-%m-%d %H:%M:%S",                                           handlers=[logging.StreamHandler(sys.stdout)]                       )                                                                      self.logger = logging.getLogger("TradingBot") # Use a named logger                                                                    
        # Apply colors (reuse the logic from the original script or suggestion 11)
        if hasattr(sys.stdout, 'isatty') and sys.stdout.isatty():                  # Example using original colors
            logging.addLevelName(logging.DEBUG, f"{Fore.CYAN}{Style.DIM}{logging.getLevelName(logging.DEBUG)}{Style.RESET_ALL}")
            logging.addLevelName(logging.INFO, f"{Fore.BLUE}{logging.getLevelName(logging.INFO)}{Style.RESET_ALL}")                                       logging.addLevelName(SUCCESS_LEVEL, f"{Fore.MAGENTA}{Style.BRIGHT}{logging.getLevelName(SUCCESS_LEVEL)}{Style.RESET_ALL}")                    logging.addLevelName(logging.WARNING, f"{Fore.YELLOW}{Style.BRIGHT}{logging.getLevelName(logging.WARNING)}{Style.RESET_ALL}")                 logging.addLevelName(logging.ERROR, f"{Fore.RED}{Style.BRIGHT}{logging.getLevelName(logging.ERROR)}{Style.RESET_ALL}")                        logging.addLevelName(logging.CRITICAL, f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}{logging.getLevelName(logging.CRITICAL)}{Style.RESET_ALL}")
        else:                                                                      logging.addLevelName(SUCCESS_LEVEL, "SUCCESS")
                                                                                                                                                  def _initialize_dependencies(self) -> bool:                                """Initializes exchange and sets leverage."""                          self.exchange = initialize_exchange() # Assumes initialize_exchange uses global CONFIG or is adapted                                          if not self.exchange:
            self.logger.critical("Failed to initialize exchange.")                 return False
                                                                               if not set_leverage(self.exchange, self.symbol, self.config.leverage): # Pass instance exchange/config                                             self.logger.critical(f"Failed to set leverage.")
             # Optional: send_sms_alert(...)                                        return False                                                      return True                                                                                                                               def run(self) -> None:                                                     """Starts the main trading loop."""                                    if not self._initialize_dependencies():
            self.logger.critical("Bot dependencies failed to initialize. Shutting down.")
            return                                                     
        self._running = True                                                   self.logger.warning(f"{Back.YELLOW}{Fore.BLACK}{Style.BRIGHT}--- !!! LIVE TRADING BOT ACTIVE - {self.symbol} !!! ---{Style.RESET_ALL}")                                                                              # Optional: Log final config summary here using self.config                                                                                   cycle_count = 0                                                        while self._running:                                                       cycle_count += 1
            self.logger.debug(f"{Fore.CYAN}--- Cycle {cycle_count} Start ---{Style.RESET_ALL}")
            try:                                                                       # Call trade_logic, passing necessary instance variables/objects                                                                              self._execute_trade_logic() # Wrap the call
                                                                                       self.logger.debug(f"Cycle {cycle_count} complete. Sleeping for {self.config.sleep_seconds} seconds...")                                       time.sleep(self.config.sleep_seconds)                                                                                                     except KeyboardInterrupt:                                                  self.logger.warning(f"\n{Fore.YELLOW}{Style.BRIGHT}KeyboardInterrupt detected. Stopping bot...{Style.RESET_ALL}")                             # Optional: send_sms_alert(...)
                self.stop() # Signal stop                              
            # --- Keep the robust exception handling from the original main loop ---
            except ccxt.AuthenticationError as e:                                       self.logger.critical(f"CRITICAL: Authentication Error: {e}. Shutting down.")                                                                  # Optional: send_sms_alert(...)                                        self.stop()                                                       except ccxt.NetworkError as e:                                             self.logger.error(f"ERROR: Network error: {e}. Retrying...")                                                                                  time.sleep(self.config.sleep_seconds * 2)
            except ccxt.RateLimitExceeded as e:                                        self.logger.warning(f"WARNING: Rate limit exceeded: {e}. Sleeping longer...")                                                                 time.sleep(self.config.sleep_seconds * 3)
            except ccxt.ExchangeNotAvailable as e:                                     self.logger.error(f"ERROR: Exchange not available: {e}. Sleeping longer...")                                                                  # Optional: send_sms_alert(...)                                        time.sleep(self.config.sleep_seconds * 5)                          except ccxt.ExchangeError as e:                                             self.logger.error(f"ERROR: Exchange error: {e}. Retrying...")                                                                                 self.logger.debug(traceback.format_exc())                              time.sleep(self.config.sleep_seconds)                             except Exception as e:
                self.logger.critical(f"FATAL: Unexpected error: {e}")                  self.logger.critical(traceback.format_exc())
                # Optional: send_sms_alert(...)                                        self.stop() # Stop on fatal errors                                 # --- End of exception handling ---                                                                                                       self.logger.info("Bot run loop finished.")                             self._shutdown() # Perform cleanup                                                                                                        def _execute_trade_logic(self) -> None:                                    """Executes one cycle of the trading logic."""                         # This assumes trade_logic is adapted to be a method or accepts necessary args                                                                # Option 1: trade_logic is a standalone function                       trade_logic(self.exchange, self.symbol, self.interval) # Pass required args                                                                                                                                          # Option 2: trade_logic becomes a method self.trade_logic()            # self.trade_logic() # Call method directly                                                                                               def stop(self) -> None:                                                    """Signals the bot to stop running."""                                 self.logger.info("Stop signal received.")                              self._running = False                                                                                                                     def _shutdown(self) -> None:                                               """Gracefully shuts down the bot, attempting to close positions."""                                                                           self.logger.warning("Initiating shutdown sequence...")                 try:                                                                       if self.exchange and self.exchange.check_required_credentials():                                                                                  self.logger.info("Checking for open position to close on exit...")                                                                            # Use standalone get_current_position and close_position functions                                                                            current_pos = get_current_position(self.exchange, self.symbol) # Pass exchange                                                                if current_pos[SIDE_KEY] != POSITION_SIDE_NONE:                            self.logger.warning(f"Attempting to close {current_pos[SIDE_KEY]} position...")                                                               close_result = close_position(self.exchange, self.symbol, current_pos, reason="Shutdown") # Pass exchange                                     if close_result: self.logger.info("Position closed successfully.")                                                                            else: self.logger.error("Failed to close position during shutdown.")                                                                          # Optional: send_sms_alert(...)                                    else:                                                                      self.logger.info("No open position found.")                    else:                                                                      self.logger.warning("Exchange object not available for final position check.")                                                        except Exception as e:                                                     self.logger.error(f"Error during shutdown position check/close: {e}")                                                                         self.logger.debug(traceback.format_exc())                              # Optional: send_sms_alert(...)                                                                                                           self.logger.info(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Trading Bot Deactivated ---{Style.RESET_ALL}")                                                                                                                                                                          # --- Main Execution Block (Modified) ---                              if __name__ == "__main__":                                                 bot = None                                                             try:                                                                       bot = TradingBot()                                                     bot.run() # Start the bot's main loop                              except (ValueError, ImportError, Exception) as main_init_err:              # Catch errors during bot instantiation (like config validation)                                                                              logging.critical(f"Failed to initialize and run the bot: {main_init_err}", exc_info=True)                                                     # Attempt SMS if possible, depends on where error occurred             # if bot and bot.config and bot.config.enable_sms_alerts: send_sms_alert(...)                                                                 sys.exit(1)                                                        except KeyboardInterrupt:                                                  # This is caught inside bot.run(), but adding here for completeness                                                                           # if bot is not None:                                                  #     bot.logger.info("KeyboardInterrupt caught in main block.")                                                                              # else:                                                                #     logging.info("KeyboardInterrupt caught before bot fully started.")                                                                      pass # Bot handles shutdown internally                             finally:                                                                   # Ensure shutdown sequence runs even if run() exits unexpectedly without calling _shutdown()                                                  if bot is not None and bot._running: # If loop was running but exited uncleanly                                                                    logging.warning("Main block finally: Forcing shutdown sequence.")                                                                             bot.stop() # Ensure flag is set                                        # bot._shutdown() # Consider if calling this here is safe or redundant                                                                   logging.info("Main execution finished.")                       ```
``python
# Suggestion 1: Add Timeout to API Calls (Conceptual Example using signal, requires platform support)                                         import signal                                                                                                                                 class TimeoutException(Exception): pass                                                                                                       def timeout_handler(signum, frame):                                        raise TimeoutException("API Call Timed Out")                                                                                              # Inside functions like get_current_position, fetch_ohlcv, etc.        # NOTE: signal doesn't work reliably on Windows for this purpose. asyncio would be better cross-platform.                                     # signal.signal(signal.SIGALRM, timeout_handler)
# signal.alarm(15) # Set 15 second timeout                             # try:                                                                 #     # Original API call here, e.g.:                                  #     # positions = exchange.fetch_positions(symbols=[ccxt_unified_symbol], params=params)                                                    #     pass # Replace pass with the actual call                         # except TimeoutException as e:                                        #     logger.error(f"API call timed out: {e}")                         #     # Handle timeout (e.g., return default, retry later)             # finally:                                                             #     signal.alarm(0) # Disable the alarm                              ```                                                                    
```python                                                              # Suggestion 2: Dynamic ATR Multipliers based on Volatility Index (Conceptual)                                                                def calculate_volatility_index(df: pd.DataFrame) -> float:                 # Example: Use recent ATR range or standard deviation                  recent_atr = df[f'ATRr_{CONFIG.atr_calculation_period}'].iloc[-5:].mean()                                                                     historical_atr_avg = df[f'ATRr_{CONFIG.atr_calculation_period}'].rolling(window=50).mean().iloc[-1]                                           if historical_atr_avg > 0:                                                 vol_index = recent_atr / historical_atr_avg                            return max(0.5, min(2.0, vol_index)) # Clamp between 0.5x and 2.0x
    return 1.0 # Default if calculation fails                                                                                                 # In trade_logic, before calculating SL/TP:                            # volatility_factor = calculate_volatility_index(df) # Ensure ATR is calculated first                                                         # dynamic_sl_multiplier = CONFIG.atr_stop_loss_multiplier * Decimal(str(volatility_factor))                                                   # dynamic_tp_multiplier = CONFIG.atr_take_profit_multiplier / Decimal(str(volatility_factor)) # Inverse relationship for TP? Or direct? Decide logic.                                                                # logger.debug(f"Volatility Factor: {volatility_factor:.2f}, Dynamic SL Mult: {dynamic_sl_multiplier:.2f}, Dynamic TP Mult: {dynamic_tp_multiplier:.2f}")                                                            # Use dynamic_sl_multiplier and dynamic_tp_multiplier in SL/TP calculation                                                                    ```                                                                                                                                           ```python                                                              # Suggestion 3: Partial Take Profit (Conceptual - requires order placement adjustments)                                                       # In Config:                                                           # self.tp_levels: List[Tuple[Decimal, Decimal]] = [ # (Multiplier, Percentage)                                                                #     (Decimal("1.0"), Decimal("0.5")), # 50% at 1*ATR                 #     (Decimal("2.0"), Decimal("0.5"))  # Remaining 50% at 2*ATR       # ]                                                                                                                                           # In place_risked_market_order or a new function:                      # Calculate total quantity first                                       # Loop through CONFIG.tp_levels:                                       #   Calculate TP price for this level using entry_price + (current_atr * level_multiplier)                                                    #   Calculate quantity for this level: total_quantity * level_percentage                                                                      #   Place separate TP orders (stopMarket, reduceOnly) for each level/quantity.                                                                #   Ensure total TP quantity matches entry quantity.                   #   Handle potential errors placing individual TP orders.              ```                                                                                                                                           ```python                                                              # Suggestion 4: Trailing Stop Loss (Bot-Managed Example - Exchange Native preferred if available)                                             # Requires storing active trail details (e.g., activation price, trail distance)                                                              # In trade_logic loop, if in position:                                 #   Fetch current price.                                               #   If long:                                                           #       Calculate potential new SL (current_price - trail_distance)    #       If potential new SL > current active SL:                       #           Cancel existing SL order.                                  #           Place new SL order at the higher price.                    #           Update stored active SL price.                             #   If short:                                                          #       Calculate potential new SL (current_price + trail_distance)    #       If potential new SL < current active SL:                       #           Cancel existing SL order.                                  #           Place new SL order at the lower price.                     #           Update stored active SL price.                             # Add logic for initial activation price (e.g., entry_price + activation_offset).                                                             # NOTE: This adds complexity and potential race conditions. Exchange-native TSL is safer.                                                     # Bybit V5 supports trailing stops via create_order params={'trailingStop': '...'} - investigate CCXT support.                                ```                                                                                                                                           ```python                                                              # Suggestion 5: State Machine for Logic Flow                           from enum import Enum, auto                                                                                                                   class BotState(Enum):                                                      IDLE = auto()                                                          CHECKING_ENTRY = auto()                                                ENTERING_LONG = auto()                                                 IN_LONG = auto()                                                       ENTERING_SHORT = auto()                                                IN_SHORT = auto()                                                      CHECKING_EXIT = auto()                                                 EXITING = auto()                                                                                                                          # Global variable or class member                                      # current_bot_state = BotState.IDLE                                                                                                           # In trade_logic:                                                      # Based on current_bot_state and market conditions/position, transition between states.                                                       # Example:                                                             # if current_bot_state == BotState.IDLE:                               #     if long_entry_signal and confirmations_pass:                     #         current_bot_state = BotState.ENTERING_LONG                   #         # Initiate entry order placement                             #     elif short_entry_signal and confirmations_pass:                  #         current_bot_state = BotState.ENTERING_SHORT                  #         # Initiate entry order placement                             # elif current_bot_state == BotState.IN_LONG:                          #     if close_long_signal:                                            #         current_bot_state = BotState.EXITING                         #         # Initiate close order placement                             # # ... handle other state transitions (e.g., ENTERING -> IN_*, EXITING -> IDLE)                                                              # # Ensure entry/exit actions are only triggered on state *transitions*.                                                                      ```                                                                                                                                           ```python                                                              # Suggestion 6: Use Pydantic for Config Validation (Example Snippet)   # pip install pydantic                                                 # from pydantic import BaseModel, Field, validator, PositiveInt, PositiveFloat, SecretStr                                                     # from decimal import Decimal                                                                                                                 # class BotConfig(BaseModel):                                          #     BYBIT_API_KEY: SecretStr                                         #     BYBIT_API_SECRET: SecretStr                                      #     SYMBOL: str = Field(..., pattern=r"^[A-Z0-9]+/[A-Z]+:[A-Z]+$") # Regex example                                                          #     INTERVAL: str = Field("1m", pattern=r"^\d+[mhd]$") # Example pattern                                                                    #     LEVERAGE: PositiveInt = 10                                       #     RISK_PER_TRADE_PERCENTAGE: Decimal = Field(..., gt=0, lt=1)      #     # ... other fields with types and constraints ...                #     MAX_ORDER_USDT_AMOUNT: Decimal = Field(Decimal("500.0"), gt=0)                                                                          #     @validator('INTERVAL')                                           #     def interval_must_be_supported(cls, v):                          #         supported = ['1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '1d'] # Example                                                      #         if v not in supported:                                       #             raise ValueError(f"Interval '{v}' not supported. Choose from: {supported}")
#         return v                                                                                                                            # # In main script, replace Config class instantiation:                # try:                                                                 #     # Load from .env or dict                                         #     env_vars = {k: os.getenv(k) for k in BotConfig.__fields__ if os.getenv(k) is not None}                                                  #     # Add default handling if needed                                 #     CONFIG = BotConfig(**env_vars)                                   #     logger.info("Configuration loaded and validated successfully via Pydantic.")                                                            # except ValidationError as e:                                         #     logger.critical(f"Configuration validation failed:\n{e}")
#     sys.exit(1)                                                      # # Access config via CONFIG.SYMBOL, CONFIG.LEVERAGE etc. Use CONFIG.BYBIT_API_KEY.get_secret_value()                                         ```                                                                                                                                           ```python                                                              # Suggestion 7: Convert to Asyncio (Conceptual Structure)              # import asyncio                                                       # # Replace sync ccxt methods with async ones (e.g., exchange.fetch_ohlcv_async)                                                              # # Mark functions with `async def`                                                                                                           # async def fetch_ohlcv_async(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int) -> Optional[pd.DataFrame]:                    #     logger.info(f"Data Fetch Async: Gathering {limit} {timeframe} candles for {symbol}...")                                                 #     # ... use exchange.fetch_ohlcv_async inside try/except ...       #     # await asyncio.sleep(CONFIG.retry_delay_seconds)                                                                                       # async def trade_logic_async(exchange: ccxt.Exchange, symbol: str, timeframe: str) -> None:                                                  #      # ... call await fetch_ohlcv_async(...) ...                     #      # ... call await get_current_position_async(...) ...            #      # ... call await place_risked_market_order_async(...) etc. ...                                                                         # async def main_async() -> None:
#     # ... initialize exchange ...                                    #     # ... set leverage (might need async version or run_in_executor) ...                                                                    #     while run_bot:                                                   #         try:                                                         #             await trade_logic_async(exchange, CONFIG.symbol, CONFIG.interval)                                                               #             await asyncio.sleep(CONFIG.sleep_seconds)                #         except asyncio.CancelledError:                               #              logger.warning("Async task cancelled.")                 #              run_bot = False                                         #         # ... other exception handling ...                           #     # ... shutdown logic ...
                                                                       # if __name__ == "__main__":                                           #     try:                                                             #         asyncio.run(main_async())                                    #     except KeyboardInterrupt:                                        #         logger.info("KeyboardInterrupt received in main.")           #     # Add final cleanup if needed                                    ```                                                                                                                                           ```python                                                              # Suggestion 8: Backtesting Mode Flag (Conceptual)                     # Add to Config or as command-line argument:                           # self.backtesting_mode: bool = self._get_env("BACKTESTING_MODE", False, bool)                                                                # self.backtest_data_path: Optional[str] = self._get_env("BACKTEST_DATA_PATH", None, str)                                                                                                                            # In main():                                                           # if CONFIG.backtesting_mode:                                          #     logger.warning("--- RUNNING IN BACKTESTING MODE ---")            #     # Load historical data from CONFIG.backtest_data_path            #     # Initialize a backtesting engine/simulator instead of live exchange                                                                    #     # Run trade_logic against the simulator loop                     #     # Print/save backtest results (equity curve, metrics)            # else:                                                                #     logger.info("--- RUNNING IN LIVE TRADING MODE ---")              #     # Proceed with live exchange initialization and main loop                                                                               # Modify trade_logic, order placement, position checks to interact with the                                                                   # backtesting engine API instead of ccxt when in backtesting_mode.     ```                                                                                                                                           ```python                                                              # Suggestion 9: Funding Rate Consideration (Conceptual)                # Add to Config:                                                       # self.max_allowable_funding_rate: Decimal = self._get_env("MAX_ALLOWABLE_FUNDING_RATE", Decimal("0.001"), Decimal) # 0.1% threshold example  # self.check_funding_rate: bool = self._get_env("CHECK_FUNDING_RATE", True, bool)                                                                                                                                    # In trade_logic, before entry confirmation:                           # funding_rate_ok = True                                               # if CONFIG.check_funding_rate and position_side == POSITION_SIDE_NONE: # Only check when entering                                            #     try:                                                             #         ticker_data = exchange.fetch_ticker(symbol)                  #         funding_rate = ticker_data.get('info', {}).get('fundingRate') # Path might vary by exchange/ccxt version                            #         if funding_rate is not None:                                 #             funding_rate_decimal = Decimal(str(funding_rate))        #             logger.debug(f"Current Funding Rate for {symbol}: {funding_rate_decimal:.4%}")                                                  #             # Discourage LONG if funding is highly positive (paying) #             if selected_side == SIDE_BUY and funding_rate_decimal > CONFIG.max_allowable_funding_rate:                                      #                 logger.warning(f"Entry ({selected_side.upper()}) discouraged: High positive funding rate ({funding_rate_decimal:.4%}) > threshold.")                                                               #                 funding_rate_ok = False                              #             # Discourage SHORT if funding is highly negative (paying)#             elif selected_side == SIDE_SELL and funding_rate_decimal < -CONFIG.max_allowable_funding_rate:                                  #                 logger.warning(f"Entry ({selected_side.upper()}) discouraged: High negative funding rate ({funding_rate_decimal:.4%}) < threshold.")                                                               #                 funding_rate_ok = False                              #         else:                                                        #             logger.warning("Could not fetch funding rate.")          #             # Decide if failure to fetch rate should block entry (funding_rate_ok = False) or allow it.                                     #     except Exception as e:                                           #         logger.warning(f"Failed to check funding rate: {e}")         #         # Decide if failure should block entry or allow it.                                                                                 # # Modify entry condition:                                            # if volume_confirmed and ob_confirmed and funding_rate_ok:
#     # Proceed with entry calculation                                 ```                                                                                                                                           ```python                                                              # Suggestion 10: Robust SL/TP Failure Handling in place_risked_market_order                                                                   # Inside place_risked_market_order, after entry fill is confirmed:     # ... (code to extract actual_filled_qty, actual_avg_price) ...                                                                               # sl_placed_successfully = False                                       # tp_placed_successfully = False                                                                                                              # # Place SL Order
# try:                                                                 #     # ... place SL order ...                                         #     logger.success(...)                                              #     sl_placed_successfully = True                                    # except Exception as e:                                               #     logger.error(f"{Back.RED}{Fore.WHITE}{log_prefix}: FAILED to place Stop-Loss order: {e}{Style.RESET_ALL}")                              #     # CRITICAL: SL failed, try immediate emergency close             #     logger.warning(f"{log_prefix}: Attempting EMERGENCY CLOSE due to SL placement failure...")                                              #     send_sms_alert(f"[{market_base}] CRITICAL: SL FAILED for entry {order_id[-6:]}. Attempting emergency close!")                           #     emergency_close_pos = {SIDE_KEY: side, QTY_KEY: actual_filled_qty, ENTRY_PRICE_KEY: actual_avg_price}                                   #     close_success = close_position(exchange, symbol, emergency_close_pos, reason="EMERGENCY_SL_FAIL")                                       #     if close_success:                                                #         logger.info(f"{log_prefix}: Emergency close successful.")    #         send_sms_alert(f"[{market_base}] Emergency close SUCCEEDED after SL fail.")                                                         #     else:                                                            #         logger.error(f"{Back.RED}{Fore.WHITE}{log_prefix}: EMERGENCY CLOSE FAILED after SL placement failure. MANUAL INTERVENTION URGENT!{Style.RESET_ALL}")                                                       #         send_sms_alert(f"[{market_base}] CRITICAL: Emergency close FAILED after SL fail. MANUAL ACTION NEEDED!")
#     return None # Do not proceed further, position should be closed or needs manual attention
                                                                       # # Place TP Order (only if SL succeeded)
# if sl_placed_successfully:                                           #     try:                                                             #         # ... place TP order ...                                     #         logger.success(...)                                          #         tp_placed_successfully = True                                #     except Exception as e:                                           #         # TP Failure is less critical, but still needs attention     #         logger.error(f"{Back.YELLOW}{Fore.BLACK}{log_prefix}: FAILED to place Take-Profit order: {e}{Style.RESET_ALL}")
#         send_sms_alert(f"[{market_base}] WARNING: TP order FAILED for entry {order_id[-6:]}. Position open with SL only.")
#         # Decide: Continue (position open with SL) or try to close? Let's continue but warn.
                                                                       # # Final outcome based on success flags                               # if sl_placed_successfully: # TP success is optional for returning entry order                                                               #     logger.info(f"{Fore.GREEN}{log_prefix}: Entry order filled. SL placed ({'TP placed' if tp_placed_successfully else 'TP FAILED'}).{Style.RESET_ALL}")                                                           #     # Send appropriate SMS based on tp_placed_successfully           #     return confirmed_order # Return confirmed entry order            # else:                                                                #     # This case should ideally be handled by the emergency close logic above                                                                #     logger.error(f"{log_prefix}: SL placement failed, process aborted.") # Should not be reached if emergency close works                   #     return None
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def calculate_supertrend(
    df: pd.DataFrame, length: int, multiplier: float, prefix: str = ""
     ...
     # Define expected pandas_ta column names
     ...
     try:
         ...
         if pd.notna(df[volume_ma_col].iloc[-1]) and pd.notna(df['volume'].iloc[-1]):
            try:
                ...
                logger.warning(f"Scrying (Vol): Invalid Decimal value for Volume/MA. Vol: {df['volume'].iloc[-1]}, MA: {df[volume_ma_col].iloc[-1]}. Error: {e}")
            ...
            if results[VOLUME_MA_KEY] is not None and results[VOLUME_MA_KEY] > CONFIG.position_qty_epsilon and results[LAST_VOLUME_KEY] is not None:
                try:
                    ...
                    logger.warning(f"Scrying (Vol): Invalid Decimal operation for ratio. LastVol={results[LAST_VOLUME_KEY]}, VolMA={results[VOLUME_MA_KEY]}. Error: {e}")
...
def fetch_ohlcv(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int) -> Optional[pd.DataFrame]:
    ...
    for attempt in range(CONFIG.retry_count):
        try:
            ...
            # Ensure correct data types after potential ffill
            # Convert OHLCV columns to numeric, coercing errors to NaN
            for col in ['open', 'high', 'low', 'close', 'volume']:
                df[col] = pd.to_numeric(df[col], errors='coerce')
            # Final check for NaNs after numeric conversion
            if df.isnull().values.any():
                logger.error(f"{Fore.RED}Data Fetch: NaN values introduced after numeric conversion post-ffill. Data quality issue.{Style.RESET_ALL}")
                return None
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    for attempt in range(CONFIG.retry_count):
        try:
            ...
            # CCXT might handle this based on `defaultMarginMode` option or market info,
            # but explicitly passing it might be safer if issues occur.
            # Let's rely on CCXT's abstraction first.
            ...
            response = exchange.set_leverage(leverage=leverage, symbol=symbol) # Removed explicit params for now
    ...
    exchange: ccxt.Exchange
     ...
     try:
         ...
         if quantity > max_amount:
             ...
             quantity = max_amount # Cap quantity to the max limit
         ...
         logger.warning(f"{Fore.YELLOW}Risk Calc: Could not fetch market limits for {symbol}: {e}. Skipping limit checks.{Style.RESET_ALL}")
     except (InvalidOperation, ValueError, TypeError) as e:
...
def place_risked_market_order(
    ...
    try:
        ...
        if sl_tp_success:
            ...
            logger.error(f"{Back.RED}{Fore.WHITE}{log_prefix}: Entry filled (ID:...{order_id[-6:]}), but FAILED to place one/both SL/TP orders. MANUAL INTERVENTION REQUIRED!{Style.RESET_ALL}")
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, timeframe: str) -> None:
    ...
    try:
        ...
        # if pd.isna(st_long_signal) or pd.isna(st_short_signal):
        #      raise ValueError("Supertrend flip signals (st_long_flip, st_short_flip) are NaN.")
        ...
        if selected_side:
            ...
            if volume_confirmed and ob_confirmed:
                ...
                try:
                    ...
                    if price_precision_str is None: raise ValueError("Could not determine price precision.")
