...
market_data_cache = TTLCache(
    maxsize=5, ttl=API_CACHE_TTL_SECONDS
)  # Smaller cache for OHLCV
...
def quantize_value(value: Decimal, step: Decimal, rounding_mode=ROUND_DOWN) -> Decimal:
    ...
    if step is None or step <= Decimal(0):
        return value  # No quantization if step is invalid
...
def format_price(
    ...
    quantized_price = quantize_value(
        price, tick_size, ROUND_HALF_UP
    )  # Round to nearest tick for display
    ...
    if notification_type == "termux":
        try:
            ...
            if process.returncode == 0:
                ...
                logger_notify.error(
                    ...
                    extra={"data": {"stderr": stderr.decode().strip()}},
                )
        ...
        if telegram_bot_token and telegram_chat_id:
            try:
                ...
                escaped_message = (
                    full_combined_message.replace("_", "\\_")
                    .replace("*", "\\*")
                    .replace("[", "\\[")
                    .replace("`", "\\`")
                )
            ...
            logger_notify.warning(
                "Telegram notifications configured but TOKEN or CHAT_ID missing in environment."
            )
...
def send_general_notification(
    title: str, message: str, notification_id: Optional[int] = None
) -> None:
    global notification_buffer, last_notification_flush_time
    if (
    ...
    actual_notification_id = (
        notification_id if notification_id is not None else random.randint(1000, 9999)
    )
...
class TradeMetrics:
    ...
    def load_trades_from_list(self, trades_list: List[Dict[str, Any]]):
        ...
        self.logger.info(
            f"TradeMetrics: Re-inked {len(self.trades)} sagas from persistent state."
        )
...
async def trading_loop_iteration(
    ...
    try:
        ...
        if (
            config.enhancements["whipsaw_cooldown_enable"]
            and time.time() < whipsaw_cooldown_active_until
        ):
            logger_loop.warning(
                f"In whipsaw cooldown. Ends in {whipsaw_cooldown_active_until - time.time():.0f}s."
            )
        ...
        if current_balance is None:
            logger_loop.error(
                "Failed to fetch balance for current iteration. Skipping."
            )
            return
        ...
        if current_balance < config.trading["min_usdt_balance_for_trading"]:
            logger_loop.critical(
                f"Treasury ({current_balance:.2f} {config.usdt_symbol}) below prudence ward ({config.trading['min_usdt_balance_for_trading']} {config.usdt_symbol}). Halting further trade entries.",
                extra={
                    "data": {
                        "current_balance": str(current_balance),
                        "min_balance": str(
                            config.trading["min_usdt_balance_for_trading"]
                        ),
                    }
                },
            )
        ...
        if drawdown_hit:
            logger_loop.critical(
                f"Max drawdown! {dd_reason}. Pyrmethus rests. Closing all positions."
            )
            # await async_close_all_symbol_positions(async_exchange, config, "Max Drawdown Reached") # Implement this
            raise SystemExit(
                "Max Drawdown Reached - System Halt"
            )  # Or a custom exception to stop main loop
        ...
        try:
            ...
            ohlcv_df_rest = await async_get_market_data(
                async_exchange,
            ...
            if ohlcv_df_rest is not None and not ohlcv_df_rest.empty:
                latest_close_rest = safe_decimal_conversion(
                    ohlcv_df_rest["close"].iloc[-1]
                )
                if latest_close_rest is not None:
                    latest_price_data = {
                        "timestamp": int(
                            ohlcv_df_rest["timestamp"].iloc[-1].timestamp() * 1000
                        ),
                        "close": latest_close_rest,
                ...
                logger_loop.warning(
                    "Failed to get latest price from REST fallback. Skipping iteration."
                )
                return
        ...
        if ohlcv_df is None or ohlcv_df.empty:
            logger_loop.warning(
                "Market whispers faint (OHLCV for indicators). Retrying next cycle."
            )
            return
        ...
        if pd.isna(latest_close) or pd.isna(latest_atr) or latest_atr <= Decimal(0):
            logger_loop.warning(
                f"Missing latest_close ({latest_close}) or latest_atr ({latest_atr}) or invalid ATR. No decisions.",
                extra={
                    "data": {
                        "latest_close": str(latest_close),
                        "latest_atr": str(latest_atr),
                    }
                },
            )
            return
        ...
        for part in active_parts_copy:
            ...
            if config.risk_management["enable_trailing_sl"] and latest_atr > 0:
                ...
                current_unrealized_pnl_per_unit = (
                    (latest_close - part["entry_price"])
                    if part["side"] == config.pos_long
                    else (part["entry_price"] - latest_close)
                )
                current_profit_atr = (
                    current_unrealized_pnl_per_unit / latest_atr
                    if latest_atr > 0
                    else Decimal(0)
                )
                ...
                if (
                    current_profit_atr
                    >= config.risk_management["trailing_sl_trigger_atr"]
                ):
                    # Calculate new TSL price
                    new_trailing_sl_price = (
                        (
                            latest_close
                            - (
                                latest_atr
                                * config.risk_management["trailing_sl_distance_atr"]
                            )
                        )
                        if part["side"] == config.pos_long
                        else (
                    ...
                    current_sl_on_exchange = part[
                        "sl_price"
                    ]  # SL as known by our state (should reflect exchange)
                    should_update_tsl = False
                    if part["side"] == config.pos_long:
                        if (
                            new_trailing_sl_price > current_sl_on_exchange
                            and new_trailing_sl_price < latest_close
                        ):
                    ...
                    if should_update_tsl:
                        logger_loop.info(
                            f"Trailing SL triggered for part {part['part_id']}. Current SL: {format_price(current_sl_on_exchange, config.tick_size)}, Profit ATR: {current_profit_atr:.2f}. Attempting new SL: {format_price(new_trailing_sl_price, config.tick_size)}"
                        )
        ...
        logger_loop.critical(
            f"Authentication Error during trading loop: {e_auth_loop}. Bot must stop.",
            exc_info=True,
        )
        send_general_notification(
            "Pyrmethus CRITICAL Auth Fail (Loop)",
            f"API Key Invalid/Revoked: {str(e_auth_loop)[:100]}",
        )
        raise  # Propagate to stop main execution
    except ccxt.NetworkError as e_net_loop:
        logger_loop.error(
            f"Network Error during trading loop: {e_net_loop}. Will retry.",
            exc_info=LOGGING_LEVEL <= logging.DEBUG,
        )
        # Handled by retry decorators on individual functions or main loop can pause
    except Exception as e_loop:
        logger_loop.error(
            f"Unhandled error in trading loop iteration: {e_loop}", exc_info=True
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        initial_logger.critical(
            f"An unhandled astral disturbance occurred at the highest level: {e_top_level}",
            exc_info=True,
        )
        if (
            "CONFIG" in globals()
