...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.shallow_ob_fetch_depth: int = (
            5  # Depth for quick price estimates (used in order placement estimate)
        )
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.trailing_stop_percentage <= 0 or self.trailing_stop_percentage >= 1:
            raise ValueError(
                "TRAILING_STOP_PERCENTAGE must be between 0 and 1 (exclusive)."
            )
    ...
    def _get_env(
        ...
        if value_str is None:
            ...
            ):  # Changed logic: Required is only critical if default is also None
                logger.critical(
        ...
        if value_to_cast is None:
            ...
            if (
                required
            ):  # This case should now be caught above if default was also None
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}CRITICAL: Required configuration rune '{key}' resolved to None unexpectedly.{Style.RESET_ALL}"
                )
                raise ValueError(
        ...
        try:
            raw_value_str = str(
                value_to_cast
            )  # Ensure string representation for casting logic
            if cast_type == bool:
            ...
            if default is None:
                ...
                try:
                    ...
                    if cast_type == bool:
                        final_value = default_str.lower() in [
                            "true",
                            "1",
                            "yes",
                            "y",
...
try:
    ...
    if os.getenv("ENABLE_SMS_ALERTS", "false").lower() == "true" and os.getenv(
        ...
        try:
            ...
            send_sms_alert(
                f"[Pyrmethus] CRITICAL CONFIG ERROR: {config_error}. Bot failed to start."
            )
    ...
    if os.getenv("ENABLE_SMS_ALERTS", "false").lower() == "true" and os.getenv(
        ...
        try:
            ...
            globals()["CONFIG"] = temp_config_for_sms
            send_sms_alert(
                f"[Pyrmethus] UNEXPECTED CONFIG ERROR: {type(general_config_error).__name__}. Bot failed."
            )
            globals()["CONFIG"] = _original_config
        except Exception as sms_err:
            logger.error(
                f"Failed to send SMS alert about unexpected config error: {sms_err}"
            )
    sys.exit(1)
...
def format_price(exchange: ccxt.Exchange, symbol: str, price: float | Decimal) -> str:
    ...
    try:
        ...
        if Decimal(formatted_price) == 0 and Decimal(str(price)) != 0:
            logger.warning(
                f"Price formatting resulted in zero for non-zero input {price}. Using Decimal normalize."
            )
...
def format_amount(exchange: ccxt.Exchange, symbol: str, amount: float | Decimal) -> str:
    ...
    try:
        ...
        if Decimal(formatted_amount) == 0 and Decimal(str(amount)) != 0:
            logger.warning(
                f"Amount formatting resulted in zero for non-zero input {amount}. Using Decimal normalize."
            )
...
def initialize_exchange() -> ccxt.Exchange | None:
    ...
    try:
        ...
        try:
            ...
            send_sms_alert(
                f"[Pyrmethus] CRITICAL: Authentication FAILED ({auth_err}). Spell failed."
            )
            return None
            ...
            send_sms_alert(
                f"[Pyrmethus] CRITICAL: Exchange Error on Init ({ex_err}). Spell failed."
            )
            return None
    ...
    # --- Broader Error Handling for Initialization ---
        ...
        send_sms_alert(
            f"[Pyrmethus] CRITICAL: Authentication FAILED on setup ({e}). Spell failed."
        )
...
def calculate_supertrend(
    ...
    multiplier_float_str = str(float(multiplier)).replace(
        ".", "_"
    )  # Match potential pandas_ta naming convention
    ...
    try:
        ...
        actual_st_col = next(
            (
                ...
                if col.startswith(f"SUPERT_{length}_{multiplier_float_str[:3]}")
                and not col.endswith(("d", "l", "s"))
            ),
            None,
        )  # Find main ST line
        actual_trend_col = next(
            (
                col
                for col in df.columns
                if col.startswith(f"SUPERTd_{length}_{multiplier_float_str[:3]}")
            ),
            None,
        )  # Find trend direction
        actual_long_col = next(
            (
                col
                for col in df.columns
                if col.startswith(f"SUPERTl_{length}_{multiplier_float_str[:3]}")
            ),
            None,
        )  # Find long flip signal
        actual_short_col = next(
            (
                col
                for col in df.columns
                if col.startswith(f"SUPERTs_{length}_{multiplier_float_str[:3]}")
            ),
            None,
        )  # Find short flip signal

        if not all(
            [actual_st_col, actual_trend_col, actual_long_col, actual_short_col]
        ):
            ...
            logger.error(
                f"{Fore.RED}Scrying ({col_prefix}ST): Failed to find all expected output columns from pandas_ta. Missing: {missing_details}. Check pandas_ta version/behavior.{Style.RESET_ALL}"
            )
...
def analyze_volume_atr(
    ...
    )  # Need at least period+1 for reliable calculation
    ...
    try:
        ...
        if atr_col in df.columns:
            ...
            logger.warning(
                f"ATR column '{atr_col}' not found after calculation. Check pandas_ta behavior."
            )
        ...
        if (
            ...
            results["volume_ratio"] = (
                None  # Cannot calculate ratio if MA is zero/negligible or volume is NA
            )
...
def calculate_stochrsi_momentum(
    ...
    try:
        ...
        if k_col in stochrsi_df.columns:
            ...
            logger.warning(
                f"StochRSI K column '{k_col}' not found after calculation. Check pandas_ta naming."
            )
            df["stochrsi_k"] = pd.NA
        if d_col in stochrsi_df.columns:
            ...
            logger.warning(
                f"StochRSI D column '{d_col}' not found after calculation. Check pandas_ta naming."
            )
            df["stochrsi_d"] = pd.NA
        ...
        if mom_col in df.columns:
            ...
            logger.warning(
                f"Momentum column '{mom_col}' not found after calculation. Check pandas_ta naming."
            )
            df["momentum"] = pd.NA
        ...
        if pd.notna(k_val) and pd.notna(d_val) and pd.notna(mom_val):
            ...
            logger.debug(
                "Scrying (StochRSI/Mom): Resulted in NA for one or more values on last candle."
            )
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    try:
        ...
        if fish_col in fisher_df.columns:
            ...
            logger.warning(
                f"Ehlers Fisher column '{fish_col}' not found after calculation. Check pandas_ta naming."
            )
        ...
        if signal_col in fisher_df.columns:
            ...
            if signal == 1 and fish_col in fisher_df.columns:
                logger.debug(
                    f"Ehlers Fisher signal length is 1, using Fisher line '{fish_col}' as signal."
                )
                df["ehlers_signal"] = df[
                    "ehlers_fisher"
                ]  # Use Fisher line itself as signal
            else:
                logger.warning(
                    f"Ehlers Signal column '{signal_col}' not found after calculation. Check pandas_ta naming."
                )
        ...
        if pd.notna(fish_val) and pd.notna(sig_val):
            ...
            logger.debug(
                "Scrying (EhlersFisher): Resulted in NA for one or more values on last candle."
            )
    except Exception as e:
        logger.error(
...
def calculate_ehlers_ma(df: pd.DataFrame, fast_len: int, slow_len: int) -> pd.DataFrame:
    ...
    *** WARNING: This is NOT a true Ehlers MA implementation. ***
    ...
    if (
        ...
        logger.warning(
            f"{Fore.YELLOW}Scrying (EhlersMA - EMA Placeholder): Insufficient data (Len: {len(df) if df is not None else 0}, Need ~{min_len}). Cannot calculate.{Style.RESET_ALL}"
        )
        for col in target_cols:
            df[col] = pd.NA
        return df
    try:
        ...
        logger.debug(
            f"Scrying (EhlersMA - EMA Placeholder): Calculating Fast EMA({fast_len}), Slow EMA({slow_len})"
        )
        ...
        if pd.notna(fast_val) and pd.notna(slow_val):
            ...
            logger.debug(
                f"Scrying (EhlersMA({fast_len},{slow_len}) - EMA): Fast={cross_color}{fast_val:.4f}{Style.RESET_ALL}, Slow={cross_color}{slow_val:.4f}{Style.RESET_ALL}"
            )
        else:
            logger.debug(
                "Scrying (EhlersMA - EMA): Resulted in NA for one or more values on last candle."
            )
    except Exception as e:
        logger.error(
            f"{Fore.RED}Scrying (EhlersMA - EMA): Unexpected error during calculation: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        for col in target_cols:
            df[col] = pd.NA  # Nullify results on error
...
def analyze_order_book(
    ...
    try:
        ...
        best_bid = (
            safe_decimal_conversion(bids[0][0]) if bids and len(bids[0]) >= 1 else None
        )
        best_ask = (
            safe_decimal_conversion(asks[0][0]) if asks and len(asks[0]) >= 1 else None
        )
    ...
    try:
        ...
        for pos in fetched_positions:
            ...
            pos_side_v5 = pos_info.get(
                "side", "None"
            )  # V5 specific: 'Buy', 'Sell', or 'None'
            size_str = pos_info.get(
                "size", "0"
            )  # Position size as a string from the exchange
            ...
            if pos_market_id == market_id and position_idx == 0:
                ...
                if (
                    ...
                    active_pos_info = (
                        pos_info  # Store the raw info dict of the active position
                    )
        ...
        if active_pos_info:
            try:
                ...
                if abs(size) <= CONFIG.position_qty_epsilon or entry_price <= 0:
                    logger.warning(
                        f"{Fore.YELLOW}Position Check: Found active V5 pos for {market_id} but parsed invalid size/entry: Size={size}, Entry={entry_price}. Treating as flat.{Style.RESET_ALL}"
                    )
                    return default_pos
                ...
                logger.warning(
                    f"{Fore.YELLOW}Position Check: Error parsing details from active V5 position data: {parse_err}. Data: {active_pos_info}. Treating as flat.{Style.RESET_ALL}"
                )
        ...
        logger.warning(
            f"{Fore.YELLOW}Position Check: Disturbance querying V5 positions for {symbol}: {type(e).__name__} - {e}. Assuming flat.{Style.RESET_ALL}"
        )
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    try:
        ...
        if market.get("linear"):
            ...
            logger.warning(
                f"{Fore.YELLOW}Leverage Conjuring: Could not determine category for {symbol}. Assuming 'linear'.{Style.RESET_ALL}"
            )
            category = "linear"
    ...
    for attempt in range(CONFIG.retry_count + 1):  # Total attempts = retry_count + 1
        try:
            ...
            if (
                ...
                or ("params error" in err_str and "leverage" in err_str)
            ):
                logger.info(
                    f"{Fore.CYAN}Leverage Conjuring: Leverage already set to {leverage}x for {symbol} (or not modified).{Style.RESET_ALL}"
                )
                return True
            ...
            if attempt >= CONFIG.retry_count:
                logger.error(
                    f"{Fore.RED}Leverage Conjuring: Failed after {CONFIG.retry_count + 1} attempts due to ExchangeError.{Style.RESET_ALL}"
                )
...
def close_position(
    ...
    market_base = symbol.split("/")[0].split(":")[
        0
    ]  # For concise alerts (e.g., BTC from BTC/USDT:USDT)
    logger.info(
        f"{Fore.YELLOW}Banish Position Ritual: Initiated for {symbol}. Reason: {reason}. Expected state: {initial_side} Qty={initial_qty:.8f}{Style.RESET_ALL}"
    )
    ...
    if (
        ...
        or abs(live_amount_to_close) <= CONFIG.position_qty_epsilon
    ):
        logger.warning(
        ...
        if initial_side != CONFIG.pos_none:
            ...
            logger.warning(
                f"{Fore.YELLOW}Banish Position: State Discrepancy! Expected {initial_side}, but live state is None/Zero.{Style.RESET_ALL}"
            )
    ...
    try:
        ...
        amount_dec = safe_decimal_conversion(
            amount_str
        ...
        market = exchange.market(symbol)
        category = (
            "linear" if market.get("linear") else "inverse"
        )  # Re-determine category
        params = {
            "category": category,
        ...
        if qty_diff < (
            ...
            logger.warning(
                f"{Fore.YELLOW}Banish Position: Order ({reason}) status/fill uncertain. Expected {amount_dec:.8f}, Filled: {filled_qty:.8f} (Diff: {qty_diff:.8e}). "
                f"ID:{order_id_short}, Status: {status}. Re-checking position state soon is advised.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] WARNING Banishing ({reason}): Fill mismatch? (Exp:{amount_dec:.8f}, Got:{filled_qty:.8f}). ID:{order_id_short}"
            )
    ...
    # --- Handle Specific Errors During Order Placement ---
        ...
        logger.error(
            f"{Fore.RED}Banish Position ({reason}): Insufficient funds error for {symbol}: {e}. This might indicate margin issues, incorrect state, or exchange problems.{Style.RESET_ALL}"
        )
        send_sms_alert(
        ...
        if any(indicator in err_str for indicator in already_closed_indicators):
            logger.warning(
                f"{Fore.YELLOW}Banish Position: Exchange indicates position likely already closed or zero size ({e}). Assuming banished.{Style.RESET_ALL}"
            )
            ...
            indicator in err_str for indicator in reduce_only_violation_indicators
        ):
            logger.error(
                f"{Fore.RED}Banish Position ({reason}): Reduce-Only Violation for {symbol}: {e}. Order likely incorrect or state mismatch.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ERROR Banishing ({reason}): Reduce-Only Violation! Check state."
            )
        else:
        ...
        logger.error(
            f"{Fore.RED}Banish Position ({reason}): Network/Timeout Error for {symbol}: {e}. Position state uncertain. Manual check advised.{Style.RESET_ALL}"
        )
        send_sms_alert(
        ...
        ValueError
    ) as e:  # Catches potential issues from format_amount or float conversion
        logger.error(
...
def calculate_position_size(
    ...
    try:
        ...
        logger.debug(
            f"Risk Calc: RiskAmt={risk_amount_usdt:.4f} USDT, PriceDiff={price_diff:.4f} USDT/Unit, Raw Qty={quantity_raw:.8f} Units"
        )

    except (DivisionByZero, InvalidOperation) as calc_err:
        logger.error(
            f"{Fore.RED}Risk Calc Error: Calculation failed (likely division by zero due to price_diff={price_diff}): {calc_err}{Style.RESET_ALL}"
        )
        return None, None
    ...
    try:
        ...
        logger.warning(
            f"{Fore.YELLOW}Risk Calc Warning: Failed precision shaping for quantity {quantity_raw:.8f}. Error: {fmt_err}. Attempting fallback quantization.{Style.RESET_ALL}"
        )
        ...
        try:
            ...
            amount_precision_digits = (
                exchange.markets[symbol].get("precision", {}).get("amount")
            )
            if amount_precision_digits:
                quantizer = Decimal(
                    "1e-" + str(int(amount_precision_digits))
                )  # e.g., Decimal('1e-3') for 3 digits
                quantity_precise = quantity_raw.quantize(
                    quantizer, rounding=ROUND_HALF_UP
                )  # Round half up
                logger.debug(
                    f"Risk Calc: Fallback Quantized Qty (Market Precision {amount_precision_digits})={quantity_precise:.8f}"
                )
            else:
                ...
                logger.debug(
                    f"Risk Calc: Fallback Quantized Qty (Default 8 Decimals)={quantity_precise:.8f}"
                )
    ...
    try:
        ...
        return (
            quantity_precise,
            None,
        )  # Allow potential capping logic to proceed if quantity is valid
...
def wait_for_order_fill(
    ...
    while time.monotonic() - start_time < timeout_seconds:
        ...
        try:
            ...
            logger.debug(
                f"Order {order_id_short} status: {status}, Filled: {filled_qty:.8f} ({elapsed_time:.1f}s elapsed)"
            )

            # --- Check for Terminal Statuses ---
            if (
                status == "closed"
            ):  # 'closed' typically means fully filled for market/limit, or triggered & filled for stop
                logger.success(
            ...
            sleep_interval = min(
                0.5 + elapsed_time * 0.05, 3.0
            )  # Start at 0.5s, increase slightly, max 3s
            time.sleep(sleep_interval)

        except ccxt.OrderNotFound:
            # This can happen briefly after placing, especially on busy exchanges. Keep trying within timeout.
            logger.warning(
                f"{Fore.YELLOW}Order {order_id_short} not found yet by exchange ({elapsed_time:.1f}s elapsed). Retrying...{Style.RESET_ALL}"
            )
            ...
            logger.warning(
                f"{Fore.YELLOW}Disturbance checking order {order_id_short} ({elapsed_time:.1f}s elapsed): {type(e).__name__} - {e}. Retrying...{Style.RESET_ALL}"
            )
            time.sleep(
                min(2.0 + elapsed_time * 0.1, 5.0)
            )  # Wait longer on error before retrying
        except Exception as e_unexp:
            # Handle unexpected errors during polling
            logger.error(
                f"{Fore.RED}Unexpected error checking order {order_id_short} ({elapsed_time:.1f}s elapsed): {e_unexp}. Stopping check.{Style.RESET_ALL}"
            )
            logger.debug(traceback.format_exc())
    ...
    try:
        ...
        logger.warning(
            f"Final status check for timed-out order {order_id_short}: Status='{final_check_order.get('status')}', Filled='{final_check_order.get('filled')}'"
        )
...
def place_risked_market_order(
    ...
    if current_atr is None or current_atr <= Decimal("0"):
        logger.error(
            f"{Fore.RED}Place Order Error ({side.upper()}): Invalid ATR ({current_atr}) provided. Cannot calculate SL distance or place order.{Style.RESET_ALL}"
        )
        return None
    ...
    try:
        ...
        price_tick_size = (
            safe_decimal_conversion(price_tick_size_str)
            if price_tick_size_str
            else None
        )
        amount_step = (
            safe_decimal_conversion(amount_step_str) if amount_step_str else None
        )
        ...
        usdt_total = safe_decimal_conversion(
            usdt_balance.get("total")
        )  # Total equity (incl. PnL)
        usdt_free = safe_decimal_conversion(
            usdt_balance.get("free")
        )  # Available for new orders/margin
        ...
        if usdt_equity is None or usdt_equity <= Decimal("0"):
            ...
            return None
        if usdt_free is None or usdt_free < Decimal(
            "0"
        ):  # Free margin shouldn't be negative
            logger.error(
        ...
        if price_tick_size:
            logger.debug(f"Market Precision: Price Tick={price_tick_size:.8f}")
        if amount_step:
            logger.debug(f"Market Precision: Amount Step={amount_step:.8f}")
        ...
        if side == CONFIG.side_buy and best_ask and best_ask > 0:
            ...
            try:
                ...
                if last_price > 0:
                    ...
                    logger.debug(
                        f"Using ticker last price for estimate: {entry_price_estimate}"
                    )
                else:
        ...
        if min_price is not None and initial_sl_price_raw < min_price:
            logger.warning(
                f"{Fore.YELLOW}Initial SL price estimate {initial_sl_price_raw:.4f} is below market min price {min_price}. Adjusting SL estimate up to min price.{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if initial_sl_price_estimate <= 0:
                raise ValueError("Formatted SL price estimate invalid (<=0)")
            # Ensure SL estimate isn't identical to entry estimate after formatting
            if abs(initial_sl_price_estimate - entry_price_estimate) < (
                price_tick_size or Decimal("1e-8")
            ):
                logger.error(
                    f"{Fore.RED}Place Order Error ({side.upper()}): Formatted initial SL price ({initial_sl_price_estimate}) is too close to entry estimate ({entry_price_estimate}). Increase ATR multiplier or check precision.{Style.RESET_ALL}"
                )
                return None
            logger.info(
                f"Calculated Initial SL Price (Estimate) ~ {Fore.YELLOW}{initial_sl_price_estimate:.4f}{Style.RESET_ALL} (Based on ATR: {current_atr:.4f}, Multiplier: {sl_atr_multiplier}, Dist: {sl_distance:.4f})"
            )
        ...
        if max_order_cap_usdt > 0 and pos_value_estimate > max_order_cap_usdt:
            ...
            try:
                ...
                final_quantity_str = format_amount(
                    exchange, symbol, final_quantity_capped_raw
                )
                ...
                if final_quantity > 0 and leverage > 0:
                    ...
                    req_margin_est = (
                        None  # Cannot estimate margin if capped qty is zero
                    )
        ...
        if req_margin_est is None:  # Check if margin calc failed at any point
            logger.error(
                f"{Fore.RED}Place Order Error ({side.upper()}): Failed to estimate required margin for quantity {final_quantity:.8f}. Cannot proceed.{Style.RESET_ALL}"
            )
            return None
        ...
        if usdt_free < req_margin_buffered:
            logger.error(
                f"{Fore.RED}Place Order Error ({side.upper()}): Insufficient FREE margin. Need ~{req_margin_buffered:.4f} (incl. buffer), Have {usdt_free:.4f}{Style.RESET_ALL}"
            )
            send_sms_alert(
        ...
        try:
            ...
            entry_fg_color = (
                Fore.BLACK
            )  # Use black text on green/red background for visibility
            logger.warning(
            ...
            entry_params = {"category": category, "reduceOnly": False}
            ...
            if isinstance(e, ccxt.InsufficientFunds):
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Insufficient Funds on Entry!"
                )
            else:
                send_sms_alert(
        ...
        if not filled_entry:
            ...
            logger.error(
                f"{Fore.RED}Entry order {format_order_id(order_id)} fill status UNKNOWN due to timeout or check error. Position state uncertain.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Entry {format_order_id(order_id)} fill TIMEOUT/ERROR. MANUAL CHECK!"
            )
            # Attempt to cancel the potentially stuck/unfilled order as a safety measure
            try:
                logger.warning(
                    f"Attempting to cancel potentially unconfirmed order {format_order_id(order_id)}"
                )
        ...
        if final_status != "closed":
            ...
            logger.error(
                f"{Fore.RED}Entry order {format_order_id(order_id)} did not confirm filled. Final Status: '{final_status}'. Position state uncertain.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Entry {format_order_id(order_id)} final status '{final_status}'. MANUAL CHECK!"
            )
            ...
            if final_status == "open":
                try:
                    logger.warning(
                        f"Attempting to cancel order {format_order_id(order_id)} stuck in '{final_status}' status."
                    )
                    cancel_params = {"category": category}
                    exchange.cancel_order(order_id, symbol, params=cancel_params)
                    logger.info(
                        f"Cancel request sent for stuck order {format_order_id(order_id)}"
                    )
                except Exception as cancel_e:
                    logger.warning(
                        f"Could not cancel stuck order {format_order_id(order_id)}: {cancel_e}"
                    )
        ...
        if qty_diff > (
            ...
            logger.warning(
                f"{Fore.YELLOW}Fill quantity mismatch! Expected: {final_quantity:.8f}, Filled: {filled_qty:.8f} (Diff: {qty_diff:.8e}). Using actual filled quantity for SL/TSL.{Style.RESET_ALL}"
            )
            # Proceed using the actual filled_qty, but log the discrepancy.

        if abs(filled_qty) <= CONFIG.position_qty_epsilon:
            logger.error(
                f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid fill quantity ({filled_qty}) reported for order {format_order_id(order_id)} despite 'closed' status. Position state unknown! MANUAL CHECK REQUIRED!{Style.RESET_ALL}"
            )
            send_sms_alert(
            ...
            if cost > 0 and filled_qty > 0:
                try:
                    ...
                    avg_fill_price = Decimal(
                        "-1"
                    )  # Mark as invalid if estimation fails
            else:
                avg_fill_price = Decimal("-1")  # Mark as invalid

        if avg_fill_price <= 0:
            logger.error(
                f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid fill price ({avg_fill_price}) reported or estimated for order {format_order_id(order_id)}. Cannot calculate SL accurately! MANUAL CHECK REQUIRED!{Style.RESET_ALL}"
            )
            send_sms_alert(
            ...
            close_position(
                ...
                symbol,
                emergency_pos_state,
                reason="Invalid Fill Price Post-Entry",
            )
        ...
        if actual_sl_price_raw <= 0:
            logger.error(
                f"{Back.RED}{Fore.WHITE}CRITICAL FAILURE: ACTUAL SL price calculation resulted in zero or negative value ({actual_sl_price_raw:.4f}) based on fill price {avg_fill_price:.4f}. Cannot place Fixed SL!{Style.RESET_ALL}"
            )
            # Position is open without fixed SL protection. TSL might still be placed.
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ({side.upper()}): Invalid ACTUAL Fixed SL price ({actual_sl_price_raw:.4f})! Fixed SL FAILED."
            )
            # Do not attempt emergency close here, let TSL attempt proceed.
        else:
            if min_price is not None and actual_sl_price_raw < min_price:
                logger.warning(
                    f"{Fore.YELLOW}Actual Fixed SL price {actual_sl_price_raw:.4f} is below market min price {min_price}. Adjusting SL up to min price.{Style.RESET_ALL}"
                )
                actual_sl_price_raw = min_price  # Adjust SL upwards

            # Format the final SL price
            try:
                actual_sl_price_str = format_price(
                    exchange, symbol, actual_sl_price_raw
                )
                actual_sl_price_float = float(actual_sl_price_str)  # For CCXT param
                if actual_sl_price_float <= 0:
                    raise ValueError("Formatted actual SL price invalid (<=0)")
                logger.info(
                ...
                try:
                    ...
                    if isinstance(e, ccxt.ExchangeError):
                        logger.debug(f"Fixed SL Placement Error Details: {e.args}")
                    send_sms_alert(
                ...
                logger.error(
                    f"{Back.RED}{Fore.WHITE}CRITICAL FAILURE: Failed to format/validate ACTUAL Fixed SL price '{actual_sl_price_raw:.4f}': {e_format}. Cannot place Fixed SL!{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ({side.upper()}): Failed format/validate ACTUAL Fixed SL price! Fixed SL FAILED."
                )
        ...
        try:
            ...
            if act_price_raw <= 0:
                raise ValueError(
                    f"Invalid TSL activation price calculated (<=0): {act_price_raw:.4f}"
                )

            if min_price is not None and act_price_raw < min_price:
                logger.warning(
                    f"{Fore.YELLOW}TSL activation price {act_price_raw:.4f} is below market min price {min_price}. Adjusting activation up to min price.{Style.RESET_ALL}"
                )
            ...
            if tsl_act_price_float <= 0:
                raise ValueError("Formatted TSL activation price invalid (<=0)")
            ...
            if isinstance(e, ccxt.ExchangeError):
                logger.debug(f"TSL Placement Error Details: {e.args}")
            send_sms_alert(
        ...
        sl_status = (
            f"~{actual_sl_price_str}(ID:{sl_order_id_short})"
            if sl_placed_successfully
            else f"{Fore.RED}FAIL{Style.RESET_ALL}"
        )
        tsl_status = (
            f"{tsl_percent:.2%}@~{tsl_act_price_str}(ID:{tsl_order_id_short})"
            if tsl_placed_successfully
            else f"{Fore.RED}FAIL{Style.RESET_ALL}"
        )
        sms_msg = (
        ...
        if not sl_placed_successfully and not tsl_placed_successfully:
            ...
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ({side.upper()}): BOTH SL & TSL FAILED! POS UNPROTECTED! EntryID:{format_order_id(order_id)}"
            )
...
def cancel_open_orders(
    ...
    try:
        ...
        if not exchange.has.get("cancelAllOrders"):
            logger.warning(
                f"{Fore.YELLOW}Order Cleanup: Exchange '{exchange.id}' CCXT instance does not directly support cancelAllOrders. CCXT might emulate. Proceeding cautiously.{Style.RESET_ALL}"
            )
        ...
        logger.info(
            f"{Fore.CYAN}Order Cleanup: cancel_all_orders request sent for {symbol}. Response: {response}{Style.RESET_ALL}"
        )
        ...
        if (
            ...
            or "no orders found" in err_str
            or "110001" in err_str
        ):
            logger.info(
                f"{Fore.CYAN}Order Cleanup: No open orders found for {symbol} to cancel (or exchange reported success despite no orders).{Style.RESET_ALL}"
            )
            cancelled_count = 0  # No orders were actually cancelled
        else:
            logger.error(
                f"{Fore.RED}Order Cleanup: FAILED to cancel orders for {symbol}: {type(e).__name__} - {e}{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] WARNING: Failed cancelAllOrders ({reason}): {type(e).__name__}. Check manually."
            )
            cancelled_count = -1  # Indicate failure
    except (ccxt.NetworkError, ccxt.RequestTimeout) as e:
        logger.error(
            f"{Fore.RED}Order Cleanup: Network/Timeout error during cancelAllOrders for {symbol}: {e}{Style.RESET_ALL}"
        )
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] WARNING: Network/Timeout on cancelAllOrders ({reason}). Check manually."
        )
        cancelled_count = -1  # Indicate failure
    except Exception as e_cancel:
        logger.error(
            f"{Fore.RED}Order Cleanup: Unexpected error during cancelAllOrders for {symbol}: {type(e_cancel).__name__} - {e_cancel}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] WARNING: Unexpected error on cancelAllOrders ({reason}). Check manually."
        )
        cancelled_count = -1  # Indicate failure
    ...
    if cancelled_count >= 0:
        logger.info(
            f"{Fore.CYAN}Order Cleanup Ritual Finished for {symbol}. Attempt successful (reported {cancelled_count} actions/attempts).{Style.RESET_ALL}"
        )
    else:
        logger.error(
            f"{Fore.RED}Order Cleanup Ritual Encountered Errors for {symbol}. Manual check of open orders recommended.{Style.RESET_ALL}"
        )
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> dict[str, Any]:
    ...
    if df is None or len(df) < required_rows:
        logger.debug(
            f"Signal Gen ({strategy_name}): Insufficient data ({len(df) if df is not None else 0} rows, need {required_rows}) for signal generation."
        )
    ...
    try:
        ...
        logger.error(
            f"Signal Gen ({strategy_name}): Error accessing DataFrame rows -1 or -2 (len: {len(df)}). Cannot generate signals."
        )
        return signals
    ...
    try:
        ...
        if strategy_name == "DUAL_SUPERTREND":
            ...
            primary_long_flip = last.get(
                "st_long", False
            )  # True if primary ST flipped long this candle
            primary_short_flip = last.get(
                "st_short", False
            )  # True if primary ST flipped short this candle
            confirm_is_up = last.get(
                "confirm_trend", pd.NA
            )  # Boolean: True if confirmation ST is currently up

            # Validate necessary data is available
            if (
                pd.isna(confirm_is_up)
                or primary_long_flip is None
                or primary_short_flip is None
            ):
                logger.warning(
                    f"Signal Gen ({strategy_name}): Skipping due to missing indicator values (PrimaryFlipL={primary_long_flip}, PrimaryFlipS={primary_short_flip}, ConfirmUp={confirm_is_up})."
                )
                return signals
        ...
        # --- Ehlers MA Cross Logic (Using EMA Placeholder) ---
            ...
            if fast_ma_prev >= slow_ma_prev and fast_ma_now < slow_ma_now:
                signals["exit_long"] = True
                signals["exit_reason"] = (
                    f"Fast EMA({CONFIG.ehlers_fast_period}) crossed below Slow EMA({CONFIG.ehlers_slow_period})"
                )
            # Exit Short: Fast EMA crosses above Slow EMA (same condition as enter long)
            if fast_ma_prev <= slow_ma_prev and fast_ma_now > slow_ma_now:
                signals["exit_short"] = True
                signals["exit_reason"] = (
                    f"Fast EMA({CONFIG.ehlers_fast_period}) crossed above Slow EMA({CONFIG.ehlers_slow_period})"
                )

    # --- Error Handling for Signal Generation ---
    except KeyError as e:
        logger.error(
            f"{Fore.RED}Signal Generation Error ({strategy_name}): Missing expected indicator column in DataFrame: {e}. Check indicator calculation functions and prefixes.{Style.RESET_ALL}"
        )
        # Reset all signals to False on critical error
        signals = {k: False if isinstance(v, bool) else v for k, v in signals.items()}
    except TypeError as e:
        logger.error(
            f"{Fore.RED}Signal Generation Error ({strategy_name}): Type error during comparison, likely due to unexpected None/NA value: {e}. Check indicator data.{Style.RESET_ALL}"
        )
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    if df is None or df.empty:
        logger.warning(
            f"{Fore.YELLOW}Trade Logic Skipped: Invalid DataFrame received.{Style.RESET_ALL}"
        )
        return

    action_taken_this_cycle: bool = (
        False  # Track if an entry/exit order was placed/attempted
    )
    ...
    try:
        ...
        current_atr = vol_atr_data.get(
            "atr"
        )  # Crucial for SL calculation and entry possibility
        ...
        if pd.isna(current_price) or current_price <= 0:
            logger.warning(
                f"{Fore.YELLOW}Trade Logic Skipped: Last candle close price is invalid ({current_price}). Data might be corrupt.{Style.RESET_ALL}"
            )
            return
        ...
        if not can_place_new_order:
            ...
            logger.warning(
                f"{Fore.YELLOW}Invalid ATR ({current_atr}) calculated. Cannot calculate SL or place NEW entry orders this cycle. Exit logic may still function.{Style.RESET_ALL}"
            )
        ...
        # Log core state
        atr_str = (
            f"{current_atr:.5f}" if current_atr else f"{Fore.RED}N/A{Style.RESET_ALL}"
        )
        price_color = (
            Fore.GREEN
        ...
        logger.info(
            f"State | Price: {price_color}{current_price:.4f}{Style.RESET_ALL}, ATR({CONFIG.atr_calculation_period}): {Fore.MAGENTA}{atr_str}{Style.RESET_ALL}"
        )
        ...
        if bid_ask_ratio is not None:
            if bid_ask_ratio >= CONFIG.order_book_ratio_threshold_long:
                ob_ratio_color = Fore.GREEN
            elif bid_ask_ratio <= CONFIG.order_book_ratio_threshold_short:
                ob_ratio_color = Fore.RED
            else:
                ob_ratio_color = Fore.YELLOW
        logger.info(
            f"State | OrderBook: Ratio(B/A)={ob_ratio_color}{ob_ratio_str}{Style.RESET_ALL} (L >= {CONFIG.order_book_ratio_threshold_long}, S <= {CONFIG.order_book_ratio_threshold_short}), Spread={ob_spread_str} (Fetched This Cycle={ob_data is not None})"
        )
        ...
        if should_exit_long or should_exit_short:
            ...
            cancel_result = cancel_open_orders(
                exchange, symbol, f"Pre-Exit Cleanup ({exit_reason})"
            )
            if cancel_result < 0:  # Check if cancellation failed
                logger.error(
                    f"{Fore.RED}Failed to cancel open orders before exit attempt. Proceeding with caution.{Style.RESET_ALL}"
                )
                # Decide whether to proceed with close or abort? For now, proceed but log error.

            time.sleep(
                1.0
            )  # Increased pause after cancel request before placing market close

            # --- Attempt to close the position ---
            close_result_order = close_position(
            ...
            if close_result_order:
                ...
                logger.info(
                    f"Position close order placed for {position_side}. Pausing briefly..."
                )
                time.sleep(
                    CONFIG.post_close_delay_seconds
                )  # Pause after successful close attempt
            else:
                ...
                logger.error(
                    f"{Fore.RED}Failed to place position close order for {position_side}, or position was already closed. Check logs from close_position.{Style.RESET_ALL}"
                )
        ...
        if position_side != CONFIG.pos_none:
            ...
            logger.info(
                f"Holding {pos_color}{position_side}{Style.RESET_ALL} position. Awaiting Exchange SL/TSL trigger or next Strategy Exit signal."
            )
        ...
        if ob_data is None:
            ...
            if bid_ask_ratio is not None:
                if bid_ask_ratio >= CONFIG.order_book_ratio_threshold_long:
                    ...
                    ob_ratio_color = Fore.YELLOW
            logger.info(
                f"State | OrderBook (Post-Fetch): Ratio(B/A)={ob_ratio_color}{ob_ratio_str}{Style.RESET_ALL}, Spread={ob_spread_str_fmt}"
            )
        ...
        ob_filter_log = (
            f"Filter | OB Confirm Long: {ob_confirm_long} (Ratio: {ob_ratio_str} >= {CONFIG.order_book_ratio_threshold_long}), "
        ...
        if base_enter_long:
            ...
            logger.debug(
                f"Final Entry Check (Long): Strategy={base_enter_long}, Filters OK (Vol={vol_confirm}, OB={ob_confirm_long}) = {filter_met_long} => {Fore.GREEN if final_enter_long else Fore.RED}Enter={final_enter_long}{Style.RESET_ALL}"
            )
        if base_enter_short:
            filter_met_short = vol_confirm and ob_confirm_short
            logger.debug(
                f"Final Entry Check (Short): Strategy={base_enter_short}, Filters OK (Vol={vol_confirm}, OB={ob_confirm_short}) = {filter_met_short} => {Fore.GREEN if final_enter_short else Fore.RED}Enter={final_enter_short}{Style.RESET_ALL}"
            )
        ...
        if final_enter_long:
            ...
            logger.success(
                f"{entry_bg_color}{entry_fg_color}{Style.BRIGHT}*** CONFIRMED LONG ENTRY SIGNAL ({CONFIG.strategy_name}) for {symbol} ***{Style.RESET_ALL}"
            )
            ...
            entry_fg_color = Fore.BLACK
            logger.success(
                f"{entry_bg_color}{entry_fg_color}{Style.BRIGHT}*** CONFIRMED SHORT ENTRY SIGNAL ({CONFIG.strategy_name}) for {symbol} ***{Style.RESET_ALL}"
            )
        ...
        if entry_side:
            ...
            cancel_result = cancel_open_orders(
                exchange, symbol, f"Pre-{entry_side.upper()} Entry Cleanup"
            )
            if cancel_result < 0:
                logger.error(
                    f"{Fore.RED}Failed pre-entry order cleanup. Attempting entry anyway, but check for stray orders manually.{Style.RESET_ALL}"
                )
            ...
            if place_result_order:
                logger.info(
                    f"{entry_side.upper()} entry order placement process initiated and returned order details (ID: {format_order_id(place_result_order.get('id'))}). Check logs for SL/TSL status."
                )
                ...
                logger.error(
                    f"{Fore.RED}{entry_side.upper()} entry order placement process failed or was aborted. Check previous logs.{Style.RESET_ALL}"
                )
            ...
            if potential_entry_signal and not action_taken_this_cycle:
                reason_blocked = []
                if base_enter_long and not vol_confirm:
                    reason_blocked.append("Volume")
                if base_enter_long and not ob_confirm_long:
                    reason_blocked.append("OrderBook(L)")
                if base_enter_short and not vol_confirm:
                    reason_blocked.append("Volume")
                if base_enter_short and not ob_confirm_short:
                    reason_blocked.append("OrderBook(S)")
                logger.info(
                    f"Holding Cash. Strategy signal present but confirmation filters blocked entry (Blocked by: {', '.join(reason_blocked)})."
                )
        ...
        market_base = symbol.split("/")[0].split(":")[0]
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ERROR in trade_logic cycle: {type(e).__name__}. Check logs!"
        )
    finally:
...
def graceful_shutdown(exchange: ccxt.Exchange | None, symbol: str | None) -> None:
    ...
    if not exchange or not symbol:
        ...
        send_sms_alert(
            "[Pyrmethus] Shutdown: Exchange/Symbol missing, cleanup skipped."
        )
        return
    ...
    try:
        ...
        if cancel_result < 0:
            logger.error(
                "Shutdown Step 1: Encountered errors cancelling orders. Proceeding with position check."
            )
        else:
            logger.info("Shutdown Step 1: Cancel order attempt finished.")
        ...
        if (
            ...
            and abs(position["qty"]) > CONFIG.position_qty_epsilon
        ):
            ...
            close_result_order = close_position(
                exchange, symbol, position, reason="Graceful Shutdown"
            )

            if close_result_order:
                # Close order placed successfully (or seemed to be)
                wait_time = (
                    CONFIG.post_close_delay_seconds * 3
                )  # Wait longer during shutdown
                logger.info(
                    f"{Fore.CYAN}Shutdown Step 2: Close order placed/processed. Waiting {wait_time}s for exchange state update...{Style.RESET_ALL}"
                )
                ...
                if (
                    ...
                    or abs(final_pos["qty"]) <= CONFIG.position_qty_epsilon
                ):
                    logger.success(
                        f"{Fore.GREEN}{Style.BRIGHT}Shutdown Step 3: Position confirmed CLOSED/FLAT after shutdown attempt.{Style.RESET_ALL}"
                    )
                    send_sms_alert(
                        f"[{market_base}/{CONFIG.strategy_name}] Position confirmed CLOSED on shutdown ({mode_str})."
                    )
                else:
                    ...
                    logger.critical(
                        f"{Back.RED}{Fore.WHITE}CRITICAL SHUTDOWN FAILURE ({mode_str}): FAILED TO CONFIRM position closure after waiting! "
                    ...
                    send_sms_alert(
                        f"[{market_base}/{CONFIG.strategy_name}] CRITICAL SHUTDOWN ERROR ({mode_str}): Failed confirm closure! Final: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK!"
                    )
            else:
                # Close order placement failed initially, or position was already gone
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}CRITICAL SHUTDOWN FAILURE ({mode_str}): Failed to place position close order during shutdown (or position already closed). "
                    f"Final check recommended. MANUAL INTERVENTION MAY BE REQUIRED on Bybit!{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL SHUTDOWN ERROR ({mode_str}): Failed PLACE close order OR already closed. MANUAL CHECK!"
                )
                # Perform one last check just in case
                logger.warning(
                    "Performing final position check after close placement failure..."
                )
                final_pos_after_fail = get_current_position(exchange, symbol)
                if (
                    final_pos_after_fail["side"] == CONFIG.pos_none
                    or abs(final_pos_after_fail["qty"]) <= CONFIG.position_qty_epsilon
                ):
                    logger.info(
                        f"{Fore.GREEN}Shutdown: Final check after failed close attempt shows FLAT state. Likely closed before or by failed attempt.{Style.RESET_ALL}"
                    )
                else:
                    logger.critical(
                        f"{Back.RED}{Fore.WHITE}Shutdown: Final check confirms position STILL OPEN after failed close attempt! State: {final_pos_after_fail['side']} Qty={final_pos_after_fail['qty']:.8f}. MANUAL ACTION NEEDED!{Style.RESET_ALL}"
                    )

        else:
            # No position found initially in step 2
            logger.info(
                f"{Fore.GREEN}Shutdown Step 2: No active position found for {symbol}. Clean exit state.{Style.RESET_ALL}"
            )
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] No active position found on shutdown ({mode_str})."
            )

    except Exception as e:
        logger.error(
            f"{Fore.RED}Shutdown Error: Unexpected error during cleanup sequence for {symbol}: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] Error during shutdown cleanup ({mode_str}): {type(e).__name__}. Check logs & position manually."
        )

    logger.info(
        f"{Fore.YELLOW}{Style.BRIGHT}--- Pyrmethus Scalping Spell Shutdown Sequence Complete ({mode_str}) ---{Style.RESET_ALL}"
    )
...
def main() -> None:
    ...
    symbol_unified: str | None = (
        None  # The specific market symbol confirmed by CCXT (e.g., BTC/USDT:USDT)
    )
    ...
    try:
        ...
        try:
            ...
            symbol_unified = market[
                "symbol"
            ]  # Use the precise symbol recognized by CCXT (e.g., BTC/USDT:USDT)
            market_base = symbol_unified.split("/")[0].split(":")[
                0
            ]  # Update for SMS alerts
            ...
            logger.info(
                f"Market Details: Type={market_type}, Contract={is_contract}, Linear={is_linear}, Inverse={is_inverse}"
            )

            if not is_contract:
                raise ValueError(
                    f"Market '{symbol_unified}' (Type: {market_type}) is not a contract/futures/swap market suitable for this bot."
                )
            ...
            if not is_linear:
                ...
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}CRITICAL CONFIG WARNING: Market '{symbol_unified}' is not detected as LINEAR (USDT margined). "
                ...
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL WARNING: Market {symbol_unified} is NOT LINEAR (USDT). Bot logic may fail!"
                )
            ...
            logger.info(
                f"{Fore.GREEN}Spell successfully focused on Symbol: {symbol_unified}{Style.RESET_ALL}"
            )
            ...
            if not set_leverage(exchange, symbol_unified, CONFIG.leverage):
                ...
                raise RuntimeError(
                    f"Leverage conjuring failed for {symbol_unified}. Cannot proceed safely."
                )
            ...
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL: Symbol/Leverage setup FAILED ({type(e).__name__}). Exiting."
            )
            ...
            logger.debug(traceback.format_exc())
            send_sms_alert(
                f"[{market_base}/{CONFIG.strategy_name}] CRITICAL: Unexpected setup error ({type(e_setup).__name__}). Exiting."
            )
            run_bot = False  # Prevent loop from starting
            return  # Exit script
        ...
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] Pyrmethus Bot v2.2.1 Initialized ({mode_str}). Symbol: {symbol_unified}, Strat: {CONFIG.strategy_name}. Starting main loop."
        )
        ...
        while run_bot:
            ...
            try:
                ...
                data_limit = (
                    max(base_required, stochrsi_lookback, 100)
                    + CONFIG.api_fetch_limit_buffer
                )
                logger.debug(
                    f"Calculated data fetch limit for indicators: {data_limit}"
                )
                ...
                if (
                    df is not None and not df.empty and len(df) >= 2
                ):  # Need at least 2 rows for signal generation
                    ...
                    df = calculate_supertrend(
                        df.copy(), CONFIG.st_atr_length, CONFIG.st_multiplier
                    )  # Use copy to avoid side effects if needed elsewhere
                    df = calculate_supertrend(
                        df,
                        CONFIG.confirm_st_atr_length,
                    ...
                    )  # Uses EMA placeholder

                    # Pass the prepared DataFrame to the core logic function
                    trade_logic(
                        exchange, symbol_unified, df
                    )  # Pass the df with all indicators calculated
                elif df is not None and len(df) < 2:
                    logger.warning(
                        f"{Fore.YELLOW}Skipping trade logic: Not enough data rows ({len(df)}) returned after fetch/cleaning for signal generation.{Style.RESET_ALL}"
                    )
                else:
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] WARNING: Rate limit exceeded! Pausing {sleep_time}s."
                )
                time.sleep(sleep_time)
            except ccxt.NetworkError as e:
                # Transient network issues, usually recoverable
                logger.warning(
                    f"{Fore.YELLOW}Network disturbance in main loop: {e}. Retrying next cycle after delay.{Style.RESET_ALL}"
                )
                # Optional: Add counter for repeated network errors to trigger longer pause or alert
                time.sleep(
                    CONFIG.sleep_seconds * 2
                )  # Slightly longer sleep for network issues
            except ccxt.ExchangeNotAvailable as e:
                # Exchange might be down for maintenance or unavailable (e.g., Cloudflare issues)
                sleep_time = 60  # Wait a significant time
                logger.error(
                    f"{Back.RED}{Fore.WHITE}Exchange Not Available: {e}. Portal temporarily closed. Sleeping {sleep_time}s...{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] ERROR: Exchange unavailable ({type(e).__name__})! Pausing {sleep_time}s."
                )
                time.sleep(sleep_time)
            except ccxt.AuthenticationError as e:
                # API keys might have been revoked, expired, IP changed, or permissions altered
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}FATAL: Authentication Error encountered during operation: {e}. API keys invalid or permissions changed. Spell broken! Stopping NOW.{Style.RESET_ALL}"
                )
                send_sms_alert(
                    f"[{market_base}/{CONFIG.strategy_name}] CRITICAL: Authentication Error during operation! Bot stopping NOW. Check API keys/IP."
                )
                ...
                logger.debug(
                    f"ExchangeError Details: Type={type(e).__name__}, Args={e.args}"
                )
                ...
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}!!! UNEXPECTED CRITICAL CHAOS in Main Loop (Cycle {cycle_count}): {e} !!! Stopping spell!{Style.RESET_ALL}"
                )
                # logger.exception provides full traceback automatically in default logging setup if level >= ERROR
                logger.debug(
                    traceback.format_exc()
                )  # Explicitly log traceback for clarity
                send_sms_alert(
        ...
        logger.critical(
            f"{Back.RED}{Fore.WHITE}CRITICAL ERROR during bot startup sequence (before main loop): {startup_error}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        ...
        if CONFIG and CONFIG.enable_sms_alerts and CONFIG.sms_recipient_number:
            ...
            send_sms_alert(
                f"[{market_base}] CRITICAL STARTUP ERROR: {type(startup_error).__name__}. Bot failed to start."
            )
