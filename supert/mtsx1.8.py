...
try:
    import ccxt
    import pandas as pd
    if not hasattr(pd, 'NA'): # Pyrmethus: Ensure pandas version compatibility for pd.NA
    ...
    missing_pkg = getattr(e, 'name', 'a required dependency') # Slightly more descriptive default
    sys.stderr.write(
...
def send_termux_notification(title: str, message: str, notification_id: int = 777) -> None:
    ...
    if 'CONFIG' not in globals() or not hasattr(CONFIG, 'enable_notifications') or not CONFIG.enable_notifications:
        if 'CONFIG' in globals() and hasattr(CONFIG, 'enable_notifications'): 
            ...
            _logger.warning("Attempted Termux notification before CONFIG fully loaded or with notifications disabled.")
        return
    try:
        ...
        _logger.debug(f"{NEON['ACTION']}Attempting to send Termux notification: Title='{title}'{NEON['RESET']}")
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate(timeout=notification_timeout) # Use determined timeout
        if process.returncode == 0: 
            _logger.info(f"{NEON['SUCCESS']}Termux notification '{title}' sent successfully.{NEON['RESET']}")
        else:
            err_msg = stderr.decode().strip() if stderr else "Unknown error"
            _logger.error(f"{NEON['ERROR']}Failed to send Termux notification '{title}'. Return code: {process.returncode}. Error: {err_msg}{NEON['RESET']}")
    except FileNotFoundError: 
        _logger.error(f"{NEON['ERROR']}Termux API command 'termux-notification' not found.{NEON['RESET']}")
    except subprocess.TimeoutExpired: 
        _logger.error(f"{NEON['ERROR']}Termux notification command timed out for '{title}'.{NEON['RESET']}")
    except Exception as e:
        _logger.error(f"{NEON['ERROR']}Unexpected error sending Termux notification '{title}': {e}{NEON['RESET']}")
        _logger.debug(traceback.format_exc())

# --- Enums ---
class StrategyName(str, Enum): DUAL_SUPERTREND_MOMENTUM = "DUAL_SUPERTREND_MOMENTUM"; EHLERS_FISHER = "EHLERS_FISHER"
class VolatilityRegime(Enum): LOW = "LOW"; NORMAL = "NORMAL"; HIGH = "HIGH" # Note: VolatilityRegime is defined but not actively used in the provided script.
...
class Config:
    def __init__(self) -> None:
        ...
        self.default_recv_window: int = self._get_env("DEFAULT_RECV_WINDOW", 13000, cast_type=int) # Increased default
    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, color: str = NEON["PARAM"], secret: bool = False) -> Any:
        ...
        if value_to_cast is None: # Handles cases where default is None and env var is not set
        ...
        final_value: Any
        try:
            raw_value_str_for_cast = str(value_to_cast) # Ensures we are casting from a string representation
            ...
            elif cast_type == int: final_value = int(Decimal(raw_value_str_for_cast)) # Cast to Decimal first for float strings
            ...
            if default is None: # If casting initial value fails AND default is None
                ...
                try:
                    ...
                    else: final_value = default_str_for_cast # Fallback for unsupported cast_type on default
    ...
    def _validate_parameters(self) -> None:
        ...
        errors = []
        if not (0 < self.risk_per_trade_percentage < 1): errors.append("RISK_PER_TRADE_PERCENTAGE must be between 0 and 1 (exclusive).")
        ...
        if self.enable_anti_martingale_risk:
            ...
            if not (0 < self.max_risk_pct_anti_martingale < 1): errors.append("MAX_RISK_PCT_ANTI_MARTINGALE must be between 0 and 1 (exclusive).")
        if errors:
    ...
    for level, color_code in [(logging.DEBUG, NEON['DEBUG']), (logging.INFO, NEON['INFO']), (SUCCESS_LEVEL, NEON['SUCCESS']),
        ...
        if '\033' in level_name_str: 
            ...
            if not base_name.strip(): # If first part is empty or just spaces
                 ...
                 non_ansi_part = next((part.split('m', 1)[-1] for part in parts if 'm' in part and part.split('m', 1)[-1].strip()), None)
                 base_name = non_ansi_part if non_ansi_part else level_name_str # Fallback to original if complex
            level_name_str = base_name.strip()

        logging.addLevelName(level, f"{color_code}{level_name_str.ljust(8)}{NEON['RESET']}")
...
try: CONFIG = Config()
    ...
    elif 'termux-api' in os.getenv('PATH', ''): send_termux_notification("Pyrmethus Startup Failure", f"Config Error: {str(config_error)[:200]}") # Basic Termux check
    sys.exit(1)
...
last_key_level_update_day: Optional[int] = None # Tracks when key levels were last updated
contradiction_cooldown_active_until: float = 0.0 # Cooldown for trend contradiction
consecutive_loss_cooldown_active_until: float = 0.0 # Cooldown after max consecutive losses
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    def __init__(self, config: Config):
        super().__init__(config, df_columns=["st_long_flip", "st_short_flip", "confirm_trend", "momentum"]) # Example columns
    ...
    def generate_signals(self, df: pd.DataFrame, latest_close: Optional[Decimal] = None, latest_atr: Optional[Decimal] = None) -> Dict[str, Any]:
        ...
        confirm_is_up_current = last.get("confirm_trend", pd.NA) # True, False, or pd.NA
        # Momentum value
        ...
        if primary_long_flip and primary_short_flip:
            ...
            if confirm_is_up_current is True and (momentum_val is not pd.NA and momentum_val > 0):
                ...
                self.logger.info("Resolution: Prioritizing LONG flip due to confirm trend and momentum.")
                ...
                self.logger.info("Resolution: Prioritizing SHORT flip due to confirm trend and momentum.")
            else: # Ambiguous, ignore both flips for this candle
                primary_long_flip = False; primary_short_flip = False
                self.logger.warning("Resolution: Ambiguous. No primary signal generated from conflicting flips.")
        
        # Confirm ST Stability Check
        stable_confirm_trend = pd.NA
        if self.config.confirm_st_stability_lookback <= 1: # No stability check or 1-candle stability
            ...
            if confirm_is_up_current is True and all(trend is True for trend in recent_confirm_trends):
                stable_confirm_trend = True
            elif confirm_is_up_current is False and all(trend is False for trend in recent_confirm_trends):
        ...
        if self.config.st_max_entry_distance_atr_multiplier is not None and \
           latest_atr is not None and latest_atr > 0 and latest_close is not None:
            ...
            if primary_long_flip:
                st_line_val = safe_decimal_conversion(last.get(f"{st_p_base}l")) # SuperTrend Long line
                if st_line_val is not None and (latest_close - st_line_val) > max_allowed_distance:
                    price_proximity_ok = False
                    self.logger.debug(f"Long entry filtered: Price too far from ST line. Dist: {(latest_close - st_line_val):.2f}, MaxAllowed: {max_allowed_distance:.2f}")
            elif primary_short_flip:
                st_line_val = safe_decimal_conversion(last.get(f"{st_p_base}s")) # SuperTrend Short line
                if st_line_val is not None and (st_line_val - latest_close) > max_allowed_distance:
                    price_proximity_ok = False
                    self.logger.debug(f"Short entry filtered: Price too far from ST line. Dist: {(st_line_val - latest_close):.2f}, MaxAllowed: {max_allowed_distance:.2f}")

        # Entry Signals
        if primary_long_flip and stable_confirm_trend is True and \
           momentum_val > self.config.momentum_threshold and momentum_val > 0 and price_proximity_ok: # Ensure momentum is positive for long
            signals["enter_long"] = True
            self.logger.info(f"{NEON['SIDE_LONG']}DualST+Mom Signal: LONG Entry triggered.{NEON['RESET']}")
        elif primary_short_flip and stable_confirm_trend is False and \
             momentum_val < -self.config.momentum_threshold and momentum_val < 0 and price_proximity_ok: # Ensure momentum is negative for short
            signals["enter_short"] = True
            self.logger.info(f"{NEON['SIDE_SHORT']}DualST+Mom Signal: SHORT Entry triggered.{NEON['RESET']}")
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame, latest_close: Optional[Decimal] = None, latest_atr: Optional[Decimal] = None) -> Dict[str, Any]:
        signals = self._get_default_signals()
        min_rows_needed = self.config.ehlers_fisher_length + self.config.ehlers_fisher_signal_length + 5 # Buffer for calculation
        if not self._validate_df(df, min_rows=min_rows_needed) or len(df) < 2: # Need at least 2 rows for prev/current
            return signals
        ...
        if pd.isna(fisher_now) or pd.isna(signal_now) or pd.isna(fisher_prev) or pd.isna(signal_prev):
            self.logger.debug(f"Ehlers Fisher or Signal rune is NA. Fisher: {fisher_now}, Signal: {signal_now} (Prev Fisher: {fisher_prev}, Prev Signal: {signal_prev})")
            return signals
        ...
        if not is_fisher_extreme: # Only consider entries if NOT in an extreme zone
            if fisher_prev <= signal_prev and fisher_now > signal_now: # Fisher crosses above Signal
                signals["enter_long"] = True
                self.logger.info(f"{NEON['SIDE_LONG']}EhlersFisher Signal: LONG Entry (Fisher crossed UP).{NEON['RESET']}")
            elif fisher_prev >= signal_prev and fisher_now < signal_now: # Fisher crosses below Signal
                signals["enter_short"] = True
                self.logger.info(f"{NEON['SIDE_SHORT']}EhlersFisher Signal: SHORT Entry (Fisher crossed DOWN).{NEON['RESET']}")
        elif (fisher_prev <= signal_prev and fisher_now > signal_now) or \
             (fisher_prev >= signal_prev and fisher_now < signal_now): # Log if crossover happened in extreme zone
            self.logger.info(f"EhlersFisher: Crossover IGNORED due to Fisher in extreme zone (Value: {fisher_now:.2f}).")

        # Exit Signals (Crossover logic, can happen regardless of extreme zone)
        if fisher_prev >= signal_prev and fisher_now < signal_now: # Fisher crossed below Signal
            signals["exit_long"] = True
            signals["exit_reason"] = "Ehlers Fisher crossed BELOW Signal"
        elif fisher_prev <= signal_prev and fisher_now > signal_now: # Fisher crossed above Signal
            signals["exit_short"] = True
...
if StrategyClass:
    ...
    logger.success(f"{NEON['SUCCESS']}Strategy '{NEON['STRATEGY']}{CONFIG.strategy_name.value}{NEON['SUCCESS']}' invoked.{NEON['RESET']}") # type: ignore [attr-defined]
else:
...
class TradeMetrics:
    def __init__(self):
        ...
        self.last_daily_reset_day: Optional[int] = None # Tracks day of month for daily reset
        self.consecutive_losses: int = 0
        self.daily_trade_entry_count: int = 0 # Tracks number of new trade entries per day
        self.last_daily_trade_count_reset_day: Optional[int] = None
        self.daily_trades_rest_active_until: float = 0.0 # Timestamp until daily trade limit rest is active
    ...
    def set_initial_equity(self, equity: Decimal):
        if self.initial_equity is None: # Set overall session initial equity once
            ...
            self.logger.info(f"{NEON['INFO']}Initial Session Equity: {NEON['VALUE']}{equity:.2f} {CONFIG.usdt_symbol}{NEON['RESET']}")
        ...
        if self.last_daily_trade_count_reset_day != today:
            ...
            if self.daily_trades_rest_active_until > 0 and time.time() > self.daily_trades_rest_active_until :
                 self.daily_trades_rest_active_until = 0.0 # Reset rest period if it has passed
    ...
    def check_drawdown(self, current_equity: Decimal) -> Tuple[bool, str]:
        if not CONFIG.enable_max_drawdown_stop or self.daily_start_equity is None or self.daily_start_equity <= 0:
            return False, "" # Drawdown check disabled or daily start equity not set/invalid
    ...
    def log_trade(self, symbol: str, side: str, entry_price: Decimal, exit_price: Decimal, qty: Decimal,
                  entry_time_ms: int, exit_time_ms: int, reason: str, part_id: str, pnl_str: str, # pnl_str for consistency with state
                  scale_order_id: Optional[str] = None, mae: Optional[Decimal] = None, mfe: Optional[Decimal] = None, is_entry: bool = False): # is_entry currently unused here
        ...
        if not all([isinstance(entry_price, Decimal) and entry_price > 0, 
                    ...
                    isinstance(entry_time_ms, int) and entry_time_ms > 0, # Timestamps should be positive
                    isinstance(exit_time_ms, int) and exit_time_ms > 0]):
            self.logger.warning(f"{NEON['WARNING']}Trade log skipped due to flawed parameters for Part ID: {part_id}. Entry: {entry_price}, Exit: {exit_price}, Qty: {qty}{NEON['RESET']}")
            return

        profit = safe_decimal_conversion(pnl_str, Decimal(0)) # Convert pnl_str to Decimal for logic
        ...
        trade_type = "Scale-In" if scale_order_id else "Part" # Differentiate if it's a scale-in order part

        self.trades.append({
            "symbol": symbol, "side": side, 
            "entry_price_str": str(entry_price), "exit_price_str": str(exit_price), # Store as strings
            "qty_str": str(qty), "profit_str": str(profit), # Store PNL as string too
            "entry_time_iso": entry_dt_utc.isoformat(), 
            "exit_time_iso": exit_dt_utc.isoformat(), 
            "duration_seconds": duration_seconds, 
            "exit_reason": reason, 
            "type": trade_type, "part_id": part_id, 
            "scale_order_id": scale_order_id,
            "mae_str": str(mae) if mae is not None else None, # Max Adverse Excursion
            "mfe_str": str(mfe) if mfe is not None else None  # Max Favorable Excursion
        })
    ...
    def increment_daily_trade_entry_count(self):
        ...
        if self.last_daily_trade_count_reset_day != today:
            self.daily_trade_entry_count = 0
            self.last_daily_trade_count_reset_day = today
            self.logger.info(f"{NEON['INFO']}Daily trade entry count reset to 0 before incrementing.{NEON['RESET']}")
    ...
    def summary(self) -> str:
        ...
        profits = [safe_decimal_conversion(t["profit_str"], Decimal(0)) for t in self.trades] # Convert from string for calculations
    ...
    def load_trades_from_list(self, trades_list: List[Dict[str, Any]]):
        ...
        self.logger.info(f"{NEON['INFO']}TradeMetrics: Re-inked {len(self.trades)} sagas from persistent scroll.{NEON['RESET']}")
...
def save_persistent_state(force_heartbeat: bool = False) -> None:
    ...
    should_save = force_heartbeat or \
        ...
        (not _active_trade_parts and (now - _last_heartbeat_save_time > HEARTBEAT_INTERVAL_SECONDS * 5)) # Less frequent saves if idle
    ...
    try:
        ...
        state_data = {
            ...
            "trade_timestamps_for_whipsaw": list(trade_timestamps_for_whipsaw), # Convert deque
            "persistent_signal_counter": persistent_signal_counter,
            "last_signal_type": last_signal_type,
            "previous_day_high_str": str(previous_day_high) if previous_day_high is not None else None,
            "previous_day_low_str": str(previous_day_low) if previous_day_low is not None else None,
        ...
        temp_file_path = STATE_FILE_PATH + ".tmp_scroll" # Atomic write: write to temp then rename
        ...
        logger.error(f"{NEON['ERROR']}Phoenix Feather Error: Failed to scribe state: {e}{NEON['RESET']}")
...
def load_persistent_state() -> bool:
    ...
    if not os.path.exists(STATE_FILE_PATH):
        logger.info(f"{NEON['INFO']}Phoenix Feather: No previous scroll found. Starting with a clean slate.{NEON['RESET']}")
        return False
    ...
    try:
        ...
        if saved_version != PYRMETHUS_VERSION:
            logger.warning(f"{NEON['WARNING']}Phoenix Feather: Scroll version '{saved_version}' differs from current Pyrmethus version '{PYRMETHUS_VERSION}'. Caution during reanimation.{NEON['RESET']}")
        ...
        if state_data.get("config_symbol") != CONFIG.symbol or \
           state_data.get("config_strategy") != CONFIG.strategy_name.value:
            logger.warning(f"{NEON['WARNING']}Phoenix Scroll sigils (symbol/strategy) mismatch! "
        ...
        decimal_keys_in_part = [
            "entry_price", "qty", "sl_price", "tp_price", # Core pricing/qty
            "initial_usdt_value", "atr_at_entry", 
            "entry_fisher_value", "entry_signal_value", # Strategy-specific
        ...
        for part_data_str_values in loaded_parts_raw:
            ...
            for key, loaded_value in part_data_str_values.items():
                if key in decimal_keys_in_part:
                    ...
                    if loaded_value is None or (isinstance(loaded_value, str) and loaded_value.lower() == 'none'):
                        restored_part[key] = None
                    else:
                        restored_part[key] = safe_decimal_conversion(loaded_value, None) 
                        if restored_part[key] is None: # Conversion failed for a non-"None" value
                            logger.warning(f"Failed to restore Decimal for key '{key}' in part {part_data_str_values.get('part_id', 'N/A')}, value was '{loaded_value}'. Setting to None.")
                    ...
                    if isinstance(loaded_value, list):
                        ...
                        restored_part[key] = deque(maxlen=maxlen) # Initialize empty deque
                        logger.warning(f"Expected list for deque key '{key}' in part, got {type(loaded_value)}. Initialized empty deque.")
                elif key == "entry_time_ms": # Handle timestamp carefully
                    if isinstance(loaded_value, (int, float)):
                        restored_part[key] = int(loaded_value)
                    elif isinstance(loaded_value, str): # Could be ISO string or numeric string
                        try: # Attempt ISO format first (more robust for timezone-aware timestamps)
                            dt_obj = datetime.fromisoformat(loaded_value.replace("Z", "+00:00"))
                            restored_part[key] = int(dt_obj.timestamp() * 1000)
                        except ValueError: # Fallback to direct int conversion
                            try: restored_part[key] = int(loaded_value)
                            except ValueError:
                                logger.error(f"Malformed entry_time_ms '{loaded_value}' for part. Skipping part."); valid_part_data = False; break
                    else:
                        logger.error(f"Unexpected type for entry_time_ms '{type(loaded_value)}'. Skipping part."); valid_part_data = False; break
        ...
        if trade_metrics.last_daily_reset_day is not None: # Ensure it's an int
            try: trade_metrics.last_daily_reset_day = int(trade_metrics.last_daily_reset_day)
                ...
                logger.warning("Malformed last_daily_reset_day from state, set to None.")
        ...
        logger.success(f"{NEON['SUCCESS']}Phoenix Feather: Memories reawakened! Active Parts: {len(_active_trade_parts)}, Logged Trades: {len(trade_metrics.trades)}{NEON['RESET']}") # type: ignore [attr-defined]
        return True
    except json.JSONDecodeError as e:
        logger.error(f"{NEON['ERROR']}Phoenix Feather: Scroll '{STATE_FILE_PATH}' corrupted: {e}. Starting with a clean slate.{NEON['RESET']}")
        return False
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Phoenix Feather Error: Unexpected chaos during reawakening: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        return False
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    if df.empty:
        logger.warning(f"{NEON['WARNING']}Market data scroll empty. No alchemy possible.{NEON['RESET']}")
    ...
    for col in ['close', 'high', 'low', 'volume']:
        if col in df.columns:
            ...
            logger.error(f"{NEON['ERROR']}Essential market rune '{col}' missing from data. Indicator alchemy may fail.{NEON['RESET']}")
    ...
    if config.strategy_name == StrategyName.DUAL_SUPERTREND_MOMENTUM:
        ...
        df.ta.supertrend(length=config.st_atr_length, multiplier=float(config.st_multiplier), append=True, 
                         col_names=(st_p_base, f"{st_p_base}d", f"{st_p_base}l", f"{st_p_base}s"))
        df.ta.supertrend(length=config.confirm_st_atr_length, multiplier=float(config.confirm_st_multiplier), append=True, 
        ...
        if f"{st_p_base}d" in df.columns: # Direction column
            ...
            logger.warning(f"Primary Supertrend direction column '{st_p_base}d' not found post-calculation.")
        ...
        if f"{st_c_base}d" in df.columns:
            ...
            logger.warning(f"Confirmation Supertrend direction column '{st_c_base}d' not found post-calculation.")
            
        # Calculate Momentum
        if 'close' in df.columns and not df['close'].isnull().all():
            df.ta.mom(length=config.momentum_period, append=True, col_names=("momentum",))
        else:
            df["momentum"] = pd.NA
            logger.warning("Momentum calculation skipped due to missing or all-NaN 'close' column.")
        ...
        if 'high' in df.columns and 'low' in df.columns and \
           not df['high'].isnull().all() and not df['low'].isnull().all():
            ...
            logger.warning("Ehlers Fisher calculation skipped due to missing or all-NaN 'high'/'low' columns.")
    ...
    if 'high' in df.columns and 'low' in df.columns and 'close' in df.columns and \
       not df['high'].isnull().all() and not df['low'].isnull().all() and not df['close'].isnull().all():
        ...
        logger.warning(f"ATR ({atr_col_name}) calculation skipped due to missing or all-NaN 'high'/'low'/'close' columns.")
    ...
    if config.enable_trap_filter:
        ...
        if 'high' in df.columns and not df['high'].isnull().all():
             df[f'rolling_high_{lookback}'] = df['high'].rolling(window=lookback, min_periods=1).max()
        else: df[f'rolling_high_{lookback}'] = pd.NA
        if 'low' in df.columns and not df['low'].isnull().all():
            df[f'rolling_low_{lookback}'] = df['low'].rolling(window=lookback, min_periods=1).min()
        else: df[f'rolling_low_{lookback}'] = pd.NA
        
    return df

# --- Exchange Interaction Primitives ---
@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.ExchangeNotAvailable, ccxt.DDoSProtection), 
       tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2, logger=logger)
def get_market_data(exchange: ccxt.Exchange, symbol: str, interval: str, limit: int = OHLCV_LIMIT) -> Optional[pd.DataFrame]:
    ...
    try:
        params = {'category': 'linear'} if symbol.endswith(":USDT") else {} # Bybit specific for USDT perpetuals
        ohlcv = exchange.fetch_ohlcv(symbol, interval, limit=limit, params=params)
        if not ohlcv:
            logger.warning(f"{NEON['WARNING']}No OHLCV runes received for {symbol} ({interval}).{NEON['RESET']}")
            return None
        
        df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms', utc=True) # Ensure UTC for consistency
        for col in ['open', 'high', 'low', 'close', 'volume']: # Ensure numeric types
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        # Log first and last timestamp for verification
        if not df.empty:
            logger.debug(f"Summoned {len(df)} candles. First: {df['timestamp'].iloc[0]}, Last: {df['timestamp'].iloc[-1]}")
        return df
    except ccxt.BadSymbol as e_bs: # Symbol not recognized by exchange
        logger.critical(f"{NEON['CRITICAL']}Market realm rejects symbol '{symbol}': {e_bs}. Pyrmethus cannot proceed with this symbol.{NEON['RESET']}")
        ...
        logger.error(f"{NEON['ERROR']}Error summoning market whispers for {symbol}: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        return None
...
def fetch_account_balance(exchange: ccxt.Exchange, currency_code: str = CONFIG.usdt_symbol) -> Optional[Decimal]:
    ...
    try:
        ...
        if currency_code == "USDT": # Specify coin for Bybit unified to get specific coin balance
        ...
        if total_balance_val is None and exchange.id == 'bybit' and \
           'info' in balance_data and 'result' in balance_data['info'] and \
           'list' in balance_data['info']['result'] and balance_data['info']['result']['list']:
            # The 'list' usually contains one item for UNIFIED account type
            account_info = balance_data['info']['result']['list'][0]
            if 'totalEquity' in account_info and currency_code == CONFIG.usdt_symbol: # Assuming totalEquity is in USDT
                total_balance_val = account_info['totalEquity']
                logger.info(f"Using 'totalEquity' ({total_balance_val}) from Bybit Unified Account for {currency_code} balance.")
                 ...
                 for coin_balance_info in account_info['coin']:
                     if coin_balance_info.get('coin') == currency_code:
                         total_balance_val = coin_balance_info.get('walletBalance') # or equity, check Bybit docs
                         logger.info(f"Using '{coin_balance_info.get('walletBalance')}' from Bybit Unified Account coin list for {currency_code}.")
                         break
        
        total_balance = safe_decimal_conversion(total_balance_val)
        if total_balance is None:
            logger.warning(f"{NEON['WARNING']}{currency_code} balance rune unreadable or not found in response.{NEON['RESET']}")
        ...
        logger.error(f"{NEON['ERROR']}Error scrying treasury for {currency_code}: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        return None
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    if determined_side not in [CONFIG.pos_long, CONFIG.pos_short]:
        logger.error(f"{NEON['ERROR']}Incoherent side '{determined_side}' in first active part. State corrupted! Clearing parts.{NEON['RESET']}")
    ...
    for part in _active_trade_parts:
        ...
        if not isinstance(part_qty, Decimal) or part_qty < 0: # Qty should be positive Decimal
            logger.warning(f"Invalid qty '{part_qty}' for part {part.get('part_id')}. Ignoring this part for aggregation.")
            continue
        if part_side != determined_side:
            logger.error(f"{NEON['ERROR']}Position parts have conflicting sides ('{part_side}' vs '{determined_side}'). State corrupted! Clearing parts.{NEON['RESET']}")
            _active_trade_parts.clear()
            save_persistent_state(force_heartbeat=True)
            return CONFIG.pos_none, Decimal(0)
        total_qty += part_qty
    ...
    if not valid_parts_exist or total_qty <= CONFIG.position_qty_epsilon: # If all parts were invalid or total qty is negligible
        if total_qty < Decimal(0): # Should not happen if part_qty is validated as >= 0
             logger.error(f"{NEON['ERROR']}Negative total quantity {total_qty} after aggregation. State corrupted! Clearing parts.{NEON['RESET']}")
...
def calculate_position_size(balance: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, sl_price: Decimal, market_info: Dict) -> Optional[Decimal]:
    ...
    if not all([
        ...
        CONFIG.qty_step is not None and CONFIG.qty_step > 0 # Qty step must be valid
    ]):
        logger.warning(f"{NEON['WARNING']}Invalid runes for sizing. Bal: {balance}, Entry: {entry_price}, SL: {sl_price}, MarketInfo: {'Present' if market_info else 'Absent'}, QtyStep: {CONFIG.qty_step}{NEON['RESET']}")
        return None

    risk_per_unit = abs(entry_price - sl_price)
    if risk_per_unit <= Decimal(0): # Should be caught by sl_price != entry_price, but extra check
        logger.warning(f"{NEON['WARNING']}Risk per unit zero or negative (Entry: {entry_price}, SL: {sl_price}). Cannot divine size.{NEON['RESET']}")
        return None
    ...
    if position_usdt_value > CONFIG.max_order_usdt_amount:
        ...
        quantity_base = position_usdt_value / entry_price # Recalculate quantity based on capped USDT value
    ...
    if quantity_adjusted <= CONFIG.position_qty_epsilon: # If adjusted quantity is too small
        logger.warning(f"{NEON['WARNING']}Calculated qty {quantity_adjusted} is zero/negligible after step adjustment. Original base: {quantity_base}. No order.{NEON['RESET']}")
        return None
    ...
    if min_qty_limit is not None and quantity_adjusted < min_qty_limit:
        logger.warning(f"{NEON['WARNING']}Calculated qty {quantity_adjusted} is less than exchange minimum {min_qty_limit}. No order.{NEON['RESET']}")
        return None
    
    qty_display_precision = abs(CONFIG.qty_step.as_tuple().exponent) if CONFIG.qty_step else 8 # Default precision for display
    logger.info(f"Calculated position size: {NEON['QTY']}{quantity_adjusted:.{qty_display_precision}f}{NEON['RESET']} (USDT Value: ~{quantity_adjusted * entry_price:.2f})")
    return quantity_adjusted

@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.InsufficientFunds), tries=2, delay=3, logger=logger) # Retry on InsufficientFunds once might help with temp margin issues
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price_target: Decimal, 
                       sl_price: Decimal, tp_price: Optional[Decimal], atr_val: Optional[Decimal], 
                       df_for_entry_context: Optional[pd.DataFrame] = None) -> Optional[Dict]:
    ...
    logger.info(f"{NEON['ACTION']}Attempting {side.upper()} order for {config.symbol}... Target Entry: {NEON['PRICE']}{entry_price_target}{NEON['RESET']}, "
    ...
    if config.enable_anti_martingale_risk and trade_metrics.trades: # Check if there are past trades
        ...
        if last_trade_pnl_str is not None:
            ...
            if last_trade_pnl < 0: # Previous trade was a loss
                ...
                logger.info(f"{NEON['INFO']}Anti-Martingale: Risk reduced to {effective_risk_pct_for_trade*100:.3f}% (due to previous loss).{NEON['RESET']}")
                ...
                logger.info(f"{NEON['INFO']}Anti-Martingale: Risk adjusted to {effective_risk_pct_for_trade*100:.3f}% (due to previous win).{NEON['RESET']}")
    ...
    if balance is None or balance <= Decimal(10): # Arbitrary small threshold
        logger.error(f"{NEON['ERROR']}Insufficient treasury ({balance if balance is not None else 'Error fetching'}). Cannot cast order spell.{NEON['RESET']}")
        return None
    
    if config.MARKET_INFO is None or config.tick_size is None or config.qty_step is None: 
        logger.error(f"{NEON['ERROR']}Market runes (info, tick_size, qty_step) not divined. Cannot cast order spell.{NEON['RESET']}")
        return None
    ...
    if quantity is None or quantity <= CONFIG.position_qty_epsilon:
        logger.warning(f"{NEON['WARNING']}Position sizing yielded no valid quantity. Order spell aborted.{NEON['RESET']}")
        return None
        ...
        if config.enable_limit_order_price_improvement_check:
            try:
                ...
                if side == config.pos_long and current_best_ask is not None and current_best_ask <= calculated_limit_price:
                    logger.info(f"{NEON['INFO']}Price Improve (Long): Ask ({current_best_ask}) is better than/at calculated limit ({calculated_limit_price}). Adjusting limit price slightly above ask.{NEON['RESET']}")
                    calculated_limit_price = current_best_ask + config.tick_size # Adjust to be slightly less aggressive or match ask
                elif side == config.pos_short and current_best_bid is not None and current_best_bid >= calculated_limit_price:
                    logger.info(f"{NEON['INFO']}Price Improve (Short): Bid ({current_best_bid}) is better than/at calculated limit ({calculated_limit_price}). Adjusting limit price slightly below bid.{NEON['RESET']}")
                    calculated_limit_price = current_best_bid - config.tick_size
            except Exception as e_ticker:
                logger.warning(f"{NEON['WARNING']}Ticker fetch failed for limit order price improvement: {e_ticker}{NEON['RESET']}")
        
        # Ensure limit price conforms to tick size
        order_price_param_for_call = float(config.tick_size * (calculated_limit_price // config.tick_size))
        logger.info(f"Calculated Limit Price for {side} order: {order_price_param_for_call}")

    # Prepare order parameters for Bybit
    # positionIdx=0 for one-way mode. SL/TP set directly on order.
    params: Dict[str, Any] = {
        'timeInForce': 'GTC' if order_type_str == 'limit' else 'IOC', # GoodTillCancel for Limit, ImmediateOrCancel for Market
        'positionIdx': 0, 
        'stopLoss': str(sl_price), # Bybit API often prefers string representation for precision
    ...
    try:
        ...
        logger.success(f"{NEON['SUCCESS']}Entry Order {order.get('id', 'N/A')} ({order.get('status', 'N/A')}) cast.{NEON['RESET']}") # type: ignore [attr-defined]
        config.send_notification_method(f"Pyrmethus Order: {side.upper()}", 
        ...
        is_filled_market = (order_type_str == 'market' and filled_order.get('filled', 0) > 0 and filled_order.get('status') != 'canceled')
        is_filled_limit = (order_type_str == 'limit' and filled_order.get('status') == 'closed') # 'closed' implies fully filled for limit

        if is_filled_market or is_filled_limit:
            actual_entry_price = safe_decimal_conversion(filled_order.get('average', filled_order.get('price', entry_price_target)), entry_price_target)
            actual_qty = safe_decimal_conversion(filled_order.get('filled', quantity), quantity)
            entry_timestamp_ms = int(filled_order.get('timestamp', time.time() * 1000)) # Use order timestamp, fallback to now
            ...
            new_part = {
                ...
                "tp_price": tp_price if config.enable_take_profit else None, # TP set on exchange or None
                "atr_at_entry": atr_val, 
                "initial_usdt_value": actual_qty * actual_entry_price,
                "breakeven_set": False, # Flag for breakeven SL adjustment
                "recent_pnls": deque(maxlen=config.profit_momentum_window), # For profit momentum SL
                "last_known_pnl": Decimal(0), # For stateful PNL checks
                "adverse_candle_closes": deque(maxlen=config.last_chance_consecutive_adverse_candles), # For last chance exit
            ...
            if config.enable_whipsaw_cooldown:
                ...
                if len(trade_timestamps_for_whipsaw) == config.whipsaw_max_trades_in_period and \
                   (now_ts - trade_timestamps_for_whipsaw[0]) <= config.whipsaw_period_seconds:
                    logger.critical(f"{NEON['CRITICAL']}Whipsaw pattern detected! Pausing trading for {config.whipsaw_cooldown_seconds}s.{NEON['RESET']}")
                    config.send_notification_method("Pyrmethus Whipsaw Alert", f"Whipsaw detected. Pausing for {config.whipsaw_cooldown_seconds}s.")
            ...
            save_persistent_state(force_heartbeat=True) # Save state immediately after opening a trade
            logger.success(f"{NEON['SUCCESS']}Order {order['id']} filled! Part {part_id} created. Entry: {NEON['PRICE']}{actual_entry_price}{NEON['RESET']}, Qty: {NEON['QTY']}{actual_qty}{NEON['RESET']}") # type: ignore [attr-defined]
            config.send_notification_method(f"Pyrmethus Order Filled: {side.upper()}", 
            ...
            logger.warning(f"{NEON['WARNING']}Order {order['id']} not fully 'closed' or filled as expected. Status: {filled_order.get('status', 'N/A')}, Filled Qty: {filled_order.get('filled',0)}. Attempting to cancel if still open.{NEON['RESET']}")
            if filled_order.get('status') == 'open' or filled_order.get('status') == 'new': # If order is still open
                try:
                    exchange.cancel_order(order['id'], config.symbol, params={'category': 'linear'} if config.symbol.endswith(":USDT") else {})
                    logger.info(f"Successfully cancelled unfilled/partially-filled order {order['id']}.")
                except Exception as e_cancel:
                    logger.error(f"Error cancelling order {order['id']}: {e_cancel}")
            return None
            
    except ccxt.InsufficientFunds as e:
        logger.error(f"{NEON['ERROR']}Insufficient funds to cast order spell: {e}{NEON['RESET']}")
        return None
    except ccxt.ExchangeError as e: # Catch specific exchange errors (e.g., rate limits, invalid params)
        logger.error(f"{NEON['ERROR']}Exchange rejected order spell: {e}{NEON['RESET']}")
        # Potentially parse e.message for specific Bybit error codes if needed
        return None
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Unexpected chaos while weaving order spell: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        return None
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part_to_close: Dict, reason: str, close_price_target: Optional[Decimal] = None) -> bool:
    ...
    if not isinstance(part_qty, Decimal) or part_qty <= CONFIG.position_qty_epsilon or \
       part_side not in [config.pos_long, config.pos_short]:
        logger.warning(f"{NEON['WARNING']}Cannot unravel part {part_id}: invalid qty '{part_qty}' or side '{part_side}'. Removing from active parts if it exists.{NEON['RESET']}")
        _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_id] # Clean up bad part
        save_persistent_state(force_heartbeat=True)
        return False # Indicate failure to close as intended

    logger.info(f"{NEON['ACTION']}Unraveling part {part_id} (Side: {part_side}, Qty: {NEON['QTY']}{part_qty}{NEON['RESET']}) due to: {reason}{NEON['RESET']}")

    close_side_verb = config.side_sell if part_side == config.pos_long else config.side_buy
    params: Dict[str, Any] = {'reduceOnly': True} # Ensure this order only reduces position
    ...
    try:
        ...
        logger.success(f"{NEON['SUCCESS']}Unraveling Order {close_order.get('id', 'N/A')} ({close_order.get('status', 'N/A')}) cast.{NEON['RESET']}") # type: ignore [attr-defined]
        ...
        if filled_close_order.get('status') == 'closed' and filled_close_order.get('filled', 0) > 0:
            actual_exit_price = safe_decimal_conversion(filled_close_order.get('average', filled_close_order.get('price')), 
                                                        close_price_target or part_to_close.get('entry_price')) # Fallback for price
            ...
            if not isinstance(entry_price, Decimal): # If loaded from old state or error
                entry_price = safe_decimal_conversion(entry_price, actual_exit_price) # Fallback to avoid error
            ...
            trade_metrics.log_trade(symbol=config.symbol, side=part_side, entry_price=entry_price, 
                                    exit_price=actual_exit_price, qty=part_qty, 
                                    entry_time_ms=part_to_close.get('entry_time_ms', 0), # Ensure entry_time_ms exists
                                    exit_time_ms=exit_timestamp_ms, reason=reason, 
                                    part_id=part_id, pnl_str=str(pnl)) # Log PNL as string
            
            _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_id] # Remove closed part
            save_persistent_state(force_heartbeat=True)
            
            pnl_color_key = 'PNL_POS' if pnl > 0 else ('PNL_NEG' if pnl < 0 else 'PNL_ZERO')
            logger.success(f"{NEON['SUCCESS']}Part {part_id} successfully unraveled. Exit Price: {NEON['PRICE']}{actual_exit_price}{NEON['RESET']}, PNL: {NEON[pnl_color_key]}{pnl:.2f} {config.usdt_symbol}{NEON['RESET']}") # type: ignore [attr-defined]
            config.send_notification_method(f"Pyrmethus Position Closed: {part_side}", 
                                            f"{config.symbol} Part {part_id}. PNL: {pnl:.2f}. Reason: {reason[:50]}")
            return True
        else:
            logger.warning(f"{NEON['WARNING']}Unraveling order {close_order.get('id', 'N/A')} for part {part_id} not 'closed' or not filled. "
        ...
        logger.error(f"{NEON['ERROR']}Error during unraveling spell for part {part_id}: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        config.send_notification_method("Pyrmethus Close Error", f"Error closing {config.symbol} part {part_id}: {str(e)[:80]}")
        return False
...
def cancel_all_symbol_orders(exchange: ccxt.Exchange, symbol: str):
    ...
    logger.info(f"{NEON['ACTION']}Dispelling all non-positional open orders for {symbol}...{NEON['RESET']}")
    try:
        ...
        for order in open_orders:
            ...
            if order_type == 'limit' and (order_status == 'open' or order_status == 'new'):
                try:
                    exchange.cancel_order(order_id, symbol, params=params)
                    logger.info(f"Dispelled limit order {order_id} ({order_type}, Status: {order_status}).")
                    cancelled_count += 1
                except Exception as e_cancel:
                    logger.error(f"Failed to dispel order {order_id}: {e_cancel}")
            else:
                logger.debug(f"Skipping order {order_id} (Type: {order_type}, Status: {order_status}) from dispel action.")
        
        if cancelled_count > 0:
            logger.success(f"{NEON['SUCCESS']}{cancelled_count} open order(s) for {symbol} dispelled.{NEON['RESET']}") # type: ignore [attr-defined]
        else:
            logger.info(f"No suitable open orders (e.g., pending limit entries) found to dispel for {symbol}.")

    except ccxt.FeatureNotSupported:
        logger.warning(f"{NEON['WARNING']}Exchange {exchange.id} may not fully support `fetch_open_orders` or `cancel_order` as expected.{NEON['RESET']}")
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Error dispelling open order enchantments: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    global _active_trade_parts
    logger.warning(f"{NEON['WARNING']}Initiating forceful unraveling of ALL positions for {config.symbol} due to: {reason}{NEON['RESET']}")
    ...
    if parts_to_close_copy:
        ...
        for part in parts_to_close_copy:
            if part.get('symbol') == config.symbol: # Ensure it's for the current symbol
                close_position_part(exchange, config, part, reason + " (Global Unraveling)")
    else:
        logger.info("No internally tracked trade parts to unravel via individual market orders.")
    ...
    logger.info(f"Performing safeguard scrying on exchange for any residual {config.symbol} positions...")
    try:
        ...
        all_positions = exchange.fetch_positions(None, params=params_fetch_pos) # Fetch all, then filter
        symbol_positions = [p for p in all_positions if p.get('symbol') == config.symbol]
        
        residual_closed_count = 0
        for pos_data in symbol_positions:
            pos_qty_str = pos_data.get('contracts', pos_data.get('size', '0')) # 'contracts' for derivatives
            pos_qty = safe_decimal_conversion(pos_qty_str, Decimal(0))
            pos_side_ccxt = pos_data.get('side') # 'long' or 'short' (lowercase from ccxt)
            
            if pos_qty is not None and pos_qty > CONFIG.position_qty_epsilon: # If a position exists
                side_to_close_market = config.side_sell if pos_side_ccxt == 'long' else config.side_buy
                logger.warning(f"Found residual exchange position: {pos_side_ccxt.upper()} {pos_qty} {config.symbol}. Force market unraveling.")
                ...
                time.sleep(config.post_close_delay_seconds) # Allow time for order processing
        
        if residual_closed_count > 0:
            logger.info(f"Safeguard unraveling completed for {residual_closed_count} residual position(s).")
        else:
            logger.info("No residual positions found on exchange for safeguard unraveling.")
    except Exception as e_fetch_close: 
        logger.error(f"{NEON['ERROR']}Error during final scrying/unraveling of exchange positions: {e_fetch_close}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
    ...
    try:
        logger.info(f"Attempting to clear any remaining SL/TP settings for position {config.symbol} on exchange.")
        ...
        logger.info(f"SL/TP clearing command sent for {config.symbol}. Position should be fully flat with no pending SL/TP.")
    except Exception as e_clear_sl_tp:
        logger.warning(f"{NEON['WARNING']}Could not explicitly clear SL/TP on exchange for {config.symbol}: {e_clear_sl_tp}. "
    ...
    logger.info(f"All positions/orders for {config.symbol} should now be unraveled/dispelled. Internal state cleared.")

def modify_position_sl_tp(exchange: ccxt.Exchange, config: Config, part: Dict, 
                          new_sl: Optional[Decimal] = None, new_tp: Optional[Decimal] = None) -> bool:
    # Pyrmethus: Adjusting the protective wards (SL/TP) on the exchange.
    if not config.MARKET_INFO or not config.tick_size:
        logger.error(f"{NEON['ERROR']}Market info or tick_size not divined. Cannot modify SL/TP wards.{NEON['RESET']}")
        return False

    part_id = part.get('part_id', 'UnknownPart')
    current_sl_from_part = part.get('sl_price') # This is what the bot thinks is the current SL
    current_tp_from_part = part.get('tp_price') # This is what the bot thinks is the current TP
    ...
    if sl_to_set_on_exchange is not None:
        sl_to_set_on_exchange = config.tick_size * (sl_to_set_on_exchange // config.tick_size)
        params_sl_tp['stopLoss'] = str(sl_to_set_on_exchange)
        params_sl_tp['slTriggerBy'] = 'MarkPrice' # Or LastPrice, IndexPrice as per config/strategy
    ...
    if config.enable_take_profit:
        if tp_to_set_on_exchange is not None:
            tp_to_set_on_exchange = config.tick_size * (tp_to_set_on_exchange // config.tick_size)
    ...
    if params_sl_tp.get('stopLoss') == '0' and params_sl_tp.get('takeProfit') == '0' and \
       (current_sl_from_part is None or current_sl_from_part == 0) and \
       (current_tp_from_part is None or current_tp_from_part == 0 or not config.enable_take_profit) :
        logger.debug(f"No change in SL/TP needed for part {part_id}. Both target '0' and current state is effectively no SL/TP.")
        return True # No change needed, considered successful

    logger.info(f"{NEON['ACTION']}Modifying SL/TP for part {part_id}: Target Exchange SL {params_sl_tp.get('stopLoss')}, Target Exchange TP {params_sl_tp.get('takeProfit')}{NEON['RESET']}")
    try:
        ...
        logger.success(f"{NEON['SUCCESS']}SL/TP modification successful for part {part_id}. " 
                       f"Exchange SL set to: {params_sl_tp.get('stopLoss')}, TP set to: {params_sl_tp.get('takeProfit')}{NEON['RESET']}") # type: ignore [attr-defined]
        
        part['sl_price'] = sl_to_set_on_exchange # Update part with the (tick-adjusted) price or None
        part['tp_price'] = tp_to_set_on_exchange # Update part with the (tick-adjusted) price or None (if TP disabled)
        ...
        if "not valid" in str(e).lower() or "too close" in str(e).lower(): # Common phrases for invalid SL/TP
             logger.warning(f"SL/TP for part {part_id} rejected by exchange (likely too close to market or invalid value). Original SL/TP may remain active.")
...
def update_daily_key_levels(exchange: ccxt.Exchange, config: Config):
    ...
    logger.info(f"{NEON['INFO']}Attempting to update daily key levels (Previous Day High/Low)...{NEON['RESET']}")
    try:
        ...
        three_days_ago_utc_start = now_utc.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=3)
        since_timestamp_ms = int(three_days_ago_utc_start.timestamp() * 1000)
        
        params = {'category': 'linear'} if config.symbol.endswith(":USDT") else {}
        daily_candles = exchange.fetch_ohlcv(config.symbol, '1d', since=since_timestamp_ms, limit=3, params=params) # Fetch last 3 daily candles
        ...
        for candle_data in reversed(daily_candles): # Iterate from most recent
            ...
            candle_dt_utc = datetime.fromtimestamp(candle_timestamp_ms / 1000, tz=pytz.utc)
            ...
            if candle_dt_utc.date() == yesterday_target_date:
                ...
                logger.info(f"{NEON['INFO']}Updated key levels for {yesterday_target_date}: Prev Day High {NEON['PRICE']}{previous_day_high}{NEON['RESET']}, Prev Day Low {NEON['PRICE']}{previous_day_low}{NEON['RESET']}")
                break
        
        if not found_yesterday_candle:
             logger.warning(f"{NEON['WARNING']}Could not definitively find YESTERDAY's ({yesterday_target_date}) candle data among fetched daily candles to update key levels.{NEON['RESET']}")
             logger.debug(f"Fetched daily candles timestamps: {[datetime.fromtimestamp(c[0]/1000, tz=pytz.utc).date() for c in daily_candles]}")
...
def main_loop(exchange: ccxt.Exchange, config: Config) -> None:
    ...
    logger.info(f"{NEON['HEADING']}=== Pyrmethus Spell v{PYRMETHUS_VERSION} Awakening on {exchange.id} for {config.symbol} ==={NEON['RESET']}")
    
    if load_persistent_state():
        logger.success(f"{NEON['SUCCESS']}Reawakened from Phoenix scroll. Active parts: {len(_active_trade_parts)}.{NEON['RESET']}") # type: ignore [attr-defined]
    else:
        logger.info(f"{NEON['INFO']}No prior state found or state was incompatible. Starting fresh spellcraft.{NEON['RESET']}")
    ...
    if current_balance_init is not None:
        trade_metrics.set_initial_equity(current_balance_init) # Sets session and daily start equity
    else:
        logger.warning(f"{NEON['WARNING']}Could not set initial treasury at startup. Drawdown checks might be affected until next successful balance fetch.{NEON['RESET']}")
    ...
    try: # Outer try for the main while loop, associated with the finally block for cleanup
        while True:
            ...
            try: # Inner try for a single iteration of the trading-decision loop
                logger.debug(f"{NEON['COMMENT']}# New cycle of observation ({loop_counter}). Current time: {datetime.now(pytz.utc).isoformat()}{NEON['RESET']}")

                # --- Cooldowns & System Checks ---
                now = time.time() # Consistent timestamp for checks in this iteration
                if config.enable_whipsaw_cooldown and now < whipsaw_cooldown_active_until:
                    remaining_cooldown = whipsaw_cooldown_active_until - now
                    logger.warning(f"{NEON['WARNING']}In whipsaw cooldown. Ends in {remaining_cooldown:.0f}s.{NEON['RESET']}")
                    time.sleep(min(remaining_cooldown, config.sleep_seconds)); continue
                
                if config.enable_trend_contradiction_cooldown and now < contradiction_cooldown_active_until:
                    remaining_cooldown = contradiction_cooldown_active_until - now
                    logger.warning(f"{NEON['WARNING']}In Trend Contradiction cooldown. Ends in {remaining_cooldown:.0f}s.{NEON['RESET']}")
                    time.sleep(min(remaining_cooldown, config.sleep_seconds)); continue

                if config.enable_daily_max_trades_rest and now < trade_metrics.daily_trades_rest_active_until:
                    remaining_rest = (trade_metrics.daily_trades_rest_active_until - now)
                    logger.warning(f"{NEON['WARNING']}Resting (daily max trades limit reached). Resumes in {remaining_rest/3600:.1f} hrs ({remaining_rest:.0f}s).{NEON['RESET']}")
                    time.sleep(min(remaining_rest, config.sleep_seconds * 10)); continue # Longer sleep during rest

                if config.enable_consecutive_loss_limiter and now < consecutive_loss_cooldown_active_until:
                    remaining_cooldown = consecutive_loss_cooldown_active_until - now
                    logger.warning(f"{NEON['WARNING']}In consecutive loss cooldown. Ends in {remaining_cooldown/60:.1f} mins ({remaining_cooldown:.0f}s).{NEON['RESET']}")
                    time.sleep(min(remaining_cooldown, config.sleep_seconds)); continue
                ...
                if current_balance_iter is not None:
                    trade_metrics.set_initial_equity(current_balance_iter) # Refreshes daily equity if new day
                    drawdown_hit, dd_reason = trade_metrics.check_drawdown(current_balance_iter)
                    if drawdown_hit:
                        logger.critical(f"{NEON['CRITICAL']}Max daily drawdown! {dd_reason}. Pyrmethus must rest to preserve essence.{NEON['RESET']}")
                        close_all_symbol_positions(exchange, config, "Max Daily Drawdown Reached"); break # Exit main loop
                else:
                    logger.warning(f"{NEON['WARNING']}Failed treasury scry this cycle. Drawdown check might be on stale data.{NEON['RESET']}")
                ...
                if config.enable_session_pnl_limits and trade_metrics.initial_equity is not None and current_balance_iter is not None:
                    ...
                    if config.session_profit_target_usdt is not None and current_session_pnl >= config.session_profit_target_usdt:
                        logger.critical(f"{NEON['SUCCESS']}SESSION PROFIT TARGET ({config.session_profit_target_usdt} {config.usdt_symbol}) REACHED! Current Session PNL: {current_session_pnl:.2f}. Pyrmethus concludes this session.{NEON['RESET']}")
                        config.send_notification_method("Pyrmethus Session Goal!", f"Profit target {config.session_profit_target_usdt} hit. PNL: {current_session_pnl:.2f}")
                        close_all_symbol_positions(exchange, config, "Session Profit Target Reached"); break
                    if config.session_max_loss_usdt is not None and current_session_pnl <= -abs(config.session_max_loss_usdt): # Max loss is positive in config
                        logger.critical(f"{NEON['CRITICAL']}SESSION MAX LOSS ({config.session_max_loss_usdt} {config.usdt_symbol}) BREACHED! Current Session PNL: {current_session_pnl:.2f}. Pyrmethus concludes this session.{NEON['RESET']}")
                        config.send_notification_method("Pyrmethus Session Loss!", f"Max session loss {config.session_max_loss_usdt} hit. PNL: {current_session_pnl:.2f}")
                ...
                if config.enable_daily_max_trades_rest and \
                   ...
                   now >= trade_metrics.daily_trades_rest_active_until: # Ensure not already resting
                    logger.critical(f"{NEON['CRITICAL']}Daily max trades ({config.daily_max_trades_limit}) reached. Initiating rest for {config.daily_max_trades_rest_hours} hrs.{NEON['RESET']}")
                    config.send_notification_method("Pyrmethus Daily Trades Rest", f"Max {config.daily_max_trades_limit} trades. Resting for {config.daily_max_trades_rest_hours}h.")
                    trade_metrics.daily_trades_rest_active_until = now + config.daily_max_trades_rest_hours * 3600
                    close_all_symbol_positions(exchange, config, "Daily Max Trades Limit Reached - Initiating Rest"); continue # Skip to next iteration for rest sleep

                # Consecutive Loss Limiter Check (activate if limit hit)
                if config.enable_consecutive_loss_limiter and \
                   trade_metrics.consecutive_losses >= config.max_consecutive_losses and \
                   now >= consecutive_loss_cooldown_active_until: # Ensure not already in cooldown
                    logger.critical(f"{NEON['CRITICAL']}{config.max_consecutive_losses} consecutive losses recorded! Initiating cooldown for {config.consecutive_loss_cooldown_minutes} mins.{NEON['RESET']}")
                    config.send_notification_method("Pyrmethus Consecutive Loss Cooldown", f"{config.max_consecutive_losses} losses. Pausing for {config.consecutive_loss_cooldown_minutes}m.")
                    consecutive_loss_cooldown_active_until = now + config.consecutive_loss_cooldown_minutes * 60
                    trade_metrics.consecutive_losses = 0 # Reset counter after activating cooldown
                    close_all_symbol_positions(exchange, config, "Consecutive Loss Limit Reached - Initiating Cooldown"); continue

                # --- Market Analysis ---
                ohlcv_df = get_market_data(exchange, config.symbol, config.interval, limit=OHLCV_LIMIT + config.api_fetch_limit_buffer)
                if ohlcv_df is None or ohlcv_df.empty:
                    logger.warning(f"{NEON['WARNING']}Market whispers faint (no OHLCV data). Retrying after pause.{NEON['RESET']}")
                    time.sleep(config.sleep_seconds); continue
                
                df_with_indicators = calculate_all_indicators(ohlcv_df.copy(), config) # Use a copy
                if df_with_indicators.empty or df_with_indicators.iloc[-1].isnull().all(): # Check if last row is all NaNs
                    logger.warning(f"{NEON['WARNING']}Indicator alchemy faint (empty or all-NaN last row). Pausing.{NEON['RESET']}")
                    time.sleep(config.sleep_seconds); continue
                ...
                if pd.isna(latest_close) or pd.isna(latest_atr) or latest_atr <= Decimal(0):
                    logger.warning(f"{NEON['WARNING']}Essential market readings unclear: Close ({_format_for_log(latest_close)}) "
                ...
                if config.signal_persistence_candles <= 1: # No persistence or 1-candle persistence
                    ...
                    if current_long_sig: # Current signal is long
                        if last_signal_type == "long" or last_signal_type is None: # Previous was long or no signal
                        ...
                        if last_signal_type == "short" or last_signal_type is None: # Previous was short or no signal
                    ...
                    logger.debug(f"Signal persistence: Longs seen: {persistent_signal_counter['long']}, Shorts seen: {persistent_signal_counter['short']}. "
                ...
                if current_pos_side == config.pos_none and len(_active_trade_parts) < config.max_active_trade_parts:
                    if confirmed_enter_long or confirmed_enter_short:
                        ...
                        if config.enable_no_trade_zones and latest_close is not None:
                            ...
                            for level in key_levels_to_check:
                                ...
                                if (level - zone_half_width) <= latest_close <= (level + zone_half_width):
                                    ...
                                    logger.info(f"{NEON['INFO']}Entry suppressed: Price {latest_close} is within no-trade zone around key level {level} "
                        ...
                        if config.enable_trap_filter and latest_atr > 0 and len(df_with_indicators) > config.trap_filter_lookback_period:
                            ...
                            prev_candle_idx = -2 
                            if len(df_with_indicators) > abs(prev_candle_idx): # Ensure -2 index is valid
                                ...
                                if confirmed_enter_long and recent_high_val and prev_candle_high and latest_close:
                                    ...
                                    if abs(prev_candle_high - recent_high_val) < wick_prox_val and \
                                       (recent_high_val - latest_close) >= rejection_needed:
                                        trap_detected = True
                                        logger.info(f"{NEON['INFO']}Bull Trap Filter: Long entry suppressed. Prev candle high {prev_candle_high} near recent high {recent_high_val}, current close {latest_close} shows rejection.{NEON['RESET']}")
                                elif confirmed_enter_short and recent_low_val and prev_candle_low and latest_close:
                                    ...
                                    if abs(prev_candle_low - recent_low_val) < wick_prox_val and \
                                       (latest_close - recent_low_val) >= rejection_needed: 
                                        trap_detected = True
                                        logger.info(f"{NEON['INFO']}Bear Trap Filter: Short entry suppressed. Prev candle low {prev_candle_low} near recent low {recent_low_val}, current close {latest_close} shows rejection.{NEON['RESET']}")
                        ...
                        if trade_allowed_by_zone and not trap_detected:
                            ...
                            if entry_side != config.pos_none:
                                ...
                                sl_price = (latest_close - sl_dist) if entry_side == config.pos_long \
                                      else (latest_close + sl_dist)
                                
                                tp_price = None
                                if config.enable_take_profit:
                                    tp_dist = latest_atr * config.atr_take_profit_multiplier
                                    tp_price = (latest_close + tp_dist) if entry_side == config.pos_long \
                                          else (latest_close - tp_dist)
                                
                                # Validate SL/TP relative to entry price
                                valid_sl_tp = True
                                if entry_side == config.pos_long:
                                    if sl_price >= latest_close: valid_sl_tp = False; logger.warning(f"Invalid SL for LONG: SL {sl_price} >= Entry {latest_close}")
                                    if tp_price and tp_price <= latest_close: valid_sl_tp = False; logger.warning(f"Invalid TP for LONG: TP {tp_price} <= Entry {latest_close}")
                                elif entry_side == config.pos_short:
                                    if sl_price <= latest_close: valid_sl_tp = False; logger.warning(f"Invalid SL for SHORT: SL {sl_price} <= Entry {latest_close}")
                                    if tp_price and tp_price >= latest_close: valid_sl_tp = False; logger.warning(f"Invalid TP for SHORT: TP {tp_price} >= Entry {latest_close}")
                                
                                if valid_sl_tp:
                                    new_part_created = place_risked_order(exchange, config, entry_side, latest_close, sl_price, tp_price, latest_atr, df_with_indicators)
                                    if new_part_created: 
                                        # Reset signal persistence counters after successful entry
                                        persistent_signal_counter = {"long": 0, "short": 0}
                                        last_signal_type = None
                                else:
                                    logger.warning(f"Entry for {entry_side} skipped due to invalid SL/TP calculation relative to entry price {latest_close}.")
                elif current_pos_side != config.pos_none : # Position exists
                     logger.debug(f"{NEON['INFO']}Position {current_pos_side} exists (Qty: {current_pos_qty}). No new entries.{NEON['RESET']}")
                elif len(_active_trade_parts) >= config.max_active_trade_parts: # Max parts reached
                     logger.debug(f"{NEON['INFO']}Max active trade parts ({config.max_active_trade_parts}) reached. No new entries.{NEON['RESET']}")
                ...
                for part in active_parts_copy:
                    if part.get('symbol') != config.symbol: continue # Should not happen if bot manages one symbol

                    part_id = part.get('part_id', 'N/A')
                    part_side = part.get('side')
                    part_entry_price = part.get('entry_price')
                    part_qty = part.get('qty')
                    part_sl_price = part.get('sl_price') # SL price as known by bot (should match exchange)
                    part_tp_price = part.get('tp_price') # TP price as known by bot

                    if not all([part_id, part_side, isinstance(part_entry_price, Decimal), isinstance(part_qty, Decimal), isinstance(part_sl_price, Decimal)]):
                        logger.warning(f"Skipping management for part {part_id}: missing critical data."); continue
                    if part_qty <= CONFIG.position_qty_epsilon: # Part is effectively closed
                        logger.info(f"Part {part_id} has zero/negligible quantity. Removing from active list."); 
                    ...
                    if config.enable_trend_contradiction_cooldown and part.get('side') and \
                       not part.get('contradiction_checked', False): # Check only once per part
                        
                        time_since_entry_ms = (now * 1000) - part.get('entry_time_ms', 0)
                        ...
                        if 0 < time_since_entry_ms < check_window_ms : 
                            ...
                            if part_side == config.pos_long and confirm_trend_now is False: # Was long, trend flipped short
                                contradiction_found = True
                            elif part_side == config.pos_short and confirm_trend_now is True: # Was short, trend flipped long
                                contradiction_found = True
                            
                            if contradiction_found:
                                logger.warning(f"{NEON['WARNING']}Trend contradiction for part {part_id} shortly after entry! "
                                               f"Side: {part_side}, Confirm Trend Now: {confirm_trend_now}. Activating cooldown.{NEON['RESET']}")
                                contradiction_cooldown_active_until = now + config.trend_contradiction_cooldown_seconds
                                # Optionally, could also close the position here if desired.
                        part['contradiction_checked'] = True # Mark as checked to avoid re-triggering cooldown

                    # Calculate current unrealized PNL for this part
                    current_unrealized_pnl = Decimal(0)
                    if part_side == config.pos_long:
                        current_unrealized_pnl = (latest_close - part_entry_price) * part_qty
                    elif part_side == config.pos_short:
                        current_unrealized_pnl = (part_entry_price - latest_close) * part_qty
                    
                    part.get('recent_pnls', deque()).append(current_unrealized_pnl) # Update PNL history for momentum
                    part['last_known_pnl'] = current_unrealized_pnl # Store for other checks

                    # Profit Momentum SL Tightening (modifies SL on exchange)
                    if config.enable_profit_momentum_sl_tighten and \
                       len(part.get('recent_pnls',[])) == config.profit_momentum_window and latest_atr > 0:
                        ...
                        if is_currently_profitable and is_profit_increasing:
                            original_atr_at_entry = part.get('atr_at_entry', latest_atr) # Use ATR at entry for consistent SL distance
                            if original_atr_at_entry <= 0: original_atr_at_entry = latest_atr # Fallback if invalid

                            tightened_sl_dist = (original_atr_at_entry * config.atr_stop_loss_multiplier) * config.profit_momentum_sl_tighten_factor
                            new_sl_price_candidate = (latest_close - tightened_sl_dist) if part_side == config.pos_long \
                            ...
                            if part_side == config.pos_long and new_sl_price_candidate > part_sl_price and new_sl_price_candidate < latest_close:
                                sl_improved_and_valid = True
                            elif part_side == config.pos_short and new_sl_price_candidate < part_sl_price and new_sl_price_candidate > latest_close:
                                sl_improved_and_valid = True
                            
                            if sl_improved_and_valid:
                                logger.info(f"{NEON['ACTION']}Profit Momentum: Attempting SL tighten for part {part_id} from {part_sl_price} to {new_sl_price_candidate:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}{NEON['RESET']}")
                                modify_position_sl_tp(exchange, config, part, new_sl=new_sl_price_candidate) # TP remains unchanged by this logic
                    ...
                    if config.enable_breakeven_sl and not part.get('breakeven_set', False) and latest_atr > 0:
                        ...
                        profit_per_unit = (latest_close - part_entry_price) if part_side == config.pos_long else (part_entry_price - latest_close)
                        current_profit_atr = profit_per_unit / latest_atr if latest_atr > 0 else Decimal(0)
                        
                        atr_target_met = current_profit_atr >= config.breakeven_profit_atr_target
                        abs_pnl_target_met = current_unrealized_pnl >= config.breakeven_min_abs_pnl_usdt # Absolute PNL check
                        
                        if atr_target_met and abs_pnl_target_met:
                            new_sl_price_candidate = part_entry_price # Move SL to entry price
                            
                            # Check if moving to breakeven is an improvement and valid
                            sl_improved_and_valid_be = False
                            if part_side == config.pos_long and new_sl_price_candidate > part_sl_price: # SL moves up
                                sl_improved_and_valid_be = True
                            elif part_side == config.pos_short and new_sl_price_candidate < part_sl_price: # SL moves down
                                sl_improved_and_valid_be = True
                            
                            if sl_improved_and_valid_be:
                                logger.info(f"{NEON['ACTION']}Breakeven SL Triggered: Attempting to move SL for part {part_id} to entry price {new_sl_price_candidate}{NEON['RESET']}")
                                if modify_position_sl_tp(exchange, config, part, new_sl=new_sl_price_candidate): # TP remains unchanged
                                    part['breakeven_set'] = True # Mark as set to avoid re-triggering
                                    save_persistent_state(force_heartbeat=True) # Save state after successful BE set
                    
                    # Last Chance Exit (preemptive exit if price moves adversely near SL)
                    part.get('adverse_candle_closes', deque()).append(latest_close) # Track recent closes
                    if config.enable_last_chance_exit and latest_atr > 0 and \
                       len(part.get('adverse_candle_closes',[])) >= config.last_chance_consecutive_adverse_candles:
                        
                        adverse_closes_list = list(part['adverse_candle_closes'])
                        # Check for consecutive adverse movements
                        is_cons_adverse = False
                        if part_side == config.pos_long: # For long, adverse is price going down
                            is_cons_adverse = all(adverse_closes_list[i] < adverse_closes_list[i-1] for i in range(1, len(adverse_closes_list)))
                        else: # For short, adverse is price going up
                            is_cons_adverse = all(adverse_closes_list[i] > adverse_closes_list[i-1] for i in range(1, len(adverse_closes_list)))
                        
                        if is_cons_adverse:
                            dist_to_sl = abs(latest_close - part_sl_price)
                            sl_prox_thresh_price = latest_atr * config.last_chance_sl_proximity_atr # Proximity threshold in price terms
                            if dist_to_sl <= sl_prox_thresh_price:
                                logger.warning(f"{NEON['WARNING']}Last Chance Exit Triggered for part {part_id}: "
                                ...
                                if close_position_part(exchange, config, part, "Last Chance Preemptive Exit"):
                    ...
                    if config.strategy_name == StrategyName.EHLERS_FISHER and \
                       config.ehlers_enable_divergence_scaled_exit and \
                       not part.get('divergence_exit_taken', False) and \
                       'entry_fisher_value' in part and part['entry_fisher_value'] is not None and \
                       config.qty_step is not None: # Ensure qty_step is available
                        ...
                        if not pd.isna(fisher_now) and not pd.isna(signal_now):
                            ...
                            entry_signal = part.get('entry_signal_value', pd.NA) # May not always be set if old state
                            if not pd.isna(entry_fisher) and not pd.isna(entry_signal):
                                ...
                                if part_side == config.pos_long and fisher_now > signal_now and fisher_now < entry_fisher and \
                                   current_spread < (initial_spread * config.ehlers_divergence_threshold_factor):
                                    divergence_met = True
                                elif part_side == config.pos_short and fisher_now < signal_now and fisher_now > entry_fisher and \
                                     current_spread < (initial_spread * config.ehlers_divergence_threshold_factor):
                                    divergence_met = True
                                
                                if divergence_met:
                                    qty_to_close_f = part_qty * config.ehlers_divergence_exit_percentage
                                    # Adjust to lot size
                                    qty_to_close = (qty_to_close_f // config.qty_step) * config.qty_step if config.qty_step > 0 else qty_to_close_f

                                    if qty_to_close > CONFIG.position_qty_epsilon:
                                        logger.info(f"{NEON['ACTION']}Ehlers Divergence Scaled Exit ({config.ehlers_divergence_exit_percentage*100}%): "
                                        ...
                                        temp_part_for_partial_close['qty'] = qty_to_close 
                                        
                                        if close_position_part(exchange, config, temp_part_for_partial_close, "Ehlers Divergence Partial Exit"):
                                            part['qty'] -= qty_to_close # Reduce original part quantity
                                            part['divergence_exit_taken'] = True # Mark this type of exit as taken
                                            
                                            if part['qty'] <= CONFIG.position_qty_epsilon: # If remaining qty is negligible
                                                logger.info(f"Part {part_id} fully closed due to negligible remainder after Ehlers divergence partial exit.")
                                                _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_id]
                                            else: # Update the initial_usdt_value if part still active
                                                part['initial_usdt_value'] = part['qty'] * part_entry_price
                                            ...
                                            if part_id not in [p.get('part_id') for p in _active_trade_parts]: # Check if original part was removed
                                                continue # Move to next part in active_parts_copy
                                    else:
                                        logger.debug(f"Ehlers divergence scaled exit: calculated qty_to_close ({qty_to_close}) too small or close failed.")
                    ...
                    if strategy_exit_triggered:
                        logger.info(f"{NEON['ACTION']}Strategy omen to EXIT {part_side} for part {part_id} ({config.symbol}): {exit_reason}{NEON['RESET']}")
                    ...
                    if part_sl_price is not None: # Check if SL is set for the part
                        if (part_side == config.pos_long and latest_close <= part_sl_price) or \
                           (part_side == config.pos_short and latest_close >= part_sl_price):
                            logger.warning(f"{NEON['WARNING']}Part {part_id} SL price {part_sl_price} potentially breached by current close {latest_close}. "
                    ...
                    if config.enable_take_profit and part_tp_price is not None: # Check if TP is set and enabled
                        if (part_side == config.pos_long and latest_close >= part_tp_price) or \
                           (part_side == config.pos_short and latest_close <= part_tp_price):
                            logger.info(f"{NEON['INFO']}Part {part_id} TP price {part_tp_price} potentially reached by current close {latest_close}. "
                                        f"Exchange should handle closure. Bot will detect on next cycle.{NEON['RESET']}")
                ...
                logger.debug(f"{NEON['COMMENT']}# Cycle {loop_counter} complete. Resting {config.sleep_seconds}s...{NEON['RESET']}")
                ...
                if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Shutdown", "Manual shutdown (Ctrl+C) initiated.")
                break # Exit main while loop
            except Exception as e_iter: # Catch-all for errors within a single loop iteration
                logger.critical(f"{NEON['CRITICAL']}Critical error in main loop iteration {loop_counter}! Error: {e_iter}{NEON['RESET']}")
                logger.debug(traceback.format_exc())
                if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Critical Error", f"Loop iteration {loop_counter} crashed: {str(e_iter)[:150]}")
                logger.info(f"Resting longer ({config.sleep_seconds * 5}s) after critical error in iteration to allow recovery or manual intervention.")
        ...
        logger.info(f"{NEON['HEADING']}=== Pyrmethus Spell Concludes ({datetime.now(pytz.utc).isoformat()}) ==={NEON['RESET']}")
        if exchange: # Ensure exchange object exists
            close_all_symbol_positions(exchange, config, "Spell Ending Sequence / Shutdown")
        ...
        if hasattr(CONFIG, 'send_notification_method'): 
             CONFIG.send_notification_method("Pyrmethus Offline", f"Spell concluded for {config.symbol}. Review logs.")

if __name__ == "__main__":
    logger.info(f"{NEON['COMMENT']}# Pyrmethus prepares to breach veil to the exchange realm...{NEON['RESET']}")
    ...
    try:
        ...
        if CONFIG.PAPER_TRADING_MODE:
            logger.warning(f"{NEON['WARNING']}PAPER TRADING MODE IS ACTIVE. Connecting to Bybit Testnet.{NEON['RESET']}")
            exchange_params['urls'] = {'api': 'https://api-testnet.bybit.com'}
        
        exchange_instance = ccxt.bybit(exchange_params)
        logger.info(f"Attempting to connect to: {exchange_instance.id} (CCXT Version: {ccxt.__version__})")
        ...
        if CONFIG.symbol not in markets:
            ...
            logger.critical(f"{NEON['CRITICAL']}{err_msg}{NEON['RESET']}")
            if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Startup Fail", err_msg)
            sys.exit(1)
        ...
        if CONFIG.tick_size is None or CONFIG.tick_size <= Decimal(0):
            logger.warning(f"{NEON['WARNING']}Could not determine valid tick size from market info for {CONFIG.symbol}. Got: {price_prec_raw}. Using a small default (1e-8). This may cause issues.{NEON['RESET']}")
            CONFIG.tick_size = Decimal("1e-8") # Fallback, but orders might fail if this is wrong
        ...
        if CONFIG.qty_step is None or CONFIG.qty_step <= Decimal(0):
            logger.warning(f"{NEON['WARNING']}Could not determine valid quantity step from market info for {CONFIG.symbol}. Got: {amount_prec_raw}. Using a small default (1e-8). This may cause issues.{NEON['RESET']}")
        ...
        category = "linear" if CONFIG.symbol.endswith(":USDT") else "inverse" # Determine category for Bybit
        ...
        try:
            ...
            logger.info(f"Setting leverage to {CONFIG.leverage}x for {CONFIG.symbol} (Category: {category})...")
            ...
            logger.success(f"{NEON['SUCCESS']}Leverage for {CONFIG.symbol} set/confirmed. Response (partial): {json.dumps(response, default=str)[:200]}...{NEON['RESET']}") # type: ignore [attr-defined]
        except Exception as e_lev:
            logger.warning(f"{NEON['WARNING']}Could not set leverage for {CONFIG.symbol} (may be pre-set, or an issue with API/permissions): {e_lev}{NEON['RESET']}")
            if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Leverage Warn", f"Leverage set issue for {CONFIG.symbol}: {str(e_lev)[:60]}")

        logger.success(f"{NEON['SUCCESS']}Successfully connected to {exchange_instance.id} for trading {CONFIG.symbol}.{NEON['RESET']}") # type: ignore [attr-defined]
        if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Online", f"Connected to {exchange_instance.id} for {CONFIG.symbol} @ {CONFIG.leverage}x.")
    
    except AttributeError as e_attr: # e.g. ccxt.bybit misspelled or ccxt version issue
        logger.critical(f"{NEON['CRITICAL']}Exchange attribute error during initialization: {e_attr}. CCXT library issue or invalid exchange ID?{NEON['RESET']}")
        sys.exit(1)
    except ccxt.AuthenticationError as e_auth:
        logger.critical(f"{NEON['CRITICAL']}Authentication failed! Please check API keys and permissions: {e_auth}{NEON['RESET']}")
        if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Auth Fail", f"API Authentication Error. Check keys.")
        sys.exit(1)
    except ccxt.NetworkError as e_net: # Connectivity issues
        logger.critical(f"{NEON['CRITICAL']}Network Error during exchange connection: {e_net}. Check internet connection and DNS.{NEON['RESET']}")
        if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Network Error", f"Cannot connect to exchange: {str(e_net)[:80]}")
        sys.exit(1)
    except ccxt.ExchangeError as e_exch: # General exchange API errors not caught by specific types
        logger.critical(f"{NEON['CRITICAL']}Exchange API Error during initialization: {e_exch}. Check API status, symbol, or account permissions.{NEON['RESET']}")
        if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus API Error", f"Exchange API issue: {str(e_exch)[:80]}")
        sys.exit(1)
    except Exception as e_general_init: # Catch-all for any other startup errors
        logger.critical(f"{NEON['CRITICAL']}A critical error occurred during Pyrmethus's awakening (exchange init or pre-requisites): {e_general_init}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        if hasattr(CONFIG, 'send_notification_method'): CONFIG.send_notification_method("Pyrmethus Init Error", f"General exchange init failed: {str(e_general_init)[:80]}")
        sys.exit(1)
    ...
    if exchange_instance:
        ...
        logger.critical(f"{NEON['CRITICAL']}Exchange instance not initialized. Pyrmethus cannot cast spells. Aborting.{NEON['RESET']}")
        sys.exit(1)
