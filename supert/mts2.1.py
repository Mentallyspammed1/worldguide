...
class DualSupertrendStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        confirm_is_up = last.get("confirm_trend", pd.NA) # pd.NA is better than None for pandas boolean series
...
def calculate_supertrend(df: pd.DataFrame, length: int, multiplier: Decimal, prefix: str = "") -> pd.DataFrame:
    ...
    out_supertrend_val = f"{col_prefix}supertrend" # This column from pandas_ta is the ST line value
    ...
    out_short_flip = f"{col_prefix}st_short_flip" # Boolean: True if just flipped to Short
    ...
    if df is None or df.empty or not all(c in df.columns for c in ["high", "low", "close"]) or len(df) < min_len_needed_for_pta:
        logger.warning(f"{Fore.YELLOW}Scrying ({col_prefix}ST): Insufficient data (Rows: {len(df) if df is not None else 0}, Need approx: {min_len_needed_for_pta}). Populating with NAs.{Style.RESET_ALL}")
    ...
    try:
        ...
        pta_cols_to_check = [pta_st_val_col, pta_st_dir_col] # pta_st_long_level_col, pta_st_short_level_col are also created
        ...
        df[out_supertrend_val] = temp_df[pta_st_val_col].apply(lambda x: safe_decimal_conversion(x, pd.NA)) # Keep NA for Decimal
        ...
        df[out_long_flip] = (temp_df[pta_st_dir_col].values == 1) & (prev_dir.values == -1)
        df[out_short_flip] = (temp_df[pta_st_dir_col].values == -1) & (prev_dir.values == 1)
        
        if not df.empty and not df.iloc[-1].isnull().all(): # Check last row has some data
            ...
            flip_str = "L" if last_l_flip else ("S" if last_s_flip else ("None" if not pd.isna(last_l_flip) and not pd.isna(last_s_flip) else "N/A")) # Handle NA in flip bools
...
def analyze_volume_atr(df: pd.DataFrame, atr_len: int, vol_ma_len: int) -> dict[str, Decimal | pd.NA | None]:
    results: dict[str, Decimal | pd.NA | None] = {"atr": pd.NA, "volume_ma": pd.NA, "last_volume": pd.NA, "volume_ratio": pd.NA}
    ...
    if df is None or df.empty or not all(c in df.columns for c in ["high", "low", "close", "volume"]) or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (Vol/ATR): Insufficient data (Rows: {len(df) if df is not None else 0}, Need approx: {min_len_needed}).{Style.RESET_ALL}")
    ...
    try:
        ...
        temp_df['volume'] = pd.to_numeric(temp_df['volume'], errors='coerce') # Ensure numeric
        ...
        if not temp_df.empty:
            ...
            if not pd.isna(results["volume_ma"]) and not pd.isna(results["last_volume"]) and results["volume_ma"] > CONFIG.position_qty_epsilon :
                try: 
...
def calculate_stochrsi_momentum(df: pd.DataFrame, rsi_len: int, stoch_len: int, k: int, d: int, mom_len: int) -> pd.DataFrame:
    ...
    min_len_needed = max(rsi_len + stoch_len + d, mom_len) + 10 # Add buffer for stabilization
    
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (StochRSI/Mom): Insufficient data (Rows: {len(df) if df is not None else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is not None:
            for col in target_cols: df[col] = pd.NA
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or not all(c in df.columns for c in ["high", "low"]) or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (EhlersFisher): Insufficient data (Rows: {len(df) if df is not None else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is not None:
            for col in target_cols: df[col] = pd.NA
        return df if df is not None else pd.DataFrame(columns=target_cols)
...
def calculate_ehlers_ma(df: pd.DataFrame, fast_len: int, slow_len: int, poles: int) -> pd.DataFrame:
    ...
    if df is None or df.empty or "close" not in df.columns or len(df) < min_len_needed:
        logger.warning(f"{Fore.YELLOW}Scrying (Ehlers SSF MA): Insufficient data (Rows: {len(df) if df is not None else 0}, Need approx: {min_len_needed}). Populating NAs.{Style.RESET_ALL}")
        if df is not None:
            for col in target_cols: df[col] = pd.NA
        return df if df is not None else pd.DataFrame(columns=target_cols)
    ...
    try:
        ...
        df["ehlers_ssf_fast"] = ssf_fast_series.apply(lambda x: safe_decimal_conversion(x, pd.NA)) if ssf_fast_series is not None else pd.NA
        
        ssf_slow_series = df.ta.ssf(length=slow_len, poles=poles, append=False) # type: ignore
        df["ehlers_ssf_slow"] = ssf_slow_series.apply(lambda x: safe_decimal_conversion(x, pd.NA)) if ssf_slow_series is not None else pd.NA
...
def analyze_order_book(exchange: ccxt.Exchange, symbol: str, depth: int, fetch_limit: int) -> dict[str, Decimal | pd.NA | None]:
    results: dict[str, Decimal | pd.NA | None] = {"bid_ask_ratio": pd.NA, "spread": pd.NA, "best_bid": pd.NA, "best_ask": pd.NA}
    if not exchange.has.get("fetchL2OrderBook"): # fetchL2OrderBook is generally preferred over fetchOrderBook
    ...
    try:
        ...
        if not pd.isna(results["best_bid"]) and not pd.isna(results["best_ask"]) and results["best_bid"] > 0 and results["best_ask"] > 0: # type: ignore
...
def get_market_data(exchange: ccxt.Exchange, symbol: str, interval: str, limit: int) -> pd.DataFrame | None:
    ...
    try:
        ...
        if df.isnull().values.any():
            ...
            df.bfill(inplace=True) # Backward fill remaining NaNs (usually at the beginning)
...
def check_account_health(exchange: ccxt.Exchange, config: Config) -> bool:
    ...
    try:
        ...
        total_equity = safe_decimal_conversion(usdt_balance.get("total")) # Total equity
        used_margin = safe_decimal_conversion(usdt_balance.get("used"))   # Margin used for positions/orders
        ...
        if total_equity <= Decimal("0"):
            ...
            if used_margin > Decimal("0"):
                 ...
                 send_sms_alert(f"[{config.symbol.split('/')[0]}/{config.strategy_name.value}] CRITICAL: Zero/Neg Equity with Used Margin. BOT PAUSED.")
                 return False # Critically unhealthy
            return True # No used margin, so technically "healthy" but no funds.

        margin_ratio = used_margin / total_equity if total_equity > 0 else Decimal("Infinity") # Avoid DivByZero
        ...
        if margin_ratio > config.max_account_margin_ratio:
            ...
            send_sms_alert(f"[{config.symbol.split('/')[0]}/{config.strategy_name.value}] CRITICAL: High margin ratio {margin_ratio:.2%}. BOT PAUSED.")
        ...
        send_sms_alert(f"[{config.symbol.split('/')[0]}/{config.strategy_name.value}] WARNING: Account health check FAILED: {type(e).__name__}.")
        return False
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        for pos in fetched_positions:
            ...
            if int(pos_info.get("positionIdx", -1)) == 0: # Ensure One-Way mode position
                size_str = pos_info.get("size", "0")
                size_dec = safe_decimal_conversion(size_str)

                if size_dec > CONFIG.position_qty_epsilon: # If position has significant size
...
def close_position(exchange: ccxt.Exchange, symbol: str, position_to_close: dict[str, Any], reason: str = "Signal") -> dict[str, Any] | None:
    ...
    amount_to_close_str = format_amount(exchange, symbol, live_position["qty"]) # Use live qty
    ...
    try:
        ...
        avg_fill_price_closed = safe_decimal_conversion(order.get("average", "0")) # Average fill price of this close order
        ...
        if status == "closed" and abs(filled_qty_closed - live_position["qty"]) < CONFIG.position_qty_epsilon:
            ...
            if _active_trade_details.get("entry_price") is not None and order.get("timestamp") is not None:
                # Ensure all required details for TradeMetrics are valid
                if _active_trade_details.get("side") and _active_trade_details.get("qty"):
                    trade_metrics.log_trade(
                        symbol=symbol, 
                        side=_active_trade_details["side"], # Original side of the trade
                        ...
                        exit_time_ms=order["timestamp"], # Timestamp of this close order
                        reason=reason
                    )
                else:
                    logger.warning(f"TradeMetrics: Skipping log for {symbol} due to missing original side/qty in _active_trade_details.")
            else:
...
def calculate_position_size(equity: Decimal, risk_pct: Decimal, entry_px: Decimal, sl_px: Decimal, lev: int, sym: str, ex: ccxt.Exchange) -> tuple[Decimal | None, Decimal | None]:
    if not (entry_px > 0 and sl_px > 0 and 0 < risk_pct < 1 and equity > 0 and lev > 0):
        logger.error(f"{Fore.RED}RiskCalc: Invalid inputs for position sizing (Equity: {equity}, Risk%: {risk_pct}, Entry: {entry_px}, SL: {sl_px}, Lev: {lev}).{Style.RESET_ALL}"); return None, None
    
    price_diff_per_unit = abs(entry_px - sl_px) # Potential loss per unit if SL hits
    if price_diff_per_unit < CONFIG.position_qty_epsilon: # Avoid division by zero if entry and SL are identical
        logger.error(f"{Fore.RED}RiskCalc: Entry price and Stop Loss price are too close or identical. Cannot calculate position size.{Style.RESET_ALL}"); return None, None
    ...
    try:
        ...
        qty_prec_str = format_amount(ex, sym, qty_raw) # Uses exchange.amount_to_precision
        qty_prec = Decimal(qty_prec_str)
        
        if qty_prec <= CONFIG.position_qty_epsilon: # If calculated quantity is too small (e.g., due to high price_diff or low risk_amt)
            logger.warning(f"{Fore.YELLOW}RiskCalc: Calculated quantity ({_format_for_log(qty_prec,8)}) is negligible or zero after precision adjustment. Min risk not met or stop too wide.{Style.RESET_ALL}"); return None, None
        
        # Calculate estimated position value and margin required
        position_value_usdt = qty_prec * entry_px
        margin_required = position_value_usdt / Decimal(lev) # Margin = (Qty * EntryPx) / Leverage
...
def wait_for_order_fill(exchange: ccxt.Exchange, order_id: str, symbol: str, timeout_s: int) -> dict[str, Any] | None:
    ...
    while time.time() - start_time < timeout_s:
        try:
            ...
            status = order.get("status") # CCXT normalized status: 'open', 'closed', 'canceled', etc.
            
            if status == "closed": # 'closed' usually means fully filled for market/limit orders that executed
...
def place_risked_market_order(exchange: ccxt.Exchange, symbol: str, side: str, risk_percentage: Decimal, current_atr: Decimal | pd.NA | None, sl_atr_multiplier: Decimal, leverage: int, max_order_cap_usdt: Decimal, margin_check_buffer: Decimal, tsl_percent: Decimal, tsl_activation_offset_percent: Decimal) -> dict[str, Any] | None:
    ...
    if pd.isna(current_atr) or current_atr is None or current_atr <= 0:
        logger.error(f"{Fore.RED}Place Order Error: Invalid ATR ({current_atr}). Cannot calculate Stop Loss or position size.{Style.RESET_ALL}"); return None
    ...
    try:
        ...
        if usdt_equity <= 0 or usdt_free < 0: # free can be slightly negative due to fees sometimes
            logger.error(f"{Fore.RED}Place Order Error: Invalid equity ({_format_for_log(usdt_equity,2)}) or free margin ({_format_for_log(usdt_free,2)}).{Style.RESET_ALL}"); return None
        ...
        if pd.isna(entry_px_est) or entry_px_est is None or entry_px_est <= 0:
            ticker = safe_api_call(exchange.fetch_ticker, symbol) # Fallback to last price
            entry_px_est = safe_decimal_conversion(ticker.get("last"))
        if pd.isna(entry_px_est) or entry_px_est is None or entry_px_est <= 0:
            logger.error(f"{Fore.RED}Place Order Error: Failed to get a valid entry price estimate for {symbol}.{Style.RESET_ALL}"); return None
        ...
        sl_px_est_str = format_price(exchange, symbol, sl_px_raw_est) # Format to exchange precision
        sl_px_est = Decimal(sl_px_est_str)
        if sl_px_est <= 0: 
            logger.error(f"{Fore.RED}Place Order Error: Invalid estimated SL price ({_format_for_log(sl_px_est,4)}) after calculation.{Style.RESET_ALL}"); return None
        ...
        if final_qty is None or margin_est is None or final_qty <= 0 or margin_est <=0:
            logger.error(f"{Fore.RED}Place Order Error: Position size calculation failed or resulted in zero/negative qty/margin.{Style.RESET_ALL}"); return None
        ...
        if estimated_pos_value > max_order_cap_usdt:
            ...
            final_qty = Decimal(format_amount(exchange, symbol, max_order_cap_usdt / entry_px_est)) # Recalculate qty based on cap
            margin_est = (final_qty * entry_px_est) / Decimal(leverage) # Recalculate margin
            if final_qty <= 0: logger.error(f"{Fore.RED}Place Order: Quantity became zero after cap adjustment.{Style.RESET_ALL}"); return None
        ...
        if min_qty_allowed > 0 and final_qty < min_qty_allowed:
            logger.error(f"{Fore.RED}Place Order Error: Calculated qty {_format_for_log(final_qty,8)} is less than min allowed {_format_for_log(min_qty_allowed,8)} for {symbol}.{Style.RESET_ALL}"); return None
        if usdt_free < margin_est * margin_check_buffer:
            logger.error(f"{Fore.RED}Place Order Error: Insufficient FREE margin. Need ~{_format_for_log(margin_est * margin_check_buffer,2)}, Have {_format_for_log(usdt_free,2)}.{Style.RESET_ALL}"); return None

        # --- Place Entry Order ---
        entry_params = {"reduceOnly": False, "category": v5_category, "positionIdx": 0} # For One-Way mode entry
        ...
        tsl_placed_successfully = False # Assume TSL is optional or best-effort if fixed SL is primary
        ...
        if CONFIG.trailing_stop_percentage > 0 and sl_placed_successfully: # Only place TSL if fixed SL was attempted (even if failed, to see if TSL works) or if SL succeeded
            ...
            tsl_value_for_api = str((tsl_percent * Decimal("100")).normalize()) # e.g., 0.005 -> "0.5"
            ...
            if Decimal(tsl_activation_price_str) <= 0:
                ...
                tsl_params = {
                    ...
                    "activePrice": float(tsl_activation_price_str), # Price at which TSL activates
                    "reduceOnly": True,
                    "positionIdx": 0, # For One-Way mode
                }
                try:
                    ...
                    tsl_order = safe_api_call(exchange.create_order, symbol, "StopMarket", tsl_order_side, float(filled_qty_val), price=None, params=tsl_params)
                    ...
                    tsl_placed_successfully = True # Or some other logic if TSL is optional
                except Exception as e_tsl:
                    logger.error(f"{Back.YELLOW}WARNING: FAILED to place Trailing SL order: {e_tsl}{Style.RESET_ALL}") # Warning as TSL might be secondary
                    logger.debug(traceback.format_exc())
        ...
        if not sl_placed_successfully: # Fixed SL is considered essential
            ...
            close_order_details = close_position(exchange, symbol, _active_trade_details.copy(), reason=emergency_close_reason) # Pass copy
...
def cancel_open_orders(exchange: ccxt.Exchange, symbol: str, reason: str = "Cleanup") -> int:
    ...
    try:
        ...
        if not open_orders:
            logger.info(f"{Fore.CYAN}Order Cleanup: No open orders found for {symbol} (Category: {v5_category}).{Style.RESET_ALL}")
            return 0
        ...
        if failed_count > 0:
            send_sms_alert(f"[{symbol.split('/')[0]}/{CONFIG.strategy_name.value}] WARNING: Failed to cancel {failed_count} orders during cleanup ({reason}). Manual check recommended.")
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    cycle_time_str = df.index[-1].strftime("%Y-%m-%d %H:%M:%S %Z") if not df.empty and df.index[-1] else "N/A"
    ...
    if df is None or len(df) < required_rows_for_any_indicator:
        logger.warning(f"{Fore.YELLOW}Trade Logic: Insufficient data for {symbol} ({len(df) if df is not None else 0} rows, need ~{required_rows_for_any_indicator}). Skipping logic cycle.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        current_price = safe_decimal_conversion(last_candle_data.get("close"), pd.NA) # Decimal or pd.NA

        if pd.isna(current_price) or current_price <= 0:
            logger.warning(f"{Fore.YELLOW}Trade Logic: Invalid or missing last close price for {symbol}. Last candle: {last_candle_data.to_dict()}{Style.RESET_ALL}"); return
        
        # Condition for being able to place an order (mainly for ATR-based SL)
        can_place_order_with_atr_sl = not pd.isna(current_atr) and current_atr is not None and current_atr > 0
        ...
        strategy_logger = CONFIG.strategy_instance.logger # Use the strategy's own logger instance
        ...
        for col_name in CONFIG.strategy_instance.required_columns:
             if col_name in last_candle_data.index:
                 is_trend_col = "trend" in col_name.lower() # Simple check for ST trend columns
        ...
        logger.info(f"{Fore.MAGENTA}{'-'* (len(f'--- Indicator & Market Snapshot for {symbol} ---') -2)}{Style.RESET_ALL}")
        # --- End Snapshot ---
        ...
        if should_exit_long or should_exit_short:
            ...
            cancel_open_orders(exchange, symbol, f"Pre-Exit ({exit_reason})"); time.sleep(0.5) # Cancel stops before market close
            close_result = close_position(exchange, symbol, position, reason=exit_reason) # position dict from get_current_position
        ...
        # Order Book Filter
        ob_confirm_long, ob_confirm_short = True, True # Default to true if filter not used or data unavailable
        if ob_data and not pd.isna(ob_data.get("bid_ask_ratio")) and ob_data.get("bid_ask_ratio") is not None:
            ...
            if CONFIG.order_book_ratio_threshold_short > Decimal(0): # Assuming short ratio is < 1
            ...
            logger.warning(f"{Fore.YELLOW}Order book filter active, but no valid OB data. Entry blocked by OB filter.{Style.RESET_ALL}")
        ...
        if final_enter_long:
            ...
            cancel_open_orders(exchange, symbol, "Pre-Long Entry"); time.sleep(0.5) # Cancel any existing stops/limits
        ...
        elif potential_enter_long or potential_enter_short: # Signal was there, but filters blocked
    ...
    except Exception as e: # Catch-all for unexpected errors within a single trade logic cycle
        ...
        try:
            ...
            if position["side"] != CONFIG.pos_none and position["qty"] > CONFIG.position_qty_epsilon:
                ...
                if close_result: # If close order was placed
                    ...
                    time.sleep(CONFIG.post_close_delay_seconds * 2) # Longer delay to allow fill
...
def main() -> None:
    ...
    exchange: ccxt.Exchange | None = None # Ensure it's defined in the broader scope for finally block
    ...
    try:
        ...
        while run_bot:
            ...
            if not check_account_health(exchange, CONFIG):
                ...
                time.sleep(CONFIG.sleep_seconds * 10) # Extended pause on health failure
            ...
            try:
                ...
                indicator_lookbacks = [
                    ...
                    CONFIG.stochrsi_rsi_length + CONFIG.stochrsi_stoch_length + CONFIG.stochrsi_d_period, # StochRSI needs sum of lengths
            ...
            except Exception as e_mainloop: # Catch any other unexpected errors in the loop
    ...
    except Exception as startup_error: # Catch errors during initial setup (before main loop)
