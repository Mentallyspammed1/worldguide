...
if not any(isinstance(h, ColorStreamHandler) for h in root_logger.handlers): # Avoid duplicate handlers
...
def _flush_single_notification_type(messages: List[Dict[str, Any]], notification_type: str, config_notifications: Dict[str, Any]) -> None:
    ...
    if notification_type == "termux":
        try:
            ...
            else: logger_notify.error(f"Failed to send batched Termux notification. RC: {process.returncode}. Err: {stderr.decode().strip()}")
        ...
        if telegram_bot_token and telegram_chat_id:
            ...
            except requests.exceptions.RequestException as e: logger_notify.error(f"Telegram API error: {e}", exc_info=True)
            except Exception as e: logger_notify.error(f"Error sending Telegram notification: {e}", exc_info=True)
        else: logger_notify.warning("Telegram notifications configured but TOKEN or CHAT_ID missing.")
...
class Config:
    def __init__(self) -> None:
        ...
        if not self.exchange["paper_trading"]:
            self.logger.warning(f"{NEON['CRITICAL']}LIVE TRADING MODE ENABLED. Ensure API keys are for MAINNET and you understand the risks.{NEON['RESET']}")
        else:
...
@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.RateLimitExceeded, ccxt.DDoSProtection), tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2)
async def async_fetch_account_balance(exchange: ccxt_async.Exchange, currency: str, config: Config) -> Optional[Decimal]:
    ...
    try:
        ...
        params = {'accountType': 'UNIFIED', 'coin': currency} if currency == config.usdt_symbol else {'accountType': 'UNIFIED'}
                ...
                if 'coin' in acct_list[0]:
                    for coin_entry in acct_list[0]['coin']:
                        if coin_entry.get('coin') == currency:
                            total_balance = safe_decimal_conversion(coin_entry.get('walletBalance')) # Or 'equity' or 'availableToWithdraw'
                            break
        ...
        if total_balance is None: # Final fallback
            total_balance = safe_decimal_conversion(balance_data.get('total', {}).get(currency))

        if total_balance is None or total_balance < Decimal(0):
            logger.error(f"Invalid or unreadable balance for {currency}. Data: {balance_data}", extra={"data": balance_data})
            return None
        
        balance_cache[cache_key] = total_balance
        logger.info(f"Current {currency} Treasury: {NEON['VALUE']}{total_balance:.2f}{NEON['RESET']}")
        return total_balance
    except ccxt.AuthenticationError as e:
        logger.critical(f"Authentication error fetching balance: {e}", exc_info=True)
        send_general_notification("Pyrmethus Auth Error", f"API Key Invalid (Balance Fetch): {str(e)[:80]}")
        raise # Re-raise to be caught by main error handler
    except Exception as e:
        logger.error(f"Failed to fetch account balance for {currency}: {e}", exc_info=True)
        return None

@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.RateLimitExceeded, ccxt.DDoSProtection), tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2)
async def async_get_market_data(exchange: ccxt_async.Exchange, symbol: str, timeframe: str, limit: int, config: Config) -> Optional[pd.DataFrame]:
    ...
    if cache_key in market_data_cache:
        ...
        return market_data_cache[cache_key].copy() # Return a copy to prevent cache mutation

    logger.debug(f"Fetching OHLCV data for {symbol} {timeframe} (limit {limit}) from exchange.")
    try:
        params = {'category': 'linear'} if symbol.endswith(f":{config.usdt_symbol}") else {}
        ...
        if df.isnull().values.any():
            logger.warning(f"NaN values found in OHLCV data for {symbol} {timeframe}. Proceeding with caution.",
                           extra={"data": {"symbol":symbol, "timeframe":timeframe, "nan_info": df.isnull().sum().to_dict()}})
        
        market_data_cache[cache_key] = df.copy() # Store a copy in cache
        logger.debug(f"Summoned {len(df)} candles for {symbol} {timeframe}. Latest: {df['timestamp'].iloc[-1].strftime('%Y-%m-%d %H:%M:%S %Z') if not df.empty else 'N/A'}")
        return df
    except ccxt.AuthenticationError as e:
        logger.critical(f"Authentication error fetching market data: {e}", exc_info=True)
        send_general_notification("Pyrmethus Auth Error", f"API Key Invalid (Market Data): {str(e)[:80]}")
        raise
    except Exception as e:
        logger.error(f"Failed to fetch market data for {symbol} {timeframe}: {e}", exc_info=True)
        return None
...
async def stream_price_data(exchange: ccxt_async.Exchange, config: Config, price_queue: asyncio.Queue):
    ...
    while True:
        try:
                ...
                if latest_close is not None:
                    if price_queue.full():
                        try:
                            ...
                            logger_ws.warning(f"Price queue was full for {symbol}. Discarded oldest price.")
                        except asyncio.QueueEmpty:
                            pass # Should not happen if full() was true, but good practice
                    
                    await price_queue.put({"timestamp": timestamp_ms, "close": latest_close, "symbol": symbol})
                    logger_ws.debug(f"WS Ticker {symbol}: Price {format_price(latest_close, config.tick_size)} @ {datetime.fromtimestamp(timestamp_ms/1000, tz=pytz.utc).isoformat()}")
                else:
                    logger_ws.warning(f"Received ticker for {symbol} without a valid close price: {ticker}")
            # If watch_ticker loop exits, it means connection was closed or an issue occurred
            logger_ws.warning(f"WebSocket watch_ticker loop for {symbol} exited. Attempting to reconnect...")

        except ccxt.AuthenticationError as e:
            logger_ws.critical(f"Authentication error in WebSocket for {symbol}: {e}", exc_info=True)
            send_general_notification("Pyrmethus Auth Error", f"WS Auth Error ({symbol}): {str(e)[:80]}")
            raise # Propagate to main error handler to stop the bot
        except ccxt.NetworkError as e:
            consecutive_errors += 1
            logger_ws.error(f"WebSocket NetworkError for {symbol} (Attempt {consecutive_errors}/{max_consecutive_errors}): {e}. Retrying in {5 * consecutive_errors}s.", exc_info=LOGGING_LEVEL <= logging.DEBUG)
            if consecutive_errors >= max_consecutive_errors:
                logger_ws.critical(f"Max WebSocket reconnection attempts reached for {symbol}. Critical failure.")
                send_general_notification("Pyrmethus WS Critical", f"WS Max Retries ({symbol}): {str(e)[:80]}")
            ...
            consecutive_errors += 1
            logger_ws.error(f"Unexpected WebSocket error for {symbol} (Attempt {consecutive_errors}/{max_consecutive_errors}): {e}. Retrying in {5 * consecutive_errors}s.", exc_info=True)
            if consecutive_errors >= max_consecutive_errors:
                logger_ws.critical(f"Max WebSocket reconnection attempts reached for {symbol} due to unexpected errors.")
                send_general_notification("Pyrmethus WS Critical", f"WS Max Retries Unexpected ({symbol}): {str(e)[:80]}")
                raise
...
async def perform_health_check(exchange: ccxt_async.Exchange, config: Config) -> bool:
    ...
    try:
        ...
        logger_health.debug(f"Health check passed: Exchange API responsive. Server time: {datetime.fromtimestamp(server_time/1000, tz=pytz.utc).isoformat()}")
        _last_health_check_time = now
        return True
    except ccxt.AuthenticationError as e:
        logger_health.critical(f"Authentication error during health check: {e}", exc_info=True)
        send_general_notification("Pyrmethus Auth Error", f"API Key Invalid (Health Check): {str(e)[:80]}")
        raise # Critical, bot should stop
    except Exception as e:
        logger_health.error(f"Health check failed: Exchange API unresponsive or error: {e}", exc_info=LOGGING_LEVEL <= logging.DEBUG)
        send_general_notification("Pyrmethus Health Alert", f"Exchange API Unresponsive: {str(e)[:80]}")
        return False
...
async def trading_loop_iteration(async_exchange: ccxt_async.Exchange, config: Config, price_queue: asyncio.Queue):
    ...
    if latest_price_data:
        ...
        logger_loop.info(f"Processing with latest close: {format_price(latest_close, config.tick_size)}")
    else:
...
async def main():
    ...
    try:
        ...
        if CONFIG.exchange["paper_trading"]: 
            ...
            async_exchange_params['urls'] = {'api': ccxt_async.bybit.urls['api']['public']} # Mainnet
        ...
        try:
            await async_exchange_instance.fetch_balance({'accountType': 'UNIFIED'}) # Test call
            logger.success("API Key validated successfully with exchange.")
        except ccxt.AuthenticationError as e_auth_val:
            logger.critical(f"CRITICAL: API Key Authentication Failed during initial validation: {e_auth_val}. Please check your .env file for correct API_KEY and API_SECRET for the target environment (Live/Testnet) and ensure keys have necessary permissions.", exc_info=True)
        ...
        while True: # The true main loop for continuous operation
            ...
            if not await perform_health_check(async_exchange_instance, CONFIG):
                ...
                await asyncio.sleep(CONFIG.trading["sleep_seconds"] * 10) # Longer pause on health fail
            ...
            await trading_loop_iteration(async_exchange_instance, CONFIG, price_queue) # Pass the queue
