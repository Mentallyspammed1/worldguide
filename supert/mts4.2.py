...
class Config:
    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, color: str = NEON["PARAM"], secret: bool = False) -> Any:
        ...
        if value_to_cast is None:  # This can happen if default is None and env var is not set
            ...
            if default is None:
                ...
                try:
                    ...
                    if cast_type == bool:
                        ...
                        final_value = default_str_for_cast  # Fallback for unknown cast_type
                    _logger.warning(
...
if sys.stdout.isatty():  # Only add colors if output is a TTY
    ...
    file_handler = next((h for h in logging.getLogger(
    ).handlers if isinstance(h, logging.FileHandler)), None)
...
try:
    ...
    if 'termux-api' in os.getenv('PATH', ''):  # Basic check
        ...
        except Exception:  # Suppress errors from notification itself during early failure
            pass
    sys.exit(1)
...
class TradingStrategy(ABC):
    ...
    def _validate_df(self, df: pd.DataFrame, min_rows: int = 2) -> bool:
        ...
        if self.required_columns:  # Check for NaNs in the latest row for required columns
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    def __init__(self, config: Config):
        super().__init__(config, df_columns=[  # Ensure these are the actual column names generated
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        min_rows_needed = max(self.config.st_atr_length,
                              self.config.confirm_st_atr_length, self.config.momentum_period) + 10  # Heuristic buffer
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        min_rows_needed = self.config.ehlers_fisher_length + \
            ...
            5  # Heuristic buffer, +2 for prev/current
    ...
    def summary(self) -> str:
        ...
        if self.initial_equity is not None:
            ...
            overall_pnl_pct = (total_profit / self.initial_equity) * \
                100 if self.initial_equity > Decimal(
                    0) else Decimal(0)  # Avoid DivisionByZero
        ...
        if self.daily_start_equity is not None:
            daily_pnl = (current_equity_approx if self.initial_equity is not None else Decimal(
                0)) - self.daily_start_equity
...
def save_persistent_state(force_heartbeat: bool = False) -> None:
    ...
    should_save = force_heartbeat or \
        ...
        (now - _last_heartbeat_save_time >
         HEARTBEAT_INTERVAL_SECONDS * 5)  # Less frequent save if idle
    ...
    try:
        ...
        temp_file_path = STATE_FILE_PATH + ".tmp_scroll"  # Use a distinct temp extension
...
def calculate_position_size(balance: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, sl_price: Decimal, market_info: Dict) -> Optional[Decimal]:
    ...
    if not all(isinstance(val, Decimal) and val > 0 for val in [balance, entry_price]) or not (isinstance(sl_price, Decimal) and sl_price != entry_price):
        logger.warning(
    ...
    if quantity_base <= CONFIG.position_qty_epsilon:  # Effectively zero after precision adjustment
        logger.warning(
...
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price_target: Decimal, sl_price: Decimal, atr_val: Optional[Decimal]) -> Optional[Dict]:
    ...
    if quantity is None or quantity <= CONFIG.position_qty_epsilon:  # Check against epsilon
        logger.warning(
    ...
    try:
        ...
        if filled_order.get('status') == 'closed':
            ...
            new_part = {
                ...
                "sl_price": sl_price, "atr_at_entry": atr_val,
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part_to_close: Dict, reason: str, close_price_target: Optional[Decimal] = None) -> bool:
    ...
    logger.info(f"{NEON['ACTION']}Attempting to unravel trade part {part_id} ({part_side} {NEON['QTY']}{part_qty}{NEON['RESET']} {config.symbol}) for reason: {reason}{NEON['RESET']}")
    ...
    try:
        ...
        if filled_close_order.get('status') == 'closed':
            ...
            entry_price = part_to_close.get(
                'entry_price', Decimal(0))  # Should be Decimal
            ...
            trade_metrics.log_trade(
                symbol=config.symbol, side=part_side,
                entry_price=entry_price, exit_price=actual_exit_price,
                # Should be int
                qty=part_qty, entry_time_ms=part_to_close['entry_time_ms'],
            ...
            config.send_notification_method(
                f"Pyrmethus Position Closed", f"{config.symbol} Part {part_id} closed. PNL: {pnl:.2f} USDT. Reason: {reason}")
            return True
        else:
            logger.warning(
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    try:
        ...
        positions = exchange.fetch_positions(
            [config.symbol], params=params_fetch_pos)  # Pass symbol as list
        ...
        for pos_data in positions:
            ...
            if pos_data and pos_data.get('symbol') == config.symbol:
                ...
                if pos_qty > CONFIG.position_qty_epsilon:  # If a residual position exists
        ...
        try:
            ...
            if current_balance is not None:
                ...
                if drawdown_hit:
                    ...
                    config.send_notification_method(
                        "Pyrmethus Max Drawdown", f"Trading halted for {config.symbol}. Reason: {dd_reason}")
            ...
            side_color = 'SIDE_LONG' if current_pos_side == config.pos_long else \
            ...
            if latest_atr is None or latest_close is None or pd.isna(latest_atr) or pd.isna(latest_close):
                logger.warning(
            ...
            logger.debug(traceback.format_exc())
            config.send_notification_method(
                "Pyrmethus Critical Error", f"Bot loop crashed for {config.symbol}: {str(e)[:150]}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        amount_prec_raw = CONFIG.MARKET_INFO.get('precision', {}).get(
            'amount')  # This is typically the step size

        # Convert precision to number of decimal places if it's a step value like 0.01
        price_prec_display = int(-Decimal(str(price_prec_raw)).log10(
        )) if price_prec_raw is not None and Decimal(str(price_prec_raw)) < 1 else price_prec_raw
        amount_prec_display = int(-Decimal(str(amount_prec_raw)).log10(
        )) if amount_prec_raw is not None and Decimal(str(amount_prec_raw)) < 1 else amount_prec_raw
        ...
        CONFIG.send_notification_method(
            "Pyrmethus Online", f"Connected to {exchange.id} for {CONFIG.symbol} @ {CONFIG.leverage}x lev.")
