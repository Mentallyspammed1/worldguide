Okay, I've reviewed the Python script. It's already quite comprehensive and well-structured, particularly with its robust configuration, error handling, detailed logging, and support for multiple strategies and Bybit V5 features.

The "enhancements" I'll make are subtle, focusing on minor refinements and maintaining the existing high quality:

1.  **Configuration Default for SMS Recipient**:
    *   The `SMS_RECIPIENT_NUMBER` in the `Config` class had a hardcoded example phone number as an integer default (`+16364866381`). This isn't ideal as it loses the `+` and is specific.
    *   I've changed the default to `None` and ensured `cast_type=str`. The `send_sms_alert` function already correctly handles the case where the number is not set by logging a warning and not attempting to send an SMS.

2.  **Minor Comment Formatting**:
    *   A comment in the `Config` class for `ehlers_ssf_poles` was missing a space before the `#`. I've corrected this for consistency.

The core logic, indicator calculations, order management, and error handling appear solid and reflect the "Fortified Configuration & Enhanced Clarity/Robustness" mentioned in its version notes. The enhanced indicator logging is also well-implemented.

Here's the complete script with these minor improvements:
...
def calculate_supertrend(
    ...
    try:
        ...
        # Modify df directly with pandas_ta
        ...
        if not all(c in df.columns for c in [st_col, st_trend_col, st_long_col, st_short_col]):
            ...
            raise KeyError(f"pandas_ta failed to create expected raw columns for {col_prefix}ST (direct mod): {missing}")
        ...
        cols_to_drop = [c for c in raw_st_cols if c in df.columns and c not in target_cols]
        if cols_to_drop:
        ...
        if not df.empty:
            ...
            if pd.notna(last_st_val):
                ...
                last_trend = "Up" if last_trend_val is True else ("Down" if last_trend_val is False else "N/A")
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        for pos in fetched_positions:
            ...
            pos_side_v5 = pos_info.get("side", "None") # Bybit: "Buy", "Sell", or "None"
            size_str = pos_info.get("size", "0")

            # For One-Way mode, we are interested in positionIdx 0.
            # The 'side' from Bybit indicates the direction of the position.
            # 'size' is the quantity. If size is "0" or side is "None", there's no position for this index.
            if pos_market_id_from_api == market_id and position_idx == 0:
                ...
                if abs(size_dec) > CONFIG.position_qty_epsilon and pos_side_v5 != "None":
                    ...
                    break # Found the one-way mode position
        ...
        if active_pos_info:
            try:
                ...
                if size <= CONFIG.position_qty_epsilon or entry_price <= 0:
                    logger.warning(f"{Fore.YELLOW}Position Check: Found active V5 pos with invalid size/entry: Size={size}, Entry={entry_price}. Treating as flat.{Style.RESET_ALL}")
                    return default_pos
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    for attempt in range(CONFIG.retry_count + 1):
        try:
            # CCXT's set_leverage method for Bybit correctly handles V5 with these params.
            ...
            err_str = str(e).lower()
            # Bybit error code 110044: "Set leverage not modified"
            if "leverage not modified" in err_str or "same leverage" in err_str or "110044" in err_str:
                ...
                return True
            elif "cannot be lower than 1" in err_str and leverage < 1: # Should be caught by config validation ideally
...
def close_position(
    ...
    logger.info(f"{Fore.YELLOW}Banish Position Ritual: Initiated for {symbol}. Reason: {reason}. Initial state: {initial_side} Qty={initial_qty:.8f}{Style.RESET_ALL}")
    ...
    try:
        ...
        params = {"reduceOnly": True} # Crucial for Bybit V5 to ensure it closes a position
        ...
        if abs(filled_qty - amount_dec) < CONFIG.position_qty_epsilon:
            logger.success(f"{Fore.GREEN}{Style.BRIGHT}Banish Position: Order ({reason}) appears FILLED for {symbol}. Filled: {filled_qty:.8f}, AvgFill: {fill_price_avg:.4f}, Cost: {cost:.2f} USDT. ID:...{order_id_short}, Status: {status}{Style.RESET_ALL}")
            ...
            logger.warning(f"{Fore.YELLOW}Banish Position: Order ({reason}) status uncertain. Expected {amount_dec:.8f}, Filled: {filled_qty:.8f}. ID:...{order_id_short}, Status: {status}. Re-checking position state soon.{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name}] WARNING Banishing ({reason}): Fill uncertain (Exp:{amount_dec:.8f}, Got:{filled_qty:.8f}). ID:...{order_id_short}")
        ...
        # Other phrases: "position is zero", "position size is zero", "order would not reduce position size"
        if "order quantity exceeds open position size" in err_str or \
           "position is zero" in err_str or "position size is zero" in err_str or \
           "order would not reduce position size" in err_str or \
           "110007" in err_str or "110015" in err_str:
...
def calculate_position_size(
    ...
    if price_diff < CONFIG.position_qty_epsilon: # Avoid division by zero or tiny diff
        ...
        return None, None
    if not 0 < risk_per_trade_pct < 1: # Risk percent should be like 0.01 for 1%
        logger.error(f"{Fore.RED}Risk Calc Error: Invalid risk percentage: {risk_per_trade_pct:.4%}. Must be between 0 and 1.{Style.RESET_ALL}")
        return None, None
    if equity <= 0:
    ...
    try:
        ...
        logger.debug(f"Risk Calc: RiskAmt={risk_amount_usdt:.4f} USDT, PriceDiff={price_diff:.4f} USDT, Raw Qty={quantity_raw:.8f}")
    ...
    try:
        ...
        logger.warning(f"{Fore.YELLOW}Risk Calc Warning: Failed precision shaping for quantity {quantity_raw:.8f}. Error: {fmt_err}. Using raw value with fallback quantization.{Style.RESET_ALL}")
        try:
            ...
            quantity_precise = quantity_raw.quantize(Decimal("1e-8"), rounding=ROUND_HALF_UP) # Adjust precision as needed
    ...
    if quantity_precise <= CONFIG.position_qty_epsilon: # Check if calculated quantity is too small
        logger.warning(f"{Fore.YELLOW}Risk Calc Warning: Calculated quantity negligible ({quantity_precise:.8f}). Cannot place order.{Style.RESET_ALL}")
        return None, None
    ...
    except (DivisionByZero, Exception) as margin_err: # DivisionByZero if leverage is bad (already checked)
...
def wait_for_order_fill(
    ...
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            if status == "closed": # For market orders, 'closed' means fully filled.
...
def place_risked_market_order(
    ...
    market: dict | None = None # To store market details
    min_qty: Decimal | None = None
    max_qty: Decimal | None = None
    min_price: Decimal | None = None # For SL price validation

    try:
        # --- 1. Gather Pre-requisites (Balance, Market Info) ---
        logger.debug("Gathering resources: Balance, Market Structure, Limits...")
        balance_params = {"category": "linear"} # For USDT futures on Bybit V5
        balance = exchange.fetch_balance(params=balance_params)
        market = exchange.market(symbol)
        limits = market.get("limits", {})
        amount_limits = limits.get("amount", {})
        price_limits = limits.get("price", {}) # For price tick size and min/max price
        min_qty_str = amount_limits.get("min")
        max_qty_str = amount_limits.get("max")
        min_price_str = price_limits.get("min") # Minimum allowed price by exchange
        ...
        usdt_equity = usdt_total if usdt_total is not None and usdt_total > 0 else usdt_free # Use total if available, else free as equity

        if usdt_equity is None or usdt_equity <= Decimal("0"):
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Zero or Invalid Equity ({usdt_equity}).{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Zero/Invalid Equity ({usdt_equity:.2f})")
            return None
        if usdt_free is None or usdt_free < Decimal("0"): # Free margin can't be negative
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Invalid Free Margin ({usdt_free}).{Style.RESET_ALL}")
        ...
        if side == CONFIG.side_buy and best_ask and best_ask > 0: entry_price_estimate = best_ask
        elif side == CONFIG.side_sell and best_bid and best_bid > 0: entry_price_estimate = best_bid
        else: # Fallback to ticker if OB fails
            logger.warning(f"{Fore.YELLOW}Shallow OB failed for entry price estimate. Fetching ticker...{Style.RESET_ALL}")
            try:
                ticker = exchange.fetch_ticker(symbol)
                last_price = safe_decimal_conversion(ticker.get("last"))
                if last_price > 0 : entry_price_estimate = last_price
                else: raise ValueError("Ticker price invalid")
            except Exception as e:
                logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Failed to get entry price estimate: {e}{Style.RESET_ALL}")
                return None
        ...
        if initial_sl_price_raw <= 0: # SL price cannot be zero or negative
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Initial SL price calc invalid ({initial_sl_price_raw:.4f}).{Style.RESET_ALL}")
            return None
        if min_price is not None and initial_sl_price_raw < min_price: # Ensure SL is not below exchange minimum price
            logger.warning(f"{Fore.YELLOW}Initial SL price {initial_sl_price_raw:.4f} below min allowed {min_price:.4f}. Adjusting to min price.{Style.RESET_ALL}")
        ...
        try:
            ...
            if initial_sl_price_estimate <= 0: raise ValueError("Formatted SL estimate invalid") # Re-check after formatting
            logger.info(f"Calculated Initial SL Price (Estimate) ~ {Fore.YELLOW}{initial_sl_price_estimate:.4f}{Style.RESET_ALL}")
        except Exception as e:
        ...
        calc_qty, req_margin_est = calculate_position_size(usdt_equity, risk_percentage, entry_price_estimate, initial_sl_price_estimate, leverage, symbol, exchange)
        ...
        if pos_value_estimate > max_order_cap_usdt:
            logger.warning(f"{Fore.YELLOW}Calculated order value {pos_value_estimate:.4f} exceeds Max Cap {max_order_cap_usdt:.4f}. Capping.{Style.RESET_ALL}")
            try:
                final_quantity_capped = max_order_cap_usdt / entry_price_estimate
                final_quantity_str = format_amount(exchange, symbol, final_quantity_capped) # Format capped qty
                ...
                logger.info(f"Quantity capped to: {final_quantity:.8f}, New Est. Margin: {req_margin_est:.4f} USDT")
            except Exception as cap_err:
                logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Failed to cap quantity: {cap_err}{Style.RESET_ALL}")
                return None

        if final_quantity <= CONFIG.position_qty_epsilon: # Final check on quantity
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Final Quantity negligible ({final_quantity:.8f}).{Style.RESET_ALL}")
            return None
        if min_qty is not None and final_quantity < min_qty:
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Qty {final_quantity:.8f} < Min Qty {min_qty:.8f}.{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name}] ORDER FAIL ({side.upper()}): Qty {final_quantity:.8f} < Min {min_qty:.8f}")
            return None
        if max_qty is not None and final_quantity > max_qty: # Should ideally not happen if cap is reasonable
            logger.warning(f"{Fore.YELLOW}Qty {final_quantity:.8f} > Max Qty {max_qty:.8f}. Adjusting to max exchange limit.{Style.RESET_ALL}")
            final_quantity = max_qty # Use exchange's max
            try: # Reformat and re-calculate margin
        ...
        if usdt_free < req_margin_buffered:
            logger.error(f"{Fore.RED}Place Order Error ({side.upper()}): Insufficient FREE margin. Need ~{req_margin_buffered:.4f}, Have {usdt_free:.4f}{Style.RESET_ALL}")
            ...
            return None
        logger.info(f"{Fore.GREEN}Final Order Details Pre-Submission: Side={side.upper()}, Qty={final_quantity:.8f}, EstValue={final_quantity * entry_price_estimate:.4f}, EstMargin={req_margin_est:.4f}. Margin OK.{Style.RESET_ALL}")
        ...
        try:
            ...
            entry_order = exchange.create_market_order(symbol=symbol, side=side, amount=qty_float, params={"reduceOnly": False})
            order_id = entry_order.get("id")
            if not order_id: # Should always get an ID if order submission was accepted by API
            ...
            return None
        except Exception as e_place: # Catch any other unexpected error during placement
        ...
        if not filled_entry or filled_entry.get("status") != "closed":
            ...
            # Attempt to cancel if not filled, as a precaution
            try:
                logger.warning(f"Attempting to cancel unconfirmed order ...{format_order_id(order_id)}")
                exchange.cancel_order(order_id, symbol) # This might fail if already filled/gone
                logger.info(f"Cancel request sent for order ...{format_order_id(order_id)}")
            except Exception as cancel_e:
                logger.warning(f"Could not cancel order ...{format_order_id(order_id)}: {cancel_e}")
            return None # Do not proceed to SL/TSL if entry is uncertain
        ...
        if filled_qty <= CONFIG.position_qty_epsilon:
            logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid fill quantity ({filled_qty}) for ...{format_order_id(order_id)}. MANUAL CHECK!{Style.RESET_ALL}")
            ...
            return filled_entry # Return order, but it's a failure state
        if avg_fill_price <= 0 and cost > 0 and filled_qty > 0: # If 'average' is missing, try to derive from cost/filled
            avg_fill_price = cost / filled_qty
            logger.warning(f"{Fore.YELLOW}Fill price 'average' missing/zero. Estimated from cost/filled: {avg_fill_price:.4f}{Style.RESET_ALL}")
        if avg_fill_price <= 0:
            logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL: Invalid fill price ({avg_fill_price}) for ...{format_order_id(order_id)}. MANUAL CHECK!{Style.RESET_ALL}")
            ...
            return filled_entry # Failure state
        ...
        # Recalculate SL price based on actual average fill price
        ...
        if actual_sl_price_raw <= 0:
            logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL FAILURE: ACTUAL SL price invalid ({actual_sl_price_raw:.4f}). Cannot place SL!{Style.RESET_ALL}")
            send_sms_alert(f"[{market_base}/{CONFIG.strategy_name}] CRITICAL ({side.upper()}): Invalid ACTUAL SL price ({actual_sl_price_raw:.4f})! Emergency close attempt.")
            close_position(exchange, symbol, {"side": side, "qty": filled_qty}, reason="Invalid SL Calc Post-Entry")
            return filled_entry # Return entry order but indicate overall failure
        if min_price is not None and actual_sl_price_raw < min_price: # Check against market min price
            logger.warning(f"{Fore.YELLOW}Actual SL price {actual_sl_price_raw:.4f} below min allowed {min_price:.4f}. Adjusting to min price.{Style.RESET_ALL}")
            actual_sl_price_raw = min_price
        try:
            actual_sl_price_str = format_price(exchange, symbol, actual_sl_price_raw)
            actual_sl_price_float = float(actual_sl_price_str) # For CCXT order placement
            if actual_sl_price_float <= 0: raise ValueError("Formatted actual SL price invalid")
            logger.info(f"Calculated ACTUAL Fixed SL Trigger Price: {Fore.YELLOW}{actual_sl_price_str}{Style.RESET_ALL}")
        except Exception as e:
            logger.error(f"{Back.RED}{Fore.WHITE}CRITICAL FAILURE: Failed to format ACTUAL SL price '{actual_sl_price_raw:.4f}': {e}. Cannot place SL!{Style.RESET_ALL}")
        ...
        try:
            ...
            # Bybit V5: stopMarket order with stopPrice for fixed SL
        ...
        tsl_act_price_str = "N/A" # Activation price for TSL
        tsl_placed_successfully = False
        try:
            # Calculate TSL activation price (price must move this much in profit before TSL activates)
            ...
            if min_price is not None and act_price_raw < min_price: act_price_raw = min_price # Check against market min price
            tsl_act_price_str = format_price(exchange, symbol, act_price_raw)
            tsl_act_price_float = float(tsl_act_price_str)
            if tsl_act_price_float <= 0: raise ValueError("Formatted TSL activation price invalid")

            tsl_side = CONFIG.side_sell if side == CONFIG.side_buy else CONFIG.side_buy
            # Bybit V5 expects trailingStop as percentage string, e.g., "0.5" for 0.5%
            tsl_trail_value_str = str((tsl_percent * Decimal("100")).normalize()) # e.g., 0.005 -> "0.5"
            ...
            # Bybit V5: stopMarket order with trailingStop (percentage string) and activePrice
        ...
        if sl_placed_successfully or tsl_placed_successfully:
            ...
            # No emergency close here as position is open; user should manually manage.
            # An alternative could be to attempt an immediate close if no protection can be set.

        return filled_entry # Return the successful entry order details

    except (ccxt.InsufficientFunds, ccxt.NetworkError, ccxt.ExchangeError, ValueError, InvalidOperation) as e:
        # These are errors during the setup phase (pre-entry order placement)
    ...
    except Exception as e_overall: # Catch-all for truly unexpected issues in this function
    ...
    return None # Indicates failure at some point in the process
...
def cancel_open_orders(
    ...
    try:
        ...
        for order in open_orders:
            ...
            if order_id:
                try:
                    ...
                    cancelled_count += 1
                    time.sleep(0.15) # Small delay to avoid overwhelming API
                except ccxt.OrderNotFound: # Order already gone (filled, cancelled, etc.)
        ...
        logger.info(f"{Fore.CYAN}Order Cleanup Ritual Finished for {symbol}. Successfully Cancelled/Closed: {cancelled_count}, Failed: {failed_count}.{Style.RESET_ALL}")
        if failed_count > 0:
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> dict[str, Any]:
    ...
    except IndexError: # Should not happen if len(df) >= required_rows
        logger.error(f"Signal Gen ({strategy_name}): Error accessing DataFrame rows (len: {len(df)}).")
        return signals

    try:
        if strategy_name == "DUAL_SUPERTREND":
            primary_long_flip = last.get("st_long", False)      # True if primary ST flipped to Long this candle
            primary_short_flip = last.get("st_short", False)    # True if primary ST flipped to Short this candle
            confirm_is_up = last.get("confirm_trend", pd.NA) # True if confirmation ST is in uptrend

            if pd.isna(confirm_is_up): # Cannot make decision without confirmation trend
                logger.warning(f"Signal Gen ({strategy_name}): Confirmation trend is NA. No signal.")
                return signals

            if primary_long_flip and confirm_is_up: signals["enter_long"] = True
            if primary_short_flip and not confirm_is_up: signals["enter_short"] = True
            
            # Exit signals based on primary Supertrend flip against current position
            ...
            # Long Entry: K crosses above D, K is below oversold (e.g. < 20), and momentum is positive
            ...
            # Short Entry: K crosses below D, K is above overbought (e.g. > 80), and momentum is negative
y            if k_prev >= d_prev and k_now < d_now and k_now > CONFIG.stochrsi_overbought and mom_now < -CONFIG.position_qty_epsilon:
                signals["enter_short"] = True

            # Exit Long: K crosses below D (general bearish sign)
            if k_prev >= d_prev and k_now < d_now:
                signals["exit_long"] = True
                signals["exit_reason"] = "StochRSI K crossed below D"
            # Exit Short: K crosses above D (general bullish sign)
            if k_prev <= d_prev and k_now > d_now:
                signals["exit_short"] = True
            ...
            # Long Entry: Fisher line crosses above its signal line
            if fish_prev <= sig_prev and fish_now > sig_now: signals["enter_long"] = True
            # Short Entry: Fisher line crosses below its signal line
    ...
    except KeyError as e: # If a required indicator column is missing from df
    ...
    if signals["enter_long"] or signals["enter_short"] or signals["exit_long"] or signals["exit_short"]:
        active_signals = {k: v for k, v in signals.items() if isinstance(v, bool) and v} # Log only active signals
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    # Determine required rows for DataFrame based on configured indicator periods
    indicator_periods = [
    ...
    # StochRSI needs more than just sum of lengths due to internal RSI and smoothing
    ...
    # Ensure enough data for all indicators; add a small buffer. Min 50 rows for robustness.
    required_rows = max(max(indicator_periods) + 10 if indicator_periods else 0, stochrsi_lookback, ehlers_ssf_lookback, 50)
    ...
    try:
        ...
        # These functions modify df in place or return the modified df
        ...
        last_candle_data = df.iloc[-1] if not df.empty else pd.Series(dtype='object') # Ensure Series even if df becomes empty
        current_price = safe_decimal_conversion(last_candle_data.get("close"))

        if pd.isna(current_price) or current_price <= 0:
            logger.warning(f"{Fore.YELLOW}Trade Logic Skipped: Last candle close price invalid ({current_price}).{Style.RESET_ALL}")
            return
        ...
        if not can_place_new_order:
            ...
            # Note: Exits might still be possible if based on signals not requiring ATR.
        ...
        ob_data = None # Order book data
        if CONFIG.fetch_order_book_per_cycle: # Fetch OB every cycle if configured
        ...
        if ob_data: # If OB was fetched
            ob_ratio_color = (Fore.GREEN if bid_ask_ratio and bid_ask_ratio >= CONFIG.order_book_ratio_threshold_long
                              else (Fore.RED if bid_ask_ratio and bid_ask_ratio <= CONFIG.order_book_ratio_threshold_short
                                    else Fore.YELLOW))
        ...
        if strat_name == "DUAL_SUPERTREND":
            ...
            st_flip_str = "Long" if st_long_flip else ("Short" if st_short_flip else "None")
        ...
        logger.info(f"  Position State: Side={pos_color}{position_side}{Style.RESET_ALL}, Qty={position_qty:.8f}, Entry={position_entry:.4f}")
        ...
        if should_exit_long or should_exit_short:
            ...
            cancel_open_orders(exchange, symbol, f"Pre-Exit Cleanup ({exit_reason})") # Cancel SL/TSL orders
            time.sleep(0.75) # Brief pause for orders to clear
            ...
            if close_result:
                ...
                # Pause even on failure to allow manual intervention or next cycle re-check
                time.sleep(CONFIG.sleep_seconds // 2) 
            logger.info("Exiting trade logic cycle after processing exit signal.")
            return # End cycle after exit action
        ...
        # At this point, position_side is CONFIG.pos_none (flat)
        if not can_place_new_order: # Check if ATR is valid for new orders
        ...
        if ob_data is None and (CONFIG.order_book_ratio_threshold_long < Decimal('Infinity') or CONFIG.order_book_ratio_threshold_short > Decimal(0)): # Fetch OB only if needed and not already fetched
        ...
        else: # If OB data is required but not available, no confirmation
            if CONFIG.order_book_ratio_threshold_long < Decimal('Infinity') or CONFIG.order_book_ratio_threshold_short > Decimal(0):
                logger.warning("OB confirmation skipped: No OB data available.")
        ...
        if final_enter_long:
            ...
            action_taken_this_cycle = True
            cancel_open_orders(exchange, symbol, "Pre-Long Entry Cleanup") # Should be no orders if flat, but defensive
            time.sleep(0.5)
            place_result = place_risked_market_order(side=CONFIG.side_buy, **entry_params)
            ...
            time.sleep(0.5)
            place_result = place_risked_market_order(side=CONFIG.side_sell, **entry_params)
            ...
            if potential_entry_signal and not action_taken_this_cycle: # Signal was there but filters blocked it
...
def graceful_shutdown(exchange: ccxt.Exchange | None, symbol: str | None) -> None:
    ...
    try:
        ...
        position = get_current_position(exchange, symbol) # Re-fetch current position
        ...
        if position["side"] != CONFIG.pos_none and position["qty"] > CONFIG.position_qty_epsilon:
            ...
            logger.warning(f"{Fore.YELLOW}Shutdown Step 2: Active {pos_color}{position['side']}{Style.RESET_ALL} position (Qty: {position['qty']:.8f}). Attempting market close...{Style.RESET_ALL}")
            close_result = close_position(exchange, symbol, position, reason="Shutdown")

            if close_result: # If close order was successfully placed
                ...
                time.sleep(CONFIG.post_close_delay_seconds * 2) # Wait longer for fill during shutdown
                ...
                if final_pos["side"] == CONFIG.pos_none or final_pos["qty"] <= CONFIG.position_qty_epsilon:
                    ...
                    logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL SHUTDOWN FAILURE: FAILED TO CONFIRM closure! Final: {final_pos_color}{final_pos['side']}{Style.RESET_ALL} Qty={final_pos['qty']:.8f}. MANUAL INTERVENTION!{Style.RESET_ALL}")
                    send_sms_alert(f"[{market_base}/{CONFIG.strategy_name}] CRITICAL SHUTDOWN ERROR: Failed confirm closure! Final: {final_pos['side']} Qty={final_pos['qty']:.8f}. MANUAL CHECK!")
            else: # If close_position returned None (failed to place close order)
...
def main() -> None:
    ...
    symbol_unified: str | None = None # Store the CCXT unified symbol
    run_bot: bool = True
    cycle_count: int = 0
    ...
    try:
        ...
        # Validate symbol and set leverage
        try:
            symbol_to_use = CONFIG.symbol # Get symbol from config
            logger.info(f"Attempting to focus spell on symbol: {symbol_to_use}")
            market = exchange.market(symbol_to_use) # Load market data for the symbol
            symbol_unified = market["symbol"] # Use the CCXT unified symbol
            ...
            if not is_linear: logger.warning(f"{Fore.YELLOW}Market '{symbol_unified}' not detected as LINEAR (USDT-margined). Ensure compatibility if using other margin types.{Style.RESET_ALL}")
        ...
        market_base = symbol_unified.split("/")[0].split(":")[0] # e.g., "BTC" from "BTC/USDT:USDT"
        ...
        while run_bot:
            ...
            try:
                # Calculate data limit dynamically based on longest indicator period
                ...
                stochrsi_lookback_main = CONFIG.stochrsi_rsi_length + CONFIG.stochrsi_stoch_length + CONFIG.stochrsi_d_period + 5
                ehlers_ssf_lookback_main = max(CONFIG.ehlers_fast_period, CONFIG.ehlers_slow_period) + CONFIG.ehlers_ssf_poles + 5
                # Ensure a base minimum (e.g., 100) and add buffer
                data_limit = (max(max(indicator_periods_main) + 15 if indicator_periods_main else 0, stochrsi_lookback_main, ehlers_ssf_lookback_main, 100) + CONFIG.api_fetch_limit_buffer)

                df = get_market_data(exchange, symbol_unified, CONFIG.interval, limit=data_limit)
                if df is not None and not df.empty:
                    trade_logic(exchange, symbol_unified, df.copy()) # Pass a copy to avoid modifications affecting other parts
                else:
                    logger.warning(f"{Fore.YELLOW}Skipping trade logic: invalid/missing market data for {symbol_unified}.{Style.RESET_ALL}")
            
            # --- Handle Specific CCXT/Network Errors in Loop ---
                ...
                time.sleep(CONFIG.sleep_seconds * 6) # Extended sleep
                ...
                time.sleep(CONFIG.sleep_seconds * 2) # Moderate sleep
            except ccxt.ExchangeNotAvailable as e: # Exchange undergoing maintenance
            ...
            except ccxt.AuthenticationError as e: # API keys revoked or expired
                ...
                run_bot = False # Terminate bot
            except ccxt.ExchangeError as e: # Other general exchange errors
                ...
                time.sleep(CONFIG.sleep_seconds) # Standard sleep
    ...
    except Exception as startup_error: # Errors during initial setup before main loop
        logger.critical(f"{Back.RED}{Fore.WHITE}CRITICAL ERROR during bot startup: {startup_error}{Style.RESET_ALL}")
        logger.debug(traceback.format_exc())
        # Ensure CONFIG exists before trying to use its attributes for SMS
        ...
        run_bot = False # Prevent loop if startup fails
    finally:
