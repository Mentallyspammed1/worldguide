...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.retry_delay_seconds: int = self._get_env(
            "RETRY_DELAY_SECONDS", 1, int, color=Fore.WHITE
        )
        self.api_fetch_limit_buffer: int = self._get_env(
            "API_FETCH_LIMIT_BUFFER", 5, int, color=Fore.WHITE
        )
        ...
        self.post_close_delay_seconds: int = self._get_env(
            "POST_CLOSE_DELAY_SECONDS", 2, int, color=Fore.WHITE
        )
        self.post_entry_delay_seconds: float = float(
            self._get_env(
                "POST_ENTRY_DELAY_SECONDS", Decimal("1.0"), Decimal, color=Fore.WHITE
            )
        )
        self.fetch_order_status_retries: int = self._get_env(
            "FETCH_ORDER_STATUS_RETRIES", 5, int, color=Fore.WHITE
        )
        ...
        self.confirm_fill_lookback_seconds: int = self._get_env(
            "CONFIRM_FILL_LOOKBACK_SECONDS", 600, int, color=Fore.WHITE
        )  # Lookback for fetch_closed_orders fallback
        self.emergency_close_on_sl_fail: bool = self._get_env(
            "EMERGENCY_CLOSE_ON_SL_FAIL", True, bool, color=Fore.RED
...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        }  # Adjust if using USDC or other account types
...
def calculate_supertrend(
    ...
    try:
        ...
        if missing_pta_cols:
            raise KeyError(
                f"pandas_ta failed to create expected raw columns: {', '.join(missing_pta_cols)}"
            )
        ...
        df.drop(
            columns=[target_st_val_col, target_st_trend_col],
            inplace=True,
            errors="ignore",
        )
        df.rename(
            columns={pta_st_col_name: target_st_val_col}, inplace=True
        )  # Supertrend value
        df.rename(
            columns={pta_st_trend_col: target_st_trend_col}, inplace=True
        )  # Trend direction (-1, 1)
    ...
    try:
        ...
        if atr_col in df.columns and pd.notna(df[atr_col].iloc[-1]):
            ...
            if results[ATR_KEY] is None:
                logger.warning(
                    f"Scrying (ATR): Invalid Decimal value for ATR: {df[atr_col].iloc[-1]}."
                )
        else:
            logger.warning(
        ...
        if pd.notna(df[volume_ma_col].iloc[-1]) and pd.notna(df["volume"].iloc[-1]):
            ...
            if results[VOLUME_MA_KEY] is None or results[LAST_VOLUME_KEY] is None:
                logger.warning(
                    f"Scrying (Vol): Invalid Decimal value for Volume/MA. Vol: {df['volume'].iloc[-1]}, MA: {df[volume_ma_col].iloc[-1]}."
                )
            ...
            if (
                ...
                if results[VOLUME_RATIO_KEY] is None:
                    logger.warning(
                        f"Scrying (Vol): Invalid Decimal operation for ratio. LastVol={results[LAST_VOLUME_KEY]}, VolMA={results[VOLUME_MA_KEY]}."
                    )
            else:
        ...
        try:
            ...
            if ask_volume_sum > CONFIG.position_qty_epsilon:
                ...
                if results[BID_ASK_RATIO_KEY] is not None:
                    ...
                    logger.warning(
                        f"OB Scrying Ratio calculation failed (InvalidOp/ZeroDiv). BidSum={bid_volume_sum}, AskSum={ask_volume_sum}"
                    )
            else:
                logger.debug(
    ...
    try:
        ...
        for pos in positions:
            ...
            try:
                ...
                logger.warning(
                    f"Could not parse positionIdx '{position_idx_str}' for {market_id}. Skipping."
                )
                continue
            if position_idx != 0:
            ...
            try:
                ...
                if abs(size) > CONFIG.position_qty_epsilon:
                    ...
                    if entry_price_str is not None and entry_price_str != "":
                        with contextlib.suppress(
                            InvalidOperation, ValueError, TypeError
                        ):
                        ...
                        logger.warning(
                            f"Position Check: Entry price field (avgPrice/entryPrice) missing or empty for active position {market_id}. Defaulting to 0.0."
                        )
...
def place_risked_market_order(
    ...
    try:
        ...
        cost_limits = limits.get(COST_KEY, {})
        min_amount = (
            Decimal(str(amount_limits.get(MIN_KEY, "0")))
            if amount_limits.get(MIN_KEY) is not None
            else Decimal("0")
        )
        min_cost = (
            Decimal(str(cost_limits.get(MIN_KEY, "0")))
            if cost_limits.get(MIN_KEY) is not None
            else Decimal("0")
        )
        ...
        try:
            ...
            if CONFIG.emergency_close_on_sl_fail:
                ...
                if close_result:
                    ...
                    logger.error(
                        f"{log_prefix}: Emergency close FAILED after SL placement failure. MANUAL INTERVENTION URGENT!"
                    )
                    send_sms_alert(
            ...
            if volume_confirmed and ob_confirmed:
                ...
                try:
                    ...
                    if equity_str is None:
                        raise ValueError(
                            "Could not fetch USDT total equity from balance."
                        )
