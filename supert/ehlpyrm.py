...
# ██║        ██║   ███████║██║ ╚═╝ ██║███████║   ██║   ╚██████╔╝╚██████╔╝███████║
# ╚═╝        ╚═╝   ╚══════╝╚═╝     ╚═╝╚══════╝   ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝
# Pyrmethus - Unified Scalping Spell v2.2.2 (Fortified Configuration & Enhanced Clarity/Robustness + Ehlers SSF MA)
...
Version: 2.2.2 (Unified: Selectable Strategies + Precision + Native SL/TSL + Fortified Config + Pyrmethus Enhancements + Robustness + Ehlers SSF MA).
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.default_recv_window: int = (
            10000  # Milliseconds for API request validity (Bybit default is 5000)
        )
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Division by zero encountered calculating volume ratio (Volume MA near zero)."
                )
...
def calculate_stochrsi_momentum(
    ...
    min_len = (
        max(rsi_len + stoch_len + d, mom_len) + 5
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    try:
        ...
        if fish_col in fisher_df.columns:
            ...
            logger.warning(
                f"Ehlers Fisher column '{fish_col}' not found after calculation."
            )
        ...
        if signal_col in fisher_df.columns:
            ...
            logger.warning(
                f"Ehlers Signal column '{signal_col}' not found after calculation."
            )
...
def calculate_ehlers_ma(
    ...
    try:
        ...
        if pd.notna(fast_val) and pd.notna(slow_val):
            ...
            logger.debug(
                "Scrying (Ehlers SSF MA): Resulted in NA for one or more values on last candle."
            )
    except Exception as e:
        logger.error(
            f"{Fore.RED}Scrying (Ehlers SSF MA): Unexpected error during calculation: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        for col in target_cols:
            df[col] = pd.NA  # Nullify results on error
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        market_id = market[
            "id"
        ]  # The exchange's specific ID for the symbol (e.g., BTCUSDT)
        category = (
            "linear"
...
def set_leverage(exchange: ccxt.Exchange, symbol: str, leverage: int) -> bool:
    ...
    ):  # Add 1 to retry_count for number of attempts
        try:
            ...
            time.sleep(
                CONFIG.retry_delay_seconds * (attempt + 1)
...
def close_position(
    ...
    try:
        ...
        params = {
            "reduceOnly": True
        }  # Crucial: ensures this order only reduces/closes position
...
def place_risked_market_order(
    ...
    try:
        ...
        if min_price is not None and initial_sl_price_raw < min_price:
            ...
            initial_sl_price_raw = (
                min_price  # Adjust SL estimate upwards to meet minimum
            )
        ...
        for order in open_orders:
            ...
            if order_id:
                try:
                    ...
                    time.sleep(
                        0.15
                    )  # Small delay between cancels to avoid strict rate limits
                except ccxt.OrderNotFound:
    ...
    try:
        ...
        if strategy_name == "DUAL_SUPERTREND":
            ...
            confirm_is_up = last.get(
                "confirm_trend", pd.NA
            )  # True if confirmation ST is currently up

            if pd.isna(confirm_is_up):
                logger.warning(
                    f"Signal Gen ({strategy_name}): Confirmation trend is NA. Cannot generate signals."
                )
                return signals
        ...
        # --- Ehlers MA Cross Logic (Using Ehlers Super Smoother Filter - SSF) ---
        elif strategy_name == "EHLERS_MA_CROSS":
            fast_ma_now = last.get("ehlers_ssf_fast", pd.NA)
            slow_ma_now = last.get("ehlers_ssf_slow", pd.NA)
            fast_ma_prev = prev.get("ehlers_ssf_fast", pd.NA)
            slow_ma_prev = prev.get("ehlers_ssf_slow", pd.NA)
            ...
            if any(
                ...
                logger.debug(
                    f"Signal Gen ({strategy_name} - SSF): Skipping due to NA values (fast={fast_ma_now}, slow={slow_ma_now}, fast_prev={fast_ma_prev}, slow_prev={slow_ma_prev})"
                )
                return signals

            # Enter Long: Fast SSF MA crosses above Slow SSF MA
            if fast_ma_prev <= slow_ma_prev and fast_ma_now > slow_ma_now:
                signals["enter_long"] = True
            # Enter Short: Fast SSF MA crosses below Slow SSF MA
            if fast_ma_prev >= slow_ma_prev and fast_ma_now < slow_ma_now:
                signals["enter_short"] = True
            # Exit Long: Fast SSF MA crosses below Slow SSF MA (same condition as enter short)
            if fast_ma_prev >= slow_ma_prev and fast_ma_now < slow_ma_now:
                signals["exit_long"] = True
                signals["exit_reason"] = "Fast Ehlers SSF MA crossed below Slow SSF MA"
            # Exit Short: Fast SSF MA crosses above Slow SSF MA (same condition as enter long)
            if fast_ma_prev <= slow_ma_prev and fast_ma_now > slow_ma_now:
                signals["exit_short"] = True
                signals["exit_reason"] = "Fast Ehlers SSF MA crossed above Slow SSF MA"
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    )  # Ensure at least 50 rows minimum
    ...
    try:
        ...
        if should_exit_long or should_exit_short:
            ...
            time.sleep(
                0.75
            )  # Small pause after cancel request before placing market close
        ...
        if final_enter_long:
            ...
            if place_result:
                ...
                logger.error(
                    f"{Fore.RED}Long entry order placement process failed.{Style.RESET_ALL}"
                )
            ...
            if place_result:
                ...
                logger.error(
                    f"{Fore.RED}Short entry order placement process failed.{Style.RESET_ALL}"
                )
            ...
            if potential_entry_signal and not action_taken_this_cycle:
                logger.info(
                    "Holding Cash. Strategy signal present but confirmation filters (Volume/OB) not met."
                )
    ...
    try:
        ...
        if (
            ...
            if close_result:
                ...
                )  # Wait a bit longer to be reasonably sure it closed
...
def main() -> None:
    ...
    logger.info(
        f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}--- Pyrmethus Bybit Scalping Spell v2.2.2 Initializing ({start_time_str}) ---{Style.RESET_ALL}"
    )
    logger.info(
    ...
    try:
        ...
        try:
            ...
            is_linear = market.get(
                "linear", False
            )  # Check if it's linear (USDT margined)
        ...
        send_sms_alert(
            f"[{market_base}/{CONFIG.strategy_name}] Pyrmethus Bot v2.2.2 Initialized. Symbol: {symbol_unified}, Strat: {CONFIG.strategy_name}. Starting main loop."
        )
        ...
        while run_bot:
            ...
            # --- Robust Error Handling within the Loop ---
                ...
                ccxt.ExchangeError
            ) as e:  # Catch other specific exchange errors not handled elsewhere
                logger.error(
                ...
                )  # Standard sleep before retrying after general exchange error
            except Exception as e:
        ...
        if CONFIG and CONFIG.enable_sms_alerts and CONFIG.sms_recipient_number:
            send_sms_alert(
                f"[Pyrmethus] CRITICAL STARTUP ERROR: {type(startup_error).__name__}. Bot failed to start."
            )
        ...
        # market_base_final = symbol_unified.split("/")[0].split(":")[0] if symbol_unified else "Bot" # Redundant after shutdown SMS
