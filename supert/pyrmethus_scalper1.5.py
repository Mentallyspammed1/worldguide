...
def trade_logic(exchange: ccxt.Exchange, symbol: str, timeframe: str) -> None:
    ...
    try:
        ...
        if not isinstance(st_long_signal, (bool, pd._libs.missing.NAType)): # Handle potential pandas NA type if fillna fails
            if pd.isna(st_long_signal): st_long_signal = False # Treat NA as False
            elif not isinstance(st_long_signal, bool): # If still not boolean, raise error
                 raise ValueError(f"Supertrend long flip signal (st_long_flip) is not boolean or NA, type: {type(st_long_signal)}")
        if not isinstance(st_short_signal, (bool, pd._libs.missing.NAType)):
            if pd.isna(st_short_signal): st_short_signal = False
            elif not isinstance(st_short_signal, bool):
                 raise ValueError(f"Supertrend short flip signal (st_short_flip) is not boolean or NA, type: {type(st_short_signal)}")
