...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        )  # params={'accountType': 'CONTRACT'} might be needed
    ...
    try:
        ...
        if atr_col in df.columns and pd.notna(df[atr_col].iloc[-1]):
            try:
                ...
                logger.warning(
                    f"Scrying (ATR): Invalid Decimal value for ATR: {df[atr_col].iloc[-1]}. Error: {e}"
                )
        else:
            logger.warning(
        ...
        if pd.notna(df[volume_ma_col].iloc[-1]) and pd.notna(df["volume"].iloc[-1]):
            ...
            if (
                ...
                try:
                    ...
                    InvalidOperation,
                    ZeroDivisionError,
                ) as e:  # Catch division by zero explicitly
                    logger.warning(
            ...
            if ask_volume_sum > CONFIG.position_qty_epsilon:
                ...
                ratio_color = (
                    ...
                    if bid_ask_ratio >= CONFIG.order_book_ratio_threshold_long
                        ...
                        Fore.RED
                        if bid_ask_ratio <= CONFIG.order_book_ratio_threshold_short
                        else Fore.YELLOW
                    )
                )
                logger.debug(
                    f"OB Scrying Ratio (Depth {depth}) = {ratio_color}{bid_ask_ratio:.3f}{Style.RESET_ALL}"
                )
            else:
                logger.debug(
            ...
            try:
                ...
                if abs(size) > CONFIG.position_qty_epsilon:
                    ...
                    entry_price_str = pos_info.get(
                        AVG_PRICE_KEY
                    ) or active_position_found.get(ENTRY_PRICE_KEY)
...
def confirm_order_fill(
    ...
    if not confirmed_order:
        ...
        try:
            ...
            logger.debug(
                f"{log_prefix}: Fallback fetched recent closed orders: {[o.get('id') for o in closed_orders]}"
            )
...
def place_risked_market_order(
    ...
    try:
        ...
        cost_limits = limits.get("cost", {})
        min_amount = (
            Decimal(str(amount_limits.get("min", "0")))
    ...
    try:
        ...
        if not isinstance(
            ...
            if pd.isna(st_long_signal):
                ...
                st_long_signal, bool
            ):  # If still not boolean, raise error
                raise ValueError(
        ...
        if selected_side:
            ...
            if volume_confirmed and CONFIG.use_ob_confirm:
                ...
                if (
                    ...
                    ob_ratio = order_book_data[
                        BID_ASK_RATIO_KEY
                    ]  # Should not be None here
                    if ob_ratio is None:  # Defensive check
                        ...
                        if (
                            ...
                            ob_color = (
                                Fore.GREEN if selected_side == SIDE_BUY else Fore.RED
                            )
                            logger.info(
