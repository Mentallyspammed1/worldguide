...
API_CACHE_TTL_SECONDS = (
    30  # Reduced TTL for more responsive data, adjust based on rate limits
)
    ...
    def format(self, record):
        ...
        try:
            ...
            if len(parts) > 1:
                ...
                if level_part_idx != -1:
                    level_name_in_log = parts[0][
                        level_part_idx + 1 :
                    ].strip()  # Extract level name
...
logger_pre_config = logging.getLogger(
    "Pyrmethus.PreConfig"
)  # Specific logger for pre-config phase
...
market_data_cache = TTLCache(
    maxsize=10, ttl=API_CACHE_TTL_SECONDS
)  # Smaller cache for OHLCV as it changes often
balance_cache = TTLCache(
    maxsize=2, ttl=API_CACHE_TTL_SECONDS * 2
...
def _flush_single_notification_type(
    messages: List[Dict[str, Any]], notification_type: str, config: "Config"
) -> None:
    ...
    if notification_type == "termux":
        try:
            ...
            if process.returncode == 0:
                ...
                logger_notify.error(
                    f"Failed to send batched Termux notification. Return code: {process.returncode}. Error: {stderr.decode().strip()}"
                )
        except FileNotFoundError:
            logger_notify.error("Termux API command 'termux-notification' not found.")
...
def send_general_notification(
    ...
    if (
        ...
        or len(notification_buffer) >= NOTIFICATION_BATCH_MAX_SIZE * 2
    ):  # *2 to account for dual entries
        flush_notifications()
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.trading = {
            "risk_per_trade_percentage": self._get_env(
                "RISK_PER_TRADE_PERCENTAGE", "0.002", cast_type=Decimal
            ),  # Example: 0.2%
            "max_order_usdt_amount": self._get_env(
                "MAX_ORDER_USDT_AMOUNT", "25.0", cast_type=Decimal
        ...
        self.risk_management = {
            ...
            "max_drawdown_percent": self._get_env(
                "MAX_DRAWDOWN_PERCENT", "0.02", cast_type=Decimal
            ),  # Example: 2% daily drawdown
            ...
            "session_profit_target_usdt": self._get_env(
                "SESSION_PROFIT_TARGET_USDT", "2.0", cast_type=Decimal, required=False
            ),  # Optional
            "session_max_loss_usdt": self._get_env(
                "SESSION_MAX_LOSS_USDT", "10.0", cast_type=Decimal, required=False
            ),  # Optional
        }
    ...
    def _get_env(
        ...
        if value_str is None:
            if required:
                self.logger.critical(
                    f"CRITICAL: Required configuration rune '{key}' not found in environment. Pyrmethus cannot proceed."
                )
        ...
        if value_to_cast is None:  # Can happen if default is None
            if (
                required
            ):  # Should have been caught above if value_str was None and required
                self.logger.critical(
                    f"CRITICAL: Required configuration rune '{key}' resolved to None after environment/default check."
                )
                raise ValueError(
        ...
        try:
            ...
            self.logger.error(
                f"Invalid type/value for '{key}': '{value_to_cast}' (Source: {source}). Expected {cast_type.__name__}. Err: {e}. Using Default: '{default}'."
            )
            if default is None:
                if required:
                    self.logger.critical(
                    ...
                    self.logger.warning(
                        f"Cast fail for optional '{key}', default is None. Final: None"
                    )
                    return None
                ...
                try:
                    ...
                    if cast_type == bool:
                        ...
                        final_value = default_str_for_cast  # Fallback for unsupported
                    self.logger.warning(
                        f"Used casted default for {key}: '{final_value}'"
                    )
                except (ValueError, TypeError, InvalidOperation) as e_default:
                    self.logger.critical(
                        f"CRITICAL: Cast fail for value AND default for '{key}'. Original: '{value_to_cast}', Default: '{default}'. Err: {e_default}"
                    )
                    raise ValueError(
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.enhancements["profit_momentum_window"] < 1:
            errors.append("PROFIT_MOMENTUM_WINDOW must be >= 1.")
        if self.enhancements["whipsaw_max_trades_in_period"] < 1:
            errors.append("WHIPSAW_MAX_TRADES_IN_PERIOD must be >= 1.")
        if self.enhancements["signal_persistence_candles"] < 1:
            errors.append("SIGNAL_PERSISTENCE_CANDLES must be >= 1.")
        if (
            self.strategy_params.get("st_max_entry_distance_atr_multiplier") is not None
            and self.strategy_params["st_max_entry_distance_atr_multiplier"] < 0
        ):
            errors.append("ST_MAX_ENTRY_DISTANCE_ATR_MULTIPLIER cannot be negative.")
        if self.enhancements["anti_martingale_risk_enable"]:
            if not (0 < self.enhancements["risk_reduction_factor_on_loss"] <= 1):
                errors.append(
                    "RISK_REDUCTION_FACTOR_ON_LOSS must be between 0 (exclusive) and 1 (inclusive)."
                )
            if self.enhancements["risk_increase_factor_on_win"] < 1:
                errors.append("RISK_INCREASE_FACTOR_ON_WIN must be >= 1.")
            if not (0 < self.enhancements["max_risk_pct_anti_martingale"] < 1):
...
try:
    ...
    print(
        f"CRITICAL: Configuration loading failed. Error: {config_error}",
        file=sys.stderr,
    )
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    ...
    async def generate_signals(
        ...
        if primary_long_flip and primary_short_flip:
            self.logger.warning(
                "Conflicting primary Supertrend flips on the same candle. Attempting to resolve..."
            )
            if confirm_is_up_current is True and (
                ...
                self.logger.info(
                    "Resolution: Prioritizing LONG flip (Confirm ST Up & Positive Momentum)."
                )
                ...
                self.logger.info(
                    "Resolution: Prioritizing SHORT flip (Confirm ST Down & Negative Momentum)."
                )
            else:
                primary_long_flip = False
                primary_short_flip = False
                self.logger.warning(
                    "Resolution: Conflicting flips ambiguous. No primary signal."
                )
        ...
        if self.config.strategy_params["confirm_st_stability_lookback"] <= 1:
            ...
            if confirm_is_up_current is True and all(
                ...
                stable_confirm_trend = pd.NA  # Mixed or not enough matching

        if pd.isna(stable_confirm_trend) or pd.isna(momentum_val):
            self.logger.debug(
                f"Stable Confirm ST ({_format_for_log(stable_confirm_trend, True)}) or Mom ({_format_for_log(momentum_val)}) is NA. No signal."
            )
            return signals
        ...
        if (
            ...
            if primary_long_flip:
                ...
                if (
                    ...
                    self.logger.debug(
                        f"Long ST proximity fail: Price {latest_close} vs ST Line {st_line_val}, Dist > {max_allowed_distance}"
                    )
                ...
                if (
                    ...
                    price_proximity_ok = False
                    self.logger.debug(
                        f"Short ST proximity fail: Price {latest_close} vs ST Line {st_line_val}, Dist > {max_allowed_distance}"
                    )
        ...
        if (
        ...
        ):  # Added momentum_val > 0
            signals["enter_long"] = True
            self.logger.info(
                "DualST+Mom Signal: LONG Entry - ST Flip, Stable Confirm Up, Positive Mom, Prox OK."
            )
        elif (
            primary_short_flip
            ...
            self.logger.info(
                "DualST+Mom Signal: SHORT Entry - ST Flip, Stable Confirm Down, Negative Mom, Prox OK."
            )
    ...
    async def generate_signals(
        ...
        if not is_fisher_extreme:
            if fisher_prev <= signal_prev and fisher_now > signal_now:
                signals["enter_long"] = True
                self.logger.info(
                    f"EhlersFisher Signal: LONG Entry - Fisher ({_format_for_log(fisher_now)}) crossed ABOVE Signal ({_format_for_log(signal_now)})."
                )
            elif fisher_prev >= signal_prev and fisher_now < signal_now:
                signals["enter_short"] = True
                self.logger.info(
                    f"EhlersFisher Signal: SHORT Entry - Fisher ({_format_for_log(fisher_now)}) crossed BELOW Signal ({_format_for_log(signal_now)})."
                )
        elif (fisher_prev <= signal_prev and fisher_now > signal_now) or (
            fisher_prev >= signal_prev and fisher_now < signal_now
        ):  # Log if crossover ignored
            self.logger.info(
                f"EhlersFisher: Crossover signal ignored due to Fisher in extreme zone ({_format_for_log(fisher_now)})."
            )
...
class TradeMetrics:
    ...
    def set_initial_equity(self, equity: Decimal):
        ...
        if self.last_daily_reset_day != today or self.daily_start_equity is None:
            ...
            self.logger.info(
                f"Daily Equity Ward reset. Dawn Equity: {equity:.2f} {self.config.usdt_symbol}"
            )
        ...
        if self.last_daily_trade_count_reset_day != today:
            ...
            if (
                ...
                self.daily_trades_rest_active_until = (
                    0.0  # Reset cooldown if it was for previous day
                )
    ...
    def log_trade(
        ...
        if not all(
            ...
            self.logger.warning(
                f"Trade log skipped due to flawed parameters for Part ID: {part_id}."
            )
            return

        profit = safe_decimal_conversion(pnl_str, Decimal(0))
        if profit is not pd.NA and profit <= 0:
...
async def main():
    ...
    try:
        ...
        if CONFIG.exchange["paper_trading"]:
            sync_exchange_params["urls"] = {
                "api": ccxt.bybit.urls["api"]["test"]
            }  # Use ccxt's predefined testnet URL
        ...
        logger_main.info(
            f"Connecting to {sync_exchange_instance.id} (CCXT: {ccxt.__version__})"
        )

        markets = (
            await async_exchange_instance.load_markets()
        )  # Use async for loading markets
        ...
        try:
            ...
            logger_main.success(
                f"Leverage for {CONFIG.exchange['symbol']} set/confirmed."
            )
        except Exception as e_lev:
            logger_main.warning(
                f"Could not set leverage (may be pre-set or issue): {e_lev}",
        ...
        logger_main.success(
            f"Connected to {sync_exchange_instance.id} for {CONFIG.exchange['symbol']}."
        )
        send_general_notification(
            "Pyrmethus Online",
            f"Connected to {sync_exchange_instance.id} for {CONFIG.exchange['symbol']} @ {CONFIG.exchange['leverage']}x.",
        )

    except ccxt.AuthenticationError as e_auth:
        logger_main.critical(
            f"Authentication failed! Check API keys: {e_auth}", exc_info=True
        )
        ...
        logger_main.critical(
            f"Network Error: {e_net}. Check connection/DNS.", exc_info=True
        )
        send_general_notification(
        ...
        logger_main.critical(
            f"Exchange API Error: {e_exch}. Check API/symbol/status.", exc_info=True
        )
        send_general_notification(
        ...
        logger_main.critical(
            f"Failed to initialize exchange/spell pre-requisites: {e_general}",
            exc_info=True,
        )
        send_general_notification(
    ...
    logger_main.warning(
        "Pyrmethus: Full async trading loop not implemented in this snippet. This is a structural example."
    )
    ...
    if (
        sync_exchange_instance
    ):  # Though not strictly necessary to close sync if only used for setup
        try:
            # Some exchanges might not have a close method on sync objects or it might be a no-op
            if hasattr(sync_exchange_instance, "close") and callable(
                sync_exchange_instance.close
            ):
            ...
            logger_main.warning(
                f"Minor issue closing synchronous exchange object: {e_close_sync}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(
            f"An unhandled astral disturbance occurred: {e_top_level}", exc_info=True
        )
        if (
            "CONFIG" in globals()
            and CONFIG
            ...
            send_general_notification(
                "Pyrmethus CRASH", f"Fatal error: {str(e_top_level)[:100]}"
            )
            flush_notifications()
    finally:
