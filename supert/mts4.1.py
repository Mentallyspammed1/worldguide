...
class TradeMetrics:
    ...
    def log_trade(self, symbol: str, side: str, entry_price: Decimal, exit_price: Decimal, qty: Decimal,
                  ...
                  # Keep optional params from v2.8.1
        ...
        profit = safe_decimal_conversion(
            pnl_str, Decimal(0))  # Use provided PNL string

        entry_dt_iso = datetime.fromtimestamp(
            entry_time_ms / 1000, tz=pytz.utc).isoformat()
        exit_dt_iso = datetime.fromtimestamp(
            exit_time_ms / 1000, tz=pytz.utc).isoformat()
        duration_seconds = (datetime.fromisoformat(exit_dt_iso.replace(
            "Z", "+00:00")) - datetime.fromisoformat(entry_dt_iso.replace("Z", "+00:00"))).total_seconds()
    ...
    def summary(self) -> str:
        ...
        if self.initial_equity is not None:
            ...
            overall_pnl_pct = (total_profit / self.initial_equity) * \
                100 if self.initial_equity > 0 else Decimal(0)
...
def save_persistent_state(force_heartbeat: bool = False) -> None:
    ...
    now = time.time()
    if not (force_heartbeat or (now - _last_heartbeat_save_time > HEARTBEAT_INTERVAL_SECONDS)):
        return
    ...
    try:
        ...
        logger.log(
            log_level, f"{NEON['SUCCESS']}Phoenix Feather: State memories scribed to '{STATE_FILE_NAME}'.{NEON['RESET']}")
    except Exception as e:
        logger.error(
            ...
            for k, v_str_or_val in restored.items():
                if k in decimal_keys and isinstance(v_str_or_val, str):
                    ...
                    if isinstance(v_str_or_val, str):
                        try:
                            restored[k] = int(datetime.fromisoformat(
                                v_str_or_val.replace("Z", "+00:00")).timestamp() * 1000)
                        except ValueError:
                            try:
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    if config.strategy_name == StrategyName.DUAL_SUPERTREND_MOMENTUM:
        ...
        st_c = f"CONFIRM_ST_{config.confirm_st_atr_length}_{config.confirm_st_multiplier}"
        df.ta.supertrend(length=config.st_atr_length, multiplier=float(
            config.st_multiplier), append=True, col_names=(st_p, f"{st_p}d", f"{st_p}l", f"{st_p}s"))
        df.ta.supertrend(length=config.confirm_st_atr_length, multiplier=float(
            config.confirm_st_multiplier), append=True, col_names=(st_c, f"{st_c}d", f"{st_c}l", f"{st_c}s"))
        ...
        if 'close' in df.columns and not df['close'].isnull().all():
            ...
            df["momentum"] = pd.NA
            logger.warning(
                "Cannot calculate momentum, 'close' missing/all NaNs.")
        logger.debug(
...
def fetch_account_balance(exchange: ccxt.Exchange, currency_code: str = CONFIG.usdt_symbol) -> Optional[Decimal]:
    ...
    try:
        ...
        total_balance = safe_decimal_conversion(balance_data.get(
            'total', {}).get(currency_code))  # Try this first
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    current_side = _active_trade_parts[0]['side'] if total_qty > CONFIG.position_qty_epsilon else CONFIG.pos_none
...
def calculate_position_size(balance: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, sl_price: Decimal, market_info: Dict) -> Optional[Decimal]:
    ...
    min_qty = safe_decimal_conversion(market_info.get(
        'limits', {}).get('amount', {}).get('min'))
    qty_precision_digits = market_info.get('precision', {}).get(
        'amount')  # Number of decimal places for quantity
    ...
    if qty_precision_digits is None:  # Fallback if precision not found
        ...
        if qty_precision_digits >= 0:  # If 0, means integer quantity. If >0, decimal places.
            # e.g., 3 -> Decimal('0.001')
            quantizer = Decimal('1e-' + str(qty_precision_digits))
            quantity_base = (quantity_base // quantizer) * \
...
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price_target: Decimal, sl_price: Decimal, atr_val: Optional[Decimal]) -> Optional[Dict]:
    ...
    params = {
        # Good-Til-Cancelled for limit orders, often ignored for market.
        'timeInForce': 'GTC',
        # Set SL price directly with the order (Bybit V5 feature)
        'stopLoss': float(sl_price),
        # Or 'LastPrice', 'IndexPrice'. MarkPrice is common for SL.
        'slTriggerBy': 'MarkPrice',
        # 'takeProfit': float(tp_price), # Optional TP can also be set here
        # For Hedge Mode: 0 for one-way. For Bybit V5, 1=Buy, 2=Sell under hedge. Assume one-way for now.
        'positionIdx': 0
    ...
    try:
        ...
        order = exchange.create_order(config.symbol, order_type_str, side, float(
        ...
        config.send_notification_method(
            f"Pyrmethus Order Placed: {side.upper()}", f"{config.symbol} Qty: {quantity} @ {entry_price_target if order_type_str == 'Limit' else 'Market'}")
        ...
        if filled_order.get('status') == 'closed':
            ...
            new_part = {
                ...
                # SL is part of entry order with Bybit V5, not a separate order ID here
                "sl_order_id": None,
            ...
            config.send_notification_method(
                f"Pyrmethus Order Filled: {side.upper()}", f"{config.symbol} Part {part_id} @ {actual_entry_price}")
            return new_part
        else:
            logger.warning(
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part_to_close: Dict, reason: str, close_price_target: Optional[Decimal] = None) -> bool:
    ...
    try:
        ...
        if filled_close_order.get('status') == 'closed':  # Filled
            ...
            pnl_per_unit = (actual_exit_price - part_to_close['entry_price']) if part_to_close['side'] == config.pos_long else (
                part_to_close['entry_price'] - actual_exit_price)
            ...
            trade_metrics.log_trade(
                ...
                reason=reason, part_id=part_to_close['part_id'], pnl_str=str(
                    pnl)  # Pass calculated PNL as string
            )

            # Remove the closed part from active trades
            _active_trade_parts = [
                p for p in _active_trade_parts if p['part_id'] != part_to_close['part_id']]
            ...
            config.send_notification_method(
                f"Pyrmethus Position Closed", f"{config.symbol} Part {part_to_close['part_id']} closed. PNL: {pnl:.2f}")
            return True
        else:
            logger.warning(
        ...
        logger.debug(traceback.format_exc())
        config.send_notification_method(
            "Pyrmethus Close Fail", f"Error closing {config.symbol} part {part_to_close['part_id']}: {str(e)[:80]}")
        return False
...
def cancel_all_symbol_orders(exchange: ccxt.Exchange, symbol: str):
    ...
    try:
        ...
        # Simpler call, ccxt might handle category
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    for part in parts_to_close_copy:
        if part['symbol'] == config.symbol:
            logger.info(
                f"Unraveling internally tracked part: {part['part_id']}")
    ...
    try:
        ...
        for pos_data in positions:
            # 'contracts' or 'size'
            if pos_data and safe_decimal_conversion(pos_data.get('contracts', '0')) != Decimal(0):
                ...
                if pos_symbol == config.symbol and pos_qty > CONFIG.position_qty_epsilon:
                    ...
                    exchange.create_order(config.symbol, 'market', side_to_close_market, float(
                        pos_qty), params={'reduceOnly': True})
                    logger.info(
    ...
    while True:
        try:
            ...
            current_balance = fetch_account_balance(
                exchange, config.usdt_symbol)  # Update balance each cycle
            if current_balance is not None:
            ...
            latest_atr = safe_decimal_conversion(
                df_with_indicators[atr_col].iloc[-1]) if atr_col in df_with_indicators.columns and not df_with_indicators.empty else None
            latest_close = safe_decimal_conversion(
                df_with_indicators['close'].iloc[-1]) if 'close' in df_with_indicators.columns and not df_with_indicators.empty else None
                ...
                if signals.get("exit_long"):
                    ...
                    for part in list(_active_trade_parts):
                        if part['side'] == config.pos_long:
                            close_position_part(exchange, config, part, signals.get(
            ...
            # Longer pause after major error
            time.sleep(config.sleep_seconds * 5)

    finally:
        logger.info(
            f"{NEON['HEADING']}=== Pyrmethus Spell Concludes ==={NEON['RESET']}")
        if exchange:  # Ensure exchange object exists before trying to use it
            close_all_symbol_positions(
                exchange, config, "Spell Ending Sequence")
...
if __name__ == "__main__":
    ...
    try:
        exchange_params = {
            ...
            'options': {
                'defaultType': 'swap',  # Assuming futures/swaps
        ...
        price_prec = CONFIG.MARKET_INFO.get(
            'precision', {}).get('price', 'N/A')
        amount_prec = CONFIG.MARKET_INFO.get(
            'precision', {}).get('amount', 'N/A')
        logger.success(
        ...
        try:
            ...
            category = "linear" if CONFIG.symbol.endswith(
                "USDT") else "inverse"
            ...
            if hasattr(exchange, 'set_margin_mode') and CONFIG.MARKET_INFO.get('margin', False):
        ...
        CONFIG.send_notification_method(
            "Pyrmethus Auth Fail", "API Key Auth Error.")
        sys.exit(1)
