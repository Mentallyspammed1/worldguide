...
_active_trade_parts: List[
    Dict[str, Any]
] = []  # Pyrmethus: If MAX_ACTIVE_TRADE_PARTS > 1, this list will grow.
...
def load_persistent_state() -> bool:
    ...
    try:
        ...
        if (
            saved_version != PYRMETHUS_VERSION
        ):  # Simple version check, could be more sophisticated
            logger.warning(
                f"{NEON['WARNING']}Phoenix Feather: Scroll version '{saved_version}' differs from current '{PYRMETHUS_VERSION}'. Caution during reanimation.{NEON['RESET']}"
            )
            ...
            for k, v_loaded in part_data_str_values.items():
                if k in decimal_keys_in_part:
                    restored_part[k] = safe_decimal_conversion(
                        v_loaded, None
                    )  # Allow None for optional fields like tp_price
                    if (
                        restored_part[k] is None
                        and v_loaded is not None
                        and v_loaded != "None"
                    ):  # Check if conversion failed for non-None string
                        logger.warning(
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    if len(_active_trade_parts) > 1 and CONFIG.max_active_trade_parts == 1:
        logger.warning(
            f"{NEON['WARNING']}More than one active part detected ({len(_active_trade_parts)}) while MAX_ACTIVE_TRADE_PARTS is 1. State inconsistency? Prioritizing first part.{NEON['RESET']}"
        )
    ...
    if current_side_str not in [CONFIG.pos_long, CONFIG.pos_short]:
        ...
        _active_trade_parts.clear()
        save_persistent_state(force_heartbeat=True)
        return CONFIG.pos_none, total_qty  # total_qty here is actually 0 due to clear
...
def calculate_position_size(
    ...
    if quantity_base <= CONFIG.position_qty_epsilon:
        logger.warning(
            f"{NEON['WARNING']}Calculated qty {quantity_base} is zero/negligible. No order.{NEON['RESET']}"
        )
        return None
    if min_qty is not None and quantity_base < min_qty:
        logger.warning(
            f"{NEON['WARNING']}Calculated qty {quantity_base} < exchange min {min_qty}. No order.{NEON['RESET']}"
        )
        return None

    qty_display_precision = (
        abs(qty_step.as_tuple().exponent) if qty_step else 8
    )  # Default precision
    logger.info(
...
def place_risked_order(
    ...
    if balance is None or balance <= Decimal(10):
        logger.error(
            f"{NEON['ERROR']}Insufficient treasury ({balance}). Cannot order.{NEON['RESET']}"
        )
        return None
    if (
    ...
    if order_type_str == "Limit":
        ...
        )  # Ensure price conforms to tick size
    ...
    params = {
        "timeInForce": "GTC"
        if order_type_str == "Limit"
        else "IOC",  # IOC for Market, GTC for Limit
        "positionIdx": 0,
    ...
    try:
        ...
        if filled_order.get("status") == "closed" or (
        ...
        ):  # Market orders might not be 'closed' immediately but filled
            ...
            new_part = {
                ...
                else None,  # This is the TP set on the exchange
...
def close_position_part(
    ...
    logger.info(
        f"{NEON['ACTION']}Unraveling part {part_id} ({part_side} {NEON['QTY']}{part_qty}{NEON['RESET']}) for: {reason}{NEON['RESET']}"
    )
    if not isinstance(part_qty, Decimal) or part_qty <= CONFIG.position_qty_epsilon:
        logger.warning(
            f"{NEON['WARNING']}Cannot unravel part {part_id}: qty {part_qty} invalid.{NEON['RESET']}"
        )
        _active_trade_parts = [
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    if parts_to_close_copy:
        ...
        for (
            part
        ) in parts_to_close_copy:  # This will iterate and call close_position_part
        ...
        for pos_data in positions:
            if pos_data and pos_data.get("symbol") == config.symbol:
                ...
                if pos_qty is not None and pos_qty > CONFIG.position_qty_epsilon:
                    ...
                    )  # Give time for order to process
    ...
    try:
        logger.info(
            f"Attempting to clear SL/TP for position {config.symbol} on exchange."
        )
        params_clear_sltp = {
            "positionIdx": 0,
            "stopLoss": "0",
            "takeProfit": "0",
        }  # Setting to 0 typically cancels
        if config.symbol.endswith(":USDT"):
...
def modify_position_sl_tp(
    ...
    if "stopLoss" not in params_sl_tp:
        params_sl_tp["stopLoss"] = "0"
    if "takeProfit" not in params_sl_tp and config.enable_take_profit:
        params_sl_tp["takeProfit"] = "0"
    if (
        not config.enable_take_profit and "takeProfit" in params_sl_tp
    ):  # Ensure TP is not set if disabled
        params_sl_tp["takeProfit"] = "0"

    if not params_sl_tp.get("stopLoss") and not params_sl_tp.get(
        "takeProfit"
    ):  # Avoid API call if nothing to set
        logger.debug(
            f"No valid SL ({params_sl_tp.get('stopLoss')}) or TP ({params_sl_tp.get('takeProfit')}) to modify for part {part_id}."
        )
        return False  # Nothing to do

    logger.info(
        f"{NEON['ACTION']}Modifying SL/TP for part {part_id}: New SL {sl_to_set}, New TP {tp_to_set}{NEON['RESET']}"
    )
    try:
        exchange.set_trading_stop(config.symbol, params=params_sl_tp)
        logger.success(
            f"{NEON['SUCCESS']}SL/TP modification successful for part {part_id}. Exchange SL: {sl_to_set}, TP: {tp_to_set}{NEON['RESET']}"
        )  # type: ignore [attr-defined]

        # Update part state
        if new_sl is not None:
            part["sl_price"] = sl_to_set
        if new_tp is not None and config.enable_take_profit:
            part["tp_price"] = tp_to_set
        elif not config.enable_take_profit:
            part["tp_price"] = None  # Clear TP if disabled
        ...
        if "Stop loss price is not valid" in str(
            ...
            logger.warning(
                f"SL/TP rejected for part {part_id} (likely too close to market or invalid). Original SL/TP may remain."
            )
    ...
    try:  # Outer try for the main while loop, associated with the finally block
        while True:
            ...
            try:  # Inner try for a single iteration of the loop
                ...
                for part in (
                    active_parts_copy
                ):  # Should only be one if MAX_ACTIVE_TRADE_PARTS = 1
                    if part.get("symbol") != config.symbol:
                        continue
                    ...
                    if (
                        ...
                        time_since_entry_ms = (time.time() * 1000) - part[
                            "entry_time_ms"
                        ]
                        check_window_ms = (
                        ...
                        if (
                            ...
                            if (
                                ...
                                logger.warning(
                                    f"{NEON['WARNING']}Trend contradiction for part {part['part_id']} shortly after entry. Cooldown activated.{NEON['RESET']}"
                                )
                    ...
                    if part["side"] == config.pos_long:
                        current_unrealized_pnl = (
                            latest_close - part["entry_price"]
                        ) * part["qty"]
                    elif part["side"] == config.pos_short:
                        current_unrealized_pnl = (
                            part["entry_price"] - latest_close
                        ) * part["qty"]
                    ...
                    if (
                        ...
                        if (
                            ...
                            )  # Use ATR at entry for consistency
                            tightened_sl_dist = (
                            ...
                            if (
                                ...
                                logger.info(
                                    f"{NEON['ACTION']}Profit Momentum: Attempting SL tighten for part {part['part_id']} to {new_sl_price_candidate:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}{NEON['RESET']}"
                                )
                    ...
                    if (
                        ...
                        current_profit_atr = (
                            (current_unrealized_pnl / part["qty"]) / latest_atr
                            if part["qty"] > 0 and latest_atr > 0
                            else Decimal(0)
                        )
                        atr_target_met = (
                        ...
                        if atr_target_met and abs_pnl_target_met:
                            ...
                            if (
                                ...
                                logger.info(
                                    f"{NEON['ACTION']}Breakeven SL: Attempting for part {part['part_id']} to {new_sl_price_candidate}{NEON['RESET']}"
                                )
                    ...
                    if (
                        ...
                        if is_cons_adverse:
                            ...
                            if dist_to_sl <= sl_prox_thresh_price:
                                logger.warning(
                                    f"{NEON['WARNING']}Last Chance Exit: Part {part['part_id']} ({dist_to_sl:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}} <= {sl_prox_thresh_price:.{config.MARKET_INFO['precision']['price'] if config.MARKET_INFO else 2}}).{NEON['RESET']}"
                                )
                    ...
                    if (
                        ...
                        if fisher_now is not pd.NA and signal_now is not pd.NA:
                            ...
                            entry_signal = part.get("entry_signal_value", pd.NA)
                            if entry_fisher is not pd.NA and entry_signal is not pd.NA:
                                ...
                                if divergence_met:
                                    ...
                                    if qty_to_close > config.position_qty_epsilon:
                                        logger.info(
                                            f"{NEON['ACTION']}Ehlers Divergence Exit ({config.ehlers_divergence_exit_percentage * 100}%): Part {part['part_id']}, Qty {qty_to_close}{NEON['RESET']}"
                                        )
                                        temp_part_to_close = (
                                            part.copy()
                                        )  # Create a conceptual part for closing
                                        ...
                                        if close_position_part(
                                            ...
                                            part["divergence_exit_taken"] = (
                                                True  # Mark that this specific type of partial exit was taken
                                            )
                                            if (
                                                part["qty"]
                                                ...
                                                logger.info(
                                                    f"Part {part['part_id']} fully closed due to negligible remainder after partial Ehlers div exit."
                                                )
                                            save_persistent_state(force_heartbeat=True)
                                            continue
                                    else:
                    ...
                    if (
                        part["side"] == config.pos_long and signals.get("exit_long")
                    ) or (
                        part["side"] == config.pos_short and signals.get("exit_short")
                    ):
                        exit_reason = signals.get("exit_reason", "Strategy Exit Signal")
                        logger.info(
                            f"{NEON['ACTION']}Omen to EXIT {part['side']} for {config.symbol}: {exit_reason}{NEON['RESET']}"
                        )
                    ...
                    if part.get("sl_price") is not None:
                        if (
                            part["side"] == config.pos_long
                            and latest_close <= part["sl_price"]
                        ) or (
                            part["side"] == config.pos_short
                            and latest_close >= part["sl_price"]
                        ):
                            logger.warning(
                                f"{NEON['WARNING']}Part {part['part_id']} SL price {part['sl_price']} potentially breached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}"
                            )
                    ...
                    if part.get("tp_price") is not None and config.enable_take_profit:
                        if (
                            part["side"] == config.pos_long
                            and latest_close >= part["tp_price"]
                        ) or (
                            part["side"] == config.pos_short
                            and latest_close <= part["tp_price"]
                        ):
                            logger.info(
                                f"{NEON['INFO']}Part {part['part_id']} TP price {part['tp_price']} potentially reached by close {latest_close}. Exchange should handle closure.{NEON['RESET']}"
                            )
