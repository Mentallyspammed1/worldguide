...
if load_dotenv(dotenv_path=env_path):
    ...
    logging.getLogger("PreConfig").warning(
        f"{NEON['WARNING']}No .env scroll found. Relying on system environment variables or defaults.{NEON['RESET']}"
    )
...
def _format_for_log(value: Any, is_bool_trend: bool = False, precision: int = 4) -> str:
    ...
    if is_bool_trend:
        return (
            "Up" if value is True else ("Down" if value is False else "Indeterminate")
        )
...
def send_termux_notification(
    ...
    try:
        ...
        if "CONFIG" in globals() and CONFIG is not None:
            ...
            logging.getLogger("TermuxNotify").debug(
                "CONFIG object not available for send_termux_notification. Using defaults for enabled/timeout."
            )
    except Exception:  # Catch any issue accessing CONFIG, though 'in globals()' should prevent NameError
        logging.getLogger("TermuxNotify").debug(
            "Error accessing CONFIG in send_termux_notification. Using defaults."
        )

    if not notifications_enabled:
        logging.getLogger("TermuxNotify").debug(
            "Termux notifications are disabled by configuration (or CONFIG not found)."
        )
        return
...
class Config:
    ...
    def _get_env(
        ...
        if value_str is None:
            if required:
                _logger.critical(
                    f"{NEON['CRITICAL']}CRITICAL: Required config rune '{key}' not found.{NEON['RESET']}"
                )
                raise ValueError(f"Required environment variable '{key}' not set.")
            _logger.debug(
                f"{color}Config Rune {NEON['VALUE']}'{key}'{color}: Not Found. Default: '{NEON['VALUE']}{default}{color}'{NEON['RESET']}"
            )
            value_to_cast = default
            source = "Default"
        else:
            _logger.debug(
                f"{color}Config Rune {NEON['VALUE']}'{key}'{color}: Env Value: '{NEON['VALUE']}{display_value}{color}'{NEON['RESET']}"
            )
        ...
        try:
            ...
            if cast_type == bool:
                ...
                _logger.warning(
                    f"{NEON['WARNING']}Unsupported cast_type '{cast_type.__name__}' for key '{key}'. Raw: '{raw_value_str_for_cast}'.{NEON['RESET']}"
                )
                final_value = raw_value_str_for_cast
        except (ValueError, TypeError, InvalidOperation) as e:
            _logger.error(
                f"{NEON['ERROR']}Cast error for '{key}': '{value_to_cast}' (Source: {source}). Expected {cast_type.__name__}. Err: {e}. Default: '{default}'.{NEON['RESET']}"
            )
            if default is None:
                if required:
                    _logger.critical(
                        f"{NEON['CRITICAL']}CRITICAL: Cast fail for required '{key}', default is None.{NEON['RESET']}"
                    )
                    raise ValueError(
    ...
    def _validate_parameters(self) -> None:
        ...
        if (
            ...
            errors.append(
                "Strategy periods (ST_ATR_LENGTH, CONFIRM_ST_ATR_LENGTH, MOMENTUM_PERIOD) must be positive."
            )
        ...
        if self.enable_dynamic_risk:
            if not (0 < self.dynamic_risk_min_pct < 1):
                errors.append(
                    f"DYNAMIC_RISK_MIN_PCT ({self.dynamic_risk_min_pct}) must be between 0 and 1."
                )
            if not (0 < self.dynamic_risk_max_pct < 1):
                errors.append(
                    f"DYNAMIC_RISK_MAX_PCT ({self.dynamic_risk_max_pct}) must be between 0 and 1."
                )
            if self.dynamic_risk_min_pct >= self.dynamic_risk_max_pct:
                errors.append(
                    f"DYNAMIC_RISK_MIN_PCT ({self.dynamic_risk_min_pct}) must be less than DYNAMIC_RISK_MAX_PCT ({self.dynamic_risk_max_pct})."
                )

        if self.enable_max_drawdown_stop and not (0 < self.max_drawdown_percent <= 1):
            errors.append(
                f"MAX_DRAWDOWN_PERCENT ({self.max_drawdown_percent}) must be between 0 (exclusive) and 1 (inclusive)."
            )

        if self.enable_dynamic_atr_sl and (
            self.atr_sl_multiplier_low_vol <= 0
            or self.atr_sl_multiplier_normal_vol <= 0
            or self.atr_sl_multiplier_high_vol <= 0
        ):
            errors.append(
                "Dynamic ATR SL multipliers (low, normal, high vol) must be positive."
            )
        ...
        if self.sleep_seconds <= 0:
            errors.append(f"SLEEP_SECONDS ({self.sleep_seconds}) must be positive.")
        if self.limit_order_offset_pips < 0:
            errors.append(
                f"LIMIT_ORDER_OFFSET_PIPS ({self.limit_order_offset_pips}) cannot be negative."
            )

        if errors:
            error_message = (
                f"Configuration validation failed with {len(errors)} flaws:\n"
            ...
            _logger.info(
                f"{NEON['SUCCESS']}All configuration runes appear potent and well-formed.{NEON['RESET']}"
            )
...
class TradingStrategy(ABC):
    ...
    def _validate_df(self, df: pd.DataFrame, min_rows: int = 2) -> bool:
        if df is None or df.empty or len(df) < min_rows:
            self.logger.debug(
                f"Insufficient market whispers (Rows: {len(df) if df is not None else 0}, Min: {min_rows})."
            )
            return False
        if self.required_columns and not all(
            ...
            self.logger.warning(
                f"{NEON['WARNING']}Market scroll missing runes: {missing}{NEON['WARNING']}."
            )
            return False
    ...
    def _get_default_signals(self) -> Dict[str, Any]:
        return {
            ...
            "exit_long": False,
            "exit_short": False,
            "exit_reason": "Awaiting True Omens",
        }
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        momentum_val = safe_decimal_conversion(last.get("momentum", pd.NA), pd.NA)
        if pd.isna(confirm_is_up) or pd.isna(momentum_val):
            self.logger.debug(
                f"Confirm Trend ({_format_for_log(confirm_is_up, True)}) or Momentum ({_format_for_log(momentum_val)}) is NA."
            )
            return signals
        if (
            primary_long_flip
            ...
            self.logger.info(
                f"{NEON['SIDE_LONG']}DualST+Mom: LONG Entry - ST Flip, Confirm Up, Mom ({_format_for_log(momentum_val)}) > {_format_for_log(self.config.momentum_threshold)}{NEON['RESET']}"
            )
        elif (
            primary_short_flip
            ...
            self.logger.info(
                f"{NEON['SIDE_SHORT']}DualST+Mom: SHORT Entry - ST Flip, Confirm Down, Mom ({_format_for_log(momentum_val)}) < -{_format_for_log(self.config.momentum_threshold)}{NEON['RESET']}"
            )
        if primary_short_flip:
            signals["exit_long"] = True
            signals["exit_reason"] = "Primary ST Flipped Short"
            self.logger.info(
                f"{NEON['ACTION']}DualST+Mom: EXIT LONG - Primary ST Flipped Short{NEON['RESET']}"
            )
        if primary_long_flip:
            signals["exit_short"] = True
            signals["exit_reason"] = "Primary ST Flipped Long"
            self.logger.info(
                f"{NEON['ACTION']}DualST+Mom: EXIT SHORT - Primary ST Flipped Long{NEON['RESET']}"
            )
        return signals
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not self._validate_df(df, min_rows=min_rows) or len(df) < 2:
            return signals
        last = df.iloc[-1]
        prev = df.iloc[-2]
        ...
        if fisher_prev <= signal_prev and fisher_now > signal_now:
            signals["enter_long"] = True
            self.logger.info(
                f"{NEON['SIDE_LONG']}Ehlers: LONG Entry - Fisher cross ABOVE Signal.{NEON['RESET']}"
            )
        elif fisher_prev >= signal_prev and fisher_now < signal_now:
            signals["enter_short"] = True
            self.logger.info(
                f"{NEON['SIDE_SHORT']}Ehlers: SHORT Entry - Fisher cross BELOW Signal.{NEON['RESET']}"
            )
        if fisher_prev >= signal_prev and fisher_now < signal_now:
            signals["exit_long"] = True
            signals["exit_reason"] = "Ehlers Fisher cross BELOW Signal"
            self.logger.info(
                f"{NEON['ACTION']}Ehlers: EXIT LONG - Fisher cross BELOW Signal.{NEON['RESET']}"
            )
        elif fisher_prev <= signal_prev and fisher_now > signal_now:
            signals["exit_short"] = True
            signals["exit_reason"] = "Ehlers Fisher cross ABOVE Signal"
            self.logger.info(
                f"{NEON['ACTION']}Ehlers: EXIT SHORT - Fisher cross ABOVE Signal.{NEON['RESET']}"
            )
        return signals
...
class TradeMetrics:
    ...
    def set_initial_equity(self, equity: Decimal):
        ...
        if self.last_daily_reset_day != today or self.daily_start_equity is None:
            ...
            self.logger.info(
                f"{NEON['INFO']}Daily Equity Ward reset for drawdown tracking. Dawn Equity: {NEON['VALUE']}{equity:.2f} {CONFIG.usdt_symbol}{NEON['RESET']}"
            )
    ...
    def check_drawdown(self, current_equity: Decimal) -> Tuple[bool, str]:
        ...
        if drawdown_pct >= CONFIG.max_drawdown_percent:
            ...
            self.logger.warning(
                f"{NEON['WARNING']}{reason}. Trading must halt for the day to conserve essence.{NEON['RESET']}"
            )
    ...
    def log_trade(
        ...
        part_id: str,
        pnl_str: str,
        mae_str: Optional[str] = None,
        mfe_str: Optional[str] = None,
    ):
    ...
    def calculate_mae_mfe(
        ...
        """
        self.logger.debug(
            f"{NEON['COMMENT']}# Divining MAE/MFE for trade part {part_id}... A quest into the recent past.{NEON['RESET']}"
        )

        if exit_time_ms <= entry_time_ms:
            self.logger.warning(
                f"{NEON['WARNING']}MAE/MFE calc: Exit time ({datetime.fromtimestamp(exit_time_ms / 1000, tz=pytz.utc).isoformat()}) "
        ...
        self.logger.warning(
            f"{NEON['WARNING']}MAE/MFE calculation for part {part_id} is a rite yet to be fully mastered. "
    ...
    def get_performance_trend(self, window: int) -> float:
        ...
        if window <= 0 or not self.trades:
            self.logger.debug(
                f"{NEON['COMMENT']}Performance trend: Crystal ball is cloudy (no trades or invalid window). Assuming neutral winds (0.5).{NEON['RESET']}"
            )
            return 0.5
        ...
        if not recent_trades:
            self.logger.debug(
                f"{NEON['COMMENT']}Performance trend: Not enough recent trades for window {window}. Assuming neutral winds (0.5).{NEON['RESET']}"
            )
            return 0.5
        ...
        wins = sum(
            ...
            if safe_decimal_conversion(t.get("profit_str"), Decimal(0)) > 0
        )
        trend = float(wins / len(recent_trades))
        self.logger.debug(
            f"{NEON['INFO']}Performance Trend (last {len(recent_trades)} trades): {NEON['VALUE']}{trend:.2%}{NEON['RESET']} wins."
        )
        return trend
    ...
    def summary(self) -> str:
        ...
        total_profit_val = sum(
            safe_decimal_conversion(t.get("profit_str"), Decimal(0))
            for t in self.trades
        )
        total_profit_color = (
            NEON["PNL_POS"]
            if total_profit_val > 0
            else (NEON["PNL_NEG"] if total_profit_val < 0 else NEON["PNL_ZERO"])
        )
        total_profit_str = f"{total_profit_color}{total_profit_val:.2f} {CONFIG.usdt_symbol}{NEON['RESET']}"

        avg_profit_val = (
            (total_profit_val / Decimal(total_trades))
            if total_trades > 0
            else Decimal(0)
        )
        avg_profit_color = (
            NEON["PNL_POS"]
            if avg_profit_val > 0
            else (NEON["PNL_NEG"] if avg_profit_val < 0 else NEON["PNL_ZERO"])
        )
        ...
        if self.initial_equity is not None:
            ...
            overall_pnl_pct_val = (
                (total_profit_val / self.initial_equity) * 100
                if self.initial_equity > 0
                else Decimal(0)
            )
            overall_pnl_pct_color = (
                NEON["PNL_POS"]
                if overall_pnl_pct_val > 0
                else (NEON["PNL_NEG"] if overall_pnl_pct_val < 0 else NEON["PNL_ZERO"])
            )
            overall_pnl_pct_str = (
                f"{overall_pnl_pct_color}{overall_pnl_pct_val:.2%}{NEON['RESET']}"
            )
            ...
            summary_str += (
                f"Overall Session P/L % (based on ledger): {overall_pnl_pct_str}\n"
            )

        summary_str += (
            f"{NEON['HEADING']}--- End of Grand Ledger Reading ---{NEON['RESET']}"
        )
    ...
    def load_trades_from_list(self, trades_list: List[Dict[str, Any]]):
        ...
        self.logger.info(
            f"{NEON['INFO']}TradeMetrics: Re-inked {len(self.trades)} trades.{NEON['RESET']}"
        )
...
def save_persistent_state(force_heartbeat: bool = False) -> None:
    ...
    if not (
        ...
        return
    logger.debug(
        f"{NEON['COMMENT']}# Phoenix Feather scribing memories...{NEON['RESET']}"
    )
    try:
        serializable_parts = []
        ...
        logger.log(
            logging.INFO if force_heartbeat else logging.DEBUG,
            f"{NEON['SUCCESS']}Phoenix Feather: State memories scribed.{NEON['RESET']}",
        )
    except Exception as e:
        logger.error(
...
def load_persistent_state() -> bool:
    ...
    logger.info(
        f"{NEON['COMMENT']}# Phoenix Feather seeks past memories...{NEON['RESET']}"
    )
    if not os.path.exists(STATE_FILE_PATH):
        logger.info(
            f"{NEON['INFO']}Phoenix Feather: No previous scroll found.{NEON['RESET']}"
        )
        return False
    try:
        with open(STATE_FILE_PATH, "r") as f:
            state_data = json.load(f)
        if state_data.get("pyrmethus_version") != "2.8.2":
            logger.warning(
                f"{NEON['WARNING']}Phoenix Scroll version mismatch! Saved: {state_data.get('pyrmethus_version')}, Current: 2.8.2. Caution advised.{NEON['RESET']}"
            )
        if (
            ...
            logger.warning(
                f"{NEON['WARNING']}Phoenix Scroll sigils mismatch (symbol/strategy). Ignoring old scroll.{NEON['RESET']}"
            )
            archive_name = f"{STATE_FILE_PATH}.archived_{time.strftime('%Y%m%d%H%M%S')}"
            try:
                os.rename(STATE_FILE_PATH, archive_name)
                logger.info(f"Archived scroll to {archive_name}")
            except OSError as e_archive:
                logger.error(f"Could not archive scroll: {e_archive}")
            return False
        ...
        for part_data in state_data.get("active_trade_parts", []):
            ...
            for k, v_str in restored.items():
                if k in [
                    ...
                    "sl_price",
                    "initial_usdt_value",
                ] and isinstance(v_str, str):
                    try:
                        restored[k] = Decimal(v_str)
                    except InvalidOperation:
                        logger.warning(
                            f"Could not convert '{v_str}' to Decimal for key '{k}' in loaded part."
                        )
                        ...
                        try:
                            restored[k] = int(
                                datetime.fromisoformat(
                                    v_str.replace("Z", "+00:00")
                                ).timestamp()
                                * 1000
                            )
                        except ValueError:
                            try:
                                restored[k] = int(v_str)  # if it's a numeric string
                            except ValueError:
                                logger.warning(
                                    f"Could not convert '{v_str}' to int for entry_time_ms."
                                )
                    elif isinstance(v_str, (float, int)):
                        restored[k] = int(v_str)  # if saved as number
        ...
        if daily_equity_str and daily_equity_str.lower() != "none":
            try:
                ...
                logger.warning(
                    f"Could not restore daily_start_equity from '{daily_equity_str}'."
                )
        trade_metrics.last_daily_reset_day = state_data.get("last_daily_reset_day")
        logger.success(
            f"{NEON['SUCCESS']}Phoenix Feather: Memories reawakened! Active Parts: {len(_active_trade_parts)}{NEON['RESET']}"
        )
        return True
    except json.JSONDecodeError as e_json:
        logger.error(
            f"{NEON['ERROR']}Phoenix Feather: Scroll '{STATE_FILE_PATH}' is corrupted. Error: {e_json}{NEON['RESET']}"
        )
        archive_name = f"{STATE_FILE_PATH}.corrupted_{time.strftime('%Y%m%d%H%M%S')}"
        try:
            os.rename(STATE_FILE_PATH, archive_name)
            logger.info(f"Archived corrupted scroll to {archive_name}")
        except OSError as e_archive:
            logger.error(f"Could not archive corrupted scroll: {e_archive}")
        return False
    except Exception as e:
        logger.error(
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    if config.strategy_name == StrategyName.DUAL_SUPERTREND_MOMENTUM:
        ...
        df.ta.supertrend(
            ...
            multiplier=config.confirm_st_multiplier,
            append=True,
...
def get_market_data(
    ...
    logger.debug(
        f"{NEON['COMMENT']}# Fetching {limit} candles for {symbol} ({interval})...{NEON['RESET']}"
    )
    try:
        ohlcv = exchange.fetch_ohlcv(symbol, interval, limit=limit)
        if not ohlcv:
            logger.warning(
                f"{NEON['WARNING']}No OHLCV data returned for {symbol}.{NEON['RESET']}"
            )
            return None
        df = pd.DataFrame(
        ...
        for col in ["open", "high", "low", "close", "volume"]:
            df[col] = pd.to_numeric(df[col], errors="coerce")
        logger.debug(
            f"Fetched {len(df)} candles. Latest: {df['timestamp'].iloc[-1] if not df.empty else 'N/A'}"
        )
        return df
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error fetching market data for {symbol}: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        return None
...
def fetch_account_balance(
    ...
    logger.debug(
        f"{NEON['COMMENT']}# Fetching account balance for {currency_code}...{NEON['RESET']}"
    )
    try:
        ...
        if total_balance is None:
            logger.warning(
                f"{NEON['WARNING']}{currency_code} balance not found or invalid.{NEON['RESET']}"
            )
            return None
        logger.info(
            f"{NEON['INFO']}Current {currency_code} Balance: {NEON['VALUE']}{total_balance:.2f}{NEON['RESET']}"
        )
        return total_balance
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error fetching account balance: {e}{NEON['RESET']}"
        )
        return None
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    )  # Ensure qty exists and default to 0
    current_side = (
        _active_trade_parts[0]["side"]
        if _active_trade_parts and "side" in _active_trade_parts[0]
...
def calculate_position_size(
    ...
    logger.debug(
        f"{NEON['COMMENT']}# Calculating position size... Balance: {balance:.2f}, Risk %: {risk_per_trade_pct:.4f}{NEON['RESET']}"
    )
    if balance <= 0 or entry_price <= 0 or sl_price <= 0:
        logger.warning(
            f"{NEON['WARNING']}Invalid inputs for position sizing.{NEON['RESET']}"
        )
        return None
    ...
    if risk_per_unit == 0:
        logger.warning(
            f"{NEON['WARNING']}Risk per unit is zero (entry=SL). Cannot size.{NEON['RESET']}"
        )
        return None
    ...
    if position_usdt_value > CONFIG.max_order_usdt_amount:
        ...
        if entry_price > 0:
            quantity = position_usdt_value / entry_price
        else:
            logger.warning(
                f"{NEON['WARNING']}Entry price is zero, cannot recalculate quantity for capped USDT value.{NEON['RESET']}"
            )
            return None

    min_qty_val = market_info.get("limits", {}).get("amount", {}).get("min")
    min_qty = (
        safe_decimal_conversion(min_qty_val) if min_qty_val is not None else None
    )  # Ensure Decimal
    ...
    if isinstance(qty_precision_str, (int, float)) or (
        isinstance(qty_precision_str, str) and qty_precision_str.isdigit()
    ):
        qty_precision = int(qty_precision_str)
    elif (
        isinstance(qty_precision_str, str) and "." in qty_precision_str
        ...
        try:
            ...
            logger.warning(
                f"{NEON['WARNING']}Could not interpret quantity precision string '{qty_precision_str}'. Defaulting to 0 decimal places.{NEON['RESET']}"
            )

    if min_qty is not None and quantity < min_qty:
        logger.warning(
            f"{NEON['WARNING']}Calculated quantity {quantity} is below minimum {min_qty}. Cannot place order.{NEON['RESET']}"
        )
        return None
    ...
    if quantity <= 0:
        logger.warning(
            f"{NEON['WARNING']}Final quantity is zero or negative after adjustments. Cannot place order.{NEON['RESET']}"
        )
        return None
    ...
    logger.info(
        f"Calculated position size: {NEON['QTY']}{quantity:.{qty_precision}f}{NEON['RESET']} {base_currency}"
    )
    return quantity
...
def place_risked_order(
    ...
    global _active_trade_parts
    logger.info(
        f"{NEON['ACTION']}Attempting to place {side.upper()} order for {config.symbol}... Target Entry: {entry_price_target}, SL: {sl_price}{NEON['RESET']}"
    )

    balance = fetch_account_balance(exchange, config.usdt_symbol)
    if balance is None or balance <= Decimal(10):
        logger.error(
            f"{NEON['ERROR']}Insufficient balance ({balance}) or failed to fetch. Cannot place order.{NEON['RESET']}"
        )
        config.send_notification_method(
    ...
    if config.MARKET_INFO is None:
        logger.error(
            f"{NEON['ERROR']}Market info not loaded. Cannot calculate position size or place order.{NEON['RESET']}"
        )
        return None
    ...
    params = {
        ...
        else "IOC",  # IOC for Market, GTC for Limit often
    ...
    try:
        logger.info(
            f"Placing {order_type_str.upper()} {side.upper()} order: Qty {quantity}, Symbol {config.symbol}, SL {sl_price}"
        )
        order = exchange.create_order(
            config.symbol,
            order_type_str.lower(),
            side.lower(),
            float(quantity),
        ...
        logger.success(
            f"{NEON['SUCCESS']}Entry Order Sent: ID {order['id']}, Status {order.get('status', 'N/A')}{NEON['RESET']}"
        )
        config.send_notification_method(
        ...
        if filled_order.get("status") == "closed":
            ...
            logger.success(
                f"{NEON['SUCCESS']}Order Filled! Part ID {part_id} added. Entry: {actual_entry_price}, Qty: {actual_qty}{NEON['RESET']}"
            )
            config.send_notification_method(
            ...
            logger.warning(
                f"{NEON['WARNING']}Order {order['id']} not filled promptly. Status: {filled_order.get('status')}. Manual check advised.{NEON['RESET']}"
            )
            if order_type_str == "Limit" and filled_order.get("status") == "open":
                logger.info(
                    f"Limit order {order['id']} still open. Consider cancellation logic after timeout."
                )
            return None

    except ccxt.InsufficientFunds as e:
        logger.error(
            f"{NEON['ERROR']}Insufficient funds to place order: {e}{NEON['RESET']}"
        )
        config.send_notification_method(
        ...
        logger.error(
            f"{NEON['ERROR']}Unexpected error placing order: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        return None
...
def close_position_part(
    ...
    global _active_trade_parts
    logger.info(
        f"{NEON['ACTION']}Attempting to close trade part {part_to_close['part_id']} ({part_to_close['side']} {part_to_close['qty']} {config.symbol}) for reason: {reason}{NEON['RESET']}"
    )

    close_side = (
        config.side_sell
        if part_to_close["side"].lower() == config.pos_long.lower()
    ...
    try:
        ...
        logger.info(
            f"Placing MARKET {close_side.upper()} order to close part {part_to_close['part_id']}: Qty {qty_to_close}"
        )
        ...
        logger.success(
            f"{NEON['SUCCESS']}Close Order Sent: ID {close_order['id']}, Status {close_order.get('status', 'N/A')}{NEON['RESET']}"
        )
        ...
        if filled_close_order.get("status") == "closed":
            ...
            trade_metrics.log_trade(
                ...
                pnl_str=str(pnl),
                mae_str=None,
                mfe_str=None,  # Placeholder for future MAE/MFE
            )
            _active_trade_parts = [
                p
            ...
            logger.warning(
                f"{NEON['WARNING']}Close order {close_order['id']} for part {part_to_close['part_id']} not filled. Status: {filled_close_order.get('status')}. Manual check required.{NEON['RESET']}"
            )
            return False

    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error closing position part {part_to_close['part_id']}: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        config.send_notification_method(
...
def cancel_all_symbol_orders(exchange: ccxt.Exchange, symbol: str):
    logger.info(
        f"{NEON['ACTION']}Cancelling all open orders for {symbol}...{NEON['RESET']}"
    )
    try:
        ...
        logger.success(
            f"{NEON['SUCCESS']}All open orders for {symbol} cancelled.{NEON['RESET']}"
        )
    except Exception as e:
        logger.error(
            f"{NEON['ERROR']}Error cancelling orders for {symbol}: {e}{NEON['RESET']}"
        )
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    global _active_trade_parts
    logger.warning(
        f"{NEON['WARNING']}Closing all positions for {config.symbol} due to: {reason}{NEON['RESET']}"
    )
    ...
    try:
        ...
        for pos in positions:
            ...
            if pos and pos_contracts != Decimal(0):
                ...
                logger.warning(
                    f"Found residual exchange position: {pos.get('side')} {qty_to_close} {config.symbol}. Attempting market close."
                )
                ...
                logger.info(
                    f"Residual position closure order sent for {qty_to_close} {config.symbol}."
                )
    except Exception as e:
...
def main_loop(exchange: ccxt.Exchange, config: Config) -> None:
    ...
    if load_persistent_state():
        ...
        logger.info(
            f"{NEON['INFO']}No valid prior state. Starting fresh.{NEON['RESET']}"
        )
    ...
    if current_balance is not None:
        ...
        logger.warning(
            f"{NEON['WARNING']}Could not set initial equity. Balance fetch failed.{NEON['RESET']}"
        )
    ...
    while True:
        try:
            ...
            if current_balance is not None:
                ...
                if drawdown_hit:
                    logger.critical(
                        f"{NEON['CRITICAL']}Max drawdown! {dd_reason}. Pyrmethus must rest.{NEON['RESET']}"
                    )
                    close_all_symbol_positions(exchange, config, "Max Drawdown Reached")
                    break
            else:
                logger.warning(
                    f"{NEON['WARNING']}Failed to fetch balance this cycle.{NEON['RESET']}"
                )
            ...
            if ohlcv_df is None or ohlcv_df.empty:
                logger.warning(
                    f"{NEON['WARNING']}No market data. Retrying after pause.{NEON['RESET']}"
                )
            ...
            logger.debug(
                f"Signals: EnterL={signals['enter_long']}, EnterS={signals['enter_short']}, ExitL={signals['exit_long']}, ExitS={signals['exit_short']}"
            )

            current_pos_side, current_pos_qty = get_current_position_info()
            logger.debug(
                f"Current Position: Side={current_pos_side}, Qty={current_pos_qty}"
            )
            ...
            latest_atr = (
                ...
                and not df_with_indicators[atr_col].empty
                else None
            )
            latest_close = (
                ...
                and not df_with_indicators.empty
                and not df_with_indicators["close"].empty
                else None
            )

            if (
                latest_atr is None or latest_close is None or latest_atr <= 0
            ):  # Added latest_atr > 0 check
                logger.warning(
                    f"{NEON['WARNING']}Missing or invalid latest ATR ({latest_atr}) or Close price ({latest_close}). Skipping trade decisions.{NEON['RESET']}"
                )
                time.sleep(config.sleep_seconds)
                continue
            ...
            if current_pos_side == config.pos_none:
                ...
                if signals.get("exit_long"):
                    logger.info(
                        f"Exit LONG signal received: {signals.get('exit_reason')}"
                    )
                ...
                if signals.get("exit_short"):
                    logger.info(
                        f"Exit SHORT signal received: {signals.get('exit_reason')}"
                    )
                    for part in list(_active_trade_parts):
            ...
            logger.debug(
                f"{NEON['COMMENT']}# Cycle complete. Resting for {config.sleep_seconds}s...{NEON['RESET']}"
            )
            time.sleep(config.sleep_seconds)
...
if __name__ == "__main__":
    logger.info(
        f"{NEON['COMMENT']}# Pyrmethus prepares to connect to the exchange realm...{NEON['RESET']}"
    )
    exchange = None
    try:
        exchange_params = {
        ...
        if CONFIG.PAPER_TRADING_MODE:
            logger.warning(
                f"{NEON['WARNING']}PAPER_TRADING_MODE is True. Ensure API keys are for Bybit TESTNET.{NEON['RESET']}"
            )
        ...
        if CONFIG.symbol not in markets:
            logger.critical(
                f"{NEON['CRITICAL']}Symbol {CONFIG.symbol} not found in exchange markets. Pyrmethus cannot trade this ether.{NEON['RESET']}"
            )
            CONFIG.send_notification_method(
        ...
        market_precision_price = CONFIG.MARKET_INFO.get("precision", {}).get(
            "price", "unknown"
        )
        market_precision_amount = CONFIG.MARKET_INFO.get("precision", {}).get(
            "amount", "unknown"
        )
        logger.success(
            f"{NEON['SUCCESS']}Market info for {CONFIG.symbol} loaded. Precision: Price {market_precision_price}, Amount {market_precision_amount}{NEON['RESET']}"
        )
        ...
        try:
            ...
            if (
                ":" in CONFIG.symbol
                and CONFIG.symbol.split(":")[1] == CONFIG.usdt_symbol
            ):
                category = "linear"
            elif (
                "/" in CONFIG.symbol
                and CONFIG.symbol.split("/")[1] != CONFIG.usdt_symbol
            ):  # e.g. BTC/USD (not :USDT)
            ...
            logger.info(
                f"Deduced category for {CONFIG.symbol} as '{category}' for leverage setting."
            )
            ...
            if (
                exchange.δύο_directional_margin
            ):  # Made-up attribute, check real ccxt hedge mode detection
                leverage_params.update(
                    {"buyLeverage": CONFIG.leverage, "sellLeverage": CONFIG.leverage}
                )

            response = exchange.set_leverage(
                CONFIG.leverage, CONFIG.symbol, leverage_params
            )
            logger.success(
                f"{NEON['SUCCESS']}Leverage for {CONFIG.symbol} ({category}) set to {CONFIG.leverage}x. Response: {response}{NEON['RESET']}"
            )
        ...
        logger.success(
            f"{NEON['SUCCESS']}Successfully connected to exchange: {exchange.id}{NEON['RESET']}"
        )
        CONFIG.send_notification_method(
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Authentication failed! Check API keys: {e}{NEON['RESET']}"
        )
        CONFIG.send_notification_method("Pyrmethus Auth Fail", "API Key Auth Error.")
        sys.exit(1)
    except ccxt.NetworkError as e:
        logger.critical(
            f"{NEON['CRITICAL']}Network Error: {e}. Check connectivity.{NEON['RESET']}"
        )
        CONFIG.send_notification_method(
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Failed to initialize exchange: {e}{NEON['RESET']}"
        )
        logger.debug(traceback.format_exc())
        CONFIG.send_notification_method(
    ...
    if exchange:
        ...
        logger.critical(
            f"{NEON['CRITICAL']}Exchange not initialized. Pyrmethus sleeps.{NEON['RESET']}"
        )
