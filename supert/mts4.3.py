...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.position_qty_epsilon: Decimal = Decimal(
            "1e-9"
        )  # Smallest quantity considered non-zero
...
class TradeMetrics:
    ...
    def summary(self) -> str:
        ...
        if self.daily_start_equity is not None:
            ...
            )  # Fallback if initial_equity was somehow None but daily_start_equity is set
...
def fetch_account_balance(
    ...
    try:
        ...
        ):  # If still not found, log a warning and try a more general total if applicable
            logger.debug(
...
def calculate_position_size(
    ...
    if not (
        isinstance(balance, Decimal)
        and balance > 0
        and isinstance(entry_price, Decimal)
        and entry_price > 0
        and isinstance(sl_price, Decimal)
        and sl_price > 0
    ...
    try:
        ...
        if filled_order.get("status") == "closed":
            ...
            new_part = {
                ...
                "side": side,  # Store internal "Long"/"Short"
...
def close_position_part(
    ...
    logger.info(
        f"{NEON['ACTION']}Attempting to unravel trade part {part_id} ({part_side} {NEON['QTY']}{part_qty}{NEON['RESET']} {config.symbol}) for: {reason}{NEON['RESET']}"
    )
        ...
        for pos_data in positions:
            if pos_data and pos_data.get("symbol") == config.symbol:
                pos_qty_val = pos_data.get(
                    "contracts", pos_data.get("size", pos_data.get("contractSize", "0"))
                )
    ...
    while True:
        try:
            ...
            if current_balance is not None:
                ...
                if drawdown_hit:
                    ...
                    config.send_notification_method(
                        ...
                        f"Halted for {config.symbol}. {dd_reason}",
                    )
                    break
            else:
                logger.warning(
            ...
            config.send_notification_method(
                ...
                f"Loop crashed for {config.symbol}: {str(e)[:150]}",
            )
            logger.info(
...
if __name__ == "__main__":
    ...
    try:
        ...
        amount_prec_display = get_decimal_places_from_step(str(amount_prec_raw))
        ...
        CONFIG.send_notification_method(
            "Pyrmethus Online",
            f"Connected to {exchange.id} for {CONFIG.symbol} @ {CONFIG.leverage}x.",
        )
