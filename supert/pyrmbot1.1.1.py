...
class DualSupertrendMomentumStrategy(TradingStrategy):
    ...
    async def generate_signals(
        ...
        st_max_dist_atr_mult = self.config.strategy_params[
            "st_max_entry_distance_atr_multiplier"
        ]
        ...
        confirm_st_stability_lookback = self.config.strategy_params[
        ...
        if (
            ...
            if primary_long_flip:
                current_st_line_val_key = f"{st_p_base}l"
            elif primary_short_flip:
                current_st_line_val_key = f"{st_p_base}s"
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    async def generate_signals(
        ...
        signals = self._get_default_signals()
        extreme_pos = self.config.strategy_params[
            "ehlers_fisher_extreme_threshold_positive"
        ]
        extreme_neg = self.config.strategy_params[
            ...
            for k, v_loaded in part_data_str_values.items():
                if k in decimal_keys_in_part:
                    ...
                    if isinstance(v_loaded, dict):
                        restored_part[k] = {
                            key_ind: safe_decimal_conversion(val_ind, val_ind)
...
def extract_indicators_from_df(df_row: pd.Series, config: Config) -> Dict[str, Any]:
    ...
    atr_period = config.strategy_params[
        "atr_calculation_period"
    ]  # Type guaranteed by Config
    ...
    if config.strategy_params["name"] == StrategyName.DUAL_SUPERTREND_MOMENTUM:
        st_atr_len = config.strategy_params[
            "st_atr_length"
        ]  # Type guaranteed by Config
...
async def async_place_risked_order(
    ...
    if not _placeholder_balance:
        ...
        logger_order.warning(
            "Balance not found in cache for placeholder logic. Using default for calculation."
        )
        _placeholder_balance = await async_fetch_account_balance(
            exchange, config.usdt_symbol, config
        )  # Ensure it's fetched if not in cache
        ...
        if (
            ...
            if config.enhancements["whipsaw_cooldown_enable"]:
                ...
                if (
                    ...
                    == config.enhancements["whipsaw_max_trades_in_period"]
                ):
                    if (
                        trade_timestamps_for_whipsaw[-1]
                        - trade_timestamps_for_whipsaw[0]
                    ) < config.enhancements["whipsaw_period_seconds"]:
                        whipsaw_cooldown_active_until = (
                            time.time()
                            + config.enhancements["whipsaw_cooldown_seconds"]
                        )
                        logger_order.warning(
                            f"Whipsaw condition met. Trading paused until {datetime.fromtimestamp(whipsaw_cooldown_active_until).isoformat()}"
                        )
...
async def async_fetch_account_balance(
    ...
    try:
        ...
        simulated_balance_value = safe_decimal_conversion(
            balance_response.get(currency, {}).get("total", Decimal("1000.0"))
        )
        if simulated_balance_value is None:  # Fallback if parsing fails
            simulated_balance_value = Decimal(
                "1000.0"
            )  # Default for placeholder if parsing fails
            logger_balance.warning(
                f"Could not parse live balance for {currency}, using default placeholder value: {simulated_balance_value}"
            )

        balance_cache[f"{currency}_balance_value"] = simulated_balance_value
        logger_balance.info(
            f"Fetched and cached live balance for {currency}: {simulated_balance_value}"
        )
        return simulated_balance_value
    except Exception as e:
        logger_balance.error(
...
async def perform_health_check(exchange: ccxt_async.Exchange, config: Config) -> bool:
    ...
    try:
        ...
        logger_health.warning(
            f"Health check failed: {e}", exc_info=LOGGING_LEVEL <= logging.DEBUG
        )
        return False
...
async def main():
    ...
    try:
        ...
        if CONFIG.exchange["paper_trading"]:
            ...
            logger.info(
                f"{NEON['WARNING']}Targeting BYBIT TESTNET environment. URLs: {async_exchange_instance.urls.get('api')}{NEON['RESET']}"
            )
        else:
            logger.info(
                f"{NEON['CRITICAL']}Targeting BYBIT MAINNET (LIVE) environment. URLs: {async_exchange_instance.urls.get('api')}. EXTREME CAUTION ADVISED.{NEON['RESET']}"
            )
        ...
        if category:
            try:
                ...
                logger.info(
                    f"Setting leverage to {leverage_val}x for {CONFIG.exchange['symbol']} (Category: {category})..."
                )
            ...
            logger.info(
                f"Leverage setting skipped for {CONFIG.exchange['symbol']} as it does not appear to be a linear/inverse derivative or category is undetermined."
            )
            ...
            if current_time - _last_health_check_time > HEALTH_CHECK_INTERVAL_SECONDS:
                if not await perform_health_check(async_exchange_instance, CONFIG):
                    ...
                    _last_health_check_time = (
                        time.time()
                    )  # Update time even on failure to avoid rapid checks
                    continue
