    ...
    try:
        ...
        if stochrsi_df is not None and not stochrsi_df.empty:
            if k_col_ta in stochrsi_df.columns:
                ...
                logger.warning(
                    f"StochRSI K column '{k_col_ta}' not found in pandas_ta output."
                )
                df["stochrsi_k"] = pd.NA
            if d_col_ta in stochrsi_df.columns:
                ...
                logger.warning(
                    f"StochRSI D column '{d_col_ta}' not found in pandas_ta output."
                )
                df["stochrsi_d"] = pd.NA
        else:
        ...
        if mom_col_ta in df.columns:
            ...
            logger.warning(
                f"Momentum column '{mom_col_ta}' not found after ta.mom calculation."
            )
            df["momentum"] = pd.NA
    ...
    try:
        ...
        if fisher_df is not None and not fisher_df.empty:
            if fish_col_ta in fisher_df.columns:
                df["ehlers_fisher"] = fisher_df[fish_col_ta].apply(
                    safe_decimal_conversion
                )
            else:
                logger.warning(
                    f"Ehlers Fisher column '{fish_col_ta}' not found in pandas_ta output."
                )
            ...
            if signal_col_ta in fisher_df.columns:
                ...
                logger.warning(
                    f"Ehlers Signal column '{signal_col_ta}' not found in pandas_ta output."
                )
                df["ehlers_signal"] = pd.NA
        else:
            logger.warning(
                "Ehlers Fisher calculation returned empty or None DataFrame."
            )
            df["ehlers_fisher"] = pd.NA
            df["ehlers_signal"] = pd.NA
...
def calculate_ehlers_ma(
    ...
    try:
        ...
        if ssf_fast_series is not None:
            ...
            logger.warning(
                f"Fast Ehlers SSF ({fast_len}, {poles}) calculation returned None."
            )
        ...
        if ssf_slow_series is not None:
            ...
            logger.warning(
                f"Slow Ehlers SSF ({slow_len}, {poles}) calculation returned None."
            )
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        for pos in fetched_positions:
            ...
            )  # Bybit: 0 for One-Way Mode
            pos_side_v5 = pos_info.get(
                "side", "None"
            )  # Bybit: "Buy", "Sell", or "None"
            ...
            if pos_market_id_from_api == market_id and position_idx == 0:
                ...
                if (
                    ...
                    logger.debug(
                        f"Found active V5 position candidate (idx 0): Side={pos_side_v5}, Size={size_str}"
                    )
    ...
    for attempt in range(CONFIG.retry_count + 1):
        try:
            ...
            if (
                ...
                "cannot be lower than 1" in err_str and leverage < 1
            ):  # Should be caught by config validation ideally
                logger.error(
...
def close_position(
    ...
    try:
        ...
        if (
            amount_dec <= CONFIG.position_qty_epsilon
        ):  # Should be caught by live_amount_to_close check, but defensive
            logger.error(
        ...
        params = {
            "reduceOnly": True
        }  # Crucial for Bybit V5 to ensure it closes a position
    ...
    ) as e:  # Should not happen with reduceOnly if position exists
        logger.error(
        ...
        ValueError
    ) as e:  # From format_amount or Decimal conversion if something unexpected happens
        logger.error(
...
def calculate_position_size(
    ...
    if (
        quantity_precise <= CONFIG.position_qty_epsilon
    ):  # Check if calculated quantity is too small
        logger.warning(
    ...
    try:
        ...
        DivisionByZero,
        Exception,
    ) as margin_err:  # DivisionByZero if leverage is bad (already checked)
        logger.error(
...
def place_risked_market_order(
    ...
    try:
        ...
        if initial_sl_price_raw <= 0:  # SL price cannot be zero or negative
            ...
            return None
        if (
            min_price is not None and initial_sl_price_raw < min_price
        ):  # Ensure SL is not below exchange minimum price
            logger.warning(
                f"{Fore.YELLOW}Initial SL price {initial_sl_price_raw:.4f} below min allowed {min_price:.4f}. Adjusting to min price.{Style.RESET_ALL}"
            )
        ...
        if min_qty is not None and final_quantity < min_qty:
            logger.error(
                f"{Fore.RED}Place Order Error ({side.upper()}): Qty {final_quantity:.8f} < Min Qty {min_qty:.8f}.{Style.RESET_ALL}"
            )
            send_sms_alert(
            ...
            return None
        if (
            max_qty is not None and final_quantity > max_qty
        ):  # Should ideally not happen if cap is reasonable
            logger.warning(
                f"{Fore.YELLOW}Qty {final_quantity:.8f} > Max Qty {max_qty:.8f}. Adjusting to max exchange limit.{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if (
                not order_id
            ):  # Should always get an ID if order submission was accepted by API
                logger.critical(
            ...
            Exception
        ) as e_place:  # Catch any other unexpected error during placement
            logger.error(
        ...
        if not filled_entry or filled_entry.get("status") != "closed":
            ...
            try:
                ...
                exchange.cancel_order(
                    order_id, symbol
                )  # This might fail if already filled/gone
                logger.info(
        ...
        if (
            avg_fill_price <= 0 and cost > 0 and filled_qty > 0
        ):  # If 'average' is missing, try to derive from cost/filled
        ...
        if (
            min_price is not None and actual_sl_price_raw < min_price
        ):  # Check against market min price
            logger.warning(
                f"{Fore.YELLOW}Actual SL price {actual_sl_price_raw:.4f} below min allowed {min_price:.4f}. Adjusting to min price.{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if min_price is not None and act_price_raw < min_price:
                act_price_raw = min_price  # Check against market min price
        ...
        Exception
    ) as e_overall:  # Catch-all for truly unexpected issues in this function
        logger.error(
        ...
        for order in open_orders:
            ...
            if order_id:
                try:
                    ...
                    ccxt.OrderNotFound
                ):  # Order already gone (filled, cancelled, etc.)
                    logger.warning(
...
def generate_signals(df: pd.DataFrame, strategy_name: str) -> dict[str, Any]:
    ...
    try:
        if strategy_name == "DUAL_SUPERTREND":
            primary_long_flip = last.get(
                "st_long", False
            )  # True if primary ST flipped to Long this candle
            primary_short_flip = last.get(
                "st_short", False
            )  # True if primary ST flipped to Short this candle
            confirm_is_up = last.get(
                "confirm_trend", pd.NA
            )  # True if confirmation ST is in uptrend

            if pd.isna(
                confirm_is_up
            ):  # Cannot make decision without confirmation trend
                logger.warning(
                    f"Signal Gen ({strategy_name}): Confirmation trend is NA. No signal."
                )
                return signals
        ...
        logger.error(
            f"{Fore.RED}Signal Generation Error ({strategy_name}): Missing indicator column: {e}. Check calculations.{Style.RESET_ALL}"
        )
    ...
    if (
        ...
        active_signals = {
            k: v for k, v in signals.items() if isinstance(v, bool) and v
        }  # Log only active signals
...
def trade_logic(exchange: ccxt.Exchange, symbol: str, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        last_candle_data = (
            df.iloc[-1] if not df.empty else pd.Series(dtype="object")
        )  # Ensure Series even if df becomes empty
        ...
        if should_exit_long or should_exit_short:
            ...
            if close_result:
                logger.info(
                    f"Position close order placed for {position_side}. Pausing for settlement/state update..."
                )
        ...
        if ob_data is None and (
            CONFIG.order_book_ratio_threshold_long < Decimal("Infinity")
            or CONFIG.order_book_ratio_threshold_short > Decimal(0)
        ):  # Fetch OB only if needed and not already fetched
        ...
        if final_enter_long:
            ...
            action_taken_this_cycle = True
            cancel_open_orders(
                exchange, symbol, "Pre-Long Entry Cleanup"
            )  # Should be no orders if flat, but defensive
            time.sleep(0.5)
            ...
            if (
                potential_entry_signal and not action_taken_this_cycle
            ):  # Signal was there but filters blocked it
                logger.info(
...
def graceful_shutdown(exchange: ccxt.Exchange | None, symbol: str | None) -> None:
    ...
    send_sms_alert(
        f"[{market_base}/{CONFIG.strategy_name if 'CONFIG' in globals() and hasattr(CONFIG, 'strategy_name') else 'N/A'}] Shutdown initiated. Attempting cleanup..."
    )
    ...
    try:
        ...
        if (
            ...
            if close_result:  # If close order was successfully placed
                ...
                )  # Wait longer for fill during shutdown
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            if not is_contract:
                raise ValueError(
                    f"Market '{symbol_unified}' (Type: {market_type}) is not a contract/futures market."
                )
            if not is_linear:
                logger.warning(
                    f"{Fore.YELLOW}Market '{symbol_unified}' not detected as LINEAR (USDT-margined). Ensure compatibility if using other margin types.{Style.RESET_ALL}"
                )
            ...
            if not set_leverage(exchange, symbol_unified, CONFIG.leverage):
                raise RuntimeError(
                    f"Leverage conjuring failed for {symbol_unified}. Bot cannot ensure correct leverage setting."
                )
        ...
        while run_bot:
            ...
            try:
                ...
                if df is not None and not df.empty:
                    trade_logic(
                        exchange, symbol_unified, df.copy()
                    )  # Pass a copy to avoid modifications affecting other parts
                else:
                    logger.warning(
            ...
            # --- Handle Specific CCXT/Network Errors in Loop ---
                ...
                ccxt.NetworkError
            ) as e:  # Includes RequestTimeout, DDoSProtection, etc.
                logger.warning(
                    f"{Fore.YELLOW}Network disturbance in main loop: {e}. Retrying next cycle after a pause.{Style.RESET_ALL}"
                )
                ...
                logger.critical(
                    f"{Back.RED}{Fore.WHITE}{Style.BRIGHT}FATAL: Authentication Error: {e}. API keys likely invalid. Stopping NOW.{Style.RESET_ALL}"
                )
                send_sms_alert(
