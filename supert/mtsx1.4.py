...
if load_dotenv(dotenv_path=env_path):
    logging.getLogger("PreConfig").info(f"Secrets whispered from .env scroll: {env_path}")
else:
    logging.getLogger("PreConfig").warning(f"No .env scroll found at {env_path}. Relying on system environment variables or defaults.")
getcontext().prec = 18
...
def send_termux_notification(title: str, message: str, notification_id: int = 777) -> None:
    ...
    if 'CONFIG' not in globals() or not hasattr(CONFIG, 'enable_notifications') or not CONFIG.enable_notifications:
        return
    notification_buffer.append({"title": title, "message": message, "id": notification_id})
    now = time.time()
    if now - last_notification_flush_time >= NOTIFICATION_BATCH_INTERVAL_SECONDS or len(notification_buffer) >= 10:
        flush_notifications()
...
def flush_notifications() -> None:
    ...
    if not notification_buffer:
        return
    notification_timeout = CONFIG.notification_timeout_seconds if 'CONFIG' in globals() else 10
    logger = logging.getLogger("TermuxNotification")
    combined_message = "\n".join([f"{n['title']}: {n['message']}" for n in notification_buffer])
    try:
        safe_title = json.dumps("Pyrmethus Batch Notification")
        safe_message = json.dumps(combined_message[:1000])
        command = ["termux-notification", "--title", safe_title, "--content", safe_message, "--id", str(notification_buffer[0]["id"])]
        ...
        if process.returncode == 0:
            ...
            logger.error(f"Failed to send batched Termux notification. Error: {stderr.decode().strip()}")
...
class Config:
    def __init__(self) -> None:
        ...
        # Trading Parameters
        self.trading = {
            "risk_per_trade_percentage": self._get_env("RISK_PER_TRADE_PERCENTAGE", "0.005", cast_type=Decimal),
            "max_order_usdt_amount": self._get_env("MAX_ORDER_USDT_AMOUNT", "1000.0", cast_type=Decimal),
            "min_usdt_balance": self._get_env("MIN_USDT_BALANCE_FOR_TRADING", "20.0", cast_type=Decimal),
            "max_active_trade_parts": self._get_env("MAX_ACTIVE_TRADE_PARTS", 1, cast_type=int),
            "order_fill_timeout_seconds": self._get_env("ORDER_FILL_TIMEOUT_SECONDS", 15, cast_type=int),
            "sleep_seconds": self._get_env("SLEEP_SECONDS", 10, cast_type=int),
        }
        # Risk Management
        self.risk = {
            "atr_stop_loss_multiplier": self._get_env("ATR_STOP_LOSS_MULTIPLIER", "1.5", cast_type=Decimal),
            "enable_take_profit": self._get_env("ENABLE_TAKE_PROFIT", "true", cast_type=bool),
            "atr_take_profit_multiplier": self._get_env("ATR_TAKE_PROFIT_MULTIPLIER", "2.0", cast_type=Decimal),
            "enable_trailing_sl": self._get_env("ENABLE_TRAILING_SL", "true", cast_type=bool),
            "trailing_sl_trigger_atr": self._get_env("TRAILING_SL_TRIGGER_ATR", "1.0", cast_type=Decimal),
            "trailing_sl_distance_atr": self._get_env("TRAILING_SL_DISTANCE_ATR", "0.5", cast_type=Decimal),
            "enable_max_drawdown_stop": self._get_env("ENABLE_MAX_DRAWDOWN_STOP", "true", cast_type=bool),
            "max_drawdown_percent": self._get_env("MAX_DRAWDOWN_PERCENT", "0.05", cast_type=Decimal),
            "enable_session_pnl_limits": self._get_env("ENABLE_SESSION_PNL_LIMITS", "false", cast_type=bool),
            "session_profit_target_usdt": self._get_env("SESSION_PROFIT_TARGET_USDT", None, cast_type=Decimal, required=False),
            "session_max_loss_usdt": self._get_env("SESSION_MAX_LOSS_USDT", None, cast_type=Decimal, required=False),
        }
        # Strategy Parameters
        self.strategy = {
            ...
            "confirm_st_atr_length": self._get_env("CONFIRM_ST_ATR_LENGTH", 20, cast_type=int),
            "confirm_st_multiplier": self._get_env("CONFIRM_ST_MULTIPLIER", "3.0", cast_type=Decimal),
            "momentum_period": self._get_env("MOMENTUM_PERIOD", 14, cast_type=int),
            "momentum_threshold": self._get_env("MOMENTUM_THRESHOLD", "0", cast_type=Decimal),
            "confirm_st_stability_lookback": self._get_env("CONFIRM_ST_STABILITY_LOOKBACK", 3, cast_type=int),
            "st_max_entry_distance_atr_multiplier": self._get_env("ST_MAX_ENTRY_DISTANCE_ATR_MULTIPLIER", "0.5", cast_type=Decimal, required=False),
        ...
        self.notifications = {
            ...
            "timeout_seconds": self._get_env("NOTIFICATION_TIMEOUT_SECONDS", 10, cast_type=int),
        }
        # Other Settings
        ...
        self.logger.info("Configuration Runes v%s Summoned and Verified" % PYRMETHUS_VERSION)
    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, secret: bool = False) -> Any:
        ...
        if value_str is None:
            if required:
                self.logger.critical(f"CRITICAL: Required config rune '{key}' not found.")
        ...
        if value_to_cast is None and required:
            self.logger.critical(f"CRITICAL: Required config rune '{key}' resolved to None.")
            raise ValueError(f"Required environment variable '{key}' resolved to None.")
        try:
            raw_value_str = str(value_to_cast)
            if cast_type == bool: return raw_value_str.lower() in ["true", "1", "yes", "y"]
            elif cast_type == Decimal: return Decimal(raw_value_str)
            elif cast_type == int: return int(Decimal(raw_value_str))
            elif cast_type == float: return float(raw_value_str)
            elif cast_type == str: return raw_value_str
            else:
            ...
            self.logger.error(f"Invalid type/value for '{key}': '{value_to_cast}' (Source: {source}). Expected {cast_type.__name__}. Err: {e}.")
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.risk["atr_stop_loss_multiplier"] <= 0: errors.append("ATR_STOP_LOSS_MULTIPLIER must be positive.")
        if self.risk["enable_take_profit"] and self.risk["atr_take_profit_multiplier"] <= 0: errors.append("ATR_TAKE_PROFIT_MULTIPLIER must be positive if take profit is enabled.")
        if self.trading["min_usdt_balance"] < 0: errors.append("MIN_USDT_BALANCE_FOR_TRADING cannot be negative.")
        if errors:
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.handlers.RotatingFileHandler(log_file_name, maxBytes=10*1024*1024, backupCount=5)
    ]
)
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame, latest_close: Optional[Decimal] = None, latest_atr: Optional[Decimal] = None) -> Dict[str, Any]:
        ...
        elif 'confirm_trend' in df.columns and len(df) >= self.config.strategy["confirm_st_stability_lookback"]:
            recent_confirm_trends = df['confirm_trend'].iloc[-self.config.strategy["confirm_st_stability_lookback"]:]
        ...
        if pd.isna(stable_confirm_trend) or pd.isna(momentum_val):
            self.logger.debug(f"Stable Confirm ST ({stable_confirm_trend}) or Mom ({momentum_val}) is NA.")
            return signals
        price_proximity_ok = True
        if self.config.strategy["st_max_entry_distance_atr_multiplier"] is not None and latest_atr is not None and latest_atr > 0 and latest_close is not None:
            max_allowed_distance = latest_atr * self.config.strategy["st_max_entry_distance_atr_multiplier"]
        ...
        if (primary_long_flip and stable_confirm_trend is True and
                momentum_val > self.config.strategy["momentum_threshold"] and momentum_val > 0 and price_proximity_ok):
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame, latest_close: Optional[Decimal] = None, latest_atr: Optional[Decimal] = None) -> Dict[str, Any]:
        signals = self._get_default_signals()
        min_rows_needed = self.config.strategy["ehlers_fisher_length"] + self.config.strategy["ehlers_fisher_signal_length"] + 5
        ...
        if pd.isna(fisher_now) or pd.isna(signal_now) or pd.isna(fisher_prev) or pd.isna(signal_prev):
            ...
            return signals
        is_fisher_extreme = (fisher_now > self.config.strategy["ehlers_fisher_extreme_threshold_positive"] or
                             fisher_now < self.config.strategy["ehlers_fisher_extreme_threshold_negative"])
...
class TradeMetrics:
    ...
    def set_initial_equity(self, equity: Decimal):
        if self.initial_equity is None:
            ...
            self.logger.info(f"Initial Session Equity: {equity:.2f} {CONFIG.usdt_symbol}")
        ...
        if self.last_daily_reset_day != today or self.daily_start_equity is None:
            ...
            self.logger.info(f"Daily Equity Ward reset. Dawn Equity: {equity:.2f} {CONFIG.usdt_symbol}")
    ...
    def check_drawdown(self, current_equity: Decimal) -> Tuple[bool, str]:
        if not CONFIG.risk["enable_max_drawdown_stop"] or self.daily_start_equity is None or self.daily_start_equity <= 0:
            return False, ""
    ...
    def log_trade(self, symbol: str, side: str, entry_price: Decimal, exit_price: Decimal, qty: Decimal,
        ...
        self.logger.success(f"Trade Chronicle ({trade_type}:{part_id}): {side.upper()} {qty} {symbol.split('/')[0]} | P/L: {profit:.2f} {CONFIG.usdt_symbol} | Reason: {reason}")
    ...
    def summary(self) -> str:
        ...
        if self.daily_start_equity is not None:
            daily_pnl = (self.initial_equity or self.daily_start_equity) + total_profit - self.daily_start_equity
...
def load_persistent_state() -> bool:
    ...
    try:
        ...
        if state_data.get("pyrmethus_version") != PYRMETHUS_VERSION:
            ...
            return False
        if state_data.get("config_symbol") != CONFIG.exchange["symbol"] or state_data.get("config_strategy") != CONFIG.strategy["name"].value:
        ...
        for part in state_data.get("active_trade_parts", []):
            p_copy = part.copy()
            for key, value in p_copy.items():
                if key in ["entry_price", "sl_price", "tp_price", "qty", "atr_at_entry"] and value is not None:
                    p_copy[key] = Decimal(value)
                elif key in ["recent_pnls", "adverse_candle_closes"] and value is not None:
        ...
        trade_metrics.initial_equity = Decimal(state_data["initial_equity_str"]) if state_data.get("initial_equity_str") else None
        trade_metrics.daily_start_equity = Decimal(state_data["daily_start_equity_str"]) if state_data.get("daily_start_equity_str") else None
        ...
        trade_timestamps_for_whipsaw = deque(state_data.get("trade_timestamps_for_whipsaw", []), maxlen=CONFIG.trading["max_active_trade_parts"])
        ...
        previous_day_high = Decimal(state_data["previous_day_high_str"]) if state_data.get("previous_day_high_str") else None
        previous_day_low = Decimal(state_data["previous_day_low_str"]) if state_data.get("previous_day_low_str") else None
        ...
        logger.info(f"Loaded state: {len(_active_trade_parts)} active parts, {len(trade_metrics.trades)} trades.")
        return True
    except Exception as e:
...
# --- Exchange Functions ---
@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.RateLimitExceeded), tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2)
def fetch_account_balance(exchange: ccxt.Exchange, currency: str) -> Optional[Decimal]:
    ...
    try:
        ...
        total_balance = safe_decimal_conversion(balance.get('total', {}).get(currency, 0))
...
@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.RateLimitExceeded), tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2)
def get_market_data(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int) -> Optional[pd.DataFrame]:
...
def _synchronize_part_sltp(sl_price: Optional[Decimal], tp_price: Optional[Decimal]) -> None:
...
@retry((ccxt.NetworkError, ccxt.RequestTimeout, ccxt.RateLimitExceeded), tries=CONFIG.retry_count, delay=CONFIG.retry_delay_seconds, backoff=2)
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price: Decimal, sl_price: Decimal,
                       tp_price: Optional[Decimal], atr: Decimal, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    try:
        # Placeholder: Implement actual order placement logic
        part_id = str(uuid.uuid4())
        part = {
            "part_id": part_id, "symbol": config.exchange["symbol"], "side": side,
            "entry_price": entry_price, "sl_price": sl_price, "tp_price": tp_price,
            "qty": Decimal("0.01"), "entry_time_ms": int(time.time() * 1000),
            "atr_at_entry": atr, "recent_pnls": deque(maxlen=3),
            "adverse_candle_closes": deque(maxlen=2), "last_trailing_sl_update": time.time()
        }
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part: Dict[str, Any], reason: str,
                        close_price_target: Optional[Decimal] = None) -> bool:
    global _active_trade_parts
    ...
    params = {'reduceOnly': True, 'category': 'linear' if config.exchange["symbol"].endswith(":USDT") else {}}
    try:
        close_order = exchange.create_order(config.exchange["symbol"], 'market', close_side, float(qty), None, params)
        logger.success(f"Unraveling Order {close_order['id']} cast.")
        time.sleep(config.trading["order_fill_timeout_seconds"] / 2)
        filled_order = exchange.fetch_order(close_order['id'], config.exchange["symbol"])
        if filled_order.get('status') == 'closed':
            actual_exit_price = Decimal(str(filled_order.get('average', filled_order.get('price', close_price_target or part['entry_price']))))
            exit_timestamp_ms = int(filled_order.get('timestamp', time.time() * 1000))
            entry_price = part.get('entry_price', Decimal(0))
            pnl = (actual_exit_price - entry_price) * qty if side == config.pos_long else (entry_price - actual_exit_price) * qty
            trade_metrics.log_trade(
                symbol=config.exchange["symbol"], side=side, entry_price=entry_price, exit_price=actual_exit_price,
                qty=qty, entry_time_ms=part['entry_time_ms'], exit_time_ms=exit_timestamp_ms,
                reason=reason, part_id=part_id, pnl_str=str(pnl)
            )
            _active_trade_parts = [p for p in _active_trade_parts if p.get('part_id') != part_id]
            save_persistent_state(force_heartbeat=True)
            logger.success(f"Part {part_id} unraveled. Exit: {actual_exit_price}, PNL: {pnl:.2f} {config.usdt_symbol}")
            send_termux_notification(f"Pyrmethus Position Closed", f"{config.exchange['symbol']} Part {part_id}. PNL: {pnl:.2f}. Reason: {reason}")
            return True
        else:
            logger.warning(f"Unraveling order {close_order['id']} not closed. Status: {filled_order.get('status', 'N/A')}.")
            return False
    except Exception as e:
        logger.error(f"Error unraveling part {part_id}: {e}")
        send_termux_notification("Pyrmethus Close Fail", f"Error closing {config.exchange['symbol']} part {part_id}: {str(e)[:80]}")
        return False
...
def modify_position_sl_tp(exchange: ccxt.Exchange, config: Config, new_sl: Optional[Decimal] = None, new_tp: Optional[Decimal] = None) -> bool:
    ...
    params = {'positionIdx': 0, 'category': 'linear' if config.exchange["symbol"].endswith(":USDT") else {}}
    ...
    if config.risk["enable_take_profit"] and tp_to_set is not None:
        ...
        params['takeProfit'] = '0'
        tp_to_set = None
    logger.info(f"Modifying Position SL/TP: New SL {sl_to_set}, New TP {tp_to_set if config.risk['enable_take_profit'] else 'Disabled'}")
    try:
        exchange.set_trading_stop(config.exchange["symbol"], params=params)
        logger.success(f"Position SL/TP modification successful. SL: {sl_to_set}, TP: {tp_to_set if config.risk['enable_take_profit'] else 'N/A'}")
...
async def stream_price_data(exchange: ccxt_async.Exchange, config: Config, price_queue: asyncio.Queue):
    try:
        while True:
            ...
            latest_close = safe_decimal_conversion(ticker.get('last', ticker.get('close')))
            if latest_close is not None:
                await price_queue.put({"timestamp": int(time.time() * 1000), "close": latest_close})
...
def perform_health_check(exchange: ccxt.Exchange) -> bool:
    ...
    try:
        ...
        send_termux_notification("Pyrmethus Health Alert", f"Exchange API unresponsive: {str(e)[:80]}")
        return False

# --- Main Loop ---
async def main_loop(exchange: ccxt.Exchange, async_exchange: ccxt_async.Exchange, config: Config):
    ...
    logger.info(f"=== Pyrmethus Spell v{PYRMETHUS_VERSION} Awakening on {exchange.id} ===")
        ...
        while True:
            ...
            if current_balance is not None:
                ...
                if current_balance < config.trading["min_usdt_balance"]:
                    logger.warning(f"Treasury ({current_balance:.2f} {config.usdt_symbol}) below prudence ward ({config.trading['min_usdt_balance']}).")
            ...
            if not price_data:
                ohlcv_df = get_market_data(exchange, config.exchange["symbol"], config.exchange["interval"], OHLCV_LIMIT + config.api_fetch_limit_buffer)
            ...
            df_with_indicators = calculate_all_indicators(pd.DataFrame(price_data, columns=['timestamp', 'close']), config)
            atr = safe_decimal_conversion(df_with_indicators.get(f"ATR_{config.strategy.get('atr_calculation_period', 14)}", pd.NA))
            signals = config.strategy_instance.generate_signals(df_with_indicators, latest_close, atr)
            ...
            if current_pos_side == config.pos_none and len(_active_trade_parts) < config.trading["max_active_trade_parts"]:
                if confirmed_enter_long or confirmed_enter_short:
                    entry_side = config.pos_long if confirmed_enter_long else config.pos_short
                    sl_price = (latest_close - (atr * config.risk["atr_stop_loss_multiplier"])) if entry_side == config.pos_long \
                        else (latest_close + (atr * config.risk["atr_stop_loss_multiplier"]))
                    tp_price = (latest_close + (atr * config.risk["atr_take_profit_multiplier"])) if entry_side == config.pos_long \
                        else (latest_close - (atr * config.risk["atr_take_profit_multiplier"])) if config.risk["enable_take_profit"] else None
                    new_part = place_risked_order(exchange, config, entry_side, latest_close, sl_price, tp_price, atr, df_with_indicators)
                    if new_part:
                        persistent_signal_counter = {"long": 0, "short": 0}
                        last_signal_type = None
            for part in list(_active_trade_parts):
                if config.risk["enable_trailing_sl"] and atr > 0:
                    current_pnl = (latest_close - part['entry_price']) * part['qty'] if part['side'] == config.pos_long \
                        else (part['entry_price'] - latest_close) * part['qty']
                    profit_atr = (current_pnl / part['qty']) / atr if part['qty'] > 0 else Decimal(0)
                    if profit_atr >= config.risk["trailing_sl_trigger_atr"]:
                        trailing_sl = (latest_close - (atr * config.risk["trailing_sl_distance_atr"])) if part['side'] == config.pos_long \
                            else (latest_close + (atr * config.risk["trailing_sl_distance_atr"]))
                        if (part['side'] == config.pos_long and trailing_sl > part['sl_price']) or \
                           (part['side'] == config.pos_short and trailing_sl < part['sl_price']):
                            logger.info(f"Updating trailing SL for part {part['part_id']} to {trailing_sl}")
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    global _active_trade_parts
    logger.warning(f"Forceful unraveling of ALL positions for {config.exchange['symbol']} due to: {reason}")
    for part in list(_active_trade_parts):
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.success(f"Market runes for {CONFIG.exchange['symbol']}: Price Tick {CONFIG.tick_size}, Amount Step {CONFIG.qty_step}")
        try:
            leverage_params = {'category': 'linear' if CONFIG.exchange["symbol"].endswith(":USDT") else {}}
            exchange_instance.set_leverage(CONFIG.exchange["leverage"], CONFIG.exchange["symbol"], params=leverage_params)
            logger.success(f"Leverage for {CONFIG.exchange['symbol']} set to {CONFIG.exchange['leverage']}x.")
        except Exception as e:
        ...
        send_termux_notification("Pyrmethus Init Error", f"Exchange init failed: {str(e)[:80]}")
        sys.exit(1)
    finally:
