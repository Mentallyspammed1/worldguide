...
STATE_FILE_NAME = "pyrmethus_phoenix_state_v282.json" # Updated version for the new weave
...
def send_termux_notification(title: str, message: str, notification_id: int = 777) -> None:
    ...
    if 'CONFIG' not in globals() or not CONFIG.enable_notifications:
        if 'CONFIG' in globals(): # Config is loaded but notifications disabled
            ...
            logging.getLogger("TermuxNotification").warning("Attempted Termux notification before CONFIG loaded or with notifications disabled.")
        return
    try:
        ...
        if process.returncode == 0:
            logging.getLogger("TermuxNotification").info(f"{NEON['SUCCESS']}Termux notification '{title}' sent successfully.{NEON['RESET']}")
        else:
        ...
        logging.getLogger("TermuxNotification").error(f"{NEON['ERROR']}Termux API command 'termux-notification' not found. Is 'termux-api' package installed and accessible?{NEON['RESET']}")
    except subprocess.TimeoutExpired:
        logging.getLogger("TermuxNotification").error(f"{NEON['ERROR']}Termux notification command timed out for '{title}'.{NEON['RESET']}")
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.atr_stop_loss_multiplier: Decimal = self._get_env("ATR_STOP_LOSS_MULTIPLIER", "1.5", cast_type=Decimal) # Fallback
    ...
    def _get_env(self, key: str, default: Any = None, cast_type: type = str, required: bool = False, color: str = NEON["PARAM"], secret: bool = False) -> Any:
        ...
        try:
            ...
            if cast_type == bool: final_value = raw_value_str_for_cast.lower() in ["true", "1", "yes", "y"]
                ...
                _logger.warning(f"{NEON['WARNING']}Unsupported cast_type '{cast_type.__name__}' for key '{key}'. Returning raw string: '{raw_value_str_for_cast}'.{NEON['RESET']}")
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.trailing_stop_activation_offset_percent < 0:
             errors.append(f"TRAILING_STOP_ACTIVATION_PRICE_OFFSET_PERCENT ({self.trailing_stop_activation_offset_percent}) must be positive.")
...
class DualSupertrendMomentumStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if primary_long_flip and confirm_is_up is True and momentum_val > self.config.momentum_threshold:
            ...
            self.logger.info(f"{NEON['SIDE_LONG']}DualST+Mom Signal: LONG Entry - ST Flip, Confirm Up, Mom ({_format_for_log(momentum_val)}) > {_format_for_log(self.config.momentum_threshold)}{NEON['RESET']}")
            ...
            self.logger.info(f"{NEON['SIDE_SHORT']}DualST+Mom Signal: SHORT Entry - ST Flip, Confirm Down, Mom ({_format_for_log(momentum_val)}) < -{_format_for_log(self.config.momentum_threshold)}{NEON['RESET']}")
...
class EhlersFisherStrategy(TradingStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if pd.isna(fisher_now) or pd.isna(signal_now) or pd.isna(fisher_prev) or pd.isna(signal_prev):
            self.logger.debug(f"Ehlers Fisher or Signal rune is NA. Fisher: {_format_for_log(fisher_now)}, Signal: {_format_for_log(signal_now)}. No signal.")
            return signals
...
class TradeMetrics:
    ...
    def log_trade(self, symbol: str, side: str, entry_price: Decimal, exit_price: Decimal, qty: Decimal,
                  entry_time_ms: int, exit_time_ms: int, reason: str, part_id: str, pnl_str: str, # Accept pnl_str
        ...
        if not all([isinstance(entry_price, Decimal) and entry_price > 0, 
            ...
            self.logger.warning(f"{NEON['WARNING']}Trade log skipped due to flawed parameters. Part ID: {part_id}{NEON['RESET']}")
            return
        ...
        trade_type = "Scale-In" if scale_order_id else "Part" # Simplified type based on scale_order_id
    ...
    def load_trades_from_list(self, trades_list: List[Dict[str, Any]]):
        ...
        self.logger.info(f"{NEON['INFO']}TradeMetrics: Re-inked {NEON['VALUE']}{len(self.trades)}{NEON['INFO']} trade sagas.{NEON['RESET']}")
    ...
    try:
        ...
        for part in _active_trade_parts:
            ...
            for key, value in p_copy.items():
                ...
                elif isinstance(value, (datetime, pd.Timestamp)): p_copy[key] = value.isoformat()
        ...
        logger.log(log_level, f"{NEON['SUCCESS']}Phoenix Feather: State memories scribed to '{STATE_FILE_NAME}'.{NEON['RESET']}")
    except Exception as e:
...
def load_persistent_state() -> bool:
    ...
    if not os.path.exists(STATE_FILE_PATH):
        logger.info(f"{NEON['INFO']}Phoenix Feather: No previous scroll found. Starting fresh.{NEON['RESET']}")
        return False
    try:
        ...
        if saved_version != "2.8.2": # Check against current version "2.8.2"
             logger.warning(f"{NEON['WARNING']}Phoenix Feather: Scroll version '{saved_version}' differs from current '2.8.2'. Caution advised.{NEON['RESET']}")
                            
        if state_data.get("config_symbol") != CONFIG.symbol or \
           state_data.get("config_strategy") != CONFIG.strategy_name.value:
            logger.warning(f"{NEON['WARNING']}Phoenix Scroll sigils mismatch (symbol/strategy). Current: {CONFIG.symbol}/{CONFIG.strategy_name.value}. Saved: {state_data.get('config_symbol')}/{state_data.get('config_strategy')}. Ignoring old scroll.{NEON['RESET']}")
            archive_name = f"{STATE_FILE_PATH}.archived_{time.strftime('%Y%m%d%H%M%S')}"
            try: os.rename(STATE_FILE_PATH, archive_name); logger.info(f"Archived to: {archive_name}")
            except OSError as e_mv: logger.error(f"Error archiving: {e_mv}")
            return False
        ...
        for part_data in state_data.get("active_trade_parts", []):
            ...
            decimal_keys = ["entry_price", "qty", "sl_price", "initial_usdt_value", "atr_at_entry"]
            for k, v_str_or_val in restored.items():
                if k in decimal_keys and isinstance(v_str_or_val, str):
                    try: restored[k] = Decimal(v_str_or_val)
                    except InvalidOperation: logger.warning(f"Bad Decimal '{v_str_or_val}' for '{k}' in loaded part.")
                elif k == "entry_time_ms":
                    if isinstance(v_str_or_val, str):
                         try: restored[k] = int(datetime.fromisoformat(v_str_or_val.replace("Z", "+00:00")).timestamp() * 1000)
                         except ValueError: 
                             try: restored[k] = int(v_str_or_val) # Try direct int if not ISO
                             except ValueError: logger.warning(f"Bad entry_time_ms '{v_str_or_val}' in loaded part.")
                    elif isinstance(v_str_or_val, (float, int)): restored[k] = int(v_str_or_val)
        ...
        if daily_equity_str and daily_equity_str.lower() != 'none':
            ...
            except InvalidOperation: logger.warning(f"Bad daily_start_equity '{daily_equity_str}' in scroll.")
        trade_metrics.last_daily_reset_day = state_data.get("last_daily_reset_day")
        
        logger.success(f"{NEON['SUCCESS']}Phoenix Feather: Memories reawakened! Active Parts: {len(_active_trade_parts)}, Trades: {len(trade_metrics.trades)}{NEON['RESET']}")
        return True
        ...
        logger.error(f"{NEON['ERROR']}Phoenix Feather Error reawakening: {e}{NEON['RESET']}"); logger.debug(traceback.format_exc())
        return False
...
def calculate_all_indicators(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    if df.empty:
        logger.warning(f"{NEON['WARNING']}Market data scroll empty. No indicators conjured.{NEON['RESET']}")
        return df
    ...
    if config.strategy_name == StrategyName.DUAL_SUPERTREND_MOMENTUM:
        ...
        df.ta.supertrend(length=config.st_atr_length, multiplier=float(config.st_multiplier), append=True, col_names=(st_p, f"{st_p}d", f"{st_p}l", f"{st_p}s"))
        df.ta.supertrend(length=config.confirm_st_atr_length, multiplier=float(config.confirm_st_multiplier), append=True, col_names=(st_c, f"{st_c}d", f"{st_c}l", f"{st_c}s"))
        ...
        else: df["momentum"] = pd.NA; logger.warning("Cannot calculate momentum, 'close' missing/all NaNs.")
        logger.debug(f"Dual ST & Mom. Last Mom: {_format_for_log(df['momentum'].iloc[-1] if 'momentum' in df.columns and not df.empty else pd.NA)}")
        ...
        else: df["ehlers_fisher"], df["ehlers_signal"] = pd.NA, pd.NA; logger.warning("Cannot calc Ehlers, H/L missing/all NaNs.")
        logger.debug(f"Ehlers Fisher. Last Fisher: {_format_for_log(df['ehlers_fisher'].iloc[-1] if 'ehlers_fisher' in df.columns and not df.empty else pd.NA)}")
...
def get_market_data(exchange: ccxt.Exchange, symbol: str, interval: str, limit: int = OHLCV_LIMIT) -> Optional[pd.DataFrame]:
    logger.debug(f"{NEON['COMMENT']}# Summoning {limit} market whispers (candles) for {symbol} ({interval})...{NEON['RESET']}")
    try:
        ohlcv = exchange.fetch_ohlcv(symbol, interval, limit=limit)
        if not ohlcv: logger.warning(f"{NEON['WARNING']}No OHLCV runes returned for {symbol}. The market is silent.{NEON['RESET']}"); return None
        df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms', utc=True)
        for col in ['open', 'high', 'low', 'close', 'volume']: df[col] = pd.to_numeric(df[col], errors='coerce')
        logger.debug(f"Summoned {len(df)} candles. Latest omen: {df['timestamp'].iloc[-1] if not df.empty else 'N/A'}")
        return df
    except Exception as e: logger.error(f"{NEON['ERROR']}Error summoning market whispers for {symbol}: {e}{NEON['RESET']}"); logger.debug(traceback.format_exc()); return None
...
def fetch_account_balance(exchange: ccxt.Exchange, currency_code: str = CONFIG.usdt_symbol) -> Optional[Decimal]:
    logger.debug(f"{NEON['COMMENT']}# Scrying the treasury for {currency_code} balance...{NEON['RESET']}")
    try:
        balance_data = exchange.fetch_balance()
        # Bybit V5 unified account might structure total differently, check 'total' or 'walletBalance' under USDT
        total_balance = safe_decimal_conversion(balance_data.get('total', {}).get(currency_code)) # Try this first
        ...
        if total_balance is None: logger.warning(f"{NEON['WARNING']}{currency_code} balance not found or rune unreadable.{NEON['RESET']}"); return None
        logger.info(f"{NEON['INFO']}Current {currency_code} Treasury: {NEON['VALUE']}{total_balance:.2f}{NEON['RESET']}")
        return total_balance
    except Exception as e: logger.error(f"{NEON['ERROR']}Error scrying treasury: {e}{NEON['RESET']}"); return None
...
def get_current_position_info() -> Tuple[str, Decimal]:
    ...
    total_qty = sum(part.get('qty', Decimal(0)) for part in _active_trade_parts) # Sum quantities of all active parts
    current_side = _active_trade_parts[0]['side'] if total_qty > CONFIG.position_qty_epsilon else CONFIG.pos_none # If total qty is effectively zero, consider flat
...
def calculate_position_size(balance: Decimal, risk_per_trade_pct: Decimal, entry_price: Decimal, sl_price: Decimal, market_info: Dict) -> Optional[Decimal]:
    logger.debug(f"{NEON['COMMENT']}# Calculating alchemical proportions for position size... Balance: {balance:.2f}, Risk %: {risk_per_trade_pct:.4f}{NEON['RESET']}")
    if balance <= 0 or entry_price <= 0 or sl_price <= 0: logger.warning(f"{NEON['WARNING']}Invalid runes for position sizing (balance/prices <=0).{NEON['RESET']}"); return None
    
    risk_per_unit = abs(entry_price - sl_price) # The potential loss per unit of the base asset
    if risk_per_unit == 0: logger.warning(f"{NEON['WARNING']}Risk per unit is zero (entry matches SL). Cannot divine size.{NEON['RESET']}"); return None

    usdt_at_risk = balance * risk_per_trade_pct # Total USDT amount Pyrmethus is willing to risk
    ...
    if position_usdt_value > CONFIG.max_order_usdt_amount:
        logger.info(f"Calculated position USDT value {NEON['VALUE']}{position_usdt_value:.2f}{NEON['RESET']} exceeds MAX_ORDER_USDT_AMOUNT {NEON['VALUE']}{CONFIG.max_order_usdt_amount}{NEON['RESET']}. Capping.")
        position_usdt_value = CONFIG.max_order_usdt_amount
        quantity_base = position_usdt_value / entry_price # Recalculate base quantity based on capped USDT value
    ...
    qty_precision_digits = market_info.get('precision', {}).get('amount') # Number of decimal places for quantity
    ...
    if qty_precision_digits is None: # Fallback if precision not found
        logger.warning(f"{NEON['WARNING']}Quantity precision rune not found for {market_info.get('symbol')}. Using raw calculation.{NEON['RESET']}")
    else:
        qty_precision_digits = int(qty_precision_digits) # Ensure it's int
        if qty_precision_digits >= 0: # If 0, means integer quantity. If >0, decimal places.
            quantizer = Decimal('1e-' + str(qty_precision_digits)) # e.g., 3 -> Decimal('0.001')
            quantity_base = (quantity_base // quantizer) * quantizer # Floor to the nearest step size
    ...
    if min_qty is not None and quantity_base < min_qty: # Check against min order size
        logger.warning(f"{NEON['WARNING']}Calculated quantity {NEON['QTY']}{quantity_base}{NEON['WARNING']} is below min {NEON['QTY']}{min_qty}{NEON['WARNING']}. Cannot place order.{NEON['RESET']}")
        return None
    
    if quantity_base <= CONFIG.position_qty_epsilon: # Effectively zero
        logger.warning(f"{NEON['WARNING']}Final quantity is zero or negligible after adjustments. Cannot place order.{NEON['RESET']}")
        return None
    
    qty_display_format = f".{qty_precision_digits}f" if qty_precision_digits is not None and qty_precision_digits >= 0 else ""
    logger.info(f"Calculated position size: {NEON['QTY']}{quantity_base:{qty_display_format}}{NEON['RESET']} {market_info.get('base', '')}")
...
def place_risked_order(exchange: ccxt.Exchange, config: Config, side: str, entry_price_target: Decimal, sl_price: Decimal, atr_val: Optional[Decimal]) -> Optional[Dict]:
    ...
    if balance is None or balance <= Decimal(10): # Min balance ward
        logger.error(f"{NEON['ERROR']}Insufficient treasury balance or failed to scry. Cannot weave new order.{NEON['RESET']}")
        config.send_notification_method("Pyrmethus Order Fail", "Insufficient balance for new order.")
        return None

    if config.MARKET_INFO is None:
        logger.error(f"{NEON['ERROR']}Market runes (MARKET_INFO) not loaded. Cannot size or place order.{NEON['RESET']}")
        return None

    quantity = calculate_position_size(balance, config.risk_per_trade_percentage, entry_price_target, sl_price, config.MARKET_INFO)
    if quantity is None or quantity <= 0: return None # Sizing failed or resulted in zero quantity
    ...
    params = {
        ...
        'stopLoss': float(sl_price), # Set SL price directly with the order (Bybit V5 feature)
    ...
    try:
        order_price_param = float(entry_price_target) if order_type_str == 'Limit' else None # Price param only for Limit
        logger.info(f"Weaving {order_type_str.upper()} {side.upper()} order: Qty {NEON['QTY']}{quantity}{NEON['RESET']}, Symbol {config.symbol}, SL {NEON['PRICE']}{sl_price}{NEON['RESET']}")
        
        # Ensure quantity is float for ccxt
        order = exchange.create_order(config.symbol, order_type_str, side, float(quantity), order_price_param, params)
        
        logger.success(f"{NEON['SUCCESS']}Entry Order cast forth: ID {order['id']}, Status {order.get('status', 'N/A')}{NEON['RESET']}")
        ...
        time.sleep(config.order_fill_timeout_seconds / 3) # Partial timeout to check status
        filled_order = exchange.fetch_order(order['id'], config.symbol)
        
        if filled_order.get('status') == 'closed': # 'closed' indicates filled for Bybit
            ...
            new_part = {
                "part_id": part_id, "entry_order_id": order['id'], 
                ...
                "sl_price": sl_price, "atr_at_entry": atr_val, # Store ATR at entry for reference
                "initial_usdt_value": actual_qty * actual_entry_price # Notional value at entry
            }
            _active_trade_parts.append(new_part)
            save_persistent_state(force_heartbeat=True) # Scribe this new venture
            logger.success(f"{NEON['SUCCESS']}Order filled! Part {part_id} forged. Entry: {NEON['PRICE']}{actual_entry_price}{NEON['RESET']}, Qty: {NEON['QTY']}{actual_qty}{NEON['RESET']}{NEON['RESET']}")
            config.send_notification_method(f"Pyrmethus Order Filled: {side.upper()}", f"{config.symbol} Part {part_id} @ {actual_entry_price}")
            return new_part
        else:
            logger.warning(f"{NEON['WARNING']}Order {order['id']} not immediately filled. Status: {filled_order.get('status')}. Manual check advised. Consider cancellation for Limit orders.{NEON['RESET']}")
        ...
        logger.error(f"{NEON['ERROR']}Insufficient funds in treasury to weave order: {e}{NEON['RESET']}")
        ...
        logger.error(f"{NEON['ERROR']}Exchange rejected the spell (order): {e}{NEON['RESET']}")
        ...
        if "Stop loss price is too close" in str(e): # More specific handling
            logger.warning(f"{NEON['WARNING']}SL price {sl_price} likely too close to entry {entry_price_target}. Adjust ATR multiplier or spread.{NEON['RESET']}")
        return None
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Unexpected chaos while weaving order: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
        return None
...
def close_position_part(exchange: ccxt.Exchange, config: Config, part_to_close: Dict, reason: str, close_price_target: Optional[Decimal] = None) -> bool:
    ...
    global _active_trade_parts
    logger.info(f"{NEON['ACTION']}Attempting to unravel trade part {part_to_close['part_id']} ({part_to_close['side']} {part_to_close['qty']} {config.symbol}) for reason: {reason}{NEON['RESET']}")
    ...
    try:
        ...
        logger.info(f"Casting MARKET {close_side.upper()} order to unravel part {part_to_close['part_id']}: Qty {NEON['QTY']}{qty_to_close}{NEON['RESET']}")
        
        # Ensure quantity is float
        close_order = exchange.create_order(config.symbol, 'market', close_side, float(qty_to_close), None, params)
        
        logger.success(f"{NEON['SUCCESS']}Unraveling Order cast forth: ID {close_order['id']}, Status {close_order.get('status', 'N/A')}{NEON['RESET']}")
        ...
        if filled_close_order.get('status') == 'closed': # Filled
            ...
            trade_metrics.log_trade(
                ...
                reason=reason, part_id=part_to_close['part_id'], pnl_str=str(pnl) # Pass calculated PNL as string
            )
            ...
            logger.success(f"{NEON['SUCCESS']}Part {part_to_close['part_id']} unraveled. Exit: {NEON['PRICE']}{actual_exit_price}{NEON['RESET']}, PNL: {NEON[('PNL_POS' if pnl > 0 else ('PNL_NEG' if pnl < 0 else 'PNL_ZERO'))]}{pnl:.2f}{NEON['RESET']}")
            config.send_notification_method(f"Pyrmethus Position Closed", f"{config.symbol} Part {part_to_close['part_id']} closed. PNL: {pnl:.2f}")
            return True
        else:
            logger.warning(f"{NEON['WARNING']}Unraveling order {close_order['id']} for part {part_to_close['part_id']} not filled. Status: {filled_close_order.get('status')}. Manual check required.{NEON['RESET']}")
            return False

    except Exception as e:
        logger.error(f"{NEON['ERROR']}Error unraveling position part {part_to_close['part_id']}: {e}{NEON['RESET']}")
        logger.debug(traceback.format_exc())
...
def cancel_all_symbol_orders(exchange: ccxt.Exchange, symbol: str):
    ...
    logger.info(f"{NEON['ACTION']}Dispelling all open enchantments (orders) for {symbol}...{NEON['RESET']}")
    try:
        ...
        logger.success(f"{NEON['SUCCESS']}All open enchantments for {symbol} dispelled.{NEON['RESET']}")
    except Exception as e:
        logger.error(f"{NEON['ERROR']}Error dispelling enchantments for {symbol}: {e}{NEON['RESET']}")
...
def close_all_symbol_positions(exchange: ccxt.Exchange, config: Config, reason: str):
    ...
    global _active_trade_parts
    logger.warning(f"{NEON['WARNING']}Unraveling ALL positions for {config.symbol} due to: {reason}{NEON['RESET']}")
    
    # Close parts known to Pyrmethus
    parts_to_close_copy = list(_active_trade_parts) # Iterate over a copy as list is modified
    ...
    try:
        ...
        for pos_data in positions:
            if pos_data and safe_decimal_conversion(pos_data.get('contracts', '0')) != Decimal(0): # 'contracts' or 'size'
                ...
                if pos_symbol == config.symbol and pos_qty > CONFIG.position_qty_epsilon:
                    side_to_close_market = config.side_sell if pos_side == 'long' else config.side_buy
                    logger.warning(f"Found residual exchange position: {pos_side} {NEON['QTY']}{pos_qty}{NEON['RESET']} {config.symbol}. Attempting forceful market unraveling.")
                    exchange.create_order(config.symbol, 'market', side_to_close_market, float(pos_qty), params={'reduceOnly': True})
                    logger.info(f"Residual position unraveling order cast for {NEON['QTY']}{pos_qty}{NEON['RESET']} {config.symbol}.")
    ...
    cancel_all_symbol_orders(exchange, config.symbol) # Also dispel any straggling conditional orders
...
def main_loop(exchange: ccxt.Exchange, config: Config) -> None:
    ...
    else: logger.info(f"{NEON['INFO']}No valid prior state. Starting with a fresh essence.{NEON['RESET']}")
    ...
    else: logger.warning(f"{NEON['WARNING']}Could not set initial treasury reading. Balance scrying failed.{NEON['RESET']}")

    while True:
        try:
            logger.debug(f"{NEON['COMMENT']}# New cycle of observation... The market's currents shift...{NEON['RESET']}")
            current_balance = fetch_account_balance(exchange, config.usdt_symbol) # Update balance each cycle
            if current_balance is not None:
                trade_metrics.set_initial_equity(current_balance) # This also handles daily reset for drawdown
                drawdown_hit, dd_reason = trade_metrics.check_drawdown(current_balance)
                if drawdown_hit:
                    logger.critical(f"{NEON['CRITICAL']}Max drawdown ward triggered! {dd_reason}. Pyrmethus must rest to conserve essence.{NEON['RESET']}")
                    close_all_symbol_positions(exchange, config, "Max Drawdown Reached")
                    break # Exit the main loop, ending the spell for now
            else: logger.warning(f"{NEON['WARNING']}Failed to scry treasury balance this cycle.{NEON['RESET']}")

            # Fetch market data (OHLCV)
            ohlcv_df = get_market_data(exchange, config.symbol, config.interval, limit=OHLCV_LIMIT)
            if ohlcv_df is None or ohlcv_df.empty:
                logger.warning(f"{NEON['WARNING']}Market whispers are faint (no data). Retrying after a brief pause.{NEON['RESET']}")
            ...
            if df_with_indicators.empty or df_with_indicators.iloc[-1].isnull().all(): # Check if last row is all NaN
                logger.warning(f"{NEON['WARNING']}Indicator alchemy yielded no clear runes or latest runes are faint. Pausing.{NEON['RESET']}")
                time.sleep(config.sleep_seconds); continue

            # Generate trading signals from the chosen strategy
            signals = config.strategy_instance.generate_signals(df_with_indicators)
            logger.debug(f"Divine Omens (Signals): EnterL={signals['enter_long']}, EnterS={signals['enter_short']}, ExitL={signals['exit_long']}, ExitS={signals['exit_short']}")

            # Get current position state from Pyrmethus's memory
            current_pos_side, current_pos_qty = get_current_position_info()
            logger.debug(f"Current Stance: Side={NEON[('SIDE_LONG' if current_pos_side == config.pos_long else ('SIDE_SHORT' if current_pos_side == config.pos_short else 'SIDE_FLAT'))]}{current_pos_side}{NEON['RESET']}, Qty={NEON['QTY']}{current_pos_qty}{NEON['RESET']}")
            ...
            if latest_atr is None or latest_close is None:
                logger.warning(f"{NEON['WARNING']}Missing latest ATR or Close price runes. Cannot make trade decisions.{NEON['RESET']}")
                time.sleep(config.sleep_seconds); continue
            if latest_atr <= Decimal(0): # ATR should be positive
                logger.warning(f"{NEON['WARNING']}ATR rune ({latest_atr}) is not positive. SL calculation would be flawed. Skipping.{NEON['RESET']}")
                time.sleep(config.sleep_seconds); continue


            # --- Position Management Decisions ---
            if current_pos_side == config.pos_none: # If FLAT (no active position parts)
                if signals.get("enter_long"):
                    logger.info(f"{NEON['ACTION']}Favorable omens for a LONG venture detected.{NEON['RESET']}")
                    ...
                    logger.info(f"{NEON['ACTION']}Shadowy omens for a SHORT venture detected.{NEON['RESET']}")
                    sl_price = latest_close + (latest_atr * config.atr_stop_loss_multiplier)
                ...
                if signals.get("exit_long"):
                    logger.info(f"{NEON['ACTION']}Omen to EXIT LONG position: {signals.get('exit_reason', 'Strategy Exit Signal')}{NEON['RESET']}")
                    # Close all parts of the long position
                    for part in list(_active_trade_parts): # Iterate copy as list might change
                ...
                if signals.get("exit_short"):
                    logger.info(f"{NEON['ACTION']}Omen to EXIT SHORT position: {signals.get('exit_reason', 'Strategy Exit Signal')}{NEON['RESET']}")
            ...
            logger.debug(f"{NEON['COMMENT']}# Cycle complete. Pyrmethus rests, observing the aether for {config.sleep_seconds}s...{NEON['RESET']}")
            ...
            break
        except Exception as e: # Catch-all for unexpected errors in the main loop's weave
...
if __name__ == "__main__":
    logger.info(f"{NEON['COMMENT']}# Pyrmethus prepares to breach the veil to the exchange realm...{NEON['RESET']}")
    ...
    try:
        ...
        if CONFIG.PAPER_TRADING_MODE:
            logger.warning(f"{NEON['WARNING']}PAPER_TRADING_MODE is True. Ensure API keys are for Bybit TESTNET and endpoints are correct.{NEON['RESET']}")
        ...
        exchange = ccxt.bybit(exchange_params) # Ensure 'bybit' is the correct CCXT ID for Bybit V5
        ...
        CONFIG.MARKET_INFO = markets[CONFIG.symbol] # Store an imprint of the market's rules
        price_prec = CONFIG.MARKET_INFO.get('precision', {}).get('price', 'N/A')
        amount_prec = CONFIG.MARKET_INFO.get('precision', {}).get('amount', 'N/A')
        logger.success(f"{NEON['SUCCESS']}Market runes for {CONFIG.symbol} loaded. Precision: Price {price_prec}, Amount {amount_prec}{NEON['RESET']}")
        ...
        try:
            ...
            if hasattr(exchange, 'set_margin_mode') and CONFIG.MARKET_INFO.get('margin', False): # If margin trading, check mode
            ...
            logger.info(f"Attempting to set leverage to {CONFIG.leverage}x for {CONFIG.symbol} (Category: {category})")
            response = exchange.set_leverage(CONFIG.leverage, CONFIG.symbol, params_leverage)
            logger.success(f"{NEON['SUCCESS']}Leverage for {CONFIG.symbol} set/confirmed to {CONFIG.leverage}x. Response: {response}{NEON['RESET']}")
        ...
        logger.success(f"{NEON['SUCCESS']}Successfully connected to the exchange realm: {exchange.id}{NEON['RESET']}")
    ...
    except ccxt.AuthenticationError as e: logger.critical(f"{NEON['CRITICAL']}Authentication with exchange failed! Check API key runes: {e}{NEON['RESET']}"); CONFIG.send_notification_method("Pyrmethus Auth Fail", "API Key Auth Error."); sys.exit(1)
    except ccxt.NetworkError as e: logger.critical(f"{NEON['CRITICAL']}Network Error connecting to exchange: {e}. Check connectivity.{NEON['RESET']}"); CONFIG.send_notification_method("Pyrmethus Network Error", f"Cannot connect: {str(e)[:80]}"); sys.exit(1)
    ...
    except Exception as e: logger.critical(f"{NEON['CRITICAL']}Failed to initialize exchange connection: {e}{NEON['RESET']}"); logger.debug(traceback.format_exc()); CONFIG.send_notification_method("Pyrmethus Init Error", f"Exchange init failed: {str(e)[:80]}"); sys.exit(1)
    
    if exchange: # Only proceed if the connection to the exchange realm was successful
        main_loop(exchange, CONFIG)
    else:
        logger.critical(f"{NEON['CRITICAL']}Exchange not initialized. Pyrmethus cannot weave spells in an empty void. Spell aborted.{NEON['RESET']}")
