    ...
    def _get_env(
        ...
        if value_to_cast is None:
            if required:
                ...
                raise ValueError(
                    f"Required environment variable '{key}' resolved to None during casting preparation."
                )
            else:
                logger.debug(
        ...
        try:
            raw_value_str = (
                str(value_to_cast).strip()
            )  # Ensure string representation and remove leading/trailing whitespace
            if cast_type == bool:
        ...
        try:
            ...
            temp_config_for_sms = type(
                ...
                (object,),
                {
                    ...
                    ),  # Use fallback for symbol in SMS
...
def format_price(
    exchange: ccxt.Exchange, symbol: str, price: float | Decimal | int
) -> str:
    ...
    if (
        price_decimal.is_zero() and Decimal(str(price)).is_zero()
    ):  # Handle price is actually 0
        return "0"
    ...
    try:
        ...
        if (
            safe_decimal_conversion(formatted_price).is_zero()
            and not price_decimal.is_zero()
        ):
            logger.warning(
...
def format_amount(
    exchange: ccxt.Exchange, symbol: str, amount: float | Decimal | int
) -> str:
    ...
    if (
        amount_decimal.is_zero() and Decimal(str(amount)).is_zero()
    ):  # Handle amount is actually 0
        return "0"
    ...
    try:
        ...
        # Use Decimal comparison to avoid float issues
        if (
            safe_decimal_conversion(formatted_amount).is_zero()
            and not amount_decimal.is_zero()
        ):
            logger.warning(
    ...
    try:
        ...
        try:
            ...
            total_equity = safe_decimal_conversion(
                balance.get("total", {}).get(CONFIG.usdt_symbol)
            )
            if (
            ...
            send_sms_alert(
                f"[{CONFIG.strategy_name}] Portal opened & authenticated on {CONFIG.symbol}."
            )
            return exchange
...
def find_pandas_ta_column(
    ...
    matching_cols = [
        col for col in df.columns if prefix_hint in col and col.endswith(suffix_hint)
    ]

    if not matching_cols:
        logger.debug(
            f"pandas_ta Finder: No columns found matching prefix '{prefix_hint}' and suffix '{suffix_hint}'."
        )
        return None
        ...
        return matching_cols[
            -1
        ]  # Heuristic: assume the most recent is the intended one
    elif len(matching_cols) < expected_count:
        logger.warning(
...
def analyze_volume_atr(
    ...
    try:
        ...
        df_cleaned_vol["volume"] = pd.to_numeric(
            df_cleaned_vol["volume"], errors="coerce"
        )
        ...
        df.drop(
            columns=[volume_ma_col_name], errors="ignore", inplace=True
        ...
        if results["atr"] is None:
            logger.warning(
                f"{Fore.YELLOW}Scrying (ATR): Final ATR result is NA.{Style.RESET_ALL}"
            )
        if results["volume_ma"] is None:
            logger.warning(
                f"{Fore.YELLOW}Scrying (Volume MA): Final Volume MA result is NA.{Style.RESET_ALL}"
            )
        if results["last_volume"] is None:
            logger.warning(
                f"{Fore.YELLOW}Scrying (Volume): Final Last Volume result is NA.{Style.RESET_ALL}"
            )
        if results["volume_ratio"] is None:
            logger.warning(
                f"{Fore.YELLOW}Scrying (Volume Ratio): Final Volume Ratio result is NA.{Style.RESET_ALL}"
            )
...
def calculate_stochrsi_momentum(
    ...
    try:
        ...
        stochrsi_k_col = find_pandas_ta_column(
            df, "STOCHRSIk_", suffix_hint=f"_{stoch_len}_{rsi_len}_{k}_{d}"
        )
        stochrsi_d_col = find_pandas_ta_column(
            df, "STOCHRSId_", suffix_hint=f"_{stoch_len}_{rsi_len}_{k}_{d}"
        )
        ...
        cols_to_drop = [
            c for c in set(df.columns) - initial_columns if c not in target_cols
        ]
...
def calculate_ehlers_fisher(df: pd.DataFrame, length: int, signal: int) -> pd.DataFrame:
    ...
    try:
        ...
        fisher_col = find_pandas_ta_column(
            df, "FISHERT_", suffix_hint=f"_{length}_{signal}"
        )
        signal_col = find_pandas_ta_column(
            df, "FISHERTs_", suffix_hint=f"_{length}_{signal}"
        )
        ...
        if signal_col and signal_col in df.columns:
            ...
            if (
                ...
                and pd.notna(df[fisher_col].iloc[-1])
            ):
                logger.debug(
                    f"Ehlers Fisher signal length is 1, using Fisher line '{fisher_col}' as signal."
                )
                df["ehlers_signal"] = df[
                    "ehlers_fisher"
    ...
    *** WARNING: This uses standard EMAs and is NOT an Ehlers Super Smoother filter. ***
    ...
    try:
        ...
        if pd.notna(fast_val) and pd.notna(slow_val):
            ...
            logger.debug(
                "Scrying (EMA Cross): Resulted in NA for one or more values on last candle."
            )
    except Exception as e:
        logger.error(
...
def analyze_order_book(
    ...
    try:
        ...
        bid_vol = sum(
            safe_decimal_conversion(bid[1], Decimal("0.0"))
            for bid in bids[: min(depth, len(bids))]
            if len(bid) >= 2
        )
        ask_vol = sum(
            safe_decimal_conversion(ask[1], Decimal("0.0"))
...
def get_market_data(
    ...
    try:
        ...
        if len(ohlcv) < limit:
            ...
            if (
                len(ohlcv) < CONFIG.api_fetch_limit_buffer
            ):  # Basic check if data is critically short
                logger.error(
        ...
        if len(df) > 0:
            ...
            if (
                time_diff_seconds < interval_seconds * 0.8
            ):  # If last candle is less than 80% through interval ago
                logger.warning(
    ...
    try:
        ...
        relevant_position = next(
            (
                p
                for p in positions
                if str(p.get("info", {}).get("positionIdx", "")) == "0"
        ...
        if (
            relevant_position
            and safe_decimal_conversion(
                relevant_position.get("info", {}).get("size", "0")
            )
            ...
            pos_avg_entry = safe_decimal_conversion(
                pos_info.get("entryPrice", "0")
            )  # Entry price
            pos_leverage = safe_decimal_conversion(
                pos_info.get("leverage", "0")
            )  # Effective leverage
            ...
            pos_stop_loss = safe_decimal_conversion(
                pos_info.get("stopLoss", None)
            )  # Native SL price
            pos_take_profit = safe_decimal_conversion(
                pos_info.get("takeProfit", None)
            )  # Native TP price
            ...
            pos_unrealized_pnl = safe_decimal_conversion(
                pos_info.get("unrealisedPnl", None)
            )  # Unrealized PNL
            pos_liquidation_price = safe_decimal_conversion(
                pos_info.get("liqPrice", None)
            )  # Liquidation price
            ...
            if (
                pos_trailing_stop_price is not None
                and not pos_trailing_stop_price.is_zero()
            ):
                stop_details.append(
...
def calculate_order_quantity(
    ...
    try:
        ...
        market = exchange.market(
            symbol
        )  # Market details already loaded in initialize_exchange
        min_amount = safe_decimal_conversion(
            market.get("limits", {}).get("amount", {}).get("min", "0")
        )
        amount_step = safe_decimal_conversion(
            market.get("limits", {}).get("amount", {}).get("step", "0")
        )
        ...
        if (
            amount_step > CONFIG.position_qty_epsilon
            and final_quantity_dec > CONFIG.position_qty_epsilon
        ):
        ...
        estimated_margin_required = (
            final_quantity_dec * current_price_dec * estimated_margin_rate
        )
        logger.debug(
        ...
        try:
            ...
            free_balance_usdt = safe_decimal_conversion(
                balance.get("free", {}).get(CONFIG.usdt_symbol)
            )
            logger.debug(
                f"Qty Calc: Available Free Balance: {free_balance_usdt.normalize()} {CONFIG.usdt_symbol}"
            )
...
def create_order(
    ...
    formatted_price = (
        format_price(exchange, symbol, price) if price is not None else None
    )
    formatted_stop_loss = (
    ...
    if (
        ...
        if (
            CONFIG.trailing_stop_activation_offset_percent > CONFIG.position_qty_epsilon
            and price is not None
        ):
            offset_factor = (
                Decimal("1") + CONFIG.trailing_stop_activation_offset_percent
            )
            ...
            logger.debug(
                f"Create Order: Calculated TSL activation price: {params['activationPrice']}"
            )
        else:
    ...
    try:
        ...
        order = exchange.create_order(
            ...
            amount=float(
                amount
            ),  # CCXT expects float for amount/price in the main call
        ...
        if type == "market":
            ...
            try:
                ...
                retries = 0
                while (
                    retries < CONFIG.order_fill_timeout_seconds / 2
                ):  # Poll approx every 2 seconds
                    time.sleep(2)
                ...
                if filled_order:
                    filled_qty = safe_decimal_conversion(
                        filled_order.get("filled", "0")
                    )
                    avg_price = safe_decimal_conversion(
                        filled_order.get("average", "0")
                    )
                    if (
                        filled_qty
                        >= amount * (Decimal("1") - CONFIG.position_qty_epsilon)
                    ...
                    return (
                        fetched_order
                        if "fetched_order" in locals() and fetched_order
                        else order
                    )
        ...
        logger.warning(
            f"{Fore.YELLOW}Order Failed ({symbol}): Network error - {e}. Check connection.{Style.RESET_ALL}"
        )
        ...
        logger.error(
            f"{Fore.RED}Order Failed ({symbol}): Unexpected error - {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
        send_sms_alert(
...
def confirm_stops_attached(
    ...
    tsl_attached = (
        expected_tsl_percentage is None
        or expected_tsl_percentage <= CONFIG.position_qty_epsilon
    ...
    for attempt in range(1, attempts + 1):
        ...
        if (
            not tp_attached
            and expected_tp_price is not None
    ...
    if (
        expected_tsl_percentage is not None
        and expected_tsl_percentage > 0
        and not tsl_attached
    ):
        missing_stops.append("TSL")

    if missing_stops:
        logger.error(
            f"{Fore.RED}Confirm Stops: Missing stops: {', '.join(missing_stops)}{Style.RESET_ALL}"
        )
        send_sms_alert(
        ...
        try:
            ...
            while (
                ...
                if fetched_order and fetched_order.get("status") == "closed":
                    ...
                    logger.debug(
                        f"Close market order {format_order_id(order_id)} detected as 'closed'."
                    )
                    break
                retries += 1
            ...
            if filled_close_order:
                filled_qty = safe_decimal_conversion(
                    filled_close_order.get("filled", "0")
                )
                avg_price = safe_decimal_conversion(
                    filled_close_order.get("average", "0")
                )
                if (
                ...
                if (
                    post_close_pos["side"] == CONFIG.pos_none
                    or post_close_pos["qty"] <= CONFIG.position_qty_epsilon
                ):
                    logger.success(
                        f"{Fore.GREEN}Position confirmed closed after fill check timeout.{Style.RESET_ALL}"
                    )
...
def generate_trading_signal(
    ...
    if CONFIG.strategy_name == "DUAL_SUPERTREND":
        ...
        if (
            primary_flip_long
            and confirm_is_uptrend
            and last_price > safe_decimal_conversion(confirm_st_val)
        ):
        ...
        primary_is_uptrend = last_candle.get(
            "trend", pd.NA
        )  # Get primary trend for exit check
        primary_st_val = last_candle.get(
            "supertrend", pd.NA
        )  # Get primary ST value for exit check
        ...
        if pd.notna(primary_is_uptrend) and pd.notna(primary_st_val):
            ...
            if position_side == CONFIG.pos_long and (
                primary_flip_short
                or last_price < safe_decimal_conversion(primary_st_val)
            ):
                exit_signal = "close_long"
                if primary_flip_short:
                    logger.debug(
                        "DUAL_ST: Primary ST flipped Short. Close Long signal generated."
                    )
                else:
                    logger.debug(
            ...
            # Close Short: Have Short position AND Primary ST flips long OR price crosses above Primary ST
                ...
                if primary_flip_long:
                    logger.debug(
                        "DUAL_ST: Primary ST flipped Long. Close Short signal generated."
                    )
                else:
                    logger.debug(
        ...
        if (
            stoch_k > stoch_d
            and stoch_k.shift(1, fill_value=stoch_k)
            <= stoch_d.shift(1, fill_value=stoch_d)
        ...
        if position_side == CONFIG.pos_long and (
            ...
            if momentum < -CONFIG.position_qty_epsilon:
                logger.debug(
                    f"STOCHRSI_MOM: Momentum ({momentum:.4f}) turned negative. Close Long signal generated."
                )
            else:
                logger.debug(
        ...
        # Close Short: Have Short position AND (Momentum turns positive OR Stoch %K crosses above %D below oversold)
            ...
            if momentum > CONFIG.position_qty_epsilon:
                logger.debug(
                    f"STOCHRSI_MOM: Momentum ({momentum:.4f}) turned positive. Close Short signal generated."
                )
            else:
                logger.debug(
        ...
        if (
            pd.isna(fast_ema)
            or pd.isna(slow_ema)
            or pd.isna(prev_fast_ema)
            or pd.isna(prev_slow_ema)
        ):
            missing = []
            if pd.isna(fast_ema):
...
def check_entry_filters(
    df: pd.DataFrame, signal_side: str, order_book_data: dict[str, Decimal | None]
) -> bool:
    ...
    if CONFIG.require_volume_spike_for_entry:
        volume_ratio = order_book_data.get(
            "volume_ratio"
        )  # Volume ratio is calculated with ATR/Vol analysis
        if volume_ratio is None:
            logger.debug(
                "Filters: Volume spike required, but volume ratio is NA. Filter FAIL."
            )
    ...
    if (
        ...
        if order_book_ratio is None:
            logger.debug(
                "Filters: Order Book filter enabled, but ratio is NA. Filter FAIL."
            )
            return False  # Cannot check if data is missing
...
def main_trade_logic(exchange: ccxt.Exchange) -> None:
    ...
    try:
        ...
        if (
            not market.get("spot")
            and not market.get("margin")
            and not market.get("futures")
            and not market.get("swap")
        ):
            logger.warning(
    ...
    if (
        "DUAL_SUPERTREND" in CONFIG.valid_strategies
    ):  # Check against all valid strategies, not just selected
        max_indicator_length = max(
            max_indicator_length, CONFIG.st_atr_length, CONFIG.confirm_st_atr_length
        )
    ...
    candles_needed = (
        max_indicator_length + CONFIG.api_fetch_limit_buffer + 10
    )  # Add extra buffer
    ...
    while running:
        try:
            ...
            vol_spike_check = (
                "YES"
                if last_volume_ratio is not None
                and last_volume_ratio >= CONFIG.volume_spike_threshold
                else "NO"
            )
            logger.info(
            ...
            ob_filter_state = (
                f"Ratio(B/A)={ob_ratio:.3f}" if ob_ratio is not None else "Ratio=N/A"
            )
            ob_spread_state = (
                f"Spread={ob_spread:.4f}" if ob_spread is not None else "Spread=N/A"
            )
            logger.info(
            ...
            if position_side != CONFIG.pos_none:
                ...
                if current_position.get("stop_loss") is not None:
                    pos_stops_details.append(
                        f"SL={current_position['stop_loss'].normalize():.4f}"
                    )
                if current_position.get("take_profit") is not None:
                    pos_stops_details.append(
                        f"TP={current_position['take_profit'].normalize():.4f}"
                    )
                if current_position.get("trailing_stop_price") is not None:
                    pos_stops_details.append(
                        f"TSL(Trig)={current_position['trailing_stop_price'].normalize():.4f}"
                    )
            ...
            if signal == "long" and position_side == CONFIG.pos_none:
                ...
                if order_quantity is not None:
                    ...
                    if created_order:
                        ...
                        current_position_after_entry = get_current_position(
                            exchange, CONFIG.symbol
                        )
                        if current_position_after_entry[
                            "side"
                        ] == CONFIG.pos_long and current_position_after_entry[
                            "qty"
                        ] >= order_quantity * (
                            Decimal("1") - CONFIG.position_qty_epsilon
                        ):  # Check if position quantity is close to order quantity
                            logger.success(
                                f"{Fore.GREEN}Position confirmed active after long entry.{Style.RESET_ALL}"
                            )
                ...
                if order_quantity is not None:
                    ...
                    if created_order:
                        ...
                        if current_position_after_entry[
                            "side"
                        ] == CONFIG.pos_short and current_position_after_entry[
                            "qty"
                        ] >= order_quantity * (
                            Decimal("1") - CONFIG.position_qty_epsilon
                        ):  # Check if position quantity is close to order quantity
                            logger.success(
                                f"{Fore.GREEN}Position confirmed active after short entry.{Style.RESET_ALL}"
                            )
                            # Confirm native stops are attached
                            confirm_stops_attached(
                                exchange,
                ...
                logger.info(
                    "Holding Cash. No entry signal generated by strategy or filters, or no exit signal needed."
                )
            ...
            logger.error(
                f"{Fore.RED}Major Exchange Error during cycle: {e}. Retrying after delay...{Style.RESET_ALL}"
            )
            send_sms_alert(
...
def graceful_shutdown(exchange: ccxt.Exchange | None) -> None:
    ...
    send_sms_alert(
        f"[{CONFIG.strategy_name}/{CONFIG.symbol}] Initiating graceful shutdown."
    )
    ...
    if is_live:
        logger.warning(
            f"{Back.YELLOW}{Fore.BLACK}Shutdown operating in LIVE mode.{Style.RESET_ALL}"
        )
    else:
    ...
    logger.warning(
        f"Shutdown Step 1: Cancelling all open orders for {CONFIG.symbol}...{Style.RESET_ALL}"
    )
    if exchange:
        try:
            cancel_attempts = cancel_all_orders_for_symbol(
        ...
        logger.warning(
            "Shutdown Step 1: Exchange object not initialized. Cannot cancel orders.{Style.RESET_ALL}"
        )

    # --- Step 2: Close Active Position ---
    logger.warning(
        f"Shutdown Step 2: Checking for active position to close...{Style.RESET_ALL}"
    )
    if exchange:
        try:
            ...
            if (
                current_position["side"] != CONFIG.pos_none
                and current_position["qty"] > CONFIG.position_qty_epsilon
            ):
                logger.info(
    ...
    logger.info(
        f"{Fore.BLUE}--- Pyrmethus Scalping Spell Deactivated ---{Style.RESET_ALL}"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(
            f"{Back.RED}{Fore.WHITE}An unhandled exception caused the spell to fail: {e}{Style.RESET_ALL}"
        )
        logger.debug(traceback.format_exc())
