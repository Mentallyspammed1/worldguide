...
import { OrderbookData } from '../../twin-range-bot/src/services/bybitService';
...
interface TradeRecord {
  ...
  fee: number; // Add fee to trade record
}
...
export class AdvancedMarketMakingStrategy {
  ...
  private basePrices: { [key: string]: number } = {}; // Current price per symbol
  ...
  private positions: { [key: string]: number } = {}; // Current position per symbol
  private trades: { [key: string]: TradeRecord[] } = {}; // Organized by symbol
  private equityCurve: number[] = [];

  // ATR calculation properties
  private previousPrices: { [key: string]: { high: number; low: number; close: number; atr?: number } } = {};
  ...
  private simulateOrder(symbol: string, side: 'Buy' | 'Sell', price: number, qty: number) {
  ...
  async runBacktest(historicalDataInput?: HistoricalData) {
