...
import { BotConfig, TradeState, KlineData, OrderbookData, Execution, OrderData, PositionData } from '../../types';
...
export class Backtester {
  ...
  constructor(config: Partial<BotConfig> = {}) {
    ...
    this.geminiService = new GeminiService(process.env.GEMINI_API_KEY || null, this.config);
  ...
  private simulateExecution(symbol: string, order: OrderData): Execution | null {
    ...
    if ((parseFloat(order.price) >= price && order.side === 'Buy') || (parseFloat(order.price) <= price && order.side === 'Sell')) {
    ...
    while (this.currentTimestamp <= endTime) {
      for (const symbol of this.config.symbols) {
        ...
        if (kline && orderbook) {
          ...
          const signal = await this.geminiService.analyzeMarketData(symbol, this.mockData.klines[symbol].slice(-5));
          logger.debug('Gemini signal', { symbol, signal });
          ...
          if (executions.length) this.bot.handleExecutionUpdate(executions);
