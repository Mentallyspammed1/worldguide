...
Below are 25 suggestion snippets for improving the provided `kbot2.8.py` trading bot script. Each snippet addresses specific areas for enhancement, such as performance, robustness, security, usability, or maintainability. The suggestions include code modifications, configuration additions, or procedural improvements, all wrapped in `<xaiArtifact/>` tags as per the requirements. Each artifact has a unique UUID for `artifact_id`, a descriptive `title`, and the appropriate `contentType`.

---

### 1. Add Configuration for Maximum Position Size
Limit the maximum position size to prevent overexposure.

```python
# In TradingConfig.__init__
...
### 2. Implement Cooldown Period After Trades
Add a cooldown period after a trade to avoid rapid re-entry.
...
# In trading_spell_cycle, before executing trades
last_trade_time = 0  # Global or persistent storage
if time.time() - last_trade_time < CONFIG.trade_cooldown_seconds:
    logger.info(f"Cooldown active, skipping trade entry for {CONFIG.trade_cooldown_seconds - (time.time() - last_trade_time):.0f}s.")
    signals = {"long": False, "short": False, "reason": "Cooldown active"}
else:
    # Proceed with trade logic
    last_trade_time = time.time()  # Update on successful trade
```

---

### 3. Enhance Logging with Structured JSON Output
Add JSON logging for easier log parsing in production.

```python
import json

# At the start of the script, add JSON handler
json_handler = logging.StreamHandler(sys.stdout)
json_handler.setFormatter(logging.Formatter('{"timestamp": "%(asctime)s", "level": "%(levelname)s", "module": "%(filename)s", "line": %(lineno)d, "message": "%(message)s"}'))
logger.addHandler(json_handler)
```

---

### 4. Add Circuit Breaker for Consecutive Losses
Halt trading after a series of losing trades to prevent drawdowns.
...
# In trading_spell_cycle, after fetching realized PnL
if current_cycle_realized_pnl is not None and not current_cycle_realized_pnl.is_nan():
    if current_cycle_realized_pnl < 0:
        consecutive_losses += 1
        if consecutive_losses >= CONFIG.max_consecutive_losses:
            logger.critical(Fore.RED + f"Max consecutive losses ({CONFIG.max_consecutive_losses}) reached. Halting.")
            termux_notify("EMERGENCY!", f"{CONFIG.symbol} Trading Halted - Loss Limit!")
            sys.exit(1)
    else:
        consecutive_losses = 0
```
...
### 5. Validate API Rate Limits
Monitor and respect API rate limits explicitly.

```python
# In fetch_with_retries, add rate limit tracking
rate_limit_remaining = None
if isinstance(response, dict) and 'RateLimit-Remaining' in response.get('info', {}):
    rate_limit_remaining = int(response['info']['RateLimit-Remaining'])
    if rate_limit_remaining < 10:
        logger.warning(f"{Fore.YELLOW}API Rate Limit Low: {rate_limit_remaining} requests remaining.")
        time.sleep(5)  # Extra delay to avoid hitting limit
```

---

### 6. Add Support for Take-Profit Orders
Implement take-profit levels based on ATR multiples.
...
# In place_risked_market_order, after setting SL
if order_successful:
    tp_price = entry_price + (current_atr * CONFIG.tp_atr_multiplier) if side == "buy" else entry_price - (current_atr * CONFIG.tp_atr_multiplier)
    tp_price_str = format_price(symbol, tp_price)
    tp_params = {
        'stopPrice': tp_price_str,
        'triggerBy': CONFIG.sl_trigger_by,
        'reduceOnly': True,
        'category': CONFIG.market_type
    }
    tp_order = fetch_with_retries(EXCHANGE.create_order, symbol, 'takeProfitMarket', 'sell' if side == "buy" else 'buy', float(qty), params=tp_params)
    if tp_order:
        order_tracker[direction]["tp_id"] = f"POS_TP_{direction.upper()}"
        logger.trade(Fore.GREEN + f"Take-Profit set at {tp_price_str} for {direction} position.")
```

---

### 7. Improve Error Handling for Decimal Conversions
Add robust Decimal conversion with fallbacks.
...
### 8. Add Market Volatility Filter
...
# In trading_spell_cycle, before generating signals
if not current_atr.is_nan() and current_atr > current_price * CONFIG.max_atr_volatility:
    logger.info(f"High volatility detected (ATR: {current_atr:.4f}). Skipping signals.")
    signals = {"long": False, "short": False, "reason": "High Volatility"}
```

---

### 9. Persist State to File
Save order_tracker state to disk for recovery after crashes.

```python
import json

def save_state():
    with open("order_tracker.json", "w") as f:
        json.dump(order_tracker, f)
    logger.debug("Saved order_tracker state.")

def load_state():
    global order_tracker
    try:
        with open("order_tracker.json", "r") as f:
            order_tracker = json.load(f)
        logger.debug("Loaded order_tracker state.")
    except FileNotFoundError:
        logger.debug("No state file found, starting fresh.")

# Call load_state() after config, save_state() after tracker updates
load_state()
```
...
# In critical error paths, e.g., graceful_shutdown
send_email("Bot Shutdown", f"{CONFIG.symbol} bot stopped due to signal.")
```

---

### 11. Optimize OHLCV Fetch with Cache
Cache OHLCV data to reduce API calls.

```python
from cachetools import TTLCache

ohlcv_cache = TTLCache(maxsize=10, ttl=60)  # Cache for 60 seconds

def fetch_market_data(symbol: str, timeframe: str, limit: int) -> Optional[pd.DataFrame]:
    cache_key = (symbol, timeframe, limit)
    if cache_key in ohlcv_cache:
        logger.debug("Returning cached OHLCV data.")
        return ohlcv_cache[cache_key]
    df = fetch_market_data_original(symbol, timeframe, limit)  # Original function renamed
...
### 12. Add Support for Multiple Symbols
Allow trading multiple symbols simultaneously.
...
for symbol in CONFIG.symbols:
    logger.info(f"Processing cycle for {symbol}")
    order_tracker[symbol] = {"long": {"sl_id": None, "tsl_id": None}, "short": {"sl_id": None, "tsl_id": None}}
    trading_spell_cycle(cycle, prev_indicators.get(symbol), last_realized_pnl.get(symbol), symbol)
```

---

### 13. Add Leverage Configuration
Allow configurable leverage per symbol.
...
# After EXCHANGE initialization
EXCHANGE.set_leverage(CONFIG.leverage, CONFIG.symbol, params={'category': CONFIG.market_type})
logger.info(f"Set leverage to {CONFIG.leverage}x for {CONFIG.symbol}")
```

---

### 14. Implement Health Check Endpoint
...
### 15. Add Backtesting Mode
Enable backtesting with historical data.

```python
# In TradingConfig.__init__
self.backtest_mode = self._get_env(
    "BACKTEST_MODE", "False", Fore.YELLOW, 
    cast_type=bool
)

# In main loop
if CONFIG.backtest_mode:
    historical_data = pd.read_csv("historical_data.csv")  # Assume pre-downloaded
    for index, row in historical_data.iterrows():
        df = historical_data[:index + 1]
        trading_spell_cycle(cycle, prev_indicators, last_realized_pnl)
        cycle += 1
else:
    # Normal live trading loop
```

---

### 16. Improve Signal Reason Logging
Log detailed signal reasons for debugging.

```python
# In generate_signals
logger.debug(f"Signal Inputs: price={current_price}, indicators={indicators}, prev_indicators={prev_indicators}")
if long_signal or short_signal:
    logger.info(Fore.CYAN + f"Signal Details: {signal_reason}, Inputs: price={current_price}, indicators={indicators}")
else:
    logger.debug(f"No Signal: {signal_reason}, Inputs: price={current_price}, indicators={indicators}")
```

---

### 17. Add Funding Rate Check
Avoid trading during high funding rate periods.
...
# In trading_spell_cycle
funding_rate = fetch_with_retries(EXCHANGE.fetch_funding_rate, CONFIG.symbol)
if funding_rate and abs(Decimal(str(funding_rate['fundingRate']))) > CONFIG.max_funding_rate:
    logger.info(f"High funding rate ({funding_rate['fundingRate']}). Skipping signals.")
    signals = {"long": False, "short": False, "reason": "High Funding Rate"}
```

---

### 18. Add Position Age Timeout
Close positions held too long to limit risk.
...
# In trading_spell_cycle
if has_long_pos:
    position_age = (pd.Timestamp.now(tz='UTC') - active_long_pos.get('open_time')).total_seconds() / 3600
    if position_age > CONFIG.max_position_age_hours:
        close_position("long")
        logger.info(f"Closed LONG position due to age timeout ({position_age:.1f}h > {CONFIG.max_position_age_hours}h)")
```

---

### 19. Enhance Order Confirmation
Wait for order fill confirmation with timeout.

```python
def confirm_order_filled(symbol: str, order_id: str, timeout: int) -> bool:
    start_time = time.time()
    while time.time() - start_time < timeout:
        order = fetch_with_retries(EXCHANGE.fetch_order, order_id, symbol, params={'category': CONFIG.market_type})
...
# In place_risked_market_order
if order and order['id']:
    if confirm_order_filled(symbol, order['id'], CONFIG.order_check_timeout_seconds):
        logger.trade(Fore.GREEN + f"Order {order['id']} filled.")
    else:
        logger.error(f"Order {order['id']} fill confirmation timed out.")
        return False
```
...
# In place_risked_market_order, on successful trade
trade_record = {
...
# In print_status_panel
win_rate = sum(1 for t in trades if t['pnl'] > 0) / len(trades) if trades else 0
print(f" {section_color}Trade Stats: Total={len(trades)}, Win Rate={win_rate:.2%}{reset_all}")
```

---

### 21. Add Blackout Periods
Disable trading during specified times (e.g., major news).

```python
# In TradingConfig.__init__
self.blackout_hours = self._get_env(
    "BLACKOUT_HOURS", "", Fore.YELLOW
).split(",")  # e.g., "00:00-01:00,12:00-13:00"

# In trading_spell_cycle
from datetime import datetime
now = datetime.utcnow()
for blackout in CONFIG.blackout_hours:
    if blackout:
        start_hour, end_hour = blackout.split("-")
        start = datetime.strptime(start_hour, "%H:%M").time()
        end = datetime.strptime(end_hour, "%H:%M").time()
        if start <= now.time() <= end:
            logger.info(f"Blackout period active ({blackout}). Skipping signals.")
            signals = {"long": False, "short": False, "reason": f"Blackout {blackout}"}
            break
```

---

### 22. Improve Documentation with Docstrings
Add comprehensive docstrings for key functions.

```python
def trading_spell_cycle(cycle_count: int, prev_indicators: Optional[Dict[str, Decimal]], last_realized_pnl: Optional[Decimal]) -> Tuple[Optional[Dict[str, Decimal]], bool, Optional[Decimal]]:
    """
    Executes one trading cycle, including data fetching, indicator calculation, state management, and trade execution.

    Args:
        cycle_count (int): Current cycle number for logging.
        prev_indicators (Optional[Dict[str, Decimal]]): Indicators from the previous cycle for signal generation.
        last_realized_pnl (Optional[Decimal]): Last fetched realized PnL for display.

    Returns:
        Tuple[Optional[Dict[str, Decimal]], bool, Optional[Decimal]]: Current indicators, cycle success status, and realized PnL.
    """
```
...
class TestTradingBot(unittest.TestCase):
    def test_format_price(self):
        ...
        price = format_price(CONFIG.symbol, Decimal("123.456789"))
        self.assertTrue(isinstance(price, str))
        self.assertTrue(Decimal(price) <= Decimal("123.456789"))
...
### 24. Add Configurable Logging Levels per Module
Allow different logging levels for different modules.

```python
# At script start
module_log_levels = {
    'ccxt': os.getenv("CCXT_LOG_LEVEL", "WARNING").upper(),
    'bot': os.getenv("BOT_LOG_LEVEL", "INFO").upper()
}
...
# In place_risked_market_order
if CONFIG.dry_run:
    logger.info(Fore.CYAN + f"DRY RUN: Would place {side} order for {qty} @ market price with SL @ {sl_price_str}")
...
These snippets provide targeted improvements to enhance the bot’s functionality, reliability, and usability. Each suggestion is designed to be modular, allowing for selective implementation based on specific needs. Let me know if you'd like to expand on any of these or need help integrating them into the script!
...
class TradingConfig:
    ...
    def __init__(self):
        ...
        min_ohlcv_required_indicators = self.atr_period + 1 # Minimum for ATR

        if self.strategy == 'ecc_scalp':
            min_ohlcv_required_indicators = max(min_ohlcv_required_indicators, self.ecc_lookback + 4, self.trend_ema_period) # ECC formula needs ~4 + lookback, trend EMA
        ...
        try:
            ...
            if amount_step_size is not None:
                try:
                    ...
                    # Example: if step is 0.001, epsilon could be 1E-6. If step is 1, epsilon could be 1E-9.
                    # Using a fixed tiny Decimal like 1E-12 is generally safe and simpler.
        ...
        if len(df) > 1:
            ...
            try:
                ...
                if abs(time_diff.total_seconds() - expected_interval_td.total_seconds()) > tolerance.total_seconds():
                    logger.warning(f"Unexpected large time gap between last two candles: {time_diff} (expected ~{expected_interval_td}). Difference: {abs(time_diff.total_seconds() - expected_interval_td.total_seconds()):.2f}s")
            except ValueError:
...
def calculate_ehlers_cyber_cycle(prices: np.ndarray, alpha: Decimal, lookback: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        ...
        Arrays will be empty if input prices are insufficient or calculation fails.
    """
    ...
    # Smooth loop starts at i=3 (for prices[i-3]), needs min 4 prices (index 0-3). First smooth value at index 3.
    # Cycle loop starts at i=4 (for smooth[i-2] and cycle[i-2]), needs min 5 prices (index 0-4). First cycle value at index 4.
    # So minimum length to get *any* cycle value is 5.
    # A safe minimum length is roughly `lookback + initial points`. Let's enforce min 15 or lookback+5, whichever is larger.
    min_safe_len = max(lookback + 5, 15) # Ensure at least 15 bars for some stability, or lookback+5 if larger.
    ...
    try:
        ...
        # Return the calculated values. We return all calculated values, not just 'lookback',
        # as the calculation process needs the full history. The signal logic will use the latest value.
        # However, the original code sliced [-lookback:]. Let's stick to that if it's intended
        # that only the last `lookback` points are relevant for the signal logic itself (e.g. for plotting/visualizing).
        # But for a simple cross signal on the *latest* bar, we only need the last value.
        # Let's return the full calculated arrays and let the indicator extraction function handle picking the latest.
        # The slice [-lookback:] might be misleading as the actual calculation uses the full history.
        # Let's return the full arrays and fix the calling code.

        # Reverting to original logic returning last 'lookback' points, assuming the first few points are less stable
        # and the signal might rely on the shape/trend of the last N points.
        if len(cycle) < lookback or len(trigger) < lookback:
             logger.warning(f"Not enough *stable* ECC values calculated ({len(cycle)}) for requested lookback ({lookback}). Returning available calculated values.")
             # Return the parts that were successfully calculated, padding with NaNs if necessary,
             # or just return the calculated parts. Returning the calculated parts seems safer.
             return cycle[4:], trigger[4:] # Return all calculated values from index 4 onwards

        else:
             # Return the last `lookback` values for consistency with original slicing logic
        ...
        return np.array([]), np.array([]) # Return empty arrays on calculation error
...
def calculate_indicators(df: pd.DataFrame) -> Optional[Dict[str, Decimal]]:
    ...
    try:
        ...
        indicators_ok = True # Flag to track if critical indicators could be calculated
        ...
        if current_data_length < min_required_atr:
            logger.warning(f"{Fore.YELLOW}Insufficient data ({current_data_length}) for ATR calculation (minimum required: {min_required_atr}). ATR will be NaN.")
            atr_series = pd.Series([np.nan] * len(df_calc), index=df_calc.index)  # Not enough data for ATR
            indicators_ok = False # ATR is often critical
        else:
        ...
        if current_data_length < min_required_trend_ema:
             logger.warning(f"{Fore.YELLOW}Insufficient data ({current_data_length}) for Trend EMA calculation (minimum required: {min_required_trend_ema}). Trend EMA will be NaN.")
             indicators_ok = False # Trend EMA is critical for filtering
        else:
        ...
        if CONFIG.strategy == 'ecc_scalp':
            ...
            if current_data_length < min_required_ecc:
                 logger.warning(f"{Fore.YELLOW}Insufficient data ({current_data_length}) for ECC calculation (minimum required: {min_required_ecc}). ECC will be NaN.")
                 indicators_ok = False # ECC is critical for this strategy
            else:
            ...
            if current_data_length < min_required_ema:
                logger.warning(f"{Fore.YELLOW}Insufficient data ({current_data_length}) for Fast/Slow EMA calculation (minimum required: {min_required_ema}). EMAs will be NaN.")
                indicators_ok = False # EMAs are critical for this strategy
            else:
            ...
            if current_data_length < min_required_stoch:
                 logger.warning(f"{Fore.YELLOW}Insufficient data ({current_data_length}) for Stochastic calculation (minimum required: {min_required_stoch}). Stoch will be NaN.")
                 indicators_ok = False # Stoch is critical for this strategy
            ...
            try:
                ...
                if not entry_price.is_nan():
                    ...
                    aggregated_positions[side]["entry_price_sum_qty"] += entry_price * contracts.copy_abs()
                     ...
                     logger.warning(f"Missing entry price for non-zero position entry ({contracts.normalize()}) for {side} side.")
            ...
            if weighted_sum.copy_abs() > Decimal("0") and total_qty.copy_abs() > Decimal("0"):
                try:
                    final_pos_dict[side]["entry_price"] = weighted_sum / total_qty.copy_abs()
                     ...
                     logger.error(f"Division by zero calculating weighted entry price for {side} side (total_qty_abs is zero).")
...
def fetch_trade_history_wrapper(symbol: Optional[str] = None, lookback_days: int = CONFIG.pnl_lookback_days) -> Optional[List[Dict]]:
    ...
    while True:
        ...
        if isinstance(response, dict) and isinstance(response.get('info'), dict) and response['info'].get('retCode') == 0:
            ...
            logger.error(Fore.RED + f"Failed to fetch trade history page (Code: {error_code}). Exchange message: {error_msg}. Stopping pagination.")
            return None # Indicate failure
...
def fetch_realized_pnl_summary(symbol: Optional[str] = None) -> Optional[Dict]:
    ...
    """
    logger.info(Fore.CYAN + f"# Consulting Realized PnL scrolls for {symbol or 'All Symbols'} (Last {CONFIG.pnl_lookback_days} Days)...")
    ...
    if trades is None:
        logger.error(Fore.RED + "Failed to fetch trade history for Realized PnL summary.")
        return None # Indicate failure
    ...
    for trade in trades:
        try:
            ...
            if isinstance(realized_pnl, Decimal) and not realized_pnl.is_nan():
                 ...
                 if realized_pnl.copy_abs() > Decimal('0'):
                      total_realized += realized_pnl
                      trade_count_with_pnl += 1
            else:
                 logger.debug(f"Skipping trade with invalid or missing PnL value for {trade.get('symbol', 'Unknown')}: {realized_pnl}")

        except Exception as e:
            logger.error(f"Error processing trade for realized PnL {trade.get('symbol', 'Unknown')}: {e}")

    logger.info(f"Realized PnL Summary (Last {CONFIG.pnl_lookback_days} Days): Total Realized = {total_realized:+.4f} ({trade_count_with_pnl} trades contributing PnL)")
...
def check_order_status(order_id: str, symbol: str, timeout: int = CONFIG.order_check_timeout_seconds) -> Optional[Dict]:
    ...
    if order_id is None or order_id == "":
        logger.warning("Received None or empty order_id to check status. Skipping check.")
    ...
    if symbol_id is None:
         logger.error("Market Info not available, cannot get exchange symbol ID for order status check.")
         return None
...
def place_risked_market_order(symbol: str, side: str, risk_percentage: Decimal, atr: Decimal) -> bool:
    ...
    if market_id is None:
         logger.error(Fore.RED + f"{trade_action} failed: Exchange symbol ID not available from Market Info.")
         return False
    ...
    try:
        ...
        if side == "buy" and sl_price >= price - tolerance_ticks:
            logger.error(Fore.RED + f"{trade_action} failed: Calculated SL price ({sl_price}) is too close to or above current price ({price}) [Tolerance: {tolerance_ticks.normalize()}]. Check ATR/multiplier or market precision. Aborting.")
            return False
        if side == "sell" and sl_price <= price + tolerance_ticks:
            logger.error(Fore.RED + f"{trade_action} failed: Calculated SL price ({sl_price}) is too close to or below current price ({price}) [Tolerance: {tolerance_ticks.normalize()}]. Check ATR/multiplier or market precision. Aborting.")
            return False
        ...
        if CONFIG.market_type == 'linear':
            ...
            # Risk (Quote) = (Price - SL Price) * Qty (Contracts) * (Contract Size)
            # Assuming Contract Size is in Quote/Contract units (e.g., USD/BTC contract for BTC/USD:BTC)
            # Risk (Quote) = Stop Distance (Quote) * Qty (Contracts) * Contract Size (Quote/Contract)
            # Qty (Contracts) = Risk (Quote) / (Stop Distance (Quote) * Contract Size (Quote/Contract))
            # However, Bybit's API often expects quantity in Contracts for inverse.
            # And the contractSize parameter in market info seems to be in BASE units per contract (e.g., 0.0001 BTC per contract for BTCUSD inverse).
            # Let's re-evaluate based on Bybit V5 docs and common practice:
            # Risk Amount (in Quote) = Position_Size (in Contracts) * Contract_Size (in Base per Contract) * Stop_Distance (in Quote per Base)
            # This doesn't seem right. Let's use the dollar value approach often used for risk.
            # Stop Loss Distance in Dollar Value = abs(Entry Price - SL Price) * Qty (in Base)
            # Max Risk (in Quote) = Qty (in Base) * Stop Loss Distance (in Quote per Base)
            # Qty (in Base) = Max Risk (in Quote) / Stop Loss Distance (in Quote per Base)
            # This is the same formula as Linear, but we need Contracts.
            # Qty (in Contracts) = Qty (in Base) / Contract_Size (in Base per Contract)
            # Qty (in Contracts) = (Max Risk (in Quote) / Stop Loss Distance (in Quote per Base)) / Contract_Size (in Base per Contract)
            # Let's use the Quote/Base price for stop distance, which is what `stop_distance_quote` is.
            # Stop Loss Distance (in Quote) = abs(Entry Price - SL Price)
            # Risk (Quote) = Qty (Contracts) * Contract Size (Base/Contract) * Stop Distance (Quote/Base)
            # This still seems circular. Let's use the risk per contract approach.
            # Risk per Contract (in Quote) = abs(Entry Price - SL Price) * Contract Size (Base/Contract)
            # Number of Contracts = Total Risk (in Quote) / Risk per Contract (in Quote)
            # Number of Contracts = Total Risk (in Quote) / (abs(Entry Price - SL Price) * Contract Size (Base/Contract))
            # Number of Contracts = Risk Amount (Quote) / (Stop Distance (Quote/Base) * Contract Size (Base/Contract))
            if contract_size_dec.is_nan() or contract_size_dec <= Decimal("0"):
                 logger.error(Fore.RED + f"{trade_action} failed: Invalid contract size ({contract_size_dec}). Cannot calculate inverse size.")
                 return False

            qty_raw = risk_amount_quote / (stop_distance_quote * contract_size_dec)
            logger.debug(f"Inverse Sizing (Contract Size = {contract_size_dec.normalize()} Base/Contract): Qty (Contracts) = {risk_amount_quote:.8f} {quote_currency} / ({stop_distance_quote:.8f} {quote_currency} * {contract_size_dec.normalize()}) = {qty_raw:.8f}")
        ...
        try:
            ...
            logger.error(f"Could not parse minimum quantity '{min_qty_str}'. Assuming 0.")
            min_qty = Decimal("0")
        ...
        try:
            max_qty = Decimal(str(max_qty_str)) if max_qty_str is not None else Decimal('Infinity')
             ...
             logger.error(f"Could not parse maximum quantity '{max_qty_str}'. Assuming Infinity.")
        ...
        if min_cost_str is not None:
            try:
                ...
                if CONFIG.market_type == 'linear':
                    ...
                    # Bybit V5 Contract Size is in Base units per contract (e.g., 0.0001 BTC per contract)
                    ...
                    if not contract_size_dec.is_nan() and contract_size_dec > Decimal('0'):
                        ...
                        logger.debug(f"Inverse cost estimation: Qty({qty.normalize()}) * ContractSize({contract_size_dec.normalize()}) * Price({price.normalize()}) = {estimated_cost.normalize()}")
                    else:
    ...
    try:
        ...
        try:
            ...
            if isinstance(sl_set_response, dict) and isinstance(sl_set_response.get('info'), dict) and sl_set_response['info'].get('retCode') == 0:
        ...
        # --- Handle SL Setting Failures ---
            ...
            try:
                ...
                if close_qty_decimal < min_qty_close:
                    logger.critical(f"{Fore.RED}Emergency closure quantity {close_qty_decimal.normalize()} is below minimum {min_qty_close.normalize()}. MANUAL CLOSURE REQUIRED for {position_side.upper()} position!")
...
def manage_trailing_stop(
    ...
    market_id = MARKET_INFO.get('id')
    if market_id is None:
         logger.error(f"Cannot manage TSL: Exchange symbol ID not available from Market Info.")
         return
...
def generate_signals(indicators: Optional[Dict[str, Decimal]], current_price: Optional[Decimal], prev_indicators: Optional[Dict[str, Decimal]]) -> Dict[str, Union[bool, str]]:
    ...
    if CONFIG.strategy == 'ecc_scalp' and not prev_indicators:
    ...
    try:
        ...
        if CONFIG.strategy == 'ecc_scalp':
             ...
             # Previous values needed for cross logic
        ...
        price_above_trend = not trend_ema.is_nan() and not current_price.is_nan() and current_price > trend_ema + trend_buffer
        price_below_trend = not trend_ema.is_nan() and not current_price.is_nan() and current_price < trend_ema - trend_buffer
        # price_at_trend = not trend_ema.is_nan() and not current_price.is_nan() and abs(current_price - trend_ema) <= trend_buffer # Not explicitly needed for filter logic
...
def trading_spell_cycle(cycle_count: int, prev_indicators: Optional[Dict[str, Decimal]], last_realized_pnl: Optional[Decimal]) -> Tuple[Optional[Dict[str, Decimal]], bool, Optional[Decimal]]:
    ...
    Returns current indicators, cycle success status, and realized PnL (fetched this cycle or last).
    """
    ...
    if df is None or df.empty:
        ...
        # Return None for indicators, indicate failure, and return the last known PnL
        return None, False, last_realized_pnl
    ...
    try:
        ...
        now_utc = pd.Timestamp.utcnow().tz_localize(None) # Make naive to compare with naive Bybit timestamps from CCXT fetch_time? No, CCXT timestamps are ms epoch, converted to UTC datetime.now(timezone.utc) is better.
        # Better to use UTC timezone aware objects throughout if possible. CCXT fetch_ohlcv gives UTC=True.
        now_utc = pd.Timestamp.now(tz='UTC')
        time_diff = now_utc - last_timestamp
        # Allow for interval duration + some buffer (e.g., 1.5 * interval + 10s)
        try:
            interval_seconds = EXCHANGE.parse_timeframe(CONFIG.interval)
            allowed_lag = pd.Timedelta(seconds=interval_seconds * 1.5 + 10) # Reduced buffer
            if time_diff > allowed_lag:
                logger.warning(Fore.YELLOW + f"Market data may be stale. Last candle: {last_timestamp.strftime('%H:%M:%S %Z')} ({time_diff} ago). Allowed lag: ~{allowed_lag}")
        except ValueError:
    ...
    # Note: calculate_indicators now returns dict with NaNs for failed indicators, or None if critical failure (e.g., not enough data structure)
    # If indicators is None, it's a critical failure for signal generation/trading.
    if indicators is None:
        logger.error(Fore.RED + "Indicator calculation failed critically (e.g., invalid DataFrame structure). Continuing cycle but skipping trade actions.")
    ...
    # total_realized_pnl: Optional[Decimal] = Decimal("NaN") # Moved initialization outside
    current_cycle_realized_pnl = last_realized_pnl # Start with the last known value

    if cycle_count == 1 or cycle_count % CONFIG.fetch_pnl_interval_cycles == 0:
         pnl_result = fetch_realized_pnl_summary(CONFIG.symbol)
         if pnl_result is not None:
              current_cycle_realized_pnl = pnl_result.get('total_realized', Decimal('NaN'))
         else:
              logger.warning("Failed to fetch realized PnL this cycle. Using last known value.")
              # current_cycle_realized_pnl remains last_realized_pnl (could be NaN if first fetch failed)
    else:
         ...
         # current_cycle_realized_pnl remains last_realized_pnl
    ...
    # No, pass the *latest* state of the tracker for display accuracy. Deepcopy is unnecessary here.
    order_tracker_snapshot = order_tracker # Reference to the live tracker state
    ...
    if can_trade_logic:
        ...
        if indicators is not None and not current_price.is_nan() and not current_atr.is_nan() and (has_long_pos or has_short_pos):
            ...
            if order_tracker["long"]["sl_id"] or order_tracker["long"]["tsl_id"] or \
               order_tracker["short"]["sl_id"] or order_tracker["short"]["tsl_id"]:
                ...
                # No need to update snapshot, it's a reference to the same tracker
        ...
        # Also need the trend EMA for filtering, which is part of indicators.
        # Check if indicators is None or if the required indicators within the dict are NaN.
        signal_indicators_valid = True
        if indicators is None:
             signal_indicators_valid = False
             logger.warning("Skipping signal generation: Indicators dictionary is None.")
        elif not current_price.is_nan(): # Only check indicators if price is valid
            # Define indicators required for trading based on the chosen strategy
            required_for_signal_current = ['trend_ema'] # Trend EMA is always used for filter
            required_for_signal_prev = []

            if CONFIG.strategy == 'ecc_scalp':
                required_for_signal_current.extend(['ecc_cycle', 'ecc_trigger'])
                required_for_signal_prev.extend(['ecc_cycle', 'ecc_trigger'])
                if prev_indicators is None:
                     logger.warning("Skipping ECC signal generation: Previous indicators are required for cross detection.")
                     signal_indicators_valid = False
                else:
                     # Check previous indicator values for NaN if needed for the cross logic
                     for key in required_for_signal_prev:
                          if prev_indicators.get(key, Decimal('NaN')).is_nan():
                               logger.warning(f"Skipping ECC signal generation: Required previous indicator '{key}' is NaN.")
                               signal_indicators_valid = False
                               break # No need to check further indicators

            elif CONFIG.strategy == 'ema_stoch':
                required_for_signal_current.extend(['fast_ema', 'slow_ema', 'stoch_k']) # Stoch D not strictly needed for signal logic here
            ...
            if signal_indicators_valid: # Only check current if previous check passed (if applicable)
                 for key in required_for_signal_current:
                      if indicators.get(key, Decimal('NaN')).is_nan():
                           logger.warning(f"Skipping signal generation: Required current indicator '{key}' is NaN.")
                           signal_indicators_valid = False
                           break  # No need to check further indicators
        else:
             logger.warning("Skipping signal generation: Current price is invalid.")
             signal_indicators_valid = False
        ...
        if signal_indicators_valid:
            ...
            signals = {"long": False, "short": False, "reason": "Skipped due to missing/invalid indicators"}
        ...
        # Need current_atr to be valid for sizing.
        if is_flat and signals.get("long") or signals.get("short"): # Check if ANY signal is true
             if indicators is not None and not current_atr.is_nan(): # Check indicators and ATR validity
                 if signals.get("long"):
                     ...
                     if place_risked_market_order(CONFIG.symbol, "buy", CONFIG.risk_percentage, current_atr):
                         ...
                         positions = get_current_position(CONFIG.symbol)
                         # Update snapshot for panel (positions_snapshot is just a reference now)
                         # No need to reassign positions_snapshot, it points to the updated positions dict.
                         # If positions fetch failed here, positions will be None, panel will show N/A.
                     else:
                  ...
                  logger.warning("Skipping trade entry despite signal due to invalid indicators or ATR.")
            ...
            # No signal generated and position is flat
            logger.debug(f"Position is flat, no trade signal generated.")
    ...
    print_status_panel(
        ...
        positions_snapshot, current_equity, signals, current_cycle_realized_pnl, order_tracker_snapshot # Use the *latest* order_tracker state snapshot
    )
    ...
    # the success status, and the realized PnL fetched in this cycle.
    return indicators, cycle_success, current_cycle_realized_pnl
...
def graceful_shutdown() -> None:
    ...
    if market_id is None:
         logger.error(Fore.RED + "Exchange symbol ID not available. Cannot cancel orders or close positions safely.")
         termux_notify("Shutdown Warning!", f"{CONFIG.symbol} Cannot get market ID. Manual check required!")
         return
    ...
    try:
        ...
        # fetch_open_orders might require symbol for V5. Let's add it.
        fetch_params['symbol'] = market_id
        ...
        if open_orders_list is None:
            ...
            try:
                ...
                if hasattr(EXCHANGE, 'private_post_order_cancel_all'):
                    ...
                    elif isinstance(response, dict) and isinstance(response.get('info'), dict) and response['info'].get('retCode') == 0:
                    ...
                    for order in open_orders_list:
                        try:
                            order_id = order['id']
                            if order_id is None: continue # Skip if ID is missing
                            logger.debug(f"Cancelling order {order_id}...")
                            # Bybit V5 cancel_order requires category and symbol
                            individual_cancel_params = {'category': CONFIG.market_type, 'symbol': market_id}
...
if __name__ == "__main__":
    ...
    if CONFIG.strategy == 'ecc_scalp':
         logger.info(f"  ECC Parameters: Alpha={CONFIG.ecc_alpha.normalize()}, Lookback={CONFIG.ecc_lookback}")
    elif CONFIG.strategy == 'ema_stoch':
    ...
    logger.info(f"Trend Filter EMA({CONFIG.trend_ema_period}): {'ON' if CONFIG.trade_only_with_trend else 'OFF'}")
    logger.info(f"Risk per trade: {CONFIG.risk_percentage * 100:.5f}%")  # Show more precision for risk
    logger.info(f"SL Multiplier: {CONFIG.sl_atr_multiplier.normalize()} * ATR ({CONFIG.atr_period} periods)")
    ...
    last_realized_pnl: Optional[Decimal] = Decimal("NaN") # Store the last fetched realized PnL

    try:
        while True:
            cycle += 1
            try:
                # Pass previous indicators and receive current indicators, cycle success, and realized PnL
                current_indicators, cycle_ok, current_cycle_realized_pnl = trading_spell_cycle(cycle, prev_indicators, last_realized_pnl)

                # Update prev_indicators for the next cycle *only if indicators were calculated successfully this cycle*
                # This prevents using potentially inconsistent indicator data if calculation failed.
                if current_indicators is not None:
                     prev_indicators = current_indicators
                else:
                     logger.warning("Current indicators calculation failed. Previous indicators will not be updated.")
                     # prev_indicators remains the value from the cycle *before* this failed one.

                # Update last_realized_pnl if it was fetched this cycle (i.e., current_cycle_realized_pnl is not NaN)
                if current_cycle_realized_pnl is not None and not current_cycle_realized_pnl.is_nan():
                     last_realized_pnl = current_cycle_realized_pnl
                # If fetch_pnl_wrapper returned None, or it returned NaN, last_realized_pnl keeps its old value.
                ...
                # If a cycle fails unexpectedly, we should probably reset prev_indicators to be safe, as current data might be inconsistent.
                logger.warning("Cycle failed unexpectedly. Resetting previous indicators.")
                prev_indicators = None
                # Keep the last known PnL value.
