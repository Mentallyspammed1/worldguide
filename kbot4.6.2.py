...
DEFAULT_AMOUNT_DP = (
    6  # Default decimal places for amount formatting if market info fails
)
    ...
    **kwargs: Any,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt > 0:
                logger.info(
                    f"{Fore.GREEN}{Style.BRIGHT}Successfully executed {func_name} on attempt {attempt + 1}/{max_retries + 1}.{Style.RESET_ALL}"
                )
            return result
            ...
            error_str = str(e)
            error_display = (
                (error_str[:150] + "...") if len(error_str) > 150 else error_str
            )
            ...
            logger.error(
                f"{Fore.RED}Unhandled Exchange error during {func_name}: {error_display}{Style.RESET_ALL}"
            )
            if attempt < max_retries:
                logger.warning(
    ...
    if last_exception:
        ...
        raise RuntimeError(
            f"Function {func_name} failed after {max_retries + 1} attempts without raising a recognized exception."
        )
...
class TradingConfig:
    ...
    def __init__(self, env_file: Union[str, Path] = DEFAULT_ENV_FILE) -> None:
        ...
        )  # Used in verification attempts implicitly
        ...
        self.journal_file_path: Path = Path(
            self._get_env("JOURNAL_FILE_PATH", DEFAULT_JOURNAL_FILE, Style.DIM)
        )
    ...
    def _validate_config(self) -> None:
        ...
        if (
            self.tsl_activation_atr_multiplier > 0
            and self.tsl_activation_atr_multiplier < self.sl_atr_multiplier
        ):
            logger.warning(
        ...
        if self.enable_journaling:
            try:
                ...
                logger.debug(
                    f"Ensured journal directory exists: {self.journal_file_path.parent}"
                )
            except OSError as e:
                logger.error(
                    f"Failed to create directory for journal file {self.journal_file_path}: {e}. Journaling disabled."
                )
    ...
    def _cast_value(
        ...
        if not val_to_cast:  # Handle empty string after strip
            logger.warning(
                f"Empty value string provided for {key}. Using default '{default}'."
            )
            return default
        try:
            if cast_type == bool:
                ...
                if dec_val.is_nan():
                    raise ValueError(
                        "Non-numeric or invalid string cannot be cast to Decimal"
                    )
                return dec_val
            elif cast_type == int:
                ...
                return cast_type(
                    val_to_cast
                )  # Use constructor directly for other types
    ...
    def _validate_value(
        ...
        if isinstance(value, Decimal) and not value.is_finite():
            logger.error(
                f"Validation failed for {key}: Non-finite Decimal value '{value}'. Cannot perform checks."
            )
            return (
                False  # Treat non-finite Decimals as invalid for range/allowed checks
            )
        ...
        if is_numeric:
            ...
            value_dec = safe_decimal(
                value
            )  # Value is already numeric, convert for consistent comparison
        ...
        if allowed_values:
            ...
            if comp_value not in lower_allowed:
                logger.error(
                    f"{Fore.RED}Validation failed for {key}: Invalid value '{value}'. Allowed values: {allowed_values}.{Style.RESET_ALL}"
                )
    ...
    def _get_env(
        ...
        log_method(
            f"{color}Using {key}: {log_value_display} (Source: {source}){Style.RESET_ALL}"
        )
...
class ExchangeManager:
    ...
    def _initialize_exchange(self) -> None:
        ...
        logger.info(
            f"Initializing Bybit exchange interface (V5 {self.config.market_type} / {self.config.bybit_v5_category})..."
        )
        try:
            # Check if ccxt.bybit exists
            if not hasattr(ccxt, "bybit"):
                logger.critical(
                    f"{Fore.RED}{Style.BRIGHT}CCXT 'bybit' exchange class not found. Update CCXT? Halting.{Style.RESET_ALL}"
                )
                sys.exit(1)
            ...
            exchange_params = {
                ...
                "options": {
                    ...
                    ),  # Required for V5 signatures? Check CCXT handling. Usually handled internally.
            ...
            if exchange_params["urls"]:
                logger.warning(
                    f"{Fore.YELLOW}Using Bybit Testnet API endpoint.{Style.RESET_ALL}"
                )
            ...
            logger.debug(
                f"Exchange server time fetched: {datetime.fromtimestamp(server_time / 1000, tz=timezone.utc)}"
            )
            logger.info(
    ...
    def _load_market_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            def get_dp_from_precision(
                ...
                if prec_dec.is_nan() or not prec_dec.is_finite():
                    return default_dp
                if prec_dec == 0:
                    return 0  # Handle case where precision is exactly 0
                ...
                # Check if it's likely number of decimal places directly (integer >= 0)
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not convert precision value {prec_dec} to integer DP, using default {default_dp}dp."
                        )
                        return default_dp
                # Handle other cases (e.g., precision < 0 ?) - use default
                else:
                    logger.warning(
                        f"Unexpected precision format '{precision_val}', using default {default_dp}dp."
                    )
                    return default_dp
            ...
            tick_size_raw = market.get("precision", {}).get(
                "price"
            )  # Often same as price precision step
            market["tick_size"] = safe_decimal(
                tick_size_raw, default=Decimal("1e-" + str(price_dp))
            )
            if market["tick_size"].is_nan() or market["tick_size"] <= 0:
                market["tick_size"] = Decimal(
                    "1e-" + str(price_dp)
                )  # Fallback based on DP
                logger.warning(
                    f"Using calculated tick size based on price DP: {market['tick_size']}"
                )
            ...
            if market["contract_size"].is_nan() or market["contract_size"] <= 0:
                logger.warning(
                    f"Invalid contract size found ('{market.get('contractSize')}'). Defaulting to 1."
                )
    ...
    def get_market_precision(self, precision_type: str) -> int:
        ...
        default_dp = (
            DEFAULT_PRICE_DP if precision_type == "price" else DEFAULT_AMOUNT_DP
        )
        ...
        logger.warning(
            f"Market info or precision_dp missing, using default DP {default_dp} for '{precision_type}'."
        )
        return default_dp
    ...
    def _format_v5_param(
        ...
        if decimal_value.is_nan() or not decimal_value.is_finite():
            logger.warning(
                f"V5 Param Formatting: Input '{value}' resulted in non-finite Decimal for param_type '{param_type}'."
            )
        ...
        if is_zero:
            if allow_zero:
                ...
                precision_dp = self.get_market_precision(
                    "price" if param_type in ["price", "distance"] else "amount"
                )
            ...
            logger.warning(
                f"V5 Param Formatting: Input value '{value}' is negative, which is usually invalid for param_type '{param_type}'."
            )
    ...
    def fetch_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not isinstance(ohlcv_list, list) or not ohlcv_list:
                logger.error(
                    f"fetch_ohlcv for {self.config.symbol} returned invalid data: {ohlcv_list}"
                )
                return None
            if len(ohlcv_list) < 10:  # Check for suspiciously small amount of data
                logger.warning(
                    f"Fetched only {len(ohlcv_list)} candles, which might be insufficient for some indicators."
                )
            ...
            try:
                ...
                if len(df) < rows_before_ts_drop:
                    logger.warning(
                        f"Dropped {rows_before_ts_drop - len(df)} rows with invalid timestamps."
                    )
            except Exception as e:
                logger.error(
                    f"Error processing timestamps in OHLCV data: {e}", exc_info=True
                )
                return None
            ...
            if nan_cols:
                logger.warning(
                    f"Column(s) '{', '.join(nan_cols)}' contain NaN values after Decimal conversion. Check API data source."
                )
            ...
            if rows_dropped > 0:
                logger.warning(
                    f"Dropped {rows_dropped} rows with NaN OHLC Decimal values."
                )

            if df.empty:
                logger.error(
                    "DataFrame became empty after processing and dropping NaN OHLCV data."
                )
                return None
        ...
        try:
            ...
            if settle_currency in balance_data.get("total", {}):
                ...
                if total_equity.is_nan():
                    logger.warning(
                        f"Parsed NaN total equity from top-level balance for {settle_currency}. Value: '{total_equity_raw}'"
                    )
            ...
            if settle_currency in balance_data.get("free", {}):
                ...
                if available_balance.is_nan():
                    logger.warning(
                        f"Parsed NaN available balance from top-level balance for {settle_currency}. Value: '{available_balance_raw}'"
                    )
            ...
            if (
                ...
                logger.debug(
                    "Parsing balance from 'info' field as fallback or refinement..."
                )
                    ...
                    if unified_acc_info:
                        ...
                        if total_equity is None or total_equity.is_nan():
                            ...
                            if not total_equity_info.is_nan():
                                ...
                                logger.debug(
                                    f"Using totalEquity from info: {total_equity.normalize()}"
                                )
                            else:
                                logger.warning(
                                    f"Parsed NaN total equity from info field. Value: '{total_equity_info_raw}'"
                                )
                        ...
                        if available_balance is None or available_balance.is_nan():
                            ...
                            if coin_list and isinstance(coin_list, list):
                                ...
                                if settle_coin_info:
                                    ...
                                    available_info_raw = settle_coin_info.get(
                                        "availableToWithdraw"
                                    )
                                    ...
                                    if not available_info.is_nan():
                                        ...
                                        logger.debug(
                                            f"Using availableToWithdraw from info coin list: {available_balance.normalize()}"
                                        )
                                    else:
                                        logger.warning(
                                            f"Parsed NaN available balance from info coin list. Value: '{available_info_raw}'"
                                        )
                                    # Optionally refine total_equity using coin equity if still needed
                                    if (
                                        total_equity is None or total_equity.is_nan()
                                    ) and "equity" in settle_coin_info:
                                        ...
                                        if not coin_equity.is_nan():
                                            ...
                                            logger.debug(
                                                f"Refined total equity using info coin equity: {total_equity.normalize()}"
                                            )
                                        else:
                                            logger.warning(
                                                f"Parsed NaN equity from info coin list. Value: '{coin_equity_raw}'"
                                            )
            ...
            equity_str = (
                total_equity.normalize() if total_equity is not None else "[N/A]"
            )
            avail_str = (
                available_balance.normalize()
                if available_balance is not None
                else "[N/A]"
            )  # Should be Decimal(0) if defaulted
            logger.info(
            ...
            return (
                total_equity,
                available_balance,
            )  # Return potentially None equity, but Decimal available
    ...
    def get_current_position(
        ...
        try:
            ...
            if not isinstance(fetched_positions_list, list):
                logger.error(
                    f"fetch_positions returned unexpected data type: {type(fetched_positions_list)}. Expected list."
                )
                return None  # Treat as failure

            if not fetched_positions_list:
                logger.debug(
                    "No position data returned from fetch_positions (list is empty). Assuming flat."
                )
            ...
            for pos_ccxt in fetched_positions_list:
                ...
                if not isinstance(info, dict):
                    continue  # Skip if info is not a dict
                ...
                if pos_symbol_info != market_id:
                    continue  # Symbol doesn't match
                if pos_category_info != self.config.bybit_v5_category:
                    continue  # Category doesn't match

                try:
                    pos_idx = (
                        int(pos_idx_str) if pos_idx_str is not None else -1
                    )  # Default if missing
                except (ValueError, TypeError):
                    logger.warning(
                        f"Non-integer positionIdx '{pos_idx_str}' found in position data: {info}"
                    )
                ...
                if pos_idx == self.config.position_idx:
                    ...
                    logger.debug(
                        f"Found matching position info in list (Symbol: {pos_symbol_info}, Idx: {pos_idx}): {target_pos_info}"
                    )
            ...
            side_api = str(
                target_pos_info.get("side", "None")
            ).lower()  # 'Buy' -> 'buy', 'Sell' -> 'sell', 'None' -> 'none'
            entry_price_raw = target_pos_info.get("avgPrice", "0")
            liq_price_raw = target_pos_info.get(
                "liqPrice", "0"
            )  # Note: Liq price might be 0 if no risk
            unrealized_pnl_raw = target_pos_info.get("unrealisedPnl", "0")
            sl_price_raw = target_pos_info.get(
                "stopLoss", "0"
            )  # This is the price level
            tp_price_raw = target_pos_info.get(
                "takeProfit", "0"
            )  # This is the price level
            ...
            if not is_position_open:
                logger.debug(
                    f"Position size {qty_raw} is negligible or zero after parsing. Considered flat."
                )
            ...
            liq_price = (
                liq_price
                if not liq_price.is_nan() and liq_price > 0
                else Decimal("NaN")
            )
            # SL/TP prices: Treat 0 or NaN as not set
            sl_price_active = (
                sl_price if not sl_price.is_nan() and sl_price > 0 else None
            )
            tp_price_active = (
                tp_price if not tp_price.is_nan() and tp_price > 0 else None
            )
            ...
            tsl_distance_active = (
                tsl_distance if not tsl_distance.is_nan() and tsl_distance > 0 else None
            )
            tsl_activation_price_active = (
                tsl_activation_price
                if not tsl_activation_price.is_nan() and tsl_activation_price > 0
                else None
            )
            ...
            if side_api == "buy":
                ...
                if self.config.position_idx == 1:  # Hedge Buy Pos Index
                    position_side_key = "long"
                    logger.debug(
                        "Inferred LONG position from posIdx=1 despite API side='None'."
                    )
                elif self.config.position_idx == 2:  # Hedge Sell Pos Index
                    position_side_key = "short"
                    logger.debug(
                        "Inferred SHORT position from posIdx=2 despite API side='None'."
                    )
                else:  # One-Way mode (posIdx=0) or unknown state
                    logger.warning(
                        f"Position found with size {qty_abs} but side is 'None' and posIdx is {self.config.position_idx}. Could not determine long/short state reliably. Treating as flat for safety."
                    )
                    return (
                        positions_dict  # Return empty dict if side cannot be determined
                    )
...
class IndicatorCalculator:
    ...
    def calculate_indicators(
        ...
        if df is None or df.empty:
            logger.error(
                f"{Fore.RED}{func_name} failed: No DataFrame provided.{Style.RESET_ALL}"
            )
            return None
        ...
        if not all(c in df.columns for c in required_cols):
            ...
            logger.error(
                f"{Fore.RED}{func_name} failed: DataFrame missing required columns: {missing}{Style.RESET_ALL}"
            )
            return None
        ...
        try:
            ...
            for col in required_cols:
                if df_calc[col].empty:
                    logger.warning(
                        f"{func_name}: Column '{col}' is empty, ensuring float type."
                    )
                    df_calc[col] = pd.Series(
                        dtype=float
                    )  # Ensure column exists with float type
                    continue

                # Define a safe conversion function from Any (including Decimal) to float
                def safe_to_float(x: Any) -> float:
                    dec_val = safe_decimal(
                        x, default=Decimal("NaN")
                    )  # Use safe_decimal first
                    if dec_val.is_nan() or not dec_val.is_finite():
                        return (
                            np.nan
                        )  # Convert non-finite Decimals to np.nan for pandas/numpy
                    try:
                        return float(dec_val)
                    except (ValueError, TypeError, OverflowError):
                        # Log specific conversion issues if they occur after safe_decimal
                        logger.debug(
                            f"Could not convert Decimal '{dec_val}' to float in column '{col}'."
                        )
                        return np.nan
                ...
                try:
                    ...
                    logger.error(
                        f"{func_name}: Could not cast column '{col}' to float after mapping: {e}. Contains problematic values."
                    )
            ...
            if rows_dropped > 0:
                logger.debug(
                    f"{func_name}: Dropped {rows_dropped} rows with NaN in OHLC after float conversion."
                )

            if df_calc.empty:
                logger.error(
                    f"{Fore.RED}{func_name} failed: DataFrame empty after NaN drop during processing.{Style.RESET_ALL}"
                )
                return None
            ...
            periods = [
                ...
                - 1,  # Rolling window needs this many points
                self.config.atr_period,
                self.config.adx_period * 2,  # Heuristic for ADX warmup
            ]
            min_required_len = (
                max(periods) + 20
            )  # Add a buffer for stability/convergence
            ...
            stoch_k_raw = np.where(
                (stoch_range > 1e-12)
                & stoch_range.notna(),  # Check range > 0 and not NaN
            ...
            adx_s, pdi_s, mdi_s = self._calculate_adx(
                ...
                low_s,
                close_s,
                self.config.adx_period,  # Pass close_s for ADX calc
            )
            ...
            def get_latest_decimal(series: pd.Series, name: str) -> Decimal:
                ...
                if valid_series.empty:
                    logger.warning(
                        f"{func_name}: No valid finite values found for indicator '{name}'."
                    )
                    return Decimal("NaN")
                ...
                if dec_val.is_nan():
                    logger.error(
                        f"{func_name}: Failed converting latest float value '{last_valid_float}' for '{name}' back to Decimal."
                    )
                return dec_val
            ...
            indicators_out: Dict[str, Union[Decimal, bool, int]] = {
                ...
                "pdi": get_latest_decimal(
                    pdi_s, "pdi"
                ),  # Positive Directional Indicator (+DI)
                "mdi": get_latest_decimal(
                    mdi_s, "mdi"
            ...
            stoch_values_valid = not any(
                v.is_nan() for v in [k_last, d_last, k_prev, d_prev]
            )
            ...
            if indicators_out.get("atr", Decimal("NaN")).is_nan():
                logger.error(
                    f"{Fore.RED}{func_name} failed: ATR calculation resulted in NaN. Cannot proceed with risk assessment.{Style.RESET_ALL}"
                )
...
class SignalGenerator:
    ...
    def generate_signals(
        ...
        if df_last_candles is None or len(df_last_candles) < 2:
            ...
            reason = (
                f"No Signal: Insufficient candle data (<2 rows, got {candle_count})"
            )
        ...
        try:
            ...
            for key in required_indicator_keys:
                ...
                if isinstance(val, Decimal) and not val.is_nan() and val.is_finite():
                    ...
                    missing_or_invalid_keys.append(
                        f"{key}({val})"
                    )  # Include value for context
            ...
            kd_bull = (
                indicators.get("stoch_kd_bullish", False)
                if isinstance(indicators.get("stoch_kd_bullish"), bool)
                else False
            )
            kd_bear = (
                indicators.get("stoch_kd_bearish", False)
                if isinstance(indicators.get("stoch_kd_bearish"), bool)
                else False
            )
            ...
            if self.config.trade_only_with_trend:
                ...
                price_above_trend_ema = current_price > (
                    trend_ema + trend_buffer
                )  # Price clearly above trend EMA
                price_below_trend_ema = current_price < (
                    trend_ema - trend_buffer
                )  # Price clearly below trend EMA
            ...
            if final_long_signal:
                result["long"] = True
                result["reason"] = (
                    f"Long Signal: {ema_reason} | {stoch_reason} | {trend_reason} | {atr_reason} | {adx_reason}"
                )
            elif final_short_signal:
                result["short"] = True
                result["reason"] = (
                    f"Short Signal: {ema_reason} | {stoch_reason} | {trend_reason} | {atr_reason} | {adx_reason}"
                )
            else:
                ...
                # Explain filter blocks if base signal was present
                    ...
                    if not trend_allows_long:
                        reason_parts.append(f"Long Blocked ({trend_reason})")
                    elif not significant_move:
                        reason_parts.append(f"Long Blocked ({atr_reason})")
                    elif not adx_allows_long:
                        reason_parts.append(f"Long Blocked ({adx_reason})")
                    else:
                        reason_parts.append("Unknown Long Block")  # Fallback
                elif base_short_signal:  # Base short was true, check blocks
                    if not trend_allows_short:
                        reason_parts.append(f"Short Blocked ({trend_reason})")
                    elif not significant_move:
                        reason_parts.append(f"Short Blocked ({atr_reason})")
                    elif not adx_allows_short:
                        reason_parts.append(f"Short Blocked ({adx_reason})")
                    else:
                        reason_parts.append("Unknown Short Block")  # Fallback
            ...
            logger.error(
                f"{Fore.RED}{func_name}: Error generating signals: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
    ...
    def check_exit_signals(
        ...
        if not indicators:
            logger.warning(
                f"{func_name}: Cannot check exit signals: indicators dictionary missing."
            )
            return None
        ...
        stoch_k_prev = indicators.get(
            "stoch_k_prev"
        )  # Previous K value is crucial for reversal
        ...
        invalid_inds = [
            ...
            if not isinstance(v, Decimal) or v.is_nan() or not v.is_finite()
        ]
        ...
        if position_side == "long":
            ...
            # Informational Logging: Stochastic is in the zone but hasn't confirmed reversal
                ...
                logger.debug(
                    f"{func_name} (Long): Stoch K ({stoch_k:.1f}) >= Overbought ({overbought_level}), awaiting cross below for exit signal."
                )
            ...
            # Informational Logging: Stochastic is in the zone but hasn't confirmed reversal
                ...
                logger.debug(
                    f"{func_name} (Short): Stoch K ({stoch_k:.1f}) <= Oversold ({oversold_level}), awaiting cross above for exit signal."
                )
...
class OrderManager:
    ...
    def __init__(
        ...
        if not exchange_manager or not exchange_manager.exchange:
            logger.critical(
                f"{Fore.RED}{Style.BRIGHT}OrderManager cannot initialize: ExchangeManager or CCXT Exchange instance missing.{Style.RESET_ALL}"
            )
            # Raise ValueError to prevent instantiation with invalid state
            raise ValueError(
                "OrderManager requires a valid ExchangeManager with an initialized CCXT exchange."
            )
    ...
    def _calculate_trade_parameters(
        ...
        if total_equity.is_nan() or total_equity <= 0 or not total_equity.is_finite():
            logger.error(
                f"{func_name} failed: Invalid total equity value ({total_equity})."
            )
            return None
        if (
            ...
            logger.error(
                f"{func_name} failed: Invalid current price value ({current_price})."
            )
            return None
        if not self.market_info or not all(
            k in self.market_info
            for k in ["tick_size", "contract_size", "min_order_size", "precision_dp"]
        ):
            logger.error(
                f"{func_name} failed: Market info missing required keys (tick_size, contract_size, min_order_size, precision_dp)."
            )
            return None
        ...
        try:
            ...
            logger.debug(
                f"{func_name}: Risk Amount = {risk_amount_per_trade.normalize()} (Equity {total_equity.normalize()} * Risk {self.config.risk_percentage * 100}%)"
            )
            ...
            if side == "buy":
                sl_price_raw = current_price - sl_distance_atr
            else:
                sl_price_raw = current_price + sl_distance_atr
            ...
            if sl_price.is_nan() or sl_price <= 0:
                logger.error(
                    f"{func_name} failed: Calculated SL price ({sl_price_str}) is invalid (<=0 or NaN)."
                )
                return None
            ...
            if sl_distance_price < min_tick_size:
                ...
                sl_price = safe_decimal(sl_price_str)
                logger.warning(
                    f"{func_name}: Initial SL distance < tick size. Adjusted SL price to {sl_price_str} (distance = {sl_distance_price.normalize()})."
                )
                # Final check if adjusted SL is still valid
                if sl_price.is_nan() or sl_price <= 0:
                    logger.error(
                        f"{func_name} failed: Adjusted SL price ({sl_price_str}) is still invalid (<=0 or NaN)."
                    )
                    return None
            ...
            if sl_distance_price <= 0:
                logger.error(
                    f"{func_name} failed: Calculated SL distance ({sl_distance_price}) is zero or negative."
                )
                return None
            ...
            try:
                if self.config.market_type == "inverse":
                    ...
                    if contract_size <= 0:
                        raise ValueError("Invalid contract size for inverse calc.")
                    if sl_price <= 0:
                        raise ValueError(
                            "Invalid SL price for inverse calc."
                        )  # Already checked but defensive
                    quantity = (risk_amount_per_trade * sl_price) / (
                        contract_size * sl_distance_price
                    )
                    logger.debug(
                        f"{func_name}: Inverse Qty Calc: ({risk_amount_per_trade} * {sl_price}) / ({contract_size} * {sl_distance_price}) = {quantity}"
                    )
                    ...
                    if contract_size != Decimal(1):
                        logger.warning(
                            f"{func_name}: Linear contract size is {contract_size}, not 1. Quantity calculation assumes 1 contract = 1 base unit. Verify market details if order size seems wrong."
                        )
                    ...
                    logger.debug(
                        f"{func_name}: Linear Qty Calc: {risk_amount_per_trade} / {sl_distance_price} = {quantity}"
                    )

            except (DivisionByZero, ValueError, InvalidOperation) as calc_err:
                logger.error(
                    f"{func_name} failed during quantity calculation: {calc_err}",
                    exc_info=True,
                )
                return None
            ...
            if quantity is None or quantity.is_nan() or quantity <= 0:
                logger.error(
                    f"{func_name} failed: Calculated quantity ({quantity}) is invalid or zero."
                )
                return None
            ...
            if quantity_decimal.is_nan() or quantity_decimal <= 0:
                logger.error(
                    f"{func_name} failed: Formatted quantity ({quantity_str}) is invalid or zero."
                )
                return None
            ...
            if self.config.tp_atr_multiplier > 0:
                ...
                if side == "buy":
                    tp_price_raw = current_price + tp_distance_atr
                else:
                    tp_price_raw = current_price - tp_distance_atr
                ...
                if tp_price_check.is_nan() or tp_price_check <= 0:
                    logger.warning(
                        f"{func_name}: Calculated TP price ({tp_price_str}) invalid (<=0 or NaN). Disabling TP for this trade."
                    )
                    tp_price = None
                else:
            ...
            if (
                tsl_distance_check.is_nan() or tsl_distance_check <= min_tick_size
            ):  # Ensure distance > tick size
                logger.warning(
                    f"{func_name}: Calculated TSL distance ({tsl_distance_str}) is invalid or <= tick size. Using tick size ({min_tick_size}) as minimum TSL distance."
                )
            ...
            if self.config.tsl_activation_atr_multiplier > 0:
                tsl_activation_distance = (
                    atr * self.config.tsl_activation_atr_multiplier
                )
                ...
                tsl_act_price_str = self.exchange_manager.format_price(
                    tsl_activation_price_raw
                )
                tsl_activation_price_check = safe_decimal(tsl_act_price_str)

                if (
                    tsl_activation_price_check.is_nan()
                    or tsl_activation_price_check <= 0
                ):
                    logger.warning(
                        f"{func_name}: Calculated TSL activation price ({tsl_act_price_str}) invalid (<=0 or NaN). TSL may not activate via price."
                    )
                    tsl_activation_price = None  # Mark as invalid
                else:
                    tsl_activation_price = (
                        tsl_activation_price_check  # Use valid, formatted price
                    )
            else:
                logger.debug(
                    f"{func_name}: TSL activation multiplier is 0, TSL activation price not set."
                )
            ...
            log_tsl_dist_str = (
                ...
                else "Invalid/MinTick"
            )
            log_tsl_act_str = (
                f"{params_out['tsl_activation_price'].normalize()}"
                if params_out["tsl_activation_price"]
                else "None"
            )
            logger.info(
    ...
    def _execute_market_order(
        ...
        if final_qty_decimal.is_nan() or final_qty_decimal <= 0:
            logger.error(
                f"Market order aborted: Zero/invalid formatted quantity '{qty_str}' from original '{qty_decimal}'."
            )
            return None
        ...
        try:
            ...
            if order is None:
                ...
                logger.error(
                    f"{Fore.RED}Market {side.upper()} {order_type_desc} order submission failed (returned None). Check previous logs for fail-fast reasons (e.g., Insufficient Funds).{Style.RESET_ALL}"
                )
                return None
            ...
            order_status = order.get(
                "status", "[unknown]"
            )  # e.g., 'open', 'closed', 'canceled', 'rejected', 'expired'
            ...
            termux_notify(
                ...
                f"Market {side.upper()} {order_type_desc} {final_qty_decimal.normalize()} ID:{order_id} Status:{order_status}",
            )
            ...
            if order_status in ["rejected", "canceled", "expired"]:
                ...
                reason = order.get("info", {}).get(
                    "rejectReason", "Unknown reason"
                )  # V5 field? Check Bybit docs/CCXT source
                ...
                logger.error(
                    f"{Fore.RED}Market {order_type_desc} order {order_id} was {order_status}. Reason: {reason}. Check exchange info: {order.get('info')}{Style.RESET_ALL}"
                )
            ...
            logger.debug(
                f"Waiting short delay ({wait_time}s) after order {order_id} submission before verification..."
            )
            ...
            logger.error(
                f"{Fore.RED}Market {order_type_desc} order placement failed ({type(e).__name__}): {e}{Style.RESET_ALL}"
            )
            termux_notify(
                f"{symbol} Order FAILED",
                f"Market {side.upper()} {order_type_desc} failed: {str(e)[:50]}",
            )
            return None
        except Exception as e:
            # Includes errors raised by fetch_with_retries after max retries fail
            logger.error(
                f"{Fore.RED}Unexpected error or max retries reached placing market {order_type_desc} order: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            termux_notify(
                f"{symbol} Order ERROR",
                f"Market {side.upper()} {order_type_desc} error.",
            )
            return None
    ...
    def _set_position_protection(
        ...
        sl_str = self.exchange_manager._format_v5_param(
            sl_price, "price", allow_zero=True
        )
        tp_str = self.exchange_manager._format_v5_param(
            tp_price, "price", allow_zero=True
        )
        ...
        if is_tsl:
            if tsl_distance_str and tsl_activation_price_str:
                ...
                logger.error(
                    f"Cannot activate TSL for {position_side.upper()}: Missing or invalid formatted TSL distance ('{tsl_distance_str}') or activation price ('{tsl_activation_price_str}')."
                )
        ...
        # Allows setting only SL, only TP, or both. Sending "0" cancels that specific type.
            ...
            if (
                sl_str is not None
            ):  # If sl_price was provided (even if formatted to "0")
                params["stopLoss"] = sl_str
                has_sltp_action = True
            if (
                tp_str is not None
            ):  # If tp_price was provided (even if formatted to "0")
            ...
            if has_sltp_action:
                ...
                action_desc = (
                    f"SET/UPDATE SL={params['stopLoss']} TP={params['takeProfit']}"
                )
                ...
                logger.debug(
                    f"Clearing all stops for {position_side.upper()} position (no valid SL/TP/TSL provided). Params: {params}"
                )
        ...
        logger.trade(
            f"{Fore.CYAN}Attempting to {action_desc} for {position_side.upper()} {symbol_log}...{Style.RESET_ALL}"
        )
        ...
        try:
            ...
            if (
                ...
                and response.get("retCode") == V5_SUCCESS_RETCODE
            ):
                logger.trade(
                    f"{Fore.GREEN}{Style.BRIGHT}{action_desc} successful for {position_side.upper()} {symbol_log}.{Style.RESET_ALL}"
                )
                termux_notify(
                    f"{symbol_log} Protection Set",
                    f"{action_desc} {position_side.upper()}",
                )
                # Update internal tracker on success
                self.protection_tracker[tracker_key] = new_tracker_state
                logger.debug(
                    f"Protection tracker state for {tracker_key} updated to: {new_tracker_state}"
                )
                return True
            else:
                # Handle API error response
                ret_code = (
                    response.get("retCode", "[N/A]")
                    if isinstance(response, dict)
                    else "[No Response Dict]"
                )
                ret_msg = (
                ...
                termux_notify(
                    f"{symbol_log} Protection FAILED",
            ...
            logger.error(
                f"{Fore.RED}Invalid parameters during {action_desc} for {position_side.upper()} {symbol_log}: {e}{Style.RESET_ALL}"
            )
            termux_notify(
                f"{symbol_log} Protection FAILED",
                f"{action_desc[:30]}... invalid params.",
            )
            return False
        except Exception as e:
            # Includes errors raised by fetch_with_retries after max retries fail
            logger.error(
                ...
                f"for {position_side.upper()} {symbol_log}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            termux_notify(
                f"{symbol_log} Protection ERROR", f"{action_desc[:30]}... error."
            )
            return False
    ...
    def _verify_position_state(
        ...
        """
        logger.debug(
            f"Verifying position state ({action_context}): Expecting '{expected_side or 'FLAT'}' (MinQty: {expected_qty_min.normalize() if expected_side else 'N/A'}). Will attempt {max_attempts} times..."
        )
        last_known_position_state: Optional[Dict[str, Dict[str, Any]]] = None

        for attempt in range(max_attempts):
            logger.debug(
                f"{action_context} verification attempt {attempt + 1}/{max_attempts}..."
            )
            ...
            if (
                not active_pos_data
                and short_pos_data
                and isinstance(short_pos_data.get("qty"), Decimal)
            ):
            ...
            if expected_side is None:  # Expecting flat
            ...
            ):  # Expecting a specific side, and it matches
                # Check if quantity meets the minimum expectation
                qty_met = actual_qty >= expected_qty_min
                verification_met = (
                    qty_met  # Success only if side AND quantity match expectation
                )
                log_msg_suffix = (
                    f"Expected {expected_side.upper()} (MinQty~{expected_qty_min.normalize()}), "
                ...
                verification_met = False
                log_msg_suffix = (
                    f"Expected {expected_side.upper()}, "
            ...
            if verification_met:
                ...
                return (
                    True,
                    current_positions,
                )  # Verification succeeded, return the confirmed state

            # Verification not met, wait for next attempt if any remain
            if attempt < max_attempts - 1:
                logger.debug(
                    f"State not as expected, waiting {delay_seconds}s before next check..."
                )
                time.sleep(delay_seconds)
            else:
                # Max attempts reached, verification failed
                logger.error(
                    f"{Fore.RED}{action_context} Verification FAILED after {max_attempts} attempts. Final state check: {log_msg_suffix}{Style.RESET_ALL}"
                )
        ...
        )  # Return last known state even if loop exits unexpectedly
    ...
    def place_risked_market_order(
        ...
        if not self.exchange or not self.market_info:
            ...
            return False, None
        if side not in ["buy", "sell"]:
            logger.error(f"Entry Aborted: Invalid side '{side}'.")
            return False, None
        # Basic validation of inputs (should be pre-validated, but double-check)
        if atr.is_nan() or atr <= 0:
            logger.error("Entry Aborted: Invalid ATR provided.")
            return False, None
        if total_equity.is_nan() or total_equity <= 0:
            logger.error("Entry Aborted: Invalid Equity provided.")
            return False, None
        if current_price.is_nan() or current_price <= 0:
            logger.error("Entry Aborted: Invalid Price provided.")
            return False, None
        ...
        if (
            not trade_params
            or not isinstance(trade_params.get("qty"), Decimal)
            or not isinstance(trade_params.get("sl_price"), Decimal)
        ):
            logger.error(
                "Entry Aborted: Failed to calculate valid trade parameters (qty or sl_price missing/invalid)."
            )
        ...
        initial_tsl_activation_price = trade_params.get(
            "tsl_activation_price"
        )  # Can be None
        ...
        if not order_info:
            logger.error(
                "Entry Aborted: Market order execution failed or was rejected."
            )
        ...
        if not verification_ok:
            logger.error(
                f"{Fore.RED}Entry Failed: Position verification FAILED after market order {order_id}. State may be inconsistent. Manual check required! Attempting cleanup...{Style.RESET_ALL}"
            )
            # Attempt to handle the failure (e.g., close unexpected position)
            self._handle_entry_failure(
                position_side, qty_to_order, final_pos_state_dict
            )
        ...
        if not final_pos_state_dict or not final_pos_state_dict.get(position_side):
            ...
            logger.error(
                f"{Fore.RED}Internal Error: Position verified OK but data missing for '{position_side}' in final state dict {final_pos_state_dict}. Halting entry.{Style.RESET_ALL}"
            )
            self._handle_entry_failure(
                position_side, qty_to_order, final_pos_state_dict
            )
