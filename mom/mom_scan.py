...
MAX_KLINE_LIMIT_PER_REQUEST = 2000  # Maximum klines fetchable in one V5 API call
MIN_KLINE_RECORDS_FOR_CALC = 2 # Minimum records needed for most basic calculations
...
def instantaneous_trendline(series: pd.Series, period: int) -> pd.Series:
    ...
    if len(series_cleaned) < 4:
        log_console(logging.DEBUG, f"Instantaneous Trendline: Series length ({len(series_cleaned)} non-NaN) is less than required minimum (4). Returning NaNs.")
        return pd.Series(np.nan, index=series.index, dtype=np.float64)
...
def calculate_indicators_momentum(df: pd.DataFrame, strategy_cfg: Dict[str, Any], config: Dict[str, Any]) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not all(col in df_out.columns for col in trend_req_cols):
            ...
            # Fill initial shift NaN with the first available trendline value
            trendline_shift = df_out[trendline_col].shift(1).ffill().fillna(method='bfill')
    ...
    try:
        ...
        if balance_info and balance_info.get("retCode") == 0:
            ...
            if account_type.upper() == "UNIFIED":
                ...
                # For CONTRACT/SPOT, 'list' might contain the coin balances directly
...
def calculate_position_size_atr(
    ...
    if balance <= 0 or risk_percent <= 0 or entry_price <= 0 or min_order_qty <= 0 or qty_step_float <= 0:
    ...
    if sl_distance_price < min_sl_value:
        log_console(logging.WARNING, f"Position Size Calc: Stop loss distance ({sl_distance_price}) is too small (< {min_sl_value}). Cannot calculate size reliably. Returning 0.")
        return 0.0
    ...
    def log_summary(self, symbol: Optional[str] = None, force: bool = False, interval: int = 3600):
        ...
        with self.lock:
            ...
            if self.total_trades > 0:
                ...
                if total_losses > 1e-9: # Avoid division by zero
                    profit_factor = total_gains / total_losses
                elif total_gains > 1e-9: # Handle case with wins but no losses
...
class SymbolTrader:
            ...
            if info and info.get("retCode") == 0:
                ...
                if result_list:
                    ...
                    try:
                        ...
                        if '.' in self.qty_step and self.qty_step_float > 0:
                            decimal_part = self.qty_step.split('.')[-1]
                            self.qty_precision = len(decimal_part) if decimal_part != '0' else 0
                        else: self.qty_precision = 0

                        if '.' in self.tick_size and tick_size_float > 0:
                            decimal_part = self.tick_size.split('.')[-1]
                            self.price_precision = len(decimal_part) if decimal_part != '0' else 0
                    ...
                    if self.min_order_qty <= 0 or self.qty_step_float <= 0 or tick_size_float <= 0:
                    ...
                    log_console(
                        ...
                        f"Tick Size={self.tick_size} (Float: {tick_size_float:.{self.price_precision+1}f}, Precision: {self.price_precision})",
                        symbol=self.symbol,
                    )
                    return True
                else:
    ...
    def _initial_setup(self):
        ...
        try:
            ...
            # Example: session.switch_margin_mode(...)
    ...
    def _process_kline_queue(self) -> bool:
        ...
        while not self.kline_queue.empty() and processed_count < max_items_per_cycle:
            try:
                ...
                log_console(logging.ERROR, f"Error processing kline queue item: {e}", symbol=self.symbol, exc_info=True)
        ...
        if df_updated and self.kline_df is not None and not self.kline_df.empty:
            ...
            if calculated_df is not None and not calculated_df.empty:
                self.kline_df = calculated_df # Replace internal DF with the one containing new indicators
            ...
            if response and response.get("retCode") == 0:
                ...
                if kline_list:
                    ...
                    if df_with_indicators is None or df_with_indicators.empty:
                        log_console(logging.WARNING, "REST KLine: Indicator calculation failed.", symbol=self.symbol)
    ...
    def get_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        if self.kline_df is None or self.kline_df.empty:
            ...
            # Always fetch via REST if WS disabled, but use existing DF if available and recent
        ...
        if needs_rest_fetch:
            ...
            if rest_df_with_indicators is not None and not rest_df_with_indicators.empty:
                self.kline_df = rest_df_with_indicators # Update internal state
                ...
                for pos in position_list:
                    ...
                    if pos.get("symbol") == self.symbol:
                        try:
                            ...
                            min_significant_qty = self.min_order_qty * 0.01 if self.min_order_qty > 0 else 1e-9
                ...
                if response and response.get("retCode") == 0:
                    ...
                    if order_list:
                        ...
                        try:
                            ...
                            # Use avgPrice only if valid positive number string and qty > 0
                            if avg_price_str and filled_qty > 1e-9:
                                try:
                                    temp_avg_price = float(avg_price_str)
                                    if temp_avg_price > 1e-9:
    ...
    def place_order(self, side: str, qty: float, stop_loss_price: Optional[float] = None,
        ...
        if take_profit_price is not None:
            if not isinstance(take_profit_price, (float, int)) or take_profit_price <= 0:
        ...
        if self.dry_run:
            ...
            if sim_entry_price <= 0: sim_entry_price = 1.0 # Dummy fallback
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                elif error_code == 110073: log_console(logging.ERROR,"ORDER REJECTED: TP/SL price invalid or too close to market.", symbol=self.symbol)
                return None
        except Exception as e:
    ...
    def close_position(self, position_data: Dict[str, Any], exit_reason: str = "Signal", exit_price_est: Optional[float] = None) -> bool:
        ...
        if not (side in ["Buy", "Sell"] and isinstance(size, float) and abs(size) > 1e-9 and isinstance(entry_price, float) and entry_price > 0):
        ...
        abs_size = abs(size)
        # Check if position size is negligible (e.g., dust)
        min_significant_qty = self.min_order_qty * 0.01 if self.min_order_qty > 0 else 1e-9
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    ...
                    if filled_qty >= abs_size * min_close_fill_ratio:
                        ...
                        final_exit_price_log = avg_exit_price if avg_exit_price is not None and avg_exit_price > 0 else exit_price_est
                ...
                pos_already_closed_codes = [110025, 110066, 3400074] # Add relevant codes as found
                ...
                if already_closed:
                    ...
                    if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                        ...
                        final_exit_price = exit_price_est if exit_price_est is not None and exit_price_est > 0 else entry_price
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        # Check if fetch and indicator calculation succeeded
        ...
        try:
            ...
            log_console(logging.WARNING, f"Index error accessing HTF ({higher_tf}) data (need >= {MIN_KLINE_RECORDS_FOR_CALC} rows). Allowing trade.", symbol=self.symbol)
            self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
            return True
        except Exception as e:
...
class MomentumScannerTrader:
    ...
    def __init__(self, api_key: str, api_secret: str, config: Dict[str, Any], dry_run: bool = False):
        ...
        elif self.account_type == "UNIFIED": self.category = "linear" # Default UNIFIED to linear perpetuals
        elif self.account_type == "CONTRACT": self.category = "linear" # Default CONTRACT to linear (could be inverse)
    ...
    def _initialize_traders(self):
        ...
        for i, symbol_cfg in enumerate(deepcopy(symbols_cfg_list)):
            ...
            try:
                ...
                # Pass the shared HTTP session and category
                trader = SymbolTrader(
                ...
                try:
                    ...
                    for symbol, trader in self.traders.items():
                        if trader.use_websocket:
                            ...
                            # Subscribe using the trader's specific callback method
                            temp_ws.subscribe(topic, callback=trader._websocket_callback)
                    ...
                    if topics_to_subscribe:
                        ...
                        # Start the WebSocket connection implicitly by interacting (e.g., subscribing handled above)
                        # The on_open callback will set self.ws_active = True
                        log_console(logging.INFO, f"WebSocket connection initiated for {len(topics_to_subscribe)} streams.")

                        # Wait for connection to establish or fail (check ws_active flag)
                        wait_time = 0; max_wait = 30 # Wait up to 30s for connection
                    ...
                    log_console(logging.ERROR, f"Failed to init/start WS connection: {e}. Retrying in {retry_delay}s.", exc_info=False)
                    self._close_websocket_connection() # Ensure cleanup
                    if not self.shutdown_requested: time.sleep(retry_delay)
                    retry_delay = min(max_retry_delay, retry_delay * 2) # Exponential backoff

            else: # WS is active, sleep and check later
                time.sleep(10) # Check less frequently when connection is stable
        ...
        with self.ws_lock:
            if self.ws:
                ...
                try:
                    ...
                    # Ensure variables are reset even if exit() fails
                    self.ws = None
    ...
    def _fetch_initial_data_worker(self):
        ...
        for symbol, trader in traders_to_fetch:
            ...
            if trader.use_websocket and (trader.kline_df is None or trader.kline_df.empty):
                ...
                time.sleep(0.5) # Throttle requests slightly between symbols
        ...
        try:
            ...
            if isinstance(message, dict):
                ...
                # Handle subscription response
                    ...
                    topic_info = message.get("ret_msg", "")
                ...
                # Handle other potential ops if necessary (e.g., 'ping')
                # else: log_console(logging.DEBUG, f"Unhandled WS Op: {op}")
    ...
    def _handle_ws_error(self, error):
        ...
        # Log the error, including exception info if it's an exception object
        log_console(logging.ERROR, f"WebSocket Error Encountered: {error}", exc_info=isinstance(error, Exception))
    ...
    def _handle_ws_close(self, ws_app):
        ...
        if not self.shutdown_requested:
            ...
            with self.ws_lock:
                ...
                self.ws_active = False
                self.ws = None # Clear the WS object reference
        else:
    ...
    def run(self):
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                balance_per_trader = balance / num_active_traders if num_active_traders > 0 and balance > 0 else 0.0
                max_pos_usdt_config = self.risk_cfg.get("MAX_POSITION_USDT")
                max_pos_usdt_per_trader: Optional[float] = None
                if isinstance(max_pos_usdt_config, (int, float)) and max_pos_usdt_config > 0 and num_active_traders > 0:
                ...
                for symbol in list(self.traders.keys()):
                    ...
                    try:
                        ...
                        if df_with_indicators is None or df_with_indicators.empty or len(df_with_indicators) < MIN_KLINE_RECORDS_FOR_CALC:
                        ...
                        try:
                            ...
                            log_console(logging.WARNING, f"Cannot access candle data (need >= {MIN_KLINE_RECORDS_FOR_CALC} rows, got {len(df_with_indicators)}). Skipping.", symbol=symbol)
                            continue
                        ...
                        if position:
                            ...
                            if not (pos_side in ["Buy", "Sell"] and isinstance(pos_size, float) and abs(pos_size) > 1e-9 and isinstance(entry_price, float) and entry_price > 0):
                                ...
                                continue # Re-evaluate in next cycle if needed
                            ...
                            if entry_side:
                                ...
                                if stop_loss_price is None or stop_loss_price <= 0:
                                    ...
                                    continue
                                # Ensure TP is valid and logical relative to entry
                                if take_profit_price is not None:
                                    if take_profit_price <= 0:
                                        log_console(logging.DEBUG, f"Invalid TP calculated ({take_profit_price}). TP not set.", symbol=symbol)
                                        take_profit_price = None
                                    elif entry_side == "Buy" and take_profit_price <= entry_price_est:
                                        log_console(logging.DEBUG, f"Long TP ({take_profit_price}) <= Entry ({entry_price_est}). TP not set.", symbol=symbol)
                                        take_profit_price = None
                                    elif entry_side == "Sell" and take_profit_price >= entry_price_est:
                                        log_console(logging.DEBUG, f"Short TP ({take_profit_price}) >= Entry ({entry_price_est}). TP not set.", symbol=symbol)
                                        take_profit_price = None
    ...
    def shutdown(self):
        ...
        if self.ws_connection_thread and self.ws_connection_thread.is_alive():
            ...
            if self.ws_connection_thread.is_alive():
                log_console(logging.WARNING,"WebSocket thread join timed out. Forcing WS closure.")
        ...
        if close_on_exit and not self.dry_run:
            ...
            log_console(logging.INFO, "Dry run mode: No real positions to close.")
            ...
            for symbol, trader in list(self.traders.items()):
                 if trader.current_trade:
                      ...
                      if not last_price_sim: last_price_sim = trader.current_trade.get("avgPrice", 0)

                      trader.metrics.add_trade(
                           symbol=symbol,
                           entry_time=trader.current_trade.get("entry_time", dt.datetime.now(dt.timezone.utc)),
                           exit_time=dt.datetime.now(dt.timezone.utc),
                           side=trader.current_trade.get("side", "Buy"),
                           entry_price=trader.current_trade.get("avgPrice", 0),
                           exit_price=last_price_sim,
                           qty=trader.current_trade.get("size", 0),
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not validation_errors: # Proceed with detailed checks only if top sections are okay
            ...
            if not isinstance(risk_percent, (int, float)) or not (0 < risk_percent <= 10): validation_errors.append(f"RISK_CONFIG: RISK_PER_TRADE_PERCENT ({risk_percent}) invalid (must be a positive number, e.g., 0.1 to 10).")
            ...
            if max_pos_usdt is not None and (not isinstance(max_pos_usdt, (int, float)) or max_pos_usdt <= 0): validation_errors.append(f"RISK_CONFIG: MAX_POSITION_USDT ({max_pos_usdt}) must be a positive number if set.")
            if args.dry_run:
                dry_run_balance = risk_cfg.get("DRY_RUN_DUMMY_BALANCE")
                if dry_run_balance is None or (not isinstance(dry_run_balance, (int, float)) or dry_run_balance <= 0): validation_errors.append(f"RISK_CONFIG: DRY_RUN_DUMMY_BALANCE ({dry_run_balance}) must be a positive number when using --dry-run.")
            if not isinstance(risk_cfg.get("ATR_MULTIPLIER_SL", 1.5), (int, float)) or risk_cfg.get("ATR_MULTIPLIER_SL", 1.5) <= 0: validation_errors.append("RISK_CONFIG: ATR_MULTIPLIER_SL must be a positive number.")
            ...
            # Add validation for TSL parameters if TSL is enabled

            # Bot Config Validation
            if not isinstance(bot_cfg.get("SLEEP_INTERVAL_SECONDS"), (int, float)) or bot_cfg.get("SLEEP_INTERVAL_SECONDS") <= 0: validation_errors.append("BOT_CONFIG: SLEEP_INTERVAL_SECONDS must be a positive number.")
            if not isinstance(bot_cfg.get("KLINE_LIMIT_BUFFER"), int) or bot_cfg.get("KLINE_LIMIT_BUFFER") < 0: validation_errors.append("BOT_CONFIG: KLINE_LIMIT_BUFFER must be a non-negative integer.")
            if not isinstance(bot_cfg.get("API_RECV_WINDOW", 10000), int) or bot_cfg.get("API_RECV_WINDOW", 10000) <= 0: validation_errors.append("BOT_CONFIG: API_RECV_WINDOW must be a positive integer.")
            if not isinstance(bot_cfg.get("WS_PING_INTERVAL", 20), int) or bot_cfg.get("WS_PING_INTERVAL", 20) <= 0: validation_errors.append("BOT_CONFIG: WS_PING_INTERVAL must be a positive integer.")
            if not isinstance(bot_cfg.get("WS_PING_TIMEOUT", 10), int) or bot_cfg.get("WS_PING_TIMEOUT", 10) <= 0: validation_errors.append("BOT_CONFIG: WS_PING_TIMEOUT must be a positive integer.")
            if not isinstance(bot_cfg.get("WS_STARTUP_DELAY_SECONDS", 5.0), (int, float)) or bot_cfg.get("WS_STARTUP_DELAY_SECONDS", 5.0) < 0: validation_errors.append("BOT_CONFIG: WS_STARTUP_DELAY_SECONDS must be non-negative.")
            if not isinstance(bot_cfg.get("ORDER_CONFIRM_RETRIES", 3), int) or bot_cfg.get("ORDER_CONFIRM_RETRIES", 3) < 0: validation_errors.append("BOT_CONFIG: ORDER_CONFIRM_RETRIES must be a non-negative integer.")
            if not isinstance(bot_cfg.get("ORDER_CONFIRM_DELAY_SECONDS", 2.0), (int, float)) or bot_cfg.get("ORDER_CONFIRM_DELAY_SECONDS", 2.0) <= 0: validation_errors.append("BOT_CONFIG: ORDER_CONFIRM_DELAY_SECONDS must be a positive number.")
            ...
            if not isinstance(bot_cfg.get("HIGHER_TF_CACHE_SECONDS", 3600), int) or bot_cfg.get("HIGHER_TF_CACHE_SECONDS", 3600) < 0: validation_errors.append("BOT_CONFIG: HIGHER_TF_CACHE_SECONDS must be a non-negative integer.")
            if not isinstance(bot_cfg.get("KLINE_STALENESS_SECONDS", 300), int) or bot_cfg.get("KLINE_STALENESS_SECONDS", 300) <= 0: validation_errors.append("BOT_CONFIG: KLINE_STALENESS_SECONDS must be a positive integer.")
            if not isinstance(bot_cfg.get("BALANCE_CHECK_INTERVAL_SECONDS", 300), int) or bot_cfg.get("BALANCE_CHECK_INTERVAL_SECONDS", 300) <= 0: validation_errors.append("BOT_CONFIG: BALANCE_CHECK_INTERVAL_SECONDS must be a positive integer.")
            if not isinstance(bot_cfg.get("COOLDOWN_PERIOD_SECONDS", 300), int) or bot_cfg.get("COOLDOWN_PERIOD_SECONDS", 300) < 0: validation_errors.append("BOT_CONFIG: COOLDOWN_PERIOD_SECONDS must be a non-negative integer.")
            if not isinstance(bot_cfg.get("METRICS_LOG_INTERVAL_SECONDS", 3600), int) or bot_cfg.get("METRICS_LOG_INTERVAL_SECONDS", 3600) <= 0: validation_errors.append("BOT_CONFIG: METRICS_LOG_INTERVAL_SECONDS must be a positive integer.")
            ...
            if not isinstance(symbols, list): validation_errors.append("BYBIT_CONFIG: SYMBOLS must be a list.")
                ...
                seen_symbols = set()
                # Define valid timeframes based on Bybit V5 API documentation
                ...
                for i, symbol_cfg in enumerate(symbols):
                    ...
                    # Allow leverage 0 or 1 for spot? Check Bybit rules. Assuming 1-100 for derivatives.
                    # For spot, leverage is irrelevant, maybe validate based on category later?
                    if account_type != "SPOT" and (not isinstance(leverage, int) or not (1 <= leverage <= 100)): validation_errors.append(f"Symbol {symbol}: LEVERAGE ({leverage}) invalid (must be an integer between 1 and 100 for {account_type}).")
                    ...
                    if isinstance(strategy_cfg, dict):
                         ...
                         for k, v in strategy_cfg.items():
                              ...
                              if k.upper() in ["MOM_THRESHOLD_PERCENT", "ADX_THRESHOLD", "MIN_SIGNAL_STRENGTH", "RSI_LOW_THRESHOLD", "RSI_HIGH_THRESHOLD"] and not isinstance(v, (int, float)): validation_errors.append(f"Symbol {symbol}: Strategy param '{k}' ({v}) must be a number.")
                              ...
                              # Check price source
                              if k.upper() == "PRICE_SOURCE" and not isinstance(v, str): validation_errors.append(f"Symbol {symbol}: Strategy param 'PRICE_SOURCE' ({v}) must be a string (e.g., 'close', 'hlc3').")
