Okay, I've enhanced the provided Python script and incorporated fixes based on the log errors and general best practices for clarity, robustness, and safety.

**Key Improvements Made:**

1.  **BUG FIX (Log Error):** Corrected the invalid format specifier (`.0.0001f`) error. Logging now dynamically uses market precision fetched from CCXT for prices and amounts (e.g., `{value:.{precision_digits}f}`). This fix was applied primarily in `analyze_order_book` and `trade_logic` where market data is logged.
2.  **Robustness (`get_current_position`):**
    *   Improved parsing of quantity and entry price, prioritizing unified fields but reliably falling back to Bybit V5 `info` fields (`size`, `avgPrice`).
    *   Added explicit checks for `positionIdx == 0` (One-Way mode).
    *   Enhanced logging to clearly show which fields are used and why an entry might be skipped.
    *   Uses `FLOAT_COMPARISON_EPSILON` for checking if quantity is effectively zero.
3.  **Robustness (Order Placement & Closing):**
    *   `place_risked_market_order`: Uses market precision for calculations and logging, handles potential `None` values from balance/OB/ticker fetches more gracefully, performs epsilon checks on critical values (ATR, prices, quantity), recalculates margin estimate if quantity is capped. Includes calculated SL price in success SMS.
    *   `close_position`: Uses market precision for logging, relies on its internal re-validation step before closing.
    *   `calculate_position_size`: Added epsilon check for price difference.
4.  **Error Handling & Logging:**
    *   Improved SMS alert function (`send_sms_alert`) to log `stderr`/`stdout` from the `termux-sms-send` command on failure, providing more diagnostic information.
    *   Added more `try...except` blocks for specific API calls within functions to prevent entire cycles/operations from failing due to recoverable errors (e.g., fetching OB or ticker).
    *   Refined logging messages throughout for clarity and consistency. Debug logs added for complex sections like position checking and risk calculation.
    *   Main loop includes more specific `ccxt` exception handling (`RateLimitExceeded`, `NetworkError`, `ExchangeNotAvailable`, `AuthenticationError`, `InsufficientFunds`) with appropriate actions (sleep, retry, stop).
    *   Graceful shutdown includes a final confirmation check after attempting to close the position and sends clearer SMS alerts about the outcome.
5.  **Consistency (Bybit V5):** Ensured `params={'category': 'linear'}` is consistently added to relevant CCXT calls (`fetch_balance`, `fetch_ohlcv`, `fetch_ticker`, `fetch_l2_order_book`, `fetch_positions`, `set_leverage`, `create_market_order`).
6.  **Safety Warnings:** Made the warnings about `ENABLE_LOOP_SLTP_CHECK` even more prominent in the docstring, main function logging, and initial console output.
7.  **Readability:** Added minor comments and ensured consistent use of constants and type hints where appropriate.

Here is the complete improved version of the script:
...
Version: 2.0.2 (Bug Fixes, Enhanced Robustness & Logging, V5 API Focus)
...
- Dynamic precision formatting for logging based on fetched market data.
...
BOT_VERSION = "2.0.2" # Updated version
...
def send_sms_alert(message: str) -> bool:
    ...
    Requires Termux and Termux:API with SMS permissions granted. Logs detailed errors.
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Log stderr and stdout for better diagnostics
...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        # Use try-except block specifically for the balance fetch to give clearer auth errors
        try:
            exchange.fetch_balance(params={'category': 'linear'}) # Throws AuthenticationError on bad keys
        except ccxt.AuthenticationError as auth_err:
             logger.critical(f"Authentication failed during balance check: {auth_err}. Check API key/secret, ensure IP whitelist (if used) is correct, and API permissions (read/trade) are sufficient for contract trading.")
             send_sms_alert(f"[ScalpBot] CRITICAL: Authentication FAILED during balance check: {auth_err}. Bot stopped.")
             return None
        ...
        # This might catch issues during load_markets if auth is needed early
        logger.critical(f"Authentication failed during initialization: {e}. Check API key/secret, IP whitelist, permissions.")
        send_sms_alert(f"[ScalpBot] CRITICAL: Authentication FAILED on Init: {e}. Bot stopped.")
...
def calculate_supertrend(
    ...
    try:
        ...
        df[st_trend_col] = df[st_trend_col].ffill() # Fill potential NaNs from calculation start
        ...
        if pd.notna(df[f'{col_prefix}trend'].iloc[-1]) and pd.notna(df[f'{col_prefix}supertrend'].iloc[-1]):
            ...
            logger.debug(f"Indicator Calc: Calculated {col_prefix}Supertrend({length}, {multiplier}). "
                         f"Last trend: {last_trend_str}, Last value: {last_supertrend_val:.4f}") # Use fixed precision for log
        else:
...
def analyze_volume_atr(
    ...
    try:
        ...
        if atr_col in df.columns and pd.notna(df[atr_col].iloc[-1]):
            ...
            # Use epsilon comparison to check if ATR is significantly positive
            results["atr"] = last_atr if last_atr > FLOAT_COMPARISON_EPSILON else None
            if results["atr"] is None:
                logger.warning(f"Indicator Calc: Calculated ATR({atr_len}) is zero or negative ({last_atr:.5f}). Setting ATR to None.")
        else:
...
def analyze_order_book(
    ...
    and spread within the specified depth. Uses dynamic precision for logging.
    ...
    try:
        # Get market info for category determination and precision
        ...
        if best_bid > FLOAT_COMPARISON_EPSILON and best_ask > FLOAT_COMPARISON_EPSILON:
            ...
            logger.debug(f"Order Book: Best Bid={best_bid:.{price_precision_digits}f}, Best Ask={best_ask:.{price_precision_digits}f} (Spread requires both > {FLOAT_COMPARISON_EPSILON})")
        ...
        if ask_volume_sum > FLOAT_COMPARISON_EPSILON:
            ...
            logger.debug(f"Order Book: Bid/Ask Ratio (Depth {depth}) = {bid_ask_ratio:.3f}") # Fixed precision for ratio is ok
        else:
            logger.debug(f"Order Book: Bid/Ask Ratio calculation skipped (Ask volume at depth {depth} is zero or negligible)")

    except ccxt.BadSymbol: # Handle case where symbol becomes invalid mid-run? Unlikely but possible
        logger.error(f"Order Book: Invalid symbol {symbol} encountered during analysis.")
...
def get_market_data(
    ...
    try:
        ...
        # Bybit V5 requires 'category' param for linear contracts
        params = {}
        market = exchange.market(symbol) # Get market info for category
        if market.get('linear'):
            params = {'category': 'linear'}
        elif market.get('inverse'):
        ...
        logger.error(f"Data Fetch: Invalid symbol {symbol} encountered during OHLCV fetch.")
        return None
    except ccxt.NetworkError as e:
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
    ...
    for Bybit V5 API structure via CCXT. Assumes One-Way mode (`positionIdx=0`).
    ...
    market = None
    market_id = None
    qty_precision_digits = 8 # Default for logging internal steps
    ...
    try:
        ...
        for pos in all_positions:
            ...
            if pos_symbol_raw != market_id:
                # logger.debug(f"Skipping position entry: Raw symbol '{pos_symbol_raw}' != Target '{market_id}'")
            ...
            # logger.debug(f"Position Check ({market_id}): Unified Side = '{unified_side}' -> Determined Side = {determined_side}")
            ...
            if determined_side == POSITION_SIDE_NONE:
                ...
                # logger.debug(f"Position Check ({market_id}): Falling back to V5 info.side = '{pos_side_v5}'")
                if pos_side_v5 == 'Buy':
                    determined_side = POSITION_SIDE_LONG
                elif pos_side_v5 == 'Sell':
                    determined_side = POSITION_SIDE_SHORT
                # logger.debug(f"Position Check ({market_id}): Fallback resulted in Determined Side = {determined_side}")
            ...
            quantity = 0.0
            qty_source = "None" # Track source for logging

            if quantity_unified is not None:
                try:
                    quantity = float(quantity_unified)
                    qty_source = "Unified ('contracts')"
                except (ValueError, TypeError):
                    logger.warning(f"Position Check ({market_id}): Could not parse unified quantity 'contracts': '{quantity_unified}'. Trying fallback.")
            ...
            if quantity_unified is None:
                ...
                # logger.debug(f"Position Check ({market_id}): Falling back to V5 info.size = '{size_str_v5}'")
                if size_str_v5 is not None and size_str_v5 != "":
                    try:
                        quantity = float(size_str_v5)
                        qty_source = "V5 ('info.size')"
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Position Check ({market_id}): Error parsing fallback quantity 'info.size': '{size_str_v5}'. Error: {e}. Treating as zero.")
                        quantity = 0.0
                        qty_source = "V5 Parse Error"
                else:
                    logger.warning(f"Position Check ({market_id}): Could not find valid quantity in unified 'contracts' or fallback 'info.size'. Treating as zero.")
                    quantity = 0.0
                    qty_source = "Missing"
            # logger.debug(f"Position Check ({market_id}): Qty = {quantity:.{qty_precision_digits}f} (Source: {qty_source})")
            ...
            if abs(quantity) > FLOAT_COMPARISON_EPSILON:
                ...
                if entry_price_unified is not None:
                    try:
                        ...
                        # Check if price is valid (greater than epsilon)
                        if entry_price <= FLOAT_COMPARISON_EPSILON:
                            logger.warning(f"Position Check ({market_id}): Unified entry price '{entry_price_unified}' is <= epsilon. Treating as invalid.")
                            entry_price = 0.0 # Treat invalid price as 0
                            entry_price_unified = None # Force fallback
                        else:
                            price_source = "Unified ('entryPrice')"
                    except (ValueError, TypeError):
                        logger.warning(f"Position Check ({market_id}): Could not parse unified entry price '{entry_price_unified}'. Trying fallback.")
                        entry_price_unified = None # Force fallback
                ...
                if entry_price_unified is None:
                    ...
                    # logger.debug(f"Position Check ({market_id}): Falling back to V5 info.avgPrice = '{avg_price_str_v5}'")
                    if avg_price_str_v5 is not None and avg_price_str_v5 != "":
                        try:
                            entry_price = float(avg_price_str_v5)
                            # Check if price is valid
                            if entry_price <= FLOAT_COMPARISON_EPSILON:
                                logger.warning(f"Position Check ({market_id}): Fallback entry price 'info.avgPrice' '{avg_price_str_v5}' is <= epsilon. Defaulting to 0.0.")
                                entry_price = 0.0
                                price_source = "V5 Invalid"
                            else:
                                price_source = "V5 ('info.avgPrice')"
                        except (ValueError, TypeError):
                            logger.warning(f"Position Check ({market_id}): Could not parse fallback entry price 'info.avgPrice': '{avg_price_str_v5}'. Defaulting to 0.0.")
                            entry_price = 0.0
                            price_source = "V5 Parse Error"
                    else:
                         logger.warning(f"Position Check ({market_id}): Fallback entry price 'info.avgPrice' is missing or empty. Defaulting to 0.0.")
                         entry_price = 0.0
                         price_source = "Missing"
                # logger.debug(f"Position Check ({market_id}): Entry Price = {entry_price:.4f} (Source: {price_source})") # Use fixed precision for log

                # Get market price precision for final logging
                price_precision_digits = market.get('precision', {}).get('price', 4)

                # Store the found position details
                found_position = {'side': determined_side, 'qty': abs(quantity), 'entry_price': entry_price}
                logger.info(f"Position Check: Found ACTIVE position: {determined_side} {abs(quantity):.{qty_precision_digits}f} {market_id} @ Entry={entry_price:.{price_precision_digits}f} (QtySrc: {qty_source}, PxSrc: {price_source})")
                ...
                logger.debug(f"Skipping position entry for {market_id}: Quantity {quantity:.{qty_precision_digits+2}f} (Source: {qty_source}) is <= epsilon {FLOAT_COMPARISON_EPSILON}. Treating as closed.")
...
def close_position(
    ...
    try:
        ...
        # Use dynamic precision for logging fill price and quantity
        ...
        cost_str = f"{order_cost:.2f}" if order_cost is not None else "N/A" # USDT cost usually 2 decimal places
...
def calculate_position_size(
    ...
    try:
        ...
        price_precision_digits = market.get('precision', {}).get('price', 4)
        qty_precision_digits = market.get('precision', {}).get('amount', 8)
        logger.debug(f"Risk Calc: Using Price Precision: {price_precision_digits}, Amount Precision: {qty_precision_digits}")
    except Exception as market_err:
    ...
    if not (entry_price > FLOAT_COMPARISON_EPSILON and stop_loss_price > FLOAT_COMPARISON_EPSILON):
        logger.error(f"Risk Calc: Invalid entry ({entry_price:.{price_precision_digits}f}) or stop-loss ({stop_loss_price:.{price_precision_digits}f}) price (must be > ~0).")
        return None, None
    price_difference_per_unit = abs(entry_price - stop_loss_price)
    # Use epsilon comparison for float check
    if price_difference_per_unit < FLOAT_COMPARISON_EPSILON:
...
def place_risked_market_order(
    ...
    Sends SMS alerts. Includes calculated SL price in success SMS.
    ...
    try:
        ...
        except (ccxt.NetworkError, ccxt.ExchangeError) as bal_err:
             logger.error(f"Place Order: Failed to fetch balance: {bal_err}. Aborting order.")
             return None
        ...
        ob_data = analyze_order_book(exchange, symbol, SHALLOW_OB_FETCH_DEPTH, ORDER_BOOK_FETCH_LIMIT) # analyze_order_book handles its own errors
        ...
        if side == SIDE_BUY and best_ask and best_ask > FLOAT_COMPARISON_EPSILON:
            ...
            try:
                ...
                if last_price and isinstance(last_price, (int, float)) and float(last_price) > FLOAT_COMPARISON_EPSILON:
                    ...
                    logger.error(f"Place Order: Ticker 'last' price is invalid ({last_price}). Cannot estimate entry price.")
                    entry_price_estimate = None # Ensure it's None if ticker price is invalid
            except (ccxt.NetworkError, ccxt.ExchangeError) as ticker_err:
            ...
            except Exception as ticker_err: # Catch other unexpected errors
                logger.error(f"Place Order: Unexpected error fetching ticker price as fallback: {ticker_err}")
                logger.debug(traceback.format_exc())
        ...
        if min_price_float is not None and stop_loss_price < min_price_float:
            logger.warning(f"Place Order: Calculated SL price {stop_loss_price:.{price_precision_digits}f} is below minimum limit {min_price_float:.{price_precision_digits}f}. Adjusting SL to minimum limit.")
            stop_loss_price = min_price_float
        ...
        min_qty_float = float(min_qty) if min_qty is not None else None # Already fetched min_qty
        max_qty_float = float(max_qty) if max_qty is not None else None # Already fetched max_qty

        if min_qty_float is not None and final_quantity < min_qty_float:
            logger.error(f"Place Order ({side.upper()}): Final Qty {final_quantity_str} is LESS than exchange minimum quantity {min_qty_float:.{qty_precision_digits}f}. Skipping order.")
            send_sms_alert(f"[{market_base}] ORDER FAIL ({side.upper()}): Qty {final_quantity_str} < Min {min_qty_float:.{qty_precision_digits}f}.")
            return None
        if max_qty_float is not None and final_quantity > max_qty_float:
            logger.warning(f"Place Order ({side.upper()}): Final Qty {final_quantity_str} is GREATER than exchange maximum quantity {max_qty_float:.{qty_precision_digits}f}. Capping to max qty.")
        ...
        # Use the potentially recalculated margin
        final_req_margin = required_margin_estimate if required_margin_estimate is not None else 0.0
        ...
        # Check if the order actually filled significantly using epsilon comparison
        if filled_qty_float > FLOAT_COMPARISON_EPSILON:
            ...
            # Send SMS Alert for successful entry, including calculated SL
            # stop_loss_price_str should be available from step 3
            sms_msg = (f"[{market_base}] ENTER {side.upper()} {filled_qty_str} @ ~{fill_price_str}. "
                       f"Calc SL ~ {stop_loss_price_str}. ID:...{order_id_short}")
            send_sms_alert(sms_msg)
            ...
            logger.critical(f"!!! `exchange.private_post_v5_position_set_trading_stop({{'category': 'linear', 'symbol': market['id'], 'stopLoss': '{stop_loss_price_str}', 'slTriggerBy': 'LastPrice', 'positionIdx': 0}})` !!!") # Use calculated SL
            ...
            #     try:
            #         # Recalculate SL based on actual fill price if desired, or use the pre-calculated one
            ...
            #         # Using pre-calculated SL for simplicity in example:
            #         actual_sl_price_str = stop_loss_price_str # From step 3
            #
...
def trade_logic(
    ...
    4. Logs current market state using dynamic precision.
    ...
    *** WARNING: Contains optional loop-based SL/TP check which is unsuitable for live scalping. ***
    ...
    if df is None or len(df) < required_rows:
        ...
        logger.info(f"========== Cycle Check End (Insufficient Data): {symbol} ==========\n")
        return

    # Get market precision for dynamic logging
    market = None
    price_precision_digits = 4 # Default
    qty_precision_digits = 8 # Default
    try:
    ...
    try:
        ...
        if not all(col in df.columns for col in required_indicator_cols):
             ...
             logger.info(f"========== Cycle Check End (Indicator Column Missing): {symbol} ==========\n")
             return
        if df.iloc[-1][required_indicator_cols].isnull().any():
             ...
             logger.info(f"========== Cycle Check End (Indicator NaN): {symbol} ==========\n")
             return
        ...
        if current_atr is None or current_atr <= FLOAT_COMPARISON_EPSILON: # Check against epsilon
             ...
             logger.info(f"========== Cycle Check End (Invalid ATR): {symbol} ==========\n")
             return
        ...
        if needs_ob_check:
            ...
            # analyze_order_book handles its own errors and returns None values on failure
        ...
        # === 4. Log Current State ===
        # Prepare formatted strings for logging, handling None values and using dynamic precision
        ...
        atr_str = f"{current_atr:.5f}" # Fixed precision for ATR is usually fine
        ...
        bid_ask_ratio_str = f"{bid_ask_ratio:.3f}" if bid_ask_ratio is not None else 'N/A' # Fixed precision for ratio
...
def main() -> None:
    ...
    if ENABLE_LOOP_SLTP_CHECK:
        logger.critical("--- ########################################################################## ---")
        logger.critical("--- ### CRITICAL WARNING: LOOP-BASED SL/TP IS ACTIVE (ENABLE_LOOP_SLTP_CHECK=true) ### ---")
        logger.critical("--- ### THIS MECHANISM IS DANGEROUSLY INACCURATE AND UNSUITABLE FOR SCALPING ### ---")
        logger.critical("--- ### DISABLE THIS AND IMPLEMENT EXCHANGE-NATIVE ORDERS FOR SAFE OPERATION ### ---")
        logger.critical("--- ########################################################################## ---")
    else:
...
if __name__ == "__main__":
    ...
    if ENABLE_LOOP_SLTP_CHECK:
        ...
        print("\033[91m\033[1m### CRITICAL: LOOP-BASED SL/TP IS ACTIVE (ENABLE_LOOP_SLTP_CHECK=true) ###\033[0m")
        print("\033[91m\033[1m### CRITICAL: This feature is HIGHLY INACCURATE & UNSAFE for live scalping.###\033[0m")
        print("\033[91m\033[1m### CRITICAL: DISABLE IT (set to false in .env) for real trading.      ###\033[0m")
        print("\033[91m\033[1m##########################################################################\033[0m")
    else:
