...
class MomentumScannerTrader:
                ...
                try:
                    ...
                    if initial_df is not None and not initial_df.empty:
                        ...
                        if not initial_df.index.tz or initial_df.index.tz != dt.timezone.utc:
                             log_console(logging.ERROR, f"Initial fetch: REST data returned without UTC index for {symbol}! Fixing.", symbol=symbol)
        ...
        try:
            ...
            if isinstance(message, dict):
                ...
                if op == "auth":
                    ...
                    if not success:
                        log_console(logging.ERROR, "WebSocket authentication failed. Connection will be reset by monitor.")
            ...
            try:
                ...
                for symbol in list(self.traders.keys()):
                    ...
                    if not trader or not trader.initialization_successful:
                         log_console(logging.WARNING, f"Trader instance for {symbol} is invalid or not initialized. Skipping.", symbol=symbol)
                         continue
                    ...
                    try:
                        ...
                        if position:
                            ...
                            if entry_side:
                                ...
                                if qty_to_trade > FLOAT_COMPARISON_TOLERANCE:
                                    ...
                                    log_console(logging.INFO, f"Attempting {entry_side} entry for {symbol}: Qty={qty_to_trade:.{trader.qty_precision}f}, Est. Entry={entry_price_est:.{trader.price_precision}f}, SL={sl_log_str}, TP={tp_log_str}", symbol=symbol)
                                    ...
                                    if order_id:
                                        log_console(logging.INFO, f"Entry order {order_id} placed successfully for {symbol}.", symbol=symbol)
                                        # Position state (`trader.current_trade`) is updated inside place_order on success
                                    else:
                                        log_console(logging.ERROR, f"Failed to place entry order for {symbol}.", symbol=symbol)
                                    ...
                                    log_console(logging.DEBUG, f"Calculated position size ({qty_to_trade:.{trader.qty_precision}f}) is zero or too small for {symbol} based on risk/balance/constraints. No trade placed.", symbol=symbol)
                            else: # No entry signal found or strength too low
                                log_console(logging.DEBUG, f"No valid entry signal found for {symbol} this cycle.", symbol=symbol)
                        ...
                        log_console(logging.ERROR, f"Error processing trading logic for symbol {symbol}: {symbol_e}", symbol=symbol, exc_info=True)
                        # Continue to the next symbol

                # --- End of Symbol Loop ---
                if not self.running or self.shutdown_requested: break # Exit main loop if shutdown requested during symbol processing
                ...
                print(f"{Fore.BLUE}{Style.BRIGHT}===== CYCLE END ({cycle_duration:.2f}s) | Sleeping for {sleep_time:.2f}s =====")
                if sleep_time > 0:
        ...
        if close_on_exit and not self.dry_run:
            ...
            for symbol, trader in list(self.traders.items()):
                try:
                    ...
                    if position:
                        log_console(logging.INFO, f"Closing position for {symbol} due to bot shutdown...", symbol=symbol)
                        ...
                        if trader.kline_df is not None and not trader.kline_df.empty:
                            try: current_price_est = float(trader.kline_df['close'].iloc[-1])
                            except: pass
                        if not trader.close_position(position, exit_reason="Shutdown", exit_price_est=current_price_est):
                            log_console(logging.ERROR, f"Failed to close position for {symbol} during shutdown. Manual check may be needed.", symbol=symbol)
                    # Small delay between closing attempts
                    time.sleep(0.5)
                except Exception as close_e:
                    log_console(logging.ERROR, f"Error closing position for {symbol} during shutdown: {close_e}", symbol=symbol)
            log_console(logging.INFO, "Position closing attempts finished.")
             ...
             for symbol, trader in list(self.traders.items()):
                 if trader.current_trade:
                     ...
                     sim_exit_price = None
                     if trader.kline_df is not None and not trader.kline_df.empty:
                         try: sim_exit_price = float(trader.kline_df['close'].iloc[-1])
                         except: pass
                     # Pass a deepcopy to avoid modifying dict while iterating (list() should handle this, but safer)
                     if not trader.close_position(deepcopy(trader.current_trade), exit_reason="Shutdown", exit_price_est=sim_exit_price):
                          log_console(logging.WARNING, f"[DRY RUN] Failed to simulate closing for {symbol}.", symbol=symbol)
    ...
    def _setup_signal_handlers(self):
        ...
        try:
            ...
            log_console(logging.WARNING, "Could not set signal handlers (might be running in an unsupported environment).")
        except Exception as e:
...
def validate_config(config: Dict[str, Any]):
    ...
    valid_timeframes = ["1", "3", "5", "15", "30", "60", "120", "240", "360", "720", "D", "W", "M"] # Check Bybit V5 docs for current list
    for i, sym_cfg in enumerate(symbols_list):
        if not isinstance(sym_cfg, dict): raise ValueError(f"Symbol config entry #{i} must be a dictionary.")
        for key in required_symbol_keys:
            if key not in sym_cfg: raise ValueError(f"Symbol config #{i} missing required key: '{key}'")
            if not sym_cfg[key]: raise ValueError(f"Symbol config #{i} key '{key}' cannot be empty.")
        # Check timeframe validity
        tf = str(sym_cfg["TIMEFRAME"]) # Ensure string comparison
        if tf not in valid_timeframes:
            raise ValueError(f"Symbol '{sym_cfg.get('SYMBOL')}' has invalid TIMEFRAME '{tf}'. Must be one of: {valid_timeframes}")
        ...
        if acc_type != "SPOT" and "LEVERAGE" in sym_cfg:
            ...
            if not isinstance(lev, int) or lev <= 0:
                raise ValueError(f"Symbol '{sym_cfg.get('SYMBOL')}' has invalid LEVERAGE '{lev}'. Must be a positive integer.")
        ...
        if isinstance(strat_cfg, dict):
             for period_key in ["ULTRA_FAST_EMA_PERIOD", "FAST_EMA_PERIOD", "MID_EMA_PERIOD", "SLOW_EMA_PERIOD",
...
def main():
    ...
    parser.add_argument(
        "-c", "--config", type=str, default=DEFAULT_CONFIG_FILE, help=f"Path to the configuration file (default: {DEFAULT_CONFIG_FILE})"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Run in simulation mode (no real orders)"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable detailed DEBUG logging to console and file"
    )
    ...
    # --- Check API Keys ---
    if not API_KEY or not API_SECRET:
        log_console(logging.CRITICAL,"API keys are missing. Ensure BYBIT_API_KEY and BYBIT_API_SECRET are set in environment or .env file.")
        sys.exit(1)
    ...
    try:
        ...
        if bot_instance: bot_instance._cleanup_resources()
        sys.exit(1)
    except Exception as e:
