...
- Ehlers filters (Super Smoother, Instantaneous Trendline - optional, sensitive).
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for symbol_index, symbol_cfg in enumerate(symbols_cfg):
            if (
                not isinstance(symbol_cfg, dict)
                or "SYMBOL" not in symbol_cfg
                or not isinstance(symbol_cfg["SYMBOL"], str)
            ):
                raise ValueError(
                    f"Each item in 'SYMBOLS' (index {symbol_index}) must be a dictionary with a valid string 'SYMBOL' key."
                )
            ...
            if not isinstance(strategy_cfg, dict):
                log_console(
                    logging.WARNING,
                    f"Missing or invalid 'STRATEGY_CONFIG' for {symbol}. Using defaults/basic checks.",
                    symbol=symbol,
                )
            ...
            if not valid_periods:
                log_console(
                    logging.WARNING,
                    f"No valid positive indicator periods found for {symbol}. Using default max period (200).",
                    symbol=symbol,
                )
            ...
            log_console(
                logging.DEBUG,
                f"Calculated KLINE_LIMIT for {symbol}: {kline_limit} (Max Period: {max_period:.0f}, Buffer: {kline_limit_buffer}, ADX Buf: {adx_warmup_buffer}, Min Needed: {min_records_needed})",
                symbol=symbol,
            )
        ...
        log_console(
            logging.CRITICAL,
            f"Unexpected error loading or performing initial validation of configuration: {e}",
            exc_info=True,
        )
        sys.exit(1)
...
def super_smoother(series: pd.Series, period: int) -> pd.Series:
    ...
    if len(series_cleaned) < min_len_needed:
        log_console(
            logging.DEBUG,
            f"Super Smoother: Series length ({len(series_cleaned)} non-NaN) is less than required minimum ({min_len_needed}). Returning NaNs.",
        )
        return pd.Series(np.nan, index=series.index, dtype=np.float64)
    ...
    try:
        ...
        if len(series_values) > 0 and not np.isnan(series_values[0]):
            ss[0] = series_values[0]
        if len(series_values) > 1 and not np.isnan(series_values[1]):
        ...
        for i in range(2, len(series_values)):
            ...
            if (
                np.isnan(series_values[i])
                or np.isnan(series_values[i - 1])
                or np.isnan(ss[i - 1])
                or np.isnan(ss[i - 2])
            ):
...
def instantaneous_trendline(series: pd.Series, period: int) -> pd.Series:
    ...
    try:
        ...
        if len(series_values) > 2 and not np.any(np.isnan(series_values[0:3])):
            it[2] = (series_values[2] + 2.0 * series_values[1] + series_values[0]) / 4.0
        # Requires indices 1, 2, 3 to be valid
        if len(series_values) > 3 and not np.any(np.isnan(series_values[1:4])):
            it[3] = (series_values[3] + 2.0 * series_values[2] + series_values[1]) / 4.0
...
def calculate_indicators_momentum(
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        ...
        try:
            ...
            log_console(
                logging.WARNING,
                f"Indicator Calc: Index might already be timezone-aware? Error during tz_localize: {tz_err}. Proceeding.",
            )
            if df.index.tz != dt.timezone.utc:  # If localized but not UTC, convert
                try:
                    ...
                    log_console(
                        logging.ERROR,
                        f"Indicator Calc: Failed to convert existing timezone to UTC: {tz_conv_err}. Cannot proceed.",
                        exc_info=True,
                    )
                    return None
    ...
    try:
        ...
        if len(df_out) < kline_limit:
            log_console(
                logging.DEBUG,
                f"Indicator Calc: Insufficient valid data points after cleaning ({len(df_out)} < {kline_limit}). Need more historical data.",
            )
            return None
        if df_out.empty:
    ...
    try:
        ...
        if price_source_col not in df_out or df_out[price_source_col].isnull().all():
            log_console(
                logging.ERROR,
                f"Indicator Calc: Price source column '{price_source_col}' is missing or contains only NaNs.",
            )
            return None
        ...
        try:
            ...
            if bb_period > 0 and bb_std > 0:
                ...
                if (
                    ...
                    log_console(
                        logging.WARNING,
                        f"Indicator Calc: Bollinger Bands calculation failed or missing expected columns (e.g., {bbu_col}). Check pandas_ta version/output.",
                    )
                    # Column remains NaN
            else:
        ...
        numeric_cols_fill_values = {
            ...
            "trendline": df_out[price_source_col]
            if "trendline" in df_out and not df_out["trendline"].isnull().all()
            else 0.0,
        ...
        if "volume" in df_out and "volume_percentile_75" in df_out and "volume_sma" in df_out:
            ...
            log_console(
                logging.WARNING,
                "Indicator Calc: Volume columns missing or failed calculation, cannot calculate 'high_volume'. Defaulting to False.",
            )
        ...
        if "norm_atr" in df_out and atr_period_volatility > 0:
            ...
            log_console(
                logging.DEBUG,
                "Indicator Calc: Cannot calculate volatility factor (norm_atr missing or period invalid). Using default factor 1.0.",
            )
        ...
        if not trend_ma_cols_ok or not trendline_ok:
            log_console(
                logging.WARNING,
                f"Indicator Calc: Cannot define trend. MA cols OK: {trend_ma_cols_ok}, Trendline OK: {trendline_ok}. Assuming neutral trend.",
            )
        else:
        ...
        entry_ma_cols_ok = all(
            ma_col in calculated_ma_cols
            for ma_col in [fast_ma_col, mid_ma_col, ultra_fast_ma_col]
        ...
        if not required_available:
            ...
            log_console(
                logging.WARNING,
                f"Indicator Calc: Cannot generate entry signals, required columns missing or failed calculation: {missing_cols}",
            )
        else:
            ...
            if "trendline_shift" not in locals():  # Check if calculated in trend section
                if trendline_ok:  # Only recalculate if trendline itself is valid
                    ...
                    log_console(
                        logging.WARNING,
                        "Indicator Calc: Trendline invalid, cannot calculate trendline shift for entry signals.",
                    )
        ...
        # --- CORRECTED LOGIC ---
        if not exit_required_available:  # Check if required columns are available FIRST
            ...
            log_console(
                logging.WARNING,
                f"Indicator Calc: Cannot generate exit signals, required columns missing or failed calculation: {missing_cols}",
            )
            ...
            if "trendline_shift" not in locals():
                if trendline_ok:  # Only recalculate if trendline itself is valid
                    ...
                    log_console(
                        logging.WARNING,
                        "Indicator Calc: Trendline invalid, cannot calculate trendline shift for exit signals.",
                    )
        ...
        if len(df_out) >= min_rows_for_check:
            if df_out.iloc[-min_rows_for_check:][final_check_cols].isnull().any().any():
                log_console(
                    logging.ERROR,
                    f"Indicator Calc: Critical columns contain NaN in the last {min_rows_for_check} rows after processing. Check calculations and NaN fills.",
                )
                # Log the problematic rows for debugging
                log_console(
                    logging.DEBUG,
                    f"Last {min_rows_for_check} rows tail with potential NaNs:\n{df_out.iloc[-min_rows_for_check:][final_check_cols]}",
                )
                return None
        ...
        if response and response.get("retCode") == 0:
            ...
            if account_type.upper() == "UNIFIED":
                ...
                if not isinstance(coin_balance_list, list):
                    log_console(
                        logging.WARNING,
                        f"Expected list for 'coin' in UNIFIED balance response, got {type(coin_balance_list)}. Parsing failed.",
                    )
                    return 0.0
                ...
                if account_type.upper() == "SPOT":
                    if isinstance(list_data, list) and all(isinstance(item, dict) for item in list_data):
                        ...
                        log_console(
                            logging.WARNING,
                            "Expected list of dicts for 'SPOT' balance response list, got different structure. Parsing failed.",
                        )
                        return 0.0
                    ...
                    if not isinstance(coin_balance_list, list):
                        log_console(
                            logging.WARNING,
                            f"Expected list for 'coin' in CONTRACT balance response, got {type(coin_balance_list)}. Parsing failed.",
                        )
                        return 0.0
            else:
            ...
            for coin_data in coin_balance_list:
                if isinstance(coin_data, dict) and coin_data.get("coin") == coin:
                    ...
                    if balance_str is None or balance_str == "":
                        # Often used in Unified/Contract/Spot
                    ...
                    if balance_str is None or balance_str == "":
                        ...
                        log_console(
                            logging.DEBUG,
                            f"'availableToWithdraw'/'availableBalance' empty for {coin}, using '{source}' as fallback.",
                        )
                    ...
                    if balance_str is None or balance_str == "":
                        log_console(
                            logging.WARNING,
                            f"Could not find a valid available balance field (availableToWithdraw, availableBalance, walletBalance) for {coin}. Balance is 0.",
                        )
                    ...
                    try:
                        ...
                        log_console(
                            logging.ERROR,
                            f"Could not convert balance string '{balance_str}' from field '{source}' to float for {coin}: {e}",
                        )
...
def calculate_position_size_atr(
    ...
    if not isinstance(qty_precision, int) or qty_precision < 0:
        log_console(
            logging.ERROR,
            f"Position Size Calc: Invalid qty_precision ({qty_precision}). Must be non-negative integer. Returning size 0.",
        )
        return 0.0
    ...
    if (
        ...
        log_console(
            logging.DEBUG,
            f"Position Size Calc: Non-positive input values detected. Balance={balance:.2f}, Risk%={risk_percent}, Entry={entry_price}, MinQty={min_order_qty}, QtyStep={qty_step_float}. Returning size 0.",
        )
        return 0.0
    ...
    if sl_distance_price <= FLOAT_COMPARISON_TOLERANCE:
        log_console(
            logging.WARNING,
            f"Position Size Calc: Stop loss distance ({sl_distance_price}) is zero or too small. Cannot calculate size reliably. Returning 0.",
        )
        return 0.0
    ...
    if max_position_usdt is not None and (
        ...
        log_console(
            logging.WARNING,
            f"Position Size Calc: Invalid max_position_usdt ({max_position_usdt}). Ignoring constraint.",
        )
    ...
    try:
        ...
        log_console(
            logging.WARNING,
            "Position Size Calc: SL distance is effectively zero during calculation. Cannot calculate size.",
        )
        return 0.0
    ...
    if max_position_usdt is not None:
        try:
            ...
            if entry_price <= FLOAT_COMPARISON_TOLERANCE:
                log_console(
                    logging.WARNING,
                    "Position Size Calc: Entry price is zero or negative. Cannot apply Max Position USDT constraint.",
                )
            else:
    ...
    if position_size < min_order_qty - min_qty_tolerance:
        log_console(
            logging.DEBUG,
            f"Position Size Calc: Calculated size {position_size:.{qty_precision + 4}f} (after potential cap) is below minimum required {min_order_qty}. Returning size 0.",
        )
        return 0.0
    ...
    try:
        if qty_step_float <= FLOAT_COMPARISON_TOLERANCE:
            ...
            log_console(
                logging.ERROR,
                f"Position Size Calc: Quantity step ({qty_step_float}) is zero or too small during rounding. Cannot apply step constraint.",
            )
            return 0.0
    ...
    if final_size < min_order_qty - min_qty_tolerance:
        log_console(
            logging.DEBUG,
            f"Position Size Calc: Final calculated size ({final_size}) is below min qty ({min_order_qty}) after final rounding. Returning size 0.",
        )
        return 0.0
    ...
    log_console(
        logging.INFO,
        f"Position Size Calc: Balance={balance:.2f}, Risk%={risk_percent}, SL Dist={sl_distance_price:.{log_price_precision}f}, Entry={entry_price:.{log_price_precision}f} -> Final Size = {final_size:.{qty_precision}f}",
    )
    return final_size
...
class TradeMetrics:
    ...
    def __init__(self, fee_rate: float):
        ...
        if not isinstance(fee_rate, (int, float)) or fee_rate < 0:
            log_console(
                logging.ERROR,
                f"Metrics Init: Invalid fee rate provided ({fee_rate}). Setting to default 0.0006 (0.06%).",
            )
            ...
            try:
                ...
                try:
                    ...
                    if leverage_val_str:
                        ...
                        log_console(
                            logging.WARNING,
                            f"Metrics: Empty leverage value provided for {symbol}. Using 1x.",
                            symbol=symbol,
                        )
                except (ValueError, TypeError):
                    log_console(
                        logging.ERROR,
                        f"Metrics: Invalid leverage value '{leverage}' for {symbol}. Using 1x.",
                        symbol=symbol,
                    )
...
class SymbolTrader:
    ...
    def __init__(
        ...
        if self.dry_run:
            log_console(
                logging.WARNING,
                f"DRY RUN MODE enabled. No real orders will be placed for {self.symbol}.",
                symbol=self.symbol,
            )
    ...
    def _get_precision_from_string(value_str: str) -> int:
        ...
        if "." in value_str:
            ...
            # Handle potential trailing zeros significance if necessary (usually not for precision calculation)
    ...
    def _fetch_instrument_info(self) -> bool:
        ...
        log_console(
            logging.DEBUG,
            f"Fetching instrument info for {self.symbol} (Category: {self.category})...",
            symbol=self.symbol,
        )
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if result_list and isinstance(result_list[0], dict):
                    ...
                    try:
                        ...
                        log_console(
                            logging.ERROR,
                            f"Failed to convert instrument filter values to numbers: {conv_e}. Filters: LotSize={lot_size_filter}, Price={price_filter}",
                            symbol=self.symbol,
                        )
                        return False
                    ...
                    if (
                        ...
                        or self.tick_size_float <= FLOAT_COMPARISON_TOLERANCE
                    ):
                        log_console(
                            logging.ERROR,
                            f"Fetched invalid instrument details (non-positive values): MinQty={self.min_order_qty}, QtyStep={self.qty_step_float}, TickSize={self.tick_size_float}",
                            symbol=self.symbol,
                        )
                        return False
                    ...
                    log_console(
                        logging.ERROR,
                        f"Instrument info fetch failed: Empty or invalid result list in response for {self.symbol}.",
                        symbol=self.symbol,
                    )
                    return False
                ...
                log_console(
                    logging.ERROR,
                    f"Instrument info fetch API call failed: {error_msg} (Code: {error_code})",
                    symbol=self.symbol,
                )
                ...
                if error_code == 10001 or "invalid symbol" in error_msg.lower():
                    log_console(
                        logging.ERROR,
                        f"----> Is symbol '{self.symbol}' valid for the '{self.category}' category on Bybit?",
                        symbol=self.symbol,
                    )
                return False
        except Exception as e:
    ...
    def _initial_setup(self):
        ...
        log_console(
            logging.INFO,
            f"Performing initial setup: Setting leverage to {self.leverage}x for {self.symbol}...",
            symbol=self.symbol,
        )
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if error_code in already_set_codes or any(msg in error_msg.lower() for msg in already_set_msgs):
                    ...
                    log_console(
                        logging.WARNING,
                        f"Failed to set leverage: {error_msg} (Code: {error_code}). Check API permissions and account settings (e.g., margin mode). Continuing...",
                        symbol=self.symbol,
                    )
    ...
    def _websocket_callback(self, message: Dict[str, Any]):
        ...
        try:
            ...
            log_console(
                logging.ERROR,
                f"Unexpected error in WebSocket kline callback for {self.symbol}: {e}",
                symbol=self.symbol,
                exc_info=True,
            )
        ...
        while not self.kline_queue.empty() and processed_count < max_items_per_cycle:
            try:
                ...
                if self.kline_df is None or self.kline_df.empty:
                    ...
                    log_console(
                        logging.DEBUG,
                        f"Initializing internal kline_df from first WS kline for {self.symbol}.",
                        symbol=self.symbol,
                    )
                    ...
                    if not isinstance(self.kline_df.index, pd.DatetimeIndex):
                        log_console(
                            logging.ERROR,
                            f"Internal kline_df has invalid index type! Resetting DF with new WS kline for {self.symbol}.",
                            symbol=self.symbol,
                        )
                    ...
                    if not self.kline_df.index.tz or self.kline_df.index.tz != dt.timezone.utc:
                        ...
                        try:
                            ...
                            log_console(
                                logging.ERROR,
                                f"Error converting existing index to UTC: {tz_err}. Resetting DF.",
                                symbol=self.symbol,
                            )
                            self.kline_df = new_row
                            df_updated = True
        ...
        if df_updated and self.kline_df is not None and not self.kline_df.empty:
            ...
            try:
                ...
                if calculated_df is not None and not calculated_df.empty:
                    ...
                    log_console(
                        logging.DEBUG,
                        f"Indicators recalculated successfully for {self.symbol} after WS update.",
                        symbol=self.symbol,
                    )
                else:
                    ...
                    log_console(
                        logging.WARNING,
                        f"Indicator recalculation failed after WS update for {self.symbol}. Stale indicators may be used until next successful update.",
                        symbol=self.symbol,
                    )
            ...
            log_console(
                logging.ERROR,
                f"Internal kline_df became empty/None after WS processing for {self.symbol}. Cannot recalculate indicators.",
                symbol=self.symbol,
            )
            recalculated_ok = False
    ...
    def get_ohlcv_rest(self, timeframe: Optional[str] = None, limit: Optional[int] = None) -> Optional[pd.DataFrame]:
        ...
        log_console(
            logging.DEBUG,
            f"Fetching {lim_to_fetch} klines for {self.symbol} (Timeframe: {tf_to_fetch}) via REST API...",
            symbol=self.symbol,
        )
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if kline_list:
                    ...
                    try:
                        ...
                        log_console(
                            logging.ERROR,
                            f"REST KLine: Failed during timestamp conversion for {self.symbol}: {ts_e}",
                            symbol=self.symbol,
                        )
                    ...
                    if len(df) < initial_len_ts:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: Dropped {initial_len_ts - len(df)} rows with invalid timestamps for {self.symbol}.",
                            symbol=self.symbol,
                        )
                    if df.empty:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: DataFrame empty after timestamp conversion/dropna for {self.symbol}.",
                            symbol=self.symbol,
                        )
                        return None
                    ...
                    for col in ohlcv_cols:
                        if col in df.columns:
                            ...
                            log_console(
                                logging.WARNING,
                                f"REST KLine: Expected column '{col}' missing in response for {self.symbol}. Filling with NaN.",
                                symbol=self.symbol,
                            )
                    ...
                    if len(df) < initial_len_ohlcv:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: Dropped {initial_len_ohlcv - len(df)} rows with NaNs in essential OHLCV columns for {self.symbol}.",
                            symbol=self.symbol,
                        )
                    if df.empty:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: DataFrame empty after dropping rows with NaN OHLCV data for {self.symbol}.",
                            symbol=self.symbol,
                        )
                        return None
                    ...
                    if len(df) < min_len_for_calc:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: Not enough valid data ({len(df)} < {min_len_for_calc}) fetched for {self.symbol} {tf_to_fetch} after cleaning. Cannot calculate reliable indicators.",
                            symbol=self.symbol,
                        )
                    ...
                    log_console(
                        logging.DEBUG,
                        f"REST Fetch for {self.symbol} successful. Cleaned data shape: {df.shape}",
                        symbol=self.symbol,
                    )
                    ...
                    if df_with_indicators is None or df_with_indicators.empty:
                        log_console(
                            logging.WARNING,
                            f"REST KLine: Indicator calculation failed for fetched data ({self.symbol} {tf_to_fetch}).",
                            symbol=self.symbol,
                        )
                    ...
                    log_console(
                        logging.WARNING,
                        f"REST KLine fetch returned success code but an empty data list for {self.symbol} {tf_to_fetch}.",
                        symbol=self.symbol,
                    )
                    return None
                ...
                log_console(
                    logging.WARNING,
                    f"REST KLine fetch API call failed for {self.symbol} {tf_to_fetch}: {error_msg} (Code: {error_code})",
                    symbol=self.symbol,
                )
                # Check for specific errors like invalid symbol/timeframe
                if (
                    error_code == 10001
                    or "invalid symbol" in error_msg.lower()
                    or "invalid interval" in error_msg.lower()
                ):
                    log_console(
                        logging.ERROR,
                        f"----> Check if symbol '{self.symbol}' and timeframe '{tf_to_fetch}' are valid for category '{self.category}'.",
                        symbol=self.symbol,
                    )
                # Handle rate limit errors (e.g., 10006)
                    ...
                    log_console(
                        logging.WARNING,
                        f"REST KLine fetch rate limited for {self.symbol}. Consider increasing sleep interval or reducing symbols.",
                        symbol=self.symbol,
                    )
                return None
        except Exception as e:
            log_console(
                logging.ERROR,
                f"Exception during REST KLine fetch or processing for {self.symbol}: {e}",
                symbol=self.symbol,
                exc_info=True,
            )
            return None
    ...
    def get_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        if self.kline_df is None or self.kline_df.empty:
            ...
            log_console(
                logging.DEBUG,
                f"Internal kline_df is empty for {self.symbol}, attempting REST fetch.",
                symbol=self.symbol,
            )
            ...
            if (
                ...
                log_console(
                    logging.DEBUG,
                    f"WS disabled and data stale (> {staleness_limit}s) for {self.symbol}, using REST.",
                    symbol=self.symbol,
                )
            else:
                log_console(
            ...
            if (
                ...
                log_console(
                    logging.WARNING,
                    f"WS queue processing did not yield update or data stale (> {staleness_limit}s) for {self.symbol}. Attempting REST fallback.",
                    symbol=self.symbol,
                )
        ...
        if needs_rest_fetch:
            ...
            if rest_df_with_indicators is not None and not rest_df_with_indicators.empty:
                ...
                log_console(
                    logging.DEBUG,
                    f"Updated internal kline_df successfully via REST API for {self.symbol}.",
                    symbol=self.symbol,
                )
            else:
                log_console(
                    logging.WARNING,
                    f"REST API fetch fallback failed for {self.symbol}. Kline data may be missing or stale.",
                    symbol=self.symbol,
                )
        ...
        if self.kline_df is not None and not self.kline_df.empty:
            ...
            log_console(
                logging.DEBUG,
                f"No valid kline data available for {self.symbol} after WS/REST attempts.",
                symbol=self.symbol,
            )
            return None
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                for pos_data in position_list:
                    ...
                    if pos_data.get("symbol") == self.symbol:
                        try:
                            ...
                            if pos_side in ["Buy", "Sell"] and abs(pos_size) >= min_significant_qty:
                                ...
                                # 2. Fallback to API createdTime (convert ms string to datetime UTC)
                                    ...
                                    try:
                                        ...
                                        log_console(
                                            logging.DEBUG,
                                            f"Could not parse createdTime '{position_standardized['createdTime']}' for position entry time: {ts_err}",
                                            symbol=self.symbol,
                                        )
                                ...
                                log_console(
                                    logging.DEBUG,
                                    f"Active position found for {self.symbol}: Side={pos_side}, Size={pos_size}",
                                    symbol=self.symbol,
                                )
                            ...
                            log_console(
                                logging.ERROR,
                                f"Position fetch: Error converting fields for {self.symbol}: {conv_e}. PosData: {pos_data}",
                                symbol=self.symbol,
                            )
                ...
                if log_error:
                    ...
                    log_console(
                        logging.ERROR,
                        f"Position fetch API call failed for {self.symbol}: {error_msg} (Code: {error_code})",
                        symbol=self.symbol,
                    )
                    ...
                    if error_code in [10003, 10004]:
                        log_console(
                            logging.CRITICAL,
                            f"Position fetch failed due to AUTH error for {self.symbol}. Check API keys/permissions.",
                            symbol=self.symbol,
                        )
                return None
        except Exception as e:
    ...
    def confirm_order(self, order_id: str, expected_qty: float) -> Tuple[float, Optional[float]]:
        ...
        if self.dry_run:
            log_console(
                logging.INFO,
                f"[DRY RUN] Simulating confirmation for order {order_id} (Expected Qty: {expected_qty})",
                symbol=self.symbol,
            )
            ...
            if sim_entry_price is None:
                ...
                log_console(
                    logging.WARNING,
                    f"[DRY RUN] Cannot estimate realistic entry price for {order_id}, using dummy price: {sim_entry_price}.",
                    symbol=self.symbol,
                )

            log_console(
                logging.DEBUG,
                f"[DRY RUN] Using simulated avg fill price: {sim_entry_price:.{self.price_precision}f} for order {order_id}",
                symbol=self.symbol,
            )
                ...
                if response and response.get("retCode") == 0:
                    ...
                    if order_list and isinstance(order_list[0], dict):
                        ...
                        try:
                            ...
                            log_console(
                                logging.ERROR,
                                f"Confirm Order {order_id}: Error parsing qty/price from history: {e}. Data: {order}",
                                symbol=self.symbol,
                            )
                        ...
                        if status == "Filled":
                            ...
                            log_console(
                                logging.INFO,
                                f"Order {order_id} status '{status}'. Filled: {filled_qty:.{self.qty_precision}f}/{order.get('qty', '?')}. Continuing check...",
                                symbol=self.symbol,
                            )
                            ...
                            log_console(
                                logging.WARNING,
                                f"Order {order_id} reached terminal status: '{status}'. Final Filled Qty: {filled_qty:.{self.qty_precision}f}",
                                symbol=self.symbol,
                            )
                            ...
                            log_console(
                                logging.WARNING,
                                f"Order {order_id} encountered unhandled status: '{status}'. Continuing check.",
                                symbol=self.symbol,
                            )
                        ...
                        log_console(
                            logging.DEBUG,
                            f"Order {order_id} not found in history yet (Attempt {attempt + 1}). Might still be processing or submitted.",
                            symbol=self.symbol,
                        )
        ...
        log_console(
            logging.ERROR,
            f"Order {order_id} - Last Known State: FilledQty={last_filled_qty:.{self.qty_precision}f}, AvgPrice={last_avg_price or 'N/A'}. Manual check strongly recommended!",
            symbol=self.symbol,
        )
    ...
    def place_order(
        ...
        if qty < self.min_order_qty - min_qty_tolerance:
            log_console(
                logging.ERROR,
                f"Order quantity {qty:.{self.qty_precision}f} is below minimum required {self.min_order_qty}",
                symbol=self.symbol,
            )
            return None
        ...
        try:
            ...
            log_console(
                logging.ERROR,
                f"Failed to format quantity {qty} to precision {self.qty_precision}: {fmt_e}",
                symbol=self.symbol,
            )
            return None
        ...
        if self.dry_run:
            ...
            log_console(
                logging.INFO,
                f"[DRY RUN] Simulated order {order_id} placed. Simulated Entry Price: {sim_entry_price:.{self.price_precision}f}",
                symbol=self.symbol,
            )
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    log_console(
                        logging.INFO,
                        f"Order {order_id} submitted successfully. Confirming execution...",
                        symbol=self.symbol,
                    )
                    ...
                    if filled_qty >= required_fill - FLOAT_COMPARISON_TOLERANCE:
                        log_console(
                            logging.INFO,
                            f"Order {order_id} confirmed filled successfully. Actual Qty={filled_qty:.{self.qty_precision}f}, Avg Price={avg_price or 'N/A'}",
                            symbol=self.symbol,
                        )
                        ...
                        log_console(
                            logging.ERROR,
                            f"Order {order_id} fill confirmation failed or insufficient. Requested Qty={qty:.{self.qty_precision}f}, Filled Qty={filled_qty:.{self.qty_precision}f} (Required >= {required_fill:.{self.qty_precision}f}).",
                            symbol=self.symbol,
                        )
                    ...
                    log_console(
                        logging.ERROR,
                        f"Order placement API call succeeded (retCode 0) but no Order ID was returned. Response: {response}",
                        symbol=self.symbol,
                    )
                    return None
                ...
                log_console(
                    logging.ERROR,
                    f"Order placement API call failed: {error_msg} (Code: {error_code})",
                    symbol=self.symbol,
                )
                ...
                if error_code == 110007:
                    log_console(
                        logging.CRITICAL,
                        "ORDER REJECTED: INSUFFICIENT BALANCE! Check available funds.",
                        symbol=self.symbol,
                    )
                elif error_code in [110013, 110014, 110069]:
                    log_console(
                        logging.ERROR,
                        "ORDER REJECTED: Quantity issue (min/max/step). Check instrument info and calculated size.",
                        symbol=self.symbol,
                    )
                elif error_code == 110017:
                    log_console(
                        logging.ERROR,
                        "ORDER REJECTED: Risk control triggered by exchange (e.g., large price deviation). Market might be volatile.",
                        symbol=self.symbol,
                    )
                elif error_code == 110040:
                    log_console(
                        logging.ERROR,
                        "ORDER REJECTED: Account risk limit exceeded (e.g., max position size allowed).",
                        symbol=self.symbol,
                    )
                elif error_code == 110073:
                    log_console(
                        logging.ERROR,
                        f"ORDER REJECTED: TP/SL price invalid (e.g., too close to market, wrong side). Check SL={sl_str}, TP={tp_str}",
                        symbol=self.symbol,
                    )
                elif error_code == 110035:
                    log_console(
                        logging.ERROR,
                        "ORDER REJECTED: TP/SL requires tpslMode parameter for market orders in V5.",
                        symbol=self.symbol,
                    )
                elif "reduce-only" in error_msg.lower():
                    log_console(
                        logging.ERROR,
                        "ORDER REJECTED: Reduce-only conflict? Unexpected open position might exist.",
                        symbol=self.symbol,
                    )
            ...
            log_console(
                logging.ERROR,
                f"Exception occurred during order placement for {self.symbol}: {e}",
                symbol=self.symbol,
                exc_info=True,
            )
            return None
    ...
    def close_position(
        ...
        if not isinstance(position_data, dict):
            log_console(
                logging.WARNING,
                "Close attempt failed: Invalid position data provided (not a dict).",
                symbol=self.symbol,
            )
            return False
        ...
        if symbol_from_pos != self.symbol:
            log_console(
                logging.ERROR,
                f"Position data symbol mismatch ({symbol_from_pos} != {self.symbol}). Cannot close using this trader instance.",
                symbol=self.symbol,
            )
            return False
        ...
        if not (
            ...
            log_console(
                logging.ERROR,
                f"Invalid position data types for closing {self.symbol}: Side={side}({type(side)}), Size={size}({type(size)}), Entry={entry_price}({type(entry_price)}). Cannot close.",
                symbol=self.symbol,
            )
            ...
            if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                log_console(
                    logging.WARNING,
                    f"Clearing potentially inconsistent internal trade state for {self.symbol} due to invalid position data.",
                    symbol=self.symbol,
                )
                self.current_trade = None
        ...
        if abs_size < min_significant_qty_close:
            log_console(
                logging.INFO,
                f"Position size {abs_size:.{self.qty_precision}f} for {self.symbol} is negligible (below {min_significant_qty_close:.{self.qty_precision}f}). Assuming already closed or dust.",
                symbol=self.symbol,
            )
        ...
        if self.dry_run:
            ...
            if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                ...
                if sim_exit_price is None or sim_exit_price <= FLOAT_COMPARISON_TOLERANCE:
                    ...
                    log_console(
                        logging.WARNING,
                        f"[DRY RUN] Cannot estimate realistic exit price for {self.symbol}, using entry/fallback price ({sim_exit_price:.{self.price_precision}f}) for metrics. P&L may be inaccurate.",
                        symbol=self.symbol,
                    )

                log_console(
                    logging.INFO,
                    f"[DRY RUN] Simulating close for {self.symbol} at estimated price: {sim_exit_price:.{self.price_precision}f}",
                    symbol=self.symbol,
                )
                ...
                log_console(
                    logging.WARNING,
                    f"[DRY RUN] Close attempt failed: No simulated trade was open for {self.symbol}.",
                    symbol=self.symbol,
                )
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    log_console(
                        logging.INFO,
                        f"Position close order {order_id} submitted successfully. Confirming execution...",
                        symbol=self.symbol,
                    )
                    ...
                    if filled_qty >= required_fill - FLOAT_COMPARISON_TOLERANCE:
                        log_console(
                            logging.INFO,
                            f"Close order {order_id} confirmed filled successfully. Filled Qty: {filled_qty:.{self.qty_precision}f}, Avg Exit Price: {avg_exit_price or 'N/A'}",
                            symbol=self.symbol,
                        )
                        ...
                        if final_exit_price_log is None or final_exit_price_log <= FLOAT_COMPARISON_TOLERANCE:
                            ...
                            log_msg = (
                                f"using estimated exit price ({exit_price_est})"
                                if exit_price_est
                                else "using entry price as fallback"
                            )
                            log_console(
                                logging.WARNING,
                                f"Could not get valid confirmed avg exit price for {self.symbol} metrics, {log_msg}.",
                                symbol=self.symbol,
                            )
                        ...
                        log_console(
                            logging.ERROR,
                            f"Position close order {order_id} filled qty ({filled_qty:.{self.qty_precision}f}) is less than expected ({required_fill:.{self.qty_precision}f}). Position might be partially closed. Manual check required!",
                            symbol=self.symbol,
                        )
                    ...
                    log_console(
                        logging.ERROR,
                        f"Close order placement API call succeeded (retCode 0) but no Order ID returned. Response: {response}",
                        symbol=self.symbol,
                    )
                    return False
                ...
                pos_already_closed_codes = [110025, 110066, 110044, 110071, 3400074]
                ...
                if is_already_closed:
                    ...
                    log_console(
                        logging.WARNING,
                        f"Close attempt failed for {self.symbol}, but API indicates position is already closed or size issue (Code: {error_code}, Msg: '{error_msg}'). Assuming closed.",
                        symbol=self.symbol,
                    )
                    ...
                    if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                        ...
                        log_console(
                            logging.INFO,
                            f"Logging trade metrics from internal state for {self.symbol} as API indicated position already closed/size issue.",
                            symbol=self.symbol,
                        )
                        self.metrics.add_trade(
                            symbol=self.symbol,
                        ...
                        log_console(
                            logging.INFO,
                            f"No internal trade state found for {self.symbol} when API indicated already closed.",
                            symbol=self.symbol,
                        )
                    ...
                    log_console(
                        logging.ERROR,
                        f"Position close order placement failed for {self.symbol}: {error_msg} (Code: {error_code})",
                        symbol=self.symbol,
                    )
            ...
            log_console(
                logging.ERROR,
                f"Exception occurred during position close attempt for {self.symbol}: {e}",
                symbol=self.symbol,
                exc_info=True,
            )
            return False
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        if (
            ...
            log_console(
                logging.DEBUG,
                f"Using cached HTF trend result for {self.symbol}: {'Favorable/Allow' if self.higher_tf_cache else 'Unfavorable'}",
                symbol=self.symbol,
            )
        ...
        if higher_tf == self.timeframe:
            log_console(
                logging.WARNING,
                f"Higher timeframe ({higher_tf}) is the same as base timeframe ({self.timeframe}) for {self.symbol}. Disabling MTF check.",
                symbol=self.symbol,
            )
        ...
        if df_higher is None or df_higher.empty or len(df_higher) < min_rows_needed_htf:
            log_console(
                logging.WARNING,
                f"Could not get sufficient HTF ({higher_tf}) data ({len(df_higher) if df_higher is not None else 0} rows) or indicators failed for {self.symbol}. Allowing trade (fail-open).",
                symbol=self.symbol,
            )
        ...
        try:
            ...
            log_console(
                logging.WARNING,
                f"Index error accessing HTF ({higher_tf}) data (need >= {min_rows_needed_htf} rows, have {len(df_higher)}). Allowing trade.",
                symbol=self.symbol,
            )
            self.higher_tf_cache = True
            self.higher_tf_cache_time = now_utc
            ...
            log_console(
                logging.ERROR,
                f"Error during HTF ({higher_tf}) trend analysis for {self.symbol}: {e}. Allowing trade.",
                symbol=self.symbol,
                exc_info=True,
            )
            self.higher_tf_cache = True
...
class MomentumScannerTrader:
    ...
    def __init__(self, api_key: str, api_secret: str, config: Dict[str, Any], dry_run: bool = False):
        ...
        if self.account_type == "SPOT":
            ...
            log_console(
                logging.INFO,
                f"Using V5 API category '{self.category}' for {self.account_type} account. Ensure configured symbols match this category (e.g., USDT perpetuals).",
            )
        else:
            # Fallback for potentially unsupported account types, warn user.
            log_console(
                logging.WARNING,
                f"Account type '{self.account_type}' might not map directly to a standard V5 category. Defaulting to 'linear'. Verify compatibility.",
            )
            self.category = "linear"
        ...
        log_console(
            logging.INFO,
            f"Initializing Trader Orchestrator (Testnet: {self.use_testnet}, Account: {self.account_type}, Category: {self.category})...",
        )
        try:
            # --- Setup HTTP Session ---
            ...
            if self.traders and any(trader.use_websocket for trader in self.traders.values()):
                ...
                log_console(
                    logging.INFO,
                    "WebSocket is disabled for all symbols or no traders initialized. Bot will use REST API polling only.",
                )
    ...
    def _test_api_connection(self):
        ...
        try:
            ...
            log_console(
                logging.CRITICAL,
                f"API connection failed during test: {ce}. Check network, API endpoint, keys, permissions, and system time sync.",
            )
            raise ce
        except Exception as e:
    ...
    def _initialize_traders(self):
        ...
        for i, symbol_cfg_item in enumerate(deepcopy(symbols_cfg_list)):
            ...
            if not symbol or not isinstance(symbol, str):
                log_console(
                    logging.ERROR,
                    f"Symbol config entry #{i} missing or has invalid 'SYMBOL' key (must be a non-empty string). Skipping.",
                )
                continue
            ...
            if symbol in temp_traders:
                log_console(
                    logging.WARNING,
                    f"Duplicate symbol '{symbol}' found in configuration. Skipping subsequent entry #{i}.",
                    symbol=symbol,
                )
                continue
            ...
            log_console(
                logging.INFO,
                f"--- Initializing trader for {symbol} ({i + 1}/{len(symbols_cfg_list)}) ---",
                symbol=symbol,
            )
            try:
                ...
                if trader.initialization_successful:
                    ...
                    log_console(
                        logging.ERROR,
                        f"Trader initialization reported failure for {symbol} (init flag is False). Skipping.",
                        symbol=symbol,
                    )
                ...
                log_console(
                    logging.ERROR,
                    f"Initialization failed for {symbol}: {trader_init_e}. Skipping this symbol.",
                    symbol=symbol,
                )
            except Exception as e:
                ...
                log_console(
                    logging.ERROR,
                    f"Unexpected error occurred while initializing trader for {symbol}: {e}. Skipping this symbol.",
                    symbol=symbol,
                    exc_info=True,
                )
        ...
        log_console(
            logging.INFO,
            f"Successfully initialized {successful_traders}/{len(symbols_cfg_list)} traders: {', '.join(self.traders.keys())}",
        )
    ...
    def _start_and_monitor_websocket(self):
        ...
        if ws_channel_type not in ["linear", "inverse", "spot", "option"]:
            log_console(
                logging.ERROR,
                f"Unsupported category '{self.category}' for WebSocket channel_type. Defaulting to 'linear'. Verify config.",
                symbol=None,
            )
            ...
            if not ws_conn_active:
                ...
                try:
                    ...
                    temp_ws = WebSocket(
                        ...
                        retries=0,  # Disable pybit's built-in retries
                    )
                    ...
                    if subscriptions:
                        ...
                        while not self.shutdown_requested:
                            ...
                            if not ws_still_active:
                                log_console(
                                    logging.INFO,
                                    "WebSocket inactive status detected by monitor thread. Triggering reconnection attempt.",
                                )
                    ...
                    log_console(
                        logging.ERROR,
                        f"Failed to initialize or start WebSocket connection: {e}. Retrying in {retry_delay:.1f}s.",
                        exc_info=False,
                    )
                ...
                while time.time() < end_stable_sleep and not self.shutdown_requested:
                    # Check if WS became inactive during stable sleep
                    with self.ws_lock:
                        ws_still_active = self.ws_active
                    if not ws_still_active:
    ...
    def _dispatch_ws_callback(self, message: Dict[str, Any]):
        ...
        try:
                ...
                if topic.startswith("kline."):
                    ...
                    if len(parts) == 3:
                        ...
                        if symbol in self.traders:
                            ...
                            log_console(
                                logging.DEBUG,
                                f"WS Dispatch: Received kline data for unsubscribed/inactive symbol: {symbol}",
                                symbol=symbol,
                            )
                    else:
            ...
            log_console(
                logging.ERROR,
                f"Error occurred during WebSocket message dispatch: {e}. Message: {message}",
                exc_info=True,
            )
                    ...
                    if initial_df is not None and not initial_df.empty:
                        ...
                        if not initial_df.index.tz or initial_df.index.tz != dt.timezone.utc:
                            ...
                            if initial_df.index.tz:
                                initial_df.index = initial_df.index.tz_convert("UTC")
                            else:
                        ...
                        log_console(
                            logging.INFO,
                            f"Successfully initialized kline data for {symbol} from REST API.",
                            symbol=symbol,
                        )
                    else:
                        log_console(
                            logging.WARNING,
                            f"Initial REST fetch failed for {symbol}. Trader will rely solely on incoming WS updates for data.",
                            symbol=symbol,
                        )
                except Exception as e:
                    log_console(
                        logging.ERROR,
                        f"Error fetching initial REST data for {symbol} in worker thread: {e}",
                        symbol=symbol,
                        exc_info=False,
                    )
    ...
    def _handle_ws_message(self, message):
        """
        Generic WebSocket message handler. Logs auth/sub responses and pings/pongs.
        Data messages are routed via `_dispatch_ws_callback`.
        """
        try:
            # Log generic messages at DEBUG level to avoid noise
            ...
            if isinstance(message, dict):
                ...
                # Handle Subscription Response
                    ...
                    if success:
                        ...
                        log_console(
                            logging.ERROR,
                            f"WebSocket Subscribe Failed: {topic_info} - Message: {message.get('ret_msg', 'N/A')}",
                        )
    ...
    def run(self):
        ...
        if self.dry_run:
            ...
            if balance > FLOAT_COMPARISON_TOLERANCE:
                ...
                log_console(
                    logging.WARNING,
                    f"Initial {self.account_type} USDT available balance is zero or could not be fetched. Trades cannot be sized until balance is positive.",
                )
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                if ws_should_be_active and not ws_currently_active:
                    log_console(
                        logging.WARNING,
                        "WebSocket connection is currently inactive, but should be running. Strategy may rely on REST API polling until WS reconnects.",
                    )
                ...
                if (
                    ...
                    log_console(
                        logging.DEBUG,
                        f"Allocated Balance/Trader: {balance_per_trader:.2f} USDT, Max Position Value/Trader: {max_pos_usdt_per_trader:.2f} USDT",
                    )
                elif balance_per_trader > FLOAT_COMPARISON_TOLERANCE:
                    log_console(
                        logging.DEBUG,
                        f"Allocated Balance/Trader: {balance_per_trader:.2f} USDT, Max Position Value/Trader: Unlimited",
                    )
                ...
                for symbol in list(self.traders.keys()):
                    if not self.running or self.shutdown_requested:
                        break  # Check shutdown flag frequently

                    trader = self.traders[symbol]
                    # Check if the trader object itself is valid (should be if init succeeded)
                    if not trader or not trader.initialization_successful:
                        log_console(
                            logging.WARNING,
                            f"Trader instance for {symbol} is invalid or not initialized. Skipping.",
                            symbol=symbol,
                        )
                        continue
                    ...
                    try:
                        ...
                        if (
                            ...
                            or len(df_with_indicators) < min_rows_for_signals
                        ):
                            log_console(
                                logging.DEBUG,
                                f"Insufficient data ({len(df_with_indicators) if df_with_indicators is not None else 0} rows < {min_rows_for_signals}) or calculation failed for {symbol}. Skipping cycle.",
                                symbol=symbol,
                            )
                            continue
                        ...
                        try:
                            ...
                            log_console(
                                logging.WARNING,
                                f"Index error accessing candle data for {symbol} (need >= {min_rows_for_signals} rows, got {len(df_with_indicators)}). Skipping cycle.",
                                symbol=symbol,
                            )
                            continue
                        ...
                        if position:
                            ...
                            if exit_reason:
                                log_console(
                                    logging.INFO,
                                    f"Exit condition triggered for {symbol}: {exit_reason}. Attempting to close position.",
                                    symbol=symbol,
                                )
                                ...
                                if trader.close_position(
                                    ...
                                    log_console(
                                        logging.INFO,
                                        f"Position for {symbol} closed successfully based on signal.",
                                        symbol=symbol,
                                    )
                                    ...
                                    log_console(
                                        logging.ERROR,
                                        f"CRITICAL: Failed to close position for {symbol} after exit signal '{exit_reason}'. Manual intervention may be required!",
                                        symbol=symbol,
                                    )
                                ...
                                log_console(
                                    logging.DEBUG,
                                    f"Holding {pos_side} position for {symbol}. No exit signal detected.",
                                    symbol=symbol,
                                )
                            ...
                            log_console(
                                logging.DEBUG,
                                f"No active position for {symbol}. Checking for entry signals...",
                                symbol=symbol,
                            )
                            ...
                            if not htf_allows_trade:
                                log_console(
                                    logging.INFO,
                                    f"HTF filter is unfavorable for {symbol}. Skipping entry check.",
                                    symbol=symbol,
                                )
                            ...
                            if entry_side:
                                log_console(
                                    logging.DEBUG,
                                    f"Potential Entry Signal Detected: Side={entry_side}, Strength={signal_strength:.2f}, Est. Entry={entry_price_est:.{trader.price_precision}f}",
                                    symbol=symbol,
                                )
                                ...
                                if sl_distance_price_abs <= FLOAT_COMPARISON_TOLERANCE:
                                    log_console(
                                        logging.WARNING,
                                        f"Stop Loss distance is zero or negative ({sl_distance_price_abs:.{trader.price_precision}f}) for {symbol}. Cannot size trade. Skipping entry.",
                                        symbol=symbol,
                                    )
                                    continue
                                ...
                                if qty_to_trade > FLOAT_COMPARISON_TOLERANCE:
                                    ...
                                    sl_log_str = f"{stop_loss_price:.{trader.price_precision}f}"  # SL is mandatory here
                                    log_console(
                                        logging.INFO,
                                        f"Attempting {entry_side} entry for {symbol}: Qty={qty_to_trade:.{trader.qty_precision}f}, Est. Entry={entry_price_est:.{trader.price_precision}f}, SL={sl_log_str}, TP={tp_log_str}",
                                        symbol=symbol,
                                    )

                                    # Place the order using the trader method
                                    ...
                                    if order_id:
                                        log_console(
                                            logging.INFO,
                                            f"Entry order {order_id} placed successfully for {symbol}.",
                                            symbol=symbol,
                                        )
                                        ...
                                        # Optional: Maybe add a short cooldown after a failed entry attempt?
                                        # trader.last_exit_time = dt.datetime.now(dt.timezone.utc)

                                else:  # Qty to trade is <= 0
                                    log_console(
                                        logging.DEBUG,
                                        f"Calculated position size ({qty_to_trade:.{trader.qty_precision}f}) is zero or too small for {symbol} based on risk/balance/constraints. No trade placed.",
                                        symbol=symbol,
                                    )
                            else:  # No entry signal found or strength too low
                                log_console(
                                    logging.DEBUG,
                                    f"No valid entry signal found for {symbol} this cycle.",
                                    symbol=symbol,
                                )
                                # Potential signals check (for debugging):
                                # log_console(logging.DEBUG, f"Signals: Long={long_signal}, Short={short_signal}, LStr={long_strength:.2f}, SStr={short_strength:.2f}", symbol=symbol)

                    except Exception as symbol_e:
                        # Catch errors specific to processing a single symbol's cycle
                        log_console(
                            logging.ERROR,
                            f"Error processing trading logic for symbol {symbol}: {symbol_e}",
                            symbol=symbol,
                            exc_info=True,
                        )
                ...
                # --- End of Symbol Loop ---
                if not self.running or self.shutdown_requested:
                    break  # Exit main loop if shutdown requested during symbol processing

                # --- Log Performance Summary Periodically ---
                self.metrics.log_summary(interval=metrics_interval)
                ...
                self.stop()
                # Consider adding a delay before potentially restarting or exiting completely
                time.sleep(5)  # Allow time for cleanup/logging
                break  # Exit the loop after critical error

        # --- Loop Exit ---
        log_console(logging.INFO, "Main trading loop has finished.")
        # Final cleanup and metrics summary are handled in the stop() method if called,
        # or should be called explicitly if loop exits differently.
        # Ensure final cleanup if stop wasn't triggered by signal/error
        if not self.shutdown_requested:
            self.stop()  # Call stop explicitly if loop finished naturally

    def stop(self):
        """Initiates graceful shutdown of the bot."""
        if self.shutdown_requested:  # Prevent multiple shutdown calls
        ...
        close_on_exit = self.bot_cfg.get("CLOSE_POSITIONS_ON_EXIT", False)
        if close_on_exit and not self.dry_run:
            log_console(logging.INFO, "Attempting to close all open positions...")
                ...
                try:
                    ...
                    if position:
                        ...
                        if not trader.close_position(
                            ...
                            log_console(
                                logging.ERROR,
                                f"Failed to close position for {symbol} during shutdown. Manual check may be needed.",
                                symbol=symbol,
                            )
                    # Small delay between closing attempts
                    time.sleep(0.5)
            ...
            for symbol, trader in list(self.traders.items()):
                if trader.current_trade:
                    ...
                    if trader.kline_df is not None and not trader.kline_df.empty:
                        try:
                            sim_exit_price = float(trader.kline_df["close"].iloc[-1])
                        except:
                            pass
    ...
    def _cleanup_resources(self):
        ...
        log_console(logging.DEBUG, "Performing resource cleanup...")
        # Ensure WebSocket is closed if not already done
        self._close_websocket_connection()
        # Potentially close HTTP session if pybit doesn't handle it automatically or if explicit cleanup is desired
        # (Note: pybit HTTP sessions often don't need explicit closing unless using specific session features)
        # if self.session:
        #     try:
        #         # Example if session had a close method: self.session.close()
        #         log_console(logging.DEBUG, "HTTP session closed.")
        #     except Exception as e:
        #         log_console(logging.WARNING, f"Error closing HTTP session: {e}")
        #     self.session = None
        log_console(logging.DEBUG, "Resource cleanup finished.")
    ...
    def _signal_handler(self, signum, frame):
        ...
        if not self.shutdown_requested:
            ...
            # especially if stop() involves network operations or joining threads.
            shutdown_thread = threading.Thread(target=self.stop, name="ShutdownHandler")
            shutdown_thread.start()
        else:
            log_console(logging.INFO, "Shutdown already initiated.")


# --- Configuration Validation ---
def validate_config(config: Dict[str, Any]):
    """
    Performs more detailed validation of the loaded configuration dictionary.
    ...
    """
    log_console(logging.DEBUG, "Performing detailed configuration validation...")

    # --- BYBIT_CONFIG Validation ---
    bybit_cfg = config.get("BYBIT_CONFIG")
    if not isinstance(bybit_cfg, dict):
        raise ValueError("'BYBIT_CONFIG' must be a dictionary.")
    # Account Type validation (must be supported)
    ...
    # Validate each symbol's config
    required_symbol_keys = ["SYMBOL", "TIMEFRAME"]  # LEVERAGE is optional for spot
    valid_timeframes = [
        "1",
        "3",
        "5",
        ...
        "W",
        "M",
    ]  # Check Bybit V5 docs for current list
    for i, sym_cfg in enumerate(symbols_list):
        if not isinstance(sym_cfg, dict):
            raise ValueError(f"Symbol config entry #{i} must be a dictionary.")
        for key in required_symbol_keys:
            if key not in sym_cfg:
                raise ValueError(f"Symbol config #{i} missing required key: '{key}'")
            if not sym_cfg[key]:
                raise ValueError(f"Symbol config #{i} key '{key}' cannot be empty.")
        # Check timeframe validity
        tf = str(sym_cfg["TIMEFRAME"])  # Ensure string comparison
        if tf not in valid_timeframes:
            raise ValueError(
                f"Symbol '{sym_cfg.get('SYMBOL')}' has invalid TIMEFRAME '{tf}'. Must be one of: {valid_timeframes}"
            )
        # Check leverage (if applicable and present)
        if acc_type != "SPOT" and "LEVERAGE" in sym_cfg:
            lev = sym_cfg.get("LEVERAGE")
            if not isinstance(lev, int) or lev <= 0:
                raise ValueError(
                    f"Symbol '{sym_cfg.get('SYMBOL')}' has invalid LEVERAGE '{lev}'. Must be a positive integer."
                )
        ...
        if isinstance(strat_cfg, dict):
            for period_key in [
                "ULTRA_FAST_EMA_PERIOD",
                "FAST_EMA_PERIOD",
                "MID_EMA_PERIOD",
                "SLOW_EMA_PERIOD",
                "ROC_PERIOD",
                "ATR_PERIOD_RISK",
                "ATR_PERIOD_VOLATILITY",
                "RSI_PERIOD",
                "VOLUME_SMA_PERIOD",
                "ADX_PERIOD",
                "BBANDS_PERIOD",
    ...
    #          raise ValueError(f"BOT_CONFIG parameter '{key}' ('{val}') must be a boolean (true/false).")

    # --- RISK_CONFIG Validation ---
    risk_cfg = config.get("RISK_CONFIG")
    if not isinstance(risk_cfg, dict):
        raise ValueError("'RISK_CONFIG' must be a dictionary.")
    numeric_risk_params = [
        "RISK_PER_TRADE_PERCENT",
        "FEE_RATE",
        "ATR_MULTIPLIER_SL",
        "ATR_MULTIPLIER_TP",
        "DRY_RUN_DUMMY_BALANCE",
        "MAX_POSITION_USDT",
    ]
    for key in numeric_risk_params:
...
def main():
    ...
    # Console logging is handled by log_console function
    # File logger level needs to be set
    if args.debug:
        logger.setLevel(logging.DEBUG)
        log_console(logging.DEBUG, "Debug mode enabled. Detailed logging activated.")
    else:
        logger.setLevel(logging.INFO)

    # --- Load and Validate Configuration ---
    try:
        # Load config (includes initial structure checks and KLINE_LIMIT calculation)
        config = load_config(args.config)
        # Perform detailed validation on the loaded config
    ...
    if not API_KEY or not API_SECRET:
        log_console(
            logging.CRITICAL,
            "API keys are missing. Ensure BYBIT_API_KEY and BYBIT_API_SECRET are set in environment or .env file.",
        )
        sys.exit(1)

    # --- Initialize and Run the Bot ---
    bot_instance: Optional[MomentumScannerTrader] = None
    try:
        bot_instance = MomentumScannerTrader(
            api_key=API_KEY, api_secret=API_SECRET, config=config, dry_run=args.dry_run
        )
        # Set up signal handlers using the bot instance *after* it's initialized
        bot_instance._setup_signal_handlers()
        # Start the main trading loop
        bot_instance.run()

    except (ConnectionError, RuntimeError, ValueError) as e:
        # Catch critical initialization errors from MomentumScannerTrader
        log_console(logging.CRITICAL, f"Bot failed to initialize or run: {e}")
        ...
        # Code here runs after the bot.run() loop finishes naturally or via exception/signal
        log_console(logging.INFO, "Bot execution finished or was interrupted.")
        # Final metrics summary is handled within bot.stop() if called
        # Ensure logs are flushed if necessary (usually handled by Python's logging on exit)
        logging.shutdown()
        print(f"{Fore.CYAN}--- Exiting Application ---")


if __name__ == "__main__":
    main()

# --- END OF FILE mom_scan1.2.py ---
