...
def calculate_indicators_momentum(df: pd.DataFrame, strategy_cfg: Dict[str, Any], config: Dict[str, Any]) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        try:
            ...
            df_out["roc_smooth"] = ta.sma(df_out["roc"].fillna(0), length=3) # Fill NaNs before smoothing
        except Exception as e:
        ...
        try:
            ...
            # Normalized ATR
            close_safe = df_out["close"].replace(0, np.nan).ffill().fillna(1) # Avoid division by zero
            df_out["norm_atr"] = ((df_out["atr_risk"] / close_safe) * 100).ffill().fillna(0)
        except Exception as e:
        ...
        try:
            ...
            df_out["volume_percentile_75"] = df_out["volume"].rolling(window=vol_period, min_periods=min_periods_vol).quantile(0.75) # Handle NaNs later
        ...
        numeric_cols_to_fill.update({f"{smoother_prefix}_{ma}": 0.0 for ma in ["ultra", "fast", "mid", "slow"]})
        ...
        df_out["high_volume"] = (df_out["volume"].fillna(0) > df_out["volume_percentile_75"]) & \
        ...
        if not all(col in df_out.columns for col in trend_req_cols):
            ...
            trendline_shift = df_out[trendline_col].shift(1).ffill().fillna(df_out[trendline_col]) # Fill initial shift NaN
        ...
        if not all(col in df_out.columns for col in entry_req_cols):
            ...
            trendline_shift = df_out[trendline_col].shift(1).ffill().fillna(df_out[trendline_col])
            ...
            df_out["long_signal_strength"] = sum(cond.astype(int) for cond in long_conditions) / len(long_conditions)
            df_out["short_signal_strength"] = sum(cond.astype(int) for cond in short_conditions) / len(short_conditions)
        ...
        if len(df_out) >= 2 and df_out.iloc[-2:][final_check_cols].isnull().any().any():
             log_console(logging.ERROR, "Indicator Calc: Critical columns contain NaN in the last two rows after processing. Check calculations.")
             log_console(logging.DEBUG, f"Last 2 rows tail:\n{df_out.iloc[-2:]}") # Log problematic rows
             return None
        elif len(df_out) == 1 and df_out.iloc[-1:][final_check_cols].isnull().any().any():
...
class SymbolTrader:
            ...
            if info and info.get("retCode") == 0:
                ...
                if result_list:
                    ...
                    try:
                        ...
                        self.qty_precision = len(self.qty_step.split('.')[-1]) if '.' in self.qty_step and self.qty_step_float > 0 and self.qty_step.split('.')[-1] != '0' else 0
                        self.price_precision = len(self.tick_size.split('.')[-1]) if '.' in self.tick_size and tick_size_float > 0 and self.tick_size.split('.')[-1] != '0' else 0
                    ...
                    log_console(logging.ERROR, "Instrument info fetch failed: Empty result list.", symbol=self.symbol)
                    return False
            else:
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if error_code == 110043 or "leverage not modified" in error_msg.lower() or "same leverage" in error_msg.lower():
                    ...
                    log_console(logging.WARNING, f"Failed to set leverage: {error_msg} (Code: {error_code}).", symbol=self.symbol)

            # Add margin mode setting here if needed (requires careful checking of V5 parameters)

        except Exception as e:
            log_console(logging.WARNING, f"Exception during initial setup (leverage/margin): {e}", symbol=self.symbol, exc_info=True)
    ...
    def _process_kline_queue(self) -> bool:
            ...
            try:
                ...
                if self.kline_df is None or self.kline_df.empty:
                    ...
                    if idx_to_check in self.kline_df.index:
                        ...
                        self.kline_df.update(new_row) # More efficient update for existing rows
                        ...
                        max_rows = self.internal_cfg.get("KLINE_LIMIT", 150) + 50 # Increased buffer
        ...
        if df_updated:
            ...
            calculated_df = calculate_indicators_momentum(self.kline_df, self.strategy_cfg, self.config)
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if kline_list:
                    ...
                    log_console(logging.WARNING, "REST KLine fetch returned empty list.", symbol=self.symbol)
                    return None
            else: # API error
                error_msg = response.get('retMsg', 'N/A') if response else "Empty response"
                error_code = response.get('retCode', 'N/A')
    ...
    def get_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        if self.kline_df is None or self.kline_df.empty:
            ...
            log_console(logging.DEBUG, "WS disabled, using REST for kline data.", symbol=self.symbol)
                    ...
                    if pos.get("symbol") == self.symbol:
                        try:
                            ...
                            if pos_side in ["Buy", "Sell"] and abs(pos_size) > min_significant_qty:
                                ...
                                pos['side'] = pos_side
                                pos['leverage'] = pos.get('leverage', str(self.leverage)) # Store as string
                                ...
                                elif pos.get('createdTime'): # Try createdTime from API (ms string)
    ...
    def confirm_order(self, order_id: str, expected_qty: float) -> Tuple[float, Optional[float]]:
        ...
        if not order_id:
            log_console(logging.ERROR, "Confirm Order: Invalid Order ID.", symbol=self.symbol)
            return 0.0, None
            ...
            try:
                ...
                if response and response.get("retCode") == 0:
                    ...
                    if order_list:
                        ...
                        log_console(logging.DEBUG, f"Order {order_id} not in history yet (Attempt {attempt + 1}).", symbol=self.symbol)
    ...
    def place_order(self, side: str, qty: float, stop_loss_price: Optional[float] = None,
        ...
        if self.dry_run:
            ...
            if sim_entry_price <= 0: sim_entry_price = 1.0 # Dummy
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    ...
                    if filled_qty >= (qty * min_fill_ratio):
                        ...
                        self.current_trade = {
                            "orderId": order_id, "symbol": self.symbol, "side": side,
                            "size": filled_qty, "avgPrice": avg_price if avg_price else 0.0,
                            "entry_time": dt.datetime.now(dt.timezone.utc),
                            "stopLoss": sl_str, "takeProfit": tp_str,
                            "leverage": str(self.leverage), "orderStatus": "Filled",
                        }
                ...
                elif error_code in [110013, 110014]: log_console(logging.ERROR,"ORDER REJECTED: Qty issue.", symbol=self.symbol)
                elif error_code == 110017: log_console(logging.ERROR,"ORDER REJECTED: Risk control.", symbol=self.symbol)
                elif error_code == 110040: log_console(logging.ERROR,"ORDER REJECTED: Risk limit.", symbol=self.symbol)
                elif error_code == 110073: log_console(logging.ERROR,"ORDER REJECTED: TP/SL invalid.", symbol=self.symbol)
                return None
        except Exception as e:
    ...
    def close_position(self, position_data: Dict[str, Any], exit_reason: str = "Signal", exit_price_est: Optional[float] = None) -> bool:
        ...
        if self.dry_run:
            if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                ...
                if sim_exit_price is None or sim_exit_price <= 0:
                    sim_exit_price = self.current_trade.get("avgPrice", 1.0) # Fallback entry
                ...
                entry_time_sim = self.current_trade.get("entry_time") or (dt.datetime.now(dt.timezone.utc) - dt.timedelta(minutes=15))
                ...
                log_console(logging.WARNING, "[DRY RUN] Close attempt failed: No simulated trade open.", symbol=self.symbol)
                self.current_trade = None
                return False
        ...
        try:
            ...
            response = self.session.place_order(
                ...
                reduceOnly=True, orderLinkId=close_order_link_id,
            )
            ...
            if response and response.get("retCode") == 0:
                    ...
                    if filled_qty >= abs_size * min_close_fill_ratio:
                        ...
                        if final_pos_check is not None:
                            ...
                            if abs(final_pos_size) > self.min_order_qty * 0.01:
                                log_console(logging.WARNING, f"Position check after close shows remaining size: {final_pos_size:.{self.qty_precision}f}. Close incomplete?", symbol=self.symbol)
                        ...
                        if not isinstance(entry_time_log, dt.datetime):
                            if self.current_trade and self.current_trade.get('symbol') == self.symbol: entry_time_log = self.current_trade.get('entry_time')
                            if not isinstance(entry_time_log, dt.datetime):
                                log_console(logging.WARNING, "Entry time missing for metrics. Using fallback.", symbol=self.symbol)
                        ...
                        self.metrics.add_trade(
                            ...
                            qty=filled_qty, leverage=leverage,
                        )
                        self.current_trade = None
                        return True
                    else:
                        log_console(logging.ERROR, f"Close order {order_id} filled qty ({filled_qty:.{self.qty_precision}f}) < expected ({abs_size:.{self.qty_precision}f}). Manual check!", symbol=self.symbol)
                        return False
                else:
                ...
                already_closed_msg = "position size is zero" in error_msg.lower() or \
                                     "order qty exceeded position size" in error_msg.lower() or \
                                     "reduceonly" in error_msg.lower()

                if error_code in pos_already_closed_codes or already_closed_msg:
                    log_console(logging.WARNING, f"Close attempt failed, API indicates already closed (Code: {error_code}, Msg: '{error_msg}'). Assuming closed.", symbol=self.symbol)
                    if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                        exit_time = dt.datetime.now(dt.timezone.utc)
                        ...
                        log_console(logging.INFO, "Logging trade metrics from internal state (API showed already closed).", symbol=self.symbol)
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        if higher_tf == self.timeframe:
            log_console(logging.WARNING, "HTF same as base timeframe. Disabling MTF check.", symbol=self.symbol)
        ...
        if df_higher is None or df_higher.empty or len(df_higher) < 2:
            log_console(logging.WARNING, f"Could not get HTF ({higher_tf}) data. Allowing trade (neutral).", symbol=self.symbol)
            self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
        ...
        try:
            ...
            if fast_ma_col not in df_higher.columns or mid_ma_col not in df_higher.columns:
                log_console(logging.ERROR, f"Required MA cols ({fast_ma_col}, {mid_ma_col}) not found in HTF ({higher_tf}) data. Allowing trade.", symbol=self.symbol)
                self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
                return True
            ...
            if pd.isna(fast_ma_prev) or pd.isna(mid_ma_prev):
                ...
                trend_desc = "Bullish (Fast>Mid)" if trend_is_favorable else "Bearish (Fast<=Mid)"
            ...
            log_console(logging.WARNING, f"Index error accessing HTF ({higher_tf}) data (need >= 2 rows). Allowing trade.", symbol=self.symbol)
            self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
            return True
        except Exception as e:
...
class MomentumScannerTrader:
    ...
    def __init__(self, api_key: str, api_secret: str, config: Dict[str, Any], dry_run: bool = False):
        ...
        elif self.account_type == "UNIFIED": self.category = "linear" # Default UNIFIED to linear
        elif self.account_type == "CONTRACT": self.category = "linear" # Default CONTRACT to linear (can be inverse too)
        ...
        try:
            ...
            if self.traders and any(trader.use_websocket for trader in self.traders.values()):
                ...
                log_console(logging.INFO, "WebSocket disabled or no traders. Using REST API only.")
    ...
    def _test_api_connection(self):
        ...
        try:
            ...
            if server_time_resp and server_time_resp.get('retCode') == 0:
                ...
                if time_diff > 60:
                    log_console(logging.WARNING, f"System clock differs from server by {time_diff:.1f}s. Check time sync.")
            ...
            log_console(logging.CRITICAL, f"API connection failed: {ce}. Check keys, permissions, network.")
            raise ce
        except Exception as e:
    ...
    def _initialize_traders(self):
        ...
        for i, symbol_cfg in enumerate(deepcopy(symbols_cfg_list)):
            ...
            if not symbol or not isinstance(symbol, str):
                log_console(logging.ERROR, f"Symbol config #{i} missing/invalid 'SYMBOL'. Skipping.")
                continue
            if symbol in temp_traders:
                log_console(logging.WARNING, f"Duplicate symbol {symbol} in config. Skipping.", symbol=symbol)
                continue

            log_console(logging.INFO, f"--- Initializing trader for {symbol} ---", symbol=symbol)
            try:
                if not self.session: raise RuntimeError("HTTP session not available.")
                trader = SymbolTrader(
                ...
                log_console(logging.ERROR, f"Initialization failed for {symbol} (RuntimeError). Skipping.", symbol=symbol)
            except Exception as e:
    ...
    def _start_and_monitor_websocket(self):
        ...
        if ws_channel_type not in ["linear", "inverse", "spot", "option"]:
             log_console(logging.ERROR, f"Unsupported category '{self.category}' for WS. Defaulting to 'linear'.")
             ws_channel_type = "linear"

        retry_delay = 10; max_retry_delay = 120
        ...
        while not self.shutdown_requested:
            ...
            if not ws_conn_active:
                try:
                    ...
                    with self.ws_lock: self.ws = None; self.ws_active = False
    ...
    def _close_websocket_connection(self):
        ...
        with self.ws_lock:
            if self.ws:
                ...
                try: self.ws.exit()
                except Exception as e: log_console(logging.ERROR, f"Error closing WS object: {e}", exc_info=False)
                finally: self.ws = None; self.ws_active = False
            ...
            if trader.use_websocket and (trader.kline_df is None or trader.kline_df.empty):
                ...
                try:
                    ...
                    if initial_df is not None and not initial_df.empty:
                        ...
                        log_console(logging.WARNING, f"Failed initial REST fetch for {symbol}. Relying on WS updates.", symbol=symbol)
                except Exception as e:
        ...
        try:
            ...
            if isinstance(message, dict):
                ...
                if op == "auth":
                    ...
                    success = message.get("success", False)
                    topic = message.get("ret_msg", message.get("req_id")) # Topic might be in ret_msg or req_id? Check docs.
                    if success: log_console(logging.DEBUG, f"WS Subscribe Ack: {topic}")
                    else: log_console(logging.ERROR, f"WS Subscribe Fail: {topic} - {message.get('ret_msg', 'N/A')}")
        except Exception as e:
            log_console(logging.ERROR, f"Error handling generic WS message: {e}", exc_info=True)

    def _handle_ws_error(self, error):
        """WS error callback."""
        log_console(logging.ERROR, f"WebSocket Error: {error}", exc_info=isinstance(error, Exception))
    ...
    def _handle_ws_close(self, ws_app):
        ...
        with self.ws_lock:
            ...
            if self.ws_active or self.ws is not None:
                self.ws_active = False; self.ws = None
    ...
    def run(self):
        ...
        if self.dry_run:
            ...
            if balance > 0:
                log_console(logging.INFO, f"Initial Balance Check: {balance:.2f} USDT Available")
                last_balance_check_time = time.time()
            else:
                log_console(logging.WARNING, f"Initial {self.account_type} USDT balance zero or fetch failed. Trades cannot be sized.")
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                if self.ws_connection_thread and not ws_currently_active:
                    log_console(logging.WARNING, "WebSocket connection inactive. Relying on REST API.")
                ...
                if not self.dry_run and (current_time - last_balance_check_time > balance_check_interval):
                    ...
                    if abs(fetched_balance - balance) > 0.01: # Log if changed
                        log_console(logging.INFO, f"Checked Balance: {fetched_balance:.2f} USDT Available")
                    balance = fetched_balance
                    last_balance_check_time = current_time
                    if balance <= 0:
                        log_console(logging.WARNING, f"{self.account_type} USDT balance zero. No trades can be sized.")
                ...
                for symbol in list(self.traders.keys()):
                    if not self.running or self.shutdown_requested: break
                    ...
                    try:
                        ...
                        if trader.last_exit_time:
                            ...
                            if cooldown_seconds > 0:
                                ...
                                if time_since_exit < cooldown_seconds:
                                    log_console(logging.INFO, f"Cooldown active ({cooldown_seconds - time_since_exit:.0f}s left).", symbol=symbol)
                                    continue
                        ...
                        if df_with_indicators is None or df_with_indicators.empty or len(df_with_indicators) < 2:
                            log_console(logging.DEBUG, "Insufficient data or calc failed. Skipping cycle.", symbol=symbol)
                            continue
                        ...
                        try:
                            ...
                            log_console(logging.WARNING, f"Cannot access candle data (need >= 2 rows, got {len(df_with_indicators)}). Skipping.", symbol=symbol)
                            continue
                        ...
                        if any(pd.isna(last_closed_candle.get(ind)) for ind in critical_indicators):
                            log_console(logging.WARNING, f"Critical indicator NaN on closed candle ({last_closed_candle.name}). Skipping.", symbol=symbol)
                            continue
                        ...
                        if pd.isnull(current_price) or current_price <= 0:
                            ...
                            if pd.isnull(current_price) or current_price <= 0:
                                log_console(logging.ERROR, "Cannot get valid current price. Skipping.", symbol=symbol)
                                continue
                        ...
                        if position:
                            ...
                            if not (pos_side in ["Buy", "Sell"] and isinstance(pos_size, float) and abs(pos_size) > 1e-9 and isinstance(entry_price, float) and entry_price > 0):
                                ...
                                trader.current_trade = None; position = None
                                continue
                            ...
                            if exit_reason:
                                log_console(logging.INFO, f"Exit condition: {exit_reason}. Closing position.", symbol=symbol)
                                if trader.close_position(position, exit_reason=exit_reason, exit_price_est=current_price):
                                    log_console(logging.INFO, "Position closed successfully.", symbol=symbol)
                                else:
                                    log_console(logging.ERROR, f"Failed to close position after exit signal '{exit_reason}'. Manual check!", symbol=symbol)
                                continue # Process next symbol after close attempt
                            else:
                                log_console(logging.DEBUG, "Holding position. No exit signal.", symbol=symbol)
                            ...
                            if entry_side:
                                ...
                                if stop_loss_price is None or stop_loss_price <= 0:
                                    ...
                                    continue
                                if take_profit_price is not None:
                                    if take_profit_price <= 0: take_profit_price = None # Ignore invalid TP
                                    elif entry_side == "Buy" and take_profit_price <= entry_price_est: take_profit_price = None
                                    elif entry_side == "Sell" and take_profit_price >= entry_price_est: take_profit_price = None
                                ...
                                if qty_to_trade > 0:
                                    log_console(logging.INFO, f"{entry_side.upper()} ENTRY SIGNAL. Str={signal_strength:.2f}. Size={qty_to_trade:.{trader.qty_precision}f}. SL={stop_loss_price:.{trader.price_precision}f}, TP={take_profit_price or 'None'}", symbol=symbol)
                                    order_id = trader.place_order(entry_side, qty_to_trade, stop_loss_price, take_profit_price)
                                    if order_id: log_console(logging.INFO, f"{entry_side.upper()} order placed successfully (ID: {order_id}).", symbol=symbol)
                                    else: log_console(logging.ERROR, f"{entry_side.upper()} order placement failed.", symbol=symbol)
                                    continue # Process next symbol after entry attempt
                                else:
                                    log_console(logging.INFO, f"{entry_side.upper()} signal, but calculated size is zero. Skipping entry.", symbol=symbol)
                            else: # No entry signal
                                 log_console(logging.DEBUG, "No valid entry signal detected.", symbol=symbol)
                ...
                log_console(logging.INFO,"KeyboardInterrupt in main loop. Initiating shutdown...")
                self.shutdown_requested = True; self.running = False
                break
            except Exception as loop_e:
                log_console(logging.CRITICAL, f"CRITICAL ERROR in main loop: {loop_e}. Shutting down.", exc_info=True)
                self.shutdown_requested = True; self.running = False
                break
            ...
            if self.running and not self.shutdown_requested:
                ...
                try: time.sleep(sleep_duration)
                except KeyboardInterrupt:
                    log_console(logging.INFO,"KeyboardInterrupt during sleep. Shutting down...")
                    self.shutdown_requested = True; self.running = False
                    break
        # End of main loop
        ...
        if close_on_exit and not self.dry_run:
            ...
            for symbol, trader in list(self.traders.items()):
                try:
                    ...
                    if position is None: time.sleep(0.5); position = trader.get_position(log_error=True) # Retry once

                    if position:
                        log_console(logging.WARNING, f"Closing position for {symbol}...", symbol=symbol)
                        ...
                        if trader.close_position(position, exit_reason="Shutdown", exit_price_est=last_price):
                            closed_count += 1; time.sleep(0.5)
                        else: failed_closes.append(symbol)
                except Exception as e:
            ...
            if failed_closes: log_console(logging.CRITICAL, f"FAILED TO CLOSE: {', '.join(failed_closes)}. MANUAL ACTION REQUIRED.")
            elif close_on_exit: log_console(logging.INFO, "Position closing attempt complete.")
        elif self.dry_run: log_console(logging.INFO, "Dry run: No real positions to close.")
        else: log_console(logging.INFO, "Close on shutdown disabled. Positions may remain open.")

        # Save final metrics
        try: self.metrics.save_on_shutdown()
        except Exception as metrics_e: log_console(logging.ERROR, f"Error saving final metrics: {metrics_e}", exc_info=True)
