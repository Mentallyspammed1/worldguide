    ...
    def add_trade(
        ...
        with self.lock:
            try:
                ...
                log_console(
                    logging.ERROR, f"Metrics: Invalid leverage value '{leverage}' for {symbol}. Using 1.", symbol=symbol
                )
...
class SymbolTrader:
                ...
                if result_list:
                    ...
                    try:
                        ...
                        self.qty_precision = (
                            ...
                            if "." in self.qty_step and self.qty_step_float > 0 and self.qty_step.split(".")[-1] != "0"
                            else 0
                        )
                        self.price_precision = (
                            len(self.tick_size.split(".")[-1])
                            if "." in self.tick_size and tick_size_float > 0 and self.tick_size.split(".")[-1] != "0"
                            else 0
                        )
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if error_code == 110007:
                    ...
                    log_console(logging.ERROR, "ORDER REJECTED: Qty issue.", symbol=self.symbol)
                elif error_code == 110017:
                    log_console(logging.ERROR, "ORDER REJECTED: Risk control.", symbol=self.symbol)
                elif error_code == 110040:
                    log_console(logging.ERROR, "ORDER REJECTED: Risk limit.", symbol=self.symbol)
                elif error_code == 110073:
                    log_console(logging.ERROR, "ORDER REJECTED: TP/SL invalid.", symbol=self.symbol)
                return None
        except Exception as e:
    ...
    def close_position(
        ...
        if self.dry_run:
            if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                ...
                log_console(
                    logging.WARNING, "[DRY RUN] Close attempt failed: No simulated trade open.", symbol=self.symbol
                )
                self.current_trade = None
                return False
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        if df_higher is None or df_higher.empty or len(df_higher) < 2:
            log_console(
                logging.WARNING, f"Could not get HTF ({higher_tf}) data. Allowing trade (neutral).", symbol=self.symbol
            )
            self.higher_tf_cache = True
            self.higher_tf_cache_time = now_utc
...
class MomentumScannerTrader:
    ...
    def _test_api_connection(self):
        ...
        try:
            ...
            if server_time_resp and server_time_resp.get("retCode") == 0:
                ...
                if time_diff > 60:
                    log_console(
                        logging.WARNING, f"System clock differs from server by {time_diff:.1f}s. Check time sync."
                    )
    ...
    def _initialize_traders(self):
        ...
        for i, symbol_cfg in enumerate(deepcopy(symbols_cfg_list)):
            ...
            try:
                ...
                log_console(
                    logging.ERROR, f"Initialization failed for {symbol} (RuntimeError). Skipping.", symbol=symbol
                )
            except Exception as e:
                log_console(
                    logging.ERROR,
            ...
            if isinstance(message, dict):
                ...
                if op == "auth":
                    ...
                    if success:
                        ...
                        log_console(logging.ERROR, f"WS Subscribe Fail: {topic} - {message.get('ret_msg', 'N/A')}")
        except Exception as e:
                ...
                for symbol in list(self.traders.keys()):
                    ...
                    try:
                        ...
                        if df_with_indicators is None or df_with_indicators.empty or len(df_with_indicators) < 2:
                            log_console(
                                logging.DEBUG, "Insufficient data or calc failed. Skipping cycle.", symbol=symbol
                            )
                            continue
                        ...
                        if position:
                            ...
                            if exit_reason:
                                log_console(
                                    logging.INFO, f"Exit condition: {exit_reason}. Closing position.", symbol=symbol
                                )
    ...
    def shutdown(self):
        ...
        if close_on_exit and not self.dry_run:
            ...
            if failed_closes:
                log_console(logging.CRITICAL, f"FAILED TO CLOSE: {', '.join(failed_closes)}. MANUAL ACTION REQUIRED.")
            elif close_on_exit:
...
if __name__ == "__main__":
    ...
    try:
        ...
        for section, expected_type in required_sections.items():
            if section not in config:
                ...
                validation_errors.append(f"Section '{section}' must be {expected_type.__name__}.")
        ...
        if not validation_errors:  # Proceed only if top sections are okay
            ...
            if not isinstance(bybit_cfg.get("USE_TESTNET", False), bool):
                validation_errors.append("BYBIT_CONFIG: USE_TESTNET must be true/false.")
            ...
            if not isinstance(fee_rate, (int, float)) or fee_rate < 0:
                validation_errors.append(f"RISK_CONFIG: FEE_RATE ({fee_rate}) must be non-negative.")
            ...
            if max_pos_usdt is not None and (not isinstance(max_pos_usdt, (int, float)) or max_pos_usdt <= 0):
                validation_errors.append(f"RISK_CONFIG: MAX_POSITION_USDT ({max_pos_usdt}) must be positive if set.")
            if args.dry_run:
            ...
            if (
                ...
                validation_errors.append("RISK_CONFIG: ATR_MULTIPLIER_SL must be positive.")
            if (
                not isinstance(risk_cfg.get("ATR_MULTIPLIER_TP", 2.0), (int, float))
                or risk_cfg.get("ATR_MULTIPLIER_TP", 2.0) < 0
            ):
                validation_errors.append("RISK_CONFIG: ATR_MULTIPLIER_TP must be non-negative.")
            if not isinstance(risk_cfg.get("ENABLE_TRAILING_STOP", False), bool):
                validation_errors.append("RISK_CONFIG: ENABLE_TRAILING_STOP must be true/false.")
            ...
            if (
                ...
                validation_errors.append("BOT_CONFIG: SLEEP_INTERVAL_SECONDS must be positive.")
            if not isinstance(bot_cfg.get("KLINE_LIMIT_BUFFER"), int) or bot_cfg.get("KLINE_LIMIT_BUFFER") < 0:
                validation_errors.append("BOT_CONFIG: KLINE_LIMIT_BUFFER must be non-negative.")
            ...
            if (
                ...
                validation_errors.append("BOT_CONFIG: ORDER_CONFIRM_RETRIES must be non-negative.")
            if (
                not isinstance(bot_cfg.get("ORDER_CONFIRM_DELAY_SECONDS", 2.0), (int, float))
                or bot_cfg.get("ORDER_CONFIRM_DELAY_SECONDS", 2.0) <= 0
            ):
                validation_errors.append("BOT_CONFIG: ORDER_CONFIRM_DELAY_SECONDS must be positive.")
            if not isinstance(bot_cfg.get("PROCESS_CONFIRMED_KLINE_ONLY", True), bool):
                validation_errors.append("BOT_CONFIG: PROCESS_CONFIRMED_KLINE_ONLY must be true/false.")
            if not isinstance(bot_cfg.get("CLOSE_POSITIONS_ON_SHUTDOWN", False), bool):
                validation_errors.append("BOT_CONFIG: CLOSE_POSITIONS_ON_SHUTDOWN must be true/false.")
            if (
                not isinstance(bot_cfg.get("HIGHER_TF_CACHE_SECONDS", 3600), int)
                or bot_cfg.get("HIGHER_TF_CACHE_SECONDS", 3600) < 0
            ):
                validation_errors.append("BOT_CONFIG: HIGHER_TF_CACHE_SECONDS must be non-negative.")
            if (
                not isinstance(bot_cfg.get("KLINE_STALENESS_SECONDS", 300), int)
                or bot_cfg.get("KLINE_STALENESS_SECONDS", 300) <= 0
            ):
                validation_errors.append("BOT_CONFIG: KLINE_STALENESS_SECONDS must be positive.")
            ...
            if not isinstance(symbols, list):
                ...
                for i, symbol_cfg in enumerate(symbols):
                    ...
                    symbol = symbol_cfg.get("SYMBOL")
                    if not symbol or not isinstance(symbol, str):
                        validation_errors.append(f"SYMBOLS entry {i}: Missing/invalid 'SYMBOL'.")
                        continue
                    ...
                    if "STRATEGY_CONFIG" not in symbol_cfg or not isinstance(symbol_cfg.get("STRATEGY_CONFIG"), dict):
                        validation_errors.append(f"Symbol {symbol}: Missing/invalid 'STRATEGY_CONFIG'.")
                    leverage = symbol_cfg.get("LEVERAGE")
                    if not isinstance(leverage, int) or not (0 < leverage <= 100):
                        validation_errors.append(f"Symbol {symbol}: LEVERAGE ({leverage}) invalid (1-100 integer).")
                    ...
                    if not isinstance(symbol_cfg.get("USE_WEBSOCKET", True), bool):
                        validation_errors.append(f"Symbol {symbol}: USE_WEBSOCKET must be true/false.")
                    ...
                    if isinstance(strategy_cfg, dict):
                        for k, v in strategy_cfg.items():
                            ...
                            if k.upper() in ["USE_EHLERS_SMOOTHER", "ENABLE_MULTI_TIMEFRAME"] and not isinstance(
                                ...
                                validation_errors.append(f"Symbol {symbol}: Param '{k}' ({v}) must be true/false.")
                            if k.upper() in [
                                "MOM_THRESHOLD_PERCENT",
                                "ADX_THRESHOLD",
                                "MIN_SIGNAL_STRENGTH",
                            ] and not isinstance(v, (int, float)):
                                validation_errors.append(f"Symbol {symbol}: Param '{k}' ({v}) must be a number.")
                            if k.upper() in ["SL_TRIGGER_BY", "TP_TRIGGER_BY"] and v not in [
                                "LastPrice",
                                "MarkPrice",
                                "IndexPrice",
                            ]:
                        ...
                        if strategy_cfg.get("ENABLE_MULTI_TIMEFRAME"):
                            ...
                            if htf not in valid_timeframes:
                                validation_errors.append(f"Symbol {symbol}: Invalid HIGHER_TIMEFRAME '{htf}'.")
                            elif htf == timeframe:
                                validation_errors.append(
    ...
    if validation_errors:
        log_console(
            logging.CRITICAL, f"{Back.RED}{Fore.WHITE}Config validation failed ({len(validation_errors)} errors):"
        )
    ...
    try:
        ...
        if trader_instance:
            ...
            log_console(logging.WARNING, "Trader instance not fully initialized before interrupt.")
