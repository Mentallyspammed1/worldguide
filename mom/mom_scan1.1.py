...
"""
Enhanced Momentum Scanner Trading Bot for Bybit (V5 API) - mom_scan1.0.py
        ...
        try:
         ...
         try:
             df.index = df.index.tz_localize('UTC', ambiguous='infer')
              ...
              log_console(logging.WARNING, f"Indicator Calc: Index might already be timezone-aware? Error during tz_localize: {tz_err}. Proceeding.")
    ...
    try:
        ...
        try:
            ...
            if roc_period > 0:
                ...
                roc_smooth_series = ta.sma(roc_filled, length=3) # Fixed 3-period smoothing for ROC
        ...
        try:
            ...
            if bb_period > 0 and bb_std > 0:
                ...
                bbu_col = f"BBU_{bb_period}_{bb_std:.1f}" # Note: pandas_ta often uses float in col name
                ...
                if bbands is not None and not bbands.empty and all(c in bbands.columns for c in [bbu_col, bbl_col, bbm_col]):
                    ...
                    log_console(logging.WARNING, f"Indicator Calc: Bollinger Bands calculation failed or missing expected columns (e.g., {bbu_col}). Check pandas_ta version/output.")
                    # Column remains NaN
        ...
        if not trend_ma_cols_ok or not trendline_ok:
            log_console(logging.WARNING, f"Indicator Calc: Cannot define trend. MA cols OK: {trend_ma_cols_ok}, Trendline OK: {trendline_ok}. Assuming neutral trend.")
        else:
        ...
        entry_ma_cols_ok = all(ma_col in calculated_ma_cols for ma_col in [fast_ma_col, mid_ma_col, ultra_fast_ma_col] if ma_col in entry_req_cols)
        entry_other_cols_ok = all(col in df_out.columns for col in entry_req_cols if col not in [fast_ma_col, mid_ma_col, ultra_fast_ma_col])
        ...
        if not required_available:
            ...
            if 'trendline_shift' not in locals(): # Check if calculated in trend section
                if trendline_ok: # Only recalculate if trendline itself is valid
                     ...
                     log_console(logging.WARNING,"Indicator Calc: Trendline invalid, cannot calculate trendline shift for entry signals.")
            ...
            long_cond_itrend = df_out[trendline_col] > trendline_shift * (1 + FLOAT_COMPARISON_TOLERANCE) if trendline_ok else False # Only check if trendline valid
            ...
            short_cond_itrend = df_out[trendline_col] < trendline_shift * (1 - FLOAT_COMPARISON_TOLERANCE) if trendline_ok else False # Only check if trendline valid
        ...
        exit_ma_cols_ok = all(ma_col in calculated_ma_cols for ma_col in [fast_ma_col, mid_ma_col] if ma_col in exit_req_cols)
        ...
        if not exit_required_available:
            ...
            if 'trendline_shift' not in locals():
                if trendline_ok: # Only recalculate if trendline itself is valid
                     ...
                     log_console(logging.WARNING,"Indicator Calc: Trendline invalid, cannot calculate trendline shift for exit signals.")
                     trendline_shift = pd.Series(0.0, index=df_out.index)
            ...
            exit_long_cond3 = df_out[trendline_col] < trendline_shift * (1 - FLOAT_COMPARISON_TOLERANCE) if trendline_ok else False
            ...
            exit_short_cond3 = df_out[trendline_col] > trendline_shift * (1 + FLOAT_COMPARISON_TOLERANCE) if trendline_ok else False
                ...
                if not isinstance(coin_balance_list, list):
                 ...
                 if account_type.upper() == "SPOT":
                     if isinstance(list_data, list) and all(isinstance(item, dict) for item in list_data):
                         ...
                         log_console(logging.WARNING, f"Expected list of dicts for 'SPOT' balance response list, got different structure. Parsing failed.")
                         return 0.0
                 elif account_type.upper() == "CONTRACT":
                     account_data = list_data[0] if list_data else {}
                     coin_balance_list = account_data.get("coin", [])
                     if not isinstance(coin_balance_list, list):
                          log_console(logging.WARNING, f"Expected list for 'coin' in CONTRACT balance response, got {type(coin_balance_list)}. Parsing failed.")
                          return 0.0
            else:
            ...
            for coin_data in coin_balance_list:
                if isinstance(coin_data, dict) and coin_data.get("coin") == coin:
                    ...
                    if balance_str is None or balance_str == '':
                        balance_str = coin_data.get("availableBalance") # Often used in Unified/Contract/Spot
...
def calculate_position_size_atr(
    ...
    try:
        ...
        if abs(position_size_adjusted - position_size) > FLOAT_COMPARISON_TOLERANCE: # Log only if rounding occurred significantly
            ...
            try:
                ...
                if calculated_df is not None and not calculated_df.empty:
                    ...
                    if not calculated_df.index.tz or calculated_df.index.tz != dt.timezone.utc:
                         log_console(logging.WARNING, f"Indicator calculation for {self.symbol} returned DF without UTC timezone. Setting/Converting UTC.", symbol=self.symbol)
                         if calculated_df.index.tz: # If localized but not UTC
                              calculated_df.index = calculated_df.index.tz_convert('UTC')
        ...
        if needs_rest_fetch:
            ...
            if rest_df_with_indicators is not None and not rest_df_with_indicators.empty:
                ...
                if not rest_df_with_indicators.index.tz or rest_df_with_indicators.index.tz != dt.timezone.utc:
                     log_console(logging.ERROR, f"REST fetch for {self.symbol} returned DF without UTC timezone! Attempting to set/convert UTC.", symbol=self.symbol)
                            ...
                            if pos_side in ["Buy", "Sell"] and abs(pos_size) >= min_significant_qty:
                                ...
                                # 2. Fallback to API createdTime (convert ms string to datetime UTC)
                                    ...
                                    try:
                                        ...
                                        else: raise ValueError("Timestamp conversion resulted in NaT")
                                    except (ValueError, TypeError, KeyError) as ts_err:
                                        log_console(logging.DEBUG, f"Could not parse createdTime '{position_standardized['createdTime']}' for position entry time: {ts_err}", symbol=self.symbol)
    ...
    def close_position(self, position_data: Dict[str, Any], exit_reason: str = "Signal", exit_price_est: Optional[float] = None) -> bool:
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                pos_already_closed_codes = [110025, 110066, 110044, 110071, 3400074] # Added 3400074 tentatively
...
class MomentumScannerTrader:
                ...
                try:
                    ...
                    if initial_df is not None and not initial_df.empty:
                        ...
                        if not initial_df.index.tz or initial_df.index.tz != dt.timezone.utc:
                             if initial_df.index.tz: initial_df.index = initial_df.index.tz_convert('UTC')
                             else: initial_df.index = initial_df.index.tz_localize('UTC')
    ...
    def run(self):
        ...
        if "norm_atr" in df_out and atr_period_volatility > 0:
            ...
            volatility_factor = df_out["norm_atr"].rolling(window=atr_period_volatility, min_periods=min_periods_vol_atr).mean().ffill().fillna(1.0)
            # Clip volatility factor to prevent extreme adjustments
            volatility_factor = np.clip(volatility_factor,
                                        strategy_cfg.get("VOLATILITY_FACTOR_MIN", 0.5),
                                        strategy_cfg.get("VOLATILITY_FACTOR_MAX", 2.0))
        else:
            log_console(logging.DEBUG,"Indicator Calc: Cannot calculate volatility factor (norm_atr missing or period invalid). Using factor 1.0.")
            volatility_factor = pd.Series(1.0, index=df_out.index) # Neutral factor

        rsi_base_low = strategy_cfg.get("RSI_LOW_THRESHOLD", 40)
        rsi_base_high = strategy_cfg.get("RSI_HIGH_THRESHOLD", 75)
        # Use pd.Series constructor to ensure index alignment
        rsi_low = pd.Series(np.clip(rsi_base_low - (strategy_cfg.get("RSI_VOLATILITY_ADJUST", 5) * volatility_factor), 10, 50), index=df_out.index)
        rsi_high = pd.Series(np.clip(rsi_base_high + (strategy_cfg.get("RSI_VOLATILITY_ADJUST", 5) * volatility_factor), 60, 90), index=df_out.index)
        ...
        for col in bool_cols:
            if col in df_out.columns:
                df_out[col] = df_out[col].fillna(False).astype(bool) # Fill potential NaNs from logic gaps before converting

        # Fill NaNs in strength signals (shouldn't occur if sources were filled, but safeguard)
        if "long_signal_strength" in df_out: df_out["long_signal_strength"] = df_out["long_signal_strength"].fillna(0.0)
        if "short_signal_strength" in df_out: df_out["short_signal_strength"] = df_out["short_signal_strength"].fillna(0.0)
        ...
        if balance_info and balance_info.get("retCode") == 0:
            ...
            for coin_data in coin_list:
                if isinstance(coin_data, dict) and coin_data.get("coin") == coin:
                    ...
                    source = "availableToWithdraw"
                    if balance_str is None or balance_str == '':
                        balance_str = coin_data.get("availableBalance") # Unified/Contract use this
                        source = "availableBalance"
                    ...
                    if balance_str is None or balance_str == '':
                        balance_str = coin_data.get("walletBalance") # General total balance
                        source = "walletBalance"
                        log_console(logging.DEBUG, f"'availableToWithdraw'/'availableBalance' empty for {coin}, using '{source}': {balance_str}")

                    # Handle if all relevant balance fields are empty/missing
                    if balance_str is None or balance_str == '':
                        log_console(logging.WARNING, f"Could not find a valid available balance field for {coin}. Balance is 0.")
                        balance_str = "0"
...
def calculate_position_size_atr(
    ...
    if balance <= FLOAT_COMPARISON_TOLERANCE or risk_percent <= FLOAT_COMPARISON_TOLERANCE or entry_price <= FLOAT_COMPARISON_TOLERANCE or min_order_qty <= FLOAT_COMPARISON_TOLERANCE or qty_step_float <= FLOAT_COMPARISON_TOLERANCE:
        # Use DEBUG as this might happen normally with zero balance or zero risk setting
        log_console(logging.DEBUG, f"Position Size Calc: Non-positive input values. Balance={balance:.2f}, Risk%={risk_percent}, Entry={entry_price}, MinQty={min_order_qty}, QtyStep={qty_step_float}. Returning size 0.")
        return 0.0
    ...
    if max_position_usdt is not None and max_position_usdt > FLOAT_COMPARISON_TOLERANCE:
        try:
            if entry_price <= FLOAT_COMPARISON_TOLERANCE:
                log_console(logging.WARNING, "Position Size Calc: Entry price is zero or too small. Cannot apply Max Position USDT constraint.")
            else:
                max_size_by_value = max_position_usdt / entry_price
                if position_size > max_size_by_value:
                    log_console(logging.INFO, f"Position Size Calc: Capping size from {position_size:.{qty_precision}f} to {max_size_by_value:.{qty_precision}f} due to Max Position USDT (${max_position_usdt:.2f})")
    ...
    try:
        if qty_step_float <= FLOAT_COMPARISON_TOLERANCE:
            log_console(logging.ERROR, f"Position Size Calc: Quantity step ({qty_step_float}) is zero or too small. Cannot apply step constraint.")
    ...
    log_console(logging.INFO, f"Position Size Calc: Balance={balance:.2f}, Risk%={risk_percent}, SL Dist={sl_distance_price:.{qty_precision+2}f}, Entry={entry_price:.{qty_precision+2}f} -> Final Size = {final_size:.{qty_precision}f}")
    return final_size
        ...
        with self.lock:
            try:
                ...
                try:
                    ...
                    leverage_int = max(1, int(leverage_val)) # Ensure positive integer leverage >= 1
                except (ValueError, TypeError):
...
class SymbolTrader:
        ...
        try:
            ...
            if info and info.get("retCode") == 0:
                ...
                if result_list:
                    ...
                    if self.min_order_qty <= FLOAT_COMPARISON_TOLERANCE or self.qty_step_float <= FLOAT_COMPARISON_TOLERANCE or tick_size_float <= FLOAT_COMPARISON_TOLERANCE:
                        log_console(logging.ERROR, f"Fetched invalid instrument details: MinQty={self.min_order_qty}, QtyStep={self.qty_step_float}, TickSize={tick_size_float}", symbol=self.symbol)
                        return False
    ...
    def _initial_setup(self):
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                # V5 Error Code 110043: leverage not modified
                # V5 Error Code 110026: Margin mode related error (can sometimes occur if leverage matches cross margin mode default)
                if error_code == 110043 or "leverage not modified" in error_msg.lower() or "same leverage" in error_msg.lower() or error_code == 110026:
                    ...
                    log_console(logging.WARNING, f"Failed to set leverage: {error_msg} (Code: {error_code}). Check API permissions and account settings. Continuing...", symbol=self.symbol)
    ...
    def _websocket_callback(self, message: Dict[str, Any]):
        ...
        try:
                ...
                if not process_confirmed_only or (process_confirmed_only and is_confirmed):
                    try:
                        ...
                        kline_processed = {
                            ...
                            "close": float(kline_raw["close"]),
                            "volume": float(kline_raw["volume"]),
                            "turnover": float(kline_raw.get("turnover", 0.0)), # Turnover might not always be present
                        }
            ...
            log_console(logging.ERROR, f"Unexpected error in WebSocket callback for {self.symbol}: {e}", symbol=self.symbol, exc_info=True)
    ...
    def _process_kline_queue(self) -> bool:
        ...
        while not self.kline_queue.empty() and processed_count < max_items_per_cycle:
            try:
                ...
                new_row = pd.DataFrame([kline_dict]).set_index("timestamp")
                if not new_row.index.tz: new_row.index = new_row.index.tz_localize('UTC')

                if self.kline_df is None or self.kline_df.empty:
                    log_console(logging.DEBUG, "kline_df empty, initializing from first WS kline.", symbol=self.symbol)
                    self.kline_df = new_row
                    df_updated = True
                else:
                    ...
                    if not isinstance(self.kline_df.index, pd.DatetimeIndex):
                         ...
                         continue # Skip to next item or recalc
                    if not self.kline_df.index.tz:
                         log_console(logging.WARNING, "Internal kline_df index missing timezone. Localizing to UTC.", symbol=self.symbol)
                         self.kline_df.index = self.kline_df.index.tz_localize('UTC', ambiguous='infer')
        ...
        if df_updated and self.kline_df is not None and not self.kline_df.empty:
            ...
            try:
                ...
                if calculated_df is not None and not calculated_df.empty:
                    ...
                    if not calculated_df.index.tz: calculated_df.index = calculated_df.index.tz_localize('UTC')
                    self.kline_df = calculated_df
                    self.last_kline_update_time = dt.datetime.now(dt.timezone.utc)
                    recalculated_ok = True
        ...
        elif df_updated: # df_updated is True but kline_df became None or empty (should not happen ideally)
            log_console(logging.ERROR, "kline_df became empty/None after WS processing. Cannot recalculate.", symbol=self.symbol)
            recalculated_ok = False
    ...
    def get_ohlcv_rest(self, timeframe: Optional[str] = None, limit: Optional[int] = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            log_console(logging.ERROR, f"Exception during REST KLine fetch for {self.symbol}: {e}", symbol=self.symbol, exc_info=True)
            return None
    ...
    def get_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        if self.kline_df is None or self.kline_df.empty:
            ...
            if self.last_kline_update_time is None or \
               (now_utc - self.last_kline_update_time).total_seconds() > staleness_limit:
                needs_rest_fetch = True
                log_console(logging.WARNING, f"Kline data appears stale (> {staleness_limit}s ago) or WS recalc failed. Attempting REST.", symbol=self.symbol)
                        ...
                        try:
                            ...
                            if pos_side in ["Buy", "Sell"] and abs(pos_size) >= min_significant_qty:
                                ...
                                if self.current_trade and \
                                   self.current_trade.get("symbol") == self.symbol and \
                                   self.current_trade.get("side") == pos_side and \
                                   math.isclose(abs(self.current_trade.get("size", 0)), abs(pos_size), rel_tol=0.01):
                                ...
                                # Fallback to API createdTime (convert ms string to datetime)
                                    ...
                                    except (ValueError, TypeError, KeyError): pass # Ignore parsing errors
    ...
    def confirm_order(self, order_id: str, expected_qty: float) -> Tuple[float, Optional[float]]:
        ...
        if self.dry_run:
            ...
            if sim_entry_price is None or sim_entry_price <= FLOAT_COMPARISON_TOLERANCE:
                ...
                log_console(logging.WARNING, f"[DRY RUN] Cannot estimate entry price for {order_id}, using dummy {sim_entry_price}.", symbol=self.symbol)
            log_console(logging.DEBUG, f"[DRY RUN] Using simulated avg fill price: {sim_entry_price} for {order_id}", symbol=self.symbol)
            ...
            try:
                ...
                if response and response.get("retCode") == 0:
                    ...
                    if order_list:
                        ...
                        log_console(logging.DEBUG, f"Order {order_id} not in history yet (Attempt {attempt + 1}). Might still be 'New' or processing.", symbol=self.symbol)
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    ...
                    if filled_qty >= required_fill - FLOAT_COMPARISON_TOLERANCE:
                        ...
                        log_console(logging.ERROR, f"Order {order_id} filled qty ({filled_qty:.{self.qty_precision}f}) < required ({required_fill:.{self.qty_precision}f}). Fill failed/incomplete.", symbol=self.symbol)
                ...
                elif error_code in [110013, 110014]: log_console(logging.ERROR,"ORDER REJECTED: Quantity issue (min/max/step). Check instrument info.", symbol=self.symbol)
                elif error_code == 110017: log_console(logging.ERROR,"ORDER REJECTED: Risk control triggered by exchange (e.g., price deviation).", symbol=self.symbol)
                ...
                elif error_code == 110035: log_console(logging.ERROR, f"ORDER REJECTED: SL/TP requires tpslMode parameter for market orders.", symbol=self.symbol)
                elif "reduce-only" in error_msg.lower(): log_console(logging.ERROR, f"ORDER REJECTED: Reduce-only conflict? Check position.", symbol=self.symbol)
                return None
        except Exception as e:
            log_console(logging.ERROR, f"Exception during order placement for {self.symbol}: {e}", symbol=self.symbol, exc_info=True)
            return None
    ...
    def close_position(self, position_data: Dict[str, Any], exit_reason: str = "Signal", exit_price_est: Optional[float] = None) -> bool:
        ...
        if symbol_from_pos != self.symbol:
             log_console(logging.ERROR, f"Position data symbol mismatch ({symbol_from_pos} != {self.symbol}). Cannot close.", symbol=self.symbol)
             return False
        ...
        if abs_size < min_significant_qty:
            log_console(logging.INFO, f"Position size {abs_size:.{self.qty_precision}f} negligible. Assuming already closed.", symbol=self.symbol)
            if self.current_trade and self.current_trade.get("symbol") == self.symbol: self.current_trade = None
            self.last_exit_time = dt.datetime.now(dt.timezone.utc)
            return True
        ...
        if self.dry_run:
            ...
            if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                ...
                if sim_exit_price is None or sim_exit_price <= FLOAT_COMPARISON_TOLERANCE:
                    sim_entry_price_fallback = self.current_trade.get("avgPrice", 1.0)
                    sim_exit_price = sim_entry_price_fallback if sim_entry_price_fallback > FLOAT_COMPARISON_TOLERANCE else 1.0
                    log_console(logging.WARNING, f"[DRY RUN] Cannot estimate exit price for {self.symbol}, using entry/fallback ({sim_exit_price}). P&L inaccurate.", symbol=self.symbol)

                log_console(logging.INFO, f"[DRY RUN] Simulating close for {self.symbol} at {sim_exit_price:.{self.price_precision}f}", symbol=self.symbol)
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    ...
                    if filled_qty >= required_fill - FLOAT_COMPARISON_TOLERANCE:
                        ...
                        if final_exit_price_log is None or final_exit_price_log <= FLOAT_COMPARISON_TOLERANCE:
                            log_console(logging.WARNING, f"Cannot determine valid exit price for metrics for {self.symbol}. Using entry {entry_price}.", symbol=self.symbol)
                            final_exit_price_log = entry_price
                        ...
                        if not isinstance(entry_time_log, dt.datetime):
                            ...
                            if not isinstance(entry_time_log, dt.datetime):
                                log_console(logging.WARNING, f"Entry time missing for metrics for {self.symbol}. Using fallback time.", symbol=self.symbol)
                                entry_time_log = exit_time - dt.timedelta(minutes=15) # Arbitrary fallback
                        ...
                        log_console(logging.ERROR, f"Close order {order_id} filled qty ({filled_qty:.{self.qty_precision}f}) < expected ({required_fill:.{self.qty_precision}f}). Manual check required!", symbol=self.symbol)
                        # Do not clear internal state here, as position might still be partially open
                ...
                already_closed_msg_fragments = ["position size is zero", "order qty exceeded position size", "reduceonly", "reduce-only", "position idx not match"]
                already_closed = error_code in pos_already_closed_codes or \
                                 any(frag in error_msg.lower() for frag in already_closed_msg_fragments)

                if already_closed:
                    log_console(logging.WARNING, f"Close attempt failed for {self.symbol}, API indicates already closed or size issue (Code: {error_code}, Msg: '{error_msg}'). Assuming closed.", symbol=self.symbol)
                    ...
                    if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                        ...
                        internal_side = self.current_trade.get("side", side)
                        internal_leverage = self.current_trade.get("leverage", leverage)
                        log_console(logging.INFO, f"Logging trade metrics from internal state for {self.symbol} (API showed already closed/size issue).", symbol=self.symbol)
                    ...
                    log_console(logging.ERROR, f"Position close order failed for {self.symbol}: {error_msg} (Code: {error_code})", symbol=self.symbol)
                    return False
        except Exception as e:
            log_console(logging.ERROR, f"Exception during position close for {self.symbol}: {e}", symbol=self.symbol, exc_info=True)
            return False
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        if df_higher is None or df_higher.empty or len(df_higher) < MIN_KLINE_RECORDS_FOR_CALC + 1: # Need at least 2 rows for iloc[-2]
        ...
        try:
            ...
            log_console(logging.WARNING, f"Index error accessing HTF ({higher_tf}) data (need >= {MIN_KLINE_RECORDS_FOR_CALC + 1} rows). Allowing trade.", symbol=self.symbol)
            self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
            return True
        except Exception as e:
            log_console(logging.ERROR, f"Error during HTF ({higher_tf}) trend analysis: {e}. Allowing trade.", symbol=self.symbol, exc_info=True)
            self.higher_tf_cache = True; self.higher_tf_cache_time = now_utc
            return True
...
class MomentumScannerTrader:
    ...
    def __init__(self, api_key: str, api_secret: str, config: Dict[str, Any], dry_run: bool = False):
        ...
        if self.account_type == "SPOT": self.category = "spot"
        elif self.account_type in ["UNIFIED", "CONTRACT"]:
            # Default to linear, but could be overridden per symbol if needed
            self.category = "linear"
            log_console(logging.INFO, f"Using category '{self.category}' for {self.account_type} account. Ensure symbols match (e.g., USDT pairs).")
        else:
             log_console(logging.WARNING, f"Account type '{self.account_type}' might not map directly to V5 category. Defaulting to 'linear'.")
             self.category = "linear"
        ...
        log_console(logging.INFO, f"Initializing Trader (Testnet: {self.use_testnet}, Account: {self.account_type}, Category: {self.category})...")
        try:
            ...
            if self.traders and any(trader.use_websocket for trader in self.traders.values()):
                ...
                log_console(logging.INFO, "WebSocket disabled or no traders configured for WS. Using REST API only.")
    ...
    def _test_api_connection(self):
        ...
        if not self.session: raise ConnectionError("HTTP session not initialized.")
        try:
            ...
            if server_time_resp and server_time_resp.get('retCode') == 0:
                ...
                if time_diff > max_allowable_skew:
                    log_console(logging.WARNING, f"System clock differs from server by {time_diff:.1f}s (>{max_allowable_skew}s). Check time synchronization (e.g., NTP).")
                log_console(logging.INFO, f"API connection successful. Server Time: {server_dt.isoformat()}")
            else:
                error_msg = server_time_resp.get('retMsg', 'N/A') if server_time_resp else "Empty Response"
                error_code = server_time_resp.get('retCode', 'N/A')
                # Check for auth errors (e.g., 10003, 10004)
                if error_code in [10003, 10004]:
                     log_console(logging.CRITICAL, f"API Authentication Failed: {error_msg} (Code: {error_code}). Check API Key/Secret/Permissions.")
                raise ConnectionError(f"API test failed: {error_msg} (Code: {error_code})")
        except ConnectionError as ce:
            log_console(logging.CRITICAL, f"API connection failed: {ce}. Check keys, permissions, network, and time sync.")
            raise ce
        except Exception as e:
    ...
    def _initialize_traders(self):
        ...
        for i, symbol_cfg in enumerate(deepcopy(symbols_cfg_list)):
            if not isinstance(symbol_cfg, dict):
                log_console(logging.ERROR, f"Symbol config #{i} invalid (not a dictionary). Skipping.")
                continue
            symbol = symbol_cfg.get("SYMBOL")
            if not symbol or not isinstance(symbol, str):
                log_console(logging.ERROR, f"Symbol config #{i} missing/invalid 'SYMBOL' key. Skipping.")
                continue
            if symbol in temp_traders:
                log_console(logging.WARNING, f"Duplicate symbol '{symbol}' in config. Skipping subsequent entry.", symbol=symbol)
                continue

            log_console(logging.INFO, f"--- Initializing trader for {symbol} ---", symbol=symbol)
            try:
                if not self.session: raise RuntimeError("HTTP session not available for trader initialization.")
                # Pass the shared HTTP session and determined category
                trader = SymbolTrader(
                    self.api_key, self.api_secret, self.config, symbol_cfg,
                    self.dry_run, self.metrics, self.session, self.category
                )
                ...
                if trader.initialization_successful:
                    ...
                    successful_traders += 1
                    log_console(logging.INFO, f"--- Trader {symbol} initialized successfully ---", symbol=symbol)
                else:
                    # Should have been raised as RuntimeError from SymbolTrader, but catch here just in case
                    log_console(logging.ERROR, f"Trader initialization reported failure for {symbol}. Skipping.", symbol=symbol)

            except RuntimeError as trader_init_e:
                # Catch specific RuntimeError raised by SymbolTrader on failure
                log_console(logging.ERROR, f"Initialization failed for {symbol}: {trader_init_e}. Skipping.", symbol=symbol)
            except Exception as e:
                log_console(logging.ERROR, f"Unexpected error initializing {symbol}: {e}. Skipping.", symbol=symbol, exc_info=True)
        ...
        log_console(logging.INFO, f"Initialized {successful_traders}/{len(symbols_cfg_list)} traders: {', '.join(self.traders.keys())}")
    ...
    def _start_and_monitor_websocket(self):
        ...
        ws_channel_type = self.category.lower()
        if ws_channel_type not in ["linear", "inverse", "spot", "option"]:
             log_console(logging.ERROR, f"Unsupported category '{self.category}' for WS channel_type. Defaulting to 'linear'.")
        ...
        while not self.shutdown_requested:
            ...
            if not ws_conn_active:
                log_console(logging.INFO, f"Attempting WS connection (Channel: {ws_channel_type})...")
                try:
                    # Create a new WebSocket instance for each connection attempt
                    temp_ws = WebSocket(
                        testnet=self.use_testnet, channel_type=ws_channel_type,
                        api_key=self.api_key, api_secret=self.api_secret,
                    ...
                    for symbol, trader in self.traders.items():
                        if trader.use_websocket:
                            ...
                            log_console(logging.DEBUG, f"WS prepared subscription: {topic}", symbol=symbol)
                    ...
                    if subscriptions:
                        ...
                        log_console(logging.INFO, f"WebSocket subscribing to {len(subscriptions)} streams...")
                        self.ws.subscribe(subscriptions, callback=self._dispatch_ws_callback) # Use dispatcher
                        ...
                        while wait_time < max_wait:
                            with self.ws_lock: ws_established = self.ws_active
                            if ws_established or self.shutdown_requested: break
                            time.sleep(1); wait_time += 1

                        if self.shutdown_requested: break # Exit loop if shutdown requested

                        with self.ws_lock: ws_established = self.ws_active
                        if ws_established:
                            log_console(logging.INFO,"WebSocket connection established. Monitor active.")
                            ...
                            while not self.shutdown_requested:
                                with self.ws_lock: ws_still_active = self.ws_active
                                if not ws_still_active:
                                    log_console(logging.INFO, "WS inactive detected by monitor, triggering reconnect.")
                                    break # Break inner loop to reconnect
                                time.sleep(self.bot_cfg.get("WS_MONITOR_CHECK_INTERVAL_SECONDS", 5)) # Check interval
                        else:
                             log_console(logging.ERROR, f"WS connection failed to establish within {max_wait}s. Retrying...")
                             self._close_websocket_connection() # Ensure cleanup before retry
                             if not self.shutdown_requested: time.sleep(retry_delay)
                             retry_delay = min(max_retry_delay, retry_delay * 1.5) # Exponential backoff (less aggressive)
                    ...
                    log_console(logging.ERROR, f"Failed to init/start WS connection: {e}. Retrying in {retry_delay:.1f}s.", exc_info=False)
                    self._close_websocket_connection() # Ensure cleanup
                    if not self.shutdown_requested: time.sleep(retry_delay)
                    retry_delay = min(max_retry_delay, retry_delay * 1.5) # Exponential backoff

            else: # WS is active, sleep and check later
                time.sleep(self.bot_cfg.get("WS_MONITOR_CHECK_INTERVAL_SECONDS", 10)) # Check less frequently when stable
    ...
    def _dispatch_ws_callback(self, message: Dict[str, Any]):
        ...
        try:
            ...
            log_console(logging.ERROR, f"Error dispatching WS message: {e}. Message: {message}", exc_info=True)
    ...
    def _handle_ws_open(self, ws_app):
        ...
        log_console(logging.INFO,"WebSocket connection opened successfully.")
        with self.ws_lock: self.ws_active = True
    ...
    def _fetch_initial_data_worker(self):
        ...
        traders_to_fetch = list(self.traders.items())
        for symbol, trader in traders_to_fetch:
            if self.shutdown_requested: break # Check for shutdown signal
            # Fetch only if WS is enabled AND internal DF is still empty/None
            if trader.use_websocket and (trader.kline_df is None or trader.kline_df.empty):
                log_console(logging.DEBUG, f"Fetching initial REST data for {symbol}...", symbol=symbol)
                try:
                    ...
                    if initial_df is not None and not initial_df.empty:
                        ...
                        log_console(logging.INFO, f"Initialized kline data for {symbol} from REST.", symbol=symbol)
                    else:
                        log_console(logging.WARNING, f"Failed initial REST fetch for {symbol}. Will rely solely on WS updates.", symbol=symbol)
                except Exception as e:
                    log_console(logging.ERROR, f"Error fetching initial REST data for {symbol}: {e}", symbol=symbol, exc_info=False)
    ...
    def _handle_ws_message(self, message):
        ...
        try:
            ...
            if isinstance(message, dict):
                ...
                if op == "auth":
                    success = message.get("success", False)
                    log_console(logging.INFO, f"WebSocket Auth: {'Success' if success else 'Failed'}")
                    if not success: log_console(logging.ERROR, f"WS Auth Fail: {message.get('ret_msg', 'N/A')}")
                # Handle subscription response
                elif op == "subscribe":
                    success = message.get("success", False)
                    ...
                    if not topic_info and "req_id" in message: topic_info = f"ReqID: {message['req_id']}"

                    if success:
                        log_console(logging.DEBUG, f"WS Subscribe Success: {topic_info}")
                    else:
                        log_console(logging.ERROR, f"WS Subscribe Fail: {topic_info} - {message.get('ret_msg', 'N/A')}")
    ...
    def _handle_ws_error(self, ws_app_or_error, error=None):
        ...
        with self.ws_lock: self.ws_active = False # Signal monitor to reconnect
    ...
    def run(self):
        ...
        log_console(logging.INFO, f"--- Starting Trading Cycles ({len(self.traders)} Symbols) ---")
        sleep_interval: float = float(self.bot_cfg.get("SLEEP_INTERVAL_SECONDS", 60.0))
        metrics_interval: int = int(self.bot_cfg.get("METRICS_LOG_INTERVAL_SECONDS", 3600))
        last_balance_check_time: float = 0.0
        balance_check_interval: int = self.bot_cfg.get("BALANCE_CHECK_INTERVAL_SECONDS", 300)
        ...
        if self.dry_run:
            ...
            if balance > FLOAT_COMPARISON_TOLERANCE:
                ...
                log_console(logging.WARNING, f"Initial {self.account_type} USDT balance is zero or fetch failed. Trades cannot be sized.")
        ...
        while self.running and not self.shutdown_requested:
            try:
                ...
                ws_currently_active = False
                with self.ws_lock: ws_currently_active = self.ws_active
                if self.ws_connection_thread and not ws_currently_active and any(t.use_websocket for t in self.traders.values()):
                    log_console(logging.WARNING, "WebSocket connection inactive. Strategy relies on REST API polling.")
                ...
                if not self.dry_run and (current_time - last_balance_check_time > balance_check_interval):
                    ...
                    if abs(fetched_balance - balance) > 0.01: # Log if changed significantly
                        log_console(logging.INFO, f"Checked Balance ({self.account_type}): {fetched_balance:.2f} USDT Available")
                    balance = fetched_balance
                    last_balance_check_time = current_time
                    if balance <= FLOAT_COMPARISON_TOLERANCE:
                        log_console(logging.WARNING, f"{self.account_type} USDT balance is zero. No new trades can be sized.")
                ...
                if isinstance(max_pos_usdt_config, (int, float)) and max_pos_usdt_config > FLOAT_COMPARISON_TOLERANCE and num_active_traders > 0:
                    max_pos_usdt_per_trader = max_pos_usdt_config / num_active_traders
                    log_console(logging.DEBUG, f"Allocated Bal/Trader: {balance_per_trader:.2f}, Max Pos Value/Trader: {max_pos_usdt_per_trader:.2f}")
                elif balance_per_trader > FLOAT_COMPARISON_TOLERANCE:
                    log_console(logging.DEBUG, f"Allocated Bal/Trader: {balance_per_trader:.2f}, Max Pos Value/Trader: Unlimited")

                # Iterate Through Symbols (use list keys for safe iteration if traders could be removed)
                for symbol in list(self.traders.keys()):
                    if not self.running or self.shutdown_requested: break # Check shutdown flag within loop
                    trader = self.traders[symbol]
                    symbol_color_prefix = f"{get_symbol_color(symbol)}[{symbol}]{Style.RESET_ALL}"
                    print(f"{symbol_color_prefix} --- Processing Symbol ---")
                    ...
                    try:
                        ...
                        if trader.last_exit_time:
                            ...
                            if cooldown_seconds > 0:
                                time_since_exit = (current_dt_utc - trader.last_exit_time).total_seconds()
                                if time_since_exit < cooldown_seconds:
                                    log_console(logging.INFO, f"Cooldown active ({cooldown_seconds - time_since_exit:.0f}s left). Skipping cycle.", symbol=symbol)
                        ...
                        df_with_indicators = trader.get_ohlcv()
                        if df_with_indicators is None or df_with_indicators.empty or len(df_with_indicators) < MIN_KLINE_RECORDS_FOR_CALC + 1: # Need +1 for iloc[-2]
                            log_console(logging.DEBUG, f"Insufficient data ({len(df_with_indicators) if df_with_indicators is not None else 0} rows) or calculation failed. Skipping cycle.", symbol=symbol)
                            continue
                        ...
                        try:
                            ...
                            log_console(logging.WARNING, f"Cannot access candle data (need >= {MIN_KLINE_RECORDS_FOR_CALC + 1} rows, got {len(df_with_indicators)}). Skipping.", symbol=symbol)
                            continue

                        # Validate key data on the closed candle used for signals
                        critical_indicators = ["close", "atr_risk", "long_signal", "short_signal", "exit_long_signal", "exit_short_signal"]
                        if any(pd.isna(last_closed_candle.get(ind)) for ind in critical_indicators):
                            log_console(logging.WARNING, f"Critical indicator NaN on closed candle ({last_closed_candle.name}). Skipping cycle.", symbol=symbol)
                            continue
                        ...
                        if pd.isnull(current_price) or current_price <= FLOAT_COMPARISON_TOLERANCE:
                            ...
                            if pd.isnull(current_price) or current_price <= FLOAT_COMPARISON_TOLERANCE:
                                log_console(logging.ERROR, "Cannot get valid current price. Skipping cycle.", symbol=symbol)
                                continue

                        # Position Management: Check current position status
                        position = trader.get_position(log_error=True) # Log errors on position fetch
                        ...
                        if position:
                            ...
                            min_significant_qty_check = max(FLOAT_COMPARISON_TOLERANCE, trader.min_order_qty * 0.01, trader.qty_step_float * 0.1)
                            if not (pos_side in ["Buy", "Sell"] and isinstance(pos_size, float) and abs(pos_size) >= min_significant_qty_check and isinstance(entry_price, float) and entry_price > FLOAT_COMPARISON_TOLERANCE):
                                log_console(logging.ERROR, f"Invalid position data retrieved: {position}. Clearing internal state.", symbol=symbol)
                                trader.current_trade = None; position = None # Clear state and proceed as if no position
                            ...
                            exit_long_sig = last_closed_candle.get("exit_long_signal", False)
                            exit_short_sig = last_closed_candle.get("exit_short_signal", False)
                            if pos_side == "Buy" and exit_long_sig: exit_reason = "Strategy Exit (Long)"
                            elif pos_side == "Sell" and exit_short_sig: exit_reason = "Strategy Exit (Short)"
                            ...
                            if exit_reason:
                                log_console(logging.INFO, f"Exit condition triggered: {exit_reason}. Attempting to close position.", symbol=symbol)
                                # Attempt to close the position
                                if trader.close_position(position, exit_reason=exit_reason, exit_price_est=current_price):
                                    log_console(logging.INFO, "Position closed successfully based on signal.", symbol=symbol)
                                    ...
                                    log_console(logging.ERROR, f"Failed to close position after exit signal '{exit_reason}'. Manual check required!", symbol=symbol)
                                # Continue to next symbol after close attempt (successful or not)
                                continue
                            else:
                                log_console(logging.DEBUG, "Holding position. No exit signal detected.", symbol=symbol)
                            ...
                            htf_allows_trade = trader.get_higher_tf_trend()
                            if not htf_allows_trade:
                                log_console(logging.INFO, "HTF filter unfavorable. Skipping entry check.", symbol=symbol)
                                continue # Skip entry if HTF filter blocks
                            ...
                            if long_signal and long_strength >= min_strength:
                                entry_side = "Buy"; signal_strength = long_strength
                                ...
                                entry_side = "Sell"; signal_strength = short_strength
                                if atr_val > FLOAT_COMPARISON_TOLERANCE and sl_mult > FLOAT_COMPARISON_TOLERANCE:
                                     stop_loss_price = entry_price_est + (atr_val * sl_mult)
                                if atr_val > FLOAT_COMPARISON_TOLERANCE and tp_mult > FLOAT_COMPARISON_TOLERANCE:
                                     take_profit_price = entry_price_est - (atr_val * tp_mult)
                            ...
                            if entry_side:
                                log_console(logging.DEBUG,f"Entry Signal: Side={entry_side}, Str={signal_strength:.2f}, EstEntry={entry_price_est:.{trader.price_precision}f}", symbol=symbol)

                                # Validate calculated SL/TP before proceeding
                                if stop_loss_price is None or stop_loss_price <= FLOAT_COMPARISON_TOLERANCE:
                                    log_console(logging.WARNING, f"Invalid SL calculated ({stop_loss_price}) from ATR={atr_val}. Skipping entry.", symbol=symbol)
                                    continue
                                # Ensure TP is valid and logical relative to entry (avoid TP inside SL)
                                if take_profit_price is not None:
                                    if take_profit_price <= FLOAT_COMPARISON_TOLERANCE:
                                        log_console(logging.DEBUG, f"Invalid TP calculated ({take_profit_price}). TP not set.", symbol=symbol)
                                        take_profit_price = None
                                    elif entry_side == "Buy" and take_profit_price <= stop_loss_price:
                                        log_console(logging.DEBUG, f"Long TP ({take_profit_price}) <= SL ({stop_loss_price}). TP not set.", symbol=symbol)
                                        take_profit_price = None
                                    elif entry_side == "Sell" and take_profit_price >= stop_loss_price:
                                        log_console(logging.DEBUG, f"Short TP ({take_profit_price}) >= SL ({stop_loss_price}). TP not set.", symbol=symbol)
                                        take_profit_price = None

                                # Calculate Position Size
                                sl_distance_price_abs = abs(entry_price_est - stop_loss_price)
                                if sl_distance_price_abs <= FLOAT_COMPARISON_TOLERANCE:
                                     log_console(logging.WARNING, f"SL distance is zero or negative ({sl_distance_price_abs}). Cannot size trade. Skipping.", symbol=symbol)
                                     continue

                                qty_to_trade = calculate_position_size_atr(
                                    balance=balance_per_trader, risk_percent=risk_percent,
                                    sl_distance_price=sl_distance_price_abs, entry_price=entry_price_est,
                                    min_order_qty=trader.min_order_qty, qty_step_float=trader.qty_step_float,
                                    qty_precision=trader.qty_precision, max_position_usdt=max_pos_usdt_per_trader
                                )
                                ...
                                if qty_to_trade > FLOAT_COMPARISON_TOLERANCE:
                                    ...
                                    log_console(logging.INFO, f"{entry_side.upper()} ENTRY SIGNAL. Str={signal_strength:.2f}. Size={qty_to_trade:.{trader.qty_precision}f}. SL={sl_log_str}, TP={tp_log_str}", symbol=symbol)
                                    # Place the order
                                    order_id = trader.place_order(entry_side, qty_to_trade, stop_loss_price, take_profit_price)
                                    if order_id:
                                        log_console(logging.INFO, f"{entry_side.upper()} order placed successfully (ID: {order_id}).", symbol=symbol)
                                        ...
                                        log_console(logging.ERROR, f"{entry_side.upper()} order placement failed.", symbol=symbol)
                                    # Continue to next symbol after entry attempt
                                    continue
                                else:
                                    log_console(logging.INFO, f"{entry_side.upper()} signal detected, but calculated size is zero (Balance={balance_per_trader:.2f}, Risk%={risk_percent}, SLDist={sl_distance_price_abs:.{trader.price_precision}f}). Skipping entry.", symbol=symbol)
                            else: # No entry signal met criteria
                                 log_console(logging.DEBUG, "No valid entry signal detected or strength below threshold.", symbol=symbol)

                    except Exception as symbol_e:
                        log_console(logging.ERROR, f"Error processing symbol {symbol}: {symbol_e}", symbol=symbol, exc_info=True)
                        # Continue to the next symbol

                    log_console(logging.DEBUG, f"--- Finished Processing Symbol: {symbol} ---", symbol=symbol)
                # End of symbol loop

            except KeyboardInterrupt:
                log_console(logging.INFO,"KeyboardInterrupt detected in main loop. Initiating shutdown...")
                self.shutdown_requested = True; self.running = False
                break # Exit the main loop
            except Exception as loop_e:
                log_console(logging.CRITICAL, f"CRITICAL ERROR in main trading loop: {loop_e}. Attempting shutdown.", exc_info=True)
                self.shutdown_requested = True; self.running = False
                break # Exit the main loop
            ...
            if self.running and not self.shutdown_requested:
                ...
                sleep_duration = max(0.1, sleep_interval - cycle_duration) # Ensure minimum sleep
                log_console(logging.DEBUG, f"Cycle duration: {cycle_duration:.2f}s. Sleeping for {sleep_duration:.2f}s...")
                print(f"{Fore.BLUE}{Style.BRIGHT}===== CYCLE END =====")
                try:
                    # Sleep, but allow KeyboardInterrupt to break sleep
                    time.sleep(sleep_duration)
                except KeyboardInterrupt:
                    log_console(logging.INFO,"KeyboardInterrupt detected during sleep. Initiating shutdown...")
                    self.shutdown_requested = True; self.running = False
                    break # Exit the main loop
    ...
    def shutdown(self):
        ...
        if self.ws_connection_thread and self.ws_connection_thread.is_alive():
            ...
            if self.ws_connection_thread.is_alive():
                log_console(logging.WARNING,f"WebSocket thread join timed out after {ws_join_timeout}s. Forcing WS closure.")
        ...
        if close_on_exit and not self.dry_run:
            ...
            for symbol, trader in list(self.traders.items()):
                try:
                    ...
                    position = trader.get_position(log_error=False) # Don't flood logs initially
                    if position is None: time.sleep(0.5); position = trader.get_position(log_error=True)

                    if position:
                        log_console(logging.WARNING, f"Closing position for {symbol} due to shutdown...", symbol=symbol)
                        # Get last price estimate for metrics fallback
                        last_price: Optional[float] = None
                        if trader.kline_df is not None and not trader.kline_df.empty:
                            try: last_price = float(trader.kline_df['close'].iloc[-1])
                            except (IndexError, ValueError, TypeError): pass
                        # Attempt close
                        if trader.close_position(position, exit_reason="Shutdown", exit_price_est=last_price):
                            closed_count += 1; time.sleep(0.5) # Small delay between close attempts
                        else:
                            failed_closes.append(symbol)
                            log_console(logging.ERROR, f"Failed to confirm closure for {symbol} during shutdown.", symbol=symbol)
                except Exception as e:
                    log_console(logging.ERROR, f"Error closing {symbol} during shutdown: {e}", symbol=symbol, exc_info=False)
                    failed_closes.append(symbol)

            if closed_count > 0: log_console(logging.WARNING, f"Submitted close orders for {closed_count} positions.")
            if failed_closes: log_console(logging.CRITICAL, f"FAILED TO CLOSE POSITIONS FOR: {', '.join(failed_closes)}. MANUAL ACTION REQUIRED.")
            elif close_on_exit: log_console(logging.INFO, "Position closing attempt complete.")

        elif self.dry_run and close_on_exit:
            log_console(logging.INFO, "Dry run mode: No real positions to close. Logging metrics for simulated trades.")
            # Log metrics for any open simulated trades
            closed_sim_count = 0
            for symbol, trader in list(self.traders.items()):
                 if trader.current_trade:
                      log_console(logging.INFO, f"[DRY RUN] Logging final metrics for simulated position {symbol}", symbol=symbol)
                      # Simulate close at last known price for metrics
                      last_price_sim: Optional[float] = None
                      if trader.kline_df is not None and not trader.kline_df.empty:
                          try: last_price_sim = float(trader.kline_df['close'].iloc[-1])
                          except: pass
                      # Fallback if price unavailable
                      ...
                      trader.metrics.add_trade(
                           symbol=symbol,
                           entry_time=trader.current_trade.get("entry_time", dt.datetime.now(dt.timezone.utc) - dt.timedelta(minutes=1)), # Fallback entry time
                           exit_time=dt.datetime.now(dt.timezone.utc),
                      ...
                      closed_sim_count += 1
            if closed_sim_count > 0: log_console(logging.INFO, f"[DRY RUN] Logged final metrics for {closed_sim_count} simulated positions.")
...
if __name__ == "__main__":
    ...
    logger.setLevel(logging.DEBUG if args.debug else logging.INFO) # Set file logger level too
    ...
    try:
        ...
        for section, expected_type in required_sections.items():
            if section not in config: validation_errors.append(f"Missing required section '{section}'.")
            elif not isinstance(config.get(section), expected_type): validation_errors.append(f"Section '{section}' must be a {expected_type.__name__}.")
        ...
        if not validation_errors: # Proceed with detailed checks only if top sections are okay
            ...
            if not isinstance(bybit_cfg.get("USE_TESTNET", False), bool): validation_errors.append("BYBIT_CONFIG: USE_TESTNET must be true or false.")
            account_type = bybit_cfg.get("ACCOUNT_TYPE", "UNIFIED").upper()
            valid_account_types = ["UNIFIED", "CONTRACT", "SPOT"] # V5 relevant types
            if account_type not in valid_account_types: validation_errors.append(f"BYBIT_CONFIG: ACCOUNT_TYPE '{account_type}' invalid (must be one of {valid_account_types}).")

            # Risk Config Validation
            risk_percent = risk_cfg.get("RISK_PER_TRADE_PERCENT")
            if not isinstance(risk_percent, (int, float)) or not (FLOAT_COMPARISON_TOLERANCE < risk_percent <= 10): validation_errors.append(f"RISK_CONFIG: RISK_PER_TRADE_PERCENT ({risk_percent}) invalid (must be a positive number, e.g., 0.1 to 10).")
            fee_rate = risk_cfg.get("FEE_RATE")
            if not isinstance(fee_rate, (int, float)) or fee_rate < 0: validation_errors.append(f"RISK_CONFIG: FEE_RATE ({fee_rate}) must be a non-negative number.")
            max_pos_usdt = risk_cfg.get("MAX_POSITION_USDT")
            if max_pos_usdt is not None and (not isinstance(max_pos_usdt, (int, float)) or max_pos_usdt <= FLOAT_COMPARISON_TOLERANCE): validation_errors.append(f"RISK_CONFIG: MAX_POSITION_USDT ({max_pos_usdt}) must be a positive number if set.")
            if args.dry_run:
                dry_run_balance = risk_cfg.get("DRY_RUN_DUMMY_BALANCE")
                if dry_run_balance is None or (not isinstance(dry_run_balance, (int, float)) or dry_run_balance <= FLOAT_COMPARISON_TOLERANCE): validation_errors.append(f"RISK_CONFIG: DRY_RUN_DUMMY_BALANCE ({dry_run_balance}) must be a positive number when using --dry-run.")
            if not isinstance(risk_cfg.get("ATR_MULTIPLIER_SL", 1.5), (int, float)) or risk_cfg.get("ATR_MULTIPLIER_SL", 1.5) <= FLOAT_COMPARISON_TOLERANCE: validation_errors.append("RISK_CONFIG: ATR_MULTIPLIER_SL must be a positive number.")
            if not isinstance(risk_cfg.get("ATR_MULTIPLIER_TP", 2.0), (int, float)) or risk_cfg.get("ATR_MULTIPLIER_TP", 2.0) < 0: validation_errors.append("RISK_CONFIG: ATR_MULTIPLIER_TP must be a non-negative number (0 to disable).")
            if not isinstance(risk_cfg.get("ENABLE_TRAILING_STOP", False), bool): validation_errors.append("RISK_CONFIG: ENABLE_TRAILING_STOP must be true or false.")
            ...
            def validate_numeric(cfg: dict, key: str, req_type: type, min_val: Optional[Union[int, float]] = None, max_val: Optional[Union[int, float]] = None, allow_none: bool = False):
                val = cfg.get(key)
                if val is None and allow_none: return
                if val is None and not allow_none: validation_errors.append(f"BOT_CONFIG: '{key}' is missing."); return
                if not isinstance(val, req_type): validation_errors.append(f"BOT_CONFIG: '{key}' ({val}) must be type {req_type.__name__}."); return
                if min_val is not None and val < min_val: validation_errors.append(f"BOT_CONFIG: '{key}' ({val}) must be >= {min_val}.")
                if max_val is not None and val > max_val: validation_errors.append(f"BOT_CONFIG: '{key}' ({val}) must be <= {max_val}.")
            ...
            if not isinstance(bot_cfg.get("PROCESS_CONFIRMED_KLINE_ONLY", True), bool): validation_errors.append("BOT_CONFIG: PROCESS_CONFIRMED_KLINE_ONLY must be true or false.")
            if not isinstance(bot_cfg.get("CLOSE_POSITIONS_ON_SHUTDOWN", False), bool): validation_errors.append("BOT_CONFIG: CLOSE_POSITIONS_ON_SHUTDOWN must be true or false.")
            ...
            if not isinstance(symbols, list): validation_errors.append("BYBIT_CONFIG: SYMBOLS must be a list.")
            elif not symbols: validation_errors.append("BYBIT_CONFIG: SYMBOLS list is empty. Add at least one symbol configuration.")
            else:
                seen_symbols = set()
                ...
                for i, symbol_cfg in enumerate(symbols):
                    ...
                    symbol = symbol_cfg.get("SYMBOL")
                    if not symbol or not isinstance(symbol, str): validation_errors.append(f"SYMBOLS entry {i}: Missing/invalid 'SYMBOL' key (must be a string)."); continue
                    if symbol in seen_symbols: validation_errors.append(f"Duplicate symbol found: {symbol}"); continue
                    seen_symbols.add(symbol)

                    if "STRATEGY_CONFIG" not in symbol_cfg or not isinstance(symbol_cfg.get("STRATEGY_CONFIG"), dict): validation_errors.append(f"Symbol {symbol}: Missing or invalid 'STRATEGY_CONFIG' (must be a dictionary).")
                    ...
                    if account_type != "SPOT":
                         ...
                         if not isinstance(leverage, int) or not (1 <= leverage <= 150): validation_errors.append(f"Symbol {symbol}: LEVERAGE ({leverage}) invalid (must be an integer between 1 and 150 for {account_type}).")
                    timeframe = str(symbol_cfg.get("TIMEFRAME", ""))
                    if timeframe not in valid_timeframes: validation_errors.append(f"Symbol {symbol}: Invalid TIMEFRAME '{timeframe}' (must be one of {valid_timeframes})")
                    if not isinstance(symbol_cfg.get("USE_WEBSOCKET", True), bool): validation_errors.append(f"Symbol {symbol}: USE_WEBSOCKET must be true or false.")
                    ...
                    if isinstance(strategy_cfg, dict):
                         ...
                         for k, v in strategy_cfg.items():
                              if "PERIOD" in k.upper() and (not isinstance(v, int) or v <= 0): validation_errors.append(f"Symbol {symbol}: Strategy param '{k}' ({v}) must be a positive integer.")
                              # Check boolean flags
                              if k.upper() in ["USE_EHLERS_SMOOTHER", "ENABLE_MULTI_TIMEFRAME"] and not isinstance(v, bool): validation_errors.append(f"Symbol {symbol}: Strategy param '{k}' ({v}) must be true or false.")
                              # Check numeric thresholds (allow floats or ints)
                              num_thresholds = ["MOM_THRESHOLD_PERCENT", "ADX_THRESHOLD", "MIN_SIGNAL_STRENGTH", "RSI_LOW_THRESHOLD", "RSI_HIGH_THRESHOLD", "VOLATILITY_FACTOR_MIN", "VOLATILITY_FACTOR_MAX", "RSI_VOLATILITY_ADJUST", "BBANDS_STDDEV"]
                              if k.upper() in num_thresholds and not isinstance(v, (int, float)): validation_errors.append(f"Symbol {symbol}: Strategy param '{k}' ({v}) must be a number.")
                              # Check trigger price types
                              valid_trigger_types = ["LastPrice", "MarkPrice", "IndexPrice"]
                              if k.upper() in ["SL_TRIGGER_BY", "TP_TRIGGER_BY"] and v not in valid_trigger_types: validation_errors.append(f"Symbol {symbol}: Strategy param '{k}' ({v}) must be one of {valid_trigger_types}.")
                              # Check TPSL mode
                              if k.upper() == "TPSL_MODE" and v not in ["Full", "Partial"]: validation_errors.append(f"Symbol {symbol}: Strategy param 'TPSL_MODE' ({v}) must be 'Full' or 'Partial'.")
                              # Check price source string
                              valid_price_sources = ["open", "high", "low", "close", "hlc3", "ohlc4"] # Add others if implemented
                              if k.upper() == "PRICE_SOURCE" and (not isinstance(v, str) or v not in valid_price_sources): validation_errors.append(f"Symbol {symbol}: Strategy param 'PRICE_SOURCE' ({v}) must be a valid string (e.g., {valid_price_sources}).")
                         ...
                         if strategy_cfg.get("ENABLE_MULTI_TIMEFRAME"):
                              ...
                              if htf not in valid_timeframes: validation_errors.append(f"Symbol {symbol}: Invalid HIGHER_TIMEFRAME '{htf}' (must be one of {valid_timeframes}).")
                              elif htf == timeframe: validation_errors.append(f"Symbol {symbol}: HIGHER_TIMEFRAME cannot be the same as TIMEFRAME.")

    except SystemExit: # Raised by load_config on file/JSON errors
        sys.exit(1)
    except Exception as cfg_e:
        log_console(logging.CRITICAL, f"Unexpected error loading or validating config: {cfg_e}", exc_info=True)
        sys.exit(1)

    # Report Validation Results
    if validation_errors:
        log_console(logging.CRITICAL, f"{Back.RED}{Fore.WHITE} Configuration validation failed with {len(validation_errors)} error(s): {Style.RESET_ALL}")
    ...
    try:
        ...
        if trader_instance and trader_instance.running:
            ...
            log_console(logging.CRITICAL, "Trader initialization failed or did not start. Bot cannot run.")
            exit_code = 1
        ...
        if trader_instance:
            ...
            log_console(logging.WARNING, "Trader instance was not fully initialized before interrupt.")
        ...
        if trader_instance: trader_instance.shutdown() # Attempt shutdown
        exit_code = 1
    except Exception as e:
