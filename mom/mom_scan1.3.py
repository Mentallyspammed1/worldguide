...
# Minimum records needed for basic calcs like IT (>= 4) - will be overridden by dynamic calc
...
# Max retries for initial population
MAX_INITIAL_FETCH_RETRIES = 3
INITIAL_FETCH_RETRY_DELAY = 5 # seconds
...
def log_console(level: int, message: Any, symbol: Optional[str] = None, exc_info: bool = False, *args, **kwargs):
    ...
    # Capture traceback if exc_info is requested and an exception is active
    exc_info_data = sys.exc_info() if should_log_exc_info and sys.exc_info()[0] else None
    logger.log(level, file_message, exc_info=exc_info_data)
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for symbol_index, symbol_cfg in enumerate(symbols_cfg):
            ...
            log_console(
                logging.DEBUG, f"Calculated KLINE_LIMIT for {symbol}: {kline_limit} (Max Period: {max_period:.0f}, Buffer: {kline_limit_buffer}, ADX Buf: {adx_warmup_buffer}, Min Needed: {min_records_needed})", symbol=symbol)
...
def validate_config(config: Dict[str, Any]):
    """
    Performs detailed validation of the configuration dictionary structure and values.
    ...
    if not isinstance(bybit_cfg, dict): raise ValueError("'BYBIT_CONFIG' must be a dictionary.")
    if not isinstance(bybit_cfg.get("USE_TESTNET"), bool): raise ValueError("'BYBIT_CONFIG.USE_TESTNET' must be a boolean (true/false).")
    if not isinstance(bybit_cfg.get("ACCOUNT_TYPE"), str) or not bybit_cfg["ACCOUNT_TYPE"]: raise ValueError("'BYBIT_CONFIG.ACCOUNT_TYPE' must be a non-empty string (e.g., UNIFIED, SPOT).")
    # Validate SYMBOLS list (already checked basic structure in load_config)
    symbols_list = bybit_cfg.get("SYMBOLS", [])
    if not symbols_list: raise ValueError("'BYBIT_CONFIG.SYMBOLS' list cannot be empty.")
    seen_symbols = set()
    for i, sym_cfg in enumerate(symbols_list):
        if not isinstance(sym_cfg, dict): raise ValueError(f"Item {i} in 'SYMBOLS' must be a dictionary.")
        symbol = sym_cfg.get("SYMBOL")
        if not isinstance(symbol, str) or not symbol: raise ValueError(f"Item {i} in 'SYMBOLS' must have a non-empty string 'SYMBOL' key.")
        if symbol in seen_symbols: raise ValueError(f"Duplicate symbol '{symbol}' found in 'SYMBOLS' list.")
        seen_symbols.add(symbol)
        # Validate TIMEFRAME (must be string)
        if not isinstance(sym_cfg.get("TIMEFRAME"), str) or not sym_cfg["TIMEFRAME"]: raise ValueError(f"Symbol '{symbol}': 'TIMEFRAME' must be a non-empty string (e.g., '15', '60', 'D').")
        # Validate LEVERAGE (must be positive integer)
        leverage = sym_cfg.get("LEVERAGE")
        if not isinstance(leverage, int) or leverage <= 0: raise ValueError(f"Symbol '{symbol}': 'LEVERAGE' must be a positive integer.")
        # Validate USE_WEBSOCKET (must be boolean)
        if not isinstance(sym_cfg.get("USE_WEBSOCKET"), bool): raise ValueError(f"Symbol '{symbol}': 'USE_WEBSOCKET' must be a boolean (true/false).")
        # Validate STRATEGY_CONFIG (must be dict)
        strategy_cfg = sym_cfg.get("STRATEGY_CONFIG")
        if strategy_cfg is not None and not isinstance(strategy_cfg, dict): raise ValueError(f"Symbol '{symbol}': 'STRATEGY_CONFIG' must be a dictionary if provided.")
        # Add checks for specific required strategy params if necessary (e.g., periods must be positive ints)
        if strategy_cfg:
             numeric_strategy_params = [
                 "ULTRA_FAST_EMA_PERIOD", "FAST_EMA_PERIOD", "MID_EMA_PERIOD", "SLOW_EMA_PERIOD",
                 "ROC_PERIOD", "ATR_PERIOD_RISK", "ATR_PERIOD_VOLATILITY", "RSI_PERIOD",
                 "VOLUME_SMA_PERIOD", "ADX_PERIOD", "BBANDS_PERIOD", "INSTANTANEOUS_TRENDLINE_PERIOD",
                 "RSI_LOW_THRESHOLD", "RSI_HIGH_THRESHOLD", "RSI_VOLATILITY_ADJUST",
                 "MOM_THRESHOLD_PERCENT", "ADX_THRESHOLD", "BBANDS_STDDEV",
                 "VOLATILITY_FACTOR_MIN", "VOLATILITY_FACTOR_MAX"
             ]
             for key in numeric_strategy_params:
                 val = strategy_cfg.get(key)
                 if val is not None and (not isinstance(val, (int, float)) or val <= 0):
                     # Allow thresholds/adjustments to be zero potentially, but periods/multipliers must be positive
                     if key in ["RSI_LOW_THRESHOLD", "RSI_HIGH_THRESHOLD", "RSI_VOLATILITY_ADJUST", "MOM_THRESHOLD_PERCENT", "ADX_THRESHOLD"] and val == 0:
                         continue # Allow zero for these specific threshold/adjust params
                     # Allow min/max volatility factor to be zero or slightly negative if intended (though positive usually makes sense)
                     if key in ["VOLATILITY_FACTOR_MIN", "VOLATILITY_FACTOR_MAX"] and isinstance(val, (int, float)):
                         continue # Allow non-positive for these factors if needed
                     raise ValueError(f"Symbol '{symbol}': STRATEGY_CONFIG parameter '{key}' ('{val}') must be a positive number.")

             boolean_strategy_params = ["USE_EHLERS_SMOOTHER", "ENABLE_MULTI_TIMEFRAME"]
             for key in boolean_strategy_params:
                 val = strategy_cfg.get(key)
                 if val is not None and not isinstance(val, bool):
                     raise ValueError(f"Symbol '{symbol}': STRATEGY_CONFIG parameter '{key}' ('{val}') must be a boolean (true/false).")

             if strategy_cfg.get("ENABLE_MULTI_TIMEFRAME", False):
                 htf = strategy_cfg.get("HIGHER_TIMEFRAME")
                 base_tf = sym_cfg.get("TIMEFRAME")
                 if not isinstance(htf, str) or not htf: raise ValueError(f"Symbol '{symbol}': 'HIGHER_TIMEFRAME' must be a non-empty string when ENABLE_MULTI_TIMEFRAME is true.")
                 if htf == base_tf: raise ValueError(f"Symbol '{symbol}': 'HIGHER_TIMEFRAME' ({htf}) cannot be the same as 'TIMEFRAME' ({base_tf}).")
                 # Optional: Add validation for allowed timeframe strings if known

    # --- BOT_CONFIG Validation ---
    bot_cfg = config.get("BOT_CONFIG")
    if not isinstance(bot_cfg, dict): raise ValueError("'BOT_CONFIG' must be a dictionary.")
    numeric_bot_params = ["LOOP_SLEEP_SECONDS", "ORDER_CONFIRM_DELAY_SECONDS", "WS_RECONNECT_DELAY_SECONDS",
                          "WS_STARTUP_DELAY_SECONDS", "HIGHER_TF_CACHE_SECONDS", "KLINE_STALENESS_SECONDS",
                          "API_RECV_WINDOW", "KLINE_LIMIT_BUFFER", "ORDER_CONFIRM_RETRIES",
                          "SHUTDOWN_WAIT_SECONDS"]
    for key in numeric_bot_params:
        val = bot_cfg.get(key)
        if val is not None and (not isinstance(val, (int, float)) or val < 0):
            raise ValueError(f"BOT_CONFIG parameter '{key}' ('{val}') must be a non-negative number.")
    # Validate specific integer params
    integer_bot_params = ["KLINE_LIMIT_BUFFER", "ORDER_CONFIRM_RETRIES", "SHUTDOWN_WAIT_SECONDS", "API_RECV_WINDOW"]
    for key in integer_bot_params:
        val = bot_cfg.get(key)
        if val is not None and (not isinstance(val, int) or val < 0):
            raise ValueError(f"BOT_CONFIG parameter '{key}' ('{val}') must be a non-negative integer.")
    # Validate boolean params
    boolean_bot_params = ["PROCESS_CONFIRMED_KLINE_ONLY", "CLOSE_POSITIONS_ON_EXIT"]
    for key in boolean_bot_params:
         val = bot_cfg.get(key)
         if val is not None and not isinstance(val, bool):
    ...
    if not isinstance(risk_cfg, dict): raise ValueError("'RISK_CONFIG' must be a dictionary.")
    numeric_risk_params = ["RISK_PER_TRADE_PERCENT", "FEE_RATE", "ATR_MULTIPLIER_SL",
                           "ATR_MULTIPLIER_TP", "DRY_RUN_DUMMY_BALANCE", "MAX_POSITION_USDT"]
    for key in numeric_risk_params:
         val = risk_cfg.get(key)
         if val is not None and (not isinstance(val, (int, float)) or val < 0):
             # Allow MAX_POSITION_USDT to be zero or None explicitly (meaning disabled)
             if key == "MAX_POSITION_USDT" and (val == 0 or val is None): continue
...
def super_smoother(series: pd.Series, period: int) -> pd.Series:
    ...
    try:
        ...
        if len(series_values) > 0 and not np.isnan(
            series_values[0]): ss[0] = series_values[0]
        if len(series_values) > 1 and not np.isnan(
            series_values[1]): ss[1] = series_values[1]
...
def instantaneous_trendline(series: pd.Series, period: int) -> pd.Series:
    ...
    try:
        ...
        if len(series_values) > 0 and not np.isnan(
            series_values[0]): it[0] = series_values[0]
        if len(series_values) > 1 and not np.isnan(
            series_values[1]): it[1] = series_values[1]
    ...
    try:
        ...
        try:
            ...
            if rsi_period > 0:
                ...
                if rsi_series is not None: df_out["rsi"] = rsi_series
        ...
        trendline_shift = pd.Series(np.nan, index=df_out.index) # Initialize trendline_shift
        ...
        if not required_available:
            ...
            # Ensure trendline_shift is available (it was calculated in trend section or initialized)
            if trendline_shift.isnull().all(): # Check if trendline shift calculation failed or was skipped
            ...
            # Only check if trendline valid and shift is valid
            long_cond_itrend = (df_out[trendline_col] > trendline_shift * (1 + FLOAT_COMPARISON_TOLERANCE)) if trendline_ok and not trendline_shift.isnull().all() else False
            ...
            # Only check if trendline valid and shift is valid
            short_cond_itrend = (df_out[trendline_col] < trendline_shift * (1 - FLOAT_COMPARISON_TOLERANCE)) if trendline_ok and not trendline_shift.isnull().all() else False
        ...
        if not exit_required_available:
            ...
            # Only check if trendline valid and shift is valid
            exit_long_cond3 = (df_out[trendline_col] < trendline_shift * (1 - FLOAT_COMPARISON_TOLERANCE)) if trendline_ok and not trendline_shift.isnull().all() else False
            ...
            # Only check if trendline valid and shift is valid
            exit_short_cond3 = (df_out[trendline_col] > trendline_shift * (1 + FLOAT_COMPARISON_TOLERANCE)) if trendline_ok and not trendline_shift.isnull().all() else False
            ...
            for coin_data in coin_balance_list:
                if isinstance(coin_data, dict) and coin_data.get("coin") == coin:
                    ...
                    if balance_str is None or balance_str == '':
                        ...
                        log_console(
                            logging.DEBUG, f"'availableToWithdraw'/'availableBalance' empty for {coin}, using '{source}' as fallback.")

                    # Handle case where all relevant balance fields are empty or missing
                    if balance_str is None or balance_str == '':
                        log_console(
                            logging.WARNING, f"Could not find a valid available balance field (availableToWithdraw, availableBalance, walletBalance) for {coin}. Balance is 0.")
        ...
        try:
            ...
            if entry_price <= FLOAT_COMPARISON_TOLERANCE:
                ...
                if position_size > max_size_by_value:
                    log_console(
                        logging.INFO, f"Position Size Calc: Capping size from {position_size:.{qty_precision}f} to {max_size_by_value:.{qty_precision}f} due to Max Position USDT (${max_position_usdt:.2f})")
...
class SymbolTrader:
    ...
    def __init__(self, api_key: str, api_secret: str, config: Dict[str, Any], symbol_cfg: Dict[str, Any],
        ...
        # Lock for thread-safe access to kline_df (if needed, currently handled sequentially)
        self.kline_df_lock = threading.Lock()
        ...
        try:
            ...
            # 3. Fetch Initial Historical Data (mandatory after instrument info)
            if not self._fetch_initial_ohlcv():
                raise RuntimeError(
                    f"Failed to fetch initial historical data for {self.symbol}. Cannot initialize trader.")
    ...
    def _get_precision_from_string(value_str: str) -> int:
        ...
        if '.' in value_str:
            ...
            if 'e-' in decimal_part.lower():
                try:
                    return int(decimal_part.split('e-')[-1])
                except ValueError:
                    return len(decimal_part) # Fallback if parsing fails
            else:
                # Count characters after the decimal point
    ...
    def _fetch_initial_ohlcv(self) -> bool:
        """Fetches initial historical OHLCV data using REST to populate kline_df."""
        log_console(logging.INFO, f"Fetching initial historical data for {self.symbol}...", symbol=self.symbol)
        # Use the calculated KLINE_LIMIT + buffer needed for indicators
        limit = self.internal_cfg.get("KLINE_LIMIT", 150) + self.bot_cfg.get("KLINE_LIMIT_BUFFER", 50)
        # Ensure limit doesn't exceed API max per request, but fetch enough for calcs
        limit = min(limit, MAX_KLINE_LIMIT_PER_REQUEST)
        limit = max(limit, MIN_KLINE_RECORDS_FOR_CALC + 5) # Ensure at least minimum + buffer

        for attempt in range(MAX_INITIAL_FETCH_RETRIES):
            log_console(logging.DEBUG, f"Initial fetch attempt {attempt + 1}/{MAX_INITIAL_FETCH_RETRIES} for {self.symbol}...", symbol=self.symbol)
            initial_df = self.get_ohlcv_rest(limit=limit) # Fetches and calculates indicators

            if initial_df is not None and not initial_df.empty:
                with self.kline_df_lock: # Lock for thread safety if WS starts concurrently
                    self.kline_df = initial_df
                    self.last_kline_update_time = dt.datetime.now(dt.timezone.utc)
                log_console(logging.INFO, f"Successfully fetched and processed initial {len(initial_df)} klines for {self.symbol}.", symbol=self.symbol)
                return True
            else:
                log_console(logging.WARNING, f"Initial fetch attempt {attempt + 1} failed for {self.symbol}. Retrying in {INITIAL_FETCH_RETRY_DELAY}s...", symbol=self.symbol)
                time.sleep(INITIAL_FETCH_RETRY_DELAY)

        log_console(logging.ERROR, f"Failed to fetch initial OHLCV data for {self.symbol} after {MAX_INITIAL_FETCH_RETRIES} attempts.", symbol=self.symbol)
        return False
                ...
                if not process_confirmed_only or (process_confirmed_only and is_confirmed):
                    try:
                        ...
                        ts_str = kline_processed['timestamp'].strftime(
                            '%Y-%m-%d %H:%M:%S')
                        log_console(
                            logging.DEBUG, f"WS Queued {'Confirmed ' if is_confirmed else ''}Kline @ {ts_str}", symbol=self.symbol)
    ...
    def _process_kline_queue(self) -> bool:
        ...
        while not self.kline_queue.empty() and processed_count < max_items_per_cycle:
            try:
                ...
                # Acquire lock before modifying/accessing shared kline_df
                with self.kline_df_lock:
                    if self.kline_df is None or self.kline_df.empty:
                        # Initialize the DataFrame if it's empty (should have been populated initially)
                        log_console(
                            logging.WARNING, f"Internal kline_df was empty during WS processing for {self.symbol}. Initializing with WS kline.", symbol=self.symbol)
        ...
        if df_updated:
             # Acquire lock before reading/writing shared kline_df
            with self.kline_df_lock:
                if self.kline_df is not None and not self.kline_df.empty:
                    log_console(
                        logging.DEBUG, f"Recalculating indicators for {self.symbol} after processing {processed_count} WS item(s)...", symbol=self.symbol)
                    ...
                    try:
                        ...
                        if calculated_df is not None and not calculated_df.empty:
                            ...
                            log_console(
                                logging.DEBUG, f"Indicators recalculated successfully for {self.symbol} after WS update.", symbol=self.symbol)
                        else:
                         ...
                         recalculated_ok = False
                else: # df_updated is True but kline_df somehow became None or empty
                    log_console(
    ...
    def get_ohlcv_rest(self, timeframe: Optional[str] = None, limit: Optional[int] = None) -> Optional[pd.DataFrame]:
        ...
        default_lim = self.internal_cfg.get(
            "KLINE_LIMIT", 120) + self.bot_cfg.get("KLINE_LIMIT_BUFFER", 50) # Add buffer for fetch
        ...
        # Ensure we fetch at least the minimum required for calculations
        min_len_for_calc = self.internal_cfg.get("KLINE_LIMIT", MIN_KLINE_RECORDS_FOR_CALC)
        lim_to_fetch = max(lim_to_fetch, min_len_for_calc)
            ...
            if response and response.get("retCode") == 0:
                ...
                if kline_list:
                    ...
                    if df_with_indicators is None or df_with_indicators.empty:
                        ...
                        # Note: This specific call might be for HTF, so don't update the main last_kline_update_time here.
                        # The main get_ohlcv will update it when fetching for the base timeframe.
                        # self.last_kline_update_time = dt.datetime.now(dt.timezone.utc) # Removed this update
    ...
    def get_ohlcv(self) -> Optional[pd.DataFrame]:
        ...
        # Acquire lock to check internal kline_df state safely
        with self.kline_df_lock:
            if self.kline_df is None or self.kline_df.empty:
                # Need REST if internal DataFrame is completely missing (should have been populated initially)
                needs_rest_fetch = True
                log_console(
                    logging.WARNING, f"Internal kline_df is empty for {self.symbol}, attempting REST fetch (should have been populated at init).", symbol=self.symbol)
        ...
        if needs_rest_fetch:
            ...
            rest_df_with_indicators = self.get_ohlcv_rest(timeframe=self.timeframe) # Fetch base timeframe
            ...
            if rest_df_with_indicators is not None and not rest_df_with_indicators.empty:
                ...
                if not rest_df_with_indicators.index.tz or rest_df_with_indicators.index.tz != dt.timezone.utc:
                     ...
                     try:
                         ...
                         log_console(logging.ERROR, f"Failed to set UTC on REST fetched DF: {tz_err}. Data might be unusable.", symbol=self.symbol)
                         # Proceed cautiously or consider returning None

                # Acquire lock before writing to shared kline_df
                with self.kline_df_lock:
                    self.kline_df = rest_df_with_indicators
                    self.last_kline_update_time = dt.datetime.now(dt.timezone.utc) # Update time on successful REST fetch
                log_console(
        ...
        # Acquire lock before reading shared kline_df
        with self.kline_df_lock:
    ...
    def get_position(self, log_error: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                    ...
                    if pos_data.get("symbol") == self.symbol:
                        try:
                            ...
                            if pos_side in ["Buy", "Sell"] and abs(pos_size) >= min_significant_qty:
                                ...
                                log_console(
                                    logging.DEBUG, f"Active position found for {self.symbol}: Side={pos_side}, Size={pos_size}", symbol=self.symbol)
                ...
                log_console(
                    logging.DEBUG, f"No active position found for {self.symbol}.", symbol=self.symbol)
                return None
            else:  # API call failed
                if log_error:
                    error_msg = response.get(
                    ...
                    if error_code in [10003, 10004]: log_console(
    ...
    def confirm_order(self, order_id: str, expected_qty: float) -> Tuple[float, Optional[float]]:
        ...
        if self.dry_run:
            ...
            with self.kline_df_lock: # Lock for reading kline_df
                if self.kline_df is not None and not self.kline_df.empty:
                    try:
            ...
            try:
                ...
                if response and response.get("retCode") == 0:
                    ...
                    if order_list and isinstance(order_list[0], dict):
                        ...
                        elif status in ["New", "Untriggered", "Active", "Created", "Triggered", "PendingCancel"]: # Added PendingCancel
                    ...
                    if error_code == 10006: time.sleep(
    ...
    def place_order(self, side: str, qty: float, stop_loss_price: Optional[float] = None,
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if error_code == 110007: log_console(
    ...
    def close_position(self, position_data: Dict[str, Any], exit_reason: str = "Signal", exit_price_est: Optional[float] = None) -> bool:
        ...
        if abs_size < min_significant_qty_close:
            ...
            if self.current_trade and self.current_trade.get(
                "symbol") == self.symbol: self.current_trade = None
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                if order_id:
                    ...
                    log_console(
                        logging.ERROR, f"Close order placement API call succeeded (retCode 0) but no Order ID returned. Response: {response}", symbol=self.symbol)
                    return False
                ...
                if is_already_closed:
                    ...
                    if self.current_trade and self.current_trade.get("symbol") == self.symbol:
                        ...
                        # Absolute fallback
                        if final_exit_price <= FLOAT_COMPARISON_TOLERANCE: final_exit_price = 1.0
    ...
    def get_higher_tf_trend(self) -> bool:
        ...
        if (self.higher_tf_cache is not None and self.higher_tf_cache_time and
            ...
            # Use cached result
            log_console(
                logging.DEBUG, f"Using cached HTF trend result for {self.symbol}: {'Favorable/Allow' if self.higher_tf_cache else 'Unfavorable'}", symbol=self.symbol)
...
class MomentumScannerTrader:
    ...
    def _websocket_message_handler(self, message: Dict[str, Any]):
        """
        Generic WebSocket message handler. Distributes messages to the appropriate
        SymbolTrader based on the topic/symbol.
        """
        # log_console(logging.DEBUG, f"WS Main Handler Received: {message}") # Very verbose
        try:
            # --- Handle Pong for Heartbeat ---
            if isinstance(message, dict) and message.get("op") == "pong":
                # log_console(logging.DEBUG, "WebSocket Pong received.")
                return # Ignore pong messages here
            ...
            if isinstance(message, dict) and message.get("op") == "subscribe":
                ...
                log_console(log_level, f"WS Subscription Response: Success={success}, Msg='{ret_msg}', Args={sub_args}")
                return
            ...
            if topic and topic.startswith("kline."):
                ...
                if len(parts) == 3:
                    symbol = parts[2]
                    interval = parts[1]
                    # Find the trader responsible for this symbol and interval
                    trader = self.traders.get(symbol)
                    if trader and trader.timeframe == interval and trader.use_websocket:
                        # Pass the message to the specific trader's callback
                        trader._websocket_callback(message)
                    else:
                         # Log if message received for an unexpected symbol/interval or WS disabled trader
                         log_console(logging.DEBUG, f"WS received kline for untracked/disabled symbol/interval: {topic}", symbol=symbol)
                else:
                    log_console(logging.WARNING, f"WS received malformed kline topic: {topic}")

            # --- Handle Other Potential Topics (e.g., Order Updates, Position Updates) ---
            # Example: Handle public order book updates if needed
            # elif topic and topic.startswith("orderbook."):
            #     # Process order book data...
            #     pass

            # Example: Handle private order/position updates (requires private channel subscription)
            # elif isinstance(message, dict) and message.get("topic") == "order":
            #     # Process order updates... dispatch to relevant trader?
            #     pass
            # elif isinstance(message, dict) and message.get("topic") == "position":
            #     # Process position updates... dispatch to relevant trader?
            #     pass

            # --- Handle Authentication Responses (Private Channels) ---
            elif isinstance(message, dict) and message.get("op") == "auth":
                 success = message.get("success", False)
                 ret_msg = message.get("ret_msg", "")
                 log_level = logging.INFO if success else logging.ERROR
                 log_console(log_level, f"WS Authentication Response: Success={success}, Msg='{ret_msg}'")

            # --- Handle Other/Unknown Messages ---
            else:
                # Log unexpected message types for debugging, but avoid flooding
                # Check if it's just a connection success message
                if isinstance(message, dict) and "conn_id" in message and message.get("op") == "connect":
                    log_console(logging.DEBUG, f"WS Connection message received: {message}")
                else:
                    log_console(logging.DEBUG, f"Unhandled WS message type: {type(message)} Content: {str(message)[:200]}...") # Log truncated content

        except Exception as e:
            log_console(logging.ERROR, f"Error processing WebSocket message: {e}", exc_info=True)
            log_console(logging.DEBUG, f"Problematic WS message: {message}")
    ...
    def _start_and_monitor_websocket(self):
        ...
        """
        log_console(logging.INFO, "WebSocket monitoring thread starting...")
        # Determine V5 WebSocket channel type based on the trading category
        # Options: public, private. Public for klines, private for orders/positions.
        # We primarily need public klines here. Private might be added later.
        ws_channel_type = "public" # For public klines
        # Construct the correct endpoint URL based on testnet/mainnet and category
        ws_url = f"wss://stream{'