...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    validate_param("interval", default_config["interval"],
                   ...
                   "Invalid interval '{value}' for '{key}'. Resetting to default '{default}'. Valid: " + str(VALID_INTERVALS) + ".")
    ...
    validate_param("bybit_account_type", default_config["bybit_account_type"],
    ...
    current_config["bybit_account_type"] = current_config["bybit_account_type"].upper() # Ensure uppercase

    validate_param("bybit_category", default_config["bybit_category"],
    ...
    current_config["bybit_category"] = current_config["bybit_category"].lower() # Ensure lowercase
    ...
    numeric_params = {
        ...
        "stop_loss_multiple": ('0', 'inf', False, True, False, default_config["stop_loss_multiple"]),
        "take_profit_multiple": ('0', 'inf', False, True, False, default_config["take_profit_multiple"]),
        "trailing_stop_callback_rate": ('0', '1', False, False, False, default_config["trailing_stop_callback_rate"]),
        "trailing_stop_activation_percentage": ('0', '1', True, False, False, default_config["trailing_stop_activation_percentage"]),
        "break_even_trigger_atr_multiple": ('0', 'inf', False, True, False, default_config["break_even_trigger_atr_multiple"]),
        "break_even_offset_ticks": ('0', '1000', True, True, True, default_config["break_even_offset_ticks"]),
        "signal_score_threshold": ('0', 'inf', False, True, False, default_config["signal_score_threshold"]),
        ...
        "fibonacci_period": ('2', '1000', True, True, True, default_config["fibonacci_period"]),
        "orderbook_limit": ('1', '200', True, True, True, default_config["orderbook_limit"]), # Bybit V5 linear up to 200
        ...
        "volume_confirmation_multiplier": ('0', 'inf', False, True, False, default_config["volume_confirmation_multiplier"]),
        "limit_order_offset_buy": ('0', '0.1', True, False, False, default_config["limit_order_offset_buy"]), # 10% max offset
        ...
        "max_concurrent_positions": ('1', '10', True, True, True, default_config["max_concurrent_positions"]),
    }
    for key, (min_str, max_str, allow_min, allow_max, is_int, default_val) in numeric_params.items():
                ...
                if lower_ok and upper_ok:
                    if is_int:
                        if val_dec % 1 == 0:
                            ...
                            is_valid = True
                        else: raise ValueError("Non-integer for integer param")
                    else:
                        current_config[key] = float(val_dec) # Store validated non-integers as float
                        is_valid = True
            except (ValueError, TypeError, InvalidOperation):
                 pass # Handled below

        if not is_valid:
            err_msg = (f"Invalid value for '{{key}}' ({{value}}). Must be {'integer' if is_int else 'number'} "
    ...
    if time_exit_value is None:
        ...
        try:
            ...
            if time_exit_float > 0:
                 ...
                 time_exit_valid = True
            else: raise ValueError("Must be positive")
        except (ValueError, TypeError): pass

    if not time_exit_valid:
         validate_param(time_exit_key, default_config[time_exit_key], lambda v: False,
    ...
    if indicators_key in current_config and isinstance(current_config[indicators_key], dict):
        ...
        for ind_key, ind_val in list(indicators_dict.items()): # Use list copy for safe iteration/deletion
            if ind_key not in default_indicators:
                print(f"{NEON_YELLOW}Warning: Unknown key '{ind_key}' found in '{indicators_key}'. Removing.")
            ...
            if not isinstance(ind_val, bool):
                ...
                print(f"{NEON_RED}Invalid value for '{indicators_key}.{ind_key}' ({repr(ind_val)}). Must be boolean. Resetting to '{default_ind_val}'.{RESET}")
    ...
    if ws_key in current_config and isinstance(current_config[ws_key], dict):
        ...
        for set_name, weights in list(weight_sets.items()): # Use list copy
            if not isinstance(weights, dict):
                 print(f"{NEON_RED}Invalid structure for weight set '{set_name}'. Removing.")
            ...
            for ind_key, weight_val in list(weights.items()): # Use list copy
                if ind_key not in default_indicators_keys:
                    print(f"{NEON_YELLOW}Warning: Weight defined for unknown indicator '{ind_key}' in set '{set_name}'. Removing.{RESET}")
                    del weights[ind_key]
                    continue
                try:
                    weight_dec = Decimal(str(weight_val))
                    if not weight_dec.is_finite() or weight_dec < 0: raise ValueError("Non-negative finite")
                    weights[ind_key] = float(weight_dec) # Store as float
                except (ValueError, TypeError, InvalidOperation):
                     default_weight = default_config[ws_key].get("default", {}).get(ind_key, 0.0) # Fallback to default set, then 0
                     print(f"{NEON_RED}Invalid weight '{repr(weight_val)}' for '{ind_key}' in set '{set_name}'. Resetting to default '{default_weight}'.{RESET}")
...
def initialize_exchange(config: Dict[str, Any], logger: logging.Logger) -> Optional[ccxt.Exchange]:
    ...
    try:
        exchange_options = {
            ...
            'options': {
                'defaultType': config.get('bybit_category', 'linear'), # Use category from config
        ...
        if config.get('use_sandbox', True):
            ...
            try:
                ...
                lg.info(f"Sandbox mode enabled via set_sandbox_mode(True) for {exchange.id}.")
                # Verify URL (CCXT can be inconsistent)
                current_api_url = exchange.urls.get('api', '')
                if 'testnet' not in current_api_url:
                    lg.warning(f"set_sandbox_mode did not change API URL. Current: {current_api_url}")
                    # Attempt manual override
                    test_url = exchange.describe().get('urls', {}).get('test')
                    api_test_url = None
                    if isinstance(test_url, str): api_test_url = test_url
                    ...
                    if api_test_url and isinstance(api_test_url, str):
                        ...
                        lg.warning(f"Could not determine testnet URL reliably. Hardcoding fallback: {fallback_test_url}")
                        exchange.urls['api'] = fallback_test_url
                else:
                    lg.info(f"Confirmed API URL is set to Testnet: {exchange.urls['api']}")

            except AttributeError:
                lg.warning(f"{exchange.id} ccxt version might not support set_sandbox_mode. Manually setting Testnet URL.")
                ...
                lg.error(f"Error enabling sandbox mode: {e_sandbox}. Ensure API keys are for Testnet.", exc_info=True)
        else:
            ...
            if 'testnet' in current_api_url:
                lg.warning(f"Detected testnet URL in live mode ({current_api_url}). Resetting to production.")
                # Find production URL
                prod_url_info = exchange.describe().get('urls', {}).get('api')
                prod_url = None
                if isinstance(prod_url_info, str): prod_url = prod_url_info
                elif isinstance(prod_url_info, dict):
                     prod_url = prod_url_info.get('public') or prod_url_info.get('private') or list(prod_url_info.values())[0]
                ...
                if prod_url and isinstance(prod_url, str):
                    ...
                    if www_url and isinstance(www_url, str):
                         ...
                         lg.info(f"Reset API URL using 'www' URL: {exchange.urls['api']}")
                    else:
                         lg.warning(f"Could not determine production URL. Hardcoding fallback: {fallback_prod_url}")
        ...
        try:
             ...
             lg.info(f"Markets loaded successfully. Found {len(exchange.symbols)} symbols.")
             # Validate target symbol
             target_symbol = config.get("symbol")
             if target_symbol and target_symbol not in exchange.markets:
                  lg.error(f"{NEON_RED}FATAL: Target symbol '{target_symbol}' not found in loaded markets!{RESET}")
                  if '/' in target_symbol and ':' not in target_symbol:
                       base, quote = target_symbol.split('/')
                       suggested = f"{base}/{quote}:{quote}"
                       lg.warning(f"{NEON_YELLOW}Hint: Bybit V5 linear format is like '{suggested}'.{RESET}")
                  if 0 < len(exchange.symbols) < 50: lg.debug(f"Available: {sorted(exchange.symbols)}")
                  ...
                  market_category = 'linear' if market.get('linear') else 'inverse' if market.get('inverse') else 'spot'
                  config_category = config.get('bybit_category')
                  if market.get('spot') and config_category != 'spot':
                      lg.warning(f"Config category '{config_category}' mismatch for SPOT market '{target_symbol}'. API calls may fail.")
                  elif market.get('contract') and market_category != config_category:
                       lg.warning(f"Config category '{config_category}' mismatch for {market_category.upper()} contract '{target_symbol}'. API calls may fail.")


        except Exception as market_err:
             lg.critical(f"{NEON_RED}CRITICAL: Failed to load markets: {market_err}. Exiting.{RESET}", exc_info=True)
             return None
        ...
        lg.info(f"Performing initial connection test (Fetch Balance - Account: {account_type_to_test})...")
        quote_curr = config.get("quote_currency", "USDT")
        balance_decimal = fetch_balance(exchange, quote_curr, lg, config) # Pass config for account type

        if balance_decimal is not None:
             lg.info(f"{NEON_GREEN}Connected & fetched {quote_curr} balance: {balance_decimal:.4f}{RESET}")
             if balance_decimal == 0:
                  lg.warning(f"{NEON_YELLOW}Initial {quote_curr} balance is zero in '{account_type_to_test}' account. Check funds.{RESET}")
        else:
             lg.critical(f"{NEON_RED}CRITICAL: Initial balance fetch failed. Check API permissions, Account Type ({account_type_to_test}), network.{RESET}")
        ...
        lg.critical(f"{NEON_RED}>> Check API Key/Secret, permissions, IP whitelist (.env & exchange).{RESET}")
        ...
        lg.critical(f"{NEON_RED}Unexpected error initializing CCXT: {e}{RESET}", exc_info=True)
...
def safe_api_call(func, logger: logging.Logger, *args, **kwargs):
    ...
    global config
    max_retries = config.get("max_api_retries", DEFAULT_MAX_API_RETRIES) if 'config' in globals() else DEFAULT_MAX_API_RETRIES
    base_retry_delay = config.get("retry_delay", DEFAULT_RETRY_DELAY_SECONDS) if 'config' in globals() else DEFAULT_RETRY_DELAY_SECONDS
    ...
    while attempts <= max_retries:
        ...
        # --- Rate Limit Errors ---
            ...
            if retry_after_header:
                try:
                    ...
                    if header_wait > 1000: header_wait = (header_wait / 1000.0) + 0.5 # Convert ms
                    lg.debug(f"Rate limit Retry-After header: {retry_after_header} -> Parsed wait: {header_wait:.1f}s")
            ...
            lg.warning(f"{NEON_YELLOW}Rate limit exceeded in '{func.__name__}'. Waiting {wait_time:.1f}s "
        ...
        # --- Authentication Errors (Non-Retryable) ---
             ...
             lg.error(f"{NEON_RED}>> Check API Key/Secret, permissions, IP whitelist, Environment (Live/Testnet).{RESET}")
             raise e
        ...
        # --- Exchange Specific Errors (Check if Retryable) ---
            ...
            try:
                 ...
                 if '"retcode":' in err_str:
                      ...
                      if code_str.isdigit(): exch_code = int(code_str)
            ...
            bybit_retry_codes = [10001, 10002, 10006, 10016, 10018, 130021, 130150, 131204, 170131]
            retryable_messages = ["internal server error", "service unavailable", "system busy",
                                  "matching engine busy", "please try again", "nonce is too small",
                                  "order placement optimization"] # Common transient messages

            is_retryable = (exch_code in bybit_retry_codes) or \
                           (http_code in RETRYABLE_HTTP_CODES) or \
                           any(msg in err_str for msg in retryable_messages)
            ...
            if is_retryable:
                 ...
                 lg.warning(f"{NEON_YELLOW}Potentially retryable exchange error in '{func.__name__}': {e} (Code: {exch_code}, HTTP: {http_code}). "
                            f"Waiting {wait_time:.1f}s (Attempt {attempts + 1}/{max_retries + 1})...{RESET}")
            else:
                 # Non-retryable exchange error
                 lg.error(f"{NEON_RED}Non-retryable Exchange Error in '{func.__name__}': {e} (Code: {exch_code}, HTTP: {http_code}){RESET}")
                 raise e
        ...
        if attempts < max_retries: # Only sleep if we are going to retry
            if wait_time <= 0: # Should not happen if an exception was caught, but safety check
                lg.warning(f"Wait time invalid ({wait_time}) for retry attempt {attempts + 1}. Using base delay.")
...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    try:
        ...
        def to_decimal(value, context_str: str = "price") -> Optional[Decimal]:
            ...
            try:
                d = Decimal(str(value))
                if d.is_finite() and d > 0: return d
                lg.debug(f"Invalid {context_str} value (non-finite/non-positive): {value}.")
                return None
        ...
        p_close = to_decimal(ticker.get('close', ticker.get('last')), 'close/last') # Use 'close', fallback to 'last'
            ...
            if p_val is not None:
                ...
                if p_src.startswith("Ask") and p_bid and p_ask:
                     ...
                     if spread_pct > Decimal('2.0'):
                          lg.warning(f"Using 'ask' ({p_ask}) as fallback, spread > 2% ({spread_pct:.2f}%)")
        ...
        if price is not None and price.is_finite() and price > 0:
            ...
            price_str = f"{price:.{max(4, abs(price.normalize().as_tuple().exponent))}f}"
            lg.info(f"Current price ({symbol}): {price_str} (Source: {source})")
            return price
        else:
            lg.error(f"{NEON_RED}Failed to extract valid price for {symbol}. Ticker: {ticker}{RESET}")
            return None

    except Exception as e:
        lg.error(f"{NEON_RED}Error fetching/processing price for {symbol}: {e}{RESET}", exc_info=False)
        return None
...
def fetch_klines_ccxt(
    ...
    if not ccxt_timeframe:
        lg.error(f"Invalid timeframe '{timeframe}'. Valid: {list(VALID_INTERVALS)}. Cannot fetch klines.")
        return empty_df
    ...
    try:
        ...
        if len(df) < initial_len: lg.debug(f"Dropped {initial_len - len(df)} rows with invalid timestamps.")
        if df.empty: lg.warning("DataFrame empty after timestamp processing."); return empty_df
        df.set_index('timestamp', inplace=True)

        # 2. Convert OHLCV columns to Decimal, robustly handling errors/NaNs
        cols_to_convert = ['open', 'high', 'low', 'close', 'volume']
        for col in cols_to_convert:
            if col not in df.columns: lg.warning(f"Column '{col}' missing."); continue
            try:
                # Convert to string first, then Decimal, replace errors/non-finite with NaN
                df[col] = df[col].astype(str).apply(
                    lambda x: Decimal(x) if str(x).strip() and Decimal(x).is_finite() else Decimal('NaN')
                )
                # Ensure prices>0, volume>=0
                is_price = col in ['open', 'high', 'low', 'close']
                df[col] = df[col].apply(
                    lambda d: d if isinstance(d, Decimal) and (d > 0 if is_price else d >= 0) else Decimal('NaN')
                )
            except (InvalidOperation, TypeError, ValueError) as conv_err:
                lg.error(f"Error converting column '{col}' to Decimal: {conv_err}. Trying float fallback.")
                df[col] = pd.to_numeric(df[col], errors='coerce') # Fallback to float
                df[col] = df[col].apply(lambda x: x if np.isfinite(x) else np.nan) # Ensure float NaNs

        # 3. Drop rows with NaN in essential price columns (O, H, L, C)
        initial_len = len(df)
        essential_cols = ['open', 'high', 'low', 'close']
        df.dropna(subset=essential_cols, how='any', inplace=True)
        if len(df) < initial_len: lg.debug(f"Dropped {initial_len - len(df)} rows with NaN price data.")
        ...
        try:
            ...
            lg.warning(f"Could not perform OHLC check (type error: {cmp_err}). Skipping.")
...
def fetch_orderbook_ccxt(exchange: ccxt.Exchange, symbol: str, limit: int, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        ...
        cleaned_book = {'bids': [], 'asks': [], 'timestamp': orderbook.get('timestamp'),
                        'datetime': orderbook.get('datetime'), 'nonce': orderbook.get('nonce')}
        errors = 0
        for side in ['bids', 'asks']:
            for entry in orderbook[side]:
                if isinstance(entry, list) and len(entry) == 2:
                    ...
                    except (InvalidOperation, ValueError, TypeError): errors += 1
                else: errors += 1 # Invalid format

        if errors > 0:
            lg.debug(f"Orderbook ({symbol}): Found {errors} invalid entries (format/value).")
        ...
        elif not cleaned_book['bids']: lg.warning(f"Orderbook ({symbol}) has no valid bids.")
        elif not cleaned_book['asks']: lg.warning(f"Orderbook ({symbol}) has no valid asks.")

        lg.debug(f"Processed orderbook for {symbol} ({len(cleaned_book['bids'])} bids, {len(cleaned_book['asks'])} asks).")
        return cleaned_book

    except Exception as e:
        lg.error(f"{NEON_RED}Error fetching/processing order book for {symbol}: {e}{RESET}", exc_info=False)
        return None
...
class TradingAnalyzer:
    ...
    def __init__(
        ...
        if not self.weights:
            logger.warning(f"{NEON_YELLOW}Weight set '{self.active_weight_set_name}' empty for {self.symbol}. Score=0.{RESET}")
            self.weights = {}
    ...
    def _initialize_analysis(self) -> None:
        ...
        if self.df.empty:
             self.logger.warning(f"Analyzer: Empty DataFrame for {self.symbol}. No calculations.")
             return

        required_cols = ['open', 'high', 'low', 'close', 'volume']
        if not all(col in self.df.columns for col in required_cols):
            self.logger.error(f"Analyzer: DF missing columns {required_cols}. Cannot analyze.")
        ...
        try:
            ...
            self.calculate_fibonacci_levels()
             ...
             self.logger.error(f"Error during initial analysis for {self.symbol}: {init_err}", exc_info=True)
    ...
    def _get_ta_col_name(self, base_name: str, result_df_columns: List[str]) -> Optional[str]:
        ...
        param_keys = [
            ...
            ('volume_ma_period', DEFAULT_VOLUME_MA_PERIOD), ('stoch_rsi_period', DEFAULT_STOCH_RSI_PERIOD),
            ('stoch_rsi_rsi_period', DEFAULT_STOCH_RSI_RSI_PERIOD), ('stoch_rsi_k_period', DEFAULT_STOCH_RSI_K_PERIOD),
            ('stoch_rsi_d_period', DEFAULT_STOCH_RSI_D_PERIOD)
        ]
        p = {key: self.config.get(key, default) for key, default in param_keys} # Config params

        # Potential pandas_ta column names for each internal key
        patterns = {
            "ATR": [f"ATRr_{p['atr_period']}"], "EMA_Short": [f"EMA_{p['ema_short_period']}"],
            "EMA_Long": [f"EMA_{p['ema_long_period']}"], "Momentum": [f"MOM_{p['momentum_period']}"],
            "CCI": [f"CCI_{p['cci_period']}", f"CCI_{p['cci_period']}_0.015"],
            "Williams_R": [f"WILLR_{p['williams_r_period']}"], "MFI": [f"MFI_{p['mfi_period']}"],
            "VWAP": ["VWAP_D"], "PSAR_long": [f"PSARl_{psar_step}_{psar_max}"],
            "PSAR_short": [f"PSARs_{psar_step}_{psar_max}"], "SMA_10": [f"SMA_{p['sma_10_period']}"],
            "StochRSI_K": [f"STOCHRSIk_{p['stoch_rsi_period']}_{p['stoch_rsi_rsi_period']}_{p['stoch_rsi_k_period']}"],
            "StochRSI_D": [f"STOCHRSId_{p['stoch_rsi_period']}_{p['stoch_rsi_rsi_period']}_{p['stoch_rsi_k_period']}_{p['stoch_rsi_d_period']}"],
            "RSI": [f"RSI_{p['rsi_period']}"], "BB_Lower": [f"BBL_{p['bollinger_bands_period']}_{bb_std_str}"],
            "BB_Middle": [f"BBM_{p['bollinger_bands_period']}_{bb_std_str}"], "BB_Upper": [f"BBU_{p['bollinger_bands_period']}_{bb_std_str}"],
            "Volume_MA": [f"VOL_SMA_{p['volume_ma_period']}"] # Custom name
        }

        patterns_to_check = patterns.get(base_name, [])
        if not patterns_to_check: return None

        # Search Strategy: Exact -> Case-Insensitive -> StartsWith -> Substring Fallback
        for pattern in patterns_to_check: # 1. Exact Match
            if pattern in result_df_columns: return pattern

        cols_lower_map = {col.lower(): col for col in result_df_columns}
        for pattern in patterns_to_check: # 2. Case-Insensitive Exact Match
             if pattern.lower() in cols_lower_map: return cols_lower_map[pattern.lower()]
        ...
        for pattern in patterns_to_check: # 3. Starts With Match
            ...
            for col in result_df_columns:
                ...
                if col.startswith(pattern) or col_lower.startswith(pattern_lower):
                     ...
                     if not any(c.isalpha() for c in suffix): # Allow numbers/_/. in suffix
                          return col
        ...
        if len(matches) > 1: # Ambiguous, try to resolve using full patterns
             for pattern in patterns_to_check:
                 if pattern in matches: return pattern
             self.logger.warning(f"Ambiguous substring match for '{base_name}': {matches}. Cannot resolve.")
             return None

        self.logger.debug(f"Could not map indicator '{base_name}' (Patterns: {patterns_to_check})")
        return None

    def _calculate_all_indicators(self):
        """Calculates enabled indicators using pandas_ta."""
        if self.df.empty: return self.logger.warning(f"DataFrame empty, cannot calculate indicators for {self.symbol}.")
        ...
        def add_req(ind_key, cfg_key, default):
            ...
            if is_enabled and weight > 0:
                ...
                except (ValueError, TypeError): pass # Ignore invalid format
        ...
        if indicators_cfg.get("ema_alignment", False) and float(weights.get("ema_alignment", 0.0)) > 0:
        ...
        if len(self.df) < min_required_data:
             self.logger.warning(f"{NEON_YELLOW}Insufficient data ({len(self.df)} points) for {self.symbol} "
        ...
        try:
            ...
            ta_map = {
                 ...
                 "stoch_rsi": {"kind": "stochrsi", "length": lambda: int(self.config.get("stoch_rsi_period", DEFAULT_STOCH_RSI_PERIOD)), "rsi_length": lambda: int(self.config.get("stoch_rsi_rsi_period", DEFAULT_STOCH_RSI_RSI_PERIOD)), "k": lambda: int(self.config.get("stoch_rsi_k_period", DEFAULT_STOCH_RSI_K_PERIOD)), "d": lambda: int(self.config.get("stoch_rsi_d_period", DEFAULT_STOCH_RSI_D_PERIOD))},
                 "bollinger_bands": {"kind": "bbands", "length": lambda: int(self.config.get("bollinger_bands_period", DEFAULT_BBANDS_PERIOD)), "std": lambda: float(self.config.get("bollinger_bands_std_dev", DEFAULT_BBANDS_STDDEV))},
            }
            ...
            if "atr" in ta_map:
                 ...
                 except Exception as e: self.logger.error(f"Error preparing ATR: {e}")

            # Add others based on config/weights
            for key, is_enabled in indicators_cfg.items():
                 if key == "atr": continue
                 weight = float(weights.get(key, 0.0))
                 if not is_enabled or weight == 0.0: continue
                 ...
                 if key == "ema_alignment":
                      for ema_key in ["ema_short", "ema_long"]:
                          if ema_key not in calculated_indicator_keys and ema_key in ta_map:
                               ...
                               except Exception as e: self.logger.error(f"Error preparing {ema_key}: {e}")
                 elif key in ta_map:
                      if key not in calculated_indicator_keys:
                           ...
                           except Exception as e: self.logger.error(f"Error preparing {key}: {e}")
                 # Skip volume_confirmation, orderbook (handled separately)

            # Run the TA Strategy
            if ta_strategy.ta:
                 self.logger.info(f"Running pandas_ta strategy with {len(ta_strategy.ta)} indicators...")
                 try: df_calc.ta.strategy(ta_strategy, append=True)
                 except Exception as ta_err: self.logger.error(f"Error running TA strategy: {ta_err}", exc_info=True)
            else: self.logger.info("No pandas_ta indicators added to strategy.")

            # Calculate Volume MA separately
            vol_key = "volume_confirmation"
            vol_ma_p = int(self.config.get("volume_ma_period", DEFAULT_VOLUME_MA_PERIOD))
            if indicators_cfg.get(vol_key, False) and float(weights.get(vol_key, 0.0)) > 0 and vol_ma_p > 0:
                 try:
                     ...
                     else: self.logger.warning("Cannot calculate Volume MA (Volume column missing/invalid).")
                 except Exception as vol_ma_err: self.logger.error(f"Error calculating Volume MA: {vol_ma_err}")
            ...
            indicator_mapping = { # Internal Name : TA Base Name for _get_ta_col_name
                "ATR": "ATR", "EMA_Short": "EMA_Short", "EMA_Long": "EMA_Long", "Momentum": "Momentum",
                "CCI": "CCI", "Williams_R": "Williams_R", "MFI": "MFI", "SMA_10": "SMA_10", "RSI": "RSI",
                "VWAP": "VWAP", "PSAR_long": "PSAR_long", "PSAR_short": "PSAR_short",
                "StochRSI_K": "StochRSI_K", "StochRSI_D": "StochRSI_D", "BB_Lower": "BB_Lower",
                "BB_Middle": "BB_Middle", "BB_Upper": "BB_Upper", "Volume_MA": "Volume_MA"
            }
            ...
            for internal_name, ta_base_name in indicator_mapping.items():
                 ...
                 if mapped_col: self.ta_column_names[internal_name] = mapped_col
                 ...
                 for key in cols_to_decimalize:
                     ...
                     if col_name and col_name in df_calc.columns:
                          try:
                              ...
                              self.logger.error(f"Failed converting '{col_name}' back to Decimal: {conv_err}.")

            self.logger.debug(f"Indicator calculations complete. Mapped columns: {self.ta_column_names}")

        except Exception as e:
            self.logger.error(f"{NEON_RED}Error during indicator calculation: {e}{RESET}", exc_info=True)
    ...
    def _update_latest_indicator_values(self):
        ...
        if self.df.empty: return self.logger.warning("Cannot update latest values: DataFrame empty.")
        ...
        try:
            ...
            self.indicator_values["Timestamp"] = self.df.index[-1] # Store timestamp (datetime)

        except IndexError: return self.logger.error("Error accessing latest row (empty DF?).")
        except Exception as e: return self.logger.error(f"Error getting latest row: {e}")

        # Process Base OHLCV (expect Decimal)
        for base_col in ['open', 'high', 'low', 'close', 'volume']:
            key_name = base_col.capitalize()
            raw_value = latest.get(base_col)
            value = raw_value if isinstance(raw_value, Decimal) and raw_value.is_finite() else Decimal('NaN')
            if value.is_nan() and pd.notna(raw_value): # Handle case where it might be float/int after fallback
                 try: value = Decimal(str(raw_value))
                 except: value = Decimal('NaN')
            # Ensure prices>0, vol>=0
            if key_name != 'Volume' and isinstance(value, Decimal) and value <= 0: value = Decimal('NaN')
            if key_name == 'Volume' and isinstance(value, Decimal) and value < 0: value = Decimal('NaN')
        ...
        for key, col_name in self.ta_column_names.items():
            is_price_based = key in ["ATR", "BB_Lower", "BB_Middle", "BB_Upper", "PSAR_long", "PSAR_short", "VWAP", "SMA_10", "EMA_Short", "EMA_Long"]
            target_type = Decimal if is_price_based else float
            value = Decimal('NaN') if target_type == Decimal else np.nan # Default NaN type
                ...
                if pd.notna(raw_value):
                    try:
                        if target_type == Decimal:
                            ...
                            value = d_val if d_val.is_finite() and d_val > 0 else Decimal('NaN')
                        else: # Float
                            f_val = float(raw_value)
                            value = f_val if np.isfinite(f_val) else np.nan
                    except (ValueError, TypeError, InvalidOperation): pass # Keep default NaN
        ...
        for k, v in self.indicator_values.items():
            if k == "Timestamp": log_vals[k] = v.strftime('%Y-%m-%d %H:%M:%S %Z'); continue
            formatted_val = "NaN"
            if isinstance(v, Decimal) and v.is_finite():
                 prec = price_prec if k in price_keys else amt_prec if k in amount_keys else 8
            ...
            if formatted_val != "NaN": log_vals[k] = formatted_val

        if log_vals:
             sorted_log_vals = dict(sorted(log_vals.items()))
             self.logger.debug(f"Latest values ({self.symbol}): {json.dumps(sorted_log_vals)}")
        else: self.logger.warning(f"No valid latest indicator values found for {self.symbol}.")
    ...
    def calculate_fibonacci_levels(self, window: Optional[int] = None) -> Dict[str, Decimal]:
        ...
        if self.df.empty or len(self.df) < window:
            self.logger.debug(f"Fibonacci skipped: Not enough data ({len(self.df)}/{window}).")
            return {}
        ...
        try:
            ...
            if high_series.empty or low_series.empty: return {} # No valid data

            high_price = high_series.max()
            low_price = low_series.min()

            if not isinstance(high_price, Decimal) or not high_price.is_finite() or \
               not isinstance(low_price, Decimal) or not low_price.is_finite() or low_price <= 0:
                self.logger.warning(f"Invalid high/low for Fibonacci: H={high_price}, L={low_price}")
                return {}

            diff = high_price - low_price
            levels = {}
            min_tick = self.get_min_tick_size()
            quantizer = min_tick if min_tick.is_finite() and min_tick > 0 else Decimal('1e-' + str(self.get_price_precision()))

            if diff <= 0: # Handle flat or error case
                level_price = high_price.quantize(quantizer, rounding=ROUND_DOWN)
                levels = {f"Fib_{float(lvl) * 100:.1f}%": level_price for lvl in FIB_LEVELS}
            else:
                for level_pct_str in map(str, FIB_LEVELS):
                    level_pct = Decimal(level_pct_str)
                    level_name = f"Fib_{level_pct * 100:.1f}%"
                    level_price = (high_price - (diff * level_pct)).quantize(quantizer, rounding=ROUND_DOWN)
    ...
    def get_price_precision(self) -> int:
        ...
        if self._cached_price_precision is not None: return self._cached_price_precision
        ...
        try:
            ...
            if precision is None:
                ...
                if isinstance(last_close, Decimal) and last_close.is_finite() and last_close > 0:
                    ...
                    if 0 <= p <= 12: precision, source = p, f"Last Close ({last_close})"
        except Exception as e: self.logger.warning(f"Error determining price precision: {e}")

        if precision is None: precision, source = 4, "Default (4)" # Final fallback
        self._cached_price_precision = precision
        self.logger.debug(f"Price precision ({self.symbol}): {precision} (Source: {source})")
        return precision

    def get_min_tick_size(self) -> Decimal:
        """Gets the minimum price increment (tick size) as Decimal, cached."""
        if self._cached_min_tick_size is not None: return self._cached_min_tick_size
        ...
        try:
            ...
            if price_prec_val is not None and not isinstance(price_prec_val, int):
                try:
                    tick = Decimal(str(price_prec_val))
                    if tick.is_finite() and tick > 0: tick_size, source = tick, "precision.price (value)"
                except: pass
            ...
            if tick_size is None:
                ...
                if min_price is not None:
                    try:
                        tick = Decimal(str(min_price))
                        if tick.is_finite() and tick > 0: tick_size, source = tick, "limits.price.min"
                    except: pass
            # Fallback: Calculate from integer precision
            if tick_size is None and isinstance(price_prec_val, int) and price_prec_val >= 0:
        ...
        if not isinstance(tick_size, Decimal) or not tick_size.is_finite() or tick_size <= 0:
             ...
             self.logger.error(f"Failed to get valid tick size! Using fallback: {tick_size}")

        self._cached_min_tick_size = tick_size
        self.logger.debug(f"Min Tick Size ({self.symbol}): {tick_size} (Source: {source})")
        return tick_size

    def get_amount_precision_places(self) -> int:
        """Determines amount precision (decimal places) from market info, cached."""
        if self._cached_amount_precision is not None: return self._cached_amount_precision
        precision, source = None, "Unknown"
        try:
            prec_info = self.market_info.get('precision', {})
            ...
            if precision is None:
                 ...
                 if min_amt is not None:
                     try:
                         ...
                         if step.is_finite() and 0 < step <= 1: # Treat as step if fractional
                            if step < 1 or '.' in str(min_amt):
                                precision = abs(step.normalize().as_tuple().exponent)
                                source = f"limits.amount.min (step: {step})"
                         elif step.is_finite() and step >= 1 and step % 1 == 0: # Integer min amount -> 0 places
                                precision = 0
                                source = f"limits.amount.min (int: {step})"
                     except: pass
        except Exception as e: self.logger.warning(f"Error determining amount precision: {e}")

        if precision is None: precision, source = 8, "Default (8)" # Final fallback
        self._cached_amount_precision = precision
        self.logger.debug(f"Amount precision ({self.symbol}): {precision} (Source: {source})")
        return precision

    def get_min_amount_step(self) -> Decimal:
        """Gets the minimum amount increment (step size) as Decimal, cached."""
        if self._cached_min_amount_step is not None: return self._cached_min_amount_step
        ...
        try:
            ...
            if amt_prec_val is not None and not isinstance(amt_prec_val, int):
                try:
                    step = Decimal(str(amt_prec_val))
                    if step.is_finite() and step > 0: step_size, source = step, "precision.amount (value)"
                except: pass
            ...
            if step_size is None:
                ...
                if min_amt is not None:
                    try:
                        step = Decimal(str(min_amt))
                        if step.is_finite() and step > 0: step_size, source = step, "limits.amount.min"
                    except: pass
            # Fallback: Calculate from integer precision
            if step_size is None and isinstance(amt_prec_val, int) and amt_prec_val >= 0:
        ...
        if not isinstance(step_size, Decimal) or not step_size.is_finite() or step_size <= 0:
             ...
             source = "Emergency Fallback"
             self.logger.error(f"Failed to get valid amount step! Using fallback: {step_size}")

        self._cached_min_amount_step = step_size
        self.logger.debug(f"Min Amount Step ({self.symbol}): {step_size} (Source: {source})")
        return step_size
    ...
    def get_nearest_fibonacci_levels(self, current_price: Decimal, num_levels: int = 5) -> List[Tuple[str, Decimal]]:
        ...
        if not isinstance(current_price, Decimal) or not current_price.is_finite() or current_price <= 0:
            self.logger.warning(f"Invalid current price ({current_price}) for Fib comparison.")
            return []
        ...
        try:
            ...
            self.logger.error(f"{NEON_RED}Error finding nearest Fib levels: {e}{RESET}", exc_info=True)
            return []
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: Optional[Dict]) -> str:
        ...
        if not self.indicator_values: return "HOLD" # No data
        if not isinstance(current_price, Decimal) or not current_price.is_finite() or current_price <= 0: return "HOLD"
        if not self.weights: return "HOLD" # No weights

        available_checks = {m.replace('_check_', '') for m in dir(self) if m.startswith('_check_')}

        for indicator_key, is_enabled in self.config.get("indicators", {}).items():
            if not is_enabled: continue
            if indicator_key not in available_checks: continue # Skip if no check method

            weight_val = self.weights.get(indicator_key)
            if weight_val is None: continue
            try:
                weight = Decimal(str(weight_val))
                if not weight.is_finite() or weight < 0: raise ValueError("Invalid weight")
            ...
            try:
                ...
                if indicator_key == "orderbook":
                    score_float = method(orderbook_data, current_price) if orderbook_data else np.nan
                else:
                    score_float = method()
            except Exception as e:
                self.logger.error(f"Error in check '{check_method_name}': {e}", exc_info=True)

            if pd.notna(score_float) and np.isfinite(score_float):
                try:
                    score_dec = max(Decimal("-1.0"), min(Decimal("1.0"), Decimal(str(score_float)))) # Clamp [-1, 1]
                    final_score += score_dec * weight
                    total_weight += weight
                    active_count += 1
                    contrib_scores[indicator_key] = f"{score_dec:.3f}"
                except (ValueError, TypeError, InvalidOperation): pass # Ignore conversion errors

        final_signal = "HOLD"
        if total_weight > Decimal('1e-9'):
            try:
                threshold = Decimal(str(self.config.get("signal_score_threshold", "1.5")))
                if not threshold.is_finite() or threshold <= 0: raise ValueError("Invalid threshold")
            except (ValueError, TypeError, InvalidOperation):
                threshold = Decimal(str(default_config["signal_score_threshold"]))
                self.logger.warning(f"Invalid signal threshold. Using default {threshold}.")
        ...
        log_msg = (f"Signal ({self.symbol} @ {current_price:.{price_prec}f}): "
        ...
        if self.logger.isEnabledFor(logging.DEBUG) and contrib_scores:
             self.logger.debug(f"  Scores ({self.symbol}): {json.dumps(dict(sorted(contrib_scores.items())))}")
    ...
    def _check_ema_alignment(self) -> float:
        ...
        if not all(isinstance(v, Decimal) and v.is_finite() for v in [ema_s, ema_l, close]): return np.nan
        try:
            short_above_long = ema_s > ema_l
            price_above_short = close > ema_s
            price_above_long = close > ema_l
            if price_above_short and short_above_long: return 1.0   # Strong Bullish
            if not price_above_long and not short_above_long: return -1.0 # Strong Bearish
            if short_above_long: return 0.3 if price_above_short else -0.2 # Weak Bullish EMA / Disagree
            else: return -0.3 if not price_above_long else 0.2 # Weak Bearish EMA / Disagree
    ...
    def _check_momentum(self) -> float:
        ...
        if not isinstance(mom, (float, int)) or not np.isfinite(mom): return np.nan
        # Simple thresholding (tune thresholds as needed)
        if mom >= 0.5: return 1.0
        if mom >= 0.1: return 0.5
        if mom <= -0.5: return -1.0
        if mom <= -0.1: return -0.5
        return 0.0
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        if not isinstance(vol, Decimal) or not vol.is_finite() or vol < 0: return np.nan
        if not isinstance(vol_ma, (float, int)) or not np.isfinite(vol_ma) or vol_ma <= 0: return np.nan
        try:
            vol_ma_d = Decimal(str(vol_ma))
            multiplier = Decimal(str(self.config.get("volume_confirmation_multiplier", 1.5)))
            if multiplier <= 0: multiplier = Decimal('1.5')
            ratio = vol / vol_ma_d if vol_ma_d > Decimal('1e-12') else Decimal('1')
            if ratio >= multiplier * Decimal('1.5'): return 1.0 # Very high vol
            if ratio >= multiplier: return 0.7                 # High vol
            if ratio <= (Decimal('1') / multiplier): return -0.4 # Low vol (weak confirmation)
            return 0.0
        except (InvalidOperation, ZeroDivisionError): return np.nan
    ...
    def _check_stoch_rsi(self) -> float:
        ...
        if not all(isinstance(v, (float, int)) and np.isfinite(v) for v in [k, d]): return np.nan
        try:
            os = float(self.config.get("stoch_rsi_oversold_threshold", 25))
            ob = float(self.config.get("stoch_rsi_overbought_threshold", 75))
            if not (0 < os < 100 and 0 < ob < 100 and os < ob): raise ValueError()
        except: os, ob = 25.0, 75.0
        score = 0.0
        if k < os and d < os: score = 1.0 # Strong Oversold -> Buy
        elif k > ob and d > ob: score = -1.0 # Strong Overbought -> Sell
        if k > d + 1.0 and d < ob: score = max(score, 0.7) # Bullish cross
        elif d > k + 1.0 and k > os: score = min(score, -0.7) # Bearish cross
        if k > 50 and d > 50 and score >= 0: score = max(score, 0.2) # Both > 50
        elif k < 50 and d < 50 and score <= 0: score = min(score, -0.2) # Both < 50
    ...
    def _check_rsi(self) -> float:
        ...
        if rsi >= 80: return -1.0 # Extreme OB
        if rsi >= 70: return -0.7 # OB
        if rsi > 60: return -0.3
        if rsi <= 20: return 1.0 # Extreme OS
        if rsi <= 30: return 0.7 # OS
    ...
    def _check_cci(self) -> float:
        ...
        if cci >= 200: return -1.0
        if cci >= 100: return -0.7
        if cci > 0: return -0.2
        if cci <= -200: return 1.0
        if cci <= -100: return 0.7
        if cci < 0: return 0.2
    ...
    def _check_wr(self) -> float:
        ...
        if wr >= -10: return -1.0 # Extreme OB
        if wr >= -20: return -0.7 # OB
        if wr > -50: return -0.2
        if wr <= -90: return 1.0 # Extreme OS
        if wr <= -80: return 0.7 # OS
        if wr < -50: return 0.2
    ...
    def _check_psar(self) -> float:
        ...
        if l_active and not s_active: return 1.0  # Uptrend (PSAR below price)
        if s_active and not l_active: return -1.0 # Downtrend (PSAR above price)
        if not l_active and not s_active: return np.nan # Indeterminate
        self.logger.warning(f"PSAR check: Both Long ({psar_l}) and Short ({psar_s}) seem active.")
    ...
    def _check_sma_10(self) -> float:
        ...
        close = self.indicator_values.get("Close") # Decimal | NaN
        if not all(isinstance(v, Decimal) and v.is_finite() for v in [sma, close]): return np.nan
        try:
    ...
    def _check_vwap(self) -> float:
        ...
        close = self.indicator_values.get("Close") # Decimal | NaN
        if not all(isinstance(v, Decimal) and v.is_finite() for v in [vwap, close]): return np.nan
        try:
    ...
    def _check_mfi(self) -> float:
        ...
        if mfi >= 90: return -1.0 # Extreme OB
        if mfi >= 80: return -0.7 # OB
        if mfi > 65: return -0.3
        if mfi <= 10: return 1.0 # Extreme OS
        if mfi <= 20: return 0.7 # OS
        if mfi < 35: return 0.3
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        close = self.indicator_values.get("Close")    # Decimal | NaN
        if not all(isinstance(v, Decimal) and v.is_finite() for v in [bbl, bbm, bbu, close]): return np.nan
        try:
            if close <= bbl: return 1.0 # Touch/Below Lower -> Buy
            if close >= bbu: return -1.0 # Touch/Above Upper -> Sell
            band_width = bbu - bbl
            if band_width <= 0: return np.nan # Avoid division by zero
        ...
        except (TypeError, ZeroDivisionError, InvalidOperation): return np.nan
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict], current_price: Decimal) -> float:
        ...
        if not orderbook_data or not isinstance(orderbook_data.get('bids'), list) or not isinstance(orderbook_data.get('asks'), list):
            return np.nan
        bids = orderbook_data['bids'] # Assumes list of [Decimal(price), Decimal(amount)]
        asks = orderbook_data['asks']
        if not bids or not asks: return np.nan

        try:
            levels = min(len(bids), len(asks), int(self.config.get("orderbook_limit", 10)))
            if levels <= 0: return 0.0
            bid_vol = sum(b[1] for b in bids[:levels])
            ask_vol = sum(a[1] for a in asks[:levels])
            total_vol = bid_vol + ask_vol
            if total_vol <= Decimal('1e-12'): return 0.0
    ...
    def calculate_entry_tp_sl(
        ...
        if not isinstance(entry_price_estimate, Decimal) or not entry_price_estimate.is_finite() or entry_price_estimate <= 0:
            self.logger.warning(f"TP/SL Calc Fail ({signal}): Invalid entry estimate ({entry_price_estimate}).")
            return entry_price_estimate, None, None
        ...
        try:
            ...
            if final_sl is not None:
                if abs(final_sl - entry_price_estimate) < min_tick or final_sl <= 0:
                    ...
                    adj_sl = (entry_price_estimate - min_tick).quantize(quantizer, rounding_sl) if signal == "BUY" else \
                             (entry_price_estimate + min_tick).quantize(quantizer, rounding_sl)
                    if adj_sl.is_finite() and adj_sl > 0 and abs(adj_sl - entry_price_estimate) >= min_tick:
                        self.logger.debug(f"Adjusted SL ({final_sl}) to {adj_sl} (1 tick away from {entry_price_estimate})")
                        final_sl = adj_sl
                    else:
                        self.logger.warning(f"Invalid/too close SL ({final_sl}) for Entry {entry_price_estimate}. Nullifying SL.")
                        final_sl = None

            if final_tp is not None:
                if abs(final_tp - entry_price_estimate) < min_tick or final_tp <= 0:
                    self.logger.warning(f"Invalid/too close TP ({final_tp}) for Entry {entry_price_estimate}. Nullifying TP.")
                    final_tp = None

            price_prec = self.get_price_precision()
            tp_str = f"{final_tp:.{price_prec}f}" if final_tp else "None"
            sl_str = f"{final_sl:.{price_prec}f}" if final_sl else "None"
            self.logger.debug(f"Calc TP/SL ({signal}): Entry={entry_price_estimate:.{price_prec}f}, ATR={atr_val:.{price_prec+2}f}, "
...
def fetch_balance(
    ...
    if exchange.id == 'bybit':
        ...
        lg.debug(f"Attempting Bybit balance fetch for {currency} (Account: {account_type})...")
        try:
            balance_info = safe_api_call(exchange.fetch_balance, lg, params=params)
        except ccxt.ExchangeError as e:
            # Handle account type errors gracefully, maybe fallback?
            if "account type does not exist" in str(e).lower() or "30082" in str(e): # Bybit code for invalid account type
                lg.warning(f"Account type {account_type} invalid/not found: {e}. Trying default fetch.")
                params = {} # Clear params for default fetch
                account_type_used = "Default (Fallback)"
            else:
                lg.warning(f"Exchange error fetching {account_type} balance: {e}. Trying default.")
                params = {}
                account_type_used = "Default (Fallback)"
    ...
    if balance_info is None:
        if account_type_used == "N/A": account_type_used = "Default" # Mark if default was first try
        lg.debug(f"Fetching balance for {currency} (Account: {account_type_used})...")
        try:
            balance_info = safe_api_call(exchange.fetch_balance, lg, params=params) # Use potentially cleared params
        except Exception as e:
    ...
    if currency in balance_info and isinstance(balance_info.get(currency), dict):
        free = balance_info[currency].get('free') or balance_info[currency].get('available')
        if free is not None: free_balance_str, parse_source = str(free), f"Standard ['{currency}']['free/avail']"
    elif isinstance(balance_info.get('free'), dict) and balance_info['free'].get(currency) is not None:
        free_balance_str, parse_source = str(balance_info['free'][currency]), f"Top-level ['free']['{currency}']"
    elif isinstance(balance_info.get('available'), dict) and balance_info['available'].get(currency) is not None:
        free_balance_str, parse_source = str(balance_info['available'][currency]), f"Top-level ['available']['{currency}']"

    # Bybit V5 Specific Parsing (if standard failed or we know it's Bybit)
    if free_balance_str is None and exchange.id == 'bybit' and isinstance(balance_info.get('info'), dict):
         ...
         if isinstance(result.get('list'), list) and result['list']:
             ...
             for account in result['list']:
                 ...
                 is_target_type = acc_type_resp == config.get('bybit_account_type', 'CONTRACT')
                 is_spot_match = acc_type_resp == 'SPOT' and config.get('bybit_account_type') == 'SPOT'
                 ...
                 if is_target_type or is_spot_match:
                      ...
                      if isinstance(account.get('coin'), list):
                          ...
                          if free_balance_str: break # Found, exit outer loop
                      # CONTRACT: Direct fields
                      elif acc_type_resp == 'CONTRACT':
                            free = account.get('availableBalance') or account.get('walletBalance')
                            if free is not None:
                                free_balance_str = str(free)
                                parse_source = f"Bybit V5 info.list[CONTRACT]"
             ...
             if free_balance_str is None and result['list']:
                  ...
                  if free is not None:
                       free_balance_str = str(free)
                       parse_source = f"Bybit V5 info.list[0] (Fallback)"
    ...
    if free_balance_str is None:
         ...
         if currency in balance_info and isinstance(balance_info.get(currency), dict) and balance_info[currency].get('total') is not None:
             total_str, total_src = str(balance_info[currency]['total']), f"Standard ['{currency}']['total']"
         # Add other total paths if needed
         if total_str is not None:
              lg.warning(f"{NEON_YELLOW}Using 'total' balance for {currency} ({total_str}) as fallback ({total_src}).{RESET}")
              free_balance_str = total_str
              parse_source = total_src + " (Fallback)"
         else:
              lg.error(f"{NEON_RED}Could not find any balance for {currency} (Account: {account_type_used}).{RESET}")
    ...
    try:
        ...
        if not final_balance.is_finite(): final_balance = Decimal('0')
        if final_balance < 0: final_balance = Decimal('0') # Treat negative available as zero
        lg.info(f"Available {currency} balance ({parse_source}, Acc: {account_type_used}): {final_balance:.4f}")
        return final_balance
    except (ValueError, TypeError, InvalidOperation) as e:
        lg.error(f"Failed converting balance '{free_balance_str}' ({parse_source}) to Decimal: {e}")
        return None
    ...
    try:
        if not exchange.markets or not exchange.markets_by_id:
             ...
             try: safe_api_call(exchange.load_markets, lg, reload=True)
                  ...
                  lg.error(f"{NEON_RED}Failed loading markets: {load_err}. Cannot get market info.{RESET}")
                  return None
        ...
        if not market or not isinstance(market, dict):
             ...
             if '/' in symbol and ':' not in symbol and exchange.id == 'bybit':
                  ...
                  suggested = f"{base}/{quote}:{quote}"
                  lg.warning(f"{NEON_YELLOW}Hint: Try Bybit V5 linear format like '{suggested}'.{RESET}")
             return None
        ...
        if is_contract and not is_linear and not is_inverse:
            ...
            if default_type == 'linear' or quote_id in ['USDT', 'USDC', 'BUSD']: is_linear = True
            elif default_type == 'inverse' or quote_id == 'USD': is_inverse = True
            else: is_linear = True # Default assumption

        market.update({
            'is_spot': is_spot, 'is_contract': is_contract,
            'is_linear': is_linear, 'is_inverse': is_inverse
        })

        lg.debug(f"Market Info ({symbol}): ID={market.get('id')}, Type={m_type}, "
...
def calculate_position_size(
    ...
    if not (isinstance(balance, Decimal) and balance.is_finite() and balance > 0):
        lg.error(f"Size Calc Fail ({symbol}): Invalid balance ({balance})."); return None
    if not (isinstance(risk_per_trade, (float, int)) and 0 < risk_per_trade < 1):
        lg.error(f"Size Calc Fail ({symbol}): Invalid risk_per_trade ({risk_per_trade})."); return None
    if not (isinstance(sl_price := initial_stop_loss_price, Decimal) and sl_price.is_finite() and sl_price > 0):
        lg.error(f"Size Calc Fail ({symbol}): Invalid SL price ({sl_price})."); return None
    if not (isinstance(entry_price, Decimal) and entry_price.is_finite() and entry_price > 0):
        lg.error(f"Size Calc Fail ({symbol}): Invalid entry price ({entry_price})."); return None
    if sl_price == entry_price:
        lg.error(f"Size Calc Fail ({symbol}): SL price equals entry price."); return None
    if 'limits' not in market_info or 'precision' not in market_info:
        lg.error(f"Size Calc Fail ({symbol}): Market info missing limits/precision."); return None

    # --- Block Inverse Contracts (Requires different logic) ---
    if is_inverse:
        lg.error(f"{NEON_RED}Inverse contract sizing not implemented for {symbol}.{RESET}"); return None
    if is_contract and not is_linear:
         lg.warning(f"{NEON_YELLOW}Market {symbol} is contract but not Linear. Assuming Linear sizing.{RESET}")
    ...
    try:
        ...
        if not sl_distance_quote.is_finite() or sl_distance_quote <= 0:
            lg.error(f"Size Calc Fail ({symbol}): Invalid SL distance ({sl_distance_quote})."); return None
        ...
        if is_contract: # Should be linear here
            ...
            if cs_str is not None:
                try: cs = Decimal(str(cs_str)); contract_size = cs if cs.is_finite() and cs > 0 else contract_size
                except: lg.warning(f"Invalid contract size '{cs_str}', using {contract_size}.")
        ...
        if sl_distance_quote <= 0 or contract_size <= 0:
            lg.error(f"Size Calc Fail ({symbol}): Invalid SL distance or contract size."); return None
        calculated_size = risk_amount_quote / (sl_distance_quote * contract_size)

        if not calculated_size.is_finite() or calculated_size <= 0:
            lg.error(f"Initial size calc resulted in {calculated_size}. Check inputs."); return None
        ...
        lg.info(f"  Initial Calc Size = {calculated_size:.{amount_prec_places+4}f} {size_unit}")
        ...
        if min_amount_step.is_finite() and min_amount_step > 0:
            ...
            if adj_size_step != adjusted_size:
                 lg.info(f"  Size adjusted by Step ({min_amount_step}): {adjusted_size:.{amount_prec_places+2}f} -> {adj_size_step:.{amount_prec_places}f}")
            adjusted_size = adj_size_step
        else: # Fallback: Round to precision places
             adj_size_prec = adjusted_size.quantize(Decimal('1e-' + str(amount_prec_places)), rounding=ROUND_DOWN)
             if adj_size_prec != adjusted_size:
                  lg.warning(f"Invalid amount step ({min_amount_step}). Adjusting by precision ({amount_prec_places} places).")
                  lg.info(f"  Size adjusted by Precision: {adjusted_size:.{amount_prec_places+2}f} -> {adj_size_prec:.{amount_prec_places}f}")
             adjusted_size = adj_size_prec

        # 2. Check Min Amount Limit
        if min_amt.is_finite() and min_amt > 0 and adjusted_size < min_amt:
             lg.error(f"{NEON_RED}Size Calc Fail: Adjusted size {adjusted_size:.{amount_prec_places}f} < Min Amount {min_amt}.{RESET}"); return None
        ...
        if max_amt.is_finite() and adjusted_size > max_amt:
             ...
             else: adjusted_size = adjusted_size.quantize(Decimal('1e-' + str(amount_prec_places)), rounding=ROUND_DOWN)
             lg.warning(f"{NEON_YELLOW}Size capped by Max Amount: {orig_size:.{amount_prec_places}f} -> {adjusted_size:.{amount_prec_places}f}{RESET}")
             # Re-check min amount after capping
             if min_amt.is_finite() and min_amt > 0 and adjusted_size < min_amt:
                  lg.error(f"{NEON_RED}Size Calc Fail: Size after max cap {adjusted_size} < Min Amount {min_amt}.{RESET}"); return None

        # 4. Check Cost Limits (Min/Max)
        est_cost = adjusted_size * entry_price * contract_size
        lg.debug(f"  Cost Check: Final Size={adjusted_size:.{amount_prec_places}f}, Est. Cost={est_cost:.4f} (Min:{min_cost}, Max:{max_cost})")

        if min_cost.is_finite() and min_cost > 0 and est_cost < min_cost:
             lg.error(f"{NEON_RED}Size Calc Fail: Est. Cost {est_cost:.4f} < Min Cost {min_cost}.{RESET}"); return None
        ...
        if max_cost.is_finite() and max_cost > 0 and est_cost > max_cost:
             if entry_price > 0 and contract_size > 0:
                  ...
                  adj_size_cost_capped = (size_from_max_cost // min_amount_step) * min_amount_step if min_amount_step > 0 \
                                          else size_from_max_cost.quantize(Decimal('1e-' + str(amount_prec_places)), rounding=ROUND_DOWN)

                  if adj_size_cost_capped < adjusted_size and adj_size_cost_capped > 0:
                      lg.warning(f"{NEON_YELLOW}Size capped by Max Cost: {adjusted_size:.{amount_prec_places}f} -> {adj_size_cost_capped:.{amount_prec_places}f}{RESET}")
                      adjusted_size = adj_size_cost_capped
                      # Re-check min amount after cost capping
                      if min_amt.is_finite() and min_amt > 0 and adjusted_size < min_amt:
                           lg.error(f"{NEON_RED}Size Calc Fail: Size after max cost cap {adjusted_size} < Min Amount {min_amt}.{RESET}"); return None
                  else: lg.warning(f"Max Cost capping ineffective. Original Size: {adjusted_size}")
             else:
                  lg.error(f"{NEON_RED}Size Calc Fail: Est. Cost {est_cost:.4f} > Max Cost {max_cost}, cannot recalc.{RESET}"); return None

        # --- Final Validation ---
        final_size = adjusted_size
        if not final_size.is_finite() or final_size <= 0:
             lg.error(f"{NEON_RED}Final size is invalid ({final_size}) after adjustments.{RESET}"); return None
        if min_amt.is_finite() and min_amt > 0 and final_size < min_amt:
             lg.error(f"{NEON_RED}Final size {final_size} < Min Amount {min_amt}.{RESET}"); return None
        ...
        try:
            ...
            return final_size_decimal
             ...
             lg.warning(f"Final size formatting failed: {fmt_err}. Using unformatted Decimal.")
...
def get_open_position(
    ...
    if not has_fetch_pos and not has_fetch_pos_list:
        lg.warning(f"{exchange.id} supports neither fetchPosition nor fetchPositions."); return None

    market_info = get_market_info(exchange, symbol, lg)
    if not market_info: lg.error(f"Cannot get position: Failed getting market info for {symbol}."); return None
    market_id = market_info.get('id')
    if not market_id: lg.error(f"Cannot get position: Market ID missing for {symbol}."); return None

    # Params for Bybit V5 (category is crucial)
    params = {}
    if exchange.id == 'bybit':
        params = {'symbol': market_id, 'category': config.get('bybit_category', 'linear')}
    ...
    try:
        ...
        if has_fetch_pos:
            ...
            try:
                ...
                if isinstance(pos, dict) and pos: positions_data = [pos] # Wrap for consistency
                elif pos is not None: lg.info(f"No active position via {fetch_method} (empty return).")
            except ccxt.PositionNotFound: lg.info(f"No active position via {fetch_method} (PositionNotFound).")
            except ccxt.ExchangeError as e:
                 # Check for Bybit V5 "position not found" code/message
                 no_pos_codes = [110025]
                 no_pos_msgs = ["position not found", "no position"]
                 if getattr(e, 'code', None) in no_pos_codes or any(m in str(e).lower() for m in no_pos_msgs):
                     lg.info(f"No active position via {fetch_method} (Code/Msg: {e}).")
                 else: lg.warning(f"Exchange error during {fetch_method}: {e}. Falling back.")
            except ccxt.NotSupported: lg.debug(f"{fetch_method} not supported. Falling back.")
            except Exception as e: lg.warning(f"Error during {fetch_method}: {e}. Falling back.")
        ...
        if not positions_data and has_fetch_pos_list:
            ...
            lg.debug(f"Attempting {fetch_method} with params: {params}...")
            try:
                fetched = safe_api_call(exchange.fetch_positions, lg, symbols=[symbol], params=params)
                if fetched is not None: positions_data = fetched
                if not positions_data: lg.info(f"No active position via {fetch_method}.")
            except ccxt.NotSupported: lg.debug(f"{fetch_method} filtering not supported. Falling back.")
            except Exception as e: lg.warning(f"Error during {fetch_method}: {e}. Falling back.")

        # Attempt 3: Fetch ALL positions (last resort)
        if not positions_data and has_fetch_pos_list and fetch_method.endswith("Falling back."):
             fetch_method = "fetchPositions (all, filtered locally)"
             lg.debug(f"Attempting {fetch_method} with params: {params}...") # Keep category param for V5
             try:
                 all_pos = safe_api_call(exchange.fetch_positions, lg, params=params)
                 if all_pos:
                     # Filter locally by market_id (more reliable) or symbol
                     positions_data = [p for p in all_pos if p.get('info', {}).get('symbol') == market_id or p.get('symbol') == symbol]
                     if not positions_data: lg.info(f"No position matched {symbol} after fetching all.")
                 else: lg.info("Fallback fetch of all positions returned no data.")
             except Exception as e: lg.error(f"Error during {fetch_method}: {e}")

    except Exception as fetch_err:
         lg.error(f"{NEON_RED}Unexpected error during position fetch logic: {fetch_err}{RESET}", exc_info=True)
         return None
    ...
    analyzer = TradingAnalyzer(pd.DataFrame(), lg, config, market_info)
    min_size_thresh = analyzer.get_min_amount_step() / 2 if analyzer.get_min_amount_step() > 0 else Decimal('1e-9')

    for pos_data in positions_data:
        if not isinstance(pos_data, dict): continue
        pos_symbol = pos_data.get('symbol')
        info_symbol = pos_data.get('info', {}).get('symbol')
        if pos_symbol != symbol and info_symbol != market_id: continue # Skip if symbol mismatch
        ...
        try:
            ...
            if abs(size_dec) > min_size_thresh:
                ...
                active_position['contractsDecimal'] = size_dec # Standardized Decimal size
                active_position['sizeSource'] = size_src
                lg.debug(f"Found potential active pos for {symbol} (Size: {size_dec} from {size_src})")
                break # Found the active position
            else: lg.debug(f"Ignoring pos entry for {symbol} (Size {size_dec} <= Threshold {min_size_thresh})")
        except (ValueError, TypeError, InvalidOperation): continue
    ...
    if active_position:
        try:
            ...
            if side not in ['long', 'short']:
                side = 'long' if size_dec > min_size_thresh else 'short' if size_dec < -min_size_thresh else None
                if side is None: lg.warning("Could not determine side reliably."); return None
                active_position['side'] = side

            # Helper for safe Decimal conversion (returns None if invalid/zero)
            def safe_decimal(val_str):
                if val_str is None or str(val_str).strip() in ['', '0', '0.0']: return None
                try: d = Decimal(str(val_str)); return d if d.is_finite() and d > 0 else None
                except: return None

            # Standardize key values as Decimal
            active_position['entryPriceDecimal'] = safe_decimal(active_position.get('entryPrice') or info.get('entryPrice') or info.get('avgPrice'))
            active_position['liquidationPriceDecimal'] = safe_decimal(active_position.get('liquidationPrice') or info.get('liqPrice'))
            active_position['leverageDecimal'] = safe_decimal(active_position.get('leverage') or info.get('leverage'))
            active_position['stopLossPriceDecimal'] = safe_decimal(info.get('stopLoss'))
            active_position['takeProfitPriceDecimal'] = safe_decimal(info.get('takeProfit'))
            active_position['trailingStopLossValueDecimal'] = safe_decimal(info.get('trailingStop')) # V5: Price Distance
            active_position['trailingStopActivationPriceDecimal'] = safe_decimal(info.get('activePrice')) # V5: TSL Activation Price

            # PNL can be negative
            pnl_val = active_position.get('unrealizedPnl') or info.get('unrealisedPnl')
            try: active_position['unrealizedPnlDecimal'] = Decimal(str(pnl_val)) if pnl_val is not None else None
            except: active_position['unrealizedPnlDecimal'] = None

            # Timestamp (ms)
            ts_val = info.get('updatedTime') or active_position.get('timestamp')
            try: active_position['timestamp_ms'] = int(ts_val) if ts_val is not None else None
            except: active_position['timestamp_ms'] = None

            # Log Formatted Info
            pp = analyzer.get_price_precision()
            ap = analyzer.get_amount_precision_places()
            def fmt(v, p): return f"{v:.{p}f}" if isinstance(v, Decimal) and v.is_finite() else 'N/A'
            ts_str = datetime.fromtimestamp(active_position['timestamp_ms']/1000, tz=timezone.utc).strftime('%H:%M:%S %Z') if active_position['timestamp_ms'] else "N/A"

            log_parts = [
                f"{NEON_GREEN}Active {active_position['side'].upper()} ({symbol}):{RESET}",
                f"Size={fmt(abs(size_dec), ap)}", f"Entry={fmt(active_position['entryPriceDecimal'], pp)}",
                f"Liq={fmt(active_position['liquidationPriceDecimal'], pp)}",
                f"Lev={fmt(active_position['leverageDecimal'], 1)}x" if active_position['leverageDecimal'] else 'Lev=N/A',
            ...
            lg.info(" ".join(p.replace('=N/A', '=None') for p in log_parts)) # Clean up N/A representation
            lg.debug(f"Full processed position: {json.dumps(active_position, default=str, indent=2)}")
            return active_position

        except Exception as proc_err:
             lg.error(f"Error processing active position for {symbol}: {proc_err}", exc_info=True)
...
def set_leverage_ccxt(
    ...
    if not isinstance(leverage, int) or leverage <= 0:
        lg.error(f"Invalid leverage {leverage} for {symbol}. Must be positive integer."); return False
    if not exchange.has.get('setLeverage'):
        lg.warning(f"Exchange {exchange.id} might not support setLeverage. Attempting anyway...")

    market_id = market_info.get('id')
    if not market_id: lg.error(f"Cannot set leverage: Market ID missing for {symbol}."); return False
    ...
    try:
        ...
        err_str = str(e).lower()
        already_set_msgs = ["leverage not modified", "same leverage", "leverage is already"]
        # Bybit V5 codes: 110043 (Leverage not modified)
        already_set_codes = [110043]
        if getattr(e, 'code', None) in already_set_codes or any(msg in err_str for msg in already_set_msgs):
            lg.info(f"Leverage for {symbol} likely already set to {leverage}x (Message: {e}).")
...
if __name__ == "__main__":
    ...
    logger = setup_logger("sxsBot", config, level=logging.INFO) # Use INFO for console by default
    exchange = initialize_exchange(config, logger)

    if not exchange:
        logger.critical(f"{NEON_RED}Exchange initialization failed. Bot cannot start.{RESET}")
    ...
    logger.info(f"Trading Enabled: {config.get('enable_trading')}, Sandbox: {config.get('use_sandbox')}")
    logger.info(f"Risk/Trade: {config.get('risk_per_trade'):.2%}, Leverage: {config.get('leverage')}x")
    ...
    if symbol and interval:
        try:
            ...
            # 1. Fetch Klines
            klines_df = fetch_klines_ccxt(exchange, symbol, interval, limit=200, logger=logger)
                ...
                if market_info:
                    ...
                    if current_price:
                        ...
                        if signal in ["BUY", "SELL"]:
                             _, tp_price, sl_price = analyzer.calculate_entry_tp_sl(current_price, signal)
                             logger.info(f"Example TP: {tp_price}, Example SL: {sl_price}")
                        ...
                        if not open_pos:
                            ...
                            if signal in ["BUY", "SELL"] and sl_price:
                                balance = fetch_balance(exchange, config['quote_currency'], logger, config)
                                if balance is not None:
                                     pos_size = calculate_position_size(
                                         balance, config['risk_per_trade'], sl_price, current_price,
                                         market_info, exchange, logger, config
                                     )
                                     if pos_size:
                                          logger.info(f"Example Position Size calculated: {pos_size}")
                                     else: logger.warning("Example Size Calculation failed.")
            else:
            ...
            logger.error(f"{NEON_RED}Error during example run: {e}{RESET}", exc_info=True)
