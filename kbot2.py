...
try:
    ...
    if CONFIG.symbol not in exchange.markets:
         ...
         logger.info(Fore.CYAN + f"Available active {CONFIG.market_type} symbols with {CONFIG.symbol.split('/')[1].split(':')[0]} quote (sample): " + ", ".join(available_symbols))
         sys.exit(1)
    else:
        ...
        try:
            ...
            Decimal(str(contract_size))
             ...
             logger.critical(f"{Fore.RED + Style.BRIGHT}Failed to parse critical market info (precision/limits/size) as numbers: {e}. Halting.")
             sys.exit(1)
...
def termux_notify(title: str, content: str) -> None:
    ...
    if not sys.platform.startswith('linux'):  # Basic check if not on Linux (Termux runs on Linux kernel)
...
def fetch_market_data(symbol: str, timeframe: str, limit: int, retries: int = CONFIG.max_fetch_retries) -> pd.DataFrame | None:
    ...
    for attempt in range(retries):
        try:
            ...
            ohlcv: list[list[int | float]] = exchange.fetch_ohlcv(symbol, timeframe, limit=limit)
            if not ohlcv:
                ...
                if attempt < retries - 1:
                    ...
                    logger.error(Fore.RED + f"Received empty OHLCV data after {retries} attempts.")
                    return None
            ...
            df.dropna(subset=["open", "high", "low", "close"], inplace=True)  # Drop only if price data is missing

            if df.empty:
                logger.error(Fore.RED + "DataFrame is empty after processing OHLCV data.")
                return None
...
def calculate_indicators(df: pd.DataFrame) -> dict[str, Decimal] | None:
    ...
    try:
        ...
        smooth_d = 3
        if len(df) < period:
             logger.warning(f"Not enough data ({len(df)}) for Stochastic period {period}. Skipping Stoch.")
            ...
            if pd.isna(k_now): k_now = 50.0  # Handle potential NaN at start
            if pd.isna(d_now): d_now = 50.0  # Handle potential NaN at start

        # ATR (Average True Range)
        atr_period = 10
        if len(df) < atr_period + 1:  # Need at least period+1 for shift()
             logger.warning(f"Not enough data ({len(df)}) for ATR period {atr_period}. Skipping ATR.")
        ...
        return {
            "fast_ema": Decimal(str(fast_ema_series.iloc[-1])).quantize(Decimal("0.00000001")) if not pd.isna(fast_ema_series.iloc[-1]) else Decimal(0),
            "slow_ema": Decimal(str(slow_ema_series.iloc[-1])).quantize(Decimal("0.00000001")) if not pd.isna(slow_ema_series.iloc[-1]) else Decimal(0),
            "trend_ema": Decimal(str(trend_ema_series.iloc[-1])).quantize(Decimal("0.00000001")) if not pd.isna(trend_ema_series.iloc[-1]) else Decimal(0),
            "confirm_ema": Decimal(str(confirm_ema_series.iloc[-1])).quantize(Decimal("0.00000001")) if not pd.isna(confirm_ema_series.iloc[-1]) else Decimal(0),
...
def get_current_position(symbol: str) -> dict[str, dict[str, Any]] | None:
    ...
    try:
        ...
        positions = exchange.fetch_positions(symbols=[symbol])  # Fetch for specific unified symbol
        ...
        if not symbol_positions:
            logger.info(Fore.BLUE + f"No matching position details found for {symbol} in fetched data.")
            return pos_dict
        ...
        for pos in symbol_positions:
            ...
            if side in pos_dict and contracts_str is not None:
                ...
                if contracts.copy_abs() < CONFIG.position_qty_epsilon:
                    logger.debug(f"Skipping effectively zero size {side} position for {symbol}.")
                    continue
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if order_status:
                ...
                logger.warning(f"fetch_order returned empty/unexpected structure for {order_id}. Retrying...")
...
def place_risked_market_order(symbol: str, side: str, risk_percentage: Decimal, atr: Decimal) -> bool:
    ...
    try:
        ...
        logger.debug(f"Current Price: {price}, ATR: {atr:.6f}, SL Distance Pts: {sl_distance_points:.6f}")
        ...
        if side == "buy" and sl_price >= price:
            logger.error(Fore.RED + f"Calculated SL price ({sl_price}) is >= current price ({price}). Aborting.")
            return False
        ...
        if CONFIG.market_type == 'linear':
            ...
            logger.warning(Fore.YELLOW + "Inverse contract sizing calculation is simplified and may need adjustment based on specific contract details.")
            qty_raw = risk_amount_usd / stop_distance_usd  # Simplified - NEEDS VERIFICATION FOR INVERSE
        ...
        if order_status_data:
            ...
            if order_final_status == 'closed':  # 'closed' usually means fully filled for market orders
                logger.info(Fore.GREEN + Style.BRIGHT + f"Order {order_id} confirmed filled: {filled_qty} @ {average_price:.4f}")
                 ...
                 logger.warning(Fore.YELLOW + f"Order {order_id} partially filled or status unclear: Status '{order_final_status}', Filled {filled_qty}. SL will be based on filled amount.")
        ...
        try:
            ...
            if not sl_order_id:
                raise ccxt.ExchangeError("Stop loss order placement did not return an ID.")
            ...
            logger.warning(Fore.YELLOW + "Position may be open without Stop Loss due to SL placement error. Consider emergency closure.")
...
def manage_trailing_stop(
    ...
    if has_tsl:
        logger.debug(f"{position_side.upper()} TSL (ID: {order_tracker[position_side]['tsl_id']}) is already active. Exchange is managing trail.")
    ...
    logger.debug(f"{position_side.upper()} Profit: {profit:.4f}, TSL Activation Threshold (Points): {activation_threshold_points:.4f}")
    ...
    if profit > activation_threshold_points and has_initial_sl:
        ...
        try:
            ...
            logger.warning(Fore.YELLOW + f"Initial SL (ID: {initial_sl_id}) not found when trying to cancel. Might have been triggered or already cancelled.")
            order_tracker[position_side]["sl_id"] = None  # Assume it's gone
        except (ccxt.ExchangeError, ccxt.NetworkError) as e:
            logger.error(Fore.RED + f"Failed to cancel initial SL (ID: {initial_sl_id}): {e}. Proceeding with TSL placement cautiously, but risk of double orders exists.")
        ...
        tsl_qty_str = format_amount(symbol, position_qty)  # Use current position quantity
        ...
        try:
            ...
            if not tsl_order_id:
                 raise ccxt.ExchangeError("Trailing stop order placement did not return an ID.")
            ...
            logger.error(Fore.RED + Style.BRIGHT + f"Failed to place TSL order (tried type 'trailing_stop_market'): {e}")
            logger.warning(Fore.YELLOW + "Position might be unprotected after failed TSL placement. Initial SL was likely cancelled. MANUAL INTERVENTION MAY BE NEEDED.")
...
def print_status_panel(
    ...
    [
        ...
        [Fore.CYAN + "EMA Fast/Slow", f"{ema_cross_color}{fast_ema:.4f} / {slow_ema:.4f}", f"{Fore.GREEN + ' bullish' if ema_cross_color == Fore.GREEN else Fore.RED + ' bearish' if ema_cross_color == Fore.RED else ''}"],
        [Fore.CYAN + "EMA Trend", f"{Fore.WHITE}{trend_ema:.4f}", f"{price_color}{'(Above)' if price > trend_ema else '(Below)' if price < trend_ema else '(At)'}"],
        [Fore.CYAN + "Stoch %K/%D", f"{stoch_color}{stoch_k:.2f} / {stoch_d:.2f}", f"{Fore.GREEN + ' Oversold' if stoch_color == Fore.GREEN else Fore.RED + ' Overbought' if stoch_color == Fore.RED else ''}"],
    ]
...
def generate_signals(indicators: dict[str, Decimal], current_price: Decimal) -> dict[str, bool]:
    ...
    try:
        ...
        stoch_oversold = k < Decimal(25)  # Basic level check, consider adding 'and d < 25' or crossover
        stoch_overbought = k > Decimal(75)  # Basic level check, consider adding 'and d > 75' or crossover
        ...
        if ema_bullish_cross and stoch_oversold:
            if CONFIG.trade_only_with_trend:
                if price_above_trend:
                    long_signal = True
                    logger.debug("Long Signal Criteria Met: EMA Cross Bullish, Stoch Oversold, Price Above Trend EMA")
                else:
                    logger.debug("Long Signal Blocked: Price Below Trend EMA (Trend Filter ON)")
            else:  # Trend filter off
                long_signal = True
                logger.debug("Long Signal Criteria Met: EMA Cross Bullish, Stoch Oversold (Trend Filter OFF)")
        ...
        if ema_bearish_cross and stoch_overbought:
             if CONFIG.trade_only_with_trend:
                 if price_below_trend:
                     short_signal = True
                     logger.debug("Short Signal Criteria Met: EMA Cross Bearish, Stoch Overbought, Price Below Trend EMA")
                 else:
                     logger.debug("Short Signal Blocked: Price Above Trend EMA (Trend Filter ON)")
             else:  # Trend filter off
                 short_signal = True
                 logger.debug("Short Signal Criteria Met: EMA Cross Bearish, Stoch Overbought (Trend Filter OFF)")
...
def trading_spell_cycle(cycle_count: int) -> None:
    ...
    if current_equity is None:
        ...
        logger.warning(Fore.YELLOW + "Failed to fetch current balance/equity. Status panel may be incomplete.")
...
def graceful_shutdown() -> None:
    ...
    try:
        ...
        if open_orders:
            ...
            logger.info(Fore.GREEN + f"Cancel command sent. Exchange response: {response}")  # Response varies by exchange
        else:
        ...
        logger.error(Fore.RED + f"Error dispelling orders: {e}. MANUAL CHECK REQUIRED on exchange.")
    except Exception as e:
        logger.error(Fore.RED + f"Unexpected error dispelling orders: {e}. MANUAL CHECK REQUIRED.", exc_info=True)
    ...
    try:
        ...
        if closed_count == 0 and positions is not None:  # Check positions was successfully fetched
            logger.info(Fore.GREEN + "No open positions found requiring closure.")
        elif positions is None:
             logger.error(Fore.RED + "Could not fetch positions during shutdown. MANUAL CHECK REQUIRED.")
...
if __name__ == "__main__":
    ...
    if MARKET_INFO:  # Check if market info loaded successfully
         ...
         logger.critical(Fore.RED + Style.BRIGHT + "Market info failed to load. Cannot start trading loop.")
         sys.exit(1)
