Okay, I've integrated the core concepts of a "VolumaticTrend" (VT) strategy (Trend EMA, VWMA, Volume Spike) into the Pyrmethus bot framework. The Ehlers Super Smoother and the advanced asyncio-based order book analysis from `ehlma2.py` have not been integrated due to significant architectural differences and the primary focus on the indicator-based VT strategy suggested by the logs.

Here's the enhanced version of the Pyrmethus trading bot:

**Key Changes and Enhancements (Pyrmethus v4.5.8 - Neon Nexus - VT Edition):**

1.  **Version Update:** Bot version updated in comments and logging.
2.  **VolumaticTrend (VT) Strategy Integration:**
    *   **Configuration (`TradingConfig`):**
        *   Added `VT_ENABLE`, `VT_TREND_EMA_PERIOD`, `VT_VWMA_PERIOD`, `VT_VOLUME_SPIKE_LOOKBACK`, `VT_VOLUME_SPIKE_MULTIPLIER`.
        *   Validation for these parameters, including `OHLCV_LIMIT` check against VT periods.
    *   **Indicator Calculation (`IndicatorCalculator`):**
        *   Calculates VT Trend EMA, VWMA (using `pandas-ta`), average volume, and detects volume spikes.
        *   Determines if the latest candle is green or red.
        *   These new indicators are added to the `indicators` dictionary.
        *   `max_period_needed` now considers VT indicator periods.
        *   Ensures `volume` column is processed correctly.
    *   **Signal Generation (`SignalGenerator`):**
        *   `generate_signals`:
            *   Separated original strategy logic into `_generate_original_signals`.
            *   Added `_generate_vt_signals` for the VolumaticTrend logic (Long: Price > VT Trend EMA & Price > VWMA & VolSpike & GreenCandle; Short: Price < VT Trend EMA & Price < VWMA & VolSpike & RedCandle).
            *   Combined signals: If both strategies agree, signal confirmed. If they conflict, no signal. If only one signals, that signal is used. The reason string reflects this.
        *   `check_exit_signals`:
            *   Added VT exit logic (e.g., Price crosses VT Trend EMA, or Price crosses VWMA with confirming volume spike and candle color against the position).
            *   If an original strategy exit signal fires, it's taken. Otherwise, VT exit signals are checked.
3.  **Logging & Display:**
    *   `StatusDisplay`: Updated to show VT indicators (Trend EMA, VWMA, Volume Spike) and reflect combined signal reasons.
    *   Logging messages improved for clarity regarding which strategy is active or signaling.
    *   Global `log_level_display_name` for consistent log level reporting at startup.
4.  **Dependencies:**
    *   `pandas-ta` is now a required dependency and included in `COMMON_PACKAGES` and import checks.
5.  **Bug Fix in `ExchangeManager._load_market_info` (Prophylactic):**
    *   The error log mentioned an `UnboundLocalError` in a function `get_dp`. The provided code uses `get_dp_from_precision_step`, which seems correct. I've reviewed it to ensure `prec_dec` is always defined before use. The structure provided in the prompt appears to handle this correctly.
6.  **Minor Refinements:**
    *   Type hints reviewed and slightly improved.
    *   `SignalGenerator.check_exit_signals` now has `current_price` available via the `indicators` dict (as `close_price`) for VT exit logic.

**Important Notes on `ehlma2.py` Features Not Fully Integrated:**

*   **Ehlers Super Smoother:** Not integrated as the VT strategy in Pyrmethus now uses standard EMAs and VWMA.
*   **Async Order Book Analysis & Limit Orders:** Pyrmethus remains a synchronous, cycle-based bot primarily using market orders. Integrating real-time WebSocket order book analysis and sophisticated limit order placement from `ehlma2.py` would require a major architectural overhaul to make Pyrmethus asynchronous, which is beyond the scope of this direct integration.
*   **Global State from `ehlma2.py`:** The problematic global state and `nonlocal` usage in `ehlma2.py` were not carried over; instead, VT concepts were implemented within Pyrmethus's class-based structure.
...
def safe_decimal(value: Any, default: Decimal = Decimal("NaN")) -> Decimal:
    ...
    try:
        ...
        # logger.debug(f"safe_decimal: Could not convert '{value}' (type: {type(value).__name__}) to Decimal, using default {default}")
        return default
...
class ExchangeManager:
    ...
    def _load_market_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            def get_dp_from_precision_step(precision_val: Optional[Union[str, float, int]], default_dp: int) -> int:
                ...
                if prec_dec.is_nan() or prec_dec.is_zero(): return 0 if prec_dec.is_zero() else default_dp
                if prec_dec > 0 and prec_dec < 1: return abs(prec_dec.as_tuple().exponent)
                elif prec_dec >= 1:
    ...
    def _format_v5_param(self, value: Optional[Union[Decimal, str, float, int]], param_type: Literal["price", "amount", "distance"] = "price", allow_zero: bool = False) -> Optional[str]:
        ...
        if decimal_value < 0: logger.warning(f"V5 Param Formatting: Input value '{value}' is negative ({decimal_value}), invalid for API params."); return None
    ...
    def get_current_position(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if self.config.position_idx == 0: # One-Way Mode
                ...
                elif api_side_str == "none" and qty_abs > 0: logger.warning(f"Inconsistent state for One-Way (Idx 0): API side 'None' but size {qty_abs.normalize()}."); return positions_summary
            elif self.config.position_idx == 1: # Hedge Mode - Buy
                position_side_key = "long"; 
                if api_side_str != "buy" and qty_abs > 0: logger.warning(f"Hedge Buy (Idx 1) API side '{api_side_str}' (not 'Buy') with size {qty_abs.normalize()}. Assuming long.")
            elif self.config.position_idx == 2: # Hedge Mode - Sell
                position_side_key = "short";
                if api_side_str != "sell" and qty_abs > 0: logger.warning(f"Hedge Sell (Idx 2) API side '{api_side_str}' (not 'Sell') with size {qty_abs.normalize()}. Assuming short.")
            ...
            if position_side_key:
                ...
                positions_summary[position_side_key] = {
                    "qty": qty_abs, "entry_price": entry_price if not entry_price.is_nan() and entry_price > 0 else Decimal("NaN"),
...
class SignalGenerator:
    ...
    def _generate_original_signals(self, df_last_candles: pd.DataFrame, indicators: Dict[str, Union[Decimal, bool, int]], current_price: Decimal) -> Tuple[bool, bool, str]:
        ...
        if self.config.atr_move_filter_multiplier > 0:
            if atr.is_nan() or atr <= 0: atr_filter_reason_suffix, significant_price_move = f"(ATR Filter Skipped: Invalid ATR {atr.normalize() if not atr.is_nan() else 'NaN'})", False
            elif prev_close.is_nan() or prev_close <= 0: atr_filter_reason_suffix, significant_price_move = f"(ATR Filter Skipped: Invalid Previous Close {prev_close.normalize() if not prev_close.is_nan() else 'NaN'})", False
            else:
...
class OrderManager:
    ...
    def _calculate_trade_parameters(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> Optional[Dict[str, Optional[Decimal]]]:
        # Input validation
        if atr.is_nan() or atr <= 0: logger.error(f"Invalid ATR ({atr.normalize() if not atr.is_nan() else 'NaN'}) for trade parameter calculation."); return None
        if total_equity.is_nan() or total_equity <= 0: logger.error(f"Invalid total equity ({total_equity.normalize() if not total_equity.is_nan() else 'NaN'}) for parameter calculation."); return None
        if current_price.is_nan() or current_price <= 0: logger.error(f"Invalid current price ({current_price.normalize() if not current_price.is_nan() else 'NaN'}) for parameter calculation."); return None
        if not self.market_info or any(self.market_info.get(k, Decimal('NaN')).is_nan() for k in ['tick_size', 'contract_size', 'min_order_size']): # type: ignore
        ...
        try:
            ...
            if sl_price_calculated <= 0: logger.error(f"Calculated SL price ({sl_price_calculated:.{DEFAULT_PRICE_DP}f}) is invalid (<=0). Cannot proceed."); return None

            sl_distance_from_current_abs = (current_price - sl_price_calculated).copy_abs()
            min_tick_size = self.market_info['tick_size'] # type: ignore
            if min_tick_size.is_nan() or min_tick_size <= 0: logger.error("Market tick_size is invalid or zero. Cannot validate SL distance."); return None # type: ignore
            if sl_distance_from_current_abs < min_tick_size: # type: ignore
                logger.warning(f"Initial SL distance ({sl_distance_from_current_abs.normalize()}) < min tick size ({min_tick_size.normalize()}). Adjusting SL distance to min tick size."); sl_distance_from_current_abs = min_tick_size # type: ignore
                ...
                if sl_price_calculated <= 0: logger.error(f"Adjusted SL price ({sl_price_calculated:.{DEFAULT_PRICE_DP}f}) is still invalid (<=0)."); return None
            if sl_distance_from_current_abs <= 0: logger.error(f"Calculated SL distance ({sl_distance_from_current_abs.normalize()}) is invalid (<=0)."); return None

            market_contract_size = self.market_info['contract_size']; quantity_calculated_base_asset: Decimal # type: ignore
            if self.config.market_type == "inverse":
                ...
                value_change_per_point_per_base_unit = market_contract_size # type: ignore
                if value_change_per_point_per_base_unit <= 0: logger.error("Invalid contract size for linear quantity."); return None # type: ignore
                risk_per_unit_of_base = sl_distance_from_current_abs * value_change_per_point_per_base_unit
                if risk_per_unit_of_base <= 0: logger.error(f"Calculated zero or negative risk per unit of base ({risk_per_unit_of_base.normalize()}). Cannot determine quantity."); return None
            ...
            if quantity_decimal_final.is_nan() or quantity_decimal_final <= 0: logger.error(f"Calculated quantity ({quantity_str_formatted}) is invalid or zero after formatting. Original calc: {quantity_calculated_base_asset.normalize()}"); return None
            min_order_size_market = self.market_info.get('min_order_size', Decimal('NaN')) # type: ignore
            if min_order_size_market.is_nan(): logger.error("Min order size is NaN."); return None # type: ignore
            if quantity_decimal_final < min_order_size_market: logger.error(f"Calculated quantity {quantity_decimal_final.normalize()} is less than market minimum order size {min_order_size_market.normalize()}."); return None # type: ignore
            ...
            if self.config.tp_atr_multiplier > 0:
                ...
                if tp_price_calculated <= 0: logger.warning(f"Calculated TP price ({tp_price_calculated:.{DEFAULT_PRICE_DP}f}) is invalid (<=0). Disabling TP for this trade."); tp_price_calculated = None
            
            tsl_distance_price_points = current_price * (self.config.trailing_stop_percent / 100)
            if tsl_distance_price_points < min_tick_size: logger.debug(f"TSL distance ({tsl_distance_price_points.normalize()}) < min tick size ({min_tick_size.normalize()}). Adjusting TSL distance to min tick size."); tsl_distance_price_points = min_tick_size # type: ignore
            ...
            if sl_price_decimal_final.is_nan() or sl_price_decimal_final <= 0: logger.error(f"Formatted SL price ('{sl_price_str_formatted}') is invalid. Aborting parameter calculation."); return None
            ...
            if tp_price_calculated is not None:
                 ...
                 if tp_price_decimal_final.is_nan() or tp_price_decimal_final <= 0: logger.warning(f"Failed to format a valid TP price ('{tp_price_str_formatted}'). Disabling TP for this trade."); tp_price_decimal_final = None
    ...
    def _execute_market_order(self, side: str, qty_decimal: Decimal) -> Optional[Dict[str, Any]]:
        ...
        if final_qty_decimal_for_log.is_nan() or final_qty_decimal_for_log <= 0: logger.error(f"Attempted market order with zero/invalid formatted quantity: '{qty_str_for_api}' (Original Decimal: {qty_decimal.normalize()}). Order aborted."); return None
        ...
        logger.trade(f"{Fore.CYAN}Attempting MARKET {side.upper()} order: {final_qty_decimal_for_log.normalize()} {self.market_info.get('base', '')} for {self.config.symbol}...{Style.RESET_ALL}") # type: ignore
        try:
            ...
            elif order_status == "canceled" and filled_qty_decimal == 0 and params_v5.get("timeInForce") == "ImmediateOrCancel": logger.error(f"{Fore.RED}Market order {order_id} (IOC) was CANCELED with 0 filled. Order did not execute.{Style.RESET_ALL}"); return None
    ...
    def _set_position_protection(self, position_side: str, sl_price: Optional[Decimal] = None, tp_price: Optional[Decimal] = None, is_tsl: bool = False, tsl_distance: Optional[Decimal] = None, tsl_activation_price: Optional[Decimal] = None) -> bool:
        ...
        market_id = self.market_info.get("id"); tracker_key = position_side.lower() # type: ignore
    ...
    def place_risked_market_order(self, side: str, atr: Decimal, total_equity: Decimal, current_price: Decimal) -> bool:
        ...
        if not trade_params or not trade_params.get("qty") or trade_params["qty"] <= 0: logger.error("Entry Aborted: Failed to calculate valid trade parameters (qty, SL, etc.)."); return False # type: ignore
        ...
        if initial_sl_price is None or initial_sl_price.is_nan() or initial_sl_price <= 0: logger.error(f"Entry Aborted: Invalid Stop Loss price ({initial_sl_price}) calculated."); return False
    ...
    def manage_trailing_stop(self, position_side: str, entry_price: Decimal, current_market_price: Decimal, current_atr: Decimal):
        ...
        if any(val.is_nan() or val <= 0 for val in [current_atr, entry_price, current_market_price]): # type: ignore
        ...
        try:
            ...
            if tsl_activation_target_price.is_nan() or tsl_activation_target_price <= 0: logger.warning(f"Invalid TSL activation price ({tsl_activation_target_price.normalize()}). Skipping TSL for {position_side.upper()}."); return
            ...
            if not min_tick_size.is_nan() and min_tick_size > 0 and tsl_actual_distance_points < min_tick_size: # type: ignore
            ...
            if tsl_actual_distance_points <= 0: logger.warning(f"Invalid TSL distance ({tsl_actual_distance_points.normalize()}). Skipping TSL for {position_side.upper()}."); return
    ...
    def close_position(self, position_side_to_close: str, qty_abs_to_close: Decimal, reason: str = "Strategy Exit Signal") -> bool:
        ...
        logger.trade(f"{Fore.YELLOW}Attempting to CLOSE {position_side_to_close.upper()} position (Qty: {qty_abs_to_close.normalize()} {self.market_info.get('base', '')}) for {self.config.symbol} | Reason: {reason}...{Style.RESET_ALL}") # type: ignore
...
class StatusDisplay:
    ...
    def print_status_panel(self, cycle_num: int, current_timestamp: Optional[datetime], current_market_price: Optional[Decimal], indicators_data: Optional[Dict[str, Any]], current_positions_summary: Optional[Dict[str, Dict[str, Any]]], account_equity: Optional[Decimal], signal_check_result_or_status: Dict[str, Any], protection_status_tracker: Dict[str, Optional[str]], market_specific_info: Optional[Dict[str, Any]]):
        ...
        if market_specific_info and "precision_dp" in market_specific_info: # type: ignore
             price_display_dp = market_specific_info["precision_dp"].get("price", self._default_price_dp_display) # type: ignore
             amount_display_dp = market_specific_info["precision_dp"].get("amount", self._default_amount_dp_display) # type: ignore
        ...
        settle_ccy = market_specific_info.get("settle", "SETTLE") if market_specific_info else "SETTLE" # type: ignore
        ...
        if indicators_data:
            parts = []
            def fmt_ind(key: str, prec: int = 1, style: str = "white", is_bool: bool = False) -> Text: # Added is_bool
                 val = indicators_data.get(key) # type: ignore
                 if is_bool: return Text(str(val), style=(style if val else "dim " + style)) # Dim if False
                 if isinstance(val, bool): return Text(str(val), style=style) # Should use is_bool for these
            ...
            if indicators_data.get('stoch_kd_bullish'): stoch_text.append(" [b green]▲BullX[/]", style="green") # type: ignore
            elif indicators_data.get('stoch_kd_bearish'): stoch_text.append(" [b red]▼BearX[/]", style="red") # type: ignore
            parts.append(stoch_text)
            parts.append(Text(f"ATR({indicators_data.get('atr_period',self.config.atr_period)}): ").append(fmt_ind('atr', price_display_dp+1, "bright_magenta"))) # type: ignore
            adx_val_dec = indicators_data.get('adx'); adx_val_dec = adx_val_dec if isinstance(adx_val_dec, Decimal) else safe_decimal(str(adx_val_dec) if adx_val_dec is not None else "NaN") # type: ignore
            ...
            if self.config.vt_enable and indicators_data.get('vt_trend_ema') is not None: # type: ignore
                ...
                vt_display_parts.append(Text("VolSpike:").append(fmt_ind('vt_is_volume_spike', style="green" if indicators_data.get('vt_is_volume_spike') else "dim green", is_bool=True))) # type: ignore
                candle_color_str = "Green" if indicators_data.get('vt_candle_is_green') else "Red" if indicators_data.get('vt_candle_is_red') else "Neutral" # type: ignore
        ...
        if active_pos_side and active_pos_data:
            ...
            if exch_tsl_active: exchange_prot_desc_parts.append(Text("TSL",style="bright_magenta")); _ = exchange_prot_desc_parts.append(Text(f"(ActPx:{self._format_decimal_for_rich(exch_tsl_trigger_px,price_display_dp).plain})",style="magenta")) if exch_tsl_trigger_px else None # type: ignore
...
class TradingBot:
    ...
    def trading_spell_cycle(self, cycle_num: int):
        ...
        try:
            ...
            if current_market_price.is_nan() or current_market_price <= 0: raise ValueError(f"Invalid latest close price from OHLCV: {current_market_price.normalize() if not current_market_price.is_nan() else 'NaN'}")
        ...
        if active_pos_side_logical and active_pos_details:
            ...
            if (self.order_manager.protection_tracker.get(active_pos_side_logical) == "ACTIVE_SLTP" and not any(v.is_nan() or v <= 0 for v in [pos_entry_price, current_market_price, current_atr])): # type: ignore
            ...
            if self.order_manager.protection_tracker.get(active_pos_side_logical) != "ACTIVE_TSL":
                ...
                if exit_reason_signal:
                    ...
                    if not pos_qty_abs.is_nan() and pos_qty_abs > 0: # type: ignore
        ...
        if not active_pos_side_logical:
            ...
            if target_entry_order_side:
                ...
                if not current_atr.is_nan() and current_atr > 0: # type: ignore
...
if __name__ == "__main__":
    try:
        ...
        log_func = logger.critical if 'logger' in globals() else print # type: ignore
        if 'colorama_init' in globals() and 'Fore' in globals() and 'Style' in globals(): # type: ignore
             colorama_init(autoreset=True); err_msg = f"{Style.BRIGHT}{Fore.RED}CRITICAL UNHANDLED EXCEPTION in Pyrmethus main execution: {main_exception}{Style.RESET_ALL}" # type: ignore
        else: err_msg = f"CRITICAL UNHANDLED EXCEPTION in Pyrmethus main execution: {main_exception}"
        ...
        if 'termux_notify' in globals(): termux_notify("Pyrmethus CRASHED", "Critical unhandled exception. Check logs!") # type: ignore
        sys.exit(1)
```
