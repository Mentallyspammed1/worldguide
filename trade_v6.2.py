...
STRONG_SIGNAL_THRESHOLD_ABS: int = (
    3  # Absolute score threshold for strong signal adjustment
)
...
STATUS_CLOSED_EXT: str = (
    "closed_externally"  # Position closed by SL/TP/Manual action detected
)
...
def retry_api_call(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any | None:
            ...
            func_name = (
                f"{instance_name}.{func.__name__}" if instance_name else func.__name__
            )

            while retries <= max_retries:
                try:
                    logger.debug(
                        f"API Call: {func_name} - Attempt {retries + 1}/{max_retries + 1}"
                    )
                ...
                # --- Exchange Errors - Distinguish Retryable vs Non-Retryable ---
                    ...
                    if any(
                        ...
                        for phrase in [
                            ...
                            "unknown order",  # Order Life Cycle (Often expected)
                            "order already cancelled",
                            "order was canceled",
                            "cancel order failed",
                            "order is finished",
                            "order hs been filled",
                            "already closed",
                            "order status error",
                            "cannot be modified",
                            "position status",  # Bybit: position already closed etc.
                            "insufficient balance",
                            "insufficient margin",
                            "margin is insufficient",  # Funding
                            "available balance insufficient",
                            "insufficient funds",
                            "risk limit",  # Funding/Limits
                            "position side is not match",  # Bybit hedge mode issue?
                            "invalid order",
                            "parameter error",
                            "size too small",
                            "price too low",  # Invalid Request / Limits
                            "price too high",
                            ...
                            "api key is invalid",
                            "invalid api key",  # Auth (redundant?)
                            "position mode not modified",
                            "leverage not modified",  # Config/Setup issues
                            "reduceonly",
                            "reduce-only",  # Position logic errors
                            "bad symbol",
                        ...
                        log_level = (
                            ...
                            if (
                                ...
                                or "is finished" in err_str
                            )
                            else logging.ERROR
                        )
                        log_color = (
                            Fore.YELLOW if log_level == logging.WARNING else Fore.RED
                        )
                ...
                # --- Catch-all for other unexpected exceptions ---
                    ...
                    logger.error(
                        f"{Fore.RED}{log_msg}{Style.RESET_ALL}", exc_info=True
                    )  # Log traceback
                ...
                if retries < max_retries:
                    ...
                    logger.error(
                        f"{Fore.RED}Max retries ({max_retries + 1}) reached for {func_name}. Spell falters.{Style.RESET_ALL}"
                    )
...
class ScalpingBot:
    ...
    **Disclaimer:** Trading cryptocurrencies involves significant risk. Use at your own risk.
    ...
    def __init__(
        ...
        logger.info(
            f"{Fore.MAGENTA}--- Pyrmethus Scalping Bot v6.2 Awakening ---{Style.RESET_ALL}"
        )
        ...
        self.api_key: str | None = os.getenv("BYBIT_API_KEY") or self.config.get(
            "exchange", {}
        ).get("api_key")
        self.api_secret: str | None = os.getenv("BYBIT_API_SECRET") or self.config.get(
            "exchange", {}
        ).get("api_secret")
        ...
        self.close_positions_on_exit: bool = self.config["trading"][
        ...
        self.atr_sl_multiplier: float = self.config["risk_management"].get(
            "atr_sl_multiplier", ATR_MULTIPLIER_SL
        )
        self.atr_tp_multiplier: float = self.config["risk_management"].get(
        ...
        logger.info(
            f"{Fore.CYAN}Scalping Bot V6.2 initialized. Symbol: {self.symbol}, Timeframe: {self.timeframe}{Style.RESET_ALL}"
        )
    ...
    def _configure_logging_level(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Effective Bot Logger Level: {logging.getLevelName(logger.level)}"
            )
            logger.debug(
                f"Effective Console Handler Level: {logging.getLevelName(console_handler.level)}"
            )
            if file_handler:
                logger.debug(
                    f"Effective File Handler Level: {logging.getLevelName(file_handler.level)}"
                )
        except Exception as e:
            logger.error(
                f"{Fore.RED}Error configuring logging level: {e}. Using default INFO.{Style.RESET_ALL}"
            )
    ...
    def _validate_state_entry(self, entry: Any, index: int) -> bool:
        ...
        if not isinstance(entry, dict):
            logger.warning(
                f"State file entry #{index + 1} is not a dictionary, skipping: {str(entry)[:100]}..."
            )
            return False
        ...
        if missing_keys:
            logger.warning(
                f"State file entry #{index + 1} (ID: {entry.get('id', 'N/A')}) missing required keys: {missing_keys}, skipping."
            )
            return False
        ...
        if entry["status"] == STATUS_ACTIVE:
            ...
            if missing_active:
                logger.warning(
                    f"State file entry #{index + 1} (ID: {entry['id']}) has status ACTIVE but missing keys: {missing_active}, skipping."
                )
                return False
            if (
                entry.get("entry_price") is None
                or entry.get("size") is None
                or entry.get("size", 0) <= 0
            ):
                logger.warning(
                    f"State file entry #{index + 1} (ID: {entry['id']}) has status ACTIVE but invalid entry_price/size. Skipping."
                )
                return False
        ...
        for key in [
            ...
            if (
                ...
                and not isinstance(entry[key], (float, int))
            ):
                ...
                try:
                    ...
                    logger.warning(
                        f"State file entry #{index + 1} (ID: {entry['id']}) has non-numeric value for '{key}': {entry[key]}. Keeping original, but might cause issues."
                    )
    ...
    def _load_state(self) -> None:
        ...
        try:
                            ...
                            if isinstance(saved_state_raw, list):
                                ...
                                if os.path.exists(state_backup_file):
                                    try:
                                        ...
                                        logger.debug(
                                            f"Removed old state backup: {state_backup_file}"
                                        )
                                    except OSError as remove_err:
                                        logger.warning(
                                            f"Could not remove old state backup {state_backup_file}: {remove_err}"
                                        )
                            else:
                                raise ValueError(
                                    f"Invalid state format - expected a list, got {type(saved_state_raw)}."
                                )
            else:
                logger.info(
            ...
            logger.error(
                f"{Fore.RED}Error decoding/validating primary state file {self.state_file}: {e}. Attempting recovery...{Style.RESET_ALL}"
            )
            ...
            if os.path.exists(state_backup_file):
                ...
                try:
                    ...
                    if os.path.exists(self.state_file):
                        ...
                        logger.warning(
                            f"Backed up corrupted state file to {corrupted_path}"
                        )

                    shutil.copy2(state_backup_file, self.state_file)
                    logger.info(
                        f"{Fore.GREEN}State restored from backup. Retrying load...{Style.RESET_ALL}"
                    )
                ...
                logger.warning(
                    f"{Fore.YELLOW}No backup state file found. Corrupted primary file backed up (if possible). Starting fresh.{Style.RESET_ALL}"
                )
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            temp_fd, temp_path = tempfile.mkstemp(
                dir=state_dir, prefix=os.path.basename(self.state_file) + ".tmp_"
            )
            logger.debug(
                f"Saving state ({len(self.open_positions)} positions) via temp file {temp_path}..."
            )

            # Serialize State Data (ensure basic types)
            # Use default=str as fallback, but ensure numbers are numbers where possible
            state_data_to_save = json.loads(
                json.dumps(self.open_positions, default=str)
            )
            ...
            if os.path.exists(self.state_file):
                try:
                    shutil.copy2(
                        self.state_file, state_backup_file
                    )  # copy2 preserves metadata
                    logger.debug(f"Created state backup: {state_backup_file}")
                except Exception as backup_err:
                    logger.warning(
                        f"Could not create state backup {state_backup_file}: {backup_err}. Proceeding cautiously."
                    )
            ...
            logger.error(
                f"{Fore.RED}Could not scribe state to {self.state_file} (Error Type: {type(e).__name__}): {e}{Style.RESET_ALL}"
            )
        except Exception as e:
            logger.error(
            ...
            if (
                temp_fd is not None
            ):  # If fd was opened but not closed by 'with' (e.g., error during dump)
            ...
            if temp_path is not None and os.path.exists(temp_path):
                try:
                    ...
                    logger.error(
                        f"Error removing temp state file {temp_path}: {rm_err}"
                    )
    ...
    def load_config(self, config_file: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"{Fore.RED}Failed to create default config: {e}{Style.RESET_ALL}"
                )
            ...
            logger.critical(
                f"{Fore.RED}Fatal: Unexpected chaos loading configuration: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            sys.exit(1)
    ...
    def create_default_config(self, config_file: str) -> None:
        ...
        if env_order_pct:
            try:
                ...
                if 0 < override_val <= 1:
                    default_config["risk_management"]["order_size_percentage"] = (
                        override_val
                    )
                    logger.info(
                        f"Overrode order_size_percentage from environment to {override_val}."
                    )
                else:
                    logger.warning(
                        "Invalid ORDER_SIZE_PERCENTAGE in env, using default."
                    )
            except ValueError:
                logger.warning("Invalid ORDER_SIZE_PERCENTAGE in env, using default.")
        ...
        try:
            ...
            logger.info(
                f"{Fore.YELLOW}A default spellbook has been crafted: '{config_file}'.{Style.RESET_ALL}"
            )
            logger.info(
    ...
    def validate_config(self) -> None:
        ...
        try:
            ...
            def _get_nested(
                ...
                req_type: type | tuple[type, ...] | None = None,
                ...
                if req_type is not None and not isinstance(value, req_type):
                    ...
                    if req_type is float and isinstance(value, int):
                        return float(value)
                    raise TypeError(
                        f"Key '{full_key_path}' expects type {req_type} but got {type(value)}."
                    )
                return value
            ...
            def _check_range(
                ...
                if min_val is not None:
                    if min_exclusive and value <= min_val:
                        raise ValueError(f"{key_path} ({value}) must be > {min_val}")
                    if not min_exclusive and value < min_val:
                        raise ValueError(f"{key_path} ({value}) must be >= {min_val}")
                if max_val is not None:
                    if max_exclusive and value >= max_val:
                        raise ValueError(f"{key_path} ({value}) must be < {max_val}")
            ...
            for section in required_sections:
                if section not in self.config or not isinstance(
                    ...
                    raise ValueError(
                        f"Config Error: Missing or invalid section '{section}'. Must be a dictionary."
                    )

            # --- Logging ---
            log_level = _get_nested(
                self.config, ["level"], "logging", default="INFO", req_type=str
            ).upper()
            if not hasattr(logging, log_level):
                raise ValueError(f"logging.level: Invalid level '{log_level}'.")

            # --- Exchange ---
            ex_id = _get_nested(self.config, ["exchange_id"], "exchange", req_type=str)
            if not ex_id or ex_id not in ccxt.exchanges:
                raise ValueError(f"exchange_id: Invalid or unsupported '{ex_id}'.")
            ...
            entry_type = _get_nested(
                self.config, ["entry_order_type"], "trading", req_type=str
            ).lower()
            ...
            _get_nested(
                self.config, ["close_positions_on_exit"], "trading", req_type=bool
            )

            # --- Order Book ---
            _check_range(
                _get_nested(self.config, ["depth"], "order_book", req_type=int),
                "order_book.depth",
                min_val=1,
            )
            _check_range(
            ...
            for p in periods:
                _check_range(
                    _get_nested(self.config, [p], "indicators", req_type=int),
                    f"indicators.{p}",
                    min_val=1,
                )
            _check_range(
            ...
            if (
                self.config["indicators"]["macd_short_period"]
                >= self.config["indicators"]["macd_long_period"]
            ):
                raise ValueError(
                    "indicators: macd_short_period must be less than macd_long_period."
                )

            # --- Risk Management ---
            use_atr = _get_nested(
                self.config, ["use_atr_sl_tp"], "risk_management", req_type=bool
            )
            if use_atr:
                _check_range(
                    _get_nested(
            ...
            sl_trig = _get_nested(
                self.config, ["sl_trigger_by"], "risk_management", allow_none=True
            )
            tp_trig = _get_nested(
                self.config, ["tp_trigger_by"], "risk_management", allow_none=True
            )
            if sl_trig not in valid_triggers:
                raise ValueError(
                    f"risk_management.sl_trigger_by: Invalid trigger '{sl_trig}'."
                )
            if tp_trig not in valid_triggers:
                raise ValueError(
                    f"risk_management.tp_trigger_by: Invalid trigger '{tp_trig}'."
                )

            enable_tsl = _get_nested(
                self.config,
                ["enable_trailing_stop_loss"],
                "risk_management",
                req_type=bool,
            ...
            _check_range(
                _get_nested(
                    self.config, ["max_open_positions"], "risk_management", req_type=int
                ),
                "risk_management.max_open_positions",
                min_val=1,
            )
            time_exit = _get_nested(
                self.config,
                ["time_based_exit_minutes"],
                "risk_management",
                req_type=int,
            ...
            if time_exit is not None:
                _check_range(
                    time_exit, "risk_management.time_based_exit_minutes", min_val=0
                )
            _check_range(
                _get_nested(
                    self.config,
            ...
            logger.critical(
                f"{Fore.RED}Unexpected chaos during configuration validation: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            sys.exit(1)
    ...
    def _initialize_exchange(self) -> ccxt.Exchange:
        ...
        if not self.simulation_mode and not creds_found:
            logger.critical(
                f"{Fore.RED}API Key/Secret essence missing (check .env or config). Cannot trade live/testnet. Aborting.{Style.RESET_ALL}"
            )
            sys.exit(1)
            ...
            logger.info(
                f"{Fore.CYAN}Running in internal simulation mode. API credentials not required/found.{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if not self.simulation_mode and self.testnet_mode:
                ...
                try:
                    ...
                    logger.info(
                        f"{Fore.YELLOW}Exchange sandbox mode explicitly enabled via CCXT.{Style.RESET_ALL}"
                    )
                except ccxt.NotSupported:
                    logger.warning(
                        f"{Fore.YELLOW}{self.exchange_id} may not support unified set_sandbox_mode. Testnet depends on API key type/URL.{Style.RESET_ALL}"
                    )
                ...
                logger.info(
                    f"{Fore.GREEN}Exchange sandbox mode is OFF. Operating on Mainnet.{Style.RESET_ALL}"
                )
            ...
            if self.timeframe not in exchange.timeframes:
                ...
                logger.critical(
                    f"{Fore.RED}Timeframe '{self.timeframe}' unsupported by {self.exchange_id}. Available: {available_tf}... Aborting.{Style.RESET_ALL}"
                )
                sys.exit(1)
            if self.symbol not in exchange.markets:
                available_sym = list(exchange.markets.keys())[:10]
                logger.critical(
                    f"{Fore.RED}Symbol '{self.symbol}' not found on {self.exchange_id}. Available: {available_sym}... Aborting.{Style.RESET_ALL}"
                )
                sys.exit(1)
            ...
            if not market_details.get("active", True):
                logger.warning(
                    f"{Fore.YELLOW}Warning: Market '{self.symbol}' is marked as inactive.{Style.RESET_ALL}"
                )

            logger.info(
                f"Symbol '{self.symbol}' and timeframe '{self.timeframe}' confirmed available."
            )

            # Perform initial API connectivity test (if not simulating)
            if not self.simulation_mode:
                logger.debug(
                    "Performing initial API connectivity test (fetch balance)..."
                )
                # Use fetch_balance as a more comprehensive initial test than fetch_time
                initial_balance_check = self.fetch_balance(
                    currency_code=market_details.get("quote")
                )
                if (
                    initial_balance_check is not None
                ):  # fetch_balance returns float or 0.0 on error
                    logger.info(
                        f"{Fore.GREEN}API connection and authentication successful (Balance Check OK).{Style.RESET_ALL}"
                    )
                else:
                    # Fetch_balance logs specific errors, just add a critical failure message here
                    logger.critical(
                        f"{Fore.RED}Initial API connectivity/auth test (fetch_balance) failed. Check logs. Aborting.{Style.RESET_ALL}"
                    )
                    sys.exit(1)

            logger.info(
                f"{Fore.GREEN}Exchange spirits aligned for {self.exchange_id.upper()}.{Style.RESET_ALL}"
            )
            return exchange

        except ccxt.AuthenticationError as e:
            logger.critical(
                f"{Fore.RED}Authentication failed for {self.exchange_id}: {e}. Check API keys/permissions. Aborting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except (ccxt.ExchangeNotAvailable, ccxt.RequestTimeout, ccxt.NetworkError) as e:
            logger.critical(
                f"{Fore.RED}Connection failed to {self.exchange_id} ({type(e).__name__}): {e}. Check network/status. Aborting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except Exception as e:
            logger.critical(
                f"{Fore.RED}Unexpected error initializing exchange {self.exchange_id}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            sys.exit(1)
    ...
    def _calculate_decimal_places(self, tick_size: float | int | str | None) -> int:
        ...
        try:
            ...
            if isinstance(tick_size, int) or tick_size.is_integer():
                return 0  # Integer tick size means 0 decimals

            # Use string formatting method for robustness with various float representations
            s = format(tick_size, ".16f").rstrip(
                "0"
            ...
            logger.warning(
                f"Could not parse tick size '{tick_size}' for decimal calculation: {e}. Using fallback {fallback_decimals}."
            )
            return fallback_decimals
        except Exception as e:
            logger.error(
                f"Unexpected error calculating decimals for tick size {tick_size}: {e}. Using fallback {fallback_decimals}."
            )
            return fallback_decimals
    ...
    def _load_market_info(self) -> None:
        ...
        try:
            ...
            if not self.market_info:
                raise ValueError(
                    f"Market info for '{self.symbol}' not found after loading markets."
                )
            ...
            if amount_tick is None:
                logger.warning(
                    f"{Fore.YELLOW}Amount tick size missing for {self.symbol}. Using default amount decimals: {self.amount_decimals}.{Style.RESET_ALL}"
                )
            if price_tick is None:
                logger.warning(
                    f"{Fore.YELLOW}Price tick size missing for {self.symbol}. Using default price decimals: {self.price_decimals}.{Style.RESET_ALL}"
                )
            if min_amount is None:
                logger.warning(
                    f"{Fore.YELLOW}Min order amount limit missing for {self.symbol}.{Style.RESET_ALL}"
                )
            if min_cost is None:
                logger.warning(
                    f"{Fore.YELLOW}Min order cost limit missing for {self.symbol}.{Style.RESET_ALL}"
                )

            logger.info(
                f"Market Details for {self.symbol}: Price Decimals={self.price_decimals} (Tick: {price_tick}), Amount Decimals={self.amount_decimals} (Tick: {amount_tick})"
            )
            ...
            logger.critical(
                f"{Fore.RED}Error loading/parsing market info for {self.symbol}: {e}. Market Info: {self.market_info}. Aborting.{Style.RESET_ALL}",
                exc_info=False,
            )
            sys.exit(1)
        except Exception as e:
            logger.critical(
                f"{Fore.RED}Failed to load crucial market info for {self.symbol}: {e}. Aborting.{Style.RESET_ALL}",
                exc_info=True,
            )
            sys.exit(1)
    ...
    def fetch_market_price(self) -> float | None:
        ...
        if last_price is not None:
            try:
                ...
                logger.debug(
                    f"Current market price ({self.symbol}): {self.format_price(price)}"
                )
                return price
            except (ValueError, TypeError) as e:
                logger.error(
                    f"{Fore.RED}Error converting ticker 'last' price ({last_price}) to float for {self.symbol}: {e}{Style.RESET_ALL}"
                )
                return None
        else:
            logger.warning(
    ...
    def fetch_order_book(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            valid_bids = [
                ...
                and len(bid) >= 2
                and isinstance(bid[1], (int, float))
                and bid[1] >= 0
            ]
            valid_asks = [
                ask
                ...
                and len(ask) >= 2
                and isinstance(ask[1], (int, float))
                and ask[1] >= 0
            ]

            if not valid_bids or not valid_asks:
                logger.debug(
                    f"Order book data invalid/incomplete for {self.symbol}. Bids: {len(valid_bids)}/{len(bids)}, Asks: {len(valid_asks)}/{len(asks)}"
                )
            ...
            if bid_volume > epsilon:
                ...
                logger.debug(
                    f"Order Book ({self.symbol}) Imbalance: Inf (Near-Zero Bid Vol)"
                )
            else:  # Both near zero
                result["imbalance"] = None
                logger.debug(
                    f"Order Book ({self.symbol}) Imbalance: N/A (Near-Zero Volumes)"
                )
            return result
        except Exception as e:
            logger.error(
                f"{Fore.RED}Error fetching/processing order book for {self.symbol}: {e}{Style.RESET_ALL}",
                exc_info=False,
            )
            return None
    ...
    def fetch_historical_data(self, limit: int | None = None) -> pd.DataFrame | None:
        ...
        if limit is None:
            ...
            valid_periods = [
                p for p in required_periods if isinstance(p, int) and p > 0
            ]
            max_lookback = (
                max(valid_periods) if valid_periods else 50
            )  # Default lookback if no valid periods
            required_limit = max(
                max_lookback + 50, 100
            )  # Add buffer, ensure minimum fetch
            required_limit = min(required_limit, 1000)  # Cap request size
            fetch_limit = required_limit
            min_required_len_for_calc = (
                max_lookback  # Min valid rows needed AFTER cleaning
            )
        else:
            fetch_limit = limit
            min_required_len_for_calc = (
                limit - 50
            )  # Estimate minimum needed if limit overridden

        logger.debug(
            f"Fetching ~{fetch_limit} historical candles for {self.symbol} ({self.timeframe})..."
        )
        try:
            ...
            if not ohlcv:
                logger.warning(
                    f"{Fore.YELLOW}No historical OHLCV data returned for {self.symbol} ({self.timeframe}, limit {fetch_limit}).{Style.RESET_ALL}"
                )
                return None
            if len(ohlcv) < 5:
                logger.warning(
                    f"{Fore.YELLOW}Very few historical OHLCV data points returned ({len(ohlcv)}). Insufficient.{Style.RESET_ALL}"
                )
                return None
            ...
            if final_len < initial_len:
                logger.debug(
                    f"Dropped {initial_len - final_len} rows with NaNs in OHLC from history."
                )

            if final_len < min_required_len_for_calc:
                logger.warning(
                    f"{Fore.YELLOW}Insufficient valid historical data after cleaning for {self.symbol}. Got {final_len} rows, need ~{min_required_len_for_calc} for full indicator calculation. Indicators might be inaccurate.{Style.RESET_ALL}"
                )
                if final_len == 0:
                    return None  # Cannot proceed if empty

            logger.debug(
                f"Fetched and processed {final_len} valid historical candles for {self.symbol}."
            )
            return df

        except ccxt.BadSymbol as e:
            logger.critical(
                f"{Fore.RED}BadSymbol fetching history for {self.symbol}: {e}. Aborting.{Style.RESET_ALL}"
            )
            sys.exit(1)
        except Exception as e:
            logger.error(
    ...
    def fetch_balance(self, currency_code: str | None = None) -> float | None:
        ...
        if not quote_currency:
            logger.error(
                f"{Fore.RED}[{func_name}] Cannot determine quote currency. Market Info: {self.market_info}.{Style.RESET_ALL}"
            )
            return None  # Indicate failure explicitly

        logger.debug(
            f"[{func_name}] Fetching available balance for {quote_currency}..."
        )

        if self.simulation_mode:
            dummy_balance = 10000.0
            logger.warning(
                f"{Fore.YELLOW}[SIMULATION][{func_name}] Returning dummy balance: {self.format_price(dummy_balance)} {quote_currency}{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if (
                quote_currency in balance_data
                and balance_data[quote_currency].get("free") is not None
            ):
                available_balance_str = str(balance_data[quote_currency]["free"])
                logger.debug(
                    f"[{func_name}] Found standard 'free' balance: {available_balance_str}"
                )
            ...
            with contextlib.suppress(ValueError, TypeError):
                is_free_zero = (
                    ...
                    or float(available_balance_str) < 1e-12
                )

            if is_free_zero and self.exchange_id.lower() == "bybit":
                logger.debug(
                    f"[{func_name}] Standard 'free' zero/missing. Checking Bybit 'info'..."
                )
                ...
                try:
                    ...
                    if result_list and isinstance(result_list[0], dict):
                        ...
                        logger.debug(
                            f"[{func_name}] Parsing Bybit 'info': Account Type = {account_type}"
                        )
                        ...
                        if account_type in ["UNIFIED", "CONTRACT"]:
                            ...
                            for coin in coin_list:
                                if (
                                    ...
                                    and coin.get("coin") == quote_currency
                                ):
                                    ...
                                    if avail_str is None or float(avail_str) < 1e-12:
                                        avail_str = coin.get(
                                            "availableBalance"
                                        )  # May include borrow? check docs
                                    if avail_str is None or float(avail_str) < 1e-12:
                                        avail_str = coin.get(
                                            "walletBalance"
                                        )  # Less ideal, might include unrealized PNL

                                    if avail_str is not None:
                                        available_balance_str = str(avail_str)
                                        logger.debug(
                                            f"[{func_name}] Using Bybit {account_type} coin balance ({quote_currency}): {available_balance_str}"
                                        )
                                        break
                            ...
                            for coin in coin_list:
                                if (
                                    ...
                                    if avail_str is not None:
                                        available_balance_str = str(avail_str)
                                        logger.debug(
                                            f"[{func_name}] Using Bybit SPOT coin balance ({quote_currency}): {available_balance_str}"
                                        )
                                        break
                    else:
                        logger.debug(
                            f"[{func_name}] Bybit 'info' structure not as expected."
                        )
                except (
                    AttributeError,
                    IndexError,
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Could not parse Bybit balance 'info': {e}. Raw: {str(info_data)[:200]}...{Style.RESET_ALL}"
                    )
            ...
            if (
                is_available_zero
                and quote_currency in balance_data
                and balance_data[quote_currency].get("total") is not None
            ):
                total_balance_str = str(balance_data[quote_currency]["total"])
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Available balance zero/missing. Using 'total' ({total_balance_str}) as fallback for {quote_currency}. (Includes used margin/collateral){Style.RESET_ALL}"
                )
            ...
            if available_balance_str is not None:
                try:
                    ...
                    if final_balance < 0:
                        logger.warning(
                            f"{Fore.YELLOW}[{func_name}] Fetched balance for {quote_currency} is negative ({final_balance}). Treating as 0.0.{Style.RESET_ALL}"
                        )
                        final_balance = 0.0
                    logger.info(
                        f"[{func_name}] Fetched available balance: {self.format_price(final_balance)} {quote_currency}"
                    )
                    return final_balance
                except (ValueError, TypeError):
                    logger.error(
                        f"{Fore.RED}[{func_name}] Could not convert final balance '{available_balance_str}' to float for {quote_currency}. Returning None.{Style.RESET_ALL}"
                    )
                    return None
            else:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Could not determine available balance for {quote_currency}. Returning None.{Style.RESET_ALL}"
                )
                return None  # Explicitly return None on failure

        except ccxt.AuthenticationError as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Authentication failed fetching balance: {e}.{Style.RESET_ALL}"
            )
            return None
        except Exception as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error fetching balance: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
    ...
    def fetch_order_status(
        ...
        if not order_id:
            logger.warning(f"[{func_name}] Called with empty order_id.")
            return None

        target_symbol = symbol or self.symbol
        logger.debug(
            f"[{func_name}] Fetching status for order {order_id} ({target_symbol})..."
        )
        ...
        if self.simulation_mode:
            for pos in self.open_positions:
                if pos["id"] == order_id:
                    logger.debug(
                        f"[SIMULATION][{func_name}] Returning cached status for order {order_id}."
                    )
                    ...
                    )  # Pass others like 'unknown', 'rejected'
                    sim_avg = (
                        pos.get("entry_price") if sim_status == STATUS_ACTIVE else None
                    )
                    ...
                    return {
                        ...
                        "datetime": pd.to_datetime(
                            sim_timestamp, unit="ms", utc=True
                        ).isoformat(),
            ...
            logger.warning(
                f"[SIMULATION][{func_name}] Simulated order {order_id} not found in state."
            )
        ...
        # --- Live Mode ---
            ...
            try:
                ...
                if self.exchange_id.lower() == "bybit":
                    market_type = self.exchange.market(target_symbol).get(
                        "type", "swap"
                    )
                    is_linear = self.exchange.market(target_symbol).get(
                        "linear", True
                ...
                order_info = self.exchange.fetch_order(
                    order_id, target_symbol, params=params
                )
                ...
                logger.debug(
                    f"[{func_name}] Order {order_id}: Status={status}, Filled={self.format_amount(filled)}, AvgPrice={self.format_price(avg_price)}"
                )
                return order_info

            except ccxt.OrderNotFound as e:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Order {order_id} ({target_symbol}) not found. Assumed closed/cancelled/invalid. Error: {e}{Style.RESET_ALL}"
                )
                return None
            except ccxt.ExchangeError as e:
                err_str = str(e).lower()
                ...
                if any(
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Order {order_id} reported as finished/filled/closed. Assuming final state. Error: {e}{Style.RESET_ALL}"
                    )
                    return None
                else:
                    logger.error(
                        f"{Fore.RED}[{func_name}] Exchange error fetching order {order_id}: {e}. Returning None.{Style.RESET_ALL}"
                    )
                    return None
            except Exception as e:
                logger.error(
                    f"{Fore.RED}[{func_name}] Unexpected error fetching order {order_id}: {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
                return None
    ...
    def calculate_volatility(close_prices: pd.Series, window: int) -> float | None:
        if close_prices is None or window <= 0 or len(close_prices) < window + 1:
            return None
        try:
            if (close_prices <= 0).any():
                return None  # Log returns require positive prices
            log_returns = np.log(close_prices / close_prices.shift(1))
            volatility = (
                log_returns.rolling(window=window, min_periods=window)
                .std(ddof=1)
                .iloc[-1]
            )
            return float(volatility) if pd.notna(volatility) else None
        except Exception as e:
            logger.error(f"Volatility calc error: {e}", exc_info=False)
            return None

    @staticmethod
    def calculate_ema(close_prices: pd.Series, period: int) -> float | None:
        if close_prices is None or period <= 0 or len(close_prices) < period:
            return None
        try:
            ema = (
                close_prices.ewm(span=period, adjust=False, min_periods=period)
                .mean()
                .iloc[-1]
            )
            return float(ema) if pd.notna(ema) else None
        except Exception as e:
            logger.error(f"EMA-{period} calc error: {e}", exc_info=False)
            return None

    @staticmethod
    def calculate_rsi(close_prices: pd.Series, period: int) -> float | None:
        if close_prices is None or period <= 0 or len(close_prices) < period + 1:
            return None
        try:
            ...
            logger.error(f"RSI-{period} calc error: {e}", exc_info=False)
            return None

    @staticmethod
    def calculate_macd(
        close_prices: pd.Series, short_p: int, long_p: int, signal_p: int
    ) -> tuple[float | None, float | None, float | None]:
        min_len = long_p + signal_p  # Rough estimate
        if (
            close_prices is None
            or not all(p > 0 for p in [short_p, long_p, signal_p])
            or short_p >= long_p
            or len(close_prices) < min_len
        ):
            return None, None, None
        try:
            ema_short = close_prices.ewm(
                span=short_p, adjust=False, min_periods=short_p
            ).mean()
            ema_long = close_prices.ewm(
                span=long_p, adjust=False, min_periods=long_p
            ).mean()
            ...
            logger.error(f"MACD calc error: {e}", exc_info=False)
            return None, None, None
    ...
    def calculate_rsi_series(close_prices: pd.Series, period: int) -> pd.Series | None:
        ...
        try:
            ...
            logger.error(f"RSI Series calc error: {e}", exc_info=False)
            return None

    @staticmethod
    def calculate_stoch_rsi(
        close_prices: pd.Series, rsi_p: int, stoch_p: int, k_p: int, d_p: int
    ) -> tuple[float | None, float | None]:
        min_len_est = rsi_p + stoch_p + max(k_p, d_p)  # Rough estimate
        if (
            close_prices is None
            or not all(p > 0 for p in [rsi_p, stoch_p, k_p, d_p])
            or len(close_prices) < min_len_est
        ):
            return None, None
        try:
            ...
            if len(rsi_series) < stoch_p + max(k_p, d_p):
                return None, None  # Check length after dropping NaNs
            ...
            stoch_rsi_raw = (
                100.0 * (rsi_series - min_rsi) / (max_rsi - min_rsi + 1e-12)
            ).clip(0, 100)
            ...
            if pd.isna(k_val) or pd.isna(d_val):
                return None, None
            return float(max(0.0, min(100.0, k_val))), float(
                max(0.0, min(100.0, d_val))
            )
        except Exception as e:
            logger.error(f"StochRSI calc error: {e}", exc_info=False)
            return None, None
    ...
    def calculate_atr(
        ...
        if (
            ...
            or not (
                ...
                and len(low_prices) >= period + 1
                and len(close_prices) >= period + 1
            )
        ):
            return None
        try:
            ...
            atr = (
                tr.ewm(com=period - 1, adjust=False, min_periods=period).mean().iloc[-1]
            )
            ...
            logger.error(f"ATR-{period} calc error: {e}", exc_info=False)
            return None
    ...
    def calculate_order_size(
        ...
        if self.market_info is None:
            logger.error(
                f"[{func_name}] Market info not loaded. Cannot calculate size."
            )
            return 0.0
        if current_price <= 1e-12:
            logger.error(
                f"[{func_name}] Current price ({current_price}) is zero or negative. Cannot calculate size."
            )
            return 0.0
        if not (0 < self.order_size_percentage <= 1):
            logger.error(
                f"[{func_name}] Invalid order_size_percentage ({self.order_size_percentage}). Must be > 0 and <= 1."
            )
            return 0.0
        ...
        try:
            ...
            logger.error(
                f"[{func_name}] Missing critical market info key: {e}. Cannot calculate size."
            )
            return 0.0
        except Exception as e:
            logger.error(
                f"[{func_name}] Error accessing market/precision details: {e}. Cannot calculate size."
            )
            return 0.0

        logger.debug(
            f"[{func_name}] Starting size calculation for {base_currency}/{quote_currency} at {self.format_price(current_price)}."
        )
        ...
        if balance is None or balance <= 1e-12:
            ...
            logger.info(
                f"[{func_name}] Insufficient balance or failed fetch. Balance: {balance}. Cannot place order."
            )
            return 0.0
        logger.debug(
            f"[{func_name}] Available balance: {self.format_price(balance)} {quote_currency}"
        )

        # --- 2. Calculate Base Order Size (Quote Value) ---
        target_quote_value = balance * self.order_size_percentage
        logger.debug(
            f"[{func_name}] Target Quote Value (Balance * %): {self.format_price(target_quote_value)} {quote_currency} (Risk %: {self.order_size_percentage * 100:.2f}%)"
        )
        ...
        if self.volatility_multiplier > 0:
            ...
            if volatility is not None and volatility > 1e-9:
                ...
                logger.info(
                    f"[{func_name}] Volatility ({volatility:.5f}) factor: {vol_adj_factor:.3f}. Total Factor: {adjustment_factor:.3f}"
                )
            else:
                logger.debug(
                    f"[{func_name}] Volatility adjustment skipped: Volatility N/A ({volatility})."
                )
        else:
            logger.debug(
                f"[{func_name}] Volatility adjustment disabled (multiplier={self.volatility_multiplier})."
            )

        # 3b. Signal Strength Adjustment
        if signal_score is not None and (
            self.strong_signal_adjustment_factor != 1.0
            or self.weak_signal_adjustment_factor != 1.0
        ):
            ...
            elif abs_score >= ENTRY_SIGNAL_THRESHOLD_ABS:
                sig_adj_factor = 1.0
                adj_type = "Normal"  # Normal entry = 1.0x unless configured otherwise
            else:
                sig_adj_factor = self.weak_signal_adjustment_factor
                adj_type = "Weak"  # Should not happen

            if abs(sig_adj_factor - 1.0) > 1e-9:
                adjustment_factor *= sig_adj_factor
                logger.info(
                    f"[{func_name}] Signal Score ({signal_score}, {adj_type}) factor: {sig_adj_factor:.3f}. Total Factor: {adjustment_factor:.3f}"
                )
            else:
                logger.debug(
                    f"[{func_name}] Signal Score ({signal_score}, {adj_type}) factor is {sig_adj_factor:.3f}, no adjustment."
                )
        else:
            logger.debug(f"[{func_name}] Signal score adjustment skipped.")

        # Clamp final adjustment factor
        adjustment_factor = max(
            0.05, min(2.5, adjustment_factor)
        )  # Clamp: 0.05x to 2.5x
        if abs(adjustment_factor - 1.0) > 1e-9:
            logger.debug(
                f"[{func_name}] Final clamped adjustment factor: {adjustment_factor:.3f}"
            )

        final_quote_value = target_quote_value * adjustment_factor
        if final_quote_value <= 1e-12:
            logger.warning(
                f"[{func_name}] Quote value zero/negative after adjustments. Cannot proceed."
            )
            return 0.0
        logger.debug(
            f"[{func_name}] Final adjusted quote value: {self.format_price(final_quote_value)} {quote_currency}"
        )

        # --- 4. Convert Quote Value to Base Amount ---
        order_size_base_raw = final_quote_value / current_price
        logger.debug(
            f"[{func_name}] Raw base amount: {self.format_amount(order_size_base_raw, amount_decimals + 4)} {base_currency}"
        )

        # --- 5. Apply Exchange Precision AND Check Limits ---
        try:
            amount_precise_str = self.exchange.amount_to_precision(
                self.symbol, order_size_base_raw
            )
            amount_precise = float(amount_precise_str)
            logger.debug(
                f"[{func_name}] Amount after precision ({amount_tick_size}): {self.format_amount(amount_precise)} {base_currency}"
            )

            if amount_precise <= 1e-12:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Amount zero after precision ({amount_tick_size}). Raw: {self.format_amount(order_size_base_raw, amount_decimals + 4)}. Cannot place order.{Style.RESET_ALL}"
                )
                return 0.0
            if min_amount is not None and amount_precise < min_amount:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Precise amount {self.format_amount(amount_precise)} {base_currency} < Min Amount {self.format_amount(min_amount)}. Cannot place order.{Style.RESET_ALL}"
                )
                return 0.0

            price_precise_str = self.exchange.price_to_precision(
            ...
            if min_cost is not None and estimated_cost < min_cost:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Estimated cost {self.format_price(estimated_cost)} {quote_currency} < Min Cost {self.format_price(min_cost)}. Cannot place order.{Style.RESET_ALL}"
                )
                return 0.0

            # --- Success ---
            logger.info(
                f"{Fore.CYAN}[{func_name}] Calculated final order size: {self.format_amount(amount_precise)} {base_currency} (Est. Cost: {self.format_price(estimated_cost)} {quote_currency}){Style.RESET_ALL}"
            )
            ...
            logger.error(
                f"{Fore.RED}[{func_name}] Error applying precision/limits: {type(e).__name__} - {e}. Raw: {self.format_amount(order_size_base_raw, amount_decimals + 4)}.{Style.RESET_ALL}"
            )
            return 0.0
        except Exception as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error in size precision/limit checks: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return 0.0
    ...
    def _calculate_sl_tp_prices(
        ...
        if self.market_info is None or entry_price <= 1e-12:
            logger.error(
                f"[{func_name}] Market info missing or invalid entry price ({entry_price})."
            )
            return None, None
        ...
        if self.use_atr_sl_tp:
            if atr is None or atr <= 1e-12:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] ATR SL/TP enabled, but ATR invalid ({atr}). Cannot calculate.{Style.RESET_ALL}"
                )
                return None, None
            if not (self.atr_sl_multiplier > 0 and self.atr_tp_multiplier > 0):
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] ATR SL/TP enabled, but multipliers invalid (SL={self.atr_sl_multiplier}, TP={self.atr_tp_multiplier}). Cannot calculate.{Style.RESET_ALL}"
                )
                return None, None
            logger.debug(
                f"[{func_name}] Calculating SL/TP using ATR={self.format_price(atr)}, SL Mult={self.atr_sl_multiplier}, TP Mult={self.atr_tp_multiplier}"
            )
            sl_delta, tp_delta = (
                atr * self.atr_sl_multiplier,
                atr * self.atr_tp_multiplier,
            )
            stop_loss_price_raw = (
                entry_price - sl_delta if side == "buy" else entry_price + sl_delta
            )
            take_profit_price_raw = (
                entry_price + tp_delta if side == "buy" else entry_price - tp_delta
            )
            ...
            if not (sl_pct and tp_pct and 0 < sl_pct < 1 and 0 < tp_pct < 1):
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Fixed % SL/TP enabled, but percentages invalid (SL={sl_pct}, TP={tp_pct}). Cannot calculate.{Style.RESET_ALL}"
                )
                return None, None
            logger.debug(
                f"[{func_name}] Calculating SL/TP using Fixed %: SL={sl_pct * 100:.2f}%, TP={tp_pct * 100:.2f}%"
            )
            stop_loss_price_raw = (
        ...
        try:
            ...
            if stop_loss_price_raw is not None and stop_loss_price_raw > 1e-12:
                sl_precise = float(
                    self.exchange.price_to_precision(self.symbol, stop_loss_price_raw)
                )
                if (side == "buy" and sl_precise < entry_price) or (
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] SL {self.format_price(sl_precise)} invalid vs entry {self.format_price(entry_price)}. SL set to None.{Style.RESET_ALL}"
                    )
            elif stop_loss_price_raw is not None:
                logger.warning(
                    f"[{func_name}] Raw SL price ({stop_loss_price_raw}) zero/negative. SL set to None."
                )

            # Process TP
            if take_profit_price_raw is not None and take_profit_price_raw > 1e-12:
                tp_precise = float(
                    self.exchange.price_to_precision(self.symbol, take_profit_price_raw)
                )
                if (side == "buy" and tp_precise > entry_price) or (
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] TP {self.format_price(tp_precise)} invalid vs entry {self.format_price(entry_price)}. TP set to None.{Style.RESET_ALL}"
                    )
            elif take_profit_price_raw is not None:
                logger.warning(
                    f"[{func_name}] Raw TP price ({take_profit_price_raw}) zero/negative. TP set to None."
                )

        except (ccxt.ExchangeError, ValueError, TypeError) as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Error applying precision to SL/TP: {e}. Raw SL={stop_loss_price_raw}, TP={take_profit_price_raw}. Setting both None.{Style.RESET_ALL}"
            )
            return None, None
        except Exception as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error in SL/TP precision: {e}. Both set to None.{Style.RESET_ALL}",
                exc_info=True,
            )
            return None, None
        ...
        if stop_loss_price_final is not None and take_profit_price_final is not None:
            if (side == "buy" and stop_loss_price_final >= take_profit_price_final) or (
                ...
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Final SL {self.format_price(stop_loss_price_final)} conflicts with TP {self.format_price(take_profit_price_final)}. Setting TP to None.{Style.RESET_ALL}"
                )
                take_profit_price_final = None

        logger.debug(
            f"[{func_name}] Final calculated SL={self.format_price(stop_loss_price_final)}, TP={self.format_price(take_profit_price_final)}"
        )
    ...
    def compute_trade_signal_score(
        ...
        if orderbook_imbalance is not None:
            ...
            if self.imbalance_threshold <= 0:
                reason = "[ 0.0] OB Invalid Threshold"
            elif imb == float("inf"):
                score -= 1.0
                reason = f"{Fore.RED}[-1.0] OB Sell (Imb: Inf){Style.RESET_ALL}"
            else:
                imb_buy_thresh = 1.0 / self.imbalance_threshold
                if imb < imb_buy_thresh:
                    score += 1.0
                    reason = f"{Fore.GREEN}[+1.0] OB Buy (Imb < {imb_buy_thresh:.2f}){Style.RESET_ALL}"
                elif imb > self.imbalance_threshold:
                    score -= 1.0
                    reason = f"{Fore.RED}[-1.0] OB Sell (Imb > {self.imbalance_threshold:.2f}){Style.RESET_ALL}"
                else:
                    reason = f"{Fore.WHITE}[ 0.0] OB Neutral{Style.RESET_ALL}"
        else:
            reason = f"{Fore.WHITE}[ 0.0] OB N/A{Style.RESET_ALL}"
        ...
        if ema is not None and ema > 1e-9:
            if price > ema * (1 + EMA_THRESH_MULT):
                score += 1.0
                reason = f"{Fore.GREEN}[+1.0] Price > EMA{Style.RESET_ALL}"
            elif price < ema * (1 - EMA_THRESH_MULT):
                score -= 1.0
                reason = f"{Fore.RED}[-1.0] Price < EMA{Style.RESET_ALL}"
            else:
                reason = f"{Fore.WHITE}[ 0.0] Price ~ EMA{Style.RESET_ALL}"
        else:
            reason = f"{Fore.WHITE}[ 0.0] EMA N/A{Style.RESET_ALL}"
        ...
        if rsi is not None:
            if rsi < RSI_OS:
                score += 1.0
                reason = f"{Fore.GREEN}[+1.0] RSI Oversold (<{RSI_OS}){Style.RESET_ALL}"
            elif rsi > RSI_OB:
                score -= 1.0
                reason = f"{Fore.RED}[-1.0] RSI Overbought (>{RSI_OB}){Style.RESET_ALL}"
            else:
                reason = f"{Fore.WHITE}[ 0.0] RSI Neutral{Style.RESET_ALL}"
        else:
            reason = f"{Fore.WHITE}[ 0.0] RSI N/A{Style.RESET_ALL}"
        ...
        macd_str = (
            f"L:{self.format_price(macd_line, self.price_decimals + 1)}/S:{self.format_price(macd_signal, self.price_decimals + 1)}"
        ...
        if macd_line is not None and macd_signal is not None:
            if macd_line > macd_signal:
                score += 1.0
                reason = f"{Fore.GREEN}[+1.0] MACD Line > Signal{Style.RESET_ALL}"
            else:
                score -= 1.0
                reason = f"{Fore.RED}[-1.0] MACD Line <= Signal{Style.RESET_ALL}"
        else:
            reason = f"{Fore.WHITE}[ 0.0] MACD N/A{Style.RESET_ALL}"
        ...
        if stoch_k is not None and stoch_d is not None:
            if stoch_k < STOCH_OS and stoch_d < STOCH_OS:
                score += 1.0
                reason = f"{Fore.GREEN}[+1.0] StochRSI Oversold (<{STOCH_OS}){Style.RESET_ALL}"
            elif stoch_k > STOCH_OB and stoch_d > STOCH_OB:
                score -= 1.0
                reason = f"{Fore.RED}[-1.0] StochRSI Overbought (>{STOCH_OB}){Style.RESET_ALL}"
            else:
                reason = f"{Fore.WHITE}[ 0.0] StochRSI Neutral{Style.RESET_ALL}"
        else:
            reason = f"{Fore.WHITE}[ 0.0] StochRSI N/A{Style.RESET_ALL}"
        reasons.append(f"{reason} ({stoch_str})")

        # --- Final Score ---
        final_score = int(round(score))
        logger.debug(
            f"[{func_name}] Raw Score: {score:.2f}, Final Integer Score: {final_score}"
        )
    ...
    def place_entry_order(
        ...
        if self.market_info is None:
            logger.error(f"[{func_name}] Market info missing.")
            return None
        if order_size_base <= 0:
            logger.error(f"[{func_name}] Invalid order size {order_size_base}.")
            return None
        if current_price <= 1e-12:
            logger.error(f"[{func_name}] Invalid current price {current_price}.")
            return None
        ...
        try:
            ...
            logger.debug(
                f"[{func_name}] Using validated precise amount: {self.format_amount(amount_precise)} {base_currency}"
            )
            ...
            if order_type == "limit":
                ...
                if limit_price_raw <= 1e-12:
                    raise ValueError("Calculated limit price zero/negative")
                limit_price = float(
                    self.exchange.price_to_precision(self.symbol, limit_price_raw)
                )
                logger.debug(
                    f"[{func_name}] Calculated precise limit price: {self.format_price(limit_price)}"
                )
                if (side == "buy" and limit_price >= current_price) or (
                    side == "sell" and limit_price <= current_price
                ):
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Limit price {self.format_price(limit_price)} not favorable vs current {self.format_price(current_price)} for {side}. Check offset.{Style.RESET_ALL}"
                    )

            # Add Bybit V5 SL/TP Params (as strings)
            if stop_loss_price is not None:
                params["stopLoss"] = self.format_price(
                    stop_loss_price
                )  # Format to string with correct decimals
            ...
            if self.exchange_id.lower() == "bybit":
                ...
                if "category" in params:
                    logger.debug(
                        f"[{func_name}] Added Bybit category: {params['category']}"
                    )

        except (ccxt.ExchangeError, ValueError, TypeError, KeyError) as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Error preparing order values/params: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
        except Exception as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error preparing order: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
        ...
        limit_info = (
            f"Limit={self.format_price(limit_price)}"
            if limit_price
            else f"Market (~{self.format_price(current_price)})"
        )
        ...
        if self.simulation_mode:
            ...
            sim_status = (
                "open" if order_type == "limit" else "closed"
            )  # Market assumed filled
            ...
            simulated_order = {
                "id": sim_id,
                "timestamp": sim_ts,
                "datetime": pd.to_datetime(sim_ts, unit="ms", utc=True).isoformat(),
                "symbol": self.symbol,
                "type": order_type,
                "side": side,
                "status": sim_status,
                ...
                "filled": sim_filled,
                "remaining": amount_precise - sim_filled,
                "average": sim_avg,
                "cost": sim_filled * sim_avg if sim_avg else 0.0,
            ...
            try:
                if order_type == "market":
                    ...
                    if limit_price is None:
                        raise ValueError("Limit price is None for limit order")
                ...
                if order:
                    ...
                    logger.error(
                        f"{Fore.RED}LIVE {action_desc} order placement API call returned None. Check exchange status/logs.{Style.RESET_ALL}"
                    )
                    return None

            except (ccxt.InsufficientFunds, ccxt.InvalidOrder) as e:
                logger.error(
                    f"{Fore.RED}LIVE {action_desc} Failed ({type(e).__name__}): {e}{Style.RESET_ALL}"
                )
                if isinstance(e, ccxt.InsufficientFunds):
                    self.fetch_balance(quote_currency)  # Log current balance
                return None
            except ccxt.ExchangeError as e:
                logger.error(
                    f"{Fore.RED}LIVE {action_desc} Failed (ExchangeError): {e}{Style.RESET_ALL}"
                )
                return None
            except Exception as e:
                logger.error(
                    f"{Fore.RED}LIVE {action_desc} Failed (Unexpected Python Error): {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
                return None
    ...
    def cancel_order_by_id(self, order_id: str, symbol: str | None = None) -> bool:
        ...
        if not order_id:
            logger.warning(f"[{func_name}] Empty order_id.")
            return False
        ...
        if self.simulation_mode:
            ...
            try:
                ...
                logger.info(
                    f"{Fore.YELLOW}---> Cancellation initiated for order {order_id}. Response: {str(response)[:100]}...{Style.RESET_ALL}"
                )
                return True
            except ccxt.OrderNotFound:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Order {order_id} not found (already closed/cancelled?). Treating as success.{Style.RESET_ALL}"
                )
                return True
            except ccxt.NetworkError as e:
                raise e  # Let retry handle
            except ccxt.ExchangeError as e:
                err_str = str(e).lower()
                if any(
                    p in err_str
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Cannot cancel {order_id}: Already final state. Error: {e}{Style.RESET_ALL}"
                    )
                    return True  # Treat as 'success' (no action needed)
                else:
                    logger.error(
                        f"{Fore.RED}[{func_name}] Exchange error cancelling {order_id}: {e}{Style.RESET_ALL}"
                    )
                    return False
            except Exception as e:
                logger.error(
                    f"{Fore.RED}[{func_name}] Unexpected error cancelling {order_id}: {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
                return False
    ...
    def cancel_all_symbol_orders(self, symbol: str | None = None) -> int:
        ...
        target_symbol = symbol or self.symbol
        logger.info(
            f"{Fore.YELLOW}Attempting to cancel all OPEN orders for {target_symbol}...{Style.RESET_ALL}"
        )
        cancelled_count = 0

        if self.simulation_mode:
            logger.info(
                f"[SIMULATION][{func_name}] Simulating cancel all for {target_symbol}."
            )
            ...
            try:
                ...
                if self.exchange.has["cancelAllOrders"]:
                    logger.debug(
                        f"[{func_name}] Using unified 'cancelAllOrders' for {target_symbol}..."
                    )
                    response = self.exchange.cancel_all_orders(
                        target_symbol, params=params
                    )
                    logger.info(
                        f"[{func_name}] 'cancelAllOrders' response: {str(response)[:100]}..."
                    )
                ...
                # Fallback to fetch + individual cancel (more reliable feedback)
                    ...
                    logger.warning(
                        f"[{func_name}] 'cancelAllOrders' unavailable, using fetchOpenOrders + individual cancel for {target_symbol}..."
                    )
                    ...
                    if not open_orders:
                        logger.info(
                            f"[{func_name}] No open orders found via fetchOpenOrders for {target_symbol}."
                        )
                        return 0

                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Found {len(open_orders)} open order(s). Cancelling individually...{Style.RESET_ALL}"
                    )
                    for order in open_orders:
                        if self.cancel_order_by_id(order["id"], target_symbol):
                            cancelled_count += 1
                            time.sleep(
                                max(0.2, self.exchange.rateLimit / 1000)
                            )  # Rate limit delay
                        else:
                            logger.error(
                                f"[{func_name}] Failed to cancel order {order['id']} during bulk attempt."
                            )
                else:
                    logger.error(
                        f"{Fore.RED}[{func_name}] Exchange lacks 'cancelAllOrders' and 'fetchOpenOrders'. Cannot cancel automatically.{Style.RESET_ALL}"
                    )
                    return 0

                final_msg = (
                    f"[{func_name}] Bulk cancel process finished for {target_symbol}. "
                )
                final_msg += f"Result: {cancelled_count}" + (
                    " (unknown count)" if cancelled_count == -1 else " orders"
                )
                ...
                logger.error(
                    f"{Fore.RED}[{func_name}] Error during bulk cancel for {target_symbol}: {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
    ...
    def _check_pending_entries(self, indicators: dict) -> None:
        ...
        logger.debug(
            f"[{func_name}] Checking status of {len(pending_positions)} pending entry order(s)..."
        )
        ...
        for position in pending_positions:
            ...
            if not entry_order_id:
                logger.error(f"[{func_name}] Pending position missing ID: {position}")
                continue
            ...
            if order_info is None:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Pending order {entry_order_id} not found. Assuming closed/cancelled externally. Removing.{Style.RESET_ALL}"
                )
            ...
            if order_status == "closed" and filled_amount > 1e-12:
                if entry_price is None or float(entry_price) <= 1e-12:
                    logger.error(
                        f"{Fore.RED}[{func_name}] Order {entry_order_id} 'closed' but invalid fill price ({entry_price}). Cannot activate. Removing.{Style.RESET_ALL}"
                    )
                    positions_to_remove_ids.add(entry_order_id)
                    continue
                ...
                if orig_size is not None and abs(filled_amount - orig_size) > max(
                    ...
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Filled amount {self.format_amount(filled_amount)} differs from requested {self.format_amount(orig_size)}. Using actual.{Style.RESET_ALL}"
                    )

                logger.info(
                    f"{Fore.GREEN}---> Pending entry order {entry_order_id} FILLED! Amount: {self.format_amount(filled_amount)}, Avg Price: {self.format_price(entry_price)}{Style.RESET_ALL}"
                )
                ...
                fill_time_ms = order_info.get("lastTradeTimestamp") or order_info.get(
                    "timestamp"
                )
                updated_pos["entry_time"] = (
                    fill_time_ms / 1000 if fill_time_ms else time.time()
                )
                ...
                if current_price_for_check:
                    ...
                    sl_price, tp_price = self._calculate_sl_tp_prices(
                        ...
                        current_price=current_price_for_check,
                        atr=atr_value,
                    )
                    updated_pos["stop_loss_price"] = sl_price
                    updated_pos["take_profit_price"] = tp_price
                    logger.info(
                        f"[{func_name}] Stored internal SL={self.format_price(sl_price)}, TP={self.format_price(tp_price)} for activated pos {entry_order_id}."
                    )
                else:
                    logger.warning(
                        f"[{func_name}] Could not fetch price for SL/TP recalc after fill for {entry_order_id}."
                    )
            ...
            # Case 3: Order Failed (canceled, rejected, expired)
                ...
                reason = order_info.get("info", {}).get(
                    "rejectReason",
                    order_info.get("info", {}).get("cancelType", order_status),
                )
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Pending order {entry_order_id} failed (Status: {order_status}, Reason: {reason}). Removing.{Style.RESET_ALL}"
                )
                positions_to_remove_ids.add(entry_order_id)
            ...
            # Case 4: Still Open or Partially Filled
                ...
                logger.debug(
                    f"[{func_name}] Pending order {entry_order_id} still 'open'. Filled: {self.format_amount(filled_amount)}. Waiting..."
                )
            ...
            # Case 5: Closed but zero filled (treat as cancelled/failed)
                ...
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Pending order {entry_order_id} 'closed' but zero filled. Assuming cancelled pre-fill. Removing.{Style.RESET_ALL}"
                )
                positions_to_remove_ids.add(entry_order_id)

            else:  # Unexpected status
                logger.warning(
                    f"[{func_name}] Pending order {entry_order_id} has unexpected status: {order_status}. Filled: {self.format_amount(filled_amount)}. Leaving pending."
                )
        ...
        if positions_to_remove_ids or positions_to_update_data:
            ...
            for pos in self.open_positions:
                ...
                if pos_id in positions_to_update_data:
                    ...
                    if (
                        pos["status"] == STATUS_PENDING_ENTRY
                        and positions_to_update_data[pos_id]["status"] == STATUS_ACTIVE
                    ):
                        new_positions_list.append(
                            positions_to_update_data[pos_id]
                        )  # Full replace
                        updated_count += 1
                    else:
                        pos.update(
                            positions_to_update_data[pos_id]
                        )  # Partial update (e.g., timestamp)
            ...
            if removed_count > 0 or updated_count > 0:
                ...
                logger.debug(
                    f"[{func_name}] Checks complete. Activated: {updated_count}, Removed: {removed_count}. Total positions: {len(self.open_positions)}"
                )
                self._save_state()
    ...
    def _manage_active_positions(self, current_price: float, indicators: dict) -> None:
        ...
        active_positions = [
            pos for pos in self.open_positions if pos.get("status") == STATUS_ACTIVE
        ]
        if not active_positions:
            return

        logger.debug(
            f"[{func_name}] Managing {len(active_positions)} active position(s) against price: {self.format_price(current_price)}..."
        )
        ...
        for position in active_positions:
            ...
            if not pos_id:
                logger.error(f"[{func_name}] Active position missing ID: {position}")
                continue
            ...
            if not all([symbol, side, entry_price, position_size, entry_time]):
                logger.error(
                    f"[{func_name}] Active pos {pos_id} missing essential data. Skipping."
                )
                continue
            if position_size <= 1e-12:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Active pos {pos_id} size ~0. Assuming closed. Removing.{Style.RESET_ALL}"
                )
            ...
            if order_info is None:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Active pos main order {pos_id} not found. Assuming closed externally. Removing.{Style.RESET_ALL}"
                )
                ...
                if (
                    order_remaining <= 1e-9
                ):  # Check if order is fully settled (no remaining amount)
                    logger.info(
                        f"[{func_name}] Order {pos_id} status '{order_status}' with near-zero remaining. Inferring external closure."
                    )
                    ...
                    exit_reason = self._infer_exit_reason(
                        order_info, position, exit_price, current_price
                    )
                    log_color = (
                        Fore.RED
                        if "SL" in exit_reason
                        else (Fore.GREEN if "TP" in exit_reason else Fore.YELLOW)
                    )
                    logger.info(
                        f"{log_color}{exit_reason} at ~{self.format_price(exit_price)}{Style.RESET_ALL}"
                    )
                else:
                    # Order status not 'open' but has remaining amount? Unusual state.
                    logger.warning(
                        f"[{func_name}] Order {pos_id} has status '{order_status}' but non-zero remaining ({order_remaining}). State unclear, skipping exit check based on this."
                    )
            ...
            if not exit_reason:
                ...
                if (
                    self.time_based_exit_minutes is not None
                    ...
                    if time_elapsed_min >= self.time_based_exit_minutes:
                        logger.info(
                            f"{Fore.YELLOW}[{func_name}] Time limit ({self.time_based_exit_minutes} min) reached for {pos_id} (Age: {time_elapsed_min:.1f}m). Initiating market close.{Style.RESET_ALL}"
                        )
                        ...
                        if market_close_order:
                            exit_reason = (
                                f"Time Limit ({self.time_based_exit_minutes}min)"
                            )
                            exit_price = (
                                market_close_order.get("average") or current_price
                            )  # Use fill price if available
                            logger.info(
                                f"[{func_name}] Market close for time exit placed for {pos_id}. Exit ~{self.format_price(exit_price)}"
                            )
                        else:
                            logger.critical(
                                f"{Fore.RED}{Style.BRIGHT}[{func_name}] CRITICAL FAILURE closing timed-out position {pos_id}. REMAINS OPEN! Manual action required!{Style.RESET_ALL}"
                            )
                ...
                if not exit_reason and self.enable_trailing_stop_loss:
                    if not self.trailing_stop_loss_percentage or not (
                        0 < self.trailing_stop_loss_percentage < 1
                    ):
                        logger.error(
                            f"[{func_name}] TSL enabled but percentage invalid. Disabling TSL this cycle."
                        )
                    else:
                        new_tsl_price = self._update_trailing_stop_price(
                            position, current_price
                        )
                        if new_tsl_price is not None:
                            logger.warning(
                                f"{Fore.YELLOW}[EXPERIMENTAL] TSL update for {pos_id}. Attempting edit_order to SL={self.format_price(new_tsl_price)}. VERIFY SUPPORT!{Style.RESET_ALL}"
                            )
                            edit_success = self._attempt_edit_order_for_tsl(
                                position, new_tsl_price
                            )
                            if edit_success:
                                update_payload = {
                                ...
                                current_updates = positions_to_update_state.get(
                                    pos_id, {}
                                )
                                ...
                                logger.info(
                                    f"{Fore.MAGENTA}Internal TSL state for {pos_id} updated to {self.format_price(new_tsl_price)} after edit attempt.{Style.RESET_ALL}"
                                )
                            else:
                                logger.error(
                                    f"{Fore.RED}TSL update via edit_order for {pos_id} failed/skipped. TSL state NOT updated.{Style.RESET_ALL}"
                                )
        ...
        if positions_to_remove_ids or positions_to_update_state:
            ...
            if removed_count > 0 or updated_count > 0:
                self.open_positions = new_positions_list
                logger.debug(
                    f"[{func_name}] Management complete. Updated: {updated_count}, Removed/Closed: {removed_count}. Total: {len(self.open_positions)}"
                )
                self._save_state()

    def _infer_exit_reason(
        self, order_info: dict, position: dict, exit_price: float, current_price: float
    ) -> str:
        ...
        bybit_close_type = info.get(
            "closeOnTrigger", False
        )  # Check if it was a closing order

        # Use Bybit specific info if available
        if (
            "stop-loss order triggered" in bybit_status
            or bybit_stop_order_type == "stoploss"
        ):
            reason = f"SL Triggered (Exchange: {pos_id})"
        elif (
            "take-profit order triggered" in bybit_status
            or bybit_stop_order_type == "takeprofit"
        ):
            reason = f"TP Triggered (Exchange: {pos_id})"
        elif "cancel" in bybit_status or order_status == "canceled":
            reason = f"Cancelled Externally ({pos_id})"
        elif bybit_close_type:
            reason = f"Closed via Trigger Order ({pos_id})"  # ADL, Liquidation might also appear here?
        # Fallback to price comparison
            ...
            if stored_sl is not None:
                sl_triggered = (
                    side == "buy" and exit_price <= stored_sl + tolerance
                ) or (side == "sell" and exit_price >= stored_sl - tolerance)
            if stored_tp is not None:
                tp_triggered = (
                    side == "buy" and exit_price >= stored_tp - tolerance
                ) or (side == "sell" and exit_price <= stored_tp + tolerance)

            if sl_triggered and tp_triggered:
                reason = f"SL/TP Hit (Price Ambiguous: {pos_id})"
            elif sl_triggered:
                reason = f"SL Hit (Inferred Price: {pos_id})"
            elif tp_triggered:
                reason = f"TP Hit (Inferred Price: {pos_id})"
            else:
                reason = f"Closed Externally (Reason Unclear: {pos_id}, Status: {order_status}/{bybit_status})"

        return reason

    def _update_trailing_stop_price(
        ...
        try:
            potential_tsl = float(
                self.exchange.price_to_precision(self.symbol, potential_tsl_raw)
            )
            if potential_tsl <= 0:
                potential_tsl = None
        except Exception as e:
            logger.error(
                f"[{func_name}] Error formatting potential TSL {potential_tsl_raw}: {e}"
            )
            potential_tsl = None
        if potential_tsl is None:
            return None
        ...
        if not is_tsl_active:
            ...
            price_tick = float(self.market_info["precision"]["price"] or 0.0001)
            activation_buffer = max(
                entry_price * 0.001, price_tick * 5
            )  # 0.1% or 5 ticks
            activation_price = (
                entry_price + activation_buffer
                if side == "buy"
                else entry_price - activation_buffer
            )

            price_moved_enough = (
                side == "buy" and current_price > activation_price
            ) or (side == "sell" and current_price < activation_price)
            ...
            if price_moved_enough:
                ...
                if base_sl is not None:
                    is_better_than_base = (
                        side == "buy" and potential_tsl > base_sl
                    ) or (side == "sell" and potential_tsl < base_sl)

                if is_better_than_base:
                    new_tsl_price = potential_tsl
                    logger.info(
                        f"{Fore.MAGENTA}[{func_name}] TSL ACTIVATING for {side} {position['id']} at {self.format_price(new_tsl_price)} (Price: {self.format_price(current_price)}){Style.RESET_ALL}"
                    )
                else:
                    logger.debug(
                        f"[{func_name}] TSL activation condition met for {position['id']}, but potential TSL {self.format_price(potential_tsl)} not better than base SL {self.format_price(base_sl)}. No activation."
                    )
        ...
        # TSL Update Check (if already active)
            ...
            if update_needed:
                new_tsl_price = potential_tsl
                logger.info(
                    f"{Fore.MAGENTA}[{func_name}] TSL UPDATING for {side} {position['id']} from {self.format_price(current_tsl)} to {self.format_price(new_tsl_price)} (Price: {self.format_price(current_price)}){Style.RESET_ALL}"
                )
    ...
    def _attempt_edit_order_for_tsl(self, position: dict, new_tsl_price: float) -> bool:
        ...
        logger.warning(
            f"{Fore.YELLOW}[EXPERIMENTAL][{func_name}] Attempting TSL update via edit_order for {pos_id}. THIS IS EXPERIMENTAL AND MAY NOT WORK RELIABLY.{Style.RESET_ALL}"
        )
        ...
        try:
            ...
            if stored_tp_price:
                ...
                logger.debug(
                    f"[{func_name}] Resending TP={edit_params['takeProfit']} with SL update."
                )
            else:
                logger.debug(
                    f"[{func_name}] No existing TP found for {pos_id}, not resending TP."
                )
            ...
            if not fresh_order_info or fresh_order_info.get("status") != "open":
                logger.warning(
                    f"[{func_name}] Cannot edit TSL: Order {pos_id} not found or not 'open' (Status: {fresh_order_info.get('status')})."
                )
                return False
            ...
            if not all([order_type, order_side, order_amount]):
                raise ValueError(
                    "Missing essential fields in fresh order info for edit."
                )
            if order_type == "limit" and order_price is None:
                raise ValueError("Missing price in fresh limit order info for edit.")

            logger.debug(
                f"[{func_name}] Calling edit_order for {pos_id}. New SL: {new_tsl_price}. Params: {edit_params}"
            )
            ...
            if edited_order:
                confirmed_sl = edited_order.get("info", {}).get(
                    "stopLoss"
                ) or edited_order.get("stopLossPrice")
                confirmed_tp = edited_order.get("info", {}).get(
                    "takeProfit"
                ) or edited_order.get("takeProfitPrice")
                logger.info(
                    f"{Fore.MAGENTA}---> edit_order attempt for {pos_id} complete. Exchange Response SL: {confirmed_sl}, TP: {confirmed_tp}{Style.RESET_ALL}"
                )
                ...
                if confirmed_sl and abs(float(confirmed_sl) - new_tsl_price) < 1e-9:
                    logger.info(
                        f"{Fore.GREEN}---> TSL edit CONFIRMED via response SL for {pos_id}.{Style.RESET_ALL}"
                    )
                    return True  # High confidence
                else:
                    logger.warning(
                        f"{Fore.YELLOW}---> TSL edit response SL ({confirmed_sl}) mismatch target ({new_tsl_price}). Edit *may* have failed or confirmation lag.{Style.RESET_ALL}"
                    )
                ...
                logger.error(
                    f"{Fore.RED}[{func_name}] TSL update via edit_order failed (API returned None) for {pos_id}.{Style.RESET_ALL}"
                )
                return False

        except ccxt.NotSupported as e:
            logger.error(
                f"{Fore.RED}[{func_name}] TSL update FAILED: edit_order for SL/TP mod NOT SUPPORTED. Error: {e}. Disable TSL or use alternative.{Style.RESET_ALL}"
            )
            ...
            logger.error(
                f"{Fore.RED}[{func_name}] TSL edit_order failed ({type(e).__name__}): {e}.{Style.RESET_ALL}"
            )
            return False
        except Exception as e:
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error attempting TSL edit for {pos_id}: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return False

    def _log_position_pnl(
        self, position: dict, exit_price: float | None, reason: str
    ) -> None:
        ...
        if not all([pos_id, side, entry_price, position_size, exit_price, symbol]):
            logger.warning(
                f"{Fore.YELLOW}---> Pos {pos_id} Closed ({reason}). PnL calc failed: Missing data (E={entry_price}, X={exit_price}, S={position_size}).{Style.RESET_ALL}"
            )
            return
        if entry_price <= 0 or exit_price <= 0 or position_size <= 0:
            logger.warning(
                f"{Fore.YELLOW}---> Pos {pos_id} Closed ({reason}). PnL calc failed: Invalid data (E={entry_price}, X={exit_price}, S={position_size}).{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Daily PnL Updated: {self.format_price(self.daily_pnl)} {quote_ccy}"
            )

        except ZeroDivisionError:
            logger.error(
                f"[{func_name}] PnL calc failed for {pos_id}: Division by zero."
            )
        except Exception as e:
            logger.error(
                f"[{func_name}] Unexpected error calculating PnL for {pos_id}: {e}",
                exc_info=True,
            )
    ...
    def _place_market_close_order(
        ...
        logger.warning(
            f"{log_color}[{func_name}] Initiating MARKET CLOSE for {pos_id} ({symbol}). "
        ...
        if self.simulation_mode:
            ...
            logger.info(
                f"{log_color}[SIMULATION] Market Close Order: ID {sim_close_id}, AvgPrice {self.format_price(sim_avg_close)}{Style.RESET_ALL}"
            )
            ...
            try:
                ...
                logger.debug(
                    f"[{func_name}] Placing live market close for {pos_id}. Side: {close_side}, Size: {size}, Params: {params}"
                )
                order = self.exchange.create_market_order(
                    symbol, close_side, size, params=params
                )
                ...
                if order:
                    ...
                    logger.error(
                        f"{Fore.RED}[{func_name}] LIVE Market Close failed: API returned None for {pos_id}.{Style.RESET_ALL}"
                    )
                    return None

            except (ccxt.InsufficientFunds, ccxt.InvalidOrder) as e:
                logger.error(
                    f"{Fore.RED}[{func_name}] LIVE Close Failed for {pos_id} ({type(e).__name__}): {e}. Position already closed or size/params wrong?{Style.RESET_ALL}"
                )
                return None
            except ccxt.ExchangeError as e:
                err_str = str(e).lower()
                if (
                    "order cost not meet" in err_str
                    or "position size is zero" in err_str
                    or "reduce-only" in err_str
                    or "position idx not match position mode" in err_str
                ):
                    logger.warning(
                        f"{Fore.YELLOW}[{func_name}] Market close for {pos_id} failed, likely already closed or reduce-only issue. Error: {e}{Style.RESET_ALL}"
                    )
                    return None  # Treat as closed potentially
                else:
                    logger.error(
                        f"{Fore.RED}[{func_name}] LIVE Close Failed for {pos_id} (ExchangeError): {e}{Style.RESET_ALL}"
                    )
                    return None
            except Exception as e:
                logger.error(
                    f"{Fore.RED}[{func_name}] LIVE Close Failed for {pos_id} (Unexpected Error): {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
                return None
    ...
    def _fetch_market_data(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            if current_price is None:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Failed to fetch current price. Skipping iteration.{Style.RESET_ALL}"
                )
                return None
            if historical_data is None or historical_data.empty:
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Failed to fetch sufficient historical data. Skipping iteration.{Style.RESET_ALL}"
                )
                return None
            ...
            logger.error(
                f"{Fore.RED}[{func_name}] Unexpected error: {e}{Style.RESET_ALL}",
                exc_info=True,
            )
            return None
    ...
    def _calculate_indicators(self, historical_data: pd.DataFrame) -> dict[str, Any]:
        ...
        if not isinstance(historical_data, pd.DataFrame) or historical_data.empty:
            logger.error(f"[{func_name}] Invalid historical data.")
            return {}
        required_cols = ["open", "high", "low", "close", "volume"]
        if not all(col in historical_data.columns for col in required_cols):
            logger.error(f"[{func_name}] History missing columns ({required_cols}).")
            return {}
        if len(historical_data) < 2:
            logger.warning(
                f"[{func_name}] Very short history ({len(historical_data)} rows)."
            )
        ...
        if nones:
            logger.warning(
                f"{Fore.YELLOW}[{func_name}] Indicators returning None: {nones}. Check data/periods.{Style.RESET_ALL}"
            )
        return indicators
    ...
    def _process_signals_and_entry(self, market_data: dict, indicators: dict) -> None:
        ...
        active_or_pending_count = sum(
            ...
            if p.get("status") in [STATUS_ACTIVE, STATUS_PENDING_ENTRY]
        )
        if active_or_pending_count >= self.max_open_positions:
            logger.info(
                f"{Fore.CYAN}[{func_name}] Max positions ({self.max_open_positions}) reached. Skipping entry eval.{Style.RESET_ALL}"
            )
            return

        # --- 2. Compute Signal Score ---
        signal_score, reasons = self.compute_trade_signal_score(
            current_price, indicators, imbalance
        )
        score_color = (
            Fore.GREEN
            if signal_score > 0
            else (Fore.RED if signal_score < 0 else Fore.WHITE)
        )
        logger.info(f"Trade Signal Score: {score_color}{signal_score}{Style.RESET_ALL}")
        if (
            logger.isEnabledFor(logging.DEBUG)
            or abs(signal_score) >= ENTRY_SIGNAL_THRESHOLD_ABS
        ):
        ...
        if not entry_side:
            if signal_score is not None:
                logger.info(
                    f"[{func_name}] Neutral signal ({signal_score}). Threshold ({ENTRY_SIGNAL_THRESHOLD_ABS}) not met."
                )
            return

        log_color = Fore.GREEN if entry_side == "buy" else Fore.RED
        logger.info(
            f"{log_color}[{func_name}] {entry_side.upper()} signal score ({signal_score}) meets threshold. Preparing entry...{Style.RESET_ALL}"
        )

        # --- 4. Calculate Order Size ---
        # Pass indicators and score for potential adjustments
        order_size_base = self.calculate_order_size(
            current_price, indicators, signal_score
        )
        if order_size_base <= 0:
            logger.warning(
                f"{Fore.YELLOW}[{func_name}] Entry aborted: Calculated order size is zero or invalid (check logs from calculate_order_size).{Style.RESET_ALL}"
            )
            return
        ...
        sl_tp_required = self.use_atr_sl_tp or (
            self.base_stop_loss_pct is not None
            and self.base_take_profit_pct is not None
        )
        sl_tp_failed = sl_price is None and tp_price is None  # Check if BOTH failed
        if sl_tp_required and sl_tp_failed:
            logger.error(
                f"{Fore.RED}[{func_name}] Entry aborted: Required SL/TP calculation failed. Check ATR/percentage settings.{Style.RESET_ALL}"
            )
            return
        elif (
            sl_tp_failed
        ):  # Calculation failed but maybe not strictly required (e.g., only SL set)
            logger.warning(
                f"{Fore.YELLOW}[{func_name}] SL/TP calculation failed or returned None. Proceeding without SL/TP params or only partial.{Style.RESET_ALL}"
            )
        ...
        if entry_order:
            ...
            if not order_id:
                logger.critical(
                    f"{Fore.RED}[{func_name}] Entry order placed but received NO ID! Cannot track. Manual check required! Response: {str(entry_order)[:200]}{Style.RESET_ALL}"
                )
            ...
            if order_status == "open":
                ...
                logger.warning(
                    f"{Fore.YELLOW}[{func_name}] Entry order {order_id} failed immediately (Status: {order_status}). Not adding position.{Style.RESET_ALL}"
                )
                return
            else:
                initial_pos_status = STATUS_PENDING_ENTRY
                logger.warning(
                    f"[{func_name}] Entry order {order_id} unusual initial status '{order_status}'. Treating as PENDING."
                )

            filled_amount = float(entry_order.get("filled", 0.0))
            avg_fill_price = (
                float(entry_order.get("average"))
            ...
            entry_price_state = (
                avg_fill_price if is_filled_immediately and avg_fill_price else None
            )
            entry_time_state = (
                timestamp_ms / 1000 if is_filled_immediately and timestamp_ms else None
            )
            size_state = (
                filled_amount if is_filled_immediately else 0.0
            )  # Store 0 size if pending
            ...
            logger.info(
                f"{Fore.CYAN}---> Position {order_id} added to state. Status: {initial_pos_status}, Entry Price: {self.format_price(entry_price_state)}{Style.RESET_ALL}"
            )
            self._save_state()
        else:
            logger.error(
                f"{Fore.RED}[{func_name}] Entry order placement failed (API call error). No position added.{Style.RESET_ALL}"
            )

    def run(self) -> None:
        """Starts the main trading loop."""
        logger.info(
            f"{Fore.CYAN}--- Initiating Trading Loop (Symbol: {self.symbol}, TF: {self.timeframe}) ---{Style.RESET_ALL}"
        )
        while True:
            self.iteration += 1
            ...
            loop_prefix = (
                f"{Fore.BLUE}===== Iter {self.iteration} ====={Style.RESET_ALL}"
            )
            ...
            try:
                ...
                current_price = market_data["price"]
                logger.info(
                    f"{loop_prefix} Price: {self.format_price(current_price)} "
                ...
                logger.debug(
                    f"{loop_prefix} Loop took {exec_time:.2f}s. Waiting {wait_time:.2f}s..."
                )
                ...
                logger.critical(
                    f"{Fore.RED}{Style.BRIGHT}{loop_prefix} CRITICAL UNHANDLED ERROR: {type(e).__name__} - {e}{Style.RESET_ALL}",
                    exc_info=True,
                )
                error_pause = 60
                logger.warning(
                    f"{Fore.YELLOW}Pausing {error_pause}s due to critical error...{Style.RESET_ALL}"
                )
                try:
                    time.sleep(error_pause)
                except KeyboardInterrupt:
                    logger.warning("\nInterrupt during error pause. Exiting...")
                    break
    ...
    def shutdown(self) -> None:
        ...
        logger.warning(
            f"{Fore.YELLOW}{Style.BRIGHT}--- Initiating Graceful Shutdown ---{Style.RESET_ALL}"
        )
        ...
        pending_to_cancel = [
            ...
            if p.get("status") == STATUS_PENDING_ENTRY
        ]
        cancelled_pending = 0
        if pending_to_cancel:
            logger.info(
                f"Attempting to cancel {len(pending_to_cancel)} PENDING entry order(s)..."
            )
            for pos in pending_to_cancel:
                ...
                if self.cancel_order_by_id(pos_id, symbol=pos_symbol):
                    ...
                    try:  # Find original position to update status
                        original_pos = next(
                            p for p in self.open_positions if p.get("id") == pos_id
                        )
                        ...
                        logger.warning(
                            f"Could not find pos {pos_id} in state list after simulated cancel success."
                        )
                else:
                    logger.error(
                        f"{Fore.RED}Failed to cancel pending order {pos_id} ({pos_symbol}). Manual check needed.{Style.RESET_ALL}"
                    )
            logger.info(
                f"Pending order cancellation finished. Cancelled/Gone: {cancelled_pending}/{len(pending_to_cancel)}."
            )
        else:
            logger.info("No pending orders in state to cancel.")
        ...
        if self.close_positions_on_exit and active_to_manage:
            logger.warning(
                f"{Fore.YELLOW}Closing {len(active_to_manage)} ACTIVE position(s) due to config 'close_positions_on_exit=True'...{Style.RESET_ALL}"
            )
            current_price = self.fetch_market_price()
            if current_price is None:
                logger.critical(
                    f"{Fore.RED}{Style.BRIGHT}CRITICAL: Cannot fetch price for market close! {len(active_to_manage)} positions remain OPEN! Manual action required!{Style.RESET_ALL}"
                )
            else:
                closed_count = 0
                failed_ids = []
                for position in list(active_to_manage):  # Iterate copy
                    ...
                    pos_symbol = position.get("symbol", self.symbol)
                    logger.info(
                        f"Attempting market close for active {pos_id} ({pos_symbol})..."
                    )
                    ...
                    if close_order:
                        closed_count += 1
                        logger.info(
                            f"{Fore.YELLOW}---> Market close placed for {pos_id}. Marking as '{STATUS_CLOSED_ON_EXIT}'.{Style.RESET_ALL}"
                        )
                        position["status"] = (
                            STATUS_CLOSED_ON_EXIT  # Update the item from the *original* list
                        )
                        ...
                        self._log_position_pnl(
                            position,
                            close_fill,
                            f"Closed on Exit (Order {close_order.get('id')})",
                        )
                    else:
                        failed_ids.append(pos_id)
                        logger.error(
                            f"{Fore.RED}{Style.BRIGHT}CRITICAL: Failed market close for {pos_id} ({pos_symbol}). REMAINS OPEN! Manual action required!{Style.RESET_ALL}"
                        )
                logger.info(
                    f"Active position closure attempt: Closed {closed_count}/{len(active_to_manage)}. Failed: {len(failed_ids)}."
                )
                if failed_ids:
                    logger.error(
                        f"Failed close IDs: {failed_ids}. MANUAL ACTION NEEDED."
                    )

        elif active_to_manage:
            logger.warning(
                f"{Fore.YELLOW}{len(active_to_manage)} position(s) remain active ('close_positions_on_exit' is false):{Style.RESET_ALL}"
            )
            for pos in active_to_manage:
                logger.warning(
                    f" -> ID: {pos.get('id')} ({pos.get('symbol')}), Side: {pos.get('side')}, Size: {self.format_amount(pos.get('size'))}, Entry: {self.format_price(pos.get('entry_price'))}"
                )
        else:
            logger.info("No active positions found to manage during shutdown.")
        ...
        logger.info(
            f"{Fore.CYAN}{Style.BRIGHT}--- Scalping Bot Shutdown Complete ---{Style.RESET_ALL}"
        )
        logging.shutdown()  # Ensure log handlers are flushed/closed

    # --- Helper Formatting Functions ---
    def format_price(
        self, price: float | int | str | None, decimals: int | None = None
    ) -> str:
    ...
    def format_amount(
        self, amount: float | int | str | None, decimals: int | None = None
    ) -> str:
    ...
    def format_indicator(
        self, value: float | int | str | None, precision: int = 4
    ) -> str:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if exit_code not in [0, 130]:
            logger.error(f"Bot exited via SystemExit with code: {exit_code}.")
        else:
            logger.info(f"Bot exited via SystemExit with code: {exit_code}.")
    except Exception as e:
        logger.critical(
            f"{Fore.RED}{Style.BRIGHT}Unhandled critical error occurred outside main loop: {type(e).__name__} - {e}{Style.RESET_ALL}",
            exc_info=True,
        )
        exit_code = 1
    finally:
