...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(symbols, list) or not symbols or not all(isinstance(s, str) for s in symbols):
             ...
             print(f"{NEON_YELLOW}Using default value for 'symbols_to_trade': {updated_config['symbols_to_trade']}{RESET}")
             save_needed = True
...
class TradingAnalyzer:
    ...
    def _update_latest_indicator_values(self):
        ...
        decimal_keys = ["ATR", "Open", "High", "Low", "Close", "Volume",
                        "BB_Lower", "BB_Middle", "BB_Upper", "PSAR_long", "PSAR_short"]
        # Keys that are typically floats (oscillators, MAs, etc.)
        float_keys = ["EMA_Short", "EMA_Long", "Momentum", "CCI", "Williams_R",
                      "MFI", "VWAP", "SMA10", "StochRSI_K", "StochRSI_D", "RSI",
                      "Volume_MA"] # Volume MA is calculated as float
        ...
        try:
            ...
            if volume_ma > 0: # Avoid division by zero if MA is zero
                ...
                if volume_ratio > multiplier_decimal:
                    ...
                    return min(1.0, 0.5 + float(volume_ratio - multiplier_decimal) / (multiplier_decimal * 2)) # Example scaling
    ...
    def _check_cci(self) -> float:
        ...
        if -80 <= cci < 0: return 0.1 # Weak bullish (below zero but not oversold)
        if 0 < cci <= 80: return -0.1 # Weak bearish (above zero but not overbought)
        if cci == 0: return 0.0
    ...
    def _check_wr(self) -> float:
        ...
        if -80 < wr < -50: return 0.4 # Moving out of oversold potentially
        if -50 < wr < -20: return -0.4 # Moving out of overbought potentially
        if wr == -50: return 0.0 # Midpoint is neutral
        return 0.0
            ...
            if ret_code == 110013: # Parameter error
                 ...
                 lg.error(f"{NEON_YELLOW} >> Hint (110086): Stop Loss price cannot be equal to Take Profit price.{RESET}")
                  ...
                  if "leverage not modified" in ret_msg.lower():
                      lg.info(f"{NEON_YELLOW} >> Hint (110043 - Leverage Not Modified): Leverage likely already set. Protection setting might still have failed due to other reasons.{RESET}")
                  else:
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger) -> None:
                ...
                # If Limit Order -> Log and wait for next cycle (or implement order monitoring)
                     ...
                     if order_status == 'open':
                         ...
                         # e.g., store order_id and check its status in subsequent loops
                          ...
                          confirm_delay = config.get("position_confirm_delay_seconds", POSITION_CONFIRM_DELAY_SECONDS) # Short delay even for limit fill
                          time.sleep(confirm_delay)
                          ...
                          if confirmed_position:
                               ...
                               try:
                                   ...
                                   if entry_price_actual is None or entry_price_actual <= 0:
                                       lg.warning(f"Could not get valid actual entry price from confirmed position. Using intended limit price {limit_entry_price} for protection.")
    ...
    # ==============================================
        ...
        if pos_side not in ['long', 'short'] or pos_size == 0 or entry_price is None or entry_price <= 0:
             lg.error(f"{NEON_RED}Cannot manage position for {symbol}: Invalid details retrieved (Side: {pos_side}, Size: {pos_size}, Entry: {entry_price}). Skipping management.{RESET}")
        ...
        # --- If No Exit Signal, Proceed with Position Management ---
                ...
                if pos_timestamp_ms:
                    try:
                        ...
                        if time_elapsed_minutes >= time_exit_minutes_config:
                         ...
                         lg.error(f"{NEON_RED}Error converting position timestamp for time-based exit check: {pos_timestamp_ms} -> {time_conv_err}{RESET}")
            ...
            try:
                 ...
                 if tsl_value_str and str(tsl_value_str).strip() and str(tsl_value_str) != '0': # Check if not None, empty or '0' string
                 ...
                 lg.warning(f"Could not reliably determine if exchange TSL is active (value: {tsl_value_str}): {tsl_check_err}")
                ...
                try:
                    ...
                    if profit_in_atr >= profit_target_atr:
                        ...
                        if pos_side == 'long':
                            ...
                            be_stop_price = (raw_be_stop / min_tick_size).quantize(Decimal('1'), rounding=ROUND_UP) * min_tick_size
                        else: # short
                            # Place BE SL slightly below entry, quantize to tick size
                            raw_be_stop = entry_price - tick_offset
                            be_stop_price = (raw_be_stop / min_tick_size).quantize(Decimal('1'), rounding=ROUND_DOWN) * min_tick_size
                     ...
                     lg.error(f"{NEON_RED}Error during break-even check ({symbol}) (Decimal/Type Error): {dec_err}{RESET}", exc_info=False)
...
def main() -> None:
        ...
        try:
            ...
            for symbol in symbols_to_trade:
                ...
                try:
                    ...
                    symbol_logger.error(f"{NEON_RED}!!! Unhandled Exception during analysis for {symbol}: {symbol_err} !!!{RESET}", exc_info=True)
    ...
    init_logger.info(f"--- XR Scalper Bot Shutdown Complete ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---")
