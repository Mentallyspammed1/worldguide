...
try:
    ...
    print(
        "Error: 'zoneinfo' requires Python 3.9+ and potentially the 'tzdata' package."
    )
    ...
    print(
        "Warning: Using basic UTC fallback for timezone handling. Displayed times will be UTC."
    )
    ...
    class ZoneInfo:  # Basic fallback implementation
        ...
        def __init__(self, key: str):
            ...
            if key.lower() != "utc":
                print(
                    f"Warning: Timezone '{key}' not supported by fallback. Using UTC."
                )
        ...
        def __repr__(self) -> str:
            return f"ZoneInfo(key='{self._name}')"  # Show the effective key
...
try:
    ...
    print(
        f"Warning: Could not load timezone '{configured_timezone}'. Using UTC. Error: {tz_err}"
    )
...
POSITION_CONFIRM_DELAY_SECONDS = (
    8  # Wait time after placing order before confirming position
)
...
# --- Logger Setup ---
def setup_logger(logger_name_suffix: str) -> logging.Logger:
    ...
    if logger.hasHandlers():
        ...
        # This prevents clearing handlers if the same logger is requested multiple times
        # within a short period (e.g., during a single analysis cycle)
        return logger
        # Alternative: Clear existing handlers to ensure fresh setup if called again
    ...
    # Include Timezone Abbreviation (e.g., CDT, UTC). Note: %Z behavior can vary.
    stream_formatter = SensitiveFormatter(
        "%(asctime)s %(levelname)-8s [%(name)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S %Z",
    )
    # Set converter to use the configured TIMEZONE for log record times.
    # This ensures log timestamps reflect the desired local time.
...
def initialize_exchange(
    ...
    try:
        ...
        if config.get("use_sandbox"):
            ...
            if hasattr(exchange, "set_sandbox_mode"):
                try:
                    ...
                    lg.warning(
                        f"Error calling set_sandbox_mode(True) for {exchange.id}: {sandbox_err}. Attempting manual URL override."
                    )
                ...
                lg.warning(
                    f"{exchange.id} does not support set_sandbox_mode via ccxt. Ensure API keys are for Testnet."
                )
        ...
        lg.info(
            f"Attempting initial balance fetch (Account Type: {account_type_to_test})..."
        )
        try:
            ...
            lg.error(
                f"{NEON_RED}>> Ensure API keys are correct, have necessary permissions (Read, Trade), match the account type (Real/Testnet), and IP whitelist is correctly set if enabled on the exchange.{RESET}"
            )
            return None
            ...
            try:
                ...
                lg.warning(
                    f"{NEON_YELLOW}Default balance fetch also failed: {fallback_err}. Check API permissions/account type.{RESET}"
                )
        except Exception as balance_err:
            # Catch other potential errors like network issues during balance fetch
            lg.warning(
                f"{NEON_YELLOW}Could not perform initial balance fetch: {balance_err}. Check API permissions/account type/network.{RESET}"
            )
        ...
        lg.error(
            f"{NEON_RED}CCXT Authentication Error during initialization: {e}{RESET}"
        )
        lg.error(
            f"{NEON_RED}>> Check API Key/Secret format and validity in your .env file.{RESET}"
        )
...
def fetch_current_price_ccxt(
        ...
        try:
            ...
            if last_price is not None:
                try:
                    ...
                    if p > 0:
                        price = p
                        lg.debug(f"Using 'last' price: {p}")
                except (InvalidOperation, ValueError, TypeError):
                    lg.warning(f"Invalid 'last' price format: {last_price}")
            ...
            if price is None and bid_price is not None and ask_price is not None:
                try:
                    ...
                    lg.warning(f"Invalid bid/ask format: {bid_price}, {ask_price}")
            ...
            if price is None and ask_price is not None:
                try:
                    ...
                    if p > 0:
                        price = p
                        lg.warning(f"Using 'ask' price fallback: {p}")
                except (InvalidOperation, ValueError, TypeError):
                    lg.warning(f"Invalid 'ask' price format: {ask_price}")
            ...
            if price is None and bid_price is not None:
                try:
                    ...
                    if p > 0:
                        price = p
                        lg.warning(f"Using 'bid' price fallback: {p}")
                except (InvalidOperation, ValueError, TypeError):
                    lg.warning(f"Invalid 'bid' price format: {bid_price}")
            ...
            if price is not None and price > 0:
                ...
                lg.warning(
                    f"Failed to get a valid price from ticker data on attempt {attempts + 1}."
                )
            ...
            lg.warning(
                f"{NEON_YELLOW}Rate limit exceeded fetching price: {e}. Waiting longer...{RESET}"
            )
    ...
    lg.error(
        f"{NEON_RED}Failed to fetch a valid current price for {symbol} after {MAX_API_RETRIES + 1} attempts.{RESET}"
    )
    return None
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        for attempt in range(MAX_API_RETRIES + 1):
            try:
                ...
                if ohlcv is not None and isinstance(ohlcv, list):  # Ensure it's a list
                    ...
                    if not ohlcv:
                        lg.warning(
                            f"fetch_ohlcv returned an empty list for {symbol} (Attempt {attempt + 1}). Retrying..."
                        )
                    else:
                        break  # Success
                else:
                    lg.warning(
                        f"fetch_ohlcv returned invalid data (type: {type(ohlcv)}) for {symbol} (Attempt {attempt + 1}). Retrying..."
                    )
                ...
                if attempt < MAX_API_RETRIES:
                    lg.warning(
                        f"Network error fetching klines for {symbol} (Attempt {attempt + 1}): {e}. Retrying in {RETRY_DELAY_SECONDS}s..."
                    )
                    time.sleep(RETRY_DELAY_SECONDS)
                else:
                    lg.error(
                        f"{NEON_RED}Max retries reached fetching klines for {symbol} after network errors.{RESET}"
                    )
                ...
                if attempt < MAX_API_RETRIES:
                    ...
                    raise e  # Re-raise after retries
            except Exception as e:
                # Catch any other unexpected error during the fetch attempt
                lg.error(
                    f"{NEON_RED}Unexpected error during kline fetch attempt {attempt + 1} for {symbol}: {e}{RESET}",
                    exc_info=True,
                )
                if attempt < MAX_API_RETRIES:
                    time.sleep(RETRY_DELAY_SECONDS)
        ...
        if not ohlcv or not isinstance(ohlcv, list):
            lg.warning(
                f"{NEON_YELLOW}No valid kline data returned for {symbol} {timeframe} after retries.{RESET}"
            )
            return pd.DataFrame()
        ...
        if df.empty:
            lg.warning(
                f"{NEON_YELLOW}Kline data DataFrame is empty for {symbol} {timeframe}.{RESET}"
            )
            return df
        ...
        if rows_dropped > 0:
            lg.debug(
                f"Dropped {rows_dropped} rows with NaN/invalid price data for {symbol}."
            )

        # Check if DataFrame became empty after cleaning
        if df.empty:
            lg.warning(
                f"{NEON_YELLOW}Kline data for {symbol} {timeframe} empty after cleaning.{RESET}"
            )
            return pd.DataFrame()
    ...
    # Catch errors that occur outside the retry loop (e.g., during DataFrame processing)
        ...
        lg.error(
            f"{NEON_RED}Network error occurred during kline processing for {symbol}: {e}{RESET}"
        )
    except ccxt.ExchangeError as e:
        lg.error(
            f"{NEON_RED}Exchange error occurred during kline processing for {symbol}: {e}{RESET}"
        )
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected error processing klines for {symbol}: {e}{RESET}",
            exc_info=True,
        )
...
def fetch_orderbook_ccxt(
    ...
    while attempts <= MAX_API_RETRIES:
        try:
            ...
            lg.debug(
                f"Fetching order book for {symbol}, limit={limit} (Attempt {attempts + 1}/{MAX_API_RETRIES + 1})"
            )
            ...
            if not orderbook:
                lg.warning(
                    f"fetch_order_book returned None/empty for {symbol} (Attempt {attempts + 1})."
                )
            elif not isinstance(orderbook, dict):
                lg.warning(
                    f"{NEON_YELLOW}Invalid orderbook type received for {symbol}. Expected dict, got {type(orderbook)}. Attempt {attempts + 1}.{RESET}"
                )
            elif "bids" not in orderbook or "asks" not in orderbook:
                lg.warning(
                    f"{NEON_YELLOW}Invalid orderbook structure for {symbol}: missing 'bids' or 'asks'. Attempt {attempts + 1}. Response keys: {list(orderbook.keys())}{RESET}"
                )
            elif not isinstance(orderbook["bids"], list) or not isinstance(
                orderbook["asks"], list
            ):
                lg.warning(
                    f"{NEON_YELLOW}Invalid orderbook structure for {symbol}: 'bids' or 'asks' are not lists. Attempt {attempts + 1}. bids type: {type(orderbook['bids'])}, asks type: {type(orderbook['asks'])}{RESET}"
                )
        ...
        # Handle specific CCXT exceptions
            ...
            lg.warning(
                f"{NEON_YELLOW}Orderbook fetch network error for {symbol}: {e}. Retrying...{RESET}"
            )
        except ccxt.RateLimitExceeded:
            wait_time = RETRY_DELAY_SECONDS * 5  # Longer wait for rate limits
            lg.warning(
                f"Rate limit exceeded fetching orderbook for {symbol}. Retrying in {wait_time}s..."
            )
            time.sleep(wait_time)
            ...
            lg.error(
                f"{NEON_RED}Exchange error fetching orderbook for {symbol}: {e}{RESET}"
            )
            ...
            lg.error(
                f"{NEON_RED}Unexpected error fetching orderbook for {symbol}: {e}{RESET}",
                exc_info=True,
            )
...
class TradingAnalyzer:
    ...
    def __init__(
        ...
        if not self.ccxt_interval:
            self.logger.error(
                f"Invalid interval '{self.interval}' in config, cannot map to CCXT timeframe. Defaulting calculation logic if possible, but fetching will fail."
            )
        ...
        # This is necessary because pandas_ta often appends parameters to column names.
        self.ta_column_names: Dict[str, Optional[str]] = {}

        if not self.weights:
            logger.error(
                f"Active weight set '{self.active_weight_set_name}' not found or empty in config for {self.symbol}."
            )
    ...
    def _get_ta_col_name(
        self, base_name: str, result_df: pd.DataFrame
    ) -> Optional[str]:
        """
        Helper to find the actual column name generated by pandas_ta based on common patterns.
        pandas_ta often appends parameters to the base indicator name (e.g., 'EMA_10').
        ...
        expected_patterns = {
            "ATR": [
                f"ATRr_{self.config.get('atr_period', DEFAULT_INDICATOR_PERIODS['atr_period'])}"
            ],
            "EMA_Short": [
                f"EMA_{self.config.get('ema_short_period', DEFAULT_INDICATOR_PERIODS['ema_short_period'])}"
            ],
            "EMA_Long": [
                f"EMA_{self.config.get('ema_long_period', DEFAULT_INDICATOR_PERIODS['ema_long_period'])}"
            ],
            "Momentum": [
                f"MOM_{self.config.get('momentum_period', DEFAULT_INDICATOR_PERIODS['momentum_period'])}"
            ],
            "CCI": [
                f"CCI_{self.config.get('cci_window', DEFAULT_INDICATOR_PERIODS['cci_window'])}"
            ],  # May have suffix like _100.0
            "Williams_R": [
                f"WILLR_{self.config.get('williams_r_window', DEFAULT_INDICATOR_PERIODS['williams_r_window'])}"
            ],
            "MFI": [
                f"MFI_{self.config.get('mfi_window', DEFAULT_INDICATOR_PERIODS['mfi_window'])}"
            ],
            "VWAP": ["VWAP_D"],  # Often includes anchor like 'D' for daily
            "PSAR_long": [
                f"PSARl_{self.config.get('psar_af', DEFAULT_INDICATOR_PERIODS['psar_af'])}_{self.config.get('psar_max_af', DEFAULT_INDICATOR_PERIODS['psar_max_af'])}"
            ],
            "PSAR_short": [
                f"PSARs_{self.config.get('psar_af', DEFAULT_INDICATOR_PERIODS['psar_af'])}_{self.config.get('psar_max_af', DEFAULT_INDICATOR_PERIODS['psar_max_af'])}"
            ],
            "SMA10": [
                f"SMA_{self.config.get('sma_10_window', DEFAULT_INDICATOR_PERIODS['sma_10_window'])}"
            ],
            ...
            "RSI": [
                f"RSI_{self.config.get('rsi_period', DEFAULT_INDICATOR_PERIODS['rsi_period'])}"
            ],
            ...
            "Volume_MA": [
                f"VOL_SMA_{self.config.get('volume_ma_period', DEFAULT_INDICATOR_PERIODS['volume_ma_period'])}"
            ],  # Custom name used in calc
        }
        ...
        for col in result_df.columns:
            ...
            if base_name_lower in col.lower():
                self.logger.debug(
                    f"Found column '{col}' for base '{base_name}' using fallback substring search."
                )
                return col
    ...
    def _calculate_all_indicators(self):
        ...
        if indicators_config.get("atr_period"):
        ...
        if indicators_config.get("momentum"):
            required_periods.append(get_period("momentum_period"))
        if indicators_config.get("cci"):
            required_periods.append(get_period("cci_window"))
        if indicators_config.get("wr"):
            required_periods.append(get_period("williams_r_window"))
        if indicators_config.get("mfi"):
            required_periods.append(get_period("mfi_window"))
        # VWAP doesn't have a standard period parameter in basic ta.vwap
        # PSAR doesn't have a length period
        if indicators_config.get("sma_10"):
            required_periods.append(get_period("sma_10_window"))
        ...
        if indicators_config.get("rsi"):
            required_periods.append(get_period("rsi_period"))
        if indicators_config.get("bollinger_bands"):
            required_periods.append(get_period("bollinger_bands_period"))
        if indicators_config.get("volume_confirmation"):
            required_periods.append(get_period("volume_ma_period"))
        ...
        min_required_data = (
            max(required_periods) + 20 if required_periods else 50
        )  # Default min 50 if no periods found

        if len(self.df) < min_required_data:
            self.logger.warning(
                f"{NEON_YELLOW}Insufficient data ({len(self.df)} points) for {self.symbol} to calculate all indicators reliably (min recommended: {min_required_data}). Results may contain NaNs.{RESET}"
            )
        ...
        try:
            ...
            if indicators_config.get("stoch_rsi", False):
                ...
                stochrsi_result = df_calc.ta.stochrsi(
                    length=stoch_rsi_len,
                    rsi_length=stoch_rsi_rsi_len,
                    k=stoch_rsi_k,
                    d=stoch_rsi_d,
                )
            ...
            self.logger.error(
                f"{NEON_RED}AttributeError calculating indicators for {self.symbol}: {e}{RESET}. Check pandas_ta usage and data.",
                exc_info=True,
            )
        except Exception as e:
    ...
    def _update_latest_indicator_values(self):
        ...
        # Define keys that should be stored as Decimal (prices, volume, ATR)
        decimal_keys = [
            "ATR",
            "Open",
            "High",
        ...
        # Keys that are typically floats (oscillators, MAs, etc.)
        float_keys = [
            "EMA_Short",
            "EMA_Long",
            "Momentum",
            ...
            "RSI",
            "Volume_MA",
        ]  # Volume MA is calculated as float

        if self.df.empty:
            self.logger.warning(
                f"Cannot update latest values: DataFrame empty for {self.symbol}."
            )
            # Initialize with NaNs if empty
            self.indicator_values = {
                k: np.nan
                for k in list(self.ta_column_names.keys())
                + ["Close", "Volume", "High", "Low", "Open"]
            }
            return
        if self.df.iloc[-1].isnull().all():
            self.logger.warning(
                f"Cannot update latest values: Last row contains all NaNs for {self.symbol}."
            )
        ...
        try:
                ...
                if col_name and col_name in latest.index:
                    ...
                    if pd.notna(value):
                        try:
                            ...
                            else:  # Fallback for unclassified keys (store as float)
                                self.logger.debug(
                                    f"Indicator key '{key}' not pre-classified, storing as float."
                                )
                                updated_values[key] = float(value)
                        except (ValueError, TypeError, InvalidOperation) as conv_err:
                            self.logger.warning(
                                f"Could not convert value for {key} ('{col_name}': {value}) for {self.symbol}. Storing NaN. Error: {conv_err}"
                            )
                    ...
                    if (
                        key in self.ta_column_names
                    ):  # Only log if calculation was attempted
                        self.logger.debug(
                            f"Indicator column '{col_name}' for key '{key}' not found in latest data row for {self.symbol}. Storing NaN."
                        )
                    updated_values[key] = np.nan
            ...
            for base_col in ["open", "high", "low", "close", "volume"]:
                ...
                if pd.notna(value):
                    try:
                        ...
                        self.logger.warning(
                            f"Could not convert base value for '{base_col}' ({value}) to Decimal for {self.symbol}. Storing NaN. Error: {conv_err}"
                        )
                        updated_values[key_name] = np.nan
                else:
                    updated_values[key_name] = (
                        np.nan
                    )  # Store NaN if base value is missing/NaN
            ...
            valid_values_log = {}
            price_prec = TradingAnalyzer.get_price_precision(
                self.market_info, self.logger
            )  # Use static method
            ...
            self.logger.debug(
                f"Latest indicator values updated for {self.symbol}: {valid_values_log}"
            )

        except IndexError:
            # This error occurs if the DataFrame is empty or iloc[-1] fails
            self.logger.error(
                f"Error accessing latest row (iloc[-1]) for {self.symbol}. DataFrame might be empty or too short after cleaning."
            )
    ...
    def calculate_fibonacci_levels(
        ...
        window = window or self.config.get(
            "fibonacci_window", DEFAULT_INDICATOR_PERIODS["fibonacci_window"]
        )

        # Check if DataFrame is long enough for the window
        if len(self.df) < window:
            self.logger.debug(
                f"Not enough data ({len(self.df)} points) for Fibonacci window ({window}) on {self.symbol}."
            )
        ...
        try:
            ...
            if pd.isna(high_price_raw) or pd.isna(low_price_raw):
                self.logger.warning(
                    f"Could not find valid high/low prices within the last {window} periods for Fibonacci calculation on {self.symbol}."
                )
            ...
            price_precision = TradingAnalyzer.get_price_precision(
                self.market_info, self.logger
            )
            min_tick_size = TradingAnalyzer.get_min_tick_size(
                self.market_info, self.logger
            )
            ...
            if diff > 0:
                ...
                for level_pct in FIB_LEVELS:
                    ...
                    if min_tick_size > 0:
                        level_price_quantized = (
                            level_price_raw / min_tick_size
                        ...
                        level_price_quantized = level_price_raw.quantize(
                            rounding_factor, rounding=ROUND_DOWN
                        )
                    levels[level_name] = level_price_quantized
            else:
                # Handle case where high == low (no range)
                self.logger.debug(
                    f"Fibonacci range is zero or negative (High={high}, Low={low}) for {self.symbol} over last {window} periods. All levels set to High/Low."
                )
                # Quantize the single price level
                if min_tick_size > 0:
                    level_price_quantized = (high / min_tick_size).quantize(
                        Decimal("1"), rounding=ROUND_DOWN
                    ) * min_tick_size
                else:  # Fallback
                    rounding_factor = Decimal("1e-" + str(price_precision))
                    level_price_quantized = high.quantize(
                        rounding_factor, rounding=ROUND_DOWN
                    )
            ...
            self.logger.debug(
                f"Calculated Fibonacci levels for {self.symbol} (Window: {window}): {log_levels}"
            )
            return levels

        except KeyError as e:
            self.logger.error(
                f"{NEON_RED}Fibonacci calculation error for {self.symbol}: Missing column '{e}'. Ensure 'high' and 'low' columns exist.{RESET}"
            )
            self.fib_levels_data = {}
            return {}
        except Exception as e:
            ...
            self.logger.error(
                f"{NEON_RED}Unexpected Fibonacci calculation error for {self.symbol}: {e}{RESET}",
                exc_info=True,
            )
            self.fib_levels_data = {}
            return {}
    ...
    def get_price_precision(market_info: Dict[str, Any], logger: logging.Logger) -> int:
        ...
        try:
            ...
            if price_precision_val is not None:
                ...
                if isinstance(price_precision_val, int):
                    if price_precision_val >= 0:
                        logger.debug(
                            f"Using price precision (decimal places) from market_info.precision.price: {price_precision_val} for {symbol}"
                        )
                ...
                # If it's float/str, it often represents the tick size
                    ...
                    try:
                        ...
                        if tick_size > 0:
                            ...
                            logger.debug(
                                f"Calculated price precision from market_info.precision.price (tick size {tick_size}): {precision} for {symbol}"
                            )
                            return precision
                    except (InvalidOperation, ValueError, TypeError) as e:
                        logger.warning(
                            f"Could not parse precision.price '{price_precision_val}' as tick size for {symbol}: {e}"
                        )
                ...
                try:
                    ...
                    if min_price_tick > 0:
                        ...
                        if min_price_tick < Decimal("0.1"):
                            ...
                            logger.debug(
                                f"Inferred price precision from limits.price.min ({min_price_tick}): {precision} for {symbol}"
                            )
                            return precision
                        else:
                            logger.debug(
                                f"limits.price.min ({min_price_tick}) for {symbol} seems too large for tick size, likely minimum order price. Ignoring for precision."
                            )
                except (InvalidOperation, ValueError, TypeError) as e:
                    logger.warning(
                        f"Could not parse limits.price.min '{min_price_val}' for precision inference for {symbol}: {e}"
                    )
            ...
            logger.warning(
                f"Error determining price precision for {symbol} from market info: {e}. Falling back."
            )

        # --- Final Fallback ---
        # Use a reasonable default if no other method worked
        default_precision = (
            4  # Common default, adjust if needed for your typical markets
        )
        logger.warning(
            f"Could not determine price precision for {symbol}. Using default: {default_precision}."
        )
        return default_precision

    @staticmethod
    def get_min_tick_size(
        market_info: Dict[str, Any], logger: logging.Logger
    ) -> Decimal:
        """
            ...
            if price_precision_val is not None:
                if isinstance(price_precision_val, (float, str)):
                    try:
                        ...
                        if tick_size > 0:
                            logger.debug(
                                f"Using tick size from precision.price: {tick_size} for {symbol}"
                            )
                            return tick_size
                    except (InvalidOperation, ValueError, TypeError) as e:
                        logger.warning(
                ...
                # If it's an integer (decimal places), calculate tick size
                    ...
                    logger.debug(
                        f"Calculated tick size from precision.price (decimal places {price_precision_val}): {tick_size} for {symbol}"
                    )
                    return tick_size
                ...
                try:
                    ...
                    if min_tick_from_limit > 0:
                        ...
                        if min_tick_from_limit < Decimal("0.1"):
                            logger.debug(
                                f"Using tick size from limits.price.min: {min_tick_from_limit} for {symbol}"
                            )
                            return min_tick_from_limit
                        else:
                            logger.debug(
                                f"limits.price.min ({min_tick_from_limit}) for {symbol} seems too large for tick size, potentially min order price."
                            )
                except (InvalidOperation, ValueError, TypeError) as e:
                    logger.warning(
                        f"Could not parse limits.price.min '{min_price_val}' for tick size inference for {symbol}: {e}"
                    )

        except Exception as e:
            logger.warning(
                f"Could not determine min tick size for {symbol} from market info: {e}. Using precision fallback."
            )

        # --- Final Fallback: Calculate from get_price_precision (decimal places) ---
        price_precision_places = TradingAnalyzer.get_price_precision(
            market_info, logger
        )
        fallback_tick = Decimal("1e-" + str(price_precision_places))
        logger.debug(
            f"Using fallback tick size based on derived precision places ({price_precision_places}): {fallback_tick} for {symbol}"
        )
    ...
    def get_nearest_fibonacci_levels(
        ...
        if not self.fib_levels_data:
            self.logger.debug(
                f"Fibonacci levels not calculated yet for {self.symbol}. Cannot find nearest."
            )
            return []
        ...
        if (
            ...
            self.logger.warning(
                f"Invalid current price ({current_price}) provided for Fibonacci comparison on {self.symbol}."
            )
            return []
        ...
        try:
            ...
            for name, level_price in self.fib_levels_data.items():
                ...
                if isinstance(level_price, Decimal) and level_price > 0:
                    ...
                    level_distances.append(
                        {"name": name, "level": level_price, "distance": distance}
                    )
                else:
                    self.logger.warning(
                        f"Invalid or non-decimal value found in fib_levels_data: {name}={level_price}. Skipping."
                    )
            ...
            self.logger.error(
                f"{NEON_RED}Error finding nearest Fibonacci levels for {self.symbol}: {e}{RESET}",
                exc_info=True,
            )
            return []
    ...
    def calculate_ema_alignment_score(self) -> float:
        ...
        current_price_float = (
            float(close_decimal) if isinstance(close_decimal, Decimal) else np.nan
        )
        ...
        if pd.isna(ema_short) or pd.isna(ema_long) or pd.isna(current_price_float):
            self.logger.debug(
                "EMA alignment check skipped: Missing required values (EMA_Short, EMA_Long, or Close)."
            )
    ...
    def generate_trading_signal(
        ...
        if not active_weights:
            self.logger.error(
                f"Active weight set '{self.active_weight_set_name}' missing or empty in config for {self.symbol}. Cannot generate signal."
            )
            return "HOLD"
            ...
            if hasattr(self, check_method_name) and callable(
                    ...
                    if indicator_key == "orderbook":
                        ...
                        if orderbook_data:
                            ...
                            if weight != 0:  # Only log if it would have contributed
                                self.logger.debug(
                                    f"Orderbook check skipped for {self.symbol}: No orderbook data provided."
                                )
                    else:
                        # Call standard check methods
                        indicator_score_float = (
                            method_to_call()
                        )  # Expected to return float score or np.nan

                except Exception as e:
                    self.logger.error(
                        f"Error executing indicator check method {check_method_name} for {self.symbol}: {e}",
                        exc_info=True,
                    )
                    # Keep score as NaN
                ...
                if pd.notna(indicator_score_float):
                    try:
                        ...
                        clamped_score = max(
                            Decimal("-1.0"), min(Decimal("1.0"), score_decimal)
                        )
                        ...
                        self.logger.error(
                            f"Error processing score for {indicator_key} (Score: {indicator_score_float}, Weight: {weight}): {calc_err}"
                        )
                ...
                if weight != 0:  # Only warn if it was supposed to contribute
                    self.logger.warning(
                        f"Indicator check method '{check_method_name}' not found for enabled/weighted indicator: {indicator_key} ({self.symbol})"
                    )
        ...
        if total_weight_applied == 0:
            self.logger.warning(
                f"No indicators contributed valid scores to the signal calculation for {self.symbol}. Defaulting to HOLD."
            )
        else:
            ...
            try:
                ...
                self.logger.warning(
                    f"Invalid signal_score_threshold '{threshold_str}'. Using default 1.5."
                )
        ...
        # --- Log Summary ---
        price_prec = TradingAnalyzer.get_price_precision(self.market_info, self.logger)
        log_msg = (
        ...
        if final_signal == "BUY":
            self.signals = {"BUY": 1, "SELL": 0, "HOLD": 0}
        elif final_signal == "SELL":
            self.signals = {"BUY": 0, "SELL": 1, "HOLD": 0}
        else:
            self.signals = {"BUY": 0, "SELL": 0, "HOLD": 1}  # HOLD
    ...
    def _check_ema_alignment(self) -> float:
        ...
        if (
            "EMA_Short" not in self.indicator_values
            or "EMA_Long" not in self.indicator_values
        ):
            self.logger.debug(
                "EMA Alignment check skipped: EMA values not found in indicator_values."
            )
            return np.nan
    ...
    def _check_momentum(self) -> float:
        ...
        # Example: Assume significant momentum is > 0.1 or < -0.1 relative to price?
        # Scaling relative to price might be better than absolute threshold
        last_close = self.indicator_values.get("Close")  # Decimal
        if not isinstance(last_close, Decimal) or last_close <= 0:
            return 0.0  # Cannot scale if price is invalid

        try:
            # Calculate momentum as a percentage of price
            mom_pct = Decimal(str(momentum)) / last_close if last_close else Decimal(0)
            threshold_pct = Decimal("0.001")  # e.g., 0.1% price change momentum

            if mom_pct > threshold_pct:
                # Scale score up to 1 based on how much it exceeds threshold (capped at 1)
                return min(1.0, float(mom_pct / (threshold_pct * 5)))  # Example scaling
            elif mom_pct < -threshold_pct:
                # Scale score down to -1
                return max(
                    -1.0, float(mom_pct / (threshold_pct * 5))
                )  # Example scaling
            else:
                # Scale linearly within the threshold range
                return float(mom_pct / threshold_pct)
        except (InvalidOperation, ValueError, TypeError):
            return 0.0  # Error during calculation
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        volume_ma_float = self.indicator_values.get(
            "Volume_MA"
        )  # Should be float or NaN
        ...
        if (
            ...
            or not isinstance(current_volume, Decimal)
            or pd.isna(volume_ma_float)
        ):
            return np.nan  # Cannot compare if either is NaN
        ...
        try:
            ...
            if volume_ma > 0:  # Avoid division by zero if MA is zero
                ...
                if volume_ratio > multiplier_decimal:
                    # High volume can confirm trend (positive score, scale with ratio?)
                    # Scale score from 0.5 up to 1.0 as ratio increases
                    return min(
                        1.0,
                        0.5
                        + float(volume_ratio - multiplier_decimal)
                ...
                # Volume MA is zero, cannot compare ratio. Check if current volume is also zero.
                return (
                    0.0 if current_volume == 0 else 0.5
                )  # Slight positive if current vol > 0 but MA is 0
        except (InvalidOperation, ValueError, TypeError) as e:
    ...
    def _check_rsi(self) -> float:
        ...
        if rsi <= 30:
            return 1.0  # Oversold -> Strong Buy Signal
        if rsi >= 70:
            return -1.0  # Overbought -> Strong Sell Signal
        # Intermediate levels for weaker signals
        if rsi < 40:
            return 0.5  # Approaching Oversold -> Moderate Buy
        if rsi > 60:
            return -0.5  # Approaching Overbought -> Moderate Sell
    ...
    def _check_cci(self) -> float:
        ...
        if cci <= -150:
            return 1.0  # Strongly Oversold -> Buy
        if cci >= 150:
            return -1.0  # Strongly Overbought -> Sell
        if cci < -80:
        ...
        if -80 <= cci < 0:
            return 0.1  # Weak bullish (below zero but not oversold)
        if 0 < cci <= 80:
            return -0.1  # Weak bearish (above zero but not overbought)
        if cci == 0:
            return 0.0
    ...
    def _check_wr(self) -> float:
        ...
        if -80 < wr < -50:
            return 0.4  # Moving out of oversold potentially
        if -50 < wr < -20:
            return -0.4  # Moving out of overbought potentially
        if wr == -50:
            return 0.0  # Midpoint is neutral
        return 0.0

    def _check_psar(self) -> float:
        """Checks Parabolic SAR (PSAR) trend direction."""
        psar_long_signal = self.indicator_values.get(
            "PSAR_long"
        )  # Decimal price if long active, else NaN
        psar_short_signal = self.indicator_values.get(
            "PSAR_short"
        )  # Decimal price if short active, else NaN
        ...
        if long_active and not short_active:
            ...
            self.logger.warning(
                f"PSAR check encountered unexpected state: Long={psar_long_signal}, Short={psar_short_signal}"
            )
    ...
    def _check_sma_10(self) -> float:
        ...
        last_close_float = (
            float(last_close_decimal)
            if isinstance(last_close_decimal, Decimal)
            else np.nan
        )
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        bb_lower_dec = self.indicator_values.get("BB_Lower")  # Decimal or NaN
        bb_middle_dec = self.indicator_values.get("BB_Middle")  # Decimal or NaN
        bb_upper_dec = self.indicator_values.get("BB_Upper")  # Decimal or NaN
        last_close_decimal = self.indicator_values.get("Close")  # Decimal or NaN

        # Check if all values are valid Decimals
        if not all(
            isinstance(val, Decimal)
            for val in [bb_lower_dec, bb_middle_dec, bb_upper_dec, last_close_decimal]
        ):
            return np.nan

        # Check if price touches or crosses bands (potential reversal/breakout)
        if last_close_decimal <= bb_lower_dec:
            # Price at or below lower band -> Potential bounce (Buy signal)
            return 1.0
        if last_close_decimal >= bb_upper_dec:
    ...
    def _check_orderbook(
        ...
        try:
            ...
            if total_volume == 0:
                self.logger.debug(
                    f"Orderbook check ({self.symbol}): Zero total volume in top {num_levels_to_check} levels."
                )
            ...
            self.logger.warning(
                f"{NEON_YELLOW}Orderbook analysis failed for {self.symbol} (data conversion error): {e}{RESET}",
                exc_info=False,
            )
            return np.nan  # Return NaN on error
        except Exception as e:
            self.logger.warning(
                f"{NEON_YELLOW}Orderbook analysis failed for {self.symbol} (unexpected error): {e}{RESET}",
                exc_info=True,
            )
            return np.nan  # Return NaN on error
    ...
    def calculate_entry_tp_sl(
        ...
        if not isinstance(atr_val, Decimal) or pd.isna(atr_val) or atr_val <= 0:
            self.logger.warning(
                f"{NEON_YELLOW}Cannot calculate TP/SL for {self.symbol} {signal}: Invalid or missing ATR ({atr_val}).{RESET}"
            )
            return entry_price_estimate, None, None
        if (
            not isinstance(entry_price_estimate, Decimal)
            or pd.isna(entry_price_estimate)
            or entry_price_estimate <= 0
        ):
            self.logger.warning(
                f"{NEON_YELLOW}Cannot calculate TP/SL for {self.symbol} {signal}: Invalid entry price estimate ({entry_price_estimate}).{RESET}"
            )
            return entry_price_estimate, None, None
        ...
        try:
            ...
            tp_multiple_str = self.config.get(
                "take_profit_multiple", "1.0"
            )  # Default 1.0 if missing
            sl_multiple_str = self.config.get(
                "stop_loss_multiple", "1.5"
            )  # Default 1.5 if missing
            ...
            if take_profit_raw is not None:
                # Use min_tick for quantization if available and positive
                if min_tick > 0:
                    tp_rounding = ROUND_UP if signal == "BUY" else ROUND_DOWN
                    take_profit_quantized = (take_profit_raw / min_tick).quantize(
                        Decimal("1"), rounding=tp_rounding
                    ) * min_tick
                else:  # Fallback to decimal places if tick size invalid
                    rounding_factor = Decimal("1e-" + str(price_precision))
                    tp_rounding = ROUND_UP if signal == "BUY" else ROUND_DOWN
                    take_profit_quantized = take_profit_raw.quantize(
                        rounding_factor, rounding=tp_rounding
                    )

            if stop_loss_raw is not None:
                if min_tick > 0:
                    sl_rounding = ROUND_DOWN if signal == "BUY" else ROUND_UP
                    stop_loss_quantized = (stop_loss_raw / min_tick).quantize(
                        Decimal("1"), rounding=sl_rounding
                    ) * min_tick
                else:  # Fallback
                    rounding_factor = Decimal("1e-" + str(price_precision))
                    sl_rounding = ROUND_DOWN if signal == "BUY" else ROUND_UP
                    stop_loss_quantized = stop_loss_raw.quantize(
                        rounding_factor, rounding=sl_rounding
                    )
            ...
            if final_sl is not None and min_tick > 0:
                if signal == "BUY" and final_sl >= entry_price_estimate:
                    ...
                    # Move down by at least one tick
                    final_sl = ((entry_price_estimate - min_tick) / min_tick).quantize(
                        Decimal("1"), rounding=ROUND_DOWN
                    ) * min_tick
                    self.logger.debug(
                        f"Adjusted BUY SL below entry: {original_sl} -> {final_sl}"
                    )
                elif signal == "SELL" and final_sl <= entry_price_estimate:
                    original_sl = final_sl
                    # Move up by at least one tick
                    final_sl = ((entry_price_estimate + min_tick) / min_tick).quantize(
                        Decimal("1"), rounding=ROUND_UP
                    ) * min_tick
                    self.logger.debug(
                        f"Adjusted SELL SL above entry: {original_sl} -> {final_sl}"
                    )
            ...
            if final_tp is not None and min_tick > 0:
                if signal == "BUY" and final_tp <= entry_price_estimate:
                    self.logger.warning(
                        f"{NEON_YELLOW}BUY TP calculation non-profitable (TP {final_tp} <= Entry {entry_price_estimate}). Setting TP to None.{RESET}"
                    )
                    final_tp = None
                elif signal == "SELL" and final_tp >= entry_price_estimate:
                    self.logger.warning(
                        f"{NEON_YELLOW}SELL TP calculation non-profitable (TP {final_tp} >= Entry {entry_price_estimate}). Setting TP to None.{RESET}"
                    )
                    final_tp = None
            ...
            if final_sl is not None and final_sl <= 0:
                self.logger.error(
                    f"{NEON_RED}Stop loss calculation resulted in non-positive price ({final_sl}). Setting SL to None.{RESET}"
                )
                final_sl = None
            if final_tp is not None and final_tp <= 0:
                self.logger.warning(
                    f"{NEON_YELLOW}Take profit calculation resulted in non-positive price ({final_tp}). Setting TP to None.{RESET}"
                )
                final_tp = None
            ...
            self.logger.error(
                f"{NEON_RED}Error calculating TP/SL for {self.symbol} {signal} (Decimal/Type Error): {e}{RESET}",
                exc_info=False,
            )
            return entry_price_estimate, None, None
        except Exception as e:
            # Catch any other unexpected errors during calculation
            self.logger.error(
                f"{NEON_RED}Unexpected error calculating TP/SL for {self.symbol} {signal}: {e}{RESET}",
                exc_info=True,
            )
            return entry_price_estimate, None, None
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        try:
            ...
            for acc_type in account_types_to_try:
                try:
                    lg.debug(
                        f"Fetching balance using params={{'type': '{acc_type}'}} for {currency}... (Attempt {attempt + 1})"
                    )
                    ...
                    lg.debug(
                        f"Currency '{currency}' not directly found using type '{acc_type}'. Checking V5 structure..."
                    )
                    ...
                    lg.debug(
                        f"Error fetching balance for type '{acc_type}': {e}. Trying next."
                    )
                    continue
                except Exception as e:
                    lg.warning(
                        f"Unexpected error fetching balance type '{acc_type}': {e}. Trying next."
                    )
                    continue
            ...
            if not found_structure:
                ...
                try:
                    ...
                    lg.error(
                        f"{NEON_RED}Failed to fetch balance using default parameters: {e}{RESET}"
                    )
            ...
            if balance_info:
                ...
                if (
                    ...
                    lg.debug(
                        f"Found balance via standard ['{currency}']['free']: {available_balance_str}"
                    )

                # 2. Bybit V5 Nested: info.result.list[].coin[].availableToWithdraw/availableBalance
                elif (
                    not available_balance_str
                            ...
                            for coin_data in account["coin"]:
                                if coin_data.get("coin") == currency:
                                    ...
                                    if free is not None:
                                        ...
                                        lg.debug(
                                            f"Found balance via Bybit V5 nested ['available...']: {available_balance_str}"
                                        )
                                        break  # Found it
                    ...
                    if not available_balance_str:
                        lg.warning(
                            f"{currency} balance details not found within Bybit V5 'info.result.list[].coin[]'."
                        )
                ...
                # 3. Fallback: Top-level 'free' dictionary
                    ...
                    lg.debug(
                        f"Found balance via top-level 'free' dict: {available_balance_str}"
                    )
                ...
                if available_balance_str is None:
                    ...
                    if total_balance is not None:
                        lg.warning(
                            f"{NEON_YELLOW}Using 'total' balance ({total_balance}) as fallback for available {currency}.{RESET}"
                        )
                        available_balance_str = str(total_balance)
                    else:
                        lg.error(
                            f"{NEON_RED}Could not determine any balance ('free' or 'total') for {currency}.{RESET}"
                        )
            ...
            raise ccxt.ExchangeError(
                "Balance parsing failed or data missing"
            )  # Trigger retry

        except (ccxt.NetworkError, ccxt.RequestTimeout) as e:
            lg.warning(
                f"Network error fetching balance: {e}. Retrying ({attempt + 1}/{MAX_API_RETRIES})..."
            )
        except ccxt.RateLimitExceeded as e:
            wait_time = RETRY_DELAY_SECONDS * 5
            lg.warning(
                f"Rate limit exceeded fetching balance: {e}. Waiting {wait_time}s ({attempt + 1}/{MAX_API_RETRIES})..."
            )
            time.sleep(wait_time)
            continue  # Skip standard delay after rate limit wait
        except ccxt.AuthenticationError as e:
            lg.error(
                f"{NEON_RED}Authentication error fetching balance: {e}. Aborting balance fetch.{RESET}"
            )
            return None  # Don't retry auth errors
        except ccxt.ExchangeError as e:
            lg.warning(
                f"Exchange error fetching balance: {e}. Retrying ({attempt + 1}/{MAX_API_RETRIES})..."
            )
        except Exception as e:
            lg.error(
...
def get_market_info(
    ...
    try:
        ...
        if not exchange.markets or symbol not in exchange.markets:
            lg.info(
                f"Market info for {symbol} not loaded or symbol not found, reloading markets..."
            )
            try:
        ...
        if symbol not in exchange.markets:
            lg.error(
                f"{NEON_RED}Market {symbol} still not found after reloading.{RESET}"
            )
            ...
            if "/" in symbol:
                ...
                possible_matches = [
                    ...
                    if s.startswith(base) and ("PERP" in s or ":USDT" in s)
                ]
                ...
                if perp_sym_usdt in exchange.markets:
                    ...
                    lg.warning(
                        f"{NEON_YELLOW}Possible matches found: {possible_matches[:5]}{RESET}"
        ...
        if market:
            ...
            is_contract = market.get("contract", False) or market_type in [
                "swap",
                "future",
            ]
            contract_type = "N/A"
            ...
            lg.error(
                f"{NEON_RED}Market dictionary unexpectedly not found for validated symbol {symbol}.{RESET}"
            )
            return None
        ...
        lg.error(
            f"{NEON_RED}Network error getting market info for {symbol}: {e}{RESET}"
        )
...
def calculate_position_size(
    ...
    if balance is None or not isinstance(balance, Decimal) or balance <= 0:
        lg.error(
            f"Position sizing failed ({symbol}): Invalid or zero balance ({balance})."
        )
        return None
    if not isinstance(risk_per_trade, (float, int)) or not (0 < risk_per_trade < 1):
        lg.error(
            f"Position sizing failed ({symbol}): Invalid risk_per_trade ({risk_per_trade}). Must be between 0 and 1."
        )
        return None
    if (
        initial_stop_loss_price is None
        or not isinstance(initial_stop_loss_price, Decimal)
        or initial_stop_loss_price <= 0
    ):
        lg.error(
    ...
    try:
        ...
        if (
            sl_distance_per_unit <= 0
        ):  # Should be caught by earlier check, but defense-in-depth
            lg.error(
                f"Position sizing failed ({symbol}): Stop loss distance is zero or negative ({sl_distance_per_unit})."
            )
            return None
        ...
        try:
            ...
            lg.warning(
                f"Invalid contract size '{contract_size_str}' for {symbol}, using 1."
            )
        ...
        if market_info.get("linear", True) or not is_contract:  # Assume linear or spot
            ...
            if denominator > 0:
                ...
                lg.error(
                    f"Position sizing failed ({symbol}): Denominator zero/negative in size calculation (SL Dist: {sl_distance_per_unit}, ContractSize: {contract_size})."
                )
                return None
            ...
            lg.error(
                f"{NEON_RED}Inverse contract sizing not fully implemented. Aborting sizing for {symbol}.{RESET}"
            )
        ...
        if calculated_size is None or calculated_size <= 0:
            lg.error(
                f"Initial position size calculation resulted in zero or negative: {calculated_size}. RiskAmt={risk_amount_quote:.4f}, SLDist={sl_distance_per_unit}, ContractSize={contract_size}"
            )
            return None

        lg.info(
            f"Position Sizing ({symbol}): Balance={balance:.2f}, Risk={risk_per_trade:.2%}, RiskAmt={risk_amount_quote:.4f} {quote_currency}"
        )
        lg.info(
            f"  Entry={entry_price}, SL={initial_stop_loss_price}, SL Dist={sl_distance_per_unit}"
        )
        lg.info(
        ...
        amount_precision_val = precision.get(
            "amount"
        )  # Usually step size (float/str) or decimal places (int)
        ...
        min_amount = (
            Decimal(str(min_amount_str)) if min_amount_str is not None else Decimal("0")
        )
        max_amount = (
            Decimal(str(max_amount_str))
            if max_amount_str is not None
            else Decimal("inf")
        )
        ...
        min_cost = (
            Decimal(str(min_cost_str)) if min_cost_str is not None else Decimal("0")
        )
        max_cost = (
            Decimal(str(max_cost_str)) if max_cost_str is not None else Decimal("inf")
        )
        ...
        if adjusted_size < min_amount:
            lg.warning(
                f"{NEON_YELLOW}Calculated size {calculated_size:.8f} is below min amount {min_amount}. Adjusting to min amount.{RESET}"
            )
            adjusted_size = min_amount
        elif adjusted_size > max_amount:
            lg.warning(
                f"{NEON_YELLOW}Calculated size {calculated_size:.8f} exceeds max amount {max_amount}. Adjusting to max amount.{RESET}"
            )
        ...
        if market_info.get("linear", True) or not is_contract:
            ...
            if entry_price > 0:
                ...
                lg.error(
                    f"Cannot estimate cost for inverse contract {symbol}: Entry price is zero."
                )
                return None

        if estimated_cost is None:
            return None  # Should be caught above

        lg.debug(
            f"  Cost Check: Adjusted Size={adjusted_size:.8f}, Estimated Cost={estimated_cost:.4f} {quote_currency}"
        )

        # Check Min Cost
        if min_cost > 0 and estimated_cost < min_cost:
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {estimated_cost:.4f} is below min cost {min_cost}. Attempting to increase size.{RESET}"
            )
            ...
            if market_info.get("linear", True) or not is_contract:
                ...
                denominator = (
                    contract_size  # Denominator for inverse size calc is contract_size
                )
                if denominator > 0 and entry_price > 0:
                    required_size_for_min_cost = min_cost * entry_price / contract_size

            if required_size_for_min_cost is None or denominator <= 0:
                lg.error(
                    "Cannot calculate required size for min cost: Denominator zero/negative or invalid price."
                )
                return None

            lg.info(
                f"  Required size to meet min cost: {required_size_for_min_cost:.8f} {size_unit}"
            )

            # Check if required size violates other limits
            if required_size_for_min_cost > max_amount:
                lg.error(
                    f"{NEON_RED}Cannot meet min cost {min_cost} without exceeding max amount limit {max_amount}. Aborted.{RESET}"
                )
                return None
            ...
            if required_size_for_min_cost < min_amount:
                lg.error(
                    f"{NEON_RED}Cannot meet min cost: Required size {required_size_for_min_cost:.8f} is below min amount {min_amount}. Aborted.{RESET}"
                )
                ...
                lg.info(
                    f"  Adjusting size to meet min cost: {adjusted_size:.8f} -> {required_size_for_min_cost:.8f}"
                )
                ...
                if market_info.get("linear", True) or not is_contract:
                    ...
                    if entry_price > 0:
                        ...
                        estimated_cost = Decimal("inf")  # Mark as invalid

        # Check Max Cost (after potential min cost adjustment)
        elif max_cost > 0 and estimated_cost > max_cost:
            lg.warning(
                f"{NEON_YELLOW}Estimated cost {estimated_cost:.4f} exceeds max cost {max_cost}. Reducing size.{RESET}"
            )
            ...
            if adjusted_size_for_max_cost is None or denominator <= 0:
                lg.error(
                    "Cannot calculate max size for max cost: Denominator zero/negative or invalid price."
                )
                return None

            lg.info(
                f"  Reduced size allowed by max cost: {adjusted_size_for_max_cost:.8f} {size_unit}"
            )

            # Ensure the reduced size is still above the minimum amount
            if adjusted_size_for_max_cost < min_amount:
                lg.error(
                    f"{NEON_RED}Size reduced for max cost ({adjusted_size_for_max_cost:.8f}) is below min amount {min_amount}. Aborted.{RESET}"
                )
                return None
            else:
                # Adjust size down to meet max cost
                lg.info(
                    f"  Adjusting size to meet max cost: {adjusted_size:.8f} -> {adjusted_size_for_max_cost:.8f}"
                )
        ...
        try:
            ...
            formatted_size_str = exchange.amount_to_precision(
                symbol, amount_str
            )  # Default rounding mode might be ok too
            ...
            lg.info(
                f"Applied amount precision/step size: {adjusted_size:.8f} -> {final_size} {size_unit}"
            )

        except ccxt.ExchangeError as fmt_err:
            lg.warning(
                f"{NEON_YELLOW}CCXT formatting error applying amount precision ({fmt_err}). Check market data or try manual rounding.{RESET}"
            )
            ...
            if isinstance(amount_precision_val, (float, str)):
                try:
                    ...
                    if amount_step > 0:
                        ...
                        lg.info(
                            f"Applied manual amount step size ({amount_step}): {adjusted_size:.8f} -> {final_size} {size_unit}"
                        )
                    else:
                        raise ValueError("Amount step size is not positive")
                except (InvalidOperation, ValueError, TypeError) as manual_err:
                    lg.error(
                        f"Manual step size rounding failed: {manual_err}. Using unrounded size: {adjusted_size}",
                        exc_info=True,
                    )
                    final_size = adjusted_size  # Use unrounded as last resort
            else:
                lg.error(
                    f"Cannot determine amount step size for manual rounding. Using unrounded size: {adjusted_size}"
                )
                final_size = adjusted_size
        except (InvalidOperation, ValueError, TypeError) as dec_err:
            lg.error(
                f"Error converting formatted size '{formatted_size_str}' back to Decimal: {dec_err}"
            )
            return None
        ...
        if final_size is None or final_size <= 0:
            lg.error(
                f"{NEON_RED}Position size became zero or negative ({final_size}) after adjustments. Aborted.{RESET}"
            )
            return None
        ...
        if final_size < min_amount:
            ...
            lg.error(
                f"{NEON_RED}Final size {final_size} is below minimum amount {min_amount} after precision formatting. Aborted.{RESET}"
            )
            return None
        ...
        if final_cost is not None and min_cost > 0 and final_cost < min_cost:
            ...
            lg.error(
                f"{NEON_RED}Final size {final_size} results in cost {final_cost:.4f} which is below minimum cost {min_cost}. Exchange limits conflict? Aborted.{RESET}"
            )
            return None
        ...
        lg.error(
            f"{NEON_RED}Error during position size calculation ({symbol}) (Decimal/Type Error): {e}{RESET}",
            exc_info=False,
        )
        return None
    except Exception as e:
    ...
    try:
        ...
        if exchange.has.get("fetchPositions"):
            try:
                ...
                lg.debug(
                    f"Fetched single symbol position data for {symbol} (ID: {market_id}). Count: {len(positions)}"
                )
            except ccxt.ArgumentsRequired:
                # If fetchPositions requires no args (fetches all), set flag
                lg.debug(
                    f"fetchPositions for {exchange.id} requires no arguments. Fetching all."
                )
                fetch_all = True
            except ccxt.ExchangeError as e:
                ...
                no_pos_codes_v5 = [
                    110025
                ]  # Bybit V5: Position idx not match / Position is closed
                no_pos_messages = [
                    "position not found",
                    "position is closed",
                    "no position found",
                ...
                if any(msg in err_str for msg in no_pos_messages) or (
                    bybit_code in no_pos_codes_v5
                ):
                    lg.info(
                        f"No position found for {symbol} (Exchange confirmed: {e})."
                    )
                    return None  # Confirmed no position
                else:
                    # Log other exchange errors but consider them temporary failures
                    lg.error(
                        f"Exchange error fetching single position for {symbol}: {e}",
                        exc_info=False,
                    )
                ...
                lg.error(
                    f"Error fetching single position for {symbol}: {e}", exc_info=True
                )
                fetch_all = True  # Fallback to fetching all positions
                # return None # Option: Treat error as failure
        else:
            lg.warning(
                f"Exchange {exchange.id} does not support fetchPositions. Cannot check position status."
            )
            return None
        ...
        if fetch_all:
            ...
            try:
                ...
                lg.debug(
                    f"Fetched {len(all_positions)} total positions, found {len(positions)} matching {symbol}."
                )
            except Exception as e:
                lg.error(
                    f"Error fetching all positions for {symbol}: {e}", exc_info=True
                )
        ...
        try:
            ...
            if amount_prec_val is not None:
                ...
                if isinstance(amount_prec_val, (float, str)):
                    ...
                    if amount_step > 0:
                        size_threshold = amount_step / Decimal("2")
                elif isinstance(amount_prec_val, int):  # Decimal places
                    if amount_prec_val >= 0:
                        size_threshold = Decimal(
                            "1e-" + str(amount_prec_val + 1)
                        )  # One smaller decimal place
        except Exception as thresh_err:
            lg.warning(
                f"Could not determine size threshold from market precision ({thresh_err}). Using default: {size_threshold}"
            )
        ...
        for pos in positions:
            ...
            if pos.get("contracts") is not None:
                ...
                isinstance(pos.get("info"), dict)
                and pos["info"].get("size") is not None
            ):
            ...
            try:
                ...
                if abs(position_size) > size_threshold:
                    ...
                    active_position = pos
                    lg.debug(
                        f"Found potential active position entry for {symbol} with size {position_size}."
                    )
                ...
                lg.warning(
                    f"Could not parse position size '{pos_size_str}' for {symbol}: {parse_err}"
                )
                continue
            ...
            try:
                ...
                size_decimal = Decimal(
                    str(
                        active_position.get(
                            "contracts",
                            active_position.get("info", {}).get("size", "0"),
                        )
                    )
                )
                ...
                if side not in ["long", "short"]:
                    if size_decimal > size_threshold:
                        ...
                        lg.warning(
                            f"Position size {size_decimal} near zero for {symbol}, cannot reliably determine side."
                        )
                        return None  # Cannot use position if side unknown
                    active_position["side"] = side  # Store inferred side
                    lg.debug(
                        f"Inferred position side as '{side}' based on size {size_decimal}."
                    )

                # Standardize Entry Price
                entry_price_str = active_position.get(
                    "entryPrice"
                ) or active_position.get("info", {}).get("avgPrice")
                if entry_price_str:
                    active_position["entryPriceDecimal"] = Decimal(str(entry_price_str))
                else:
                    active_position["entryPriceDecimal"] = None
                ...
                active_position["trailingStopLossValue"] = info_dict.get(
                    "trailingStop"
                )  # Bybit V5: distance value
                active_position["trailingStopActivationPrice"] = info_dict.get(
                    "activePrice"
                )  # Bybit V5: activation price

                # Get timestamp (usually milliseconds)
                timestamp_ms = active_position.get("timestamp") or info_dict.get(
                    "updatedTime"
                ...
                def format_log_val(val, is_price=True, is_size=False):
                    ...
                    try:
                        ...
                        if is_size:
                            ...
                            try:
                                ...
                                if isinstance(amt_prec_val, (float, str)):
                                    ...
                                    if amt_step > 0:
                                        amt_prec = abs(
                                            amt_step.normalize().as_tuple().exponent
                                        )
                                elif isinstance(amt_prec_val, int):
                                    amt_prec = amt_prec_val
                            except Exception:
                                pass  # Ignore errors getting precision
                        ...
                        return str(val)  # Fallback to string if conversion fails

                entry_price_fmt = format_log_val(
                    active_position.get("entryPriceDecimal")
                )
                contracts_fmt = format_log_val(size_decimal, is_size=True)
                liq_price_fmt = format_log_val(active_position.get("liquidationPrice"))
                leverage_str = active_position.get(
                    "leverage", info_dict.get("leverage")
                )
                leverage_fmt = (
                    f"{Decimal(str(leverage_str)):.1f}x"
                    if leverage_str is not None
                    else "N/A"
                )
                pnl_fmt = format_log_val(
                    active_position.get("unrealizedPnl"), is_price=False
                )
                ...
                tsl_dist_fmt = format_log_val(
                    active_position.get("trailingStopLossValue"), is_price=False
                )  # TSL distance is a value/rate
                tsl_act_fmt = format_log_val(
                    active_position.get("trailingStopActivationPrice")
                )

                logger.info(
                    f"{NEON_GREEN}Active {side.upper()} position found ({symbol}):{RESET} "
                ...
                lg.error(
                    f"Error processing active position details for {symbol} (Decimal/Type Error): {proc_err}",
                    exc_info=False,
                )
                ...
                lg.error(
                    f"Error processing active position details for {symbol}: {proc_err}",
                    exc_info=True,
                )
                lg.debug(f"Problematic position data: {active_position}")
                return None  # Failed to process essential details
...
def set_leverage_ccxt(
    ...
    if not exchange.has.get("setLeverage"):
        ...
        if not exchange.has.get("setMarginMode"):
            lg.error(
                f"{NEON_RED}Exchange {exchange.id} does not support setLeverage or setMarginMode via CCXT. Cannot set leverage.{RESET}"
            )
            return False
        else:
            lg.warning(
                f"{NEON_YELLOW}Exchange {exchange.id} uses setMarginMode for leverage. Attempting via setMarginMode...{RESET}"
            )
    ...
    try:
        ...
        # else:
        lg.info(
            f"{NEON_GREEN}Leverage for {symbol} set/requested to {leverage}x (Check position details for confirmation).{RESET}"
        )
        return True
        ...
        err_str = str(e).lower()
        exchange_code = getattr(
            e, "code", None
        )  # Get exchange-specific error code if available
        lg.error(
            f"{NEON_RED}Exchange error setting leverage for {symbol}: {e} (Code: {exchange_code}){RESET}"
        )

        # Add hints for common errors based on exchange and code
        if "bybit" in exchange.id.lower():
            # Bybit V5: 110043 means "Leverage not modified" (already set)
            if exchange_code == 110043 or "leverage not modified" in err_str:
                lg.info(
                    f"{NEON_YELLOW}Leverage for {symbol} likely already set to {leverage}x (Exchange confirmation).{RESET}"
                )
                return True  # Treat as success
            elif exchange_code in [110028, 110009, 110055] or "margin mode" in err_str:
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Check Margin Mode (Isolated/Cross) compatibility with leverage setting. May need to set margin mode first.{RESET}"
                )
            elif exchange_code == 110044 or "risk limit" in err_str:
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Leverage {leverage}x may exceed the current risk limit tier for {symbol}. Check Bybit Risk Limits.{RESET}"
                )
            elif exchange_code == 110013 or "parameter error" in err_str:
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Leverage value {leverage}x might be invalid (e.g., too high) for {symbol}. Check allowed range.{RESET}"
                )
            elif "set margin mode" in err_str:
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Operation might require setting margin mode first/again using `set_margin_mode`.{RESET}"
                )
        ...
        # Catch any other unexpected errors
        lg.error(
            f"{NEON_RED}Unexpected error setting leverage for {symbol}: {e}{RESET}",
            exc_info=True,
        )
...
def place_trade(
    ...
    try:
        ...
        size_str_formatted = exchange.amount_to_precision(
            symbol, f"{position_size:.{getcontext().prec}f}"
        )
        amount_decimal = Decimal(size_str_formatted)

        if amount_decimal <= 0:
            lg.error(
                f"Trade aborted ({symbol} {side} {action_desc}): Invalid or zero position size after formatting ({amount_decimal}). Original: {position_size}"
            )
            return None
        ...
        lg.error(
            f"Trade aborted ({symbol} {side} {action_desc}): Failed to format/convert size {position_size}: {e}"
        )
        return None
    ...
    if order_type == "limit":
        if (
            limit_price is None
            or not isinstance(limit_price, Decimal)
            or limit_price <= 0
        ):
            lg.error(
                f"Trade aborted ({symbol} {side} {action_desc}): Limit order requested but invalid limit_price ({limit_price}) provided."
            )
            return None
        try:
            # Format limit price according to exchange precision
            price_str_formatted = exchange.price_to_precision(
                symbol, float(limit_price)
            )
            price_for_api = float(
                price_str_formatted
            )  # CCXT usually expects float price
            if price_for_api <= 0:
                raise ValueError("Formatted limit price is non-positive")
        except (ccxt.ExchangeError, ValueError, TypeError) as e:
            lg.error(
                f"Trade aborted ({symbol} {side} {action_desc}): Failed to format/validate limit price {limit_price}: {e}"
            )
            return None
    ...
    lg.info(
        f"  Size: {amount_for_api} {size_unit}"
    )  # Log the formatted size being sent
    if order_type == "limit" and price_str_formatted:
        lg.info(f"  Limit Price: {price_str_formatted}")  # Log formatted price
    ...
    try:
        ...
        if order:
            ...
            # Safely get 'filled' amount, default to 0.0 if None or missing
            filled_amount_raw = order.get("filled")
            filled_amount = (
                float(filled_amount_raw) if filled_amount_raw is not None else 0.0
            )
            avg_price = order.get(
                "average"
            )  # Price at which it was filled (if applicable)

            lg.info(f"{NEON_GREEN}{action_desc} Trade Placed Successfully!{RESET}")
            lg.info(f"  Order ID: {order_id}, Initial Status: {order_status}")
            # Check if filled_amount is not None before comparison (though default should handle it)
            if filled_amount is not None and filled_amount > 0:
                lg.info(f"  Filled Amount: {filled_amount}")
            if avg_price:
                lg.info(f"  Average Fill Price: {avg_price}")
            ...
            lg.error(
                f"{NEON_RED}Order placement call returned None without raising an exception for {symbol}.{RESET}"
            )
            return None
    ...
    # --- Handle Specific CCXT Exceptions ---
        ...
        lg.error(
            f"{NEON_RED}Insufficient funds to place {side} {order_type} order ({symbol}): {e}{RESET}"
        )
        ...
        lg.error(
            f"{NEON_RED}Invalid order parameters for {side} {order_type} order ({symbol}): {e}{RESET}"
        )
        lg.error(
            f"  > Used Parameters: amount={amount_for_api}, price={price_for_api if order_type == 'limit' else 'N/A'}, params={order_params}"
        )
        # Add hints based on common InvalidOrder reasons
        if "Order price is not following the tick size" in str(e):
            lg.error("  >> Hint: Check limit_price alignment with market tick size.")
        if "Order size is not following the step size" in str(e):
            lg.error(
                "  >> Hint: Check position_size alignment with market amount step size."
            )
        if (
            "minNotional" in str(e)
            or "cost" in str(e).lower()
            or "minimum value" in str(e).lower()
        ):
            lg.error(
                "  >> Hint: Order cost might be below the minimum required by the exchange."
            )
        ...
        if reduce_only and exchange_code == 110014:  # Bybit: Reduce-only order failed
            lg.error(
                f"{NEON_YELLOW}  >> Hint (Bybit 110014): Reduce-only order failed. Position might be closed, size incorrect, or side wrong?{RESET}"
            )
    except ccxt.NetworkError as e:
        lg.error(
            f"{NEON_RED}Network error placing {action_desc} order ({symbol}): {e}{RESET}"
        )
        # Network errors might warrant a retry mechanism outside this function or careful state management
    except ccxt.ExchangeError as e:
        exchange_code = getattr(e, "code", None)
        lg.error(
            f"{NEON_RED}Exchange error placing {action_desc} order ({symbol}): {e} (Code: {exchange_code}){RESET}"
        )
        # Add hints for common exchange errors
        if reduce_only and exchange_code == 110025:  # Bybit: Position not found/closed
            lg.warning(
                f"{NEON_YELLOW} >> Hint (Bybit 110025): Position might have been closed already when trying to place reduce-only order.{RESET}"
            )
        # Add more specific hints...
    except Exception as e:
        lg.error(
            f"{NEON_RED}Unexpected error placing {action_desc} order ({symbol}): {e}{RESET}",
            exc_info=True,
        )
...
def _set_position_protection(
    ...
    trailing_stop_distance: Optional[
        Decimal
    ] = None,  # Distance in price points (Decimal)
    ...
    if "bybit" not in exchange.id.lower():
        lg.error(
            f"Protection setting via private_post is currently implemented only for Bybit. Cannot set for {exchange.id}."
        )
        return False
    ...
    if pos_side not in ["long", "short"]:
        lg.error(
            f"Cannot set protection for {symbol}: Invalid or missing position side ('{pos_side}') in position_info."
        )
        return False
    ...
    try:
        ...
        if pos_idx_val is not None:
            ...
            lg.debug(
                f"Using positionIdx: {position_idx} from position info for {symbol}."
            )
        else:
            lg.debug(
                f"positionIdx not found in position_info['info'] for {symbol}. Using default {position_idx} (assuming One-Way mode)."
            )
        # Add logic here if positionIdx is stored differently for your Bybit mode/version
    except Exception as idx_err:
        lg.warning(
            f"Could not parse positionIdx from position info ({idx_err}), using default {position_idx}."
        )
    ...
    has_tsl = (
        isinstance(trailing_stop_distance, Decimal)
        and trailing_stop_distance > 0
        and isinstance(tsl_activation_price, Decimal)
    ...
    if not has_sl and not has_tp and not has_tsl:
        lg.info(
            f"No valid protection parameters provided for {symbol} (PosIdx: {position_idx}). No protection set/updated."
        )
    ...
    log_parts = [
        f"Attempting to set protection for {symbol} ({pos_side.upper()} PosIdx: {position_idx}):"
    ]

    # --- Format and Add Protection Parameters ---
    try:
        # Use static methods for precision/tick helpers
        price_precision = TradingAnalyzer.get_price_precision(market_info, lg)
        min_tick = TradingAnalyzer.get_min_tick_size(market_info, lg)

        # Helper to format price using ccxt's price_to_precision
        def format_price(price_decimal: Optional[Decimal]) -> Optional[str]:
            if not isinstance(price_decimal, Decimal) or price_decimal <= 0:
                return None
            try:
                ...
                lg.warning(
                    f"Failed to format price {price_decimal} using price_to_precision: {e}. Price will not be set."
                )
                return None
        ...
        if has_tsl:
            ...
            try:
                ...
                dist_prec = (
                    abs(min_tick.normalize().as_tuple().exponent)
                    if min_tick > 0
                ...
                if min_tick > 0 and Decimal(formatted_tsl_distance) < min_tick:
                    lg.warning(
                        f"Calculated TSL distance {formatted_tsl_distance} is less than min tick {min_tick}. Adjusting to min tick."
                    )
                    formatted_tsl_distance = str(min_tick)  # Use min tick as string
            except (ccxt.ExchangeError, InvalidOperation, ValueError, TypeError) as e:
                lg.warning(
                    f"Failed to format TSL distance {trailing_stop_distance} using decimal_to_precision: {e}. TSL distance will not be set."
                )
            ...
            if (
                formatted_tsl_distance
                and formatted_activation_price
                and Decimal(formatted_tsl_distance) > 0
            ):
                params["trailingStop"] = formatted_tsl_distance
                params["activePrice"] = formatted_activation_price
                log_parts.append(
                    f"  Trailing SL: Dist={formatted_tsl_distance}, Act={formatted_activation_price}"
                )
                ...
                has_sl = False
                lg.debug(
                    "TSL parameters added. Fixed SL will be ignored by the exchange."
                )
            else:
                lg.error(
                    f"Failed to format valid TSL parameters for {symbol}. TSL will not be set."
                )
        ...
        lg.error(
            f"Error during formatting/preparation of protection parameters for {symbol}: {fmt_err}",
            exc_info=True,
        )
    ...
    if (
        not params.get("stopLoss")
        and not params.get("takeProfit")
        and not params.get("trailingStop")
    ):
        lg.warning(
            f"No valid protection parameters could be formatted or remained after adjustments for {symbol} (PosIdx: {position_idx}). No API call made."
        )
        ...
        initial_protection_requested = (
            stop_loss_price
            or take_profit_price
            or (trailing_stop_distance and tsl_activation_price)
        )
    ...
    lg.info("\n".join(log_parts))
    lg.debug(
        f"  API Call: exchange.private_post('/v5/position/set-trading-stop', params={params})"
    )
    ...
    try:
        ...
        if ret_code == 0:
            ...
            if "not modified" in ret_msg.lower():
                ...
                lg.info(
                    f"{NEON_YELLOW}Position protection already set to target values or only partially modified for {symbol} (PosIdx: {position_idx}). Response: {ret_msg}{RESET}"
                )
            else:
                # Generic success message
                lg.info(
                    f"{NEON_GREEN}Position protection (SL/TP/TSL) set/updated successfully for {symbol} (PosIdx: {position_idx}).{RESET}"
                )
            return True  # Success
        else:
            # API call failed, log error details
            lg.error(
                f"{NEON_RED}Failed to set protection for {symbol} (PosIdx: {position_idx}): {ret_msg} (Code: {ret_code}) Ext: {ret_ext}{RESET}"
            )
            # Add hints for common Bybit V5 error codes for this endpoint
            if ret_code == 110013:  # Parameter error
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110013 - Parameter Error): Check SL/TP prices vs entry price, TSL distance/activation validity, tick size compliance, tpslMode.{RESET}"
                )
            elif ret_code == 110036:  # TSL Active price invalid
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110036 - TSL Price Invalid): TSL Activation price '{params.get('activePrice')}' likely invalid (already passed? wrong side? too close to current price?).{RESET}"
                )
            elif ret_code == 110086:  # SL Price cannot be equal to TP price
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110086): Stop Loss price cannot be equal to Take Profit price.{RESET}"
                )
            elif (
                ret_code == 110043
            ):  # Position status is not normal (e.g., leverage not modified)
                # Check message specifically for leverage issue
                if "leverage not modified" in ret_msg.lower():
                    lg.info(
                        f"{NEON_YELLOW} >> Hint (110043 - Leverage Not Modified): Leverage likely already set. Protection setting might still have failed due to other reasons.{RESET}"
                    )
                else:
                    lg.error(
                        f"{NEON_YELLOW} >> Hint (110043): Position status prevents modification (e.g., during liquidation?).{RESET}"
                    )
            elif ret_code == 110025:  # Position not found / closed
                lg.error(
                    f"{NEON_YELLOW} >> Hint (110025): Position may have closed before protection could be set, or positionIdx mismatch?{RESET}"
                )
            elif (
                "trailing stop value invalid" in ret_msg.lower()
            ):  # Check message substring
                lg.error(
                    f"{NEON_YELLOW} >> Hint: Trailing Stop distance '{params.get('trailingStop')}' likely invalid (too small? too large? violates tick size rules?).{RESET}"
                )
        ...
        lg.error(
            f"{NEON_RED}Authentication error during protection API call for {symbol}: {e}{RESET}"
        )
        # Auth errors usually mean API keys are wrong/expired, not retryable
        return False
    except ccxt.NetworkError as e:
        lg.error(
            f"{NEON_RED}Network error during protection API call for {symbol}: {e}{RESET}"
        )
        ...
        lg.error(
            f"{NEON_RED}Unexpected error during protection API call for {symbol}: {e}{RESET}",
            exc_info=True,
        )
        return False  # Failure
...
def set_trailing_stop_loss(
    ...
    if not config.get("enable_trailing_stop", False):
        lg.info(
            f"Trailing Stop Loss is disabled in config for {symbol}. Skipping TSL setup."
        )
    ...
    try:
        ...
        callback_rate_str = config.get(
            "trailing_stop_callback_rate", "0.005"
        )  # e.g., 0.5%
        activation_perc_str = config.get(
        ...
        lg.error(
            f"{NEON_RED}Invalid TSL parameter format in config ({symbol}): {e}. Cannot calculate TSL.{RESET}"
        )
        lg.error(
            f"  >> Check 'trailing_stop_callback_rate' ({callback_rate_str}) and 'trailing_stop_activation_percentage' ({activation_perc_str})."
        )
        return False
    # Ensure callback rate is positive
    if callback_rate <= 0:
        lg.error(
            f"{NEON_RED}Invalid 'trailing_stop_callback_rate' ({callback_rate}) in config. Must be positive for {symbol}.{RESET}"
        )
        return False
    # Ensure activation percentage is non-negative
    if activation_percentage < 0:
        lg.error(
            f"{NEON_RED}Invalid 'trailing_stop_activation_percentage' ({activation_percentage}) in config. Cannot be negative for {symbol}.{RESET}"
        )
        return False
    ...
    try:
        ...
        if (
            entry_price is None
            or not isinstance(entry_price, Decimal)
            or entry_price <= 0
        ):
            lg.error(
                f"{NEON_RED}Missing or invalid entry price ({entry_price}) in position info for TSL calc ({symbol}).{RESET}"
            )
            return False
        if side not in ["long", "short"]:
            lg.error(
                f"{NEON_RED}Missing or invalid position side ('{side}') in position info for TSL calc ({symbol}).{RESET}"
            )
            return False
    except Exception as e:
        # Catch errors if position_info structure is unexpected
        lg.error(
            f"{NEON_RED}Error accessing position info for TSL calculation ({symbol}): {e}.{RESET}"
        )
    ...
    try:
        ...
        if side == "long":
            ...
            if min_tick_size > 0:
                activation_price = (raw_activation / min_tick_size).quantize(
                    Decimal("1"), rounding=ROUND_UP
                ) * min_tick_size
            else:  # Fallback to decimal places
                rounding_factor = Decimal("1e-" + str(price_precision))
                activation_price = raw_activation.quantize(
                    rounding_factor, rounding=ROUND_UP
                )
            ...
            if (
                ...
                and min_tick_size > 0
                and activation_price <= entry_price
            ):
                activation_price = (
                    (entry_price + min_tick_size) / min_tick_size
                ).quantize(Decimal("1"), rounding=ROUND_UP) * min_tick_size
                lg.debug(
                    f"Adjusted LONG TSL activation price to be at least one tick above entry: {activation_price}"
                )
            # For immediate activation (0%), set slightly above entry to meet exchange requirement (usually needs to be different from entry)
                ...
                lg.debug(
                    f"Immediate TSL activation (0%) requested. Setting activation slightly above entry: {activation_price}"
                )
            ...
            if (
                ...
                lg.debug(
                    f"Adjusted SHORT TSL activation price to be at least one tick below entry: {activation_price}"
                )
            # For immediate activation (0%), set slightly below entry
                ...
                lg.debug(
                    f"Immediate TSL activation (0%) requested. Setting activation slightly below entry: {activation_price}"
                )
        ...
        if min_tick_size > 0:
            trailing_distance = (trailing_distance_raw / min_tick_size).quantize(
                Decimal("1"), rounding=ROUND_UP
            ) * min_tick_size
        else:
            # Should not happen, but fallback if tick size is zero
            lg.warning(
                "Min tick size is zero, cannot round trailing distance accurately."
            )
        ...
        if min_tick_size > 0 and trailing_distance < min_tick_size:
            lg.warning(
                f"Calculated TSL distance {trailing_distance} is smaller than min tick {min_tick_size}. Adjusting to min tick."
            )
            trailing_distance = min_tick_size
        # Ensure distance is positive
        if trailing_distance <= 0:
            lg.error(
                f"{NEON_RED}Calculated TSL distance is zero or negative ({trailing_distance}) for {symbol}. Cannot set TSL.{RESET}"
            )
            return False
        ...
        lg.info(
            f"  Entry={entry_price:.{price_precision}f}, Act%={activation_percentage:.3%}, Callback%={callback_rate:.3%}"
        )
        lg.info(
            f"  => Activation Price (Target): {activation_price:.{price_precision}f}"
        )
        lg.info(
            f"  => Trailing Distance (Target): {trailing_distance:.{price_precision}f}"
        )  # Log distance with price precision
        ...
        if isinstance(take_profit_price, Decimal) and take_profit_price > 0:
            ...
            lg.info(
                f"  Take Profit Price (Target): {tp_fmt} (Will be set simultaneously)"
            )
        else:
        ...
        lg.error(
            f"{NEON_RED}Error calculating or preparing TSL parameters for {symbol} (Decimal/Type Error): {e}{RESET}",
            exc_info=False,
        )
        return False
    except Exception as e:
        ...
        lg.error(
            f"{NEON_RED}Unexpected error calculating or preparing TSL parameters for {symbol}: {e}{RESET}",
            exc_info=True,
        )
        return False
...
def analyze_and_trade_symbol(
    ...
    if not market_info:
        lg.error(
            f"{NEON_RED}Failed to get market info for {symbol}. Skipping cycle.{RESET}"
        )
    ...
    if not ccxt_interval:
        lg.error(
            f"Invalid interval '{config['interval']}' in config. Cannot map to CCXT timeframe for {symbol}. Skipping cycle."
        )
        return
    ...
    ):  # Check for minimum reasonable data length
        lg.error(
            f"{NEON_RED}Failed to fetch sufficient kline data for {symbol} (fetched {len(klines_df)}). Skipping cycle.{RESET}"
        )
        return
    ...
    if current_price is None:
        ...
        try:
            ...
            if not isinstance(klines_df.index, pd.DatetimeIndex):
                lg.error(
                    f"{NEON_RED}Kline DataFrame index is not DatetimeIndex. Cannot reliably get last close.{RESET}"
                )
                return
            ...
            if pd.notna(last_close_val) and last_close_val > 0:
                ...
                lg.error(
                    f"{NEON_RED}Last close price from klines is invalid ({last_close_val}). Cannot proceed without current price.{RESET}"
                )
                return
        except IndexError:
            lg.error(
                f"{NEON_RED}Kline DataFrame is empty or index error getting last close.{RESET}"
            )
            return
        except Exception as e:
            lg.error(
                f"{NEON_RED}Error getting last close price from klines: {e}. Cannot proceed.{RESET}"
            )
            return
    ...
    orderbook_weight = Decimal(
        str(active_weights.get("orderbook", "0"))
    )  # Default weight 0
    if orderbook_enabled and orderbook_weight != 0:
        ...
        orderbook_data = fetch_orderbook_ccxt(
            exchange, symbol, config["orderbook_limit"], lg
        )
        if not orderbook_data:
            lg.warning(
                f"{NEON_YELLOW}Failed to fetch orderbook data for {symbol}, proceeding without it.{RESET}"
            )
    else:
    ...
    lg.info(
        f"ATR: {current_atr:.{price_precision + 1}f}"
        if isinstance(current_atr, Decimal)
        else "ATR: N/A"
    )
    lg.info(f"Calculated Initial SL (for sizing): {sl_calc if sl_calc else 'N/A'}")
    lg.info(
        f"Calculated Initial TP (potential target): {tp_calc if tp_calc else 'N/A'}"
    )
    ...
    lg.info(
        f"Position Management: TSL={'Enabled' if tsl_enabled else 'Disabled'}, BE={'Enabled' if be_enabled else 'Disabled'}, TimeExit={time_exit_str}"
    )
    ...
    if not config.get("enable_trading", False):
        ...
        lg.debug(
            f"---== Analysis Cycle End ({symbol}, {cycle_end_time - cycle_start_time:.2f}s) ==---"
        )
        return
    ...
    if open_position is None:
        # Check max concurrent positions (This check might be better placed globally if managing multiple symbols)
        # For now, assume it applies per symbol as implemented
        # if len(get_all_open_positions(exchange, lg)) >= config.get("max_concurrent_positions", 1): # Needs a function to get all positions
        #     lg.info(f"Global max concurrent positions reached. Not entering new position for {symbol}.")
        #     return
        ...
        if signal in ["BUY", "SELL"]:
            ...
            if balance is None or balance <= 0:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Cannot fetch balance or balance is zero/negative.{RESET}"
                )
            ...
            if sl_calc is None:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Initial SL calculation failed (ATR invalid?). Cannot calculate position size.{RESET}"
                )
                return  # Stop trade sequence
            ...
            if market_info.get("is_contract", False):
                ...
                if leverage > 0:
                    if not set_leverage_ccxt(
                        exchange, symbol, leverage, market_info, lg
                    ):
                        lg.error(
                            f"{NEON_RED}Trade Aborted ({symbol} {signal}): Failed to set leverage to {leverage}x.{RESET}"
                        )
                        return  # Stop trade sequence
                else:
                    lg.info(
                        f"Leverage setting skipped: Leverage config is zero or negative ({leverage})."
                    )
            else:
            ...
            if position_size is None or position_size <= 0:
                lg.error(
                    f"{NEON_RED}Trade Aborted ({symbol} {signal}): Position size calculation failed or resulted in zero/negative ({position_size}).{RESET}"
                )
                return  # Stop trade sequence
            ...
            if entry_order_type == "limit":
                offset_buy = Decimal(
                    str(config.get("limit_order_offset_buy", "0.0005"))
                )
                offset_sell = Decimal(
                    str(config.get("limit_order_offset_sell", "0.0005"))
                )

                if signal == "BUY":
                    raw_limit = current_price * (Decimal(1) - offset_buy)
                    # Round down for buy limit (get a potentially better price)
                    # Quantize using tick size if available
                    if min_tick_size > 0:
                        limit_entry_price = (raw_limit / min_tick_size).quantize(
                            Decimal("1"), rounding=ROUND_DOWN
                        ) * min_tick_size
                    else:  # Fallback
                        rounding_factor = Decimal("1e-" + str(price_precision))
                        limit_entry_price = raw_limit.quantize(
                            rounding_factor, rounding=ROUND_DOWN
                        )
                else:  # SELL
                ...
                if limit_entry_price <= 0:
                    lg.error(
                        f"{NEON_RED}Trade Aborted ({symbol} {signal}): Calculated limit entry price is non-positive ({limit_entry_price}). Switching to Market order.{RESET}"
                    )
                    entry_order_type = "market"
                    limit_entry_price = None
                else:
                    lg.info(
                        f"Calculated Limit Entry Price for {signal}: {limit_entry_price}"
                    )

            # 6. Place Entry Order
            lg.info(
                f"==> Placing {signal} {entry_order_type.upper()} order | Size: {position_size} <=="
            )
            trade_order = place_trade(
                exchange=exchange,
                symbol=symbol,
                trade_signal=signal,
                position_size=position_size,  # Pass the calculated Decimal size
                market_info=market_info,
                logger=lg,
                order_type=entry_order_type,
                limit_price=limit_entry_price,  # Pass Decimal limit price
                reduce_only=False,
            ...
            if trade_order and trade_order.get("id"):
                ...
                order_status = trade_order.get(
                    "status"
                )  # 'open', 'closed', 'canceled', etc.
                ...
                if entry_order_type == "market":
                    ...
                    lg.info(
                        f"Market order {order_id} placed. Waiting {confirm_delay}s for position confirmation..."
                    )
                    time.sleep(confirm_delay)

                    lg.info(
                        f"Attempting position confirmation for {symbol} after market order {order_id}..."
                    )
                    # Fetch position again after delay
                    confirmed_position = get_open_position(
                        exchange, symbol, market_info, lg
                    )

                    if confirmed_position:
                        lg.info(
                            f"{NEON_GREEN}Position Confirmed after Market Order!{RESET}"
                        )
                        # Proceed to set protection based on actual entry price
                        try:
                            entry_price_actual = confirmed_position.get(
                                "entryPriceDecimal"
                            )
                            if entry_price_actual is None or entry_price_actual <= 0:
                                lg.warning(
                                    f"Could not get valid actual entry price from confirmed position. Using initial estimate {current_price} for protection."
                                )
                                entry_price_actual = current_price  # Fallback

                            lg.info(
                                f"Actual Entry Price: ~{entry_price_actual:.{price_precision}f}"
                            )

                            # Recalculate protection levels based on ACTUAL entry price
                            _, tp_final, sl_final = analyzer.calculate_entry_tp_sl(
                                entry_price_actual, signal
                            )
                            ...
                            if config.get("enable_trailing_stop", False):
                                lg.info(
                                    f"Setting Exchange Trailing Stop Loss (TP target: {tp_final})..."
                                )
                                ...
                                lg.info(
                                    f"Setting Fixed SL ({sl_final}) and TP ({tp_final})..."
                                )
                                if sl_final or tp_final:  # Only set if values are valid
                                    ...
                                    lg.warning(
                                        f"{NEON_YELLOW}Fixed SL/TP calculation failed based on actual entry or returned None. No fixed protection set.{RESET}"
                                    )

                            # Log final status
                            if protection_set_success:
                                lg.info(
                                    f"{NEON_GREEN}=== TRADE ENTRY & PROTECTION SETUP COMPLETE ({symbol} {signal}) ==="
                                )
                            else:
                                lg.error(
                                    f"{NEON_RED}=== TRADE placed BUT FAILED TO SET PROTECTION ({symbol} {signal}) ==="
                                )
                                lg.warning(
                                    f"{NEON_YELLOW}>>> MANUAL MONITORING REQUIRED! <<<"
                                )

                        except Exception as post_trade_err:
                            lg.error(
                                f"{NEON_RED}Error during post-trade protection setting ({symbol}): {post_trade_err}{RESET}",
                                exc_info=True,
                            )
                            lg.warning(
                                f"{NEON_YELLOW}Position is open but protection setup failed. Manual check needed!{RESET}"
                            )
                    else:
                        # Market order placed, but position not confirmed
                        lg.error(
                            f"{NEON_RED}Market trade order {order_id} placed, but FAILED TO CONFIRM open position after {confirm_delay}s delay!{RESET}"
                        )
                        lg.warning(
                            f"{NEON_YELLOW}Order may have failed, been rejected, or API/exchange delay. Manual investigation required!{RESET}"
                        )
                ...
                # If Limit Order -> Log and wait for next cycle (or implement order monitoring)
                    ...
                    if order_status == "open":
                        lg.info(
                            f"Limit order {order_id} placed successfully and is OPEN."
                        )
                        lg.info(
                            "Will check status and set protection on next cycle if filled."
                        )
                        ...
                        lg.warning(
                            f"Limit order {order_id} filled immediately (status: {order_status}). Attempting confirmation..."
                        )
                        # Treat like market order - try to confirm and set protection
                        confirm_delay = config.get(
                            "position_confirm_delay_seconds",
                            POSITION_CONFIRM_DELAY_SECONDS,
                        )  # Short delay even for limit fill
                        time.sleep(confirm_delay)
                        lg.info(
                            f"Attempting position confirmation for {symbol} after immediate limit fill {order_id}..."
                        )
                        confirmed_position = get_open_position(
                            exchange, symbol, market_info, lg
                        )
                        if confirmed_position:
                            lg.info(
                                f"{NEON_GREEN}Position Confirmed after Immediate Limit Fill!{RESET}"
                            )
                            # Proceed to set protection based on actual entry price
                            try:
                                entry_price_actual = confirmed_position.get(
                                ...
                                # Use actual fill price if available, else fallback to intended limit price
                                if (
                                    entry_price_actual is None
                                    or entry_price_actual <= 0
                                ):
                                    lg.warning(
                                        f"Could not get valid actual entry price from confirmed position. Using intended limit price {limit_entry_price} for protection."
                                    )
                                    entry_price_actual = (
                                        limit_entry_price  # Fallback to intended limit
                                    )
                                ...
                                if (
                                    ...
                                    raise ValueError(
                                        "Cannot determine entry price for protection setup."
                                    )
                                ...
                                if config.get("enable_trailing_stop", False):
                                    ...
                                    if sl_final or tp_final:
                                        ...
                                        lg.warning(
                                            f"{NEON_YELLOW}Fixed SL/TP calculation failed. No fixed protection set.{RESET}"
                                        )
                                ...
                                lg.warning(
                                    f"{NEON_YELLOW}Position open but protection setup failed. Manual check needed!{RESET}"
                                )
                        else:
                            lg.error(
                                f"{NEON_RED}Limit order {order_id} reported 'closed', but FAILED TO CONFIRM open position! Manual check needed!{RESET}"
                            )
                    else:
                        # Limit order failed or was cancelled immediately
                        lg.error(
                            f"Limit order {order_id} placement resulted in status: {order_status}. Trade did not open."
                        )
            else:
                # place_trade returned None
                lg.error(
                    f"{NEON_RED}=== TRADE EXECUTION FAILED ({symbol} {signal}). Order placement function returned None. See previous logs. ==="
                )
        else:  # signal == HOLD
            lg.info(
                f"Signal is HOLD and no open position for {symbol}. No entry action taken."
            )
    ...
    # ==============================================
        ...
        if (
            ...
            lg.error(
                f"{NEON_RED}Cannot manage position for {symbol}: Invalid details retrieved (Side: {pos_side}, Size: {pos_size}, Entry: {entry_price}). Skipping management.{RESET}"
            )
            lg.debug(f"Problematic position data: {open_position}")
            return

        lg.info(
            f"Managing existing {pos_side.upper()} position for {symbol}. Size: {pos_size}, Entry: {entry_price}"
        )
        ...
        if exit_signal_triggered:
            lg.warning(
                f"{NEON_YELLOW}*** EXIT Signal Triggered: New signal ({signal}) opposes existing {pos_side} position. Closing position... ***{RESET}"
            )
            try:
                ...
                if size_to_close <= 0:
                    raise ValueError(
                        f"Position size to close is zero or negative ({size_to_close}). Cannot close."
                    )

                lg.info(
                    f"==> Placing {close_side_signal} MARKET order (reduceOnly=True) | Size: {size_to_close} <=="
                )
                ...
                if close_order:
                    lg.info(
                        f"{NEON_GREEN}Position CLOSE order placed successfully for {symbol}. Order ID: {close_order.get('id', 'N/A')}{RESET}"
                    )
                    ...
                    lg.error(
                        f"{NEON_RED}Failed to place CLOSE order for {symbol}. Manual check/intervention required!{RESET}"
                    )
                    # Consider what state the bot should be in - maybe skip further management this cycle

            except Exception as close_err:
                lg.error(
                    f"{NEON_RED}Error attempting to close position {symbol}: {close_err}{RESET}",
                    exc_info=True,
                )
                lg.warning(
        ...
        # --- If No Exit Signal, Proceed with Position Management ---
            ...
            lg.info(
                f"Signal ({signal}) allows holding the existing {pos_side} position. Performing position management checks..."
            )
            ...
            if time_exit_minutes_config and time_exit_minutes_config > 0:
                if pos_timestamp_ms:
                    try:
                        ...
                        time_elapsed_ms = current_time_ms - float(
                            pos_timestamp_ms
                        )  # Ensure float conversion
                        time_elapsed_minutes = time_elapsed_ms / (1000 * 60)

                        lg.debug(
                            f"Time-Based Exit Check: Elapsed = {time_elapsed_minutes:.2f} min, Limit = {time_exit_minutes_config} min"
                        )

                        if time_elapsed_minutes >= time_exit_minutes_config:
                            lg.warning(
                                f"{NEON_YELLOW}*** TIME-BASED EXIT Triggered ({time_elapsed_minutes:.1f} >= {time_exit_minutes_config} min). Closing position... ***{RESET}"
                            )
                            ...
                            if size_to_close > 0:
                                ...
                                if close_order:
                                    lg.info(
                                        f"{NEON_GREEN}Time-based CLOSE order placed successfully for {symbol}. ID: {close_order.get('id', 'N/A')}{RESET}"
                                    )
                                else:
                                    lg.error(
                                        f"{NEON_RED}Failed to place time-based CLOSE order for {symbol}. Manual check required!{RESET}"
                                    )
                            else:
                                lg.warning(
                                    "Time-based exit triggered but position size is zero. No close order placed."
                                )
                            # Exit management logic after triggering time-based close
                            return
                    except (ValueError, TypeError) as time_conv_err:
                        lg.error(
                            f"{NEON_RED}Error converting position timestamp for time-based exit check: {pos_timestamp_ms} -> {time_conv_err}{RESET}"
                        )
                    except Exception as time_err:
                        lg.error(
                            f"{NEON_RED}Error during time-based exit check: {time_err}{RESET}"
                        )
                else:
                    lg.warning(
                        "Time-based exit enabled, but position timestamp not found. Cannot perform check."
                    )
            ...
            try:
                ...
                if (
                    tsl_value_str
                    and str(tsl_value_str).strip()
                    and str(tsl_value_str) != "0"
                ):  # Check if not None, empty or '0' string
                    tsl_value = Decimal(str(tsl_value_str))
                    if tsl_value > 0:
                        is_tsl_active_exchange = True
                        lg.debug(
                            "Exchange Trailing Stop Loss appears to be active for this position."
                        )
            except (InvalidOperation, ValueError, TypeError) as tsl_check_err:
                lg.warning(
                    f"Could not reliably determine if exchange TSL is active (value: {tsl_value_str}): {tsl_check_err}"
                )
            ...
            if config.get("enable_break_even", False) and not is_tsl_active_exchange:
                ...
                try:
                    ...
                    if entry_price is None or entry_price <= 0:
                        raise ValueError("Invalid entry price for BE check")
                    if not isinstance(current_atr, Decimal) or current_atr <= 0:
                        raise ValueError("Invalid ATR for BE check")

                    # Get BE config parameters
                    be_trigger_atr_mult_str = config.get(
                    ...
                    profit_in_atr = (
                        price_diff / current_atr if current_atr > 0 else Decimal("0")
                    )

                    lg.debug(
                        f"BE Check: CurrentPrice={current_price:.{price_precision}f}, Entry={entry_price:.{price_precision}f}"
                    )
                    lg.debug(
                        f"BE Check: Price Diff={price_diff:.{price_precision}f}, Profit ATRs={profit_in_atr:.2f}, Target ATRs={profit_target_atr}"
                    )
                    ...
                    if profit_in_atr >= profit_target_atr:
                        ...
                        if min_tick_size <= 0:
                            lg.warning(
                                "Cannot calculate BE offset: Min tick size is zero or invalid."
                            )
                            raise ValueError("Invalid min_tick_size for BE offset")

                        tick_offset = min_tick_size * be_offset_ticks
                        be_stop_price: Optional[Decimal] = None
                        if pos_side == "long":
                            # Place BE SL slightly above entry, quantize to tick size
                            raw_be_stop = entry_price + tick_offset
                            be_stop_price = (raw_be_stop / min_tick_size).quantize(
                                Decimal("1"), rounding=ROUND_UP
                            ) * min_tick_size
                        else:  # short
                            # Place BE SL slightly below entry, quantize to tick size
                            raw_be_stop = entry_price - tick_offset
                            be_stop_price = (raw_be_stop / min_tick_size).quantize(
                                Decimal("1"), rounding=ROUND_DOWN
                            ) * min_tick_size
                        ...
                        if be_stop_price is None or be_stop_price <= 0:
                            raise ValueError(
                                f"Calculated BE stop price invalid: {be_stop_price}"
                            )
                        ...
                        current_sl_str = open_position.get(
                            "stopLossPrice"
                        ) or open_position.get("info", {}).get("stopLoss")
                        # Ensure it's not None, empty string, or '0' string before converting
                        if (
                            current_sl_str
                            and str(current_sl_str).strip()
                            and str(current_sl_str) != "0"
                        ):
                            try:
                                ...
                                lg.warning(
                                    f"Could not parse current stop loss '{current_sl_str}': {sl_parse_err}"
                                )
                        ...
                        if current_sl_price is None:
                            ...
                            update_be_sl = True
                            lg.info(
                                f"BE triggered: Target BE SL {be_stop_price} is tighter than Current SL {current_sl_price}. Updating."
                            )
                            ...
                            lg.debug(
                                f"BE Triggered, but current SL ({current_sl_price}) is already better than or equal to target BE SL ({be_stop_price}). No SL update needed."
                            )

                        # --- Execute SL Update if Needed ---
                        if update_be_sl:
                            lg.warning(
                                f"{NEON_PURPLE}*** Moving Stop Loss to Break-Even for {symbol} at {be_stop_price} ***{RESET}"
                            )
                            # Preserve existing Take Profit if one is set
                            current_tp_price: Optional[Decimal] = None
                            current_tp_str = open_position.get(
                                "takeProfitPrice"
                            ) or open_position.get("info", {}).get("takeProfit")
                            if (
                                current_tp_str
                                and str(current_tp_str).strip()
                                and str(current_tp_str) != "0"
                            ):
                                try:
                                    current_tp_price = Decimal(str(current_tp_str))
                                except:
                                    pass  # Ignore parsing error for TP
                            ...
                            if success:
                                ...
                                lg.error(
                                    f"{NEON_RED}Failed to set/update Break-Even SL.{RESET}"
                                )
                        ...
                        lg.debug(
                            f"BE Profit target not reached ({profit_in_atr:.2f} < {profit_target_atr} ATRs)."
                        )

                except (
                    ValueError
                ) as ve:  # Catch validation errors for entry price/ATR/tick
                    lg.warning(f"BE Check skipped for {symbol}: {ve}")
                except (InvalidOperation, TypeError) as dec_err:
                    lg.error(
                        f"{NEON_RED}Error during break-even check ({symbol}) (Decimal/Type Error): {dec_err}{RESET}",
                        exc_info=False,
                    )
                except Exception as be_err:
                    lg.error(
                        f"{NEON_RED}Error during break-even check ({symbol}): {be_err}{RESET}",
                        exc_info=True,
                    )
            elif is_tsl_active_exchange:
                lg.debug(
                    "Break-even check skipped: Exchange Trailing Stop Loss is active."
                )
...
def main() -> None:
    ...
    init_logger.info(
        f"--- Starting XR Scalper Bot ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---"
    )
    ...
    init_logger.info(
        f"Config loaded from {CONFIG_FILE}. Quote Currency: {QUOTE_CURRENCY}"
    )
    ...
    try:
        pandas_ta_version = (
            ...
            else getattr(ta, "version", "N/A")
        )
    except Exception:
        pandas_ta_version = "Error getting version"
    init_logger.info(
        f"Versions: CCXT={ccxt.__version__}, Pandas={pd.__version__}, PandasTA={pandas_ta_version}, Python={sys.version.split()[0]}"
    )
    ...
    if CONFIG.get("enable_trading"):
        ...
        if CONFIG.get("use_sandbox"):
            init_logger.warning(
                f"{NEON_YELLOW}Using SANDBOX (Testnet) Environment.{RESET}"
            )
        else:
            # Extra warning for real money
            init_logger.warning(
                f"{NEON_RED}!!! CAUTION: USING REAL MONEY ENVIRONMENT !!!{RESET}"
            )
        ...
        init_logger.info(
            f"Critical Settings: Risk/Trade={risk_pct:.2f}%, Leverage={leverage}x, Symbols={symbols_to_trade}"
        )
        init_logger.info(
            f"                     TSL Enabled={CONFIG.get('enable_trailing_stop')}, BE Enabled={CONFIG.get('enable_break_even')}"
        )
        ...
        init_logger.info(
            "Live trading is DISABLED in config. Running in analysis-only mode."
        )

    # --- Initialize Exchange ---
    exchange = initialize_exchange(CONFIG, init_logger)
    if not exchange:
        init_logger.error(
            f"{NEON_RED}Failed to initialize exchange. Exiting bot.{RESET}"
        )
    ...
    if not symbols_to_trade:
        init_logger.error(
            f"{NEON_RED}'symbols_to_trade' list is empty in config. Exiting bot.{RESET}"
        )
        return
        # --- Main Loop ---
    ...
    while True:
        ...
        try:
            # Iterate through each symbol defined in the config
            for symbol in symbols_to_trade:
                # Setup logger specific to this symbol for the cycle
                # Replace characters invalid for filenames/logger names
                safe_symbol_suffix = symbol.replace("/", "_").replace(":", "-")
                symbol_logger = setup_logger(safe_symbol_suffix)
                try:
                    analyze_and_trade_symbol(exchange, symbol, CONFIG, symbol_logger)
                except Exception as symbol_err:
                    # Catch errors specific to a symbol's analysis cycle
                    symbol_logger.error(
                        f"{NEON_RED}!!! Unhandled Exception during analysis for {symbol}: {symbol_err} !!!{RESET}",
                        exc_info=True,
                    )
                    symbol_logger.error(
                        f"{NEON_YELLOW}Continuing to next symbol or cycle after error.{RESET}"
                    )
                finally:
                    # Optional: Add a small delay between symbols if needed
                    # time.sleep(1)
                    pass
            ...
            # Catch broader errors in the main loop itself
            init_logger.error(
                f"{NEON_RED}!!! Unhandled Exception in main loop: {loop_err} !!!{RESET}",
                exc_info=True,
            )
            init_logger.warning(
                f"{NEON_YELLOW}Attempting to continue loop after error...{RESET}"
            )
            # Consider adding a longer pause here if critical errors occur frequently
            time.sleep(LOOP_DELAY_SECONDS * 2)

        # --- Calculate and Apply Loop Delay ---
        loop_end_time = time.monotonic()
        elapsed_time = loop_end_time - loop_start_time
        delay = max(0, LOOP_DELAY_SECONDS - elapsed_time)
        init_logger.debug(
            f"Loop finished in {elapsed_time:.2f}s. Waiting {delay:.2f}s for next cycle."
        )
        if delay > 0:
            time.sleep(delay)

    init_logger.info(
        f"--- XR Scalper Bot Shutdown Complete ({datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')}) ---"
    )
