...
def calculate_sma(series: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    if series is None or series.empty or length <= 0 or len(series) < length:
        return None
    try:
        return ta.sma(series, length=length)
    except Exception as e:
        logger.error(f"Error calculating SMA(length={length}): {e}")
        return None
...
def calculate_ema(series: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    try:
        ...
        logger.error(f"Error calculating EMA(length={length}): {e}")
        return None
...
def calculate_hma(series: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    try:
        ...
        logger.error(f"Error calculating HMA(length={length}): {e}")
        return None
...
def calculate_rsi(series: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    if series is None or series.empty or length <= 0 or len(series) < length + 1:
        return None
    try:
        return ta.rsi(series, length=length)
    except Exception as e:
        logger.error(f"Error calculating RSI(length={length}): {e}")
        return None
...
def calculate_macd(series: pd.Series, fast: int, slow: int, signal: int) -> Optional[pd.DataFrame]:
    ...
    if (
        ...
        or signal <= 0
        or fast >= slow
        or len(series) < slow + signal
    ):
        return None
    try:
        return ta.macd(series, fast=fast, slow=slow, signal=signal)
    except Exception as e:
        logger.error(f"Error calculating MACD(f={fast}, s={slow}, sig={signal}): {e}")
        return None
...
def calculate_bollinger_bands(series: pd.Series, length: int, std: float | Decimal) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        logger.error(f"Error calculating BBands(length={length}, std={std}): {e}")
        return None
...
def calculate_keltner_channels(
    ...
    if (
        ...
        or length <= 0
        or atr_len <= 0
        or len(close) < max(length, atr_len)
    ):
        return None
    try:
        return ta.kc(
...
def calculate_atr(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    if (
        ...
        or close.empty
        or length <= 0
        or len(high) < required_len
        or len(low) < required_len
        or len(close) < required_len
    ):
        return None
    try:
        return ta.atr(high=high, low=low, close=close, length=length)
    except Exception as e:
        logger.error(f"Error calculating ATR(length={length}): {e}")
        return None
...
def calculate_cci(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    try:
        ...
        logger.error(f"Error calculating CCI(length={length}): {e}")
        return None
...
def calculate_williams_r(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    if (
        ...
        return None
    try:
        return ta.willr(high=high, low=low, close=close, length=length)
    except Exception as e:
        logger.error(f"Error calculating Williams %R(length={length}): {e}")
        return None
...
def calculate_mfi(
    ...
    if (
        ...
        or len(low) < length
        or len(close) < length
        or len(volume) < length
    ):
        return None
    try:
        return ta.mfi(high=high, low=low, close=close, volume=volume, length=length)
    except Exception as e:
        logger.error(f"Error calculating MFI(length={length}): {e}")
        return None
...
def calculate_adx(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        logger.error(f"Error calculating ADX(length={length}): {e}")
        return None
...
def calculate_psar(high: pd.Series, low: pd.Series, step: float, max_step: float) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        logger.error(f"Error calculating PSAR(step={step}, max={max_step}): {e}")
        return None
...
def calculate_roc(series: pd.Series, length: int) -> Optional[pd.Series]:
    ...
    try:
        ...
        logger.error(f"Error calculating ROC(length={length}): {e}")
        return None
...
def ehlers_volumetric_trend(df: pd.DataFrame, length: int, multiplier: Decimal | float) -> pd.DataFrame:
    ...
    try:
        ...
        if vwma is None or vwma.isnull().all():
            raise ValueError(f"VWMA(length={length}) failed")
        ...
        vwma_vals = df[f"vwma_{length}"].values
        for i in range(2, len(df)):
            if not np.isnan(vwma_vals[i]):
                sm1 = smoothed[i - 1] if not np.isnan(smoothed[i - 1]) else 0
        ...
        df[trend_col_name] = trend
        df[trend_col_name] = df[trend_col_name].ffill().fillna(0).astype(int)
...
def calculate_fib_pivot_points(high: pd.Series, low: pd.Series, close: pd.Series) -> Dict[str, Decimal]:
    ...
    try:
        ...
        Range = H - L
        if Range == 0:
            Range = Decimal("1e-8")  # Avoid zero division
...
def calculate_levels(df: pd.DataFrame, current_price: Decimal) -> dict:
    ...
    try:
        ...
        df["high"].iloc[-2] if len(df) >= 2 else df["high"].iloc[-1]
        df["low"].iloc[-2] if len(df) >= 2 else df["low"].iloc[-1]
        ...
        if diff > 1e-9:
            fib_levels = {
                "Fib 23.6%": high_period - diff * Decimal("0.236"),
                "Fib 38.2%": high_period - diff * Decimal("0.382"),
                "Fib 50.0%": high_period - diff * Decimal("0.5"),
                "Fib 61.8%": high_period - diff * Decimal("0.618"),
        ...
        if standard_pivots:
            ...
            if pivot:
                levels["pivot"] = pivot  # Store main pivot separately
        ...
        if fib_pivots:
            ...
            for label, value in fib_pivots.items():
                if label != "PP":  # Avoid duplicating pivot point
                    if value < current_price:
                        levels["support"][f"Fib {label}"] = value
                    else:
                        levels["resistance"][f"Fib {label}"] = value
...
def calculate_all_indicators(df: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
    ...
    if len(df_clean) < min_rows_needed:
        logger.warning(
            f"Insufficient valid rows ({len(df_clean)}) after cleaning NaNs. Need at least {min_rows_needed}. Indicator results may be unreliable or NaN."
        )
    ...
    def apply_ta_indicator(func: Callable, name: str, required_flag: Optional[str] = None, **kwargs):
        ...
        series_args = {
            k: df_out[k]
            for k in ["high", "low", "close", "volume", "open"]
    ...
    apply_ta_indicator(
        ta.sma,
        "SMA",
        required_flag="ema_alignment",
        length=settings.get("sma_short_period", 10),
    ...
    apply_ta_indicator(
        ...
        "SMA",
        required_flag="ema_alignment",
        length=settings.get("sma_long_period", 50),
        required_inputs=["close"],
    )
    apply_ta_indicator(
        ta.ema,
        "EMA",
        required_flag="ema_alignment",
        length=settings.get("ema_short_period", 12),
        adjust=False,
        required_inputs=["close"],
    )
    apply_ta_indicator(
        ...
        "EMA",
        required_flag="ema_alignment",
        length=settings.get("ema_long_period", 26),
        adjust=False,
        required_inputs=["close"],
    )
    apply_ta_indicator(
    ...
    apply_ta_indicator(
        ta.stochrsi,
        "StochRSI",
        required_flag="stoch_rsi_cross",
        length=settings.get("stoch_rsi_period", 14),
        rsi_length=settings.get("rsi_period", 14),
        k=settings.get("stoch_k_period", 3),
        d=settings.get("stoch_d_period", 3),
        required_inputs=["close"],
    )
    apply_ta_indicator(
        ta.cci,
        "CCI",
        required_flag="cci_threshold",
        length=settings.get("cci_period", 20),
        required_inputs=["high", "low", "close"],
    )
    apply_ta_indicator(
        ta.willr,
        "WILLR",
        required_flag="williams_r_threshold",
        length=settings.get("williams_r_period", 14),
        required_inputs=["high", "low", "close"],
    )
    apply_ta_indicator(
        ta.mfi,
        "MFI",
        required_flag="mfi_threshold",
        length=settings.get("mfi_period", 14),
        required_inputs=["high", "low", "close", "volume"],
    )
    apply_ta_indicator(
        ta.mom,
        "Momentum",
        required_flag="momentum_crossover",
        length=settings.get("momentum_period", 10),
        required_inputs=["close"],
    )
    apply_ta_indicator(
        ta.ao, "AwesomeOscillator", fast=5, slow=34, required_inputs=["high", "low"]
    )  # Add AO flag if needed
    apply_ta_indicator(
        ta.roc, "ROC", length=settings.get("roc_length", 10), required_inputs=["close"]
    )  # Add ROC flag if needed

    # MACD
    apply_ta_indicator(
        ta.macd,
        "MACD",
        required_flag="macd_cross",
    ...
    apply_ta_indicator(
        ...
        "ATR",
        required_flag=None,
        length=settings.get("atr_period", 14),
        required_inputs=["high", "low", "close"],
    )  # Always calc ATR
    apply_ta_indicator(
        ta.bbands,
        "BollingerBands",
        required_flag="bollinger_bands_break",
        length=settings.get("bollinger_bands_period", 20),
        std=float(settings.get("bollinger_bands_std_dev", 2.0)),
        required_inputs=["close"],
    )
    apply_ta_indicator(
        ta.kc,
        "KeltnerChannels",
        length=settings.get("kc_length", 20),
        atr_length=settings.get("kc_atr_length", 10),
        multiplier=float(settings.get("kc_multiplier", 2.0)),
        required_inputs=["high", "low", "close"],
    )  # Add KC flag if needed

    # Trend
    apply_ta_indicator(
        ta.adx,
        "ADX",
        required_flag="adx_trend_strength",
        length=settings.get("adx_period", 14),
        required_inputs=["high", "low", "close"],
    )
    apply_ta_indicator(
        ta.psar,
        "PSAR",
        required_flag="psar_flip",
    ...
    if flags.get("volume_confirmation") and vol_ma_len > 0:
        ...
        if sma_vol is not None:
            df_out[f"volume_ma_{vol_ma_len}"] = sma_vol
    ...
    if config.get("strategy", {}).get("name", "").upper() == "DUAL_EHLERS_VOLUMETRIC":
        ...
        conf_evt_mult = (
            ...
            .get("confirm_evt_multiplier", Decimal("2.0"))
        )
