...
def initialize_exchange() -> Optional[ccxt.Exchange]:
    ...
    try:
        exchange = ccxt.bybit({
            ...
            "enableRateLimit": True, "enableTimeSync": True,
    ...
    except ccxt.NetworkError as e: logger.critical(f"Network error during init: {e}")
    except ccxt.ExchangeError as e: logger.critical(f"Exchange error during init: {e}")
...
def analyze_volume_atr(
    ...
    try:
        ...
        logger.debug(f"Volume Analysis: Last={last_vol_str}, "
                     f"MA({vol_ma_len})={vol_ma_str}, "
                     f"Ratio={vol_ratio_str}")
...
def analyze_order_book(
    ...
    try:
        ...
        logger.debug(f"Order Book: Best Bid={best_bid:.4f}, Best Ask={best_ask:.4f}, "
        ...
        logger.debug(f"Order Book Analysis (Depth {depth}): "
    ...
    except ccxt.NetworkError as e: logger.warning(f"Network error fetching order book: {e}")
    except ccxt.ExchangeError as e: logger.warning(f"Exchange error fetching order book: {e}")
    except Exception as e:
...
def get_market_data(
    ...
    try:
        ...
        return df
    except ccxt.NetworkError as e: logger.warning(f"Network error fetching OHLCV: {e}")
    except ccxt.ExchangeError as e: logger.warning(f"Exchange error fetching OHLCV: {e}")
    except Exception as e:
...
def get_current_position(exchange: ccxt.Exchange, symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return default_pos
    except ccxt.NetworkError as e: logger.warning(f"Network error fetching position: {e}")
    except ccxt.ExchangeError as e: logger.warning(f"Exchange error fetching position: {e}")
    except Exception as e:
...
def close_position(
    ...
    try:
        ...
        logger.warning(f"Attempting CLOSE {position['side']} position: {amount_str} {symbol} via {side_to_close.upper()} MARKET order (reduce_only)...")
        ...
        return order
    except ccxt.InsufficientFunds as e: logger.error(f"Insufficient funds to close position: {e}")
    except ccxt.NetworkError as e: logger.error(f"Network error closing position: {e}")
    except ccxt.ExchangeError as e: logger.error(f"Exchange error closing position: {e}")
    except Exception as e:
...
def calculate_position_size(
    ...
    logger.debug(f"Risk Calc Output: RiskAmt={risk_amount_usdt:.2f}, PriceDiff={price_difference:.4f} "
...
def place_risked_market_order(
    ...
    try:
        ...
        if usdt_equity <= 0: usdt_equity = usdt_free
        if usdt_equity <= 0: logger.error("Cannot determine equity."); return None
        ...
        if order_value_usdt > MAX_ORDER_USDT_AMOUNT:
            logger.warning(f"Risk-calculated order value ({order_value_usdt:.2f}) exceeds MAX Cap "
        ...
        if precise_qty <= 0: logger.error("Final order Qty <= 0."); return None
        logger.info(f"Final Order Details: Qty={precise_qty_str}, "
        ...
        logger.success(f"Order Placement Success! Size: {order.get('amount', '?')}, "
    ...
    except ccxt.InsufficientFunds as e: logger.error(f"Insufficient funds placing order: {e}")
    except ccxt.NetworkError as e: logger.error(f"Network error placing order: {e}")
    except ccxt.ExchangeError as e: logger.error(f"Exchange error placing order: {e}")
    except Exception as e:
...
def trade_logic(
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols) or df[required_cols].iloc[-1].isnull().any() or current_atr is None:
        ...
        if FETCH_ORDER_BOOK_PER_CYCLE or primary_long_signal or primary_short_signal:
            logger.debug(f"Fetching/Analyzing order book (FETCH_PER_CYCLE={FETCH_ORDER_BOOK_PER_CYCLE}, Signal={primary_long_signal or primary_short_signal})...")
        ...
        if position_side != POSITION_SIDE_NONE and entry_price > 0 and current_atr > 0:
            ...
            if position_side == POSITION_SIDE_LONG:
                ...
                logger.debug(f"Long Pos Check: Entry={entry_price:.4f}, Curr={current_price:.4f}, SL={stop_price:.4f}, TP={profit_price:.4f}")
                ...
                stop_price = entry_price + sl_distance
                profit_price = entry_price - tp_distance
                logger.debug(f"Short Pos Check: Entry={entry_price:.4f}, Curr={current_price:.4f}, SL={stop_price:.4f}, TP={profit_price:.4f}")
                if current_price >= stop_price: sl_triggered = True
                if current_price <= profit_price: tp_triggered = True
...
def main():
    ...
    try:
        ...
        if not set_leverage(exchange, symbol, DEFAULT_LEVERAGE):
             logger.critical("Leverage setup failed. Exiting."); return
    except (ccxt.BadSymbol, KeyError) as e:
         logger.critical(f"Invalid/unsupported symbol: {symbol_input}. Error: {e}")
         return
    except Exception as e:
        logger.critical(f"Symbol/Leverage setup failed: {e}"); return
    ...
    logger.info(f"SL: {ATR_STOP_LOSS_MULTIPLIER} * ATR({ATR_CALCULATION_PERIOD}), TP: {ATR_TAKE_PROFIT_MULTIPLIER} * ATR({ATR_CALCULATION_PERIOD})")
    ...
    logger.info(f"Volume: MA={VOLUME_MA_PERIOD}, Spike > {VOLUME_SPIKE_THRESHOLD}x MA (Required: {REQUIRE_VOLUME_SPIKE_FOR_ENTRY})")
    logger.info(f"Order Book: Depth={ORDER_BOOK_DEPTH}, Long Ratio > {ORDER_BOOK_RATIO_THRESHOLD_LONG}, Short Ratio < {ORDER_BOOK_RATIO_THRESHOLD_SHORT}")
