...
NEON_BLUE = (
    Fore.LIGHTBLUE_EX if COLORAMA_AVAILABLE else ""
...
IS_UNIFIED_ACCOUNT: bool = (
    False  # Flag indicating Bybit account type (detected on init)
)
...
class LocalTimeFormatter(SensitiveFormatter):
    ...
    converter = lambda _, timestamp: datetime.fromtimestamp(
...
def _ensure_config_keys(
    ...
    for key, default_value in default_config.items():
        ...
        if key not in updated_config:
            ...
            nested_config, nested_keys_added = _ensure_config_keys(
            ...
            ) and isinstance(loaded_value, int)
            # Allow common string -> bool conversion (handled later in validation)
            is_str_bool_candidate = isinstance(default_value, bool) and isinstance(
                loaded_value, str
            )
...
def _validate_config_values(config: dict[str, Any], logger: logging.Logger) -> bool:
    ...
    numeric_params: dict[str, tuple[type, int | float, int | float, bool]] = {
        ...
        "ema_long_period": (
            ...
            1000,
            True,
        ),  # Ensure long > short is checked separately
    ...
    for key, (target_type, min_val, max_val, require_int) in numeric_params.items():
        ...
        try:
            ...
            if not (min_val <= num_value_float <= max_val):
                lg.error(
                    f"Config Error: '{key}' value {num_value_float} (from {original_value_repr}) is outside the recommended range ({min_val} - {max_val})."
                )
                is_valid = False
            ...
            if target_type is int:
                ...
                if require_int and not math.isclose(
                    num_value_float, round(num_value_float), rel_tol=1e-9
                ):
                    lg.warning(
                        f"Config Warning: '{key}' requires an integer, but found {num_value_float} (from {original_value_repr}). Truncating to {int(num_value_float)}."
                    )
            ...
            lg.error(
                f"Config Error: '{key}' value {original_value_repr} could not be converted to a number."
            )
            is_valid = False
    ...
    if (
        ...
        for indi_key, weight_val in active_set_weights.items():
            try:
                ...
                lg.error(
                    f"Config Error: Invalid weight value '{weight_val}' for indicator '{indi_key}' in weight set '{active_set_name}'. Must be numeric."
                )
                is_valid = False
        ...
        for indi_key, indi_val in indicators_config.items():
            ...
            if isinstance(indi_val, str):
                ...
                if val_str in ["true", "yes", "1", "on"]:
                    ...
                    lg.debug(
                        f"Config: Converted 'indicators.{indi_key}' value '{indi_val}' to True."
                    )
                elif val_str in ["false", "no", "0", "off"]:
                    config["indicators"][indi_key] = False  # Update in place
                    lg.debug(
                        f"Config: Converted 'indicators.{indi_key}' value '{indi_val}' to False."
                    )
                else:
                    lg.error(
...
def load_config(filepath: str, logger: logging.Logger) -> dict[str, Any] | None:
    ...
    default_config: dict[str, Any] = {
        ...
        "interval": "3",  # Kline interval (string from VALID_INTERVALS)
        ...
        "enable_trading": True,  # Master switch for placing actual trades (boolean)
        "use_sandbox": False,  # Use Bybit's testnet environment (boolean)
        "risk_per_trade": 0.01,  # Fraction of balance to risk per trade (float, e.g., 0.01 = 1%)
        "leverage": 25,  # Leverage for futures trades (integer)
        "max_concurrent_positions_total": 2,  # Max open positions across all symbols (integer)
        ...
        "stop_loss_multiple": 1.0,  # ATR multiple for SL distance (float)
        "take_profit_multiple": 0.3,  # ATR multiple for TP distance (float)
    ...
    if not os.path.exists(filepath):
        ...
        try:
            ...
            original_loaded_config = (
                config_from_file.copy()
            )  # Deep copy might be needed if nested dicts are modified directly
    ...
    try:
        ...
        if json.dumps(loaded_config, sort_keys=True) != json.dumps(
            initial_state_to_compare, sort_keys=True
        ):
            config_changed_needs_save = True
...
def _check_account_type_and_balance(
    ...
    try:
        ...
        if is_account_type_error:
            ...
            lg.warning(
                f"ExchangeError checking UNIFIED balance: {e}. Will proceed to check CONTRACT/SPOT."
            )
        ...
        lg.warning(
            f"Error checking UNIFIED balance: {e}. Will proceed to check CONTRACT/SPOT if possible."
        )
    ...
    if is_unified_detected is False or is_unified_detected is None:
        ...
        for acc_type in account_types_to_try:
            try:
                ...
                if parsed_balance is not None:
                    lg.info(
                        f"Successfully fetched balance using {acc_type} account type (Balance: {parsed_balance:.4f})."
                    )
                ...
                lg.warning(
                    f"ExchangeError checking {acc_type} balance: {e}. Trying next type..."
                )
            except Exception as e:
                lg.warning(
                    f"Unexpected error checking {acc_type} balance: {e}. Trying next type..."
                )
        ...
        try:
            ...
            if bybit_code is None:
                try:
                    ...
                    if json_part_match:
                        details_dict = json.loads(
                            json_part_match.group(1).replace("'", '"')
                        )  # Basic quote fix
            ...
            if bybit_code in non_retryable_codes:
                ...
                if bybit_code == 10001:
                    ...
                    try:  # Try harder to guess the currency being checked
                        ...
                        elif "symbol" in kwargs and isinstance(kwargs["symbol"], str):
                            balance_currency = (
    ...
    try:
        ...
        if not market:
            ...
            if not market:  # Still not found
                lg.error(
                    f"Symbol '{symbol}' (or base '{base_symbol if ':' in symbol else ''}') not found in loaded markets."
                )
        ...
        def get_digits_from_precision(
            precision_val: float | int | str | None, default_digits: int = 8
        ) -> int:
        ...
        market_details = {
            "symbol": market.get(
                "symbol", symbol
            ),  # Use the symbol found in market data
        ...
        lg.debug(
            f"Market Info for {symbol}: Cat={category}, Tick={market_details['min_tick_size']}, AmtStep={market_details['amount_step_size']}, ContSize={contract_size}"
        )
...
def fetch_current_price_ccxt(
    ...
    try:
        ...
        lg.debug(
            f"Fetching ticker for {symbol} (MarketID: {market_id}) with params: {params}"
        )
        ...
        if isinstance(ticker, dict):
            ...
            if (
                ...
                for item in ticker["info"]["result"]["list"]:
                    if isinstance(item, dict) and item.get("symbol") == market_id:
                        ...
                        lg.debug(
                            f"Parsed price from V5 list structure for {symbol} (MarketID: {market_id})"
                        )
            ...
            # Check alternative V5 structure (info -> result -> single item)
                ...
                if ticker_data.get("symbol") == market_id:
                    price_str = ticker_data.get("lastPrice")  # V5 field name
                    lg.debug(
                        f"Parsed price from V5 single result structure for {symbol}"
                    )
                else:
                    lg.warning(
...
def fetch_klines_ccxt(
    ...
    try:
        ...
        params = {"category": category, "symbol": market_id}
        lg.debug(
            f"Fetching {limit} klines for {symbol} ({ccxt_timeframe}, MarketID: {market_id}) with params: {params}"
        )
        ...
        for col in cols_to_convert:
            ...
            try:
                ...
                lg.error(
                    f"Error converting column '{col}' to Decimal for {symbol}: {e}. Attempting numeric conversion with NaN coercion."
                )
...
def fetch_orderbook_ccxt(
    ...
    if limit not in valid_limits:
        ...
        if limit < 50:
            ...
            limit = 200
        lg.warning(
            f"Invalid orderbook limit requested. Adjusted to nearest valid Bybit V5 limit: {limit}"
        )
    ...
    try:
        ...
        lg.debug(
            f"Fetching order book (limit {limit}) for {symbol} (MarketID: {market_id}) with params: {params}"
        )
        ...
        if (
            ...
            for level in orderbook["bids"] + orderbook["asks"]:
                if not (
                    ...
                    and len(level) == 2
                    and isinstance(level[0], (str, int, float))
                    and isinstance(level[1], (str, int, float))
                ):
                    is_valid = False
                    break
            if not is_valid:
    ...
    for attempt in range(MAX_API_RETRIES + 1):
        ...
        for acc_type in account_types_to_query:
            try:
                params = {"accountType": acc_type, "coin": currency}
                lg.debug(
                    f"Fetching balance with params={params} (Overall Attempt {attempt + 1})"
                )
        ...
        if error_in_attempt:  # If broke from inner loop due to recoverable error
            if attempt < MAX_API_RETRIES:
                ...
                lg.warning(
                    f"Balance fetch attempt {attempt + 1} encountered recoverable error ({type(last_exception).__name__}). Retrying in {wait_time:.1f}s..."
                )
                time.sleep(wait_time)
        ...
        if not balance_found_in_attempt:
            if attempt < MAX_API_RETRIES:
                wait_time = RETRY_DELAY_SECONDS * (2**attempt)
                lg.warning(
                    f"Balance fetch attempt {attempt + 1} failed to find/parse balance for type(s): {account_types_to_query}. Retrying in {wait_time:.1f}s..."
                )
                time.sleep(wait_time)
...
class TradingAnalyzer:
        ...
        if not raw_weights or not isinstance(raw_weights, dict):
            ...
            for key, val in raw_weights.items():
                try:
                    ...
                    logger.warning(
                        f"Invalid weight '{val}' for '{key}' in set '{self.active_weight_set_name}'. Skipping."
                    )
    ...
    def _convert_df_for_ta(self) -> None:
        ...
        try:
            cols_to_convert = ["open", "high", "low", "close", "volume"]
            self.logger.debug(
                f"Converting columns {cols_to_convert} to float64 for TA..."
            )
            for col in cols_to_convert:
                if col in self.df.columns:
                    # Check if conversion is needed (not already float64)
                    if (
                        not pd.api.types.is_float_dtype(self.df[col])
                        or self.df[col].dtype != np.float64
                    ):
                        ...
                        if self.df[col].isnull().any():
                            self.logger.debug(
                                f"Column '{col}' (original: {original_type}) converted to float64, NaNs introduced."
                            )
                        # else:
    ...
    def _define_ta_strategy(self) -> None:
        ...
        if atr_p > 0:
            ...
            self.logger.error(
                f"ATR period ({atr_p}) is invalid. Risk management calculations will fail."
            )
        ...
        if ema_needed:
            if ema_s > 0:
                ...
                self.logger.warning("EMA Short period invalid, EMA features disabled.")
        ...
        if indi_cfg.get("bollinger_bands") and bb_p > 0:
            bb_std_str = f"{bb_std:.1f}".replace(
                ".", "_"
        ...
        if indi_cfg.get("vwap"):
            ...
            if all(c in self.df.columns for c in ["high", "low", "close", "volume"]):
                ...
                self.logger.warning(
                    "Cannot calculate VWAP: Missing HLCV columns in TA DataFrame."
                )
        ...
        if indi_cfg.get("mfi") and mfi_w > 0:
            ...
            if all(c in self.df.columns for c in ["high", "low", "close", "volume"]):
                ...
                self.logger.warning(
                    "Cannot calculate MFI: Missing HLCV columns in TA DataFrame."
                )
    ...
    def _calculate_all_indicators(self) -> None:
        ...
        )  # pandas_ta built-in or default guess
        ...
        try:
            ...
            if (
                ...
                self.logger.debug(
                    f"Problematic DF sample (float converted) tail:\n{self.df.tail()}"
                )
            else:
                self.logger.error(
    ...
    def _update_latest_indicator_values(self) -> None:
        ...
        try:
            ...
            if latest_series.isnull().all():
                self.logger.warning(
                    f"Last row of TA DataFrame is all NaN for {self.symbol}. Cannot update latest indicator values."
                )
                return
        except IndexError:
            self.logger.error(
                f"DataFrame index out of bounds when accessing last row for {self.symbol}."
            )
            return
        except Exception as e:
            self.logger.error(
        ...
        for generic_name, actual_col_name in self.ta_column_map.items():
            if actual_col_name in latest_series:
                ...
                self.logger.debug(
                    f"Column '{actual_col_name}' not found in TA DataFrame for indicator '{generic_name}' ({self.symbol}). Setting value to None."
                )
        ...
        for base_col in ["open", "high", "low", "close", "volume"]:
            if base_col in latest_series:
                self.indicator_values[base_col.capitalize()] = to_decimal_safe(
    ...
    def quantize_amount(
        ...
        try:
            ...
            quantized_steps = (amount_decimal / step_size).quantize(
    ...
    def _check_stoch_rsi(self) -> float | None:
        ...
        if k < oversold and d < oversold:
            score = (
                1.0 if k > d else 0.8
            )  # Bullish crossover deep OS (stronger if k>d crosses up)
        elif k > overbought and d > overbought:
            score = (
                -1.0 if k < d else -0.8
            )  # Bearish crossover deep OB (stronger if k<d crosses down)
        # Weaker signals: Just entering OB/OS without crossover confirmation yet
        elif k < oversold and d >= oversold:
            score = 0.6  # K entered OS
        elif k > overbought and d <= overbought:
            score = -0.6  # K entered OB
        # Mid-range crossover signals (weakest confirmation)
        elif k > d and not (k > overbought and d > overbought):
            score = 0.3  # Bullish crossover mid-range
        elif k < d and not (k < oversold and d < oversold):
    ...
    def _check_cci(self) -> float | None:
        ...
        if cci >= ob:
            ...
            score = (
                -cci / ob
            )  # Linear scale between -100 and +100 (e.g., cci=50 -> -0.5, cci=-50 -> 0.5)
        return max(-1.0, min(1.0, score))  # Clamp final score

    def _check_wr(self) -> float | None:
        """Checks Williams %R against standard OB/OS levels (-20 / -80). Score scaled linearly, clamped."""
        wr = self._get_indicator_float(
            "WR"
        )  # Williams %R typically ranges from -100 (most oversold) to 0 (most overbought)
        if wr is None:
            return None
        ...
        extreme_os = -90.0
        if wr >= extreme_ob:
            score = max(-1.0, score * 1.1)  # Increase bearishness magnitude near 0
        if wr <= extreme_os:
            score = min(1.0, score * 1.1)  # Increase bullishness magnitude near -100
        return max(-1.0, min(1.0, score))  # Clamp final score
    ...
    def _check_mfi(self) -> float | None:
        ...
        extreme_os = 10.0
        if mfi >= extreme_ob:
            score = max(-1.0, score * 1.1)  # Boost bearishness magnitude
        if mfi <= extreme_os:
            score = min(1.0, score * 1.1)  # Boost bullishness magnitude
        return max(-1.0, min(1.0, score))  # Clamp final score
        ...
        try:
            ...
            for b in top_bids:
                ...
                if isinstance(b, (list, tuple)) and len(b) > 1:
                    try:
                        bid_vol += Decimal(str(b[1]))  # Use amount (index 1)
            ...
            for a in top_asks:
                if isinstance(a, (list, tuple)) and len(a) > 1:
                    try:
                        ask_vol += Decimal(str(a[1]))  # Use amount (index 1)
                    except (InvalidOperation, TypeError, IndexError, ValueError):
                        self.logger.debug(
            ...
            self.logger.debug(
                f"OBI Calc ({self.symbol}): BidVol={bid_vol:.4f}, AskVol={ask_vol:.4f}, OBI={obi_decimal:.4f} -> Score={score:.3f}"
            )
            return score
    ...
    def generate_trading_signal(
        ...
        for indicator_key, check_method in indicator_check_methods.items():
            ...
            try:
                ...
                self.logger.error(
                    f"Error executing check method for '{indicator_key}': {e}",
                    exc_info=True,
                )
            ...
            if indicator_score_float is not None and math.isfinite(
                ...
                try:
                    ...
                    weight_decimal = Decimal(
                        str(weight)
                    )  # Convert float weight to Decimal
                    ...
                    total_weight_applied += abs(
                        weight_decimal
                    )  # Sum absolute weights for normalization/debug
        ...
        if total_weight_applied > 0:
            ...
            self.logger.warning(
                f"Calculated signal score is {final_signal_score} but total weight applied is zero for {self.symbol}. Check weights."
            )
        ...
        threshold_key = (
            "scalping_signal_threshold" if is_scalping_set else "signal_score_threshold"
        )
        ...
        if self.logger.level <= logging.DEBUG:  # Log details only if debugging
            ...
            contributing_scores = {
                ...
                if v not in ["Disabled", "No/Invalid Wt", "Wt=0"]
            }
            score_details_str = ", ".join(
                [f"{k}: {v}" for k, v in contributing_scores.items()]
            )
    ...
    def calculate_entry_tp_sl(
        ...
        if (
            not entry_price_signal
            or not entry_price_signal.is_finite()
            or entry_price_signal <= 0
        ):
            self.logger.error(
                f"Invalid entry signal price ({entry_price_signal}) for TP/SL calc."
            )
            return None, None, None
        ...
        if atr_val is None or not atr_val.is_finite() or atr_val <= 0:
            self.logger.warning(
                f"{NEON_YELLOW}Cannot calculate dynamic TP/SL for {self.symbol}: Invalid ATR value ({atr_val}). SL/TP will be None.{RESET}"
            )
            # We might still be able to quantize entry, but SL/TP depend on ATR.
            quantized_entry = self.quantize_price(
                entry_price_signal, rounding=ROUND_DOWN if signal == "BUY" else ROUND_UP
            )
            return quantized_entry, None, None
        if min_tick is None:  # Already logged warning in get_min_tick_size if invalid
            self.logger.error(
                f"{NEON_RED}Cannot calculate TP/SL for {self.symbol}: Minimum tick size is unavailable.{RESET}"
            )
        ...
        quantized_entry = self.quantize_price(
            entry_price_signal, rounding=entry_rounding
        )
        if quantized_entry is None:
            self.logger.error(
                f"Failed to quantize entry signal price {entry_price_signal} for {self.symbol}."
            )
            return None, None, None
        ...
        try:
            ...
            if sl_offset != sl_offset_raw:
                self.logger.debug(
                    f"Adjusted SL offset from {sl_offset_raw} to minimum {sl_offset} ({MIN_TICKS_AWAY_FOR_SLTP} ticks) for {self.symbol}."
                )

            tp_offset = max(tp_offset_raw, min_offset_value)
            if tp_offset != tp_offset_raw:
                self.logger.debug(
                    f"Adjusted TP offset from {tp_offset_raw} to minimum {tp_offset} ({MIN_TICKS_AWAY_FOR_SLTP} ticks) for {self.symbol}."
                )
            ...
            if stop_loss is not None:
                sl_boundary = (
                    quantized_entry - min_offset_value
                    if signal == "BUY"
                    else quantized_entry + min_offset_value
                )
                needs_sl_adjustment = False
                if signal == "BUY" and stop_loss >= sl_boundary:
                    needs_sl_adjustment = True
                if signal == "SELL" and stop_loss <= sl_boundary:
                    needs_sl_adjustment = True
                ...
                if needs_sl_adjustment:
                    ...
                    if adjusted_sl is not None and adjusted_sl != stop_loss:
                        self.logger.warning(
                            f"Initial SL ({stop_loss}) too close to entry ({quantized_entry}) after quantization. Adjusting to {adjusted_sl} ({MIN_TICKS_AWAY_FOR_SLTP}+ ticks away)."
                        )
                        stop_loss = adjusted_sl
                    else:
                        self.logger.error(
                            f"Could not adjust SL ({stop_loss}) to be minimum distance from entry ({quantized_entry}). Setting SL to None for safety."
                        )
                ...
                if stop_loss is not None and stop_loss <= 0:
                    self.logger.error(
                        f"Calculated SL is zero or negative ({stop_loss}) after adjustments. Setting SL to None."
                    )
                    stop_loss = None
            ...
            if take_profit is not None:
                ...
                if signal == "BUY" and take_profit <= tp_boundary:
                    needs_tp_adjustment = True
                if signal == "SELL" and take_profit >= tp_boundary:
                    needs_tp_adjustment = True
                ...
                if needs_tp_adjustment:
                    ...
                    if adjusted_tp is not None and adjusted_tp != take_profit:
                        self.logger.warning(
                            f"Initial TP ({take_profit}) too close to entry ({quantized_entry}) after quantization. Adjusting to {adjusted_tp} ({MIN_TICKS_AWAY_FOR_SLTP}+ ticks away)."
                        )
                        take_profit = adjusted_tp
                    else:
                        self.logger.error(
                            f"Could not adjust TP ({take_profit}) to be minimum distance from entry ({quantized_entry}). Setting TP to None for safety."
                        )
                ...
                if take_profit is not None and take_profit <= 0:
                    self.logger.error(
                        f"Calculated TP is zero or negative ({take_profit}) after adjustments. Setting TP to None."
                    )
                    take_profit = None
            ...
            self.logger.info(
                f"Calc TP/SL ({signal}): EntryRef={entry_log}, TP={tp_log}, SL={sl_log} (ATR={atr:.{prec + 1}f}, SLx={sl_mult}, TPx={tp_mult})"
            )
...
def calculate_position_size(
    ...
    contract_size: Decimal = market_info.get(
        "contract_size", Decimal("1")
    )  # Default 1 (for spot/linear)
    ...
    if balance is None or not isinstance(balance, Decimal) or balance <= 0:
        lg.error(f"Size Calc Error ({symbol}): Invalid balance {balance}")
        return None
    ...
    if entry_price == stop_loss_price:
        lg.error(
            f"Size Calc Error ({symbol}): Entry price equals SL price ({entry_price})"
        )
        return None
    if amount_step_size is None or amount_step_size <= 0:
        ...
        if amount_digits is None:
            lg.error(
                f"Size Calc Error ({symbol}): Amount step size and precision digits are missing. Cannot quantize size."
            )
            return None
        else:
            amount_step_size = Decimal("1") / (Decimal("10") ** amount_digits)
            lg.warning(
                f"Amount step size missing, using precision digits ({amount_digits}) -> step={amount_step_size}"
            )
    if not (0 < risk_per_trade < 1):
        lg.error(
            f"Size Calc Error ({symbol}): Invalid risk_per_trade {risk_per_trade} (must be > 0 and < 1)"
        )
        return None
    if is_contract and leverage <= 0:
        lg.error(
            f"Size Calc Error ({symbol}): Invalid leverage {leverage} for contract"
        )
        return None

    try:
        # --- Calculate Risk Amount and SL Distance ---
        risk_amount_quote: Decimal = balance * Decimal(
            str(risk_per_trade)
        )  # Risk amount in quote currency
        sl_distance_points: Decimal = abs(entry_price - stop_loss_price)
        if sl_distance_points <= 0:
            lg.error(f"Size Calc Error ({symbol}): SL distance is zero or negative")
            return None
        ...
        if is_contract:
            if is_inverse:
                ...
                if entry_price == 0 or stop_loss_price == 0:
                    lg.error(
                        f"Size Calc Error ({symbol}): Zero price encountered for inverse calculation"
                    )
                    return None
                risk_per_unit_or_contract_quote = (
                    contract_size
                    * abs(Decimal("1") / stop_loss_price - Decimal("1") / entry_price)
                    * entry_price
                )
        ...
        if not size_unquantized.is_finite() or size_unquantized <= 0:
            lg.error(
                f"Size Calc Error ({symbol}): Invalid unquantized size ({size_unquantized}) from RiskAmt={risk_amount_quote:.4f} / RiskPerUnit={risk_per_unit_or_contract_quote:.8f}"
            )
            return None

        lg.debug(
            f"Size Calc ({symbol}): Bal={balance:.2f}, Risk={risk_per_trade * 100:.2f}%, RiskAmt={risk_amount_quote:.4f}, SLDist={sl_distance_points:.{market_info.get('price_precision_digits', 4)}f}, RiskPerUnit={risk_per_unit_or_contract_quote:.8f}, UnquantSize={size_unquantized:.8f} {size_unit_name}"
        )
        ...
        temp_analyzer = TradingAnalyzer(
            pd.DataFrame(), lg, config, market_info, {}
        )  # Minimal valid init
        quantized_size = temp_analyzer.quantize_amount(
            size_unquantized, rounding=ROUND_DOWN
        )

        if quantized_size is None or quantized_size <= 0:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Size is zero or invalid after quantization (Unquantized: {size_unquantized:.8f}). Cannot place order.{RESET}"
            )
            return None

        lg.debug(
            f"Quantized Size ({symbol}): {quantized_size} {size_unit_name} (Step: {amount_step_size})"
        )

        # --- Validate Against Market Limits (Amount) ---
        if min_order_amount is not None and quantized_size < min_order_amount:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Calculated size {quantized_size} {size_unit_name} < Min Amount {min_order_amount}. Cannot place order.{RESET}"
            )
            return None
        if max_order_amount is not None and quantized_size > max_order_amount:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Calculated size {quantized_size} {size_unit_name} > Max Amount {max_order_amount}. Capping size to max.{RESET}"
            )
            # Cap the size and re-quantize using floor (ROUND_DOWN)
            quantized_size = temp_analyzer.quantize_amount(
                max_order_amount, rounding=ROUND_DOWN
            )
            if (
                quantized_size is None
                or quantized_size <= 0
                or (min_order_amount is not None and quantized_size < min_order_amount)
            ):
                lg.error(
                    f"Size Calc Error ({symbol}): Capped size {quantized_size} is zero or below min amount {min_order_amount}."
                )
                return None
        ...
        if is_contract:
            if is_inverse:
                ...
                order_value_quote = (
                    quantized_size * contract_size
                )  # Value in Base at par
                # Approximate Quote value for cost check:
                order_value_quote_approx = (
                    quantized_size * contract_size * entry_price
                )  # Rough quote value
                margin_required = (
                    (quantized_size * contract_size) / Decimal(leverage) * entry_price
                )  # Approx margin in quote terms
            else:  # Linear Contract
        ...
        if min_order_cost is not None and cost_check_value < min_order_cost:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Order value ~{cost_check_value:.4f} {quote_currency} < Min Cost {min_order_cost}. Cannot place order.{RESET}"
            )
            return None
        ...
        if required_margin_with_buffer > balance:
            lg.warning(
                f"{NEON_YELLOW}Size Calc ({symbol}): Required margin ~{margin_required:.4f} {quote_currency} (Buffered: {required_margin_with_buffer:.4f}) > Available balance {balance:.4f}. Cannot place order.{RESET}"
            )
        ...
        # --- Success ---
        lg.info(
            f"Calculated position size for {symbol}: {quantized_size} {size_unit_name} (Value: ~{cost_check_value:.2f} {quote_currency}, Margin: ~{margin_required:.2f} {quote_currency})"
        )
        ...
        for pos_raw in all_positions_raw:
            ...
            try:
                ...
                entry_price_str = pos_raw.get("info", {}).get(
                    "avgPrice", standardized_pos.get("entryPrice")
                )
                ...
                liq_price_str = pos_raw.get("info", {}).get(
                    "liqPrice", standardized_pos.get("liquidationPrice")
                )
                ...
                pnl_str = pos_raw.get("info", {}).get(
                    "unrealisedPnl", standardized_pos.get("unrealizedPnl")
                )
                ...
                leverage_str = pos_raw.get("info", {}).get(
                    "leverage", standardized_pos.get("leverage")
                )
                try:
                    standardized_pos["leverage"] = (
                        float(leverage_str) if leverage_str else None
                    )
                except (ValueError, TypeError):
                    standardized_pos["leverage"] = None
                ...
                liq_log = (
                    ...
                    if standardized_pos.get("liqPrice")
                    else ""
                )
                pnl_log = (
                    f"PnL={standardized_pos['unrealizedPnl']:.2f}"
                    if standardized_pos.get("unrealizedPnl")
                    else ""
                )
                lev_log = (
...
def create_order_ccxt(
    ...
    try:
        ...
        lg.debug(
            f"CCXT create_order Args: symbol='{symbol}', type='{order_type}', side='{side}', amount={amount_float}, price={price_float}, params={order_params}"
        )
        ...
        if order_result and isinstance(order_result, dict) and order_result.get("id"):
            ...
            if ret_code == 0:
                ...
                if ret_code == 110007:
                    lg.warning(
                        f"Hint: Order rejected due to insufficient balance (Code {ret_code})."
                    )
                elif ret_code == 110017:
                    lg.warning(
                        f"Hint: Order rejected due to price/qty precision error (Code {ret_code}). Check market limits."
                    )
                elif ret_code == 110045 or ret_code == 170007:
                    lg.warning(
                        f"Hint: Order rejected due to risk limit exceeded (Code {ret_code})."
                    )
                elif ret_code == 110025:
                    lg.warning(
                        f"Hint: Order rejected due to positionIdx mismatch (Code {ret_code}). Ensure Hedge Mode params are correct."
                    )
...
def set_protection_ccxt(
    ...
    | None = None,  # TSL *distance/offset* value (e.g., 100 for $100 behind)
    ...
    def format_value(
        value: Decimal | None, name: str, is_distance: bool = False
    ) -> str:
        ...
        if value is not None and isinstance(value, Decimal) and value.is_finite():
            if value > 0:
                ...
                lg.warning(
                    f"Invalid negative value '{value}' provided for '{name}' in set_protection. Using '0' (cancel)."
                )
                return "0"
    ...
    if tsl_dist_str != "0" or (
        trailing_stop_price is not None and trailing_stop_price == 0
    ):
        ...
        if tsl_dist_str != "0":
            params["activePrice"] = (
                tsl_act_str  # "0" means immediate activation for activePrice
            )
    ...
    try:
        ...
        if result and isinstance(result, dict) and result.get("retCode") == 0:
            ...
            if ret_code in [170140, 110041, 110024]:
                lg.warning(
                    f"Hint: Set protection failed because position might not exist or size is zero (Code {ret_code})."
                )
            elif ret_code == 110025:
                lg.warning(
                    f"Hint: Set protection failed due to positionIdx mismatch (Code {ret_code}). Check Hedge Mode settings."
                )
            elif ret_code in [170131, 110042]:
                lg.warning(
                    f"Hint: Set protection failed due to invalid TP/SL price (Code {ret_code}). Check price vs mark/index/entry."
                )
            elif ret_code == 170133:
                lg.warning(
                    f"Hint: Cannot set TP/SL/TSL, potentially due to order status or existing orders (Code {ret_code})."
                )
            return False
...
def attempt_new_entry(
    ...
    entry_ref, tp_price, sl_price = analyzer.calculate_entry_tp_sl(
        entry_price_signal, signal
    )
    ...
    if entry_ref is None or sl_price is None:
        lg.error(
            f"Entry Aborted ({symbol}): Failed to calculate valid entry/SL price. EntryRef={entry_ref}, SL={sl_price}"
        )
        return False
    ...
    if market_info.get("is_contract", False):
        if not set_leverage_ccxt(exchange, symbol, leverage, lg, market_info):
            lg.error(
                f"Entry Aborted ({symbol}): Failed to set leverage to {leverage}x."
            )
    ...
    if position_mode == "Hedge":
        ...
        lg.info(
            f"Using positionIdx={entry_position_idx} for {entry_side} order in Hedge Mode."
        )
    ...
    lg.info(
        f"Entry order placed ({symbol}, ID: {entry_order.get('id')}). Waiting {POSITION_CONFIRM_DELAY}s to confirm position..."
    )
    ...
    if not confirmed_position:
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Position not found after placing order {entry_order.get('id')}. Manual check required!{RESET}"
        )
    ...
    if confirmed_side != entry_side.lower():
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Confirmed position side ({confirmed_side}) differs from order side ({entry_side}). Manual check required!{RESET}"
        )
        return False
    if confirmed_pos_idx != entry_position_idx:
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Confirmed position index ({confirmed_pos_idx}) differs from order index ({entry_position_idx}). Manual check required!{RESET}"
        )
        return False
    ...
    if confirmed_size and position_size:
        size_diff_pct = (
            abs(confirmed_size - position_size) / position_size
            if position_size
            else Decimal("0")
        )
        if size_diff_pct > Decimal("0.05"):  # Allow up to 5% difference
            lg.warning(
                f"{NEON_YELLOW}Position size mismatch ({symbol}): Ordered={position_size}, Confirmed={confirmed_size} (Diff: {size_diff_pct:.2%}). Proceeding, but check if expected.{RESET}"
            )
    else:
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Invalid confirmed size ({confirmed_size}) or order size ({position_size}). Manual check required!{RESET}"
        )
        return False

    if not confirmed_entry_price:
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Could not retrieve entry price from confirmed position. Cannot set protection accurately.{RESET}"
        )
        # If we can't get entry price, setting accurate SL/TP is impossible. Consider this a failure.
        return False

    lg.info(
        f"Position Confirmed ({symbol}): Side={confirmed_side}, Size={confirmed_size}, Entry={confirmed_entry_price}, Idx={confirmed_pos_idx}"
    )
    ...
    if entry_ref != confirmed_entry_price:
        lg.debug(
            f"Note: Using initial TP/SL based on EntryRef={entry_ref}, Actual Entry={confirmed_entry_price}. Difference: {abs(entry_ref - confirmed_entry_price)}"
        )
    ...
    if config.get("enable_trailing_stop", False) and market_info.get("is_contract"):
        ...
        tsl_callback_rate = Decimal(
            str(config.get("trailing_stop_callback_rate", 0.005))
        )  # e.g., 0.5%
        tsl_activation_perc = Decimal(
        ...
        tsl_price_distance = analyzer.quantize_price(
            confirmed_entry_price * tsl_callback_rate, rounding=ROUND_UP
        )  # Round distance up
        ...
        if tsl_price_distance is not None and tsl_price_distance > 0:
            ...
            if tsl_activation_perc > 0:
                ...
                if confirmed_side == "long":
                    ...
                    tsl_active_price = analyzer.quantize_price(
                        raw_active_price, rounding=ROUND_UP
                    )
                else:  # Short
                    raw_active_price = confirmed_entry_price - activation_offset
                    tsl_active_price = analyzer.quantize_price(
                        raw_active_price, rounding=ROUND_DOWN
                    )
                # Ensure activation price is valid (positive)
                if tsl_active_price is not None and tsl_active_price <= 0:
                    lg.warning(
                        f"Calculated TSL activation price {tsl_active_price} is invalid. Setting activation to immediate (0)."
                    )
            ...
            if min_tick and tsl_price_distance < min_tick:
                lg.warning(
                    f"Calculated TSL distance {tsl_price_distance} is less than min tick {min_tick}. Disabling TSL."
                )
                tsl_price_distance = None
                tsl_active_price = None
        else:
            lg.warning(
                f"Failed to calculate valid TSL distance ({tsl_price_distance}). Disabling TSL."
            )
    ...
    if not protection_set:
        lg.error(
            f"{NEON_RED}Entry Failed ({symbol}): Failed to set initial TP/SL/TSL protection after confirming position. Closing position for safety...{RESET}"
        )
        # If protection setting fails, close the position immediately to avoid uncontrolled risk.
        close_result = close_position_ccxt(
            exchange, symbol, confirmed_position, lg, market_info
        )
        if close_result:
            lg.info(
                f"Successfully closed position for {symbol} due to failed protection setting."
            )
        else:
            lg.critical(
                f"{NEON_RED}CRITICAL: Failed to set protection AND failed to close position for {symbol}. MANUAL INTERVENTION REQUIRED!{RESET}"
            )
    ...
    # --- Success ---
    lg.info(
        f"{NEON_GREEN}Entry sequence for {signal} {symbol} completed successfully.{RESET}"
    )
...
def check_exit_conditions(
    ...
    if analyzer.signals["BUY"] == 1 or analyzer.signals["SELL"] == 1:
        pass
...
def manage_existing_position(
    ...
    if not position_side or not entry_price:
        lg.error(
            f"Position management error ({symbol}): Missing side or entry price in position data."
        )
    ...
    )  # Check state from analyzer property (which reads symbol_state)
    ...
    if enable_be and not be_triggered:
        ...
        if atr_val is not None and atr_val > 0:
            ...
            lg.debug(
                f"BE Check ({symbol}): CurrentProfit={current_profit:.{price_prec}f}, TargetProfit(ATR*Mult) > {profit_target_raw:.{price_prec}f} (ATR={atr_val:.{price_prec + 1}f}, Mult={be_trigger_atr_mult})"
            )

            if current_profit >= profit_target_raw:
                lg.info(
                    f"{NEON_GREEN}Break-Even condition met for {symbol}! Current profit ({current_profit:.{price_prec}f}) >= Target ({profit_target_raw:.{price_prec}f}). Moving SL...{RESET}"
                )
                ...
                if min_tick is None:
                    lg.error(
                        f"Cannot set Break-Even SL for {symbol}: Minimum tick size unavailable."
                    )
                    ...
                    be_sl_price = analyzer.quantize_price(
                        be_sl_price_raw, rounding=rounding
                    )

                    if be_sl_price is None or be_sl_price <= 0:
                        lg.error(
                            f"Failed to calculate valid BE SL price ({be_sl_price_raw} -> {be_sl_price}) for {symbol}. Cannot set BE SL."
                        )
                    else:
                        lg.info(
                            f"Calculated Break-Even SL price for {symbol}: {be_sl_price:.{price_prec}f} (Entry={entry_price:.{price_prec}f}, Offset={be_offset_ticks} ticks)"
                        )
                        ...
                        if be_force_fixed:
                            lg.info(
                                f"Break-Even forcing fixed SL. Attempting to remove existing TP/TSL for {symbol}."
                            )
                        ...
                        be_set_success = set_protection_ccxt(
                            exchange=exchange,
                            symbol=symbol,
                            **protection_params,  # Unpack prepared params
                            logger=lg,
                            market_info=market_info,
                        )
                        ...
                        if be_set_success:
                            lg.info(
                                f"{NEON_GREEN}Break-Even SL successfully set to {be_sl_price:.{price_prec}f} for {symbol}.{RESET}"
                            )
                            # Update state ONLY if SL was set successfully
                            analyzer.break_even_triggered = (
                                True  # Use setter to update shared state
                            )
                        else:
                            lg.error(
                                f"{NEON_RED}Failed to set Break-Even SL for {symbol}. Position remains open with previous protection (if any).{RESET}"
                            )
    ...
    exit_condition_met, exit_reason = check_exit_conditions(
        analyzer, position_data, current_price, config, lg
    )

    if exit_condition_met:
        lg.warning(
            f"{NEON_YELLOW}Exit condition '{exit_reason}' met for {symbol}. Closing position...{RESET}"
        )
        close_result = close_position_ccxt(
            exchange, symbol, position_data, lg, market_info
        )
        if close_result:
            lg.info(
                f"Position closed successfully due to exit condition: {exit_reason}"
            )
            ...
            lg.error(
                f"{NEON_RED}Failed to close position for {symbol} despite exit condition '{exit_reason}'. Manual check required!{RESET}"
            )
    ...
    # --- Position still open ---
    lg.debug(
        f"No management actions triggered or position closed for {symbol}. Continuing."
    )
...
def run_bot_for_symbol(
    ...
    """
    symbol_logger = get_logger(
        symbol, is_symbol_logger=True
    ...
    try:
        # --- Fetch Required Data ---
        # 1. Current Price
        current_price = fetch_current_price_ccxt(
        ...
        required_klines = (
            max(
                config.get("ema_long_period", 21),
                config.get("bollinger_bands_period", 20),
                config.get("fibonacci_window", 50),
        ...
        klines_df_raw = fetch_klines_ccxt(
            exchange, symbol, interval, kline_limit, symbol_logger, market_info
        )
        if klines_df_raw.empty:
            symbol_logger.error(
                f"Failed to fetch sufficient kline data ({len(klines_df_raw)} rows). Skipping cycle."
            )
        ...
        orderbook_data = None
        if (
            config.get("indicators", {}).get("orderbook")
            and config.get("weight_sets", {})
            .get(config.get("active_weight_set", "default"), {})
            .get("orderbook", 0.0)
            != 0.0
        ):
            orderbook_limit = config.get("orderbook_limit", 25)
            orderbook_data = fetch_orderbook_ccxt(
                exchange, symbol, orderbook_limit, symbol_logger, market_info
            )
            if orderbook_data is None:
                symbol_logger.warning(
                    "Failed to fetch order book data. Orderbook signal component will be skipped."
                )

        # --- Initialize Analyzer (Calculates indicators) ---
        try:
            analyzer = TradingAnalyzer(
                klines_df_raw, symbol_logger, config, market_info, symbol_state
            )
            # Check if essential indicators (like ATR) calculated successfully
            if analyzer.indicator_values.get("ATR") is None:
                symbol_logger.error(
                    "ATR calculation failed. Cannot proceed with risk management. Skipping cycle."
                )
                return symbol_state
        except ValueError as e:
            symbol_logger.error(
                f"Failed to initialize TradingAnalyzer: {e}. Skipping cycle."
            )
            return symbol_state
        except Exception as e:
            symbol_logger.error(
                f"Unexpected error initializing TradingAnalyzer: {e}. Skipping cycle.",
                exc_info=True,
            )
            return symbol_state
        ...
        if market_info.get("is_contract"):
            current_position = fetch_positions_ccxt(
                exchange, symbol, symbol_logger, market_info
            )
        ...
        if current_position:
            ...
            # --- Not in Position ---
            symbol_logger.debug(
                f"No active position found for {symbol}. Evaluating entry signals..."
            )
            ...
            if signal != "HOLD":
                ...
                if max_total_pos == 1:  # We already know current_position is None here
                    ...
                    symbol_logger.warning(
                        f"Max concurrent positions > 1 ({max_total_pos}) requires global position tracking (not fully implemented). Allowing entry attempt for {symbol} assuming limit not reached."
                    )
                ...
                if not config.get("enable_trading", False):
                    symbol_logger.warning(
                        f"Trading signal '{signal}' generated for {symbol}, but 'enable_trading' is FALSE. No entry order will be placed."
                    )
            ...
            if can_open_new_position and signal != "HOLD":
                ...
                if current_balance is not None and current_balance > 0:
                    ...
                    if entry_successful:
                        ...
                        symbol_logger.error(
                            f"Entry attempt failed for {signal} {symbol}."
                        )
                        # State should be clean unless entry partially failed (handled in attempt_new_entry)
                else:
                    symbol_logger.error(
                        f"Cannot attempt entry for {symbol}: Failed to fetch balance or balance is zero ({current_balance})."
                    )
            else:
                symbol_logger.debug(
                    f"Holding {symbol}. Signal: {signal}, Can Open: {can_open_new_position}"
                )

        # --- Cycle End ---
        if position_closed_this_cycle:
            # If closed, ensure state reflects no position (BE flag reset, etc.)
            symbol_state = bot_state.get(
                symbol, {}
            )  # Re-fetch potentially updated state
        ...
        symbol_logger.warning(
            f"Network error during cycle for {symbol}: {e}. Skipping cycle."
        )
        return symbol_state  # Return existing state, hope connection recovers
    except ccxt.ExchangeError as e:
        symbol_logger.error(
            f"Exchange error during cycle for {symbol}: {e}. Skipping cycle.",
            exc_info=True,
        )
        return symbol_state  # Return existing state, might be non-retryable API issue
    except Exception as e:
        symbol_logger.error(
            f"Unhandled exception in main loop for {symbol}: {e}", exc_info=True
        )
...
def main() -> None:
    ...
    if args.delay is not None:
        if 5 <= args.delay <= 3600:
            ...
            main_logger.info(
                f"Loop delay overridden by command line argument: {LOOP_DELAY_SECONDS}s"
            )
        else:
            main_logger.warning(
                f"Invalid --delay value ({args.delay}). Must be between 5 and 3600. Using config value."
            )
    ...
    if exchange is None:
        ...
        sys.exit(1)
    main_logger.info(
        f"Exchange {exchange.id} initialized successfully. Sandbox: {config.get('use_sandbox')}. Account Type: {'UNIFIED' if IS_UNIFIED_ACCOUNT else 'Non-UTA'}"
    )
    ...
    for sym in symbols_to_trade:
        ...
        if market:
            ...
            if config.get("position_mode") == "Hedge" and market.get("category") in [
                "linear",
                "inverse",
            ]:
                main_logger.info(
                    f"Hedge Mode active for symbol {sym}. Ensure positionIdx is handled correctly."
                )
        else:
            main_logger.error(
                f"Could not fetch market info for {sym}. It will be skipped."
            )

    if not successful_symbols:
        main_logger.critical(
            "Failed to fetch market info for ALL configured symbols. Exiting."
        )
        sys.exit(1)

    main_logger.info(
        f"Successfully loaded market info for: {successful_symbols}. Bot starting main loop..."
    )
    ...
    while True:
        ...
        main_logger.info(
            f"--- Starting Bot Cycle {datetime.now(tz=TIMEZONE).strftime('%Y-%m-%d %H:%M:%S %Z')} ---"
        )
        ...
        if time.time() - last_market_reload_time > MARKET_RELOAD_INTERVAL_SECONDS:
            ...
            try:
                ...
                for (
                    sym
                ) in successful_symbols:  # Only retry symbols that worked initially
                    market = get_market_info(exchange, sym, main_logger)
                    if market:
                        all_market_info[sym] = market
                        ...
                        main_logger.error(
                            f"Could not refresh market info for {sym} after reload. Skipping until next reload."
                        )
                successful_symbols = (
                    temp_successful_symbols  # Update list of active symbols
                )
                if not successful_symbols:
                    main_logger.critical(
                        "Failed to refresh market info for ALL symbols after reload. Exiting."
                    )
                    break  # Exit loop if no symbols are valid anymore
            except Exception as reload_err:
                main_logger.error(
                    f"Error reloading markets: {reload_err}. Continuing with potentially stale market data."
                )
        ...
        for symbol in successful_symbols:
            ...
            if updated_symbol_state is not None:
                ...
                main_logger.error(
                    f"run_bot_for_symbol returned None for {symbol}. State might not be saved correctly."
                )
...
if __name__ == "__main__":
    try:
        ...
        main_logger.critical(
            f"Critical unhandled exception in main thread: {e}", exc_info=True
        )
