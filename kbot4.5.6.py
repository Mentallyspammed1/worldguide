...
#                                                             |_|                       |_|
# fmt: on
# Removed stray 'm .km' line causing syntax error
"""
    ...
    **kwargs: Any,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt > 0:
                logger.info(
                    f"{Fore.BRIGHT_GREEN}Successfully executed {func_name} on attempt {attempt + 1}/{max_retries + 1} after previous failures."
                )
            return result
        except fatal_exceptions as e:
            logger.critical(
                f"{Fore.BRIGHT_RED}Fatal error ({type(e).__name__}) executing {func_name}: {e}. Halting immediately.",
                exc_info=False,
            )
            ...
            last_exception = e
            retry_msg = f"{Fore.YELLOW}Retryable error ({type(e).__name__}) on attempt {attempt + 1}/{max_retries + 1} for {func_name}: {str(e)[:150]}."
...
class TradingConfig:
    ...
    def _validate_value(
        ...
        if min_val is not None and is_numeric and value < min_val:
            logger.critical(
                f"{Fore.BRIGHT_RED}Validation failed for {key}: Value '{value}' < minimum '{min_val}'. Halting."
            )
            sys.exit(1)  # Critical failure
        if max_val is not None and is_numeric and value > max_val:
            logger.critical(
                f"{Fore.BRIGHT_RED}Validation failed for {key}: Value '{value}' > maximum '{max_val}'. Halting."
            )
            sys.exit(1)  # Critical failure
    ...
    def _get_env(
        ...
        log_method(f"{color}Using {key}: {log_value_display} (from {source})")
...
class ExchangeManager:
    ...
    def _format_v5_param(
        ...
        if param_type == "price" or param_type == "distance":
            ...
            # formatter = lambda amt: self.format_amount(amt, rounding_mode=ROUND_DOWN)
            def formatter(amt):
                return self.format_amount(amt, rounding_mode=ROUND_DOWN)  # pylint: disable=unnecessary-lambda
    ...
    def calculate_indicators(
        ...
        try:
            ...
            if len(df_calc) < min_required_len:
                logger.error(
                    f"{Fore.RED}Insufficient data ({len(df_calc)} rows < required ~{min_required_len}) for indicators."
                )
                return None
            ...
            if failed_indicators:
                ...
                if indicators_out.get("atr", Decimal("NaN")).is_nan():
                    logger.error(
                        f"{Fore.RED}ATR is NaN, cannot proceed with risk calculations. Aborting indicator calc."
                    )
                    return None
...
class OrderManager:
    ...
    def _verify_position_state(
        ...
        for attempt in range(max_attempts):
            ...
            if current_positions is None:
                logger.warning(
                    f"{action_context} Verification Warning: Failed to fetch position state on attempt {attempt + 1}."
                )
                if attempt < max_attempts - 1:
                    ...
                    logger.error(
                        f"{Fore.RED}{action_context} Verification Failed: Could not fetch position state after {max_attempts} attempts."
                    )
                    return (
                        False,
                        last_known_position_state,
    ...
    def place_risked_market_order(
        ...
        logger.info(
            f"{Fore.BRIGHT_MAGENTA}--- Initiating Entry Sequence for {position_side.upper()} ---{Style.RESET_ALL}"
        )
        ...
        if not verification_ok:
            logger.error(
                f"{Fore.RED}Entry Failed: Position verification FAILED after market order {order_id}. Manual check required! Attempting cleanup..."
            )
            self._handle_entry_failure(side, qty_to_order)  # Attempt cleanup
            return False
        ...
        logger.info(
            f"{Fore.BRIGHT_GREEN}--- Entry Sequence for {position_side.upper()} Completed Successfully ---{Style.RESET_ALL}"
        )
        return True
    ...
    def close_position(
        ...
        logger.trade(
            f"{Fore.YELLOW}Attempting to CLOSE {position_side.upper()} pos ({qty_to_close.normalize()} {symbol}) | Reason: {reason}..."
        )
...
class StatusDisplay:
    ...
    def print_status_panel(
        ...
        if signals.get("long"):
            ...
            sig_style = "yellow"
        elif "No Signal:" not in sig_reason and "Initializing" not in sig_reason:
            sig_style = "white"

        wrapped_reason = "\n        ".join(textwrap.wrap(sig_reason, width=100))
...
# Removed stray 'k2q' line causing syntax error
...
class TradingBot:
    ...
    def __init__(self):
        ...
        # Removed stray 'la' causing syntax error
        ...
        # Fixed syntax errors in except block ('a. s e.' and 'qq1qq.f"..."')
        except ValueError as e:
    ...
    def trading_spell_cycle(self, cycle_count: int) -> None:
        ...
        if can_run_trade_logic:
            ...
            if positions_after_tsl is None:
                logger.error(
                    f"{Fore.RED}Failed re-fetching positions after TSL check (Cycle {cycle_count}). Using potentially stale state."
                )
                ...
                for side in ["long", "short"]:
                    ...
                    if pos_data:
                        ...
                        if qty.copy_abs() >= POSITION_QTY_EPSILON:
                            logger.warning(
                                f"{Fore.YELLOW}Lingering {side.upper()} position (Qty: {qty.normalize()}) found."
                            )
