...
def display_position_status(position: Dict[str, Any], price_precision: int = 4, amount_precision: int = 8) -> None:
    ...
    tsl_str: str = f" | TSL: {tsl:.{price_precision}f}" if isinstance(tsl, (float, int)) else "" # Show TSL only if active
...
def display_market_stats(current_price: float, rsi: float, stoch_k: float, stoch_d: float, atr: Optional[float], price_precision: int) -> None:
...
def display_order_blocks(bullish_ob: Optional[Dict[str, Any]], bearish_ob: Optional[Dict[str, Any]], price_precision: int) -> None:
            ...
            while retries < max_retries:
                try:
                    ...
                    if retries >= max_retries:
                        ...
                        log_warning(f"API call '{func.__name__}' failed with {type(e).__name__}: {e}. Retrying in {delay:.1f}s... (Attempt {retries}/{max_retries})")
...
def load_config(filename: str = CONFIG_FILE) -> Dict[str, Any]:
    ...
    try:
        ...
        required_keys: List[str] = [
            "exchange_id", "symbol", "timeframe", "risk_percentage", "simulation_mode",
            "rsi_length", "rsi_overbought", "rsi_oversold",
            "stoch_k", "stoch_d", "stoch_smooth_k", "stoch_overbought", "stoch_oversold",
            "data_limit", "sleep_interval_seconds",
            "ob_volume_threshold_multiplier", "ob_lookback",
        ...
        if not isinstance(config_data["risk_percentage"], (float, int)) or not (0 < config_data["risk_percentage"] < 1):
             log_error(f"CRITICAL: 'risk_percentage' ({config_data['risk_percentage']}) must be a number between 0 and 1 (exclusive).")
             sys.exit(1)
        if not isinstance(config_data["sleep_interval_seconds"], int) or config_data["sleep_interval_seconds"] <= 0:
             log_error(f"CRITICAL: 'sleep_interval_seconds' ({config_data['sleep_interval_seconds']}) must be a positive integer.")
             sys.exit(1)
...
if not api_key or not secret:
    log_error(f"CRITICAL: API Key ('{api_key_env_var}') or Secret ('{secret_key_env_var}') not found in environment/.env file.")
    sys.exit(1)
...
try:
    ...
    exchange_config: Dict[str, Any] = {
        ...
        'options': {
            ...
            'defaultType': 'swap' if ':' in config.get("symbol", "") or 'SWAP' in config.get("symbol", "").upper() else 'spot',
    ...
    log_info(f"Successfully connected to {exchange_id}. Markets loaded ({len(exchange.markets)} symbols found). Default type: {exchange.options.get('defaultType')}")
...
try:
    ...
    if min_tick_value_str is not None:
         try:
             ...
             log_warning(f"Could not parse 'precision.price' value '{min_tick_value_str}' to float. Using default min_tick calculation.")
             min_tick = 1 / (10 ** price_precision_digits) if price_precision_digits >= 0 else 0.01
    else:
         min_tick = 1 / (10 ** price_precision_digits) if price_precision_digits >= 0 else 0.01 # Fallback

    log_info(f"Symbol Precision | Price: {price_precision_digits} decimals (Min Tick: {min_tick:.{price_precision_digits+2}f}), Amount: {amount_precision_digits} decimals")
...
if needs_atr:
    if not isinstance(atr_length, int) or atr_length <= 0:
        log_error(f"CRITICAL: 'atr_length' ({atr_length}) must be a positive integer if ATR SL/TP or Trailing Stop is enabled.")
        sys.exit(1)
...
if enable_atr_sl_tp:
    ...
    if not isinstance(atr_sl_multiplier, (float, int)) or atr_sl_multiplier <= 0 or \
       not isinstance(atr_tp_multiplier, (float, int)) or atr_tp_multiplier <= 0:
    ...
    if not isinstance(stop_loss_percentage, (float, int)) or stop_loss_percentage <= 0 or \
       not isinstance(take_profit_percentage, (float, int)) or take_profit_percentage <= 0:
...
if enable_trailing_stop:
    ...
    if not isinstance(trailing_stop_atr_multiplier, (float, int)) or trailing_stop_atr_multiplier <= 0 or \
       not isinstance(trailing_stop_activation_atr_multiplier, (float, int)) or trailing_stop_activation_atr_multiplier < 0: # Activation can be 0
         log_error("CRITICAL: Trailing stop ATR multiplier must be positive number, activation multiplier non-negative number.")
         sys.exit(1)
else:
...
if entry_volume_confirmation_enabled:
    ...
    if not isinstance(entry_volume_ma_length, int) or entry_volume_ma_length <= 0 or \
       not isinstance(entry_volume_multiplier, (float, int)) or entry_volume_multiplier <= 0:
         log_error("CRITICAL: Volume MA length and multiplier must be positive numbers when volume confirmation is enabled.")
         sys.exit(1)
else:
...
def save_position_state(filename: str = POSITION_STATE_FILE) -> None:
    ...
    try:
        ...
        log_error(f"Serialization error saving position state to {filename}. Check data types. Error: {e}", exc_info=True)
    except IOError as e:
    ...
    try:
        ...
        for key, default_value in position_default_structure.items():
             if key in loaded_keys:
                 ...
                 if loaded_value is not None and default_value is not None and not is_type_match and not allow_int_for_float:
                     type_mismatches.append(f"'{key}' (Expected {expected_type.__name__}, Got {type(loaded_value).__name__})")
                     ...
                     if key != 'entry_time' or parsed_entry_time is not None: # Count update unless entry_time parse failed
                        updated_count += 1
             else:
        ...
        if updated_count > 0 or not missing_in_file:
             ...
             log_warning(f"Loaded state file {filename}, but applied 0 values due to missing keys or type mismatches. Using default state.")
             position = position_default_structure.copy() # Ensure reset to default

    except json.JSONDecodeError as e:
        log_error(f"Error decoding JSON from state file {filename}: {e}. Starting with default empty state.", exc_info=False)
        position = position_default_structure.copy() # Reset to default
    except Exception as e:
        log_error(f"Error loading position state from {filename}: {e}. Starting with default empty state.", exc_info=True)
        position = position_default_structure.copy() # Reset to default
...
def fetch_ohlcv_data(exchange_instance: ccxt.Exchange, trading_symbol: str, tf: str, limit_count: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if trading_symbol not in exchange_instance.markets:
             log_error(f"Symbol '{trading_symbol}' not found in locally loaded markets for exchange '{exchange_instance.id}'.")
             return None
        ...
        if not ohlcv:
            log_warning(f"No OHLCV data returned for {trading_symbol} ({tf}). Exchange might be down, symbol inactive, or no recent trades.")
            return None
    ...
    # Handle specific non-retryable CCXT errors after retries have failed
         ...
         log_error(f"BadSymbol error fetching OHLCV for {trading_symbol}: {e}. Check symbol format/availability on {exchange_instance.id}.")
         return None
...
def identify_potential_order_block(df: pd.DataFrame, vol_thresh_mult: float, lookback_len: int) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if len(completed_candles_df) >= lookback_len:
             ...
             rolling_vol = completed_candles_df['volume'].rolling(window=lookback_len, min_periods=max(1, lookback_len // 2)).mean()
             ...
             if pd.isna(avg_volume): avg_volume = 0.0 # Handle potential NaN mean if all volumes were NaN

        volume_threshold = avg_volume * vol_thresh_mult if avg_volume > 0 else float('inf') # Avoid threshold=0 or negative threshold
        log_debug(f"OB Analysis | Lookback: {lookback_len}, Avg Vol (Completed Candles): {avg_volume:.2f}, Threshold Vol: {volume_threshold:.2f}")
        ...
        for i in range(start_index, end_index, -1): # Iterate backwards
            ...
            if i < 1: break # Need at least one previous candle (index i-1)
            ...
            is_bullish_impulse = impulse_candle['close'] > impulse_candle['open'] and impulse_candle['close'] > ob_candle['high']
            # Bearish impulse: Strong close, ideally closing below OB candle's low
            is_bearish_impulse = impulse_candle['close'] < impulse_candle['open'] and impulse_candle['close'] < ob_candle['low']
            # OB candle type
            ...
            if (not bullish_ob and # Find only the most recent one
                ob_is_bearish and is_bullish_impulse and is_high_volume_impulse and impulse_sweeps_ob_low):
                bullish_ob = {
                    'high': ob_candle['high'], 'low': ob_candle['low'],
                    'time': ob_candle.name, # Timestamp of the bearish candle (the OB)
                    'type': 'bullish'
                }
                log_debug(f"Potential Bullish OB identified at {ob_candle.name.strftime('%Y-%m-%d %H:%M')} (Triggered by: {impulse_candle.name.strftime('%H:%M')})")
            ...
            elif (not bearish_ob and # Find only the most recent one
                  ob_is_bullish and is_bearish_impulse and is_high_volume_impulse and impulse_sweeps_ob_high):
                bearish_ob = {
                    'high': ob_candle['high'], 'low': ob_candle['low'],
                    'time': ob_candle.name, # Timestamp of the bullish candle (the OB)
                    'type': 'bearish'
                }
                log_debug(f"Potential Bearish OB identified at {ob_candle.name.strftime('%Y-%m-%d %H:%M')} (Triggered by: {impulse_candle.name.strftime('%H:%M')})")
                # Optimization: If we found both types, stop searching further back
                if bullish_ob: break
...
def calculate_technical_indicators(
    ...
    if calc_vol_ma: required_base_cols.append('volume')
    if not all(col in df.columns for col in required_base_cols):
        log_error(f"Missing required base columns for indicator calculation: Need {required_base_cols}, found {df.columns.tolist()}")
        return None
    ...
    try:
        ...
        df_processed.ta.stoch(k=stoch_params['k'], d=stoch_params['d'], smooth_k=stoch_params['smooth_k'], append=True, col_names=(stoch_k_col, stoch_d_col))
        ...
        if calc_atr:
            ...
            if atr_col_name: calculated_indicator_names.append(atr_col_name)
        ...
        if calc_vol_ma:
            if 'volume' in df_processed.columns:
                ...
                df_processed[vol_ma_col_name] = df_processed['volume'].rolling(window=vol_ma_len, min_periods=max(1, vol_ma_len // 2)).mean()
                log_debug(f"Volume MA ({vol_ma_len}) calculated.")
                if vol_ma_col_name: calculated_indicator_names.append(vol_ma_col_name)
            else:
...
def calculate_position_size(
    ...
    try:
        log_debug(f"Calculating position size: Symbol={trading_symbol}, Entry={current_price:.{price_precision_digits}f}, SL={stop_loss_price:.{price_precision_digits}f}, Risk={risk_perc*100:.2f}%")
        ...
        if not base_currency or not quote_currency:
             ...
             return None
        log_debug(f"Market details: Base={base_currency}, Quote={quote_currency}, Inverse={is_inverse}, ContractSize={contract_size}")
        ...
        elif isinstance(balance.get(quote_currency), dict) and 'total' in balance[quote_currency] and available_balance == 0.0:
        ...
        # 4. Exchange-specific checks (Example for Bybit Unified Margin Account - USDT)
             ...
             if 'info' in balance and 'result' in balance['info'] and isinstance(balance['info']['result'], dict) and 'list' in balance['info']['result']:
                 for wallet_info in balance['info']['result']['list']:
                      if isinstance(wallet_info, dict) and wallet_info.get('accountType') == 'UNIFIED' and wallet_info.get('coin') == quote_currency:
                          ...
                          balance_info_to_log = f"Bybit Unified 'availableToWithdraw' or 'walletBalance'"
                          break
             # Check contract account structure (older or non-unified)
             elif isinstance(balance.get(quote_currency), dict) and 'available' in balance[quote_currency]: # Bybit contracts might use 'available'
                  available_balance = float(balance[quote_currency]['available'])
                  balance_info_to_log = f"Bybit Contract 'available'"

        # Add other exchange-specific checks here if needed...

        if available_balance <= 0:
            log_error(f"No available {quote_currency} balance ({available_balance}) found for trading. Check funds or balance structure. Balance info checked: {balance_info_to_log}. Full balance details: {balance.get(quote_currency, balance.get('info', 'Not Available'))}")
            return None
        log_info(f"Available balance ({quote_currency}): {available_balance:.{price_precision_digits}f} (Source: {balance_info_to_log})")

        # --- Calculate Risk Amount ---
        risk_amount_quote = available_balance * risk_perc
        log_info(f"Risk per trade ({risk_perc*100:.2f}%): {risk_amount_quote:.{price_precision_digits}f} {quote_currency}")
        ...
        if price_diff <= min_tick / 2: # Ensure SL is meaningfully far from entry
            log_error(f"Stop-loss price {stop_loss_price:.{price_precision_digits}f} is too close to entry price {current_price:.{price_precision_digits}f} (Diff: {price_diff:.{price_precision_digits+4}f} <= Min Tick/2: {min_tick/2:.{price_precision_digits+4}f}). Cannot calculate size.")
            return None
        ...
        if is_inverse:
            ...
            log_warning(f"Calculating size for INVERSE contract ({trading_symbol}). Ensure risk calculation logic matches exchange specifications.")
            ...
            log_debug(f"Inverse contract intermediate quantity (base units): {quantity_base:.{amount_precision_digits+4}f}")
        ...
        try:
            ...
            log_debug(f"Quantity adjusted for precision: {quantity_adjusted:.{amount_precision_digits}f} {base_currency}")
        except ccxt.ExchangeError as precision_error:
             log_warning(f"Could not use exchange.amount_to_precision: {precision_error}. Using manual rounding (less precise).")
        ...
        if quantity_adjusted <= 0:
            log_error(f"Calculated adjusted quantity ({quantity_adjusted:.{amount_precision_digits}f}) is zero or negative. Check risk % or balance.")
            return None
        ...
        if min_amount is not None and quantity_adjusted < min_amount:
            log_error(f"Calculated quantity {quantity_adjusted:.{amount_precision_digits}f} {base_currency} is below minimum required amount {min_amount} {base_currency}.")
            return None
        if max_amount is not None and quantity_adjusted > max_amount:
            log_warning(f"Calculated quantity {quantity_adjusted:.{amount_precision_digits}f} exceeds max amount {max_amount}. Capping quantity to max amount.")
        ...
        if min_cost is not None and estimated_cost < min_cost:
             log_error(f"Estimated order cost ({estimated_cost:.{price_precision_digits}f} {quote_currency}) is below minimum cost {min_cost} {quote_currency}. Required Qty > {min_cost / current_price:.{amount_precision_digits}f} {base_currency}")
             return None
        ...
        if estimated_cost > available_balance * cost_buffer_factor:
             log_error(f"Estimated cost ({estimated_cost:.{price_precision_digits}f} {quote_currency}) exceeds {cost_buffer_factor*100:.0f}% of available balance ({available_balance:.{price_precision_digits}f} {quote_currency}). Reduce risk % or add funds.")
             return None

        log_info(f"{NEON_GREEN}Position size calculated: {quantity_adjusted:.{amount_precision_digits}f} {base_currency}{RESET} (Risking ~{risk_amount_quote:.2f} {quote_currency})")
...
def place_market_order(
    ...
    try:
        ...
        try:
             ...
             if amount_formatted <= 0:
                  log_error(f"Amount {amount} became zero or negative ({amount_formatted}) after applying precision. Cannot place order.")
                  return None
        ...
        log_info(f"Attempting to place {side.upper()} market order for {amount_formatted:.{amount_precision_digits}f} {base_currency} on {trading_symbol} {'(ReduceOnly)' if reduce_only else ''}...")
        ...
        if SIMULATION_MODE:
            ...
            try:
                 ...
                 if sim_price <= 0: log_warning("Could not fetch valid ticker price for simulation, using 0.")
            ...
            order = {
                'id': sim_order_id, 'clientOrderId': sim_order_id,
                ...
                'info': {'simulated': True, 'reduceOnly_applied': params.get('reduceOnly', False)} # Add simulation info
            }
        else:
            # --- LIVE TRADING ---
    ...
    # --- Handle Specific Non-Retryable Errors (after decorator retries) ---
        ...
        log_error(f"Insufficient funds for {side} {amount_formatted:.{amount_precision_digits}f} {trading_symbol}. Check balance and risk settings. Error: {e}")
        return None
    except ccxt.InvalidOrder as e:
         # Indicates incorrect parameters, symbol state (e.g., suspended), or order logic violation.
         log_error(f"Invalid market order parameters for {side} {amount_formatted:.{amount_precision_digits}f} {trading_symbol}. Check market limits, amount precision, or symbol status. Error: {e}")
         return None
    except ccxt.OrderNotFound as e: # Can happen if order rejected immediately or right after placement attempt
        log_error(f"OrderNotFound error placing {side} {amount_formatted:.{amount_precision_digits}f} {trading_symbol}. Likely rejected by exchange immediately. Error: {e}")
        return None
...
def place_sl_tp_orders(
    ...
    if position_side not in ['long', 'short']:
        ...
        return None, None
    if not isinstance(sl_price, (float, int)) or sl_price <= 0 or \
       not isinstance(tp_price, (float, int)) or tp_price <= 0:
    ...
    try:
        ...
        try:
             ...
             if qty_formatted <= 0: raise ValueError("Quantity became non-positive after precision formatting.")
        ...
        if reduce_only_param: log_debug("Applying 'reduceOnly=True' to SL/TP orders.")
        ...
        supports_stop_market_param = 'stopPrice' in unified_order_options.get('params', {}) and 'market' in unified_order_options.get('types', {})
        supports_stop_limit_param = 'stopPrice' in unified_order_options.get('params', {}) and 'limit' in unified_order_options.get('types', {}) # Assuming limit type is standard
        ...
        if exchange_instance.has.get('createStopMarketOrder', False): # Check explicit method first
        ...
        elif supports_stop_market_param or exchange_instance.id in ['binance', 'bybit', 'okx']: # Assume common exchanges support via params
             stop_loss_order_type = 'stopMarket'
            ...
            log_error(f"Could not determine a supported stop order type (stopMarket or stopLimit) via createOrder or explicit methods for {exchange_instance.id}. Cannot place automated SL.")
        ...
        if stop_loss_order_type == 'stopMarket':
            ...
            log_info(f"Attempting SL ({stop_loss_order_type}): {close_side.upper()} {qty_formatted:.{amount_precision_digits}f} {base_currency} @ trigger ~{sl_price_formatted:.{price_precision_digits}f} {'(ReduceOnly)' if 'reduceOnly' in sl_params else ''}")
            ...
            sl_limit_price_raw = sl_price_formatted - limit_offset_ticks if close_side == 'sell' else sl_price_formatted + limit_offset_ticks
            ...
            log_warning(f"Using stopLimit type for SL. Trigger: {sl_price_formatted:.{price_precision_digits}f}, Limit: {stop_loss_limit_price:.{price_precision_digits}f}. Fill is not guaranteed if price gaps past limit.")
            log_info(f"Attempting SL ({stop_loss_order_type}): {close_side.upper()} {qty_formatted:.{amount_precision_digits}f} {base_currency} @ trigger {sl_price_formatted:.{price_precision_digits}f}, limit {stop_loss_limit_price:.{price_precision_digits}f} {'(ReduceOnly)' if 'reduceOnly' in sl_params else ''}")
        else:
        ...
        if stop_loss_order_type:
            if SIMULATION_MODE:
                ...
                sl_order = {
                    'id': sl_order_id, 'status': 'open', 'symbol': trading_symbol, 'type': stop_loss_order_type,
                    'side': close_side, 'amount': qty_formatted, 'price': stop_loss_limit_price, # Price is limit price for stopLimit, None for stopMarket in some exchanges
                    'stopPrice': sl_params.get('stopPrice'), 'average': None, 'filled': 0.0,
                    'remaining': qty_formatted, 'cost': 0.0, 'reduceOnly': sl_params.get('reduceOnly', False),
        ...
        log_info(f"Attempting TP ({tp_order_type}): {close_side.upper()} {qty_formatted:.{amount_precision_digits}f} {base_currency} @ limit {tp_price_formatted:.{price_precision_digits}f} {'(ReduceOnly)' if 'reduceOnly' in tp_params else ''}")
        ...
        if exchange_instance.has.get('createLimitOrder', True):
             if SIMULATION_MODE:
                 ...
                 tp_order = {
                     ...
                     'side': close_side, 'amount': qty_formatted, 'price': tp_price_formatted, # Limit price
             ...
             log_error(f"Exchange {exchange_instance.id} does not explicitly support limit orders (highly unlikely). Cannot place take-profit.")
        ...
        if sl_ok and not tp_ok and exchange_instance.has.get('createLimitOrder', True):
             ...
             log_warning("TP order placed successfully, but SL order placement failed or did not return an ID. POSITION IS UNPROTECTED!")
             ...
             log_warning("TP order placed, but SL order type not supported by exchange config/heuristics. POSITION IS UNPROTECTED!")
...
def check_position_and_orders(exchange_instance: ccxt.Exchange, trading_symbol: str) -> bool:
    ...
    *other* corresponding order (TP or SL) and resets the local position state.
    ...
    try:
        ...
        log_debug(f"Found {len(open_orders)} open orders for {trading_symbol}. IDs: {open_order_ids if open_order_ids else 'None'}")
        ...
        if not sl_order_id and not tp_order_id:
             ...
             log_warning(f"In {position['status']} position but no SL/TP order IDs are tracked in state. Cannot verify position closure via orders.")
        ...
        if position_reset_required:
            log_info(f"Position closure detected (assumed via {assumed_close_reason}). Resetting local state and cancelling leftover order.")
            ...
            if order_to_cancel_id:
                 ...
                 try:
                     ...
                     if cancel_success:
                         log_info(f"Leftover order {order_to_cancel_id} cancellation request sent or order already closed.")
                     else:
                         # This indicates a potential issue if cancellation failed unexpectedly after retries
                         log_error(f"Failed to confirm cancellation of leftover order {order_to_cancel_id} after retries. Manual check advised.")
                 except Exception as e:
                     # Catch potential exceptions raised by cancel_order_with_retry if retries failed
                     log_error(f"Error occurred trying to cancel leftover order {order_to_cancel_id}: {e}. Manual check advised.", exc_info=False)
            ...
            display_position_status(position, price_precision_digits, amount_precision_digits) # Show updated (empty) status
...
def update_trailing_stop(
    ...
    log_debug(f"TSL Check: Active {position['status']} position. Current SL Order: {position['sl_order_id']}, Last Tracked TSL Price: {position.get('current_trailing_sl_price')}")

    # --- Get Required State Variables ---
    entry_price: Optional[float] = position.get('entry_price')
    initial_sl_price: Optional[float] = position.get('stop_loss') # The SL price *before* any TSL updates
    current_tsl_price: Optional[float] = position.get('current_trailing_sl_price') # The currently active TSL price, if any
    ...
    if position['status'] == 'long':
        ...
        if current_high > activation_price_threshold:
            ...
            log_debug(f"Long TSL Activation Met. High ({current_high:.{price_precision_digits}f}) > Threshold ({activation_price_threshold:.{price_precision_digits}f}). Potential New TSL: {potential_new_tsl_price:.{price_precision_digits}f}")
        else:
             log_debug(f"Long TSL Activation NOT Met. High ({current_high:.{price_precision_digits}f}) <= Threshold ({activation_price_threshold:.{price_precision_digits}f})")
        ...
        if current_low < activation_price_threshold:
            ...
            log_debug(f"Short TSL Activation Met. Low ({current_low:.{price_precision_digits}f}) < Threshold ({activation_price_threshold:.{price_precision_digits}f}). Potential New TSL: {potential_new_tsl_price:.{price_precision_digits}f}")
        else:
             log_debug(f"Short TSL Activation NOT Met. Low ({current_low:.{price_precision_digits}f}) >= Threshold ({activation_price_threshold:.{price_precision_digits}f})")
    ...
    if tsl_activated and potential_new_tsl_price is not None:
        try:
            ...
            new_tsl_price_formatted = float(exchange_instance.price_to_precision(trading_symbol, potential_new_tsl_price))
        except ccxt.ExchangeError as fmt_e:
             log_error(f"TSL Update: Failed to format potential new TSL price {potential_new_tsl_price} to precision: {fmt_e}")
        ...
        if position['status'] == 'long':
             if new_tsl_price_formatted > current_effective_sl_price and new_tsl_price_formatted < current_price:
                 ...
                 log_debug(f"Long TSL Improvement Check: New ({new_tsl_price_formatted:.{price_precision_digits}f}) > Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) AND < Price ({current_price:.{price_precision_digits}f}) -> OK")
             else:
                 log_debug(f"Long TSL Improvement Check: New ({new_tsl_price_formatted:.{price_precision_digits}f}) vs Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) vs Price ({current_price:.{price_precision_digits}f}) -> NO UPDATE")

        elif position['status'] == 'short':
             if new_tsl_price_formatted < current_effective_sl_price and new_tsl_price_formatted > current_price:
                 should_update_tsl = True
                 log_debug(f"Short TSL Improvement Check: New ({new_tsl_price_formatted:.{price_precision_digits}f}) < Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) AND > Price ({current_price:.{price_precision_digits}f}) -> OK")
             else:
                  log_debug(f"Short TSL Improvement Check: New ({new_tsl_price_formatted:.{price_precision_digits}f}) vs Current Eff. SL ({current_effective_sl_price:.{price_precision_digits}f}) vs Price ({current_price:.{price_precision_digits}f}) -> NO UPDATE")

    # --- Execute TSL Update (Cancel Old SL, Place New SL) ---
    if should_update_tsl and new_tsl_price_formatted is not None:
        log_info(f"{NEON_YELLOW}Trailing Stop Update Triggered! Moving SL from ~{current_effective_sl_price:.{price_precision_digits}f} to {new_tsl_price_formatted:.{price_precision_digits}f}{RESET}")
        ...
        try:
            ...
            if cancel_success:
                 ...
                 log_error(f"TSL Update: Failed to cancel old SL order {old_sl_order_id}. Aborting TSL update. Position remains protected by potentially existing old SL.")
                 return # Do not proceed if cancellation failed

            # --- Step 2: Place the new TSL order (as a new stop-loss order) ---
            log_info(f"TSL Update: Attempting to place new SL order at {new_tsl_price_formatted:.{price_precision_digits}f}")
            ...
            try:
                ...
                if current_qty is None or not isinstance(current_qty, (float, int)) or current_qty <= 0:
                    ...
                    raise ValueError("Invalid position quantity for TSL placement.") # Raise to prevent state update below
                ...
                unified_order_options = exchange_instance.options.get('createOrder', {})
                supports_stop_market_param = 'stopPrice' in unified_order_options.get('params', {}) and 'market' in unified_order_options.get('types', {})
                supports_stop_limit_param = 'stopPrice' in unified_order_options.get('params', {}) and 'limit' in unified_order_options.get('types', {})

                if exchange_instance.has.get('createStopMarketOrder', False) or supports_stop_market_param or exchange_instance.id in ['binance', 'bybit', 'okx']:
                    ...
                    original_tp_price = position.get('take_profit', current_price * (1.05 if position['status'] == 'long' else 0.95)) # Use original TP for range, fallback if missing
                    limit_offset_ticks = min_tick * 10
                    sl_limit_price_raw = new_tsl_price_formatted - limit_offset_ticks if close_side == 'sell' else new_tsl_price_formatted + limit_offset_ticks
                    ...
                    log_warning(f"TSL Update: Using stopLimit type. Trigger: {new_tsl_price_formatted:.{price_precision_digits}f}, Limit: {new_sl_limit_price:.{price_precision_digits}f}.")
                else:
                     log_error(f"TSL Update: Exchange {exchange_instance.id} supports neither stopMarket nor stopLimit via createOrder for TSL update. Cannot place new TSL order.")
                     raise ccxt.NotSupported(f"Exchange {exchange_instance.id} does not support required stop order types for TSL update.")
                ...
                if SIMULATION_MODE:
                    ...
                    new_sl_order_result = {
                        'id': new_sl_order_id, 'status': 'open', 'type': new_sl_order_type,
                        'price': new_sl_limit_price, 'stopPrice': new_sl_params.get('stopPrice'),
                        'info': {'simulated': True}
                    }
                else:
                    ...
                    def place_new_tsl_order_live(qty_fmt):
                        ...
                        return exchange_instance.create_order(
                            ...
                            amount=qty_fmt, price=new_sl_limit_price, params=new_sl_params
                        )
                    ...
                    if qty_formatted_live <= 0: raise ValueError("Quantity became non-positive after precision formatting for live TSL order.")
                ...
                if new_sl_order_result and new_sl_order_result.get('id'):
                    ...
                    log_info(f"{NEON_GREEN}TSL Update: New trailing SL order placed successfully: ID {new_sl_order_id} at {new_tsl_price_formatted:.{price_precision_digits}f}{RESET}")
                    ...
                    log_error(f"TSL Update CRITICAL ERROR: Failed to place new trailing SL order after cancelling old one {old_sl_order_id}. POSITION IS NOW UNPROTECTED.")
                ...
                log_error(f"TSL Update CRITICAL ERROR: Error placing new trailing SL order: {place_e}. POSITION MAY BE UNPROTECTED.", exc_info=True)
            ...
            log_error(f"TSL Update: Error cancelling old SL order {old_sl_order_id} during TSL update: {cancel_e}. Aborting TSL placement.", exc_info=False)
    ...
    while True:
        try:
            ...
            if position_was_reset:
                 log_info("Position state was reset by order check (SL/TP likely hit). Checking for new entry signals immediately.")
            ...
            df_with_indicators: Optional[pd.DataFrame] = calculate_technical_indicators(
                ...
                calc_atr=needs_atr_calc, atr_len=atr_length,
                calc_vol_ma=needs_vol_ma_calc, vol_ma_len=entry_volume_ma_length
            )
            ...
            required_cols_for_logic: List[str] = ['close', 'high', 'low', 'open', 'volume', rsi_col_name, stoch_k_col_name, stoch_d_col_name]
            if needs_atr_calc and atr_col_name: required_cols_for_logic.append(atr_col_name)
            if needs_vol_ma_calc and vol_ma_col_name: required_cols_for_logic.append(vol_ma_col_name)

            missing_cols = [col for col in required_cols_for_logic if col not in df_with_indicators.columns]
            if missing_cols:
                log_error(f"CRITICAL: Required columns missing in DataFrame after indicator calculation: {missing_cols}. Check config/data/indicator logic. Available: {df_with_indicators.columns.tolist()}")
                neon_sleep_timer(sleep_interval_seconds)
                continue
            ...
            try:
                ...
                if needs_atr_calc and atr_col_name:
                    ...
                    if pd.notna(atr_val) and isinstance(atr_val, (float, int)) and atr_val > 0:
                        last_atr = float(atr_val)
                         ...
                         log_warning(f"ATR value ({atr_val}) is invalid (NaN, zero, or negative) for latest candle {latest_timestamp}. ATR-based logic will be skipped or may fail.")
                ...
                if needs_vol_ma_calc and vol_ma_col_name:
                     ...
                     if pd.notna(vol_ma_val) and isinstance(vol_ma_val, (float, int)) and vol_ma_val > 0:
                         ...
                         log_debug(f"Volume MA value ({vol_ma_val}) is invalid (NaN, zero, or negative) for latest candle {latest_timestamp}. Volume confirmation might be skipped.")
                ...
                if any(pd.isna(v) for v in essential_values):
                 ...
                 log_error(f"Error extracting latest data or essential value is NaN/invalid type at {latest_timestamp}: {e}. Data: {latest_data.to_dict()}", exc_info=False)
                 neon_sleep_timer(sleep_interval_seconds)
                 continue
            ...
            if position['status'] is not None:
                log_info(f"Currently in {position['status'].upper()} position entered at {position.get('entry_price', 'N/A'):.{price_precision_digits}f}. Monitoring...")
                ...
                if execute_fallback_exit:
                    ...
                    log_warning(f"Attempting to close {position['status'].upper()} position via FALLBACK MARKET ORDER due to indicator signal: {fallback_exit_reason}")
                    ...
                    if exit_qty is None or exit_qty <= 0:
                        ...
                        if not orders_cancelled_successfully:
                             log_error("Failed to confirm cancellation of one or both SL/TP orders. Aborting fallback market exit to avoid potential issues (e.g., double close). MANUAL INTERVENTION MAY BE REQUIRED.")
                             # Do not proceed with market exit if cancellations failed
                        else:
                            log_info("SL/TP orders cancelled successfully or already closed. Proceeding with fallback market exit.")
                            close_side = 'sell' if position['status'] == 'long' else 'buy'
                            # Place the market order to close the position (use reduceOnly=True)
                            fallback_exit_order_result = place_market_order(exchange, symbol, close_side, exit_qty, reduce_only=True)

                            # Check if market order placement was likely successful
                            if fallback_exit_order_result and fallback_exit_order_result.get('id'):
                                log_info(f"Fallback {position['status']} position close market order placed/processed: ID {fallback_exit_order_result.get('id', 'N/A')}")
                                ...
                                log_info("Resetting local position state immediately after fallback market exit attempt.")
                                ...
                                log_error(f"Fallback market order placement FAILED for {position['status']} position. Result: {fallback_exit_order_result}. POSITION MIGHT STILL BE OPEN. MANUAL INTERVENTION REQUIRED.")
                     ...
                     tsl_price_str = f" (Active TSL: {position['current_trailing_sl_price']:.{price_precision_digits}f})" if position.get('current_trailing_sl_price') else ""
                     log_info(f"Monitoring {position['status'].upper()} position. Waiting for SL (ID: {sl_id_str}{tsl_price_str}) / TP (ID: {tp_id_str}) or TSL update.")
            ...
            # === B. LOGIC TO CHECK FOR NEW ENTRIES (Only if not currently in a position) ===
                ...
                if entry_volume_confirmation_enabled:
                    if current_volume is not None and last_volume_ma is not None and last_volume_ma > 0:
                        ...
                        if current_volume > volume_threshold:
                            ...
                            log_debug(f"Volume confirmed: Current Vol ({current_volume:.2f}) > Threshold ({volume_threshold:.2f} = {last_volume_ma:.2f} * {entry_volume_multiplier})")
                        else:
                ...
                base_long_signal = last_rsi < rsi_oversold and last_stoch_k < stoch_oversold and last_stoch_d < stoch_oversold # Added Stoch D check
                base_short_signal = last_rsi > rsi_overbought and last_stoch_k > stoch_overbought and last_stoch_d > stoch_overbought # Added Stoch D check
                if base_long_signal: log_debug(f"Base Long Signal Met: RSI={last_rsi:.1f}, StochK={last_stoch_k:.1f}, StochD={last_stoch_d:.1f}")
                if base_short_signal: log_debug(f"Base Short Signal Met: RSI={last_rsi:.1f}, StochK={last_stoch_k:.1f}, StochD={last_stoch_d:.1f}")
                ...
                if base_long_signal and bullish_ob:
                    ...
                    if entry_zone_low <= current_price <= entry_zone_high:
                        ...
                        log_debug(f"Base Long signal met, but Price ({current_price:.{price_precision_digits}f}) outside Bullish OB zone [{entry_zone_low:.{price_precision_digits}f} - {entry_zone_high:.{price_precision_digits}f}].")
                ...
                if not ob_price_confirmed and base_short_signal and bearish_ob:
                    ...
                    if entry_zone_low <= current_price <= entry_zone_high:
                        ...
                        log_debug(f"Bearish OB Price Confirmed: {ob_reason_part}")
                         ...
                         log_debug(f"Base Short signal met, but Price ({current_price:.{price_precision_digits}f}) outside Bearish OB zone [{entry_zone_low:.{price_precision_digits}f} - {entry_zone_high:.{price_precision_digits}f}].")
                ...
                long_entry_signal = base_long_signal and ob_price_confirmed and volume_confirmed and active_ob_for_entry and active_ob_for_entry['type'] == 'bullish'
                short_entry_signal = base_short_signal and ob_price_confirmed and volume_confirmed and active_ob_for_entry and active_ob_for_entry['type'] == 'bearish'
                ...
                if long_entry_signal:
                     ...
                     entry_reason = (f"RSI ({last_rsi:.1f} < {rsi_oversold}), "
                     ...
                     side_to_enter = 'short'
                     entry_reason = (f"RSI ({last_rsi:.1f} > {rsi_overbought}), "
                                     f"Stoch ({last_stoch_k:.1f},{last_stoch_d:.1f} > {stoch_overbought}), "
                                     f"{ob_reason_part}" +
                                     (", Volume Confirmed" if entry_volume_confirmation_enabled else ""))
                # Log reasons for non-entry if signals were close
                elif base_long_signal and ob_price_confirmed and not volume_confirmed and entry_volume_confirmation_enabled:
                     log_debug("Long entry blocked: Volume not confirmed.")
                elif base_short_signal and ob_price_confirmed and not volume_confirmed and entry_volume_confirmation_enabled:
                     log_debug("Short entry blocked: Volume not confirmed.")
                elif (base_long_signal or base_short_signal) and not ob_price_confirmed:
                     log_debug(f"Entry blocked: Base signal met, but OB price condition not met (Price: {current_price:.{price_precision_digits}f}).")
                ...
                if side_to_enter and entry_reason and active_ob_for_entry:
                    ...
                    if enable_atr_sl_tp:
                        ...
                        if last_atr: # Ensure last_atr is valid (positive)
                            if side_to_enter == 'long':
                                ...
                                stop_loss_price = current_price + (last_atr * atr_sl_multiplier)
                                take_profit_price = current_price - (last_atr * atr_tp_multiplier)
                            log_info(f"Calculated ATR SL/TP: SL={stop_loss_price:.{price_precision_digits}f} ({atr_sl_multiplier}x ATR), TP={take_profit_price:.{price_precision_digits}f} ({atr_tp_multiplier}x ATR)")
                        else:
                            log_error(f"Cannot calculate ATR SL/TP for {side_to_enter.upper()} entry: Invalid ATR value ({last_atr}). Skipping entry.")
                        ...
                        if side_to_enter == 'long':
                            ...
                            stop_loss_price = current_price * (1 + stop_loss_percentage)
                            take_profit_price = current_price * (1 - take_profit_percentage)
                        log_info(f"Calculated Fixed % SL/TP: SL={stop_loss_price:.{price_precision_digits}f} ({stop_loss_percentage*100:.1f}%), TP={take_profit_price:.{price_precision_digits}f} ({take_profit_percentage*100:.1f}%)")
                    ...
                    if side_to_enter == 'long':
                        ...
                        if potential_ob_sl > stop_loss_price and potential_ob_sl < current_price:
                            ...
                            log_info(f"Adjusting SL tighter based on Bullish OB low: {adjusted_sl:.{price_precision_digits}f} (Orig: {stop_loss_price:.{price_precision_digits}f})")
                        else:
                             log_debug(f"Original {sl_tp_method} SL ({stop_loss_price:.{price_precision_digits}f}) kept. OB-based SL ({potential_ob_sl:.{price_precision_digits}f}) was not tighter or invalid.")
                    else: # short
                        ...
                        if potential_ob_sl < stop_loss_price and potential_ob_sl > current_price:
                             adjusted_sl = potential_ob_sl
                             log_info(f"Adjusting SL tighter based on Bearish OB high: {adjusted_sl:.{price_precision_digits}f} (Orig: {stop_loss_price:.{price_precision_digits}f})")
                        else:
                             log_debug(f"Original {sl_tp_method} SL ({stop_loss_price:.{price_precision_digits}f}) kept. OB-based SL ({potential_ob_sl:.{price_precision_digits}f}) was not tighter or invalid.")
                    ...
                    if adjusted_sl is not None:
                         ...
                         try:
                              ...
                              log_error(f"Failed to format OB-adjusted SL {adjusted_sl} to precision: {fmt_e}. Using original SL {stop_loss_price}.")
                              # Fallback to original calculated SL if formatting fails


                    # --- Final SL/TP Validation ---
                    if stop_loss_price is None or take_profit_price is None:
                         log_error(f"SL or TP price calculation failed unexpectedly. Skipping {side_to_enter.upper()} entry.")
                         continue
                    try:
                    ...
                    if side_to_enter == 'long' and (sl_price_final >= current_price or tp_price_final <= current_price):
                         log_error(f"Invalid LONG SL/TP after adjustments: SL {sl_price_final:.{price_precision_digits}f}, TP {tp_price_final:.{price_precision_digits}f} relative to Price {current_price:.{price_precision_digits}f}. Skipping entry.")
                         continue
                    if side_to_enter == 'short' and (sl_price_final <= current_price or tp_price_final >= current_price):
                         log_error(f"Invalid SHORT SL/TP after adjustments: SL {sl_price_final:.{price_precision_digits}f}, TP {tp_price_final:.{price_precision_digits}f} relative to Price {current_price:.{price_precision_digits}f}. Skipping entry.")
                         continue

                    # --- Calculate Position Size ---
                    # Use the final validated SL price for sizing
                    entry_quantity = calculate_position_size(exchange, symbol, current_price, sl_price_final, risk_percentage)

                    if entry_quantity is None or entry_quantity <= 0:
                        log_error(f"Failed to calculate valid position size (Result: {entry_quantity}). Skipping {side_to_enter.upper()} entry.")
                        continue # Skip entry attempt

                    # --- Place Entry Market Order ---
                    entry_order_side = 'buy' if side_to_enter == 'long' else 'sell'
                    entry_order_result = place_market_order(exchange, symbol, entry_order_side, entry_quantity, reduce_only=False) # Entry is never reduceOnly
                    ...
                    if entry_order_result and entry_order_result.get('id'):
                        ...
                        if order_status == 'closed' or (order_status == 'open' and filled_quantity_actual > 0):
                             ...
                             log_info(f"{side_to_enter.upper()} position entry order processed: ID {entry_order_id}, Status: {order_status}, AvgPrice: {entry_price_actual:.{price_precision_digits}f}, FilledQty: {filled_quantity_actual:.{amount_precision_digits}f}")
                        else:
                             # Order might be pending, rejected, or failed unexpectedly
                             log_error(f"Entry market order {entry_order_id} placed but status is '{order_status}' with filled amount {filled_qty}. Assuming entry failed or pending. State not updated.")
                    else:
                        log_error(f"Failed to place or confirm {side_to_enter.upper()} entry market order. Result: {entry_order_result}")
                    ...
                    if entry_successful:
                        ...
                        position.update({
                            ...
                            'highest_price_since_entry': entry_price_actual if side_to_enter == 'long' else entry_price_actual, # Start peak/low at entry
                            'lowest_price_since_entry': entry_price_actual if side_to_enter == 'short' else entry_price_actual,
                        ...
                        if not sl_order_id_final:
                            log_warning("Entry successful, but SL order placement failed or did not return ID. POSITION IS UNPROTECTED!")
            ...
            log_info(f"Cycle complete ({pd.Timestamp.now(tz='UTC') - cycle_start_time}). Waiting for {sleep_interval_seconds} seconds...")
            neon_sleep_timer(sleep_interval_seconds)
            ...
            if not SIMULATION_MODE:
                ...
                try:
                    ...
                    if not open_orders:
                        ...
                        log_info(f"Shutdown order cancellation attempt complete. Success/Already Closed: {cancelled_count}, Failed: {failed_count}")
                        if failed_count > 0:
                            log_error(f"{failed_count} orders could not be cancelled automatically. Please check the exchange manually: {symbol}")
        ...
        # --- Robust Error Handling for the Main Loop ---
            ...
            log_error(f"Main loop caught RateLimitExceeded: {e}. Waiting longer ({sleep_interval_seconds + 60}s)...", exc_info=False)
            neon_sleep_timer(sleep_interval_seconds + 60) # Wait longer than usual
        except ccxt.NetworkError as e:
            log_error(f"Main loop caught NetworkError: {e}. Waiting {sleep_interval_seconds}s and relying on API call retries within next cycle...", exc_info=False)
            ...
            log_error(f"Main loop caught ExchangeError: {e}. Waiting {sleep_interval_seconds}s and retrying...", exc_info=False)
...
if __name__ == "__main__":
    try:
        ...
        log_error(f"A critical error occurred outside the main loop during initialization: {main_exec_e}", exc_info=True)
        print_shutdown_message()
