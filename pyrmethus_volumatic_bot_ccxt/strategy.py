...
import logging
from decimal import Decimal, ROUND_DOWN, getcontext, ROUND_HALF_UP, InvalidOperation
...
class VolumaticOBStrategy:
    ...
    def _set_precision(self, market: Dict[str, Any]):
        ...
        try:
            ...
            amount_tick_str = (
                precision.get("amount")  # Amount tick often in precision dict
    ...
    def round_price(self, price: float) -> float:
        ...
        """
        if pd.isna(price) or self.price_tick <= 0:
            return np.nan
        try:
            price_decimal = Decimal(str(price))
    ...
    def round_amount(self, amount: float) -> float:
        ...
        """
        if pd.isna(amount) or self.amount_tick <= 0:
            return np.nan
        try:
            amount_decimal = Decimal(str(amount))
    ...
    def _find_pivots(
        ...
        if source_col not in df.columns:
            log.error(
                f"Source column '{source_col}' not found for pivot calculation based on ob_source='{self.ob_source}'."
            )
            ...
            if pd.notna(pivot_high_price):
                ...
                if not any(b["id"] == pivot_occur_int_idx for b in self.bear_boxes):
                    ...
                    if (
                        ...
                        try:
                            if (
                                Decimal(str(top_price)) - Decimal(str(bottom_price))
                                >= self.price_tick * MIN_OB_RANGE_FACTOR
                            ):
                                new_box = OrderBlock(
                                    id=pivot_occur_int_idx,
                                    type="bear",
                                ...
                                log.info(
                                    f"{Fore.RED}New Bear OB [{pivot_occur_int_idx}] @ {df.index[pivot_occur_int_idx]}: "
                            ...
                            log.warning(
                                f"Could not create Bear OB [{pivot_occur_int_idx}] due to invalid price for Decimal: T={top_price}, B={bottom_price}"
                            )
                    ...
                    if (
                        ...
                        try:
                            if (
                                ...
                                new_boxes_created_count += 1
                                log.info(
                                    f"{Fore.GREEN}New Bull OB [{pivot_occur_int_idx}] @ {df.index[pivot_occur_int_idx]}: "
                                    f"T={self.format_price(new_box['top'])}, B={self.format_price(new_box['bottom'])}{Style.RESET_ALL}"
                                )
                            # else: log.debug(f"Skipping Bull OB [{pivot_occur_int_idx}]: Range too small.")
                        except InvalidOperation:
                            log.warning(
                                f"Could not create Bull OB [{pivot_occur_int_idx}] due to invalid price for Decimal: T={top_price}, B={bottom_price}"
                            )
        ...
        for box in self.bull_boxes:
            if box["active"]:
                ...
                if current_close < box["bottom"] - FLOAT_EPSILON:
                    ...
                    closed_bull_count += 1
                    log.debug(
                        f"Closing Bull OB [{box['id']}] (B={self.format_price(box['bottom'])}) due to close {self.format_price(current_close)}"
                    )
                else:
        ...
        for box in self.bear_boxes:
            if box["active"]:
                ...
                if current_close > box["top"] + FLOAT_EPSILON:
                    ...
                    closed_bear_count += 1
                    log.debug(
                        f"Closing Bear OB [{box['id']}] (T={self.format_price(box['top'])}) due to close {self.format_price(current_close)}"
                    )
                else:
                    # Keep extending the validity (right_idx) if still active
                    box["right_idx"] = current_bar_int_idx

        if closed_bull_count:
            log.info(
                f"{Fore.YELLOW}Closed {closed_bull_count} Bull OB(s) @ {df.index[-1]} due to price ({self.format_price(current_close)}) breaking below.{Style.RESET_ALL}"
            )
        if closed_bear_count:
            log.info(
                f"{Fore.YELLOW}Closed {closed_bear_count} Bear OB(s) @ {df.index[-1]} due to price ({self.format_price(current_close)}) breaking above.{Style.RESET_ALL}"
            )
    ...
    def _generate_signal(self, df: pd.DataFrame) -> str:
        ...
        if df.empty:
            return signal  # Cannot generate signal on empty data
        ...
        if signal == "HOLD":
            log.debug(
                f"Signal: HOLD (Internal State: {self.last_signal_state}, Trend: {trend_str}, Close: {self.format_price(current_close)})"
            )
    ...
    def update(self, df_input: pd.DataFrame) -> AnalysisResults:
        ...
        if len(df_input) < self.min_data_len:
            log.warning(
                f"Not enough data ({len(df_input)}/{self.min_data_len}) for analysis. Need more historical bars."
            )
