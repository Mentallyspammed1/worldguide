...
try:
    ...
    log.critical(
        "CRITICAL: Failed to import VolumaticOBStrategy from strategy.py. Ensure the file exists and is correct."
    )
    sys.exit(1)

    # Define dummy class and type hint to prevent further NameErrors
    # if import fails but script continues somehow (shouldn't happen due to sys.exit)
    class VolumaticOBStrategy:  # type: ignore
        ...
        def round_amount(self, amount: Union[float, Decimal]) -> float:
            return float(amount)

        def round_price(self, price: Union[float, Decimal]) -> float:
            return float(price)

        def format_amount(self, amount: Union[float, Decimal]) -> str:
            return str(amount)

        def format_price(self, price: Union[float, Decimal]) -> str:
            return str(price)

        price_tick: Decimal = Decimal("0.01")  # Dummy value
        min_data_len: int = 50  # Dummy value
...
def load_config(path_str: str = "config.json") -> Dict[str, Any]:
    ...
    try:
        with config_path.open("r", encoding="utf-8") as f:  # Specify encoding
            ...
            if missing_keys:
                raise ValueError(
                    f"Config file '{config_path}' missing required top-level keys: {', '.join(missing_keys)}"
                )
            ...
            try:
                ...
                if not (risk_percent > 0):
                    raise ValueError("'order.risk_per_trade_percent' must be positive.")
                if not (tp_ratio > 0):
                    raise ValueError("'order.tp_ratio' must be positive.")
...
async def connect_ccxt() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        exchange_options = {
            ...
            "recvWindow": config.get("ccxt_options", {}).get(
                "recvWindow", 10000
            ),  # Increase recvWindow if timestamp errors occur (default 5000)
        ...
        if TESTNET:
            if hasattr(exchange, "set_sandbox_mode"):
                ...
                try:
                    ...
                    log.error(
                        f"Failed to enable sandbox mode via set_sandbox_mode: {e}. Ensure TESTNET API keys are used if needed."
                    )
            else:
                log.warning(
                    f"Exchange '{exchange_id}' may not support set_sandbox_mode via CCXT. Ensure TESTNET API keys/URLs are used if applicable."
                )
        ...
        log.info(
            f"{Fore.GREEN}Successfully connected to {exchange.name}. Loaded {len(exchange.markets)} markets.{Style.RESET_ALL}"
        )
        ...
        if config["order"]["type"].lower() == ORDER_TYPE_LIMIT:
            ...
            required_methods.extend(["fetchTicker"])  # Or fetchOrderBook depending on strategy
        ...
        if missing_methods:
            log.warning(
                f"Exchange {exchange.name} might be missing required capabilities for this bot's configuration: {', '.join(missing_methods)}"
            )
            # Decide if this is critical (e.g., missing fetchOHLCV or createOrder)
            if (
                "fetchOHLCV" in missing_methods
                or "createOrder" in missing_methods
                or "fetchPositions" in missing_methods
            ):
                log.critical(f"CRITICAL: Exchange is missing essential methods: {missing_methods}. Cannot continue.")
        ...
        log.critical(
            f"CRITICAL: CCXT Authentication Error: {e}. Check API keys, permissions, and ensure IP whitelist (if used) is correct."
        )
        return None
    except ccxt.NetworkError as e:
        log.critical(
            f"CRITICAL: CCXT Network Error connecting to exchange: {e}. Check internet connection and firewall."
        )
        return None
    except Exception as e:
        ...
        if exchange and hasattr(exchange, "close"):
            try:
                ...
                pass  # Ignore errors during cleanup on critical failure
        exchange = None
        return None
...
async def get_current_position(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    async with position_lock:  # Lock to prevent concurrent updates from REST/WS
        ...
        try:
            ...
            # Pass symbol in list for filtering potential
            ...
            if pos_data:
                ...
                try:
                    ...
                    log.error(
                        f"Error converting position data to Decimal: size='{size_str}', entry='{entry_price_str}'. Assuming flat."
                    )
                    size = Decimal(0)
                ...
                if is_flat:
                    ...
                    log.warning(
                        f"Position has size {size} but side is '{side}'. Interpreting as '{SIDE_NONE}'. Exchange data: {pos_data.get('info')}"
                    )
                    current_side_str = SIDE_NONE
                ...
                if (
                    new_position_state["size"] != current_position["size"]
                    or new_position_state["side"] != current_position["side"]
                ):
                    log.info(
                        f"Updated Position State: Side={new_position_state['side']}, Size={new_position_state['size']}, Entry={new_position_state['entry_price']}"
                    )
                    ...
                    log.debug(
                        f"Position state confirmed: Side={current_position['side']}, Size={current_position['size']}"
                    )

            else:  # No position found for the symbol
                if current_position["size"] != Decimal(0):
                    log.info(
                        f"Position for {symbol} now reported as flat (previously {current_position['side']} {current_position['size']})."
                    )
    ...
    try:
        ...
        if quote_currency in balance_data.get("total", {}):
            try:
                ...
                log.warning(
                    f"Could not parse CCXT total balance for {quote_currency}: {balance_data['total'].get(quote_currency)}"
                )
        if quote_currency in balance_data.get("free", {}):
            try:
                ...
                log.warning(
                    f"Could not parse CCXT free balance for {quote_currency}: {balance_data['free'].get(quote_currency)}"
                )
                ...
                for account in result["list"]:
                    ...
                    # Contract accounts might not have 'coin' per item, check common equity keys
                        ...
                        if any(key in account for key in equity_keys_check):
                            ...
                            account_info = account
                            # Don't break here, might find a coin-specific one later if multiple items exist
                            # break # Re-enabled break: Assume first relevant dict is the main one for contract

                if isinstance(account_info, dict):
                    # Check multiple possible keys for equity/balance in Bybit V5 response
                    equity_keys = ["totalEquity", "equity", "walletBalance"]  # Order preference?
        ...
        # As last resort, use free balance, but warn
            ...
            log.warning(
                f"Total equity not found or zero, using 'free' balance: {free_balance} {quote_currency}. This might underestimate risk capacity if margin is used."
            )
            return free_balance
        else:
...
async def calculate_order_qty(entry_price: float, sl_price: float, risk_percent: float) -> Optional[float]:
    ...
    if abs(sl_decimal - entry_decimal) < min_sl_distance:
        log.error(
            f"Stop loss price {sl_decimal} is too close to entry price {entry_decimal} (Min Distance: {min_sl_distance}). Cannot calculate quantity."
        )
        return None
    ...
    log.debug(
        f"Calculating Qty: Balance={balance:.4f} {quote_currency}, Risk={risk_percent_decimal}%, Entry={entry_decimal}, SL={sl_decimal}"
    )
    ...
    if qty_final <= 0:
        log.error(
            f"Calculated order quantity is zero or negative ({qty_final}) after rounding. Min Qty required: {min_qty}. Aborting trade."
        )
        return None
    ...
    log.info(
        f"Calculated Order Qty: {formatted_qty} {market.get('base', '')} "
        f"(Balance={balance:.2f}, Risk={risk_percent}%, TargetRiskAmt={risk_amount:.2f}, FinalQty={qty_final})"
    )  # Log exact float qty
...
def safe_format(method_name: str, value: Any, default_str: str = "N/A") -> str:
    ...
    if (
        strategy_instance
        and hasattr(strategy_instance, method_name)
...
async def place_order(
    ...
    if order_side_lower not in [ORDER_SIDE_BUY, ORDER_SIDE_SELL]:
        log.error(
            f"Invalid order side '{side}' provided to place_order. Must be '{ORDER_SIDE_BUY}' or '{ORDER_SIDE_SELL}'."
        )
        return None
    ...
    if mode == MODE_PAPER:
        ...
        log.warning(
            f"{Fore.YELLOW}[PAPER MODE] Simulating {order_side_lower.upper()} {order_type.upper()} order: "
        ...
        if not simulated_fill_price:
            try:
                if exchange.has.get("fetchTicker"):
                    ...
                    log.warning(
                        "[PAPER MODE] Cannot fetch ticker for simulated fill price (not supported). Using limit price or 0."
                    )
        ...
        log.info(
            f"[PAPER MODE] Assuming order filled at ~{safe_format('format_price', simulated_fill_price)}. Update paper position state accordingly."
        )  # Placeholder
    ...
    async with order_lock:  # Prevent concurrent order placements
            ...
            if current_price_for_cost_check:
                try:
                    ...
                    if order_cost < min_cost:
                        log.error(
                            f"Estimated order cost {order_cost:.4f} (Qty:{amount_rounded} * Price:{current_price_for_cost_check}) is below minimum {min_cost}. Aborting order."
                        )
                        return None
        ...
        if sl_price:
            ...
            if not check_price and exchange.has.get("fetchTicker"):
                try:
                    ...
                    pass  # Ignore errors, validation might be less strict
        ...
        # Log the attempt
        log.warning(
            f"{Fore.CYAN}Attempting to place {order_side_lower.upper()} {order_type.upper()} order:{Style.RESET_ALL}\n"
            f"  Symbol: {symbol}\n"
        ...
        try:
            ...
            log.error(
                f"{Fore.RED}Order Failed: Invalid Order Parameters.{Style.RESET_ALL} Check config, calculations, market limits, and SL/TP validity. {e}"
            )
            log.error(
                f"Order details attempted: symbol={symbol}, type={order_type}, side={order_side_lower}, amount={amount_rounded}, price={limit_price_rounded}, params={ccxt_params}"
            )
            ...
            elif "size" in error_str and "too small" in error_str:
    ...
    if mode == MODE_PAPER:
        if current_size > Decimal(0):
            ...
            return {
                ...
                "filled": float(current_size),
                "remaining": 0.0,
                "price": None,
                "average": None,  # Cannot easily simulate fill price here
                "status": "closed",
                "fee": None,
                "cost": None,
    ...
    async with order_lock:  # Ensure only one closing order attempt at a time
        ...
        try:
            ...
            if exchange.has.get("cancelAllOrders"):
                ...
                log.warning(
                    "Exchange does not support efficient cancellation of all/multiple orders via CCXT. Manual cancellation of SL/TP might be needed if they weren't attached to the entry order."
                )
        except (ccxt.NetworkError, ccxt.ExchangeError) as e:
        ...
        params = {
            ...
            **(
                {"positionIdx": int(config["order"]["positionIdx"])}
                if config["order"].get("positionIdx") is not None
                else {}
            ),
        ...
        try:
            ...
            log.warning(
                f"{Fore.YELLOW}Close order failed, likely position already closed or size mismatch: {e}{Style.RESET_ALL}"
            )
            ...
            log.error(
                f"{Fore.RED}Close order failed: Insufficient Funds (unexpected for reduceOnly). {e}{Style.RESET_ALL}"
            )
            async with position_lock:  # Use inner lock only to modify check time
                last_position_check_time = 0
            return None
...
async def set_leverage(symbol: str, leverage: Union[int, float, str]):
    ...
    if not exchange.has.get("setLeverage"):
        log.warning(
            f"Exchange {exchange.name} does not support setting leverage via CCXT standardized method 'setLeverage'. Manual setting might be required, or check for exchange-specific methods/params."
        )
        return
    ...
    if max_leverage is not None and leverage_val_final > max_leverage:
        log.error(
            f"Requested leverage {leverage_val_final} exceeds market maximum ({max_leverage}) for {symbol}. Leverage not set."
        )
        return
    ...
    try:
        ...
        if "not modified" in error_str or "110044" in str(e):
            log.warning(
                f"Leverage for {symbol} likely already set to {leverage_val_final}x (Exchange response: Not modified)."
            )
        elif "set leverage not supported" in error_str:
            log.warning(
                f"Exchange reports 'setLeverage' is not supported for {symbol} (or current account/market settings). {e}"
            )
        else:
...
async def watch_kline_loop(symbol: str, timeframe: str):
    ...
    if not exchange.has.get("watchOHLCV"):
        log.error(
            f"{Fore.RED}Kline watcher cannot start: Exchange '{exchange.name}' reports 'watchOHLCV' is not supported via CCXT for the current configuration.{Style.RESET_ALL}"
        )
        log.error(
            "The bot relies on WebSocket klines. Consider implementing polling with fetch_ohlcv as an alternative if WS is unavailable, or ensure your exchange/account supports WS klines."
        )
    ...
    while not stop_event.is_set():
        try:
            ...
            if stop_event.is_set():
                break  # Check immediately after await returns
            ...
            now_mono = time.monotonic()
            log.debug(
                f"Kline WS received {len(candles)} candle(s) (Last update: {now_mono - last_ws_update_time:.1f}s ago)"
            )
...
async def process_candle(timestamp: pd.Timestamp, data: Dict[str, float]):
    """Adds candle data to the DataFrame and triggers strategy analysis. Uses data_lock."""
    ...
    async with data_lock:  # Ensure exclusive access to the dataframe
        ...
        # and prevent strategy from modifying the main df unintentionally
        df_copy = latest_dataframe.copy()
        # End analysis block
        # Release lock before potentially long-running analysis?
        # No, keep lock during analysis to ensure consistency if analysis reads other global state?
        # For now, keep lock during analysis. If analysis is very slow, reconsider.

    # --- Run analysis outside the data_lock ---
    # This allows new candles to be added while analysis runs, but analysis uses a snapshot.
    try:
        ...
        analysis_results: Optional[AnalysisResults] = strategy_instance.update(df_copy)
        ...
        if "'numpy.ndarray' object has no attribute 'fillna'" in str(e):
            log.error(
                f"{Fore.RED}CRITICAL ERROR during strategy analysis: {e}. "
                f"This likely means an indicator calculation in strategy.py returned a NumPy array "
                f"and '.fillna()' was called on it directly. FIX REQUIRED IN strategy.py.{Style.RESET_ALL}",
                exc_info=True,
            )
            log.error(
        ...
        try:
            ...
            if pos_update_for_symbol:
                ...
                async with position_lock:
                    if current_position["size"] != Decimal(0):
                        log.warning(
                            f"{Fore.MAGENTA}WS position update no longer includes {symbol} or shows flat. Forcing REST check.{Style.RESET_ALL}"
                        )
                        last_position_check_time = 0

        except ccxt.NetworkError as e:
            log.warning(f"Position Watcher Network Error: {e}. Reconnecting...")
            await asyncio.sleep(5)
        except ccxt.ExchangeError as e:
            log.warning(f"Position Watcher Exchange Error: {e}. Retrying...")
            await asyncio.sleep(15)
        except asyncio.CancelledError:
            log.info("Position watcher task cancelled.")
            break
        except Exception as e:
            log.error(f"Unexpected error in Position watcher: {e}", exc_info=True)
    ...
    while not stop_event.is_set():
        try:
            ...
            for order_update in orders:
                ...
                avg_price_str = (
                    safe_format("format_price", avg_price_val, str(avg_price_val))
                ...
                log.info(
                    f"{Fore.CYAN}Order Update via WS [{symbol}]: ID={order_id}, ClientID={client_order_id}, Side={order_side}, Type={order_type}, Status={status}, Filled={filled_str}, AvgPrice={avg_price_str}, ReduceOnly={reduce_only}{Style.RESET_ALL}"
                )
                ...
                if status in ["closed", "canceled", "rejected", "expired"] and order_id:
                    log.warning(
                        f"{Fore.YELLOW}Order {order_id} ({order_side} {order_type}) reached terminal state '{status}' via WS.{Style.RESET_ALL}"
                    )
            ...
            log.warning(f"Order Watcher Network Error: {e}. Reconnecting...")
            await asyncio.sleep(5)
        except ccxt.ExchangeError as e:
            log.warning(f"Order Watcher Exchange Error: {e}. Retrying...")
            await asyncio.sleep(15)
        except asyncio.CancelledError:
            log.info("Order watcher task cancelled.")
            break
        except Exception as e:
            log.error(f"Unexpected error in Order watcher: {e}", exc_info=True)
            await asyncio.sleep(30)
...
async def process_signals(results: AnalysisResults):
    ...
    log.info(
        f"Processing signal '{signal}' | Current Position: {'Long' if is_long else 'Short' if is_short else 'Flat'} (Size: {current_pos_size})"
    )
    ...
    def calculate_sl_tp(
        is_buy_signal: bool, entry_price: float, atr: Optional[float]
        ...
        if sl_method == "ATR":
            if atr and atr > 0 and np.isfinite(atr):
                try:
                    ...
                    log.error(
                        f"TypeError during ATR SL calculation: {e}. Inputs: entry={entry_price}, atr={atr}, mult={sl_atr_multiplier}"
                    )
                    return None, None
            else:
            ...
            try:
                ...
                if is_buy_signal:
                    ...
                    relevant_obs = [
                        Decimal(str(b["bottom"]))
                        for b in active_bull_boxes
                        if isinstance(b, dict)
                        and "bottom" in b
                        and isinstance(b["bottom"], (float, int))
                        and np.isfinite(b["bottom"])
                    ...
                    if relevant_obs:
                        ...
                        log.debug(
                            f"Calculated OB SL (Buy): Max Bull Bottom={ob_sl_level}, Buffer={sl_buffer}, SL={sl_price_raw}"
                        )
                    else:
                        log.warning(
                            "OB SL method selected (Buy), but no valid Bull OB found below price. Cannot calculate SL."
                        )
                        return None, None
                else:  # Sell signal
                    relevant_obs = [
                        ...
                        if isinstance(b, dict)
                        and "top" in b
                        and isinstance(b["top"], (float, int))
                        and np.isfinite(b["top"])
                    ...
                    if relevant_obs:
                        ...
                        sl_price_raw = float(sl_price_raw_dec)
                        log.debug(
                            f"Calculated OB SL (Sell): Min Bear Top={ob_sl_level}, Buffer={sl_buffer}, SL={sl_price_raw}"
                        )
                    else:
                        log.warning(
                            "OB SL method selected (Sell), but no valid Bear OB found above price. Cannot calculate SL."
                        )
                        return None, None
        ...
        if sl_price_raw is None or not np.isfinite(sl_price_raw):
            log.error(f"Invalid raw SL price calculated: {sl_price_raw}. Aborting entry.")
            return None, None
        ...
        if sl_price_final is None or not np.isfinite(sl_price_final):
            log.error(f"Invalid final SL price after rounding: {sl_price_final}. Aborting entry.")
            return None, None
        ...
        try:
            ...
            if sl_distance > price_tick_decimal / Decimal(2):
                ...
                if tp_price_final is None or not np.isfinite(tp_price_final):
                    log.warning(f"Invalid final TP price after rounding: {tp_price_final}. Setting TP to None.")
                    tp_price_final = None
                elif (is_buy_signal and tp_price_final <= entry_price) or (
                    not is_buy_signal and tp_price_final >= entry_price
                ):
                    log.warning(
                        f"Calculated TP price {tp_price_final} is not beyond entry price {entry_price}. Setting TP to None."
                    )
                    tp_price_final = None
                else:
                    log.info(
    ...
    # --- No Action / Already in State / Hold Signal ---
        ...
        log.debug(
            f"Signal '{signal}' received but no matching action criteria met (Position: {current_pos_side} {current_pos_size})."
        )
...
async def periodic_check_loop():
    ...
    try:
        ...
        log.error(
            f"Invalid interval configuration in 'checks': {e}. Using defaults (60s health, 30s pos, 90s WS timeout)."
        )
        check_interval = 60
    ...
    log.info(
        f"Starting Periodic Check loop (Health Interval: {check_interval}s, Position Interval: {pos_check_interval}s, WS Timeout: {ws_timeout}s)"
    )
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            while time.monotonic() < next_check_time:
                if stop_event.is_set():
                    break
                await asyncio.sleep(1)  # Check stop event periodically during sleep
            if stop_event.is_set():
            ...
            if last_ws_update_time > 0:  # Ensure WS has sent at least one message
                ...
                if time_since_last_ws >= ws_timeout:
                    log.warning(
                        f"{Fore.RED}WebSocket potentially stale! No updates received for {time_since_last_ws:.1f}s (Timeout: {ws_timeout}s).{Style.RESET_ALL}"
                    )
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                try:
                    ...
                    pass  # Ignore if name cannot be retrieved
                ...
                if isinstance(result, asyncio.CancelledError):
                    ...
                    log.error(
                        f"Task '{task_name}' raised an exception during shutdown: {result}",
                        exc_info=result if log_exc_info else False,
                    )
...
async def close_open_position_on_exit():
    ...
    try:
        ...
        if pos_data and pos_data.get("size", Decimal(0)) > Decimal(0):
            log.warning(
                f"{Fore.YELLOW}Found open {pos_data['side']} position (Size: {pos_data['size']}). Attempting to close...{Style.RESET_ALL}"
            )
...
async def main():
    ...
    print(
        Fore.MAGENTA
        + Style.BRIGHT
        + "\n~~~ Pyrmethus Volumatic+OB Trading Bot Starting (CCXT Async - Enhanced) ~~~"
        + Style.RESET_ALL
    )
    ...
    try:
        leverage_to_set = config["order"]["leverage"]  # Can be int or float string
    ...
    try:
        ...
        log.critical(
            f"Failed to initialize strategy: {e}. Check config.json['strategy']['params'] and strategy.py.",
            exc_info=True,
        )
        if exchange and hasattr(exchange, "close"):
            await exchange.close()
    ...
    async with data_lock:  # Lock dataframe during initial population
        ...
        if latest_dataframe is None or latest_dataframe.empty:  # Check for None or empty DF
            ...
            log.warning(
                f"Initial data fetched ({len(latest_dataframe)}) is less than minimum required by strategy ({min_data_len_strat}). Strategy results may be unreliable initially."
            )
        ...
        try:
            ...
            if initial_results is None or not isinstance(initial_results, dict):
                ...
                price_prec = (
                    price_prec_attr if isinstance(price_prec_attr, int) else market.get("precision", {}).get("price", 2)
                )
                last_close_val = initial_results.get("last_close")
                close_str = (
                    f"{last_close_val:.{price_prec}f}"
                    if last_close_val is not None and price_prec is not None
                    else "N/A"
                )

                log.info(
                    f"Initial Analysis Complete: Trend={trend_str}, "
            ...
            if "'numpy.ndarray' object has no attribute 'fillna'" in str(e):
                log.error(
                    f"{Fore.RED}CRITICAL ERROR during initial strategy analysis: {e}. FIX REQUIRED IN strategy.py.{Style.RESET_ALL}",
                    exc_info=True,
                )
    ...
    log.info(
        f"Initial Position State: {current_position['side']} (Size: {current_position['size']}, Entry: {current_position['entry_price']})"
    )
    ...
    # Optional watchers based on config/needs
    ...
    while not stop_event.is_set():
        ...
        if kline_task.done():
            log.critical(
                f"{Fore.RED}{Style.BRIGHT}CRITICAL: Kline watcher task has terminated unexpectedly!{Style.RESET_ALL}"
            )
            try:
            ...
            if not stop_event.is_set():
                asyncio.create_task(shutdown(signal_type=None), name="CriticalShutdown")  # Give it a name
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not stop_event.is_set() and (shutdown_task_ref is None or shutdown_task_ref.done()):
            ...
            try:
                ...
                if current_loop.is_running():
                    # Create and run the shutdown task until completion
                    shutdown_task_ref = current_loop.create_task(
                        shutdown(signal_type=None), name="ShutdownHandler_MainException"
                    )
                    try:
                        loop.run_until_complete(shutdown_task_ref)
                    except RuntimeError as e:
        ...
        try:
            current_loop = asyncio.get_event_loop_policy().get_event_loop()  # Get loop safely
            if current_loop and not current_loop.is_closed():
