...
try:
    ...
    class VolumaticOBStrategy: # type: ignore
        ...
        def update(self, df: pd.DataFrame) -> Optional['AnalysisResults']: return None
        def round_amount(self, amount: Union[float, Decimal]) -> float: return float(amount)
        def round_price(self, price: Union[float, Decimal]) -> float: return float(price)
        def format_amount(self, amount: Union[float, Decimal]) -> str: return str(amount)
        def format_price(self, price: Union[float, Decimal]) -> str: return str(price)
...
async def connect_ccxt() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        required_methods = ['fetchOHLCV', 'fetchPositions', 'fetchBalance', 'createOrder', 'cancelOrder'] # Base requirements
        ...
        if missing_methods:
             ...
             if 'fetchOHLCV' in missing_methods or 'createOrder' in missing_methods or 'fetchPositions' in missing_methods:
...
async def calculate_order_qty(entry_price: float, sl_price: float, risk_percent: float) -> Optional[float]:
    ...
    log.info(f"Calculated Order Qty: {formatted_qty} {market.get('base', '')} "
             f"(Balance={balance:.2f}, Risk={risk_percent}%, TargetRiskAmt={risk_amount:.2f}, FinalQty={qty_final})") # Log exact float qty
    ...
    async with data_lock: # Lock dataframe during initial population
        ...
        try:
            ...
            if initial_results is None or not isinstance(initial_results, dict):
                 ...
                 price_prec = price_prec_attr if isinstance(price_prec_attr, int) else market.get('precision', {}).get('price', 2)
