...
try:
    ...
    class VolumaticOBStrategy:  # type: ignore
        ...
        def round_amount(self, amount: Union[float, Decimal]) -> float:
            return float(f"{Decimal(str(amount)):.{self.amount_precision}f}")

        def round_price(self, price: Union[float, Decimal]) -> float:
            return float(f"{Decimal(str(price)):.{self.price_precision}f}")
...
async def connect_ccxt() -> Optional[ccxt.Exchange]:
    ...
    try:
        ...
        if missing_critical:
            log.critical(
                f"CRITICAL: Exchange is missing essential methods: {', '.join(missing_critical)}. Cannot continue."
            )
...
async def load_exchange_market(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if symbol in exchange.markets:
            ...
            log.error(
                f"Available symbols sample ({sample_size}/{len(available_symbols)}): {available_symbols[:sample_size]}..."
            )
            return None
            ...
            if (
                ...
                if isinstance(account_info, dict):
                    ...
                    equity_keys = [
                        ...
                        "equity",
                        "walletBalance",
                    ]  # Order preference? totalEquity > equity > walletBalance?
...
async def calculate_order_qty(
    ...
    log.info(
        ...
        f"(Balance={balance:.2f}, Risk={risk_percent}%, TargetRiskAmt={risk_amount:.4f}, FinalQty={qty_final})"
    )  # Log exact float qty
        ...
        try:
            ...
            if initial_results is None or not isinstance(initial_results, dict):
                ...
                price_prec = (
                    ...
                    else int(market.get("precision", {}).get("price", 2))
                )
