...
# ██║        ██║   ██║  ██║██║ ╚═╝ ██║███████║╚██████╗██║  ██║███████╗██║
# ╚═╝        ╚═╝   ╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝
# Pyrmethus - Termux Trading Spell (v2.3.1 - Enhanced Precision, V5 API & Robustness)
...
try:
    ...
    if os.getenv("TERMUX_VERSION"):
         # Provide more robust Termux instructions
         print(f"{Fore.YELLOW}First, ensure core dependencies are installed via pkg:")
         print(f"{Style.BRIGHT}pkg update && pkg upgrade")
         print(f"{Style.BRIGHT}pkg install python clang libjpeg-turbo libcrypt libffi openssl fftw make") # Common build dependencies
         print(f"{Style.BRIGHT}LDFLAGS=\"-L/data/data/com.termux/files/usr/lib\" CFLAGS=\"-I/data/data/com.termux/files/usr/include\" pip install --no-binary :all: {missing_pkg}{Style.RESET_ALL}")
         print(f"{Fore.YELLOW}If the above pip install fails, try installing pre-built packages first (if available):")
         print(f"{Style.BRIGHT}pkg install python-pandas python-numpy") # Often needed via pkg
         print(f"{Style.BRIGHT}Then install the specific missing package: pip install {missing_pkg}{Style.RESET_ALL}")
    else:
    ...
    if os.getenv("TERMUX_VERSION"):
         # Advise separate steps more strongly
         print(f"{Style.BRIGHT}pkg install python clang libjpeg-turbo libcrypt libffi openssl fftw make")
         print(f"{Style.BRIGHT}pkg install python-pandas python-numpy")
         print(f"{Style.BRIGHT}LDFLAGS=\"-L/data/data/com.termux/files/usr/lib\" CFLAGS=\"-I/data/data/com.termux/files/usr/include\" pip install --no-binary :all: {' '.join([p for p in COMMON_PACKAGES if p not in ['pandas', 'numpy']])}{Style.RESET_ALL}")
         print(f"{Fore.YELLOW}Note: Installing pandas/numpy/scipy etc. via pip in Termux can be complex. Using 'pkg install' is highly recommended first.")
    else:
