...
def load_config(filepath: str) -> Dict[str, Any]:
    ...
    default_config = {
        ...
        "trailing_stop_callback_rate": 0.005, # Trail distance % from *activation price* (e.g., 0.005 = 0.5%) - Note: Bybit V5 uses price *distance*
    ...
    for key in bool_params:
         ...
         if key == 'indicators' and isinstance(current_config.get('indicators'), dict):
             for ind_key, ind_val in current_config['indicators'].items():
                 if not isinstance(ind_val, bool):
                      print(f"{NEON_RED}Invalid value for 'indicators.{ind_key}' ({ind_val}). Must be boolean. Resetting to default.{RESET}")
                      current_config['indicators'][ind_key] = default_config['indicators'].get(ind_key, False) # Default to False if key somehow missing in default
                      needs_saving = True
...
def _merge_configs(loaded_config: Dict, default_config: Dict) -> Dict:
    ...
    for key, value in loaded_config.items():
        if key in merged:
            ...
            # Otherwise, overwrite default with loaded value (if same type or default allows override)
            elif isinstance(value, type(merged[key])) or merged[key] is None:
                merged[key] = value
            else:
                # Type mismatch, could warn here, but validation step handles it later
                # For now, just keep the default value if types mismatch severely
                # Or prioritize loaded if that's desired (but less safe)
                merged[key] = value # Prioritize loaded value for flexibility, validation will correct later
        else:
    ...
    try:
        exchange_options = {
            ...
            'options': {
                ...
                'user-agent': 'sxsBot/1.0 (+https://github.com/your_repo)', # Optional: Identify your bot
        ...
        if config.get('use_sandbox'):
            ...
            try:
                ...
                if 'testnet' not in exchange.urls['api']:
                    ...
                    exchange.urls['api'] = exchange.urls['test'] # Use the known testnet URL
                    if 'testnet' in exchange.urls['api']:
                         lg.info(f"Manually set API URL to Testnet: {exchange.urls['api']}")
                    else:
                         lg.error("Failed to manually set API URL to Testnet.")
                         # Could hardcode the Bybit testnet URL here as a last resort
            ...
            if 'testnet' in exchange.urls['api']:
                ...
                # Find the production URL (might be under 'www' or 'api' key in describe)
                prod_url = exchange.describe().get('urls', {}).get('api', {}).get('public') \
                        or exchange.describe().get('urls', {}).get('www')
                if prod_url and isinstance(prod_url, str):
                     exchange.urls['api'] = prod_url
                     lg.info(f"Reset API URL to Production: {exchange.urls['api']}")
                else: lg.error("Could not determine production API URL automatically.")
        ...
        try:
             safe_api_call(exchange.load_markets, lg, reload=True) # Force reload with retry
             lg.info(f"Markets loaded successfully for {exchange.id}.")
             # Quick check if target symbol exists after loading
             target_symbol = config.get("symbol")
             if target_symbol and target_symbol not in exchange.markets:
                  lg.warning(f"{NEON_YELLOW}Target symbol '{target_symbol}' not found in loaded markets! Check symbol format/availability.{RESET}")
                  # Optional: List available markets if few, or suggest checking exchange
                  if len(exchange.symbols) < 50:
                       lg.debug(f"Available symbols: {exchange.symbols}")
                  else:
                       lg.warning(f"{NEON_YELLOW}Check the symbol '{target_symbol}' on the Bybit website/API docs.{RESET}")

        except Exception as market_err:
             lg.critical(f"{NEON_RED}CRITICAL: Failed to load markets after retries: {market_err}. Cannot operate without market data. Exiting.{RESET}")
...
def safe_api_call(func, logger: logging.Logger, *args, **kwargs):
    ...
    lg = logger
    attempts = 0
    max_retries = config.get("max_api_retries", MAX_API_RETRIES) # Use config value if available
    ...
    while attempts <= max_retries:
        try:
            ...
            retryable_messages = [
                 ...
                 "service unavailable",
                 "system busy",
                 "request validation failed", # Sometimes transient
                 "matching engine busy",
            ]

            exchange_code = None
            err_str = str(e).lower()
            # Try extracting Bybit's retCode from the message if possible
            try:
            ...
            if is_retryable_bybit_code or is_retryable_message:
                 ...
                 attempts += 1
                 continue # Skip incrementing attempt at the end and retry
            else:
        ...
        # Sleep only if a retry is needed (moved inside specific except blocks for clarity)
...
def fetch_current_price_ccxt(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    try:
        ...
        if price is not None and price.is_finite() and price > 0:
            ...
            lg.info(f"Current price ({symbol}): {price} (Source: determined by priority)")
            return price
        else:
...
class TradingAnalyzer:
    ...
    def _get_ta_col_name(self, base_name: str, result_df: pd.DataFrame) -> Optional[str]:
        ...
        expected_patterns = {
            ...
            "PSAR_long": [f"PSARl_{self.config.get('psar_step', DEFAULT_PSAR_STEP)}_{self.config.get('psar_max_step', DEFAULT_PSAR_MAX_STEP)}"],
            "PSAR_short": [f"PSARs_{self.config.get('psar_step', DEFAULT_PSAR_STEP)}_{self.config.get('psar_max_step', DEFAULT_PSAR_MAX_STEP)}"],
    ...
    def _calculate_all_indicators(self):
        ...
        def add_req_if_active(indicator_key, config_period_key, default_period):
            if indicators_config.get(indicator_key, False) and float(active_weights.get(indicator_key, 0.0)) > 0:
        ...
        add_req_if_active("atr", "atr_period", DEFAULT_ATR_PERIOD) # ATR always useful, calculate regardless of weight? Maybe not.
        ...
        # Fibonacci requires its own period
        ...
        try:
                     ...
                     # If column exists but is all NaN, store type as None and don't convert
                          ...
                          if any(t == Decimal for t in original_types.values()):
                               try:
                                    ...
                                    original_types[col] = type(df_calc[col].iloc[0]) # Store the actual type (likely float/object)
            ...
            # Always calculate ATR if possible (often needed for SL/TP/BE)
            if "atr" in ta_map:
            ...
            for key, enabled in indicators_config.items():
                 if key == "atr": continue # Already added
            ...
            if indicators_config.get(vol_key, False) and float(active_weights.get(vol_key, 0.0)) > 0:
                 try:
            ...
            for internal_name, ta_key in indicator_mapping.items():
                 # Only try to map if the indicator was supposed to be calculated
                 if indicators_config.get(ta_key) or (ta_key == "atr"): # ATR always attempted
    ...
    def _update_latest_indicator_values(self):
        ...
        default_values = {k: Decimal('NaN') if k in ["Open", "High", "Low", "Close", "Volume", "ATR"] else np.nan for k in expected_keys}
        ...
        for key, col_name in self.ta_column_names.items():
            ...
            if col_name and col_name in latest.index:
                ...
                if is_valid and isinstance(raw_value, (float, int, Decimal)):
                    is_valid = np.isfinite(float(raw_value)) # Check finiteness
                ...
                if is_valid:
                    try:
                        ...
                        value = np.nan if key != "ATR" else Decimal('NaN') # Use appropriate NaN type
                # else: value remains np.nan or Decimal('NaN')
            else:
                 # Log only if calculation was attempted (key exists in ta_column_names but col_name is None or missing)
        ...
        for k, v in self.indicator_values.items():
            ...
            if isinstance(v, Decimal):
                 if v.is_finite():
                      prec = price_prec if k in ['Open','High','Low','Close','ATR','BB_Lower','BB_Middle','BB_Upper','PSAR_long','PSAR_short','VWAP','SMA_10','EMA_Short','EMA_Long'] else amount_prec if k=='Volume' else 8
            ...
            if min_amount_val is not None:
                try:
                    ...
                    if min_amount_step.is_finite() and 0 < min_amount_step <= Decimal('1'):
                       if min_amount_step < 1 or '.' in str(min_amount_val): # Looks like step size
                           ...
                           return precision
                       elif min_amount_step == 1 and '.' not in str(min_amount_val): return 0 # Integer min amount like 1 suggests 0 precision
    ...
    def generate_trading_signal(self, current_price: Decimal, orderbook_data: Optional[Dict]) -> str:
        ...
        for key, value in self.indicator_values.items():
            weight = Decimal(str(self.weights.get(key.lower().replace('_',''), 0.0))) # Match weight keys (e.g., "EMA_Short" -> "ema_alignment"?) - Needs careful mapping
            # Let's map the indicator_values keys to the weight keys explicitly
            value_key_to_weight_key = {
                "EMA_Short": "ema_alignment", "EMA_Long": "ema_alignment", # Both contribute to ema_alignment score
                "Momentum": "momentum", "Volume": "volume_confirmation", "Volume_MA": "volume_confirmation",
                "StochRSI_K": "stoch_rsi", "StochRSI_D": "stoch_rsi",
                "RSI": "rsi", "BB_Lower": "bollinger_bands", "BB_Middle": "bollinger_bands", "BB_Upper": "bollinger_bands",
                "VWAP": "vwap", "CCI": "cci", "Williams_R": "wr",
                "PSAR_long": "psar", "PSAR_short": "psar",
                "SMA_10": "sma_10", "MFI": "mfi"
                # Orderbook handled separately
            }
            weight_key = value_key_to_weight_key.get(key)
            if weight_key: weight = Decimal(str(self.weights.get(weight_key, 0.0)))
        ...
        if not core_values_present and not (self.config.get("indicators",{}).get("orderbook") and orderbook_data):
            self.logger.warning("Signal Generation Skipped: All weighted core indicators have NaN/invalid values (and no Orderbook data).")
            return "HOLD"
    ...
    def _check_momentum(self) -> float:
        ...
        # This is highly asset-dependent. Let's use a simple threshold approach.
    ...
    def _check_volume_confirmation(self) -> float:
        ...
        try:
            ...
            if ratio < (Decimal('1') / multiplier): return -0.4 # Unusually low volume (potential lack of interest)
    ...
    def _check_stoch_rsi(self) -> float:
        ...
        # Consider crosses (K relative to D)
        cross_threshold = 3 # How much K needs to cross D by to be significant
        if k > d + cross_threshold: # Bullish cross
            score = max(score, 0.6) # Give bullish cross a reasonably strong score
        elif d > k + cross_threshold: # Bearish cross
            score = min(score, -0.6) # Give bearish cross a reasonably strong score

        # Consider general position (above/below 50)
        if 40 < k < 60 and 40 < d < 60: # Both in neutral zone
            score *= 0.5 # Dampen signal if in neutral zone
        elif k > 50 and d > 50 and score >= 0: # Both bullish, reinforce positive score slightly
             score = max(score, 0.2)
        elif k < 50 and d < 50 and score <= 0: # Both bearish, reinforce negative score slightly
             score = min(score, -0.2)
    ...
    def _check_rsi(self) -> float:
        ...
        # Scale linearly between 40 and 60 for neutral zone?
        if 40 <= rsi <= 60:
             # Maps 40 to 0.0, 50 to 0.0, 60 to 0.0 - Maybe slight bias based on side?
             # Let's keep it simple: neutral zone = 0 score
             return 0.0

        # Should cover all cases, but default return if logic missed something
        return 0.0
    ...
    def _check_psar(self) -> float:
        ...
        if l_active and not s_active: return 1.0  # Uptrend signaled by PSAR Long
        elif s_active and not l_active: return -1.0 # Downtrend signaled by PSAR Short
        elif not l_active and not s_active: return np.nan # Indeterminate or insufficient data
        else:
             # Both active should ideally not happen if calculation is correct
    ...
    def _check_bollinger_bands(self) -> float:
        ...
        if close > bbm: # Above middle band
             # position = (close - bbm) / (bbu - bbm)
             # Score approaches -1 as price nears upper band (fade signal)
             # Score closer to 0 near middle band
             # Let's use a simple gradient: -0.5 near middle, -0.9 near top
             score = -0.5 - 0.4 * float((close - bbm) / (bbu - bbm))
             return max(-0.9, score) # Clamp score range

        else: # Below middle band (close < bbm)
             # position = (bbm - close) / (bbm - bbl)
             # Score approaches +1 as price nears lower band (fade signal)
             # Score closer to 0 near middle band
             # Let's use a simple gradient: 0.5 near middle, 0.9 near bottom
             score = 0.5 + 0.4 * float((bbm - close) / (bbm - bbl))
             return min(0.9, score) # Clamp score range

        # Should not be reached if close == bbm, handled by logic above
    ...
    def _check_orderbook(self, orderbook_data: Optional[Dict], current_price: Decimal) -> float:
        ...
        try:
            # Consider only levels within a certain percentage of the current price?
            # Or use a fixed number of levels as before? Let's stick to fixed levels for simplicity.
            levels_to_analyze = min(len(bids), len(asks), self.config.get("orderbook_limit", 10))
            if levels_to_analyze == 0: return 0.0

            # Calculate total volume within the analyzed levels
            # Ensure entries are valid [Decimal, Decimal]
    ...
    def calculate_entry_tp_sl(
        ...
        try:
            ...
            sl_quantized = None
            if sl_raw.is_finite():
                rounding_mode_sl = ROUND_DOWN if signal == "BUY" else ROUND_UP # Note: SL rounding is AWAY from entry
            ...
            if final_sl is not None:
                if signal == "BUY" and final_sl >= entry_price_estimate:
                    ...
                    else: self.logger.warning(f"BUY SL {final_sl} >= Entry {entry_price_estimate}, but correction failed to move it.")
                    ...
                    if corrected_sl > final_sl: # Only update if correction actually moved it further
                        ...
                        final_sl = corrected_sl
                    else: self.logger.warning(f"SELL SL {final_sl} <= Entry {entry_price_estimate}, but correction failed to move it.")
...
def fetch_balance(exchange: ccxt.Exchange, currency: str, logger: logging.Logger) -> Optional[Decimal]:
    ...
    if exchange.id == 'bybit':
        ...
        try:
            ...
            # Proceed to default fetch below
    ...
    # --- Fallback to Default Fetch (or if not Bybit) ---
    ...
    # This structure can vary based on account type (CONTRACT, UNIFIED, SPOT)
         ...
         if isinstance(result_data.get('list'), list) and result_data['list']:
             ...
             if isinstance(account_details.get('coin'), list): # Unified account asset list
             ...
             elif account_details.get('accountType') == account_type_tried: # Contract account structure?
                 # Might have walletBalance, availableBalance directly in account_details
                 free = account_details.get('availableBalance') or account_details.get('walletBalance')
                 if free is not None:
                      free_balance_str = str(free)
                      lg.debug(f"Found Bybit V5 balance via info.result.list[0].available/wallet: {free_balance_str}")

         # Alternative: Look for the currency directly under 'info' or 'info.result'
    ...
    if free_balance_str is None:
         ...
         # Add V5 total check if needed

         if total_balance_str is not None:
              lg.warning(f"{NEON_YELLOW}Could not find 'free' balance for {currency}. Using 'total' balance ({total_balance_str}) as fallback. This may include collateral/unrealized PNL.{RESET}")
              free_balance_str = total_balance_str
         else:
              lg.error(f"{NEON_RED}Could not determine any balance ('free' or 'total') for {currency} after checking known structures.{RESET}")
...
def calculate_position_size(
    ...
    cfg = config_override or config # Use global config if no override
    ...
    size_unit = base_currency if not is_contract else base_currency # For linear contracts, size is in base currency units (e.g., BTC)
    ...
    try:
        ...
        # Risk per unit = SL distance (quote) * Contract Size (quote/base or quote/contract)
        # For Linear: ContractSize=1, Risk per unit = SL distance (quote/base)
        # Size (base) = Risk Amount (quote) / Risk per unit (quote/base)
        # Size (Contracts for Inverse) = Risk Amount (quote) / (SL distance (quote/contract) * ContractSize (quote/contract)) -> Not implemented

        if not is_inverse: # Linear or Spot
            # Risk per unit is simply the SL distance in quote currency per unit of base currency
             risk_per_unit_quote = sl_distance_per_unit_quote * contract_size_val # Contract size handles non-1 values if needed
             if not risk_per_unit_quote.is_finite() or risk_per_unit_quote <= 0:
                 lg.error(f"Size Calc Fail ({symbol}): Risk per unit calculation resulted in zero/negative/NaN ({risk_per_unit_quote}).")
                 return None
        ...
        lg.info(f"  ContractValue={contract_size_val}, Size Unit={size_unit}")
        lg.info(f"  Initial Calculated Size = {calculated_size:.8f} {size_unit}")

        # --- Apply Market Limits and Precision ---
        min_amount_limit = Decimal(str(market_info.get('limits', {}).get('amount', {}).get('min', '0')))
        max_amount_limit = Decimal(str(market_info.get('limits', {}).get('amount', {}).get('max', 'inf')))
        min_cost_limit = Decimal(str(market_info.get('limits', {}).get('cost', {}).get('min', '0')))
        max_cost_limit = Decimal(str(market_info.get('limits', {}).get('cost', {}).get('max', 'inf')))
        ...
        if min_amount_step.is_finite() and min_amount_step > 0:
            ...
            # Use floor division to round down to the nearest multiple of step size
            ...
            if adjusted_size != original_size_before_step:
             ...
             # Optionally, round to a reasonable number of decimal places as a fallback?
        ...
        if min_amount_limit.is_finite() and adjusted_size < min_amount_limit:
             # If rounded size is below min, can we increase it to min? Only if that doesn't violate risk.
             # For now, just report error if it falls below min after rounding.
        ...
        if min_amount_limit.is_finite() and final_size < min_amount_limit:
...
def get_open_position(exchange: ccxt.Exchange, symbol: str, logger: logging.Logger) -> Optional[Dict]:
    ...
    try:
        # --- Attempt 1: Fetch single position using market ID (preferred for V5) ---
        if exchange.has.get('fetchPosition'):
            fetch_method_used = "fetchPosition"
            lg.debug(f"Attempting {fetch_method_used} for {symbol} (ID: {market.get('id')})...")
            params = {'symbol': market['id']} if exchange.id == 'bybit' else {} # Bybit V5 requires symbol ID
            # fetchPosition returns a single position dict or raises NotSupported/errors
            try:
                position_data = safe_api_call(exchange.fetch_position, lg, symbol, params=params)
                if position_data:
                    positions = [position_data] # Wrap in list for consistent processing
                    lg.debug(f"{fetch_method_used} successful.")
            except ccxt.NotSupported as e:
                lg.debug(f"{fetch_method_used} not supported by exchange/ccxt version ({e}). Falling back.")
                fetch_method_used = "None" # Reset for next attempt
            except ccxt.ExchangeError as e:
                 # If error indicates symbol not found in positions (common if no position)
                 # Bybit V5 might return error 110001 "Order does not exist" or similar
                 err_str = str(e).lower()
                 bybit_no_pos_codes = [110001, 170001] # Add relevant codes indicating no position found
                 if getattr(e, 'code', None) in bybit_no_pos_codes or "position not found" in err_str or "order does not exist" in err_str:
                     lg.info(f"No active position found for {symbol} via {fetch_method_used} (Code: {getattr(e, 'code', 'N/A')}).")
                     return None # No position exists
                 else:
                     lg.warning(f"Exchange error during {fetch_method_used} for {symbol}: {e}. Falling back.")
                     fetch_method_used = "None" # Reset for next attempt
            # Catch other potential errors from safe_api_call
            except Exception as e:
                 lg.warning(f"Error during {fetch_method_used} for {symbol}: {e}. Falling back.")
                 fetch_method_used = "None" # Reset for next attempt

        # --- Attempt 2: Fetch positions for specific symbol(s) (if fetchPosition failed/not supported) ---
        ...
        # --- Attempt 3: Fetch ALL positions (fallback) ---
        if not positions and fetch_method_used == "None" and exchange.has.get('fetchPositions'): # Only if previous attempts failed or weren't possible
            ...
            try:
                all_positions_data = safe_api_call(exchange.fetch_positions, lg)
                if all_positions_data:
                     # Filter the results for the target symbol
                     positions = [p for p in all_positions_data if p.get('symbol') == symbol]
                     lg.debug(f"Fetched {len(all_positions_data)} total positions, found {len(positions)} matching {symbol}.")
                else:
    ...
    size_threshold = Decimal('1e-9') # Adjust based on typical minimum contract sizes if needed
    ...
    for pos_data in positions:
        ...
        if pos_symbol != symbol: continue # Should already be filtered, but double-check
        ...
        # Fallback to Bybit V5 'info' field
        ...
        if pos_size_str is None:
             lg.warning(f"Could not determine position size for an entry of {symbol}. Skipping entry.")
             continue
        ...
        try:
            ...
            # else: position size is effectively zero, ignore this entry
    ...
    if active_position:
        try:
            ...
            # --- Standardize Side ---
            # Use 'side' if present, otherwise infer from size
            ...
            entry_price_str = active_position.get('entryPrice') or info_dict.get('avgPrice') # V5 uses avgPrice in info
            ...
            liq_price_str = active_position.get('liquidationPrice') or info_dict.get('liqPrice') # V5 uses liqPrice
            active_position['liquidationPriceDecimal'] = Decimal(str(liq_price_str)) if liq_price_str is not None and str(liq_price_str) != '0' else None # Ignore '0' liq price

            # --- Standardize Unrealized PNL (Decimal) ---
            pnl_str = active_position.get('unrealizedPnl') or info_dict.get('unrealisedPnl') # V5 uses unrealisedPnl
            ...
            try:
                active_position['timestamp_ms'] = int(timestamp_ms) if timestamp_ms is not None else None
                timestamp_dt_str = datetime.fromtimestamp(active_position['timestamp_ms'] / 1000, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z') if active_position['timestamp_ms'] else "N/A"
            except (ValueError, TypeError):
                active_position['timestamp_ms'] = None
                timestamp_dt_str = "Invalid Timestamp"

            # --- Log Formatted Position Info ---
            analyzer = TradingAnalyzer(pd.DataFrame(), lg, config, market) # Temp instance for precision
            price_prec = analyzer.get_price_precision()
            amt_prec = analyzer.get_amount_precision_places() # Use amount precision for size

            entry_fmt = f"{active_position['entryPriceDecimal']:.{price_prec}f}" if active_position['entryPriceDecimal'] else 'N/A'
            size_fmt = f"{abs(active_position['contractsDecimal']):.{amt_prec}f}" # Display absolute size with correct precision
...
def set_leverage_ccxt(exchange: ccxt.Exchange, symbol: str, leverage: int, market_info: Dict, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        if exchange.id == 'bybit':
            ...
            leverage_str = str(float(leverage)) # Ensure it's a string representation of the number
...
def place_trade(
    ...
    if not isinstance(position_size, Decimal) or not position_size.is_finite() or position_size <= 0:
        ...
        return None
    try:
        # CCXT generally requires amount as float
        amount_float = float(position_size)
        if amount_float <= 0: raise ValueError("Position size float conversion resulted in non-positive value")
    except (ValueError, TypeError) as e:
        lg.error(f"Trade Aborted ({action_desc} {side} {symbol}): Failed to convert size {position_size} to float: {e}")
        return None
    ...
    if order_type == 'limit':
         if not isinstance(limit_price, Decimal) or not limit_price.is_finite() or limit_price <= 0:
             ...
             return None
         try:
             # Convert limit price to float for CCXT
             price_float = float(limit_price)
             if price_float <= 0: raise ValueError("Limit price float conversion resulted in non-positive value")
         except (ValueError, TypeError):
              lg.error(f"Trade Aborted ({action_desc} {side} {symbol}): Invalid limit_price format for float conversion ({limit_price}).")
              return None
    ...
    if exchange.id == 'bybit':
        ...
        # This logic should be added *before* calling place_trade if desired

    # Merge external params, ensuring our essential params override
    if params and isinstance(params, dict):
        # Prioritize params passed externally, but don't let them overwrite core args
        core_keys = ['symbol', 'type', 'side', 'amount', 'price', 'reduceOnly', 'positionIdx', 'timeInForce']
    ...
    # --- Log Order Details ---
    analyzer = TradingAnalyzer(pd.DataFrame(), lg, config, market_info) # Temp instance for precision formatting
    amt_prec_places = analyzer.get_amount_precision_places()
    price_prec_places = analyzer.get_price_precision()
    size_fmt = f"{position_size:.{amt_prec_places}f}" # Format original Decimal size
    ...
    lg.info(f"  Size: {size_fmt} {size_unit}")
    if order_type == 'limit': lg.info(f"  Limit Price: {limit_fmt}")
    ...
    try:
        ...
        if order_response and isinstance(order_response, dict) and order_response.get('id'):
            ...
            avg_price = order_response.get('average') # Might be None if not filled

            lg.info(f"{NEON_GREEN}{action_desc} Order Placed/Accepted!{RESET}")
            lg.info(f"  ID: {order_id}, Status: {status}, Filled: {filled_amount}, AvgPx: {avg_price or 'N/A'}")
    ...
    # --- Specific Error Handling ---
        ...
        err_msg = str(e).lower()
        if "price" in err_msg or "tick size" in err_msg: lg.error(f"{NEON_YELLOW} >> Hint: Check price precision / tick size. Limit Price: {limit_fmt}{RESET}")
        if "size" in err_msg or "step size" in err_msg or "lot size" in err_msg: lg.error(f"{NEON_YELLOW} >> Hint: Check amount precision / step size. Size: {size_fmt}{RESET}")
        if "minnotional" in err_msg or "minimum order value" in err_msg or "cost" in err_msg: lg.error(f"{NEON_YELLOW} >> Hint: Order cost likely below minimum required.{RESET}")
        if "reduceonly" in err_msg or "reduce-only" in err_msg or (getattr(e,'code',None) == 110014 and exchange.id == 'bybit'):
        ...
        lg.error(f"{NEON_RED}Exchange error placing {action_desc} {side} order ({symbol}): {e} (Code: {getattr(e,'code',None)}){RESET}")
        # Add hints for specific Bybit codes if needed (e.g., risk limit errors)
        if exchange.id == 'bybit' and "risk limit" in str(e).lower():
...
def _set_position_protection(
    ...
    if not market_info.get('is_contract'):
        lg.warning(f"Protection setting skipped for {symbol}: Market is not a contract.")
        return False
    ...
    if pos_side not in ['long', 'short']:
        ...
        return False
    if not isinstance(entry_price, Decimal) or not entry_price.is_finite() or entry_price <= 0:
        lg.error(f"Cannot set protection ({symbol}): Invalid entry price '{entry_price}'.")
        return False
    try:
    ...
    # Validate Stop Loss
    if isinstance(stop_loss_price, Decimal) and stop_loss_price.is_finite() and stop_loss_price > 0:
        # Check validity relative to entry price
    ...
    elif stop_loss_price is not None: # If it's provided but invalid (e.g., 0, NaN)
    ...
    # Bybit V5 TSL uses price *distance*, not percentage callback rate.
    # The caller (set_trailing_stop_loss) should provide the calculated distance.
    if isinstance(trailing_stop_distance, Decimal) and trailing_stop_distance.is_finite() and trailing_stop_distance > 0 and \
    ...
    # --- Compare Target with Current Protection ---
    # Check SL change
    if target_sl != current_sl: needs_update = True
    # Check TP change
    if target_tp != current_tp: needs_update = True
    # Check TSL change (both distance and activation)
    if target_tsl_dist != current_tsl_dist or target_tsl_act != current_tsl_act: needs_update = True
    # Special case: If we want to set TSL, but currently have Fixed SL, we need to update.
    if target_tsl_dist is not None and current_tsl_dist is None: needs_update = True
    # Special case: If we want to set Fixed SL, but currently have TSL, we need to update (clear TSL).
    if target_sl is not None and current_tsl_dist is not None: needs_update = True
    # Special case: If we want to clear protection (pass None) but something is set
    if target_sl is None and current_sl is not None: needs_update = True; target_sl = Decimal('0') # Use '0' to clear
    if target_tp is None and current_tp is not None: needs_update = True; target_tp = Decimal('0') # Use '0' to clear
    if target_tsl_dist is None and current_tsl_dist is not None: needs_update = True; target_tsl_dist = Decimal('0') # Use '0' to clear TSL
    ...
    params = {
        ...
        # Partial mode is for SL/TP on order placement affecting average position SL/TP
        'tpslMode': 'Full',
        # Trigger prices (LastPrice, MarkPrice, IndexPrice)
        'tpTriggerBy': 'LastPrice', # Or MarkPrice? Choose based on strategy
        'slTriggerBy': 'LastPrice', # Or MarkPrice?
    }
    ...
    try: # Format parameters to strings with correct precision
        ...
        def fmt_price(p):
            if not isinstance(p, Decimal) or not p.is_finite(): return None # Allow None
            if p == 0: return '0' # Explicitly allow '0' to clear protection
            if p < 0: return None # Prices must be positive
        ...
        def fmt_tsl_distance(d): # TSL distance might need different formatting/precision
            if not isinstance(d, Decimal) or not d.is_finite(): return None
            if d == 0: return '0'
            if d < 0: return None
            try:
                # Use price precision for TSL distance as it's a price value
                return exchange.price_to_precision(symbol, float(d))
        ...
        # If target_tsl_dist is provided (and > 0), set it. If it's None or 0, clear TSL.
        if target_tsl_dist is not None: # This includes the case where target_tsl_dist is Decimal('0') for clearing
            tsl_d_fmt = fmt_tsl_distance(target_tsl_dist)
            # Activation price is only needed if setting a TSL (dist > 0)
            tsl_a_fmt = fmt_price(target_tsl_act) if target_tsl_dist > 0 else None

            if tsl_d_fmt is not None and (tsl_d_fmt == '0' or tsl_a_fmt is not None):
                 params['trailingStop'] = tsl_d_fmt
                 log_parts.append(f"  Set TSL Dist: {tsl_d_fmt}")
                 if tsl_a_fmt:
                      params['activePrice'] = tsl_a_fmt
                      log_parts.append(f"  Set TSL ActPx: {tsl_a_fmt}")
                 # If TSL is being set (dist > 0), clear target SL to avoid conflicts
                 if target_tsl_dist > 0: target_sl = None
            else:
                 lg.error(f"Failed formatting TSL params (Dist: {target_tsl_dist}, Act: {target_tsl_act}). TSL change aborted.")
                 # Abort only the TSL part, SL/TP might still proceed if needed

        # Set Fixed SL (only if TSL wasn't set or is being cleared)
        if target_sl is not None: # Includes Decimal('0') for clearing
             sl_fmt = fmt_price(target_sl)
             if sl_fmt is not None:
                  params['stopLoss'] = sl_fmt
                  log_parts.append(f"  Set FixSL: {sl_fmt}")
             else: lg.error("Failed formatting Fixed SL price. SL change aborted.")

        # Set Fixed TP
        if target_tp is not None: # Includes Decimal('0') for clearing
             tp_fmt = fmt_price(target_tp)
             if tp_fmt is not None:
                  params['takeProfit'] = tp_fmt
                  log_parts.append(f"  Set FixTP: {tp_fmt}")
             else: lg.error("Failed formatting Fixed TP price. TP change aborted.")
    ...
    valid_keys_added = ['stopLoss', 'takeProfit', 'trailingStop']
    if not any(key in params for key in valid_keys_added):
        lg.warning(f"No valid protection parameters formatted for {symbol}. No API call made.")
        # Return True because no update was needed *after formatting attempt*
        return True
    ...
    try:
        ...
        response = safe_api_call(exchange.private_post, lg, endpoint, params=params)
        ...
        if isinstance(response, dict):
            ...
            if code == 0:
                ...
                if "not modified" in msg.lower() or msg == "success" or msg == "ok":
...
def set_trailing_stop_loss(
    ...
    try: # --- Validate Inputs from Config and Position ---
        ...
        callback_rate_pct = Decimal(str(config["trailing_stop_callback_rate"]))
        # trailing_stop_activation_percentage: % move from entry price to trigger activation
        activation_trigger_pct = Decimal(str(config["trailing_stop_activation_percentage"]))
    ...
    try: # --- Calculate TSL Parameters (Activation Price and Trail Distance) ---
        ...
        if not trail_distance.is_finite() or trail_distance <= 0:
            lg.error(f"Invalid TSL Trail Distance calculated ({trail_distance}) for {symbol}. Cannot set TSL."); return False

        # --- Log Calculated Parameters ---
        act_fmt = f"{activation_price:.{price_prec}f}"
        dist_fmt = f"{trail_distance:.{price_prec}f}"
        lg.info(f"Calculated TSL Params ({symbol} {pos_side.upper()}): Activation Price={act_fmt}, Trail Distance={dist_fmt}")
        # Optionally log the target TP if provided
        if isinstance(take_profit_price, Decimal) and take_profit_price.is_finite() and take_profit_price > 0:
            tp_fmt = f"{take_profit_price:.{price_prec}f}"
            lg.info(f"  Will also set concurrent TP: {tp_fmt}")
        else:
             take_profit_price = None # Ensure invalid TP is not passed

        # --- Call the Protection Setting Function ---
        lg.info(f"Attempting to set Trailing Stop Loss via API for {symbol}...")
        success = _set_position_protection(
            ...
            logger=lg,
            stop_loss_price=None, # TSL overrides fixed SL, explicitly pass None
            take_profit_price=take_profit_price, # Pass concurrent TP if valid
            trailing_stop_distance=trail_distance,
            tsl_activation_price=activation_price
        )

        if success:
            lg.info(f"{NEON_GREEN}Trailing Stop Loss update request sent successfully for {symbol}.{RESET}")
        else:
            lg.error(f"{NEON_RED}Failed to set Trailing Stop Loss for {symbol} via API.{RESET}")

        return success

    except Exception as e:
        lg.error(f"Error calculating or setting Trailing Stop Loss for {symbol}: {e}", exc_info=True)
        return False
...
def analyze_and_trade_symbol(exchange: ccxt.Exchange, symbol: str, config: Dict[str, Any], logger: logging.Logger) -> None:
    ...
    lg = logger
    lg.info(f"{NEON_CYAN}---=== Cycle Start: {symbol} ({config['interval']}m) ===---{RESET}")
    cycle_start_time = time.monotonic()

    try:
        # --- 1. Get Market Info (Critical for Precision, Limits, IDs) ---
        market_info = get_market_info(exchange, symbol, lg)
        if not market_info:
            # Error logged in get_market_info, raise to stop cycle
            raise ValueError(f"Fatal: Failed to get market info for {symbol}. Cannot proceed.")

        # --- 2. Fetch Required Data ---
        # Kline / OHLCV Data
        # Use interval from config, map to CCXT format
        kline_interval_key = config["interval"]
        ccxt_interval = CCXT_INTERVAL_MAP.get(kline_interval_key)
        if not ccxt_interval:
            raise ValueError(f"Invalid kline interval '{kline_interval_key}' in config.")
        # Fetch a bit more data than strictly needed for indicator stability
        klines_df = fetch_klines_ccxt(exchange, symbol, kline_interval_key, limit=750, logger=lg) # Fetch more for longer periods
        # Basic validation of kline data
        if klines_df.empty: raise ValueError("Kline data fetch returned an empty DataFrame.")
        if len(klines_df) < 50: raise ValueError(f"Insufficient kline data ({len(klines_df)} bars) fetched.")

        # Current Price
        current_price = fetch_current_price_ccxt(exchange, symbol, lg)
        if current_price is None:
             # Fallback to last close price from klines
             lg.warning("Current price fetch failed. Using last close price from klines as fallback.")
             try:
                 # Ensure klines_df has 'close' column and it's Decimal
                 last_close = klines_df['close'].iloc[-1]
                 if not isinstance(last_close, Decimal) or not last_close.is_finite() or last_close <= 0:
                     raise ValueError(f"Last close price from klines is invalid ({last_close}).")
                 current_price = last_close
                 lg.info(f"Using fallback current price (last close): {current_price}")
             except (IndexError, KeyError, ValueError, TypeError, InvalidOperation) as e:
                 raise ValueError(f"Failed to get valid last close price as fallback: {e}")

        # Order Book Data (if needed for scoring)
        orderbook_data = None
        active_weight_set = config.get("weight_sets", {}).get(config.get("active_weight_set", "default"), {})
        if config.get("indicators", {}).get("orderbook", False) and float(active_weight_set.get("orderbook", 0.0)) > 0:
            ob_limit = int(config.get("orderbook_limit", 25)) # Ensure integer limit
            orderbook_data = fetch_orderbook_ccxt(exchange, symbol, ob_limit, lg)
            if not orderbook_data:
                 lg.warning(f"Orderbook fetch failed or returned invalid data for {symbol}. Orderbook score contribution will be skipped.")

        # --- 3. Initialize Analyzer & Generate Signal ---
        analyzer = TradingAnalyzer(klines_df, lg, config, market_info) # Pass the fetched klines
        # Check if analyzer initialized correctly and has values
        if not analyzer.indicator_values:
             raise ValueError("TradingAnalyzer failed to initialize or calculate indicator values.")

        # Get the trading signal
        signal = analyzer.generate_trading_signal(current_price, orderbook_data)

        # --- 4. Calculate Potential TP/SL & Log Summary ---
        # Calculate potential SL/TP based on current price estimate (useful for logging and initial sizing)
        _, potential_tp, potential_sl = analyzer.calculate_entry_tp_sl(current_price, signal)
        price_prec = analyzer.get_price_precision()
        current_atr_val = analyzer.indicator_values.get("ATR") # Should be Decimal or NaN
        atr_str = f"{current_atr_val:.{price_prec+2}f}" if isinstance(current_atr_val, Decimal) and current_atr_val.is_finite() else 'N/A'
        sl_str = f"{potential_sl:.{price_prec}f}" if potential_sl else 'N/A'
        tp_str = f"{potential_tp:.{price_prec}f}" if potential_tp else 'N/A'

        lg.info(f"Analysis Summary ({symbol}):")
        lg.info(f"  Current Price: {current_price:.{price_prec}f}")
        lg.info(f"  ATR ({config.get('atr_period')}): {atr_str}")
        lg.info(f"  Generated Signal: {signal}")
        lg.info(f"  Potential Initial SL (for sizing): {sl_str}")
        lg.info(f"  Potential Initial TP (target): {tp_str}")
        lg.info(f"  Position Mgmt Enabled: TSL={config['enable_trailing_stop']}, BE={config['enable_break_even']}, TimeExit={config.get('time_based_exit_minutes') or 'Off'}")

        # --- 5. Trading Execution Logic ---
        if not config.get("enable_trading"):
            lg.debug("Trading is disabled in config. Skipping execution logic.")
            return # End cycle here if trading disabled

        # Get current open position status
        ...
        if open_position is None:
            # Check if signal triggers a new trade
            if signal in ["BUY", "SELL"]:
                lg.info(f"*** {signal} Signal received and NO current position for {symbol}. Evaluating trade entry... ***")

                # --- Pre-Trade Checks ---
                # Check Max Concurrent Positions (basic check, assumes single symbol focus here)
                # More complex logic needed if bot manages multiple symbols concurrently
                # if get_total_open_positions(exchange) >= config["max_concurrent_positions"]:
                #      lg.warning(f"Max concurrent positions ({config['max_concurrent_positions']}) reached. Skipping new entry for {symbol}.")
                #      return

                # Fetch current balance
                quote_curr = config["quote_currency"]
                balance = fetch_balance(exchange, quote_curr, lg)
                if balance is None or balance <= 0:
                    raise ValueError(f"Cannot place trade: Failed to fetch valid balance for {quote_curr} or balance is zero/negative.")

                # Ensure initial SL was calculated (required for sizing)
                if potential_sl is None:
                    raise ValueError(f"Cannot place trade: Initial Stop Loss calculation failed (required for position sizing). Check ATR and config.")

                # Set Leverage (if applicable contract market)
                leverage_val = int(config.get("leverage", 0))
                if market_info.get('is_contract') and leverage_val > 0:
                    if not set_leverage_ccxt(exchange, symbol, leverage_val, market_info, lg):
                        # Logged in set_leverage_ccxt, decide whether to proceed without leverage or fail
                        lg.warning(f"Failed to set leverage to {leverage_val}x for {symbol}. Proceeding without leverage confirmation (or with existing leverage).")
                        # raise ValueError("Failed to set required leverage.") # Option: Make leverage setting critical

                # Calculate Position Size
                pos_size = calculate_position_size(balance, config["risk_per_trade"], potential_sl, current_price, market_info, exchange, lg)
                if pos_size is None or pos_size <= 0:
                    raise ValueError(f"Position size calculation failed or resulted in zero/negative size ({pos_size}). Cannot place trade.")
